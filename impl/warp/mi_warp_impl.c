...
//#define MI_WARP_DEBUG_SETBIN_BLOCK_ISSUE
...
MI_U8 _gu8DumpChn = 0xff;
MI_U8 _gpSavePath[256];


static MI_U64 _MI_WARP_StatSpendTime(struct timespec *pstTimeStart)
{
...
static MI_U32 _MI_WARP_SetStatFpsDepth(MI_WARP_CalcFps_t *pstCalcPts, MI_U32 u32Depth)
{
...
void _MI_WARP_InitialDebugInfo(MI_WARP_DebugInfo_t *pstDebugInfo)
{
    memset(&pstDebugInfo->stCalcHalTime, 0, sizeof(MI_WARP_CalcSpendTime_t));
    pstDebugInfo->stCalcHalTime.u64MinSpendTime = 0xffffffff;

    memset(&pstDebugInfo->stBufCnt, 0, sizeof(MI_WARP_BufOptCnt_t));

    memset(&pstDebugInfo->stCalcInputFps, 0, sizeof(MI_WARP_CalcFps_t));
    _MI_WARP_SetStatFpsDepth(&pstDebugInfo->stCalcInputFps, 20);

    memset(&pstDebugInfo->stCalcOutputFps, 0, sizeof(MI_WARP_CalcFps_t));
    _MI_WARP_SetStatFpsDepth(&pstDebugInfo->stCalcOutputFps, 20);

    memset(&pstDebugInfo->stInputPortInfo, 0, sizeof(MI_WARP_StatPortInfo_t));
    memset(&pstDebugInfo->stOutputPortInfo, 0, sizeof(MI_WARP_StatPortInfo_t));
}

void _MI_WARP_DeinitDebugInfo(MI_WARP_DebugInfo_t *pstDebugInfo)
{
    ...
    memset(pstDebugInfo, 0, sizeof(MI_WARP_DebugInfo_t));
}

void _MI_WARP_CalcTimeSpend(MI_WARP_CalcSpendTime_t *pstCalcSpendTime)
{
    pstCalcSpendTime->u64CurSpendTime = _MI_WARP_StatSpendTime(&pstCalcSpendTime->stStartTmSpec);
...
void _MI_WARP_StatPts(MI_WARP_CalcFps_t *pstCalcFps, MI_U32 u32FrameCnt, MI_U64 u64CurPts)
{
...
MI_U32 _MI_WARP_ClacFps(MI_WARP_CalcFps_t *pstCalcFps)
{
    MI_U32 i = 0;
...
#if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_WARP_PROCFS_DEBUG == 1)
#define MI_WARP_GET_FORMAT(fmt, strFmt) do { \
                            memset(strFmt, 0, sizeof(strFmt));  \
                            if (E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_422 == fmt)    \
                            {   \
                                strcpy(strFmt, "YUV422SP");  \
                            }   \
...
//    MI_U32 j = 0;
    ...
    for (i = 0; i < MI_WARP_MAX_CHN_NUM; i++)
    {
        ...
        {
            MI_WARP_GET_FORMAT(pstDev->stChannel[i].stDebugInfo.stInputPortInfo.eFormat, szFormat);
            handle.OnPrintOut(handle, "%7d%12s%7d%7d%12d%15d%15d%10d.%1d%12d\n", i, szFormat, pstDev->stChannel[i].stDebugInfo.stInputPortInfo.u16Width
                              ...
                              , _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcInputFps)/10, _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcInputFps)%10
    ...
    for (i = 0; i < MI_WARP_MAX_CHN_NUM; i++)
    {
        if (pstDev->stChannel[i].eChnStatus != E_MI_WARP_CHN_UNINIT)
        {
            MI_WARP_GET_FORMAT(pstDev->stChannel[i].stDebugInfo.stOutputPortInfo.eFormat, szFormat);
            handle.OnPrintOut(handle, "%7d%12s%7d%7d%12d%15d%15d%10d.%1d%12d\n", i, szFormat, pstDev->stChannel[i].stDebugInfo.stOutputPortInfo.u16Width
                              ...
                              , _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcOutputFps)/10, _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcOutputFps)%10
...
static MI_S32 _MI_WARP_IMPL_DumpInfoToFile(const MI_U8 *pszFileName, const MI_U8 *pszBuffer, MI_U32 u32Length)
{
...
static MI_S32 _MI_WARP_IMPL_DumpChnBufToFile(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_WARP_Device_t *pstDev = NULL;
    char *ps8After = NULL;
    ...
    _gu8DumpChn = simple_strtoul(argv[1], &ps8After, 10);
    if (MI_WARP_MAX_CHN_NUM <= _gu8DumpChn)
    {
    ...
    if (_gpSavePath[0])
    {
        handle.OnPrintOut(handle, "Another input file is writing!!!\n");
        mutex_unlock(&pstDev->mtx);
        return MI_SUCCESS;
    }

    sprintf(_gpSavePath, "%s_Chn%d", argv[2], _gu8DumpChn);
    mutex_unlock(&pstDev->mtx);
...
static MI_S32 _MI_WARP_IMPL_DumpPortBufToFile(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    if (MI_WARP_MAX_CHN_NUM <= u8Channel)
    {
        handle.OnPrintOut(handle, "Error Channel id!!!\n");
        return MI_SUCCESS;
    }
    ...
    {
        sprintf(pFileName, "%s/warp_inBuf_chn%d_%dx%d_fmt%d", argv[3], u8Channel
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.u16Width
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.u16Height
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.ePixelFormat);

        phyAddr = pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.phyAddr[0];
        u32Size = pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.u32BufSize;
        handle.OnPrintOut(handle, "warp inputbuf phyAddr:%llu, size:%d\n", phyAddr, u32Size);
        pu8VirAddr = mi_sys_Vmap(phyAddr, u32Size, FALSE);
        if (NULL == pu8VirAddr)
        {
        ...
        _MI_WARP_IMPL_DumpInfoToFile(pFileName, pu8VirAddr, u32Size);
        mi_sys_UnVmap(pu8VirAddr);
        handle.OnPrintOut(handle, "dump inputport buf to %s size %d\n", pFileName, u32Size);
    }
    else if (strcmp(argv[2], "out") == 0)
    {
        sprintf(pFileName, "%s/warp_outBuf_chn%d_%dx%d_fmt%d", argv[3], u8Channel
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.u16Width
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.u16Height
                , pstDev->stChannel[u8Channel].stInputPort.pstInBufInfo->stFrameData.ePixelFormat);

        phyAddr = pstDev->stChannel[u8Channel].stOutputPort.pstOutBufInfo->stFrameData.phyAddr[0];
        u32Size = pstDev->stChannel[u8Channel].stOutputPort.pstOutBufInfo->stFrameData.u32BufSize;
        handle.OnPrintOut(handle, "warp outputbuf phyAddr:%llu, size:%d\n", phyAddr, u32Size);
        pu8VirAddr = mi_sys_Vmap(phyAddr, u32Size, FALSE);
        if (NULL == pu8VirAddr)
        {
            DBG_EXIT_ERR("Sys map error!\n");
        ...
        _MI_WARP_IMPL_DumpInfoToFile(pFileName, pu8VirAddr, u32Size);
        mi_sys_UnVmap(pu8VirAddr);
        handle.OnPrintOut(handle, "dump outputport buf to %s size %d\n", pFileName, u32Size);
    }
    else
    {
        handle.OnPrintOut(handle, "bad command\n");
    }
    mutex_unlock(&pstDev->mtx);
    kfree(pFileName);
...
static MI_S32 _MI_WARP_IMPL_DumpTableToFile(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_U8 *pFileName = NULL;
    MI_WARP_Device_t *pstDev = NULL;
    char *ps8After = NULL;
    ...
    if (u8Choice == 0)
    {
        ...
        _MI_WARP_IMPL_DumpInfoToFile(pFileName, pu8ConfAddr, u32Size);
...
static MI_S32 _MI_WARP_IMPL_DumpFps(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_WARP_Device_t *pstDev = NULL;
    ...
    for (i = 0; i < MI_WARP_MAX_CHN_NUM; i++)
    {
        if (pstDev->stChannel[i].eChnStatus != E_MI_WARP_CHN_UNINIT)
        {
            u32InputFps = _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcInputFps);
            u32OutputFps = _MI_WARP_ClacFps(&pstDev->stChannel[i].stDebugInfo.stCalcOutputFps);
...
static MI_S32 _MI_WARP_IMPL_DumpFrameCnt(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_WARP_Device_t *pstDev = NULL;
    MI_U32 i = 0;
...
static MI_S32 _MI_WARP_IMPL_DumpHalTimeConsume(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_WARP_Device_t *pstDev = NULL;
...
static MI_S32 _MI_WARP_IMPL_DumpSetStatDepth(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_WARP_Device_t *pstDev = NULL;
    char *ps8After = NULL;
    MI_U32 i = 0;
    ...
    for (i = 0; i < MI_WARP_MAX_CHN_NUM; i++)
    {
        if (pstDev->stChannel[i].eChnStatus != E_MI_WARP_CHN_UNINIT)
        {
            _MI_WARP_SetStatFpsDepth(&pstDev->stChannel[i].stDebugInfo.stCalcInputFps, u32Depth);
            handle.OnPrintOut(handle, "set warp inputport stat depth to %d, devId=%d, chnId=%d\n", u32Depth, u32DevId, i);

            _MI_WARP_SetStatFpsDepth(&pstDev->stChannel[i].stDebugInfo.stCalcOutputFps, u32Depth);
...
{
    MI_WARP_Channel_t *pstChnCtx = (MI_WARP_Channel_t*)usrData;
    MI_WARP_Device_t *pstDev = NULL;
    MI_U64 u64Pts = 0;
    ...
    if (pstChnCtx->stInputPort.pstInBufInfo)
    {
        ...
        if((pstChnCtx->stOutputPort.pstOutBufInfo) && (pstDev->stChannel[pstChnCtx->u32ChnId].stOutputPort.stBindPort.bEnableLowLatencyReceiveMode == FALSE))
        {
        	mi_sys_buf_mgr_sync_trackinfo(pstChnCtx->stInputPort.pstInBufInfo ,pstChnCtx->stOutputPort.pstOutBufInfo);
        }
#endif
    ...
    if (pstChnCtx->stOutputPort.pstOutBufInfo)
    {
        ...
        // calc output fps
        _MI_WARP_StatPts(&pstChnCtx->stDebugInfo.stCalcOutputFps, pstChnCtx->stDebugInfo.stBufCnt.u32FinishOutputCnt
        ...
        // dump outputBuf
        if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
        {
            // create inputBufFileName
            MI_U8 *pFileName = kmalloc(256, GFP_ATOMIC);
            MI_PHY phyAddr = pstChnCtx->stOutputPort.pstOutBufInfo->stFrameData.phyAddr[0];
            MI_U32 u32Size = pstChnCtx->stOutputPort.pstOutBufInfo->stFrameData.u32BufSize;
            void *pVirAddr = NULL;

            if (pFileName)
            {
                memset(pFileName, 0, 256);
                sprintf(pFileName, "%s_outBuf_%dx%d_fmt%d_%lld", _gpSavePath, pstChnCtx->stOutputPort.pstOutBufInfo->stFrameData.u16Width
                                    , pstChnCtx->stOutputPort.pstOutBufInfo->stFrameData.u16Height, pstChnCtx->stOutputPort.pstOutBufInfo->stFrameData.ePixelFormat
                                    , pstChnCtx->stOutputPort.pstOutBufInfo->u64Pts);
                DBG_INFO("warp chn%d outputbuf phyAddr:%llu, size:%d\n", pstChnCtx->u32ChnId, phyAddr, u32Size);
                pVirAddr = mi_sys_Vmap(phyAddr, u32Size, FALSE);
                if (NULL != pVirAddr)
                {
                    _MI_WARP_IMPL_DumpInfoToFile(pFileName, pVirAddr, u32Size);

                    mi_sys_UnVmap(pVirAddr);
                    DBG_INFO("dump chn%d outputport buf to %s, size %d\n", pstChnCtx->u32ChnId, pFileName, u32Size);
                }
                else
                {
                    DBG_ERR("dump chn%d outputport buf to %s failed\n", pstChnCtx->u32ChnId, pFileName);
                }
                kfree(pFileName);
            }
            else
            {
                DBG_ERR("dump chn%d outputport buf failed\n", pstChnCtx->u32ChnId);
            }

            // dump complete, reset save file path
            memset(_gpSavePath, 0, sizeof(_gpSavePath));
        }
#endif
			if (pstDev->stChannel[pstChnCtx->u32ChnId].stOutputPort.stBindPort.bEnableLowLatencyReceiveMode == TRUE)
			{
				mi_sys_FinishBuf_LL(pstChnCtx->stOutputPort.pstOutBufInfo);        
				//pstChnCtx->stOutputPort.pstOutBufInfo = NULL;
			}
			else
    ...
    _MI_WARP_CalcTimeSpend(&pstChnCtx->stDebugInfo.stCalcHalTime);
#endif
...
static MI_S32 _MI_WARP_WorkThread(void *pData)
{
    MI_WARP_Device_t *pstDev = NULL;
    ...
    while(!kthread_should_stop())
    {
        ...
        // calc input fps
        _MI_WARP_StatPts(&pstChnCtx->stDebugInfo.stCalcInputFps, pstChnCtx->stDebugInfo.stBufCnt.u32GetInputCnt, pstInBufInfo->u64Pts);
        ...
        if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
        {
            // create inputBufFileName
            MI_U8 *pFileName = kmalloc(256, GFP_ATOMIC);
            MI_PHY phyAddr = pstInBufInfo->stFrameData.phyAddr[0];
            MI_U32 u32Size = pstInBufInfo->stFrameData.u32BufSize;
            void *pVirAddr = NULL;

            if (pFileName)
            {
                memset(pFileName, 0, 256);
                sprintf(pFileName, "%s_inBuf_%dx%d_fmt%d_%lld", _gpSavePath, pstInBufInfo->stFrameData.u16Width
                                    , pstInBufInfo->stFrameData.u16Height, pstInBufInfo->stFrameData.ePixelFormat, pstInBufInfo->u64Pts);
                DBG_INFO("warp chn%d inputbuf phyAddr:%llu, size:%d\n", pstChnCtx->u32ChnId, phyAddr, u32Size);
                pVirAddr = mi_sys_Vmap(phyAddr, u32Size, FALSE);
                if (NULL != pVirAddr)
                {
                    _MI_WARP_IMPL_DumpInfoToFile(pFileName, pVirAddr, u32Size);
                    mi_sys_UnVmap(pVirAddr);
                    DBG_INFO("dump chn%d inputport buf to %s, size %d\n", pstChnCtx->u32ChnId, pFileName, u32Size);
                }
                else
                {
                    DBG_ERR("dump chn%d inputport buf to %s failed\n", pstChnCtx->u32ChnId, pFileName);
                }
                kfree(pFileName);
            }
            else
            {
                DBG_ERR("dump chn%d inputport buf failed\n", pstChnCtx->u32ChnId);
            }
        }
        ...
        if(!pstOutBufInfo)
        {
            ...
            // get outputbuf fail, reset dump file path
            if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
            {
                memset(_gpSavePath, 0, sizeof(_gpSavePath));
            }
        ...
        switch (pstInBufInfo->stFrameData.ePixelFormat)
        {
            ...
            default:
                ...
                // inbuf format error, reset dump file path
                if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
                {
                    memset(_gpSavePath, 0, sizeof(_gpSavePath));
                }
        ...
        switch (pstOutBufInfo->stFrameData.ePixelFormat)
        {
            ...
            default:
                ...
                // outbuf format error, reset dump file path
                if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
                {
                    memset(_gpSavePath, 0, sizeof(_gpSavePath));
                }
        ...
        if (MHAL_WARP_INSTANCE_STATE_READY != eState)
        {
            ...
            // check state fail, reset dump file path
            if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
            {
                memset(_gpSavePath, 0, sizeof(_gpSavePath));
            }
        ...
        if (MHAL_SUCCESS != MHAL_WARP_Trigger(pstChnCtx->hInstHandle, pConfig, (MHAL_WARP_CALLBACK)_MI_WARP_Trigger_Callback, pstChnCtx))
        {
            ...
            // trigger fail, reset dump file path
            if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
            {
                memset(_gpSavePath, 0, sizeof(_gpSavePath));
            }
		...
		if (pstDev->stChannel[pstChnCtx->u32ChnId].stOutputPort.stBindPort.bEnableLowLatencyReceiveMode == TRUE)
        {
            MI_SYS_BufRef_t *pstBufRef = container_of(pstOutBufInfo, MI_SYS_BufRef_t, bufinfo);
...
#endif
                mi_sys_NotifyPreProcessBuf(pstOutBufInfo); 
        ...
        while (!wait_for_completion_timeout(&pstChnCtx->stChnCompletion, msecs_to_jiffies(400)))
        {
            ...
            DBG_INFO("Trigger time: avg=%llu max=%llu min=%llu cur=%llu\n", pstChnCtx->stDebugInfo.stCalcHalTime.u64TotalTime/pstChnCtx->stDebugInfo.stBufCnt.u32TryTriggerCnt
                    ...
                    , _MI_WARP_StatSpendTime(&pstChnCtx->stDebugInfo.stCalcHalTime.stStartTmSpec));

            // trigger timeout, cancel dump file, reset dump file path
            if (_gu8DumpChn == pstChnCtx->u32ChnId && _gpSavePath[0])
            {
                memset(_gpSavePath, 0, sizeof(_gpSavePath));
            }
...
MI_S32 MI_WARP_IMPL_CreateDevice(MI_WARP_DEV devId)
{
...
#if (MI_WARP_PROCFS_DEBUG == 1)
    mi_sys_RegistCommand("dump_chnbuf2file", 2, _MI_WARP_IMPL_DumpChnBufToFile, pstDev->hDevHandle);   // chnId, savePath
    mi_sys_RegistCommand("dump_portbuf2file", 3, _MI_WARP_IMPL_DumpPortBufToFile, pstDev->hDevHandle);
    mi_sys_RegistCommand("dump_table2file", 3, _MI_WARP_IMPL_DumpTableToFile, pstDev->hDevHandle);
    mi_sys_RegistCommand("dump_fps", 0, _MI_WARP_IMPL_DumpFps, pstDev->hDevHandle);
    mi_sys_RegistCommand("dump_frameCnt", 0, _MI_WARP_IMPL_DumpFrameCnt, pstDev->hDevHandle);
    mi_sys_RegistCommand("dump_halTimeConsume", 0, _MI_WARP_IMPL_DumpHalTimeConsume, pstDev->hDevHandle);
    mi_sys_RegistCommand("dump_SetStatDepth", 1, _MI_WARP_IMPL_DumpSetStatDepth, pstDev->hDevHandle);
#endif
    // create mhal_warp
    ...
    DBG_INFO("u32IrqId = %d.\n", u32IrqId);
    s32IrqRet = request_irq(u32IrqId, _MI_WARP_ISR_Proc, IRQ_TYPE_LEVEL_HIGH, "mi_warp_isr", (void *)pstDev->hMhalHandle);
    if(0 != s32IrqRet)
    {
...
MI_S32 MI_WARP_IMPL_DestroyDevice(MI_WARP_DEV devId)
{
    ...
    pstDev->eStatus = E_MI_WARP_DEVICE_UNINIT;
...
#if defined(MI_SYS_PROC_FS_DEBUG) && (MI_WARP_PROCFS_DEBUG == 1)
    _MI_WARP_InitialDebugInfo(&pstDev->stChannel[chnId].stDebugInfo);
#endif

    init_completion(&pstDev->stChannel[chnId].stChnCompletion);

    // create chn workthread
    pstDev->stChannel[chnId].stWorkThread = kthread_run(_MI_WARP_WorkThread, &pstDev->stChannel[chnId], "WarpTest-Chn%d", chnId);
...
//        DBG_ERR("stop chn thread finished\n");
    ...
    if (pstDev->stChannel[chnId].eChnStatus != E_MI_WARP_CHN_UNINIT)
    {
        _MI_WARP_DeinitDebugInfo(&pstDev->stChannel[chnId].stDebugInfo);
    }
#endif
...
MI_S32 MI_WARP_IMPL_SetTable(MI_WARP_DEV devId, MI_WARP_CHN chnId, MI_WARP_TableType_e eTableType, void *pTableAddr, MI_U32 u32TableSize)
{
...
#ifdef MI_WARP_DEBUG_SETBIN_BLOCK_ISSUE
    struct timespec stStartTime, stEndTime;
    DBG_WRN("Enter MI_WARP_IMPL_SetTable\n");
#endif
    ...
    if (eTableType >= MI_WARP_TABLE_TYPE_MAX)
    {
        DBG_ERR("Table type error!\n");
...
#ifdef MI_WARP_DEBUG_SETBIN_BLOCK_ISSUE
    memset(&stStartTime, 0, sizeof(stStartTime));
    do_posix_clock_monotonic_gettime(&stStartTime);
#endif

    if (MI_SUCCESS != mi_sys_MMA_Alloc(NULL, u32TableSize, &phyTableAddr))
    {
        DBG_EXIT_ERR("Sys alloc error!\n");
        s32Ret = MI_ERR_WARP_NULL_PTR;
        mutex_unlock(&pstDev->mtx);
...
#ifdef MI_WARP_DEBUG_SETBIN_BLOCK_ISSUE
    memset(&stEndTime, 0, sizeof(stEndTime));
    do_posix_clock_monotonic_gettime(&stEndTime);
    DBG_WRN("mi_sys_MMA_Alloc spend time: %d\n", (stEndTime.tv_sec - stStartTime.tv_sec)*1000*1000 + (stEndTime.tv_nsec - stStartTime.tv_nsec)/1000);
    ...
    memset(&stEndTime, 0, sizeof(stEndTime));
    do_posix_clock_monotonic_gettime(&stEndTime);
    DBG_WRN("mi_sys_Vmap spend time: %d\n", (stEndTime.tv_sec - stStartTime.tv_sec)*1000*1000 + (stEndTime.tv_nsec - stStartTime.tv_nsec)/1000);
    stStartTime = stEndTime;
#endif
    ...
    memset(&stEndTime, 0, sizeof(stEndTime));
    do_posix_clock_monotonic_gettime(&stEndTime);
    DBG_WRN("copy_from_user spend time: %d\n", (stEndTime.tv_sec - stStartTime.tv_sec)*1000*1000 + (stEndTime.tv_nsec - stStartTime.tv_nsec)/1000);
    stStartTime = stEndTime;
#endif

    switch(eTableType)
    {
        case MI_WARP_DISP_ABSOLUTE_TABLE:
        {
            if (pstMHalWarpCfg->disp_table.table)
            {
                mi_sys_MMA_Free(pstMHalWarpCfg->disp_table.table);
            }
            pstMHalWarpCfg->disp_table.table = phyTableAddr;
            pstMHalWarpCfg->disp_table.size = u32TableSize;
            pstMHalWarpCfg->disp_table.format = MHAL_WARP_MAP_FORMAT_ABSOLUTE;
            DBG_INFO("Absolute table 0x%p size %d\n", pu8Addr, u32TableSize);
        }
        break;
        case MI_WARP_DISP_RELATIVE_TABLE:
        {
            if (pstMHalWarpCfg->disp_table.table)
            {
                mi_sys_MMA_Free(pstMHalWarpCfg->disp_table.table);
            ...
            pstMHalWarpCfg->disp_table.table = phyTableAddr;
            pstMHalWarpCfg->disp_table.size = u32TableSize;
            pstMHalWarpCfg->disp_table.format = MHAL_WARP_MAP_FORMAT_RELATIVE;
            DBG_INFO("Relative table 0x%p size %d\n", pu8Addr, u32TableSize);
        }
        break;
        case MI_WARP_BOUND_BOX_TABLE:
        {
    ...
    memset(&stEndTime, 0, sizeof(stEndTime));
    do_posix_clock_monotonic_gettime(&stEndTime);
    DBG_WRN("mi_sys_UnVmap spend time: %d\n", (stEndTime.tv_sec - stStartTime.tv_sec)*1000*1000 + (stEndTime.tv_nsec - stStartTime.tv_nsec)/1000);
    stStartTime = stEndTime;
#endif
...
#ifdef MI_WARP_DEBUG_SETBIN_BLOCK_ISSUE
    DBG_WRN("Leave MI_WARP_IMPL_SetTable\n");
#endif
