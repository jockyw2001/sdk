...
typedef enum//cys need delete it
{
    MI_DISP_DEV_0 = 0,
    MI_DISP_DEV_1 = 1,
    E_MI_DISP_Dev_MAX
...
typedef struct mi_divp_CaptureInfo_s
{
    ...
    MI_U64 u64Reserved0;//task start runing time : seconds
    MI_U64 u64Reserved1;//task start runing time : Nanoseconds
    MI_U64 u64Reserved2;//create script end time : seconds
    MI_U64 u64Reserved3;//create script end time : Nanoseconds
    MI_PHY phyBufAddr[3];//output buffer address
    MI_U32 u32BufSize;//output buffer size
    MI_U16 u16Width;//output frame width
    MI_U16 u16Height;//output frame height
    MI_U16 u16Stride[3];//output(unit: byte)
    MI_SYS_PixelFormat_e eInputPxlFmt;//input pixel format
    MI_SYS_PixelFormat_e eOutputPxlFmt;//output frame pixel format
...
typedef struct mi_divp_ChnContex_s
{
    ...
    MI_DIVP_CHN u32ChnId;
    MI_U32 u32OverlayMask;
    MI_BOOL bChnAttrChg;//Is channel attribute change
    mi_divp_ChnStatus_e eStatus;
    MI_DIVP_ChnAttr_t stChnAttr;
    MI_DIVP_ChnAttr_t stChnAttrPre;//channel attribute of last.
    ...
    MI_DIVP_ChnAttr_t stChnAttrOrg;//original attribute of channel
    MI_BOOL bIPchg;//I mode to P mode change by divp module, not by APP
    MI_SYS_FieldType_e eaFieldType[2];//last two fields's type
    MI_BOOL bInputChange;
    MI_BOOL bOutputChange;
    MI_BOOL bSendFrameTwice;//if the channel's TNR is opend, when I/P mode change or the first frame is comming in, the frame nned to be send twice to avoid showwing garbage.
    void* pHalCtx;
    mi_sys_ChnTaskInfo_t *pstLastDelayFreeTask;
    MI_BOOL b2Pmode;

    #if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
    MI_U64 u64GetToReleaseTaskSumTime;
    MI_U64 u64GetToReleaseTaskMaxTime;
    MI_U64 u64ReleaseTaskCnt;

    MI_U64 u64GetBufferCnt;
    MI_U64 u64PrepareOutBufCnt;
    MI_U64 u64FinishOutBufCnt;

    MI_BOOL bCheckFrameID;
    MI_BOOL bProcSet2Pmode;
...
typedef struct mi_divp_ProcessOneChnl_s
{
    MI_BOOL bEnable;
    MI_DIVP_CHN  u32ChnId;
    MI_U16  u16AddtaskCnt;
}mi_divp_ProcessOneChnl_t;
#endif
