...
int vif_dh9931_Init(int VifDev , int channel, int pclk, int edge)
{
    ...
    else if(pid == 0)
    {
        ...
        if(Adda_I2COpsRegister)
            Adda_I2COpsRegister(i2c_dummy_write, i2c_dummy_read);
        else
            ST_ERR("adda is not defined, please check!!!\n");
        ...
        for(i = ChnnaleOffset; i < ChnnaleOffset + ChannelRange; i++)
        {
            ...
            while (count--)
            {
                ...
                else if ((AD_VIDEO_IN_HD_1080P_25HZ == ad_status.iVideoFormat) ||
                    (AD_VIDEO_IN_HD_1080P_30HZ == ad_status.iVideoFormat))
                {
...
MI_S32 ST_Vif_CreateDev(MI_VIF_DEV VifDev, VIF_AD_WORK_MODE_E e_WorkMode)
{
    ...
    else if (e_WorkMode == SAMPLE_VI_MODE_MIPI_1_1080P_REALTIME_HDR)
    {
        bSkip9931 = TRUE;
...
MI_S32 ST_Vif_CreatePort(MI_VIF_CHN VifChn, MI_VIF_PORT VifPort, ST_VIF_PortInfo_t *pstPortInfoInfo)
{
	...
	stChnPortAttr.u32FrameModeLineCount = pstPortInfoInfo->u32FrameModeLineCount;
