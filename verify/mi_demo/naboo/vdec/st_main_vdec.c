...
MI_S32 ST_ToggleZoomChn0()
{
    ...
    ST_DBG("u32CurWndNum:%d, disp: %dx%d, max:%dx%d\n", u32CurWndNum, u16CurDispWidth, u16CurDispHeight,
        stDivpChnAttr.u32MaxWidth, stDivpChnAttr.u32MaxHeight);
    if (u32ToggleFlag == 0)
    {
        stDivpChnAttr.stCropRect.u16Width = ALIGN_BACK(stDivpChnAttr.u32MaxWidth / 2, 16);
        stDivpChnAttr.stCropRect.u16Height = ALIGN_BACK(stDivpChnAttr.u32MaxHeight / 2, 2);;
        u32ToggleFlag = 1;
    }
    else if (u32ToggleFlag == 1)
    {
        stDivpChnAttr.stCropRect.u16Width = stDivpChnAttr.u32MaxWidth;
        stDivpChnAttr.stCropRect.u16Height = stDivpChnAttr.u32MaxHeight;
        u32ToggleFlag = 0;
    }
