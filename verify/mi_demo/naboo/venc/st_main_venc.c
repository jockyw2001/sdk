...
int main(int argc, char **argv)
{
    ...
    while (!g_bExit)
    {
        ...
        if (u32Index == ARRAY_SIZE(g_stCaseDesc) + 1)
        {
            // exit
            break;
        }

        if (u32Index <= 0 || u32Index > ARRAY_SIZE(g_stCaseDesc) + 1)
        {
            continue;
        }
    ...
    for(i = 0; i < g_stCaseDesc[g_u32CaseIndex].stDesc.u32VencNum; i++)
    {
        g_stVencThreadArgs[i].bRunFlag = FALSE;
        pthread_join(g_stVencThreadArgs[i].ptGetEs, NULL);
        pthread_join(g_stVencThreadArgs[i].ptFillYuv, NULL);
    }
