...
#define A0_ExecFunc_RET(func, _ret_) \
...
#define A0_ExecFunc_VOID(func, _ret_) \
    printf("%d Start test: %s\n", __LINE__, #func);\
    if (func != _ret_)\
    {\
        printf("AO_TEST [%d] %s exec function failed\n",__LINE__, #func);\
...
void *_mi_ao_GetOutPortBuf(void* data)
{
    ...
    A0_ExecFunc_VOID(MI_AO_GetPubAttr(pstAoOutChn->AoDevId, &stGetAttr), MI_SUCCESS);
...
//Must Init Disp than Init hdmi, if not, hdmi EDID check will be error
MI_S32 ST_AO_InitHdmi(void)
{
    ...
    A0_ExecFunc_RET(MI_DISP_SetPubAttr(DispDev,  &stPubAttr), MI_SUCCESS);

    //A0_ExecFunc_RET(MI_DISP_Enable(DispDev), MI_SUCCESS);
    //A0_ExecFunc_RET(MI_DISP_Disable(DispDev), MI_SUCCESS);

    A0_ExecFunc_RET(ST_Hdmi_Init(), MI_SUCCESS); //Set hdmi outout 1080P
    A0_ExecFunc_RET(ST_Hdmi_Start(E_MI_HDMI_ID_0, E_MI_HDMI_TIMING_1080_60P), MI_SUCCESS);
...
int main(int argc, char *argv[])
{
    ...
    //MI_SYS init
    A0_ExecFunc_RET(MI_SYS_Init(),MI_SUCCESS);

    A0_ExecFunc_RET(ST_AO_InitHdmi(), MI_SUCCESS);
    ...
    for (s32Idx = 0; s32Idx < MI_MAX_AUDIO_DEV; s32Idx ++)
    {
        ...
        /* get ao device*/
        A0_ExecFunc_RET(MI_AO_GetPubAttr(AoDevId[s32Idx], &stGetAttr), MI_SUCCESS);

        /* enable ao device */
        A0_ExecFunc_RET(MI_AO_Enable(AoDevId[s32Idx]), MI_SUCCESS);

        /* enable ao channel of device*/
        A0_ExecFunc_RET(MI_AO_EnableChn(AoDevId[s32Idx], AoChn), MI_SUCCESS);

        // if test resample, enable Resample
        if(bEnableRes)
        {
            A0_ExecFunc_RET(MI_AO_EnableReSmp(AoDevId[s32Idx], AoChn, eInSampleRate), MI_SUCCESS);
        }
        ...
        if(bEnableVqe)
        {
            ...
            ExecFunc(MI_AO_SetVqeAttr(AoDevId[s32Idx], AoChn, &stSetVqeConfig), MI_SUCCESS);
            ExecFunc(MI_AO_GetVqeAttr(AoDevId[s32Idx], AoChn, &stGetVqeConfig), MI_SUCCESS);
            ExecFunc(MI_AO_EnableVqe(AoDevId[s32Idx], AoChn), MI_SUCCESS);
        }
        ...
        if(0 != s32VolumeDb)
        {
            A0_ExecFunc_RET(MI_AO_SetVolume(AoDevId[s32Idx], s32SetVolumeDb), MI_SUCCESS);
            /* get AO volume */
            A0_ExecFunc_RET(MI_AO_GetVolume(AoDevId[s32Idx], &s32GetVolumeDb), MI_SUCCESS);
        }
        ...
        A0_ExecFunc_RET(MI_SYS_SetChnOutputPortDepth(&stAoChn0OutputPort0, 12, 13), MI_SUCCESS);
    }
    ...
    for (s32Idx = 0; s32Idx < MI_MAX_AUDIO_DEV; s32Idx ++)
    {
        ...
        if(bEnableRes)
        {
            stSendParam[s32Idx].stAudioAttr.eSamplerate = eInSampleRate;
        }
    ...
    for (s32Idx = 0; s32Idx < MI_MAX_AUDIO_DEV; s32Idx ++)
    {
        ...
        if(bEnableRes)
        {
            A0_ExecFunc_RET(MI_AO_DisableReSmp(AoDevId[s32Idx], AoChn), MI_SUCCESS);
        }

        /* Disable VQE */
        if(bEnableVqe)
        {
            A0_ExecFunc_RET(MI_AO_DisableVqe(AoDevId[s32Idx], AoChn), MI_SUCCESS);
        }

        /* disable ao channel of */
        A0_ExecFunc_RET(MI_AO_DisableChn(AoDevId[s32Idx], AoChn), MI_SUCCESS);

        /* disable ao device */
        A0_ExecFunc_RET(MI_AO_Disable(AoDevId[s32Idx]), MI_SUCCESS);
    }
