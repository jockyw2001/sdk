...
#define MDB_RGN_BMP_MAXNUM     28
#define MDB_RGN_MIN(x, y) (((x) < (y)) ? (x) : (y))
#define MDB_RGN_MAX(x, y) (((x) > (y)) ? (x) : (y))
#define MDB_RGN_ALIGN_DOWN(val, alignment) (((val)/(alignment))*(alignment))
#define MDB_RGN_ALIGN_UP(val, alignment) ((( (val)+(alignment)-1)/(alignment))*(alignment))
...
static MI_RGN_PaletteTable_t _gstPaletteTable =
{
    ...
    {0, 255, 255, 0}, {0, 0, 112, 255}, {0, 0, 255, 255}, {0, 255, 255, 255},
    {0, 128, 0, 0}, {0, 128, 128, 0}, {0, 128, 0, 128}, {0, 0, 128, 0},
    {0, 0, 0, 128}, {0, 0, 128, 128}, {0, 128, 128, 128}, {0, 64, 64, 64}}
};

static const char * _gszFileName[MDB_RGN_BMP_MAXNUM] = {"/mnt/1555/128X96.argb1555", "/mnt/1555/200X301.argb1555", "/mnt/1555/300X360.argb1555", "/mnt/1555/400X266.argb1555",
                                                        "/mnt/4444/128X96.argb4444", "/mnt/4444/200X301.argb4444", "/mnt/4444/300X360.argb4444", "/mnt/4444/400X266.argb4444",
                                                        "/mnt/i2/64X48.i2", "/mnt/i2/80X80.i2", "/mnt/i2/200X200.i2", "/mnt/i2/304X200.i2",
                                                        "/mnt/i4/64X48.i4", "/mnt/i4/80X80.i4", "/mnt/i4/200X200.i4", "/mnt/i4/304X200.i4",
                                                        "/mnt/i8/64X48.i8", "/mnt/i8/80X80.i8", "/mnt/i8/200X200.i8", "/mnt/i8/304X200.i8",
                                                        "/mnt/565/128X96.rgb565", "/mnt/565/200X301.rgb565", "/mnt/565/300X360.rgb565", "/mnt/565/400X266.rgb565",
                                                        "/mnt/8888/128X96.argb8888", "/mnt/8888/200X301.argb8888", "/mnt/8888/300X360.argb8888", "/mnt/8888/400X266.argb8888"};
...
Mdb_Region::Mdb_Region()
{
    ...
    PREPARE_COMMAND("attach", &Mdb_Region::Attach, 12);
    PREPARE_COMMAND("dettach", &Mdb_Region::Dettach, 5);
    PREPARE_COMMAND("getattr", &Mdb_Region::GetAttr, 5);
    PREPARE_COMMAND("setdisplayattr", &Mdb_Region::SetDisplayAttr, 12);
    ...
    PREPARE_COMMAND("injectpic", &Mdb_Region::InjectPic, 11);
...
void Mdb_Region::Attach(std::vector<std::string> &inStrings, std::string &strOut)
{
    MI_RGN_HANDLE hHandle;
    ...
    stChnAttr.stCoverPara.u32Layer = (MI_U32)Atoi(inStrings[8]);
    stChnAttr.stCoverPara.stSize.u32Width = (MI_U32)Atoi(inStrings[9]);
    stChnAttr.stCoverPara.stSize.u32Height = (MI_U32)Atoi(inStrings[10]);
    stChnAttr.stCoverPara.u32Color = Rgb2Yuv((MI_U32)Atoi(inStrings[11]));
...
void Mdb_Region::SetDisplayAttr(std::vector<std::string> &inStrings, std::string &strOut)
{
    MI_RGN_HANDLE hHandle;
    MI_RGN_ChnPort_t stChnPort;
    ...
    stChnPortAttr.stCoverPara.u32Layer = (MI_U32)Atoi(inStrings[8]);
    stChnPortAttr.stCoverPara.stSize.u32Width = (MI_U32)Atoi(inStrings[9]);
    stChnPortAttr.stCoverPara.stSize.u32Height = (MI_U32)Atoi(inStrings[10]);
    stChnPortAttr.stCoverPara.u32Color = Rgb2Yuv((MI_U32)Atoi(inStrings[11]));
...
void Mdb_Region::SetBitMap(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    u32FileSize = _gstSize[u8FileId].u32Height * MDB_RGN_ALIGN_UP((u8BitCount*_gstSize[u8FileId].u32Width) , 8) / 8;
...
void Mdb_Region::SetCanvas(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    u32CopyWidthBytes = MDB_RGN_ALIGN_UP((u8BitCount * u16CopyWidth), 8) / 8;
...
void Mdb_Region::TestCase005(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    switch(_gstRgnAttr.eType)
    {
        ...
        case E_MI_RGN_TYPE_COVER:
            ...
            u32RgnWidth = _gstChnPortParam.stCoverPara.stSize.u32Width;
            u32RgnHeight = _gstChnPortParam.stCoverPara.stSize.u32Height;
            break;
        default:
            printf("wrong region type\n");
            SetDefaultParam();
    ...
    for (MI_U8 i = 0; i < u8RgnMaxNum+1; i++)
    {
        ...
        {
            _gstChnPortParam.stCoverPara.u32Layer++;
        }
    }
    ...
    switch(u8FlowId)
    {
        case 0:
            ...
            MDB_EXPECT_OK_ERRCASE("MI_RGN_GetCanvasInfo", strOut, MI_RGN_GetCanvasInfo(_ghHandle, &stCanvasInfo), MI_RGN_OK,
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            u32CopyBytes = MDB_RGN_MIN(stCanvasInfo.stSize.u32Width, _gstSize[u8FileId].u32Width) * 2;
            for (MI_U32 i = 0; i < MDB_RGN_MIN(stCanvasInfo.stSize.u32Height, _gstSize[u8FileId].u32Height); i++)
            {
            ...
            MDB_EXPECT_OK_ERRCASE("MI_RGN_UpdateCanvas", strOut, MI_RGN_UpdateCanvas(_ghHandle), MI_RGN_OK,
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            MDB_EXPECT_OK_ERRCASE("MI_RGN_AttachToChn", strOut, MI_RGN_AttachToChn(_ghHandle, &_gstChnPort, &_gstChnPortParam), MI_RGN_OK,
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            vecStrIn.clear();
            memset(szParamIn, 0, sizeof(szParamIn));
            sprintf(szParamIn, "%d %d", (MI_U8)_gstChnPort.eModId, (MI_U8)_gstChnPort.s32ChnId);
...
void Mdb_Region::TestCase011(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    for (MI_U8 i = 0; i < MDB_RGN_OSD_MAX_NUM; i++)
    {
        ...
        {
            u32RgnMaxHeight = MDB_RGN_MAX(u32RgnMaxHeight, _gstSize[i].u32Height);
        }
...
void Mdb_Region::OneRegionOneAttach(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    stChnAttr.stCoverPara.stSize.u32Height = 300;
    stChnAttr.stCoverPara.stSize.u32Width = 300;
    stChnAttr.stCoverPara.u32Color = Rgb2Yuv(0x0333);
    stChnAttr.stCoverPara.u32Layer = 1;
...
void Mdb_Region::OneRegionMaxAttach(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    memcpy(&stChnPortParam, &_gstChnPortParam, sizeof(MI_RGN_ChnPortParam_t));
...
void Mdb_Region::SetDefaultParam()
{
    ...
    _gstChnPortParam.stCoverPara.u32Layer = 0;
    _gstChnPortParam.stCoverPara.stSize.u32Width = 40;
    _gstChnPortParam.stCoverPara.stSize.u32Height = 40;
    _gstChnPortParam.stCoverPara.u32Color = 0;
...
void Mdb_Region::VpeDivpAddData(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    ss << "0 0 " <<intFormat<< " " << _gstSize[intFormat * 4].u32Width << " " << _gstSize[intFormat * 4].u32Height;
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "1 0 " <<intFormat<< " " << _gstSize[intFormat * 4 + 1].u32Width << " " << _gstSize[intFormat * 4 + 1].u32Height;
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "2 0 " <<intFormat<< " " << _gstSize[intFormat * 4 + 2].u32Width << " " << _gstSize[intFormat * 4 + 2].u32Height;
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "3 0 " <<intFormat<< " " << _gstSize[intFormat * 4 + 3].u32Width << " " << _gstSize[intFormat * 4 + 3].u32Height;
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "0 " <<intFormat * 4<<" "<<intFormat;
    ParseStrings(ss.str().c_str(), vecStrIn);
    if (bSelectMode)
...
void Mdb_Region::VpeDivpAttachData(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    for (i = 0; i < intMaxChn; i++)
    {
        for (j = 0; j < 4; j++)
        {
            if (!intPort[j])
                continue;
            ss << "0 " << intMod << " 0 " << i << " " << j << " 1 " << _gstPos[intCurFormat * 4].u32X <<" " << _gstPos[intCurFormat * 4].u32Y <<" 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "1 " << intMod << " 0 " << i << " " << j << " 1 " << _gstPos[intCurFormat * 4 + 1].u32X <<" " << _gstPos[intCurFormat * 4 + 1].u32Y <<" 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "2 " << intMod << " 0 " << i << " " << j << " 1 " << _gstPos[intCurFormat * 4 + 2].u32X <<" " << _gstPos[intCurFormat * 4 + 2].u32Y <<" 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "3 " << intMod << " 0 " << i << " " << j << " 1 " << _gstPos[intCurFormat * 4 + 3].u32X <<" " << _gstPos[intCurFormat * 4 + 3].u32Y <<" 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "4 " << intMod << " 0 " << i << " " << j << " 1 1200 1200 0 1000 1000 0xFF";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "5 " << intMod << " 0 " << i << " " << j << " 1 1800 1800 1 1000 1000 0xFF00";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "6 " << intMod << " 0 " << i << " " << j << " 1 2400 2400 2 1000 1000 0xFF0000";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "7 " << intMod << " 0 " << i << " " << j << " 1 3000 3000 3 1000 1000 0x1F";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");
    ...
    for (i = 0; i < PROCESS_CNT; i++)
    {
        for (j = 0; j < intMaxChn; j++)
        {
            for (k = 0; k < 4; k++)
            {
                if (!intPort[k])
                    continue;
                ss << "0 " << intMod << " 0 "<< j << " " << k << " 1 0 "<< (int)(i * PROCESS_STEP) << " 0 0 0 0";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "1 " << intMod << " 0 "<< j << " " << k << " 1 64 "<< (int)(i * PROCESS_STEP) << " 0 0 0 0";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "2 " << intMod << " 0 "<< j << " " << k << " 1 200 "<< (int)(i * PROCESS_STEP) << " 0 0 0 0";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "3 " << intMod << " 0 "<< j << " " << k << " 1 "<< (int)(i * PROCESS_STEP) << " 400 0 0 0 0";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "4 " << intMod << " 0 "<< j << " " << k << " 1 "<< (int)(1200 + i * PROCESS_STEP) << " " << (int)(1200 + i * PROCESS_STEP) << " 0 1000 1000 0xFF";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "5 " << intMod << " 0 "<< j << " " << k << " 1 "<< (int)(1800 + i * PROCESS_STEP) << " " << (int)(1800 + i * PROCESS_STEP) << " 1 1000 1000 0xFF00";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "6 " << intMod << " 0 "<< j << " " << k << " 1 "<< (int)(2400 + i * PROCESS_STEP) << " " << (int)(2400 + i * PROCESS_STEP) << " 2 1000 1000 0xFF0000";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");

                ss << "7 " << intMod << " 0 "<< j << " " << k << " 1 "<< (int)(3000 + i * PROCESS_STEP) << " " << (int)(3000 + i * PROCESS_STEP) << " 3 1000 1000 0x1F";
                ParseStrings(ss.str().c_str(), vecStrIn);
                SetDisplayAttr(vecStrIn, strOut);
                vecStrIn.clear();
                ss.str("");
...
void Mdb_Region::InjectPic(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    u32Stride = (MDB_RGN_ALIGN_UP(u8BitCount * MDB_RGN_ALIGN_UP(u32Width, u8Pitch) , 8) / 8);
    ...
    u32CopyWidthBytes =  MDB_RGN_ALIGN_UP(u8BitCount * u32Width , 8) / 8;
    ...
    ss << hRgnHandle << " " << eMod << " 0 " << s32Chn << " " << s32Port << " 1 " << u32PosX <<" " << u32PoxY <<" 0 0 0 0";
    printf("%s\n", ss.str().c_str());
    ParseStrings(ss.str().c_str(), vecStrIn);
    Attach(vecStrIn, strOut);
    vecStrIn.clear();
