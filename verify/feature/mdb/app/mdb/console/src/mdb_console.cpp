...
#ifdef USE_TVOS
static void binder_send_data(sp<TvManager> pBinder, char *pTransData, int intTransDataSize)
{
    char *pRet = NULL;
    String8  s8SetStr;
    int32_t int32DataLenth = 0;
    int intRet = 0;

    if (pTransData == NULL || pBinder == NULL || intTransDataSize == 0)
    {
        return;
    }
    s8SetStr.setTo(pTransData);
    pRet = (char *)pBinder->setTvosCommonCommand(s8SetStr, int32DataLenth);
    if(pRet != NULL)
    {
       intRet = int32DataLenth;
       memset(pTransData, 0, intTransDataSize);
       strncpy(pTransData, pRet, intRet);
    }
}
#endif
#ifdef USE_SOCKET
static int socket_init(const char *pAddr)
{
    ...
    strcpy (stAddress.sun_path, pAddr);
...
static int send_cmd(char *pTransData, int intTransDataSize)
{
    ...
    {
#ifdef USE_TVOS
        binder_send_data(pBinderClient, pTransData, intTransDataSize);
#endif
...
int mdb_console(int bExitService)
{
...
#ifdef USE_SOCKET
    gsocketFd = socket_init(SOCKET_ADDR);
#endif
