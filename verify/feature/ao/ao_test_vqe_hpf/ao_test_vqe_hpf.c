...
{
    printf("\n");
    printf("Please enter: feature_ao_test_vqe_hpf -i xxx -o yyy -h zzz -d aaa\n");
    printf("xxx: input file path \n");
    printf("yyy: output file path \n");
    printf("zzz:  Cut Frequency of HPF \n");
...
int main(int argc, char *argv[])
{
    ...
    MI_BOOL bVqeHpfEnable = FALSE;
    MI_AUDIO_HpfFreq_e eHpfFreq;
    ...
    while ((s32Opt = getopt(argc, argv, "i:o:h:d:")) != -1)
    {
            switch(s32Opt)
            {
                case 'i':
                ...
                case 'h':
                    bVqeHpfEnable = TRUE;
                    eHpfFreq = (MI_AUDIO_HpfFreq_e)atoi(optarg);
                    printf("Enable HPF of VQE, Set cut Frequency of HPF is %d \n", eHpfFreq);
                    break;
                ...
                case '?':
                    if(optopt == 'i')
                        ...
                        printf("Missing cut Frequency of HPF setting, please -h 'eHpfFreq:[80, 120, 150]' \n");
                    else if(optopt == 'd')
    ...
    if(bEnableVqe)
    {
        ...
        stSetVqeConfig.bHpfOpen = bVqeHpfEnable;
