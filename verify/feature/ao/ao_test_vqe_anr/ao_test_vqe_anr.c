...
static MI_U16  _gau16GaindB[129] = {
...
{
    printf("\n");
    printf("Please enter: feature_ao_test_vqe_anr -i xxx -o yyy -n zzz -d aaa \n");
    printf("xxx: input file path \n");
    printf("yyy: output file path \n");
    printf("zzz: anr intensity [0, 30]\n");
    printf("aaa: AO device ID\n");
}
...
void *_mi_ao_GetOutPortBuf(void* data)
{
    ...
    if(fdWr > 0)
    {
        while(1)
        {
            ...
            {
                MI_U32 u32WriteSize = stBufInfo.stRawData.u32BufSize;
...
int main(int argc, char *argv[])
{
    ...
    MI_BOOL bVqeAnrEnable = FALSE;
    MI_S16 s16SetNrIntensity = 0;
    ...
    while ((s32Opt = getopt(argc, argv, "i:o:n:d:")) != -1)
    {
            switch(s32Opt)
            {
                case 'i':
                ...
                case 'n':
                    bVqeAnrEnable = TRUE;
                    s16SetNrIntensity = atoi(optarg);
                    printf("Enable ANR of VQE, Set NrIntensity is %d \n", s16SetNrIntensity);
                    break;
                ...
                case '?':
                    if(optopt == 'i')
                        ...
                        printf("Missing output file path, please -o 'output file path' \n");
                    else if(optopt == 'n')
                        printf("Missing NR intensity value, please -n 'NR intensity value, [0, 30]' \n");
                    else if(optopt == 'd')
    ...
    if(bEnableVqe)
    {
        ...
        stSetVqeConfig.bAnrOpen = bVqeAnrEnable;
