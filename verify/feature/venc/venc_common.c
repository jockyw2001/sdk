...
int venc_stream_open(Chn_t *pstChn, MI_VENC_ModType_e eType)
{
    ...
    if(pstChn->pstPrivate->u32DumpFrame > 0)
    {
        ...
        pstChn->pstPrivate->fdOutEs = open(szOutputPath, O_RDWR | O_CREAT | O_TRUNC, 0644);
...
int venc_stream_write(Chn_t *pstChn, MI_VENC_Stream_t *pstStream, MI_U32 u32ContentSize)
{
    ...
    if(u32ContentSize > 0)
    {
        ...
        if(pstChn->pstPrivate->bDumpEachFrame && pstStream->u32Seq < pstChn->pstPrivate->u32DumpFrame)
        {
            ...
            //S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH
            pstChn->pstPrivate->fdJpg = open(szOutputPath, O_RDWR|O_CREAT|O_TRUNC, 0644);
...
void *venc_channel_func(void *arg)
{
    ...
    if(venc_stream_open(pstChn, stAttr.stVeAttr.eType) != 0)
    {
        DBG_ERR("Unable to open stream %d\n", pstChn->u32ChnId);
        return NULL;
    }
    ...
    if(venc_stream_close(pstChn) != 0)
    {
        DBG_ERR("Unable to close stream %d\n", pstChn->u32ChnId);
        return NULL;
    }
