...
int _VENC_PutFrames(void)
{
    ...
    while (1)
    {
        ...
        if(iRet != 0)
            break;
        stParam.u32FrameSeq++;
        iRet = _VENC_DoInputFrameEnd(&stParam);

        if (stBufInfo.bEndOfStream || (iRet != 0))
        {
            break;
        }
...
int _VENC_ConfigH264(int iChn)
{
    ...
    if (iVal == 1) //enable deblocking filter control
    {
        ...
        memset(&stIntra, 0, sizeof(stIntra));
        iVal = get_cfg_int("constrained_intra_pred_flag", &bErr);
        if(bErr == FALSE)
            stIntra.constrained_intra_pred_flag = iVal;
        iVal = get_cfg_int("u32Intra16x16Penalty", &bErr);
        if(bErr == FALSE)
            stIntra.u32Intra16x16Penalty = iVal;
