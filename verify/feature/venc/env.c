...
VENC_Config_t CFG[] =
{
    ...
    DECL_INT(VENC_GLOB_APPLY_CFG,  0),
        ...
        DECL_INT(VENC_ROI_SRC_RATE, 0),//set 0 to skip rate setting
        DECL_INT(VENC_ROI_DST_RATE, 0),//set 0 to skip rate setting
    ...
    DECL_INT(VENC_SUPER_FRAME_MODE, -1), //This is now tested as CFG only but it should be set as dynamic. -1: don't care, 0: set to none, 1: set to discard, 2: re-encode
        ...
        DECL_INT(RcPrio, -1),//-1:don change the setting, 1:Bit rate first 2:Threshold first
    DECL_INT(VENC_FRAME_LOST, -1), //This is now tested as CFG only but it should be set as dynamic. -1: don't care, 0: set to disable, 1: set to enable
        DECL_INT(FrmLostGap, 1),
        DECL_INT(FrmLostBps, 1),
    // ==== post-frame-putting ====
