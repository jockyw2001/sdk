...
#define VENC_SUB_CHN_START  9
#define VPE_SUB_CHN_START   9

#define DIVP_VDEC_CHN_OFFSET 9

#define VDISP_VENC_W 1920
#define VDISP_VENC_H 1080
...
{
    {
        ...
        {
            .u32CaseIndex = 0,
            .szDesc = "8x1080P Capture, 8xVENC(Main@1080P+Sub@CIF), 8xVif Preview",
            .u32WndNum = 8,
            .u32VencNum = 8,
            .bNeedVdisp = TRUE,
        },
        ...
        {
            {
                .eVideoChnType = E_ST_VIF_CHN | E_ST_VIF_SUB_CHN,
                .uChnArg =
                {
                    .stVifChnArg =
                    {
...
void *ST_VDFGetResult(void *args)
{
    ...
    while(pstArgs->bRunFlag)
    {
            ...
            (1 == stVdfResult.unVdfResult.stOdResult.u8Enable)))
		{
		    if (pstArgs->enWorkMode == E_MI_VDF_WORK_MODE_MD)
            {
                pu8MdRstData = stVdfResult.unVdfResult.stMdResult.pstMdResultSad;
...
#if 1
            if (8 == pArgs->stVencAttr[0].vencChn)
            {
...
int ST_SplitWindow()
{
    ...
    for (i = 0; i < u32LastWndNum; i++)
    {
        ...
        STCHECKRESULT(MI_DISP_HideInputPort(ST_DISP_LAYER0, i));
    }
    ...
    if (8 == u32CurWndNum)
    {
        MI_U16 u16DivW = ALIGN_BACK(u16DispWidth / 4, 2);
        MI_U16 u16DivH = ALIGN_BACK(u16DispHeight / 4, 2);
    ...
    else
    {
        ...
        {
            stDispWndRect[i].s32X    = ALIGN_BACK((u16DispWidth / u32Square) * (i % u32Square), 2);
            stDispWndRect[i].s32Y    = ALIGN_BACK((u16DispHeight / u32Square) * (i / u32Square), 2);
            stDispWndRect[i].u16PicW = ALIGN_BACK((u16DispWidth / u32Square), 2);
            stDispWndRect[i].u16PicH = ALIGN_BACK((u16DispHeight / u32Square), 2);
        }
    }
    ...
    for (i = 0; i < u32CurWndNum; i++)
    {
        ...
        stOutputPortAttr.u32Width = stDispWndRect[i].u16PicW;
        stOutputPortAttr.u32Height = stDispWndRect[i].u16PicH;
    ...
    for (i = 0; i < u32CurWndNum; i++)
    {
        vpeChn = i + VPE_SUB_CHN_START;

        memset(&stVpeMode, 0, sizeof(MI_VPE_PortMode_t));
        ExecFunc(MI_VPE_GetPortMode(vpeChn, DISP_PORT, &stVpeMode), MI_VPE_OK);
        stVpeMode.u16Width = stDispWndRect[i].u16PicW;
        stVpeMode.u16Height = stDispWndRect[i].u16PicH;
    ...
    for (i = 0; i < u32CurWndNum; i++)
    {
        ...
        stInputPortAttr.stDispWin.u16X      = stDispWndRect[i].s32X;
        stInputPortAttr.stDispWin.u16Y      = stDispWndRect[i].s32Y;
        stInputPortAttr.stDispWin.u16Width  = stDispWndRect[i].u16PicW;
        stInputPortAttr.stDispWin.u16Height = stDispWndRect[i].u16PicH;
    ...
    {
        vpeChn = i;
        STCHECKRESULT(MI_DISP_EnableInputPort(ST_DISP_LAYER0, i));
        STCHECKRESULT(MI_DISP_ShowInputPort(ST_DISP_LAYER0, i));
    }
...
MI_S32 ST_SubExit()
{
    ...
    MI_U32 u32VencNum = 0;
    MI_S32 s32CaseIndex = g_u32CaseIndex;
    MI_S32 s32CapChnNum = u32WndNum;
    MI_U32 u32DevId = 0;
    MI_S32 s32Ret = MI_SUCCESS;
    MI_S32 s32DispChnNum;
    ...
    *************************************************/
    for (i = 0; i < u32ShowWndNum; i++)
    {
        ...
        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VIF_SUB_CHN)
        {
            ...
            stBindInfo.stDstChnPort.eModId = E_MI_MODULE_ID_VPE;
            stBindInfo.stDstChnPort.u32DevId = 0;
            stBindInfo.stDstChnPort.u32ChnId = i + VPE_SUB_CHN_START;
            stBindInfo.stDstChnPort.u32PortId = 0;
            STCHECKRESULT(ST_Sys_UnBind(&stBindInfo));
        }
    }
    ...
    *************************************************/
    for (i = 0; i < u32ShowWndNum; i++)
    {
        ...
        stBindInfo.stSrcChnPort.u32DevId = 0;
        stBindInfo.stSrcChnPort.u32ChnId = i + VPE_SUB_CHN_START; //Need cover vdec preview
    ...
    *************************************************/
    ...
    for (i = 0; i < u32VencNum; i++)
    {
        ...
        s32Ret = MI_VENC_GetChnDevid(i + VENC_SUB_CHN_START, &u32DevId);
        printf("sub device id ---(%d)...\n", u32DevId);
        if (MI_SUCCESS != s32Ret)
        {
            ST_ERR("MI_VENC_GetChnDevid %d error, %X\n",i, s32Ret);
        }
        stBindInfo.stDstChnPort.u32DevId = u32DevId;
        stBindInfo.stDstChnPort.u32ChnId = i + VENC_SUB_CHN_START;
        stBindInfo.stDstChnPort.u32PortId = 0;
        //STCHECKRESULT(ST_Sys_UnBind(&stBindInfo));
    ...
    if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
    {
        ...
        s32Ret = MI_VENC_GetChnDevid(u32VencNum, &u32DevId);
        if (MI_SUCCESS != s32Ret)
        {
            ST_ERR("MI_VENC_GetChnDevid %d error, %X\n",i, s32Ret);
        }
        stBindInfo.stDstChnPort.eModId = E_MI_MODULE_ID_VENC;
        stBindInfo.stDstChnPort.u32DevId = u32DevId;
        stBindInfo.stDstChnPort.u32ChnId = u32VencNum;
        stBindInfo.stDstChnPort.u32PortId = 0;
        //STCHECKRESULT(ST_Sys_UnBind(&stBindInfo));
    ...
    *************************************************/
    ...
    for (i = 0; i < u32ShowWndNum; i++)
    {
        ...
        stAttachChnPort.s32ChnId = i;
        stAttachChnPort.s32OutputPortId = DISP_PORT;

        s32Ret = MI_RGN_DetachFromChn(hHandle, &stAttachChnPort);
        if (MI_RGN_OK != s32Ret)
        {
            ST_ERR("MI_RGN_DetachFromChn error, hHandle:%d, %X\n", hHandle, s32Ret);
            return 1;
        }
        ...
        if (MI_RGN_OK != s32Ret)
        {
            ST_ERR("MI_RGN_Destroy error, hHandle:%d, %X\n", hHandle, s32Ret);
            return 1;
        }
#endif
        ...
        stAttachChnPort.eModId = E_MI_RGN_MODID_VPE;
        stAttachChnPort.s32DevId = 0;
        stAttachChnPort.s32ChnId = i + VPE_SUB_CHN_START;
        stAttachChnPort.s32OutputPortId = MAIN_VENC_PORT;
    ...
    *************************************************/
    for (i = 0; i < u32ShowWndNum; i++)
    {
        ...
        STCHECKRESULT(ST_Vpe_StopChannel(i + VPE_SUB_CHN_START));

#ifdef SUPPORT_VIDEO_PREVIEW
        STCHECKRESULT(ST_Vpe_StopPort(i, DISP_PORT));
#endif

#ifdef SUPPORT_VIDEO_ENCODE
        STCHECKRESULT(ST_Vpe_StopPort(i, MAIN_VENC_PORT));
        STCHECKRESULT(ST_Vpe_StopPort(i, SUB_VENC_PORT));
        if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
        {
            STCHECKRESULT(ST_Vpe_StopPort(i, VDISP_PORT));
        }
#endif

        STCHECKRESULT(ST_Vpe_DestroyChannel(i));
        STCHECKRESULT(ST_Vpe_DestroyChannel(i + VPE_SUB_CHN_START));
    }
    ...
    *************************************************/
    ...
    {
        ST_DBG("ST_Vif_DisableDev....DEV(%d)...\n", i);
        STCHECKRESULT(ST_Vif_DisableDev(i));
    }
    ...
    *************************************************/
    ...
    {
#ifdef SUPPORT_VIDEO_ENCODE
        STCHECKRESULT(MI_VENC_StopRecvPic(i));
        STCHECKRESULT(MI_VENC_DestroyChn(i));
        STCHECKRESULT(MI_VENC_StopRecvPic(i + VENC_SUB_CHN_START));
        STCHECKRESULT(MI_VENC_DestroyChn(i + VENC_SUB_CHN_START));
#endif
    }
    if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
    {
        STCHECKRESULT(MI_VENC_StopRecvPic(u32VencNum));
        STCHECKRESULT(MI_VENC_DestroyChn(u32VencNum));
    }
...
MI_S32 ST_CreateVencChannel(MI_S32 s32VencChn, MI_S32 s32VencType, MI_U16 u16Width, MI_U16 u16Height, MI_S32 s32FrameRate)
{
    ...
    printf("Venc: chn(%d) venctype(%d) w(%d) h(%d) frate(%d)...\n",
        s32VencChn, s32VencType, u16Width, u16Height, s32FrameRate);
    if (E_MI_VENC_MODTYPE_H264E == s32VencType)
    {
        ...
        stChnAttr.stRcAttr.stAttrH264Cbr.u32Gop = s32FrameRate;
    ...
    else if (E_MI_VENC_MODTYPE_H265E == s32VencType)
    {
        ...
        stChnAttr.stRcAttr.stAttrH265FixQp.u32Gop = s32FrameRate;
    ...
    else if (E_MI_VENC_MODTYPE_JPEGE == s32VencType)
    {
        ...
        stChnAttr.stVeAttr.stAttrJpeg.u32PicWidth = ALIGN_N(u16Width, 32);
        stChnAttr.stVeAttr.stAttrJpeg.u32PicHeight = ALIGN_N(u16Height, 32);
        stChnAttr.stVeAttr.stAttrJpeg.u32MaxPicWidth = ALIGN_N(u16Width, 32);
        stChnAttr.stVeAttr.stAttrJpeg.u32MaxPicHeight = ALIGN_N(u16Height, 32);
    }
...
MI_S32 ST_VifToDisp(MI_S32 s32CaseIndex)
{
    ...
    ST_VPE_PortInfo_t stPort1Info;
    ST_VPE_PortInfo_t stPort2Info;
    ST_VPE_PortInfo_t stPort3Info;

    MI_SYS_ChnPort_t stChnPort;
    MI_SYS_ChnPort_t stChnPort1;
    MI_SYS_ChnPort_t stChnPort2;
    MI_SYS_ChnPort_t stChnPort3;

    MI_SYS_ChnPort_t stvifChnPort0;
    MI_SYS_ChnPort_t stvifChnPort1;
    ST_Rect_t stRect;
    ST_Sys_BindInfo_t stBindInfo;
    ...
    for (i = 0; i < u32WndNum; i++) //8xWnd Vdisp
    {
        stVDispWndRect[i].s32X    = ALIGN_BACK((VDISP_VENC_W / u32Square) * (i % u32Square), 2);
        stVDispWndRect[i].s32Y    = ALIGN_BACK((VDISP_VENC_H / u32Square) * (i / u32Square), 2);
        stVDispWndRect[i].u16PicW = ALIGN_BACK((VDISP_VENC_W / u32Square), 2);
        stVDispWndRect[i].u16PicH = ALIGN_BACK((VDISP_VENC_H / u32Square), 2);
    }
    switch (s32CaseIndex)
    {
        case 0:
        case 3:
        case 4://8xWnd average
        {
            for (i = 0; i < u32WndNum; i++)
            {
        ...
        case 2: //1xWnd
        {
            ...
            stDispWndRect[0].u16PicW = stdispRect.u16PicW;
            stDispWndRect[0].u16PicH = stdispRect.u16PicH;
            break;
        }
        default:
            ST_ERR("CaseIndex over range (%d)\n", s32CaseIndex);
            return -1;
    }
    /************************************************
    Step1:  init SYS
    ...
    *************************************************/
    ...
    switch (s32CaseIndex)
    {
        ...
        case 3:
        {
            ...
            for (i = 0; i < 8; i++)
            {
                ...
                stVifChnCfg[i].u8ViChn = 2*i;
            }
            s32AdWorkMode = SAMPLE_VI_MODE_2X_DOUBLE_1080P;
            break;
        }
        case 4:
        {
    ...
    for (i = 0; i < s32CapChnNum; i++) //init vif channel
    {
        ...
        stVifPortInfoInfo.u32FrameRate = pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.s32FrmRate;
        STCHECKRESULT(ST_Vif_CreatePort(stVifChnCfg[i].u8ViChn, 0, &stVifPortInfoInfo));
        ST_DBG("============vif channel(%d) port(0) x(%d)-y(%d)-w(%d)-h(%d)..\n", i, stVifPortInfoInfo.u32RectX,
            stVifPortInfoInfo.u32RectY, stVifPortInfoInfo.u32RectWidth, stVifPortInfoInfo.u32RectHeight);

        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VDF_CHN)
        {
            stPortInfo.ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420;//E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
            ...
            STCHECKRESULT(ST_Vpe_CreatePort(VDISP_PORT, &stPortInfo));
        }
    ...
    *************************************************/
    ...
    MI_U16 u16DivpOutWidth = 0;
    MI_BOOL bDiFlag = FALSE;
    u16DivpOutWidth = 640;
    u16DivpOutHeight = 360;
    for (i = 0; i < s32CapChnNum; i++)
    {
        ST_CreateDivpChannel(i, 1920,
            1080,
            stDispWndRect[i].u16PicW,
            stDispWndRect[i].u16PicH, FALSE);
        memset(&stChnPort, 0x0, sizeof(MI_SYS_ChnPort_t));
        stChnPort.eModId = E_MI_MODULE_ID_DIVP;
        stChnPort.u32DevId = 0;
    ...
    *************************************************/
    ...
    for (i = 0; i < s32CapChnNum; i++)
    {
        ...
        STCHECKRESULT(ST_Vpe_CreatePort(MAIN_VENC_PORT, &stPortInfo)); //default support port0 --->>> vdisp

        memset(&stChnPort, 0x0, sizeof(MI_SYS_ChnPort_t));
        stChnPort.eModId = E_MI_MODULE_ID_VPE;
        stChnPort.u32DevId = 0;
        stChnPort.u32ChnId = i;//0 1 2 3
        stChnPort.u32PortId = MAIN_VENC_PORT;
        MI_SYS_SetChnOutputPortDepth(&stChnPort, 0,7);

        #if 1
        stPort1Info.DepVpeChannel = i;
        stPort1Info.ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420;//E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
        stPort1Info.u16OutputWidth = SUB_VENC_W;
        stPort1Info.u16OutputHeight = SUB_VENC_H;
        STCHECKRESULT(ST_Vpe_CreatePort(SUB_VENC_PORT, &stPort1Info)); //default support port0 --->>> vdisp
        memset(&stChnPort1, 0x0, sizeof(MI_SYS_ChnPort_t));
        stChnPort1.eModId = E_MI_MODULE_ID_VPE;
        stChnPort1.u32DevId = 0;
        stChnPort1.u32ChnId = i;//0 1 2 3
        stChnPort1.u32PortId = SUB_VENC_PORT;
        MI_SYS_SetChnOutputPortDepth(&stChnPort1, 0,7);

        stPort2Info.DepVpeChannel = i;
        stPort2Info.ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420;//E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
        stPort2Info.u16OutputWidth = 640;
        stPort2Info.u16OutputHeight = 360;
        STCHECKRESULT(ST_Vpe_CreatePort(VDISP_PORT, &stPort2Info)); //default support port0 --->>> vdisp
        memset(&stChnPort2, 0x0, sizeof(MI_SYS_ChnPort_t));
        stChnPort2.eModId = E_MI_MODULE_ID_VPE;
        stChnPort2.u32DevId = 0;
        stChnPort2.u32ChnId = i;//0 1 2 3
        stChnPort2.u32PortId = VDISP_PORT;
        MI_SYS_SetChnOutputPortDepth(&stChnPort2, 0,7);
        #endif
        ...
        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VIF_SUB_CHN)
        {
            ...
            stPortInfo.DepVpeChannel = vpeChn;

#ifdef SUPPORT_VIDEO_PREVIEW
            stPortInfo.ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV_MST_420;
            ...
            memset(&stChnPort3, 0x0, sizeof(MI_SYS_ChnPort_t));
            stChnPort3.eModId = E_MI_MODULE_ID_VPE;
            stChnPort3.u32DevId = 0;
            stChnPort3.u32ChnId = vpeChn;//0 1 2 3
            stChnPort3.u32PortId = DISP_PORT;
            MI_SYS_SetChnOutputPortDepth(&stChnPort3, 0,5);
#endif
        }
    ...
    ///// vdisp
    MI_VDISP_DEV vdispDev = MI_VDISP_DEV_0;
    MI_S32 s32FrmRate = 13;
    MI_S32 s32OutputPort = 0;
    MI_VDISP_PORT vdispPort = 0;
    if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
    {
        MI_SYS_ChnPort_t stvdispChnPort;
        STCHECKRESULT(ST_Vdisp_Init());
        STCHECKRESULT(ST_Vdisp_SetOutputPortAttr(vdispDev,
                        s32OutputPort, &stdispRect, s32FrmRate, 1));
        STCHECKRESULT(ST_Vdisp_StartDevice(vdispDev));
        memset(&stvdispChnPort, 0x0, sizeof(MI_SYS_ChnPort_t));
        stvdispChnPort.eModId = E_MI_MODULE_ID_VDISP;
        stvdispChnPort.u32DevId = 0;
        stvdispChnPort.u32ChnId = 0;//0 1 2 3
        stvdispChnPort.u32PortId = 0;
        MI_SYS_SetChnOutputPortDepth(&stvdispChnPort, 0,5);
    }
    ...
    *************************************************/
    ...
    if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
    {
        ...
        {
            vdispPort = i;
            stRect.s32X     = ALIGN_BACK(stDispWndRect[i].s32X, 16);
            stRect.s32Y     = ALIGN_BACK(stDispWndRect[i].s32Y, 2);
            stRect.u16PicW  = ALIGN_BACK(stDispWndRect[i].u16PicW, 16);
            stRect.u16PicH  = ALIGN_BACK(stDispWndRect[i].u16PicH, 2);
        ...
        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VDF_CHN)
        {
            ...
            for (j = 0; j < pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.u16MdNum; j ++)
            {
                ...
                stVdfAttr.unAttr.stMdAttr.stMdStaticParamsIn.roi_md.pnt[1].x =
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stMdArea[j].s32X +
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stMdArea[j].u16PicW;
                    //720 - 1;//g_width - 1;
                stVdfAttr.unAttr.stMdAttr.stMdStaticParamsIn.roi_md.pnt[1].y =
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stMdArea[j].s32Y;//0;
                ...
                // STCHECKRESULT(MI_VDF_CreateChn(vdfChn, &stVdfAttr));
    	        // STCHECKRESULT(MI_VDF_EnableSubWindow(vdfChn, 0, 0, 1));
            }
            ...
            {
                // vdfChn = i * s32CapChnNum + j + OD_START_PORT;
                ...
                stVdfAttr.unAttr.stOdAttr.stOdStaticParamsIn.roi_od.pnt[0].y =
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stOdArea[j].s32Y;//0;
                stVdfAttr.unAttr.stOdAttr.stOdStaticParamsIn.roi_od.pnt[1].x =
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stOdArea[j].s32X +
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stOdArea[j].u16PicW;//g_width - 1;
                ...
                stVdfAttr.unAttr.stOdAttr.stOdStaticParamsIn.roi_od.pnt[2].y =
                    ...
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stOdArea[j].u16PicH;//g_height - 1;
                stVdfAttr.unAttr.stOdAttr.stOdStaticParamsIn.roi_od.pnt[3].x =
                    pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.stOdArea[j].s32X;//0;
                ...
                for (ii = 0; ii < 3; ii++)
        		{
        			for (jj = 0; jj < 3; jj++)
        			{
        			    if (MI_SUCCESS != (s32Ret = MI_VDF_EnableSubWindow(vdfChn, ii, jj, 1)))
        				{
                            ST_ERR("MI_VDF_EnableSubWindow err, chn %d, %d,%d %x\n",
                                vdfChn, ii, jj, s32Ret);
                            return 1;
                        }
        			}
    ...
    *************************************************/
    for (i = 0; i < s32CapChnNum; i++)
    {
        ...
        stBindInfo.u32SrcFrmrate = 30;
        stBindInfo.u32DstFrmrate = 13;
        STCHECKRESULT(ST_Sys_Bind(&stBindInfo)); //For venc stream
        ...
        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VIF_SUB_CHN)
        {
            ...
            stBindInfo.u32SrcFrmrate = 30;
            stBindInfo.u32DstFrmrate = 30;
            STCHECKRESULT(ST_Sys_Bind(&stBindInfo)); //For disp stream
...
#if 1 //def SUPPORT_VIDEO_ENCODE //Create Video encode Channel
    // main+sub venc
    ...
    {
        VencChn = i;
        MI_U32 u32Venc_W, u32Venc_H;
        MI_S32 s32FrameRate = 13;

        if (MI_SUCCESS == ST_CreateVencChannel(VencChn, pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.eType,
            ...
            s32FrameRate))
        {
            ST_DBG("Create Main Venc Chn(%d) success.\n", VencChn);
        }
        else
        {
            ST_ERR("Create Main Venc Chn(%d) fail.\n", VencChn);
            return -1;
        }
        VencChn = i + VENC_SUB_CHN_START;
        if (MI_SUCCESS == ST_CreateVencChannel(VencChn, pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.eSubType,
            SUB_VENC_W,
            SUB_VENC_H,
            s32FrameRate))
        {
            ST_DBG("Create Sub Venc Chn(%d) success.\n", VencChn);
        }
        else
        {
            ST_ERR("Create Sub Venc Chn(%d) fail.\n", VencChn);
            return -1;
        }
    }
    ...
    if (pstCaseDesc[s32CaseIndex].stDesc.bNeedVdisp)
    {
        MI_S32 s32FrameRate = 13;
        VencChn = u32VencNum;
        if (MI_SUCCESS == ST_CreateVencChannel(VencChn, pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVencChnArg.eType,
            ...
            s32FrameRate))
        {
            ST_DBG("Create Vdisp Venc Chn(%d) success.\n", VencChn);
        }
        else
        {
            ST_ERR("Create Vdisp Venc Chn(%d) fail.\n", VencChn);
            return -1;
        }
    }
#endif
    ...
    for (i = 0; i < s32DispChnNum; i ++)
    {
        ...
        s32Ret = MI_RGN_AttachToChn(hHandle, &stAttachChnPort, &stChnPortParam);
        if (MI_RGN_OK != s32Ret)
        {
    ...
    *************************************************/
    for (i = 0; i < s32DispChnNum; i++)
    {
        if (SUPPORT_DIVP_DI && (288 == pstCaseDesc[s32CaseIndex].stCaseArgs[i].uChnArg.stVifChnArg.u16CapHeight))
        {
            ...
            stBindInfo.u32SrcFrmrate = 30;
            stBindInfo.u32DstFrmrate = 30;
            STCHECKRESULT(ST_Sys_Bind(&stBindInfo)); //SD video Deinterlace
        }
    }
    ...
    *************************************************/
    if (bVdecFlag == 0)
    {
        for (i = 0; i < s32DispChnNum; i++)
        {
            ...
            stBindInfo.u32SrcFrmrate = 30;
            stBindInfo.u32DstFrmrate = 30;
            STCHECKRESULT(ST_Sys_Bind(&stBindInfo)); //DVR Preview
        }
    }
    ...
    for (i = 0; i < s32DispChnNum; i++)
    {
        if (pstCaseDesc[s32CaseIndex].stCaseArgs[i].eVideoChnType & E_ST_VDEC_CHN)
        {
            ...
            stBindInfo.stDstChnPort.eModId = E_MI_MODULE_ID_DIVP;
            stBindInfo.stDstChnPort.u32DevId = 0;
            stBindInfo.stDstChnPort.u32ChnId = i + DIVP_VDEC_CHN_OFFSET;
            stBindInfo.stDstChnPort.u32PortId = 0;
            ...
            stBindInfo.stSrcChnPort.eModId = E_MI_MODULE_ID_DIVP;
            stBindInfo.stSrcChnPort.u32DevId = 0;
            stBindInfo.stSrcChnPort.u32ChnId = i + DIVP_VDEC_CHN_OFFSET;
            stBindInfo.stSrcChnPort.u32PortId = 0;
