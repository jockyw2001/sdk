...
int main(int argc, const char *argv[])
{
    ...
    MI_U16 u16Column = 0;
    MI_U16 u16Avg = 0;
    MI_U16 u16ChangeNum = u64Times / 2;
            ...
            for(u32PortId = 0; u32PortId < u32PortNum; u32PortId ++)
            {
                ...
                if(MI_SUCCESS == MI_SYS_ChnInputPortGetBuf(&stDispChnInputPort[u32PortId],&stBufConf,&stBufInfo,&hHandle, -1))
                {
                    ...
                    if (_mi_disp_GetFrame(srcFd, stBufInfo.stFrameData.pVirAddr[0], framesize) == 1)
                    {
                        ...
                        u64Times--;
                        if(u64Times == u16ChangeNum)
                        {
                            MI_U8 u8Layer = 0;
                            MI_U8 u8Port = 0;
                            MI_U32 u32X = 0;
                            MI_U32 u32Y = 0;
                            char* pchFilePath = "/data/128x128_8.yuv422";
                            MI_DISP_InputPortAttr_t stInputPortAttr2;
                            memset(&stInputPortAttr2, 0, sizeof(MI_DISP_InputPortAttr_t));
                            usleep(10*1000*1000);

                            u16Width = 128;
                            u16Height = 128;
                            ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
                            MI_DISP_DisableInputPort(u8Layer, u8Port);
                            MI_DISP_GetInputPortAttr(u8Layer, u8Port, &stInputPortAttr);
                            stInputPortAttr2.stDispWin.u16X = 64;
                            stInputPortAttr2.stDispWin.u16Y = 64;
                            stInputPortAttr2.stDispWin.u16Width = u16Width;
                            stInputPortAttr2.stDispWin.u16Height = u16Height;
                            MI_DISP_SetInputPortAttr(u8Layer, u8Port, &stInputPortAttr2);
                            MI_DISP_EnableInputPort(u8Layer, u8Port);

                            test_disp_CloseFd(srcFd);
                            if(TRUE != test_disp_OpenSourceFile(pchFilePath, &srcFd))
                            {
                                DBG_INFO("open file failed. pchFilePath = %s, srcFd = %d.\n", pchFilePath,srcFd);
                            }
                            usleep(1000*1000);
                        }
