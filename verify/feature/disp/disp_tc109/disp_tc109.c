...
{
    printf("\n _mi_disp_GetFrame : srcFd = %d, pData = %p, yuvSize = %d \n", srcFd, pData, yuvSize);
...
//process multi ports at one time
int main(int argc, const char *argv[])
{
    ...
    char* filePath2 = "/data/128x192_20.yuv422";
    int srcFd2 = 0;
    MI_U32 u32FrameNum = u64Times / 2;
    MI_BOOL bRet = false;
    MI_U32 u32Index = 0;
    ...
    if(argc < 6)
    {
        printf("Test Pattern: [disp filePath width heigt FrameCount]\n");
        return 0;
    }
...
CHNANGE_VIDEO_INPUT_PORT_ATTR:
    DBG_INFO(" =====>@@@@@ u32FrameNum = %u, u64Times = %llu\n",u32FrameNum,u64Times);
    ...
    if (bReset || (TRUE == test_disp_OpenSourceFile(filePath, &srcFd)))
    {
        while(1)
        {
            ...
            for(u32PortId = 0; u32PortId < u32PortNum; u32PortId ++)
            {
                ...
                if(bReset && (0 == u32PortId))
                {
                    stBufConf.stFrameCfg.u16Width = 128;
                    stBufConf.stFrameCfg.u16Height = 192;
                }
                else
                {
                    stBufConf.stFrameCfg.u16Width = u16Width;
                ...
                if(MI_SUCCESS == MI_SYS_ChnInputPortGetBuf(&stDispChnInputPort[u32PortId],&stBufConf,&stBufInfo,&hHandle, -1))
                {
                    ...
                    framesize = stBufInfo.stFrameData.u16Height * (stBufInfo.stFrameData.u32Stride[0] + stBufInfo.stFrameData.u32Stride[1] +stBufInfo.stFrameData.u32Stride[2]);
                    ...
                    if(bReset && (0 == u32PortId))
                    {
                        bRet = _mi_disp_GetFrame(srcFd2, stBufInfo.stFrameData.pVirAddr[0], framesize);
                    }
                    else
                    {
                        bRet = _mi_disp_GetFrame(srcFd, stBufInfo.stFrameData.pVirAddr[0], framesize);
                    }
                    ...
                    if (bRet)
                    {
                        ...
                        u64Times--;
                        if(u32FrameNum == u64Times)
                        {
                            DBG_INFO("=====>@@@@@ u32FrameNum = %u, u64Times = %llu\n",u32FrameNum,u64Times);
                            if(TRUE == test_disp_OpenSourceFile(filePath2, &srcFd2))
                            {
                                bReset = true;
                            }
                            ...
                            stInputPortAttr[0].stDispWin.u16Width = 128;
                            stInputPortAttr[0].stDispWin.u16Height = 192;
                            stInputPortAttr[0].stDispWin.u16X = 0;
                            stInputPortAttr[0].stDispWin.u16Y = 0;

                            stInputPortAttr[1].stDispWin.u16Width = u16Width;
                            stInputPortAttr[1].stDispWin.u16Height = u16Height;
                            stInputPortAttr[1].stDispWin.u16X = u16Width * 2;
                            stInputPortAttr[1].stDispWin.u16Y = 0;

                            stInputPortAttr[2].stDispWin.u16Width = u16Width;
                            stInputPortAttr[2].stDispWin.u16Height = u16Height;
                            stInputPortAttr[2].stDispWin.u16X = u16Width * 2;
                            stInputPortAttr[2].stDispWin.u16Y = u16Height;

                            stInputPortAttr[3].stDispWin.u16Width = u16Width;
                            stInputPortAttr[3].stDispWin.u16Height = u16Height;
                            stInputPortAttr[3].stDispWin.u16X = u16Width * 2;
                            stInputPortAttr[3].stDispWin.u16Y = 2 * u16Height;

                            goto CHNANGE_VIDEO_INPUT_PORT_ATTR;
                        }
...
EXIT_TEST:
    ...
    if(srcFd > 0)
    {
        ...
        test_disp_CloseFd(srcFd2);
        DBG_INFO(" [%s %d]  ==========>  prepare to exit test case 109 ==========> \n", __FUNCTION__, __LINE__);
