...
static void* _mi_disp_HandlerThread(void *argv)
{
    ...
    while(!_bHandlerThreadExit)
    {
        MI_U8 u8Layer = 0;
        MI_U8 u8Port = 0;
        MI_U8 u8PauseStatus = 0;
        MI_BOOL bPause = FALSE;
        printf("Input Specific Layer:\n");
        scanf("%d", &u8Layer);
        printf("Input Specific Port:\n");
        scanf("%d", &u8Port);
        printf("Input Pause status:\n");
        scanf("%d", &u8PauseStatus);
        bPause = (u8PauseStatus == 0)? FALSE: TRUE;
        if(bPause)
        {
            MI_DISP_PauseInputPort(u8Layer, u8Port);
        }
        else
        {
            MI_DISP_ResumeInputPort(u8Layer, u8Port);
        }
        getchar();
    }
    pthread_exit(NULL);
