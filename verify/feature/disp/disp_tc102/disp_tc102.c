...
#define DBG_INFO(fmt, args...)      ({do{printf("\n [MI INFO]:%s[%d]: ", __FUNCTION__,__LINE__);printf(fmt, ##args);}while(0);})
...
int main(int argc, const char *argv[])
{
    ...
    if(argc < 6)
    {
        DBG_INFO("Test Pattern: [disp filePath format width heigt FrameCount portnum]\n");
        return 0;
    }
    ...
    for(u32PortId = 0; u32PortId < u32PortNum; u32PortId ++)
    {
        ...
        stInputPortAttr[u32PortId].stDispWin.u16X = (u32PortId%u16Avg)* u16Width;
        stInputPortAttr[u32PortId].stDispWin.u16Y = (u32PortId/u16Avg) * u16Height;

        printf("Port Num %d\n",u32PortNum);
        printf("Layer %d, PortId %d, X %d, Y %d, width %d, Height %d\n",DispLayer,u32PortId,stInputPortAttr[u32PortId].stDispWin.u16X,stInputPortAttr[u32PortId].stDispWin.u16Y,stInputPortAttr[u32PortId].stDispWin.u16Width,stInputPortAttr[u32PortId].stDispWin.u16Height);
