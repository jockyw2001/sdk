...
typedef enum {
    // Main menu
    E_DISP_TEST_OVERLAP_WIN0 = 0,
    E_DISP_TEST_OVERLAP_WIN1,
    E_DISP_TEST_OVERLAP_GET_WIN,
    E_DISP_TEST_OVERLAP_SET_ZOOM,

    E_DISP_TEST_OVERLAP_MAX,
}E_DISP_TEST_OVERLAP;

static void *test_disp_OverLapSetZoom(void *pData);
static void * test_disp_OverLapSetWin0(void *pData);
static void * test_disp_OverLapSetWin1(void *pData);
static void * test_disp_OverLapVpeInCreate(void *pData);
static void * test_disp_OverLapVpeInStop(void *pData);
static void * test_disp_OverLapGetWin(void *pData);

static test_disp_TestZoomMenu_t _gstDispTestZoomMenu[] = {
    // Main menu
    [E_DISP_TEST_OVERLAP_WIN0] = {"Disp overlap test: set Layer0 win0 Pos", test_disp_OverLapSetWin0, E_DISP_TEST_OVERLAP_WIN0, FALSE},
    [E_DISP_TEST_OVERLAP_WIN1] = {"Disp overlap test: set Layer1 win0 Pos", test_disp_OverLapSetWin1, E_DISP_TEST_OVERLAP_WIN0, FALSE},
    [E_DISP_TEST_OVERLAP_GET_WIN] = {"Disp overlap test: Get win Pos", test_disp_OverLapGetWin, E_DISP_TEST_OVERLAP_WIN0, FALSE},
    [E_DISP_TEST_OVERLAP_SET_ZOOM] = {"Disp overlap test: set zoom", test_disp_OverLapSetZoom, E_DISP_TEST_OVERLAP_WIN0, TRUE},
...
static void * test_disp_OverLapSetZoom(void *pData)
{
    ...
    printf("input: layer x y width height.\n");
    scanf("%d %d %d %d %d.\n", &DispLayer, &x, &y, &width, &height);
    memset(&stCropWin, 0, sizeof(MI_DISP_VidWinRect_t));
    stCropWin.u16X = x;
    stCropWin.u16Y = y;
    stCropWin.u16Width = width;
    stCropWin.u16Height = height;
    MI_DISP_SetZoomInWindow(DispLayer, 0, &stCropWin);
...
static void * test_disp_OverLapSetWin(MI_DISP_LAYER DispLayer, MI_DISP_VidWinRect_t *pstDispWin, void *pData)
{
    test_disp_OverLapPara_t *pstPara = (test_disp_OverLapPara_t *)pData;
    MI_DISP_InputPortAttr_t stInputPortAttr;
    memset(&stInputPortAttr, 0, sizeof(stInputPortAttr));
    stInputPortAttr.stDispWin.u16X = pstDispWin->u16X;
    stInputPortAttr.stDispWin.u16Y = pstDispWin->u16Y;
    stInputPortAttr.stDispWin.u16Width = pstDispWin->u16Width;
    stInputPortAttr.stDispWin.u16Height = pstDispWin->u16Height;
...
static void * test_disp_OverLapSetWin0(void *pData)
{
    ...
    int x, y, width, height;
    printf("Layer 0 input port: x y width height.\n");
    scanf("%d %d %d %d.\n", &x, &y, &width, &height);

    stDispWin.u16X = x;
    stDispWin.u16Y = y;
    stDispWin.u16Width = width;
    stDispWin.u16Height = height;

    test_disp_OverLapSetWin(0, &stDispWin, pData);
...
static void * test_disp_OverLapSetWin1(void *pData)
{
    MI_DISP_VidWinRect_t stDispWin;
    int x, y, width, height;
    printf("Layer 1 input port: x y width height.\n");
    scanf("%d %d %d %d.\n", &x, &y, &width, &height);
    ...
    test_disp_OverLapSetWin(1, &stDispWin, pData);
...
static void * test_disp_OverLapGetWin(void *pData)
{
    ...
    memset(&stInputPortAttr, 0, sizeof(stInputPortAttr));
    MI_DISP_GetInputPortAttr(DispLayer, LayerInputPort, &stInputPortAttr);
    printf("\nLayer%d Win%d: {%d, %d, %d, %d}\n", DispLayer, LayerInputPort,
...
int main(int argc, const char *argv[])
{
    ...
    stInputPortAttr.stDispWin.u16Width = u16Width/2;
    stInputPortAttr.stDispWin.u16Height= u16Height/2;
    ...
    stInputPortAttr.stDispWin.u16X    = 100;
    stInputPortAttr.stDispWin.u16Y    = 50;
    ...
    _gstDispOverLap.stDispWin.u16Width= u16Width/2;
    _gstDispOverLap.stDispWin.u16Height= u16Height/2;
    ...
    ExecFunc(MI_DISP_DisableInputPort(1, LayerInputPort), MI_DISP_SUCCESS);
