...
int main(int argc, const char *argv[])
{
    ...
    if(argc < 5)
    {
        printf("%s:\n\t%s <filePath> <width> <heigt> <FrameCount>\n", DISP_TEST_001_DESC, argv[0]);
        return 0;
    }
    ...
    stPubAttr.u32BgColor = 0xFF6A6A;//0XFF Blue
    ...
    if (TRUE == test_disp_OpenSourceFile(filePath, &srcFd))
    {
        while (1)
        {
            ...
            if(MI_SUCCESS == MI_SYS_ChnInputPortGetBuf(&stDispChn0InputPort0,&stBufConf,&stBufInfo,&hHandle, -1))
            {
                ...
                DISP_TEST_INFO(" [%s %d] size = %d  \n", __FUNCTION__, __LINE__, size);

                //test_disp_PutOneFrame(srcFd, 0, stBufInfo.stFrameData.pVirAddr[0], stBufInfo.stFrameData.u32Stride[0], u16Width*2, u16Height);
    ...
    DISP_TEST_INFO("PrePare to Exit.\n");
    if(srcFd > 0)
    {
