...
static void adda_config(int dev, int max_chs, int edge, int pclk)
{
    ...
    /*Í¨µÀÓ³ÉäÅäÖÃ£¬Âß¼­Í¨µÀ0 ~3 ¶ÔÓ¦µÚÒ»Æ¬ÎïÀíÍ¨µÀ0,1,2,3*/
    ...
    AddaInitRes.AdRes[0].enEqMode = EQ_MODE_IN;/*Ê¹ÓÃÄÚ²¿EQ*/


    /*ÅäÖÃÃ¿¸öVO¿ÚµÄÊ±ÐòÄ£Ê½*/
    //AddaInitRes.AdRes[0].stVdPortDev.ucCount=1;/*4¸öVO¿Ú¾ùÆôÓÃ*/
    ...
    //AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enVdPortMuxMode = VD_PORT_MUX_MODE_2MUX;/*2Â·¸´ÓÃÄ£Ê½*/
    //AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enOutHeadMode = VIDEO_HEAD_SINGEL;/*µ¥Í·Ä£Ê½*/
    //AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enVdPortOutFormat = VD_PORT_OUT_FORMAT_BT656;/*8bit bt656Ä£Ê½*/
    ...
    AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enVdPortMuxMode = tmux_mode;/*2Â·¸´ÓÃÄ£Ê½*/
    AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enOutHeadMode = VIDEO_HEAD_SINGEL;/*µ¥Í·Ä£Ê½*/
    AddaInitRes.AdRes[0].stVdPortDev.stVdPort[0].enVdPortOutFormat = VD_PORT_OUT_FORMAT_BT656;/*8bit bt656Ä£Ê½*/
    AddaInitRes.AdRes[0].enDriverPower = DRIVER_POWER_18V;/*BT656µÄÂß¼­µçÆ½*/
...
int vif_dh9931_Init(int device , int channel, int pclk)
{
    ...
    system("sleep 6");
    DBG_INFO("ahd %d init success,chn:%d pclkï¼š%d!\n", device, channel, pclk);
    return 0;
}
#endif
