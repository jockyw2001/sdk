...
static test_vpe_Config stTest007 = {
    .inputFile  = TEST_VPE_CHNN_FILE420(007, 0, 640x360),
...
retry:
    ...
    if (MI_VPE_OK == MI_VPE_GetChannelRegionLuma(0, &stRegionInfo, &au32LumaData[0], 100000))
    {
        for (i = 0; i < stRegionInfo.u32RegionNum; i++)
        {
    ...
    {
        MI_VPE_TEST_INFO("Fail to get  RegionLuma.\n");
        goto retry;
    }
    if (cnt++ > 5)
    {
...
int main(int argc, const char *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        printf("Channel: %d.\n", 0);
        printf("InputFile: %s.\n", stTest007.inputFile);
        printf("OutputFile: %s.\n", stTest007.stOutPort[0].outputFile);
        return 0;
    }
    ...
    sprintf(dest_file, "%s/%s", pbaseDir, stTest007.stOutPort[0].outputFile);
    ExecFunc(test_vpe_OpenDestFile(dest_file, &stTest007.stOutPort[0].dest_fd), TRUE);
    stTest007.count = 0;
    ...
    VpeChannel = 0;
    VpePort = 0;
    test_vpe_CreatChannel(VpeChannel, VpePort, &stTest007.stCropWin, &stTest007.stOutPort[0].stPortWin,E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420,E_MI_SYS_PIXEL_FRAME_YUV422_YUYV);
    ...
    do {
        ...
        if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0,&stBufConf,&stBufInfo,&hHandle , 0))
        {
            ...
            if (1 == test_vpe_GetOneFrameYUV420(stTest007.src_fd, stBufInfo.stFrameData.pVirAddr[0], stBufInfo.stFrameData.pVirAddr[1], y_size, uv_size))
            {
                stTest007.src_offset += y_size + uv_size;
        ...
        if (MI_SUCCESS == MI_SYS_ChnOutputPortGetBuf(&stVpeChnOutputPort0 , &stBufInfo,&hHandle))
        {
            ...
            test_vpe_PutOneFrame(stTest007.stOutPort[0].dest_fd, stTest007.stOutPort[0].dest_offset, stBufInfo.stFrameData.pVirAddr[0], stBufInfo.stFrameData.u32Stride[0], width*YUV422_PIXEL_PER_BYTE, height);
            stTest007.stOutPort[0].dest_offset += frame_size;
    ...
    test_vpe_CloseFd(stTest007.stOutPort[0].dest_fd);
    sync();
