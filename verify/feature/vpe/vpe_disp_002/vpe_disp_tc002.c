...
static test_vpe_Config stTest002[] = {
    ...
    {
        .inputFile  = TEST_VPE_CHNN_FILE(002, 3, 640x360),
        .stSrcWin   = {0, 0, 640, 360},
        .stCropWin   = {0, 0, 640, 360},
        .stOutPort  = {
            [VPE_OUTPORT] = {
    ...
    for (i = 0; i < u32ChannelNum; i++)
    {
        for (j = 0; j < 4; j++)
        {
            if (stTest002[i].stOutPort[j].bEnable == TRUE)
            {
                ...
                test_vpe_CreatChannel(i, j, &stTest002[i].stCropWin, &stTest002[i].stOutPort[j].stPortWin,E_MI_SYS_PIXEL_FRAME_YUV422_YUYV,ePixel);
                ...
                if (stTest002[i].stOutPort[j].bEnable == TRUE)
                {
                    if (1)//(stTest002[i].product == 0)
                    {
                        ...
                        if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0,&stBufConf,&stBufInfo,&hHandle, 0))
                        {
                            ...
                            //if (1 == test_vpe_GetOneFrame(stTest002[i].src_fd, stTest002[i].src_offset, stBufInfo.stFrameData.pVirAddr[0], frame_size))
                            if (1 == test_vpe_GetOneFrameYUV422ByStride(stTest002[i].src_fd, stBufInfo.stFrameData.pVirAddr[0], height, width, stBufInfo.stFrameData.u32Stride[0]))
                            {
