...
int bind(MI_U8 SrcChnl,MI_U8 SrcPort,MI_U8 u8DevId, MI_U8 DstChn,MI_U8 DstPort)
{
...
int main(int argc, const char *argv[])
{
    ...
    MI_SYS_WindowRect_t stUser_crop;
    MI_U8 DispDev=0;
    MI_U8 DispLayer=0;
    ...
    if (argc < 10)
    {
        printf("%s <test_dir> <count> <ChannelNum> <CROP X Y W H> <DispDev> <DispLayer>.\n", argv[0]);
        printf("%s.\n", VPE_TEST_002_DESC);
    ...
    stUser_crop.u16X = atoi(argv[4]);
    stUser_crop.u16Y = atoi(argv[5]);
    stUser_crop.u16Width = atoi(argv[6]);
    stUser_crop.u16Height = atoi(argv[7]);
    DispDev = atoi(argv[8]);
    DispLayer = atoi(argv[9]);
    ...
    u16VpeOutportWidth = layerwidth / u8Canvs;
    u16VpeOutportHeight = layerHeight / u8Canvs;
    ...
    for (i = 0; i < u32ChannelNum; i++)
    {
        for (j = 0; j < 4; j++)
        {
            if (stTest002[i].stOutPort[j].bEnable == TRUE)
            {
                ...
                stTest002[i].stCropWin.u16X = stUser_crop.u16X;
                stTest002[i].stCropWin.u16Y = stUser_crop.u16Y;
                stTest002[i].stCropWin.u16Width = stUser_crop.u16Width;
                stTest002[i].stCropWin.u16Height = stUser_crop.u16Height;
       ...
       ExecFunc(MI_DISP_EnableVideoLayer(DispLayer), MI_DISP_SUCCESS);

        {
            ...
            for(i = 0; i<u32ChannelNum;i++)
                bind(i,VPE_OUTPORT,DispDev,0,i);
        }
