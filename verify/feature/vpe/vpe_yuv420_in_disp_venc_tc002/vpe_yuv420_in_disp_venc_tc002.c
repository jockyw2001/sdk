...
static test_vpe_Config stTest002[] = {
    ...
    {
        .inputFile = TEST_VPE_CHNN_FILE420(002, 1, 1920x1080),
            .stSrcWin   = {0, 0, 1920, 1080},
            .stCropWin   = {0, 0, 1920, 1080},
        .stOutPort  = {
             [VPE_Port_VENC] = {
    ...
    {
        .inputFile = TEST_VPE_CHNN_FILE420(002, 2, 1920x1080),
        .stSrcWin   = {0, 0, 1920, 1080},
        .stCropWin  = {0, 0, 1920, 1080},
        .stOutPort  = {
            [VPE_Port_VENC] = {
    ...
    {
        .inputFile = TEST_VPE_CHNN_FILE420(002, 3, 1920x1080),
        .stSrcWin   = {0, 0, 1920, 1080},
        .stCropWin  = {0, 0, 1920, 1080},
        .stOutPort  = {
            [VPE_Port_VENC] = {
...
int vpe_creat_channel(MI_U8 VpeChannel, MI_SYS_PixelFormat_e eInPixel, MI_SYS_WindowRect_t *pstCropWin)
{
    ...
    stChannelVpssAttr.eRunningMode = E_MI_VPE_RUNNING_MODE_FRAMEBUF_DVR_MODE;
...
void *GetESBuffer(void *args)
{
    ...
    while(1)
    {
        if(MI_SUCCESS == MI_SYS_ChnOutputPortGetBuf(&stVencChnInputPort, &stBufInfo, &hHandle))
        {
            ...
            write(stTest002[u8ChnId].stOutPort[VPE_Port_VENC].dest_fd, stBufInfo.stRawData.pVirAddr, stBufInfo.stRawData.u32ContentSize);
...
void *SendYUVBuffer(void *args)
{
    ...
    while(1)
    {
        ...
        if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0,&stBufConf,&stBufInfo,&hHandle, 0))
        {
            ...
            if (1 == test_vpe_GetOneFrameYUV420(stTest002[u8ChnId].src_fd, stBufInfo.stFrameData.pVirAddr[0],stBufInfo.stFrameData.pVirAddr[1], y_size, uv_size))
            {
                stTest002[u8ChnId].src_offset += y_size + uv_size;

                MI_VPE_TEST_DBG("%s()@line%d: Start put input buffer.\n", __func__, __LINE__);
                MI_SYS_ChnInputPortPutBuf(hHandle,&stBufInfo, FALSE);
                stTest002[u8ChnId].count++;
            }
            else
            {
                MI_SYS_ChnInputPortPutBuf(hHandle,&stBufInfo, TRUE);
                stTest002[u8ChnId].src_offset = 0;
                stTest002[u8ChnId].src_count = 0;
                test_vpe_FdRewind(stTest002[u8ChnId].src_fd);
...
//        for (i = 0; i < sizeof(stTest002)/sizeof(stTest002[0]); i++)
    ...
    for (i = 0; i < u32ChannelNum; i++)
    {
        ...
        MI_U8 u8VpeChn = i;
        pthread_create(&ptSendYUV[i], NULL, SendYUVBuffer, (void *)&u8VpeChn);
    }
       ...
       for (i = 0; i < 1; i++)
       {
            ...
            stChnAttr.stRcAttr.stAttrH265FixQp.u32SrcFrmRateNum = 25;
