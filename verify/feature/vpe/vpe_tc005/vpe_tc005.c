...
int main(int argc, const char *argv[])
{
    ...
    {
        sprintf(dest_file, "%s/%s", pbaseDir, stTest005.stOutPort[i].outputFile);
        ExecFunc(test_vpe_OpenDestFile(dest_file, &stTest005.stOutPort[i].dest_fd), TRUE);
        stTest005.stOutPort[i].dest_offset = 0;
    }
    stTest005.count = 0;
    ...
    do {
        ...
        if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0,&stBufConf,&stBufInfo,&hHandle , 0))
        {
            ...
            if (1 == test_vpe_GetOneFrame(stTest005.src_fd, stTest005.src_offset, stBufInfo.stFrameData.pVirAddr[0], frame_size))
            {
                ...
                MI_VPE_TEST_INFO("dispWin = {width: %u, height: %u}.\n", stVpeMode.u16Width, stVpeMode.u16Height);
    ...
    test_vpe_CloseFd(stTest005.stOutPort[0].dest_fd);
    sync();
