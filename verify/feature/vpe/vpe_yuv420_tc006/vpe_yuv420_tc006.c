...
static test_vpe_Config stTest006 = {
    .inputFile  = TEST_VPE_CHNN_FILE420(006, 0, 640x360),
...
int main(int argc, const char *argv[])
{
    ...
    VpeChannel = 0;
    VpePort = 0;
    test_vpe_CreatChannel(VpeChannel, VpePort, &stTest006.stCropWin, &stTest006.stOutPort[0].stPortWin,E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420,E_MI_SYS_PIXEL_FRAME_YUV422_YUYV);
    ...
    do {
        ...
        if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0,&stBufConf,&stBufInfo,&hHandle , 0))
        {
            ...
            if (1 == test_vpe_GetOneFrameYUV420(stTest006.src_fd, stBufInfo.stFrameData.pVirAddr[0], stBufInfo.stFrameData.pVirAddr[1], y_size, uv_size))
            {
                stTest006.src_offset += y_size + uv_size;
