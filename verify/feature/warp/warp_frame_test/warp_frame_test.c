...
void ST_WARP_FinishInjectBufYuv422(void *pHandle, ST_WARP_FrameBuf_t *pFrameBuf, MI_BOOL bValidData)
{
    ...
    MI_SYS_FlushInvCache(pFrameBuf->stPlane[0].pAddr, (pFrameBuf->stPlane[0].u32Stride * pFrameBuf->stPlane[0].u32Height));
...
void ST_WARP_FinishInjectBufYuv420(void *pHandle, ST_WARP_FrameBuf_t *pFrameBuf, MI_BOOL bValidData)
{
    ...
    MI_SYS_FlushInvCache(pFrameBuf->stPlane[0].pAddr, (pFrameBuf->stPlane[0].u32Stride * pFrameBuf->stPlane[0].u32Height));
    MI_SYS_FlushInvCache(pFrameBuf->stPlane[1].pAddr, (pFrameBuf->stPlane[1].u32Stride * pFrameBuf->stPlane[1].u32Height));
    MI_SYS_ChnInputPortPutBuf(pPriv->bufHandle, &pPriv->stBufInfo, !bValidData);
    free(pPriv);
}
...
int main(int argc, const char *argv[])
{
    ...
    MI_SYS_BufInfo_t stBufInfo, stOutBufInfo;
    MI_SYS_BufConf_t stBufConf, stOutBufConf;
    MI_SYS_BUF_HANDLE hHandle, hOutHandle;
    time_t stTime = 0;

    char szSrcFilePath[256];
    char szDstFilePath[256];
    FILE *pSrcFile = NULL;
    FILE *pDstFile = NULL;
    MI_S32 s32Size = 0;
    void *pInFrame = NULL;
    memset(szSrcFilePath, 0, sizeof(szSrcFilePath));
    memset(szDstFilePath, 0, sizeof(szDstFilePath));
    ...
    sprintf(szSrcFilePath, "input/%d.bin", atoi(argv[1]));
    sprintf(szDstFilePath, "output/%d.bin", atoi(argv[1]));
    ...
    if (MI_WARP_OK != MI_WARP_SetChnBin(0, 0, MI_WARP_BOUND_BOX_TABLE, _gszBbTablePath))
    {
        DBG_ERR("MI_WARP_SetChnBin bb table failed\n");
        goto stop_dev;
    }

    if (MI_WARP_OK != MI_WARP_SetChnBin(0, 0, MI_WARP_DISP_ABSOLUTE_TABLE, _gszDisplaceTablePath))
    {
        DBG_ERR("MI_WARP_SetChnBin disp table failed\n");
        goto stop_dev;
    }
    ...
    s32Ret = MI_SYS_SetChnOutputPortDepth(&stChnPort, 5, 20);
    ...
    // read srcFile data to inFrame
    pSrcFile = fopen(szSrcFilePath, "r");
    if (!pSrcFile)
    {
        printf("open srcFile %s failed, err:%d\n", szSrcFilePath, errno);
        goto destroy_chn;
    }
    ...
    // dump read
    pDstFile = fopen("dumpRead", "a+");
    if (!pDstFile)
    {
        printf("open dump Filefailed\n");
    }

    fwrite(pInFrame, 1, s32Size, pDstFile);
    fclose(pDstFile);
    pDstFile = NULL;
    // dump end
    ...
    while (1)
    {
        if(MI_SUCCESS == MI_SYS_ChnOutputPortGetBuf(&stChnPort, &stOutBufInfo, &hOutHandle))
        {

            // save outFrame to dstFile
            pDstFile = fopen(szDstFilePath, "a+");
            if (!pDstFile)
            {
            ...
            fwrite(stOutBufInfo.stFrameData.pVirAddr[0], 1, stOutBufInfo.stFrameData.u32Stride[0]*stOutBufInfo.stFrameData.u16Height, pDstFile);
            fwrite(stOutBufInfo.stFrameData.pVirAddr[1], 1, stOutBufInfo.stFrameData.u32Stride[1]*stOutBufInfo.stFrameData.u16Height/2, pDstFile);
            fclose(pDstFile);
            pDstFile = NULL;

put_ouputBuf:
            MI_SYS_ChnOutputPortPutBuf(hOutHandle);
            break;
        }
        usleep(10000);
    }
