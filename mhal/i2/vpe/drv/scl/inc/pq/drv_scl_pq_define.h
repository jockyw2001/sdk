...
extern s32 _Scl_PQ_Mutex ;

#if defined (SCLOS_TYPE_LINUX)
extern pthread_mutex_t _Scl_PQ_MLoad_Mutex;
#endif

#if PQ_ENABLE_DEBUG
extern bool _u16SclDbgSwitch;
#define PQ_DBG(x)  \
do{                \
    if (_u16SclDbgSwitch){ \
        x;         \
...
typedef enum {
    ...
    PQ_TABTYPE_ColorEng_WDR_alpha_c_SRAM,
    PQ_TABTYPE_ColorEng_WDR_f_c_SRAM,
    PQ_TABTYPE_ColorEng_WDR_linear_c_SRAM,
    PQ_TABTYPE_ColorEng_WDR_nonlinear_c_SRAM,
    PQ_TABTYPE_ColorEng_WDR_sat_c_SRAM,
...
#define SCALER_REGISTER_SPREAD 1
    ...
    #define BK_SCALER_BASE              0x150000

#if defined(SCLOS_TYPE_LINUX_KERNEL_I2)
    #define BK_NE_BASE                  0x310000
#else
    #define BK_NE_BASE                  0x160000
#endif
    ...
    #define SC_BK_STORE_MUTEX     \
            if(!DrvSclOsObtainMutex(_Scl_PQ_Mutex, SCLOS_WAIT_FOREVER))                     \
            {                                                                        \
                sclprintf("==========================\n");                              \
    ...
    #define SC_BK_RESTORE_MUTEX   \
            DrvSclOsReleaseMutex(_Scl_PQ_Mutex);
    #else
    #define SC_BK_STORE_MUTEX
