...
static u8 _u8ModeIndex[PQ_MAX_WINDOW];
...
static bool _gbMemfmt422[PQ_MAX_WINDOW]={1};
static bool _gIsSrcHDMode[PQ_MAX_WINDOW];
static PQ_FOURCE_COLOR_FMT _gFourceColorFmt[PQ_MAX_WINDOW];
static bool _gIs3D_En = 0;
static bool _gIsH264 = 0;
static bool _gIsMMplayer=0;
#endif


u16 QM_InputSourceToIndex(PQ_WIN eWindow, u8 enInputSourceType)
{
    u16 u16SrcType = 0;
#if (PQ_QM_HDMI_PC) || (PQ_QM_PC)

    u16 u16Input_HSize = QM_GetInputHSize(eWindow);
    u16 u16Input_VSize = QM_GetInputVSize(eWindow);
    u16 u16Input_VFreq = QM_GetInputVFreq(eWindow);
#endif

#if(PQ_ENABLE_UNUSED_FUNC)
    bool bIsInterlaced = QM_IsInterlaced(eWindow);
#else
    bool bIsInterlaced = 0;
#endif
...
#if (PQ_QM_HDMI_PC) || (PQ_QM_PC)
    u16 u16Disp_HSize = QM_GetDispHSize(eWindow);
    u16 u16Disp_VSize = QM_GetDispVSize(eWindow);

    PQMODE_DBG(sclprintf("\r\n QM: Src = %u, H = %u, V = %u", enInputSourceType, u16Input_HSize, u16Input_VSize));
    PQMODE_DBG(sclprintf("\r\n     VFreq = %u, IsInterlace = %u", u16Input_VFreq, (u16)bIsInterlaced));
    ...
    if(QM_HDMIPC_COLORRGB(eWindow))
        ...
        sclprintf("HDMI YUV 444\n");
    else
        sclprintf("HDMI YUV 422\n");
#endif
