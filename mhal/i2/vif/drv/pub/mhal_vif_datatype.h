...
typedef enum
{
    ...
    E_MHAL_VIF_WORK_MODE_RAW_REALTIME,
    E_MHAL_VIF_WORK_MODE_RAW_FRAMEMODE,
...
typedef struct MHal_VIF_ChnStat_s
{
    ...
    u32 nOutputHeight;
    u32 nSubOutputWidth;
    u32 nSubOutputHeight;
    u8  eStatus[8];
} MHal_VIF_ChnStat_t;

typedef enum
{
    E_VIF_FIELDTYPE_NONE,        //< no field.
    E_VIF_FIELDTYPE_TOP,           //< Top field only.
    E_VIF_FIELDTYPE_BOTTOM,    //< Bottom field only.
    E_VIF_FIELDTYPE_BOTH,        //< Both fields.
    E_VIF_FIELDTYPE_NUM
} MHal_VIF_FieldType_e;

typedef enum
{
    E_VIF_BUF_EMPTY     = 0, /**< Vif buffer is ready to write*/
    E_VIF_BUF_FILLING   = 1, /**< DMA occupied */
    E_VIF_BUF_READY     = 2, /**< Image ready */
    E_VIF_BUF_INVALID   = 3, /**< Do not use */
} MHal_VIF_BufStatus_e;

/*! @brief ring buffer element*/
typedef struct MHal_VIF_RingBufElm_s// stride ,   yuv 420   422  ....
{
    u64 u64PhyAddr[2]; /**< physical buffer address, HDR mode, u64PhyAddr[0]=long, u64PhyAddr[1]=short*/
    u64 u64PhyAddr_hdr[2]; /**< physical buffer address for hdr 2nd buffer*/
    u32 u32Stride[2];
    ...
    MHal_VIF_BufStatus_e nStatus;      /**< Vif buffer status*/
    MHalFieldType_e  eFieldType;
    u32 nMiPriv;
...
typedef enum
{
    E_VIF_EVENT_FRAME_START,
    E_VIF_EVENT_FRAME_END,
    E_VIF_EVENT_LINE_COUNT_HIT0,
    E_VIF_EVENT_MAX,
} MHal_VIF_EventType_e;

typedef void (*MHal_VIF_EventCallback_fp)(MHal_VIF_CHN u32VifChn, MHal_VIF_EventType_e eType, MHal_VIF_RingBufElm_t *pstFbInfo, void *pPrivateData);

typedef enum
{
    E_MHAL_VIF_HDR_SOURCE_VC0,
    E_MHAL_VIF_HDR_SOURCE_VC1,
    E_MHAL_VIF_HDR_SOURCE_VC2,
    E_MHAL_VIF_HDR_SOURCE_VC3,
    E_MHAL_VIF_HDR_SOURCE_MAX
} MHal_VIF_HDRSource_e;
...
{
    MHalWindowRect_t        stCapRect;  /* Crop config  */
    MHalFieldType_e         eCapSel;    /* AHD only     */
    MHalFrameScanMode_e     eScanMode;  /* AHD only     */
    MHalPixelFormat_e       ePixFormat;
    MHal_VIF_FrameRate_e    eFrameRate;
    u32 u32FrameModeLineCount;     /* Notify user when DAM line count equal to s32FrameModeLineCount*/
    void  (*OnFrameStart)(MHal_VIF_CHN u32VifChn, MHal_VIF_EventType_e eType, MHal_VIF_RingBufElm_t *pstFbInfo, void *pPrivateData);
    void  (*OnFrameLineCount)(MHal_VIF_CHN u32VifChn, MHal_VIF_EventType_e eType, MHal_VIF_RingBufElm_t *pstFbInfo, void *pPrivateData);
    void  (*OnFrameEnd)(MHal_VIF_CHN u32VifChn, MHal_VIF_EventType_e eType, MHal_VIF_RingBufElm_t *pstFbInfo, void *pPrivateData);
    MHal_VIF_HDRSource_e    eHDRSource;  /* New field for HDR*/
} MHal_VIF_ChnCfg_t;
