...
void Hal_VE_set_color_convert(MS_VE_VIDEOSYS videosys, MS_U8 u8dactype)
{
    ...
    { // NTSC
        ...
        case VE_OUT_CVBS_RGB:
            ...
            //Derek finetune for ScartRGB 480i performance
            MDrv_WriteByte(L_BK_VE_ENC(0x25), 0xF9);
            MDrv_WriteByte(L_BK_VE_ENC(0x26), 0x98);
            MDrv_WriteByte(H_BK_VE_ENC(0x26), 0x06);
            MDrv_WriteByte(L_BK_VE_ENC(0x28), 0xFC);
            MDrv_WriteByte(H_BK_VE_ENC(0x28), 0x34);
            MDrv_WriteByte(L_BK_VE_ENC(0x2F), 0xF9);
            break;
        }
    }
    else
...
static void _Hal_VE_dolby_verifier_task(void)
{
    ...
    while(_enVE_DolbyVerifierStatus == EN_VE_DOLBY_VERIFIER_ONGOING)
    {
        ...
        *(_pu8DolbyVerifierBufVA+9) = 0x01; // Packet index
        ...
        MsOS_Dcache_Flush((MS_VIRT)_pu8DolbyVerifierBufVA, TTX_PACKET_SIZE);
...
void Hal_VE_init(void)
{
    ...
    //U02 support scaler1 DE & VE OSD z-order
    //Enable eco bit : reg_osd_sel_mde
    if(MDrv_SYS_GetChipRev() >= ECO_U02)
    {
        MDrv_WriteByteMask(L_BK_VE_SRC(0x07), BIT(2), BIT(2));
    }
    ...
    if(_phyDolbyVerifierBufAddr != 0)
    {
        ...
        if(1 == _u8VE_MIUSel)
        {
            _pu8DolbyVerifierBufVA = (MS_U8*)MsOS_PA2KSEG0(_phyDolbyVerifierBufAddr + HAL_MIU1_BASE);
        }
        else
        {
            _pu8DolbyVerifierBufVA = (MS_U8*)MsOS_PA2KSEG0(_phyDolbyVerifierBufAddr);
        }
...
void Hal_VE_set_pre_scaling_mux(MS_BOOL ben)
{
    MApi_XC_W2BYTEMSK(_SC_PK_L_(0x62, 0x1C), ben ? 0x00 : 0x31, 0x31);
}
...
void Hal_VE_set_inputsource(PMS_VE_InputSrc_Info pInputSrcInfo)
{
    ...
    switch(pInputSrcInfo->eInputSrcType)
    {
        ...
        case MS_VE_SRC_SCALER: //OP2
            ...
            MDrv_WriteByteMask(L_BK_VE_ENC_EX(0x7C), BIT(1), BIT(1)|BIT(0));
...
{
    MDrv_WriteByteMask(H_BK_VE_SRC(0x5F), u8Val, 0xFF);
    return E_VE_OK;
}
...
{
    *pu8Val = MDrv_ReadByte(H_BK_VE_SRC(0x5F));
    return E_VE_OK;
}
...
static void _Hal_VE_MV_OnOff(InterruptNum eIntNum)
{
...

    {
        if(_bMV_Enable)
        {
            if((_eMvType >= MS_VE_MV_NTSC_START) && (_eMvType <= MS_VE_MV_PAL_TEST_P01_TTX))
            {
             ...
             {
                MDrv_WriteByte(L_BK_COMPONENT(0x00),0x03); //change sub bank 3
                MDrv_WriteByte(H_BK_COMPONENT(0x00),0x00); //change sub bank 3
                MDrv_WriteByteMask(L_BK_COMPONENT(0x20),BIT(0),BIT(0)); //Disable Macrovision
             }
        }
        else
        {
            ...
            else
            {
                MDrv_WriteByte(L_BK_COMPONENT(0x00),0x03); //change sub bank 3
                MDrv_WriteByte(H_BK_COMPONENT(0x00),0x00); //change sub bank 3
                MDrv_WriteByteMask(L_BK_COMPONENT(0x20),0,BIT(0)); //Disable Macrovision
            }
        }
    }
...
VE_Result Hal_VE_SetMV(MS_BOOL bEnable, MS_VE_MV_TYPE eMvType)
{
    ...
    if((eMvType >= MS_VE_MV_NTSC_START) && (eMvType <= MS_VE_MV_PAL_TEST_P01_TTX))
    {
    ...
    else
    {
        ...
        switch(eMvType)
        {
            ...
            case MS_VE_MV_PAL_TEST_P01_TTX:
                ...
                break;
            case MS_VE_MV_COMPONENT_480I_TYPE2:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_480I_TYPE2];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_480I_TYPE3:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_480I_TYPE3];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_480P_EIA_7701:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_480P_EIA_7701];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_480P_EIA_7702:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_480P_EIA_7702];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_576I_P01:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_576I_P01];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_576I_P02:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_576I_P02];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
            case MS_VE_MV_COMPONENT_576P:
                mTbl = MACROVISIONMAP_Main[MACROVISION_TYPE_MACROVISION_COMPONENT_576P];
                VE_DBG(printf("[%s][%d] Macrovision case TEST P01 TTX!!\n", __FUNCTION__, __LINE__));
                break;
        }
...
VE_Result Hal_VE_DCS_SetType(MS_BOOL bEnable, MS_VE_DCS_TYPE eDCSType)
{
    ...
    if(!bEnable)
    {
        ...
        else if((eDCSType >= MS_VE_DCS_COMPONENT_MIN) && (eDCSType <= MS_VE_DCS_COMPONENT_MAX))
        {
            ...
            MDrv_WriteByteMask(H_BK_CHIPTOP(0x5A),0x01,0x01); //Disable clock of DCS
        }
    }
    else
    {
        ...
        if((eDCSType >= MS_VE_DCS_COMPONENT_MIN) && (eDCSType <= MS_VE_DCS_COMPONENT_MAX))
        {
            MDrv_WriteByte(H_BK_CHIPTOP(0x5A), 0x00);   // enable clock of DCS
        }
