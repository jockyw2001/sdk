...
static void _PostHVSP(MS_U16 u16Vin, MS_U16 u16Vout, MS_U16 u16Hin, MS_U16 u16Hout)
{
    MS_U32 u32Vratio, u32Hratio;

    if(u16Vin == u16Vout && u16Hin == u16Hout)
    {
        MApi_XC_WriteByteMask(0x112CB0 , 0x00, 0x31); // [5]en_hvsp_p2i, [4]en_ext_hvsp, [0]madi_en
        MApi_XC_WriteByteMask(L_BK_VE_SRC(0x5C), 0x10, 0x00);
    }
    else
    {
        if(u16Hin == u16Hout)
        ...
        {
            u32Hratio = VE_H_POSTSCALE_RATIO(u16Hin, u16Hout) | BIT(23);
        }

        MApi_XC_W4BYTE(_SC_PK_L_(0x5D, 0x01), u32Hratio);
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x06), u16Hin);  // HSP input H size
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x07), u16Hout); // HSP output H size
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x41), u16Hin -1);  // before scaling width
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x43), u16Hout -1); // after scaling height
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x5E), u16Hout); // after scaling height
        ...
        {
            u32Vratio = VE_V_POSTSCALE_RATIO(u16Vin, u16Vout);
        }

        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x42), u16Vin -1); // before scaling height
        MApi_XC_W2BYTE(_SC_PK_L_(0x5D, 0x44), u16Vout -1); // after scaling height
        MApi_XC_W4BYTE(_SC_PK_L_(0x5D, 0x47), u32Vratio);
        MApi_XC_W2BYTEMSK(_SC_PK_L_(0x5D, 0x5F), u32Vratio ?  0 : 0x0002, 0x0002); // VSP ByPass


        MApi_XC_WriteByteMask(L_BK_VE_SRC(0x5C), 0x1E, 0x1F);
        MApi_XC_WriteByteMask(0x112CB0 , 0x31, 0x31); // [5]en_hvsp_p2i, [4]en_ext_hvsp, [0]madi_en
        MApi_XC_Write2ByteMask(0x112CD0, u16Vout, 0xFFFF);
        MApi_XC_Write2ByteMask(0x112CD2, u16Hout, 0xFFFF);
    }
}
...
static void _PreVscaleDown(MS_BOOL ben, MS_U16 In, MS_U16 Out)
{
#if VE_PRESCALE_SUPPORT
    MS_U32 u32V_Ratio = 0;
    MS_U32 u32FixPoint = 0;
    if(ben)
    {
        u32V_Ratio = ((In * 10000) / Out);
        u32FixPoint = ((In * 10000) / Out) % 10;

        if(u32FixPoint)
        {
            u32V_Ratio = VE_V_PRESCALE_DOWN_RATIO_1((((u32V_Ratio - u32FixPoint) * Out)/10000), Out);
        }
        else
        {
            u32V_Ratio = (MS_U32)VE_V_PRESCALE_DOWN_RATIO_1(In, Out);
        }
    ...
    MApi_XC_W2BYTEMSK(_SC_PK_L_(0x62, 0x45), BIT(2)|BIT(10), BIT(2)|BIT(10));  //bilinear
...
void MDrv_VE_SetMemoryBaseAddr(MS_PHY phyMIUAddress, MS_U32 u32MIUSize)
{
    ...
    u32Ret = UtopiaIoctl(pu32TvencoderInst, E_MDRV_CMD_TVE_SetMemoryBaseAddr, (void*)pArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        printf("[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_VE_set_scaling_ratio(MS_U16 u16Out_Hsize, MS_U16 u16Out_Vsize)
{
    ...
    {
        Hal_VE_set_pre_scaling_mux(TRUE);
    }
    else
    {
        Hal_VE_set_pre_scaling_mux(FALSE);
    }
    ...
    MSG_DRV_VE("PostHvsp:: H:(%d %d), V:(%d %d) \n", u16In_Hsize, u16Out_Hsize, u16In_Vsize, u16Out_Vsize);

    _PostHVSP(u16In_Vsize ,u16Out_Vsize ,u16In_Hsize, u16Out_Hsize);
