...
{
    VE_DrvStatus* pDrvStatus=NULL;
    pDrvStatus = (VE_DrvStatus*)malloc(sizeof(VE_DrvStatus));
    if(pDrvStatus == NULL)
    {
        MdbPrint(paraCmdInfo->u64ReqHdl,"----------------  Mdb VE return -----------------\n");
        return;
    }
    ...
    {
         MdbPrint(paraCmdInfo->u64ReqHdl,"----------------  Mdb VE GetStatus Fail-----------------\n");
    }
    free(pDrvStatus);
}
...
void TVENCODERMdb_EchoCmd(MDBCMD_CMDLINE_PARAMETER* paraCmdLine)
{
    char pch[] = " =,";
    char* psep=NULL;
    char* pcCmdLine = paraCmdLine->pcCmdLine;
    ...
    else if(strcmp(psep, "help") == 0)
    {
        MdbPrint(paraCmdLine->u64ReqHdl, "------------------- Mstar VE help --------------------\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "==================Test Pattern==================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo pattern num=var1,var2 > /proc/utopia_mdb/ve\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "pattern=0/1, enable=0/1\n");
    }
}
...
MS_U32 TVENCODERMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            TVENCODERMdb_EchoCmd(paraCmdLine);
