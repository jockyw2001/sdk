...
void HAL_TSIO_SGDMAIN_Reset(void)
{
    ...
    _REG16_SET(&(_TSOCtrl3->SGDMA_IN_CONFIG0), TSO3_NODE_LINK_ECO); //For k6u02 Node link ECO
}
...
MS_BOOL HAL_TSIO_SGDMAIN_Flt_SetBypassTSIO(MS_U16 u16FltId, MS_BOOL bBypassTSIO)
{
    // not support
    return FALSE;
}
...
void HAL_TSIO_Analogphase_Init(HAL_TSIO_CLK eClk, MS_BOOL bLoopback)
{
    ...
    do
    {
        if(u32Timeout++ > TSIO_RX_CAL_TIMEOUT)
        {
            HAL_TSIO_DBGMSG(E_HAL_TSIO_DBG_LEVEL_WARN, printf("TSIO Rx calibration timeout\n"));
            break;
        }
