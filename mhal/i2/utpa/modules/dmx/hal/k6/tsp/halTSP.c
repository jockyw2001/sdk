...
static REG_Ctrl*    _RegCtrl                          = NULL;  // TSP0 and TSP1
static REG_Ctrl2*   _RegCtrl2                         = NULL;  // TSP3
static REG_Ctrl3*   _RegCtrl3                         = NULL;  // TSP4
static REG_Ctrl4*   _RegCtrl4                         = NULL;  // TSP6
static REG_Ctrl5*   _RegCtrl5                         = NULL;  // TSP7
static REG_Ctrl6*   _RegCtrl6                         = NULL;  // TSP8
static REG_Ctrl7*   _RegCtrl7                         = NULL;  // TSP9
static REG_Ctrl8*   _RegCtrl8                         = NULL;  // TSP10
static REG_Ctrl8_1* _RegCtrl8_1                       = NULL;  // TSP10
...
typedef struct //  structure for PVR Buffer
{
    MS_BOOL         bOverWrite;           // True  : means the PVR buffer has been written already
                                          // False : means the PVR buffer has not been written yet
    MS_U32          u32Start;             // PVR buffer physical start address
    MS_U32          u32End;               // PVR buffer physical end address
    MS_BOOL         bPA2KSEG1_Mapping;    // True   : means PA2KSEG1 mapping success

}PVR_Buf;

static PVR_Buf   _stPvrBuf[PVR_NUM];
...
void HAL_TSP_SetBank(MS_VIRT u32BankAddr)
{
    ...
    _RegCtrl8_1 = (REG_Ctrl8_1*)(u32BankAddr  + 0xE1B00UL);   // TSP10 0x170D , offset(16-bits): 0x40
}

void HAL_TSP_SetExtRiuBank(MS_VIRT u32BankAddr)
{
    // not support
}
...
static void _REG32_DUMP(REG32* reg, MS_U32 value )
{
    //`RIU_W(`TSP_REG_BASE + 7'h06, 2'b11, 16'h400a);   //..bank 15 ,7h06 ....reg ,16h400a ......
    //`RIU_W(`TSP_REG_BASE1 + 7'h72, 2'b11, 16'hc000); // ..bank 16 ,7h06 ....reg ,16h400a ......
...
// [HW Issue]
// The K3 series PVR write pointer's return value is different from K1 series chip.
// For example, if PVR MIU alignment is 16 bytes. Write 32 bytes to PVR buffer from PVR start address 0.
// The 32 bytes data would be allocated to PVR buffer(address 0~31).
// And then we get the write pointer. The K3 series write pointer will return the value 16(W).
// The last MIU alignment address of the written data.
// The K1 series write pointer will return the value 32(W').
// The next MIU alignment address for the next writting.
// PVR write pointer value has 1 MIU alignment difference between K3 and K1 series.
// To SW, the K1 series write pointer's return value is more meaningful than K3 series.
// Now we use _Adjust_PVR_WritePtr function to make K3 series write pointer's return value consistent with K1 series.
//  --------------------------------->
// S          W          W'         E
// ######################
// +----------+----------+----------+
...
static MS_U32 _Adjust_PVR_WritePtr(MS_U32 u32Eng, MS_U32 u32WritePtr)
{
    if(_stPvrBuf[u32Eng].bPA2KSEG1_Mapping != TRUE)
    {
        return u32WritePtr;
    }

    if (_stPvrBuf[u32Eng].u32Start == 0)
    {
        return u32WritePtr;
    }

    if (!((E_TSP_PVR_PVRENG_START <= u32Eng) && (E_TSP_PVR_PVRENG_END > u32Eng))) // PVR Eng number check
    {
        return u32WritePtr;
    }

    if( _stPvrBuf[u32Eng].bOverWrite ) // If PVR buffer in OverWrite state
    {
        u32WritePtr = u32WritePtr + (1 << MIU_BUS);
    }
    else // If PVR buffer in non-OverWrite state
    {
        MsOS_ReadMemory();

        if(*((MS_U64*)(MsOS_PA2KSEG1(_stPvrBuf[u32Eng].u32Start)))!= PVR_NON_OVERWRITE)
        {
            _stPvrBuf[u32Eng].bOverWrite = TRUE; // Set PVR buffer to OverWrite state
            u32WritePtr = u32WritePtr + (1 << MIU_BUS);
        }
    }

    if( u32WritePtr >= _stPvrBuf[u32Eng].u32End )
    {
        //reset write address
        u32WritePtr = _stPvrBuf[u32Eng].u32Start;
    }
...
void HAL_TSP_HwPatch(void)
{
    ...
    // load lpcr1 for all pvr engines, HW default value may be wrong
    REG16_SET(&_RegCtrl->reg160C, TSP_PVR1_LPCR1_RLD);
    REG32_SET(&_RegCtrl->PVR2_Config, TSP_PVR2_LPCR1_RLD);
    REG16_SET((&_RegCtrl2->CFG_16), CFG_16_PVR3_LPCR1_RLD);
    REG16_SET((&_RegCtrl2->CFG_23), CFG_23_PVR4_LPCR1_RLD);
    ...
    REG16_SET(&_RegCtrl->PVRConfig, TSP_MATCH_PID_LD | TSP_REC_NULL);
    ...
    REG16_SET(&_RegCtrl6->CFG6_60, TSP_INIT_TIMESTAMP_RESTART_EN);

    // Merge stream through FIQ ECO config
    REG16_SET(&_RegCtrl6->CFG6_2B, TSP_ECO_TS_SYNC_OUT_DELAY);
    REG16_SET(&_RegCtrl6->CFG6_2B, TSP_ECO_TS_SYNC_OUT_REVERSE_BLOCK);

    // Fix filter null packet
    REG16_SET(&_RegCtrl6->CFG6_2B, TSP_FIX_FILTER_NULL_PKT);

    // Fixed miu_req & miu_last
    REG16_SET(&_RegCtrl6->CFG6_2A, FIXED_VQ_MIU_REG_FLUSH);

    // File/Live in 130 bytes stream will reference
    REG16_SET(&_RegCtrl3->CFG3_18, CFG3_18_HD_0000_TO_SECTION_RVU | CFG3_18_HD_0000_TO_SECTION_DIRECTV);

    // Enable all pktdmx and all FIFO's trace mark
    for(u32TSIF=0; u32TSIF<TSP_TSIF_NUM; u32TSIF++)
    {
        for(eDst=E_TSP_DST_FIFO_VIDEO; eDst<E_TSP_DST_INVALID; eDst++)
        {
            HAL_TSP_TRACE_MARK_En(u32TSIF, eDst, TRUE);
        }
    }
    ...
    if(K6U02)
    {
        REG16_SET(&_RegCtrl2->CFG_04,  CFG_04_MIU_FIXED_LAST_DONE_Z_ABT_ALL);
        REG16_SET(&_RegCtrl2->CFG_10,  CFG_10_MIU_CHECK_MI2RDY_ABT_ALL);
        REG16_SET(&_RegCtrl6->CFG6_2B, TSP_OR_WRITE_FIX_FOR_NEW_MIU_ARBITER_DISABLE);
    }
}
...
void HAL_TSP_Path_Reset(MS_U32 tsIf,MS_BOOL bEn)
{
    switch(tsIf)
    {
        ...
        case 3: if(bEn)
                {
                    REG16_SET(&_RegCtrl2->CFG_0A,CFG_0A_RST_TS_FIN3);
                    REG16_SET(&_RegCtrl2->CFG_10,CFG_10_RESET_PDFLT3);
                    REG16_SET(&_RegCtrl2->CFG_12,CFG_12_REG_REST_RBF3 | CFG_12_REG_REST_PDBF3);
                }
                else
                {
                    REG16_CLR(&_RegCtrl2->CFG_12,CFG_12_REG_REST_RBF3 | CFG_12_REG_REST_PDBF3);
                    REG16_CLR(&_RegCtrl2->CFG_10,CFG_10_RESET_PDFLT3);
                    REG16_CLR(&_RegCtrl2->CFG_0A,CFG_0A_RST_TS_FIN3);
                }
                break;
        default: break;
    }
...
MS_BOOL HAL_TSP_GetClockSetting(MS_U32 u32TSIF, MS_U16* pu16Clk)
{
    switch(u32TSIF)
    {
        case 0:
            *pu16Clk   = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS0_CLK) >> REG_CLKGEN0_TS0_SHIFT) & REG_CLKGEN0_TS_MASK;
            break;
        case 1:
            *pu16Clk   = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS1_CLK) >> REG_CLKGEN0_TS1_SHIFT) & REG_CLKGEN0_TS_MASK;
            break;
        case 2:
            *pu16Clk   = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS2_CLK) >> REG_CLKGEN0_TS2_SHIFT) & REG_CLKGEN0_TS_MASK;
            break;
        case 3:
            *pu16Clk   = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS3_CLK) >> REG_CLKGEN0_TS3_SHIFT) & REG_CLKGEN0_TS_MASK;
            break;
        default:
            *pu16Clk    = 0;
            return FALSE;
...
void HAL_TSP_Power(MS_BOOL bEn)
{
    if(bEn)
    {
        ...
        // Enable STC1,2 Clk
        // STC0
        s32Handle = Drv_Clkm_Get_Handle("g_clk_stc0");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_STC0_BUF");
        // STC1
        s32Handle = Drv_Clkm_Get_Handle("g_clk_stc1");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_STC0_BUF");
        // Stamp
        ...
        //TS0
        s32Handle = Drv_Clkm_Get_Handle("g_clk_ts");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_TS0_PAD0");
        //TS1
        s32Handle = Drv_Clkm_Get_Handle("g_clk_ts1");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_TS0_PAD0");
        //TS2
        s32Handle = Drv_Clkm_Get_Handle("g_clk_ts2");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_TS0_PAD0");
        //TS3
        s32Handle = Drv_Clkm_Get_Handle("g_clk_ts3");
        Drv_Clkm_Set_Clk_Source(s32Handle,"CLK_TS0_PAD0");
    ...
    else
    {
        ...
        // Disable TSP Clk
        // [2016.10.14] Because AESDMA share clk with TSP. We can't disable TSP clk.
        // s32Handle = Drv_Clkm_Get_Handle("g_clk_tsp");
...
#else
        // Disable TSP Clk
        // [2016.10.11] Because AESDMA share clk with TSP. We can't disable TSP clk.
...
#define _TSP_QMEM_SIZE              0x1000 // 16K bytes, 32bit aligment  //0x4000  this is 4 byte address
    ...
    REG16_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG16_R(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM) & (~REG_MIU_SEL_FILEIN0_MASK)) | ((u8MiuSel << REG_MIU_SEL_FILEIN0_SHIFT) & REG_MIU_SEL_FILEIN0_MASK));
    ...
    if(bEn)
    {
        switch(pktDmxId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_12, CFG_12_REG_REST_PDBF3);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(pktDmxId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_12, CFG_12_REG_REST_PDBF3);
                break;
            default:
                break;
        }
    ...
    if(bEn)
    {
        switch(pktDmxId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_12, CFG_12_REG_REST_RBF3);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(pktDmxId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_12, CFG_12_REG_REST_RBF3);
                break;
            default:
                break;
        }
    ...
    if(bEnable)
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_TS_IF3_EN);
                break;
            default:
                return FALSE;
        }
    ...
    else
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_TS_IF3_EN);
                break;
            default:
                return FALSE;
        }
...
MS_BOOL HAL_TSP_TSIF_SelPad(MS_U32 tsIf, TSP_TS_PAD eTSPad) // @FIXME modify this parameter to enum plz
{
    ...
    switch(clk_src)
    {
        case REG_CLKGEN0_TS_SRC_EXT0:
            u8NameIdx = 0;
            break;
        case REG_CLKGEN0_TS_SRC_EXT1:
            u8NameIdx = 1;
            break;
        case REG_CLKGEN0_TS_SRC_EXT2:
            u8NameIdx = 2;
            break;
        case REG_CLKGEN0_TS_SRC_EXT3:
            u8NameIdx = 3;
            break;
        case REG_CLKGEN0_TS_SRC_EXT4:
            u8NameIdx = 4;
            break;
        case REG_CLKGEN0_TS_SRC_EXT5:
            u8NameIdx = 5;
            break;
        case REG_CLKGEN0_TS_SRC_EXT6:
            u8NameIdx = 6;
            break;
        case REG_CLKGEN0_TS_SRC_TSO0:
            u8NameIdx = 7;
            break;
        case REG_CLKGEN0_TS_SRC_TSIO0:
            u8NameIdx = 8;
            break;
        default:
    ...
    switch (tsIf)
    {
        ...
        #ifdef CONFIG_MSTAR_CLKM
            handle = Drv_Clkm_Get_Handle("g_clk_ts");
        ...
        #ifdef CONFIG_MSTAR_CLKM
            handle = Drv_Clkm_Get_Handle("g_clk_ts1");
            Drv_Clkm_Set_Clk_Source(handle,u8ClkSrcNames[u8NameIdx]);
        #else
            TSP_CLKGEN0_REG(REG_CLKGEN0_TS1_CLK)  = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS1_CLK) & ~(REG_CLKGEN0_TS_MASK<<REG_CLKGEN0_TS1_SHIFT))
        ...
        case 2:
        ...
        #ifdef CONFIG_MSTAR_CLKM
            handle = Drv_Clkm_Get_Handle("g_clk_ts2");
            Drv_Clkm_Set_Clk_Source(handle,u8ClkSrcNames[u8NameIdx]);
        #else
            TSP_CLKGEN0_REG(REG_CLKGEN0_TS2_CLK)  = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS2_CLK) & ~(REG_CLKGEN0_TS_MASK<<REG_CLKGEN0_TS2_SHIFT))
            ...
            break;
        case 3:
            TSP_TOP_REG(REG_TOP_TS3_MUX)          =  (TSP_TOP_REG(REG_TOP_TS3_MUX) & ~(REG_TOP_TS_SRC_MASK<<REG_TOP_TS3_SHIFT))
                                                        | (pad_src<<REG_TOP_TS3_SHIFT);
        #ifdef CONFIG_MSTAR_CLKM
            handle = Drv_Clkm_Get_Handle("g_clk_ts3");
            Drv_Clkm_Set_Clk_Source(handle,u8ClkSrcNames[u8NameIdx]);
        #else
            TSP_CLKGEN0_REG(REG_CLKGEN0_TS3_CLK)  = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS3_CLK) & ~(REG_CLKGEN0_TS_MASK<<REG_CLKGEN0_TS3_SHIFT))
                                                        | (clk_src<<(REG_CLKGEN0_TS3_SHIFT+REG_CLKGEN0_TS_SRC_SHIFT));
        #endif
            break;
...
static void _HAL_TSO_SetPad1OutMux(MS_BOOL bSet)
{
    if(bSet)
    {
        TSP_TOP_REG(REG_TOP_TS_PADMUX_MODE) = TSP_TOP_REG(REG_TOP_TS_PADMUX_MODE) & ~(REG_TOP_TS1MODE_MASK << REG_TOP_TS1MODE_SHIFT);
    }
    else
    {
        TSP_TOP_REG(REG_TOP_TS_PADMUX_MODE) = ((TSP_TOP_REG(REG_TOP_TS_PADMUX_MODE) & ~(REG_TOP_TS1MODE_MASK << REG_TOP_TS1MODE_SHIFT))
                                            | (REG_TOP_TS1MODE_INPUT << REG_TOP_TS1MODE_SHIFT));
    }
}

void HAL_TSO_SetTSOOutMUX(MS_BOOL bSet)
{
    _HAL_TSO_SetPad1OutMux(bSet);
}
...
MS_BOOL HAL_TSP_TsOutPadCfg(TSP_TS_PAD eOutPad, TSP_TS_PAD_MUX_MODE eOutPadMode, TSP_TS_PAD eInPad, TSP_TS_PAD_MUX_MODE eInPadMode, MS_BOOL bEnable)
{
    ...
    switch(eOutPad) // output pad
    {
        case E_TSP_TS_PAD_EXT1: // TS_pad #1
            if(eInPad == E_TSP_TS_PAD_EXT1)  // output pad == input pad ?
            {
                return FALSE;
            }

            _HAL_TSO_SetPad1OutMux(bEnable); // set pad mode (input or output)

            switch(eOutPadMode) // set pad output mode
            {
                case E_TSP_TS_PAD_MUX_TSO:
                    // input from TSO
                    TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) = (TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) & ~(REG_TOP_TS_OUT_MODE_MASK << REG_TOP_TS_OUT_MODE_SHIFT))
                                                        | (REG_TOP_TS_OUT_MODE_TSO << REG_TOP_TS_OUT_MODE_SHIFT);
                    break;
                case E_TSP_TS_PAD_MUX_S2P:
                    // input from S2P
                    TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) = (TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) & ~(REG_TOP_TS_OUT_MODE_MASK << REG_TOP_TS_OUT_MODE_SHIFT))
                                                        | (REG_TOP_TS_OUT_MODE_S2P << REG_TOP_TS_OUT_MODE_SHIFT);

                    u16Temp = REG_TSO_TSP_S2P_EN | REG_TSO_TSP_S2P_TS_SIN_C0 | REG_TSO_TSP_S2P_TS_SIN_C1;   // S2P enable bit , serial bit count setting
                    if(eInPadMode == E_TSP_TS_PAD_MUX_PARALLEL)
                    {
                        u16Temp |= REG_TSO_TSP_BYPASS_S2P;                                                  // bypass S2P: if input is parallel , set it
                    }
                    else if(eInPadMode == E_TSP_TS_PAD_MUX_3WIRED_SERIAL)
                    {
                        u16Temp |= REG_TSO_TSP_S2P_3WIRE;                                                   // input pad is 3-wire serial
                    }

                    // S2P config
                    TSP_TSO_REG(REG_TSO_TSP_CONFIG0) = (TSP_TSO_REG(REG_TSO_TSP_CONFIG0) & ~REG_TSO_TSP_S2P_MASK) | u16Temp;
                    // select S2P input pad
                    TSP_TOP_REG(REG_TOP_TSO4_5_MUX) = (TSP_TOP_REG(REG_TOP_TSO4_5_MUX) & ~REG_TOP_TSO4_MASK)
                                                    | (eInPad << REG_TOP_TSO4_SHIFT);
                    // select S2P input clk source
#ifdef CONFIG_MSTAR_CLKM
                    {
                        MS_S32 handle;
                        MS_U8  u8NameIdx = 0;
                        ...
                        handle = Drv_Clkm_Get_Handle("g_clk_s2p_in");
                        Drv_Clkm_Set_Clk_Source(handle, u8ClkSrcNames[u8NameIdx]);
                    }
#else
                    TSP_CLKGEN0_REG(REG_CLKGEN0_S2P_IN_CLK_SRC) = (TSP_CLKGEN0_REG(REG_CLKGEN0_S2P_IN_CLK_SRC) & ~(REG_CLKGEN0_S2P_IN_CLK_MASK << REG_CLKGEN0_S2P_IN_CLK_SHIFT))
                                                                | ((eInPad & REG_CLKGEN0_S2P_IN_CLK_SRC_MASK) << (REG_CLKGEN0_S2P_IN_CLK_SHIFT + REG_CLKGEN0_S2P_IN_CLK_SRC_SHIFT));
#endif
                    break;
                case E_TSP_TS_PAD_MUX_S2P1:
                    // input from S2P1
                    TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) = (TSP_TOP_REG(REG_TOP_TS_OUTPUT_MODE) & ~(REG_TOP_TS_OUT_MODE_MASK << REG_TOP_TS_OUT_MODE_SHIFT))
                                                        | (REG_TOP_TS_OUT_MODE_S2P1 << REG_TOP_TS_OUT_MODE_SHIFT);

                    u16Temp = REG_TSO_TSP_S2P1_EN | REG_TSO_TSP_S2P1_TS_SIN_C0 | REG_TSO_TSP_S2P1_TS_SIN_C1;// S2P1 enable bit , serial bit count setting
                    if(eInPadMode == E_TSP_TS_PAD_MUX_PARALLEL)
                    {
                        u16Temp |= REG_TSO_TSP_BYPASS_S2P1;                                                 // bypass S2P1: if input is parallel , set it
                    }
                    else if(eInPadMode == E_TSP_TS_PAD_MUX_3WIRED_SERIAL)
                    {
                        u16Temp |= REG_TSO_TSP_S2P1_3WIRE;                                                  // input pad is 3-wire serial
                    }
                    // S2P1 config
                    TSP_TSO_REG(REG_TSO_TSP_CONFIG0) = (TSP_TSO_REG(REG_TSO_TSP_CONFIG0) & ~REG_TSO_TSP_S2P1_MASK) | u16Temp;
                    // select S2P1 input pad
                    TSP_TOP_REG(REG_TOP_TSO4_5_MUX) = (TSP_TOP_REG(REG_TOP_TSO4_5_MUX) & ~REG_TOP_TSO5_MASK)
                                                    | (eInPad << REG_TOP_TSO5_SHIFT);
#ifdef CONFIG_MSTAR_CLKM
                    {
                        MS_S32 handle;
                        MS_U8  u8NameIdx = 0;
                        ...
                        handle = Drv_Clkm_Get_Handle("g_clk_s2p1_in");
                        Drv_Clkm_Set_Clk_Source(handle, u8ClkSrcNames[u8NameIdx]);
                    }
#else
                    // select S2P1 input clk source
                    TSP_CLKGEN0_REG(REG_CLKGEN0_S2P_IN_CLK_SRC) = (TSP_CLKGEN0_REG(REG_CLKGEN0_S2P_IN_CLK_SRC) & ~(REG_CLKGEN0_S2P_IN_CLK_MASK << REG_CLKGEN0_S2P1_IN_CLK_SHIFT))
                                                                | ((eInPad & REG_CLKGEN0_S2P_IN_CLK_SRC_MASK) << (REG_CLKGEN0_S2P1_IN_CLK_SHIFT + REG_CLKGEN0_S2P_IN_CLK_SRC_SHIFT));
#endif

                    break;
                default:
                    HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%d] Warining wrong output PAD Mode:%u\n",__FUNCTION__,__LINE__, eOutPadMode));
                    return FALSE;
            }
            break;
        default:
    ...
    switch (eTSPad)
    {
        ...
        case E_TSP_TS_PAD_INTER0:
            ...
            *pu32PADSrc = REG_TOP_TS_SRC_DMD0;
            break;
        case E_TSP_TS_PAD_TSOUT0:
            *pu32CLKSrc = REG_CLKGEN0_TS_SRC_TSO0;
            *pu32PADSrc = REG_TOP_TS_SRC_TSO0;
            break;
    ...
    switch(u32TSOEng)
    {
        case 0:
...
#ifdef CONFIG_MSTAR_CLKM
            {
                ...
                handle = Drv_Clkm_Get_Handle("g_clk_tso_in");
                Drv_Clkm_Set_Clk_Source(handle,u8ClkSrcNames[u8NameIdx]);
            }
#else
            TSP_CLKGEN0_REG(REG_CLKGEN0_TSO0_CLK) = (TSP_CLKGEN0_REG(REG_CLKGEN0_TSO0_CLK) & ~REG_CLKGEN0_TS_MASK)
...
MS_BOOL HAL_TSP_SetOutPadCfg(TSP_TS_PAD eOutPad, TSP_TS_PAD_MUX_MODE eOutPadMode, TSP_TS_PAD eInPad, MS_BOOL bInParallel)
{
    //not support
    return FALSE;
}
...
MS_BOOL HAL_TSP_TSIF_SelPad_ClkDis(MS_U32 tsIf , MS_BOOL bClkDis)
{
    ...
    else
    {
        ...
        switch (tsIf)
        {
            case 0:
                s32Handle = Drv_Clkm_Get_Handle("g_clk_ts");
                u16ClkSrc = (TSP_CLKGEN0_REG(REG_CLKGEN0_TS0_CLK) & REG_CLKGEN0_TS_MASK) >> (REG_CLKGEN0_TS0_SHIFT + REG_CLKGEN0_TS_SRC_SHIFT);
                break;
            case 1:
                s32Handle = Drv_Clkm_Get_Handle("g_clk_ts1");
    ...
    if(bEnable)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_TSP_FILE_SEGMENT3);
                REG16_SET(&_RegCtrl2->CFG_0A, CFG_0A_TSP_FILE_SEGMENT_TSIF3);
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_TS_DATA_PORT_SEL3);
                REG16_SET(&_RegCtrl2->CFG_0A, CFG_0A_TSP_FILE_IN_TSIF3);
                break;
            default:
                return FALSE;
        }
    ...
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_TSP_FILE_SEGMENT3);
                REG16_CLR(&_RegCtrl2->CFG_0A, CFG_0A_TSP_FILE_SEGMENT_TSIF3);
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_TS_DATA_PORT_SEL3);
                REG16_CLR(&_RegCtrl2->CFG_0A, CFG_0A_TSP_FILE_IN_TSIF3);
                break;
            default:
                return FALSE;
        }
    ...
    if(bEnable)
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_TS_DATA3_SWAP);
                break;
            default:
                return;
        }
    ...
    else
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_TS_DATA3_SWAP);
                break;
            default:
                return;
        }
    ...
    if(bEnable)
    {
        ...
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_EXT_SYNC_SEL3);
                break;
            default:
                return;
        }
    ...
    else
    {
        ...
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_EXT_SYNC_SEL3);
                break;
            default:
                return;
        }
    ...
    if(bBypass)
    {
        switch (eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl->TSP_Ctrl2, TSP_AV_DIRECT_STOP3);
                break;
            default:
                HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupported File Eng : %d !\n",__FILE__,__FUNCTION__,__LINE__,eFileEng));
                break;
        }
        ...
        switch (eFileEng)
        {
           case E_FILEENG_TSIF0:
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl->TSP_Ctrl2, TSP_AV_DIRECT_STOP3);
                break;
            default:
                HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupported File Eng : %d !\n",__FILE__,__FUNCTION__,__LINE__,eFileEng));
                break;
    ...
    if(bEnable)
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&(_RegCtrl2->CFG_0B), CFG_0B_P_SEL3);
                break;
            default:
                return;
        }
    ...
    else
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&(_RegCtrl2->CFG_0B), CFG_0B_P_SEL3);
                break;
            default:
                return;
        }
    ...
    if(bEnable)
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&(_RegCtrl3->CFG3_37), HW4_CFG37_3WIRE_SERIAL_MODE_TS3);
                break;
            default:
                return;
        }
    ...
    else
    {
        switch(tsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&(_RegCtrl3->CFG3_37), HW4_CFG37_3WIRE_SERIAL_MODE_TS3);
                break;
            default:
                return;
        }
    ...
    if(bEn)
    {
        switch(u32Tsif)
        {
            case 0:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_SET(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX0_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_SET(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX0_TRACE_MARK_V4_EN);
                        break;
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_SET(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX0_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX0_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 1:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_SET(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX1_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_SET(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX1_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_SET(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX1_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX1_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 2:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_SET(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX2_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_SET(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX2_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_SET(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX2_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX2_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 3:
                ...
                {
                    case E_TSP_DST_FIFO_VIDEO:
                        REG16_SET(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_V_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO3D:
                        REG16_SET(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_V3D_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_SET(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX3_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_SET(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX3_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                        REG16_SET(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_A_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO2:
                        REG16_SET(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_AB_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_SET(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX3_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
    ...
    else
    {
        switch(u32Tsif)
        {
            case 0:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX0_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX0_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX0_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX0_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 1:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX1_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX1_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX1_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX1_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 2:
                switch (eFltType)
                {
                    ...
                    case E_TSP_DST_FIFO_VIDEO3D:
                        ...
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX2_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX2_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                    ...
                    case E_TSP_DST_FIFO_AUDIO2:
                        ...
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2A, CFG3_2A_PKTDMX2_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX2_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
            ...
            case 3:
                ...
                {
                    case E_TSP_DST_FIFO_VIDEO:
                        REG16_CLR(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_V_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO3D:
                        REG16_CLR(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_V3D_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2E, CFG3_2E_PKTDMX3_TRACE_MARK_V3_EN);
                        break;
                    case E_TSP_DST_FIFO_VIDEO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2F, CFG3_2F_PKTDMX3_TRACE_MARK_V4_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO:
                        REG16_CLR(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_A_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO2:
                        REG16_CLR(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_AB_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO3:
                        REG16_CLR(&_RegCtrl3->CFG3_2B, CFG3_2B_PKTDMX3_TRACE_MARK_AC_EN);
                        break;
                    case E_TSP_DST_FIFO_AUDIO4:
                        REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_PKTDMX3_TRACE_MARK_AD_EN);
                        break;
                    default:
                        break;
                }
...
void HAL_TSP_FIFO_ClkGating(TSP_DST_SEQ eFltType, MS_BOOL bEnable)
{
    // not support
}
...
void HAL_TSP_Filein_PktSize(FILEENG_SEQ eFileEng, MS_U32 u32PktSize)
{
    ...
    {
        case E_FILEENG_TSIF0:
            u16value = HW4_CFG36_RVU_PSI_EN0 | HW4_CFG36_RVU_EN0 | HW4_CFG36_RVU_TIMESTAMP_EN0;
            REG16_CLR(&_RegCtrl3->CFG3_36, u16value);
            break;
        case E_FILEENG_TSIF1:
            u16value = HW4_CFG36_RVU_PSI_EN1 | HW4_CFG36_RVU_EN1 | HW4_CFG36_RVU_TIMESTAMP_EN1;
            REG16_CLR(&_RegCtrl3->CFG3_36, u16value);
            break;
        case E_FILEENG_TSIF2:
            u16value = CFG4_54_RVU_PSI_EN2 | CFG4_54_RVU_EN2 | CFG4_54_RVU_TIMESTAMP_EN2;
            REG16_CLR(&_RegCtrl4->CFG4_54, u16value);
            break;
        case E_FILEENG_TSIF3:
            u16value = CFG4_54_RVU_PSI_EN3 | CFG4_54_RVU_EN3 | CFG4_54_RVU_TIMESTAMP_EN3;
            REG16_CLR(&_RegCtrl4->CFG4_54, u16value);
            break;
        default:
            printf("Not support !!\n");
    ...
    if(K6U02)
    {
        ...
        {
            case E_FILEENG_TSIF0:
                REG16_CLR(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH0);
                REG16_CLR(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH0_RVU_ALWAYS_PAYLOAD_128);
                break;
            case E_FILEENG_TSIF1:
                REG16_CLR(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH1);
                REG16_CLR(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH1_RVU_ALWAYS_PAYLOAD_128);
                break;
            case E_FILEENG_TSIF2:
                REG16_CLR(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH2);
                REG16_CLR(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH2_RVU_ALWAYS_PAYLOAD_128);
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH3);
                REG16_CLR(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH3_RVU_ALWAYS_PAYLOAD_128);
                break;
            default:
                printf("Not support !!\n");
                return;
    ...
    if(u32PktSize == 130 || u32PktSize == 134) //RVU
    {
        ...
        if(u32PktSize == 134)
        {
            ...
            {
                case E_FILEENG_TSIF0:
                    u16value = HW4_CFG36_RVU_TIMESTAMP_EN0;
                    break;
                case E_FILEENG_TSIF1:
                    u16value = HW4_CFG36_RVU_TIMESTAMP_EN1;
                    break;
                case E_FILEENG_TSIF2:
                    u16value = CFG4_54_RVU_TIMESTAMP_EN2;
                    break;
                case E_FILEENG_TSIF3:
                    u16value = CFG4_54_RVU_TIMESTAMP_EN3;
                    break;
                default:
                    printf("Not support !!\n");
                    break;
        ...
        {
            case E_FILEENG_TSIF0:
                u16value |= (HW4_CFG36_RVU_PSI_EN0 | HW4_CFG36_RVU_EN0);
                REG16_SET(&_RegCtrl3->CFG3_36, u16value);
                break;
            case E_FILEENG_TSIF1:
                u16value |= (HW4_CFG36_RVU_PSI_EN1 | HW4_CFG36_RVU_EN1);
                REG16_SET(&_RegCtrl3->CFG3_36, u16value);
                break;
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                u16value |= (CFG4_54_RVU_PSI_EN3 | CFG4_54_RVU_EN3);
                REG16_SET(&_RegCtrl4->CFG4_54, u16value);
                break;
            default:
                printf("Not support !!\n");
        ...
        if(K6U02)
        {
            ...
            {
                case E_FILEENG_TSIF0:
                    REG16_SET(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH0);
                    REG16_SET(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH0_RVU_ALWAYS_PAYLOAD_128);
                    break;
                case E_FILEENG_TSIF1:
                    REG16_SET(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH1);
                    REG16_SET(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH1_RVU_ALWAYS_PAYLOAD_128);
                    break;
                case E_FILEENG_TSIF2:
                    REG16_SET(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH2);
                    REG16_SET(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH2_RVU_ALWAYS_PAYLOAD_128);
                    break;
                case E_FILEENG_TSIF3:
                    REG16_SET(&_RegCtrl3->CFG3_1A, CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH3);
                    REG16_SET(&_RegCtrl3->CFG3_1C, CFG3_1C_PATH3_RVU_ALWAYS_PAYLOAD_128);
                    break;
                default:
                    printf("Not support !!\n");
                    return;
        ...
        HAL_TSP_HCMD_TSIF_130_Enable((MS_U32)eFileEng, TRUE);
    }
    else
    {
        HAL_TSP_HCMD_TSIF_130_Enable((MS_U32)eFileEng, FALSE);
    }
    ...
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF1:
            ...
            break;
        case E_FILEENG_TSIF2:
            REG16_W(&_RegCtrl2->CFG_07, (REG16_R(&_RegCtrl2->CFG_07) & ~CFG_07_PKT_CHK_SIZE_FIN2) | (CFG_07_PKT_CHK_SIZE_FIN2 & (u32PktLen-1)));
            break;
        case E_FILEENG_TSIF3:
            REG16_W(&_RegCtrl2->CFG_0C, (REG16_R(&_RegCtrl2->CFG_0C) & ~CFG_0C_PKT_CHK_SIZE_FIN3) | (CFG_0C_PKT_CHK_SIZE_FIN3 & (u32PktLen-1)));
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_Addr(FILEENG_SEQ eFileEng, MS_U32 addr)
{
    ...
    switch(eFileEng)
    {
        case E_FILEENG_TSIF0:
            REG16_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG16_R(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM) & (~REG_MIU_SEL_FILEIN0_MASK)) | ((u8MiuSel << REG_MIU_SEL_FILEIN0_SHIFT) & REG_MIU_SEL_FILEIN0_MASK));
            REG32_W(&_RegCtrl->TsDma_Addr, phyMiuOffsetFileinAddr);
            break;
        case E_FILEENG_TSIF1:
            REG16_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG16_R(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM) & (~REG_MIU_SEL_FILEIN1_MASK)) | ((u8MiuSel << REG_MIU_SEL_FILEIN1_SHIFT) & REG_MIU_SEL_FILEIN1_MASK));
            REG32_W(&_RegCtrl2->CFG_30_31, phyMiuOffsetFileinAddr);
            break;
        case E_FILEENG_TSIF2:
            REG16_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG16_R(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM) & (~REG_MIU_SEL_FILEIN2_MASK)) | ((u8MiuSel << REG_MIU_SEL_FILEIN2_SHIFT) & REG_MIU_SEL_FILEIN2_MASK));
            REG32_W(&_RegCtrl2->CFG_35_36, phyMiuOffsetFileinAddr);
            break;
        case E_FILEENG_TSIF3:
            REG16_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG16_R(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM) & (~REG_MIU_SEL_FILEIN3_MASK)) | ((u8MiuSel << REG_MIU_SEL_FILEIN3_SHIFT) & REG_MIU_SEL_FILEIN3_MASK));
            REG32_W(&_RegCtrl2->CFG_3A_3B, phyMiuOffsetFileinAddr);
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_Size(FILEENG_SEQ eFileEng, MS_U32 size)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG32_W(&_RegCtrl2->CFG_3C_3D, size);
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_Start(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG16_SET(&_RegCtrl2->CFG_3E, CFG_3E_FILEIN_CTRL_TSIF3_START);
            break;
        default:
            break;
    }
...
MS_BOOL HAL_TSP_File_Pause(FILEENG_SEQ eFileEng)
{
    switch (eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            return TRUE;
        case E_FILEENG_TSIF3:
            REG16_SET(&_RegCtrl3->CFG3_21, CFG3_21_TSIF3_FILE_PAUSE);
            return TRUE;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSP_File_Resume(FILEENG_SEQ eFileEng)
{
    switch (eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            return TRUE;
        case E_FILEENG_TSIF3:
            REG16_CLR(&_RegCtrl3->CFG3_21, CFG3_21_TSIF3_FILE_PAUSE);
            return TRUE;
        default:
            return FALSE;
    }
...
void HAL_TSP_Filein_Init_Trust_Start(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG16_SET(&_RegCtrl2->CFG_3E, (CFG_3E_FILEIN_INIT_TRUST_TSIF3 | CFG_3E_FILEIN_CTRL_TSIF3_START));
            break;
        default:
            break;
    }
    ...
    if(bEn)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl2->CFG_04, CFG_04_TSP_FILEIN_ABORT_ECO_TSIF3);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl2->CFG_04, CFG_04_TSP_FILEIN_ABORT_ECO_TSIF3);
                break;
            default:
                break;
        }
...
MS_U32 HAL_TSP_Filein_CmdQSlot(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            return (CFG_41_REG_TSIF3_CMD_QUEUE_SIZE - (REG16_R(&_RegCtrl2->CFG_41) & CFG_41_REG_TSIF3_CMD_QUEUE_WR_CNT));
...
MS_U32 HAL_TSP_Filein_CmdQCnt(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            return (REG16_R(&_RegCtrl2->CFG_41) & CFG_41_REG_TSIF3_CMD_QUEUE_WR_CNT);
        default:
            return 0;
    }
}
...
MS_U32 HAL_TSP_Filein_CmdQLv(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            return ((REG16_R(&_RegCtrl2->CFG_41) & CFG_41_REG_TSIF3_CMD_QUEUE_WR_LEVEL) >> CFG_3F_REG_TSIF1_CMD_QUEUE_LEVEL_SHIFT);
        default:
            return 0;
    }
}
    ...
    if(bEnable)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_W(&_RegCtrl2->CFG_0D, delay & CFG_0D_TSP_FILE_TIMER3);
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_TIMER_EN3);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_TIMER_EN3);
                REG16_W(&_RegCtrl2->CFG_0D, 0x0000);
                break;
            default:
                break;
        }
...
MS_U32 HAL_TSP_Filein_Status(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            return !(REG16_R(&_RegCtrl2->CFG_3E) & CFG_3E_FILEIN_CTRL_TSIF3_DONE);
        default:
            return 0;
    }
}
...
TSP_HAL_FileState HAL_TSP_Filein_GetState(FILEENG_SEQ eFileEng)
{
    ...
    switch (eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            if (REG16_R(&_RegCtrl3->CFG3_21) & CFG3_21_TSIF3_FILE_PAUSE)
            {
                state = E_TSP_HAL_FILE_STATE_PAUSE;
            }
            else if (u32Status)
    ...
    if(bSet)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_PKT192_EN3);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_PKT192_EN3);
                break;
            default:
                break;
        }
    ...
    if(bEn)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_PKT192_BLK_DISABLE3);
            default:
                break;
        }
    }
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_PKT192_BLK_DISABLE3);
                break;
            default:
                break;
        }
...
static void _HAL_TSP_FILEIN_ResetPktTimeStamp(FILEENG_SEQ eFileEng, MS_U32 u32InitTimeStamp)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG16_SET(&_RegCtrl3->CFG3_17, CFG3_17_INIT_TIMESTAMP_TSIF_3);
            REG32_W(&_RegCtrl6->CFG6_56_57, u32InitTimeStamp);
            REG16_CLR(&_RegCtrl3->CFG3_17, CFG3_17_INIT_TIMESTAMP_TSIF_3);
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_SetTimeStamp(FILEENG_SEQ eFileEng, MS_U32 u32Stamp)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG16_SET(&_RegCtrl2->CFG_0B, CFG_0B_LPCR2_WLD3);
            REG32_W(&_RegCtrl2->CFG_54_55, u32Stamp);
            REG16_CLR(&_RegCtrl2->CFG_0B, CFG_0B_LPCR2_WLD3);
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_SetTimeStampClk(FILEENG_SEQ eFileEng, TSP_HAL_TimeStamp_Clk eTimeStampClk)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            if(eTimeStampClk==E_TSP_HAL_TIMESTAMP_CLK_27M)
            {
                REG16_SET(&_RegCtrl3->CFG3_0F, CFG3_0F_TSIF3_C27M);
            }
            else
            {
                REG16_CLR(&_RegCtrl3->CFG3_0F, CFG3_0F_TSIF3_C27M);
            }
            break;
        default:
            break;
...
MS_U32 HAL_TSP_Filein_GetTimeStamp(FILEENG_SEQ eFileEng)
{
    ...
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG16_CLR(&_RegCtrl2->CFG_0A, CFG_0A_LPCR2_LOAD_TSIF3);
            u32Stamp = REG32_R(&_RegCtrl2->CFG_54_55);
            REG16_SET(&_RegCtrl2->CFG_0A, CFG_0A_LPCR2_LOAD_TSIF3);
            break;
        default:
            u32Stamp = 0;
            break;
...
MS_U32 HAL_TSP_Filein_PktTimeStamp(FILEENG_SEQ eFileEng)
{
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF3:
            return REG32_R(&_RegCtrl2->CFG_46_47);
        default:
            break;
    }
    return 0;
    ...
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            *pu32Addr = (MS_PHY)(REG32_R(&_RegCtrl2->CFG_6C_6D) & CFG_6C_6D_TSP2MI_RADDR_S_TSIF2);
            break;
        case E_FILEENG_TSIF3:
            *pu32Addr = (MS_PHY)(REG32_R(&_RegCtrl2->CFG_6E_6F) & CFG_6E_6F_TSP2MI_RADDR_S_TSIF3);
            break;
        default:
            break;
    }
...
void HAL_TSP_Filein_WbFsmRst(FILEENG_SEQ eFileEng, MS_BOOL bEnable)
{
    if(bEnable)
    {
        ...
        {
            case E_FILEENG_TSIF0:
                REG16_SET(&_RegCtrl3->CFG3_53, CFG3_53_WB_FSM_RESET);
                break;
            case E_FILEENG_TSIF1:
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl6->CFG6_2B, TSP_RESET_WB_DMA_FSM_TSIF3);
                break;
            default:
    ...
    else
    {
        ...
        {
            case E_FILEENG_TSIF0:
                REG16_CLR(&_RegCtrl3->CFG3_53, CFG3_53_WB_FSM_RESET);
                break;
            case E_FILEENG_TSIF1:
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl6->CFG6_2B, TSP_RESET_WB_DMA_FSM_TSIF3);
                break;
            default:
                HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupported File Eng: %uld !\n",__FILE__,__FUNCTION__,__LINE__,(MS_U32)eFileEng));
                break;
    ...
    if(bEnable)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_W(&_RegCtrl2->CFG_77, (u32Key & CFG_77_FI_MOBF_INDEC_TSIF3_MASK));
                break;
            default:
                break;
        }
...
TSP_HAL_Result HAL_TSP_SecFlt_SetType(REG_SecFlt *pSecFlt, MS_U32 u32FltType)
{
    if(u32FltType > TSP_SECFLT_USER_VER)
        return E_TSP_HAL_NOT_SUPPORTED;
...
MS_BOOL HAL_TSP_SecFlt_GetMask(REG_SecFlt *pSecFlt, MS_U8 *pu8Mask)
{
    MS_U32 i;
    MS_U32 u32Temp;
    MS_U32 j = 0;
    for (i = 0; i < (TSP_FILTER_DEPTH/sizeof(MS_U32)); i++)
    {
        u32Temp = TSP32_IdrR((TSP32 *)&pSecFlt->Mask[i]);
        pu8Mask[j++] = (MS_U8)u32Temp;
        pu8Mask[j++] = (MS_U8)(u32Temp >> 8);
        pu8Mask[j++] = (MS_U8)(u32Temp >> 16);
        pu8Mask[j++] = (MS_U8)(u32Temp >> 24);
    }
    return TRUE;
}

MS_BOOL HAL_TSP_SecFlt_GetMatch(REG_SecFlt *pSecFlt, MS_U8 *pu8Match)
{
    MS_U32 i;
    MS_U32 u32Temp;
    MS_U32 j = 0;
    ...
    for (i = 0; i < (TSP_FILTER_DEPTH/sizeof(MS_U32)); i++)
    {
        u32Temp = TSP32_IdrR((TSP32 *)&pSecFlt->Match[i]);
        pu8Match[j++] = (MS_U8)u32Temp;
        pu8Match[j++] = (MS_U8)(u32Temp >> 8);
        pu8Match[j++] = (MS_U8)(u32Temp >> 16);
        pu8Match[j++] = (MS_U8)(u32Temp >> 24);
    }
    return TRUE;

}

MS_BOOL HAL_TSP_SecFlt_GetNMask(REG_SecFlt *pSecFlt, MS_U8 *pu8NMask)
{
    MS_U32 i;
    MS_U32 u32Temp;
    MS_U32 j = 0;
    ...
    for (i = 0; i < (TSP_FILTER_DEPTH/sizeof(MS_U32)); i++)
    {
        u32Temp = TSP32_IdrR((TSP32 *)&pSecFlt->NMask[i]);
        pu8NMask[j++] = (MS_U8)u32Temp;
        pu8NMask[j++] = (MS_U8)(u32Temp >> 8);
        pu8NMask[j++] = (MS_U8)(u32Temp >> 16);
        pu8NMask[j++] = (MS_U8)(u32Temp >> 24);
    }
    return TRUE;
}
#endif
...
void HAL_TSP_SecBuf_SetBuf(REG_SecBuf *pSecBuf, MS_U32 u32StartAddr, MS_U32 u32BufSize)
{
    ...
    REG16_W(&_RegCtrl6->CFG6_2D_REG_MIU_SEL_FW, (REG16_R(&_RegCtrl6->CFG6_2D_REG_MIU_SEL_FW) & (~REG_MIU_SEL_SECTION_MASK)) | ((u8MiuSel << REG_MIU_SEL_SECTION_SHIFT) & REG_MIU_SEL_SECTION_MASK));
...
void HAL_TSP_PcrFlt_Enable(MS_U32 pcrFltId, MS_BOOL bEnable)
{
    switch(pcrFltId)
    {
        ...
        case 2:
            if(bEnable)
            {
                REG16_SET(&_RegCtrl8_1->CFG8_44, CFG8_44_PIDFLT_PCR2_EN);
            }
            else
            {
                REG16_CLR(&_RegCtrl8_1->CFG8_44, CFG8_44_PIDFLT_PCR2_EN);
            }
            break;
        case 3:
            if(bEnable)
            {
                REG16_SET(&_RegCtrl8_1->CFG8_45, CFG8_45_PIDFLT_PCR3_EN);
            }
            else
            {
                REG16_CLR(&_RegCtrl8_1->CFG8_45, CFG8_45_PIDFLT_PCR3_EN);
            }
            break;
        default:
            break;
...
void HAL_TSP_PcrFlt_DssEnable(MS_U32 pcrFltId, MS_BOOL bEnable)
{
    // not support
}
...
void HAL_TSP_PcrFlt_SetPid(MS_U32 pcrFltId, MS_U32 u32Pid)
{
    switch(pcrFltId)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_44, CFG8_44_PIDFLT_PCR2_PID_MASK, u32Pid);
            break;
        case 3:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_45, CFG8_45_PIDFLT_PCR3_PID_MASK, u32Pid);
            break;
        default:
            break;
    }
...
MS_U32 HAL_TSP_PcrFlt_GetPid(MS_U32 pcrFltId)
{
    switch(pcrFltId)
    {
        ...
        case 2:
            return (REG16_R(&_RegCtrl8_1->CFG8_44) & CFG8_44_PIDFLT_PCR2_PID_MASK);
        case 3:
            return (REG16_R(&_RegCtrl8_1->CFG8_45) & CFG8_45_PIDFLT_PCR3_PID_MASK);
        default:
...
void HAL_TSP_PcrFlt_SetSrc(MS_U32 pcrFltId, TSP_PCR_SRC src)
{
    switch(pcrFltId)
    {
        ...
        case 2:
            //src 2
            REG16_MSK_W(&_RegCtrl8_1->CFG8_42, CFG8_42_PCR2_SRC_MASK, src << CFG8_42_PCR2_SRC_SHIFT);
            break;
        case 3:
            //src 3
            REG16_MSK_W(&_RegCtrl8_1->CFG8_43, CFG8_43_PCR3_SRC_MASK, src << CFG8_43_PCR3_SRC_SHIFT);
            break;
        default:
            break;
    }
...
void HAL_TSP_PcrFlt_SetSrcId(MS_U32 pcrFltId, MS_U32 u32SrcId)
{
    ...
    {
        case 0:
            REG16_MSK_W(&_RegCtrl3->CFG3_72, CFG3_72_PIDFLT_PCR0_SRC_ID_MASK, u32SrcId<<CFG3_72_PIDFLT_PCR0_SRC_ID_SHIFT);
            break;
        case 1:
            REG16_MSK_W(&_RegCtrl3->CFG3_72, CFG3_72_PIDFLT_PCR1_SRC_ID_MASK, u32SrcId<<CFG3_72_PIDFLT_PCR1_SRC_ID_SHIFT);
            break;
        case 2:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_46, CFG8_46_REG_PIDFLT_PCR2_SRC_ID_MASK, u32SrcId<<CFG8_46_REG_PIDFLT_PCR2_SRC_ID_SHIFT);
            break;
        case 3:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_46, CFG8_46_REG_PIDFLT_PCR3_SRC_ID_MASK, u32SrcId<<CFG8_46_REG_PIDFLT_PCR3_SRC_ID_SHIFT);
            break;
        default:
...
void HAL_TSP_PcrFlt_GetSrc(MS_U32 pcrFltId, TSP_PCR_SRC *pPcrSrc)
{
    switch(pcrFltId)
    {
        ...
        case 2:
            //src 2
            *pPcrSrc = (REG16_R(&_RegCtrl8_1->CFG8_42) & CFG8_42_PCR2_SRC_MASK) >> CFG8_42_PCR2_SRC_SHIFT;
            break;
        case 3:
            //src 3
            *pPcrSrc = (REG16_R(&_RegCtrl8_1->CFG8_43) & CFG8_43_PCR3_SRC_MASK) >> CFG8_43_PCR3_SRC_SHIFT;
            break;
        default:
            break;
    }
    ...
    switch(pcrFltId)
    {
        ...
        case 1:
            ...
            *pu32Pcr_H = REG32_R(&_RegCtrl->HWPCR1_H) & 0x1;
            ...
            break;
        case 2:
            REG16_SET(&_RegCtrl8_1->CFG8_42, CFG8_42_REG_PCR2_READ);
            *pu32Pcr = REG32_R(&_RegCtrl8_1->CFG8_60_61);
            *pu32Pcr_H = (MS_U32)(REG16_R(&_RegCtrl8_1->CFG8_62) & 0x1);
            REG16_CLR(&_RegCtrl8_1->CFG8_42, CFG8_42_REG_PCR2_READ);
            break;
        case 3:
            REG16_SET(&_RegCtrl8_1->CFG8_43, CFG8_43_REG_PCR3_READ);
            *pu32Pcr = REG32_R(&_RegCtrl8_1->CFG8_63_64);
            *pu32Pcr_H = (MS_U32)(REG16_R(&_RegCtrl8_1->CFG8_65) & 0x1);
            REG16_CLR(&_RegCtrl8_1->CFG8_43, CFG8_43_REG_PCR3_READ);
            break;
...
void HAL_TSP_PcrFlt_Reset(MS_U32 pcrFltId)
{
    switch(pcrFltId)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_CLR(&_RegCtrl8_1->CFG8_42, CFG8_42_REG_PCR2_RESET);
            REG16_SET(&_RegCtrl8_1->CFG8_42, CFG8_42_REG_PCR2_RESET);
            break;
        case 3:
            REG16_CLR(&_RegCtrl8_1->CFG8_43, CFG8_43_REG_PCR3_RESET);
            REG16_SET(&_RegCtrl8_1->CFG8_43, CFG8_43_REG_PCR3_RESET);
            break;
        default:
            break;
    }
...
void HAL_TSP_PcrFlt_ClearInt(MS_U32 pcrFltId)
{
    ...
    {
        case 0:
            //REG16_CLR(&_RegCtrl->HwInt2_Stat,TSP_HWINT2_PCR0_UPDATE_END);
            REG16_W(&_RegCtrl->HwInt2_Stat,
            (REG16_R(&_RegCtrl->HwInt2_Stat) & (~TSP_HWINT2_STATUS_MASK)) |
            (~TSP_HWINT2_PCR0_UPDATE_END & TSP_HWINT2_STATUS_MASK));
            break;
        case 1:
            //REG16_CLR(&_RegCtrl->HwInt2_Stat,TSP_HWINT2_PCR1_UPDATE_END);
            REG16_W(&_RegCtrl->HwInt2_Stat,
            (REG16_R(&_RegCtrl->HwInt2_Stat) & (~TSP_HWINT2_STATUS_MASK)) |
            (~TSP_HWINT2_PCR1_UPDATE_END & TSP_HWINT2_STATUS_MASK));
            break;
        case 2:
            //REG16_CLR(&_RegCtrl->HwInt3_Stat,TSP_HWINT2_PCR2_UPDATE_END);
            REG16_W(&_RegCtrl->HwInt3_Stat,
            (REG16_R(&_RegCtrl->HwInt3_Stat) & (~TSP_HWINT3_STATUS_MASK)) |
            (~TSP_HWINT3_PCR2_UPDATE_END & TSP_HWINT3_STATUS_MASK));
            break;
        case 3:
            //REG16_CLR(&_RegCtrl->HwInt3_Stat,TSP_HWINT2_PCR3_UPDATE_END);
            REG16_W(&_RegCtrl->HwInt3_Stat,
            (REG16_R(&_RegCtrl->HwInt3_Stat) & (~TSP_HWINT3_STATUS_MASK)) |
            (~TSP_HWINT3_PCR3_UPDATE_END & TSP_HWINT3_STATUS_MASK));
            break;
        default:
            break;
    }
...
MS_U32 HAL_TSP_PcrFlt_GetIntMask(MS_U32 pcrFltId)
{
    switch(pcrFltId)
    {
        ...
        case 2:
            return (TSP_HWINT3_PCR2_UPDATE_END_EN << 16);
        case 3:
            return (TSP_HWINT3_PCR3_UPDATE_END_EN << 16);
        default:
...
void HAL_TSP_STC64_Set(MS_U32 Eng, MS_U32 stcH, MS_U32 stcL)
{
    switch (Eng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG32_W(&_RegCtrl8_1->CFG8_54_55, stcL);
            REG32_W(&_RegCtrl8_1->CFG8_56_57, stcH);
            REG16_SET(&_RegCtrl->STC_Config, TSP_STC_CFG_SET_TIME_BASE_64b_3);
            REG16_CLR(&_RegCtrl->STC_Config, TSP_STC_CFG_SET_TIME_BASE_64b_3);
            break;
        case 3:
            REG32_W(&_RegCtrl8_1->CFG8_5A_5B, stcL);
            REG32_W(&_RegCtrl8_1->CFG8_5C_5D, stcH);
            REG16_SET(&_RegCtrl->STC_Config, TSP_STC_CFG_SET_TIME_BASE_64b_4);
            REG16_CLR(&_RegCtrl->STC_Config, TSP_STC_CFG_SET_TIME_BASE_64b_4);
            break;
    }
}
    ...
    switch (Eng)
    {
        ...
        case 1:
            ...
            *pStcL = REG32_R(&_RegCtrl->PCR64_2_L);
            ...
            break;
        case 2:
            REG16_CLR(&_RegCtrl->STC_Config, TSP_STC_CFG_CNT64b_3_LD);
            *pStcH = REG32_R(&_RegCtrl8_1->CFG8_56_57);
            *pStcL = REG32_R(&_RegCtrl8_1->CFG8_54_55);
            REG16_SET(&_RegCtrl->STC_Config, TSP_STC_CFG_CNT64b_3_LD);
            break;
        case 3:
            REG16_CLR(&_RegCtrl->STC_Config, TSP_STC_CFG_CNT64b_4_LD);
            *pStcH = REG32_R(&_RegCtrl8_1->CFG8_5C_5D);
            *pStcL = REG32_R(&_RegCtrl8_1->CFG8_5A_5B);
            REG16_SET(&_RegCtrl->STC_Config, TSP_STC_CFG_CNT64b_4_LD);
            break;
    }
}
...
void HAL_TSP_FIFO_SetSrc(TSP_DST_SEQ eFltType, MS_U32 pktDmxId)
{
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D :
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_40, CFG8_40_REG_VID3_SRC_MASK, ((MS_U16)pktDmxId) << CFG8_40_REG_VID3_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_41, CFG8_41_REG_VID4_SRC_MASK, ((MS_U16)pktDmxId) << CFG8_41_REG_VID4_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            REG16_MSK_W(&_RegCtrl->PCR_Cfg, TSP_AUDC_SRC_MASK, ((MS_U16)pktDmxId) << TSP_AUDC_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            REG16_MSK_W(&_RegCtrl->PCR_Cfg, TSP_AUDD_SRC_MASK, ((MS_U16)pktDmxId)  << TSP_AUDD_SRC_SHIFT);
            break;
        default:
            break;
    }
    ...
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D:
            *pktDmxId = ((REG16_R(&_RegCtrl->FIFO_Src)) & TSP_VID3D_SRC_MASK) >> TSP_VID3D_SRC_SHIFT;
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            *pktDmxId = ((REG16_R(&_RegCtrl8_1->CFG8_40)) & CFG8_40_REG_VID3_SRC_MASK) >> CFG8_40_REG_VID3_SRC_SHIFT;
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            *pktDmxId = ((REG16_R(&_RegCtrl8_1->CFG8_41)) & CFG8_41_REG_VID4_SRC_MASK) >> CFG8_41_REG_VID4_SRC_SHIFT;
             break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            *pktDmxId = ((REG16_R(&_RegCtrl->FIFO_Src)) & TSP_AUDB_SRC_MASK) >> TSP_AUDB_SRC_SHIFT;
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            *pktDmxId = ((REG16_R(&_RegCtrl->PCR_Cfg)) & TSP_AUDC_SRC_MASK) >> TSP_AUDC_SRC_SHIFT;
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            *pktDmxId = ((REG16_R(&_RegCtrl->PCR_Cfg)) & TSP_AUDD_SRC_MASK) >> TSP_AUDD_SRC_SHIFT;
            break;
        default:
            break;
    }
...
void HAL_TSP_FIFO_ClearAll()
{
    ...
    REG16_CLR(&_RegCtrl8_1->CFG8_40, CFG8_40_PS_VID3_EN);
    REG16_CLR(&_RegCtrl8_1->CFG8_41, CFG8_41_PS_VID4_EN);
...
void HAL_TSP_FIFO_ReadSrc(TSP_DST_SEQ eFltType)
{
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D:
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            REG16_MSK_W(&_RegCtrl3->CFG3_2C, CFG3_2C_AVFIFO_READ_SEL_MASK, ((MS_U16)CFG3_2C_AVFIFO_READ_SEL_V3) << CFG3_2C_AVFIFO_READ_SEL_SHIFT);
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            REG16_MSK_W(&_RegCtrl3->CFG3_2C, CFG3_2C_AVFIFO_READ_SEL_MASK, ((MS_U16)CFG3_2C_AVFIFO_READ_SEL_V4) << CFG3_2C_AVFIFO_READ_SEL_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO:
        ...
        case E_TSP_DST_FIFO_AUDIO2:
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3:
            REG16_MSK_W(&_RegCtrl3->CFG3_2C, CFG3_2C_AVFIFO_READ_SEL_MASK, ((MS_U16)CFG3_2C_AVFIFO_READ_SEL_AC) << CFG3_2C_AVFIFO_READ_SEL_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO4:
            REG16_MSK_W(&_RegCtrl3->CFG3_2C, CFG3_2C_AVFIFO_READ_SEL_MASK, ((MS_U16)CFG3_2C_AVFIFO_READ_SEL_AD) << CFG3_2C_AVFIFO_READ_SEL_SHIFT);
            break;
        default:
            break;
    }
    ...
    if(bEn)
    {
        switch (eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_SET(&_RegCtrl3->CFG3_2E, CFG3_2E_VPID_3_BYPASS);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_SET(&_RegCtrl3->CFG3_2F, CFG3_2F_VPID_4_BYPASS);
                break;
            case E_TSP_DST_FIFO_AUDIO:
            ...
            case E_TSP_DST_FIFO_AUDIO2:
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3:
                REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_APID_C_BYPASS);
                break;
            case E_TSP_DST_FIFO_AUDIO4:
                REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_APID_D_BYPASS);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch (eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_CLR(&_RegCtrl3->CFG3_2E, CFG3_2E_VPID_3_BYPASS);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_CLR(&_RegCtrl3->CFG3_2F, CFG3_2F_VPID_4_BYPASS);
                break;
            case E_TSP_DST_FIFO_AUDIO:
            ...
            case E_TSP_DST_FIFO_AUDIO2:
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3:
                REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_APID_C_BYPASS);
                break;
            case E_TSP_DST_FIFO_AUDIO4:
                REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_APID_D_BYPASS);
                break;
            default:
                break;
    ...
    if(bEn)
    {
        switch (eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_SET(&_RegCtrl8_1->CFG8_40, CFG8_40_PS_VID3_EN);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_SET(&_RegCtrl8_1->CFG8_41, CFG8_41_PS_VID4_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO:
            ...
            case E_TSP_DST_FIFO_AUDIO2:
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3:
                REG16_SET(&_RegCtrl->Hw_Config4, TSP_HW_CFG4_PS_AUDC_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO4:
                REG16_SET(&_RegCtrl->Hw_Config4, TSP_HW_CFG4_PS_AUDD_EN);
                break;
            default:
                break;
        }
...
void HAL_TSP_FIFO_Bypass_Src(FILEENG_SEQ eFileEng, TSP_DST_SEQ eFltType)
{
    ...
    {
        case E_TSP_DST_FIFO_VIDEO:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_48,CFG8_48_REG_PS_MODE_SRC_V_MASK,  (eFileEng << CFG8_48_REG_PS_MODE_SRC_V_SHIFT));
            break;
        case E_TSP_DST_FIFO_VIDEO3D:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_48,CFG8_48_REG_PS_MODE_SRC_V3D_MASK,(eFileEng << CFG8_48_REG_PS_MODE_SRC_V3D_SHIFT));
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_48,CFG8_48_REG_PS_MODE_SRC_V3_MASK, (eFileEng << CFG8_48_REG_PS_MODE_SRC_V3_SHIFT));
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_48,CFG8_48_REG_PS_MODE_SRC_V4_MASK, (eFileEng << CFG8_48_REG_PS_MODE_SRC_V4_SHIFT));
            break;
        case E_TSP_DST_FIFO_AUDIO:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_47,CFG8_47_REG_PS_MODE_SRC_A_MASK,  (eFileEng << CFG8_47_REG_PS_MODE_SRC_A_SHIFT));
            break;
        case E_TSP_DST_FIFO_AUDIO2:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_47,CFG8_47_REG_PS_MODE_SRC_AD_MASK, (eFileEng << CFG8_47_REG_PS_MODE_SRC_AD_SHIFT));
            break;
        case E_TSP_DST_FIFO_AUDIO3:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_47,CFG8_47_REG_PS_MODE_SRC_AC_MASK, (eFileEng << CFG8_47_REG_PS_MODE_SRC_AC_SHIFT));
            break;
        case E_TSP_DST_FIFO_AUDIO4:
            REG16_MSK_W(&_RegCtrl8_1->CFG8_47,CFG8_47_REG_PS_MODE_SRC_ADD_MASK,(eFileEng << CFG8_47_REG_PS_MODE_SRC_ADD_SHIFT));
            break;
        default:
            break;
    }
...
MS_U32 HAL_TSP_FIFO_PidHit(TSP_DST_SEQ eFltType)
{
    ...
    case E_TSP_DST_FIFO_VIDEO3 :
        return REG16_R(&_RegCtrl2->CFG_7D) & CFG_7D_MATCHECED_VPID_3_MASK;
    case E_TSP_DST_FIFO_VIDEO4 :
        return REG16_R(&_RegCtrl2->CFG_7E) & CFG_7E_MATCHECED_VPID_4_MASK;
    case E_TSP_DST_FIFO_AUDIO :
        return REG16_R(&_RegCtrl->Aud_Pid_Hit) & TSP_APID_MASK;
    case E_TSP_DST_FIFO_AUDIO2 :
        return REG16_R(&_RegCtrl2->CFG_71) & CFG_71_MATCHECED_APID_B_MASK;
    case E_TSP_DST_FIFO_AUDIO3 :
        return REG16_R(&_RegCtrl2->CFG_74) & CFG_74_MATCHECED_APID_C_MASK;
    case E_TSP_DST_FIFO_AUDIO4 :
        return REG16_R(&_RegCtrl2->CFG_7C) & CFG_7C_MATCHECED_APID_D_MASK;
    default:
        return PVR_PIDFLT_DEF;
    }
}
    ...
    if (bReset)
    {
        switch (eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_SET(&_RegCtrl8_1->CFG8_40, CFG8_40_RESET_VFIFO_3);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_SET(&_RegCtrl8_1->CFG8_41, CFG8_41_RESET_VFIFO_4);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_SET(&_RegCtrl->reg160E, TSP_RESET_AFIFO3);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_SET(&_RegCtrl->PktChkSizeFilein, TSP_RESET_AFIFO4);
                break;
            default :
                break;
        }
    ...
    else
    {
        switch (eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_CLR(&_RegCtrl8_1->CFG8_40, CFG8_40_RESET_VFIFO_3);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_CLR(&_RegCtrl8_1->CFG8_41, CFG8_41_RESET_VFIFO_4);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_CLR(&_RegCtrl->reg160E, TSP_RESET_AFIFO3);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_CLR(&_RegCtrl->PktChkSizeFilein, TSP_RESET_AFIFO4);
                break;
            default :
                break;
        }
    ...
    if(bSkip)
    {
        switch(eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_SET(&_RegCtrl8_1->CFG8_40, CFG8_40_MASK_SCR_V3_EN);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_SET(&_RegCtrl8_1->CFG8_45, CFG8_41_MASK_SCR_V4_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO:
            ...
            case E_TSP_DST_FIFO_AUDIO2:
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3:
                REG16_SET(&_RegCtrl3->CFG3_34, CFG3_34_MASK_SRC_AC_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO4:
                REG16_SET(&_RegCtrl3->CFG3_34, CFG3_34_MASK_SRC_AD_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_CLR(&_RegCtrl8_1->CFG8_40, CFG8_40_MASK_SCR_V3_EN);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_CLR(&_RegCtrl8_1->CFG8_45, CFG8_41_MASK_SCR_V4_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO:
            ...
            case E_TSP_DST_FIFO_AUDIO2:
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3:
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_MASK_SRC_AC_EN);
                break;
            case E_TSP_DST_FIFO_AUDIO4:
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_MASK_SRC_AD_EN);
                break;
            default:
                break;
        }
    ...
    if(bDisable)
    {
        switch(eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_SET(&_RegCtrl8_1->CFG8_40, CFG8_40_V3_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_SET(&_RegCtrl8_1->CFG8_41, CFG8_41_V4_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG32_SET(&_RegCtrl->PVR2_Config, TSP_AC_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG32_SET(&_RegCtrl->PVR2_Config, TSP_ADD_BLOCK_DIS);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFltType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D:
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3:
                REG16_CLR(&_RegCtrl8_1->CFG8_40, CFG8_40_V3_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_VIDEO4:
                REG16_CLR(&_RegCtrl8_1->CFG8_41, CFG8_41_V4_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG32_CLR(&_RegCtrl->PVR2_Config, TSP_AC_BLOCK_DIS);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG32_CLR(&_RegCtrl->PVR2_Config, TSP_ADD_BLOCK_DIS);
                break;
            default:
                break;
        }
...
MS_BOOL HAL_TSP_FIFO_IsReset(TSP_DST_SEQ eFltType)
{
    ...
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D:
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3 :
            u32Matched = REG16_R(&_RegCtrl8_1->CFG8_40) & CFG8_40_RESET_VFIFO_3;
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            u32Matched = REG16_R(&_RegCtrl8_1->CFG8_41) & CFG8_41_RESET_VFIFO_4;
            break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            u32Matched = REG16_R(&_RegCtrl->reg160E) & TSP_RESET_AFIFO3;
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            u32Matched = REG16_R(&_RegCtrl->PktChkSizeFilein) & TSP_RESET_AFIFO4;
            break;
        default :
            return FALSE;
    }
...
MS_U32 HAL_TSP_FIFO_Level(TSP_DST_SEQ eFltType)
{
    ...
    case E_TSP_DST_FIFO_VIDEO3 :
        return (REG16_R(&_RegCtrl3->CFG3_4C) & CFG3_4C_TSP_VFIFO3_LEVEL)  >> CFG3_4C_TSP_VFIFO3_LEVEL_SHFT;
    case E_TSP_DST_FIFO_VIDEO4 :
        return (REG16_R(&_RegCtrl3->CFG3_4D) & CFG3_4D_TSP_VFIFO4_LEVEL)  >> CFG3_4D_TSP_VFIFO4_LEVEL_SHFT;
    case E_TSP_DST_FIFO_AUDIO :
    ...
    case E_TSP_DST_FIFO_AUDIO3 :
        return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOC_LEVEL)  >> TSP_AFIFOC_LEVEL_SHFT;
    case E_TSP_DST_FIFO_AUDIO4:
        return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOD_LEVEL)  >> TSP_AFIFOD_LEVEL_SHFT;
    default :
        return 0;
    }
}
...
MS_BOOL HAL_TSP_FIFO_Overflow(TSP_DST_SEQ eFltType)
{
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3 :
            return (REG16_R(&_RegCtrl3->CFG3_4C) & CFG3_4C_TSP_VFIFO3_FULL)  >> CFG3_4C_TSP_VFIFO3_FULL_SHFT;
        case E_TSP_DST_FIFO_VIDEO4 :
            return (REG16_R(&_RegCtrl3->CFG3_4C) & CFG3_4D_TSP_VFIFO4_FULL)  >> CFG3_4D_TSP_VFIFO4_FULL_SHFT;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO3 :
            return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOC_FULL)  >> TSP_AFIFOC_FULL_SHFT;
        case E_TSP_DST_FIFO_AUDIO4 :
            return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOD_FULL)  >> TSP_AFIFOD_FULL_SHFT;
        default :
            return FALSE;
    }
}
...
MS_BOOL HAL_TSP_FIFO_Empty(TSP_DST_SEQ eFltType)
{
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3 :
            return (REG16_R(&_RegCtrl3->CFG3_4C) & CFG3_4C_TSP_VFIFO3_EMPTY)  >> CFG3_4C_TSP_VFIFO3_EMPTY_SHFT;
        case E_TSP_DST_FIFO_VIDEO4 :
            return (REG16_R(&_RegCtrl3->CFG3_4D) & CFG3_4D_TSP_VFIFO4_EMPTY)  >> CFG3_4D_TSP_VFIFO4_EMPTY_SHFT;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO3 :
            return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOC_EMPTY)  >> TSP_AFIFOC_EMPTY_SHFT;
        case E_TSP_DST_FIFO_AUDIO4 :
            return (REG16_R(&_RegCtrl3->CFG3_33) & TSP_AFIFOD_EMPTY)  >> TSP_AFIFOD_EMPTY_SHFT;
        default :
            return FALSE;
    }
}
...
static MS_BOOL _HAL_TSP_VQ_TxConfig(MS_U32 vqId)
{
    ...
    switch(vqId)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            REG16_W(&_RegCtrl->VQ3_Config, (REG16_R(&_RegCtrl->VQ3_Config) & (~TSP_VQ3_WR_THRESHOLD_MASK)) | ((0x8 << TSP_VQ3_WR_THRESHOLD_SHIFT) & TSP_VQ3_WR_THRESHOLD_MASK));
            REG16_W(&_RegCtrl->VQ3_Config, (REG16_R(&_RegCtrl->VQ3_Config) & (~TSP_VQ3_FORCEFIRE_CNT_1K_MASK)) | ((0xC << TSP_VQ3_FORCEFIRE_CNT_1K_SHIFT) & TSP_VQ3_FORCEFIRE_CNT_1K_MASK));
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSP_SetVQ( MS_PHYADDR u32BaseAddr, MS_U32 u32BufLen)
{
    ...
    REG16_W(&_RegCtrl6->CFG6_2D_REG_MIU_SEL_FW, (REG16_R(&_RegCtrl6->CFG6_2D_REG_MIU_SEL_FW) & (~REG_MIU_SEL_VQ_MASK)) | ((u8MiuSel << REG_MIU_SEL_VQ_SHIFT) & REG_MIU_SEL_VQ_MASK));
...
MS_BOOL HAL_TSP_VQ_Buffer(MS_U32 vqId, MS_PHYADDR u32BaseAddr, MS_U32 u32VQ_PktNum)
{
    switch(vqId)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            REG32_W(&_RegCtrl->VQ3_Base, (u32BaseAddr >> MIU_BUS));
            REG16_W(&_RegCtrl->VQ3_Size, u32VQ_PktNum);
            break;
        default:
            return FALSE;
    }
    ...
    if(bDis == TRUE)
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl->reg160E, TSP_VQTX3_BLOCK_DIS);
                break;
            default:
                return FALSE;
        }
    ...
    else
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl->reg160E, TSP_VQTX3_BLOCK_DIS);
                break;
            default:
                return FALSE;
        }
    ...
    if(bEn)
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
          case 3:
                REG16_SET(&_RegCtrl->VQ3_Config, TSP_VQ3_RESET);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl->VQ3_Config, TSP_VQ3_RESET);
                break;
            default:
                break;
        }
    ...
    if(bEn)
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl->VQ3_Config, TSP_VQ3_OVF_INT_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl->VQ3_Config, TSP_VQ3_OVF_INT_EN);
                break;
            default:
                break;
        }
    ...
    if(bEn)
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl->VQ3_Config, TSP_VQ3_CLR_OVF_INT);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(vqId)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl->VQ3_Config, TSP_VQ3_CLR_OVF_INT);
                break;
            default:
                break;
        }
...
void HAL_PVR_Init(MS_U32 u32PVREng, MS_U32 pktDmxId)
{
    switch(u32PVREng)
    {
        ...
        case 2:
            // PVR 3
            REG16_MSK_W(&(_RegCtrl2->CFG_15), CFG_15_PVR3_SRC, pktDmxId << CFG_15_PVR3_SRC_SHIFT);
            REG16_SET(&(_RegCtrl2->CFG_16), CFG_16_PVR3_REG_PINGPONG_EN);
            break;
        case 3:
            // PVR 3
            REG16_MSK_W(&(_RegCtrl2->CFG_15), CFG_15_PVR4_SRC, pktDmxId << CFG_15_PVR4_SRC_SHIFT);
            REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_PVR4_REG_PINGPONG_EN);
            break;
        default:
            break;
    }
...
void HAL_PVR_Exit(MS_U32 u32PVREng)
{
    switch(u32PVREng)
    {
        ...
        case 2:
            //reset pvr control registers
            REG16_CLR(&(_RegCtrl2->CFG_15), CFG_15_PVR3_SRC);
            REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_PVR3_REG_PINGPONG_EN);

            //reset write address
            REG16_SET(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_RST_WADR);
            REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_RST_WADR);
            break;
        case 3:
            //reset pvr control registers
            REG16_CLR(&(_RegCtrl2->CFG_15), CFG_15_PVR4_SRC);
            REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_PVR4_REG_PINGPONG_EN);

            //reset write address
            REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_RST_WADR);
            REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_RST_WADR);
            break;
        default:
            break;
    }

    if(_stPvrBuf[u32PVREng].bPA2KSEG1_Mapping== TRUE)
    {
        _stPvrBuf[u32PVREng].bOverWrite = FALSE;
    }
...
void HAL_PVR_Start(MS_U32 u32PVREng)
{
    switch(u32PVREng)
    {
        ...
        case 2:
            ...
            //enable string to miu
            REG16_SET(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_EN);
            break;
        case 3:
            //reset write address
            REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_RST_WADR);
            ...
            //enable string to miu
            REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_EN);
            break;
        default:
            break;
    }
    if(_stPvrBuf[u32PVREng].bPA2KSEG1_Mapping== TRUE)
    {
        if((E_TSP_PVR_PVRENG_START <= u32PVREng) && (E_TSP_PVR_PVRENG_END > u32PVREng)) // PVR Eng number check
        {
            _stPvrBuf[u32PVREng].bOverWrite = FALSE; // Set PVR buffer to non-OverWrite state
        }
    }
}
...
void HAL_PVR_Stop(MS_U32 u32PVREng)
{
    switch(u32PVREng)
    {
        ...
        case 2:
            //clear string to miu
            REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_EN);
            break;
        case 3:
            //clear string to miu
            REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_EN);
            break;
        default:
            break;
    }
    ...
    if(bPause)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_PAUSE);
                break;
            case 3:
                REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_PAUSE);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_PVR3_STR2MI_PAUSE);
                break;
            case 3:
                REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_PVR4_STR2MI_PAUSE);
                break;
            default:
                break;
        }
    ...
    if(bSet)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_REC_ALL3);
                break;
            case 3:
                REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_REC_ALL4);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&(_RegCtrl2->CFG_16), CFG_16_REC_ALL3);
                break;
            case 3:
                REG16_SET(&(_RegCtrl2->CFG_23), CFG_23_REC_ALL4);
                break;
            default:
                break;
        }
...
void HAL_PVR_RecTsMode(MS_U32 u32PVREng, MS_BOOL bSet)
{
    if(bSet)
    {
        switch(u32PVREng)
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&(_RegCtrl2->CFG_16), CFG_16_PID_BYPASS3_REC);
                break;
            case 3:
                REG16_CLR(&(_RegCtrl2->CFG_23), CFG_23_PID_BYPASS4_REC);
                break;
            default:
                break;
        }
...
void HAL_PVR_SetBuf(MS_U32 u32PVREng , MS_U32 u32StartAddr0, MS_U32 u32BufSize0, MS_U32 u32StartAddr1, MS_U32 u32BufSize1)
{
    ...
    MS_U32 u32EndAddr0 = phyMiuOffsetPvrBuf0 + u32BufSize0;
    MS_U32 u32EndAddr1 = phyMiuOffsetPvrBuf1 + u32BufSize1;
    //MS_U32  u32Temp;
    switch(u32PVREng)
    {
        case 0:
            // Select MIU
            REG16_W(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ, (REG16_R(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ) & (~REG_MIU_SEL_PVR1_MASK)) | ((u8MiuSel << REG_MIU_SEL_PVR1_SHIFT) & REG_MIU_SEL_PVR1_MASK));

            //head1
            REG32_W(&_RegCtrl->TsRec_Head, (phyMiuOffsetPvrBuf0 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //end1
            REG32_W(&(_RegCtrl->TsRec_Tail), (u32EndAddr0 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //mid1
            REG32_W(&(_RegCtrl->TsRec_Mid_PVR1_WPTR), (phyMiuOffsetPvrBuf0 >>MIU_BUS) & TSP_STR2MI2_ADDR_MASK);

            //head2
            REG32_W(&_RegCtrl->Str2mi_head2pvr1, (phyMiuOffsetPvrBuf1>> MIU_BUS) & TSP_HW_PVR1_BUF_HEAD2_MASK);
            //end2
            REG32_W(&(_RegCtrl->Str2mi_tail2pvr1), (u32EndAddr1 >> MIU_BUS) &TSP_HW_PVR1_BUF_TAIL2_MASK);
            //mid2
            REG32_W(&(_RegCtrl->Str2mi_mid2pvr1), (phyMiuOffsetPvrBuf1>>MIU_BUS) & TSP_HW_PVR1_BUF_MID2_MASK);
            break;
        case 1:
            // Select MIU
            REG16_W(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ, (REG16_R(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ) & (~REG_MIU_SEL_PVR2_MASK)) | ((u8MiuSel << REG_MIU_SEL_PVR2_SHIFT) & REG_MIU_SEL_PVR2_MASK));

            //head1
            REG32_W(&_RegCtrl->Str2mi_head1_pvr2, (phyMiuOffsetPvrBuf0 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //end1
            REG32_W(&_RegCtrl->Str2mi_tail1_pvr2, (u32EndAddr0>> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //mid1
            REG32_W(&(_RegCtrl->Str2mi_mid1_wptr_pvr2), (phyMiuOffsetPvrBuf0 >>MIU_BUS) & TSP_STR2MI2_ADDR_MASK);

            //head2
            REG32_W(&_RegCtrl->Str2mi_head2_pvr2, (phyMiuOffsetPvrBuf1 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //end2
            REG32_W(&_RegCtrl->Str2mi_tail2_pvr2, (u32EndAddr1 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            //mid2
            REG32_W(&(_RegCtrl->Str2mi_mid2_pvr2), (phyMiuOffsetPvrBuf1 >>MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            break;
        case 2:
            // Select MIU
            REG16_W(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ, (REG16_R(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ) & (~REG_MIU_SEL_PVR3_MASK)) | ((u8MiuSel << REG_MIU_SEL_PVR3_SHIFT) & REG_MIU_SEL_PVR3_MASK));

            //head1
            REG32_W(&(_RegCtrl2->CFG_17_18), (phyMiuOffsetPvrBuf0 >> MIU_BUS) & CFG_17_18_PVR3_STR2MI_HEAD);
            //end1
            REG32_W(&(_RegCtrl2->CFG_1B_1C), (u32EndAddr0 >> MIU_BUS) & CFG_1B_1C_PVR3_STR2MI_TAIL);
            //mid1
            REG32_W(&(_RegCtrl2->CFG_19_1A), (phyMiuOffsetPvrBuf0 >> MIU_BUS) & CFG_19_1A_PVR3_STR2MI_MID);

            //head2
            REG32_W(&(_RegCtrl2->CFG_1D_1E), (phyMiuOffsetPvrBuf1 >> MIU_BUS) & CFG_1D_1E_PVR3_STR2MI_HEAD2);
            //end2
            REG32_W(&(_RegCtrl2->CFG_21_22), (u32EndAddr1 >> MIU_BUS) & CFG_21_22_PVR3_STR2MI_TAIL2);
            //mid2
            REG32_W(&(_RegCtrl2->CFG_1F_20), (phyMiuOffsetPvrBuf1 >> MIU_BUS) & CFG_1F_20_PVR3_STR2MI_MID2);
            break;
        case 3:
            // Select MIU
            REG16_W(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ, (REG16_R(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ) & (~REG_MIU_SEL_PVR4_MASK)) | ((u8MiuSel << REG_MIU_SEL_PVR4_SHIFT) & REG_MIU_SEL_PVR4_MASK));

            //head1
            REG32_W(&(_RegCtrl2->CFG_24_25), (phyMiuOffsetPvrBuf0>>MIU_BUS) & CFG_24_25_PVR4_STR2MI_HEAD);
            //end1
            REG32_W(&(_RegCtrl2->CFG_28_29), (u32EndAddr0>>MIU_BUS) & CFG_28_29_PVR4_STR2MI_TAIL);
            //mid1
            REG32_W(&(_RegCtrl2->CFG_26_27), (phyMiuOffsetPvrBuf0>>MIU_BUS) & CFG_26_27_PVR4_STR2MI_MID);

            //head2
            REG32_W(&(_RegCtrl2->CFG_2A_2B), (phyMiuOffsetPvrBuf1>>MIU_BUS) & CFG_2A_2B_PVR4_STR2MI_HEAD2);
            //end2
            REG32_W(&(_RegCtrl2->CFG_2E_2F), (u32EndAddr1>>MIU_BUS) & CFG_2E_2F_PVR4_STR2MI_TAIL2);
            //mid2
            REG32_W(&(_RegCtrl2->CFG_2C_2D), (phyMiuOffsetPvrBuf1>>MIU_BUS) & CFG_2C_2D_PVR4_STR2MI_MID2);
            break;
...
void HAL_PVR_SetStr2Miu_StartAddr(MS_U32 u32PVREng , MS_U32 u32StartAddr0, MS_U32 u32StartAddr1)
{
    ...
    MS_VIRT virtStart = MsOS_PA2KSEG1(u32StartAddr0);
    //PA2KSEG1 mapping detection, 0 : not mapping.
    if(0 == virtStart)
    {
        HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_INFO, E_HAL_TSP_DBG_MODEL_ALL, printf("[PA2KSEG1_Mapping][%s][%d] PA2KSEG1_Mapping fail !!\n",__FUNCTION__,__LINE__));
        _stPvrBuf[u32PVREng].bPA2KSEG1_Mapping= FALSE;
    }
    else
    {
        if((E_TSP_PVR_PVRENG_START <= u32PVREng) && (E_TSP_PVR_PVRENG_END > u32PVREng)) // PVR Eng number check
        {
            _stPvrBuf[u32PVREng].u32Start = u32StartAddr0;

            if( (!_stPvrBuf[u32PVREng].bOverWrite) && (_stPvrBuf[u32PVREng].u32Start!= 0) )
            {
                *((MS_U64*)(MsOS_PA2KSEG1(_stPvrBuf[u32PVREng].u32Start)))= PVR_NON_OVERWRITE;
            }
        }
        _stPvrBuf[u32PVREng].bPA2KSEG1_Mapping= TRUE;
    }
}
...
void HAL_PVR_SetStr2Miu_EndAddr(MS_U32 u32PVREng , MS_U32 u32EndAddr0, MS_U32 u32EndAddr1)
{
    ...
    switch(u32PVREng)
    {
        case 0:
            //end1
            REG32_W(&(_RegCtrl->TsRec_Tail), (phyMiuOffsetPvrBuf0 >> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);

            //end2
            REG32_W(&(_RegCtrl->Str2mi_tail2pvr1), (phyMiuOffsetPvrBuf1 >> MIU_BUS) &TSP_HW_PVR1_BUF_TAIL2_MASK);
            break;
        case 1:
            //end1
            REG32_W(&_RegCtrl->Str2mi_tail1_pvr2, (phyMiuOffsetPvrBuf0>> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);

            //end2
            REG32_W(&_RegCtrl->Str2mi_tail2_pvr2, (phyMiuOffsetPvrBuf1>> MIU_BUS) & TSP_STR2MI2_ADDR_MASK);
            break;
        case 2:
            //end1
            REG32_W(&(_RegCtrl2->CFG_1B_1C), (phyMiuOffsetPvrBuf0>>MIU_BUS) & CFG_1B_1C_PVR3_STR2MI_TAIL);

            //end2
            REG32_W(&(_RegCtrl2->CFG_21_22), (phyMiuOffsetPvrBuf1>>MIU_BUS) & CFG_21_22_PVR3_STR2MI_TAIL2);
            break;
        case 3:
            //end1
            REG32_W(&(_RegCtrl2->CFG_28_29), (phyMiuOffsetPvrBuf0>>MIU_BUS) & CFG_28_29_PVR4_STR2MI_TAIL);

            //end2
            REG32_W(&(_RegCtrl2->CFG_2E_2F), (phyMiuOffsetPvrBuf1>>MIU_BUS) & CFG_2E_2F_PVR4_STR2MI_TAIL2);
            break;
        default:
            break;
    }
    if(_stPvrBuf[u32PVREng].bPA2KSEG1_Mapping == TRUE)
    {
        if((E_TSP_PVR_PVRENG_START <= u32PVREng) && (E_TSP_PVR_PVRENG_END > u32PVREng)) // PVR Eng number check
        {
            _stPvrBuf[u32PVREng].u32End = u32EndAddr1;
        }
    }
}
...
MS_U32 HAL_PVR_GetWritePtr(MS_U32 u32PVREng)
{
    ...
    REG16_CLR(&_RegCtrl->CH_BW_CTRL, TSP_CH_BW_WP_LD);

    switch(u32PVREng)
    {
        case 0:
            WritePtr = REG32_R(&_RegCtrl->TsRec_Mid_PVR1_WPTR) << MIU_BUS;
            break;
        case 1:
            WritePtr = REG32_R(&_RegCtrl->Str2mi_mid1_wptr_pvr2) << MIU_BUS;
            break;
        case 2:
            WritePtr = REG32_R(&(_RegCtrl2->CFG_66_67)) << MIU_BUS;
            break;
        case 3:
            WritePtr = REG32_R(&(_RegCtrl2->CFG_68_69)) << MIU_BUS;
            break;
        default:
            WritePtr = 0;
            break;
    ...
    REG16_SET(&_RegCtrl->CH_BW_CTRL, TSP_CH_BW_WP_LD);

    return _Adjust_PVR_WritePtr(u32PVREng, WritePtr);
}
...
void HAL_PVR_GetEngSrc(MS_U32 u32EngDst, TSP_SRC_SEQ *eSrc)
{
    ...
    switch(u32EngDst)
    {
        ...
        case 2:
            // PVR 3
            *eSrc = ((REG16_R(&(_RegCtrl2->CFG_15 )) & CFG_15_PVR3_SRC)   >> CFG_15_PVR3_SRC_SHIFT);
            break;
        case 3:
            // PVR 4
            *eSrc = ((REG16_R(&(_RegCtrl2->CFG_15 )) & CFG_15_PVR4_SRC)   >> CFG_15_PVR4_SRC_SHIFT);
            break;
        default:
            break;
    }
...
TSP_HAL_Result HAL_PVR_ClkGating(MS_U32 u32PVREng, MS_BOOL bEnable)
{
    return E_TSP_HAL_NOT_SUPPORTED;
}
    ...
    if(bSet)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET((&_RegCtrl2->CFG_16), CFG_16_PVR3_PKT192_EN);
                break;
            case 3:
                REG16_SET((&_RegCtrl2->CFG_23), CFG_23_PVR4_PKT192_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR((&_RegCtrl2->CFG_16), CFG_16_PVR3_PKT192_EN);
                break;
            case 3:
                REG16_CLR((&_RegCtrl2->CFG_23), CFG_23_PVR4_PKT192_EN);
                break;
            default:
                break;
        }
...
void HAL_PVR_SetDSS130Mode(MS_U32 u32PVREng, MS_BOOL bTimestampEn, MS_BOOL bSet)
{
    if(bSet && bTimestampEn)
    {
        HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%d] Not supprt DSS 134 mode !!\n", __FUNCTION__, __LINE__));
        return;
    }

    if(bSet)
    {
        REG16_SET(&_RegCtrl8_1->CFG8_6C, (CFG8_6C_REG_PVR1_PES_DIRECTV_130_MODE << u32PVREng));
    }
    else
    {
        REG16_CLR(&_RegCtrl8_1->CFG8_6C, (CFG8_6C_REG_PVR1_PES_DIRECTV_130_MODE << u32PVREng));
    }
}
...
MS_U32 HAL_PVR_GetPVRTimeStamp(MS_U32 u32PVREng)
{
    ...
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            return u32lpcr;
        case 2:
            REG16_CLR((&_RegCtrl2->CFG_16), CFG_16_PVR3_LPCR1_RLD);

            u32lpcr = REG32_R((&_RegCtrl2->CFG_62_63));

            REG16_SET((&_RegCtrl2->CFG_16), CFG_16_PVR3_LPCR1_RLD);
            return u32lpcr;
        case 3:
            REG16_CLR((&_RegCtrl2->CFG_23), CFG_23_PVR4_LPCR1_RLD);

            u32lpcr = REG32_R((&_RegCtrl2->CFG_64_65));
...
void HAL_PVR_SetPVRTimeStamp(MS_U32 u32PVREng , MS_U32 u32Stamp)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_SET(&_RegCtrl2->CFG_16, CFG_16_PVR3_LPCR1_WLD);

            REG32_W((&_RegCtrl2->CFG_62_63), u32Stamp);

            REG16_CLR(&_RegCtrl2->CFG_16, CFG_16_PVR3_LPCR1_WLD);
            break;
        case 3:
            REG16_SET(&_RegCtrl2->CFG_23, CFG_23_PVR4_LPCR1_WLD);

            REG32_W((&_RegCtrl2->CFG_64_65), u32Stamp);

            REG16_CLR(&_RegCtrl2->CFG_23, CFG_23_PVR4_LPCR1_WLD);
            break;
        default:
            break;
    }
...
void HAL_PVR_SetPVRTimeStamp_Stream(MS_U32 u32PVREng , MS_U32 u32Stamp)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_SET((&_RegCtrl2->CFG_13), CFG_13_LPCR_WLD2);

            REG32_W(&_RegCtrl2->CFG_5A_5B, u32Stamp);

            REG16_CLR((&_RegCtrl2->CFG_13), CFG_13_LPCR_WLD2);
            break;
        case 3:
            REG16_SET((&_RegCtrl2->CFG_13), CFG_13_LPCR_WLD3);

            REG32_W(&_RegCtrl2->CFG_5C_5D, u32Stamp);

            REG16_CLR((&_RegCtrl2->CFG_13), CFG_13_LPCR_WLD3);
            break;
        default:
            break;
    }
    ...
    if(bEnable)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&_RegCtrl2->CFG_16, CFG_16_PVR3_PVR_ALIGN_EN);
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_23, CFG_23_PVR4_PVR_ALIGN_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&_RegCtrl2->CFG_16, CFG_16_PVR3_PVR_ALIGN_EN);
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_23, CFG_23_PVR4_PVR_ALIGN_EN);
                break;
            default:
                break;
        }
...
void HAL_PVR_FlushData(MS_U32 u32PVREng)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_SET(&_RegCtrl3->CFG3_39,HW4_CFG39_FLUSH_PVR2_DATA);
            REG16_CLR(&_RegCtrl3->CFG3_39,HW4_CFG39_FLUSH_PVR2_DATA);
            break;
        case 3:
            REG16_SET(&_RegCtrl3->CFG3_39,HW4_CFG39_FLUSH_PVR3_DATA);
            REG16_CLR(&_RegCtrl3->CFG3_39,HW4_CFG39_FLUSH_PVR3_DATA);
            break;
        default:
            break;
    }
    ...
    if(bSkip)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&_RegCtrl3->CFG3_37, HW4_CFG37_MASK_SCR_PVR3_EN);
                break;
            case 3:
                REG16_SET(&_RegCtrl3->CFG3_37, HW4_CFG37_MASK_SCR_PVR4_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&_RegCtrl3->CFG3_37, HW4_CFG37_MASK_SCR_PVR3_EN);
                break;
            case 3:
                REG16_CLR(&_RegCtrl3->CFG3_37, HW4_CFG37_MASK_SCR_PVR4_EN);
                break;
            default:
                break;
        }
    ...
    if(bDisable)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&_RegCtrl2->CFG_16, CFG_16_PVR3_BLOCK_DIS);
                break;
            case 3:
                REG16_SET(&_RegCtrl2->CFG_23, CFG_23_PVR4_BLOCK_DIS);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&_RegCtrl2->CFG_16, CFG_16_PVR3_BLOCK_DIS);
                break;
            case 3:
                REG16_CLR(&_RegCtrl2->CFG_23, CFG_23_PVR4_BLOCK_DIS);
                break;
            default:
                break;
        }
...
void HAL_PVR_BurstLen(MS_U32 u32PVREng,MS_U16 u16BurstMode)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_MSK_W(&_RegCtrl2->CFG_16, CFG_16_PVR3_BURST_LEN_MASK, (u16BurstMode << CFG_16_PVR3_BURST_LEN_SHIFT));
            break;
        case 3:
            REG16_MSK_W(&_RegCtrl2->CFG_23, CFG_23_PVR4_BURST_LEN_MASK, (u16BurstMode << CFG_23_PVR4_BURST_LEN_SHIFT));
            break;
        default:
            break;
    }
    ...
    if(bEnable)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&_RegCtrl3->CFG3_35, HW4_CFG35_BYPASS_TIMESTAMP_SEL3);
                break;
            case 3:
                REG16_CLR(&_RegCtrl3->CFG3_35, HW4_CFG35_BYPASS_TIMESTAMP_SEL4);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&_RegCtrl3->CFG3_35, HW4_CFG35_BYPASS_TIMESTAMP_SEL3);
                break;
            case 3:
                REG16_SET(&_RegCtrl3->CFG3_35, HW4_CFG35_BYPASS_TIMESTAMP_SEL4);
                break;
            default:
                break;
        }
...
void HAL_PVR_SetPauseTime(MS_U32 u32PVREng,MS_U32 u32PauseTime)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG32_W(&_RegCtrl3->CFG3_48_49, u32PauseTime);
            break;
        case 3:
            REG32_W(&_RegCtrl3->CFG3_4A_4B, u32PauseTime);
            break;
        default:
            break;
    }
...
void HAL_PVR_MOBF_Enable(MS_U32 u32PVREng, MS_BOOL bEnable, MS_U32 u32Key)
{
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG16_W(&_RegCtrl2->CFG_78_7B[0], (u32Key & CFG_78_PVR3_INDEX));
            break;
        case 3:
            REG16_W(&_RegCtrl2->CFG_78_7B[2], (u32Key & CFG_78_PVR3_INDEX));
            break;
        default:
            break;
    }
...
void HAL_TSP_HCMD_FltDisable(MS_U32 u32FltId)
{
    REG32_W(&_RegCtrl->MCU_Data1, u32FltId);
    REG32_W(&_RegCtrl->MCU_Cmd, TSP_MCU_CMD_FLT_DISABLE);
}
...
MS_BOOL HAL_TSP_HCMD_VerReset(MS_U32 u32SecFltId)
{
    HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_INFO, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupport feature!\n",__FILE__,__FUNCTION__,__LINE__));
    return FALSE;
}

MS_BOOL HAL_TSP_HCMD_SetFWDataAddr(MS_PHY phyDataAddr)
{
    HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_INFO, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupport feature!\n",__FILE__,__FUNCTION__,__LINE__));
    return FALSE;
}

void HAL_TSP_HCMD_TSIF_130_Enable(MS_U32 u32TSIF, MS_BOOL bEnable)
{
    REG32_W(&_RegCtrl->MCU_Data0, u32TSIF);
    REG32_W(&_RegCtrl->MCU_Data1, bEnable);
    REG32_W(&_RegCtrl->MCU_Cmd, TSP_MCU_CMD_TSIF_130_Enable);
}
...
void HAL_TSP_INT_Enable(MS_U32 u32Mask)
{
    // u32Mask [0:7] for bank 0x15 7e bit[0:7]
        ...
        */
    REG16_SET(&_RegCtrl->HwInt_Stat, (TSP_HWINT_EN_MASK & u32Mask) | TSP_HWINT_STATUS_MASK);
    ...
    // u32Mask [8:15] for bank 0x16 6c bit[0:7]
        /*
            ...
            [0] : reserved
        */
    REG16_SET(&_RegCtrl->HwInt2_Stat, (TSP_HWINT2_EN_MASK & (u32Mask >> 8)) | TSP_HWINT2_STATUS_MASK);
     ...
     // u32Mask [16:23] for bank 0x16 6f bit[0:7]
        ...
        */
    REG16_SET(&_RegCtrl->HwInt3_Stat, (TSP_HWINT3_EN_MASK & (u32Mask >> 16)) | TSP_HWINT3_STATUS_MASK);
}
...
{
    //REG16_CLR(&_RegCtrl->HwInt_Stat, TSP_HWINT_EN_MASK & u32Mask);
    //REG16_CLR(&_RegCtrl->HwInt2_Stat, TSP_HWINT2_EN_MASK & (u32Mask >> 8));

    REG16_W(&_RegCtrl->HwInt_Stat,
    (REG16_R(&_RegCtrl->HwInt_Stat) & ~(TSP_HWINT_EN_MASK & (u32Mask))) |
        TSP_HWINT_STATUS_MASK);
...
void HAL_TSP_INT_ClrHW(MS_U32 u32Mask)
{
    ...
    //REG16_CLR(&_RegCtrl->HwInt2_Stat, u32Mask & 0xFF00);

    REG16_W(&_RegCtrl->HwInt_Stat,
    (REG16_R(&_RegCtrl->HwInt_Stat) & (~TSP_HWINT_STATUS_MASK)) |
        (((~u32Mask) << TSP_HWINT_STATUS_SHIFT) & TSP_HWINT_STATUS_MASK) );

    REG16_W(&_RegCtrl->HwInt2_Stat,
    (REG16_R(&_RegCtrl->HwInt2_Stat) & (~TSP_HWINT2_STATUS_MASK)) |
        (((~(u32Mask >> 8))<< TSP_HWINT2_STATUS_SHIFT) & TSP_HWINT2_STATUS_MASK) );

    REG16_W(&_RegCtrl->HwInt3_Stat,
    (REG16_R(&_RegCtrl->HwInt3_Stat) & (~TSP_HWINT3_STATUS_MASK)) |
        (((~(u32Mask >> 16))<< TSP_HWINT3_STATUS_SHIFT) & TSP_HWINT3_STATUS_MASK) );
}
...
void HAL_TSP_Set_RcvBuf_Src(MS_U32 bufIdx, MS_U32 inputSrc)
{
    switch (bufIdx)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            REG16_MSK_W(&_RegCtrl2->CFG_11, CFG_11_RECEIVE_BUF3_SRC, inputSrc<<(bufIdx * 2));
            break;
        default:
            break;
    }
    ...
    switch(tsif)
    {
        ...
        case 2: REG16_MSK_W(&_RegCtrl3->CFG3_43, HW4_CFG43_SRC_AES_FI_KEY_MASK, HW4_CFG43_SRC_AES_FI2_KEY << HW4_CFG43_SRC_AES_FI_KEY_SHIFT);
                ...
                */
                ...
                break;
        case 3: REG16_MSK_W(&_RegCtrl3->CFG3_43, HW4_CFG43_SRC_AES_FI_KEY_MASK, HW4_CFG43_SRC_AES_FI3_KEY << HW4_CFG43_SRC_AES_FI_KEY_SHIFT);
                /*
                REG16_W(&_RegCtrl3->CFG3_68_6F[0], 0x6666);             //file-in SPD key
                REG16_W(&_RegCtrl3->CFG3_68_6F[1], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_68_6F[2], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_68_6F[3], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_68_6F[4], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_68_6F[5], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_68_6F[6], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_68_6F[7], 0x7777);
                */
                REG16_W(&_RegCtrl3->CFG3_68_6F[0], 0x0000);             //file-in SPD key
                REG16_W(&_RegCtrl3->CFG3_68_6F[1], 0x0000);
                REG16_W(&_RegCtrl3->CFG3_68_6F[2], 0x0000);
                ...
                REG16_W(&_RegCtrl3->CFG3_68_6F[6], 0x1111);
                REG16_W(&_RegCtrl3->CFG3_68_6F[7], 0x1111);
                REG16_SET(&_RegCtrl3->CFG3_3A,HW4_CFG3A_LOAD_SPD_KEY3);
                REG16_CLR(&_RegCtrl3->CFG3_52,CFG3_52_SPD_TSIF3_BYPASS);//bypass SPD
                REG16_SET(&_RegCtrl3->CFG3_1E,CFG3_1E_TSIF3_SPD_RESET); //TSIF SPD reset
                REG16_CLR(&_RegCtrl3->CFG3_1E,CFG3_1E_TSIF3_SPD_RESET);
                break;
...
MS_BOOL HAL_TSP_GetCaps(TSP_HAL_CAP_TYPE eCap, MS_U32 *pu32CapInfo)
{
    switch (eCap)
    {
        ...
        case E_TSP_HAL_CAP_TYPE_DSCMB_ENG_NUM :
            *(pu32CapInfo) = E_TSP_HAL_CAP_VAL_DSCMB_ENG_NUM;
            break;
...
void HAL_TSP_RemoveDupAVFifoPkt(TSP_DST_SEQ eFltType, MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        ...
        {
            case E_TSP_DST_FIFO_VIDEO :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_V);
                break;
            case E_TSP_DST_FIFO_VIDEO3D :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_V3D);
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_SET(&_RegCtrl8_1->CFG8_40, CFG8_40_DUP_PKT_SKIP_V3);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_SET(&_RegCtrl8_1->CFG8_41, CFG8_41_DUP_PKT_SKIP_V4);
                break;
            case E_TSP_DST_FIFO_AUDIO :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_A);
                break;
            case E_TSP_DST_FIFO_AUDIO2 :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_AB);
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_AC);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_SET(&_RegCtrl3->CFG3_34,   CFG3_34_DUP_PKT_SKIP_AD);
                break;
            default :
                break;
    ...
    else
    {
        ...
        {
            case E_TSP_DST_FIFO_VIDEO :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_V);
                break;
            case E_TSP_DST_FIFO_VIDEO3D :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_V3D);
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_CLR(&_RegCtrl8_1->CFG8_40, CFG8_40_DUP_PKT_SKIP_V3);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_CLR(&_RegCtrl8_1->CFG8_41, CFG8_41_DUP_PKT_SKIP_V4);
                break;
            case E_TSP_DST_FIFO_AUDIO :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_A);
                break;
            case E_TSP_DST_FIFO_AUDIO2 :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_AB);
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_AC);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_CLR(&_RegCtrl3->CFG3_34, CFG3_34_DUP_PKT_SKIP_AD);
                break;
            default :
                break;
        }
...
{
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_VIDEO,   bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_VIDEO3D, bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_VIDEO3,  bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_VIDEO4,  bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_AUDIO,   bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_AUDIO2,  bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_AUDIO3,  bEnable);
    HAL_TSP_RemoveDupAVFifoPkt(E_TSP_DST_FIFO_AUDIO4,  bEnable);
}
    ...
    if(bEnable)
    {
        switch (tsIf)
       {
          ...
          case 2:
              ...
              break;
          case 3:
              REG16_SET(&_RegCtrl->reg15b8, TSP_TEI_SKIPE_PKT_PID3);
              break;
          default:
              break;
        }
    ...
    else
    {
       switch (tsIf)
       {
          ...
          case 2:
              ...
              break;
          case 3:
              REG16_CLR(&_RegCtrl->reg15b8, TSP_TEI_SKIPE_PKT_PID3);
              break;
          default:
              break;
        }
...
void HAL_TSP_DSS130_Enable(MS_U32 u32TSIF, MS_BOOL bEnable)
{
    MS_U16  u16Value0 = 0;
    MS_U16  u16Value1 = 0;
    ...
    {
        case E_TSP_HAL_TSIF_0:
            u16Value0 = CFG3_19_DIRECTV_EN0 | CFG3_19_DIRECTV_PSI_EN0;
            u16Value1 = CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH0;
            break;
        case E_TSP_HAL_TSIF_1:
            u16Value0 = CFG3_19_DIRECTV_EN1 | CFG3_19_DIRECTV_PSI_EN1;
            u16Value1 = CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH1;
            break;
        case E_TSP_HAL_TSIF_2:
            u16Value0 = CFG3_19_DIRECTV_EN2 | CFG3_19_DIRECTV_PSI_EN2;
            u16Value1 = CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH2;
            break;
        case E_TSP_HAL_TSIF_3:
            u16Value0 = CFG3_19_DIRECTV_EN3 | CFG3_19_DIRECTV_PSI_EN3;
            u16Value1 = CFG3_1A_RVU_DIRECTV_AFTER_CA_ENABLE_PATH3;
            break;
        default:
            HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_ERR, E_HAL_TSP_DBG_MODEL_ALL, printf("[TSP ERROR][%s][%d] TSIF:%d not support !!\n", __FUNCTION__, __LINE__,u32TSIF));
            return;
    }

    if(bEnable)
    {
        REG16_SET(&_RegCtrl3->CFG3_19, u16Value0);
        REG16_SET(&_RegCtrl3->CFG3_1A, u16Value1);
    }
    else
    {
        REG16_CLR(&_RegCtrl3->CFG3_19, u16Value0);
        REG16_CLR(&_RegCtrl3->CFG3_1A, u16Value1);
    }

    HAL_TSP_HCMD_TSIF_130_Enable(u32TSIF, bEnable);
}
...
void HAL_TSP_DisPKTCnt_Clear(TSP_DST_SEQ eFltType)
{
    switch (eFltType)
    {
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            REG16_SET(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_ADC_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_ADC_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            REG16_SET(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_ADD_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_ADD_CLR);
            break;
        default :
    ...
    if(bEnable)
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_SET(&_RegCtrl3->CFG3_73, CFG3_73_PVR3_DMAW_PROTECT_EN);
                break;
            case 3:
                REG16_SET(&_RegCtrl3->CFG3_73, CFG3_73_PVR4_DMAW_PROTECT_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(u32PVREng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                REG16_CLR(&_RegCtrl3->CFG3_73, CFG3_73_PVR3_DMAW_PROTECT_EN);
                break;
            case 3:
                REG16_CLR(&_RegCtrl3->CFG3_73, CFG3_73_PVR4_DMAW_PROTECT_EN);
                break;
            default:
                break;
        }
...
void HAL_TSP_PVR_Address_Protect(MS_U32 u32PVREng,MS_PHY u32AddrH, MS_PHY u32AddrL)
{
    ...
    switch(u32PVREng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            REG32_W(&_RegCtrl->DMAW_LBND4, u32LBnd);
            REG32_W(&_RegCtrl->DMAW_UBND4, u32UBnd);
            break;
        case 3:
            REG32_W(&_RegCtrl3->CFG3_12_13, u32LBnd);
            REG32_W(&_RegCtrl3->CFG3_14_15, u32UBnd);
            break;
        default:
            break;
    }
    ...
    if(bEnable)
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_SET(&_RegCtrl3->CFG3_73, CFG3_73_FILEIN3_DMAR_PROTECT_EN);
                break;
            default:
                break;
        }
    ...
    else
    {
        switch(eFileEng)
        {
            ...
            case E_FILEENG_TSIF2:
                ...
                break;
            case E_FILEENG_TSIF3:
                REG16_CLR(&_RegCtrl3->CFG3_73, CFG3_73_FILEIN3_DMAR_PROTECT_EN);
                break;
            default:
                break;
        }
...
void HAL_TSP_FILEIN_Address_Protect(FILEENG_SEQ eFileEng,MS_PHY u32AddrH, MS_PHY u32AddrL)
{
    ...
    switch(eFileEng)
    {
        ...
        case E_FILEENG_TSIF2:
            ...
            break;
        case E_FILEENG_TSIF3:
            REG32_W(&_RegCtrl6->CFG6_3C_3D, u32LBnd);
            REG32_W(&_RegCtrl6->CFG6_3E_3F, u32UBnd);
            break;
        default:
            break;
    }
...
MS_BOOL HAL_TSP_CAPVR_SPSEnable(MS_U32 u32Eng, MS_U16 u16CaPvrMode, MS_BOOL bEnable)
{
    ...
    switch(u32Eng)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            u16value &= ~(HW4_CFG38_CA_PVR3_SEL_MASK|HW4_CFG38_PKT192_SPS_EN3);
            break;
        case 3:
            u16value &= ~(HW4_CFG38_CA_PVR4_SEL_MASK|HW4_CFG38_PKT192_SPS_EN4);
            break;
        default:
            return FALSE;
    }
    ...
    if(bEnable)
    {
        switch(u32Eng)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                u16value |= (u16CaPvrMode << HW4_CFG38_CA_PVR3_SEL_SHIFT);
                u16value |= HW4_CFG38_PKT192_SPS_EN3;
                break;
            case 3:
                u16value |= (u16CaPvrMode << HW4_CFG38_CA_PVR4_SEL_SHIFT);
                u16value |= HW4_CFG38_PKT192_SPS_EN4;
                break;
        }
    }
    ...
    switch(u32Eng)
    {
        ...
        case 1: REG16_SET(&_RegCtrl3->CFG3_38,HW4_CFG38_PKT192_SPS_EN2);
                ...
                */
                ...
                break;
        case 2: REG16_SET(&_RegCtrl3->CFG3_38,HW4_CFG38_PKT192_SPS_EN3);
                REG16_MSK_W(&_RegCtrl3->CFG3_43, HW4_CFG43_SRC_AES_PVR_KEY_MASK, HW4_CFG43_SRC_AES_PVR3_KEY);
                /*
                REG16_W(&_RegCtrl3->CFG3_60_67[0], 0x4444);//PVR SPS key
                REG16_W(&_RegCtrl3->CFG3_60_67[1], 0x4444);
                REG16_W(&_RegCtrl3->CFG3_60_67[2], 0x4444);
                REG16_W(&_RegCtrl3->CFG3_60_67[3], 0x4444);
                REG16_W(&_RegCtrl3->CFG3_60_67[4], 0x5555);
                REG16_W(&_RegCtrl3->CFG3_60_67[5], 0x5555);
                REG16_W(&_RegCtrl3->CFG3_60_67[6], 0x5555);
                REG16_W(&_RegCtrl3->CFG3_60_67[7], 0x5555);
                */
                REG16_W(&_RegCtrl3->CFG3_60_67[0], 0x0000);//PVR SPS key
                REG16_W(&_RegCtrl3->CFG3_60_67[1], 0x0000);
                REG16_W(&_RegCtrl3->CFG3_60_67[2], 0x0000);
                ...
                REG16_W(&_RegCtrl3->CFG3_60_67[6], 0x1111);
                REG16_W(&_RegCtrl3->CFG3_60_67[7], 0x1111);
                REG16_SET(&_RegCtrl3->CFG3_38,HW4_CFG38_LOAD_SPS_KEY3);
                break;
        case 3: REG16_SET(&_RegCtrl3->CFG3_38,HW4_CFG38_PKT192_SPS_EN4);
                REG16_MSK_W(&_RegCtrl3->CFG3_43, HW4_CFG43_SRC_AES_PVR_KEY_MASK, HW4_CFG43_SRC_AES_PVR4_KEY);
                /*
                REG16_W(&_RegCtrl3->CFG3_60_67[0], 0x6666);//PVR SPS key
                REG16_W(&_RegCtrl3->CFG3_60_67[1], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_60_67[2], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_60_67[3], 0x6666);
                REG16_W(&_RegCtrl3->CFG3_60_67[4], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_60_67[5], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_60_67[6], 0x7777);
                REG16_W(&_RegCtrl3->CFG3_60_67[7], 0x7777);
                */
                REG16_W(&_RegCtrl3->CFG3_60_67[0], 0x0000);//PVR SPS key
                REG16_W(&_RegCtrl3->CFG3_60_67[1], 0x0000);
                REG16_W(&_RegCtrl3->CFG3_60_67[2], 0x0000);
                ...
                REG16_W(&_RegCtrl3->CFG3_60_67[6], 0x1111);
                REG16_W(&_RegCtrl3->CFG3_60_67[7], 0x1111);
                REG16_SET(&_RegCtrl3->CFG3_38,HW4_CFG38_LOAD_SPS_KEY4);
                break;
        default: printf("Not Support !!\n");
                 break;
    }
...
TSP_HAL_Result HAL_TSP_DSCMB_Source(MS_U32 u32Path, MS_U32 *pu32EngId, MS_BOOL bSet)
{
    return E_TSP_HAL_NOT_SUPPORTED;
}
...
MS_BOOL HAL_TSP_PktConverter_SetSyncByte(MS_U8 u8Path, MS_U8 u8Idx, MS_U8 *pu8SyncByte, MS_BOOL bSet)
{
    ...
    {
        case TSP_TSIF0:
            SynReg = &(_RegCtrl6->SyncByte_tsif0[u8Idx>>1]);
            break;
        case TSP_TSIF1:
            SynReg = &(_RegCtrl6->SyncByte_tsif1[u8Idx>>1]);
            break;
        case TSP_TSIF2:
            SynReg = &(_RegCtrl6->SyncByte_tsif2[u8Idx>>1]);
            break;
        case TSP_TSIF3:
            SynReg = &(_RegCtrl6->SyncByte_tsif3[u8Idx>>1]);
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSP_PktConverter_SetSrcId(MS_U8 u8Path, MS_U8 u8Idx, MS_U8 *pu8SrcId, MS_BOOL bSet)
{
    ...
    {
        case TSP_TSIF0:
            SrcIdReg = &(_RegCtrl6->SourceId_tsif0[u8Idx>>2]);
            break;
        case TSP_TSIF1:
            SrcIdReg = &(_RegCtrl6->SourceId_tsif1[u8Idx>>2]);
            break;
        case TSP_TSIF2:
            SrcIdReg = &(_RegCtrl6->SourceId_tsif2[u8Idx>>2]);
            break;
        case TSP_TSIF3:
            SrcIdReg = &(_RegCtrl6->SourceId_tsif3[u8Idx>>2]);
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSP_PktConverter_PktMode(MS_U8 u8Path, TSP_HAL_PKT_MODE ePktMode)
{
    ...
    switch(ePktMode)
    {
        ...
        case E_TSP_HAL_PKT_MODE_ND:
            REG16_W(PktConverterReg,((REG16_R(PktConverterReg) & ~(TSP_PKT_CONVERTER_MODE_MASK)) | TSP_PKT_NagraDongleMode));
...
void HAL_TSP_PktConverter_SrcIdFlt(MS_U8 u8Path, MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        REG16_W(PktConverterReg, _SET_(REG16_R(PktConverterReg), TSP_SRC_ID_FLT_EN)); //  Set 1 to enable filter sync byte and path ID match
    }
    else
    {
        REG16_W(PktConverterReg, _CLR_(REG16_R(PktConverterReg), TSP_SRC_ID_FLT_EN));
    }
}
    ...
    if(bEn == TRUE)
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl5->CFG5_7A,CFG5_7A_LOCKED_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
    ...
    else
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl5->CFG5_7A,CFG5_7A_LOCKED_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
...
void HAL_TSP_Debug_LockPktCnt_Clear(MS_U32 u32Tsif)
{
    switch (u32Tsif)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            REG16_SET(&_RegCtrl5->CFG5_7C,CFG5_7C_LOCKED_PKT_CNT_3_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7C,CFG5_7C_LOCKED_PKT_CNT_3_CLR);
            break;
        default :
            printf("[%s][%s][%d] UnSupported TSIF : %u\n\n",__FILE__,__FUNCTION__,__LINE__, (unsigned int)u32Tsif);
            break;
...
void HAL_TSP_Debug_AvPktCnt_Src(TSP_DST_SEQ eAvType, TSP_SRC_SEQ ePktDmxId)
{
    ...
    switch (eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D :
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3 :
            REG16_MSK_W(&_RegCtrl5->CFG5_7E, CFG5_7E_VID_3_SRC_MASK, u16AvSrc << CFG5_7E_VID_3_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_VIDEO4 :
            REG16_MSK_W(&_RegCtrl5->CFG5_78, CFG5_78_VID_4_SRC_MASK, u16AvSrc << CFG5_78_VID_4_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            REG16_MSK_W(&_RegCtrl5->CFG5_78, CFG5_78_AUDC_SRC_MASK, u16AvSrc << CFG5_78_AUDC_SRC_SHIFT);
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            REG16_MSK_W(&_RegCtrl5->CFG5_78, CFG5_78_AUDD_SRC_MASK, u16AvSrc << CFG5_78_AUDD_SRC_SHIFT);
            break;
        default:
    ...
    if(bEn == TRUE)
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_V3_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_V4_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_SET(&_RegCtrl5->CFG5_7A,CFG5_7A_ADC_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_SET(&_RegCtrl5->CFG5_7A,CFG5_7A_ADD_PKT_CNT_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
    ...
    else
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_V3_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_V4_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_CLR(&_RegCtrl5->CFG5_7A,CFG5_7A_ADC_PKT_CNT_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_CLR(&_RegCtrl5->CFG5_7A,CFG5_7A_ADD_PKT_CNT_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
...
MS_U16 HAL_TSP_Debug_AvPktCnt_Get(TSP_DST_SEQ eAvType)
{
    switch (eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3 :
            REG16_CLR(&_RegCtrl5->CFG5_7F,CFG5_7F_AV_PKT_SRC_SEL);
            return REG16_R(&_RegCtrl5->Av_PKT_Cnt2);
        case E_TSP_DST_FIFO_VIDEO4:
...
void HAL_TSP_Debug_AvPktCnt_Clear(TSP_DST_SEQ eAvType)
{
    switch (eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D :
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3 :
            REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_V3_PKT_CNT_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_V3_PKT_CNT_CLR);
            break;
        case E_TSP_DST_FIFO_VIDEO4 :
            REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_V4_PKT_CNT_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_V4_PKT_CNT_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            REG16_SET(&_RegCtrl5->CFG5_7C,CFG5_7C_ADC_PKT_CNT_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7C,CFG5_7C_ADC_PKT_CNT_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            REG16_SET(&_RegCtrl5->CFG5_7C,CFG5_7C_ADD_PKT_CNT_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7C,CFG5_7C_ADD_PKT_CNT_CLR);
            break;
        default :
            printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
            break;
...
void HAL_TSP_Debug_DropDisPktCnt_Src(TSP_DST_SEQ eAvType,TSP_SRC_SEQ ePktDmxId)
{
    ...
    switch(eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D:
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            u16AvType = DIS_DROP_CNT_V3;
            break;
        case E_TSP_DST_FIFO_VIDEO4:
            u16AvType = DIS_DROP_CNT_V4;
            break;
        case E_TSP_DST_FIFO_AUDIO:
        ...
        case E_TSP_DST_FIFO_AUDIO2:
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3:
            u16AvType = DIS_DROP_CNT_ADC;
            break;
        case E_TSP_DST_FIFO_AUDIO4:
            u16AvType = DIS_DROP_CNT_ADD;
            break;
        default:
            printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
            break;
    ...
    switch(ePktDmxId)
    {
        ...
        case E_TSP_SRC_PKTDMX2:
            ...
            break;
        case E_TSP_SRC_PKTDMX3:
            REG16_MSK_W(&_RegCtrl5->CFG5_77,CFG5_77_PIDFLT_SRC_SEL3_MASK,(u16AvType<<CFG5_77_PIDFLT_SRC_SEL3_SHIFT));
            break;
        case E_TSP_SRC_MMFI0:
    ...
    if(bEn == TRUE)
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_DROP_PKT_CNT_V3_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_DROP_PKT_CNT_V4_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_SET(&_RegCtrl5->CFG5_7B,CFG5_7B_DROP_PKT_CNT_ADC_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_SET(&_RegCtrl5->CFG5_7B,CFG5_7B_DROP_PKT_CNT_ADD_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
    ...
    else
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_DROP_PKT_CNT_V3_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_DROP_PKT_CNT_V4_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_CLR(&_RegCtrl5->CFG5_7B,CFG5_7B_DROP_PKT_CNT_ADC_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_CLR(&_RegCtrl5->CFG5_7B,CFG5_7B_DROP_PKT_CNT_ADD_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
    ...
    if(bEn == TRUE)
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_DIS_PKT_CNT_V3_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_DIS_PKT_CNT_V4_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_SET(&_RegCtrl5->CFG5_7B,CFG5_7B_DIS_PKT_CNT_ADC_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_SET(&_RegCtrl5->CFG5_7B,CFG5_7B_DIS_PKT_CNT_ADD_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d \n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
    ...
    else
    {
        switch (eAvType)
        {
            ...
            case E_TSP_DST_FIFO_VIDEO3D :
                ...
                break;
            case E_TSP_DST_FIFO_VIDEO3 :
                REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_DIS_PKT_CNT_V3_LOAD);
                break;
            case E_TSP_DST_FIFO_VIDEO4 :
                REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_DIS_PKT_CNT_V4_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO :
            ...
            case E_TSP_DST_FIFO_AUDIO2 :
                ...
                break;
            case E_TSP_DST_FIFO_AUDIO3 :
                REG16_CLR(&_RegCtrl5->CFG5_7B,CFG5_7B_DIS_PKT_CNT_ADC_LOAD);
                break;
            case E_TSP_DST_FIFO_AUDIO4 :
                REG16_CLR(&_RegCtrl5->CFG5_7B,CFG5_7B_DIS_PKT_CNT_ADD_LOAD);
                break;
            default :
                printf("[%s][%s][%d] UnSupported AV Type : %d \n",__FILE__,__FUNCTION__,__LINE__,eAvType);
                break;
...
void HAL_TSP_Debug_DropPktCnt_Clear(TSP_DST_SEQ eAvType)
{
    switch (eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D :
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3:
            REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_DROP_PKT_CNT_V3_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_DROP_PKT_CNT_V3_CLR);
            break;
        case E_TSP_DST_FIFO_VIDEO4 :
            REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_DROP_PKT_CNT_V4_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_DROP_PKT_CNT_V4_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO :
        ...
        case E_TSP_DST_FIFO_AUDIO2 :
            ...
            break;
        case E_TSP_DST_FIFO_AUDIO3 :
            REG16_SET(&_RegCtrl5->CFG5_7D,CFG5_7D_DROP_PKT_CNT_ADC_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7D,CFG5_7D_DROP_PKT_CNT_ADC_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO4 :
            REG16_SET(&_RegCtrl5->CFG5_7D,CFG5_7D_DROP_PKT_CNT_ADD_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7D,CFG5_7D_DROP_PKT_CNT_ADD_CLR);
            break;
        default :
            printf("[%s][%s][%d] UnSupported AV Type : %d !\n",__FILE__,__FUNCTION__,__LINE__,eAvType);
            break;
...
void HAL_TSP_Debug_DisPktCnt_Clear(TSP_DST_SEQ eAvType)
{
    switch (eAvType)
    {
        ...
        case E_TSP_DST_FIFO_VIDEO3D :
            ...
            break;
        case E_TSP_DST_FIFO_VIDEO3 :
            REG16_SET(&_RegCtrl5->CFG5_74,CFG5_74_DIS_PKT_CNT_V3_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_74,CFG5_74_DIS_PKT_CNT_V3_CLR);
            break;
        case E_TSP_DST_FIFO_VIDEO4 :
            REG16_SET(&_RegCtrl5->CFG5_75,CFG5_75_DIS_PKT_CNT_V4_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_75,CFG5_75_DIS_PKT_CNT_V4_CLR);
            break;
        case E_TSP_DST_FIFO_AUDIO :
            REG16_SET(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_A_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_7D,CFG5_7D_DIS_PKT_CNT_A_CLR);
    ...
    if(bEn == TRUE)
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl5->CFG5_71,CFG5_71_ERR_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
    ...
    else
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl5->CFG5_71,CFG5_71_ERR_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
...
void HAL_TSP_Debug_ErrPktCnt_Clear(MS_U32 u32Tsif)
{
    switch (u32Tsif)
    {
        ...
        case 2 :
            ...
            break;
        case 3 :
            REG16_SET(&_RegCtrl5->CFG5_72,CFG5_72_ERR_PKT_CNT_3_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_72,CFG5_72_ERR_PKT_CNT_3_CLR);
            break;
    ...
    if(bEn == TRUE)
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_SET(&_RegCtrl5->CFG5_71,CFG5_71_INPUT_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
    ...
    else
    {
        switch(u32TsIf)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                REG16_CLR(&_RegCtrl5->CFG5_71,CFG5_71_INPUT_PKT_CNT_3_LOAD);
                break;
            default:
                printf("[%s][%s][%d] UnSupported TSIF : %u\n",__FILE__,__FUNCTION__,__LINE__,(unsigned int)u32TsIf);
                break;
...
void HAL_TSP_Debug_InputPktCnt_Clear(MS_U32 u32Tsif)
{
    switch (u32Tsif)
    {
        ...
        case 2 :
            ...
            break;
        case 3 :
            REG16_SET(&_RegCtrl5->CFG5_72,CFG5_72_INPUT_PKT_CNT_3_CLR);
            REG16_CLR(&_RegCtrl5->CFG5_72,CFG5_72_INPUT_PKT_CNT_3_CLR);
            break;
...
void HAL_TSP_Privilege_Enable(MS_BOOL bEnable)
{
    if (bEnable)
    {
        REG16_SET(&_RegCtrl3->CFG3_7C, CFG3_7C_PRIVILEGE_FLAG);
    }
    else
    {
        REG16_CLR(&_RegCtrl3->CFG3_7C, CFG3_7C_PRIVILEGE_FLAG);
    }
}

TSP_HAL_Result HAL_TSP_FQ_SetMuxSwitch(MS_U32 u32FQEng, MS_U32 u32FQSrc)
{
    HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_INFO, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupport feature!\n",__FILE__,__FUNCTION__,__LINE__));
    return E_TSP_HAL_NOT_SUPPORTED;
}

TSP_HAL_Result HAL_TSP_FQ_GetMuxSwitch(MS_U32 u32FQEng, MS_U32 *pu32FQSrc)
{
    HAL_TSP_DBGMSG(E_HAL_TSP_DBG_LEVEL_INFO, E_HAL_TSP_DBG_MODEL_ALL, printf("[%s][%s][%d] UnSupport feature!\n",__FILE__,__FUNCTION__,__LINE__));
    *pu32FQSrc = (MS_U32)-1;
    return E_TSP_HAL_NOT_SUPPORTED;
...
MS_BOOL HAL_TSP_FQ_FLT_NULL_PKT(MS_U32 u32FQEng, MS_BOOL bFltNull)
{
    if(bFltNull)
    {
        REG16_SET(&_RegCtrl3->CFG3_2D, CFG3_2D_FILTER_NULL_PKT);
    }
    else
    {
        REG16_CLR(&_RegCtrl3->CFG3_2D, CFG3_2D_FILTER_NULL_PKT);
    }
...
void HAL_TSP_FQ_MMFI_MIU_Sel(TSP_HAL_MIU_SEL_TYPE eType, MS_U8 u8Eng, MS_PHY phyBufStart)
{
    ...
    MS_U8   u8Shift = u8Eng * TSP_MIU_SEL_BITS_LEN;
    ...
    switch(eType)
    {
        case E_TSP_HAL_MIU_SEL_MMFI:
            REG16_MSK_W(&_RegCtrl6->CFG6_2C_REG_MIU_SEL_FILEIN_MM, (REG_MIU_SEL_MMFI0_MASK << u8Shift), (u8MiuSel << (REG_MIU_SEL_MMFI0_SHIFT + u8Shift)));
            break;
        case E_TSP_HAL_MIU_SEL_FQ:
            REG16_MSK_W(&_RegCtrl6->CFG6_2E_REG_MIU_PVR_FQ, (REG_MIU_SEL_FIQ0_MASK << u8Shift), (u8MiuSel << (REG_MIU_SEL_FIQ0_SHIFT + u8Shift)));
            break;
        default:
            break;
    }
