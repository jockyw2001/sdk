...
void HAL_TSO_SetExtRiuBank(MS_VIRT u32BankAddr)
{
    //not support
}
...
void HAL_TSO_Init(void)
{
    ...
    */
    ...
    if(K6U02)
    {
        _REG16_SET(&(_TSOCtrl1->REG_TSO_MIU_ABT_CONFIG_1), REG_MIU_ABT_CONFIG_1_CHECK2MI_RDY | REG_MIU_ABT_CONFIG_1_MIU_FIXED_LAST_WD_EN_DONE_Z);
    }
}
...
static void _HAL_TSO_PageTableIndW_withSid(MS_U32 u32Addr, MS_U16 u16Wdata, MS_U8 u8Sid)
{
    // addr
    ...
    _HAL_REG16_W(&(_TSOCtrl->PDTABLE_WDATA_H), (u8Sid & 0x003F));
...
{
    REG32_TSO *FileInRaddr;

    MS_U8 u8MiuSel;
    MS_PHY phyMiuOffsetFileinBuf;
    _phy_to_miu_offset(u8MiuSel, phyMiuOffsetFileinBuf, u32Addr);

    if(u8FileEng == 0)
    {
        FileInRaddr = &(_TSOCtrl->CFG_TSO_60_63[0]);
        _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_CH5FILEIN_MASK)) | ((u8MiuSel << REG_MIU_SEL_CH5FILEIN_SHIFT) & REG_MIU_SEL_CH5FILEIN_MASK));
    }
    else
    {
        FileInRaddr = &(_TSOCtrl->CFG_TSO_65_68[0]);
        _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_2, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_2) & (~REG_MIU_SEL_CH6FILEIN_MASK)) | ((u8MiuSel << REG_MIU_SEL_CH6FILEIN_SHIFT) & REG_MIU_SEL_CH6FILEIN_MASK));
    }

    _HAL_REG32_W(FileInRaddr, phyMiuOffsetFileinBuf);
}
...
void HAL_TSO_TsioMode_En(MS_BOOL bEnable)
{
    if(bEnable)
    {
        _REG16_SET(&(_TSOCtrl->TSO_CONFIG5),TSO_CONFIG5_TSIO_MODE);
    }
    else
    {
        _REG16_CLR(&(_TSOCtrl->TSO_CONFIG5),TSO_CONFIG5_TSIO_MODE);
    }
}
...
void HAL_TSO_Tsio2Opif_En(MS_BOOL bEnable)
{
    if(bEnable)
    {
        _REG16_SET(&(_TSOCtrl->TSO_CONFIG5),TSO_CONFIG5_TSIO2OPIF);
    }
    else
    {
        _REG16_CLR(&(_TSOCtrl->TSO_CONFIG5),TSO_CONFIG5_TSIO2OPIF);
    }
}
...
MS_BOOL HAL_TSO_SVQBuf_Set(MS_U8 u8Eng, MS_U8 u8ChIf, MS_PHY phyBufAddr, MS_U32 u32BufSize)
{
    ...
    // Check MIU select
    MS_U8 u8MiuSel;
    MS_PHY phyMiuOffsetSVQBuf;
    _phy_to_miu_offset(u8MiuSel, phyMiuOffsetSVQBuf, phyBufAddr);
    _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQRX_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQRX_SHIFT) & REG_MIU_SEL_SVQRX_MASK));
    ...
    switch(u8ChIf)
    {
        case HAL_TSO_TSIF_LIVE1:
            ...
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX1_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX1_SHIFT) & REG_MIU_SEL_SVQTX1_MASK));
            break;
        case HAL_TSO_TSIF_LIVE2:
            Base = &(_TSOCtrl1->SVQ2_BASE);
            Size = &(_TSOCtrl1->SVQ2_SIZE_200BYTE);
            TX_Config = &(_TSOCtrl1->SVQ2_TX_CONFIG);
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX2_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX2_SHIFT) & REG_MIU_SEL_SVQTX2_MASK));
            break;
        case HAL_TSO_TSIF_LIVE3:
            Base = &(_TSOCtrl1->SVQ3_BASE);
            Size = &(_TSOCtrl1->SVQ3_SIZE_200BYTE);
            TX_Config = &(_TSOCtrl1->SVQ3_TX_CONFIG);
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX3_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX3_SHIFT) & REG_MIU_SEL_SVQTX3_MASK));
            break;
        case HAL_TSO_TSIF_LIVE4:
            Base = &(_TSOCtrl1->SVQ4_BASE);
            Size = &(_TSOCtrl1->SVQ4_SIZE_200BYTE);
            TX_Config = &(_TSOCtrl1->SVQ4_TX_CONFIG);
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX4_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX4_SHIFT) & REG_MIU_SEL_SVQTX4_MASK));
            break;
        case HAL_TSO_TSIF_LIVE5:
            Base = &(_TSOCtrl1->SVQ5_BASE);
            Size = &(_TSOCtrl1->SVQ5_SIZE_200BYTE);
            TX_Config = &(_TSOCtrl1->SVQ5_TX_CONFIG);
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX5_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX5_SHIFT) & REG_MIU_SEL_SVQTX5_MASK));
            break;
        case HAL_TSO_TSIF_LIVE6:
            Base = &(_TSOCtrl1->SVQ6_BASE);
            Size = &(_TSOCtrl1->SVQ6_SIZE_200BYTE);
            TX_Config = &(_TSOCtrl1->SVQ6_TX_CONFIG);
            _HAL_REG16_W(&_TSOCtrl1->REG_TSO_MIU_SEL_1, (_HAL_REG16_R(&_TSOCtrl1->REG_TSO_MIU_SEL_1) & (~REG_MIU_SEL_SVQTX6_MASK)) | ((u8MiuSel << REG_MIU_SEL_SVQTX6_SHIFT) & REG_MIU_SEL_SVQTX6_MASK));
            break;
        default:
            printf("Not support !!\n");
            return FALSE;
    ...
    _HAL_REG32_W(Base, ((phyMiuOffsetSVQBuf >> TSO_MIU_BUS) & TSO1_SVQ1_BASE_MASK));
...
void HAL_TSO_Flt_SetSid(MS_U8 u8Eng, MS_U16 u16FltId, MS_U8 u8Sid)
{
    ...
    u16Temp = _HAL_TSO_PageTableIndR(TSO_PIDFLT_ADDR(u16FltId));
    _HAL_TSO_PageTableIndW_withSid(TSO_PIDFLT_ADDR(u16FltId), u16Temp, u8Sid);
}

MS_BOOL HAL_TSO_Flt_SetBypassTSIO(MS_U8 u8Eng, MS_U16 u16PidFltId, MS_BOOL bBypassTSIO)
{
    // not support
    return FALSE;
}
