...
static void _DMX_TSIO_Proc_Task(void)
{
    ...
    if(_pInstantDmx == NULL)
    {
        printf("[DMX Error][%s] Create Service task fail!!!\n", __FUNCTION__);
    }
...
static void _DMX_TSP_Proc_SecCB(MS_U32 u32FltId, DMX_EVENT NotifyEvent)
{
    ...
    {
        if((stCbParam.Event & NotifyEvent) != NotifyEvent) //check if the event is matched
        {
            printf("[DMX ERR] Event Err!! %x %x\n", (int)NotifyEvent, (int)stCbParam.Event);
            return;
        }
...
static void _DMX_TSP_Proc_Task(void)
{
    ...
    MApi_DMX_GetCap(DMX_CAP_SEC_FILTER_NUM, (void*)&u32PidFltNum);

    memset(&stTspIoSig, 0x00, sizeof(stTspIoSig));
        ...
        if(stTspIoSig.u32DmxEvt & (DMX_EVENT_DATA_READY|DMX_EVENT_SEC_CRCERROR|DMX_EVENT_BUF_OVERFLOW))
        {
            ...
            for(u32ii = 0; u32ii < u32PidGroup; u32ii++)
            {
                ...
                for(u32jj = 0; u32jj < 32; u32jj++)
                {
                    ...
                    if((stTspIoSig.u32DmxEvt & DMX_EVENT_DATA_READY) && (stTspIoSig.u32SecRdyFlag[u32ii] & u32Mask))
                    {
                        _DMX_TSP_Proc_SecCB(u32FltId, DMX_EVENT_DATA_READY);
                        stTspIoSig.u32SecRdyFlag[u32ii] &= ~u32Mask;
                    }

                    if((stTspIoSig.u32DmxEvt & DMX_EVENT_SEC_CRCERROR) && (stTspIoSig.u32SecCrcErrFlag[u32ii] & u32Mask))
                    {
                        _DMX_TSP_Proc_SecCB(u32FltId, DMX_EVENT_SEC_CRCERROR);
                        stTspIoSig.u32SecCrcErrFlag[u32ii] &= ~u32Mask;
                    }

                    if((stTspIoSig.u32DmxEvt & DMX_EVENT_BUF_OVERFLOW) && (stTspIoSig.u32SecOvfFlag[u32ii] & u32Mask))
                    {
...
DMX_FILTER_STATUS MApi_DMX_GetOwner(MS_U32 u32DmxId, MS_BOOL* pbOwner)
{
    ...
    *pbOwner = (MS_BOOL)u32Param[1];
    ...
    return _MApi_DMX_GetOwner(u32DmxId, pbOwner);
...
DMX_FILTER_STATUS MApi_DMX_Pvr_Eng_Pid_Open(DMX_PVR_ENG Eng ,MS_U32 Pid, MS_U8* pu8DmxId)
{
    ...
    *pu8DmxId = (MS_U8)(PvrFltTypeParam.u32DmxId & 0xFF);
    ...
    MS_U32  u32DmxId = 0;
    DMX_FILTER_STATUS eRet = _MApi_DMX_Pvr_Eng_Pid_Open(Eng, Pid, &u32DmxId);
    *pu8DmxId = (MS_U8)(u32DmxId & 0xFF);
    return eRet;
#endif
...
{
    return MApi_DMX_Pvr_Eng_SetPacketMode_Ex(Eng, ((bSet)? DMX_PKTMODE_192 : DMX_PKTMODE_188));
}

DMX_FILTER_STATUS MApi_DMX_Pvr_Eng_SetPacketMode_Ex(DMX_PVR_ENG Eng, DMX_Pvr_PacketMode ePvrPktMode)
{
    ...
    return _MApi_DMX_Pvr_Eng_SetPacketMode(Eng, ePvrPktMode);
...
//CILINK
DMX_FILTER_STATUS MApi_DMX_CILINK_Config(DMX_CILINK_ENG eEng,DMX_CILINK_CMD eCmd, void* pData)
{
#ifdef DMX_UTOPIA_20
    DMX_CILINK_CONFIG_PARAM sCtrlParam;
    ...
    sCtrlParam.eEng = eEng;
    sCtrlParam.eCmd = eCmd;
    sCtrlParam.pData = pData;
    sCtrlParam.u32Res = 0;

    if (UtopiaIoctl(_pInstantDmx, MApi_CMD_DMX_CILINK_CONFIG,(void*)&sCtrlParam) != UTOPIA_STATUS_SUCCESS)
    {
        DMX_V2DBGMSG(ULOGE("DMX", "[MAPI DMX][%s][%06d] fail\n", __FUNCTION__, __LINE__));
        return DMX_FILTER_STATUS_ERROR;
    }

    if(sCtrlParam.u32Res != (MS_U32)DMX_FILTER_STATUS_OK)
        return DMX_FILTER_STATUS_ERROR;
    else
        return DMX_FILTER_STATUS_OK;
#else

    return _MApi_DMX_CILINK_Config(eEng, eCmd, pData);
...
DMX_FILTER_STATUS MApi_DMX_TSIO_Service_PidOpen(MS_U16 u16Handle, TSIO_FLT_CFG *pstFltCfg)
{
#ifdef DMX_UTOPIA_20
    ...
    return _MApi_DMX_TSIO_Service_PidOpen(u16Handle, pstFltCfg);

#endif
}

DMX_FILTER_STATUS MApi_DMX_TSIO_Service_PidClose(MS_U16 u16Handle, MS_U32 u32DmxId)
{
#ifdef DMX_UTOPIA_20
    ...
    return _MApi_DMX_TSIO_Service_PidClose(u16Handle,  u32DmxId);
