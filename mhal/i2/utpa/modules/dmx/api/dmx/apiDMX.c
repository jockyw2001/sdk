...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
#if (defined(DMX_UTOPIA_2K) && defined(MSOS_TYPE_LINUX_KERNEL))
#define CPY_FROM_USER(a,b,c)                    copy_from_user((void*)a, (void*)b, c)
#define CPY_to_USER(a,b,c)                      copy_to_user((void*)a, (void*)b, c)
#else
#define CPY_FROM_USER(a,b,c)                    memcpy((void*)a, (void*)b, c)
#define CPY_to_USER(a,b,c)                      memcpy((void*)a, (void*)b, c)
#endif  //MSOS_TYPE_LINUX_KERNEL

#if (defined(DMX_UTOPIA_2K) && defined(MSOS_TYPE_LINUX_KERNEL) && defined(CONFIG_COMPAT))
#define CMP_CPY_FROM_USER(a,b,c)                (is_compat_task()? copy_from_user((void*)a, compat_ptr((unsigned long)b), c) : copy_from_user((void*)a, (void*)b, c))
#define CMP_CPY_TO_USER(a,b,c)                  (is_compat_task()? copy_to_user(compat_ptr((unsigned long)a), (void*)b, c) : copy_to_user((void*)a, (void*)b, c))
#else
...
#ifdef TSIO_ENABLE
#define TSIO_FLTID_PREFIX_MASK             (0xF0000000UL)
#define TSIO_FLTID_PREFIX_TSO              (0x00000000UL)
#define TSIO_FLTID_PREFIX_DMAIN            (0x10000000UL)
#endif
...
typedef struct _DMX_RESOURCE_PRIVATE
{
    ...
    MS_U32                      _u32OrgCAEngSel[E_DRVTSP_IF_TOTAL_NUM];  //for TV chip, E_DRVTSP_IF_NUM + E_DRVTSP_IF_FI_NUM
#endif
    ...
    MS_BOOL                     _bFileinTeeSupport[DMX_FILEIN_INVALID];
    MS_BOOL                     _bMMFITeeSupport[2];
    MS_BOOL                     _bPVRTeeSupport[DMX_MAX_IF_NUM] ;
...
#define _FLT_LIST_TYPE_GET(DMXId, pDmxFltType)                                                                                   \
                *(pDmxFltType) = _pdmx_res->_FltList[(DMXId)].DmxFltType & (DMX_FILTER_TYPE)(~DMX_FILTER_FLT_MASK);              \
...
static DMX_RESOURCE_PRIVATE         _dmx_res = {
  ...
  #endif
  #if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    ._u32OrgCAEngSel                            = {[0 ... (E_DRVTSP_IF_TOTAL_NUM-1)] = 0xFF},
  #endif
    ._u32PVREngSrc                              = {0},
...
#endif
    ._stSecBuf                                   = {0, 0, 0, 0},
    ._bFileinTeeSupport                         = {[0 ... (DMX_FILEIN_INVALID-1)] = TRUE},
    ._bMMFITeeSupport                           = {TRUE, TRUE},
    ._bPVRTeeSupport                            = {[0 ... (DMX_MAX_IF_NUM-1)] = TRUE},
...
//for user mode multiple process, init counter of the same process
static MS_U32                       _u32LocalInitCnt = 0;
...
static MS_BOOL _DMX_InitResource(DMX_RESOURCE_PRIVATE* presource)
{
    ...
    for(u32ii = 0; u32ii < DMX_MAX_IF_NUM; u32ii++)
    {
        ...
        presource->_bPVRTeeSupport[u32ii] = TRUE;
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    for(u32ii = 0; u32ii < E_DRVTSP_IF_TOTAL_NUM; u32ii++)
    {
        presource->_u32OrgCAEngSel[u32ii] = 0xFF;
    }
#endif
    ...
    for(u32ii = 0; u32ii < DMX_FILEIN_INVALID; u32ii++)
    {
        presource->_bFileinTeeSupport[u32ii] = TRUE;
    }

    for(u32ii = 0; u32ii < 2; u32ii++)
    {
        presource->_bMMFITeeSupport[u32ii] = TRUE;
    }
...
static MS_BOOL _DMX_ChkBufOverlap(MS_U32 u32DmxId, MS_PHY phyAddr, MS_U32 u32Size)
{
    ...
    for(idx = _pdmx_res->_u32TspFltNum - 1; idx >= 0 ; idx--)
    {
        ...
        if((type == DMX_FILTER_TYPE_SECTION)  ||
           ...
           (type == DMX_FILTER_TYPE_DSS_130)  ||
           (type == DMX_FILTER_TYPE_DSS_130_AUX_PKT))
        {   // filter is opened
...
static MS_BOOL _DMX_ApiDrv_DmxFlowReverseMapping(DrvTSP_If eTSIf, MS_U32 *pu32DmxFlow)
{
    *pu32DmxFlow = 0;
    ...
    switch (eTSIf)
    {
        ...
        case E_TSP_TSIF_6:
            *pu32DmxFlow = DMX_FLOW_PLAYBACK6;
            break;
        default:
            DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Unsupported DrvTSP_If type error!\n", __LINE__));
            return FALSE;
    }
...
#elif(!(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_1_0))) // For TSP_VER_3_0
static MS_BOOL _DMX_ApiDrv_DmxFlowReverseMapping(DRV_TSP_PATH_TYPE ePathType, MS_U32 u32PathID, MS_U32 *pu32DmxFlow)
{
    *pu32DmxFlow = 0;
    if(ePathType ==E_DRV_TSP_PATH_LIVE)
    {
        switch (u32PathID)
        {
            case E_DRVTSP_LIVE0:
            ...
            case E_DRVTSP_LIVE2:
                *pu32DmxFlow = DMX_FLOW_PLAYBACK2;
                break;
            case E_DRVTSP_LIVE_FI:
                *pu32DmxFlow = DMX_FLOW_PLAYBACK_FI;
                break;
            default:
                DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Unsupported Path ID error!\n", __LINE__));
                return FALSE;
        }
    }
    else
...
static MS_BOOL _DMX_ApiDrv_DmxFlowMapping(DRV_TSP_PATH_TYPE ePathType, DMX_FLOW eDmxFlow,MS_U32 *pu32TSIf)
{
    *pu32TSIf = 0;
    if(ePathType ==E_DRV_TSP_PATH_LIVE)
    {
        ...
        {
            case DMX_FLOW_PLAYBACK:
                *pu32TSIf = E_DRVTSP_LIVE0;
                break;
            case DMX_FLOW_PLAYBACK1:
                *pu32TSIf = E_DRVTSP_LIVE1;
                break;
            case DMX_FLOW_PLAYBACK2:
                *pu32TSIf = E_DRVTSP_LIVE2;
                break;
            case DMX_FLOW_PLAYBACK_FI:
                *pu32TSIf = E_DRVTSP_LIVE_FI;
                break;
            default:
                DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Unsupported DMX_FLOW type error!\n", __LINE__));
                return FALSE;
        }
    }
    else
    {
        ...
        {
            case DMX_FLOW_PLAYBACK:
                *pu32TSIf = E_DRVTSP_FILE0;
                break;
            case DMX_FLOW_PLAYBACK1:
                *pu32TSIf = E_DRVTSP_FILE1;
                break;
            case DMX_FLOW_PLAYBACK2:
                *pu32TSIf = E_DRVTSP_FILE2;
                break;
            default:
                DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Unsupported DMX_FLOW type error!\n", __LINE__));
                return FALSE;
...
#if(!(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_1_0))) // For TSP_VER_3_0 or TSP_VER_4_0
static MS_BOOL _DMX_TsPadMapping(DrvTSP_PadIn ePad, DMX_FLOW_INPUT *pDmxFlowInput)
{
    switch(ePad)
    {
        case E_DRVTSP_PAD_DEMOD0:
            *pDmxFlowInput = DMX_FLOW_INPUT_DEMOD;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: DEMOD, "));
            break;
        case E_DRVTSP_PAD_DEMOD1:
        ...
        case E_DRVTSP_PAD_EXT_INPUT5:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT5;
            ...
            break;
        case E_DRVTSP_PAD_EXT_INPUT6:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT6;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input6, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT7:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT7;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input7, "));
            break;
        case E_DRVTSP_PAD_TSO0:
            *pDmxFlowInput = DMX_FLOW_INPUT_TSO;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: TSO0, "));
            break;
        case E_DRVTSP_PAD_TSO1:
            *pDmxFlowInput = DMX_FLOW_INPUT_TSO1;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: TSO1, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT0_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT0_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input0 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT1_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT1_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input1 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT2_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT2_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input2 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT3_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT3_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input3 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT4_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT4_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input4 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT5_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT5_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input5 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT6_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT6_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input6 3WIRE, "));
            break;
        case E_DRVTSP_PAD_EXT_INPUT7_3WIRE:
            *pDmxFlowInput = DMX_FLOW_INPUT_EXT_INPUT7_3WIRE;
            DMX_DBGMSG(DMX_DBG_INFO, ULOGD("DMX", "Flow Input: Extern Input7 3WIRE, "));
            break;
        default:
...
#endif
#if((defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_1_0))) // For TSP_VER_1_0
static MS_BOOL _DMX_TsPadMapping(DrvTSP_PadIn ePad, DMX_FLOW_INPUT *pDmxFlowInput)
{
    return TRUE;
}
...
{
    DrvTSP_If Out_TSP_If = E_TSP_TSIF_INVALID;

    switch(eTsIf)
    {
        case DMX_TSIF_LIVE0:
        case DMX_TSIF_FILE0:
            Out_TSP_If = E_TSP_TSIF_0;
            break;
        case DMX_TSIF_LIVE1:
        case DMX_TSIF_FILE1:
            Out_TSP_If = E_TSP_TSIF_1;
            break;
        case DMX_TSIF_LIVE2:
        case DMX_TSIF_FILE2:
            Out_TSP_If = E_TSP_TSIF_2;
            break;
        case DMX_TSIF_LIVE3:
        case DMX_TSIF_FILE3:
            Out_TSP_If = E_TSP_TSIF_3;
            break;
        case DMX_TSIF_LIVE4:
        case DMX_TSIF_FILE4:
            Out_TSP_If = E_TSP_TSIF_4;
            break;
        case DMX_TSIF_LIVE5:
        case DMX_TSIF_FILE5:
            Out_TSP_If = E_TSP_TSIF_5;
            break;
        case DMX_TSIF_LIVE6:
        case DMX_TSIF_FILE6:
            Out_TSP_If = E_TSP_TSIF_6;
            break;
...
#if (defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_3_0)||(TSP_VERSION == TSP_VER_1_0)))
static MS_BOOL _DMX_PvrEng2AlpEng(DMX_PVR_ENG eEng, MS_U32* pu32AlpEng)
{
    *pu32AlpEng = 0xFFFFFFFF;
    if((eEng >= DMX_ALPPVR_ENG0) && (eEng <= DMX_ALPPVR_ENG3))
    {
        *pu32AlpEng = (MS_U32)(eEng & ~DMX_ALPPVR_ENG0);
        //printf("[%s][%d] eEng %x , alp %d\n", __FUNCTION__, __LINE__, (int)eEng, (int)*pu32AlpEng);
        return TRUE;
    }
    else
    {
...
static DMX_FILTER_STATUS _DMX_Open_Stream(MS_U32* pu32DmxId, DMX_FILTER_TYPE DmxFltType)
{
    ...
    _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, 0xFFFFFFFF, DmxFltType | u32TSPFltSource);
    *pu32DmxId = FltId;
...
static DMX_FILTER_STATUS _DMX_Open_Stream(MS_U32* pu32DmxId, DMX_FILTER_TYPE DmxFltType)
{
    ...
    u32tmpTSPFltSource = DmxFltType & ((MS_U32)DMX_FILTER_FLT_MASK);
    DmxFltType = DmxFltType & (DMX_FILTER_TYPE)(~DMX_FILTER_FLT_MASK);
    ...
    _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, 0xFFFFFFFF, DmxFltType | u32tmpTSPFltSource);
    *pu32DmxId = FltId;
...
_Open_Sect_end:
    ...
    _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, SecBufId, DmxFltType | u32TSPFltSource);
...
static DMX_FILTER_STATUS _DMX_Open_Sect(MS_U32* pu32DmxId, DMX_FILTER_TYPE DmxFltType)
{
    ...
    DMX_ASSERT(
        ...
        (DMX_FILTER_TYPE_PES== DmxFltType) || (DMX_FILTER_TYPE_PACKET== DmxFltType) ||
        (DMX_FILTER_TYPE_PCR== DmxFltType) || (DMX_FILTER_TYPE_SECTION_VER== DmxFltType) ||
        (DMX_FILTER_TYPE_SECTION_NO_PUSI== DmxFltType) || (DMX_FILTER_TYPE_DSS_130== DmxFltType) ||
        (DMX_FILTER_TYPE_DSS_130_AUX_PKT== DmxFltType),
        DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Unknown error. Should NOT happen\n", __LINE__)));
    ...
    switch (DmxFltType)
    {
        ...
        case DMX_FILTER_TYPE_SECTION_NO_PUSI:
            eTspFltType = E_DRVTSP_FLT_TYPE_SECTION_NO_PUSI;
            break;
        case DMX_FILTER_TYPE_DSS_130:
            eTspFltType = E_DRVTSP_FLT_TYPE_DSS_130;
            break;
        case DMX_FILTER_TYPE_DSS_130_AUX_PKT:
            eTspFltType = E_DRVTSP_FLT_TYPE_DSS_130_AUX_PKT;
            break;
    }

    if(_pdmx_res->_bIsDMXOpenCalled)
    {
        ret = MDrv_TSP_PidFlt_Alloc(_Owner, eTspFltType | u32TSPFltSource, &FltId);
    }
    else if(_pdmx_res->_bIsDMXOpenExCalled)
    {
        ret = MDrv_TSP_PidFlt_Alloc_Ex(_Owner, eTspFltType | u32TSPFltSource, &FltId);
    }
    else
    {
        return DMX_FILTER_STATUS_ERROR;
    ...
    if(ret == DRVTSP_NOT_SUPPORTED)
        return DMX_FILTER_STATUS_NOT_SUPPORT;
    else if(ret == DRVTSP_FAIL)
        return DMX_FILTER_STATUS_ERROR;
    ...
    if ( (DMX_FILTER_TYPE_SECTION == DmxFltType) || (DMX_FILTER_TYPE_PES == DmxFltType) ||
         ...
         (DMX_FILTER_TYPE_SECTION_VER == DmxFltType) || (DMX_FILTER_TYPE_SECTION_NO_PUSI == DmxFltType) ||
         (DMX_FILTER_TYPE_DSS_130 == DmxFltType) || (DMX_FILTER_TYPE_DSS_130_AUX_PKT == DmxFltType) )
    {
        if (DRVTSP_OK!= MDrv_TSP_SecFlt_Alloc(_Owner, &SecBufId))
        {
            MDrv_TSP_PidFlt_Free(0 , FltId);
...
// @F_TODO check if DmxFltType is use else where for sw status then use it to set HW status
    _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, SecBufId, DmxFltType | u32tmpTSPFltSource);
...
static MS_BOOL _DMX_PVR_FlowSet(DMX_PVR_ENG eEng, DMX_TSIF ePvrSrcTSIf, MS_BOOL bDscmbRec)
{
    ...
    switch (ePvrSrcTSIf)
    {
        case DMX_TSIF_LIVE0:
            ...
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_LIVE;
            break;
        case DMX_TSIF_LIVE1:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE1;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS1;
            break;
        case DMX_TSIF_LIVE2:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE2;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS2;
            break;
        case DMX_TSIF_LIVE3:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE3;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS3;
            break;
        case DMX_TSIF_FILE0:
            u32PVRSrc = E_TSP_FLT_SRC_FILE0;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_FILE;
            break;
        case DMX_TSIF_FILE1:
            u32PVRSrc = E_TSP_FLT_SRC_FILE1;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_FILE1;
            break;
        case DMX_TSIF_FILE2:
            u32PVRSrc = E_TSP_FLT_SRC_FILE2;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_FILE2;
            break;
        case DMX_TSIF_FILE3:
            u32PVRSrc = E_TSP_FLT_SRC_FILE3;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_FILE3;
            break;
        case DMX_TSIF_LIVE4:
        case DMX_TSIF_FILE4:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE4;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS4;
            break;
        case DMX_TSIF_LIVE5:
        case DMX_TSIF_FILE5:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE5;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS5;
            break;
        case DMX_TSIF_LIVE6:
        case DMX_TSIF_FILE6:
            u32PVRSrc = E_TSP_FLT_SRC_LIVE6;
            _pdmx_res->_u32PVREngSrc[eEng] = DMX_FILTER_SOURCE_TYPE_TS6;
            break;
        default:
            return FALSE;
    }
    ...
    if((pstInputCfg->eTSOInIf != DMX_TSIF_FILE0) && (pstInputCfg->eTSOInIf != DMX_TSIF_FILE1))
    {
        ...
        switch (pstInputCfg->stInputInfo.Input)
        {
            ...
            case DMX_FLOW_INPUT_EXT_INPUT6_3WIRE:
                ...
                break;
#ifdef CILINK_SUPPORT
            case DMX_FLOW_INPUT_CILINK1:
                IfSet.ePad = E_DRVTSO_PAD_CILINK1;
            case DMX_FLOW_INPUT_CILINK2:
                IfSet.ePad = E_DRVTSO_PAD_CILINK2;
#endif
            default:
                return FALSE;
        }
...
#if (!defined(TSO_HW_BOX_SERIES) ||(defined(TSO_HW_BOX_SERIES) && defined(CILINK_SUPPORT)))
    MS_U32  u32OutputPad = DMX_FLOW_OUTPUT_NONE;
#endif
...
#if (!defined(TSO_HW_BOX_SERIES) ||(defined(TSO_HW_BOX_SERIES) && defined(CILINK_SUPPORT)))
    if(pstOutputCfg->eOutPad != DMX_FLOW_OUTPUT_NONE)
    {
        switch(pstOutputCfg->eOutPad)
        {
            case DMX_FLOW_OUTPUT_EXT_PAD1:
                u32OutputPad = E_DRVTSP_OUTPAD_PAD1;
                break;
            case DMX_FLOW_OUTPUT_EXT_PAD3:
                u32OutputPad = E_DRVTSP_OUTPAD_PAD3;
                break;
            case DMX_FLOW_OUTPUT_CILINK1:
                u32OutputPad = E_DRVTSP_OUTPAD_CILINK1;
                break;
            case DMX_FLOW_OUTPUT_CILINK2:
                u32OutputPad = E_DRVTSP_OUTPAD_CILINK2;
                break;
            default:
                ULOGD("DMX", "[%s][%d] \n", __FUNCTION__, __LINE__);
                return FALSE;
        ...
        if(MDrv_TSP_OutputPadCfg(u32OutputPad|E_DRVTSP_OUTPAD_FROM_TSO, E_DRVTSP_PAD_TSO0, TRUE, TRUE) != DRVTSP_OK)
        {
            ULOGD("DMX", "[%s][%d] \n", __FUNCTION__, __LINE__);
            return FALSE;
        }
...
#if (defined(TSO_ENABLE) && (!defined(TSO_20_ENABLE)) && (!defined(TSO_HW_BOX_SERIES)))
...
//TSO HW 1.0/2.0  for BOX series + TSO HW 2.0 for TV series
static MS_BOOL _DMX_FlowSetTSO(DMX_FLOW DmxFlow, DMX_FLOW_INPUT DmxFlowInput, MS_BOOL bClkInv, MS_BOOL bExtSync, MS_BOOL bParallel)
{

  #ifdef TSO_20_ENABLE
    #ifdef CILINK_SUPPORT
    DrvTSP_PadOut eOutputPad;
    #endif
    ...
    if((DmxFlow == DMX_FLOW_TSO_PLAYBACK) || (DmxFlow == DMX_FLOW_TSO_PLAYBACK1))
    {
        ...
        switch (DmxFlowInput)
        {
            ...
            case DMX_FLOW_INPUT_CILINK1:
                stOutputCfg.eTsoOutClk = E_DRVTSO_OUTCLK_PTSOOUT;
                stOutputCfg.eTsoOutClkSrc = E_DRVTSO_OUTCLKSRC_P_CILINK1;
                break;
            case DMX_FLOW_INPUT_CILINK2:
                stOutputCfg.eTsoOutClk = E_DRVTSO_OUTCLK_PTSOOUT;
                stOutputCfg.eTsoOutClkSrc = E_DRVTSO_OUTCLKSRC_P_CILINK2;
                break;
#endif
            case DMX_FLOW_INPUT_MEM:
...
#ifdef CILINK_SUPPORT
        MDrv_TSP_GetDefaultOutputPad(&eOutputPad);

        if(eOutputPad == E_DRVTSP_OUTPAD_PAD1)
        {
            stOutputCfg.eOutPad = DMX_FLOW_OUTPUT_EXT_PAD1;
        }
        else if(eOutputPad == E_DRVTSP_OUTPAD_CILINK1)
        {
    ...
    if((DMX_FLOW_TSO_PLAYBACK == DmxFlow) || (DMX_FLOW_TSO_PLAYBACK1 == DmxFlow))
    {
    ...
    MS_U8 u8TSOEng = 0;
    DrvTSP_PadIn ePad = E_DRVTSP_INVALID;
    switch (DmxFlowInput)
    {
        case DMX_FLOW_INPUT_DEMOD:
            ePad = E_DRVTSP_PAD_DEMOD0;
            eDrvTSOCtrlMode = E_DRVTSO_CTRL_MODE_DEMOD0;
            break;
        case DMX_FLOW_INPUT_DEMOD1:
            ePad = E_DRVTSP_PAD_DEMOD1;
...
#endif //end of _DMX_FlowSetTSO

#ifdef TSIO_ENABLE
static MS_BOOL _DMX_FlowSetTSIO(DMX_FLOW DmxFlow, DMX_FLOW_INPUT DmxFlowInput, MS_BOOL bClkInv, MS_BOOL bExtSync, MS_BOOL bParallel)
{
    DMX_TSO_InputCfg stInputCfg = {};
    MS_U16 u16Size = 200;
...
//--------------------------------------------------------------------------
static MS_U32 _DMX_Get_DrvTSP_If_Index(DrvTSP_If eTspIf)
{
    if( (eTspIf==E_DRVTSP_IF_NUM) || (eTspIf==E_DRVTSP_IF_FI_NUM) )
    {
        DMX_DBGMSG(DMX_DBG_ERR, ULOGW("DMX", "Warning: [MAPI DMX][%06d] Invalid DrvTSP_If !\n", __LINE__));

        return 0;
    }

    if(eTspIf >= E_DRVTSP_IF_FI_BASE)
    {
        return (MS_U32)(eTspIf-E_DRVTSP_IF_FI_BASE+E_DRVTSP_IF_NUM);
    }
    else
    {
        return (MS_U32)eTspIf;
    }
}
...
static MS_BOOL _DMX_FlowSet(DMX_FLOW DmxFlow, DMX_FLOW_INPUT DmxFlowInput, MS_BOOL bClkInv, MS_BOOL bExtSync, MS_BOOL bParallel)
{
        ...
        || (DMX_FLOW_INPUT_MEM_PASSCA1 == DmxFlowInput))
    {
        ...
        {
            MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, E_DRVTSP_FILE0, TRUE);
            if(DmxFlowInput == DMX_FLOW_INPUT_MEM)
            {
                MDrv_TSP_GetDscmbEngIdx_BySource(E_DRVTSP_PKTSRC_DEMUXFILE, &u32DScmbEngId);
                if(u32DScmbEngId == 0xFF)
                {
                    u32DScmbEngId = _pdmx_res->_u32OrgCAEngSel[_DMX_Get_DrvTSP_If_Index(E_DRVTSP_IF_FI)];
                }
            ...
            else if(DmxFlowInput == DMX_FLOW_INPUT_MEM_NOPASSCA)
            {
                ...
                if(u32DScmbEngId != 0xFF)
                {
                    _pdmx_res->_u32OrgCAEngSel[_DMX_Get_DrvTSP_If_Index(E_DRVTSP_IF_FI)] = u32DScmbEngId;
                }
            ...
            else if(DmxFlowInput == DMX_FLOW_INPUT_MEM_PASSCA1)
            {
                ...
                MDrv_TSP_SetOperateMode(0, E_DRVTSP_CTRL_MODE_MEM, u32DScmbEngId);
            }
        ...
        {
            MDrv_MMFI_EngClk(E_DRVMMFI_PATH0, TRUE);
        ...
        {
            MDrv_MMFI_EngClk(E_DRVMMFI_PATH1, TRUE);
        ...
        {
            MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, E_DRVTSP_FILE1, TRUE);
            MDrv_MMFI_EngClk(E_DRVMMFI_PATH0, TRUE);
            bFileMode = TRUE;
            MDrv_MMFI_File_CmdQ_Reset(E_DRVMMFI_PATH0);
        }
        ...
        {
            MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, E_DRVTSP_FILE2, TRUE);
            MDrv_MMFI_EngClk(E_DRVMMFI_PATH1, TRUE);
            bFileMode = TRUE;
            MDrv_MMFI_File_CmdQ_Reset(E_DRVMMFI_PATH1);
        }
        ...
        {
            MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, u32PathId, FALSE);
  ...
  #ifdef CILINK_SUPPORT
        case DMX_FLOW_INPUT_CILINK1:
          IfSet.ePad = E_DRVTSP_PAD_CILINK1;
          break;
        case DMX_FLOW_INPUT_CILINK2:
          IfSet.ePad = E_DRVTSP_PAD_CILINK2;
          break;
  #endif
  #ifdef TSO_ENABLE
    ...
    //check if there is ALP module
    if((DmxFlow >= DMX_FLOW_ALP_TS0) && (DmxFlow <= DMX_FLOW_ALP_TS2))
    {
        MS_U32  u32data = 0;

        //printf("[%s][%d] ALP Eng %d\n", __FUNCTION__, __LINE__, (int)u32data);
        if(MDrv_TSP_GetCap(E_DRVTSP_CAP_ALPIF_NUM, &u32data) == FALSE)
            return FALSE;
        if(u32data < 1)
            return FALSE;
        //printf("[%s][%d] ALP Eng %d\n", __FUNCTION__, __LINE__, (int)u32data);
    ...
    DrvTSP_LIVE_PATH u32PathID = E_DRVTSP_LIVE0;

    switch(DmxFlow)
    {
        case DMX_FLOW_PLAYBACK:
        case DMX_FLOW_ALP_TS0:
        case DMX_FLOW_PLAYBACK_NOCA:
            TspIf = E_DRVTSP_IF_TS0;
            CtrlMode = E_DRVTSP_CTRL_MODE_TS0;
            u32PathID = E_DRVTSP_LIVE0;
            MDrv_TSP_GetDscmbEngIdx_BySource(E_DRVTSP_PKTSRC_DEMUX0, &u32DScmbEngId);
            if(DmxFlow == DMX_FLOW_PLAYBACK_NOCA)
            {
                if(u32DScmbEngId != 0xFF)
                    _pdmx_res->_u32OrgCAEngSel[_DMX_Get_DrvTSP_If_Index(E_DRVTSP_IF_TS0)] = u32DScmbEngId;
                u32DScmbEngId = 0xFF;
            }
            else
            {
                if(u32DScmbEngId == 0xFF)
                    u32DScmbEngId = _pdmx_res->_u32OrgCAEngSel[_DMX_Get_DrvTSP_If_Index(E_DRVTSP_IF_TS0)];
            }
            MDrv_TSP_ALP_Enable(E_DRVTSP_ALP0, (DmxFlow == DMX_FLOW_ALP_TS0));
            break;
        case DMX_FLOW_PLAYBACK1:
        case DMX_FLOW_ALP_TS1:
            TspIf = E_DRVTSP_IF_TS1;
            CtrlMode = E_DRVTSP_CTRL_MODE_TS1;
            u32PathID = E_DRVTSP_LIVE1;
            MDrv_TSP_GetDscmbEngIdx_BySource(E_DRVTSP_PKTSRC_DEMUX1, &u32DScmbEngId);
            MDrv_TSP_File_TSIFSrcSel(E_DRVTSP_FLT_SOURCE_TYPE_TS1, bFileMode); //Select MMFI CMDQ Src
            MDrv_TSP_ALP_Enable(E_DRVTSP_ALP2, (DmxFlow == DMX_FLOW_ALP_TS1));
            break;
        case DMX_FLOW_PLAYBACK2:
        case DMX_FLOW_ALP_TS2:
            TspIf = E_DRVTSP_IF_TS2;
            CtrlMode = E_DRVTSP_CTRL_MODE_TS2;
            u32PathID = E_DRVTSP_LIVE2;
            ...
            MDrv_TSP_ALP_Enable(E_DRVTSP_ALP3, (DmxFlow == DMX_FLOW_ALP_TS2));
            break;
        #ifdef MERGE_STR_SUPPORT
        case DMX_FLOW_PLAYBACK_FI:
        case DMX_FLOW_ALP_TSFI:
            TspIf = E_DRVTSP_IF_FI;
            CtrlMode = E_DRVTSP_CTRL_MODE_TSFI_LIVE;
            u32PathID = E_DRVTSP_LIVE_FI;
            MDrv_TSP_GetDscmbEngIdx_BySource(E_DRVTSP_PKTSRC_DEMUXFILE, &u32DScmbEngId);
            MDrv_TSP_File_TSIFSrcSel(E_DRVTSP_FLT_SOURCE_TYPE_FILE, bFileMode);
            MDrv_TSP_ALP_Enable(E_DRVTSP_ALP1, (DmxFlow == DMX_FLOW_ALP_TSFI));
            break;
        #endif
        case DMX_FLOW_PVR:
            TspIf = E_DRVTSP_IF_TS0;
        ...
        || (DMX_FLOW_INPUT_MEM_PASSCA1 == DmxFlowInput)))
    {
        ...
        if((TspIf == E_DRVTSP_IF_TS0) || (TspIf == E_DRVTSP_IF_TS1) || (TspIf == E_DRVTSP_IF_TS2) || (TspIf == E_DRVTSP_IF_FI))
        {
            MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, u32PathID, TRUE);
        }
    }

    #ifdef CILINK_SUPPORT
    if(DmxFlowInput == DMX_FLOW_INPUT_CILINK1)
    {
        MDrv_CILINK_Open(E_DMX_CILINK1);
    }

    if(DmxFlowInput == DMX_FLOW_INPUT_CILINK2)
    {
        MDrv_CILINK_Open(E_DMX_CILINK2);
    }
    #endif
...
static MS_BOOL _DMX_FlowSet(DMX_FLOW DmxFlow, DMX_FLOW_INPUT DmxFlowInput, MS_BOOL bClkInv, MS_BOOL bExtSync, MS_BOOL bParallel)
{
    ...
    if (DmxFlow == DMX_FLOW_TSIO_PLAYBACK)
        return _DMX_FlowSetTSIO(DmxFlow, DmxFlowInput, bClkInv, bExtSync, bParallel);
  #endif  // End of TSIO_ENABLE

    switch (DmxFlowInput)
    {
        case DMX_FLOW_INPUT_DEMOD:
            pad = E_DRVTSP_PAD_DEMOD0;
            break;
        case DMX_FLOW_INPUT_DEMOD1:
            pad = E_DRVTSP_PAD_DEMOD1;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT0:
            pad = E_DRVTSP_PAD_EXT_INPUT0;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT0_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT0_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT1:
            pad = E_DRVTSP_PAD_EXT_INPUT1;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT1_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT1_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT2:
            pad = E_DRVTSP_PAD_EXT_INPUT2;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT2_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT2_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT3:
            pad = E_DRVTSP_PAD_EXT_INPUT3;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT3_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT3_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT4:
            pad = E_DRVTSP_PAD_EXT_INPUT4;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT4_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT4_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT5:
            pad = E_DRVTSP_PAD_EXT_INPUT5;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT5_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT5_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT6:
            pad = E_DRVTSP_PAD_EXT_INPUT6;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT6_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT6_3WIRE;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT7:
            pad = E_DRVTSP_PAD_EXT_INPUT7;
            break;
        case DMX_FLOW_INPUT_EXT_INPUT7_3WIRE:
            pad = E_DRVTSP_PAD_EXT_INPUT7_3WIRE;
            break;
        case DMX_FLOW_INPUT_TSO:
            pad = E_DRVTSP_PAD_TSO0;
            break;
        case DMX_FLOW_INPUT_TSO1:
            pad = E_DRVTSP_PAD_TSO1;
            break;
        case DMX_FLOW_INPUT_MEM:
            pad = E_DRVTSP_PAD_EXT_INPUT0;
            filein = TRUE;
        ...
        case DMX_FLOW_INPUT_TSIO:
            pad = E_DRVTSP_PAD_TSIO0;
            break;
  #endif
  #ifdef CILINK_SUPPORT
        case DMX_FLOW_INPUT_CILINK1:
            pad = E_DRVTSP_PAD_CILINK1;
            break;
        case DMX_FLOW_INPUT_CILINK2:
            pad = E_DRVTSP_PAD_CILINK2;
            break;
  #endif
    ...
    if((eDst >= E_DRV_TSP_FLOW_PVR0) && (eDst <= E_DRV_TSP_FLOW_PVR3))
    {
    ...
    if(filein == TRUE)
    {
        cfg = E_TSP_TSIF_EN | E_TSP_TSIF_EXTSYNC | E_TSP_TSIF_PARL;
    }
    ...
    else
    {
        ...
        if(((DmxFlowInput >= DMX_FLOW_INPUT_EXT_INPUT0_3WIRE) && (DmxFlowInput <= DMX_FLOW_INPUT_EXT_INPUT6_3WIRE)) || (DmxFlowInput == DMX_FLOW_INPUT_EXT_INPUT7_3WIRE))
        {
            cfg |= E_TSP_TSIF_3WIRE;
        }
        if ((E_DRVTSP_INVALID != pad)) // old style we set specific path to dst
        {
...
static DMX_FILTER_STATUS _DMX_GetRingBuffer(MS_U32 u32DmxId, MS_U8* pu8Buf, MS_U32 u32BufSize, MS_U32* pu32SecSize, MS_U32* pu32RmnSize, DMX_CheckCb pfCheckCB)
{
    ...
    *pu32RmnSize = u32RbSize= _DMX_RingBuffer_DataSize(virtRead, virtWrite, virtStart, virtEnd);
    ...
    if((DmxFltType == DMX_FILTER_TYPE_PES) && (_u32PESCpy_ContMode == 0))
    {
...
DMX_FILTER_STATUS _MApi_DMX_Init(void)
{
    ...
    // printf("[%s][%d]_bEnableInitRefCnt %d, _u8InitRefCnt %d, _u32LocalInitCnt %d, _bFWInit %d, %x\n",
    //    __FUNCTION__, __LINE__, (int)_pdmx_res->_bEnableInitRefCnt, (int)_pdmx_res->_u8InitRefCnt, (int)_u32LocalInitCnt, (int)_pdmx_res->_bFWInit, (int)getpid());
    ...
    {
        if((_pdmx_res->_u8InitRefCnt > 0) && (_pdmx_res->_bFWInit == FALSE))
            return DMX_FILTER_STATUS_ERROR;

        _pdmx_res->_u8InitRefCnt++;

        if(_pdmx_res->_u32LibMode != 0) // For Utopia2K kernel mode(mix mode), Only one ISR service needs init
        {
            if(_pdmx_res->_u8InitRefCnt != 1)
                return DMX_FILTER_STATUS_OK;
        }
        else  // For user mode driver..
        {
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
        // TSP_VER_3_0 support multiple process, every process needs only init once for creating its' own ISR service
            _u32LocalInitCnt++;

            if(_u32LocalInitCnt != 1)
                return DMX_FILTER_STATUS_OK;
#else
        // Only support multiple thread now.
        if(_pdmx_res->_u8InitRefCnt != 1)
                return DMX_FILTER_STATUS_OK;
#endif
    ...
    if( ((_pdmx_res->_bEnableInitRefCnt) && (_pdmx_res->_u8InitRefCnt == 1)) || (!(_pdmx_res->_bEnableInitRefCnt)) )
    {
    ...
    // For user mode, multiple process, the init of 2nd,3rd... process, only creates ISR service
  #if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    if((_pdmx_res->_bEnableInitRefCnt) && (_pdmx_res->_u8InitRefCnt != 1) && (_u32LocalInitCnt == 1) && (_pdmx_res->_u32LibMode == 0))//not the first init
    {
        MsOS_ReleaseMutex(_pdmx_res->_Dmx_MMFI_Mutex);
        MsOS_ReleaseMutex(_pdmx_res->_Dmx_Pvr_Mutex);
...
{
    // printf("[%s] _bFWInit %d, _bEnableInitRefCnt %d, _u8InitRefCnt %d, , _u32LocalInitCnt %d, %x\n",
    //    __FUNCTION__, (int)_pdmx_res->_bFWInit, (int)_pdmx_res->_bEnableInitRefCnt, (int)_pdmx_res->_u8InitRefCnt, (int)_u32LocalInitCnt, (int)getpid());
...
DMX_FILTER_STATUS _MApi_DMX_Exit(void)
{
    ...
    // printf("[%s][%d] _bFWInit %d, _bEnableInitRefCnt %d, _u8InitRefCnt %d , _u32LocalInitCnt %d, %x\n",
    //    __FUNCTION__, __LINE__, (int)_pdmx_res->_bFWInit, (int)_pdmx_res->_bEnableInitRefCnt, (int)_pdmx_res->_u8InitRefCnt, (int)_u32LocalInitCnt, (int)getpid());

    if(_pdmx_res->_bFWInit == FALSE)
    {
        _pdmx_res->_u8InitRefCnt = 0;
        _u32LocalInitCnt = 0;
    ...
    if(_pdmx_res->_bEnableInitRefCnt)
    {
        ...
        // For Utopia2K kernel mode(mix mode)
        if(_pdmx_res->_u32LibMode != 0)
        {
            if(_pdmx_res->_u8InitRefCnt > 0)
        ...
        {
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
            // TSP_VER_3_0 support multiple process, only when local count is 0, detached ISR service
            if(_u32LocalInitCnt > 0)
                _u32LocalInitCnt--;
    ...
    if( ((_pdmx_res->_bEnableInitRefCnt) && (_pdmx_res->_u8InitRefCnt == 0)) || (!(_pdmx_res->_bEnableInitRefCnt)) )
    {
#ifdef MMFILEIN
        if (-1!= _pdmx_res->_Dmx_MMFI_Mutex)
        {
    ...
    {
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
        // TSP_VER_3_0 support multiple process, if another process still running, only detached ISR service
        if((_pdmx_res->_bEnableInitRefCnt) && (_pdmx_res->_u32LibMode == 0) && (_pdmx_res->_u8InitRefCnt > 0))
        {
            MsOS_ReleaseMutex(_pdmx_res->_Dmx_Pvr_Mutex);
            _DMX_RETURN(DMX_FILTER_STATUS_OK);
        }
...
DMX_FILTER_STATUS _MApi_DMX_Get_FlowInput_Status(DMX_FLOW DmxFlow, DMX_FLOW_INPUT *pDmxFlowInput, MS_BOOL *pbClkInv, MS_BOOL *pbExtSync, MS_BOOL *pbParallel)
{
    ...
    *pbParallel = FALSE;
    ...
    if(_DMX_TsPadMapping(Ifset.ePad, pDmxFlowInput) == FALSE)
    {
        DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "Incorrect Flow Input\n"));
        _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
...
DMX_FILTER_STATUS _MApi_DMX_Get_FlowInput_Status(DMX_FLOW DmxFlow, DMX_FLOW_INPUT *pDmxFlowInput, MS_BOOL *pbClkInv, MS_BOOL *pbExtSync, MS_BOOL *pbParallel)
{
    ...
    *pbParallel = FALSE;
    ...
    if(MDrv_TSP_GetTSIFStatus(eif, &Ifset, 0) != DRVTSP_OK)
    {
        return DMX_FILTER_STATUS_ERROR;
    }
...
DMX_FILTER_STATUS _MApi_DMX_Get_FlowInput_Status(DMX_FLOW DmxFlow, DMX_FLOW_INPUT *pDmxFlowInput, MS_BOOL *pbClkInv, MS_BOOL *pbExtSync, MS_BOOL *pbParallel)
{
    ...
    *pbParallel = FALSE;
    ...
    TsIfset.ePad = E_DRVTSP_INVALID;
    TsIfset.bClkInv = FALSE;
    TsIfset.bExtSync = FALSE;
    TsIfset.bParallel = FALSE;

    // get PAD and ClkInv for eTSIF
    if(MDrv_TSP_GetTSIFStatus((DrvTSP_If)u32TSIF, &TsIfset, &u16Clk) != E_TSP_OK)
    {
        return DMX_FILTER_STATUS_ERROR;
    }

    if(_DMX_TsPadMapping(TsIfset.ePad, pDmxFlowInput) == FALSE)
    {
        DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "Incorrect Flow Input\n"));
        _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Open(DMX_PVR_ENG Eng ,DMX_Pvr_info* pPvrInfo)
{
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    else if(_DMX_PvrEng2AlpEng(Eng, &u32AlpEng))
    {
        if(MDrv_TSP_ALP_PVR_SetBuffer(u32AlpEng, pPvrInfo->pPvrBuf0, pPvrInfo->pPvrBuf1, pPvrInfo->PvrBufSize0, (MS_U32)pPvrInfo->PvrBufSize1) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
        return DMX_FILTER_STATUS_OK;
    }
#endif

     MS_U32 u32PvrEng = (MS_U32)Eng;
#if(!(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_1_0)||(TSP_VERSION == TSP_VER_3_0))))  // For TSP_VER_4_0
    ...
    _DMX_PVR_ENTRY();
    TSP_Result ret = 0xFFFFFFFF;

#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    MDrv_TSP_FunctionClk(E_DRV_TSP_PVR, Eng, TRUE);
#endif

    _pdmx_res->_stSecBuf.u32BufId = u32PvrEng & 0xFF;
    ...
    if(_pdmx_res->_bPVRTeeSupport[u32PvrEng] == TRUE)
    {
        ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_SET_PvrBuf, (void*)&(_pdmx_res->_stSecBuf), sizeof(DrvTSP_SecureBuf));
        if(ret == DRVTSP_OK)
        {
        ...
        else if(ret == DRVTSP_TEE_NO_TA || ret == DRVTSP_TEE_NOT_SUPPORTED || ret == DRVTSP_REE_FLOW)
        {
            _pdmx_res->_bPVRTeeSupport[u32PvrEng] = FALSE;
    #if(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_1_0)||(TSP_VERSION == TSP_VER_3_0)))

      #if(defined(VQ_ENABLE) && defined(TS2_IF_SUPPORT) && (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_1_0)))
        #ifndef FIVQ_ENABLE
            if((u32PvrEng == DMX_PVR_EGN1) && _pdmx_res->_bVQEnabled)
            {
    ...
    #else // For TSP_VER_4_0
            MDrv_TSP_PVR_Eng_ClkGating(u32PvrEng, FALSE);
    #endif
            if(DRVTSP_OK != MDrv_TSP_PVR_Eng_SetBuffer(u32PvrEng, (pPvrInfo->pPvrBuf0), (pPvrInfo->pPvrBuf1),
                (MS_U32)pPvrInfo->PvrBufSize0, (MS_U32)pPvrInfo->PvrBufSize1))
            {
                _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
            }

            _pdmx_res->_PvrNotify[u32PvrEng] = pPvrInfo->pNotify;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Close(DMX_PVR_ENG Eng)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_SetBuffer(u32AlpEng, 0, 0, 0, 0) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;

        if(MDrv_TSP_ALP_Enable(u32AlpEng, FALSE) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
   ...
   MDrv_TSP_PVRCA_Close(Eng);
    ...
    if( ret == DRVTSP_OK)
    {
        DMX_DBGMSG(DMX_DBG_INFO, ULOGE("DMX", "[MAPI DMX][%06d] Ree_SendTeeCmd Success\n", __LINE__ ));
    }
#ifndef SECURE_PVR_ENABLE
    else if(ret == DRVTSP_TEE_NO_TA || ret == DRVTSP_TEE_NOT_SUPPORTED || ret == DRVTSP_REE_FLOW)
    {
    ...
    #if(defined(VQ_ENABLE) && defined(TS2_IF_SUPPORT) && (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_1_0)))
      #ifndef FIVQ_ENABLE
        if((u32PvrEng == 1) && _pdmx_res->_bVQEnabled)
        {
    ...
    #endif //VQ_ENABLE && TS2_IF_SUPPORT
  #else // For TSP_VER_4_0
        MDrv_TSP_PVR_Eng_ClkGating(u32PvrEng, TRUE);
  #endif
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    MDrv_TSP_FunctionClk(E_DRV_TSP_PVR, Eng, FALSE);
#endif

    _pdmx_res->_PvrNotify[u32PvrEng] = NULL;
    _pdmx_res->_bPVRTeeSupport[u32PvrEng]=TRUE;
    _DMX_PVR_RETURN(DMX_FILTER_STATUS_OK);
}
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Pid_Open(DMX_PVR_ENG Eng ,MS_U32 Pid, MS_U32* pu32DmxId)
{
    ...
    if (DRVTSP_OK != MDrv_TSP_PVR_Eng_FltAlloc(u32PvrEng, &FltId)) // Kaiser use tsp pidflt as PVR flt
    {
     ...
     _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, 0xFF, DMX_FILTER_TYPE_REC | _pdmx_res->_u32PVREngSrc[Eng]);
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Start(DMX_PVR_ENG Eng, MS_BOOL bPvrAll)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_Start(u32AlpEng, bPvrAll, TRUE) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
        return DMX_FILTER_STATUS_OK;
    }
#endif
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Stop(DMX_PVR_ENG Eng)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_Start(u32AlpEng, FALSE, FALSE) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_WriteGet(DMX_PVR_ENG Eng ,MS_PHY* phyWrite)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_GetWriteAddr(u32AlpEng, phyWrite) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
    ...
    if(_pdmx_res->_bPVRTeeSupport[u32PvrEng] == TRUE)
    {
        ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_GET_PvrWPtr, (void*)&(_pdmx_res->_stSecBuf), sizeof(DrvTSP_SecureBuf));
        ...
        else if(ret == DRVTSP_TEE_NO_TA || ret == DRVTSP_TEE_NOT_SUPPORTED || ret == DRVTSP_REE_FLOW)
        {
            ...
            {
                DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] \n", __LINE__ ));
                _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
            }
        }
#endif
    ...
    {
        DMX_DBGMSG(DMX_DBG_INFO, ULOGE("DMX", "[MAPI DMX][%06d] \n", __LINE__ ));
        if(DRVTSP_OK != MDrv_TSP_PVR_Eng_GetWriteAddr(u32PvrEng, phyWrite))
        {
            _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
        }
...
static TSP_PktMode _DMX_ApiDrv_PacketModeMapping(DMX_PacketMode ePacketMode)
{
    switch (ePacketMode)
    {
        ...
        case DMX_PKTMODE_PES:
            return E_TSP_PKTMODE_PES;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_SetPacketMode(DMX_PVR_ENG Eng, DMX_Pvr_PacketMode ePvrPktMode)
{
...
#ifdef TSO_PVR_SUPPORT
        _DMX_TSO_ENTRY();
        if(MDrv_TSO_PVR_SetPacketMode(Eng, (ePvrPktMode == DMX_PKTMODE_192)) == DRVTSO_OK)
        {
            _DMX_TSO_RETURN(DMX_FILTER_STATUS_OK);
        }
        else
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
    if(_DMX_PvrEng2AlpEng(Eng, &u32AlpEng))
    {
        if(MDrv_TSP_ALP_PVR_TimeStamp(u32AlpEng, (ePvrPktMode == DMX_PKTMODE_192)) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
...
#if(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_1_0)||(TSP_VERSION == TSP_VER_3_0)))
    if(DRVTSP_OK != MDrv_TSP_PVR_Eng_SetPacketMode(u32PvrEng, (ePvrPktMode == DMX_PKTMODE_192)))
#else  // For TSP_VER_4_0
    if(_DMX_ApiDrv_PvrEngMapping(Eng, &u32PvrEng) == FALSE)
    {
        _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
    ...
    if(DRVTSP_OK != MDrv_TSP_PVR_Eng_SetPacketMode(u32PvrEng, _DMX_ApiDrv_PacketModeMapping(ePvrPktMode)))
#endif
    {
        _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
    }
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_SetRecordStamp(DMX_PVR_ENG Eng ,MS_U32 u32Stamp)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_SetRecordStamp(u32AlpEng, u32Stamp) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_GetRecordStamp(DMX_PVR_ENG Eng ,MS_U32* pu32Stamp)
{
    ...
    {
        if(MDrv_TSP_ALP_PVR_GetRecordStamp(u32AlpEng, pu32Stamp) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_SetRecordStampClk(DMX_PVR_ENG Eng, DMX_TimeStamp_Clk eClkSrc)
{
    ...
    //printf("[%s][%d] Eng %d , eClkSrc %d \n", __FUNCTION__, __LINE__, (int)Eng, (int)eClkSrc);
    if(_DMX_PvrEng2AlpEng(Eng, &u32AlpEng))
    {
        if(MDrv_TSP_ALP_PVR_SetRecordStampClk(u32AlpEng, (MS_U32)eClkSrc) != DRVTSP_OK)
            return DMX_FILTER_STATUS_ERROR;
        return DMX_FILTER_STATUS_OK;
    }
...
// @F_TODO what is the difference between pvrca pid open and pvrca Eng pid open
    ...
    *pu32DmxId =  u32fltid;
    ...
    _DMX_ENTRY();
    if(MDrv_TSP_PVR_PidFlt_Reserved(Pid, (MS_U8*)pu32DmxId, TRUE) == DRVTSP_OK)
    {
        _DMX_RETURN(DMX_FILTER_STATUS_OK);
    }
    else
...
DMX_FILTER_STATUS _MApi_DMX_PvrCA_Eng_Pid_Close(DMX_PVR_ENG Eng, MS_U32 u32DmxId)
{
    ...
    if(MDrv_TSP_PVR_PidFlt_Reserved(0x1FFF, (MS_U8*)&u32DmxId, FALSE) == DRVTSP_OK)
    {
        _DMX_RETURN(DMX_FILTER_STATUS_OK);
    }
    else
...
#else   // For TSP_VER_4_0
    MDrv_TSP_File_SetPacketMode(_DMX_ApiDrv_PacketModeMapping(pFileinInfo->PKT_Mode));
#endif
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Start(DMX_FILEIN_DST Dst, MS_PHY pBuf, MS_U32 u32BufSize)
{
    ...
    if(_pdmx_res->_bFileinTeeSupport[0] == TRUE)
    {
        ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_SET_FileinBuf, (void*)&(_pdmx_res->_stSecBuf), sizeof(DrvTSP_SecureBuf));
        if( ret == DRVTSP_OK)
        {
            DMX_DBGMSG(DMX_DBG_INFO, ULOGE("DMX", "[MAPI DMX][%06d] Ree_SendTeeCmd Success\n", __LINE__ ));
        }
        ...
        {
            _pdmx_res->_bFileinTeeSupport[0] = FALSE;
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Eng_Start(DMX_FILEIN_PATH ePath, DMX_FILEIN_DST Dst, MS_PHY pBuf, MS_U32 u32BufSize)
{
    ...
    _pdmx_res->_stSecBuf.phyBufAddr = pBuf;
    _pdmx_res->_stSecBuf.u32BufSize = u32BufSize;
    if(_pdmx_res->_bFileinTeeSupport[eEng] == TRUE)
    {
        ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_SET_FileinBuf, (void*)&(_pdmx_res->_stSecBuf), sizeof(DrvTSP_SecureBuf));
        if( ret == DRVTSP_OK)
        {
        ...
        {
            _pdmx_res->_bFileinTeeSupport[eEng] = FALSE;
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Eng_Info(DMX_FILEIN_PATH ePath, DMX_Filein_info *pFileinInfo)
{
    ...
    if(ePath == DMX_FILEIN_TSIF0)
    {
        _pdmx_res->_bFileinTeeSupport[ePath] = TRUE;
        MDrv_TSP_File_SetPacketMode((DrvTSP_PacketMode)(pFileinInfo->PKT_Mode));
        return (DRVTSP_OK == MDrv_TSP_File_SetRate(pFileinInfo->Rate))? DMX_FILTER_STATUS_OK: DMX_FILTER_STATUS_ERROR;
    }
    else
    ...
    if(eEng == E_TSP_FILE_ENG_INVALID)
        return DMX_FILTER_STATUS_ERROR;
    _pdmx_res->_bFileinTeeSupport[eEng] = TRUE;
    ...
    MDrv_TSP_File_Eng_SetPacketMode(eEng, _DMX_ApiDrv_PacketModeMapping(pFileinInfo->PKT_Mode));
...
DMX_FILTER_STATUS _MApi_DMX_ResAllocate(DMX_RES_TYPE eResType, void *pRes)
{
    ...
    switch(eResType)
    {
        ...
        case DMX_RES_FLOW_PB_FILE:
            {
                ...
                #if(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
                _DMX_ApiDrv_DmxFlowReverseMapping(eDrvTspPathType, u32PathId, pRes);
                #else
                _DMX_ApiDrv_DmxFlowReverseMapping(u32PathId, pRes);
                #endif
            }
            break;
        case DMX_RES_FLOW_MMFI:
            ...
            break;
    #ifdef CILINK_SUPPORT
        case DMX_RES_CILINK1:
            eRet = MDrv_CILINK_Open(E_DMX_CILINK1);
        case DMX_RES_CILINK2:
            eRet =MDrv_CILINK_Open(E_DMX_CILINK2);
    #endif
        default:
        ...
        case DMX_RES_FLOW_PB_FILE:
            {
                ...
                #if(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
                _DMX_ApiDrv_DmxFlowMapping(eDrvTspPathType, *((MS_U32*)pRes), &u32PathId);
                #else
                _DMX_ApiDrv_DmxFlowMapping(*((MS_U32*)pRes), &u32PathId);
                #endif

                eRet = MDrv_TSP_Path_Free(eDrvTspPathType, u32PathId);

                #if(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
                MDrv_TSP_FunctionClk(E_DRV_TSP_PATH, u32PathId ,FALSE);

                if((eDrvTspPathType == E_DRV_TSP_PATH_FILE) && (u32PathId == E_DRVTSP_FILE1))  //FILE1 using MMFI0
                {
                    MDrv_MMFI_EngClk(E_DRVMMFI_PATH0, FALSE);
                }
                else if((eDrvTspPathType == E_DRV_TSP_PATH_FILE) && (u32PathId == E_DRVTSP_FILE2))  //FILE2  using MMFI1
                {
                    MDrv_MMFI_EngClk(E_DRVMMFI_PATH1, FALSE);
                }
                #endif
            }
            break;
        case DMX_RES_FLOW_MMFI:
            {
                ...
                #if(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_3_0))
                MDrv_MMFI_EngClk(ePath, FALSE);
                #endif
            }
            break;
    #ifdef CILINK_SUPPORT
        case DMX_RES_CILINK1:
            eRet = MDrv_CILINK_Close(E_DMX_CILINK1);
        case DMX_RES_CILINK2:
            eRet = MDrv_CILINK_Close(E_DMX_CILINK2);
    #endif
        default:
            DMX_DBGMSG(DMX_DBG_ERR, ULOGD("DMX", "[%s] %d  Type not support !!\n", __FUNCTION__, __LINE__));
            _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
...
DMX_FILTER_STATUS _MApi_DMX_Parl_Invert(DMX_FLOW DmxFlow, MS_BOOL bInvert)
{
    ...
    ret = MDrv_TSP_Parl_BitOrderSwap(0, (DrvTSP_If)u32TsIf, bInvert);
...
DMX_FILTER_STATUS _MApi_DMX_Flow_DscmbEng(DMX_TSIF eTsIf, MS_U32* pu32EngId, MS_BOOL bSet)
{
    ...
    DrvTSP_If eDrvTspIf = _DMX_ApiDrv_TsIfMapping(eTsIf);

    if((eRes = MDrv_TSP_DSCMB_Source(eDrvTspIf, pu32EngId, bSet)) != DRVTSP_OK)
    {
...
DMX_FILTER_STATUS _MApi_DMX_TsOutputPadCfg(DMX_FLOW_OUTPUT_PAD eOutPad, DMX_FLOW_INPUT eInSrcPad, MS_BOOL bInParallel, MS_U32 u32ResvNum, MS_U32 *pu32Resv)
{
    ...
    {
        case DMX_FLOW_OUTPUT_EXT_PAD1:
            eDrvOutPad = E_DRVTSP_OUTPAD_PAD1;
            break;
        case DMX_FLOW_OUTPUT_EXT_PAD3:
            eDrvOutPad = E_DRVTSP_OUTPAD_PAD3;
            break;
        #ifdef CILINK_SUPPORT
        case DMX_FLOW_OUTPUT_CILINK1:
            eDrvOutPad = E_DRVTSP_OUTPAD_CILINK1;
            break;
        case DMX_FLOW_OUTPUT_CILINK2:
            eDrvOutPad = E_DRVTSP_OUTPAD_CILINK2;
            break;
        #endif
        default:
            return DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_GetCap(DMX_QUERY_TYPE DmxQueryType, void* pOutput)
{
    ...
    if((DmxQueryType & DMX_CAP_TSO_FILTER_NUM) == DMX_CAP_TSO_FILTER_NUM)
    {
#ifdef TSO_ENABLE
        if(MDrv_TSO_GetCap((DrvTSO_Cap)(DmxQueryType & ~DMX_CAP_TSO_FILTER_NUM), pOutput) == DRVTSO_OK)
            return DMX_FILTER_STATUS_OK;
        else
#endif
            return DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_GetCap_Ex(char* pstrQueryType, MS_S32 s32Strlen, void* pOutput, MS_S32* ps32OutSize)
{
    ...
    *ps32OutSize = 0;
    ...
    ptr += 8;
    for(ii = 0; ii <= DMX_CAP_FIQ_NUM; ii++)
    {
        if(strcmp(ptr, _tblDmxCapStr[ii].strCap) == 0)
            break;
    }
    if(ii > DMX_CAP_FIQ_NUM)
    {
        return DMX_FILTER_STATUS_ERROR;
    }
...
DMX_FILTER_STATUS _MApi_DMX_SetPacketMode(DMX_FLOW DmxFlow, DMX_PacketMode ePktMode)
{
    ...
    if((eRes = MDrv_TSP_SetPacketMode((DrvTSP_If)u32TSIf, _DMX_ApiDrv_PacketModeMapping(ePktMode))) != DRVTSP_OK)
    {
        if(eRes == E_TSP_FAIL_NOT_SUPPORTED)
        {
            _DMX_RETURN(DMX_FILTER_STATUS_NOT_SUPPORT);
...
DMX_FILTER_STATUS _MApi_DMX_Open_MultiFlt(DMX_FILTER_TYPE DmxFltType , MS_U8* pu8DmxId, MS_U8 u8TargetDmxId)
{
    ...
    ret = MDrv_TSP_PidFlt_Alloc(_Owner , (DrvTSP_FltType)(eTspFltType|u32TSPFltSource), &FltId);
    ...
    _FLT_LIST_SECFLT_TYPE_SET(FltId, FltId, BufId, TgtDmxFltType | u32TSPFltSource);
...
DMX_FILTER_STATUS _MApi_DMX_MMFI_Filein_Start(DMX_MMFI_DST eDst, MS_PHY pBuf, MS_U32 u32BufSize)
{
...
#if defined(MMFI_VERSION) && (MMFI_VERSION >= MMFI_VER_2_0) // MMFI 2.0
    ...
    switch(eDst & (~DMX_MMFI1_PES_TYPE_MASK))
    {
        ...
        case DMX_MMFI_PES_PS_AUC:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_A3PES;
            break;
        case DMX_MMFI_PES_PS_AUD:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_A4PES;
            break;
        case DMX_MMFI_PES_PS_AUE:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_A5PES;
            break;
        case DMX_MMFI_PES_PS_AUF:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_A6PES;
            break;
        ...
        case DMX_MMFI_PES_PS_VD3:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V3PES;
            break;
        case DMX_MMFI_PES_PS_VD4:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V4PES;
            break;
        case DMX_MMFI_PES_PS_VD5:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V5PES;
            break;
        case DMX_MMFI_PES_PS_VD6:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V6PES;
            break;
        case DMX_MMFI_PES_PS_VD7:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V7PES;
            break;
        case DMX_MMFI_PES_PS_VD8:
            FileMode = E_DRVTSP_FILEIN_MODE_ENG0_V8PES;
            break;
        default:
            break;
    }

    if(FileMode != E_DRVTSP_FILEIN_MODE_ENG0_TS)
    {
        MDrv_TSP_AVFifo_SourceSelect((E_TSP_TSID_MMFI0 + path), FileMode);
    }
...
#if !(defined(MMFI_VERSION) && (MMFI_VERSION >= MMFI_VER_2_0)) // MMFI 1.0
    DrvTSP_FileinMode FileMode = E_DRVTSP_FILEIN_MODE_ENG0_TS;
    switch(eDst & (~DMX_MMFI1_PES_TYPE_MASK))
    {
        case DMX_MMFI_PES_NO_BYPASS_TS:
    ...
    MDrv_TSP_File_PS_Path_Enable(FileMode, TRUE);

#elif(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_1_0)||(TSP_VERSION == TSP_VER_3_0))) // MMFI 2.0
    ...
    if(_pdmx_res->_bMMFITeeSupport[(MS_U8)path] == TRUE)
    {
        ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_SET_MMFIBuf, (void*)&(_pdmx_res->_stSecBuf), sizeof(DrvTSP_SecureBuf));
        if( ret == DRVTSP_OK)
        {
            DMX_DBGMSG(DMX_DBG_INFO, ULOGE("DMX", "[MAPI DMX][%06d] Ree_SendTeeCmd Success\n", __LINE__ ));
        }
        ...
        {
            _pdmx_res->_bMMFITeeSupport[(MS_U8)path] = FALSE;
...
#ifdef MMFILEIN
    _DMX_MMFI_ENTRY();
    _pdmx_res->_bMMFITeeSupport[(DrvMMFI_Path)ePath] = TRUE;
        ...
        else
        {
            ...
            switch(eOutClkSrc)
            {
                ...
                case E_DRVTSO_OUTCLKSRC_P_TS5IN:
                    ...
                    break;
#ifdef CILINK_SUPPORT
                case E_DRVTSO_OUTCLKSRC_P_CILINK1:
                    pstOutputCfg->eTsoOutClkSrc = E_DMX_TSO_OUTCLKSRC_P_CILINK1;
                    break;
                case E_DRVTSO_OUTCLKSRC_P_CILINK2:
                    pstOutputCfg->eTsoOutClkSrc = E_DMX_TSO_OUTCLKSRC_P_CILINK2;
                    break;
#endif
                default:
                    break;
...
DMX_FILTER_STATUS _MApi_DMX_FQ_Configure(DMX_FQ_Cfg* pstFqCfg)
{
    ...
    if(E_DMX_FQ_CFG_SEL_SRCMUX == pstFqCfg->eCfgCmd)
    {
        if(pstFqCfg->bSet)
        {
            switch(pstFqCfg->eFqSrcSel)
            {
                ...
                case DMX_TSIF_LIVE3:
                    ...
                    break;
                case DMX_TSIF_LIVE4:
                    eFQSrcif = E_DRVFQ_SRC_TSIF4;
                    break;
                case DMX_TSIF_LIVE5:
                    eFQSrcif = E_DRVFQ_SRC_TSIF5;
                    break;
                case DMX_TSIF_LIVE6:
                    eFQSrcif = E_DRVFQ_SRC_TSIF6;
                    break;
                default:
                    _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            ...
            if(DRVTSP_NOT_SUPPORTED == MDrv_TSP_FQ_SetMuxSwitch(pstFqCfg->u32FQEng, eFQSrcif))
            {
                MDrv_FQ_MUX_SetSrc(pstFqCfg->u32FQEng, (MS_U32)eFQSrcif);
            }
            else
            {
        ...
        {
            if(DRVTSP_NOT_SUPPORTED == MDrv_TSP_FQ_GetMuxSwitch(pstFqCfg->u32FQEng, &eFQSrcif))
            {
                MS_U32  u32Path;

                MDrv_FQ_MUX_GetSrc(pstFqCfg->u32FQEng, &u32Path);
                eFQSrcif = u32Path;
            }
            else
            {
            ...
            switch(eFQSrcif)
            {
                ...
                case E_DRVFQ_SRC_TSIF3:
                    ...
                    break;
                case E_DRVFQ_SRC_TSIF4:
                    pstFqCfg->eFqSrcSel = DMX_TSIF_LIVE4;
                    break;
                case E_DRVFQ_SRC_TSIF5:
                    pstFqCfg->eFqSrcSel = DMX_TSIF_LIVE5;
                    break;
                case E_DRVFQ_SRC_TSIF6:
                    pstFqCfg->eFqSrcSel = DMX_TSIF_LIVE6;
                    break;
                default:
                    u32Ret = DMX_FILTER_STATUS_ERROR;
                    break;
    ...
    else if(E_DMX_FQ_CFG_ADDR_MODE == pstFqCfg->eCfgCmd)
    {
        if(DRVFQ_OK != MDrv_FQ_AddrMode(pstFqCfg->u32FQEng,pstFqCfg->bSet,(MS_BOOL *)pstFqCfg->pCfgPara))
            u32Ret = DMX_FILTER_STATUS_ERROR;
    }
    else
    {
...
#ifdef CILINK_SUPPORT
    if(MDrv_CILINK_InitLibResource((void*)ptrMemPos) != DRVCILINK_OK)
        return DMX_FILTER_STATUS_ERROR;
#endif
...
DMX_FILTER_STATUS _MApi_DMX_TSIO_Service_PidOpen(MS_U16 u16Handle, TSIO_FLT_CFG *pstFltCfg)
{
    ...
    DrvTSO_If eTsoif = E_DRVTSO_IF_LIVE0;
    MS_U32 u32FltIdPrefix = 0x0;
    MS_U8 u8Eng = 0;
    MS_U8 u8Sid = 0xFF;

    DMX_DBGMSG(DMX_DBG_FUNC, ULOGD("DMX", "[%s] %d\n", __FUNCTION__, __LINE__));

    switch(pstFltCfg->eFltSrc)
    {
        case E_TSIO_FLT_SRC_TSO_LIVE0:
            eTsoif = E_DRVTSO_IF_LIVE0;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_TSO_LIVE1:
            eTsoif = E_DRVTSO_IF_LIVE1;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_TSO_LIVE2:
            eTsoif = E_DRVTSO_IF_LIVE2;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_TSO_LIVE3:
            eTsoif = E_DRVTSO_IF_LIVE3;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_TSO_FILE0:
            eTsoif = E_DRVTSO_IF_FILE0;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_TSO_FILE1:
            eTsoif = E_DRVTSO_IF_FILE1;
            u32FltIdPrefix = TSIO_FLTID_PREFIX_TSO;
            break;
        case E_TSIO_FLT_SRC_DMAIN:
            u32FltIdPrefix = TSIO_FLTID_PREFIX_DMAIN;
            break;
        default:
            DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[%s][%d] Filter source(0x%x) Not Support !!\n", __FUNCTION__, __LINE__, pstFltCfg->eFltSrc));
            _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    ...
    switch(u32FltIdPrefix)
    {
        case TSIO_FLTID_PREFIX_TSO:
            if(MDrv_TSO_PidFlt_Alloc(u8Eng, (MS_U16 *)pstFltCfg->pu32DmxId) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSO_PidFlt_SetPid(u8Eng, (MS_U16)*(pstFltCfg->pu32DmxId), pstFltCfg->u16Pid) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSO_PidFlt_SetChSrc(u8Eng, (MS_U16)*(pstFltCfg->pu32DmxId), eTsoif) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSO_PidFlt_SetSid(u8Eng, (MS_U16)*(pstFltCfg->pu32DmxId), u8Sid) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSO_PidFlt_SetBypassTSIO(u8Eng, (MS_U16)*(pstFltCfg->pu32DmxId), pstFltCfg->bBypassTSIO) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            break;
        case TSIO_FLTID_PREFIX_DMAIN:
            if(MDrv_TSIO_SGDMAIN_PidFlt_Alloc((MS_U16 *)pstFltCfg->pu32DmxId) != DRVTSIO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSIO_SGDMAIN_PidFlt_SetPid((MS_U16)*(pstFltCfg->pu32DmxId), pstFltCfg->u16Pid) != DRVTSIO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSIO_SGDMAIN_PidFlt_SetSid((MS_U16)*(pstFltCfg->pu32DmxId), u8Sid) != DRVTSIO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            if(MDrv_TSIO_SGDMAIN_PidFlt_SetBypassTSIO((MS_U16)*(pstFltCfg->pu32DmxId), pstFltCfg->bBypassTSIO) != DRVTSIO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            break;
        default:
            _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR); // should not go to here
    }
    *(pstFltCfg->pu32DmxId) |= u32FltIdPrefix; // add prefix in filter id to distinguish source
    _DMX_TSO_RETURN(DMX_FILTER_STATUS_OK);
...
/// @note
//-------------------------------------------------------------------------------------------------
DMX_FILTER_STATUS _MApi_DMX_TSIO_Service_PidClose(MS_U16 u16Handle, MS_U32 u32DmxId)
{
#ifdef TSIO_ENABLE

    MS_U32 u32FltIdPrefix = u32DmxId & (MS_U32)(TSIO_FLTID_PREFIX_MASK);

    DMX_DBGMSG(DMX_DBG_FUNC, ULOGD("DMX", "[%s] %d\n", __FUNCTION__, __LINE__));

    _DMX_TSO_ENTRY();
    u32DmxId &= ~TSIO_FLTID_PREFIX_MASK;
    switch(u32FltIdPrefix)
    {
        case TSIO_FLTID_PREFIX_TSO:
            if(MDrv_TSO_PidFlt_Free(0, (MS_U16)u32DmxId) != DRVTSO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            break;
        case TSIO_FLTID_PREFIX_DMAIN:
            if(MDrv_TSIO_SGDMAIN_PidFlt_Free((MS_U16)u32DmxId) != DRVTSIO_OK)
            {
                _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
            }
            break;
        default:
            DMX_DBGMSG(DMX_DBG_ERR,  ULOGE("DMX", "[MAPI DMX][%06d] Unknown filter prifix %u\n", __LINE__, (unsigned int)u32FltIdPrefix));
            _DMX_TSO_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    _DMX_TSO_RETURN(DMX_FILTER_STATUS_OK);
...
/// @param eCmd                      \b IN: Cmd
/// @param pvPara                    \b IN/OUT: pvPara type is decided by eCmd
/// @return DMX_FILTER_STATUS
/// @note
/// It should be called before calling any TSIO API.
//-------------------------------------------------------------------------------------------------
DMX_FILTER_STATUS _MApi_DMX_CILINK_Config(DMX_CILINK_ENG eEng,DMX_CILINK_CMD eCmd, void* pData)
{
#ifdef CILINK_SUPPORT
    DrvCILINK_Result eRet;

    if(eCmd == E_DMX_CILINK_COMPANION_TO_CARD_CLK_CFG)
    {
        DMX_CILINK_ClkCfg* sClkCfg = (DMX_CILINK_ClkCfg*)pData;
        eRet = MDrv_CILINK_CPN_ToCard_CLK(eEng,sClkCfg->bSet,&(sClkCfg->bClkInv),&(sClkCfg->bSampleEnable),&(sClkCfg->u8SampleCnt));
    }
    else if(eCmd == E_DMX_CILINK_COMPANION_FROM_CARD_CLK_CFG)
    {
        DMX_CILINK_ClkCfg* sClkCfg = (DMX_CILINK_ClkCfg*)pData;
        eRet = MDrv_CILINK_CPN_FromCard_CLK(eEng,sClkCfg->bSet,&(sClkCfg->bClkInv),&(sClkCfg->bSampleEnable),&(sClkCfg->u8SampleCnt));
    }
    else
    {
        return DMX_FILTER_STATUS_ERROR;
    ...
    return (eRet == DRVCILINK_OK)?DMX_FILTER_STATUS_OK:DMX_FILTER_STATUS_ERROR;
...
static MS_BOOL _DMX_DBG_GetPidFltInfo(MS_U64* pu64ReqHdl)
{
    ...
    {
        MdbPrint(pu64ReqHdl, "%d     0x%x       ", (int)u32ii, (int)_pdmx_res->_FltList[u32ii].u16Pid);
...
static MS_BOOL _DMX_DBG_GetSecFltInfo(MS_U64* pu64ReqHdl, int StarId, int EndId)
{
    ...
    for(u32ii = StarId; u32ii <= EndId; u32ii++)
    {
        ...
        for(u32jj = 0; u32jj < DRVTSP_FILTER_DEPTH; u32jj++)
        {
            MdbPrint(pu64ReqHdl, "%x ", u8NMask[u32jj]);
            if(((u32jj+1) % 4) == 0)
                MdbPrint(pu64ReqHdl, "    ");
        }
#endif
...
DMX_FILTER_STATUS _MApi_DMX_DBG_GetModuleInfo(MS_U64* pu64ReqHdl)
{
    ...
    const char      strPad[][16] = {
                        ...
                        "TS4_3Wire", "TS5_3Wire"};
#else
    DrvTSP_If       eif = E_TSP_TSIF_INVALID;
    DMX_FLOW_INPUT  eDmxFlowInput = DMX_FLOW_INPUT_MEM;
    const char      strPad[][16] = {
                        "Inter_Dmd", "File_in", "Pad_TS0", "Pad_TS1", "Pad_TS2", "Pad_TS3", "File_In", "Inter_Dmd1",
                        "Pad_TSO", "Pad_TSO1","File_In", "File_In1", "Pad_TS4", "Pad_TS5",
                        "TS0_3Wire", "TS1_3Wire", "TS2_3Wire", "TS3_3Wire", "TS4_3Wire", "TS5_3Wire"};
    ...
    {
        eif = (DrvTSP_If)u32ii;
        Ifset.ePad = E_DRVTSP_PAD_DEMOD0;
        Ifset.bClkInv = FALSE;
        Ifset.bExtSync = FALSE;
        Ifset.bParallel = FALSE;
...
#else
        _DMX_TsPadMapping(Ifset.ePad, &eDmxFlowInput);
        MdbPrint(pu64ReqHdl,"%s   \t", &(strPad[eDmxFlowInput][0]));
#endif
...
DMX_FILTER_STATUS _MApi_DMX_DBG_EchoCmd(MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
{
    ...
    {
        MdbPrint(pu64ReqHdl,"---------MStar Demux Help---------\n");
        MdbPrint(pu64ReqHdl,"1.  Pid Filter Infomation\n");
        MdbPrint(pu64ReqHdl,"     [e.g.]echo pid_flt > dmx\n");
        MdbPrint(pu64ReqHdl,"\n");
        MdbPrint(pu64ReqHdl,"2.  Section Filter Infomation\n");
        MdbPrint(pu64ReqHdl,"     [e.g.]echo sec_flt=[start_id],[end_id] > dmx\n");
        MdbPrint(pu64ReqHdl,"\n");
        MdbPrint(pu64ReqHdl,"3.  Debug Table Infomation\n");
        MdbPrint(pu64ReqHdl,"     [e.g.]echo dgtbl=[item],[ts_src],[fifo_src] > dmx\n");
        MdbPrint(pu64ReqHdl,"     [e.g.][item]: dc_cnt, dp_cnt, lock_cnt, av_cnt\n");
        MdbPrint(pu64ReqHdl,"     [e.g.][ts_src]: ts0, ts1...\n");
        MdbPrint(pu64ReqHdl,"     [e.g.][fifo_src]: fi0, fi1, mf0, mf1...\n");
        MdbPrint(pu64ReqHdl,"\n");
        return DMX_FILTER_STATUS_OK;
    }
