...
typedef struct _TSP_RESOURCE_PRIVATE
{
    ...
    SYS_TEEINFO             _teemode;  //@jay need to check 20170821
...
static void _TSP_Isr_ProcSelf(OSAL_TSP_ProcTypeParam* pData)
{
    ...
    u32Events = u32PrcEvt;
#elif defined(MSOS_TYPE_LINUX_KERNEL)
    while(_TSP_GetEvent(u32Events, TSP_TASK_EVENT_GROUP_SELF | TSP_TASK_EVENT_FLT_FREE, OSAL_EVENT_OR_CLEAR, _TSP_MUTEX_TIMEOUT) == FALSE)
    {
        //printf("Loop wait event\n");
    }
    *pRet = DRVTSP_FAIL;
#else
...
TSP_Result MDrv_TSP_Suspend(void)
{
    ...
    MDrv_TSP_GetTSIFStatus(E_DRVTSP_IF_PLAYBACK, &(_ptsp_res->_ifset_save), 0);
...
/// @note
/// Restore TSP driver states from DRAM
/// @jay need to checkout 20170821, this function is copied directly from tsp3 temporarily
//-------------------------------------------------------------------------------------------------
...
TSP_Result MDrv_TSP_Ree_SendTeeCmd(TSP_REE_TO_TEE_CMD_TYPE cmd_type, void* param, size_t datasize)
{
    ...
    {
        //if(_TSP_Ree_SendMBXMsg(cmd_type, param) == TRUE)    @jay need to chek  20170821
            return DRVTSP_OK;
        //else      @jay need to chek  20170821
        //    return DRVTSP_FAIL;       @jay need to chek  20170821
    }
...
TSP_Result MDrv_TSP_ALP_PVR_SetRecordStampClk(DrvTsp_ALP_Eng AlpEng, MS_U32 u32Clk)
{
        ...
        return DRVTSP_FAIL;
#else
        //HAL_TSP_ALP_Pvr_SetTimeStampClk((MS_U8)AlpEng, u32Clk);      @jay need to check 20170821
        return DRVTSP_OK;
#endif
}
...
TSP_Result MDrv_TSP_MStr_SyncByte(DrvTSP_If eIf, MS_U8 u8StrId, MS_U8* pu8SyncByte, MS_BOOL bSet)
{
    ...
    return DRVTSP_FAIL;
#endif
    return DRVTSP_OK;    //@jay need to check 20170821
}
...
TSP_Result MDrv_TSP_PidFlt_Free(MS_U32 u32EngId, MS_U32 u32PidFltId)
{
    ...
    else if ((_ptsp_res->_TspState[u32EngId].SecFltEvtNotify[u32SecFltId] & E_DRVTSP_EVENT_CB_MASK) == E_DRVTSP_EVENT_CB_SELF)
    {
        ...
        {
            //printf("[%s][%d]Warning !! Filter %d callback is running %x! Try Again!\n", __FUNCTION__, __LINE__, (unsigned int)u32PidFltId, (unsigned int)stDrvState);
            _TSP_RETURN(DRVTSP_FAIL);
        }
...
TSP_Result MDrv_TSP_Init(MS_PHY phyFWAddr, MS_U32 u32FWSize)
{
    ...
    if(
        #ifdef MSOS_TYPE_LINUX_KERNEL
        (_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN) &&
        #endif
        (_ptsp_res->_bSWInitReady == TRUE)
    )
    {
