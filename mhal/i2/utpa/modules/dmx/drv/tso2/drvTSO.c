...
//Transfer TSIF ID to HW Channel ID
...
TSO_Result MDrv_TSO_Init(void)
{
    ...
    if (TRUE == MDrv_MMIO_GetBASE(&virtBank, &u32BankSize, MS_MODULE_EXTRIU))
    {
        TSO_DBGERR(ULOGE("TSO", "MDrv_TSO_GetExtBASE success\n"));
        HAL_TSO_SetExtRiuBank(virtBank);
    }
#endif
...
TSO_Result MDrv_TSO_Exit(void)
{
    ...
    {
        HAL_TSO_Cfg1_Enable(u8jj, TSO_CFG1_TSO_OUT_EN, FALSE);

        for(u8ii = 0; u8ii < TSO_TSIF_NUM; u8ii++)
        {
            HAL_TSO_ChIf_Enable(u8jj, _TSO_TSIF2ChID(u8ii), FALSE);
            HAL_TSO_Set_InClk(u8jj, _TSO_TSIF2ChID(u8ii), 0, FALSE, FALSE);
        }
...
TSO_Result MDrv_TSO_SetOutClk(MS_U8 u8Eng, DrvTSO_OutClk eOutClk, DrvTSO_OutClkSrc eOutClkSrc, MS_U16 u16DivNum, MS_BOOL bClkInv)
{
    ...
    switch(eOutClkSrc)
    {
        ...
        case E_DRVTSO_OUTCLKSRC_P_CILINK1:
            u16OutClkSrc = HAL_PRE_TSO_OUT_SEL_CILINK1;
            break;
        case E_DRVTSO_OUTCLKSRC_P_CILINK2:
            u16OutClkSrc = HAL_PRE_TSO_OUT_SEL_CILINK2;
            break;
    #endif
...
#ifdef TSIO_ENABLE
    HAL_TSO_Flt_SetSid(u8Eng, u16PidFltId, 0xFF);
    HAL_TSO_Flt_SetBypassTSIO(u8Eng, u16PidFltId, FALSE);
#endif
...
//-------------------------------------------------------------------------------------------------
/// Set bypass TSIO to PID filter
/// @param  u8Eng                               \b IN: TSO engine ID
/// @param  u16PidFltId                         \b IN: PID filter ID
/// @param  bBypassTSIO                         \b IN: Bypass TSIO
/// @return TSO_Result
/// @note
//-------------------------------------------------------------------------------------------------
TSO_Result MDrv_TSO_PidFlt_SetBypassTSIO(MS_U8 u8Eng, MS_U16 u16PidFltId, MS_BOOL bBypassTSIO)
{
    TSO_DBGERR(ULOGE("TSO", "[%d]%s\n", __LINE__, __FUNCTION__));
    ...
    if(bBypassTSIO)
    {
        if(HAL_TSO_Flt_SetBypassTSIO(u8Eng, u16PidFltId, bBypassTSIO) != TRUE)
        {
            ULOGE("TSO", "Not support bypass TSIO for TSO pid filter!!\n");
        }
    }
    _TSO_RETURN(DRVTSO_OK);
}
...
TSO_Result MDrv_TSO_SelPad(MS_U8 u8Eng, DrvTSO_If eIf, DrvTSO_If_Set* pIfSet)
{
    ...
    switch(pIfSet->ePad)
    {
        ...
        case E_DRVTSO_PAD_EXT_INPUT3:
            #if (defined(HAL_TSOIN_MUX_TS3) && defined(TSO_CLKIN_TS3))
        ...
        case E_DRVTSO_PAD_EXT_INPUT4:
            #if (defined(HAL_TSOIN_MUX_TS4) && defined(TSO_CLKIN_TS4))
        ...
        case E_DRVTSO_PAD_EXT_INPUT5:
            #if (defined(HAL_TSOIN_MUX_TS5) && defined(TSO_CLKIN_TS5))
    ...
    #endif
    #ifdef CILINK_SUPPORT
        case E_DRVTSO_PAD_CILINK1:
            u16Pad = HAL_TSOIN_MUX_CILINK1;
            u16clk = TSO_CLKIN_CILINK1;
            break;
        case E_DRVTSO_PAD_CILINK2:
            u16Pad = HAL_TSOIN_MUX_CILINK2;
            u16clk = TSO_CLKIN_CILINK2;
            break;
    #endif
        default:
            return DRVTSO_INVALID_PARAM;
