...
#define TSP_NEED_SEC_FILTER(FltType)                (E_TSP_FLT_USER_SEC == (FltType & E_TSP_FLT_USER_MASK)) || \
                                                    ...
                                                    (E_TSP_FLT_USER_SECTION_VER == (FltType & E_TSP_FLT_USER_MASK)) || \
                                                    (E_TSP_FLT_USER_SEC_NO_PUSI == (FltType & E_TSP_FLT_USER_MASK)) || \
                                                    (E_TSP_FLT_USER_DSS_130 == (FltType & E_TSP_FLT_USER_MASK)) || \
                                                    (E_TSP_FLT_USER_DSS_130_AUX_PKT == (FltType & E_TSP_FLT_USER_MASK))
...
typedef struct DLL_PACKED _TSP_RESOURCE_PRIVATE
{
    ...
    MS_U32                  u32RecFltPID;
    DrvTSP_If_Set           TsIf_Src[TSP_TSIF_NUM];
    ...
    //for A/V parser clk gating
    MS_U32                  _u32FifoRefCnt[E_TSP_DST_INVALID]; // Afifo + Vfifo max num
...
static TSP_RESOURCE_PRIVATE         _tsp_res = {
    ...
    ._stTspPathState                = {[0 ... (TSP_TSIF_NUM-1)] = {FALSE, E_TSP_PKTMODE_188}},
    ...
    .TsIf_Src                       = {[0 ... (TSP_TSIF_NUM-1)] = {E_DRVTSP_INVALID,FALSE,TRUE,TRUE}},
    ...
    ._u32FifoRefCnt                 = {[0 ... (E_TSP_DST_INVALID-1)] = 0}
};
...
#if defined(MSOS_TYPE_LINUX)
static MS_BOOL bTAInited = FALSE;
...
static TSP_Result _MDrv_PVR_Eng_SetPacketMode(MS_U32 u32Eng, TSP_PktMode eMode);
...
static MS_BOOL _TSP_InitResource(TSP_RESOURCE_PRIVATE* presource)
{
    ...
    for(u32ii = 0; u32ii < TSP_TSIF_NUM; u32ii++)
    {
        ...
        presource->_stTspPathState[u32ii].ePktMode = E_TSP_PKTMODE_188;
    }
    ...
    for(u32ii = 0; u32ii < TSP_TSIF_NUM; u32ii++)
    {
        presource->TsIf_Src[u32ii].ePad = E_DRVTSP_INVALID;
    ...
    for(u32ii = 0; u32ii < E_TSP_DST_INVALID; u32ii++)
    {
        presource->_u32FifoRefCnt[u32ii] = 0;
    }
...
static void _TSP_AllPcrFlt_ChkDssMode(MS_U32 u32TsIf, TSP_PktMode ePktMode)
{
    MS_U32      i = 0;
    ...
    {
        HAL_TSP_PcrFlt_GetSrc(i, &ePcrSrc);

        if(  (_ptsp_res->_tspInfo.FltState[TSP_PCRFLT_GET_ID(i)] & E_TSP_FLT_STATE_ALLOC)
          && (u32TsIf == ePcrSrc) )
        {
            HAL_TSP_PcrFlt_DssEnable(i, (ePktMode == E_TSP_PKTMODE_130 || ePktMode == E_TSP_PKTMODE_134));
        }
    }
}
...
static void _TSP_AV_ClkGating(MS_U32 u32FltId, MS_BOOL bEnable)
{
    MS_U32  u32Type = E_TSP_FLT_FIFO_MASK & _ptsp_res->_tspInfo.FltType[u32FltId];
    MS_U32  u32Idx  = 0;

    switch(u32Type)
    {
        case E_TSP_FLT_FIFO_VIDEO ... E_TSP_FLT_FIFO_VIDEO8:
            u32Idx = E_TSP_DST_FIFO_VIDEO + (u32Type - E_TSP_FLT_FIFO_VIDEO);
            break;
        case E_TSP_FLT_FIFO_AUDIO ... E_TSP_FLT_FIFO_AUDIO6:
            u32Idx = E_TSP_DST_FIFO_AUDIO + (u32Type - E_TSP_FLT_FIFO_AUDIO);
            break;
        default:
            return;
    }
    ...
    if(!bEnable)
    {
        _ptsp_res->_u32FifoRefCnt[u32Idx]++;

        if(_ptsp_res->_u32FifoRefCnt[u32Idx] == 1)
        {
            HAL_TSP_FIFO_ClkGating((TSP_DST_SEQ)u32Idx, FALSE);
        }
    }
    else
    {
        if(_ptsp_res->_u32FifoRefCnt[u32Idx] > 0)
           _ptsp_res->_u32FifoRefCnt[u32Idx]--;

        if(_ptsp_res->_u32FifoRefCnt[u32Idx] == 0)
        {
            HAL_TSP_FIFO_ClkGating((TSP_DST_SEQ)u32Idx, TRUE);
        }
    }
}

static TSP_TS_PAD _TSP_DrvHal_PadMapping(DrvTSP_PadIn eTSPad)
{
    switch (eTSPad)
    {
        ...
        case E_DRVTSP_PAD_CILINK1:
            return E_TSP_TS_PAD_CILINK1;
        case E_DRVTSP_PAD_CILINK2:
            return E_TSP_TS_PAD_CILINK2;
#endif
#ifdef TSIO_ENABLE
        case E_DRVTSP_PAD_TSIO0:
...
static TSP_HAL_TSIF _TSP_DrvHal_TSIFMapping(DrvTSP_If eTSIF)
{
    switch(eTSIF)
    {
        case E_TSP_TSIF_0:
...
TSP_Result MDrv_TSP_FIFO_SourceSelect(TSP_TSId eTspTSid, TSP_FileMode eFifoType)
{
    ...
    switch (eFifoType)
    {
        case E_TSP_FILE_2_AUDIO ... E_TSP_FILE_2_AUDIO6:
            eTspDstSeq = E_TSP_DST_FIFO_AUDIO+ (eFifoType - E_TSP_FILE_2_AUDIO);
            break;
        case E_TSP_FILE_2_VIDEO ... E_TSP_FILE_2_VIDEO8:
            eTspDstSeq = E_TSP_DST_FIFO_VIDEO+ (eFifoType - E_TSP_FILE_2_VIDEO);
            break;
        default:
   ...
   switch (eTspTSid)
   {
        case E_TSP_TSID_MMFI0 ... E_TSP_TSID_MMFI1:
            eTspSrcSeq = E_TSP_SRC_MMFI0 + (eTspTSid - E_TSP_TSID_MMFI0);
            break;
        default:
    ...
    HAL_TSP_FIFO_Bypass(eTspDstSeq, FALSE); // for File Eng FIFO bypass clear(PS mode clear)
                                            // File Eng PS mode has higher priority than MM File Eng
...
static TSP_Result _TSP_Path_Free(DRV_TSP_PATH_TYPE eResType, DrvTSP_If ePath)
{
    if( (eResType == E_DRV_TSP_PATH_LIVE) || (eResType == E_DRV_TSP_PATH_FILE) )
    {
        MS_U32      u32PhyTspPath = 0;
...
static void _TSP_FLT_Init(MS_U32 u32FltId)
{
...
#endif
    {
        ...
        HAL_TSP_PidFlt_SetSrcID(u32FltId, 0);
...
static void _TSP_FLT_Disable(MS_U32 u32FltId)
{
    ...
    #endif
    {
        ...
        HAL_TSP_HCMD_FltDisable(u32FltId);
    }

    _TSP_AV_ClkGating(u32FltId, TRUE);
...
static void _TSP_ISR_Task(void)
{
    ...
    TSP_SetEvent(_s32EventId, TSP_EVENT_PROC_EXIT);
}
...
*/
    ...
    ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_FW_LoadCode, (void*)(&(_ptsp_res->_stSecBuf)), sizeof(DrvTSP_SecureBuf));
    if( ret == E_TSP_OK)
    {
        TSP_DBG(E_TSP_DBG_INFO, "[MDRV TSP][%06d] Load FW SUCCESS!\n", __LINE__);
    }
#ifndef SECURE_PVR_ENABLE
    else if(ret == E_TSP_FAIL_TEE_NO_TA || ret == E_TSP_FAIL_TEE_NOT_SUPPORTED || ret == E_TSP_FAIL_TEE_OS_MODE_NOT_SUPPORTED)
    {
        TSP_DBG(E_TSP_DBG_INFO, "[MDRV TSP][%06d] Load FW tee not support \n",__LINE__);
    ...
    {
        TSP_DBG(E_TSP_DBG_MUST, "[MDRV TSP][%06d] Load FW fail!\n", __LINE__);
        return E_TSP_FAIL;
    ...
    if(ret == E_TSP_OK)
    {
        TSP_DBG(E_TSP_DBG_INFO, "[MDRV TSP][%06d] Release FW secure range SUCCESS!\n", __LINE__);
    }
#ifndef SECURE_PVR_ENABLE
    else if(ret == E_TSP_FAIL_TEE_NO_TA || ret == E_TSP_FAIL_TEE_NOT_SUPPORTED || ret == E_TSP_FAIL_TEE_OS_MODE_NOT_SUPPORTED)
    {
        TSP_DBG(E_TSP_DBG_INFO, "[MDRV TSP][%06d] \n", __LINE__);
    }
#endif
    else
    {
    ...
    //printf("[%s][%d] u32BufId %d, opte %d , bufsize %x, addr %x \n", __FUNCTION__, __LINE__, (int)_ptsp_res->_stSecBuf.u32BufId,
    //    (int)_ptsp_res->_stSecBuf.u32BufOpt, (int)_ptsp_res->_stSecBuf.u32BufSize, (int)_ptsp_res->_stSecBuf.phyBufAddr);
    ret = MDrv_TSP_Ree_SendTeeCmd(E_DRVTSP_REE_TO_TEE_CMD_SET_VqBuf, (void*)(&(_ptsp_res->_stSecBuf)), sizeof(DrvTSP_SecureBuf));
    if( ret == E_TSP_OK)
    {
    ...
    {
        TSP_DBG(E_TSP_DBG_INFO," Set VQ tee not support \n");
...
static void _TSP_Exit(void)
{
    ...
    if( ret == E_TSP_OK)
    {
        TSP_DBG(E_TSP_DBG_INFO,"[MDRV TSP][%06d] SECURE_PVR_TSP_VQ_DISABLE SUCCESS\n",__LINE__);
    ...
    {
        TSP_DBG(E_TSP_DBG_INFO,"[MDRV TSP][%06d] ",__LINE__);
    }
#endif
    else
    {
        TSP_DBG(E_TSP_DBG_MUST, "[MDRV TSP][%06d] SECURE_PVR_TSP_VQ_DISABLE FAIL\n",__LINE__);
    }
...
static void _TSP_SW_Exit(void)
{
    ...
    if(_ptsp_res->_s32TaskId != -1)
    {
        ...
        MS_U32  u32DummyEvent = 0;

        MsOS_WaitEvent(_s32EventId, TSP_EVENT_PROC_EXIT, &u32DummyEvent, E_AND_CLEAR, MSOS_WAIT_FOREVER);
...
TSP_Result MDrv_TSP_InitLibResource(void *pu32ResMemAddr)
{
    ...
    else
    {
        if(_bBankInit == FALSE)
        {
            ...
            if (TRUE == MDrv_MMIO_GetBASE(&u32Base, &u32Size, MS_MODULE_EXTRIU))
            {
                TSP_OS_Print("Get MS_MODULE_EXTRIU success\n");
                HAL_TSP_SetExtRiuBank(u32Base);
            }
...
TSP_Result MDrv_TSP_Init(TSP_InitParam *pParam)
{
    ...
    if (TRUE == MDrv_MMIO_GetBASE(&u32Base, &u32Size, MS_MODULE_EXTRIU))
    {
        TSP_OS_Print("MMIO(MS_MODULE_EXTRIU) 0x%08x\n", u32Base);
        HAL_TSP_SetExtRiuBank(u32Base);
    }
...
TSP_Result MDrv_TSP_SetFlowSource(DRV_TSP_FLOW DmxFlow, TSP_TSId eTid)
{
    ...
    // @NOTE FIFO SRC setting was moved to MDrv_TSP_FLT_Enable()
    if ((DmxFlow == E_DRV_TSP_FLOW_PVR0) || (DmxFlow == E_DRV_TSP_FLOW_PVR1) || (DmxFlow == E_DRV_TSP_FLOW_PVR2) || (DmxFlow == E_DRV_TSP_FLOW_PVR3))
    {
...
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_SelPad(DrvTSP_If eTSIF, DrvTSP_PadIn eTSPad)
{
    TSP_Result          ret;
    ...
    if((eTSPad >= E_DRVTSP_PAD_EXT_INPUT0_3WIRE) && (eTSPad <= E_DRVTSP_PAD_EXT_INPUT7_3WIRE))
    {
...
//-------------------------------------------------------------------------------------------------
/// Get default output pad
/// @return default output pad defined in DrvTSP_PadOut
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_GetDefaultOutputPad(DrvTSP_PadOut *eOutputPad)
{
#ifdef CILINK_SUPPORT
    TSP_ENTRY();
    *eOutputPad = E_DRVTSP_OUTPAD_CILINK1;
#endif
    TSP_RETURN(E_TSP_OK);
}
...
TSP_Result MDrv_TSP_OutputPadCfg(DrvTSP_PadOut eOutputPad, DrvTSP_PadIn eInputPad, MS_BOOL bInParallel, MS_BOOL bEnable)
{
    ...
    TSP_TS_PAD eInPad = _TSP_DrvHal_PadMapping(eInputPad);
    TSP_TS_PAD eOutPad = E_TSP_TS_PAD_INVALID;
    TSP_TS_PAD_MUX_MODE eOutPadMode = E_TSP_TS_PAD_MUX_INVALID;
    ...
    if(bEnable == FALSE)
    {
        eInPad = E_TSP_TS_PAD_INVALID;
    }
    else
    {
        switch(eOutputPad & E_DRVTSP_OUTPAD_MASK)
        {

#ifdef CILINK_SUPPORT
            case E_DRVTSP_OUTPAD_CILINK1:
                eOutPad = E_TSP_TS_PAD_CILINK1;
                break;
            case E_DRVTSP_OUTPAD_CILINK2:
                eOutPad = E_TSP_TS_PAD_CILINK2;
                break;
#endif
            default:
                eOutPad = E_TSP_TS_PAD_INVALID;
                break;
        }

        switch(eOutputPad & E_DRVTSP_OUTPAD_MODE_MASK)
        {

#ifdef CILINK_SUPPORT
            case E_DRVTSP_OUTPAD_FROM_DMD:
                eOutPadMode = E_TSP_TS_PAD_MUX_DEMOD;
                break;
            case E_DRVTSP_OUTPAD_FROM_S2P:
                eOutPadMode = E_TSP_TS_PAD_MUX_S2P;
                break;
            case E_DRVTSP_OUTPAD_FROM_S2P1:
                eOutPadMode = E_TSP_TS_PAD_MUX_S2P1;
                break;
            case E_DRVTSP_OUTPAD_FROM_TSO:
                eOutPadMode = E_TSP_TS_PAD_MUX_TSO;
                break;
#endif
            default:
                eOutPadMode = E_TSP_TS_PAD_MUX_INVALID;
                break;
        }
    ...
    if(HAL_TSP_SetOutPadCfg(eOutPad, eOutPadMode, eInPad, bInParallel) == FALSE)
    {
        TSP_RETURN(E_TSP_FAIL);
    }
    else
...
///// @sa
////-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_SelPad_ClkInv(DrvTSP_If eTSIF ,MS_BOOL bClkInv)
{
    TSP_DBG(E_TSP_DBG_TRACK, "%s\n", __FUNCTION__);
    TSP_ENTRY();
...
///// @sa
////-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_SelPad_ClkDis(DrvTSP_If eTSIF ,MS_BOOL bClkDis)
{
    TSP_DBG(E_TSP_DBG_TRACK, "%s\n", __FUNCTION__);
    TSP_ENTRY();
...
TSP_Result MDrv_TSP_TSO_ConfigPad(MS_U32 u32TSOEng, DrvTSP_PadIn eTSPad)
{
...
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_SetTSIF(DrvTSP_If eTSIF, TSP_TSIFCfg u32Cfg, MS_BOOL bFileIn)
{
    TSP_Result          ret;
...
/// @note
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_Path_Free(DRV_TSP_PATH_TYPE eResType, DrvTSP_If ePath)
{
    TSP_Result  ret = E_TSP_FAIL;
    ...
    *pu32FltId = 0xFFFFFFFF;
    ...
    if ( E_TSP_FLT_USER_PCR == (eFltType & E_TSP_FLT_USER_MASK) )
    {
        ...
        end_id   = _tspFltIdMap.u32PcrFltId_End;
             ...
             (E_TSP_FLT_USER_SECTION_VER == (eFltType & E_TSP_FLT_USER_MASK)) ||
             (E_TSP_FLT_USER_SEC_NO_PUSI == (eFltType & E_TSP_FLT_USER_MASK)) ||
             (E_TSP_FLT_USER_DSS_130 == (eFltType & E_TSP_FLT_USER_MASK)) ||
             (E_TSP_FLT_USER_DSS_130_AUX_PKT == (eFltType & E_TSP_FLT_USER_MASK)) )
    {
...
#ifdef MERGE_STR_SUPPORT
    {
        ...
        if(E_TSP_FLT_USER_PCR == (eFltType & E_TSP_FLT_USER_MASK))
        {
            if ((i >= TSP_PCRFLT_GET_ID(0)) && (i <= TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
            {
    ...
    if (type)
    {
        ...
        case E_TSP_FLT_USER_SECTION_VER:
        case E_TSP_FLT_USER_SEC_NO_PUSI:
        case E_TSP_FLT_USER_DSS_130:
        case E_TSP_FLT_USER_DSS_130_AUX_PKT:
            if( E_TSP_HAL_NOT_SUPPORTED==HAL_TSP_SecFlt_SetType(pSecFlt, type >> E_TSP_FLT_USER_SHFT))
            {
               return TSP_RETURN(E_TSP_FAIL_NOT_SUPPORTED);
            }
            break;
...
TSP_Result MDrv_TSP_FLT_SetPCRSrc(MS_U32 u32FltId,MS_U32 u32PCRSrc)
{
    ...
    if ((u32FltId >= TSP_PCRFLT_GET_ID(0)) && (u32FltId <= TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
    {
        ...
        if(ePcrSrc < TSP_TSIF_NUM)
        {
            TSP_PktMode ePktMode = _ptsp_res->_stTspPathState[ePcrSrc].ePktMode;

            HAL_TSP_PcrFlt_DssEnable((u32FltId - _tspFltIdMap.u32PcrFltId_Start), (ePktMode == E_TSP_PKTMODE_130 || ePktMode == E_TSP_PKTMODE_134));
        }
...
TSP_Result MDrv_TSP_FLT_SetPID(MS_U32 u32FltId, MS_U32 u32PID)
{
   TSP_DBG(E_TSP_DBG_TRACK, "%s\n", __FUNCTION__);
    ...
    {
        if ( (_ptsp_res->_tspInfo.FltType[u32FltId]& E_TSP_FLT_USER_MASK) == E_TSP_FLT_USER_DSS_130_AUX_PKT )
            HAL_TSP_PidFlt_SetPid(u32FltId, (u32PID|0x1000));
        else
...
///       not match mask -- must set 1 to compare
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_SecFlt_GetPattern(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_U8 *pu8Match, MS_U8 *pu8Mask, MS_U8 *pu8NMask)
{
    if(pu8Match == NULL || pu8Mask == NULL || pu8NMask == NULL)
    {
        return (E_TSP_FAIL_INVALID);
    }

    TSP_ENTRY();
    TSP_CHECK(TSP_SECFLT_NUM> u32SecFltId, "[%06d] Bad SecFltId %u\n", __LINE__, (unsigned int)u32SecFltId);

    HAL_TSP_SecFlt_GetMask(&(_REGSec->Flt[u32SecFltId]), pu8Mask);
    HAL_TSP_SecFlt_GetMatch(&(_REGSec->Flt[u32SecFltId]), pu8Match);
    HAL_TSP_SecFlt_GetNMask(&(_REGSec->Flt[u32SecFltId]), pu8NMask);
...
TSP_Result _MDrv_TSP_FLT_Enable(MS_U32 u32FltId, MS_BOOL bEnable)
{
    ...
    if (type)
    {
        switch (type)
        {
            case E_TSP_FLT_FIFO_VIDEO ... E_TSP_FLT_FIFO_VIDEO8:
                ...
                HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_VIDEO + (type - E_TSP_FLT_FIFO_VIDEO)),FALSE);   // for FIFO bypass clear
            ...
            case E_TSP_FLT_FIFO_AUDIO ... E_TSP_FLT_FIFO_AUDIO6:
                ...
                HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_AUDIO + (type - E_TSP_FLT_FIFO_AUDIO)),FALSE);   // for FIFO bypass clear
    ...
    else
    {
        ...
        if (type)
        {
            ...
            if (E_TSP_FLT_FIFO_PVR == type) // @FIXME WHY??????????
            {
                ...
                if(type == E_TSP_FLT_USER_SECTION_VER)
                {
                    HAL_TSP_HCMD_VerReset(u32FltId);
                }
    ...
    _TSP_AV_ClkGating(u32FltId, FALSE);
...
TSP_Result MDrv_TSP_FILE_SetPktMode(TSP_PktMode mode)
{
    ...
    _ptsp_res->_stTspPathState[eFileEng].ePktMode = mode;

    _TSP_AllPcrFlt_ChkDssMode(eFileEng, mode);
...
TSP_Result MDrv_TSP_FILE_CmdStart(TSP_FileMode mode)
{
    ...
    switch (mode)
    {
        case E_TSP_FILE_2_TSP:
            ...
            break;
        case E_TSP_FILE_2_VIDEO ... E_TSP_FILE_2_VIDEO8:
            HAL_TSP_FIFO_SetSrc((TSP_DST_SEQ)(E_TSP_DST_FIFO_VIDEO + (mode - E_TSP_FILE_2_VIDEO)), E_TSP_SRC_PKTDMX0); // clear FIFO src
            HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_VIDEO + (mode - E_TSP_FILE_2_VIDEO)),TRUE);
            HAL_TSP_FIFO_Bypass_Src(eFileEng, (TSP_DST_SEQ)(E_TSP_DST_FIFO_VIDEO + (mode - E_TSP_FILE_2_VIDEO)));
            break;
        case E_TSP_FILE_2_AUDIO ... E_TSP_FILE_2_AUDIO6:
            HAL_TSP_FIFO_SetSrc((TSP_DST_SEQ)(E_TSP_DST_FIFO_AUDIO + (mode - E_TSP_FILE_2_AUDIO)), E_TSP_SRC_PKTDMX0); // clear FIFO src
            HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_AUDIO + (mode - E_TSP_FILE_2_AUDIO)),TRUE);
            HAL_TSP_FIFO_Bypass_Src(eFileEng, (TSP_DST_SEQ)(E_TSP_DST_FIFO_AUDIO + (mode - E_TSP_FILE_2_AUDIO)));
            break;
        default:
            break;
    }
...
TSP_Result MDrv_TSP_FILE_Stop(void)
{
    ...
    HAL_TSP_Filein_Bypass(eFileEng, FALSE);
    HAL_TSP_TSIF_FileEn(eFileEng, FALSE);
...
/// @return TSP_Result
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_FILE_PS_Path_Enable(TSP_FileMode mode, MS_BOOL bEnable)
{
    TSP_ENTRY();
    ...
    switch (mode)
    {
        case E_TSP_FILE_2_TSP:
            break;
        case E_TSP_FILE_2_VIDEO ... E_TSP_FILE_2_VIDEO8:
            HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_VIDEO + (mode - E_TSP_FILE_2_VIDEO)),bEnable);
            break;
        case E_TSP_FILE_2_AUDIO ... E_TSP_FILE_2_AUDIO6:
            HAL_TSP_FIFO_Bypass((TSP_DST_SEQ)(E_TSP_DST_FIFO_AUDIO + (mode - E_TSP_FILE_2_AUDIO)),bEnable);
            break;
        default:
            break;
    }
...
static TSP_HAL_PKT_MODE _TSP_DrvHal_PktConverter_PktModeMapping(TSP_PktMode ePktMode)
{
    ...
    switch(ePktMode)
    {
        case E_TSP_PKTMODE_188:
        case E_TSP_PKTMODE_130:
            eOutPktMode = E_TSP_HAL_PKT_MODE_NORMAL;
            break;
...
TSP_Result MDrv_TSP_SetPacketMode(DrvTSP_If u32TSIf, TSP_PktMode ePktMode)
{
#ifdef MERGE_STR_SUPPORT
    TSP_ENTRY();
    ...
    {
        _ptsp_res->_stTspPathState[u32TSIF].ePktMode = ePktMode;

        if(ePktMode == E_TSP_PKTMODE_130)
        {
            HAL_TSP_DSS130_Enable(u32TSIF, TRUE);
        }
        else
        {
            HAL_TSP_DSS130_Enable(u32TSIF, FALSE);
        }

        HAL_TSP_PktConverter_PktMode(u32TSIF, _TSP_DrvHal_PktConverter_PktModeMapping(ePktMode));

        _TSP_AllPcrFlt_ChkDssMode(u32TSIF, ePktMode);
    }
...
TSP_Result MDrv_TSP_GetTSIFStatus(DrvTSP_If eTSIF, DrvTSP_If_Set* pIfSet, MS_U16* pu16Clk)
{
    ...
    pIfSet->ePad        = _ptsp_res->TsIf_Src[u32TSIF].ePad;
    pIfSet->bClkInv     = _ptsp_res->TsIf_Src[u32TSIF].bClkInv;
    pIfSet->bExtSync    = _ptsp_res->TsIf_Src[u32TSIF].bExtSync;
    pIfSet->bParallel   = _ptsp_res->TsIf_Src[u32TSIF].bParallel;
    HAL_TSP_GetClockSetting(u32TSIF, &u16Clk);
    *pu16Clk = u16Clk;
...
static TSP_HAL_CAP_TYPE _TSP_DrvHal_CapsMapping(TSP_Caps eCap)
{
    ...
    switch(eCap)
    {
        ...
        case E_TSP_CAP_VQ_NUM:
            ...
            break;
        case E_TSP_CAP_CAP_DSCMB_ENG_NUM:
            eRetCapType = E_TSP_HAL_CAP_TYPE_DSCMB_ENG_NUM;
            break;
        ...
        case E_TSP_CAP_VFIFO_NUM:
            eRetCapType = E_TSP_HAL_CAP_TYPE_VFIFO_NUM;
            break;
...
TSP_Result MDrv_TSP_PVR_Eng_SetPacketMode(MS_U32 u32Eng, TSP_PktMode eMode)
{
    if (E_TSP_PVR_PVRENG_END > u32Eng) // PVR Eng
    {
        _MDrv_PVR_Eng_SetPacketMode(u32Eng, eMode);
    }
    else
    {
        TSP_DBG(E_TSP_DBG_MUST, "[ERROR][%s][%d]Incorrect Engine type!\n",__FUNCTION__,__LINE__);
...
TSP_Result MDrv_TSP_PVR_Eng_ClkGating(MS_U32 u32Eng, MS_BOOL bEnable)
{
    TSP_ENTRY();

    TSP_HAL_Result  eRes = E_TSP_HAL_OK;
    if((eRes = HAL_PVR_ClkGating(u32Eng, bEnable)) != E_TSP_HAL_OK)
    {
        if(eRes == E_TSP_HAL_NOT_SUPPORTED)
        {
...
static TSP_Result _MDrv_PVR_Eng_SetPacketMode(MS_U32 u32Eng, TSP_PktMode eMode)
{
    ...
    {
        case E_TSP_PKTMODE_188:
            HAL_PVR_RecTsMode(u32Eng, TRUE);
            HAL_PVR_SetDSS130Mode(u32Eng, FALSE, FALSE);
            HAL_PVR_SetStrPacketMode(u32Eng, FALSE);
            break;
        case E_TSP_PKTMODE_192:
            HAL_PVR_RecTsMode(u32Eng, TRUE);
            HAL_PVR_SetDSS130Mode(u32Eng, FALSE, FALSE);
            HAL_PVR_SetStrPacketMode(u32Eng, TRUE);
            break;
        case E_TSP_PKTMODE_130:
            HAL_PVR_RecTsMode(u32Eng, FALSE);
            HAL_PVR_SetDSS130Mode(u32Eng, FALSE, TRUE);
            HAL_PVR_SetStrPacketMode(u32Eng, FALSE);
            break;
        case E_TSP_PKTMODE_PES:
            HAL_PVR_RecTsMode(u32Eng, FALSE);
            HAL_PVR_SetDSS130Mode(u32Eng, FALSE, FALSE);
            HAL_PVR_SetStrPacketMode(u32Eng, FALSE);
            break;
        default:
            TSP_OS_Print("[%s][%d] Invalid PVR packet mode\n", __FUNCTION__, __LINE__);
            break;
    }
...
/// @note
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_RemoveDupAVFifoPkt(TSP_FltType eFltType, MS_BOOL bEnable)
{
    TSP_ENTRY();
    TSP_DST_SEQ eDst = _TSP_DrvHal_FltType2DstMapping(eFltType);
    HAL_TSP_RemoveDupAVFifoPkt(eDst, bEnable);
    TSP_RETURN(E_TSP_OK);
}
...
TSP_Result MDrv_TSP_PARL_BitOrderSwap(DrvTSP_If eTSIF, MS_BOOL bInvert)
{
    TSP_HAL_TSIF eHalTSIF = _TSP_DrvHal_TSIFMapping(eTSIF);
    MS_U32 u32TSIF = HAL_TSP_TsifMapping(eHalTSIF, FALSE);
    ...
    TSP_ENTRY();
    HAL_TSP_TSIF_BitSwap(u32TSIF, bInvert); //@F_TODO check if this is the same with `HAL_TSP_SetDataSwap'
    TSP_RETURN(E_TSP_OK);
}
...
TSP_Result MDrv_TSP_FQ_SetMuxSwitch(MS_U32 u32FQEng, DrvFQ_SrcIf eTsSrc)
{
    ...
    if((eRes = HAL_TSP_FQ_SetMuxSwitch(u32FQEng,u32FQSrc)) != E_TSP_HAL_OK)
    {
        if(eRes == E_TSP_HAL_NOT_SUPPORTED)
        {
            TSP_RETURN(E_TSP_FAIL_NOT_SUPPORTED);
...
TSP_Result MDrv_TSP_FQ_GetMuxSwitch(MS_U32 u32FQEng, DrvFQ_SrcIf* peTsSrc)
{
    ...
    if((eRes = HAL_TSP_FQ_GetMuxSwitch(u32FQEng, &u32FQSrc)) != E_TSP_HAL_OK)
    {
        if(eRes == E_TSP_HAL_NOT_SUPPORTED)
        {
            TSP_RETURN(E_TSP_FAIL_NOT_SUPPORTED);
    ...
    switch(u32FQSrc)
    {
        ...
        case 3:
            *peTsSrc = E_DRVFQ_SRC_TSIF3;
            break;
        case 4:
            *peTsSrc = E_DRVFQ_SRC_TSIF4;
            break;
        case 5:
            *peTsSrc = E_DRVFQ_SRC_TSIF5;
            break;
        case 6:
            *peTsSrc = E_DRVFQ_SRC_TSIF6;
            break;
        case 7:
...
TSP_Result MDrv_TSP_DSCMB_Source(DrvTSP_If eDrvTspIf, MS_U32 *pu32EngId, MS_BOOL bSet)
{
    TSP_ENTRY();

    TSP_HAL_Result eRes = E_TSP_HAL_OK;
    TSP_HAL_TSIF eHalTSIF = _TSP_DrvHal_TSIFMapping(eDrvTspIf);
    MS_U32 u32Path = HAL_TSP_TsifMapping(eHalTSIF, FALSE);

    TSP_RES_ALLOC_TSPPATH_CHECK(u32Path);

    if((eRes = HAL_TSP_DSCMB_Source(u32Path, pu32EngId, bSet)) != E_TSP_HAL_OK)
    {
        if(eRes == E_TSP_HAL_NOT_SUPPORTED)
        {
            TSP_RETURN(E_TSP_FAIL_NOT_SUPPORTED);
...
TSP_Result MDrv_TSP_Create_IoSignal(DrvTSP_IoSignal *pstIoSignal)
{
    ...
    for(u32ii = 0; u32ii < TSP_MAX_SIGFLAG_NUM; u32ii++)
    {
        ...
        pstIoSignal->u32SecCrcErrFlag[u32ii] = 0;
    }
...
{
    TSP_ENTRY();
    HAL_TSP_OR_Address_Protect(phyDataAddr+u32Size, phyDataAddr);
    HAL_TSP_OR_Address_Protect_En(TRUE);
    if( ! HAL_TSP_HCMD_SetFWDataAddr(phyDataAddr))
    {
        TSP_RETURN(E_TSP_FAIL_NOT_SUPPORTED);
    }
...
TSP_Result MDrv_TSP_Ree_SendTeeCmd(TSP_REE_TO_TEE_CMD_TYPE cmd_type, void* param, size_t datasize)
{
    ...
    else if(_ptsp_res->_teemode.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
    {
        ...
        else if(ret == TEEC_ERROR_NOT_SUPPORTED)
        {
            return E_TSP_FAIL_TEE_NOT_SUPPORTED;
        }
#endif
    }
    else
    {
        return E_TSP_FAIL_TEE_OS_MODE_NOT_SUPPORTED;
    }
    return E_TSP_FAIL;
}
