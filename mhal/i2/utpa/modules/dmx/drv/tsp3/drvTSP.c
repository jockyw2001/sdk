...
typedef struct _TSP_RESOURCE_PRIVATE
{
    ...
    DrvTSP_Path_State       _TspPathState[TSP_IF_NUM];
...
static TSP_RESOURCE_PRIVATE         _tsp_res = {
    ...
    ._TspStcState                       = {[0 ... (TSP_STC_NUM-1)] = {0xFFFFFFFF, FALSE}},
    ._TspPathState                      = {[0 ... (TSP_IF_NUM-1)] = {0xFFFFFFFF, FALSE}},
...
static void _TSP_SW_Exit(void)
{
    ...
    // printf("[%s][%d] _u32ProcessNum %d, _bSWInitReady %d, _s32TspTaskId %x, %x\n",
    //        __FUNCTION__, __LINE__, (int)_ptsp_res->_u32ProcessNum, (int)_ptsp_res->_bSWInitReady, (int)_s32TspTaskId, (int)getpid());
    ...
    if ((_s32TspEventId != -1) && (_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN))
    {
...
TSP_Result MDrv_TSP_Path_Alloc(DRV_TSP_PATH_TYPE eResType, MS_U32 *pu32PathId)
{
    ...
    _TSP_ENTRY();
    *pu32PathId = 0xFFFFFFFF;
    if(eResType == E_DRV_TSP_PATH_FILE)
    {
        for(u8ii = E_DRVTSP_FILE0; u8ii < TSP_IF_NUM; u8ii++)
        {
            if(_ptsp_res->_TspPathState[u8ii].bUsed == FALSE)
            {
                *pu32PathId = u8ii;
                _ptsp_res->_TspPathState[u8ii].bUsed = TRUE;
                ret = DRVTSP_OK;
                break;
            }
        }
    ...
    else if(eResType == E_DRV_TSP_PATH_LIVE)
    {
        for(u8ii = E_DRVTSP_LIVE0; u8ii < TSP_IF_NUM; u8ii++)
        {
            if(u8ii == E_DRVTSP_LIVE_FI)
            {
                continue;
            }
            else if(_ptsp_res->_TspPathState[u8ii].bUsed == FALSE)
            {
                *pu32PathId = u8ii;
                _ptsp_res->_TspPathState[u8ii].bUsed = TRUE;
                ret = DRVTSP_OK;
            ...
            if((u8ii == (TSP_IF_NUM - 1)) && (_ptsp_res->_TspPathState[E_DRVTSP_LIVE_FI].bUsed == FALSE))
            {
                *pu32PathId = E_DRVTSP_LIVE_FI;
                _ptsp_res->_TspPathState[E_DRVTSP_LIVE_FI].bUsed = TRUE;
                ret = DRVTSP_OK;
            }
        }
    }
...
/// @note
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_Path_Free(DRV_TSP_PATH_TYPE eResType, MS_U32 u32Path)
{
    _TSP_ENTRY();

    TSP_ASSERT2(TSP_IF_NUM > u32Path, TSP_DBG("[TSP_ERROR][%06d] Bad ePath %d\n", __LINE__, (unsigned int)u32Path));

    _ptsp_res->_TspPathState[u32Path].bUsed = FALSE;
...
CLEAR_INT:
    ...
    if(_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN)
    {
        MsOS_CompleteInterrupt((InterruptNum)_u32IntNum); //for user mode, multiple process support
    }
#endif
...
static TSP_Result _TSP_Init(MS_BOOL bResetSwState, MS_BOOL bRestoreRegState)
{
    ...
    // printf("[%s][%d] _bSWInitReady %d, _s32TspTaskId %x, %x\n",
    //        __FUNCTION__, __LINE__, (int)_ptsp_res->_bSWInitReady, (int)_s32TspTaskId, (int)getpid());
...
//#endif
    ...
    if (!bAlive)
    {
        ...
        if( ret == DRVTSP_OK)
        {
            TSP_DBG("[MDRV TSP][%06d] Load FW SUCCESS!\n", __LINE__);
        }
#ifndef SECURE_PVR_ENABLE
        else if(ret == DRVTSP_TEE_NO_TA || DRVTSP_TEE_NOT_SUPPORTED || DRVTSP_REE_FLOW)
        {
            TSP_DBG("[MDRV TSP][%06d] Load FW tee not support \n",__LINE__);
        }
#endif
        else
        {
...
//-------------------------------------------------------------------------------------------------
/// TSP CLK Ctrl for power saving
/// @return TSP_Result
/// @note
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_FunctionClk(DRV_TSP_FUNC_TYPE u32Func, MS_U32 u32Eng, MS_BOOL bClkOn)
{
#if(defined(TSP_CLK_GATING_SUPPORT) && (TSP_CLK_GATING_SUPPORT == 1))
    _TSP_ENTRY();
    switch (u32Func)
    {
        case E_DRV_TSP_PATH:
            HAL_TSP_PATH_ClkGating(u32Eng,!bClkOn);
            break;
        case E_DRV_TSP_PVR:
            HAL_TSP_PVR_ClkGating(u32Eng,!bClkOn);
            break;
        default:
            break;
    }
...
TSP_Result MDrv_TSP_GetDefaultOutputPad(DrvTSP_PadOut *eOutputPad)
{
    ...
    MS_U32 u32OutputPad = TSP_DEFAULT_OUTPUT_PAD;
    ...
    {
        case TSP_MUX_TS1:
            *eOutputPad = E_DRVTSP_OUTPAD_PAD1;
            break;
        case TSP_MUX_CILINK1:
            *eOutputPad = E_DRVTSP_OUTPAD_CILINK1;
            break;
        default:
            *eOutputPad = E_DRVTSP_OUTPAD_NONE;
            break;
    }
#endif
    _TSP_RETURN(DRVTSP_OK);
...
TSP_Result MDrv_TSP_OutputPadCfg(DrvTSP_PadOut eOutputPad, DrvTSP_PadIn eInputPad, MS_BOOL bInParallel, MS_BOOL bEnable)
{
    ...
    if(!((u32OutPad == E_DRVTSP_OUTPAD_PAD1) || (u32OutPad == E_DRVTSP_OUTPAD_PAD3)
        || (u32OutPad == E_DRVTSP_OUTPAD_PAD0) || (u32OutPad == E_DRVTSP_OUTPAD_CILINK1) || (u32OutPad == E_DRVTSP_OUTPAD_CILINK2)))
    {
        return DRVTSP_FAIL;
    }
    ...
    else
    {
        ...
        {
            case E_DRVTSP_OUTPAD_PAD0:
                u32OutPad = TSP_MUX_TS0;
                break;
            case E_DRVTSP_OUTPAD_PAD1:
                u32OutPad = TSP_MUX_TS1;
                break;
#if defined(TSP_MUX_TS3)
            case E_DRVTSP_OUTPAD_PAD3:
                u32OutPad = TSP_MUX_TS3;
                break;
#endif
#ifdef CILINK_SUPPORT
            case E_DRVTSP_OUTPAD_CILINK1:
                u32OutPad = TSP_MUX_CILINK1;
                break;
            case E_DRVTSP_OUTPAD_CILINK2:
                u32OutPad = TSP_MUX_CILINK2;
                break;
#endif
            default:
                u32OutPad = TSP_MUX_NONE;
                break;
        }
...
TSP_Result MDrv_TSP_File_Stop(void)
{
    ...
    for(u8ii = 0; u8ii < ((MS_U8)TSP_CA_ENGINE_NUM); u8ii++)
    {
        ...
        if(_ptsp_res->_DscmbFlt_State[u8ii].u32Src & E_DRVTSP_FLT_SOURCE_TYPE_LIVE)
        {
...
/// @return TSP_Result
//-------------------------------------------------------------------------------------------------
TSP_Result MDrv_TSP_File_PS_Path_Enable(DrvTSP_FileinMode eFileinMode, MS_BOOL bEnable)
{
    _TSP_ENTRY();
    HAL_TSP_PS_Path_Enable((MS_U32)eFileinMode);
    _TSP_RETURN(DRVTSP_OK);
...
TSP_Result MDrv_TSP_Init(MS_PHY phyFWAddr, MS_U32 u32FWSize)
{
    ...
    if((_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN) && (_ptsp_res->_bSWInitReady == TRUE))
    {
       if(_s32TspTaskId == -1)
       {
...
TSP_Result MDrv_TSP_SetVQueBuf(MS_PHY phyVQBaseAddr, MS_U32 u32VQBufLen)
{
    ...
    if(ret == DRVTSP_OK)
    {
        TSP_DBG("[MDRV TSP][%06d] Set VQ buf SUCCESS int TEE!\n", __LINE__);
    }
#ifndef SECURE_PVR_ENABLE
    else if(ret == DRVTSP_TEE_NO_TA || DRVTSP_TEE_NOT_SUPPORTED || DRVTSP_REE_FLOW)
    {
...
TSP_Result MDrv_TSP_Get_SecEvents(MS_U32 u32Group, DrvTSP_IoSignal *pstIoSignal)
{
    ...
    MS_U32 u32RdyMask = 0, u32OvfMask = 0, u32SecCrc = 0;
    ...
    _TSP_GetModeSecCrc(u32Group, 0xFFFFFFFFUL, &u32SecCrc, OSAL_EVENT_OR_CLEAR);
...
TSP_Result MDrv_TSP_ALP_Enable(DrvTsp_ALP_Eng AlpEng, MS_BOOL bEnable)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    //printf("[%s][%d] AlpEng %d \n", __FUNCTION__,__LINE__,(int)AlpEng);
    HAL_TSP_ALP_Mode_Enable((MS_U8)AlpEng, bEnable);
    return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_SetBuffer(DrvTsp_ALP_Eng AlpEng, MS_PHY phyBufStart0, MS_PHY phyBufStart1, MS_U32 u32BufSize0, MS_U32 u32BufSize1)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    HAL_TSP_ALP_Pvr_SetBuffer((MS_U8)AlpEng, phyBufStart0, phyBufStart1, u32BufSize0, u32BufSize1);
    return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_SetRecordStampClk(DrvTsp_ALP_Eng AlpEng, MS_U32 u32Clk)
{
#if (TSP_ALP_IF_NUM == 0)
        return DRVTSP_FAIL;
#else
        HAL_TSP_ALP_Pvr_SetTimeStampClk((MS_U8)AlpEng, u32Clk);
        return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_SetRecordStamp(DrvTsp_ALP_Eng AlpEng, MS_U32 u32Stamp)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    HAL_TSP_ALP_Pvr_SetTimeStamp((MS_U8)AlpEng, u32Stamp);
    return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_GetRecordStamp(DrvTsp_ALP_Eng AlpEng, MS_U32 *pu32Stamp)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    *pu32Stamp = 0;
    *pu32Stamp = HAL_TSP_ALP_Pvr_GetTimeStamp((MS_U8)AlpEng);
    return DRVTSP_OK;
#endif

}

TSP_Result MDrv_TSP_ALP_PVR_Start(DrvTsp_ALP_Eng AlpEng, MS_BOOL bRecAll, MS_BOOL bStart)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    HAL_TSP_ALP_Pvr_All((MS_U8)AlpEng, bRecAll);
    HAL_TSP_ALP_Pvr_Enable((MS_U8)AlpEng, bStart);

    return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_GetWriteAddr(DrvTsp_ALP_Eng AlpEng, MS_PHY *pphy2WriteAddr)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    *pphy2WriteAddr = 0;
    *pphy2WriteAddr = HAL_TSP_ALP_Pvr_GetBufWrite((MS_U8)AlpEng);
    return DRVTSP_OK;
#endif
}

TSP_Result MDrv_TSP_ALP_PVR_TimeStamp(DrvTsp_ALP_Eng AlpEng, MS_BOOL bEnable)
{
#if (TSP_ALP_IF_NUM == 0)
    return DRVTSP_FAIL;
#else
    HAL_TSP_ALP_Pvr_Timestamp_Mode((MS_U8)AlpEng, bEnable);
    return DRVTSP_OK;
#endif
}
...
TSP_Result MDrv_TSP_Ree_SendTeeCmd(TSP_REE_TO_TEE_CMD_TYPE cmd_type, void* param, size_t datasize)
{
    ...
    else if(_ptsp_res->_teemode.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
    {
        ...
        ret = MDrv_SYS_TEEC_InvokeCmd(&dmx_session, cmd_type, &dmx_op, &ret_orig);
        if ( ret != TEEC_SUCCESS)
        {
        ...
        {
            return DRVTSP_TEE_NOT_SUPPORTED;
        }
#endif
    }
    else
    {
        return DRVTSP_REE_FLOW;
    }
    return DRVTSP_FAIL;
}
