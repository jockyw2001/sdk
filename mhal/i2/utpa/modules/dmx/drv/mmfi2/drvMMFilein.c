...
//-------------------------------------------------------------------------------------------------
/// MMCI CLK Ctrl for power saving
/// @return TSP_Result
/// @note
//-------------------------------------------------------------------------------------------------
MMFI_Result MDrv_MMFI_EngClk(DrvMMFI_Path epath, MS_BOOL bClkOn)
{
#if(defined(MMFI_CLK_GATING_SUPPORT) && (MMFI_CLK_GATING_SUPPORT == 1))
    _MMFI_ENTRY();

    HAL_MMFI_ClkGating(epath, !bClkOn); // clk gating=0 if power on (bPowerOn=1)
...
#ifdef MMFI_HW_BOX_SERIES
        HAL_MMFI_Cfg3_Set(u8EngID, (HAL_MMFI_Cfg3_Get(u8EngID) & ~MMFI_CFG3_FILEIN_MODE_MASK));
#endif
