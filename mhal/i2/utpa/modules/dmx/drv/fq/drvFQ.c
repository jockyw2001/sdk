...
#define FQ_INIT_READY_SET(idx)  (_u32FQInitReady[(idx) >> 5] |= (1 << ((idx) & 0x1F)))
#define FQ_INIT_READY_GET(idx)  (_u32FQInitReady[(idx) >> 5] & (1 << ((idx) & 0x1F)))
#define FQ_INIT_READY_CLR(idx)  (_u32FQInitReady[(idx) >> 5] &= ~(1 << ((idx) & 0x1F)))
...
static MS_U32                           _u32FQInitReady[(FQ_NUM+31)>>5] = { [0 ... (((FQ_NUM+31)>>5) - 1)] = 0 };
//--------------------------------------------------------------------------------------------------
//  Forward declaration
//--------------------------------------------------------------------------------------------------
...
{
    MS_BOOL bAddrMode = (MS_BOOL)u8AddrMode;
#if (defined(FQ_HW_BOX_SERIES) || (defined(FQ_CLK_GATING_SUPPORT) && (FQ_CLK_GATING_SUPPORT == 1)))
    HAL_FQ_ClkGating(u32FQEng, FALSE);
#endif
    HAL_FQ_SWReset(u32FQEng, FALSE);
    HAL_FQ_Bypass(u32FQEng, FALSE);
    HAL_FQ_AddrMode(u32FQEng,TRUE, &bAddrMode);
#ifdef FQ_HW_BOX_SERIES
    HAL_FQ_Read_Enable(u32FQEng, TRUE);
#endif
}

static void _FQ_Exit(MS_U32 u32FQEng)
{
    MS_BOOL bAddrMode = FALSE;
    HAL_FQ_SWReset(u32FQEng, TRUE);
    HAL_FQ_Bypass(u32FQEng, TRUE);
    HAL_FQ_AddrMode(u32FQEng, TRUE,&bAddrMode);
#ifdef FQ_HW_BOX_SERIES
    HAL_FQ_Read_Enable(u32FQEng, FALSE);
    HAL_FQ_ClkGating(u32FQEng, TRUE);
#elif(defined(FQ_CLK_GATING_SUPPORT) && (FQ_CLK_GATING_SUPPORT == 1))
    HAL_FQ_ClkGating(u32FQEng, TRUE);
#endif

}

static MS_BOOL _Is_FQ_FeatureOn(void)
{
    MS_U8 u8Idx = 0;

    for(u8Idx = 0; u8Idx < ((FQ_NUM+31)>>5); u8Idx++)
    {
        if(_u32FQInitReady[u8Idx])
        {
            return TRUE;
        }
    }
...
#ifdef FQ_HW_BOX_SERIES
static inline void _MDrv_FQ_MUX_GetSrc(MS_U32 u32FQEng, MS_U32 *pu32Path)
{
    HAL_FQ_MUX_Src(u32FQEng, pu32Path, FALSE);
    *pu32Path -= FQ_MUX_PATH_START_ID;
}
#endif
...
FQ_Result MDrv_FQ_Init(MS_U32 u32FQEng, MS_U8 u8AddrMode)
{
    ...
    if (TRUE == MDrv_MMIO_GetBASE(&virtBank, &phyBankSize, MS_MODULE_EXTRIU))
    {
        FQ_DBG("MMIO(MS_MODULE_EXTRIU) 0x%"DTC_MS_VIRT_x"\n", virtBank);
        HAL_FQ_SetExtRiuBank(virtBank);
    }
    ...
    _FQ_ENTRY();
    FQ_INIT_READY_SET(u32MapingFQEng);
    _FQ_RETURN(DRVFQ_OK);
}
...
{
    _FQ_ENTRY();
    *pbIsInit = !!FQ_INIT_READY_GET(u32FQEng);
    _FQ_RETURN(DRVFQ_OK);
}
...
FQ_Result MDrv_FQ_Exit(MS_U32 u32FQEng)
{
    ...
    if(FQ_INIT_READY_GET(u32MapingFQEng))
    {
        _FQ_RegStateReset(u32MapingFQEng);
        _FQ_Exit(u32MapingFQEng);

        FQ_INIT_READY_CLR(u32MapingFQEng);
    }

    if (!_Is_FQ_FeatureOn() && _s32FQMutexId != -1)
    {
...
#ifdef FQ_HW_BOX_SERIES
    TSP_FQ_MUX_OUT_SRC  eSrc, eOldSrc;
    MS_U32              u32Path;

    if(u32MapingFQEng >= FQ_MUX_START_ID)
    {
        _MDrv_FQ_MUX_GetSrc(u32MapingFQEng, &u32Path);
        eSrc = (TSP_FQ_MUX_OUT_SRC)(u32MapingFQEng - FQ_MUX_START_ID + 1);
    }
    else
    {
        u32Path = u32MapingFQEng;
        eSrc = E_TSP_FQ_MUX_OUT_PATH;
    }

    HAL_TSP_FQ_MuxOutPathSrc(u32Path, &eOldSrc, FALSE);

    if(eSrc != eOldSrc)
    {
        MS_U32  u32OldPath = (eOldSrc == E_TSP_FQ_MUX_OUT_PATH)? u32Path : ((MS_U32)eOldSrc + FQ_MUX_START_ID - 1);

        HAL_FQ_Read_Enable(u32OldPath, FALSE);
        HAL_TSP_FQ_MuxOutPathSrc(u32Path, &eSrc, TRUE);
        HAL_FQ_Read_Enable(u32MapingFQEng, TRUE);
        HAL_FQ_MUX_RushModeEnable(u32OldPath, FALSE);
    }
...
#ifdef FQ_HW_BOX_SERIES
    HAL_FQ_MUX_RushModeEnable(u32MapingFQEng, TRUE);
#endif
    _FQ_RETURN(DRVFQ_OK);
}
...
FQ_Result MDrv_FQ_AddrMode(MS_U32 u32FQEng,MS_BOOL bSet, MS_BOOL* pbAddrMode)
{
#ifdef FQ_HW_BOX_SERIES
    MS_U32 u32MapingFQEng = HAL_TSP_Tsif2Fq_Mapping(u32FQEng);
#else
    ...
    _FQ_ENTRY();
     ...
     HAL_FQ_AddrMode(u32MapingFQEng,bSet, pbAddrMode);

    _FQ_RETURN(DRVFQ_OK);

}

FQ_Result MDrv_FQ_MUX_SetSrc(MS_U32 u32FQEng, MS_U32 u32Path)
{
#ifdef FQ_HW_BOX_SERIES
    MS_U32 u32MapingFQEng = HAL_TSP_Tsif2Fq_Mapping(u32FQEng);

    if((u32MapingFQEng < FQ_MUX_START_ID) || (u32MapingFQEng >= FQ_NUM))
        return DRVFQ_FAIL;

    _FQ_ENTRY();

    u32Path += FQ_MUX_PATH_START_ID;
    HAL_FQ_MUX_Src(u32MapingFQEng, &u32Path, TRUE);
...
FQ_Result MDrv_FQ_MUX_GetSrc(MS_U32 u32FQEng, MS_U32 *pu32Path)
{
#ifdef FQ_HW_BOX_SERIES
    MS_U32 u32MapingFQEng = HAL_TSP_Tsif2Fq_Mapping(u32FQEng);
    ...
    _MDrv_FQ_MUX_GetSrc(u32MapingFQEng, pu32Path);
