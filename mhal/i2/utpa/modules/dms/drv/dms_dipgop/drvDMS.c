...
static void _DMS_XC_Set_ScalingDown_Condition(ST_DMS_WINDOW *ptCropWin, ST_DMS_WINDOW *ptDstWin)
{
    ...
    if(_DMS_WindowSizeOverFHD(ptCropWin->u32width,ptCropWin->u32height))
    {
        ...
        {
            if(MDrv_DMS_VDECSupportSecondBuffer() == TRUE)
            {
            ...
            {
                printf("[%s]VDEC no support 2nd buffer \n",__FUNCTION__);
            }
        }
        else
        {
...
MS_BOOL MDrv_DMS_VDECSupportSecondBuffer(void)
{
#ifdef DMS_VDEC_SUPPORT_SECOND_BUFFER

    DMS_PRINTF("\033[32m [%s][%d] === VDEC SUPPORT SECOND BUFFER === \033[0m\n",__FUNCTION__,__LINE__);
    return TRUE;

#else

    DMS_PRINTF("\033[32m [%s][%d] === NO SECOND BUFFER === \033[0m\n",__FUNCTION__,__LINE__);
    return FALSE;
