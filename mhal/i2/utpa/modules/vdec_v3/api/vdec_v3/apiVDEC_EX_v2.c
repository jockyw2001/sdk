...
void VDEC_EXRegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    UtopiaModuleSetupFunctionPtr(pModuleVDEC_EX_V2,
            ...
            UtopiaModuleSetupSTRFunctionPtr(pModuleVDEC_EX_V2,(FUtopiaSTR)VDEC_EXStr);
            UtopiaModuleSetSTRPrivate(pModuleVDEC_EX_V2, sizeof(VDEC_EX_INSTANCE_PRIVATE));
#endif
    ...
    UtopiaModuleRegisterMdbNode("vdec", (FUtopiaMdbIoctl)VDECMdbIoctl);
#endif
}
...
MS_U32 VDEC_EXStr(MS_U32 u32PowerState, void* pModule)
{
    ...
    UtopiaModuleGetSTRPrivate(pModule, (void**)&psVDEC_EX_InstPri);

    if(psVDEC_EX_InstPri == NULL || psVDEC_EX_InstPri->fpVDEC_EX_PreSetControl == NULL)
    {
        UtopiaStrSendCondition("vdec", u32PowerState, 0);
        return UTOPIA_STATUS_FAIL;
    }

    if (u32PowerState == E_POWER_SUSPEND)
    {
        EN_POWER_MODE arg = E_POWER_SUSPEND;
        void* param[8];
        param[0] = (void*)(&arg);
        VDEC_StreamId StreamId;
        u32Return = psVDEC_EX_InstPri->fpVDEC_EX_PreSetControl(&StreamId,
                                                               E_VDEC_EX_V2_USER_CMD_SETPOWERSTATE,
                                                               param);
        if(u32Return == E_VDEC_EX_OK)
            return UTOPIA_STATUS_SUCCESS;
    }
    else if (u32PowerState == E_POWER_RESUME)
    {
        EN_POWER_MODE arg = E_POWER_RESUME;
        void* param[8];
        param[0] = (void*)(&arg);
        VDEC_StreamId StreamId;
        u32Return = psVDEC_EX_InstPri->fpVDEC_EX_PreSetControl(&StreamId,
...
#if defined(MSOS_TYPE_LINUX_KERNEL)
    VDEC_EX_INSTANCE_PRIVATE* psVDEC_EX_STR_InstPri = NULL;
    UtopiaModuleGetSTRPrivate(pModuleVDEC_EX_V2, (void**)&psVDEC_EX_STR_InstPri);

    psVDEC_EX_STR_InstPri->fpVDEC_EX_GetFreeStream  = MApi_VDEC_EX_V2_GetFreeStream;
    psVDEC_EX_STR_InstPri->fpVDEC_EX_Init           = MApi_VDEC_EX_V2_Init;
    psVDEC_EX_STR_InstPri->fpVDEC_EX_SetControl     = MApi_VDEC_EX_V2_SetControl;
    psVDEC_EX_STR_InstPri->fpVDEC_EX_GetControl     = MApi_VDEC_EX_V2_GetControl;
    psVDEC_EX_STR_InstPri->fpVDEC_EX_PreSetControl  = MApi_VDEC_EX_V2_PreSetControl;
    psVDEC_EX_STR_InstPri->fpVDEC_EX_PostSetControl = MApi_VDEC_EX_V2_PostSetControl;
#endif
#endif
    return UTOPIA_STATUS_SUCCESS;
}
