...
static void _VSYNC_SetupDispFrameFormat(int VsynDispId, ST_DMS_DISPFRAMEFORMAT* pDispFrm, VDEC_EX_DispFrame* pNextDispFrm)
{
    ...
    if(pDispFrm!= NULL && pNextDispFrm != NULL)
    {
        ...
        pDispFrm->u64Pts = ((MS_U64)pNextDispFrm->stFrmInfo.u32ID_H) << 32 | pNextDispFrm->stFrmInfo.u32TimeStamp;
        ...
        for (u32FrmIdx = 0; u32FrmIdx < u32FrmCnt; u32FrmIdx++)
        {
            ...
            if (pDispFrm->u32CodecType == E_VDEC_EX_CODEC_TYPE_VP9)
            {
                pDispFrm->enColorFormat = E_DMS_COLOR_FORMAT_SW_YUV420_SEMIPLANAR;
            }
            #endif
            else
            if (pDispFrm->u32CodecType == E_VDEC_EX_CODEC_TYPE_HEVC|| pDispFrm->u32CodecType == E_VDEC_EX_CODEC_TYPE_VP9 ||
...
static VSYNC_AVSYNC_STATE _VSYNC_AVSync(int VsynDispId, MS_U64 u64Pts)
{
    ...
    if(u32diff <= pAttr->u32Sync_far_threshold )// else, free run mode
    {
        ...
        {
            res =  E_VSYNC_AVSYNC;
            VSYNC_LOG_VERBOSE(VSYNC_PRINT("\033[1;36m[%s] %d  stc = %x pts = %ud sys_time = %d  E_VSYNC_AVSYNC  u32diff = %d\033[m\n",__FUNCTION__,loopcnt,(unsigned int)(u32stc/MS_90KHZ) , (unsigned int)((u64Pts + pAttr->u32SyncDelay)&0xFFFFFFFF)/MS_90KHZ ,(int)MsOS_GetSystemTime(),(int)u32diff));
        }
        else
        {
            ...
            {
                res = E_VSYNC_DROP;
                VSYNC_LOG_VERBOSE(VSYNC_PRINT("\033[1;35m[%s] %d  stc = %ud pts = %ud sys_time = %d  E_VSYNC_DROP\033[m\n",__FUNCTION__,loopcnt,(unsigned int)(u32stc/MS_90KHZ) ,(unsigned int) ((u64Pts + pAttr->u32SyncDelay)&0xFFFFFFFF)/MS_90KHZ ,(int)MsOS_GetSystemTime()));
            }
        }
    }
    else
...
static VSYNC_Result _VSYNC_Proc(int VsynDispId)
{
    ...
    MS_U8 u8InterlaceModeDropCnt = 0;
    MS_U32 u32stc = 0;
    ...
    while(pAttr->bUsed)
    {
        ...
        if(E_VSYNC_REPEAT != sync_status || res_get_frame != E_VDEC_EX_OK)// need next frame: 1. not repeat, 2. get frame fail
        {
            ...
            if(res_get_frame == E_VDEC_EX_OK)
            {
                ...
                if((pAttr->u8DispFbQWptr + 1)%VSYNC_DISP_FB_Q_SZ == pAttr->u8DispFbQRptr)
                {
                    ...
                    MApi_VDEC_EX_ReleaseFrame(pStreamId, pNextDispFrm);
                    pAttr->u32DropCnt++;
                    u8InterlaceModeDropCnt++;
                    VSYNC_LOG_DROP(VSYNC_PRINT("\033[1;35m[%s] %d pAttr->u32DropCnt = %d\033[m\n",__FUNCTION__,__LINE__,(int)pAttr->u32DropCnt));
                    MsOS_DelayTask(TIME_10_MS);
                    continue;
                }
                if(_VSYNC_IsTrasitionTrickMode(VsynDispId) == TRUE)// mvd only
                {// display one drop one to make X2 speed effect
                    pAttr->u8TraditionalDispCnt++;
                    if(pAttr->u8TraditionalDispCnt%2 == 0)
                    {
                        MApi_VDEC_EX_ReleaseFrame(pStreamId, pNextDispFrm);
                        pAttr->u32DropCnt++;
                        VSYNC_LOG_DROP(VSYNC_PRINT("\033[1;35m[%s] %d pAttr->u32DropCnt = %d\033[m\n",__FUNCTION__,__LINE__,(int)pAttr->u32DropCnt));
                ...
                pAttr->u64NextPts = (MS_U64) ((((MS_U64)pNextDispFrm->stFrmInfo.u32ID_H) << 32 | (MS_U64)pNextDispFrm->stFrmInfo.u32TimeStamp) + (MS_U64)pAttr->u32SyncDelay);

                _VSYNC_HandleSequenceChange(VsynDispId);
                if(pAttr->bFirstFrame == FALSE )
                {
                ...
                {// check data discontinuouis to modify clock
                    ...
                    if(((MS_U32)(pNextDispFrm->stFrmInfo.u32TimeStamp) - (MS_U32)(pPrevDispFrm->stFrmInfo.u32TimeStamp))/MS_90KHZ > (pAttr->u32VsycnDuration * 5))
                    {
                        signed long long new_stc = (signed long long)(pNextDispFrm->stFrmInfo.u32TimeStamp) - (signed long long)(pAttr->u32latch_pts) + (signed long long)(pAttr->u32latch_stc);
                        pAttr->u32latch_stc = (MS_U32)(new_stc & 0xFFFFFFFF);
                        pAttr->u32latch_pts = pNextDispFrm->stFrmInfo.u32TimeStamp;
        ...
        switch(sync_status)
        {
            ...
            case E_VSYNC_REPEAT:
                if(res_get_frame == E_VDEC_EX_OK)
                    sync_status =  _VSYNC_AVSync( VsynDispId, (MS_U64)(((MS_U64)pNextDispFrm->stFrmInfo.u32ID_H) << 32 | pNextDispFrm->stFrmInfo.u32TimeStamp));
                else
            ...
            case E_VSYNC_DROP:
                ...
                if(res_get_frame == E_VDEC_EX_OK)
                {
                    ...
                    while(  (pAttr->u8DispFbQRptr + 1)%VSYNC_DISP_FB_Q_SZ != pAttr->u8DispFbQWptr)
                    {//keep the last one, drop the others
                        MS_U32 u32DroppedPts = pAttr->DispFrameQ[pAttr->u8DispFbQRptr].stFrmInfo.u32TimeStamp;
                        MApi_VDEC_EX_ReleaseFrame(pStreamId, &(pAttr->DispFrameQ[pAttr->u8DispFbQRptr]));
                        pAttr->u8DispFbQRptr = (pAttr->u8DispFbQRptr + 1)%VSYNC_DISP_FB_Q_SZ;
                        pAttr->u32DropCnt++;
                        u8InterlaceModeDropCnt++;

                        VSYNC_LOG_DROP(VSYNC_PRINT("\033[1;35m[%s] %d pAttr->u32DropCnt = %d  system_time = %d   stc = %d  u32DroppedPts = %d    (%d,%d)  \033[m\n",__FUNCTION__,__LINE__,(int)pAttr->u32DropCnt,(int)MsOS_GetSystemTime(),(int)(u32stc/MS_90KHZ),(int)(u32DroppedPts/MS_90KHZ),(int)pAttr->u8DispFbQRptr, (int)pAttr->u8DispFbQWptr ));
                    }
                ...
                {// get next frame fail
                    if(pAttr->u8DispFbQRptr!= pAttr->u8DispFbQWptr)
                     {// only one frame in queue, display it rather than drop it
                        ...
                        if(u32stc > pAttr->u32LastFlipStc && (u32stc - pAttr->u32LastFlipStc) /MS_90KHZ > (pAttr->u32VsycnDuration*2) && pAttr->u32LastFlipStc!= 0 && (!_VSYNC_IsInterlaceFieldMode(VsynDispId) ||  u8InterlaceModeDropCnt%2==0)   )
                        {
                            pAttr->u8SlowDropCnt = 0;
                            sync_status = E_VSYNC_FREERUN;
                            VSYNC_LOG_DBG(VSYNC_PRINT("\033[1;31m[%s] %d only one frame , display it (%d %d)  u32LastFlipStc = %d  u32stc = %d\033[m\n",__FUNCTION__,__LINE__,(int)pAttr->u8DispFbQRptr,(int)pAttr->u8DispFbQWptr,(int)pAttr->u32LastFlipStc,(int)u32stc));
                        }
                        else
                        {
                            while(pAttr->u8DispFbQRptr != pAttr->u8DispFbQWptr)
        ...
        if(sync_status != E_VSYNC_DROP && pNextDispFrm != NULL && pAttr->u8DispFbQRptr!=pAttr->u8DispFbQWptr)
        {
            if((sync_status == E_VSYNC_AVSYNC || sync_status == E_VSYNC_FREERUN))
            {
                ...
                if(E_VSYNC_FREEZE != pAttr->eStatus)
                {
                    ...
                    pAttr->u32DispCnt++;
                    pAttr->u64Pts = (MS_U64) (((MS_U64)pNextDispFrm->stFrmInfo.u32ID_H) << 32 | (MS_U64)pNextDispFrm->stFrmInfo.u32TimeStamp) + (MS_U64)pAttr->u32SyncDelay;
                    MS_U32 u32pts = (MS_U32)pAttr->u64Pts;
                    pAttr->s64PtsStcDelta = (MS_S64)(u32pts) -  (MS_S64)(u32stc);
                    pAttr->u32LastFlipStc = u32stc;

                    VSYNC_LOG_TIME( VSYNC_PRINT("\033[1;32m[%s] %d  u32stc = %d  u32pts = %d  systime = %d  sync_status = 0x%x  d_cnt:%d r_cnt:%d \033[m\n",__FUNCTION__,__LINE__,(int)(u32stc/MS_90KHZ),(int)(u32pts/MS_90KHZ),(int)MsOS_GetSystemTime() ,sync_status,(int)pAttr->u32DropCnt,(int)pAttr->u32RepeatCnt));
                }
            ...
            else
            {
                ...
                {
                    if(((MS_U32)(pAttr->u64Pts) )/MS_90KHZ + pAttr->u32VsycnDuration <= u32StcBefore/MS_90KHZ)
                    {
                        continue;
                    }
                }
                ...
                {
                    if(((MS_U32)pAttr->u64NextPts )/MS_90KHZ <  u32StcBefore/MS_90KHZ )
                    {
                        continue;
                    }
                }
...
VDEC_EX_Result MApi_VSync_SetControl(int VsynDispId, VSYNC_User_Cmd cmd_id, void* param)
{
    ...
    switch(cmd_id)
    {
        ...
        case E_VSYNC_USER_CMD_AVSYNC_REPEAT_TH:
            ...
            pAttr->u32RepeatThreshold = (*(MS_U32*)param);
            break;
