...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define GE_MIU_ADDR_MASK            0x7FFFFFFFUL
...
#define GE_DELAY()                  MsOS_DelayTask(2)
...
{
#if 1
    if(ge_fbaddr>=HAL_MIU2_BASE)
    {
        return 2;
    }
    else if(ge_fbaddr>=HAL_MIU1_BASE)
    {
        return 1;
    }
    else
...
MS_PHY _GFXAPI_PHYS_ADDR_IN_MIU(MS_PHY ge_fbaddr)
{
    ...
    if(ge_fbaddr>=HAL_MIU2_BASE)
    {
        return (ge_fbaddr -= HAL_MIU2_BASE);
    }
    else if(ge_fbaddr>=HAL_MIU1_BASE)
    {
...
MS_PHY _GFXAPI_PHYS_ADDR_2_API(MS_U8 u8MIUId, MS_PHY ge_addrInMIU)
{
    ...
    if(u8MIUId == 2)
    {
        return  (HAL_MIU2_BASE| (ge_addrInMIU&((1UL<<_GET_MIU_MASK_SHIFT())-1)));
    }
    else if(u8MIUId == 1)
    {
...
GE_VcmqBufSize _GE_ConvertVQSize2VQSizeEnum(MS_U32 u32VcmdqSize)
{
    GE_VcmqBufSize vqSize;

    if(u32VcmdqSize>=1024*1024)
        vqSize = E_GE_VCMD_1024K;
    else if(u32VcmdqSize>=512*1024)
        vqSize = E_GE_VCMD_512K;
    else if(u32VcmdqSize>=256*1024)
        vqSize = E_GE_VCMD_256K;
    else if(u32VcmdqSize>=128*1024)
        vqSize = E_GE_VCMD_128K;
    else if(u32VcmdqSize>=64*1024)
        vqSize = E_GE_VCMD_64K;
    else if(u32VcmdqSize>=32*1024)
        vqSize = E_GE_VCMD_32K;
    else if(u32VcmdqSize>=16*1024)
        vqSize = E_GE_VCMD_16K;
    else if(u32VcmdqSize>=8*1024)
        vqSize = E_GE_VCMD_8K;
    else
        vqSize = E_GE_VCMD_4K;
    return vqSize;
...
void GE_WaitCmdQAvail(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_U32 u32Count)
{
    ...
    else
    {
        ...
        while (GE_CMDQ_FREECNT() < u32Count)
        {
            ...
            if (waitcount >= 0x80000)
            {
                printf("[GE] Wait command queue: %d : %x, %tx \n", tmp1, (ptrdiff_t)GE_CMDQ_FREECNT(), (ptrdiff_t)u32Count);
                waitcount = 0;
                tmp1++;
                if(tmp1 > 10)
                {
...
void GE_WriteReg(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_U16 addr, MS_U16 value)
{
    ...
    {
        printf("[%s][%d] Reg Index [%d]is out of GE_TABLE_REGNUM [0x%lx]range!!!!\n",__FUNCTION__,__LINE__, addr, GE_TABLE_REGNUM);
    }
...
void GE_Init(GE_CTX_HAL_LOCAL *pGEHalLocal, GE_Config *cfg)
{
...
#ifdef GE_VQADDR_LOCK_PATCH
            u16temp = GE_ReadReg(pGEHalLocal, REG_GE_CFG);
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, u16temp | GE_CFG_VCMDQ);
            GE_SetVCmdBuffer(pGEHalLocal, PhyVQAddr, _GE_ConvertVQSize2VQSizeEnum(u32VcmdqBufSz));
#else
            GE_SetVCmdBuffer(pGEHalLocal, PhyVQAddr, _GE_ConvertVQSize2VQSizeEnum(u32VcmdqBufSz));
            u16temp = GE_ReadReg(pGEHalLocal, REG_GE_CFG);
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, u16temp | GE_CFG_VCMDQ);
    ...
    u16temp = GE_ReadReg(pGEHalLocal, REG_GE_TH);    //GE is not first init
    GE_WriteReg(pGEHalLocal, REG_GE_TH, (u16temp | GE_THRESHOLD_SETTING));  //GE Threshold setting
    GE_WriteReg(pGEHalLocal, REG_GE_CFG, GE_ReadReg(pGEHalLocal, REG_GE_CFG)|GE_BIT15);  //GE power saving
	//Mask Interrupt
	GE_WriteReg(pGEHalLocal, REG_GE_SRCMASK_GB, 0x00C0);

    GE_EnableDynaClkGate(pGEHalLocal,TRUE);
}
...
GE_Result GE_SetClock(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_BOOL bOnOff)
{
    ...
    handle = Drv_Clkm_Get_Handle("g_clk_ge");
    Drv_Clkm_Set_Clk_Source(handle,"CLK_FASTEST");
#else
    MS_U16 u16tmp = 0;
