...
GOP_CHIP_PROPERTY g_GopChipPro =
{
    ...
    .bInternalMIUSelect=        {TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE}, //{E_GOP0, E_GOP1, E_GOP2, E_GOP3, E_GOP4, E_GOP_Dwin, E_GOP_MIXER, E_GOP5}
    .bAFBC_Support=             {FALSE, FALSE, FALSE, FALSE, FALSE},
    ...
    .GOP_MUX_Delta  =           0x4,
    .GOP_Mux_Offset =           {0x0, 0x1, 0x2, 0x3, 0x4},
    .GOP_Zorder_Mux_Offset =    {0x1, 0x1, 0x1, 0x1, 0x1},
    ...
    .bOpInterlace =             FALSE,
    .bAFBC_Merge_GOP_Trig =     FALSE,
    .bAFBCMIUSelDoubleBuffer =  FALSE,
    .bGOPVscalePipeDelay  =     {FALSE, FALSE, FALSE, TRUE, FALSE, FALSE},
    .bOPHandShakeModeSupport =  FALSE,
    .bIgnoreIP1HPD =            TRUE,
    .bOPMuxDoubleBuffer =       FALSE,
...
MS_U8 _GetGwinOffsetInGOP(MS_U8 u8GwinId)
{
    if(u8GwinId >= GOP4_GwinIdBase)
        return (u8GwinId - GOP4_GwinIdBase);
    else if(u8GwinId >= GOP3_GwinIdBase)
        return (u8GwinId - GOP3_GwinIdBase);
    else if( u8GwinId >= GOP2_GwinIdBase)
        return (u8GwinId - GOP2_GwinIdBase);
    else if (u8GwinId >= GOP1_GwinIdBase)
        return (u8GwinId - GOP1_GwinIdBase);
    else
        return u8GwinId;
}
...
GOP_Result HAL_GOP_SetGOPACK(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 gop)
{
    ...
    switch(gop)
    {
        ...
        case E_GOP3:
            ...
            break;
        case E_GOP4:
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_BAK_SEL_EX, GOP_BIT2 , GOP_BIT2);
            break;
        default:
            printf("[%s][%d] Unsurrport GOP=%d\n",__FUNCTION__,__LINE__ ,gop);
            break;
    }
    if(g_GopChipPro.bAFBC_Merge_GOP_Trig ==FALSE)
    {
...
void HAL_GOP_Init(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOPNum)
{
    ...
    HAL_GOP_Write16Reg(pGOPHalLocal, u32bankoff+GOP_4G_BW, GOP_BIT15|GOP_BIT14, GOP_BIT15|GOP_BIT14);
    if(u8GOPNum==E_GOP3)
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, u32bankoff+GOP_4G_BW, GOP3_FIFO_THRESHOLD, GOP_REG_LW_MASK );  //set DMA FIFO threshold to 3/4 FIFO length
    }
    ...
    //Set handshake mode
    if(g_GopChipPro.bOPHandShakeModeSupport)
        HAL_GOP_Write16Reg(pGOPHalLocal, u32bankoff+GOP_4G_BANK_FWR, GOP_BIT10, GOP_BIT10);
    else
        HAL_GOP_Write16Reg(pGOPHalLocal, u32bankoff+GOP_4G_BANK_FWR, 0, GOP_BIT10);
...
//    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_LB_SRAMCLK, 0, BMASK(7:0));                                /* GOP Line buffer sram clock */
    ...
    switch(u8GOPNum)
    {
        ...
        case E_GOP3:
            ...
            break;
        case E_GOP4: // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
    ...
    {
        if(u8GOPNum ==E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
    ...
    if(g_GopChipPro.bOPMuxDoubleBuffer == TRUE)
    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_4G_OP_MUX_DBF, GOP_BIT15 , GOP_BIT15);
    else
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_4G_OP_MUX_DBF, 0 , GOP_BIT15);
...
void HAL_GOP_BANK_SEL(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8bank)
{
    ...
    u16Bank &= ~GOP_BIT8;
    if(u8bank&GOP_BIT4); // reg_gop_bnk_4
        u16Bank |= GOP_BIT8;
...
#if (MAX_GOP_SUPPORT>4)
    if(GOP_READ2BYTE(GOP_BAK_SEL_EX)&GOP_BIT8) //reg_gop_bnk_4
        return GOP_BIT4|(u16GetBank&GOP_BANK_MASK);
#endif
...
GOP_Result HAL_GOP_InitMux(GOP_CTX_HAL_LOCAL *pGOPHalLocal)
{
    ...
    */
    ...
    if(g_GopChipPro.bOPMuxDoubleBuffer == TRUE)
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_BAK_SEL_EX, GOP_BIT14 , GOP_BIT14); //SC_0 Mux trigger
...
void HAL_GOP_GWIN_SetMUX(GOP_CTX_HAL_LOCAL*pGOPHalLocal, MS_U8 u8GOPNum, Gop_MuxSel eGopMux)
{
    ...
    switch(eGopMux)
    {
        ...
        case E_GOP_OP1_MUX:
            ...
            if(g_GopChipPro.bOPMuxDoubleBuffer == TRUE)
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_BAK_SEL_EX, GOP_BIT13 , GOP_BIT13); //SC_1 Mux trigger
          break;
        case E_GOP_IP0_MUX:
...
GOP_Result HAL_GOP_SetGOPEnable2SC(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 gopNum, MS_BOOL bEnable)
{
    ...
    */
    ...
    MS_U8  u8GopNumTemp = 0;//save the gop num after convert
    ...
    u8GopNumTemp = GOPNUM_TRANSFORM(gopNum);
    ...
    if (u8GopNumTemp == (muxValue & GOP_MUX0_MASK))    //enable mux0 to SC
    {
        if(bOSDBEnable)
        {
            if(bEnable)
    ...
    if (u8GopNumTemp == ((muxValue & GOP_MUX1_MASK)>>(GOP_MUX_SHIFT*1))) //enable mux1
    {
        if(bOSDBEnable)
        {
            if(bEnable)
    ...
    if (u8GopNumTemp == ((muxValue & GOP_MUX2_MASK)>>(GOP_MUX_SHIFT*2))) //enable mux2
    {
        if(bOSDBEnable)
        {
            if(bEnable)
    ...
    if (u8GopNumTemp == ((muxValue & GOP_MUX3_MASK)>>(GOP_MUX_SHIFT*3))) //enable mux3
    {
        if(bOSDBEnable)
        {
            if(bEnable)
    ...
    if (u8GopNumTemp == ((muxValue & GOP_MUX4_MASK)>>(GOP_MUX_SHIFT*4))) //enable mux4
    {
        if(bOSDBEnable)
        {
            if(bEnable)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_EX, GOP_BIT3, GOP_BIT3);
            }else{
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_EX, 0, GOP_BIT3);
            }
        }
        else
    ...
    if(MAX_GOP_SUPPORT ==5)
    {
        ...
        if (u8GopNumTemp == ((mux4Value & GOP_MUX4_MASK)>> GOP_MUX4_SHIFT)) //enable mux4
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_GOPEN, bEnable?(regval |GOP_BIT11):(regval & ~GOP_BIT11), GOP_BIT11);
        }
    }
    ...
    ******************/
    ...
    switch(eDstType)
    {
        ...
        case E_DRV_GOP_DST_OP0:
            ...
            }else if (gopNum==E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP5CLK, CKG_GOPG5_ODCLK, CKG_GOPG5_MASK);
            }else
            {
                MS_ASSERT(0);
                return GOP_INVALID_PARAMETERS;
	...
	case E_DRV_GOP_DST_IP1:
	     if (gopNum==E_GOP0)
            ...
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP5CLK, CKG_GOPG5_IDCLK1, CKG_GOPG5_MASK);
            }else
            {
                MS_ASSERT(0);
                return GOP_INVALID_PARAMETERS;
        ...
        case E_DRV_GOP_DST_OP1:
	     if (gopNum==E_GOP0)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, CKG_GOPG0_OD1CLK, CKG_GOPG0_MASK);
            }
            else if (gopNum==E_GOP1)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, CKG_GOPG1_OD1CLK, CKG_GOPG1_MASK);
            }
            else if (gopNum==E_GOP2)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP2CLK, CKG_GOPG2_OD1CLK, CKG_GOPG2_MASK);
            }else if (gopNum==E_GOP3)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP3CLK, CKG_GOPG3_OD1CLK, CKG_GOPG3_MASK);
            }else if (gopNum==E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP5CLK, CKG_GOPG5_OD1CLK, CKG_GOPG5_MASK);
            }else
            {
                MS_ASSERT(0);
                return GOP_INVALID_PARAMETERS;
        ...
        case E_DRV_GOP_DST_DIP:
	     if (gopNum==E_GOP0)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, CKG_GOPG0_DIP, CKG_GOPG0_MASK);
            }
            else if (gopNum==E_GOP1)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, CKG_GOPG1_DIP, CKG_GOPG1_MASK);
            }
            else if (gopNum==E_GOP2)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP2CLK, CKG_GOPG2_DIP, CKG_GOPG2_MASK);
            }else if (gopNum==E_GOP3)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP3CLK, CKG_GOPG3_DIP, CKG_GOPG3_MASK);
            }else if (gopNum==E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP5CLK, CKG_GOPG5_DIP, CKG_GOPG5_MASK);
            }else
            {
                MS_ASSERT(0);
                return GOP_INVALID_PARAMETERS;
...
GOP_Result HAL_GOP_MIXER_EnableOldBlendMode(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP, MS_BOOL bEn)
{
    ...
    if(gopDstType == E_DRV_GOP_DST_MIXER2VE)
    {
        ...
        if(bEn)
        {
            ...
            {
                MApi_VE_W2BYTE_MSK(MS_VE_REG_BANK_3B, GOP_VE_TVS_OSD_EN, (MS_U16)(~BIT(2))|BIT(1), BIT(1)|BIT(2));
            }
            else if(u8GOP == ((regval&(GOP_BIT12|GOP_BIT13))>>12))
            {
                MApi_VE_W2BYTE_MSK(MS_VE_REG_BANK_3B, GOP_VE_TVS_OSD1_EN, (MS_U16)(~BIT(2))|BIT(1), BIT(1)|BIT(2));
            }
            else
            {
        ...
        else
        {
            ...
            if(u8GOP == ((regval&(GOP_BIT10|GOP_BIT11))>>10))
            {
                MApi_VE_W2BYTE_MSK(MS_VE_REG_BANK_3B, GOP_VE_TVS_OSD_EN, (MS_U16)(~BIT(1))|BIT(2), BIT(1)|BIT(2));
            }
            else if(u8GOP == ((regval&(GOP_BIT12|GOP_BIT13))>>12))
            {
                MApi_VE_W2BYTE_MSK(MS_VE_REG_BANK_3B, GOP_VE_TVS_OSD1_EN, (MS_U16)(~BIT(1))|BIT(2), BIT(1)|BIT(2));
            }
            else
            {
                printf("[%s][%d]GOP not exisit in VE mux\n",__FUNCTION__,__LINE__);
...
GOP_Result HAL_GOP_VE_SetOutputTiming(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U32 u32mode)
{
    ...
    {
        /* PAL System*/
        pGOPHalLocal->pGopChipPro->GOP_VE_PD = 0xB9;
    ...
    {
        /* NTSC System*/
        pGOPHalLocal->pGopChipPro->GOP_VE_PD = 0xA3;
...
GOP_Result HAL_GOP_VE_SetOSDEnable(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_BOOL bEnable, EN_VE_OSD_ENABLE eOSD, MS_U8 gopNum)
{
    if(bEnable) // enable OSD
    {
        if(eOSD == EN_OSD_0)
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN, BIT(0), BIT(0));
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN_ALPHA_INV, (bEnable? BIT(7): ~(BIT(7))), BIT(7));
        }
        else if(eOSD == EN_OSD_1)
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, BIT(0), BIT(0));
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, (bEnable? BIT(5): ~(BIT(5))), BIT(5));
        }
    }
    else // disable OSD
    {
        if(eOSD == EN_OSD_0)
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN, 0x0, BIT(0));
        else
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, 0x0, BIT(0));
    }
...
GOP_Result HAL_GOP_SetGOPToVE(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 gopNum, MS_BOOL bEn )
{
    ...
    if((gopNum != ((regval&(GOP_BIT6|GOP_BIT7|GOP_BIT8)>>6))) && (gopNum != ((regval&(GOP_BIT9|GOP_BIT10|GOP_BIT11))>>9)))
    {
        ...
        if (VE_MUX_INIT_VALUE == ((regval&(GOP_BIT6|GOP_BIT7|GOP_BIT8))>>6))
        {
            ...
            HAL_GOP_VE_SetOSDEnable(pGOPHalLocal,bEn, EN_OSD_0, gopNum);
        }
        else
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MUX4_MIX_VE, (gopNum<<9), (GOP_BIT9|GOP_BIT10|GOP_BIT11));
            HAL_GOP_VE_SetOSDEnable(pGOPHalLocal,bEn, EN_OSD_1, gopNum);
        }
    }
    else if(gopNum != ((regval&(GOP_BIT6|GOP_BIT7|GOP_BIT8))>>6))
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MUX4_MIX_VE, (gopNum<<9), (GOP_BIT9|GOP_BIT10|GOP_BIT11));
        HAL_GOP_VE_SetOSDEnable(pGOPHalLocal,bEn, EN_OSD_1, gopNum);
    }
...
GOP_Result HAL_GOP_PowerState(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U32 u32PowerState, GFLIP_REGS_SAVE_AREA* pGOP_STRPrivate)
{
    switch(u32PowerState)
    {
        case E_POWER_SUSPEND:
        {
            ...
            HAL_GOP_Read16Reg(pGOPHalLocal, GOP_GOP5CLK, &(pGOP_STRPrivate->CKG_GopReg[3]));
...
//            HAL_GOP_Read16Reg(pGOPHalLocal, GOP_LB_SRAMCLK, &(pGOP_STRPrivate->CKG_GopReg[5]));
        ...
        case E_POWER_RESUME:
        {
            ...
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP5CLK, pGOP_STRPrivate->CKG_GopReg[3], GOP_REG_WORD_MASK);
...
GOP_Result HAL_GOP_SetGopGwinHVPixel(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP, MS_U8 u8win, MS_U16 hstart, MS_U16 hend, MS_U16 vstart, MS_U16 vend)
{
    ...
    MS_U32 u32BankOffSet=0xFFFF;
    _GetBnkOfstByGop(u8GOP, &u32BankOffSet);
    HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_HSTR(_GetGwinOffsetInGOP(u8win)), hstart, GOP_REG_WORD_MASK);
    HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_HEND(_GetGwinOffsetInGOP(u8win)), hend, GOP_REG_WORD_MASK);
    HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_VSTR(_GetGwinOffsetInGOP(u8win)), vstart, GOP_REG_WORD_MASK);
    HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_VEND(_GetGwinOffsetInGOP(u8win)), vend, GOP_REG_WORD_MASK);
...
GOP_Result HAL_GOP_Set_MIU(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP,MS_U8 miusel)
{
    ...
    if(u8GOP==E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
...
{
    MS_U16 u16regval=0,u16GopMux;
    MS_U8 u8mux=0,u8mux_sel=0;

    for (u8mux=0; u8mux<MAX_GOP_MUX; u8mux++)
    {
        HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MUX, &u16GopMux);
        u8mux_sel = (u16GopMux >> (u8mux*GOP_MUX_SHIFT))& GOP_REGMUX_MASK;
        if (u8mux_sel==GOP_MIXER_MUX)
        {
            switch(u8mux)
            {
                case E_GOP_MUX0:
                {
                    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_L, &u16regval);
                    if(u16regval&GOP_BIT3)
                    {
                        *pbHDREnable = TRUE;
                        return GOP_SUCCESS;
                    }
                ...
                case E_GOP_MUX2:
                {
                    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_H, &u16regval);
                    if(u16regval&GOP_BIT3)
                    {
                        *pbHDREnable = TRUE;
                        return GOP_SUCCESS;
                ...
                case E_GOP_MUX4:
                {
                    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_EX, &u16regval);
                    if(u16regval&GOP_BIT3)
                    {
                        *pbHDREnable = TRUE;
                        return GOP_SUCCESS;
...
GOP_Result HAL_GOP_GWIN_SetARGB1555Alpha(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8win, MS_BOOL bAlpType, MS_U8 u8Val)
{
    MS_U16 regval;
    MS_U16 regmask;

    /*set GOP0,GOP1 ARGB1555 alpha define*/
    if (u8win<(MAX_GOP0_GWIN+MAX_GOP1_GWIN))
    {
        if(bAlpType)//set alpha1
        {
            regval = (u8Val<<8);
            regmask = 0xFF00;
        }
        ...
        HAL_GOP_Write16Reg(pGOPHalLocal, (u8win < MAX_GOP0_GWIN)? GOP_4G_ARGB1555_ALPHA_DEF(u8win):GOP_2G_ARGB1555_ALPHA_DEF(u8win-MAX_GOP0_GWIN), regval, regmask);

        return GOP_SUCCESS;
    }
    /*Only GOP0 GOP1 support setting ARGB1555 alpha*/
    else
    {
        return GOP_FAIL;
    }
...
GOP_Result HAL_GOP_CMDQ_SetGOPACK(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 gop)
{
    ...
    if(gop == E_GOP4) // K6 : GOP0~3 and GOP5, NO GOP4. Mapping E_GOP4 to GOP 5 reg
...
GOP_Result HAL_GOP_CMDQ_SetGOPACKMask(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U16 u16GopMask)
{
    ...
    for(gop = 0; gop<MAX_GOP_SUPPORT; gop++)
    {
        switch(gop)
        {
            ...
            {
                u16MiuClient = GOP_MIU_CLIENT_GOP5;
                HAL_GOP_AFBC_GetCore(pGOPHalLocal, (MS_U8)E_GOP4, &u8Core);
                bCoreTrig[u8Core]=TRUE;
                break;
            }
            default:
    ...
    for(i=0;i<AFBC_CORE_COUNT;i++)
    {
        if(g_GopChipPro.bAFBC_Merge_GOP_Trig == FALSE)
        {
            if(bCoreTrig[i]==TRUE)
            {
                if((g_GopChipPro.bAFBCMIUSelDoubleBuffer == FALSE)&&(bCoreMiu[i]==TRUE))
                {
                    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,REG_AFBC_MIU,u16AFBCMIUSelect[gop]<<4, 0xFFFF);
                }
                HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,REG_AFBC_TRIGGER,GOP_BIT1, 0xFFFF);
