...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#if defined(MSOS_TYPE_LINUX_KERNEL) || defined(MSOS_TYPE_NOS)
    ...
    if(u8Gop != GOPTYPE.GOP0)
    {
            ...
            if(bVMirror)
            {
                ...
                if(!g_pGOPCtxLocal->pGOPCtxShared->b32TileMode[u8Gop])  //if tile mode, we should not change dram address
                {
                    ...
                    pinfo->u32DRAMRBlkStart += ((MS_U32)pinfo->u16RBlkHRblkSize*((MS_U32)pinfo->u16DispVPixelEnd-(MS_U32)pinfo->u16DispVPixelStart-1));
...
MS_U32 GOP_GWIN_SetARGB1555ALPHA(void* pInstance,MS_U8 u8win, MS_BOOL bAlphaType, MS_U8 u8Val)
{
#ifdef INSTANT_PRIVATE
    GOP_INSTANT_PRIVATE* psGOPInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void*)&psGOPInstPri);
    ...
    MDrv_GOP_GWIN_SetARGB1555Alpha(g_pGOPCtxLocal, u8win,bAlphaType,u8Val);
    return GOP_API_SUCCESS;
...
MS_U32 Ioctl_GOP_SetProperty(void* pInstance,EN_GOP_PROPERTY type,MS_U32 gop, MS_U32* pSet, MS_U32 u32Size)
{
    ...
    switch(type)
    {
         ...
         if(ptrans_clr->transclr_property == EN_TRANSCLR_SET_COLOR)
        {
            ...
            {
                printf("[%s][%d] FORMAT SETTING ERROR!! \n",__FUNCTION__,__LINE__);
                return GOP_API_FAIL;
            }
            //MDrv_GOP_GWIN_SetTransClr_8888(g_pGOPCtxLocal,gop, fullclr, 0);
         }
         else if(ptrans_clr->transclr_property == EN_TRANSCLR_ENABLE)
         {
            ...
            if(((GOP_TransClrFmt)fmt < DRV_GOPTRANSCLR_FMT0) || ((GOP_TransClrFmt)fmt > DRV_GOPTRANSCLR_FMT3))
            {
                printf("[%s][%d] FORMAT SETTING ERROR!! \n",__FUNCTION__,__LINE__);
                return GOP_API_FAIL;
            }
        ...
        case E_GOP_R2Y_COLOR_MATRIX:
        {
            DRV_GOP_R2Y_Matrix *matrix = (DRV_GOP_R2Y_Matrix *)pSet;;
            if(u32Size != sizeof(DRV_GOP_R2Y_Matrix))
            {
                GOP_M_ERR("[%s][%d] input structure size ERROR!! \n",__FUNCTION__,__LINE__);
                return GOP_API_FAIL;
            }
            MDrv_GOP_SetR2YMatrix(g_pGOPCtxLocal,gop,matrix);
            break;
        }
...
{
    MS_U8 u8GOP_num = 0x0;
    MS_U16 u16x[MAX_GOP_SUPPORT] = {0};
    MS_U16 u16y[MAX_GOP_SUPPORT] = {0};
    MS_U16 u16Width[MAX_GOP_SUPPORT] = {0};
    MS_U16 u16Height[MAX_GOP_SUPPORT] = {0};
    ...
    if (!_GOP_IsGopNumValid(pInstance,u8GOP_num))
    {
        printf("[%s][%d]GOP %d  is out of range\n",__FUNCTION__,__LINE__,u8GOP_num);
        return GOP_API_FAIL;
    }
...
MS_U32 Ioctl_GOP_GWin_SetProperty(void* pInstance,EN_GOP_GWIN_PROPERTY type,MS_U32 gwin, MS_U32* pSet, MS_U32 u32Size)
{
    switch(type)
    {
        ...
        case E_GOP_GWIN_SETARGB1555ALPHA:
        {
            PGOP_GWIN_SETARGB1555ALPHA pInfo = (PGOP_GWIN_SETARGB1555ALPHA)pSet;

            if(u32Size != sizeof(GOP_GWIN_SETARGB1555ALPHA))
                return GOP_API_INVALID_PARAMETERS;


            GOP_GWIN_SetARGB1555ALPHA(pInstance,gwin,pInfo->bAlphaType, pInfo->Val);
...
MS_U32 GOPIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
...
#ifndef MSOS_TYPE_LINUX_KERNEL //Check size error when if(is_compat_task()==1)
            if(ptr->u32Size != sizeof(GOP_GWIN_CLEARQUEUE_INFO) && g_pGOPCtxLocal != NULL)
            {
                g_pGOPCtxLocal->pGOPCtxShared->s32GOPLockTid = 0;
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
                g_pGOPCtxLocal->pGOPCtxShared->GOPLockPid = 0;
...
#ifndef MSOS_TYPE_LINUX_KERNEL //Check size error when if(is_compat_task()==1)
            if(ptr->u32Size != sizeof(GOP_GWIN_FLIP_WIN_INFO) && g_pGOPCtxLocal != NULL)
            {
                GOP_M_INFO("[%s] (%d) Info structure Error!!\n",__FUNCTION__,__LINE__);
                g_pGOPCtxLocal->pGOPCtxShared->s32GOPLockTid = 0;
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
...
#ifndef MSOS_TYPE_LINUX_KERNEL //Check size error when if(is_compat_task()==1)
            if(ptr->u32Size != sizeof(GOP_GWIN_FLIP_MULTI_WIN_INFO) && g_pGOPCtxLocal != NULL)
            {
                GOP_M_INFO("[%s] (%d) Info structure Error!!\n",__FUNCTION__,__LINE__);
                g_pGOPCtxLocal->pGOPCtxShared->s32GOPLockTid = 0;
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
        ...
        case MAPI_CMD_GOP_PALETTE_SET_CONFIG:
        {
            ...
            if(ptr->u32Size != sizeof(GOP_PAL_OPT) && g_pGOPCtxLocal != NULL)
            {
                g_pGOPCtxLocal->pGOPCtxShared->s32GOPLockTid = 0;
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
                g_pGOPCtxLocal->pGOPCtxShared->GOPLockPid = 0;
...
#ifndef MSOS_TYPE_LINUX_KERNEL //Check size error when if(is_compat_task()==1)
            if(ptr->u32Size != sizeof(GOP_PALETTE_ENTRY) && g_pGOPCtxLocal != NULL)
            {
                g_pGOPCtxLocal->pGOPCtxShared->s32GOPLockTid = 0;
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
                g_pGOPCtxLocal->pGOPCtxShared->GOPLockPid = 0;
