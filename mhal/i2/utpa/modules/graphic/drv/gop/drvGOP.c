...
void MDrv_GOP_SetGOPEnable2Mode1(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 gopNum, MS_BOOL bEnable)
{
    ...
    if ((enGopDst == E_DRV_GOP_DST_OP0) || (E_DRV_GOP_DST_OP1))
    {
...
#if Mixer_preAlpha_setting
                bEnable = TRUE;
                Hal_GOP_Mixer_AlphaInverse(&pGOPDrvLocalCtx->halCtxLocal, TRUE);
                Hal_GOP_Mixer_RmaCtrl(&pGOPDrvLocalCtx->halCtxLocal, TRUE);
#else
            bEnable = FALSE;
#endif
        }
...
void MDrv_GOP_GWIN_SetStretchWin(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8GOP,MS_U16 x, MS_U16 y, MS_U16 width, MS_U16 height)
{
    ...
    {
        Hoffset = pGOPCtx->pGOPCtxShared->s32GOPStretchWinHOffset + x;
        if (Hoffset >= 0)
        {
            x = Hoffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_HSTART, 0x0, GOP_REG_WORD_MASK);
        }
        else
        {
            x = 0;
            u16RegVal = (-1) * Hoffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_HSTART, u16RegVal, GOP_REG_WORD_MASK);
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_HEND, 0xFFF, GOP_REG_WORD_MASK);
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_BANK_FWR, GOP_BIT4, GOP_BIT4);
        }

        Voffset = pGOPCtx->pGOPCtxShared->s32GOPStretchWinVOffset + y;
        if (Voffset >= 0)
        {
            y = y + pGOPCtx->pGOPCtxShared->s32GOPStretchWinVOffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_VSTART, 0x0, GOP_REG_WORD_MASK);
        }
        else
        {
            y = 0;
            u16RegVal = (-1) * Voffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_VSTART, u16RegVal, GOP_REG_WORD_MASK);
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_VEND, 0xFF0, GOP_REG_WORD_MASK);
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_BANK_FWR, GOP_BIT4, GOP_BIT4);
        }
    }
#endif
    ...
    if (E_DRV_GOP_DST_OP_DUAL_RATE == pGOPDrvLocalCtx->halCtxLocal.pHALShared->GOP_Dst[u8GOP])
    {
        ...
        Hal_GOP_SetOLED_4K120CropWindow(&pGOPDrvLocalCtx->halCtxLocal, u8GOP, x, y, pGOPCtx->pGOPCtxShared->s32GOPStretchWinHOffset, pGOPCtx->pGOPCtxShared->s32GOPStretchWinVOffset);
    }
#else
    if (E_DRV_GOP_DST_OP_DUAL_RATE == pGOPDrvLocalCtx->halCtxLocal.pHALShared->GOP_Dst[u8GOP])
    {
...
void MDrv_GOP_GWIN_SetStretchWinPosition(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8GOP,MS_U16 u16x, MS_U16 u16y)
{
    ...
    if (E_DRV_GOP_DST_OP_DUAL_RATE != pGOPDrvLocalCtx->halCtxLocal.pHALShared->GOP_Dst[u8GOP])
    {
        Hoffset = pGOPCtx->pGOPCtxShared->s32GOPStretchWinHOffset + u16x;
        if (Hoffset >= 0)
        {
            u16x = Hoffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_HSTART, 0x0, GOP_REG_WORD_MASK);
        ...
        Voffset = pGOPCtx->pGOPCtxShared->s32GOPStretchWinVOffset + u16y;
        if (Voffset >= 0)
        {
            u16y = Voffset;
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_CROP_VSTART, 0x0, GOP_REG_WORD_MASK);
    ...
    if (E_DRV_GOP_DST_OP_DUAL_RATE == pGOPDrvLocalCtx->halCtxLocal.pHALShared->GOP_Dst[u8GOP])
    {
        Hal_GOP_SetOLED_4K120CropWindow(&pGOPDrvLocalCtx->halCtxLocal, u8GOP, u16x, u16y, pGOPCtx->pGOPCtxShared->s32GOPStretchWinHOffset, pGOPCtx->pGOPCtxShared->s32GOPStretchWinVOffset);
    }
#endif
}
          ...
          if(bHDREnable==FALSE)
          {
              for (i=0; i<MAX_GOP_MUX; i++)
              {
                  ...
                  if (u8Gop==u8GOP_num)
                  {
                      ...
                      if((u16tmp & 0xF) != 0)
                      {
                          u16Offset = pGOPDrvLocalCtx->halCtxLocal.pGopChipPro->GOP_Zorder_Mux_Offset[i]* pGOPDrvLocalCtx->halCtxLocal.pGopChipPro->GOP_MUX_Delta;
                      }
                      else
                      {
                          u16Offset = pGOPDrvLocalCtx->halCtxLocal.pGopChipPro->GOP_Mux_Offset[i]* pGOPDrvLocalCtx->halCtxLocal.pGopChipPro->GOP_MUX_Delta + GOP_ZORDER_LAYER0_MUX_OFFSET;
                      }
#else
...
GOP_Result MDrv_GOP_GWIN_SetNewAlphaMode(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8win, MS_BOOL bEnable)
{
    ...
    u8GOP = MDrv_DumpGopByGwinId(pGOPCtx,u8win);
    DRV_GOP_XC_GET_SEMPHORE();
    HAL_GOP_SetGOPEnable2Mode1(&pGOPDrvLocalCtx->halCtxLocal, u8GOP, bEnable);
    DRV_GOP_XC_RELEASE_SEMPHORE();
#endif
    GOP_GWIN_UpdateReg(pGOPDrvLocalCtx, (E_GOP_TYPE)MDrv_DumpGopByGwinId(pGOPCtx,u8win));
...
/********************************************************************************/
///GOP Set Color Matrix
/********************************************************************************/
GOP_Result MDrv_GOP_SetR2YMatrix(MS_GOP_CTX_LOCAL *pGOPCtx,MS_U8 u8GOP,DRV_GOP_R2Y_Matrix *matrix)
{
    ...
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB11,matrix->Matrix[0][0], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB12,matrix->Matrix[0][1], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB13,matrix->Matrix[0][2], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB21,matrix->Matrix[1][0], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB22,matrix->Matrix[1][1], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB23,matrix->Matrix[1][2], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB31,matrix->Matrix[2][0], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB32,matrix->Matrix[2][1], 0xFFFF);
    HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_R2Y_SRGB33,matrix->Matrix[2][2], 0xFFFF);
...
void MDrv_GOP_GWIN_SetARGB1555Alpha(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8win, MS_BOOL bAlpType, MS_U8 u8Val)
{
#if GOP_SET_ARGB1555_ALPHA_MD
    GOP_CTX_DRV_LOCAL*pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL*)pGOPCtx;
    ...
    HAL_GOP_GWIN_SetARGB1555Alpha(&pGOPDrvLocalCtx->halCtxLocal, u8win, bAlpType, u8Val);
    GOP_GWIN_UpdateReg(pGOPDrvLocalCtx, (E_GOP_TYPE)MDrv_DumpGopByGwinId(pGOPCtx,u8win));
#endif
}

#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
GOP_Result MDrv_GOP_MDCMD_GETINFO(void* pInstance, MS_U64* u64ReqHdl)
{
...
#ifdef INSTANT_PRIVATE
    GOP_INSTANT_PRIVATE* psGOPInstPri = NULL;
    if(UtopiaInstanceGetPrivate(pInstance, (void*)&psGOPInstPri) != UTOPIA_STATUS_SUCCESS)
        return GOP_FAIL;
#endif
    ...
    MdbPrint(u64ReqHdl, "Layer mapping");
    for(gop = 0; gop < MAX_GOP_SUPPORT; gop++)
    {
        MdbPrint(u64ReqHdl, ":%d",g_gopDrvCtxLocal.halCtxLocal.pGopChipPro->GOP_MapLayer2Mux[gop]);
    }
    MdbPrint(u64ReqHdl, "\n\n");
    ...
    for(gop = 0; gop < MAX_GOP_SUPPORT; gop++)
    {
        ...
        MdbPrint(u64ReqHdl, "Stretch Output Window(%d, %d)\n", (u16temp == 0)?0:((u16Regval*2*4096)/u16temp), (u16temp1 == 0)?0:((u16Regval1*4096)/u16temp1));
        ...
        if(g_gopDrvCtxLocal.halCtxLocal.pGopChipPro->bInternalMIUSelect[gop] == TRUE)
            MdbPrint(u64ReqHdl, "GOP%d is HW or SW select: HW select\n", gop);
        else
            MdbPrint(u64ReqHdl, "GOP%d is HW or SW select: SW select\n", gop);
    }

    return GOP_SUCCESS;
}

GOP_Result MDrv_GOP_MDCMD_EchoCmd(void* pInstance, MS_U64* u64ReqHdl, char* pcCmdLine)
{
    MS_BOOL enable = FALSE;
    MS_U8 GOPnum = 0, MIUsel = 0;
    MS_U32 u32BankOffSet = 0;
    char pch[] = "=,";
    char* psep;
    ...
    if(strncmp("help", pcCmdLine, 4) == 0)
    {
        MdbPrint(u64ReqHdl,"----------MStar help----------\n");
        MdbPrint(u64ReqHdl,"=========cat cmd=========\n");
        MdbPrint(u64ReqHdl,"cat /proc/utopia_mdb/gop\n");
        MdbPrint(u64ReqHdl,"1.  Total GOP:   the number of GOP\n");
        MdbPrint(u64ReqHdl,"2.  OP mux setting:   GOP and OP mux mapping\n");
        MdbPrint(u64ReqHdl,"     [e.g.]0:2:1:4:3\n");
        MdbPrint(u64ReqHdl,"     Mux0:GOP0; Mux1:GOP2; Mux2:GOP1; Mux3:GOP4; Mux4:GOP3\n");
        MdbPrint(u64ReqHdl,"3.  Layer mapping:   layer and OP mux mapping\n");
        MdbPrint(u64ReqHdl,"     [e.g.]0:2:3:1:4\n");
        MdbPrint(u64ReqHdl,"     layer0:Mux0; layer1:Mux2; layer2:Mux3; layer3:Mux1; layer4:Mux4\n");
        MdbPrint(u64ReqHdl,"4.  Initialized:   Initial or not(True/False)\n");
        MdbPrint(u64ReqHdl,"5.  Stretch Original Window(w, h):   window resolution before scaling\n");
        MdbPrint(u64ReqHdl,"                                                 w = width, h = height\n");
        MdbPrint(u64ReqHdl,"6.  Stretch Output Window(w, h):   window resolution after scaling\n");
        MdbPrint(u64ReqHdl,"7.  Stretch Window Position(x, y):   stretch window position\n");
        MdbPrint(u64ReqHdl,"8.  Destination:   GOP destination\n");
        MdbPrint(u64ReqHdl,"9.  Address MIU:   Data buffer in which MIU\n");
        MdbPrint(u64ReqHdl,"10.  H Mirror:   H Mirror or not(Enable/Disable)\n");
        MdbPrint(u64ReqHdl,"11.  V Mirror:   V Mirror or not(Enable/Disable)\n");
        MdbPrint(u64ReqHdl,"12.  Alpha pre-multiplex mode:   Enable or not\n");
        MdbPrint(u64ReqHdl,"13.  Color Space:   RGB/YUV\n");
        MdbPrint(u64ReqHdl,"14.  Color key Enable:   Enable/Disable\n");
        MdbPrint(u64ReqHdl,"15.  Color key value:   color key value\n");
        MdbPrint(u64ReqHdl,"16.  GWIN Enable:   Enable/Disable\n");
        MdbPrint(u64ReqHdl,"17.  GWIN Position(x, y):   GWIN position\n");
        MdbPrint(u64ReqHdl,"18.  GWIN Resolution(w, h):   the data resolution of buffer\n");
        MdbPrint(u64ReqHdl,"19.  GWIN Pitch:   the pitch of data buffer\n");
        MdbPrint(u64ReqHdl,"20.  GWIN Format:   the format of data buffer\n");
        MdbPrint(u64ReqHdl,"21.  GWIN Buffer Size:   the size of buffer\n");
        MdbPrint(u64ReqHdl,"22.  GWIN Pixel Alpha:   Pixel alpha or global alpha\n");
        MdbPrint(u64ReqHdl,"23.  GWIN miu offset address:   Data buffer address\n");
        MdbPrint(u64ReqHdl,"\n");
        MdbPrint(u64ReqHdl,"=========echo cmd=========\n");
        MdbPrint(u64ReqHdl,"echo testpattern enable=[opcode] > gop\n");
        MdbPrint(u64ReqHdl,"     [e.g.]enable/disable GOP test pattern\n");
        MdbPrint(u64ReqHdl,"     [opcode] = 1 : enable GOP test pattern\n");
        MdbPrint(u64ReqHdl,"     [opcode] = 0 : disable GOP test pattern\n\n");
        MdbPrint(u64ReqHdl,"echo GOPOn num=[opcode1],[opcode2] > gop\n");
        MdbPrint(u64ReqHdl,"     [e.g.]enable/disable GOP\n");
        MdbPrint(u64ReqHdl,"     [opcode1] = 0 : it means GOP0\n");
        MdbPrint(u64ReqHdl,"     [opcode1] = 1 : it means GOP1\n");
        MdbPrint(u64ReqHdl,"     [opcode2] = 0 : disable GOP\n");
        MdbPrint(u64ReqHdl,"     [opcode2] = 1 : enable GOP\n\n");
    }
    else
    {
        ...
        else if(strncmp("testpattern", psep, 11)==0)
        {
            psep = strsep(&pcCmdLine,pch);
            enable = (MS_BOOL)atoi(psep);
        ...
        else if(strncmp("miusel", psep, 6)==0)
        {
            psep = strsep(&pcCmdLine,pch);
            GOPnum = (MS_U8)atoi(psep);
            psep = strsep(&pcCmdLine,pch);
            MIUsel = (MS_U8)atoi(psep);
            if((GOPnum < MAX_GOP_SUPPORT) && (MIUsel < MAX_GOP_MIUCOUNT))
            {
                _GetBnkOfstByGop(GOPnum, &u32BankOffSet);
                HAL_GOP_Write16Reg(&g_gopDrvCtxLocal.halCtxLocal, u32BankOffSet+GOP_4G_MIU_SEL, MIUsel, GOP_BIT0|GOP_BIT1);
                MdbPrint(u64ReqHdl,"GOP%d switch to MIU %d\n", GOPnum, MIUsel);
            }
            else
            {
                MdbPrint(u64ReqHdl,"[error arg.] GOP num %d, MIUsel %d\n",GOPnum,MIUsel);
            }
        }
    }
