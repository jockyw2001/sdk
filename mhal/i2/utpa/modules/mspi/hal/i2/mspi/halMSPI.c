...
void HAL_MSPI_Init(void)
{
    ...
    TempData &= ~(MSPI_CLK_MASK);                    //use "~" instead of "!" for coverity issue
...
MS_BOOL HAL_MSPI_CLOCK_Config(MS_U32 u32MaxClock)
{
    ...
    u16ClkLevel = u16ClkLevel << MSPI_CLK_CFG_OFFSET;
...
MS_U8 HAL_MSPI_ReadClkConfig(void)
{
    return CLK_READ(MSPI_CLK_CFG);
}
MS_U8 HAL_MSPI_ReadCtrlConfig(void)
{
    return MSP_READ(MSPI_CLK_CLOCK_OFFSET);
}
#endif