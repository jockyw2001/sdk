...
{
    MS_U8 u8ClkConfig = 0;
    MS_U8 u8CtrlConfig = 0;
    MdbPrint(u64Hdl, "\n---------MStar MSPI Info--------- \n");

    MdbPrint(u64Hdl, "MSPI Init : %d. \n", gbInitFlag);
    MdbPrint(u64Hdl, "MSPI Init in Utopia : %d. \n", gbInitInUtopia);

    MdbPrint(u64Hdl, "MSPI DC Config : %d. \n", (gu8MSPIConfig & MSPI_DC_CONFIG));
    MdbPrint(u64Hdl, "MSPI CLK Config : %d. \n", (gu8MSPIConfig & MSPI_CLK_CONFIG));
    MdbPrint(u64Hdl, "MSPI Frame Config : %d. \n", (gu8MSPIConfig & MSPI_FRAME_CONFIG));

    u8ClkConfig = HAL_MSPI_ReadClkConfig();
    switch(((u8ClkConfig<< MSPI_CLK_CFG_OFFSET) & MSPI_MAXCLKLEVEL))
    {
        default:
        case 0:
            MdbPrint(u64Hdl, "CPU Clock for MSPI : %dMHz. \n", 108);
            break;
        case 1:
            MdbPrint(u64Hdl, "CPU Clock for MSPI : %dMHz. \n", 54);
            break;
    ...
    u8CtrlConfig = HAL_MSPI_ReadCtrlConfig();
    switch(((u8CtrlConfig<<MSPI_CLK_CLOCK_BIT_OFFSET)&MSPI_CLK_CLOCK_MAX))
    {
        default:
        case 0:
            MdbPrint(u64Hdl, "MSPI Clock DIV : %d. \n", 2);
            break;
        case 1:
            MdbPrint(u64Hdl, "MSPI Clock DIV : %d. \n", 4);
            break;
