...
MS_U32 MSPIMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            MdbPrint(paraCmdLine->u64ReqHdl, "---------MStar MSPI Info---------\n");
            MdbPrint(paraCmdLine->u64ReqHdl, "Currently not supported! \n");
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MDrv_MSPI_Debug_Cat(paraGetInfo->u64ReqHdl);
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
    UtopiaModuleRegisterMdbNode("mspi", (FUtopiaMdbIoctl)MSPIMdbIoctl);
#endif
