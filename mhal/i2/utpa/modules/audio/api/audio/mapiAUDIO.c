...
static MS_BOOL _AUDIOProbeResourceToUtopia(AUDIO_INSTANT_PRIVATE *_pInstantAudio)
{
    ...
    _pInstantAudio->fpAUDIODebugCmdRead =                               _MApi_AUDIO_Debug_Cmd_Read;
    _pInstantAudio->fpAUDIODebugCmdWrite =                              _MApi_AUDIO_Debug_Cmd_Write;
    _pInstantAudio->fpAUDIOSystemControl  =                             _MApi_AUDIO_SYSTEM_Control;
//To Do
...
void AUDIORegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    UtopiaModuleRegisterMdbNode(AUDIO_MODULE_NAME, AUDIOMdbIoctl);
#endif
    ...
    for (u32Index = 0; u32Index < AUDIO_MODULE_RESOURCE_AMOUNT; u32Index++)
    {
        ...
        sprintf((char *)&strResourceName, "%s%u", AUDIO_MODULE_NAME, (unsigned int)u32Index);
...
MS_U32 AUDIOIoctl(void *pInstance, MS_U32 u32Cmd, void *pArgs)
{
    ...
    PAUDIO_ST_DEBUG_CMD_READ pDebugCmdRead = NULL;
    PAUDIO_ST_DEBUG_CMD_WRITE pDebugCmdWrite = NULL;
    PAUDIO_ST_SYSTEM_CONTROL pSystemControl = NULL;
    ...
    switch (u32Cmd)
    {
        ...
        case MApi_CMD_AUDIO_DebugCmdRead:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "AUDIOIoctl - MApi_CMD_AUDIO_DebugCmdRead\n");
            pDebugCmdRead = (PAUDIO_ST_DEBUG_CMD_READ)pArgs;
            s32Status = psAudioInstPri->fpAUDIODebugCmdRead(pDebugCmdRead->pHandle, pDebugCmdRead->pCmdLine, pDebugCmdRead->u32CmdSize);
            pDebugCmdRead->s32Ret = s32Status;
            break;

        case MApi_CMD_AUDIO_DebugCmdWrite:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "AUDIOIoctl - MApi_CMD_AUDIO_DebugCmdWrite\n");
            pDebugCmdWrite = (PAUDIO_ST_DEBUG_CMD_WRITE)pArgs;
            s32Status = psAudioInstPri->fpAUDIODebugCmdWrite(pDebugCmdWrite->pHandle, pDebugCmdWrite->pCmdLine, pDebugCmdWrite->u32CmdSize);
            pDebugCmdWrite->s32Ret = s32Status;
            break;

        case MApi_CMD_AUDIO_SYSTEM_Control:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "AUDIOIoctl - MApi_CMD_AUDIO_SYSTEM_Control\n");
            pSystemControl = (PAUDIO_ST_SYSTEM_CONTROL)pArgs;
            pSystemControl->s32Ret = psAudioInstPri->fpAUDIOSystemControl(pSystemControl->pStrCmd, pSystemControl->u32StrCmdLength);
            break;
...
MS_U32 AUDIOMdbIoctl(MS_U32 u32Cmd, const void* const pArgs)
{
    if (pArgs == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "AUDIOMdbIoctl - Error! pArgs should not be NULL\n");
        return 0;
    }
    ...
    switch (u32Cmd)
    {
        case MDBCMD_CMDLINE:
        {
            MDBCMD_CMDLINE_PARAMETER *pMdbCmdLine = (MDBCMD_CMDLINE_PARAMETER *)pArgs;

            if (strchr(pMdbCmdLine->pcCmdLine, '=') != NULL)
            {
                _MApi_AUDIO_Debug_Cmd_Write(pMdbCmdLine->u64ReqHdl, pMdbCmdLine->pcCmdLine, pMdbCmdLine->u32CmdSize);
            }
            else
            {
                _MApi_AUDIO_Debug_Cmd_Read(pMdbCmdLine->u64ReqHdl, pMdbCmdLine->pcCmdLine, pMdbCmdLine->u32CmdSize);
            }
            pMdbCmdLine->result = MDBRESULT_SUCCESS_FIN;
        ...
        case MDBCMD_GETINFO:
        {
            MDBCMD_GETINFO_PARAMETER *pMdbGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            char u8CmdLine[] = "fw_version";

            _MApi_AUDIO_Debug_Cmd_Read(pMdbGetInfo->u64ReqHdl, u8CmdLine, strlen(u8CmdLine));
            pMdbGetInfo->result = MDBRESULT_SUCCESS_FIN;
