...
void _MApi_Audio_Monitor(void)
{
    ...
    MS_U8  u8CurrentSeWhile2, u8CurrentSeISR2, u8CurrentDecR2While2, u8CurrentDecR2Timer2, u8CurrentSndR2While2, u8CurrentSndR2Timer2;
    ...
    {
        MsOS_DelayTask(2);    //Fix counter mis-detection problem
    }
    ...
    {
        MsOS_DelayTask(3);
        u8CurrentSeWhile2 = MDrv_AUDIO_ReadDspCounter(DSP_SE_MAIN_COUNTER);
        u8CurrentSeISR2 = MDrv_AUDIO_ReadDspCounter(DSP_SE_ISR_COUNTER);

        if(((u8SeWhileFailCount >= 10) && (u8CurrentSeWhile2 == u8CurrentSeWhile )) || ((u8SeISRFailCount >= 10) && (u8CurrentSeISR2 == u8CurrentSeISR)))
        {
        ...
        {
            if(u8SeWhileFailCount >= 10)
                u8SeWhileFailCount = 0;

            if(u8SeISRFailCount >= 10)
                u8SeISRFailCount = 0;
        }
    }
    ...
    {
        MsOS_DelayTask(3);
        u8CurrentDecR2While2 = MDrv_AUDIO_ReadDspCounter(DEC_R2_MAIN_COUNTER);
        u8CurrentDecR2Timer2 = MDrv_AUDIO_ReadDspCounter(DEC_R2_TIMER_COUNTER);

        if(((u8DecR2WhileFailCount >= 10) && (u8CurrentDecR2While2 == u8CurrentDecR2While )) || ((u8DecR2TimerFailCount >= 10) && (u8CurrentDecR2Timer2 == u8CurrentDecR2Timer)))
        {
        ...
        {
            if(u8DecR2WhileFailCount >= 10)
                u8DecR2WhileFailCount = 0;

            if(u8DecR2TimerFailCount >= 10)
                u8DecR2TimerFailCount = 0;
        }
    }
    ...
    {
        MsOS_DelayTask(3);
        u8CurrentSndR2While2 =  MDrv_AUDIO_ReadDspCounter(SND_R2_MAIN_COUNTER);
        u8CurrentSndR2Timer2 = MDrv_AUDIO_ReadDspCounter(SND_R2_TIMER_COUNTER);

        if(((u8SndR2WhileFailCount >= 10) && (u8CurrentSndR2While2 == u8CurrentSndR2While )) || ((u8SndR2TimerFailCount >= 10) && (u8CurrentSndR2Timer2 == u8CurrentSndR2Timer)))
        {
        ...
        {
            if(u8SndR2WhileFailCount >= 10)
                u8SndR2WhileFailCount = 0;

            if(u8SndR2TimerFailCount >= 10)
                u8SndR2TimerFailCount = 0;
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: MApi_AUDIO_Debug_Cmd_Read()
/// @brief \b Function \b Description : debug command for read
/// @param <IN>        \b MS_U64 * : handle for print message
/// @param <IN>        \b char * : command line
/// @param <IN>        \b MS_U32 : command size
/// @param <OUT>       \b NONE :
/// @param <RET>       \b MS_S32 :
/// @param <GLOBAL>    \b NONE :
////////////////////////////////////////////////////////////////////////////////
MS_S32 _MApi_AUDIO_Debug_Cmd_Read(MS_U64 *pHandle, const char *pCmdLine, MS_U32 u32CmdSize)
{
    return MDrv_AUDIO_Debug_Cmd_Read(pHandle, pCmdLine, u32CmdSize);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: _MApi_AUDIO_Debug_Cmd_Write()
/// @brief \b Function \b Description : debug command for write
/// @param <IN>        \b MS_U64 * : handle for print message
/// @param <IN>        \b char * : command line
/// @param <IN>        \b MS_U32 : command size
/// @param <OUT>       \b NONE :
...
/// @param <GLOBAL>    \b NONE :
////////////////////////////////////////////////////////////////////////////////
MS_S32 _MApi_AUDIO_Debug_Cmd_Write(MS_U64 *pHandle, const char *pCmdLine, MS_U32 u32CmdSize)
{
    return MDrv_AUDIO_Debug_Cmd_Write(pHandle, pCmdLine, u32CmdSize);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: MApi_AUDIO_SYSTEM_Control()
/// @brief \b Function \b Description:  audio system control
/// @param <IN>        \b pStrCmd : pointer to command
/// @param <IN>        \b u32StrCmdLength : command length
/// @param <RET>       \b MS_S32 : On success zero is returned. On error, a negative value is returned.
/// @param <GLOBAL>    \b NONE :
////////////////////////////////////////////////////////////////////////////////
MS_S32 _MApi_AUDIO_SYSTEM_Control(MS_U8 *pStrCmd, MS_U32 u32StrCmdLength)
{
    return MDrv_AUDIO_SYSTEM_Control(pStrCmd, u32StrCmdLength);
}