...
MS_BOOL MApi_AUDIO_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    memset(&Status, 0x00, sizeof(AUDIO_ST_SET_AUDIOPARAM2));
    Status.bRet = 0;
    Status.eDecId = DecId;
...
MS_S32 MApi_AUDIO_Debug_Cmd_Read(MS_U64 *pHandle, const char *pCmdLine, MS_U32 u32CmdSize)
{

#ifdef AUDIO_UTOPIA_20
    AUDIO_ST_DEBUG_CMD_READ Status;
    Status.s32Ret = 0;
    Status.pHandle = pHandle;
    Status.pCmdLine = pCmdLine;
    Status.u32CmdSize = u32CmdSize;

    if(_AUDIO_IOCtrlOpen() == FALSE)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] Debug Cmd Read fail\n", __LINE__);
    }

    if (UtopiaIoctl(_pInstantAudio, MApi_CMD_AUDIO_DebugCmdRead, (void*)&Status) != UTOPIA_STATUS_SUCCESS)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] Debug Cmd Read fail\n", __LINE__);
    }
    return Status.s32Ret;
    ...
    return _MApi_AUDIO_Debug_Cmd_Read(pHandle, pCmdLine, u32CmdSize);

#endif
}

MS_S32 MApi_AUDIO_Debug_Cmd_Write(MS_U64 *pHandle, const char *pCmdLine, MS_U32 u32CmdSize)
{

#ifdef AUDIO_UTOPIA_20
    AUDIO_ST_DEBUG_CMD_WRITE Status;
    Status.s32Ret = 0;
    Status.pHandle = pHandle;
    Status.pCmdLine = pCmdLine;
    Status.u32CmdSize = u32CmdSize;
    ...
    if(_AUDIO_IOCtrlOpen() == FALSE)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] Debug Cmd Write fail\n", __LINE__);
    }

    if (UtopiaIoctl(_pInstantAudio, MApi_CMD_AUDIO_DebugCmdWrite, (void*)&Status) != UTOPIA_STATUS_SUCCESS)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] Debug Cmd Write fail\n", __LINE__);
    }
    return Status.s32Ret;
    ...
    return _MApi_AUDIO_Debug_Cmd_Write(pHandle, pCmdLine, u32CmdSize);
...
#ifdef AUDIO_UTOPIA_20
    AUDIO_ST_SYSTEM_CONTROL SYSTEM_Control;

    memset(&SYSTEM_Control, 0x00, sizeof(AUDIO_ST_SYSTEM_CONTROL));
    SYSTEM_Control.s32Ret = 0;
    SYSTEM_Control.pStrCmd = pStrCmd;
    SYSTEM_Control.u32StrCmdLength = u32StrCmdLength;

    if(_AUDIO_IOCtrlOpen() == FALSE)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] SYSTEM Control fail\n", __LINE__);
    }

    if (UtopiaIoctl(_pInstantAudio, MApi_CMD_AUDIO_SYSTEM_Control, (void*)&SYSTEM_Control) != UTOPIA_STATUS_SUCCESS)
    {
        ULOGE("AUDIO", "[MAPI AUDIO][%06d] SYSTEM Control fail\n", __LINE__);
    }

    return (SYSTEM_Control.s32Ret);
#else
    return _MApi_AUDIO_SYSTEM_Control(pStrCmd, u32StrCmdLength);
#endif
}
...
#endif
#endif //end of UTOPIAXP_REMOVE_WRAPPER