...
#define MP3ENC_MAX_PTS_TBL_NUM                          64
...
typedef struct
{
    ...
    MS_S32  ddp_disable_atmos;
    MS_U32  ac4_1st_pref_lang;
    MS_U32  ac4_2nd_pref_lang;
    MS_S32  ac4_de_gain;
    MS_S32  ac4_pres_index[2];
    //MS_S32  ac4_de_gain_present;    
    //MS_S32  ac4_associated_type;
    //MS_S32  b_ac4_pref_assoc_type_over_lang;    
    //MS_S32  ac4_prog_id_type;
    //MS_U8   ac4_program_identifier_string[2*16]; 
  
    MS_U32  DOLBY_PARAM_Reserved[32];
} AUR2_DOLBY_PARAM;
...
typedef struct
{
    ...
    MS_U32  drcPercent;         //0~100, 0: no DRC 
    MS_U32  DTS_PARAM_Reserved[9];
} AUR2_DTS_PARAM;
...
typedef struct
{
    ...
    MS_U8 presentation_type[AC4_MAX_PRESENTATION_NUM];   // πÔ¿≥®Ï EN_ac4_pres_type_t enum
}ms_ac4_toc_info;
...
typedef struct
{
    ...
    ms_ac4_toc_info    shm_ac4_toc;
    MS_U32  DOLBY_INFO_Reserved[32];    //reserved for new items
} AUR2_DOLBY_INFO;
...
typedef struct
{
    ...
    /* TVOS2.0 DTV */
    MS_U32 tvos2_pcm_wrptr;
    MS_U32 tvos2_cpts;
    MS_U32 tvos2_pcm_framesize;
    ...
    MS_U32 chunk_PTS_H;               // [32:0]  33bits chunk pts
    MS_U32 chunk_PTS_L;

    MS_U32 aac_decoder_type;
    MS_U32 ADEC_INFO_SHM_Reserved[9];    //reserved for new items
...
typedef struct
{
    ...
    MS_U32     Low_Latency_Limiter_PCM_Threshold;
    MS_U32     Low_Latency_Limiter_PCM_Skip_Target;

    /* TVOS2.0 DTV */
    MS_U32      TVOS2_PCM_rdPtr;          //  byte pointer        // update when change       //range : 0 ~ PCM1_DRAM_SIZE
    MS_U32      DTV_SyncType;             //0: Normal type, 1: TVOS2.0
    MS_U32      chunk_PTS_rd_flag;
    MS_U32     es_repeatMode;
    MS_U32     ADEC_PARAM_SHM_Reserved[9];    //reserved for new items
...
typedef struct
{
    MS_U32 lcl_enable;                    //0 or 1, 1 is default
    MS_U32 leveler_enable;            //0 or 1, 1 is default
    MS_U32 modeler_enable;            //0 or 1, 1 is default
    MS_U32 half_mode_enable;          //0 or 1, 0 is default
    MS_U32 amount;                        //0 ~ 10, 9 is default

    MS_U32 avm;                             //-1536 ~ 480, default is 0
    MS_U32 dvm;                             //-1536 ~ 480, default is 0

    MS_U32 irl;                               //0 ~ 2080, default is 1360
    MS_U32 orl;                              //0 ~ 2080, default is 1360
    MS_U32 pregain;                       //-480 ~ 480, default is 0
    MS_U32 agcGain;                       //0 ~ 20, default is 0

    MS_U32 limiter_enable;                //0 or 1, default is 1
    MS_U32 limiter_peak;                  //0 ~ 0x7FFF, default is 0x3f8d
    MS_U32 limiter_rms;                    //0 ~ 0x7FFF, default is 0x7101
    MS_U32 limiter_boost;                  //0 ~ 0x7FFF, default is 0x7f19
    MS_U32 AASE_DV258_PARAM_Reserved[10];
} AUR2_AASE_DV258_PARAM;

typedef struct
{
    MS_U32 MP3E_info_ES_wrPtr;               //mp3e wrptr
    MS_U32 MP3E_info_threshold;             //threshold between rdPtr and wrPtr
    MS_U32 MP3E_info_frame_size;          //mp3e frame size
    MS_U32 MP3E_info_available_size;    //mp3e avail size
    MS_U32 MP3E_info_frame_cnt;
    MS_U32 MP3E_info_APTS;

    MS_U32 MP3E_info_wrTabIndex;                   //mp3e wrptr index
    MS_U32 MP3E_info_ES_wrPtrTab[64];        //mp3e wrptr table for PTS
    MS_U32 MP3E_info_curPTSTab[64];          //mp3e current PTS table

    MS_U32 MP3E_INFO_Reserved[6];    //reserved for new items
} AUR2_MP3E_INFO;

typedef struct
{
    MS_U32 MP3E_param_ES_rdPtr;           //mp3e rdrptr
    MS_U32 MP3E_PARAM_Reserved[7];    //reserved for new items
} AUR2_MP3E_PARAM;

typedef struct
{
    MS_U32 sound_mode;                //pcmr sound mode for sw decoder
    MS_U32 AVR_no_atmos;
    MS_U32 PCMR_PARAM_Reserved[6];    //reserved for new items
} AUR2_PCMR_PARAM;

typedef struct
{
    MS_U32  dec_ch_mode[2];              // update per frame
} AUR2_D2A_SHM;       // DECR2 TO SND R2 shm

typedef struct
{
    MS_U32  SER2_PCMOUT_WRPTR;
    MS_U32  SER2_PCMOUT_SIZE;
    MS_U32  SER2_PCMOUT_OFFSET;

    MS_U32  SER2_HDMI_NONPCM_WRPTR;
    MS_U32  SER2_HDMI_NONPCM_SIZE;
    MS_U32  SER2_HDMI_NONPCM_OFFSET;
