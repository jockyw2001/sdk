...
#include "halSOUND.h"
#include "halAUR2.h"
#ifndef CONFIG_AUDIO_CODEC_AC4_MPEGH_DISABLE
    #include "K6_mainline_ac4_mpegh/decR2_version.h"
    #if ASND_R2_SUPPORT
        #include "K6_mainline_ac4_mpegh/sndR2_version.h"
    #endif

#else
    #include "K6_mainline/decR2_version.h"
    #if ASND_R2_SUPPORT
        #include "K6_mainline/sndR2_version.h"
    #endif
#endif
...
#if AUDIO_MCH_SW_DMA_READER_SUPPORT
#define MCH_SW_DMA_RDR_BUF_ADDR (ASND_DSP_DDR_SIZE + OFFSET_MCH_SW_DMA_READER_DRAM_BASE)
#define MCH_SW_DMA_RDR_BUF_SIZE MCH_SW_DMA_READER_DRAM_SIZE
#endif
...
#define MP3_ENCODER1_BUF_ADDR    (ASND_DSP_DDR_SIZE + OFFSET_MP3_ENC_DRAM_BASE)
#define MP3_ENCODER1_BUF_SIZE    (MP3_ENC_DRAM_SIZE)
#endif

#if AUDIO_MP3_ENCODER2_SUPPORT
#define MP3_ENCODER2_BUF_ADDR    (ASND_DSP_DDR_SIZE + OFFSET_MP3_ENC2_DRAM_BASE)
#define MP3_ENCODER2_BUF_SIZE    (MP3_ENC2_DRAM_SIZE)
#endif
...
static AUDIO_DEC_ID HDMIDecPriority[AU_DEC_MAX] = {AU_DEC_ID1, AU_DEC_ID3 , AU_DEC_INVALID};
static AUDIO_DEC_ID MMDecPriority[AU_DEC_MAX] = {AU_DEC_ID1, AU_DEC_ID3 , AU_DEC_INVALID};
static AUDIO_DEC_ID VDDecPriority[AU_DEC_MAX] = {AU_DEC_ID1, AU_DEC_INVALID , AU_DEC_INVALID};
static AUDIO_DEC_ID INJECTDecPriority[AU_DEC_MAX] = {AU_DEC_ID1, AU_DEC_ID3 , AU_DEC_INVALID};
...
#endif //End of GPL contect protection
...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_0[] =
{
    ...
    {0x112c70, 0xff, 0xd0}, //[6:5]   Set AUPLL_KP0_OUT = 3840fs clock for I2S
    {0x112c71, 0xff, 0x12}, //[9:7]   Set AUPLL_KP1_OUT = 480fs clock for SRC
    {0x112c76, 0xff, 0x00}, //[7:0]   Set AUPLL power down = 0
    {0x112c77, 0xff, 0x03},

    //----------------AUR2PLL------------------------//Item 22
    {0x160342, 0xff, 0x01}, //
    {0x160343, 0xff, 0x00}, //[8]     Set AUR2PLL power down = 0
    {0x160346, 0xff, 0x24}, //[7:0]   loop_div_second, R2 clock = 0x24 * 24 = 864MHz
    {0xffffff, 0x01, 0x01}, // delay 1m sec

    //----------------AUR2_1PLL------------------------//Item 22
    {0x160362, 0xff, 0x01}, //
    {0x160363, 0xff, 0x00}, //[8]     Set AUR2PLL power down = 0
    {0x160366, 0xff, 0x19}, //[7:0]   loop_div_second, R2 clock = 0x19 * 24 = 600MHz
    {0xffffff, 0x01, 0x01}, // delay 1m sec

    //---------------- DFS ---------------------------//Item 36
    {0x163c61, 0x03, 0x00}, //[9:8]   CLK DSP DFS SEL[1:0] = 0
    ...
    {0x163c63, 0x03, 0x00}, //[9:8]   CLK R2 DFS SEL[1:0] = 0
    ...
    {0x163e04, 0x04, 0x04}, //[1:0]   REG_PasDMA_RIU_W_SEL
    {0x163e04, 0x03, 0x01}, //[2]     REG_ENABLE_CLK_PasDMA_RIU_BRG
    {0x163c65, 0x08, 0x08}, //[11]    CLK R2 WB EN
...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_1[] =
{
    ...
    {0x112c25, 0x80, 0x80},
    {0x112c25, 0x10, 0x00},
    {0x112c24, 0x80, 0x80}, // select from Decode5 from SE
    ...
    {0x112c25, 0x08, 0x08},
    {0x112c25, 0x01, 0x00},
    {0x112c24, 0x40, 0x40}, // select from Decode6 from SE
    ...
    //-----------------AudioBand----------------------------//Item 21
    //{0x112b58, 0xff, 0xff}, //[15:0]  fix value for DWA_out
    //{0x112b59, 0xff, 0x01},
    {0x112b58, 0xff, 0x00}, //[15:0]  fix value for SDM_in
    {0x112b59, 0xff, 0x00},
    //{0x112b55, 0xf0, 0xf0}, //[15:12] DWA_out fix enable
    {0x112b55, 0x08, 0x00}, //[11] SDM_in fix enable
    {0x112b46, 0xff, 0x00}, //[15:0]  Enable Group C
    {0x112b47, 0xff, 0xff},
    {0x112b56, 0xff, 0x30}, //[15:0]  Enable ADC1/2
    {0x112b57, 0xff, 0x00},
    {0x112c6e, 0xff, 0x88}, //[7:0]   Enable Grp A/B
    {0x112b50, 0xff, 0x80}, //[15:0]  DC offset 9L value
    {0x112b51, 0xff, 0x00},
    {0x112b10, 0xff, 0x80}, //[15:0]  DC offset 17L value
    {0x112b11, 0xff, 0x00},
    {0x112b60, 0xff, 0xc0},
    {0x112b61, 0xff, 0x38},
    {0x112b62, 0xff, 0x38},
    ...
    {0x112b41, 0xff, 0x00},
    {0x112b54, 0xc0, 0x80}, //[7:6]  DITHER select 17L 0.5LSB
    {0x112b55, 0x07, 0x01}, //[10:8]  DITHER select 9L 0.5LSB
    {0x112b52, 0xff, 0x0f}, //[3:0]  DC offset ENABLE
    {0x112b53, 0xff, 0xf0}, //[15:12] Dither ENABLE
    ...
    //-----------------De-POP_1-----------------------------//Item 25
    // auto de-pop start
    {0x112cdc, 0xff, 0x30},       // wriu -w 0x112cdc 0x2230
    {0x112cdd, 0xff, 0x22},
    {0x112cde, 0xff, 0x00},  // wriu -w 0x112cde 0x0000
    {0x112cdf, 0xff, 0x00},
    {0x112ce6, 0xff, 0x01},  // wriu -w 0x112ce6 0x0001
    {0x112ce7, 0xff, 0x00},
    {0x112ce8, 0xff, 0x00},  // wriu -w 0x112ce8 0x8000
    {0x112ce9, 0xff, 0x80},
    {0x112cec, 0xff, 0x90},  // wriu -w 0x112cec 0x0090
    {0x112ced, 0xff, 0x00},
    {0x112cee, 0xff, 0x00},  // wriu -w 0x112cee 0x0000
    {0x112cef, 0xff, 0x00},
    {0x112cf6, 0xff, 0x00},  // wriu -w 0x112cf6 0x0000
    {0x112cf7, 0xff, 0x00},
    {0x112cf8, 0xff, 0x00},  // wriu -w 0x112cf8 0x0000
    {0x112ce9, 0xff, 0x00},
    {0x112e70, 0xff, 0x00},  // wriu -w 0x112e70 0x2A00
    {0x112e71, 0xff, 0x2a},
    {0x112e72, 0xff, 0xf0},  // wriu -w 0x112e72 0xF0
    {0x112e73, 0xff, 0x00},
    {0x112ec0, 0xff, 0x0c},  // wriu -w 0x112ec0 0xC
    {0x112ec1, 0xff, 0x00},
    {0x112ec2, 0xff, 0x08}, // wriu -w 0x112ec2 0x8
    {0x112ec3, 0xff, 0x00},
    {0x112ec4, 0xff, 0x08}, // wriu -w 0x112ec4 0x8
    {0x112ec5, 0xff, 0x00},
    {0x112ec6, 0xff, 0x08}, // wriu -w 0x112ec6 0x8
    {0x112ec7, 0xff, 0x00},
    {0x112ec8, 0xff, 0x38}, // wriu -w 0x112ec8 0x38
    {0x112ec9, 0xff, 0x00},
    {0x112eca, 0xff, 0x08}, // wriu -w 0x112eca 0x8
    {0x112ec8, 0xff, 0x00},
    {0x112ecc, 0xff, 0x3a}, // wriu -w 0x112ecc 0x3A
    {0x112ecd, 0xff, 0x00},
    {0x112ece, 0xff, 0x08}, // wriu -w 0x112ece 0x8
    {0x112ecf, 0xff, 0x00},
    {0x112ed0, 0xff, 0x38}, // wriu -w 0x112ed0 0x138
    {0x112ed1, 0xff, 0x01},
    {0x112ed2, 0xff, 0x08}, // wriu -w 0x112ed2 0x8
    {0x112ed3, 0xff, 0x00},
    {0x112ed4, 0xff, 0x38}, // wriu -w 0x112ed4 0x138
    {0x112ed5, 0xff, 0x01},
    {0x112ed6, 0xff, 0x08}, // wriu -w 0x112ed6 0x8
    {0x112ed7, 0xff, 0x00},
    {0x112e68, 0xff, 0x00}, // wriu -w 0x112e68 0x6D00
    {0x112e69, 0xff, 0x6d},
    {0x112e6a, 0xff, 0x01}, // wriu -w 0x112e6a 0x0101
    {0x112e6b, 0xff, 0x01},
    {0x112e6c, 0xff, 0x23}, // wriu -w 0x112e6c 0x1223
    {0x112e6d, 0xff, 0x12},
    {0x112e6e, 0xff, 0x00}, // wriu -w 0x112e6e 0x0000
    {0x112e6f, 0xff, 0x00},
    {0x112ed8, 0xff, 0x09}, // wriu -w 0x112ed8 0x0009
    {0x112ed9, 0xff, 0x00},
    {0x112cdc, 0xff, 0x00}, // wriu -w 0x112cdc 0x0
    {0x112cdd, 0xff, 0x00},
    {0x112cde, 0xff, 0x00}, // wriu -w 0x112cde 0x800
    {0x112cdf, 0xff, 0x08},
    {0x112ce6, 0xff, 0x00}, // wriu -w 0x112ce6 0x0
    {0x112ce7, 0xff, 0x00},
    {0x112ce8, 0xff, 0x00}, // wriu -w 0x112ce8 0x0
    {0x112ce9, 0xff, 0x00},
    {0x112cec, 0xff, 0x00}, // wriu -w 0x112cec 0x0
    {0x112ced, 0xff, 0x00},
    {0x112cee, 0xff, 0x00}, // wriu -w 0x112cee 0x0
    {0x112cef, 0xff, 0x00},
    {0x112cf6, 0xff, 0x84}, // wriu -w 0x112cf6 0x2184
    {0x112cf7, 0xff, 0x21},
    {0x112cf8, 0xff, 0x00}, // wriu -w 0x112cf8 0x0
    {0x112cf9, 0xff, 0x00},
        // auto de-pop end
...
const AUDIO_REG_TYPE  AudioInitTbl_0[]=
{
    ...
    {0x153d19, 0xff, 0xdd}, // [8]HDMI left justified, [9]MSB first, [15:10]HDMI status block start SEL
    ...
    {0x112cae, 0x30, 0x20}, // [5:4] spdif rx synthesizer source SEL 432MHz
    ...
    {0x112cea, 0xff, 0x00},
    {0x112ceb, 0xff, 0x00},
    {0x112cf2, 0xff, 0x20}, //[11]    REG_ADC1_L_INV, [10] REG_ADC1_R_INV
    {0x112cf3, 0xff, 0x0c},

    //-----------------Decode Path--------------------------//Item 28
    {0x112c60, 0xff, 0x80}, // Decoder 1 source from DVB1 @@VVV
    {0x112c61, 0xff, 0x80}, // Decoder 3 source from DVB3 @@VVV
    {0x112c62, 0xff, 0x80}, // Decoder 2 source from DVB2 @@VVV
    {0x112c63, 0xff, 0x87}, // Decoder 4 source from SIF  @@VVV
    ...
    //-----------------Output: DAC--------------------------//Item 12
    //{0x112b55, 0x08, 0x00}, // [11] SDM FIX DISABLE
    ...
    //---------------------------------------------------//Item 30
    {0x112c94, 0x01, 0x01}, // enable CH1 HW force mute, [0] ch1 mute all @@VVV
    {0x112c95, 0x01, 0x01}, // enable CH5 HW force mute, [8] ch5 mute all @@VVV
    {0x112c96, 0x01, 0x01}, // enable CH2 HW force mute, [0] ch2 mute all @@VVV
    {0x112c97, 0x01, 0x01}, // enable CH6 HW force mute, [8] ch6 mute all @@VVV
    {0x112c98, 0x01, 0x01}, // enable CH3 HW force mute, [0] ch3 mute all @@VVV
    {0x112c99, 0x01, 0x01}, // enable CH7 HW force mute, [8] ch7 mute all @@VVV
    {0x112c9a, 0x01, 0x01}, // enable CH4 HW force mute, [0] ch4 mute all @@VVV
    {0x112c9b, 0x01, 0x01}, // enable CH8 HW force mute, [8] ch8 mute all @@VVV

    //-----------------Interrupt--------------------------//Item 35
    //IRQE1n = 0x112AA2[7] ? SPDIF_TX : (0x11304E[4] ? SPDIF_TX2 : HDMI_TX)
    //IRQL1n = 0x112AA2[4] ? SPDIF_TX : (0x11304E[8] ? SPDIF_TX2 : HDMI_TX)
    //IRQ2n  = 0x112AA2[6] ? SPDIF_TX : (0x112AA2[5] ? SPDIF_TX2 : HDMI_TX)
    {0x112aa2, 0x40, 0x40}, //[6]     IRQ2n  select SPDIF_TX
    {0x11304e, 0x10, 0x10}, //[4]     IRQE1n select SPDIF_TX2
    {0x11304f, 0x01, 0x00}, //[8]     IRQL1n select HDMI_TX
    ...
    {0x112aa1, 0x04, 0x00}, //[10]    sedma fifo new mode
    {0x112a4c, 0x20, 0x20}, //[5]     burst mode enable
    {0x112a4d, 0x20, 0x20}, //[13]    burst mode enable
    {0x112acc, 0x20, 0x20}, //[5]     burst mode enable
    {0x112acd, 0x20, 0x20}, //[13]    burst mode enable
    ...
    {0x1129d7, 0x02, 0x02}, //[9]     enable R2 dynamic clock gating
    {0x163040, 0x20, 0x00}, //[5]     enable R2_1 Write buffer burst mode
    {0x1630d7, 0x02, 0x02}, //[9]     enable R2_1 dynamic clock gating
    {0x163e06, 0xff, 0x0f}, //        MIU delay1T setting: have bridge & have mcm = 0x0F0F
    {0x163e07, 0xff, 0x0f}, //        MIU delay1T setting: have bridge & no   mcm = 0x0000
    ...
    //---------------- R2 block clkgen ---------------------------
    {0x113200, 0x0f, 0x01}, //R20: [0]reg_enable_clk_r2
    {0x113202, 0x0f, 0x01}, //R20: [0]reg_enable_clk_r2_dfs
    {0x113300, 0x0f, 0x01}, //R21: [0]reg_enable_clk_r2
    {0x113302, 0x1f, 0x1d}, //R21: [0]reg_enable_clk_r2_dfs, [4:2]reg_clk_r2_dfs_sel = 3'b111 from AUR2_1PLL

//----------------------------------------------------
//MCM off and Adress limiter bypass
//Note: Only use in audio YMY verification, need to be decided by system top
//----------------------------------------------------
#if 1
//SE DMA
    {0x163ca1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163ca8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ca9, 0xff, 0x00}, //[15:8]address low byte
    {0x163caa, 0xff, 0x00}, //[7:0]address high byte
    {0x163cab, 0xff, 0x00}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163ca8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ca9, 0xff, 0xff}, //[15:8]address low byte
    {0x163caa, 0xff, 0xff}, //[7:0]address high byte
    {0x163cab, 0xff, 0xff}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163ca8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ca9, 0xff, 0x00}, //[15:8]address low byte
    {0x163caa, 0xff, 0x00}, //[7:0]address high byte
    {0x163cab, 0xff, 0x00}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163ca8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ca9, 0xff, 0xff}, //[15:8]address low byte
    {0x163caa, 0xff, 0xff}, //[7:0]address high byte
    {0x163cab, 0xff, 0xff}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163ca8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ca9, 0xff, 0x00}, //[15:8]address low byte
    {0x163caa, 0xff, 0x00}, //[7:0]address high byte
    {0x163cab, 0xff, 0x00}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163ca8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ca9, 0xff, 0xff}, //[15:8]address low byte
    {0x163caa, 0xff, 0xff}, //[7:0]address high byte
    {0x163cab, 0xff, 0xff}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163ca8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ca9, 0xff, 0x00}, //[15:8]address low byte
    {0x163caa, 0xff, 0x00}, //[7:0]address high byte
    {0x163cab, 0xff, 0x00}, //[15:8]address high byte
    {0x163ca1, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163ca8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ca9, 0xff, 0xff}, //[15:8]address low byte
    {0x163caa, 0xff, 0xff}, //[7:0]address high byte
    {0x163cab, 0xff, 0xff}, //[15:8]address high byte
    {0x163ca1, 0x90, 0x90}, //[15]address_0 en
//SE cache
    {0x163cb1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163cb8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cb9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cba, 0xff, 0x00}, //[7:0]address high byte
    {0x163cbb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cb1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163cb8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cb9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cba, 0xff, 0xff}, //[7:0]address high byte
    {0x163cbb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cb1, 0x80, 0x80}, //[15]address_0 en
//R2 D
    {0x163cc1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163cc8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cc9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cca, 0xff, 0x00}, //[7:0]address high byte
    {0x163ccb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163cc8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cc9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cca, 0xff, 0xff}, //[7:0]address high byte
    {0x163ccb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163cc8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cc9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cca, 0xff, 0x00}, //[7:0]address high byte
    {0x163ccb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163cc8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cc9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cca, 0xff, 0xff}, //[7:0]address high byte
    {0x163ccb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163cc8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cc9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cca, 0xff, 0x00}, //[7:0]address high byte
    {0x163ccb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163cc8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cc9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cca, 0xff, 0xff}, //[7:0]address high byte
    {0x163ccb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163cc8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cc9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cca, 0xff, 0x00}, //[7:0]address high byte
    {0x163ccb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cc1, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163cc8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cc9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cca, 0xff, 0xff}, //[7:0]address high byte
    {0x163ccb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cc1, 0x90, 0x90}, //[15]address_0 en
//R2 I
    {0x163cd1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163cd8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cd9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cda, 0xff, 0x00}, //[7:0]address high byte
    {0x163cdb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cd1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163cd8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cd9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cda, 0xff, 0xff}, //[7:0]address high byte
    {0x163cdb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cd1, 0x80, 0x80}, //[15]address_0 en
//R2_1 D
    {0x163ce1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163ce8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ce9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cea, 0xff, 0x00}, //[7:0]address high byte
    {0x163ceb, 0xff, 0x00}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163ce8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ce9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cea, 0xff, 0xff}, //[7:0]address high byte
    {0x163ceb, 0xff, 0xff}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163ce8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ce9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cea, 0xff, 0x00}, //[7:0]address high byte
    {0x163ceb, 0xff, 0x00}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163ce8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ce9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cea, 0xff, 0xff}, //[7:0]address high byte
    {0x163ceb, 0xff, 0xff}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163ce8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ce9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cea, 0xff, 0x00}, //[7:0]address high byte
    {0x163ceb, 0xff, 0x00}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163ce8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ce9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cea, 0xff, 0xff}, //[7:0]address high byte
    {0x163ceb, 0xff, 0xff}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163ce8, 0xff, 0x00}, //[7:0]address low byte
    {0x163ce9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cea, 0xff, 0x00}, //[7:0]address high byte
    {0x163ceb, 0xff, 0x00}, //[15:8]address high byte
    {0x163ce1, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163ce8, 0xff, 0xff}, //[7:0]address low byte
    {0x163ce9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cea, 0xff, 0xff}, //[7:0]address high byte
    {0x163ceb, 0xff, 0xff}, //[15:8]address high byte
    {0x163ce1, 0x90, 0x90}, //[15]address_0 en
//R2_1 I
    {0x163cf1, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163cf8, 0xff, 0x00}, //[7:0]address low byte
    {0x163cf9, 0xff, 0x00}, //[15:8]address low byte
    {0x163cfa, 0xff, 0x00}, //[7:0]address high byte
    {0x163cfb, 0xff, 0x00}, //[15:8]address high byte
    {0x163cf1, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163cf8, 0xff, 0xff}, //[7:0]address low byte
    {0x163cf9, 0xff, 0xff}, //[15:8]address low byte
    {0x163cfa, 0xff, 0xff}, //[7:0]address high byte
    {0x163cfb, 0xff, 0xff}, //[15:8]address high byte
    {0x163cf1, 0x80, 0x80}, //[15]address_0 en
//DEC DMA
    {0x163d31, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163d38, 0xff, 0x00}, //[7:0]address low byte
    {0x163d39, 0xff, 0x00}, //[15:8]address low byte
    {0x163d3a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d3b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163d38, 0xff, 0xff}, //[7:0]address low byte
    {0x163d39, 0xff, 0xff}, //[15:8]address low byte
    {0x163d3a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d3b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163d38, 0xff, 0x00}, //[7:0]address low byte
    {0x163d39, 0xff, 0x00}, //[15:8]address low byte
    {0x163d3a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d3b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163d38, 0xff, 0xff}, //[7:0]address low byte
    {0x163d39, 0xff, 0xff}, //[15:8]address low byte
    {0x163d3a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d3b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163d38, 0xff, 0x00}, //[7:0]address low byte
    {0x163d39, 0xff, 0x00}, //[15:8]address low byte
    {0x163d3a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d3b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163d38, 0xff, 0xff}, //[7:0]address low byte
    {0x163d39, 0xff, 0xff}, //[15:8]address low byte
    {0x163d3a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d3b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163d38, 0xff, 0x00}, //[7:0]address low byte
    {0x163d39, 0xff, 0x00}, //[15:8]address low byte
    {0x163d3a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d3b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d31, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163d38, 0xff, 0xff}, //[7:0]address low byte
    {0x163d39, 0xff, 0xff}, //[15:8]address low byte
    {0x163d3a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d3b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d31, 0x90, 0x90}, //[15]address_0 en
//R2 BDMA
    {0x163d41, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163d48, 0xff, 0x00}, //[7:0]address low byte
    {0x163d49, 0xff, 0x00}, //[15:8]address low byte
    {0x163d4a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d4b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163d48, 0xff, 0xff}, //[7:0]address low byte
    {0x163d49, 0xff, 0xff}, //[15:8]address low byte
    {0x163d4a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d4b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163d48, 0xff, 0x00}, //[7:0]address low byte
    {0x163d49, 0xff, 0x00}, //[15:8]address low byte
    {0x163d4a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d4b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163d48, 0xff, 0xff}, //[7:0]address low byte
    {0x163d49, 0xff, 0xff}, //[15:8]address low byte
    {0x163d4a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d4b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163d48, 0xff, 0x00}, //[7:0]address low byte
    {0x163d49, 0xff, 0x00}, //[15:8]address low byte
    {0x163d4a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d4b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163d48, 0xff, 0xff}, //[7:0]address low byte
    {0x163d49, 0xff, 0xff}, //[15:8]address low byte
    {0x163d4a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d4b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163d48, 0xff, 0x00}, //[7:0]address low byte
    {0x163d49, 0xff, 0x00}, //[15:8]address low byte
    {0x163d4a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d4b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d41, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163d48, 0xff, 0xff}, //[7:0]address low byte
    {0x163d49, 0xff, 0xff}, //[15:8]address low byte
    {0x163d4a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d4b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d41, 0x90, 0x90}, //[15]address_0 en
//R2_1 BDMA
    {0x163d51, 0x0f, 0x01}, //[11:8]lower bond address
    {0x163d58, 0xff, 0x00}, //[7:0]address low byte
    {0x163d59, 0xff, 0x00}, //[15:8]address low byte
    {0x163d5a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d5b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x02}, //[11:8]upper bond address
    {0x163d58, 0xff, 0xff}, //[7:0]address low byte
    {0x163d59, 0xff, 0xff}, //[15:8]address low byte
    {0x163d5a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d5b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x03}, //[11:8]lower bond address
    {0x163d58, 0xff, 0x00}, //[7:0]address low byte
    {0x163d59, 0xff, 0x00}, //[15:8]address low byte
    {0x163d5a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d5b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x04}, //[11:8]upper bond address
    {0x163d58, 0xff, 0xff}, //[7:0]address low byte
    {0x163d59, 0xff, 0xff}, //[15:8]address low byte
    {0x163d5a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d5b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x05}, //[11:8]lower bond address
    {0x163d58, 0xff, 0x00}, //[7:0]address low byte
    {0x163d59, 0xff, 0x00}, //[15:8]address low byte
    {0x163d5a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d5b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x06}, //[11:8]upper bond address
    {0x163d58, 0xff, 0xff}, //[7:0]address low byte
    {0x163d59, 0xff, 0xff}, //[15:8]address low byte
    {0x163d5a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d5b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x07}, //[11:8]lower bond address
    {0x163d58, 0xff, 0x00}, //[7:0]address low byte
    {0x163d59, 0xff, 0x00}, //[15:8]address low byte
    {0x163d5a, 0xff, 0x00}, //[7:0]address high byte
    {0x163d5b, 0xff, 0x00}, //[15:8]address high byte
    {0x163d51, 0x0f, 0x08}, //[11:8]upper bond address
    {0x163d58, 0xff, 0xff}, //[7:0]address low byte
    {0x163d59, 0xff, 0xff}, //[15:8]address low byte
    {0x163d5a, 0xff, 0xff}, //[7:0]address high byte
    {0x163d5b, 0xff, 0xff}, //[15:8]address high byte
    {0x163d51, 0x90, 0x90}, //[15]address_0 en
#endif

    //-----------------DSP IDMA MCM ACK ---------------------------//Item 99
    {0x112a58, 0x02, 0x00}, //[1]     MCM old mode
    {0x112ad8, 0x02, 0x00}, //[1]     MCM old mode
...
const AUDIO_REG_TYPE  AudioInitTbl_1[]=
{
    ...
    {0x112C58, 0x06, 0x06},    // enable decimation mode
    {0xFFFFFF, 0x00, 0x00},    // end of table
};
...
const AUDIO_REG_TYPE2 ResetDMIOTbl[]=
{
    ...
    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa17a},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa17e},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa182},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa186},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa18a},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa176},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa1e0},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},

    {0x112a80 , 0xFFFF, 0x0053},
    {0x112a80 , 0xFFFF, 0x0043},
    {0x112a84 , 0xFFFF, 0xa1e8},
    {0x112a82 , 0xFFFF, 0x0000},
    {0x112a82 , 0xFFFF, 0x0080},
...
static const MS_U16 g_u16VolumeArray[AUDIO_VOLUME_ARRAY_MAXNUM] =
{
    0x7F00, // 00
...
#if AUDIO_MCH_SW_DMA_READER_SUPPORT
AUDIO_PCM_t Audio_Pcm_MCH_SwDma_Reader = {
    .u8Name = AUDIO_MCH_SW_DMA_READER,
    .tPcmOps = {
        .open = HAL_AUDIO_PCM_MCH_SwDma_Reader_Open,
        .close = HAL_AUDIO_PCM_MCH_SwDma_Reader_Close,
        .start = HAL_AUDIO_PCM_MCH_SwDma_Reader_Start,
        .stop = HAL_AUDIO_PCM_MCH_SwDma_Reader_Stop,
        .set = HAL_AUDIO_PCM_MCH_SwDma_Reader_Set,
        .get = HAL_AUDIO_PCM_MCH_SwDma_Reader_Get,
        .read = NULL,
        .write = HAL_AUDIO_PCM_MCH_SwDma_Reader_Write,
        .flush = HAL_AUDIO_PCM_MCH_SwDma_Reader_Flush,
    },
    .tPcmCaps = {
        .u8MultiChFlag = TRUE,
        .u8MixingFlag = FALSE,
...
#if AUDIO_MP3_ENCODER1_SUPPORT
static MS_U32 g_u32Mp3Encoder1RptrOffset = 0;
#endif

#if AUDIO_MP3_ENCODER2_SUPPORT
static MS_U32 g_u32Mp3Encoder2RptrOffset = 0;
#endif
...
MS_BOOL HAL_AUDIO_HDMI_RX_GetHdmiInAudioStatus( ST_HDMI_AUDIO_STATUS *p_hdmiAudioSts)
{
    ...
    if (HAL_AUDIO_HDMI_NonpcmMonitor())
    {
        ...
        {
            case AU_HDMI_AC3:           p_hdmiAudioSts->audio_type = HDMI_RX_DD;        break;
            case AU_HDMI_AAC:           p_hdmiAudioSts->audio_type = HDMI_RX_DP;        break;
            case AU_HDMI_DTS_TYPE1:     p_hdmiAudioSts->audio_type = HDMI_RX_DTS;       break;
            case AU_HDMI_DTS_TYPE4:     p_hdmiAudioSts->audio_type = HDMI_RX_DTS;       break;
            case AU_HDMI_AC3P:          p_hdmiAudioSts->audio_type = HDMI_RX_DDP;       break;
...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_eSpdifMode = SPDIF_OUT_NONE;
    ...
    for ( i = 0; i < AU_DEC_MAX; i++ )
    {
        pAUDIOShared->DspUsage[i].bDecInUsed = FALSE;
        pAUDIOShared->DspUsage[i].eAudioType = Audio_DEC_NULL;
        pAUDIOShared->DspUsage[i].eUsageType = AUDIO_USAGE_TYPE_NONE;
    }
    ...
    pAUDIOShared->g_AudioOutputDeviceSelection.u32StructVersion = AUDIO_OUTPUT_DEVICE_SELECTION_VERSION;
    pAUDIOShared->g_AudioOutputDeviceSelection.u32StructSize = sizeof(AUDIO_OUTPUT_DEVICE_SELECTION_t);
    ...
    pAUDIOShared->g_EdidOutViaArc = FALSE;
    pAUDIOShared->g_EdidOutputInterfaceType = DIGITAL_HDMI_OUTPUT;
    //pAUDIOShared->AudioSrcType = AUDIO_DSP1_DVB_INPUT;
    //pAUDIOShared->AudioSubSrcType = AUDIO_DSP1_DVB_INPUT;

    pAUDIOShared->g_HDMIRxHBRFlag = FALSE; //HDMI Rx HBR Flag
...
{
     return AU_CHIP_KERES;  //Need to refine
}
...
{
    HAL_AUDIO_AbsWriteMaskByte(0x112B55,0x10,0x00);  	// disable DWA fix value
    return;
}
...
{
    HAL_AUDIO_DecWriteByte(REG_SE_IDMA_CTRL0, 0x82);     // Reset DSP
    AUDIO_DELAY1MS(2);
    HAL_AUDIO_DecWriteByte(REG_SE_IDMA_CTRL0, 0x83);
}
...
void HAL_AUDIO_SetInputPath(AUDIO_INPUT_TYPE input , AUDIO_PATH_TYPE u8Path)
{
    ...
    switch(u8input_src)
    {
        case AUDIO_DSP1_INPUT:
            ...
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_DECODER1_CFG, 0x07,u8input_idx);
         break;
        ...
        case AUDIO_ADC_INPUT:
            ...
            HAL_AUDIO_WriteMaskByte(0x2CE3, 0x03, 0x00 ); //disable power down INMUX
            break;
...
void HAL_AUDIO_SetInternalPath(AUDIO_INTERNAL_PATH_TYPE u8Path,  AUDIO_OUTPUT_TYPE u8Output)
{
    ...
    MS_U8   BalanceBitMap[10]={   0,       1,      2,         3,         5,        4,     1,      7,         7,         10        };
...
void HAL_AUDIO_BT_SetBufferCounter(MS_U32 u32Counter)
{
    ...
    HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_btFrameSize, u32Counter);
}
...
static void HAL_AUDIO_DigitalOut_SetDataPath(DIGITAL_OUTPUT_TYPE ePath, DigitalOutSetting_t *stDigitalOutSetting)
{
    ...
    if (ret == FALSE)
    {
        HALAUDIO_ERROR("%s()- GetChannelStatus return FALSE!!\n",__FUNCTION__);
        return;
    }
    ...
    memcpy(&(g_AudioVars2->stDigitalOutChannelStatus[ePath]), &stDigitalOutSetting, sizeof(DigitalOutSetting_t));
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
AUDIO_SPDIF_OUTPUT_TYPE HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    // Multiple npcm source, still need total rework
    Digital_Out_Channel_Status_t stDigitalChannelStatusTx1;
    Digital_Out_Channel_Status_t stDigitalChannelStatusTx2;
    AUDIO_SOURCE_INFO_TYPE eAudioSource = E_AUDIO_INFO_DTV_IN;
    AUDIO_DSP_CODE_TYPE MainDspCodeType = MSAPI_AUD_DVB_INVALID;
    AUDIO_DSP_CODE_TYPE Dsp1CodeType = MSAPI_AUD_DVB_INVALID;
    AUDIO_DSP_CODE_TYPE Dsp2CodeType = MSAPI_AUD_DVB_INVALID;
    MS_BOOL retTx1 = FALSE;
    MS_BOOL retTx2 = FALSE;
    MS_U8 NonPcmPath = ADEC1; /* R2: ADEC1, ADEC2 */
    ...
    static MS_BOOL bPreDDPoutbyARC = FALSE;
    ...
    NonPcmPath = ADEC1;
    //HALAUDIO_ERROR("%s: u8Spdif_mode = %d, eAudioSource = %d, u8Dsp1CodeType = %d, u8Dsp2CodeType = %d, u8Input_src = %d\r\n", __FUNCTION__, u8Spdif_mode, eAudioSource, u8Dsp1CodeType, u8Dsp2CodeType, u8Input_src);
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_HDMI_IN:
        {
            ...
            {
                MainDspCodeType = Dsp1CodeType;
                NonPcmPath = ADEC1;   //DEC1
            }

            if (Dsp1CodeType == AU_DVB_STANDARD_DTS)
            {
                u32bDTSCD = HAL_MAD_GetDTSInfo(Audio_DTS_infoType_CD_MODE);
            }
    ...
    if (g_AudioVars2->g_EdidOutViaArc == FALSE)
    {
        if (retTx2 == FALSE)
        {
            HALAUDIO_ERROR("%s() : Failed to get channel status !!!\n", __FUNCTION__);
    ...
    switch (u8SpdifMode)
    {
        ...
        case SPDIF_OUT_TRANSCODE:
        {
            ...
            HAL_AUR2_WriteByte(REG_R2_HDMI_NONPCM_PATH, NonPcmPath);
            AUDIO_DELAY1MS(5);
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF_OUT_CFG, 0x07, 0x02); /* Grp C in */
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF_OUT_CFG+1, 0x01, 0x00);
            ...
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_NEW_MODE_CFG, 0x80, 0x80);                   // enable synthesizer
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_CLK_CFG6+1, 0x04, 0x04);                        //sel HDMI TX clock from synthesizer
            }
            AUDIO_DELAY1MS(5);
        ...
        default:
        {
            ...
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_CLK_CFG6+1, 0x04, 0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_NEW_MODE_CFG, 0x80, 0x00);                 // disable synthesizer
            }
            break;
        }
    }
    ...
    g_AudioVars2->g_eSpdifMode = u8SpdifMode;
...
{
    return g_AudioVars2->g_eSpdifMode;
}
...
{
    //HAL_AUDIO_Set_DecimationMode(AUDIO_HDMI_OUTPUT, g_AudioVars2->g_HDMINonPcmPath);
    return;
}
...
{
    /* check current status according to HDMI Tx Software mute bit */
    MS_BOOL bHdmiTxMuteStatus = FALSE;

    bHdmiTxMuteStatus = (((HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL+1) & 0x80) > 0) ? TRUE : FALSE);

    /* apply change only when it's a different status from current status */
    if (bHdmiTxMuteStatus != bEnMute)
    {
        if(bEnMute == 1)
        {
            HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL+1, 0x80, 0x80);        // Software Mute
            HAL_AUDIO_WriteMaskByte(REG_SOUND_HDMI_TX_VOL_FRAC+1, 0x80, 0x80);     //Fading-out mute
        }
        else
        {
            HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL+1, 0x80, 0x00);        // Software unMute
            MsOS_DelayTask(30);
            HAL_AUDIO_WriteMaskByte(REG_SOUND_HDMI_TX_VOL_FRAC+1, 0x80, 0x00);     //Fading-in unmute
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_GET_HDMI_RX_FrameRate()
/// @brief \b Function \b Description: Get HDMI Rx Frame Rate
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b MS_U32  : return Hdmi Rx Frame Rate
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_U32 HAL_AUDIO_GET_HDMI_RX_FrameRate(void)
{
    MS_U32 u32HdmiFrameRate = 48000;
    MS_U32 u32HdmiSynth = 0x1194;

    // Get HDMI Rx Frame Rate
    u32HdmiSynth = HAL_AUDIO_AbsReadReg(SYNTH_HDMI_N_F_Status);
    switch(u32HdmiSynth)
    {
        case 0x1A5D:
        case 0x1A5E:
        case 0x1A5F:
            u32HdmiFrameRate = 32000;
            break;

        case 0x1320:
        case 0x1321:
        case 0x1322:
            u32HdmiFrameRate = 44100;
            break;

        case 0x1193:
        case 0x1194:
        case 0x1195:
            u32HdmiFrameRate = 48000;
            break;

        case 0x0D2E:
        case 0x0D2F:
        case 0x0D30:
            u32HdmiFrameRate = 64000;
            break;

        case 0x098F:
        case 0x0990:
        case 0x0991:
            u32HdmiFrameRate = 88200;
            break;

        case 0x08C9:
        case 0x08CA:
        case 0x08CB:
            u32HdmiFrameRate = 96000;
            break;

        case 0x0696:
        case 0x0697:
        case 0x0698:
            u32HdmiFrameRate = 128000;
            break;

        case 0x04C7:
        case 0x04C8:
        case 0x04C9:
            u32HdmiFrameRate = 176400;
            break;

        case 0x0464:
        case 0x0465:
        case 0x0466:
            u32HdmiFrameRate = 192000;
            break;
    ...
    return u32HdmiFrameRate;
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_HDMI_HBRMonitor()
/// @brief \b Function \b Description: HDMI Rx High Bit Rate
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_HDMI_RX_HBRMonitor(void)
{
    MS_BOOL bHBRMode = 0;
    MS_U8   u8HdmiRatio = 1;
    MS_U8   u8MainDecodeId = 0;
    MS_U32  u32R2SamplingRate = 48000;
    MS_U32  u32HdmiFrameRate = 48000;
    ...
    bHBRMode = HAL_AUDIO_HDMI_Get_RxHBRFlag();
    if (bHBRMode)
    {// for HDMI Rx multi-channel NPCM decode.
        HAL_AUDIO_WriteMaskReg(REG_AUDIO_HDMI_MATRIX1, 0x8000, 0x8000);
    }
    else
    {
        HAL_AUDIO_WriteMaskReg(REG_AUDIO_HDMI_MATRIX1, 0x8000, 0x0000);
    }

    // Get r2 decoder sampling rate
    u32R2SamplingRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, u8MainDecodeId);

    // Get HDMI Rx frame rate
    u32HdmiFrameRate  = HAL_AUDIO_GET_HDMI_RX_FrameRate();

    // DTS type 4 condition
    if (HAL_AUDIO_HDMI_DolbyMonitor() == AU_HDMI_DTS_TYPE4)
    {
        u8HdmiRatio = (MS_U8)(u32HdmiFrameRate/u32R2SamplingRate);
        if (u8HdmiRatio == 4)
        {
            HAL_MAD_SetAC3PInfo(Audio_AC3P_infoType_BYTESWAP, 1, 0);
        }
        else if (u8HdmiRatio == 1)
        {
            HAL_MAD_SetAC3PInfo(Audio_AC3P_infoType_BYTESWAP, 0, 0);
        }
        else
        { // Ratio = others
            DBG_AUDIO_ERROR(printf(" %s fail !!! Not Support u8HdmiRatio = %d\r\n", __FUNCTION__, u8HdmiRatio));
            HAL_MAD_SetAC3PInfo(Audio_AC3P_infoType_BYTESWAP, 0, 0);
        }
    }
}
...
{
    MS_BOOL bHdmiNonPcmMode = FALSE;
    MS_U8 u8TargetPcmRenderControl = 0; //0: decoder mode, 1: multi-channel mode, 2: stereo mode
    MS_U8 u8CurrentPcmRenderControl = 0; //0: decoder mode, 1: multi-channel mode, 2: stereo mode
    ...
    {
        u8TargetPcmRenderControl = 0; //decoder mode
        bHdmiNonPcmMode = TRUE;
    }
    else
    {
        u8TargetPcmRenderControl = 2; //stereo mode
        bHdmiNonPcmMode = FALSE;
    }

    /* if current main input source is HDMI, then configure R2's PCM Render */
    if (g_AudioVars2->eAudioSource == E_AUDIO_INFO_HDMI_IN)
    {
        u8CurrentPcmRenderControl = (HAL_AUR2_ReadByte(REG_SNDR2_MBOX_BYTE_DAP_SEL) & 0x06) >> 1;
        if (u8TargetPcmRenderControl != u8CurrentPcmRenderControl)
        {
            HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL, 0x06, (u8TargetPcmRenderControl << 1));
        }
    }

    HAL_AUDIO_HDMI_RX_HBRMonitor();

    return bHdmiNonPcmMode;
}
...
MS_U8 HAL_AUDIO_HDMI_NonpcmMonitor2(void)
{
    ...
    HDMI_lock_status_tmp = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_hdmi_npcm_lock);
        ...
        switch(u8DecoderType)
        {
            ...
            case AU_HDMI_AAC:
                if (HAL_MAD_GetAC3PInfo(Audio_AC3P_infoType_hdmiAC3inSE) == 1)
                    ...
                    HAL_AUDIO_SetSystem( MSAPI_AUD_DVB_AAC);
                break;
        };

        if (u8DecoderType == AU_HDMI_DTS_TYPE4)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_DOWN_SAMPLE, 0x20, 0x20); // bypass HDMI nonpcm decoder
        }
        else
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_DOWN_SAMPLE, 0x20, 0x00);
        }
...
{
        *onOff = TRUE;
        *hdmi_SmpFreq = (AUDIO_FS_TYPE)(HAL_MAD_GetCommInfo(Audio_Comm_infoType_hdmiTx_outFreq));
        *outType = (HDMI_TX_OUTPUT_TYPE)(HAL_MAD_GetCommInfo(Audio_Comm_infoType_hdmiTx_outType));
}
...
void _HAL_AUDIO_InitStep1(void)
{
    ...
    HAL_AUDIO_WriteByte(REG_SE_IDMA_CTRL0, 0x0082);
    AUDIO_DELAY1MS(2);
    HAL_AUDIO_WriteByte(REG_SE_IDMA_CTRL0, 0x0083);
...
{
    MS_U32 audio_pcm_dmaRdr_bufSz = HW_DMA_RDR1_BUF_SIZE;  //must be multiple of DMA_RDR_PCM_BUF_UNIT*2 = 0x2000
...
MS_BOOL HAL_AUDIO_SetSystem(En_DVB_decSystemType enDecSystem)
{
    ...
    AUDIO_DEC_ID eDecId = AU_DEC_ID1;
    ...
    switch (enDecSystem)
    {
            ...
            //      Audio Decoder 1
            ////////////////////////////////////////////////
            case MSAPI_AUD_DVB_OPUS:
                HAL_AUR2_WriteByte(REG_R2_DECODE1_TYPE, adec_type_opus);
                HAL_AUDIO_SetDspCodeType(AU_DVB_STANDARD_OPUS, TRUE);
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_OPUS;
                break;

            case MSAPI_AUD_DVB_AC4:
                HAL_AUR2_WriteByte(REG_R2_DECODE1_TYPE, adec_type_ac4);
                HAL_AUDIO_SetDspCodeType(AU_DVB_STANDARD_AC4, TRUE);
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_AC4;
                break;
            ...
            case MSAPI_AUD_DVB_MPEG_H:
                HAL_AUR2_WriteByte(REG_R2_DECODE1_TYPE, adec_type_mpegH);
                HAL_AUDIO_SetDspCodeType(AU_DVB_STANDARD_MPEG_H, TRUE);
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_MPEG_H;
                break;
            ...
            case MSAPI_AUD_DVB_AC3P:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_AC3P;
                break;
            ...
            case MSAPI_AUD_DVB_MP3:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_MP3;
                break;
            ...
            case MSAPI_AUD_DVB_XPCM:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_XPCM;
                break;
            ...
            case MSAPI_AUD_DVB_RA8LBR:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_RA8LBR;
                break;
            ...
            case MSAPI_AUD_DVB_WMA:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_WMA;
                break;
            ...
            case MSAPI_AUD_DVB_DTS:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_DTS;
                break;
            ...
            case MSAPI_AUD_DVB_DTSLBR:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_DTSLBR;
                break;
            ...
            case MSAPI_AUD_DVB_WMA_PRO:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_WMA_PRO;
                break;
            ...
            case MSAPI_AUD_DVB_FLAC:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_FLAC;
                break;
            ...
            case MSAPI_AUD_DVB_VORBIS:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_VORBIS;
                break;

            case MSAPI_AUD_DVB_AMR_NB:
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_AMR_NB;
                break;

            case MSAPI_AUD_DVB_AMR_WB:
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_AMR_WB;
                break;
            ...
            case MSAPI_AUD_DVB_DRA:
                ...
                eDecId = AU_DEC_ID1;
                eDecSystemType = MSAPI_AUD_DVB_DRA;
                break;
            ...
            //      Audio Decoder 2
            ////////////////////////////////////////////////
            case MSAPI_AUD_DVB2_AC4:
                HAL_AUR2_WriteByte(REG_R2_DECODE2_TYPE, adec_type_ac4);
                HAL_AUDIO_SetDsp2CodeType(AU_DVB_STANDARD_AC4, TRUE);
                eDecId = AU_DEC_ID3;
                eDecSystemType = MSAPI_AUD_DVB_AC4;
                break;
            ...
            case MSAPI_AUD_DVB2_MPEG_H:
                HAL_AUR2_WriteByte(REG_R2_DECODE2_TYPE, adec_type_mpegH);
                HAL_AUDIO_SetDsp2CodeType(AU_DVB_STANDARD_MPEG_H, TRUE);
                eDecId = AU_DEC_ID3;
                eDecSystemType = MSAPI_AUD_DVB_MPEG_H;
                break;
    ...
    if (bRet == TRUE)
    {
        g_AudioVars2->AudioDecStatus[eDecId].eAudFormat = eDecSystemType;
    }
...
MS_BOOL HAL_AUDIO_AlgReloadCode(AUDIO_DSP_CODE_TYPE dspCodeType)
{
    ...
    {
        HAL_MAD_SetMcuCmd(MBOX_DBGCMD_RELOAD_DTV2_END/0x100);
    }
    else if (IS_AUDIO_DSP_CODE_TYPE_SE_ENC(dspCodeType))
    {
        HAL_MAD_SetMcuCmd(MBOX_DBGCMD_RELOAD_DTV1_END/0x100);
    }
    else
    {
        HAL_MAD_SetMcuCmd(MBOX_DBGCMD_RELOAD_DTV1_END/0x100);
    ...
    if(time_out>=1000)
    {
        DBG_AUDIO_ERROR("  DSP Re-active1: %d\r\n",time_out);
        return FALSE;
    }
    ...
    bRet = HAL_AUDSP_DspLoadCode(dspCodeType);
    if ( bRet == FALSE )
    {
        DBG_AUDIO_ERROR("  DSP reload fail !!\r\n");
        // PIO[8] interrupt
        HAL_MAD_SetPIOCmd(0xEE);        // just not enter DSP MHEG5_isr
        HAL_MAD_TriggerPIO8();
    ...
    if(time_out>=1500)
    {
        DBG_AUDIO_ERROR("  DSP Re-active2: %d\r\n",time_out);
        return FALSE;
    }
...
#if 0
{
    ...
    {
        //No Decoder in DSP
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Warning! un-support DSP version decoder. \r\n", __FUNCTION__);
    ...
    if (time_out>=2000)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "  DSP Reload timeOut1: %d\r\n", time_out);
        return FALSE;
    }
    ...
    if (time_out>=3000)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "  DSP Reload timeOut2: %d\r\n", time_out);
        return FALSE;
    }

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_AUDIO_SeReloadCode finish\r\n");
...
MS_BOOL HAL_AUDIO_SeSystemLoadCode(void)
{
    ...
    //Wait R2 boot code init finished Ack
    #ifndef CONFIG_MBOOT
        while(time_out++<100)
        {
            if((HAL_AUR2_ReadReg(REG_DECR2_BOOTCODE_DONE_ACK) == 0x1234)&&(HAL_AUR2_ReadReg(REG_SNDR2_BOOTCODE_DONE_ACK) == 0x1234))
                break;
            AUDIO_DELAY1MS(2);
        }
    #else
        while(time_out++<100)
        {
            if(HAL_AUR2_ReadReg(REG_DECR2_BOOTCODE_DONE_ACK) == 0x1234)
                break;
            AUDIO_DELAY1MS(2);
        }
    #endif
    ...
    if(time_out>=100)
    {
        AUD_ERRMSG("audio R2 boot code timeout\n");
    }
    else
    {
        AUD_DBGMSG("audio R2 boot code success\n");
    }

    HAL_SND_R2_EnableR2(FALSE);               // stop SND-R2 after load R2 boot code
    HAL_DEC_R2_EnableR2(FALSE);               // stop DEC-R2 after load R2 boot code
    HAL_AUR2_WriteMaskReg(REG_SDR_SWITCH_CTRL, 0x0001, 0x0001);   //use SDR2 address  [1]:0x1630 [0];0x1129
    ...
    HAL_AUR2_WriteReg(REG_DECR2_DCMEM_BASE_HI, (MS_U16)((DEC_R2_ADDR >> 16) & 0xFFFF));
    #ifndef CONFIG_MBOOT
        HAL_AUR2_WriteMaskReg(REG_SDR_SWITCH_CTRL, 0x0002, 0x0002);   //use SDR2 address  [1]:0x1630 [0];0x1129
        ...
        HAL_SND_R2_EnableR2(TRUE);               // start SND-R2 after load R2 code
    #endif
    HAL_DEC_R2_init_SHM_param();
    HAL_DEC_R2_EnableR2(TRUE);               // start DEC-R2 after load R2 code
#endif

    time_out = 0;
    //Wait Dsp/R2 init finished Ack
    #ifndef CONFIG_MBOOT
        while(time_out++<100)
        {
            if(HAL_AUR2_ReadByte(REG_DECR2_ACK1) == 0xE3)
                break;
            AUDIO_DELAY1MS(2);
        }
    #else
    ...
    if(time_out>=100)
    {
        AUD_ERRMSG("DSP2 or R2 Re-Active\n");
    }
    else
    {
...
////////////////////////////////////////////////////////////////////////////////
HDMI_TX_OUTPUT_TYPE HAL_AUDIO_HDMI_TX_SetMode(HDMI_TX_OUTPUT_TYPE outType, AUDIO_SOURCE_INFO_TYPE eSource)
{
    HDMI_TX_OUTPUT_TYPE outType_tmp = outType;
    AUDIO_DSP_CODE_TYPE Dsp1CodeType = AU_DVB_STANDARD_INVALID;
    AUDIO_DSP_CODE_TYPE Dsp2CodeType = AU_DVB_STANDARD_INVALID;
    AUDIO_DSP_CODE_TYPE MainDspCodeType = AU_DVB_STANDARD_INVALID;
    MS_U8 u8NonPcmPath = ADEC1; /* R2: ADEC1, ADEC2 */

    HALAUDIO_CHECK_SHM_INIT;

    DBG_AUDIO(printf("=== HAL_AUDIO_HDMI_SetMode: %d, src:%d ===\n", outType, eSource));
    ...
    {
        HALAUDIO_ERROR("%s: g_AudioVars2 should not be NULL !!\r\n", __FUNCTION__);
        return outType_tmp;
    }
    ...
    switch (eSource)
    {
        ...
        {
            MainDspCodeType = Dsp1CodeType;
            u8NonPcmPath = ADEC1; /* R2 */

            if((MainDspCodeType == AU_DVB_STANDARD_AAC)||(MainDspCodeType == AU_DVB_STANDARD_MS10_DDT))
            {
                if((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, ADEC1)&0xFFFF) < 32000)    // check if less than 32KHz
                    outType_tmp = HDMI_OUT_PCM;
            }
            else if ((MainDspCodeType != AU_DVB_STANDARD_AC3)  &&
                (MainDspCodeType != AU_DVB_STANDARD_AC3P) &&
                (MainDspCodeType != AU_DVB_STANDARD_DTS) &&
        ...
        case E_AUDIO_INFO_HDMI_IN:
        {
            ...
            if((HAL_AUDIO_HDMI_NonpcmMonitor() == FALSE) ||(outType==HDMI_OUT_PCM))   /* if Type is not Dolby type, bypass */
            {
                u8NonPcmPath = ADEC1; /* R2 */
                outType_tmp = HDMI_OUT_PCM;
            }
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            if(HAL_MAD_GetDecCmd(DVB_Audio_Decoder2) == AU_DVB_DECCMD_PLAYFRAME_GS)
            {
                MainDspCodeType = Dsp2CodeType;
                u8NonPcmPath = ADEC2; /* R2 */
            }
            ...
            {
                if ((g_AudioVars2->g_EdidOutViaArc == TRUE) &&
                (MainDspCodeType != AU_DVB_STANDARD_DTSHD_ADO) &&
                (MainDspCodeType != AU_DVB_STANDARD_DolbyTrueHD_Bypass))
                    {
                        outType_tmp = HDMI_OUT_PCM;
                        u8NonPcmPath = ADEC1; /* R2 */
                    }
            ...
            else
            {
                if((MainDspCodeType == AU_DVB_STANDARD_AAC)||(MainDspCodeType == AU_DVB_STANDARD_MS10_DDT))
                {
                    ...
                    {
                        if((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, ADEC2)&0xFFFF) < 32000)    // check if less than 32KHz
                        {
                            outType_tmp = HDMI_OUT_PCM;
                            u8NonPcmPath = ADEC1; /* R2 */
                        }
    ...
    if (outType_tmp == HDMI_OUT_NONPCM)
    {
        /* Non-PCM in R2 */
        HAL_AUR2_WriteByte(REG_R2_HDMI_NONPCM_PATH, u8NonPcmPath);
        AUDIO_DELAY1MS(5);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, ADEC1, 2, 0); /* Non-PCM */
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, ADEC2, 2, 0); /* Non-PCM */
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL, 0x0C, 0x00);               /* Select R2 */
        HAL_AUDIO_WriteMaskByte((REG_M2D_MAILBOX_SPDIF_CTRL+1), 0x20, 0x20);         /* NONPCM Mode */
        if (g_AudioVars2->g_EdidOutViaArc == TRUE)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF2_OUT_CFG, 0x07, 0x02); /* Grp C in */
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF2_OUT_CFG+1, 0x01, 0x00);
    ...
    {
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, ADEC1, 1, 0);                /* PCM Mode */
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, ADEC2, 1, 0);                /* PCM Mode */
        HAL_AUDIO_WriteMaskByte((REG_M2D_MAILBOX_SPDIF_CTRL+1), 0x20, 0x00);         /* PCM Mode */
        if (g_AudioVars2->g_EdidOutViaArc == TRUE)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF2_OUT_CFG, 0x07, 0x00); /* Grp C in */
            stDigitalChannelStatusTx2.u8PcmFormatControl = SPDIF_CS_FORMAT_PCM;
            HAL_AUDIO_DigitalOut_SetChannelStatus(DIGITAL_HDMI_ARC_OUTPUT, &stDigitalChannelStatusTx2);
        }
        else
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_CLK_CFG6+1, 0x04, 0x00);
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_NEW_MODE_CFG, 0x80, 0x00);
        }
    }

    //HAL_AUDIO_Set_DecimationMode(AUDIO_HDMI_OUTPUT, u8NonPcmPath);
    g_AudioVars2->g_HDMINonPcmPath = u8NonPcmPath;
...
MS_BOOL HAL_AUDIO_SetDataCaptureSource(AUDIO_DEVICE_TYPE eID, AUDIO_CAPTURE_SOURCE_TYPE eSource)
{
    ...
    switch(eSource)
    {
        ...
        case E_CAPTURE_ADC1_AUMIC_INPUT:
        {
            ...
            HAL_AUDIO_WriteMaskByte(0x2CE2, 0xE0, (u8temp<<5));
            HAL_AUDIO_WriteByte(u32ControlAddress, M2S_MBOX_GET_ADC1);
            break;
        }
        ...
        case E_CAPTURE_ADC2_AUMIC_INPUT:
        {
			...
			HAL_AUDIO_WriteMaskByte(0x2CE2, 0x1C, (u8temp<<2) );
            HAL_AUDIO_WriteByte(u32ControlAddress, M2S_MBOX_GET_ADC2);
            break;
         }
...
MS_BOOL _AudioReloadCodePreProcess(AUDIO_DSP_ID eDspId)
{
    return TRUE;
}
...
AUDIO_DEC_ID AU_GetDecID(AUDIO_DEC_ID *DecPriority, AudioDecStatus_t * p_AudioDecStatus)
{
    ...
    {
        u8Index = AU_DEC_MAX - 1;
    }

    do
    {
        DecID = DecPriority[u8Index];
...
AUDIO_DEC_ID HAL_AUDIO_OpenDecodeSystem(AudioDecStatus_t * p_AudioDecStatus)
{
    ...
    switch (p_AudioDecStatus->eSourceType)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
            ...
            else if (p_AudioDecStatus->eMMType == AUDIO_MM_INJECT)
            {
                DecRet = AU_GetDecID(INJECTDecPriority, p_AudioDecStatus);
            }
            else
            {
    ...
    {
        case AU_DEC_ID1:
            p_AudioDecStatus->eAfifoSource = E_AFIFO_0;
            break;
        case AU_DEC_ID2:
            p_AudioDecStatus->eAfifoSource = E_AFIFO_1;
            break;
        case AU_DEC_ID3:
            p_AudioDecStatus->eAfifoSource = E_AFIFO_2;
            break;
        default:
            p_AudioDecStatus->eAfifoSource = E_AFIFO_MAX;
            break;
    }
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    switch(p_AudioDecStatus->eStcSource)
    {
        case E_TSP_0:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC1, 0, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC2, 0, 0);
            }
            break;
        ...
        case E_TSP_1:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC1, 1, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC2, 1, 0);
            }
            break;
        ...
        case E_TSP_2:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC1, 2, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC2, 2, 0);
            }
            break;
        ...
        case E_TSP_3:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC1, 3, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECT, ADEC2, 3, 0);
            }
            break;
    ...
    //HALAUDIO_ERROR("\033[1;31m [%s: %d][DecId = %d, eAudFormat = %d, g_AudioVars2->AudioDecStatus[DecId].eAudFormat = %d] \033[0m \n", __FILE__, __LINE__,
    //    DecId,
    //    p_AudioDecStatus->eAudFormat,
    //    g_AudioVars2->AudioDecStatus[DecId].eAudFormat);
    if (p_AudioDecStatus->eGroup == E_CONNECT_MAIN)
    {
        g_AudioVars2->eAudioSource = p_AudioDecStatus->eSourceType;
    }

    if ((DecId == AU_DEC_ID1) && (g_AudioVars2->AudioDecStatus[DecId].eSourceType == E_AUDIO_INFO_DTV_IN))
    {
        HAL_AUDIO_SetInputPath(AUDIO_DSP2_DVB_INPUT,AUDIO_PATH_5);
    }
...
MS_BOOL _AudioReloadCodePostProcess(AUDIO_DSP_ID eDspId)
{
    return TRUE;
}
...
MS_BOOL HAL_AUDIO_SetSystem2(AUDIO_DEC_ID DecId, En_DVB_decSystemType Param)
{
    ...
    switch ( Param )
    {
        ...
        case MSAPI_AUD_DVB_OPUS:
            HAL_AUR2_WriteByte(regDecoderType, adec_type_opus);
            if (DecId == AU_DEC_ID3)
            {
                HAL_AUDIO_SetDsp2CodeType(AU_DVB_STANDARD_OPUS, TRUE);
            }
            else
            {
                HAL_AUDIO_SetDspCodeType(AU_DVB_STANDARD_OPUS, TRUE);
        ...
        case MSAPI_AUD_DVB_AC4:
            HAL_AUR2_WriteByte(regDecoderType, adec_type_ac4);
            if (DecId == AU_DEC_ID3)
            {
                HAL_AUDIO_SetDsp2CodeType(AU_DVB_STANDARD_AC4, TRUE);
            }
        ...
        case MSAPI_AUD_DVB_MPEG_H:
            HAL_AUR2_WriteByte(regDecoderType, adec_type_mpegH);
            if (DecId == AU_DEC_ID3)
            {
                HAL_AUDIO_SetDsp2CodeType(AU_DVB_STANDARD_MPEG_H, TRUE);
            }
...
void HAL_AUDIO_DumpDspInfo(void)
{
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_IoInfo) != 0)
    {
        ...
        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info2+IO_INFO2_RAW+0);
        printf("|CH5=0x%06X,",  tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info2+IO_INFO2_MUL_CH6+0);
        printf("CH6=0x%06X,",  tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info2+IO_INFO2_SCART+0);
        printf("CH7=0x%06X",  tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info1+IO_INFO1_DAC1_OUT+0);
        printf("|DAC1=0x%06X,",  tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info1+IO_INFO1_IIS1_OUT+0);
        printf("I2S=0x%06X,",  tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_IO_Info1+IO_INFO1_SPDIF_OUT+0);
        printf("SPDIF=0x%06X\n",  tmp_L);
    }
    ...
    {
        sysTime = MsOS_GetSystemTime();
        printf("[%08u](%03u):", sysTime, sysTime - preSysTime);
        preSysTime = sysTime;

        mmFile_APTS = HAL_MAD_GetCommInfo(Audio_Comm_infoType_1ms_PTS);
        printf("APTS:%07u(%03u)|", mmFile_APTS, mmFile_APTS - prv_mmFile_APTS);
...
Audio_id HAL_AUDIO_Convert_DecId_to_ADECId(AUDIO_DEC_ID dec_id)  //temp
{
    ...
    switch(dec_id)
    {
        ...
        case AU_DEC_MAX:
            ...
            ADEC_id = ADEC1; //no usage just take one for it...
            break;
...
{
    HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_ipSecurity, ip_auth);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_SET_IPAUTH_GROUP()
/// @brief \b Function \b Description: HAL_AUDIO_SET_IPAUTH register
/// @param <IN>        \b AUDIO_HASHKEY_GROUP eGroupId
/// @param <IN>        \b MS_U32 ip_auth
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_SET_IPAUTH_GROUP(AUDIO_HASHKEY_GROUP eGroupId,MS_U32 ip_auth)
{
    if (eGroupId == AUDIO_HASHKEY_GROUP1)
    {
        HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_ipSecurity, ip_auth);
    }
    else if (eGroupId == AUDIO_HASHKEY_GROUP2)
    {
        HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_ipSecurity2, ip_auth);
    }
    else
    {
        HALAUDIO_ERROR("%s: Error! Invalid hashkey group (%d)!\n", __FUNCTION__, eGroupId);
    }
}
...
void  HAL_AUDIO_Set_DecimationMode(AUDIO_OUTPUT_TYPE Output_Type, Audio_id ID)
{
    switch(Output_Type)
    {
        case AUDIO_HDMI_OUTPUT:  //high rate nonPCM application need decimation
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, ID) > 48000) &&
                (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL+1)&0x20))
            {
        ...
        case AUDIO_HDMI_ARC_OUTPUT:
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, ID) > 48000) &&
                (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL)&0x02))
            {
        ...
        case AUDIO_SPDIF_OUTPUT:
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SPDIFTx_SMP_RATE, ID) > 48000) &&
                (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL)&0x02))
            {
...
MS_BOOL HAL_AUDIO_CheckVersionInfo(void)
{
    ...
    if(HAL_AUDIO_AbsReadReg(REG_DECR2_VERSION) != DEC_R2_VERSION) // Compare DEC R2 Version with Mail Box
    {
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_ReadVersion()
/// @brief \b Function \b Description: Read Version info
/// @param <IN>        \pVersion :
/// @param <OUT>       \b NONE :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_S32 HAL_AUDIO_ReadVersion(char *pVersion, MS_U32 u32Size)
{
    char tmpStr[11] = {'\0'};
...
#ifdef DSP2PmAddr_mainVer
    /* SND DSP */
    u32Version = HAL_MAD2_Read_DSP_sram(DSP2PmAddr_mainVer, DSP_MEM_TYPE_PM);
    snprintf(tmpStr, 11, "0x%08X", u32Version);
    strcat(tmpVersion, tmpStr);
#endif

#ifdef REG_DECR2_VERSION
    /* DEC R2 */
    u32Version = HAL_AUDIO_AbsReadReg(REG_DECR2_VERSION);
    snprintf(tmpStr, 11, "0x%08X", u32Version);
    strcat(tmpVersion," / ");
    strcat(tmpVersion, tmpStr);
#endif

#ifdef REG_SNDR2_VERSION
    /* SND R2 */
    u32Version = HAL_AUDIO_AbsReadReg(REG_SNDR2_VERSION);
    snprintf(tmpStr, 11, "0x%08X", u32Version);
    strcat(tmpVersion," / ");
    strcat(tmpVersion, tmpStr);
#endif

    if (u32Size < strlen(tmpVersion))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: string size is too large !!\n", __FUNCTION__);
        return -A_ENOMEM;
    }

    strncpy(pVersion, tmpVersion, strlen(tmpVersion));

    return s32Ret;
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_ConvertInputType2SourceInfo()
/// @brief \b Function \b Description: Convert input type to input source info
/// @param <IN>        \b eInputType: Audio input type
/// @param <OUT>       \b : Audio input source info
////////////////////////////////////////////////////////////////////////////////
AUDIO_SOURCE_INFO_TYPE HAL_AUDIO_ConvertInputType2SourceInfo(AUDIO_INPUT_TYPE eInputType)
{
    AUDIO_SOURCE_INFO_TYPE eSourceInfo = E_AUDIO_INFO_ADC_IN;

    switch(eInputType)
    {
        case AUDIO_DSP1_DVB_INPUT:
        case AUDIO_DSP1_DVB1_INPUT:
        case AUDIO_DSP2_DVB_INPUT:
        case AUDIO_DSP2_DVB1_INPUT:
        case AUDIO_DSP3_DVB_INPUT:
        case AUDIO_DSP3_DVB1_INPUT:
        case AUDIO_DSP4_DVB_INPUT:
            eSourceInfo = E_AUDIO_INFO_DTV_IN;
            break;

        case AUDIO_HDMI_INPUT:
        case AUDIO_DSP1_HDMI_INPUT:
        case AUDIO_DSP1_HDMIx1_INPUT:
        case AUDIO_DSP2_HDMI_INPUT:
        case AUDIO_DSP2_HDMIx1_INPUT:
        case AUDIO_DSP3_HDMI_INPUT:
        case AUDIO_DSP3_HDMIx1_INPUT:
            eSourceInfo = E_AUDIO_INFO_HDMI_IN;
            break;

        case AUDIO_DSP1_SIF_INPUT:
        case AUDIO_DSP2_SIF_INPUT:
        case AUDIO_DSP3_SIF_INPUT:
        case AUDIO_DSP4_SIF_INPUT:
            eSourceInfo = E_AUDIO_INFO_ATV_IN;
            break;
        ...
        case AUDIO_ADC2_AUIN0_INPUT:
        case AUDIO_ADC2_AUIN1_INPUT:
        case AUDIO_ADC2_AUIN2_INPUT:
        case AUDIO_ADC2_AUIN3_INPUT:
        case AUDIO_ADC2_AUIN4_INPUT:
        case AUDIO_ADC2_AUIN5_INPUT:
        case AUDIO_ADC2_AUMIC_INPUT:
            eSourceInfo = E_AUDIO_INFO_ADC_IN;
            break;
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Apply_Setting(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Too large buffer size(%u) is required! Please check the settings: duration %u, sample rate %u\n", __FUNCTION__, u32TargetBufferSize, pPcmInfo->u32BufferDuration, pPcmInfo->u32SampleRate);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Force duration to %u, which is the maximun value\n", __FUNCTION__, pPcmInfo->u32BufferDuration);
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Restart(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not opened yet!\n", __FUNCTION__);
        return -A_EPERM;
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Open(void *pData)
{
    ...
    MS_U32 u32TargetAlignmentSize = 0;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Open %s\n", __FUNCTION__, pPCM->u8Name);
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support Multi-Channel!\n", __FUNCTION__);
        return -A_EINVAL;
    }

    if ((pUserPcmInfo->u8MixingFlag == TRUE) && (pPCM->tPcmCaps.u8MixingFlag != TRUE))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support mixing mode!\n", __FUNCTION__);
        return -A_EINVAL;
    }

    if (pUserPcmInfo->u8CaptureFlag == TRUE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support capture mode!\n", __FUNCTION__);
        return -A_EINVAL;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support channel(%u)!\n", __FUNCTION__, pUserPcmInfo->u32Channel);
        return -A_EINVAL;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support sample rate(%u)!\n", __FUNCTION__, pUserPcmInfo->u32SampleRate);
        return -A_EINVAL;
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Close(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Close %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Start(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Start %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Stop(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Stop %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Set(MS_U32 u32Cmd, const void *pData)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Set command %u to %s\n", __FUNCTION__, u32Cmd, pPCM->u8Name);
    ...
    switch(u32Cmd)
    {
        case AUDIO_PCM_CMD_NONBLOCKING:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u8NonBlockingFlag(%d)!\n", __FUNCTION__, u8NonBlockingFlag);
                s32Ret = -A_EINVAL;
                break;
            }
        ...
        case AUDIO_PCM_CMD_MULTICH:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u8MultiChFlag(%d)!\n", __FUNCTION__, u8MultiChFlag);
                s32Ret = -A_EINVAL;
                break;
            }
        ...
        case AUDIO_PCM_CMD_MIXING:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u8MixingFlag(%d)!\n", __FUNCTION__, u8MixingFlag);
                s32Ret = -A_EINVAL;
                break;
            }
        ...
        case AUDIO_PCM_CMD_BUFFERDURATION:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32BufferDuration(%u)!\n", __FUNCTION__, u32BufferDuration);
                s32Ret = -A_EINVAL;
                break;
            }
        ...
        case AUDIO_PCM_CMD_CHANNEL:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32Channel(%u)!\n", __FUNCTION__, u32Channel);
                return -A_EINVAL;
            }
        ...
        case AUDIO_PCM_CMD_SAMPLERATE:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32SampleRate(%u)!\n", __FUNCTION__, u32SampleRate);
                return -A_EINVAL;
            }
        ...
        case AUDIO_PCM_CMD_BIGENDIAN:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32BigEndian(%u)!\n", __FUNCTION__, u32BigEndian);
                return -A_EINVAL;
            }
        ...
        case AUDIO_PCM_CMD_WEIGHTING:
        {
            ...
            if (u32Weighting > 100)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32Weighting(%u)!\n", __FUNCTION__, u32Weighting);
                return -A_EINVAL;
            }
        ...
        {
            MS_U8 u8Volume_int = 0;
            MS_U8 u8Volume_fra = 0;
            MS_U32 u32Volume = *((MS_U32 *)pData);

            if (u32Volume > 100)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32Volume(%u)!\n", __FUNCTION__, (unsigned int)u32Volume);
                return -A_EINVAL;
            }

            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: configure volume to %u\n", __FUNCTION__, (unsigned int)u32Volume);

            u8Volume_int = (MS_U8)(g_u16VolumeArray[u32Volume] >> 8);
            u8Volume_fra = (MS_U8)(g_u16VolumeArray[u32Volume] & 0x00FF);
            HAL_SOUND_AbsoluteVolume(AUDIO_T3_PATH_MIXER_DMA_IN, u8Volume_int, u8Volume_fra);
        ...
        case AUDIO_PCM_CMD_MUTE:
        {
            ...
            if ((u32Mute != TRUE) && (u32Mute != FALSE))
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid Invalid mute command(%u)!\n", __FUNCTION__, (unsigned int)u32Mute);
                s32Ret = -A_EINVAL;
            }

            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: mute %u\n", __FUNCTION__, (unsigned int)u32Mute);

            HAL_SOUND_SetMute(AUDIO_T3_PATH_MIXER_DMA_IN, u32Mute);
        ...
        default:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! unsupported command type(%u)!\n", __FUNCTION__, u32Cmd);
            break;
        }
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Get(MS_U32 u32Cmd, void *pData)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Get command %u from %s\n", __FUNCTION__, u32Cmd, pPCM->u8Name);
    ...
    switch(u32Cmd)
    {
        case AUDIO_PCM_CMD_ALL:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! The version of PCM Info mismatch(0x%08X, 0x%08X)!\n", __FUNCTION__, pUserPcmInfo->u32StructVersion, AUDIO_PCM_INFO_VERSION);
            }
...
MS_U32 HAL_AUDIO_PCM_HwDma_Reader1_Write(const void *pBuf, MS_U32 u32Size)
{
    ...
    MS_U32 u32Loop = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Write %u bytes to %s\n", __FUNCTION__, u32Size, pPCM->u8Name);
    ...
    if (pPcmInfo->u8StartFlag == FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not started yet!\n", __FUNCTION__);
        return 0;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! PCM buffer empty!\n", __FUNCTION__);
        ...
        for (u32Loop = 0; u32Loop < u32RequestSampleCount; u32Loop++)
        {
            ...
            *pPCM->tPcmBufferInfo.pWritePtr++ = 0;
            ...
            if (pPCM->tPcmBufferInfo.pWritePtr >= (pPCM->tPcmBufferInfo.pBuffer + u32BufferSize))
            {
                if (pPCM->tPcmBufferInfo.pWritePtr > (pPCM->tPcmBufferInfo.pBuffer + u32BufferSize))
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! it's an abnormal copy!\n", __FUNCTION__);
                }
                pPCM->tPcmBufferInfo.pWritePtr = pPCM->tPcmBufferInfo.pBuffer;
            }
        }
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! PCM buffer full!\n", __FUNCTION__);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Flush(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Flush %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader2_Set(MS_U32 u32Cmd, const void *pData)
{
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_VOLUME:
        {
            ...
            u8Volume_int = (MS_U8)(g_u16VolumeArray[u32Volume] >> 8);
            u8Volume_fra = (MS_U8)(g_u16VolumeArray[u32Volume] & 0x00FF);
            HAL_SOUND_AbsoluteVolume(AUDIO_T3_PATH_MIXER_SECONDARY, u8Volume_int, u8Volume_fra);
        ...
        case AUDIO_PCM_CMD_MUTE:
        {
            ...
            HAL_SOUND_SetMute(AUDIO_T3_PATH_MIXER_SECONDARY, u32Mute);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader2_Get(MS_U32 u32Cmd, void *pData)
{
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_BUFFERLEVEL:
        {
            ...
            if (u32BufferLevel1 != u32BufferLevel2)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! fail to get correct buffer level (%u, %u)\n", __FUNCTION__, u32BufferLevel1, u32BufferLevel2);
            }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
            memcpy_toio((void *)pPCM->tPcmBufferInfo.pWritePtr, (void *)pBufTmp, u32SizeToCopy);
#else
...
MS_S32 HAL_AUDIO_PCM_SwDma_Reader1_Open(void *pData)
{
    ...
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_CH6_CFG, 0xFF, 0x8C);
...
#if AUDIO_MCH_SW_DMA_READER_SUPPORT
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Apply_Setting(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_U32 u32TargetBufferSize = 0;
    MS_U32 u32TargetChannel = 0;
    MS_U32 u32TargetBitWidth = 0;
    ...
    u32TargetBufferSize = (pPcmInfo->u32BufferDuration * pPcmInfo->u32SampleRate * u32TargetChannel * (u32TargetBitWidth / 8)) / 1000;
    AUDIO_DO_ALIGNMENT(u32TargetBufferSize, u32TargetAlignmentSize);
    if (u32TargetBufferSize > MCH_SW_DMA_RDR_BUF_SIZE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Too large buffer size(%u) is required! Please check the settings: duration %u, sample rate %u\n", __FUNCTION__, u32TargetBufferSize, pPcmInfo->u32BufferDuration, pPcmInfo->u32SampleRate);
        u32TargetBufferSize = MCH_SW_DMA_RDR_BUF_SIZE;
        pPcmInfo->u32BufferDuration = ((u32TargetBufferSize * 1000) / (pPcmInfo->u32SampleRate * u32TargetChannel * (u32TargetBitWidth / 8)));
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Force duration to %u, which is the maximun value\n", __FUNCTION__, pPcmInfo->u32BufferDuration);
    }
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Restart(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    /* clear PCM buffer */
    memset((void *)pPCM->tPcmBufferInfo.pBuffer, 0x00, MCH_SW_DMA_RDR_BUF_SIZE);

    /* flush MIU */
    MsOS_FlushMemory();

    /* clear engine's write pointer */
    HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_WtPtr, 0xFFFF, 0x0000);

    /* reset engine, no start engine here! */
    HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_Ctrl, 0xFFFF, 0x0001);
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Open(void *pData)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    AUDIO_PCM_INFO_t *pUserPcmInfo = NULL;
    ...
    pPcmInfo->u32StructSize = sizeof(AUDIO_PCM_INFO_t);
    pPcmInfo->u8ConnectFlag = TRUE;
    snprintf((char *)pPcmInfo->u8Name, AUDIO_COMMON_PCM_IO_NAME_LENGTH, "%s", (char *)pPCM->u8Name);
    snprintf((char *)pUserPcmInfo->u8Name, AUDIO_COMMON_PCM_IO_NAME_LENGTH, "%s", (char *)pPCM->u8Name);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_NONBLOCKING, &pUserPcmInfo->u8NonBlockingFlag);
    pPcmInfo->u8MultiChFlag = pUserPcmInfo->u8MultiChFlag;
    pPcmInfo->u8MixingFlag = pUserPcmInfo->u8MixingFlag;
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_BUFFERDURATION, &pUserPcmInfo->u32BufferDuration);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_CHANNEL, &pUserPcmInfo->u32Channel);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_SAMPLERATE, &pUserPcmInfo->u32SampleRate);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_BITWIDTH, &pUserPcmInfo->u32BitWidth);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_BIGENDIAN, &pUserPcmInfo->u32BigEndian);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_WEIGHTING, &pUserPcmInfo->u32Weighting);
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(AUDIO_PCM_CMD_VOLUME, &pUserPcmInfo->u32Volume);

    /* init PCM buffer address */
    u32TargetBufferAddrPa = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + MCH_SW_DMA_RDR_BUF_ADDR;
    pPCM->tPcmBufferInfo.pBuffer = (MS_S8 *)MS_PA2KSEG1(u32TargetBufferAddrPa);
    pPCM->tPcmBufferInfo.pReadPtr = pPCM->tPcmBufferInfo.pBuffer;
    pPCM->tPcmBufferInfo.pWritePtr = pPCM->tPcmBufferInfo.pBuffer;

    /* apply setting */
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Apply_Setting();

    /* restart */
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Restart();

    return s32Ret;
}

MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Close(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    if (pPcmInfo->u8ConnectFlag != FALSE)
    {
        HAL_AUDIO_PCM_MCH_SwDma_Reader_Flush();
        memset((void *)pPcmInfo, 0x00, sizeof(AUDIO_PCM_INFO_t));
    }
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Start(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    if (pPcmInfo->u8StartFlag == FALSE)
    {
        ...
        /* start engine */
        HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_Ctrl, 0xFFFF, 0x0002);
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Stop(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    {
        /* stop engine */
        HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_Ctrl, 0xFFFF, 0x0000);
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Set(MS_U32 u32Cmd, const void *pData)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Get(MS_U32 u32Cmd, void *pData)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_BUFFERLEVEL:
        {
            ...
            HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_Ctrl, 0x0008, 0x0008);
            u32BufferLevel = HAL_AUDIO_ReadReg(S2M_MBOX_MCH_SW_DMA_READER_DDR_Level) * BYTES_IN_MIU_LINE;
            HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_Ctrl, 0x0008, 0x0000);

            /*
             * We report 10 channels buffer level, but atually it's 6 channels.
             * So convert from 6 channels to 10 channels here.
             */
            u32BufferLevel = (u32BufferLevel * 10) / 6;

            pPcmInfo->u32BufferLevel = u32BufferLevel;
            *((MS_U32 *)pData) = pPcmInfo->u32BufferLevel;
        ...
        case AUDIO_PCM_CMD_BUFFERSIZE:
        {
            ...
            /*
             * We report 10 channels buffer size, but atually it's 6 channels.
             * So convert from 6 channels to 10 channels here.
             */
            u32TargetBufferSize = (u32TargetBufferSize * 10) / 6;
...
MS_U32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Write(const void *pBuf, MS_U32 u32Size)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_U16 *pBufTmp = NULL;
    MS_U16 *pWritePtr = NULL;
    MS_U32 u32BufferSize = 0;
    MS_U32 u32PcmLevel = 0;
    MS_U32 u32RequestSize = 0;
    MS_U32 u32RequestSampleCount = 0;
    ...
    HAL_AUDIO_PCM_MCH_SwDma_Reader_Get(AUDIO_PCM_CMD_BUFFERLEVEL, &u32PcmLevel);
    if (u32PcmLevel == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! PCM buffer empty!\n", __FUNCTION__);
    }

    HAL_AUDIO_PCM_MCH_SwDma_Reader_Get(AUDIO_PCM_CMD_BUFFERSIZE, &u32BufferSize);

    /* convert from 10 channels to 6 channels */
    u32PcmLevel = (u32PcmLevel / 10) * 6;
    u32BufferSize = (u32BufferSize / 10) * 6;
    u32RequestSize = (u32Size / 10) * 6;

    /* calculate total pcm sample count */
    u32RequestSampleCount = u32Size / 2;
    if ((u32RequestSampleCount % 10) != 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! u32Size %u is not in 10 channels alignment!\n", __FUNCTION__, u32Size);
    }
    ...
    if ((u32PcmLevel + u32RequestSize) <= u32BufferSize)
    {
        ...
        MS_U32 u32SampleIndex = 0;

        pWritePtr = (MS_U16 *)pPCM->tPcmBufferInfo.pWritePtr;

        for (u32Loop = 0; u32Loop < u32RequestSampleCount; u32Loop++)
        {
            /*
             * convert from 10 channels to 6 channels :
             * (DmxL, DmxR, L, C, R, Ls, Rs, LFE, Lrs, Rrs) -> (L, C, R, Ls, Rs, LFE)
             */
            u32SampleIndex = u32Loop % 10;
            if ((u32SampleIndex >= 2) && (u32SampleIndex <= 7))
            {
                *pWritePtr = *pBufTmp;

                pWritePtr++;
                if (pWritePtr >= (pPCM->tPcmBufferInfo.pBuffer + MCH_SW_DMA_RDR_BUF_SIZE))
                {
                    if (pWritePtr > (pPCM->tPcmBufferInfo.pBuffer + MCH_SW_DMA_RDR_BUF_SIZE))
                    {
                        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! it's an abnormal copy!\n", __FUNCTION__);
                    }
                    pWritePtr = pPCM->tPcmBufferInfo.pBuffer;
                }
            }
        ...
        /* update write pointers to engine */
        pPCM->tPcmBufferInfo.pWritePtr = (MS_S8 *)pWritePtr;
        u32WptrOffset = pPCM->tPcmBufferInfo.pWritePtr - pPCM->tPcmBufferInfo.pBuffer;
        HAL_AUDIO_WriteMaskReg(M2S_MBOX_MCH_SW_DMA_READER_DDR_WtPtr, 0xFFFF, (u32WptrOffset / BYTES_IN_MIU_LINE));
...
MS_S32 HAL_AUDIO_PCM_MCH_SwDma_Reader_Flush(void)
{
    AUDIO_PCM_t *pPCM = &Audio_Pcm_MCH_SwDma_Reader;
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    ...
    {
        HAL_AUDIO_PCM_MCH_SwDma_Reader_Restart();
        HAL_AUDIO_PCM_MCH_SwDma_Reader_Stop();
    }
...
MS_S32 HAL_AUDIO_PCM_R2Dma_Reader1_Apply_Setting(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Too large buffer size(%u) is required! Please check the settings: duration %u, sample rate %u, channel %u\n", __FUNCTION__, u32TargetBufferSize, pPcmInfo->u32BufferDuration, pPcmInfo->u32SampleRate, u32TargetChannel);
...
MS_S32 HAL_AUDIO_PCM_R2Dma_Reader1_Set(MS_U32 u32Cmd, const void *pData)
{
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_VOLUME:
        {
            ...
            u8Volume_int = (MS_U8)(g_u16VolumeArray[u32Volume] >> 8);
            u8Volume_fra = (MS_U8)(g_u16VolumeArray[u32Volume] & 0x00FF);
            HAL_SOUND_AbsoluteVolume(AUDIO_T3_PATH_MIXER_MCH_IN, u8Volume_int, u8Volume_fra);
        ...
        case AUDIO_PCM_CMD_MUTE:
        {
            ...
            HAL_SOUND_SetMute(AUDIO_T3_PATH_MIXER_MCH_IN, u32Mute);
...
MS_S32 HAL_AUDIO_PCM_Capture1_Open(void *pData)
{
    ...
    if (pUserPcmInfo->u8CaptureFlag != TRUE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Not support playback mode!\n", __FUNCTION__);
        return -A_EINVAL;
    }
...
MS_S32 HAL_AUDIO_PCM_Capture1_Set(MS_U32 u32Cmd, const void *pData)
{
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_BUFFERDURATION:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! Not support to configure buffer duration!\n", __FUNCTION__);
            s32Ret = -A_EPERM;
        ...
        case AUDIO_PCM_CMD_CHANNEL:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! Not support to configure channel!\n", __FUNCTION__);
            s32Ret = -A_EPERM;
        ...
        case AUDIO_PCM_CMD_SAMPLERATE:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! Not support to configure sample rate!\n", __FUNCTION__);
            s32Ret = -A_EPERM;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
            memcpy_toio((void *)pBufTmp, (void *)pPCM->tPcmBufferInfo.pReadPtr, u32SizeToCopy);
#else
...
AUDIO_DSP_CODE_TYPE HAL_AUDIO_Convert_EnDVBdecSystemType_to_AudioDspCodeType(En_DVB_decSystemType enDecSystem)
{
    ...
    {
        case MSAPI_AUD_DVB_OPUS:
            dspCodeType = AU_DVB_STANDARD_OPUS;
            break;
        case MSAPI_AUD_DVB_AC4:
            dspCodeType = AU_DVB_STANDARD_AC4;
            break;
        ...
        case MSAPI_AUD_DVB_MPEG:
            ...
            break;
        case MSAPI_AUD_DVB_MPEG_H:
            dspCodeType = AU_DVB_STANDARD_MPEG_H;
            break;
        case MSAPI_AUD_DVB_AC3:
...
MS_S32 HAL_AUDIO_InputSwitch(AUDIO_SWITCH_GROUP enGroup)
{
    ...
    switch (enGroup)
    {
        case E_AUDIO_GROUP_MAIN:
        {
            ...
            /* configure a default setting to R2's PCM Render accroding to current input source */
            printf("%s() enGroup:%x eAudioSource:%x u8PcmRenderControl << 1=%d\n", __FUNCTION__, enGroup,g_AudioVars2->eAudioSource,u8PcmRenderControl << 1);
...
MS_S32 HAL_AUDIO_SetEDIDOutputInterface(DIGITAL_OUTPUT_TYPE outpath)
{
    if (outpath == DIGITAL_HDMI_ARC_OUTPUT)
    {
        //SPDIF + HDMI ARC + HDMI TX(PCM)
        HAL_AUDIO_WriteMaskByte(0x304f, 0x01, 0x01); //IRQL1n select spdif2
        HAL_AUDIO_WriteMaskByte(0x304e, 0x10, 0x00); //IRQE1n select hdmi-tx
        HAL_AUDIO_WriteMaskByte(M2S_MBOX_HDMI_SETTING+1, 0x40, 0x40);
    }
    else if (outpath == DIGITAL_HDMI_OUTPUT)
    {
        //SPDIF + HDMI TX
        HAL_AUDIO_WriteMaskByte(0x304F, 0x01, 0x00); //IRQL1n select hdmi-tx
        HAL_AUDIO_WriteMaskByte(0x304E, 0x10, 0x10); //IRQE1n select spdif2
        HAL_AUDIO_WriteMaskByte(M2S_MBOX_HDMI_SETTING+1, 0x40, 0x00);
    }
    else
    {
        // Not Support now
        HALAUDIO_ERROR("%s: Warning! it's a not support output HBR path!\n", __FUNCTION__);
        return FALSE;
    }
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Open(void)
{
    ...
    /* enable MP3 Encoder1 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x01, 0x01);

    HAL_AUDIO_MP3_ENCODER1_Stop();

    /* reset read pointer */
    g_u32Mp3Encoder1RptrOffset = 0;
    HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder1RptrOffset);
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Close(void)
{
    ...
    HAL_AUDIO_MP3_ENCODER1_Flush();

    /* disable MP3 Encoder1 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x01, 0x00);
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Start(void)
{
    ...
    /* start MP3 Encoder1 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x02, 0x02);
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Stop(void)
{
    ...
    /* stop MP3 Encoder1 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x02, 0x00);
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Ioctl(MS_U32 u32Cmd, void *pData)
{
    ...
    switch (u32Cmd)
    {
        case AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET:
        {
            HAL_SND_R2_Set_SHM_PARAM(R2_SHM_PARAM_MP3_ENCODER_READ_PTR, u8DeviceId, *((MS_U32 *)pData), 0);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_WRITE_PTR_OFFSET:
        {
            *((MS_U32 *)pData) = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_WRITE_PTR, u8DeviceId);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_THRESHOLD:
        {
            *((MS_U32 *)pData) = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_THRESHOLD, u8DeviceId);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_FRAME_SIZE:
        {
            *((MS_U32 *)pData) = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_FRAME_SIZE, u8DeviceId);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_AVAILABLE_SIZE:
        {
            *((MS_U32 *)pData) = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_AVAILABLE_SIZE, u8DeviceId);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_FRAME_COUNT:
        {
            *((MS_U32 *)pData) = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_FRAME_COUNT, u8DeviceId);
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_TIMESTAMP_HIGH32BIT:
        {
            MS_U32 u32Data = 0;

            u32Data = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_TIMESTAMP, u8DeviceId);
            u32Data = (u32Data & 0x80000000) >> 31;
            *((MS_U32 *)pData) = u32Data;
            break;
        }

        case AUDIO_ENCODER_IOCTL_GET_TIMESTAMP_LOW32BIT:
        {
            MS_U32 u32Data = 0;

            u32Data = HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_MP3_ENCODER_TIMESTAMP, u8DeviceId);
            u32Data = (u32Data << 1) & 0xFFFFFFFF;
            *((MS_U32 *)pData) = u32Data;
            break;
        }
...
{
#if AUDIO_MP3_ENCODER1_SUPPORT
    MS_S8 *pEncoderBuf = NULL;
    MS_S8 *pEncoderReadPtr = NULL;
    MS_S8 *pBufTmp = NULL;
    MS_U32 u32TargetBufferAddrPa = 0;
    MS_U32 u32BufferSize = 0;
    MS_U32 u32Mp3Level = 0;
    MS_U32 u32RequestSize = 0;
    MS_U32 u32RequestSizeTmp = 0;
    MS_U32 u32SizeToCopy = 0;
    MS_U32 u32Threshold = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Read %u bytes from %s\n", __FUNCTION__, u32Size, pPCM->u8Name);
    ...
    u32TargetBufferAddrPa = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + MP3_ENCODER1_BUF_ADDR;
    pEncoderBuf = (MS_S8 *)MS_PA2KSEG1(u32TargetBufferAddrPa);
    if (pEncoderBuf == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pEncoderBuf should not be NULL!\n", __FUNCTION__);
        return 0;
    }

    u32BufferSize = MP3_ENCODER1_BUF_SIZE;
    HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_GET_AVAILABLE_SIZE, &u32Mp3Level);
    HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_GET_THRESHOLD, &u32Threshold);
    if (u32Mp3Level >= (u32BufferSize - u32Threshold))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! MP3 buffer full!\n", __FUNCTION__);

        u32Mp3Level = 0;
        HAL_AUDIO_MP3_ENCODER1_Stop();
        HAL_AUDIO_MP3_ENCODER1_Flush();
        HAL_AUDIO_MP3_ENCODER1_Start();
    }
    else if (u32Mp3Level == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! MP3 buffer empty!\n", __FUNCTION__);
    }

    u32RequestSize = u32Size;
    u32RequestSizeTmp = u32RequestSize;

    pEncoderReadPtr = pEncoderBuf + g_u32Mp3Encoder1RptrOffset;

    //HALAUDIO_ERROR("[%u] %s: u32BufferSize %u, u32Threshold %u, u32Mp3Level %u, u32RequestSize %u\n", MsOS_GetSystemTime(), __FUNCTION__, u32BufferSize, u32Threshold, u32Mp3Level, u32RequestSize);

    if (u32Mp3Level <= u32Threshold)
    {
        return 0;
    }
    u32Mp3Level -= u32Threshold;

    /* copy data from PCM buffer */
    if (u32Mp3Level >= u32RequestSize)
    {
        do {
            u32SizeToCopy = (pEncoderBuf + u32BufferSize) - pEncoderReadPtr;
            u32SizeToCopy = (u32SizeToCopy > u32RequestSizeTmp) ? u32RequestSizeTmp : u32SizeToCopy;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
            memcpy_toio((void *)pBufTmp, (void *)pEncoderReadPtr, u32SizeToCopy);
#else
            memcpy((void *)pBufTmp, (void *)pEncoderReadPtr, u32SizeToCopy);
#endif
            //HALAUDIO_ERROR("[%u] %s: g_u32Mp3Encoder1RptrOffset 0x%08X, SizeToCopy %u, ReadPtr 0x%08X, Buf 0x%08X, BufEnd 0x%08X\n", MsOS_GetSystemTime(), __FUNCTION__, g_u32Mp3Encoder1RptrOffset, u32SizeToCopy, pEncoderReadPtr, pEncoderBuf, (pEncoderBuf + u32BufferSize));

            pBufTmp += u32SizeToCopy;
            pEncoderReadPtr += u32SizeToCopy;
            if (pEncoderReadPtr >= (pEncoderBuf + u32BufferSize))
            {
                if (pEncoderReadPtr > (pEncoderBuf + u32BufferSize))
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! it's an abnormal copy!\n", __FUNCTION__);
                }
                pEncoderReadPtr = pEncoderBuf;
            }
        ...
        /* update read pointer to engine */
        g_u32Mp3Encoder1RptrOffset = pEncoderReadPtr - pEncoderBuf;
        HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder1RptrOffset);
...
MS_S32 HAL_AUDIO_MP3_ENCODER1_Flush(void)
{
    ...
    /* set read pointer as write pointer to flush data in buffer */
    HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_GET_WRITE_PTR_OFFSET, &u32WptrOffset);
    g_u32Mp3Encoder1RptrOffset = u32WptrOffset;
    HAL_AUDIO_MP3_ENCODER1_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder1RptrOffset);
...
MS_S32 HAL_AUDIO_MP3_ENCODER2_Open(void)
{
    ...
    /* enable MP3 Encoder2 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x04, 0x04);

    HAL_AUDIO_MP3_ENCODER2_Stop();

    /* reset read pointer */
    g_u32Mp3Encoder2RptrOffset = 0;
    HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder2RptrOffset);
...
MS_S32 HAL_AUDIO_MP3_ENCODER2_Close(void)
{
    ...
    HAL_AUDIO_MP3_ENCODER2_Flush();

    /* disable MP3 Encoder2 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x04, 0x00);
...
MS_S32 HAL_AUDIO_MP3_ENCODER2_Start(void)
{
    ...
    /* start MP3 Encoder2 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x08, 0x08);
...
MS_S32 HAL_AUDIO_MP3_ENCODER2_Stop(void)
{
    ...
    /* stop MP3 Encoder2 */
    HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_TRANSCODE_SEL, 0x08, 0x00);
...
MS_U32 HAL_AUDIO_MP3_ENCODER2_Read(void *pBuf, MS_U32 u32Size)
{
    ...
    u32TargetBufferAddrPa = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + MP3_ENCODER2_BUF_ADDR;
    pEncoderBuf = (MS_S8 *)MS_PA2KSEG1(u32TargetBufferAddrPa);
    if (pEncoderBuf == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pEncoderBuf should not be NULL!\n", __FUNCTION__);
    ...
    u32BufferSize = MP3_ENCODER2_BUF_SIZE;
    HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_GET_AVAILABLE_SIZE, &u32Mp3Level);
    HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_GET_THRESHOLD, &u32Threshold);
    if (u32Mp3Level >= (u32BufferSize - u32Threshold))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! MP3 buffer full!\n", __FUNCTION__);
    ...
    pEncoderReadPtr = pEncoderBuf + g_u32Mp3Encoder2RptrOffset;
    ...
    if (u32Mp3Level >= u32RequestSize)
    {
        ...
        /* update read pointer to engine */
        g_u32Mp3Encoder2RptrOffset = pEncoderReadPtr - pEncoderBuf;
        HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder2RptrOffset);
...
MS_S32 HAL_AUDIO_MP3_ENCODER2_Flush(void)
{
    ...
    /* set read pointer as write pointer to flush data in buffer */
    HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_GET_WRITE_PTR_OFFSET, &u32WptrOffset);
    g_u32Mp3Encoder2RptrOffset = u32WptrOffset;
    HAL_AUDIO_MP3_ENCODER2_Ioctl(AUDIO_ENCODER_IOCTL_SET_READ_PTR_OFFSET, &g_u32Mp3Encoder2RptrOffset);
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_HDMI_Set_RxHBRFlag()
/// @brief \b Function \b Description:  This is used to set HDMI Rx HBR flag.
/// @param <IN>        \b MS_BOOL : The HDMI rx Hight Bit Rate flag.
/// @param <OUT>       \b NONE :
/// @param <RET>       \b NONE :
/// @param <GLOBAL>    \b NONE :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_HDMI_Set_RxHBRFlag(MS_BOOL bFlag)
{
    g_AudioVars2->g_HDMIRxHBRFlag = bFlag;
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_HDMI_Get_RxHBRFlag()
/// @brief \b Function \b Description:  This is used to get HDMI Rx HBR flag.
/// @param <IN>        \b NONE :
/// @param <OUT>       \b NONE :
/// @param <RET>       \b MS_BOOL : The HDMI rx Hight Bit Rate flag.
/// @param <GLOBAL>    \b NONE :
////////////////////////////////////////////////////////////////////////////////
MS_BOOL HAL_AUDIO_HDMI_Get_RxHBRFlag(void)
{
    MS_BOOL bRet;
    bRet = g_AudioVars2->g_HDMIRxHBRFlag;
    return bRet;
}
