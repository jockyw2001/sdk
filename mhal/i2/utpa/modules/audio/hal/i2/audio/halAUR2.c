...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static MS_U32 g_u32Mp3EncoderRptrOffset[2] = {0};
...
void HAL_SND_R2_init_SHM_param(void)
{
    SND_R2_SHARE_MEM  *p_shm = (SND_R2_SHARE_MEM *)(MS_PA2KSEG1(REG_SNDR2_DDR_BASE+SND_R2_SHM_DDR_OFFSET));
    AUR2_MP3E_PARAM *p_shm_mp3e_param = NULL;
    AUR2_MP3E_INFO *p_shm_mp3e_info = NULL;
    MS_S32 s32DeviceId = 0;

    for (s32DeviceId = 0; s32DeviceId < 2; s32DeviceId++)
    {
        p_shm_mp3e_param = &p_shm->mp3e_param_shm[s32DeviceId];
        p_shm_mp3e_info = &p_shm->mp3e_info_shm[s32DeviceId];

#if (defined(MSOS_TYPE_LINUX_KERNEL) && defined(CONFIG_ARM64))
        memset_io(p_shm_mp3e_param, 0, sizeof(AUR2_MP3E_PARAM));
        memset_io(p_shm_mp3e_info, 0, sizeof(AUR2_MP3E_INFO));
#else
        memset(p_shm_mp3e_param, 0, sizeof(AUR2_MP3E_PARAM));
        memset(p_shm_mp3e_info, 0, sizeof(AUR2_MP3E_INFO));
#endif
    }
...
{
    volatile MS_U32 *ptr = NULL;
    SND_R2_SHARE_MEM *p_shm = NULL;
    AUR2_MP3E_INFO *p_shm_mp3e_info = NULL;
    MS_U32 ret_value = 0;

    p_shm = (SND_R2_SHARE_MEM *)(MS_PA2KSEG1(REG_SNDR2_DDR_BASE+SND_R2_SHM_DDR_OFFSET));
    if (p_shm == NULL)
    {
        AUR2_DEBUG_PRINT(printf("[%s] Error! p_shm should not be NULL!\n", __FUNCTION__));
        return FALSE;
    }

    if ((dec_id != 0) && (dec_id != 1))
    {
        AUR2_DEBUG_PRINT(printf("[%s] Error! invalid dec_id %d!\n", __FUNCTION__, dec_id));
        return FALSE;
    }

    p_shm_mp3e_info = &p_shm->mp3e_info_shm[dec_id];

    switch (decInfoType)
    {
        case R2_SHM_INFO_MP3_ENCODER_WRITE_PTR:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_ES_wrPtr;
            break;
        }

        case R2_SHM_INFO_MP3_ENCODER_THRESHOLD:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_threshold;
            break;
        }

        case R2_SHM_INFO_MP3_ENCODER_FRAME_SIZE:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_frame_size;
            break;
        }

        case R2_SHM_INFO_MP3_ENCODER_AVAILABLE_SIZE:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_available_size;
            break;
        }

        case R2_SHM_INFO_MP3_ENCODER_FRAME_COUNT:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_frame_cnt;
            break;
        }

        case R2_SHM_INFO_MP3_ENCODER_TIMESTAMP:
        {
            MS_S8 u8IndexEqual = -1;
            MS_S8 u8IndexSmaller = -1;
            MS_S8 u8IndexBigger = -1;
            MS_U8 u8Index = 0;
            MS_S32 s32WptrOffsetSmaller = -1;
            MS_S32 s32WptrOffsetBigger = -1;
            MS_U32 u32Mp3EncoderRptrOffset = g_u32Mp3EncoderRptrOffset[dec_id];

            //D-cache Invalidate
            MsOS_Dcache_Flush((MS_VIRT)p_shm_mp3e_info, sizeof(AUR2_MP3E_INFO));

            for (u8Index = 0; u8Index < MP3ENC_MAX_PTS_TBL_NUM; u8Index++)
            {
                MS_U32 u32WptrOffset = p_shm_mp3e_info->MP3E_info_ES_wrPtrTab[u8Index];

                if (u32Mp3EncoderRptrOffset == u32WptrOffset)
                {
                    u8IndexEqual = u8Index;
                    break;
                }
                else if (u32Mp3EncoderRptrOffset > u32WptrOffset)
                {
                    if ((s32WptrOffsetSmaller == -1) || (u32WptrOffset > s32WptrOffsetSmaller))
                    {
                        s32WptrOffsetSmaller = u32WptrOffset;
                        u8IndexSmaller = u8Index;
                    }
                }
                else if (u32Mp3EncoderRptrOffset < u32WptrOffset)
                {
                    if ((s32WptrOffsetBigger== -1) || (u32WptrOffset < s32WptrOffsetBigger))
                    {
                        s32WptrOffsetBigger = u32WptrOffset;
                        u8IndexBigger = u8Index;
                    }
                }
            }
            ...
            if (u8IndexEqual != -1)
            {
                ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_curPTSTab[u8IndexEqual];
            }
            else if (u8IndexSmaller != -1)
            {
                u8IndexSmaller ++;
                if (u8IndexSmaller >= MP3ENC_MAX_PTS_TBL_NUM)
                {
                    u8IndexSmaller = 0;
                }

                ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_curPTSTab[u8IndexSmaller];
            }
            else if (u8IndexBigger != -1)
            {
                if (u8IndexBigger >= MP3ENC_MAX_PTS_TBL_NUM)
                {
                    u8IndexBigger = 0;
                }

                ptr = (volatile MS_U32 *)&p_shm_mp3e_info->MP3E_info_curPTSTab[u8IndexBigger];
            }
            else
            {
                AUR2_DEBUG_PRINT(printf("R2_SHM_INFO_MP3_ENCODER_TIMESTAMP index search fail\n"));
                return 0;
            }
...
MS_U32 HAL_DEC_R2_Get_SHM_INFO2(R2_SHM_INFO_TYPE decInfoType, MS_U32 dec_id, MS_U8 *buf_ptr, MS_U32 buf_size )
{
    volatile MS_U8 *ptr;
    DEC_R2_SHARE_MEM  *p_shm = (DEC_R2_SHARE_MEM *)(MS_PA2KSEG1(REG_DECR2_DDR_BASE+DEC_R2_SHM_DDR_OFFSET));
    ...
    switch( decInfoType )
    {
        case R2_SHM_INFO_AC4_TOC:
            ptr = (volatile MS_U8 *) &p_shm_info->dolby_info.shm_ac4_toc;
            if(buf_size == sizeof(ms_ac4_toc_info))
            {
                MsOS_Dcache_Flush((MS_VIRT)p_shm_info, sizeof(AUR2_ADEC_INFO_SHM));
                memcpy(buf_ptr, (void *)ptr, sizeof(ms_ac4_toc_info));
            }
            break;
...
{
    volatile MS_U32 *ptr = NULL;
    DEC_R2_SHARE_MEM *p_shm = NULL;
    AUR2_ADEC_INFO_SHM *p_shm_info = NULL;
    AUR2_ADEC_ES_INFO_SHM *p_shm_esinfo = NULL;
    MS_U8  es_id = 0;
    MS_U32 ret_value = 0;

    p_shm = (DEC_R2_SHARE_MEM *)(MS_PA2KSEG1(REG_DECR2_DDR_BASE+DEC_R2_SHM_DDR_OFFSET));
    if (p_shm == NULL)
    {
        AUR2_DEBUG_PRINT(printf("[%s] Error! p_shm should not be NULL!\n", __FUNCTION__));
        return FALSE;
    ...
    switch( decInfoType )
    {
        ...
        case R2_SHM_INFO_PCM_OUTPUT_CHANNEL:     ptr = (volatile unsigned int *) &p_shm_info->dec_pcm_channel;          break;
		...
		case R2_SHM_INFO_CHUNK_PTS_L:        ptr = (volatile unsigned int *) &p_shm_info->chunk_PTS_L;                 break;
        case R2_SHM_INFO_CHUNK_PTS_H:        ptr = (volatile unsigned int *) &p_shm_info->chunk_PTS_H;                 break;

        case R2_SHM_INFO_DTS_PCM_FRAME_SIZE:        ptr = (volatile MS_U32 *) &p_shm_info->dts_info.dts_PCM_frame_size;   break;

#if 0 /* wait for audio firmware ready */
        /* UART INFO */
        case R2_SHM_INFO_UART_BUF_BASE: ptr = (volatile MS_U32 *) &p_shm_info->uart_buf_base;                       break;
        case R2_SHM_INFO_UART_BUF_SIZE: ptr = (volatile MS_U32 *) &p_shm_info->uart_buf_size;                       break;
        case R2_SHM_INFO_UART_BUF_WPTR: ptr = (volatile MS_U32 *) &p_shm_info->uart_buf_wptr;                       break;
#endif
...
MS_BOOL HAL_SND_R2_Set_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param, MS_U32 param2)
{
    ...
    p_shm_mp3e_param = &p_shm->mp3e_param_shm[dec_id];

    switch (decParamType)
    {
        case R2_SHM_PARAM_MP3_ENCODER_READ_PTR:
        {
            ptr = (volatile MS_U32 *)&p_shm_mp3e_param->MP3E_param_ES_rdPtr;
            *ptr = param;
            g_u32Mp3EncoderRptrOffset[dec_id] = param;
    ...
    switch( decParamType )
    {
        ...
        case R2_SHM_PARAM_MM_SLOW:      // [13:11]
                ...
                *ptr &= 0xFFFFC7FF; // clear [13:11] fisrt
                ...
                break;
		case R2_SHM_PARAM_STC_SELECT:          //[10:9]
        ...
        case R2_SHM_PARAM_CHUNK_PTS_RD_FLAG:
            ptr = (volatile unsigned int *) &p_shm_param->chunk_PTS_rd_flag;      *ptr = param;  MsOS_Dcache_Flush((unsigned int)ptr,4);

        case R2_SHM_PARAM_AC4_1ST_PREF_LANG:
            ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ac4_1st_pref_lang;    *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;

        case R2_SHM_PARAM_AC4_2ND_PREF_LANG:
            ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ac4_2nd_pref_lang;    *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;

        case R2_SHM_PARAM_AC4_DE_GAIN:
            ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ac4_de_gain;    *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;

        case R2_SHM_PARAM_AC4_MD_PRESENT_INDEX:
            ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ac4_pres_index[0];    *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;

        case R2_SHM_PARAM_AC4_AD_PRESENT_INDEX:
            ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ac4_pres_index[1];    *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;

        case R2_SHM_PARAM_DTS_SEAMLESS_MODE:
            ptr = (volatile MS_U32 *) &p_shm_param->dts_param.seamless;    *ptr = param;      MsOS_Dcache_Flush((MS_VIRT)ptr,4);        break;

        case R2_SHM_PARAM_DPGA_FLAG:
            ptr = (volatile MS_U32 *) &p_shm_param->DPGA_Disable_flag;      *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);   break;
...
MS_U32 HAL_SND_R2_Get_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param)
{
    ...
    //D-cache Invalidate
    MsOS_Dcache_Flush((MS_VIRT)p_shm_mp3e_param, sizeof(AUR2_MP3E_PARAM));
    ret_value = *ptr;
