...
MS_U32 HAL_MAD2_Read_DSP_Xbox(MS_U16 dsp_addr)
{
    ...
    HAL_AUDIO_WriteMaskReg(REG_DSP_XBOX_FLAG_CTRL, 0x8000, 0x8000); //[15] MCU info DSP, MCU is ready to access Xbox

    while((time_out++ < 5) && (access_flag == FALSE))
    {
        for (i = 0; i < 1000; i++)
        {
            if((HAL_AUDIO_ReadReg(REG_DSP_XBOX_FLAG_INFO) & 0x8000) == 0x0000) //[15] check DSP is now accessing Xbox or not. if yes, MCU can not access later.
            {
                access_flag = TRUE;
                break;
            }
    ...
    if (time_out >= 5)
    {
        printf("Error, access XBox fail, can't read XBox(0x%x)!\n", dsp_addr);
    }
    ...
    if(access_flag == TRUE)
    {
        if (HAL_AUDSP_Check_XBox_TransData_Ready() == FALSE)
        {
            ...
            HAL_AUDIO_WriteMaskReg(REG_DSP_XBOX_FLAG_CTRL, 0x8000, 0x0000); //[15] clear MCU info DSP, MCU is not accessing Xbox
            return 0;
        }

        HAL_AUDIO_WriteReg( REG_DSP_XBOX_ADDRESS, (dsp_addr&0x01FF)); // [8:0] xbox address
        HAL_AUDIO_WriteMaskByte(REG_DSP_XBOX_MISC, 0x07, 0x04);  //bit[2]:xbox read command

        dat[1] = HAL_AUDIO_ReadReg(REG_DSP_XBOX_RD_DATA_H);  //[23:8] xbox data high
        dat[0] = HAL_AUDIO_ReadReg(REG_DSP_XBOX_RD_DATA_L);  //[7:0] xbox data low
...
MS_BOOL HAL_MAD2_Write_DSP_Xbox(MS_U16 dsp_addr, MS_U32 value)
{
    ...
    if (time_out >= 5)
    {
        //DBG_MAD2("Error, check XBox access flag fail, can't write XBox(0x%x),value(0x%lx)!\n",dsp_addr,value);
        printf("Error, access XBox fail, can't write XBox(0x%x),value(0x%x)!\n", dsp_addr, value);
    }
    ...
    if( access_flag == TRUE)
    {
        ...
        HAL_AUDIO_WriteReg( REG_DSP_XBOX_WR_DATA_H, dat[1]);   //[23:8] xbox data high
        HAL_AUDIO_WriteReg( REG_DSP_XBOX_WR_DATA_L, dat[0]);   //[7:0] xbox data low

        HAL_AUDIO_WriteMaskByte(REG_DSP_XBOX_MISC, 0x07, 0x02);  //bit[1]:xbox write command
    }
