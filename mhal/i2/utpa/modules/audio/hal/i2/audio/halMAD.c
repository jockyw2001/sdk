...
#include "halSIF.h"
    ...
    #include "K6_mainline_ac4_mpegh/r2_shm_comm.h"
    #include "K6_mainline_ac4_mpegh/decR2_shm.h"
    #include "K6_mainline_ac4_mpegh/ddr_config.h"
    #if ASND_R2_SUPPORT
        #include "K6_mainline_ac4_mpegh/sndR2_shm.h"
    #endif

#else
    #include "K6_mainline/r2_shm_comm.h"
    #include "K6_mainline/decR2_shm.h"
    #include "K6_mainline/ddr_config.h"
    #if ASND_R2_SUPPORT
        #include "K6_mainline/sndR2_shm.h"
    #endif
...
#define AUDIO_HASHKEY_DEMO_MODE                   0x1000    (no used)
#define AUDIO_HASHKEY_DISABLE_MPEG_H              0x1000
...
#define AUDIO_HASHKEY_DISABLE_AC4                 0x400000
    ...
    if ( eDecId == AU_DEC_ID1 )
    {
        if(g_AudioVars2->g_hbr_bypass_enable==true)
        {
            eAudioCaps = Audio_DEC_PCM        | Audio_DEC_MPEG     |
                     ...
                     Audio_DEC_AC4		  |  Audio_DEC_MPEG_H  | Audio_DEC_OPUS;
        }
        else
        {
            eAudioCaps = Audio_DEC_PCM        | Audio_DEC_MPEG     |
...
{
    return; // Unused function, Remove Incorrect I2S Input synthesizer setting
}
...
{
    Audio_id adec_id = ADEC1;
    if ( dec_id == AU_DEC_ID1 )
    {
        adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(dec_id);
...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(dec_id);
        /* es level */
        audio_aes_info[0].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, adec_id);
        ...
        audio_aes_info[0].aes_read_addr = OFFSET_ES1_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_ES_RD_PTR, adec_id );
    ...
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(dec_id);
        /* es level */
        audio_aes_info[1].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, adec_id);
        ...
        audio_aes_info[1].aes_read_addr = OFFSET_ES3_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_ES_RD_PTR, adec_id );
...
void HAL_MAD_Monitor_DDPlus_SPDIF_Rate(void)
{
    ...
    HAL_AUDIO_SPDIF_SetMode(g_AudioVars2->g_eSpdifMode, eAudioSource);
    HAL_AUDIO_HDMI_TX_SetMode(g_AudioVars2->g_eHdmiTxOutputMode, eAudioSource);
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            else if ((eAudioSource == E_AUDIO_INFO_HDMI_IN) && (HAL_MAD_GetAC3PInfo(Audio_AC3P_infoType_hdmiAC3inSE) == 1))
            {
                MainDspCodeType = Dsp2CodeType;
                u8MainDecodeId = ADEC2;
            }
            ...
            {
                switch(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SPDIFTx_SMP_RATE, u8MainDecodeId))
                {
                    case 44100:
                    {
            ...
            if (g_AudioVars2->g_EdidOutViaArc == TRUE)
            {
                ...
                {
                    switch(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, u8MainDecodeId))
                    {
                        case 19200:
                        {
                            /* change SPDIF Tx 2 channel status --> 192KHz */
                        ...
                        case 17600:
                        {
                            /* change SPDIF Tx 2 channel status --> 176KHz */
                        ...
                        case 9600:
                        {
                            /* change SPDIF Tx 2 channel status --> 96KHz */
                        ...
                        case 8800:
                        {
                            /* change SPDIF Tx 2 channel status --> 88KHz */
                        ...
                        case 44100:
                        {
                            /* change SPDIF Tx 2 channel status --> 44.1KHz */
                            stDigitalChannelStatusTx2.u8SamplingRate = 0x00;
                            break;
                        }
                        ...
                        case 32000:
                        {
                            /* change SPDIF Tx 2 channel status --> 32KHz */
                            stDigitalChannelStatusTx2.u8SamplingRate = 0xC0;
                            break;
                        }
                        ...
                        default:
                        {
                            /* change SPDIF Tx 2 channel status --> 48KHz */
                            stDigitalChannelStatusTx2.u8SamplingRate = 0x40;
                            break;
                        }
                    }
...
MS_BOOL HAL_MAD_SetCommInfo( Audio_COMM_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    ...
    switch(infoType)
    {
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_HDMI_HWbypassmode:
        {
           if (param1)
           {
               ...
               HAL_AUDIO_WriteMaskByte(REG_AUDIO_NEW_MODE_CFG, 0x04, 0x00);
           }
           else
           {
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_ADEC1_soundmod:
            {
                ...
                switch((AUD_Dolby_SOUNDMODE)param1)
                {
                    ...
                    case AUD_Dolby_SOUNDMODE_MIXED_LR:  value = 0x3; break;
        ...
        case Audio_Comm_infoType_ADEC1_Decoder_Output_Fading:
        {
            MS_U32 u32Setting = FALSE;

            if (param1)
            {
                /* in R2 DPGA, FALSE means enable */
                u32Setting = FALSE;
            }
            else
            {
                /* in R2 DPGA, TRUE means disable */
                u32Setting = TRUE;
            }

            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DPGA_FLAG, ADEC1, u32Setting, 0);
            break;
        }
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_ADEC2_Decoder_Output_Fading:
        {
            MS_U32 u32Setting = FALSE;
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DPGA_FLAG, ADEC2, u32Setting, 0);
            break;
        }
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Set_TEE_Capibility:
            ...
            break;
#endif
        case Audio_Comm_infoType_ADEC1_set_AC4_1th_pref_lang:
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_1ST_PREF_LANG, param1, param2, 0);   //param1(dec_id)
            break;
        case Audio_Comm_infoType_ADEC1_set_AC4_2th_pref_lang:
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_2ND_PREF_LANG, param1, param2, 0);   //param1(dec_id)
            break;
        case Audio_Comm_infoType_ADEC1_set_AC4_de_gain:
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_DE_GAIN, param1, param2, 0);   //param1(dec_id)
            break;
        case Audio_Comm_infoType_ADEC1_set_AC4_MD_pres_index:
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_MD_PRESENT_INDEX, param1, param2, 0);   //param1(dec_id)
            break;
        case Audio_Comm_infoType_ADEC1_set_AC4_AD_pres_index:
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_AD_PRESENT_INDEX, param1, param2, 0);   //param1(dec_id)
            break;

        case Audio_Comm_infoType_Set_HDMI_Rx_HighBitRate:
        {
            // Set HDMI Rx High BitRate Mode
            HAL_AUDIO_HDMI_Set_RxHBRFlag(param1);
            break;
        }
...
//            result = HAL_MAD_Read_DSP_sram(REG_PTS_AccumEsCnt, DSP_MEM_TYPE_DM);
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_hdmiTx_outFreq:
        {
            if(HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL+1)&0x20)
            {
                ...
                {
                    if(g_AudioVars2->g_EdidOutViaArc == TRUE)
                        r2_hdmiTx_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SPDIFTx_SMP_RATE, ADEC1);
                    else
                ...
                {
                    if(g_AudioVars2->g_EdidOutViaArc == TRUE)
                        r2_hdmiTx_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SPDIFTx_SMP_RATE, ADEC2);
                    else
            ...
            break;
		case Audio_Comm_infoType_MMTS_KEEP_33bit_PTS_FLAG:
			result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_PARAM_REPORT_MM_TS_PTS_WITH_MSB, ADEC1);
			break;
        ...
        ********************************************************************************/
        ...
        {
            MS_U8 u8AudioCrashFlag = g_AudioVars2->g_u8AudioCrashFlag;

            g_AudioVars2->g_u8AudioCrashFlag = FALSE;

            result = (MS_U64)u8AudioCrashFlag;
...
MS_BOOL HAL_MAD_XPCM_setParam2( AUDIO_DEC_ID DecId, Audio_XPCM_Param * pParam )
{
    ...
    if(pParamXPCM->sampleRate == 22664) //patch for 88.2K sample rate
    {
        s32smprate = 88200;
    }
    else if(pParamXPCM->sampleRate == 30464) //patch for 96K sample rate
    {
        s32smprate = 96000;
    }
    else if(pParamXPCM->sampleRate == 60928) // patch for 192K sampke rate
    {
        s32smprate = 192000;
    }
    else
    {
        s32smprate = (MS_U32) pParamXPCM->sampleRate;
    }
    ...
    if ( adec_id < ADEC3 )
    {
        ...
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_SAMPLERATES, adec_id, s32smprate, 0 );
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        case Audio_ParamType_setAudioDecoder:
        {
            ...
            {
                case Audio_DEC_OPUS:        eAudioFormat = MSAPI_AUD_DVB_OPUS;        break;
                case Audio_DEC_AC4:         eAudioFormat = MSAPI_AUD_DVB_AC4;         break;
                case Audio_DEC_MPEG_H:      eAudioFormat = MSAPI_AUD_DVB_MPEG_H;      break;
                case Audio_DEC_PCM:
        ...
        case Audio_ParamType_acMode:
        {
            ...
            switch ( (AUD_Dolby_SOUNDMODE) Param )
            {
                ...
                case AUD_Dolby_SOUNDMODE_MIXED_LR:  Param = 0x3; break;
                default:                         Param = 0x0;  break;
            }
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_SOUND_MODE, adec_id, Param, 0);
            break;
        }
        ...
        case Audio_ParamType_Audio_AAC_mm_single_frame_mode:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER, adec_id, Param, 0);
            break;

        case Audio_ParamType_Multi_Channel:
        {
            if ((Param == TRUE) || (Param == FALSE))
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MULTI_CHANNEL, adec_id, Param, 0);

                HALMAD_ERROR ("%s: DecId(%d) %s Multi-Channel\n", __FUNCTION__, adec_id, Param?"Enable":"disable");
            }
            else
            {
                HALMAD_ERROR ("%s: Error! Invalid value(%u) to enable/disable Multi-Channel\n", __FUNCTION__, (unsigned int)Param);
            }
        ...
        case Audio_ParamType_SetEDIDOutputInterface:
        {
            DIGITAL_OUTPUT_TYPE eEdidOutputInterfaceType = (DIGITAL_OUTPUT_TYPE)Param;
            MS_BOOL bHdmiMuteStatus = (((HAL_AUDIO_ReadByte(M2S_MBOX_HDMI_SETTING+1) & 0x80) > 0) ? TRUE : FALSE);

            if (g_AudioVars2->g_EdidOutputInterfaceType == eEdidOutputInterfaceType)
            {
                //HALMAD_ERROR ("%s: same EDID Output Interface Type %u, so do nothing here\n", __FUNCTION__, eEdidOutputInterfaceType);
                break;
            }

            g_AudioVars2->g_EdidOutputInterfaceType = eEdidOutputInterfaceType;

            /* mute related output */
            HAL_SOUND_SetMute(AUDIO_T3_PATH_SPDIF, TRUE);
            HAL_SOUND_SetMute(AUDIO_T3_PATH_HDMI, TRUE);

            /* apply settings */
            bRet = HAL_AUDIO_SetEDIDOutputInterface(eEdidOutputInterfaceType);

            /*
             * Per STB's requirement :
             * 1. if HDMI output is configured as ARC, then SPDIF goes with ... ????
             * 2. if HDMI output is configure as regular TX, then SPDIF goes with ... ????
             */
            if (bRet == TRUE)
            {
                MS_BOOL bEdidOutViaArc = g_AudioVars2->g_EdidOutViaArc;

                if (eEdidOutputInterfaceType == DIGITAL_HDMI_ARC_OUTPUT)
                {
                    bEdidOutViaArc = TRUE;
                }
                else if (eEdidOutputInterfaceType == DIGITAL_HDMI_OUTPUT)
                {
                    bEdidOutViaArc = FALSE;
                }

                /* apply current settings if neccessary */
                if (g_AudioVars2->g_EdidOutViaArc != bEdidOutViaArc)
                {
                    g_AudioVars2->g_EdidOutViaArc = bEdidOutViaArc;
                    HAL_MAD_Monitor_DDPlus_SPDIF_Rate();
                }
            }

            /* unmute related output */
            HAL_SOUND_SetMute(AUDIO_T3_PATH_SPDIF, g_AudioVars2->g_SPDIF_MuteStatus);
            HAL_SOUND_SetMute(AUDIO_T3_PATH_HDMI, bHdmiMuteStatus);

            break;
        }

        /*
         *  Deocder : Flush PCM From PTS
         *
         *  |---- (normal) ------------>|---- (flush PCM) --->|
         *                              now          PTS
         */
        case Audio_ParamType_Decoder_FlushPcmFrom_PTS:
        {
            MS_U32 u32Pts = (MS_U32)Param;
            MS_U16 u16PtsHigh = 0;
            MS_U16 u16PtsLow = 0;

            HALMAD_ERROR ("%s: FlushPcmFrom_PTS : DecId %d, PTS 0x%08X\n", __FUNCTION__, adec_id, u32Pts);

            u16PtsHigh = (MS_U16)((u32Pts & 0xFFFF0000) >> 16);
            u16PtsLow = (MS_U16)(u32Pts & 0x0000FFFF);
            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessUntil_PTS, adec_id, u16PtsHigh, u16PtsLow);

            break;
        }

        /*
         *  Deocder : Flush PCM Immediately
         *
         *  |---- (normal) --->|---- (flush PCM) --->|
         *                              now (immediate)
         */
        case Audio_ParamType_Decoder_FlushPcmFrom_Immediately:
        {
            HALMAD_ERROR ("%s: FlushPcmFrom_Immediately : DecId %d\n", __FUNCTION__, adec_id);

            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessUntil_Immediate, adec_id, 0, 0);

            break;
        }

        case Audio_ParamType_Decoder_FlushPcmFrom_Cancel:
        {
            HALMAD_ERROR ("%s: FlushPcmFrom_Cancel : DecId %d\n", __FUNCTION__, adec_id);

            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessUntil_Cancel, adec_id, 0, 0);

            break;
        }

        /*
         *  Deocder : Flush PCM Until PTS
         *
         *  |---- (flush PCM) ------------>|---- (normal) --->|
         *                                  now          PTS
         */
        case Audio_ParamType_Decoder_FlushPcmUntil_PTS:
        {
            MS_U32 u32Pts = (MS_U32)Param;
            MS_U16 u16PtsHigh = 0;
            MS_U16 u16PtsLow = 0;

            HALMAD_ERROR ("%s: FlushPcmUntil_PTS : DecId %d, PTS 0x%08X\n", __FUNCTION__, adec_id, u32Pts);

            u16PtsHigh = (MS_U16)((u32Pts & 0xFFFF0000) >> 16);
            u16PtsLow = (MS_U16)(u32Pts & 0x0000FFFF);
            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessFrom_PTS, adec_id, u16PtsHigh, u16PtsLow);

            break;
        }

        /*
         *  Deocder : NOT to Flush PCM Immediately
         *
         *  |---- (flush PCM) --->|---- (normal) --->|
         *                                  now (immediate)
         */
        case Audio_ParamType_Decoder_FlushPcmUntil_Immediately:
        {
            HALMAD_ERROR ("%s: FlushPcmUntil_Immediately : DecId %d\n", __FUNCTION__, adec_id);

            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessFrom_Immediate, adec_id, 0, 0);

            break;
        }

        case Audio_ParamType_Decoder_FlushPcmUntil_Cancel:
        {
            HALMAD_ERROR ("%s: FlushPcmUntil_Cancel : DecId %d\n", __FUNCTION__, adec_id);

            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_ProcessFrom_Cancel, adec_id, 0, 0);

            break;
        }

        case Audio_ParamType_PUSI_Enable:
        {
            if(Param)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, adec_id, 0, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, adec_id, 1, 0);
            }
            break;
        }
    ...
    switch ( infoType )
    {
        ...
        case Audio_infoType_hashkey:
        {
            ...
            switch ( pHashkeyInfo->U32Name )
            {
                case Audio_DEC_AC4:
                {
                    if (audio_ipauth & AUDIO_HASHKEY_DISABLE_AC4)
                    {
                ...
                case Audio_DEC_MPEG_H:
                {
                    if (audio_ipauth & AUDIO_HASHKEY_DISABLE_MPEG_H)
                    {
                        pHashkeyInfo->U32IsHashkeySupported = FALSE;
                    }
                    break;
        ...
        case Audio_infoType_Is_Audio_Sync:
        {
            if ( DecId == AU_DEC_ID3 )
            {
                long long tmp = 0;
                ...
                tmp = HAL_MAD_GetCommInfo(Audio_Comm_infoType_ADEC2_1ms_timeStamp);
                if (tmp == 0)
                {
                    result = FALSE;
                    break;
                ...
                tmp = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_STATE, ADEC2)&0x0F;
                if (tmp != 2)
                {
                    result = FALSE;
                    break;
                ...
                for (i = 0;i<3;i++)
                {
                    tmp = HAL_MAD_GetCommInfo (Audio_Comm_infoType_ADEC2_33bit_STCPTS_DIFF);
                    if ((tmp >= (100*90))||(tmp <= (-100*90))) //100ms
                    {
                        no_sync_cnt ++;
                    }
        ...
        /* UART INFO */
        case Audio_infoType_R2_uart_buf_base:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UART_BUF_BASE, DecId);
            break;
        }

        case Audio_infoType_R2_uart_buf_size:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UART_BUF_SIZE, DecId);
            break;
        }


        case Audio_infoType_R2_uart_buf_wptr:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UART_BUF_WPTR, DecId);
            break;
        }
...
MS_S32 HAL_MAD_CommonCmd_TrasferData(void *pSrc, MS_U32 u32SrcSize, void *pDst, MS_U32 u32DstSzie)
{
    ...
    if (pSrc == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pSrc should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if (u32SrcSize == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! u32SrcSize should not be zero!\n", __FUNCTION__);
        return -A_EINVAL;
    }

    if (pDst == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pDst should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if (u32DstSzie == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! u32DstSzie should not be zero!\n", __FUNCTION__);
        return -A_EINVAL;
    }
...
MS_S32 HAL_MAD_SetAudioOutputDeviceSelection(AUDIO_OUTPUT_DEVICE_SELECTION_t * pAudioOutputDeviceSelection)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pAudioOutputDeviceSelection should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s(): structure version is mismatch, user is 0x%08X, driver is 0x%08X!\n", __FUNCTION__,u32UserStrucVer,u32DriverStrucVer);
        return -A_EPERM;
    }
...
MS_S32 HAL_MAD_SpecifyDigitalOutputCodec(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t * pAudioSpecifiedDigitalOutput, DIGITAL_OUTPUT_TYPE u32DigitalOutputType)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pAudioSpecifiedDigitalOutput should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }
...
MS_S32 HAL_MAD_DAP_Control(AUDIO_DAP_CONTROL_t * pDapControl)
{
    ...
    if (pDapControl == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pDapControl should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }
    ...
    if (pDapControl->u32McuCtrlEnable)
    {
        HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL, 0x02, 0x02);
    }
    else
    {
        HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL, 0x02, 0x00);
    }

    if (pDapControl->u32MultiChannelEnable)
    {
        HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL, 0x04, 0x04);
    }
    else
    {
        HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL, 0x04, 0x00);
    }
...
MS_S32 HAL_MAD_GetDecoderInfo(AUDIO_DECODER_INFO_t * pDecoderInfo)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pChannelInfo should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }
...
MS_S32 HAL_MAD_GetAC4TOC(AUDIO_AC4_TOC_t * pAC4_TOC)
{
    MS_S32 s32Ret  = 0;
    Audio_id adec_id = 0;
    ...
    if ((AUDIO_AC4_TOC_VERSION & AUDIO_MAJOR_VERSION_MASK) != (pAC4_TOC->u32StructVersion & AUDIO_MAJOR_VERSION_MASK))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s(): structure version is mismatch, user is 0x%08X, driver is 0x%08X!\n", __FUNCTION__, (unsigned int)pAC4_TOC->u32StructVersion, (unsigned int)AUDIO_AC4_TOC_VERSION);
        return -A_EPERM;
    }

    adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(pAC4_TOC->DecId);

    s32Ret = HAL_DEC_R2_Get_SHM_INFO2(R2_SHM_INFO_AC4_TOC, adec_id, (MS_U8 *)&pAC4_TOC->ac4_toc, pAC4_TOC->buf_size);
...
MS_S32 HAL_MAD_CommonCmd(AUDIO_COMMON_CMD_t * cmd)
{
    ...
    if (cmd == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! cmd should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if (!(strcmp(name, "SetAudioOutputDeviceSelection")))
    {
        s32Ret = HAL_MAD_SetAudioOutputDeviceSelection((AUDIO_OUTPUT_DEVICE_SELECTION_t *)cmd->pData);
    }
    else if (!(strcmp(name, "SpecifyDigitalOutputCodec")))
    {
    ...
    else if (!(strcmp(name, "DAP_Control")))
    {
        s32Ret = HAL_MAD_DAP_Control((AUDIO_DAP_CONTROL_t *) cmd->pData);
    }
    else if (!(strcmp(name, "GetDecoderInfo")))
    {
        s32Ret = HAL_MAD_GetDecoderInfo((AUDIO_DECODER_INFO_t *) cmd->pData);
    }
    else if (!(strcmp(name, "GetAC4TOC")))
    {
        s32Ret = HAL_MAD_GetAC4TOC((AUDIO_AC4_TOC_t *) cmd->pData);
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s(): Error! Unsupportted Common Cmd (%s)!!\n", __FUNCTION__, cmd->u8StrCmd);
        s32Ret = -A_EINVAL;
    }
