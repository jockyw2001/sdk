...
MS_U32 AUDIO_adp_Ioctl(void* pInstanceTmp, MS_U32 u32Cmd, void* const pArgs)
{
    ...
    PAUDIO_ST_DEBUG_CMD_READ    pDebugCmdReadSrc                    = NULL, pDebugCmdReadDes                        = NULL;
    PAUDIO_ST_DEBUG_CMD_WRITE   pDebugCmdWriteSrc                   = NULL, pDebugCmdWriteDes                       = NULL;
    PAUDIO_ST_SYSTEM_CONTROL    pSystemControlSrc                   = NULL, pSystemControlDes                       = NULL;
    ...
    switch(u32Cmd)
    {
        ...
        case MApi_CMD_AUDIO_RA8_Param:
            ...
            //allocate RA8 Channels,Regions,cplStart,cplQbits,FrameSize size by SN MMSDK porting audio structure "ST_MMSDK_AUDIO_COOK_INIT_INFO"
            UADP_SDT_BGN(&spt_AUDIO_tmp1[0], sizeof(MS_U16) * 5);
            UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp[0], sizeof(AUDIO_ST_RA8_Param));
            UADP_SDT_KIND(&spt_AUDIO_tmp[1], AUDIO_ST_RA8_Param, UADP_SDT_P2N, pChannels, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[2], AUDIO_ST_RA8_Param, UADP_SDT_P2N, pRegions, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[3], AUDIO_ST_RA8_Param, UADP_SDT_P2N, pcplStart, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[4], AUDIO_ST_RA8_Param, UADP_SDT_P2N, pcplQbits, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[5], AUDIO_ST_RA8_Param, UADP_SDT_P2N, pFrameSize, spt_AUDIO_tmp1);
        ...
        case MApi_CMD_AUDIO_PCMCapture_Read:
            ...
            CPY_FROM_USER((void *)&pPCMCaptureReadDes->bytes, (void *)&pPCMCaptureReadSrc->bytes, sizeof(MS_U32));

            UADP_SDT_BGN (&spt_AUDIO_tmp1[0], pPCMCaptureReadDes->bytes);
            UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);
        ...
        case MApi_CMD_AUDIO_SetAudioParam2:
            ...
            copy_from_user((void *)&pSetAudioParam2Des->eParamType, (void *)&pSetAudioParam2Src->eParamType, sizeof(MS_U32));

            if (pSetAudioParam2Des->eParamType == Audio_ParamType_CommonCmd)
            {
                AUDIO_COMMON_CMD_t sAudioCommonCmdDes;

                memset(&sAudioCommonCmdDes, 0x00, sizeof(AUDIO_COMMON_CMD_t));

                copy_from_user((void *)&sAudioCommonCmdDes, (void *)pSetAudioParam2Src->Param, sizeof(AUDIO_COMMON_CMD_t));

                if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "SetAudioOutputDeviceSelection")))
                {
                UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_OUTPUT_DEVICE_SELECTION_t));
                UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "SpecifyDigitalOutputCodec")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "SpecifyDigitalOutputCodec_Spdif")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "SpecifyDigitalOutputCodec_HdmiArc")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "SpecifyDigitalOutputCodec_HdmiTx")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "DAP_Control")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_DAP_CONTROL_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else if (!(strcmp(sAudioCommonCmdDes.u8StrCmd, "GetDecoderInfo")))
                {
                    UADP_SDT_BGN(&spt_AUDIO_tmp2[0], sizeof(AUDIO_DECODER_INFO_t));
                    UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);
                }
                else
                {
                    printk("MApi_CMD_AUDIO_SetAudioParam2: Error! Unsupportted Common Cmd (%s)!!\n", sAudioCommonCmdDes.u8StrCmd);
                    break;
                }
        ...
        case MApi_CMD_AUDIO_GetAudioInfo2:
            ...
            else if (pGetAudioInfo2Des->infoType == Audio_infoType_33bit_PTS)
            {
                UADP_SDT_BGN(&spt_AUDIO_tmp1[0], sizeof(MS_U64));
                UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);
            }
            else
            {
        ...
        case MApi_CMD_AUDIO_DebugCmdRead:
        {
            pDebugCmdReadSrc = (PAUDIO_ST_DEBUG_CMD_READ)pArgs;
            pDebugCmdReadDes = (PAUDIO_ST_DEBUG_CMD_READ)buffer_arg;
            CPY_FROM_USER((void *)&pDebugCmdReadDes->u32CmdSize, (void *)&pDebugCmdReadSrc->u32CmdSize, sizeof(MS_U32));

            UADP_SDT_BGN(&spt_AUDIO_tmp1[0], sizeof(MS_U64));
            UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp2[0], pDebugCmdReadDes->u32CmdSize);
            UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp[0], sizeof(AUDIO_ST_DEBUG_CMD_READ));
            UADP_SDT_KIND(&spt_AUDIO_tmp[1], AUDIO_ST_DEBUG_CMD_READ, UADP_SDT_P2N, pHandle, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[2], AUDIO_ST_DEBUG_CMD_READ, UADP_SDT_P2N, pCmdLine, spt_AUDIO_tmp2);
            UADP_SDT_FIN(&spt_AUDIO_tmp[3]);
        ...
        case MApi_CMD_AUDIO_DebugCmdWrite:
        {
            pDebugCmdWriteSrc = (PAUDIO_ST_DEBUG_CMD_WRITE)pArgs;
            pDebugCmdWriteDes = (PAUDIO_ST_DEBUG_CMD_WRITE)buffer_arg;
            CPY_FROM_USER((void *)&pDebugCmdWriteDes->u32CmdSize, (void *)&pDebugCmdWriteSrc->u32CmdSize, sizeof(MS_U32));

            UADP_SDT_BGN(&spt_AUDIO_tmp1[0], sizeof(MS_U64));
            UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp2[0], pDebugCmdWriteDes->u32CmdSize);
            UADP_SDT_FIN(&spt_AUDIO_tmp2[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp[0], sizeof(AUDIO_ST_DEBUG_CMD_WRITE));
            UADP_SDT_KIND(&spt_AUDIO_tmp[1], AUDIO_ST_DEBUG_CMD_WRITE, UADP_SDT_P2N, pHandle, spt_AUDIO_tmp1);
            UADP_SDT_KIND(&spt_AUDIO_tmp[1], AUDIO_ST_DEBUG_CMD_WRITE, UADP_SDT_P2N, pCmdLine, spt_AUDIO_tmp2);
            UADP_SDT_FIN(&spt_AUDIO_tmp[2]);
        ...
        case MApi_CMD_AUDIO_SYSTEM_Control:
        {
            pSystemControlSrc = (PAUDIO_ST_SYSTEM_CONTROL)pArgs;
            pSystemControlDes = (PAUDIO_ST_SYSTEM_CONTROL)buffer_arg;
            CPY_FROM_USER((void *)&pSystemControlDes->u32StrCmdLength, (void *)&pSystemControlSrc->u32StrCmdLength, sizeof(MS_U32));

            UADP_SDT_BGN(&spt_AUDIO_tmp1[0], pSystemControlSrc->u32StrCmdLength);
            UADP_SDT_FIN(&spt_AUDIO_tmp1[1]);

            UADP_SDT_BGN(&spt_AUDIO_tmp[0], sizeof(AUDIO_ST_SYSTEM_CONTROL));
            UADP_SDT_KIND(&spt_AUDIO_tmp[1], AUDIO_ST_SYSTEM_CONTROL, UADP_SDT_P2N, pStrCmd, spt_AUDIO_tmp1);
            UADP_SDT_FIN(&spt_AUDIO_tmp[2]);
