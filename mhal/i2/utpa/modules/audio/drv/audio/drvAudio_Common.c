...
void MDrv_AUDIO_CheckHashkey(void)
{
     ...
     */
    /* IP AUTH2 :
     *      ip_auth2[0] = 1: Disable Dolby MS12V1 AC4
     *      ip_auth2[1] = 1: Disable Dolby MS12V2
     *      ip_auth2[2] = 1: Disable Dolby DTS VirtualX
     *      ip_auth2[3] = 1: Disable MPEG-H
     */
    ...
    else if (!(MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DOLBY_MS11)||
               MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12_B)||
               MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12_D)||
               MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12V2)) )
    {
        ip_auth |= 0x10; /* Disable MS10_DDT */
    }
    ...
    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12V1_AC4))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash-key Support DOLBY MS12V1 AC4.\n");
        ip_auth2 &= (~0x0001);    /* Enable DOLBY MS12V1 AC4 */
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash Key Check DOLBY MS12V1 AC4 Fail, no DOLBY MS12V1 AC4 license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12V2))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash-key Support Dolby MS12V2 profile.\n");
        DolbyAACFlag = 1;
        DolbyEncFlag = 1;
        ip_auth2 &= (~0x0002);    /* Enable DOLBY MS12V2 */
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash Key Check Dolby MS12V2 Fail, no Dolby MS12V2 license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DTS_VIRTUALX))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash-key Support DTS VirtualX.\n");
        ip_auth2 &= (~0x0004);    /* Enable DTS VirtualX */
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash Key Check DTS VirtualX Fail, no DTS VirtualX license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_MPEGH))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash-key Support MPEG-H.\n");
        ip_auth2 &= (~0x0008);    /* Enable MPEG-H */
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash Key Check MPEG-H Fail, no MPEG-H license!!\n");
    }
    ...
    HAL_AUDIO_SET_IPAUTH_GROUP(AUDIO_HASHKEY_GROUP1, ip_auth);
    HAL_AUDIO_SET_IPAUTH_GROUP(AUDIO_HASHKEY_GROUP2, ip_auth2);
...
MS_BOOL MDrv_ADVSOUND_ProcessEnable(ADVSND_TYPE type)
{
    ...
    if (!ADVSND_TYPE_Compare(type, MDrv_CurrAdvSndtype))
    {
        ...
        else if (ADVSND_TYPE_Compare(type, DTS_VIRTUALX))
        {
            if(MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DTS_VIRTUALX))
            {
                status = HAL_ADVSOUND_ProcessEnable(AU_DVB2_ADVSND_DTS_VIRTUALX);
                MDrv_CurrAdvSndtype = DTS_VIRTUALX;
                MsOS_DelayTask(5);
                ULOGI("AUDIO", "\n\033[0;31m Hash-key Support : DTS VIRTUALX!!\033[0m \n");
            }
            else
            {
                ULOGI("AUDIO", "\n\033[0;31m [Warning!!]No DTS VIRTUALX license!!\033[0m \n");
                status = FALSE;
            }
        }
        else
