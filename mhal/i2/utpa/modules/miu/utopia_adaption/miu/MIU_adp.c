...
MS_U32 MIU_adp_Ioctl(void* pInstanceTmp, MS_U32 u32Cmd, void* const pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDrv_CMD_MIU_Init:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, NULL, spt_MIU_RET, NULL, 0);
            break;
        case MDrv_CMD_MIU_Mask_Req_OPM_R:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_MIU_MASK, NULL, NULL, 0);
            break;
        ...
        case MDrv_CMD_MIU_Mask_Req:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_MIUREQ_MASK, NULL, NULL, 0);
            break;
        case MDrv_CMD_MIU_UnMask_Req:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_MIUREQ_MASK, NULL, NULL, 0);
            break;
        case MDrv_CMD_MIU_SetSsc:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_SET_SSC, spt_MIU_SET_SSC, NULL, 0);
            break;
        case MDrv_CMD_MIU_SetSscValue:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_SET_SSCVALUE, spt_MIU_SET_SSCVALUE, NULL, 0);
            break;
        case MDrv_CMD_MIU_Protect:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_PROTECT_PRIVATE, spt_MIU_PROTECT_PRIVATE, NULL, 0);
            //u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_PROTECT, spt_MIU_PROTECT, NULL, 0);
            break;
        case MDrv_CMD_MIU_ProtectEx:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_PROTECTEx_PRIVATE, spt_MIU_PROTECTEx_PRIVATE, NULL, 0);
            //u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_PROTECT, spt_MIU_PROTECT, NULL, 0);
            break;

        case MDrv_CMD_MIU_GetDramType:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_GETPROTECTINFO_PRIVATE, spt_MIU_GETPROTECTINFO_PRIVATE, NULL, 0);
            break;

        case MDrv_CMD_MIU_SelMIU:
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_SELMIU, spt_MIU_SELMIU, NULL, 0);
	        break;
        }

        case MDrv_CMD_MIU_GetProtectInfo:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_GETPROTECTINFO_PRIVATE, spt_MIU_GETPROTECTINFO_PRIVATE, NULL, 0);
            //u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_GETPROTECTINFO, spt_MIU_GETPROTECTINFO, NULL, 0);
            break;
        case MDrv_CMD_MIU_SetGroupPriority:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_SETGROUPPRIORITY, spt_MIU_SETGROUPPRIORITY, NULL, 0);
            break;
        case MDrv_CMD_MIU_SetHighPriority:
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_SETHIGHPRIORITY, spt_MIU_SETHIGHPRIORITY, NULL, 0);
            break;
        ...
        {
            PMIU_DRAM_SIZE PVar;
            if(copy_from_user(&PVar,pArgs,sizeof(PMIU_DRAM_SIZE)))
            {
            }
            u32Ret = UtopiaIoctl(pInstanceTmp,u32Cmd,&PVar);
            if(copy_to_user(pArgs, &PVar, sizeof(PMIU_DRAM_SIZE)))
            {
            }
            break;
        }
        ...
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp, u32Cmd, pArgs, spt_MIU_PROTECT_ALIGN_PRIVATE, spt_MIU_PROTECT_ALIGN_PRIVATE, NULL, 0);
            break;
        }
