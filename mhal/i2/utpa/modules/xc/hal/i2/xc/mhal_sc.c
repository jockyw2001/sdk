...
static MS_U8 au8Sc1HvspFilterFc50[HAL_SC1_HVSP_SRAM_PHASE][5] =   {{0x3a,0xfe,0xb0,0x03,0x06},
...
static MS_U8 au8Sc1HvspFilterFc85[HAL_SC1_HVSP_SRAM_PHASE][5] =   {{0x17,0xa8,0x91,0x01,0x82},
...
void Hal_SC_init_riu_base(MS_VIRT u32riu_base, MS_VIRT u32PMriu_base, MS_VIRT u32EXTriu_base)
{
...
static void _Hal_SC1_LoadVSPFilter(void *pInstance)
{
    ...
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    XC_RESOURCE_PRIVATE* pXCResourcePrivate_Device0 = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(E_XC_DEVICE0)],(void**)(&pXCResourcePrivate_Device0));

    MS_U16 u16SC1VSPFilter = pXCResourcePrivate->sthal_SC.u16Sc1HvspFilterValue;
    MS_U8 *pu8filter;
    ...
    if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID)
    {
        ...
        for (i=0; i<HAL_SC1_HVSP_SRAM_PHASE; i++)
        {
            ...
            if((u16SC1VSPFilter == E_XC_UNSET)
                &&(pXCResourcePrivate_Device0->stdrvXC_MVideo._SCShared.StatusInfo[MAIN_WINDOW].stCapWin.width == 0)
                &&(pXCResourcePrivate_Device0->stdrvXC_MVideo._SCShared.StatusInfo[MAIN_WINDOW].stCapWin.height == 0))
            {
                pu8filter = au8Sc1HvspFilterFc100[0];
                pXCResourcePrivate->sthal_SC.u16Sc1HvspFilterValue = E_XC_FC100;
            }
            else
            {
                //SC0 SD input
                if((pXCResourcePrivate_Device0->stdrvXC_MVideo._SCShared.StatusInfo[MAIN_WINDOW].stCapWin.width < 1200)
                    &&(pXCResourcePrivate_Device0->stdrvXC_MVideo._SCShared.StatusInfo[MAIN_WINDOW].stCapWin.height < 700))
                {
                    if(u16SC1VSPFilter == E_XC_UNSET)
                    {
                        pu8filter = au8Sc1HvspFilterFc100[0];
                ...
                {
                    //SC0 4K2K output
                    if((pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 2000)
                        && (pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3200))
                    {
                        if(u16SC1VSPFilter == E_XC_FC50)
                        {
                            return;
                        }
                        else
                        {
                            pu8filter = au8Sc1HvspFilterFc50[0];
                            pXCResourcePrivate->sthal_SC.u16Sc1HvspFilterValue = E_XC_FC50;
                        }
                    }

                    //SC0 FHD output
                    else if((pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 1000)
                        && (pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 1600)) //consider 1680x1050 into this case
                    {
                        if(u16SC1VSPFilter == E_XC_FC50)
                        {
                            return;
                    ...
                    //SC0 HD output
                    else if((pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 700)
                        && (pXCResourcePrivate_Device0->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 1200))
                    {
                        if(u16SC1VSPFilter == E_XC_FC85)
                        {
                            return;
                        }
                        else
                        {
                            pu8filter = au8Sc1HvspFilterFc85[0];
                            pXCResourcePrivate->sthal_SC.u16Sc1HvspFilterValue = E_XC_FC85;
                        }
                    }
                    //SC0 SD output
                    else
            ...
            SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_43_L, (((MS_U16)*(pu8filter+5*i+1))<<8|(MS_U16)*(pu8filter+5*i+0)));
            SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_44_L, (((MS_U16)*(pu8filter+5*i+3))<<8|(MS_U16)*(pu8filter+5*i+2)));
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_45_L, ((MS_U16)*(pu8filter+5*i+4)), 0x00FF);
...
void Hal_SC_sw_db( void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    ...
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    MS_U32 u32_FrameBuffer_Linelimit = 0;
    if( eWindow == MAIN_WINDOW )
    {
        // refine memory write protection:
        // we reserve +4 guard-band(total 32 Bytes) for 2 frame mode
        ...
        if((gSrcInfo[MAIN_WINDOW].bInterlace == FALSE) && (gSrcInfo[MAIN_WINDOW].Status2.eFrameStoreNumber == IMAGE_STORE_2_FRAMES))
        {
            ...
            {
                u32_FrameBuffer_Linelimit = ((gSrcInfo[MAIN_WINDOW].Status2.u16PreVCusScalingDst)*2)-(((gSrcInfo[MAIN_WINDOW].Status2.u16PreVCusScalingDst)*2)*gSrcInfo[eWindow].u16InputVFreq/pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq);
            }
            else
            {
                u32_FrameBuffer_Linelimit = (gSrcInfo[MAIN_WINDOW].Status2.u16PreVCusScalingDst)*((gSrcInfo[MAIN_WINDOW].Status2.u16PreVCusScalingDst)*gSrcInfo[eWindow].u16InputVFreq/pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq);
            }
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK11_18_L,(u32_FrameBuffer_Linelimit)|BIT(15), 0x9FFF);
        }
        else
        {
...
void Hal_SC_SWDS_AddCmd(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] eWindow:%d bDynamicScalingEnable%d  \n",__func__,__LINE__,eWindow,pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow]);
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow])
        {
...
E_APIXC_ReturnValue Hal_SC_set_main_black_screen_by_mode_burst(void *pInstance, MS_BOOL bEn, EN_XC_BLACK_VIDEO_MODE eMode)
{
    ...
    if(gSrcInfo[MAIN_WINDOW].bInterlace != TRUE)
    {
        u16MemSyncSetting &= ~BIT(12);
    }
    ...
    if(bEn == FALSE)
    {
        ...
        {
            if(0 == SC_R2BYTEMSK(0, REG_SC_BK11_3F_L, BIT(10)))
            {
                SC_W2BYTEMSK(0, REG_SC_BK11_3F_L, u16MemSyncSetting, 0x1F00);
            }
            }
        }
        else
    ...
    if((eMode == E_XC_BLACK_VIDEO_MEMSYNC) && (bEn == TRUE))
    {
        ...
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK11_3F_L, u16MemSyncSetting, 0x1F00);
    }
...
void HAL_SC_SetSubLineBufferOffset(void *pInstance, MS_U16 u16Linebuffer)
{
    ...
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK20_13_L, 0x800);
...
void Hal_SC_enable_window_burst(void *pInstance, MS_BOOL bEn,SCALER_WIN eWindow)
{
    ...
    else
    {
        ...
        if(bEn)
        {
            ...
            //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK6C_01_L, 0x0, 0x1); //enable bit3
        }
        else
        {
            ...
            //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK6C_01_L, 0x1, 0x1); //enable bit3
        }
    }
...
void Hal_XC_SetFrameColor(void *pInstance, MS_U32 u32aRGB, MS_BOOL bYUV)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    MS_U32 u32YUV ;

    u32YUV = bYUV ? u32aRGB : Hal_XC_ConvertARGB2YUV(pInstance, u32aRGB);
...
E_APIXC_ReturnValue Hal_SC_Set_OSDLayer(void *pInstance, E_VOP_OSD_LAYER_SEL  eVOPOSDLayer, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
#ifdef GOP_ZORDER_SYNCPD_PATCH
    MS_S16 GOP_PD =                       -0x4A;
    MS_U16 u16GOP_MUX_Delta  =            0x4;
    MS_U16 GOP_Mux_Offset[5] =            {0x0, 0x1, 0x2, 0x3, 0x3};
    MS_U16 GOP_Zorder_Mux_Offset[5] =     {0x1, 0x1, 0x1, 0x1, 0x1};
    MS_BOOL bGOPVscalePipeDelay[6] =      {FALSE, FALSE, FALSE, TRUE, FALSE, FALSE};
    MS_U16 u16GOP_NonVS_PD_Offset =       0x5; //GOP without Vsacle might need add offset on pipedelay
    MS_U16 GOP_ZORDER_LAYER0_MUX_OFFSET = 0x8;
    MS_U16 u16HStart = 0;
    MS_U16 u16PipeDeley = 0;
    MS_U16 u16GopMux =0;
    MS_U16 u16MuxIndex = 0;
    MS_U16 u16BankOffset = 0;

    u16GopMux = GOP_R2BYTEMSK(_PK_L_(0, 0x7E), 0x1fff);
    u16HStart = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_04_L);
    for (u16MuxIndex = 0; u16MuxIndex < MAX_GOP_MUX; u16MuxIndex++)
    {
        MS_U8 u8GopIndex = (u16GopMux >> (u16MuxIndex*3))& BMASK(2:0);
        if (u8GopIndex >= MAX_GOP_SUPPORT)
        {
             // no gop on this mux
             continue;
        }
        if(eVOPOSDLayer == 0)
        {
            if(bGOPVscalePipeDelay[u8GopIndex])
            {
                u16PipeDeley = u16HStart + GOP_PD + GOP_Mux_Offset[u16MuxIndex]*u16GOP_MUX_Delta + u16GOP_NonVS_PD_Offset*2 + GOP_ZORDER_LAYER0_MUX_OFFSET;
            }
            else
            {
                u16PipeDeley = u16HStart + GOP_PD + GOP_Mux_Offset[u16MuxIndex]*u16GOP_MUX_Delta + GOP_ZORDER_LAYER0_MUX_OFFSET;
            }
        }
        else
        {
            if(bGOPVscalePipeDelay[u8GopIndex])
            {
                u16PipeDeley = u16HStart + GOP_PD + GOP_Zorder_Mux_Offset[u16MuxIndex]*u16GOP_MUX_Delta + u16GOP_NonVS_PD_Offset*2;
            }
            else
            {
                u16PipeDeley = u16HStart + GOP_PD + GOP_Zorder_Mux_Offset[u16MuxIndex]*u16GOP_MUX_Delta;
            }
        }
        u16BankOffset = u8GopIndex*3;
        GOP_W2BYTEMSK(_PK_L_(u16BankOffset, 0x0F), u16PipeDeley, BMASK(15:0));
    }
    GOP_W2BYTEMSK(_PK_L_(0x0,0x7F), 0x0100, 0x0100); //GOP all banks double buffer
    GOP_W2BYTEMSK(_PK_L_(0x0,0x7F), 0x0000, 0x0100);
#endif
...
E_APIXC_ReturnValue Hal_SC_Set_VideoAlpha(void *pInstance, MS_U8 u8Val, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_36_L, u8Val<<8, 0xFF00);
        return E_APIXC_RET_OK;
    }
    return E_APIXC_RET_FAIL;
}
...
E_APIXC_ReturnValue Hal_SC_Get_VideoAlpha(void *pInstance, MS_U8 *pu8Val, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        *pu8Val = (MS_U8)(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_36_L, 0xFF00) >> 8);
        return E_APIXC_RET_OK;
    }
    else
    {
...
static MS_U16 _Hal_SC_get_Fclk(void *pInstance)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    MS_U8 u8FClkReg = 0x00;

    if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
    {
        u8FClkReg = MDrv_ReadByte(REG_CKG_FCLK) & CKG_FCLK_MASK;

        if(CKG_FCLK_170MHZ == u8FClkReg)
        {
            return 170;
        }
        else if(CKG_FCLK_216MHZ == u8FClkReg)
        {
            return 216;
        }
        else if(CKG_FCLK_192MHZ == u8FClkReg)
        {
            return 192;
        }
        else if(CKG_FCLK_160MHZ == u8FClkReg)
        {
            return 160;
        }
        else if(CKG_FCLK_345MHZ == u8FClkReg)
        {
            return 345;
        }
        else if(CKG_FCLK_320MHZ == u8FClkReg)
        {
            return 320;
        }
        else
    ...
    {
        u8FClkReg = MDrv_ReadByte(REG_CKG_SC1_FCLK) & CKG_SC1_FCLK_MASK;

        if(CKG_SC1_FCLK_172MHZ == u8FClkReg)
        {
            return 172;
        }
        else if(CKG_SC1_FCLK_345MHZ == u8FClkReg)
        {
            return 345;
        }
        else if(CKG_SC1_FCLK_320MHZ == u8FClkReg)
        {
            return 320;
        }
        else if(CKG_SC1_FCLK_216MHZ == u8FClkReg)
        {
            return 216;
        }
        else
...
E_APIXC_ReturnValue Hal_XC_IsForcePrescaling(void *pInstance, XC_InternalStatus *pSrcInfo, MS_BOOL *pbForceV, MS_BOOL *pbForceH, SCALER_WIN eWindow)
{
    ...
    *pbForceH = FALSE;
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 2000)
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3200)
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq >= 500))
        {
            ...
            MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
            SC_DBG(printf("[%s][%d] set FCLK=320MHz\n", __FUNCTION__, __LINE__));
        }
        else
        {
            ...
            MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_320MHZ, CKG_FCLK_MASK);
            if(eWindow == SUB_WINDOW || Hal_SC_Is_subwindow_enable(pInstance) || gSrcInfo[MAIN_WINDOW].bFBL == TRUE)
            {
                //for PIP, we must use MAX clock setting
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
            }
        }
        ...
        #if 0
            ////Patch for 4Kin4Kout preview window FB mode
            if((pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow] == E_XC_3D_INPUT_MODE_NONE)&&
                (((pSrcInfo->Status2.bPreHCusScaling == TRUE)&&(pSrcInfo->Status2.u16PreHCusScalingDst <= 1920)) || ((pSrcInfo->Status2.bPreVCusScaling == TRUE)&&(pSrcInfo->Status2.u16PreVCusScalingDst <= 1088))))
            {
                printf("%s %d, 4K in 4K out, FORCE FBL!!!!\n", __FUNCTION__, __LINE__);
                gSrcInfo[MAIN_WINDOW].bFBL = FALSE;
                Hal_SC_set_fbl(pInstance, FALSE);
            }
        #endif
            return E_APIXC_RET_OK;
        }
        ...
        {
            // 4K2K@50/60Hz
            // Need force pre-scaling if disp.V size < 95% of crop.V size.
            // Remove 50Hz condition because of 4K2K30 POP case
            // In 4K2K30 POP case raw condition will error
            if((pSrcInfo->stDispWin.height * 20) < (pSrcInfo->stCropWin.height * 19))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                *pbForceV = TRUE;
                SC_DBG(printf("[%s][%d] disp.V size < 95%% of crop.V size, need force V pre-scaling, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
            else
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
        ...
        //FHD output
        else if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 1000)
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 1600)) //consider 1680x1050 into this case
        {
            ...
            // If disp.V size < 60% of crop.V size.
            else if((pSrcInfo->stDispWin.height * 5) < (pSrcInfo->stCropWin.height * 3))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                SC_DBG(printf("[%s][%d] disp.V size < 60%% of crop.V size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
            }
            // If disp.V size < 80% of crop.V size.
            else if((pSrcInfo->stDispWin.height * 5) < (pSrcInfo->stCropWin.height * 4))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_320MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_320MHZ, CKG_FCLK_MASK);
                SC_DBG(printf("[%s][%d] disp.V size < 80%% of crop.V size, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
        }
        ...
        else
        {
            ...
            // If disp.V size < 40% of crop.V size.
            else if((pSrcInfo->stDispWin.height * 5) < (pSrcInfo->stCropWin.height * 2))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                *pbForceV = TRUE;
                SC_DBG(printf("[%s][%d] disp.V size < 40%% of crop.V size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
            }
            // If disp.V size < 50% of crop.V size.
            else if((pSrcInfo->stDispWin.height * 2) < (pSrcInfo->stCropWin.height * 1))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_320MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_320MHZ, CKG_FCLK_MASK);
                SC_DBG(printf("[%s][%d] disp.V size < 50%% of crop.V size, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
        }
        ...
        {
            // 4K2K@50/60Hz
            // Need force pre-scaling if disp.H size < 95% of crop.H size.
            // Remove 50Hz condition because of 4K2K30 POP case
            // In 4K2K30 POP case raw condition will error
            if((pSrcInfo->stDispWin.width * 10) < (pSrcInfo->stCropWin.width * 9))
            {
                if(_Hal_SC_get_Fclk(pInstance) < 320)
                {
                    MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                    MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                }
                *pbForceH = TRUE;
                SC_DBG(printf("[%s][%d] disp.H size < 95%% of crop.H size, need force H pre-scaling, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
            else
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_320MHZ, CKG_FCLK_MASK);
            ...
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 1600)) //consider 1680x1050 into this case
        {
            ...
            // If disp.H size < 60% of crop.H size.
            else if((pSrcInfo->stDispWin.width * 5) < (pSrcInfo->stCropWin.width * 3))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                *pbForceH = TRUE;
                SC_DBG(printf("[%s][%d] disp.H size < 60%% of crop.H size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
            }
            // If disp.H size < 80% of crop.H size.
            else if((pSrcInfo->stDispWin.width * 5) < (pSrcInfo->stCropWin.width * 4))
            {
                if(_Hal_SC_get_Fclk(pInstance) < 320)
                {
                    MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_320MHZ, CKG_FCLK_MASK);
                ...
                SC_DBG(printf("[%s][%d] disp.H size < 80%% of crop.H size, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
        }
        // Output is smaller than FHD
        else
        {
            ...
            // If disp.H size < 40% of crop.H size.
            else if((pSrcInfo->stDispWin.width * 5) < (pSrcInfo->stCropWin.width * 2))
            {
                MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_EDCLK, CKG_EDCLK_345MHZ, CKG_FCLK_MASK);
                *pbForceH = TRUE;
                SC_DBG(printf("[%s][%d] disp.H size < 40%% of crop.H size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
            }
            // If disp.H size < 50% of crop.H size.
            else if((pSrcInfo->stDispWin.width * 2) < (pSrcInfo->stCropWin.width * 1))
            {
                if(_Hal_SC_get_Fclk(pInstance) < 320)
                {
                    MDrv_WriteByteMask(REG_CKG_FCLK, CKG_FCLK_320MHZ, CKG_FCLK_MASK);
                ...
                SC_DBG(printf("[%s][%d] disp.H size < 50%% of crop.H size, set FCLK=240MHz\n", __FUNCTION__, __LINE__));
            }
        }
        ...
        // Because scaler in "4K input ,4K output, FBL mode" case, enable fclk tuning cause post-scaling capability decrease.
        // It's need to disable fclk tuning in "4K input ,4K output, FBL mode" case.
        if((pSrcInfo->stCapWin.width * pSrcInfo->stCapWin.height) > HAL_SC_FHD_SIZE)
        {
            if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > HAL_SC_FHD_WIDTH ) &&
               (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > HAL_SC_FHD_HEIGHT))
            {
                //SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK6C_01_L, 0x0, 0x1);
            }
        }
        ...
        if((pSrcInfo->stCropWin.width * pSrcInfo->stCropWin.height) > (3840 * 2160))
        {
            if((*pbForceH) || (*pbForceV))
            {
                 //Patch : The Input source which size > 3840 * 2160 should force do prescalling
                if(pSrcInfo->stCapWin.width > 3840u)
                {
                    if(pSrcInfo->Status2.bPreVCusScaling )//if already have customer scaling,need campare with the former one
                    {
                        if(pSrcInfo->Status2.u16PreHCusScalingDst > 3840u)
                        {
                            pSrcInfo->Status2.u16PreHCusScalingDst = 3840u;
                        }
                    }
                    else
                    {
                    ...
                    if(!(pSrcInfo->Status2.u16PreHCusScalingDst == pSrcInfo->stDispWin.width))
                    {
                ...
                if(pSrcInfo->stCapWin.height > 2160u)
                {
                    if(pSrcInfo->Status2.bPreVCusScaling )//if already have customer scaling,need campare with the former one
                    {
                        if(pSrcInfo->Status2.u16PreVCusScalingDst > 2160u)
                        {
                            pSrcInfo->Status2.u16PreVCusScalingDst = 2160u;
                        }
                    }
                    else
                    {
                    ...
                    if(!(pSrcInfo->Status2.u16PreVCusScalingSrc == pSrcInfo->stDispWin.height))
                    {
                        pSrcInfo->Status2.u16PreVCusScalingSrc = pSrcInfo->stDispWin.height;
                    }
        ...
        else if((pSrcInfo->stCropWin.width * pSrcInfo->stCropWin.height) > HAL_SC_FHD_SIZE)
        {
            if((*pbForceH) || (*pbForceV))
            {
                MS_U16 u16HSizeAfterPreScaling = 0;

                //Hal_SC_SetBypassMode(pInstance, E_HAL_SC_BYPASSMODE_NORMAL);

                // IP 2P to 1P H size reduces to half
                //pSrcInfo->stCapWin.width /= 2;
                //pSrcInfo->stCropWin.width /= 2;
                u16HSizeAfterPreScaling = (*pbForceH) ?
                ...
                if(u16HSizeAfterPreScaling > HAL_SC_DI_LB_SIZE)
                {
                    ...
                    if(pSrcInfo->Status2.bPreHCusScaling )//if already have customer scaling,need campare with the former one
                    {
                        if(pSrcInfo->Status2.u16PreHCusScalingDst > HAL_SC_DI_LB_SIZE)
                        {
                ...
                if(pSrcInfo->stCapWin.height > 1080u)
                {
                    if(pSrcInfo->Status2.bPreVCusScaling )//if already have customer scaling,need campare with the former one
                    {
                        if(pSrcInfo->Status2.u16PreVCusScalingDst > 1080u)
                        {
                            pSrcInfo->Status2.u16PreVCusScalingDst = 1080u;
                        }
                    }
                    else
                    {
            ...
            {
                SC_DBG(printf("Input H size %d , max LB size %lu\n",pSrcInfo->stCapWin.width , SCALER_LINE_BUFFER_MAX));
                if((pSrcInfo->stCapWin.width > SCALER_LINE_BUFFER_MAX) ||
                    ((pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow] != E_XC_3D_INPUT_MODE_NONE) &&
                     ...
                     ((pSrcInfo->stCapWin.width > MS_3D_DISPALY_LINE_BFF_MAX) || (pSrcInfo->stCapWin.height > HAL_SC_FHD_HEIGHT))))
                {
                    SC_DBG(printf("[%s][%d] Input H size > max LB size, force pre-scaling\n", __FUNCTION__, __LINE__));
                    //Hal_SC_SetBypassMode(pInstance, E_HAL_SC_BYPASSMODE_NORMAL);
                    ...
                    SC_DBG(printf("[%s][%d] Disable 4K2K bypass mode, IP drops half pixels per line!\n", __FUNCTION__, __LINE__));
                ...
                {
                    SC_DBG(printf("[%s][%d] Input H size < max LB size, not force pre-scaling\n", __FUNCTION__, __LINE__));
                }
            }
        }
        else
    ...
    else if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID) // SC1
    {
        ...
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_320MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_320MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] set FCLK=320MHz\n", __FUNCTION__, __LINE__));
        }
        else
        {
        ...
        if((pSrcInfo->stDispWin.height * 10) < (pSrcInfo->stCropWin.height * 3))
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_345MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_345MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] disp.V size < 30%% of crop.V size, need force V pre-scaling, set FCLK=345MHz\n", __FUNCTION__, __LINE__));
        }
        // If disp.V size < 40% of crop.V size.
        else if((pSrcInfo->stDispWin.height * 5) < (pSrcInfo->stCropWin.height * 2))
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_345MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_345MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] disp.V size < 40%% of crop.V size, set FCLK=345MHz\n", __FUNCTION__, __LINE__));
        }
        // If disp.V size < 50% of crop.V size.
        else if((pSrcInfo->stDispWin.height * 2) < (pSrcInfo->stCropWin.height * 1))
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_320MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_320MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] disp.V size < 50%% of crop.V size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
        }
        // If disp.V size < 60% of crop.V size.
        else if((pSrcInfo->stDispWin.height * 5) < (pSrcInfo->stCropWin.height * 3))
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_216MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_216MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] disp.V size < 50%% of crop.V size, set FCLK=216MHz\n", __FUNCTION__, __LINE__));
        }
        ...
        else if((pSrcInfo->stDispWin.width * 5) < (pSrcInfo->stCropWin.width * 2))
        {
            MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_345MHZ, CKG_SC1_FCLK_MASK);
            MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_345MHZ, CKG_SC1_EDCLK_MASK);
            SC_DBG(printf("[%s][%d] disp.H size < 40%% of crop.H size, set FCLK=345MHz\n", __FUNCTION__, __LINE__));
        }
        // If disp.H size < 50% of crop.H size.
        else if((pSrcInfo->stDispWin.width * 2) < (pSrcInfo->stCropWin.width * 1))
        {
            ...
            SC_DBG(printf("[%s][%d] disp.H size < 50%% of crop.H size, set FCLK=320MHz\n", __FUNCTION__, __LINE__));
        }
        // If disp.H size < 40% of crop.H size.
        else if((pSrcInfo->stDispWin.width * 5) < (pSrcInfo->stCropWin.width * 3))
        {
            if(_Hal_SC_get_Fclk(pInstance) < 216)
            {
                MDrv_WriteByteMask(REG_CKG_SC1_FCLK, CKG_SC1_FCLK_216MHZ, CKG_SC1_FCLK_MASK);
                MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_216MHZ, CKG_SC1_EDCLK_MASK);
            }

            SC_DBG(printf("[%s][%d] disp.H size < 50%% of crop.H size, set FCLK=216MHz\n", __FUNCTION__, __LINE__));
        }
...
E_APIXC_ReturnValue MHal_XC_Vtrack_SetPayloadData(void *pInstance, MS_U16 u16Timecode, MS_U8 u8OperatorID)
{
    ...
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_08_L, (MS_U16)u8OperatorID, 0x00ff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0A_L, (u16Timecode << 8) , 0xff00);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0B_L, (u16Timecode >> 8) , 0x00ff);

    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_08_L, (MS_U16)u8OperatorID, 0x00ff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0A_L, (u16Timecode << 8) , 0xff00);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0B_L, (u16Timecode >> 8) , 0x00ff);
...
E_APIXC_ReturnValue MHal_XC_Vtrack_SetAllPayloadData(void *pInstance, XC_VTRACK_PAYLOAD *vtrack_payload)
{
    /*
     * reg_payload_use from 0x08 to 0x0B
     * {Operator ID[55:48], Unique ID (OTP)[47:16], TimeCode[15:0]}
     */

    //set Oprator ID
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_08_L, vtrack_payload->u16Timecode, 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_09_L, (MS_U16)((vtrack_payload->u32Vtrack_ID)&0xFFFF) , 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0A_L, (MS_U16)(((vtrack_payload->u32Vtrack_ID)&0xFFFF0000)>>16) , 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0B_L, (MS_U16)(vtrack_payload->u8OperatorID) , 0x00FF);

    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_08_L, vtrack_payload->u16Timecode, 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_09_L, (MS_U16)((vtrack_payload->u32Vtrack_ID)&0xFFFF) , 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0A_L, (MS_U16)(((vtrack_payload->u32Vtrack_ID)&0xFFFF0000)>>16) , 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0B_L, (MS_U16)(vtrack_payload->u8OperatorID) , 0x00FF);
...
E_APIXC_ReturnValue MHal_XC_Vtrack_SetVtrackKey(void *pInstance, MS_U64 u64VtrackKey)
{
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_04_L, (MS_U16)(u64VtrackKey&0xFFFF), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_05_L, (MS_U16)((u64VtrackKey&0xFFFF0000)>>16), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_06_L, (MS_U16)((u64VtrackKey&0xFFFF00000000)>>32), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_07_L, (MS_U16)((u64VtrackKey&0xFFFF000000000000)>>48), 0xffff);

    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_04_L, (MS_U16)(u64VtrackKey&0xFFFF), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_05_L, (MS_U16)((u64VtrackKey&0xFFFF0000)>>16), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_06_L, (MS_U16)((u64VtrackKey&0xFFFF00000000)>>32), 0xffff);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_07_L, (MS_U16)((u64VtrackKey&0xFFFF000000000000)>>48), 0xffff);
...
E_APIXC_ReturnValue MHal_XC_Vtrack_SetUserDefindedSetting(void *pInstance, MS_BOOL bUserDefinded, MS_U8 *pu8Setting)
{
     ...
     * setting1 [0e]              => 8'h00
     * setting2 [0f]              => 8'h00
     * setting3 [1a:10]           => 165'h04 21 08 418c6318c4 21084210842108421086318c63
     */
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    ...
    if(MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0E_L, pu8Data[0], 0xff);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_0F_L, pu8Data[1], 0xff);

        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0E_L, pu8Data[0], 0xff);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_0F_L, pu8Data[1], 0xff);
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK3D_0E_L, pu8Data[0], 0xff);
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK3D_0F_L, pu8Data[1], 0xff);

        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK3E_0E_L, pu8Data[0], 0xff);
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK3E_0F_L, pu8Data[1], 0xff);
    }

    for (u16Index = 2; u16Index < VTRACK_SETTING_LENGTH; u16Index = u16Index+2)
    {
        MS_U8 u8Offset = (( (u16Index - 2) / 2) *2 );
        MS_U32 u32Addr_h2v2 = REG_SC_BK3D_10_L + u8Offset;
        MS_U32 u32Addr_hdmi = REG_SC_BK3E_10_L + u8Offset;
        ...
        if ( (VTRACK_SETTING_LENGTH - u16Index) >= 2)
        {
            ...
            if(MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, u32Addr_h2v2 , u16Data, 0xffff); //write 16 bits
                MDrv_XC_MLoad_WriteCmd(pInstance, u32Addr_hdmi , u16Data, 0xffff); //write 16 bits
            }
            else
            {
                SC_W2BYTE(psXCInstPri->u32DeviceID, u32Addr_h2v2, u16Data);
                SC_W2BYTE(psXCInstPri->u32DeviceID, u32Addr_hdmi, u16Data);
            }
        }
        else
        {
            ...
            if(MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, u32Addr_h2v2 , u16Data, 0x00ff); //write low 8bits
                MDrv_XC_MLoad_WriteCmd(pInstance, u32Addr_hdmi , u16Data, 0x00ff); //write low 8bits
            }
            else
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr_h2v2, u16Data, 0x00ff);
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr_hdmi, u16Data, 0x00ff);
            }
        }
    }
...
{
    //FrameRateIn     => 8'h1E
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_02_L , u8FrameRate, 0xFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_02_L , u8FrameRate, 0xFF);

    //EnableIn              => 1
    //DebugEn               => 0
    if (bEnable == TRUE)
    {
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_01_L , BIT(0), BIT(1)|BIT(0) );
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_01_L , BIT(0), BIT(1)|BIT(0) );
    }
    else
    {
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3D_01_L , 0x00, BIT(1)|BIT(0) );
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK3E_01_L , 0x00, BIT(1)|BIT(0) );
    }
...
E_APIXC_ReturnValue MHal_XC_VMark_SetParameters(void* pInstance, MS_U8 u8ServiceIndex, XC_VMARK_PARAMETER *pstXC_VMarkParameters)
{
    ...
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
#else
    XC_INSTANCE_PRIVATE stXCInstPri = {0};
    psXCInstPri = &stXCInstPri;
#endif
...
#else
    MS_U16 u16HRes = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_05_L) - SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_04_L) + 1;
    MS_U16 u16VRes = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_07_L) - SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_06_L) + 1;
#endif
...
MS_BOOL Hal_SC_Init(void *pInstance)
{
    ...
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK62_05_L, 0x0200); // [14:12]hvsp_src_mux
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK62_1C_L, 0x00, BIT(0)|BIT(1)); //[1:0]hvsp_mux2ve_sel
    ...
    {
        pXCResourcePrivate->sthal_SC.u16Sc1HvspFilterValue = E_XC_UNSET; // For STR
    ...
    Hal_XC_SetFrameColor(pInstance, 0x0, 0x0);
...
//K6 is VDEC FRC, when input is 3D Interlace stream and set output timing:1920*1080_24p,
//VDEC will trancemit only TOP Field to XC. Thus XC fill Top Field to odd spaces of frame buffer
//and skip even spaces. Therefore XC de-interlace show garbage in sub window.
//To fix this problem, Invert memsyn interlace mode and let XC fill Top Field to Frame buffer in sequence
void Hal_SC_3D_InvertMemsyncInterlaceMode(void *pInstance)
{
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq == 240) //for output timing : 1920*1080 24p
    {
...
MS_BOOL Hal_SC_CheckMuteStatusByRegister(void *pInstance, SCALER_WIN eWindow)
{
    ...
    // Because of Sub window write RIU for OPM fetch will cause main window have garbage,
    // We need check all window are Mute then we can use RIU write sw_db

    //Disable input source also have garbage if sub window open and set main window

    //Do not check all window in driver layer for other chip set flow compatiable

    //Force return Mute if Main OR Sub window black screen
    UNUSED(eWindow);

    // If main and sub window all disable => return mute
    if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_10_L, BIT(1) | BIT(0)) == 0)
    {
        return TRUE;
    }

    // Check if black screen => all window black screen => return mute
    // one of windows not black screen => return unmute
    if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_20_L, BIT(2) | BIT(1)) == (BIT(2) | BIT(1)))
    {
        return TRUE;
    }
    else
...
E_XC_FB_LEVEL MHal_XC_Get_FB_Level(void* pInstance, MS_WINDOW_TYPE* pstCropWin, MS_WINDOW_TYPE* pstDispWin, MS_BOOL bInterlace, MS_U16* pu16HSize, MS_U16* pu16VSize)
{
    ...
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    E_XC_FB_LEVEL ret = E_XC_FB_LEVEL_NUM;
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        MS_BOOL bEnablePreH = FALSE;
        MS_BOOL bEnablePreV = FALSE;
        //-----------------------------------------
        // Vertical
        //-----------------------------------------
        GET_FB_LEVEL_DBG(printf("[%s][%d] V Size, Crop %d, Disp %d\n", __FUNCTION__, __LINE__,pstCropWin->height,pstDispWin->height));
        ...
        {
            // 4K2K@50/60Hz
            if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq >= 500)
            {
                // Need force pre-scaling if disp.V size < 95% of crop.V size.
                if((pstDispWin->height * 20) < (pstCropWin->height * 19))
                {
                    bEnablePreV = TRUE;
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size < 95%% of crop.V size, need force V pre-scaling\n", __FUNCTION__, __LINE__));
                }
                else
                {
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size < 95%% of crop.V size, use V post-scaling\n", __FUNCTION__, __LINE__));
                }
            }
            else
            {
                // Need force pre-scaling if disp.V size < 50% of crop.V size.
                if((pstDispWin->height * 2) < (pstCropWin->height * 1))
                {
                    bEnablePreV = TRUE;
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size < 50%% of crop.V size, need force V pre-scaling\n", __FUNCTION__, __LINE__));
                }
                else
                {
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size > 50%% of crop.V size, use V post-scaling\n", __FUNCTION__, __LINE__));
                }
            }
        }
        //FHD output
        ...
        {
            // Need force pre-scaling if disp.V size < 30% of crop.V size.
            if((pstDispWin->height * 10) < (pstCropWin->height * 3))
            {
                bEnablePreV = TRUE;
                GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size < 30%% of crop.V size, need force V pre-scaling\n", __FUNCTION__, __LINE__));
            }
            else
            {
                GET_FB_LEVEL_DBG(printf("[%s][%d] disp.V size > 30%% of crop.V size, use V post-scaling\n", __FUNCTION__, __LINE__));
            }
        }
            ...
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3200))
        {
            ...
            {
                // Need force pre-scaling if disp.H size < 95% of crop.H size.
                if((pstDispWin->width * 20) < (pstCropWin->width * 19))
                {
                    bEnablePreH = TRUE;
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size < 95%% of crop.H size, need force H pre-scaling\n", __FUNCTION__, __LINE__));
                }
                else
                {
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size < 95%% of crop.H size, use H post-scaling\n", __FUNCTION__, __LINE__));
                }
            }
            else
            {
                // Need force pre-scaling if disp.H size < 50% of crop.H size.
                if((pstDispWin->width * 2) < (pstCropWin->width * 1))
                {
                    bEnablePreH = TRUE;
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size < 50%% of crop.H size, need force H pre-scaling\n", __FUNCTION__, __LINE__));
                }
                else
                {
                    GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size > 50%% of crop.H size, use H post-scaling\n", __FUNCTION__, __LINE__));
                }
            }
        }
        //FHD output
        ...
        {
            // Need force pre-scaling if disp.H size < 50% of crop.H size.
            if((pstDispWin->width * 10) < (pstCropWin->width * 3))
            {
                bEnablePreH = TRUE;
                GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size < 30%% of crop.H size, need force H pre-scaling\n", __FUNCTION__, __LINE__));
            }
            else
            {
                GET_FB_LEVEL_DBG(printf("[%s][%d] disp.H size > 30%% of crop.H size, use H post-scaling\n", __FUNCTION__, __LINE__));
            }
        }

        if(bEnablePreH || bEnablePreV)
        {
            ret = E_XC_FB_LEVEL_FB;
        }
        else
        {
            ret = E_XC_FB_LEVEL_FBL;
        }

        //Get pre scaling size below, need refine algorithm later
        #if 0
        if((pstCropWin->width * pstCropWin->height) > (3840 * 2160))
        {
            if(bEnablePreH || bEnablePreV)
            {
                 //Patch : The Input source which size > 3840 * 2160 should force do prescalling
                if(pstCropWin->width > 3840u)
                {
                    *pu16HSize = pstDispWin->width;
                }

                if(pstCropWin->height > 2160u)
                {
                    *pu16VSize = pstDispWin->height;
                }
            }
        }
        else if((pstCropWin->width * pstCropWin->height) > HAL_SC_FHD_SIZE)
        {
            if(bEnablePreH || bEnablePreV)
            {
                MS_U16 u16HSizeAfterPreScaling = 0;
                u16HSizeAfterPreScaling = bEnablePreH ? HAL_SC_DI_LB_SIZE : pstCropWin->width;

                if(u16HSizeAfterPreScaling > HAL_SC_DI_LB_SIZE)
                {
                    *pu16HSize = HAL_SC_DI_LB_SIZE;
                    bEnablePreH = TRUE;
                    GET_FB_LEVEL_DBG(printf("[%s][%d] u16HSizeAfterPreScaling > DI LB size, force H pre-scaling\n", __FUNCTION__, __LINE__));
                }
#if 0
                if(pstCropWin->height > 1080u)
                {
                    if(pSrcInfo->Status2.bPreVCusScaling )//if already have customer scaling,need campare with the former one
                    {
                        if(pSrcInfo->Status2.u16PreVCusScalingDst > 1080u)
            ...
            {
                SC_DBG(printf("Input H size %d , max LB size %d\n",pSrcInfo->stCapWin.width , SCALER_LINE_BUFFER_MAX));
                if((pSrcInfo->stCapWin.width > SCALER_LINE_BUFFER_MAX) ||
                    ((pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow] != E_XC_3D_INPUT_MODE_NONE) &&
                     (pXCResourcePrivate->stdrvXC_3D._eOutput3DMode != E_XC_3D_OUTPUT_MODE_NONE) &&
                     ((pSrcInfo->stCapWin.width > MS_3D_DISPALY_LINE_BFF_MAX) || (pSrcInfo->stCapWin.height > HAL_SC_FHD_HEIGHT))))
                {
                    ...
                    // Temp add setting MVOP 444 output here, IP 2p to 1p need 444 input
                    //MDrv_WriteByteMask(0x10147F, 0x00, 0x01);
                    SC_DBG(printf("[%s][%d] Disable 4K2K bypass mode, IP drops half pixels per line!\n", __FUNCTION__, __LINE__));
        ...
        {
            MS_U16 u16HSizeAfterPreScaling = (bEnablePreH)?
                                            (((MS_U32)pSrcInfo->stCapWin.width * pSrcInfo->stDispWin.width + (pSrcInfo->stCropWin.width / 2)) / pSrcInfo->stCropWin.width):
                                            pSrcInfo->stCropWin.width;
...
E_APIXC_ReturnValue Hal_SC_Enable_VE_BypassPath(void *pInstance, MS_BOOL bEnable)
{
    return E_APIXC_RET_FAIL_FUN_NOT_SUPPORT;
}

void SYMBOL_WEAK MHal_XCMdbIoctl_EchoCmd(MDBCMD_CMDLINE_PARAMETER *paraCmdLine)
{
}

void SYMBOL_WEAK MHal_XCMdbIoctl_Cat(MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
}

MS_BOOL Hal_SC_IsLockFromIP(void *pInstance, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFRCEnabled
         && ((gSrcInfo[MAIN_WINDOW].stCapWin.height < gSrcInfo[eWindow].u16InputVTotal)
