...
void Hal_ACE_SetSharpness( void* pInstance, MS_BOOL bScalerWin, MS_U8 u8Sharpness)
{
    ...
    if( IS_SC1() )
    {
        SC_W2BYTEMSK(u32deviceID, REG_SC_BK19_13_L, ((MS_U16)u8Sharpness)<<8, 0x7F00); // 7 bit sharpness!

    }
    else if((MApi_XC_MLoad_GetStatus_U2(pInstance) == E_MLOAD_ENABLED) && (!Hal_ACE_GetSkipWaitVsync(pInstance, bScalerWin)))
    {
    ...
    else
    {
        if(bScalerWin == MAIN_WINDOW)
        {
            SC_W2BYTEMSK(u32deviceID, REG_SC_BK19_13_L, (u8Sharpness)<<8, 0x7F00); // 7 bit sharpness!
        }
        else
        {
            SC_W2BYTEMSK(u32deviceID, REG_SC_BK19_17_L, (u8Sharpness)<<8, 0x7F00);
        }
    }
}
...
void Hal_ACE_SetBrightness( void* pInstance, MS_BOOL bScalerWin, MS_U8 u8RedBrightness, MS_U8 u8GreenBrightness, MS_U8 u8BlueBrightness )
{
    ...
    if( IS_SC1() )
    {
        SC_W2BYTEMSK(u32deviceID,REG_SC_BK1A_0F_L, ((MS_U16)u8GreenBrightness), 0xFF);
    }
    else if(bUseMenuload)
    {
        if(bScalerWin == MAIN_WINDOW)
        {
