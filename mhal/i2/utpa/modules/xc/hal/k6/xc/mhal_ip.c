...
void Hal_SC_ip_software_reset(void* pInstance, MS_U8 u8Reset, SCALER_WIN eWindow)
{
    ...
    else if(psXCInstPri->u32DeviceID == E_XC_DEVICE1)// SC1
    {
        //MS_U8  u8CLK1Mux = MDrv_ReadByte(REG_CKG_IDCLK1); //Sub window
        MS_U8  u8CLK2Mux = MDrv_ReadByte(REG_CKG_SC1_IDCLK2); //Main window
...
//Only one output vsync in U4, so input vsync reference status from output vsync
...
void Hal_SC_ip_set_reg_usr_vspolmd(void* pInstance, MS_U8 u8Enable, SCALER_WIN eWindow )
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_21_L, u8Enable ? BIT(4) : 0, BIT(4));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_21_L, u8Enable ? BIT(4) : 0, BIT(4));
    }
}
...
void Hal_SC_ip_set_input_source( void* pInstance, MS_U8 u8InputSrcSel, MS_U8 u8SyncSel, MS_U8 u8VideoSel, MS_U8 u8isYPbPr, SCALER_WIN eWindow )
{
    ...
    {
        MS_U8 u8Sc1Ipmux = MDrv_ReadByte(0x102E06);

        // H2V2 input color is YUV format
        if((u8Sc1Ipmux & 0xF0) == 0xB0)
        {
            u8isYPbPr = TRUE;
        }
    }
...
void Hal_SC_ip_set_de_only_mode( void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow )
{
    ...
    if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
    {
        if( eWindow == MAIN_WINDOW )
        {
            ...
            MDrv_WriteByteMask(REG_DE_ONLY_F2, (bEnable ? ENABLE: DISABLE)<<7, BIT(7));
        }
        else
        {
            SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK03_03_L, (BIT(7)|(bEnable<<6)), (BIT(7)|BIT(6)) );
            MDrv_WriteByteMask(REG_DE_ONLY_F1, (bEnable ? ENABLE: DISABLE)<<6, BIT(6));
        }
    }
    else if(psXCInstPri->u32DeviceID == E_XC_DEVICE1)// SC1
    {
        ...
        {
            SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK01_03_L, (BIT(7)|(bEnable<<6)), (BIT(7)|BIT(6)) );
            MDrv_WriteByteMask(REG_CKG_SC1_SIDCLK2, (bEnable ? CKG_SC1_SIDCLK2_CLK_DVI: CKG_SC1_SIDCLK2_XTAL), CKG_SC1_SIDCLK2_MASK);
        }
    }
}
...
void Hal_SC_ip_set_coast_input( void* pInstance, MS_BOOL bInputSel, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK01_24_L, ((bInputSel<<1)|BIT(0)), 0xFF );
    }
    else
    {
        SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK03_24_L, ((bInputSel<<1)|BIT(0)), 0xFF );
    }
}
...
void Hal_SC_ip_set_IPAutoNoSignal(void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow )
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_0B_L, bEnable? BIT(4)|BIT(5):0, BIT(4)|BIT(5));
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_1D_L, bEnable? BIT(12):0, BIT(12));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_0B_L, bEnable? BIT(4)|BIT(5):0, BIT(4)|BIT(5));
...
MS_U16 Hal_SC_ip_get_auto_position_v_start( void* pInstance, SCALER_WIN eWindow )
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if(eWindow == MAIN_WINDOW)
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_12_L) & 0x1FFF);
    else
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK03_12_L) & 0x1FFF);
}
...
MS_U16 Hal_SC_ip_get_auto_position_h_start( void* pInstance, SCALER_WIN eWindow )
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if(eWindow == MAIN_WINDOW)
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_13_L) & 0x1FFF);
    else
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK03_13_L) & 0x1FFF);
}
...
MS_U16 Hal_SC_ip_get_auto_position_v_end( void* pInstance, SCALER_WIN eWindow )
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if(eWindow == MAIN_WINDOW)
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_14_L) & 0x1FFF);
    else
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK03_14_L) & 0x1FFF);
}
...
MS_U16 Hal_SC_ip_get_auto_position_h_end( void* pInstance, SCALER_WIN eWindow )
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if(eWindow == MAIN_WINDOW)
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_15_L) & 0x1FFF);
    else
        return (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK03_15_L) & 0x1FFF);
}
...
MS_U8 HAL_SC_ip_get_ipmux(void* pInstance)
{
    ...
    if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
    {
        value = (MS_U8)MDrv_ReadByte(REG_IPMUX_01_L)&0xF0;
    }
    else if(psXCInstPri->u32DeviceID == E_XC_DEVICE1)
    {
        value = (MS_U8)MDrv_ReadByte(REG_IPMUX_03_L)&0xF0;
    }
    return value;
}
void HAL_SC_ip_set_ipmux(void* pInstance, MS_U8 u8Val)
{
        ...
        (pXCResourcePrivate->sthal_Optee.op_tee_xc[SUB_WINDOW].isEnable == TRUE))
    {
        ...
        if (pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt < OP_TEE_XC_REG_MAX_CNT)
        {
            if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
            {
                ...
                pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_data[pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt].value = u8Val & 0xF0;
                pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt++;
            }
            else if(psXCInstPri->u32DeviceID == E_XC_DEVICE1)
            {
                pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_data[pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt].bk = REG_IPMUX_03_L;
                pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_data[pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt].value = u8Val & 0xF0;
                pXCResourcePrivate->sthal_Optee.op_tee_mux.regs_cnt++;
            }
            else
        ...
        (pXCResourcePrivate_GetAnotherTEEStatus->sthal_Optee.op_tee_xc[SUB_WINDOW].isEnable == FALSE)))
    {
        ...
        {
            MDrv_WriteByteMask(REG_IPMUX_03_L, u8Val,0x00F0);
        }
        else
        {
...
//F2 IP memory data format, BK12_01[10:8]
...
void Hal_SC_ip_set_handshaking_md(void* pInstance, MS_U8 u8MD, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(u8MD)
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_10_L, BIT(0)|BIT(2)|BIT(5) , BIT(0)|BIT(2)|BIT(5));

            if((enInputSourceType == INPUT_SOURCE_DTV) || (enInputSourceType == INPUT_SOURCE_STORAGE))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_10_L, 0 , BIT(3));
            }
            else if((enInputSourceType == INPUT_SOURCE_DTV2) || (enInputSourceType == INPUT_SOURCE_STORAGE2))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_10_L, BIT(3) , BIT(3));
            }

            // SC1 handshake mode patch, need to disable force outack
            if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID)
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_68_L, 0 , BIT(9));
            }
        }
        else
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_10_L, 0 , BIT(0)|BIT(2)|BIT(5));
        }
    }
    else
    {
        if(u8MD)
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK04_10_L, BIT(0)|BIT(2)|BIT(5) , BIT(0)|BIT(2)|BIT(5));

            if((enInputSourceType == INPUT_SOURCE_DTV) || (enInputSourceType == INPUT_SOURCE_STORAGE))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK04_10_L, BIT(3) , BIT(3));
            }
            else if((enInputSourceType == INPUT_SOURCE_DTV2) || (enInputSourceType == INPUT_SOURCE_STORAGE2))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK04_10_L, 0 , BIT(3));
            }
        }
        else
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK04_10_L, 0 , BIT(0)|BIT(2)|BIT(5));
        }
    }
}
...
void Hal_SC_ip_Set_TestPattern(void* pInstance, MS_U16 u8Enable, MS_U32 u6Pattern_type, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_40_L, u8Enable, 0xFF);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_42_L, u6Pattern_type);
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_40_L, u8Enable, 0xFF);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK03_42_L, u6Pattern_type);
    }
}
...
void Hal_SC_set_skp_fd(void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        if(bEnable)
        {
            if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_04_L, (BIT(10)|BIT(3))) == (BIT(10)|BIT(3)))
            {
                //RFBL Y8M4 Mode Without Film
                u8Val = BIT(4);
            }
            else if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_04_L, BIT(12)) == BIT(12))
            {
                //RFBL with film
                u8Val = BIT(5)|BIT(4);
            }
        ...
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK11_3F_L,u8Val, 0x00F0);
    }
    else
    {
        if(bEnable)
        {
            if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_44_L, (BIT(10)|BIT(3))) == (BIT(10)|BIT(3)))
            {
                //RFBL Y8M4 Mode Without Film
                u8Val = BIT(4);
            }
            else if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_44_L, BIT(12)) == BIT(12))
            {
                //RFBL with film
                u8Val = BIT(5)|BIT(4);
            }
        ...
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK11_7F_L,u8Val, 0x00F0);
    }
...
/******************************************************************************/
///Get xc fd mask master switch status by window
///@return MS_BOOL fd mask master switch on/off
/******************************************************************************/
...
/******************************************************************************/
///Get fd mask read back status from decoder
...
MS_BOOL HAL_SC_ip_Get_FD_Mask_Status(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_67_L, BIT(4)))
	        return TRUE;
	    else
	        return FALSE;
    }
	...
	{
        if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_67_L, BIT(5)))
	        return TRUE;
	    else
	        return FALSE;
	}
...
void HAL_SC_ip_set_hv_sync_status_check(void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, bEnable << 6, BIT(6));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, bEnable << 6, BIT(6));
    }
}
...
void HAL_SC_ip_set_new_mode_interlaced_detect(void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, bEnable, BIT(0));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, bEnable, BIT(0));
    }
}
...
void HAL_SC_ip_set_hv_sync_source_select(void* pInstance, MS_U8 u8Source, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, (u8Source & 0x3) << 1, BIT(2)|BIT(1));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, (u8Source & 0x3) << 1, BIT(2)|BIT(1));
    }
}
...
void HAL_SC_ip_set_vtotal_count_by_pixel_clock(void* pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, bEnable << 3, BIT(3));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, bEnable << 3, BIT(3));
    }
}
...
void HAL_SC_ip_set_vsync_invert(void* pInstance, MS_BOOL bInvert, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, bInvert << 4, BIT(4));
    }
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, bInvert << 4, BIT(4));
    }
}
...
void HAL_SC_ip_set_hsync_invert(void* pInstance, MS_BOOL bInvert, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_48_L, bInvert << 5, BIT(5));
}
    else
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK03_48_L, bInvert << 5, BIT(5));
    }
}
