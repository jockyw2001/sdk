...
#define MHL_CHIP_VERIFY_REPEATER    0
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_CheckPortSelectMux(MS_U8 ucPortSelect, MS_U8 ucHDMIInfoSource)
{
...
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_CheckYUV420PortSelect()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_CheckYUV420PortSelect(MS_U8 ucHDMIInfoSource)
{
...
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_CheckRGB444PortSelect()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_CheckRGB444PortSelect(MS_U8 ucHDMIInfoSource)
{
    MS_BOOL bRGB444Flag = FALSE;
    ...
    if(R2BYTEMSK(REG_HDMI_DUAL_0_40_L +usHDMIBankOffset, BMASK(6:5)) == 0x00)
    {
        bRGB444Flag = TRUE;
    }
...
MS_BOOL _Hal_tmds_GetClockStableFlag(E_MUX_INPUTPORT enInputPortType)
{
    ...
    {
        case INPUT_PORT_DVI0:
            //bStatusFlag = ((R2BYTE(REG_PM_SLEEP_3F_L) &BIT(14)) ?FALSE: TRUE);//Monet
            bStatusFlag = ((R2BYTE(REG_PM_SLEEP_42_L) &BIT(14)) ?FALSE: TRUE);//Kano
            break;
...
void _Hal_tmds_ClockStatusInitial(E_MUX_INPUTPORT enInputPortType, MS_U8 ucMHLSupportPath)
{
    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0:
            ...
            W2BYTEMSK(REG_PM_SLEEP_2F_L, BMASK(7:4)| BIT(1), BMASK(7:0));
            //W2BYTEMSK(REG_PM_SLEEP_3D_L, TMDS_CLOCK_CMP_VALUE0, BMASK(11:0));;//Monet
            //W2BYTEMSK(REG_PM_SLEEP_3E_L, TMDS_CLOCK_CMP_VALUE1, BMASK(11:0));//Monet
            W2BYTEMSK(REG_PM_SLEEP_40_L, TMDS_CLOCK_CMP_VALUE0, BMASK(11:0));//Kano
            W2BYTEMSK(REG_PM_SLEEP_41_L, TMDS_CLOCK_CMP_VALUE1, BMASK(11:0));//Kano
            W2BYTEMSK(REG_PM_SLEEP_00_L, 0, BIT(1));// Clock status mask
            W2BYTEMSK(REG_PM_SLEEP_02_L, BIT(1), BIT(1));// Clear clock status
            W2BYTEMSK(REG_PM_SLEEP_03_L, 0, BIT(1));
            break;
        /*//Kano only have one port
        case INPUT_PORT_DVI1:
    ...
    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0:
            if(bHDMI20Flag)
            {
                ...
                W2BYTEMSK(REG_COMBO_PHY1_P0_4C_L, TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
            else
            {
                // B/G/R channel EQ parameter
        ...
        case INPUT_PORT_DVI1:
            if(bHDMI20Flag)
            {
                ...
                W2BYTEMSK(REG_COMBO_PHY1_P1_4C_L, TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
            else
            {
                // B/G/R channel EQ parameter
        ...
        case INPUT_PORT_DVI2:
            if(bHDMI20Flag)
            {
                ...
                W2BYTEMSK(REG_COMBO_PHY1_P2_4C_L, TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
            else
            {
                // B/G/R channel EQ parameter
        ...
        case INPUT_PORT_DVI3:
            if(bHDMI20Flag)
            {
                ...
                W2BYTEMSK(REG_COMBO_PHY1_P3_4C_L, TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
            else
            {
                // B/G/R channel EQ parameter
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_GetYUV420Flag(E_MUX_INPUTPORT enInputPortType, MS_U32 *ulPacketStatus, MS_BOOL bHDMIModeFlag)
{
    MS_BOOL bYUV420Flag = FALSE;
    ...
    {
        if(_Hal_tmds_CheckPortSelectMux(HDMI_GET_PORT_SELECT(enInputPortType), ucHDMIInfoSource))
        {
            if(ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AVI_PACKET_RECEIVE_FLAG) // AVI packet receive
            {
                bYUV420Flag = _Hal_tmds_CheckYUV420PortSelect(ucHDMIInfoSource);
            }
            else if(!bHDMIModeFlag) // If DVI mode check RGB444
            {
                if(!_Hal_tmds_CheckRGB444PortSelect(ucHDMIInfoSource))
                {
                    Hal_HDMI_pkt_reset(enInputPortType, REST_Y_COLOR);
                }
            }

            if(ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_SAMPLE_PACKET_RECEIVE_FLAG)
            {
                if(ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_ACR_PACKET_RECEIVE_FLAG)
                {
...
MS_BOOL _Hal_tmds_GetErrorCountStatus20(E_MUX_INPUTPORT enInputPortType)
{
    ...
    else if((usSteerErrorStatus &BMASK(7:4)) != 0x40)
    {
        bErrorFlag = TRUE;
    }
    else if((usSteerErrorStatus &BMASK(11:8)) != 0x400)
    {
        bErrorFlag = TRUE;
    }
...
//                  
//**************************************************************************
void _Hal_tmds_AutoEQ14ModeChange(E_MUX_INPUTPORT enInputPortType, MS_U8 ucModeType, MS_BOOL HDMI20Flag)
{
    ...
    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0:
            ...
            if(HDMI20Flag)
            {
                W2BYTEMSK(REG_COMBO_PHY1_P0_4C_L, bSettingFlag2? TMDS_FINE_TUNE_UNDER_20_THRESHOLD1: TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
        ...
        case INPUT_PORT_DVI1:
            ...
            if(HDMI20Flag)
            {
                W2BYTEMSK(REG_COMBO_PHY1_P1_4C_L, bSettingFlag2? TMDS_FINE_TUNE_UNDER_20_THRESHOLD1: TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
        ...
        case INPUT_PORT_DVI2:
            ...
            if(HDMI20Flag)
            {
                W2BYTEMSK(REG_COMBO_PHY1_P2_4C_L, bSettingFlag2? TMDS_FINE_TUNE_UNDER_20_THRESHOLD1: TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
        ...
        case INPUT_PORT_DVI3:
            ...
            if(HDMI20Flag)
            {
                W2BYTEMSK(REG_COMBO_PHY1_P3_4C_L, bSettingFlag2? TMDS_FINE_TUNE_UNDER_20_THRESHOLD1: TMDS_FINE_TUNE_UNDER_20_THRESHOLD, BMASK(15:0));
            }
...
//
//**************************************************************************
void _Hal_tmds_CheckHDMI20Status(E_MUX_INPUTPORT enInputPortType, MS_U32 *ulPacketStatus, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    MS_BOOL bAutoEQTrigger = FALSE;
    ...
    if(bDEStableFlag)
    {
        if(_Hal_tmds_GetYUV420Flag(enInputPortType, ulPacketStatus, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMIModeFlag))
        {
        ...
        else // DVI mode
        {
            if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMIModeFlag)
            {
                ...
                Hal_HDMI_pkt_reset(enInputPortType, REST_PIXEL_REPETITION);
            }
        }
        ...
        do
        {
            switch(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQState)
            {
                case HDMI_AUTO_EQ_START:
                    ...
                    stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usAutoEQSwitchModeCnt = 0;
                    break;
                ...
                case HDMI_AUTO_EQ_CHECK_STABLE:
                    ...
                    {
                        _Hal_tmds_AutoEQ14ModeChange(enInputPortType, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQ14Mode, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag);
                ...
                case HDMI_AUTO_EQ_20_CHANGE_SETTING:
                    _Hal_tmds_AutoEQ14ModeChange(enInputPortType, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQ14Mode, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag);
                ...
                case HDMI_AUTO_EQ_20_CHECK_STATUS:
                    ...
                    else if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCheckErrorTimes < TMDS_20_CHECK_ERROR_TIMES)
                    {
                        ...
                        {
                            if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usAutoEQSwitchModeCnt < TMDS_20_ERROR_HAPPEN_COUNT)
                            {
                                _Hal_tmds_SetErrorCountStatus(enInputPortType, TRUE);
                                stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usAutoEQSwitchModeCnt++;
                            }
                            else
                            {
                                stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQState = HDMI_AUTO_EQ_20_CHANGE_SETTING;
                                stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usAutoEQSwitchModeCnt = 0;
                                stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCheckErrorTimes = 0;
                                bAutoEQContinueFlag = TRUE;
...
//
//**************************************************************************
void _Hal_tmds_ClearEDRVaildFlag(MS_U8 ucHDMIInfoSource)
{
    if(ucHDMIInfoSource == HDMI_INFO_SOURCE0)
    {
    ...
    // Clear status
    W2BYTEMSK(REG_HDMI2_DUAL_0_27_L, BIT(11), BIT(11)); // clear interrupt Dolby EDR Valid
	W2BYTEMSK(REG_HDMI2_DUAL_0_27_L, 0, BIT(11));

	W2BYTEMSK(REG_HDMI3_DUAL_0_2F_L, BIT(13)|BIT(15), BIT(13)|BIT(15)); // clr Dolby EDR enable
	W2BYTEMSK(REG_HDMI3_DUAL_0_2F_L, 0, BIT(13)|BIT(15));
}
...
//
//**************************************************************************
void _Hal_tmds_PowerControlHandler(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo, MS_BOOL bClockStableFlag)
{
    MS_U8 ucHDMIInfoSource = 0;

    if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bNoInputFlag) // Wait clock stable
    {
        if(_Hal_tmds_GetClockValidFlag(enInputPortType, FALSE) || bClockStableFlag) // Clock stable, then set wait clock unstable
        {
    ...
    {
        if(_Hal_tmds_GetClockValidFlag(enInputPortType, TRUE) || (!bClockStableFlag)) // Clock unstable, then set wait clock stable
        {
...
//
//**************************************************************************
void _Hal_tmds_Bypass422to444(void)
{
    //if(R2BYTE(REG_PM_EFUSE_08_L) &BIT(6)) // verify trim efuse to disable Dolby HDR
    //{
        W2BYTEMSK(REG_HDMI2_DUAL_0_20_L, 0, BIT(0)); // Disable bypass, 422to444 at HDMI
    //}
    //else
    //{
        //W2BYTEMSK(REG_HDMI2_DUAL_0_20_L, BIT(0), BIT(0)); // bypass, 422to444 at scaler
    //}
}
...
void _Hal_HDCP22_SetReAuthBit(MS_U8 ucPortIdx, MS_BOOL bEnReAuth)
{
    MS_U32 dwBKOffset = 0;

    if (bEnReAuth == TRUE)
    {
        W2BYTEMSK(REG_HDCP_DUAL_P0_65_L + dwBKOffset, BIT(10), BIT(10)); //re-auth overwrite enable
        W2BYTEMSK(REG_HDCP_DUAL_P0_65_L + dwBKOffset, BIT(11), BIT(11));
        W2BYTEMSK(REG_HDCP_DUAL_P0_65_L + dwBKOffset, 0x00, BIT(10)); //clear overwrite
    }
    else
    {
        W2BYTEMSK(REG_HDCP_DUAL_P0_65_L + dwBKOffset, BIT(12), BIT(12)); //clear
    }
}

#if(MHL_CHIP_VERIFY_REPEATER)
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_HDCPWriteX74()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void _Hal_tmds_HDCPWriteX74(E_MUX_INPUTPORT enInputPortType, MS_U8 ucAddress, MS_U8 ucLength, MS_U8 *ucData)
{
    MS_U8 uctemp = 0;
    ...
    if(ucData != NULL)
    {
        ...
        HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_17_L +u16bank_offset, ucAddress, BMASK(9:0)); // address
        HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_19_L +u16bank_offset, BIT(5), BIT(5)); // trigger latch address

        for(uctemp = 0; uctemp < ucLength; uctemp++)
        {
            HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_18_L +u16bank_offset, ucData[uctemp], BMASK(7:0)); // data
...
//
//**************************************************************************
void _Hal_tmds_HDCPVerifyProc(void)
{
    MS_U8 uctemp = 0;
    MS_U8 ucBuffer[30] = {0};

    // Test V prime 0x20~0x33
    for(uctemp = 0; uctemp < 19; uctemp++)
    {
        ucBuffer[uctemp] = (uctemp *uctemp) %100 +1;
    }

    _Hal_tmds_HDCPWriteX74(INPUT_PORT_DVI0, 0x20, 19, ucBuffer);
    ...
    _Hal_tmds_HDCPWriteX74(INPUT_PORT_DVI0, 0x40, 3, ucBuffer);

    // Test Ksv FIFO
    for(uctemp = 0; uctemp < 19; uctemp++)
    {
        ucBuffer[uctemp] = 100 -(uctemp *uctemp) %100;
    }

    Hal_HDCP_WriteKSVList(INPUT_PORT_DVI0, ucBuffer, 19);
...
void Hal_HDCP22_SetReadyBit(MS_U8 ucPortIdx, MS_BOOL bIsReady)
{
    MS_U32 dwBKOffset = 0;

    W2BYTEMSK(REG_HDCP_DUAL_P0_62_L + dwBKOffset, (bIsReady ? BIT(7) : BIT(6)), BIT(6)|BIT(7));
}
...
MS_BOOL Hal_HDCP22_RecvMsg(MS_U8 ucPortIdx, MS_U8 *ucMsgData)
{
    ...
    {
        //clear length
        W2BYTEMSK(REG_HDCP_DUAL_P0_63_L + dwBKOffset, 0x00, 0x7FF); // write length
...
{
    MS_U16 u16hdcp_status = 0;
    MS_U32 u16bank_offset = 0;
...
void Hal_HDCP_initproductionkey( MS_U8 * pu8HdcpKeyData )
{
    ...
    for(u16loop = INPUT_PORT_DVI0; u16loop <= INPUT_PORT_DVI_END; u16loop++)
    {
        switch(u16loop)
        {
            default:
            case 0: u16bank_offset = 0x0000; break; // BK 0x171200
            //fix coverity error; logically dead code due to kano only has 1 Rx port;
...
// Description:
//=========================================================//
void Hal_HDMI_init(MS_BOOL bImmeswitchSupport, MS_U8 ucMHLSupportPath)
{
    ...
    for(port_index = INPUT_PORT_DVI0; port_index <= INPUT_PORT_DVI_END; port_index++)
    {
        ...
        //Enable write offset
        W2BYTEMSK(REG_HDCP_DUAL_P0_68_L +u16bank_offset, BIT(15)| TMDS_HDCP2_SOURCE_READ_OFFSET, BIT(15)| BMASK(9:0)); //b'[0..9]offset; b'[15]: enable bit
    }

    // GPIO disable
    W2BYTEMSK(REG_PM_SLEEP_73_L, BIT(1) , BIT(1)| BIT(6)); // reg_hplugc_pu1k
    ...
    W2BYTEMSK(REG_HDMI_DUAL_0_06_L, (0x6E<<8)| BIT(6), HBMASK| BIT(6));
...
{
    W2BYTEMSK(REG_COMBO_GP_TOP_4A_L, u16MuteEvent, u16MuteMask); // combo_gp_top_4A[7:0]: audio mute event to vivaldi
}
...
{
    MS_U8 ucMuteEvent = R2BYTE(REG_COMBO_GP_TOP_4A_L) &BMASK(7:0);

    if(ucMuteEvent & HDMI_CTSN_OVERRANGE)
    {
        if(R2BYTE(REG_HDMI_DUAL_0_02_L) & BIT(6))
        {
            //load current CTS/N value first
            W2BYTE(REG_HDMI_DUAL_0_12_L, 0xFFFF); // hdmi_dual_12[15:0]: CTS[15:0] from ACR packet
            W2BYTE(REG_HDMI_DUAL_0_13_L, 0xFFFF); // hdmi_dual_13[15:0]: N[15:0] from ACR packet
            W2BYTEMSK(REG_HDMI_DUAL_0_14_L, 0xFF, 0xFF); // hdmi_dual_14[7:4]: N[19:16]; [3:0]: CTS[19:16] from ACR packet

            //update CTS value
            W2BYTE(REG_HDMI_DUAL_0_0A_L, R2BYTE(REG_HDMI_DUAL_0_12_L)); // hdmi_dual_0A[15:0]: Limited CTS[15:0]
            W2BYTEMSK(REG_HDMI_DUAL_0_09_L, (R2BYTE(REG_HDMI_DUAL_0_14_L) & 0x0F) << 8, 0x0F00); // hdmi_dual_09[11:8]: Limited CTS[19:16]

            //update N value
            W2BYTE(REG_HDMI_DUAL_0_0B_L, R2BYTE(REG_HDMI_DUAL_0_13_L)); // hdmi_dual_0B[15:0]: Limited N[15:0]
            W2BYTEMSK(REG_HDMI_DUAL_0_09_L, (R2BYTE(REG_HDMI_DUAL_0_14_L) & 0xF0) << 8, 0xF000); // hdmi_dual_09[15:12]: Limited N[19:16]

            //setup CTS/N range
            W2BYTE(REG_HDMI_DUAL_0_0C_L, 0xFFFF); // hdmi_dual_0C[15:8]: N range; [7:0]: CTS range

            //upload new CTS/N value
            W2BYTEMSK(REG_HDMI_DUAL_0_07_L, BIT(15), BIT(15)); // hdmi_dual_07[15]: reg_update_lmt_ctsn

            //clear received CTS/N over range status
            W2BYTEMSK(REG_HDMI_DUAL_0_02_L, BIT(6), BIT(6)); // hdmi_dual_02[6]: reg_ctsn_ov_range
        }
    }

    if(ucMuteEvent & HDMI_CLOCK_BIG_CHANGE)
    {
        if(R2BYTE(REG_HDMI_DUAL_0_02_L) & BIT(7))
        {
            //clear HDMI video clock freqency big change
            W2BYTEMSK(REG_HDMI_DUAL_0_02_L, BIT(7), BIT(7)); // hdmi_dual_02[7]: reg_vclk_big_chg
        }
    }
    ...
    {
        if(R2BYTE(REG_COMBO_GP_TOP_49_L) & BIT(4))
        {
            //clear flat audio sample
            W2BYTEMSK(REG_COMBO_GP_TOP_49_L, BIT(4), BIT(4)); // combo_gp_top_49[4]: reg_flat_asmp_rcvd
        }
    }

    if(ucMuteEvent &  HDMI_AUDIO_PRESENT_BIT_ERROR)
    {
        if(R2BYTE(REG_HDMI_DUAL_0_04_L) & BIT(10))
        {
            //clear audio sample packet present bit error
            W2BYTEMSK(REG_HDMI_DUAL_0_04_L, BIT(10), BIT(10)); // hdmi_dual_04[10]: reg_asp_prsnt_bit_err
        }
    }

    if(ucMuteEvent & HDMI_AUDIO_FIFO_OVERFLOW_UNDERFLOW)
    {
        if(R2BYTE(REG_COMBO_GP_TOP_49_L) & (BIT(9)|BIT(8)))
        {
            //clear audio fifo overflow/underflow
            W2BYTEMSK(REG_COMBO_GP_TOP_49_L, BIT(9)|BIT(8), BIT(9)|BIT(8)); // combo_gp_top_49[9]: reg_afifo12_over; [8]: reg_afifo12_under
        }
    }

    if(ucMuteEvent & HDMI_AUDIO_SAMPLE_PARITY_ERROR)
    {
        if(R2BYTE(REG_HDMI_DUAL_0_04_L) & (BIT(5)|BIT(4)))
        {
            //clear audio sample parity error
            W2BYTEMSK(REG_HDMI_DUAL_0_04_L, BIT(5)|BIT(4), BIT(5)|BIT(4)); // hdmi_dual_04[5]: reg_as_pbit_err; [4]: Audio Sample packet receive Error occurred; sample repeated
        }
    }
}
...
MS_U32 Hal_HDMI_packet_info(MS_U8 ucHDMIInfoSource)
{
    ...
    usPacketStatus = R2BYTE(REG_HDMI_DUAL_0_01_L);
    ...
    usPacketStatus = R2BYTE(REG_HDMI2_DUAL_0_26_L) &BIT(3);
...
MS_BOOL Hal_HDMI_get_packet_value(MS_U8 ucHDMIInfoSource, MS_HDMI_PACKET_STATE_t u8state, MS_U8 u8byte_idx, MS_U8 *pu8Value)
{
    ...
    switch(u8state)
    {
        ...
        case PKT_HDR:
            ...
            if(u8byte_idx > 29)
                return FALSE;
            u32Base_add = REG_HDMI2_DUAL_0_60_L;
            break;
        default:
            break;
    }
...
{
    DDC_WRITE_MASK(REG_DDC_76_L,BITS(15:15,1),BMASK(15:15));//Kano only
...
void Hal_HDMI_ARC_PINControl(E_MUX_INPUTPORT enInputPortType, MS_BOOL bEnable, MS_BOOL bDrivingHigh)
{
    switch(enInputPortType)
    {
        ...
        case INPUT_PORT_DVI2: // C
        case INPUT_PORT_DVI3: // D
			W2BYTEMSK(REG_CHIP_0B_L, bEnable ? BIT(4) : 0, BIT(4)); //RX PAD_ARC MODE
			W2BYTEMSK(REG_CHIP_0B_H, bEnable ? BIT(0) : 0, BIT(0)); //TX PAD_ARC MODE
            break;
        default:
            break;
    }
    ...
    // Driving current
    W2BYTEMSK(REG_CHIP_28_L, bDrivingHigh ? BIT(0) : 0, BIT(0));//RX Driving Strength
    W2BYTEMSK(REG_CHIP_28_L, bDrivingHigh ? BIT(2) : 0, BIT(2));//TX Driving Strength
}
...
//
//**************************************************************************
void Hal_HDMI_StablePolling(MS_U8 ucMHLSupportPath, MS_U32 *ulPacketStatus, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    MS_BOOL bClockStableFlag = FALSE;
    E_MUX_INPUTPORT enPortIndex = INPUT_PORT_DVI0;
    ...
    {        
        ...
        {
            bClockStableFlag = _Hal_tmds_GetClockStableFlag(enPortIndex);

#if(!TMDS_POWER_SWITCH_IRQ_ENABLE)
            _Hal_tmds_PowerControlHandler(enPortIndex, stHDMIPollingInfo, bClockStableFlag);
#endif

            if(bClockStableFlag)
            {
                _Hal_tmds_CheckHDMI20Status(enPortIndex, ulPacketStatus, stHDMIPollingInfo);
            }
...
//
//**************************************************************************
void Hal_HDMI_DataRtermControl(E_MUX_INPUTPORT enInputPortType, MS_BOOL bDataRtermEnable)
{
    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0://A
            W2BYTEMSK(REG_PM_SLEEP_4C_L, bDataRtermEnable? 0: BMASK(3:1), BMASK(3:1));
            break;
        case INPUT_PORT_DVI1://B
            W2BYTEMSK(REG_PM_SLEEP_4C_L, bDataRtermEnable? 0: BMASK(7:5), BMASK(7:5));
            break;
        case INPUT_PORT_DVI2://C
            W2BYTEMSK(REG_PM_SLEEP_4C_L, bDataRtermEnable? 0: BMASK(11:9), BMASK(11:9));
            break;
        case INPUT_PORT_DVI3://D
            W2BYTEMSK(REG_PM_SLEEP_4C_L, bDataRtermEnable? 0: BMASK(15:13), BMASK(15:13));
            break;
        default:
...
void Hal_HDCP_WriteKSVList(E_MUX_INPUTPORT enInputPortType, MS_U8* pucData, MS_U32 ulDataLen)
{
	...
	W2BYTEMSK(REG_COMBO_GP_TOP_40_L, BIT(2)| BIT(0), BMASK(2:0)); //enable XIU write
    W2BYTEMSK(REG_HDCPKEY_04_L + ucRegOffset, TMDS_HDCP2_SOURCE_READ_OFFSET, 0x7FF); //set address
