...
static PNL_TYPE _TransToDriver_APIPNL_LINK_EXT_TYPE_To_PNL_TYPE(APIPNL_LINK_EXT_TYPE enLPLL_TypeExt)
{
    ...
    switch(enLPLL_TypeExt)
    {
        ...
        case LINK_ISP_10BIT_6P_D:
            enDriverLinkType = E_PNL_LPLL_ISP_10BIT_6P_D;
            break;

        case LINK_ISP_10BIT_12P:
            enDriverLinkType = E_PNL_LPLL_ISP_10BIT_12P;
            break;

        case LINK_ISP_10BIT_8P:
            enDriverLinkType = E_PNL_LPLL_ISP_10BIT_8P;
            break;

        case LINK_ISP_8BIT_8P:
            enDriverLinkType = E_PNL_LPLL_ISP_8BIT_8P;
            break;
    }
    return enDriverLinkType;
}
   ...
   // #endif
    ...
    if( (pPNLResourcePrivate->stdrvPNL._stPnlInitData.u32PNL_MISC & E_APIPNL_MISC_PIXELSHIFT_ENABLE) == 0 )
    {
        ...
        else
        {
            ...
            pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16VStart = pSelPanelType->m_wPanelVStart;   // devPanel_VSTART(); // DE V start
        }
    }
    else
    {
    ...
    //#TCON 2.0 adjust panellinktype
    if(stTCONPanelConfig.bUsingTCON)
    {
        pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type = _TransToDriver_APIPNL_LINK_TYPE_To_PNL_TYPE(stTCONPanelConfig.stPanelInfo.u32PanelLinkType);
    }
    else
    {
    ...
    if (E_PNL_TYPE_EXT == pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type)
    {
        if(stTCONPanelConfig.bUsingTCON)
        {
            pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type = stTCONPanelConfig.stPanelInfo.u32PanelLinkExtType;
        }
        else
        {
    ...
    //#TCON 2.0 , adjust position of VDE start
    if( (E_PNL_LPLL_ISP_8BIT_12P == pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type)||
             (E_PNL_LPLL_EPI28_12P == pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type) )
    {
        if(stTCONPanelConfig.bUsingTCON)
        {
            pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16VStart = stTCONPanelConfig.stPanelInfo.u32VsyncStart;
        }
        else
        {
            //For specific TCON panel and not using TCON2.0 flow
            pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16VStart = 0x38;
        }
    }
...
#define MOVE_OUTSIDE TRUE
    ...
    pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16PanelHSyncBackPorch = pSelPanelType->m_ucPanelVBackPorch;
    pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16PanelVBackPorch = pSelPanelType->m_ucPanelVBackPorch;
...
static MS_U16 _registerTypeToSize(MS_U8 u8Registertype)
{
    ...
    switch (en)
    {
        ...
        default:
            printf("register type out of range: %d", en);
            return 0;
    }
...
MS_BOOL MApi_PNL_GetStatusEx_U2(void *pInstance, PNL_ApiExtStatus *pPnlExtStatus)
{
    ...
    pFile = filp_open(pPNLResourcePrivate->stapiPNL._pm_PNL_CUSTOMER_PATH, O_RDONLY, 0);
    if(IS_ERR(pFile))
    {
        printk("UtopiaOpen TCON file fail   PTR_ERR_fp=%ld\n",PTR_ERR(pFile));
        goto EXIT;
    }
    ...
    pTCONTable = (MS_U8*)vmalloc(lFileSize);
    if(pTCONTable == NULL)
    {
    ...
    cur_mm_seg = get_fs();
    set_fs(KERNEL_DS);
    u32Rst = vfs_read(pFile, pTCONTable, lFileSize, &pos);
    set_fs(cur_mm_seg);
    if(u32Rst != (MS_U32)lFileSize)
    {
        printk("Read TCON2.0 Error %d %ld\n", u32Rst, lFileSize);
        filp_close(pFile, NULL);
        goto EXIT;
    }
    filp_close(pFile, NULL);
    ...
    MS_U16 u16filelen = 0;
    MS_U32 u32Rst = 0;
    pTCONfile = fopen(pPNLResourcePrivate->stapiPNL._pm_PNL_CUSTOMER_PATH,"rb");
    if(pTCONfile == NULL)
    {
        printf("UtopiaOpen TCON file fail\n");
        goto EXIT;
    }
    fseek(pTCONfile,0,SEEK_END);
    u16filelen = ftell(pTCONfile);
    if(u16filelen <= 0)
    {
        fclose(pTCONfile);
        goto EXIT;
    }
    fseek(pTCONfile,0,SEEK_SET);
    pTCONTable = malloc(u16filelen);
    if(pTCONTable == NULL)
    {
        fclose(pTCONfile);
        goto EXIT;
    }
    u32Rst = fread(pTCONTable, sizeof(MS_U8), u16filelen, pTCONfile);
    if(u32Rst != (MS_U32)u16filelen)
    {
        fclose(pTCONfile);
        goto EXIT;
    }
    ...
    u8TCON_Verson = *((MS_U8*)(pTCONTable+0));
    if(u8TCON_Verson < 2)
    {
        goto EXIT;
    }
    stPnlExtStatus.u8TCON_MajorVersion    = *((MS_U8*)(pTCONTable+32));
    stPnlExtStatus.u8TCON_MinorVersion    = *((MS_U8*)(pTCONTable+33));
    stPnlExtStatus.u8TCON_PanelInterface  = *((MS_U8*)(pTCONTable+34));
    stPnlExtStatus.u8TCON_PanelSize       = *((MS_U8*)(pTCONTable+35));
    stPnlExtStatus.u16TCON_Year           = *((MS_U8*)(pTCONTable+36));
    stPnlExtStatus.u8TCON_Month           = *((MS_U8*)(pTCONTable+38));
    stPnlExtStatus.u8TCON_Day             = *((MS_U8*)(pTCONTable+39));

    memcpy(&stPnlExtStatus.u8TCON_ProjectName,(pTCONTable+40),PNL_TCON_NAME_LENGTH);
    memcpy(&stPnlExtStatus.u8TCON_CustomerName,(pTCONTable+56),PNL_TCON_NAME_LENGTH);
EXIT:
#if defined(MSOS_TYPE_LINUX_KERNEL)
    if(pTCONTable)
    {
...
MS_BOOL MApi_PNL_Init_U2(void *pInstance, PanelType *pSelPanelType/* <in > */)
{
    ...
    PanelType zeroPanel;
    memset((void*)(&zeroPanel),0,sizeof(PanelType));
    if(pSelPanelType && strncmp((void*)pSelPanelType,(void*)(&zeroPanel),sizeof(PanelType)) == 0)
    {
        printf("MMM  Zero Panel setting bypass\n");
        _PNL_SEMAPHORE_RETURN(pInstance,PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID));
        return TRUE;
    }
...
MS_BOOL MApi_PNL_TCONMAP_DumpTable_U2(void *pInstance,MS_U8* pTCONTable,MS_U8 u8Tcontype)
{
    ...
    // In mboot , panel init -> dump tcon table. In sn, Dump tcon table -> panel init
    // mboot will parse separate data location and call dump table, but in sn, we read all the bin file....

    MS_U8 u8TableType=0;
    MS_U16 u16RegisterCount=0;
    MS_U8  u8RegisterType=0;
    MS_U32 u32RegisterlistOffset=0;
    MS_U32 u32RegisterlistSize=0;
    MS_U8 u8TCON_Version = 0;
    MS_U8* pu8Table = NULL;
    ...
    loff_t pos = vfs_llseek(pFile, 0, SEEK_SET);
    pu8Table = (MS_U8*)vmalloc(lFileSize);
    if(pu8Table == NULL)
    {
        printk("out of memory \n");
        filp_close(pFile, NULL);
    ...
    cur_mm_seg = get_fs();
    set_fs(KERNEL_DS);
    u32Rst = vfs_read(pFile, pu8Table, lFileSize, &pos);
    set_fs(cur_mm_seg);
    ...
    u32Rst = fread(pu8Table, sizeof(MS_U8), u16filelen, pTCONfile);
    if(u32Rst != (MS_U32)u16filelen)
    {
        fclose(pTCONfile);
        bRet = FALSE;
    ...
    fclose(pTCONfile);
#endif
    u8TCON_Version = *((MS_U8*)(pu8Table));
    printf("[%s][%d]TCON VERSION %d \n",__FUNCTION__,__LINE__,u8TCON_Version);

    pu8Table+=32; // skip 32 bytes Main header
    pu8Table+=8; // skip 8byte   subheader

    if(u8TCON_Version==3)
    {
        pu8Table+=40; // skip 8byte TCON2.0 subheader
    }
    // sn MAY NEED version , so there needs a way to pass this info.
    u8TableType           = *((unsigned char*) (pu8Table + (u8Tcontype * 8) + 0));
    u16RegisterCount      = *((unsigned short*)(pu8Table + (u8Tcontype * 8) + 1));
    u8RegisterType        = *((unsigned char*) (pu8Table + (u8Tcontype * 8) + 3));
    u32RegisterlistOffset = *((unsigned int*)  (pu8Table + (u8Tcontype * 8) + 4));
    u32RegisterlistSize   = u16RegisterCount * _registerTypeToSize(u8RegisterType);

    if(u8TCON_Version == 3)
    {
        pu8Table+=(u32RegisterlistOffset-80);
    }
    else if(u8TCON_Version == 1)
    {
        pu8Table+=(u32RegisterlistOffset-40);
    }
    ...
    switch(u8Tcontype)
    {
        ...
        case E_APIPNL_TCON_TAB_TYPE_GPIO:
            MDrv_PNL_TCON_DumpGENRegTab(pInstance, pu8Table,u16RegisterCount);
            break;
        case E_APIPNL_TCON_TAB_TYPE_SCALER:
            MDrv_PNL_TCON_DumpSCRegTab(pInstance, pu8Table,u16RegisterCount);
            break;
        case E_APIPNL_TCON_TAB_TYPE_MOD:
            MDrv_PNL_TCON_DumpMODRegTab(pInstance, pu8Table,u16RegisterCount);
            break;
        ...
        case E_APIPNL_TCON_TAB_TYPE_POWER_SEQUENCE_OFF:
            if(u8TCON_Version > 2)
            {
                MDrv_PNL_TCON_DumpPowerSequenceRegTab(pInstance,pu8Table,u16RegisterCount);
            }
            break;
        ...
        case E_APIPNL_TCON_TAB_TYPE_PANEL_INFO:
            if(u8TCON_Version > 2)
            {
               MDrv_PNL_TCON_DumpPanelInfo(pInstance,pu8Table,u32RegisterlistSize);
            }
            break;
        case E_APIPNL_TCON_TAB_TYPE_OVERDRIVER:
            if(u8TCON_Version > 2)
            {
                 MDrv_PNL_TCON_DumpOverDriveRegTab(pInstance,pu8Table);
            }
            break;
        default:
...
APIPNL_Result MApi_PNL_OverDriver_Setting_U2(void *pInstance, MS_PNL_OD_SETTING *pPNL_ODSetting, MS_U32 u32ODInitDataLen)
{
    if(u32ODInitDataLen != sizeof(MS_PNL_OD_SETTING))
    {
        return E_APIPNL_FAIL;
    }
    ...
    pPNLResourcePrivate->stapiPNL._u32OD_MSB_Addr = pPNL_ODSetting->u32OD_MSB_Addr;
    pPNLResourcePrivate->stapiPNL._u32OD_LSB_Addr = pPNL_ODSetting->u32OD_LSB_Addr;
    pPNLResourcePrivate->stapiPNL._u32ODInitDataLen = u32ODInitDataLen;
    if(pPNL_ODSetting->pu16ODTbl)
    {
        memcpy(pPNLResourcePrivate->stapiPNL._u16ODTbl,pPNL_ODSetting->pu16ODTbl,sizeof(MS_U16)*OverDriverTblSize);
    }
    pPNLResourcePrivate->stapiPNL._ODTbl_Size = pPNL_ODSetting->u32ODTbl_Size;

    _PNL_ENTRY();
    MDrv_PNL_OverDriver_Setting(pInstance,pPNL_ODSetting->u32OD_MSB_Addr,pPNL_ODSetting->u32OD_LSB_Addr, pPNL_ODSetting->pu16ODTbl,pPNL_ODSetting->u32ODTbl_Size,E_OVERDRIVER_SEPARATE_R_ENABLE|E_OVERDRIVER_SEPARATE_G_ENABLE|E_OVERDRIVER_SEPARATE_B_ENABLE);
    _PNL_RETURN();
...
#ifndef UTOPIAXP_REMOVE_WRAPPER
APIPNL_Result MApi_PNL_OverDriver_Setting(MS_PNL_OD_SETTING *pPNL_ODSetting, MS_U32 u32ODInitDataLen)
{
    if (pu32PNLInst == NULL)
    {
        if(UtopiaOpen(MODULE_PNL, &pu32PNLInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
    ...
    stPNL_OverDriver_Setting PNLArgs;
    PNLArgs.pPNL_ODSetting = pPNL_ODSetting;
    PNLArgs.u32ODInitDataLen = u32ODInitDataLen;
    PNLArgs.eResult = E_APIPNL_FAIL;
    if(UtopiaIoctl(pu32PNLInst, E_PNL_CMD_OVERDRIVER_SETTING, (void*)&PNLArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain PNL engine fail\n");
        return E_APIPNL_FAIL;
    }
...
APIPNL_Result MApi_PNL_OverDriver_Enable_U2(void *pInstance, MS_BOOL bEnable)
{
    ...
    _PNL_SEMAPHORE_ENTRY(pInstance,PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID));
    UtopiaResourceGetPrivate(g_pPNLResource[PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID)],(void**)(&pPNLResourcePrivate));
    pPNLResourcePrivate->stapiPNL._bOD_En = bEnable;
    _PNL_ENTRY();
...
MS_U32 MApi_PNL_SetPowerState_U2(void *pInstance, EN_POWER_MODE ePowerState)
{
    ...
    else if (ePowerState == E_POWER_RESUME)
    {
        if (pPNLResourcePrivate->stapiPNL.ePrevPowerState == E_POWER_SUSPEND)
        {
            ...
            MApi_PNL_TCONMAP_DumpTable(NULL,E_APIPNL_TCON_TAB_TYPE_OVERDRIVER);
            MApi_PNL_OverDriver_Enable_U2(pInstance,pPNLResourcePrivate->stapiPNL._bOD_En);
            _PNL_SEMAPHORE_ENTRY(pInstance,PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID));
...
#endif
APIPNL_Result MApi_PNL_SetFilePath_U2(void *pInstance, E_PNL_FILE_PATH enPNLFilePath,char* const pPNLFilePath)
{
    PNL_INSTANCE_PRIVATE *pPNLInstancePrivate = NULL;
    PNL_RESOURCE_PRIVATE* pPNLResourcePrivate = NULL;
    ...
    _PNL_SEMAPHORE_ENTRY(pInstance,PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID));
    UtopiaResourceGetPrivate(g_pPNLResource[PNL_GET_INTERNAL_POOL_ID(pPNLInstancePrivate->u32DeviceID)],(void**)(&pPNLResourcePrivate));
    if(pPNLFilePath == NULL)
    {
        printf("ERROR!! NULL BIN PATH!!!!!\n");
    }
    else
    {
        switch(enPNLFilePath)
        {
            case E_PNL_FILE_TCON_BIN_PATH:
            {
                memset(pPNLResourcePrivate->stapiPNL._pm_PNL_CUSTOMER_PATH,0,PNL_FILE_PATH_LENGTH);
                strncpy(pPNLResourcePrivate->stapiPNL._pm_PNL_CUSTOMER_PATH,pPNLFilePath,PNL_FILE_PATH_LENGTH - 1);
                break;
            }
            default:
...
#ifndef UTOPIAXP_REMOVE_WRAPPER
APIPNL_Result MApi_PNL_SetFilePath(E_PNL_FILE_PATH enPNLFilePath,char* const pPNLFilePath)
{
    if (pu32PNLInst == NULL)
    {
        if(UtopiaOpen(MODULE_PNL, &pu32PNLInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
    ...
    stPNL_Set_File_Path PNLArgs;
    PNLArgs.enPNLFilePath = enPNLFilePath;
    PNLArgs.pPNLFilePath = pPNLFilePath;
    PNLArgs.eResult = E_APIPNL_FAIL;

    if(UtopiaIoctl(pu32PNLInst, E_PNL_CMD_SETFILEPATH, (void*)&PNLArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain PNL engine fail\n");
        return E_APIPNL_FAIL;
    }
