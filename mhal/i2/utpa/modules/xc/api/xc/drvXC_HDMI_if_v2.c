...
MS_U32 HDMIRxOpen(void** ppInstance, const void* const pAttribute)
{
    ...
    psHDMIRxInstPri->fpHDMIRxGetHDRInfoFrame = (IOCTL_HDMIRX_GET_HDR_INFO_FRAME)mdrv_HDMI_GetHDRInfoFrame;
    psHDMIRxInstPri->fpHDMIRxDataRtermControl = (IOCTL_HDMIRX_DATA_RTERM_CONTROL)mdrv_HDMI_DataRtermControl;
    /*************************** HDCP Repeater ***************************/
    ...
    psHDMIRxInstPri->fpHDMIRxHDCP22SetEvent = (IOCTL_HDMIRX_HDCP22_SET_EVENT)mdrv_HDCP22_SetEvent;
...
MS_U32 HDMIRxIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MAPI_CMD_HDMIRX_INITIAL:
            {
                ...
                pHDMIRxArgs->bCreateHDCPTaskFlag = psHDMIRxInstPri->fpHDMIRxInitial(pInstance, pHDMIRxArgs->stInitialTable);
            }
            break;
        ...
        case MAPI_CMD_HDMIRX_HDCP22_SET_EVENT:
            {
                pstHDCP22_SET_EVENT pHDMIRxArgs = (pstHDCP22_SET_EVENT)pArgs;
                psHDMIRxInstPri->fpHDMIRxHDCP22SetEvent(pInstance, pHDMIRxArgs->usHDCPEventIndex);
            }
            break;

        case MAPI_CMD_HDMIRX_GET_HDR_INFO_FRAME:
            {
                pstHDMI_GET_HDR_INFOFRAME pHDMIRxArgs = (pstHDMI_GET_HDR_INFOFRAME)pArgs;
                pHDMIRxArgs->bHDRReceiveFlag = psHDMIRxInstPri->fpHDMIRxGetHDRInfoFrame(pInstance, pHDMIRxArgs->ucHDMIInfoSource, pHDMIRxArgs->pusHDRInfoFrame);
            }
            break;

        case MAPI_CMD_HDMIRX_DATA_RTERM_CONTROL:
            {
                pstHDMI_DATA_RTERM_CONTROL pHDMIRxArgs = (pstHDMI_DATA_RTERM_CONTROL)pArgs;
                psHDMIRxInstPri->fpHDMIRxDataRtermControl(pInstance, pHDMIRxArgs->enInputPortType, pHDMIRxArgs->bDataRtermEnable);
            }
            break;
