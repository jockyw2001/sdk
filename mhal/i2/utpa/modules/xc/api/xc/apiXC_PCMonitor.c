...
typedef enum
{
    EN_XC_ADC_CALIBRATION_INIT = 0,
    EN_XC_ADC_DO_CALIBRATION,
    EN_XC_ADC_CALIBRATION_DONE,
    EN_XC_ADC_CALIBRATION_MAX,
} XC_PCMONITOR_ADC_CALIBRATION_STATUS;

extern PQ_Function_Info  s_PQ_Function_Info[MAX_XC_DEVICE_NUM];
static XC_PCMONITOR_ADC_CALIBRATION_STATUS enAdcCalibration = EN_XC_ADC_CALIBRATION_INIT;

static void _MApi_XC_PCMonitor_ADC_Calibration(void *pInstance, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);

    MS_U16 u16RealPixClk = 0;
    ...
    memset(&stPCMonitorInfo,0,sizeof(XC_PCMONITOR_HK_INFO));

    MDrv_XC_GetStatus(pInstance, &stXCStatus, eWindow);

    u16RealPixClk = MDrv_XC_cal_pixclk(pInstance, stXCStatus.u16DefaultHtotal, eWindow);

    if (stXCStatus.bHDuplicate)
    {
        ...
        {
            MS_BOOL bRet                                        = FALSE;
            stPQADCSamplingInfo.enPQSourceType                  = PQ_INPUT_SOURCE_YPBPR;
            stPQADCSamplingInfo.stPQModeInfo.bFBL               = stXCStatus.bFBL;
            stPQADCSamplingInfo.stPQModeInfo.bInterlace         = stXCStatus.bInterlace;
            stPQADCSamplingInfo.stPQModeInfo.u16input_hsize     = stXCStatus.stCapWin.width;
            stPQADCSamplingInfo.stPQModeInfo.u16input_vsize     = stXCStatus.stCapWin.height;
            stPQADCSamplingInfo.stPQModeInfo.u16input_vfreq     = stXCStatus.u16InputVFreq;
            stPQADCSamplingInfo.stPQModeInfo.u16input_vtotal    = stXCStatus.u16InputVTotal;
            stPQADCSamplingInfo.stPQModeInfo.u16ouput_vfreq     = stXCStatus.u16InputVFreq;
            stPQADCSamplingInfo.stPQModeInfo.u16display_hsize   = stXCStatus.stDispWin.width;
            stPQADCSamplingInfo.stPQModeInfo.u16display_vsize   = stXCStatus.stDispWin.height;

            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((PQ_WIN)eWindow, E_PQ_IOCTL_ADC_SAMPLING, (void *)&stPQADCSamplingInfo, sizeof(PQ_ADC_SAMPLING_INFO));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if (bRet == FALSE)
            {
    ...
    Hal_ADC_set_mode(pInstance, ADC_INPUTSOURCE_ONLY_YPBPR, u16RealPixClk, stXCStatus.u16DefaultHtotal, stPQADCSamplingInfo.u16ratio);
    Hal_ADC_Set_Source_Calibration(pInstance, ADC_INPUTSOURCE_ONLY_YPBPR);
...
//#endif
...
#ifdef STELLAR
        // source change, reset to init
        enAdcCalibration = EN_XC_ADC_CALIBRATION_INIT;
#endif
        ...
        else //have sync?
        {
            ...
            if(gSrcInfo[eWindow].bAutoNoSignalEnabled && pXCResourcePrivate->stapiXC_PCMonitor._bEnableAutoNoSignalDetect[eWindow])
            {
                ...
                if (( MDrv_XC_PCMonitor_InvalidTimingDetect(pInstance, TRUE, eWindow) )
                    #ifdef STELLAR
                    ||((stPCMonitorInfo.eCurrentState==E_XC_PCMONITOR_STABLE_SYNC)&&(MDrv_XC_IsInputSourceDisabled(pInstance, eWindow))))
                    #endif
                   )
                {
    ...
    if ( stPCMonitorInfo.bInputTimingChange) // input timing is unstable
    {
        ...
        #ifdef STELLAR
        if (TRUE == IsSrcTypeYPbPr(stPCMonitorInfo.eCurrentSrc))
        {
            if (EN_XC_ADC_DO_CALIBRATION == enAdcCalibration)
            {
                enAdcCalibration = EN_XC_ADC_CALIBRATION_DONE;
            }
            else if (EN_XC_ADC_CALIBRATION_INIT == enAdcCalibration)
            {
                // it means first time after timing change, do nothing
            }
        }
        else
        {
            // if source is not YPbPr, treat it as done
            enAdcCalibration = EN_XC_ADC_CALIBRATION_DONE;
        }
        #endif
    ...
    else // input timing is stable
    {
        ...
        if ( !stPCMonitorInfo.bInputTimingStable )
        {
                ...
                (MDrv_XC_PCMonitor_SyncLoss(pInstance, eWindow) && (stPCMonitorInfo.ucInputTimingStableCounter >= pXCResourcePrivate->stapiXC_PCMonitor._u16TimingNosyncCounter[eWindow])) )
            {
                ...
                {
                    // if not YPbPr, it won't come in this condition
                    enAdcCalibration = EN_XC_ADC_DO_CALIBRATION;
                    _MApi_XC_PCMonitor_ADC_Calibration(pInstance, eWindow);

                    // force set to unstable for run PC_Monitor again
                    stPCMonitorInfo = MDrv_XC_PCMonitor_GetStatus(pInstance, eWindow);
                    stPCMonitorInfo.u8SyncStatus |= XC_MD_SYNC_LOSS;
                    stPCMonitorInfo.bInputTimingChange = TRUE;
                    MDrv_XC_PCMonitor_SetStatus(pInstance, stPCMonitorInfo, eWindow);
                ...
                #ifdef STELLAR
                if (EN_XC_ADC_CALIBRATION_DONE == enAdcCalibration)
                #endif
                {
                    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
                    return stPCMonitorInfo.eCurrentState;
