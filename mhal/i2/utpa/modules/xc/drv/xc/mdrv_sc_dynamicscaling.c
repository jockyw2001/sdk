...
extern void KApi_XC_SetDSInfo(EN_KDRV_SC_DEVICE u32DeviceID, ST_KDRV_SET_XC_DS_Info *pstDSInfo, EN_KDRV_WINDOW eWindow);
extern void KApi_XC_GetDSInfo(EN_KDRV_SC_DEVICE u32DeviceID, ST_KDRV_GET_XC_DS_Info *pstDSInfo, EN_KDRV_WINDOW eWindow);
#endif
#endif
...
MS_BOOL MApi_XC_SetDynamicScaling_U2(void* pInstance, XC_DynamicScaling_Info *pstDSInfo,MS_U32 u32DSInfoLen, SCALER_WIN eWindow)
{
    ...
    {
        printf("Unmatched XC_DynamicScaling_Info, Exit\n");
        return FALSE;
    }
    ...
    //**************get DS Buf Size in shm*********************//
    _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    _XC_ENTRY(pInstance);

    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    stKDrvDsInfo.u32DSBufferSize = pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize;
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
    //********************************************************
    ...
    else
    {
        ...
        gSrcInfo[eWindow].Status2.bUpdate_DS_CMD = 0;
    }
    ...
    if(psXCInstPri->u32DeviceID == 0)
        ...
        u32DeviceID = E_KDRV_XC_SC1;
    return KApi_XC_SetDynamicScaling(u32DeviceID, (K_XC_DynamicScaling_Info*)&stKDrvDsInfo,sizeof(K_XC_DynamicScaling_Info),eWindow);
#else
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(0 > _s32FdScaler)
    {
    ...
    else
    {
        ...
        k_ds_set_DynamicScaling.u32DSInfoLen = sizeof(K_XC_DynamicScaling_Info);
...
#endif
MS_BOOL MDrv_XC_Is_SupportSWDS(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if((psXCInstPri->u32DeviceID==0) && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_A & E_XC_INIT_MISC_A_IS_ANDROID))
        ...
        return Hal_XC_Is_SupportSWDS(pInstance);
#endif
#endif//STELLAR
    ...
    if((psXCInstPri->u32DeviceID==0))
    {
        ...
        {
            if (pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow] == TRUE)
            {
...
MS_BOOL MApi_XC_Is_SupportSWDS_U2(void *pInstance)
{
    ...
    bReturn = TRUE;
#else
    bReturn = MDrv_XC_Is_SupportSWDS(pInstance, MAIN_WINDOW);
#endif
    return bReturn;
}
...
void MApi_XC_SetDSInfo_U2(void *pInstance,XC_DS_INFO *pstSetDSInfo, MS_U32 u32DSInfoLen, SCALER_WIN eWindow)
{
    ...
    memset(&DrvDSInfo, 0, sizeof(XC_DS_INFO));
    ...
    gSrcInfo[eWindow].Status2.bEnable_ForceP = DrvDSInfo.bEnable_ForceP[eWindow];
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = DrvDSInfo.u32DSBufferSize;
    if(pstSetDSInfo->u32ApiDSInfo_Version >= 5) // check API version
    {
        pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow] = DrvDSInfo.bDynamicScalingEnable[eWindow];
    }

#if (SUPPORT_KERNEL_DS == 1)
    K_XC_SETDS_Info kDrvDSInfo;
    memset(&kDrvDSInfo, 0, sizeof(K_XC_SETDS_Info));
    kDrvDSInfo.u32MFCodecInfo = DrvDSInfo.u32MFCodecInfo;
    kDrvDSInfo.bUpdate_DS_CMD = DrvDSInfo.bUpdate_DS_CMD[eWindow];
    kDrvDSInfo.bEnableDNR = DrvDSInfo.bEnableDNR[eWindow];
    kDrvDSInfo.u32DSBufferSize = DrvDSInfo.u32DSBufferSize;
    kDrvDSInfo.bEnable_ForceP = DrvDSInfo.bEnable_ForceP[eWindow];
    kDrvDSInfo.bDynamicScalingEnable = pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow];
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    KApi_XC_SetDSInfo(u32DeviceID, (ST_KDRV_SET_XC_DS_Info *)&kDrvDSInfo, eWindow);
#else
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(0 > _s32FdScaler)
    {
    ...
    {
        ST_KDRV_SET_XC_DS_Info stKDrvDSInfo;
        if(psXCInstPri->u32DeviceID == 0)
            stKDrvDSInfo.u32DeviceID = E_KDRV_XC_SC0;
        else
            stKDrvDSInfo.u32DeviceID = E_KDRV_XC_SC1;

        stKDrvDSInfo.eWindow = eWindow;
        stKDrvDSInfo.stDSInfo= (K_XC_SETDS_Info*)&kDrvDSInfo;
        if (ioctl(_s32FdScaler, MDRV_XC_IOC_SET_DSINFO,&stKDrvDSInfo))
        {
            printf("MDRV_XC_IOC_SET_DSINFO fail!!!!\n");
            return;
        }
    }
#endif
...
void MApi_XC_GetDSInfo_U2(void *pInstance,XC_GET_DS_INFO *pstGetDSInfo, MS_U32 u32DSInfoLen, SCALER_WIN eWindow)
{
    MS_U16 u16CopiedLength = sizeof(XC_GET_DS_INFO);
    if((NULL == pstGetDSInfo) || (pstGetDSInfo->u16ApiGetDSInfo_Length == 0))
    {
        //We cosider compatible operation form version2 , so reject the info init when version invalid
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MApi_XC_GetDSInfo: Null paramter or Wrong u16ApiDSInfo_Length!!\n");

        XC_LOG_TRACE(XC_DBGLEVEL_FUNCTION_TRACE, "Exit\n");
        return ;
    }
    // the version control is coming in with version 1
    if(pstGetDSInfo->u32ApiGetDSInfo_Version < 1)
    {
        //We cosider compatible operation form version1 , so reject the info init when version invalid
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MApi_XC_GetDSInfo: please check your u32ApiDSInfo_Version, it should not set to 0!!\n");
        XC_LOG_TRACE(XC_DBGLEVEL_FUNCTION_TRACE, "Exit\n");
    ...
    //new AP + old lib, only the length corresponding to old lib has meaning.
    if(pstGetDSInfo->u32ApiGetDSInfo_Version > API_GET_XCDS_INFO_VERSION)
    {
        //We cosider compatible operation form version1 , so reject the info init when version invalid
#if defined (__aarch64__)
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MApi_XC_GetDSInfo: this old version XC lib has only length:%lu driver status!!\n",
                     sizeof(XC_GET_DS_INFO))
#else
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MApi_XC_GetDSInfo: this old version XC lib has only length:%u driver status!!\n",
                     sizeof(XC_GET_DS_INFO))
#endif
        //we still copy the min size of both structure, but AP should take care of it.
        u16CopiedLength = sizeof(XC_GET_DS_INFO);
    }

    //old AP + new lib, driver shouldn't access to the space which doesn't exist in old structure
    if((pstGetDSInfo->u32ApiGetDSInfo_Version < API_GET_XCDS_INFO_VERSION) || (pstGetDSInfo->u16ApiGetDSInfo_Length < sizeof(XC_GET_DS_INFO)))
    {
        //We cosider compatible operation form version1 , so reject the info init when version invalid
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MApi_XC_GetDSInfo: new version XC lib shouldn't access to the space which doesn't exist in old structure!!\n")
        u16CopiedLength = pstGetDSInfo->u16ApiGetDSInfo_Length;
    }

    pstGetDSInfo->u32ApiGetDSInfo_Version = API_GET_XCDS_INFO_VERSION; //Return the actual version of the returned value for APP to use
    pstGetDSInfo->u16ApiGetDSInfo_Length = u16CopiedLength; //Return the actual length of the returned value for APP to know whick info is valid
    pstGetDSInfo->bDynamicScalingEnable = (MDrv_XC_Is_DSForceIndexEnabled(pInstance,eWindow)|pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow]);
    pstGetDSInfo->bUpdate_DS_CMD = gSrcInfo[eWindow].Status2.bUpdate_DS_CMD;

#if (SUPPORT_KERNEL_DS == 1)
    K_XC_GETDS_Info kDrvDSInfo;
    memset(&kDrvDSInfo, 0, sizeof(K_XC_GETDS_Info));
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    KApi_XC_GetDSInfo(u32DeviceID, (ST_KDRV_GET_XC_DS_Info *)&kDrvDSInfo, eWindow);
    pstGetDSInfo->bDynamicScalingEnable = (MDrv_XC_Is_DSForceIndexEnabled(pInstance,eWindow)|kDrvDSInfo.bDynamicScalingEnable);
#else
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(0 > _s32FdScaler)
    {
    ...
    {
        ST_KDRV_GET_XC_DS_Info stKGetDrvDSInfo;
        if(psXCInstPri->u32DeviceID == 0)
            stKGetDrvDSInfo.u32DeviceID = E_KDRV_XC_SC0;
        else
            stKGetDrvDSInfo.u32DeviceID = E_KDRV_XC_SC1;

        stKGetDrvDSInfo.eWindow = eWindow;
        stKGetDrvDSInfo.stGetDSInfo= (K_XC_GETDS_Info*)&kDrvDSInfo;
        if (-1 == ioctl(_s32FdScaler, MDRV_XC_IOC_GET_DSINFO,&stKGetDrvDSInfo))
        {
            perror("ioctl");
            return ;
        }

        pstGetDSInfo->bDynamicScalingEnable = (MDrv_XC_Is_DSForceIndexEnabled(pInstance,eWindow)|stKGetDrvDSInfo.stGetDSInfo->bDynamicScalingEnable);
    }
#endif
#endif
#endif
...
#ifdef MSOS_TYPE_LINUX_KERNEL
void MApi_XC_GetDSInfo(XC_GET_DS_INFO *pstGetDSInfo, MS_U32 u32DSInfoLen, SCALER_WIN eWindow)
#else
__attribute__((weak)) void MApi_XC_GetDSInfo(XC_GET_DS_INFO *pstGetDSInfo, MS_U32 u32DSInfoLen, SCALER_WIN eWindow)
#endif
{
    if (pu32XCInst == NULL)
    {
    ...
    stXC_GET_DSINFO XCArgs;
    XCArgs.pstGetDSInfo = pstGetDSInfo;
    XCArgs.u32DSInfoLen = u32DSInfoLen;
    XCArgs.eWindow = eWindow;
    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_GET_DSINFO, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return ;
    }
...
///     |2          |With ForceP                                    |
///     +-----------+-----------------------------------------------+
///     |3          |With ForceP (No need special MADI and SW patch)|
///     +-----------+-----------------------------------------------+
//-------------------------------------------------------------------------------------------------
MS_U16 MDrv_XC_Get_DS_Version(void* pInstance)
{
#ifdef XC_DS_VERSION
    return XC_DS_VERSION;
#else
    return 0;
#endif
}

MS_U16 MApi_XC_Get_DS_Version_U2(void* pInstance)
{
    MS_U16 u16Return = 0;
    u16Return = MDrv_XC_Get_DS_Version(pInstance);
    return u16Return;
}

__attribute__((weak)) MS_U16 MApi_XC_Get_DS_Verison(void)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    stXC_GET_DS_VERSION XCArgs;
    XCArgs.u16DSVersion = 0;

    if(UtopiaIoctl(pu32XCInst, E_XC_GET_DS_VERSION, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
    ...
    {
        printf("[%s,%d] XCArgs.u16DSVersion=%d\n", __func__, __LINE__, XCArgs.u16DSVersion);
        return XCArgs.u16DSVersion;
    }
}
