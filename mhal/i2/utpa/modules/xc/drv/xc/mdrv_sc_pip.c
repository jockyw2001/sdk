...
void MApi_XC_EnableSubWindow_U2(void* pInstance, MS_WINDOW_TYPE *pstDispWin)
{
    ...
    if((MDrv_XC_GetMWEStatus(pInstance) && IsVMirrorMode(SUB_WINDOW))
        || (!MDrv_XC_GetMWEStatus(pInstance) && IsVMirrorMode(MAIN_WINDOW)))
#else
...
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
    if((MDrv_XC_GetMWEStatus(pInstance) && IsHMirrorMode(SUB_WINDOW))
        || (!MDrv_XC_GetMWEStatus(pInstance) && IsHMirrorMode(MAIN_WINDOW)))
#else
    if(IsHMirrorMode(SUB_WINDOW))
#endif
    {
...
void MDrv_XC_EnablePIP(void *pInstance, MS_BOOL bEnable)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    if (MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        Hal_SC_enable_PIP_burst(pInstance, bEnable);
    }
    else
    {
        Hal_SC_enable_PIP(pInstance, bEnable);
    }
