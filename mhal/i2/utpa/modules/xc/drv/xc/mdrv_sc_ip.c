...
static void MDrv_XC_Access_BufferData(void *pInstance, E_XC_OUTPUTDATA_TYPE eBufType, MS_WINDOW_TYPE *pMemRect, void *pDataBuf, MS_WINDOW_TYPE *pSrcRect, SCALER_WIN eWindow, MS_BOOL bIsReadMode)
{
    ...
    MS_U16 u16Discardpixels = 0;
    MS_U8 u8Offsetpixels = 0;
    MS_U16 u16pixels_per_unit = OFFSET_PIXEL_ALIGNMENT;

    MS_U16 xRegion, accessStart;
    MS_S16 yNext, xNext, accessNext;
    ...
    if (FBInfo.bMemFormat422 == TRUE)
    {
        ...
        {
            u16pixels_per_unit = OFFSET_PIXEL_ALIGNMENT;
    ...
    else
    {
        ...
        u16pixels_per_unit = OFFSET_PIXEL_ALIGNMENT/2;
    ...
    xRegion = x1/u16pixels_per_unit - x0/u16pixels_per_unit + 1;

    accessStart = 0;
    yNext = 1;
    xNext = u16pixels_per_unit;
    accessNext = 1;
    ...
    if (bHmirror)
    {
        ...
        xRegion = x0/u16pixels_per_unit - x1/u16pixels_per_unit + 1;
        accessStart = u16pixels_per_unit -1;
        xNext = -1 * u16pixels_per_unit;
        accessNext = -1;
    }
    if (bVmirror)
    {
    ...
    u16LBOffset = x0 % u16pixels_per_unit;
        ...
        for (x = x0 - u16LBOffset, xCount = 0; xCount < xRegion; x = x + xNext, xCount+=1)
        {
            ...
            if (FBInfo.bMemFormat422 == TRUE)
            {
                if(!bHmirror)
                {
                    ...
                    if(x+u16pixels_per_unit > x1)
                        u16Discardpixels = x+u16pixels_per_unit - x1;
                    else
                        u16Discardpixels = 0;
                }
                else
                {
                    ...
                    if(x+u16pixels_per_unit > x0)
                        u16Discardpixels = x+u16pixels_per_unit - x0;
                    else
                        u16Discardpixels = 0;
                }
                ...
                for(accessCount = 0; accessCount < u16pixels_per_unit; index = index + accessNext, accessCount+=1)
                {
                    if((index < u8Offsetpixels) || (index > (u16pixels_per_unit - u16Discardpixels)))
                        continue;
                    ...
                    if(!bIsReadMode)
                    {
                        if(!bHmirror)
                        {
                            u16WriteModeSrcPoint = (xCount * u16pixels_per_unit + (index - u16LBOffset)) % pSrcRect->width;
                        }
                        else
                        {
                            u16WriteModeSrcPoint = (xCount * u16pixels_per_unit + (u16LBOffset - index)) % pSrcRect->width;
                        }
                    }
#if (_FIELD_PACKING_MODE_SUPPORTED)
                    if(true)
...
void MDrv_XC_init_riu_base(MS_VIRT u32riu_base, MS_VIRT u32PMriu_base, MS_VIRT u32EXTriu_base)
{
    Hal_SC_init_riu_base(u32riu_base, u32PMriu_base, u32EXTriu_base);
}
...
void MDrv_XC_PCMonitor_Get_Dvi_Hdmi_De_Info(void *pInstance, SCALER_WIN eWindow,MS_WINDOW_TYPE* msWin)
{
    ...
    else
    {
        ...
        {
            if(MDrv_HDMI_CheckHDMI20_Setting_U2(pInstance, MDrv_XC_Mux_GetHDMIPort(pInstance, gSrcInfo[eWindow].enInputSourceType)))
            {
