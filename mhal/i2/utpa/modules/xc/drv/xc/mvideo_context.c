...
void mvideo_sc_variable_init(void *pInstance, MS_BOOL bFirstInstance, XC_INITDATA *pXC_InitData)
{
    ...
    if (bFirstInstance)
    {
        ...
        gSrcInfo[MAIN_WINDOW].u16OSDLayer = E_VOP_LAYER_RESERVED;
        ...
        gSrcInfo[MAIN_WINDOW].Status2.bEnable_ForceP = FALSE;
#endif
        ...
        gSrcInfo[SUB_WINDOW].u16OSDLayer = E_VOP_LAYER_RESERVED;
        ...
        gSrcInfo[SUB_WINDOW].Status2.bEnable_ForceP = FALSE;
#endif
...
void MApi_XC_EnableFrameBufferLess_U2(void* pInstance, MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
         gSrcInfo[MAIN_WINDOW].eFBLevel = E_XC_FB_LEVEL_FBL;
    }
    else
    {
         gSrcInfo[MAIN_WINDOW].eFBLevel = E_XC_FB_LEVEL_FB;
...
void MApi_XC_EnableRequest_FrameBufferLess_U2(void* pInstance, MS_BOOL bEnable)
{
    ...
    if(Hal_SC_Set_RequestFBL_Mode(pInstance, bEnable))
    {
        ...
        // Mirror Mode
        // MM and DTV case always control MVOP/XC mirror by SN, need be removed.
...
E_APIXC_ReturnValue MApi_XC_Set_FB_Level_U2(void* pInstance, E_XC_FB_LEVEL eFBLevel, SCALER_WIN eWindow)
{
    ...
    printf("%s %d, [eWindow] = %d , FB LEVEL = %d!!!!!\n", __FUNCTION__, __LINE__, eWindow, eFBLevel);
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
void MDrv_XC_FreezeImg(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
...
#ifdef UFO_XC_FB_LEVEL
    if((gSrcInfo[eWindow].eFBLevel == E_XC_FB_LEVEL_FBL)  || (gSrcInfo[eWindow].eFBLevel == E_XC_FB_LEVEL_RFBL_DI) || Hal_SC_get_fbl(pInstance))
#else
    if(gSrcInfo[eWindow].bFBL || gSrcInfo[eWindow].bR_FBL || Hal_SC_get_fbl(pInstance))
#endif
    {
    ...
    if(bEnable)
    {
...
#endif
        {
            ...
            if(s_PQ_Function_Info[u32DeviceID].pq_disable_filmmode)
            {
                _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
                s_PQ_Function_Info[u32DeviceID].pq_disable_filmmode((u32DeviceID == 1)?PQ_SUB_WINDOW:PQ_MAIN_WINDOW, bEnable);
#else
                s_PQ_Function_Info[u32DeviceID].pq_disable_filmmode(ePQWin, bEnable);
#endif
                _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            }
        }
        ...
        else if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
        {
            ...
            if(s_PQ_Function_Info[u32DeviceID].pq_disable_filmmode_ex)
            {
                _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
                s_PQ_Function_Info[u32DeviceID].pq_disable_filmmode_ex(u32DeviceID, ePQWin, bEnable);
                _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            }
        }
#endif
...
E_APIXC_ReturnValue MDrv_XC_OSDC_Control(void *pInstance, MS_U32 eCtrl_type)
{
    ...
    if(gSrcInfo[MAIN_WINDOW].Status2.bIsInitOSDC)
    {
        ...
        {
            if(gSrcInfo[MAIN_WINDOW].Status2.u16OC_Tgen_HTotal == 0)
            {
                gSrcInfo[MAIN_WINDOW].Status2.u16OC_Tgen_HTotal = MHAL_SC_get_osdc_tgen_htotal(pInstance);
            }
            if(gSrcInfo[MAIN_WINDOW].Status2.u16OC_Tgen_VTotal == 0)
            {
                gSrcInfo[MAIN_WINDOW].Status2.u16OC_Tgen_VTotal = MHAL_SC_get_osdc_tgen_vtotal(pInstance);
            }

            if( ((gSrcInfo[MAIN_WINDOW].Status2.u16OC_PreLplltype != 0) &&
...
#endif
#endif
//********************XC Instance Linking List******************//
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
#ifdef MSOS_TYPE_LINUX_KERNEL
void*  MDrv_XC_Get_PID_Instance(int device_id)
{
    return utopia_get_xc_instance(device_id);
}
#endif
#endif
//********************XC Instance Linking List******************//
//use tgid to manage xc instance for multi process
#ifdef MSOS_TYPE_LINUX_KERNEL
#define MAX_XC_DEVICE_NUM  (2)
UTOPIA_PID_LIST *g_Utopia_XC_PID_root = NULL;

void*  utopia_get_xc_instance(int device_id)
{
    // 1. Check Pid Exist?
    MS_BOOL bExist = FALSE;
    UTOPIA_PID_LIST *CurrNode;
    CurrNode = g_Utopia_XC_PID_root;
    if(CurrNode == NULL)
    {
        printu("[UtopiaXC] Instance Table Error: CurrNode == NULL\n");
        return NULL;
    }
    while(CurrNode)
    {
        if (current->tgid == CurrNode->pid)
        {
            bExist = TRUE;
            break;
        }
    ...
    if (!bExist)
    {
        //printu("[UtopiaXC] Instance Table Error: pid not exit (%x)\n", current->tgid);
     CurrNode = g_Utopia_XC_PID_root;
     return CurrNode->instance_list[device_id];
    }

    if (device_id >= MAX_XC_DEVICE_NUM)
    {
        printu("[UtopiaXC] Instance Table Error: device_id(%u) >= MAX_XC_DEVICE_NUM \n", device_id);
        return NULL;
    }
    return CurrNode->instance_list[device_id];
}

void utopia_create_xc_instance_table(void* pInstance, MS_U32 u32DeviceID )
{
     // 1. Check Pid Exist?
    MS_BOOL bExist = FALSE;
    ...
    else
    {
        ...
        memset((void*)CurrNode, 0, sizeof(UTOPIA_PID_LIST));
        CurrNode->pid = current->tgid ;
        g_Utopia_XC_PID_root = CurrNode;
    }

    // 2. Add Instance.
    if (u32DeviceID >= MAX_XC_DEVICE_NUM/*CurrNode->instance_count >= MAX_XC_DEVICE_NUM*/)
    {
        printu("[UtopiaXC] Instance Table Error: instance_count >= MAX_XC_DEVICE_NUM.\n");
        return;
    }

    CurrNode->instance_list[u32DeviceID] = pInstance;
    printu("\33[0;32m [UtopiaXC] %s %d: (tgid = %x)  pInstance = %p,  u32DeviceID = %x  \33[m \n", __FUNCTION__,  __LINE__, current->tgid , pInstance, u32DeviceID);

    return;
}

MS_BOOL utopia_delete_xc_instance_table(void* pInstance)
{
    ...
    {
        printu("[UtopiaXC] Can't fin pid in table:%x\n", current->tgid );
        return FALSE;
    }

    for(count = 0; count < MAX_XC_DEVICE_NUM; count++)
    {
        if (pInstance == CurrNode->instance_list[count])
        {
            printu("\33[0;35m [UtopiaXC] %s %d: (tgid = %x)  pInstance = %p,  instance_count = %x   \33[m \n", __FUNCTION__,  __LINE__, current->tgid , CurrNode->instance_list[count], count);
    ...
    if (MAX_XC_DEVICE_NUM == zero_count)
    {
        //Cut link.
        if (CurrNode == g_Utopia_XC_PID_root)
        {
            NextNode = CurrNode->pNext;
            free(CurrNode);
            g_Utopia_XC_PID_root = NextNode;
        }
        else
        {
            NextNode = CurrNode->pNext;
