...
void MDrv_FRC_UpdateMDE(void *pInstance, MS_WINDOW_TYPE stDisplayWin)
{
    ...
    else if (MDrv_XC_Get_3D_Output_Mode(pInstance) != E_XC_3D_OUTPUT_MODE_NONE)
    {
        ...
        if(MHal_FRC_IsEnableFRC3D(pInstance, MAIN_WINDOW) && (pXCResourcePrivate->stdrvXC_3D._bLAToTB) && IS_OUTPUT_4K2K_60HZ_PANEL())
        {
            ...
            {
                u16Start = (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height
                    - (u16DispStart -pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart)
                    - u16Height) / 2 + u16DispStart + u16Height / 2;
        ...
        if(!IsVBY1_16LANE(u8LPLL_Type))
        {
            ...
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0F_09_L, 0x1FFF, 0x1FFF);//tgen's sub de can't be generated.
            }
        }
        // copy sub disp X
...
//g_u16MEMCLevel defined only by  (HW_DESIGN_4K2K_VER == 6)
        ...
        {
            pFRC_R2_Para.p1 = MEMC_LEVEL_BYPASS;
        }
    }
