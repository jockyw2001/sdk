...
void MDrv_XC_SetDualFrameBufferAddressSilently(void *pInstance, MS_PHY u32FBAddress, MS_PHY u32FBSize, SCALER_WIN eWindow)
{
#if defined(UFO_XC_SUPPORT_DUAL_MIU)
    MS_U8 u8MIUSel = E_CHIP_MIU_0;
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    //pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Dual_FB_Start_Addr = u32FBAddress;
    //pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Dual_FB_Size = u32FBSize;
#if defined (__aarch64__)
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"Dual_FB_Start_Addr=%lx\n",u32FBAddress);
#else
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"Dual_FB_Start_Addr=%tx\n",(ptrdiff_t)u32FBAddress);
#endif
...
void MDrv_SC_set_crop_window(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    if(!pSrcInfo->bDisplayNineLattice) // need refine for dircet display
    {
        ...
        // Error handling.
        if (IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow))//Error handling for mirror case
        {
            //adjust crop window in 2D and 3D mode.
...
void MDrv_XC_AdjustPrescalingForFrameNum(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    MS_U16 u16Alignment = OFFSET_PIXEL_ALIGNMENT - 1;
    ...
    {
        u16Alignment = 2 * OFFSET_PIXEL_ALIGNMENT - 1;
    }
...
#else
    if((IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow)) && (!pSrcInfo->bFBL)
...
#endif
    {
        if(MDrv_SC_IsForceFreerun(pInstance)) //when mirror+freerun, all progresive video timing need go 3 frame mode
        {
            ...
            {
                pSrcInfo->u16H_SizeAfterPreScaling = u32TempWidth & ~u16Alignment;
                u16H_SizeAfterPreScalingAlign = pSrcInfo->u16H_SizeAfterPreScaling;
            }
        }
    ...
    {
        if((!(IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow))) && (!pSrcInfo->bInterlace))
        {
        ...
        else if((IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow)) && (!pSrcInfo->bInterlace)
        ...
        || ((pSrcInfo->u16InputVFreq > 1200 + VFREQDELTA) && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq <= 1200)))
    {
        ...
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"force to %u or more frame mode for input vfreq > panel default vfreq\n", u16FrameCount);
...
void MDrv_SC_set_prescaling_ratio(void *pInstance, INPUT_SOURCE_TYPE_t enInputSourceType, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    {
        if(MDrv_XC_Is_SupportSWDS(pInstance, eWindow) && (MDrv_XC_GetDynamicScalingStatus(pInstance) || MDrv_XC_Is_DSForceIndexEnabled(pInstance, eWindow)))
        {
            // If 4k resolution scaling down to less than original height, force V pre-scaling
#ifdef UFO_XC_FB_LEVEL
...
//#endif
...
#if defined(UFO_XC_SET_DSINFO_V0)
        if(s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable && (!gSrcInfo[eWindow].Status2.bUpdate_DS_CMD))
#else
        if(s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable && !MDrv_XC_Is_SupportSWDS(pInstance, eWindow))
#endif
        {
    ...
    //    #endif
                    {
                        if((eWindow == MAIN_WINDOW) && (s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_ioctl != NULL))
                        {
                            ...
                            u32DeviceID = psXCInstPri->u32DeviceID;
                            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
                            bHSDRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((u32DeviceID == 1)?PQ_SUB_WINDOW:PQ_MAIN_WINDOW, E_PQ_IOCTL_HSD_SAMPLING, &stHSDSamplingInfo, sizeof(PQ_HSD_SAMPLING_INFO));
#else
                            bHSDRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((PQ_WIN)eWindow, E_PQ_IOCTL_HSD_SAMPLING, &stHSDSamplingInfo, sizeof(PQ_HSD_SAMPLING_INFO));
#endif
                            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
                    ...
                    else if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
                    {
                        if((eWindow == MAIN_WINDOW) && (s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_ioctl_ex != NULL))
                        {
                            ...
                            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
                            bHSDRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl_ex(u32DeviceID, (PQ_WIN)eWindow, E_PQ_IOCTL_HSD_SAMPLING, &stHSDSamplingInfo, sizeof(PQ_HSD_SAMPLING_INFO));
                            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    ...
    if (psXCInstPri->u32DeviceID == PQ_XC_ID_0)
        ...
        {
#if defined(UFO_XC_SET_DSINFO_V0)
            if( (s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable == NULL)|| (gSrcInfo[eWindow].Status2.bUpdate_DS_CMD))
#else
            if( (s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable == NULL)|| MDrv_XC_Is_SupportSWDS(pInstance, eWindow))
#endif
            {
                bAdvMode = 0;
            }
    ...
    if (u16SrcHWidth > pSrcInfo->u16H_SizeAfterPreScaling)
    {
        ...
        else
        {
            ...
            { // enable prescaling fitler
                ...
                {
                    H_PreScalingDownRatioAdv_SC1((u16SrcHWidth/u16div), pSrcInfo->u16H_SizeAfterPreScaling);
                }
                else
#endif
                {
                ...
                if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
                {
                    u32TmpPre = (H_PreScalingDownRatioAdv_SC1((u16SrcHWidth/u16div), pSrcInfo->u16H_SizeAfterPreScaling)) * u16div;
                }
                else
#endif
                {
            ...
            { // disable prescaling filter, use cb mode scaling ratio
...
#ifdef MSOS_TYPE_LINUX_KERNEL
                {
                    ...
                    if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
                    {
                        H_PreScalingDownRatio_SC1(u16SrcHWidth, pSrcInfo->u16H_SizeAfterPreScaling);
                    }
                    else
#endif
                    {
                ...
                if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
                {
                    u32TmpPre = (H_PreScalingDownRatio_SC1(u16SrcHWidth, pSrcInfo->u16H_SizeAfterPreScaling));
                }
                else
#endif
                {
...
void MDrv_SC_set_postscaling_ratio(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
...
#ifdef SUPPORT_Scaling_Sub1_Mode
    MS_U32 u32TmpPostFactor=0;
    MS_BOOL bIsScalingUp=FALSE;
#endif
    ...
    else
    {
     ...
     /// Set Phase Factor
#ifdef SUPPORT_Scaling_Sub1_Mode
        /// Set Phase Factor
        if(u16ScaleDst > u16ScaleSrc) // Scale Up
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
            H_PostScalingUpPhase(u16ScaleSrc, u16ScaleDst);
            u32TmpPostFactor = (MS_U32)u64_Phaseresult;
#else
            u32TmpPostFactor = H_PostScalingUpPhase(u16ScaleSrc, u16ScaleDst);
#endif
            bIsScalingUp = TRUE;
        }
        else
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
            H_PostScalingDownPhase(u16ScaleSrc, u16ScaleDst);
            u32TmpPostFactor = (MS_U32)u64_Phaseresult;
#else
            u32TmpPostFactor = H_PostScalingDownPhase(u16ScaleSrc, u16ScaleDst);
#endif
            bIsScalingUp = FALSE;
        }
#endif
...
#ifdef SUPPORT_Scaling_Sub1_Mode
    pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bIsHScalingUp = bIsScalingUp;
    pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingIniFactor = u32TmpPostFactor;
#endif
    //-----------------------------------------
    // Vertical
    //-----------------------------------------
    ...
    if (pSrcInfo->bVCusScaling == FALSE)
    {
        ...
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[PST]VSrc=%u, VDst=%u, eDeInterlaceMode=%u, bFBL=%u(%u), bInterlace=%u\n", u16ScaleSrc, u16ScaleDst, pSrcInfo->eDeInterlaceMode, pSrcInfo->bFBL, pSrcInfo->eFBLevel, bInterlace)
    }
    else
    {
    ...
    if (psXCInstPri->u32DeviceID == 0)
    {
        ...
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
            V_PostScalingUpPhase(u16ScaleSrc, u16ScaleDst);
            u32TmpPostFactor = (MS_U32)u64_Phaseresult;
#else
            u32TmpPostFactor = V_PostScalingUpPhase(u16ScaleSrc, u16ScaleDst);
#endif
            bIsScalingUp = TRUE;
        }
        else
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
            V_PostScalingDownPhase(u16ScaleSrc, u16ScaleDst);
            u32TmpPostFactor = (MS_U32)u64_Phaseresult;
#else
            u32TmpPostFactor = V_PostScalingDownPhase(u16ScaleSrc, u16ScaleDst);
#endif
            bIsScalingUp = FALSE;
        }
    }
...
#ifdef SUPPORT_Scaling_Sub1_Mode
    pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bIsVScalingUp = bIsScalingUp;
    pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingIniFactor = u32TmpPostFactor;
#endif
                                                         ...
                                                         XC_FRAME_STORE_NUMBER enPQSuggestedFrameNum)
{
    ...
    if((((IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow)) && (eWindow == SUB_WINDOW))
        || (IsVMirrorMode(eWindow) && (((gSrcInfo[eWindow].u16InputVFreq > 600 + VFREQDELTA) && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq <= 600))
        || ((gSrcInfo[eWindow].u16InputVFreq > 1200 + VFREQDELTA) && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq <= 1200)))))
    ...
    else if(((IsVMirrorMode(eWindow) || IsHMirrorMode(eWindow))
#if LOCK_FREQ_ONLY_WITHOUT_LOCK_PHASE
                        ...
                        SCALER_WIN eWindow)
{
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForXc && !gSrcInfo[eWindow].bInterlace)
    {
        switch(enPQSuggestedFrameNum)
        {
            case IMAGE_STORE_2_FRAMES:
...
XC_FRAME_STORE_NUMBER MDrv_XC_GetPQSuggestedFrameNum(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID) // SC0
    {
        MS_U32 u32DeviceID = 0;
        u32DeviceID = psXCInstPri->u32DeviceID;
        if(s_PQ_Function_Info[u32DeviceID].pq_ioctl)
        {
            MS_BOOL bRet = FALSE;
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((u32DeviceID == 1)?PQ_SUB_WINDOW:PQ_MAIN_WINDOW, E_PQ_IOCTL_PQ_SUGGESTED_FRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
#else
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((PQ_WIN)eWindow, E_PQ_IOCTL_PQ_SUGGESTED_FRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
#endif
            {
    ...
    else if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
    {
        MS_U32 u32DeviceID = 0;
        u32DeviceID = psXCInstPri->u32DeviceID;
        if(s_PQ_Function_Info[u32DeviceID].pq_ioctl_ex)
        {
            MS_BOOL bRet = FALSE;
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl_ex(u32DeviceID, (PQ_WIN)eWindow, E_PQ_IOCTL_PQ_SUGGESTED_FRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
            {
                u16FrameCount = _MDrv_XC_Get_FrameNumFromReg(pInstance, eWindow);
...
XC_FRAME_STORE_NUMBER MDrv_XC_GetPQSuggestedFRCMFrameNum(void *pInstance, SCALER_WIN eWindow)
{
...
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((u32DeviceID == 1)?PQ_SUB_WINDOW:PQ_MAIN_WINDOW, E_PQ_IOCTL_PQ_SUGGESTED_FRCMFRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
#else
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl((PQ_WIN)eWindow, E_PQ_IOCTL_PQ_SUGGESTED_FRCMFRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
#endif
            {
    ...
    else if (psXCInstPri->u32DeviceID == PQ_XC_ID_1)
    {
        ...
        if(s_PQ_Function_Info[u32DeviceID].pq_ioctl_ex)
        {
            MS_BOOL bRet = FALSE;
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
            bRet = s_PQ_Function_Info[u32DeviceID].pq_ioctl_ex(u32DeviceID, (PQ_WIN)eWindow, E_PQ_IOCTL_PQ_SUGGESTED_FRCMFRAMENUM, (void *)&u16FrameCount, sizeof(u16FrameCount));
            _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
            if(bRet == FALSE)
            {
...
//#if (HW_IPM_FETCH_ALIGNMENT==FALSE)
    if (pSrcInfo->bMemFmt422)
        gSrcInfo[eWindow].u16IPMFetch = (gSrcInfo[eWindow].u16IPMFetch + (FETCH_PIXEL_ALIGNMENT - 1)) & ~(FETCH_PIXEL_ALIGNMENT - 1);
//#endif
...
void MDrv_SC_set_dnrsetting(void *pInstance, XC_InternalStatus *pSrcInfo, MS_BOOL bIsConnectOPM, MS_PHY* pu32OneLineOffset, MS_BOOL bDoMirror, SCALER_WIN eWindow)
{
...
#endif
    {
        ...
        printf("%s %d, Attention! Scaler DNR Buffer is not enough for this timing, eWindow=%u!\n", __FUNCTION__, __LINE__,  eWindow);
    }
...
#if (HW_IPM_FETCH_ALIGNMENT==FALSE)
        if (IsHMirrorMode(eWindow) && bDoMirror)
        {
            u32OPMPixelOffset += pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16AlignWidth;
        }
#else
        if (IsHMirrorMode(eWindow) && pSrcInfo->bMemFmt422 && bDoMirror)
        {
...
void MDrv_SC_set_frcmwsetting(void *pInstance, XC_InternalStatus *pSrcInfo, MS_PHY* pu32OneLineOffset, SCALER_WIN eWindow)
{
    ...
    {
        //this case should always not be happened
        printf("%s %d, Attention! Scaler FRCM Buffer is not enough for this timing, eWindow=%u!\n", __FUNCTION__, __LINE__, eWindow);
    }
...
//#if (HW_IPM_FETCH_ALIGNMENT==FALSE)
    if ((pSrcInfo->bMemFmt422)
        ...
        *pu16IPMFetch = (*pu16IPMFetch + (FETCH_PIXEL_ALIGNMENT - 1)) & ~(FETCH_PIXEL_ALIGNMENT - 1);
//#endif
    XC_LOG_TRACE(XC_DGBLEVEL_CROPCALC," %d \n",(int)*pu16IPMFetch);
    ...
    if (IsEnableDualMode(eWindow))
    {
        ...
        if (IsHMirrorMode(eWindow))
        {
            ...
            *pu16IPMFetch = (*pu16IPMFetch + (FETCH_PIXEL_ALIGNMENT*2 - 1)) & ~(FETCH_PIXEL_ALIGNMENT*2 - 1);
      ...
      || (E_XC_3D_INPUT_PIXEL_ALTERNATIVE == MDrv_XC_Get_3D_Input_Mode(pInstance, eWindow)))
   {
    ...
    else
    {
        ...
        if (IsEnableDualMode(eWindow))
        {
            ...
            {
                //Mirro Case Crop.window already change in MDrv_SC_3D_AdjustMirrorCrop()
                u16CropRight = gSrcInfo[eWindow].ScaledCropWin.x;//(pSrcInfo->u16H_SizeAfterPreScaling - gSrcInfo[eWindow].ScaledCropWin.x - gSrcInfo[eWindow].ScaledCropWin.width);
    ...
    else
    {
        ...
        else if((u16OPMFetch != *pu16IPMFetch)  &&  (E_XC_3D_OUTPUT_MODE_NONE == MDrv_XC_Get_3D_Output_Mode(pInstance)))
        {
            //UCDi need grab six more pixels, not 3D case.(a7p need 6 more, others need 2 more)
            u16tempOPM = (( u16OPMFetch + 1 ) & ~1) + 6;
            if(u16tempOPM > pSrcInfo->u16H_SizeAfterPreScaling - (gSrcInfo[eWindow].ScaledCropWin.x - u16LBOffset))
...
#ifdef HW_OPMFETCH_PATCH
    // Do error handling for OPMfetch, OPMfetch can't be larger than IPMfetch
    if (u16OPMFetch > *pu16IPMFetch)
        u16OPMFetch = *pu16IPMFetch;
#endif
    ...
    if (IsEnableDualMode(eWindow))
    {
...
#if 0 //Mirro Case Crop.window already change in MDrv_SC_3D_AdjustMirrorCrop()
        if (IsHMirrorMode(eWindow))
        {
        ...
        if (u16OPMOffsetRight > u16OPMCropRight) // OPM Crop.right at Right side
        {
            if (u16OPMCropLeft > u16OPMOffsetLeft) // OPM all Left side data was cropped
            {
                if (bIsLeftFactor == TRUE)
                {
                    u16OPMFetch = FETCH_PIXEL_ALIGNMENT;
            ...
            else // OPM Left/right side have valid data after crop
            {
                ...
                if (bIsLeftFactor == TRUE)
                {
                    ...
                    if(IS_INPUT_SIDE_BY_SIDE_HALF(eWindow)
                      ...
                      || IS_INPUT_SIDE_BY_SIDE_FULL(eWindow))
                    {
                        u16OPMFetch = (u16OPMFetch + (FETCH_PIXEL_ALIGNMENT*2 - 1)) & ~(FETCH_PIXEL_ALIGNMENT*2 - 1);
                    }
                    else
                    {
                        u16OPMFetch = (u16OPMFetch + (FETCH_PIXEL_ALIGNMENT - 1)) & ~(FETCH_PIXEL_ALIGNMENT - 1);
                    }
                }
                else // OPM Right side
                {
...
static MS_BOOL _MDrv_SC_check_2p_mode(void *pInstance, MS_BOOL bEnable,SCALER_WIN eWindow)
{
...
#endif
    {
        ...
        if (IsSrcTypeDTV(gSrcInfo[eWindow].enInputSourceType) || IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType))
            u32InputCLK = (MS_U32)(u16Width+MVOP_H_BLANKING)*(MS_U32)(u16Height+MVOP_V_BLANKING)*((u32Vfreqx10+5)/10);
        else
    ...
    else if(psXCInstPri->u32DeviceID == 0x01)
    {
        if(eWindow == MAIN_WINDOW)
        {
            ...
            else if(IsSrcTypeSC0DI(gSrcInfo[eWindow].enInputSourceType))
            {
                //di2sc1 road ve bypass setting
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_0A_L, BIT(15), BIT(15));
