...
MS_BOOL MDrv_XC_Cancel_PollEvent(void)
{
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(_s16FdDolby!=0)
    {
...
MS_BOOL MDrv_XC_PollEvent(void)
{
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    struct pollfd pfds;
    ssize_t ret_in;
    ...
    _s16FdDolby = open("/proc/xc/dolby_hdr",O_RDONLY);
    if(_s16FdDolby == -1)
    {
        printf("open /proc/xc/dolby_hdr fail!\n");
        return FALSE;
    }else
    {
        //printf("\033[1;31m[%s:%d]UT:OPEN SUCCESS\033[m\n",__FUNCTION__,__LINE__);
        pfds.fd = _s16FdDolby;
        pfds.events = POLLIN;
        pfds.revents = 0;
        poll(&pfds,1,0);
        //printf("\033[1;31m[%s:%d]UT:POLL DOWN\033[m\n",__FUNCTION__,__LINE__);
        ret_in = read(_s16FdDolby,&buffer,sizeof(buffer));
        //printf("\033[1;31m[%s:%d]UT:READ DOWN\033[m\n",__FUNCTION__,__LINE__);
        close(_s16FdDolby);
        _s16FdDolby = 0;
...
MS_BOOL MApi_XC_PollEvent_U2(void *pInstance)
{
    return MDrv_XC_PollEvent();
}
MS_BOOL MApi_XC_Cancel_PollEvent_U2(void *pInstance)
{
    return MDrv_XC_Cancel_PollEvent();
}
MS_BOOL MApi_XC_PollEvent(void)
{
    ...
    stXC_POLL_EVENT XCArgs;
    XCArgs.bReturnValue = FALSE;
    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_POLL_EVENT, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
...
MS_BOOL MApi_XC_Cancel_PollEvent(void)
{
    ...
    stXC_CANCEL_POLL_EVENT XCArgs;
    XCArgs.bReturnValue = FALSE;

    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_CANCEL_POLL_EVENT, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return FALSE;
    }
...
void MDrv_SC_GenerateBlackVideo(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "SC%ld GenerateBlackVideo enable = %d,ewindow = %d\n", psXCInstPri->u32DeviceID,bEnable,eWindow);
#else
    ...
    else
    {
        ...
        if((MDrv_XC_MLoad_GetStatus(g_pDevice0Instance) == E_MLOAD_ENABLED))
                             ...
                             SCALER_WIN eWindow)
{
    ...
    /// For Force P case, IP always get 50HZ P timing,Two Fields are hiding in One Sync
    /// So We Need to use 2:2 to Lock
    if(gSrcInfo[eWindow].Status2.bEnable_ForceP == TRUE)
    {
        *u8FrcIn = 2;
        *u8FrcOut = 2;
    }
...
static void _MDrv_XC_SetHVSync(void *pInstance, MS_BOOL bHSync, MS_U16 u16SyncWidth, MS_U16 u16SyncBackPorch, MS_U16 u16HVTotal)
{
    ...
    else
    {
        ...
        {
            u16VsyncStart = (MS_U16)stTCONPanelConfig.stPanelInfo.u32VsyncStart >> 8;
            MS_U16 u16VsyncEnd = (MS_U16)stTCONPanelConfig.stPanelInfo.u32VsyncStart >> 8;

            if( (u16VsyncStart!=0)&&(u16VsyncEnd!=0))
            {
                u16NewVsyncStart = u16VsyncStart;
                u16NewVsyncEnd = u16VsyncEnd+1;
            }
        }
...
#if (defined(CONFIG_PURE_SN))
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "IP vFreqx1K= %ld\n", u32VFreq);
#else
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "IP vFreqx1K= %d\n", u32VFreq);
#endif
        if (u32VFreq != 0)
        {
...
#if (defined(CONFIG_PURE_SN))
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "IP vFreqx1K= %lx\n", u32VFreq);
#else
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "IP vFreqx1K= %x\n", u32VFreq);
#endif
        if (u32VFreq != 0)
        {
            u16IPVsyncPeriod = (MS_U16)((1000000UL + u32VFreq - 1)/u32VFreq);
...
static void _MApi_XC_FPLL_AdjustOvsVtt(void *pInstance, MS_U32 u32AccurateVPeriod)
{
    ...
    if(u32AccurateVPeriod != 0)
    {
        ...
        {
            u16OutVtt = u16OriOutVtt; //no need to change vtt if it isn't CHG_VTOTAL mode. Some panels will find the flick when the Vtt is changed even the change is 1
        }
...
static void _MApi_XC_FPLL_EnableLcokFreqOnly(void *pInstance,MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        ...
        if(pXCResourcePrivate->stdrvXC_Display._bFpllCusFlag[E_FPLL_FLAG_PHASELIMIT])
        {
            MS_U16 u16CusPhaseLimit = (MS_U16)pXCResourcePrivate->stdrvXC_Display._u32FpllCusData[E_FPLL_FLAG_PHASELIMIT];
            XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "user forces Phase Limit = 0x%X\n", u16CusPhaseLimit);
            MDrv_Write2Byte(L_BK_LPLL(0x0A), u16CusPhaseLimit);
        }
        else
        {
...
static MS_BOOL _MApi_XC_FPLL_LockFreqOnly(void *pInstance)
{
    ...
    MS_U32 u32RealInDiv=0, u32RealOutDiv=0;
    MS_U32 u32GCD = 1;
    MS_U8   i;
    MS_U32 u32MaxOutVtt =1, u32MaxGCD = 1;

    UNUSED(u32RealInDiv);
    UNUSED(u32RealOutDiv);
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
...
#else
        XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "not safe case, u16PhaseDiff %x u16Limitoffset: %x \r\n ", (ptrdiff_t)u32PhaseDiff, (ptrdiff_t)u16Limitoffset);
#endif
        return FALSE;
    }
    else
    ...
    if (eWindow == MAIN_WINDOW)
    {
        ...
        if (IsVBY1_16LANE(u8LPLL_Type))
        {
            ...
            if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled) &&(psXCInstPri->u32DeviceID != 1))
            {
                ...
                // avoid pixshift show error,HW suggest pixshift 120HZ case set different pipe delay
                if((pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange != 0))
                {
                    if (MHal_FRC_IsFHDToFSC(pInstance, eWindow))
                    {
                        ...
                        if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
                        {
                            MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_FHD, (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX + pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange));
                        }
                        else
                        {
                            MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_FHD, (FRC_PIPE_DELAY_VCNT_FSC_FHD + pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange));
                        }
                        #else
                        MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_FHD, (FRC_PIPE_DELAY_VCNT_FSC_FHD + pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange));
                        #endif
                    ...
                    {
                        MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_4K_120Hz, (FRC_PIPE_DELAY_VCNT_FSC_4K_120Hz + pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange/2));
                    }
                }
                else
                {
                ...
                if (pTimingInfo->u16InputVFreq > 0 && (gSrcInfo[eWindow].bEnableFPLL == TRUE))
                {
        ...
        if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled) &&(psXCInstPri->u32DeviceID != 1))
        {
            ...
            if (MHal_FRC_IsFHDToFSC(pInstance, eWindow))
            {
                // FHD in to UHD out case, ODCLK2 as ODCLK/4
                MHal_FRC_Set_STGEN_ODCLK(pInstance, E_FRC_STGEN_ODCLK_4, bUseMenuload);
            }
                  ...
                  && (IS_OUTPUT_4K2K(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)))
            {
                // 4K 24/25/30 + output 4K 50/60 case, ODCLK2 as ODCLK/2 for power saving
                MHal_FRC_Set_STGEN_ODCLK(pInstance, E_FRC_STGEN_ODCLK_2, bUseMenuload);
            }
            else
            {
                // 4K 50/60 to 4K output case or FHD in to FHD out case., ODLCK2 as ODCLK
            ...
            else
            {
                    ...
                    &&(pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange == 0)) //Check if PIXEL FEATURE is NOT enable.
                {
                    ...
                    MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_FHD, FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX);
                    #else
                    MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FSC_FHD, FRC_PIPE_DELAY_VCNT_FSC_FHD);
                    #endif
                }
...
#endif
void MDrv_XC_SetFrameColor(void *pInstance, MS_U32 u32aRGB, MS_BOOL bYUV )
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    Hal_XC_SetFrameColor(pInstance, u32aRGB, bYUV);
}
...
/// -  2:Blue
//-------------------------------------------------------------------------------------------------
void MApi_XC_SetFrameColor_U2 ( void* pInstance, MS_U32 u32aRGB, MS_BOOL bYUV )
{
    //MS_U8 u8Bank;
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    ...
    //MDrv_WriteByte(BK_SELECT_00, u8Bank);
    MDrv_XC_SetFrameColor(pInstance, u32aRGB, bYUV);
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);

}
#ifndef UTOPIAXP_REMOVE_WRAPPER
void MApi_XC_SetFrameColor ( MS_U32 u32aRGB, MS_BOOL bYUV)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
...
void MDrv_XC_SetFreeRunTiming (void* pInstance)
{
    ...
    switch (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type)
    {
        ...
        case E_XC_PNL_LPLL_DAC_P:
            ...
            if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_10_L, BIT(15)|BIT(14), 0xF700); // set output vsync manually, bit 11 per RD's suggestion take it to be the default
            }
            else
#endif
...
#if ((HW_DESIGN_4K2K_VER == 4)||(HW_DESIGN_4K2K_VER == 6)||(HW_DESIGN_4K2K_VER == 1))
...
E_APIXC_ReturnValue MDrv_XC_SetOSDLayer(void *pInstance, E_VOP_OSD_LAYER_SEL eVOPOSDLayer, SCALER_WIN eWindow)
{
    ...
    switch (eVOPOSDLayer)
    {
        ...
        case E_VOP_LAYER_FRAME_MUX1_MUX2_MUX3_MUX4_VIDEO:
            u16RegVal = 5;
            break;
        case E_VOP_LAYER_FRAME_MUX1_MUX2_MUX3_MUX4_MUX5_VIDEO:
            u16RegVal = 6;
            break;
        default:
...
E_APIXC_ReturnValue MDrv_SC_ShiftDisplayWindowDirectly(void *pInstance, MS_S8 s8H, MS_S8 s8V, MS_S8 s8preH, MS_S8 s8preV)
{
    ...
    if (IsFSCDisplayWindowEnable())
    {
        if((pXCResourcePrivate->stdrvXC_MVideo.gePixelShiftFeature & PIXEL_SHIFT_DO_OVERSCAN) == PIXEL_SHIFT_DO_OVERSCAN)
        {
            ...
            dsp_main.x = gSrcInfo[MAIN_WINDOW].stDispWin.x + pXCResourcePrivate->stdrvXC_MVideo.gu16HPixelShiftRange / 2;
            dsp_main.y = gSrcInfo[MAIN_WINDOW].stDispWin.y + pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange / 2;
        }
        else
        {
            dsp_main.x = gSrcInfo[MAIN_WINDOW].stDispWin.x;
            dsp_main.y = gSrcInfo[MAIN_WINDOW].stDispWin.y;
        }
        if(MHal_FRC_IsFHDToFSC(pInstance, MAIN_WINDOW))
        {
            if((pXCResourcePrivate->stdrvXC_MVideo.gePixelShiftFeature & PIXEL_SHIFT_DO_OVERSCAN) == PIXEL_SHIFT_DO_OVERSCAN)
            {
                //just do overscan led display window bigger
                dsp_main.x = dsp_main.x/2- pXCResourcePrivate->stdrvXC_MVideo.gu16HPixelShiftRange / 2;
                dsp_main.y = dsp_main.y/2- pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange / 2;
            ...
            {
                dsp_main.x = dsp_main.x/2;
                dsp_main.y = dsp_main.y/2;
        ...
        else//4K2k IN FSC
        {
            if((pXCResourcePrivate->stdrvXC_MVideo.gePixelShiftFeature & PIXEL_SHIFT_DO_OVERSCAN) == PIXEL_SHIFT_DO_OVERSCAN)
            {
                dsp_main.x = dsp_main.x- pXCResourcePrivate->stdrvXC_MVideo.gu16HPixelShiftRange / 2;
                dsp_main.y = dsp_main.y- pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange / 2;
            }
            else
            {
                dsp_main.x = dsp_main.x;
                dsp_main.y = dsp_main.y;
            }
    ...
    if(MHal_FRC_IsFHDToFSC(pInstance, MAIN_WINDOW))
    {
        dsp_main.y += (pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange/2)+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart/2 +2;
    }
    else
    {
        dsp_main.y += (pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange/2)+2;
    }
#endif
    ...
    else // BOX zorder
    {
        ...
        switch (enlayer) // translate layer to register value
        {
            case E_VIDEO_ON_OSD_LAYER_DEAFULT:
                ...
                break;
            case E_VIDEO_ON_OSD_LAYER_0:
                u16RegVal = 1;    ///< osd layer 0 -> video -> osd layer 1 -> osd layer 2 -> osd layer 3 -> osd layer 4
                break;
            case E_VIDEO_ON_OSD_LAYER_1:
                u16RegVal = 2;    ///< osd layer 0 -> osd layer 1 -> video -> osd layer 2 -> osd layer 3 -> osd layer 4
                break;
            case E_VIDEO_ON_OSD_LAYER_2:
                u16RegVal = 3;    ///< osd layer 0 -> osd layer 1 -> osd layer 2 -> video -> osd layer 3 -> osd layer 4
                break;
            case E_VIDEO_ON_OSD_LAYER_3:
                u16RegVal = 4;    ///< osd layer 0 -> osd layer 1 -> osd layer 2 -> osd layer 3 -> video -> osd layer 4
                break;
            case E_VIDEO_ON_OSD_LAYER_4:
            default:
                u16RegVal = 5;    ///< osd layer 0 -> osd layer 1 -> osd layer 2 -> osd layer 3 -> osd layer 4 -> video
                break;
        }
...
E_APIXC_ReturnValue MDrv_XC_SetPixelShift(void* pInstance, MS_S8 s8H, MS_S8 s8V)
{
    ...
    //when FHD into FSC,here update 4K window will show garbage,so mask it
#if (!(FRC_INSIDE && (HW_DESIGN_4K2K_VER == 6)))
...
E_APIXC_ReturnValue MDrv_XC_HDR_Control(void* pInstance, EN_XC_HDR_CTRL_TYPE enCtrlType, void *pParam)
{
    ...
    switch(enCtrlType)
    {
        ...
        case E_XC_HDR_CTRL_CFD_CONTROL:
        {
            ...
            ST_KDRV_XC_CFD_OSD stKdrvOsdControl;
            memset(&stKdrvOsdControl, 0, sizeof(ST_KDRV_XC_CFD_OSD));
            if (stCfdControlInfo.enCtrlType == E_XC_CFD_CTRL_TYPE_OSD)
            {
                XC_CFD_OSD_CONTROL* pstOsdControl = (XC_CFD_OSD_CONTROL*)stCfdControlInfo.pParam;
                XC_CFD_OSD_CONTROL stOsdControl;
                memset(&stOsdControl, 0, sizeof(XC_CFD_OSD_CONTROL));
                MS_U16 u16Length = pstOsdControl->u16Length;

                MS_U16 u16CpLength = sizeof(XC_CFD_OSD_CONTROL);
                if (u16Length < u16CpLength)
                {
                    u16CpLength = u16Length ;
                }
                memcpy(&stOsdControl, pstOsdControl, u16CpLength);

                stKdrvOsdControl.u32Version = KDRV_XC_OSD_CONTROL_VERSION;
                stKdrvOsdControl.u16Length = sizeof(ST_KDRV_XC_CFD_OSD);
                stKdrvOsdControl.u16Hue = stOsdControl.u16Hue;
                stKdrvOsdControl.u16Saturation = stOsdControl.u16Saturation;
                stKdrvOsdControl.u16Contrast = stOsdControl.u16Contrast;
                stKdrvOsdControl.u8OSDUIEn = stOsdControl.u8OSDUIEn;
                stKdrvOsdControl.u8OSDUIMode = stOsdControl.u8OSDUIMode;
                stKdrvOsdControl.bHueValid = stOsdControl.bHueValid;
                stKdrvOsdControl.bSaturationValid = stOsdControl.bSaturationValid;
                stKdrvOsdControl.bContrastValid = stOsdControl.bContrastValid;
                stKdrvOsdControl.bBacklightValid = stOsdControl.bBacklightValid;
                stKdrvOsdControl.u32MinBacklight = stOsdControl.u32MinBacklight;
                stKdrvOsdControl.u32MaxBacklight = stOsdControl.u32MaxBacklight;
                stKdrvOsdControl.u32Backlight = stOsdControl.u32Backlight;
                stKdrvOsdControl.u8Win = stOsdControl.u8Win;
                stKdrvOsdControl.bColorRangeValid = stOsdControl.bColorRangeValid;
                stKdrvOsdControl.u8ColorRange = stOsdControl.u8ColorRange;
                stKdrvOsdControl.bUltraBlackLevelValid = stOsdControl.bUltraBlackLevelValid;
                stKdrvOsdControl.u8UltraBlackLevel = stOsdControl.u8UltraBlackLevel;
                stKdrvOsdControl.bUltraWhiteLevelValid = stOsdControl.bUltraWhiteLevelValid;
                stKdrvOsdControl.u8UltraWhiteLevel = stOsdControl.u8UltraWhiteLevel;
                stKdrvOsdControl.bSkipPictureSettingValid = stOsdControl.bSkipPictureSettingValid;
                stKdrvOsdControl.bSkipPictureSetting = stOsdControl.bSkipPictureSetting;
                stKdrvOsdControl.bColorCorrectionValid = stOsdControl.bColorCorrectionValid;
                memcpy(stKdrvOsdControl.s16ColorCorrectionMatrix, stOsdControl.s16ColorCorrectionMatrix, sizeof(stOsdControl.s16ColorCorrectionMatrix));
                stKdrvOsdControl.bViewModeValid = stOsdControl.bViewModeValid;
                stKdrvOsdControl.u8ViewMode = stOsdControl.u8ViewMode;

                stXCCfdControlInfo.pParam = &stKdrvOsdControl;
                stXCCfdControlInfo.u32ParamLen = sizeof(ST_KDRV_XC_CFD_OSD);
            }
            else
            {
        ...
        case E_XC_HDR_CTRL_SET_HDR3DLUT://
        {
            XC_HDR_SET_HDR3DLUT stSETHDR3DLUT = *(XC_HDR_SET_HDR3DLUT *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_HDREOTF://
        {
            XC_HDR_SET_HDREOTF stSETHDREOTF = *(XC_HDR_SET_HDREOTF *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_HDROETF://
        {
            XC_HDR_SET_HDROETF stSETHDROETF = *(XC_HDR_SET_HDROETF *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_TOENMAP://
        {
            XC_HDR_SET_TOENMAP stSETTOENMAP = *(XC_HDR_SET_TOENMAP *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_HLGYGAIN://
        {
            XC_HDR_SET_HLGYGAIN stSETHLGYGAIN = *(XC_HDR_SET_HLGYGAIN *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_INIT_DOLBYPICTURECONFIG://
        {
            XC_HDR_INIT_DOLBYPICTURECONFIG stDOLBYPICTURECONFIG = *(XC_HDR_INIT_DOLBYPICTURECONFIG *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_DOLBYPICTUREMODE://
        {
            XC_HDR_SET_DOLBYPICTUREMODE stSETDOLBYPICTUREMODE = *(XC_HDR_SET_DOLBYPICTUREMODE *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_DOLBYBACKLIGHT://
        {
            XC_HDR_SET_DOLBYBACKLIGHT stSETDOLBYBACKLIGHT = *(XC_HDR_SET_DOLBYBACKLIGHT *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_GET_DOLBYSWVERSION:
        {
            XC_HDR_GET_DOLBYSWVERSION stDOLBYPICTURECONFIG = *(XC_HDR_GET_DOLBYSWVERSION *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_INVGAMMAEN://
        {
            XC_HDR_INVGAMMAEN stINVGAMMAEN = *(XC_HDR_INVGAMMAEN *)pParam;

            break;
        }
        case E_XC_HDR_CTRL_SET_PQMODEINFO://
        {
            XC_HDR_SET_PQMODEINFO stSETPQMODEINFO = *(XC_HDR_SET_PQMODEINFO *)pParam;
...
E_APIXC_ReturnValue MDrv_XC_HDR_GetCaps(void *pInstance, XC_HDR_SUPPORTED_CAPS *pstHDRCaps)
{
    ...
    if (_XC_RIU_BASE == 0)
    {
        printf("[%s,%5d]_XC_RIU_BASE is 0, please init _XC_RIU_BASE by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
        return E_APIXC_RET_FAIL;
    }
...
MS_BOOL MDrv_XC_SetSWDRInfo(void* pInstance, XC_SWDR_INFO *pSWDR_INFO)
{
    ...
    if(pSWDR_INFO == NULL)
    {
        printf("Warning : SWDR is Empty!");
        return FALSE;
    }

    stKdrvXcSwdrInfo.bDRE_En        = pSWDR_INFO->bDRE_En;
    stKdrvXcSwdrInfo.bDRE_SWDR_En   = pSWDR_INFO->bDRE_SWDR_En;
    stKdrvXcSwdrInfo.pu16Data       = pSWDR_INFO->pu16Data;
    stKdrvXcSwdrInfo.u16NumofData    = pSWDR_INFO->u16NumofData;
    stKdrvXcSwdrInfo.u16DataIndex   = pSWDR_INFO->u16DataIndex;
    stKdrvXcSwdrInfo.eWindow        = pSWDR_INFO->eWindow;
...
MS_BOOL MDrv_XC_GetSWDRInfo(void* pInstance, XC_SWDR_INFO *pSWDR_INFO)
{
    ...
    pSWDR_INFO->pu16Data        = stKdrvXcSwdrInfo.pu16Data;
    pSWDR_INFO->u16NumofData     = stKdrvXcSwdrInfo.u16NumofData;
    pSWDR_INFO->u16DataIndex    = stKdrvXcSwdrInfo.u16DataIndex;
    pSWDR_INFO->eWindow         = stKdrvXcSwdrInfo.eWindow;
#endif
...
void MDrv_XCMdbIoctl_EchoCmd(MDBCMD_CMDLINE_PARAMETER *paraCmdLine)
{
    if(MHal_XCMdbIoctl_EchoCmd != NULL)
    {
        MHal_XCMdbIoctl_EchoCmd(paraCmdLine);
    }
}
void MDrv_XCMdbIoctl_Cat(MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    if(MHal_XCMdbIoctl_Cat != NULL)
    {
        MHal_XCMdbIoctl_Cat(paraGetInfo);
    }
}
#endif
