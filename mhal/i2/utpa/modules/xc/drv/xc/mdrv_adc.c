    ...
    if (bRet)
    {
        ...
        if (bRet)
        {
            ...
            if (bRet)
            {
                ...
                if (bRet)
                {
                    Hal_ADC_gain_setting(pInstance, CAST_ALIGN(XC_AdcGainOffsetSetting)pstInitialGainOffset);
                    Hal_ADC_offset_setting(pInstance, CAST_ALIGN(XC_AdcGainOffsetSetting)pstInitialGainOffset);
                }
...
void MApi_XC_ADC_ExitExternalCalibration(void *pInstance, INPUT_SOURCE_TYPE_t eSource, APIXC_AdcGainOffsetSetting* InitialGainOffset)
{
    ...
    Hal_ADC_ExitExternalCalibration(pInstance, eADCSource,CAST_ALIGN(XC_AdcGainOffsetSetting)InitialGainOffset);
...
{
    _XC_ENTRY(pInstance);
    if(MDrv_SC_CheckMuteStatusByRegister(pInstance, MAIN_WINDOW) == FALSE)
    {
