...
static void _mdrv_HDMI_PollingTask(void)
{
    ...
    while(bHDMITaskProcFlag)
    {
        ...
        else
        {
            ...
            {
                Hal_HDMI_StablePolling(pHDMIRxResourcePrivate->ucMHLSupportPath, pHDMIRxResourcePrivate->ulPacketStatus, pHDMIRxResourcePrivate->stHDMIPollingInfo);
...
static void _mdrv_HDCP_InterruptTask(void)
{
    ...
    if(slHDCPMutexID != -1)
    {
        MsOS_ObtainMutex(slHDCPMutexID, MSOS_WAIT_FOREVER);
    }

    while(bHDCPTaskProcFlag)
    {
        //MDRV_HDMIRX_MSG_INFO("** TMDS enter wait event kernel space !!!!!!\r\n");
    ...
    {
        slHDCPInterruptTaskID = -1;
        bHDCPInterruptCreateFlag = FALSE;
    }

    if(slHDCPMutexID != -1)
    {
        MsOS_ReleaseMutex(slHDCPMutexID);
    }
}
...
void MDrv_HDMI_init( void )
{
    ...
    if(XCArgs.bCreateHDCPTaskFlag)
    {
        slHDCPMutexID = MsOS_CreateMutex(E_MSOS_FIFO, "HDCP_Mutex", MSOS_PROCESS_SHARED);
...
void mdrv_dvi_reset_U2(void* pInstance)
{
    ...
    if(Hal_DVI_irq_info(MDrv_XC_Mux_GetHDMIPort(pInstance, enInputSourceType), IRQ_DVI_CK_CHG) && (!pXCResourcePrivate->stdrv_HDMI._ResetDone))
    {
        ...
        Hal_DVI_irq_clear(MDrv_XC_Mux_GetHDMIPort(pInstance, enInputSourceType), (MS_U16)~IRQ_ALL_BIT);
...
#if defined(STELLAR) && defined(PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB) //for L project live zoom update hdmi info to main2
    XC_RESOURCE_PRIVATE* pXCResourcePrivate_1 = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(1)],(void**)(&pXCResourcePrivate_1));
    if(IsSrcTypeHDMI(gSrcInfo_1[MAIN_WINDOW].enInputSourceType))
    {
...
void MDrv_HDMI_PROG_DDCRAM_internal(void *pInstance, XC_DDCRAM_PROG_INFO *pstDDCRam_Info, MS_U32 u32SizeOfInfo)
{
    ...
    if(pstDDCRam_Info->eDDCProgType <= HDMI_GET_PORT_SELECT(INPUT_PORT_DVI_END))
    {
        MDrv_HDMI_DDCRAM_Enable(pstDDCRam_Info->eDDCProgType);
...
MS_U32 MDrv_HDMI_SetPowerState_U2(void* pInstance, EN_POWER_MODE enPowerState)
{
    ...
    else if (enPowerState == E_POWER_RESUME)
    {
        if (pXCResourcePrivate->stdrv_HDMI._prev_enPowerState == E_POWER_SUSPEND)
        {
            // EDID and DDC Ram
            {
            if(INPUT_PORT_DVI_END >= INPUT_PORT_DVI0)
            {
                // init Digital EDID 0
            ...
            if(INPUT_PORT_DVI_END >= INPUT_PORT_DVI1)
            {
                // init Digital EDID 1
            ...
            if(INPUT_PORT_DVI_END >= INPUT_PORT_DVI2)
            {
                // init Digital EDID 2
            ...
            if(INPUT_PORT_DVI_END >= INPUT_PORT_DVI3)
            {
                // init Digital EDID 3
            ...
            //clear all HPD
            for (eHDMIPort = INPUT_PORT_DVI0;eHDMIPort <= INPUT_PORT_DVI_END;eHDMIPort++)
            {
...
MS_U32 MDrv_HDMI_SetPowerState(EN_POWER_MODE enPowerState)
{
    ...
    stHDCP22_SET_EVENT stHDMIRxHDCPArgs;
    ...
    stHDMIRxHDCPArgs.usHDCPEventIndex = HDMI_HDCP_EVENT_STR_SUSPEND;

    if(enPowerState == E_POWER_SUSPEND)
    {
        if(bHDCPInterruptProcFlag)
        {
            bHDCPInterruptProcFlag = FALSE;
        }
        ...
        else if(UtopiaIoctl(pulHDMIRxInst, MAPI_CMD_HDMIRX_HDCP22_SET_EVENT, (void*)&stHDMIRxHDCPArgs) != 0)
        {
            MDRV_HDMIRX_MSG_ERROR("Obtain HDMI Rx engine fail\n");
        }
    }
...
MS_BOOL MDrv_HDMI_ParseHDRInfoFrame(void* pInstance, MS_U8 ucHDMIInfoSource, void *pPacket, sHDR_METADATA *pHdrMetadata)
{
    ...
    {
        _mdrv_HDMI_ParseHDRInfoFrame(pPacket, pHdrMetadata);
...
MS_BOOL MDrv_HDMI_Extend_Packet_Received(void* pInstance, MS_U8 ucHDMIInfoSource, MS_BOOL bEnable, MS_HDMI_EXTEND_PACKET_RECEIVE_t *pExtendPacketReceive)
{
    ...
    stExtendPacketReceive.bPKT_AUDIO_DST_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_DST_RECEIVE_FLAG)? TRUE: FALSE;
    stExtendPacketReceive.bPKT_AUDIO_3D_ASP_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_3D_ASP_RECEIVE_FLAG)? TRUE: FALSE;
    stExtendPacketReceive.bPKT_AUDIO_3D_DSD_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_3D_DSD_RECEIVE_FLAG)? TRUE: FALSE;
    stExtendPacketReceive.bPKT_AUDIO_METADATA_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_METADATA_RECEIVE_FLAG)? TRUE: FALSE;
    stExtendPacketReceive.bPKT_AUDIO_MULTI_ASP_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_MULTI_ASP_RECEIVE_FLAG)? TRUE: FALSE;
    stExtendPacketReceive.bPKT_AUDIO_MULTI_DSD_RECEIVE = (pXCResourcePrivate->stdrv_HDMI.ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AUDIO_MULTI_DSD_RECEIVE_FLAG)? TRUE: FALSE;
...
MS_BOOL MDrv_HDMI_Ctrl(MS_U32 u32Cmd, void *pBuf, MS_U32 u32BufSize)
{
    ...
    if(u32Cmd == E_HDMI_HDR_INFOFRAME_GET)
    {
        u32Cmd = E_HDMI_INTERFACE_CMD_HDR_INFOFRAME_GET;
    }
    else if(u32Cmd == E_HDMI_EXTEND_PACKET_RECEIVE_GET)
    {
        u32Cmd = E_HDMI_INTERFACE_CMD_EXTEND_PACKET_RECEIVE_GET;
    }
...
//
//**************************************************************************
MS_BOOL MDrv_HDMI_GetHDRInfoFrame(MS_U16 *pusHDRInfoFrame)
{
    stHDMI_GET_HDR_INFOFRAME stHDMIRxArgs = {0};

    stHDMIRxArgs.bHDRReceiveFlag = FALSE;
    stHDMIRxArgs.ucHDMIInfoSource = ucHDMIInfoSourceSelect;
    stHDMIRxArgs.pusHDRInfoFrame = pusHDRInfoFrame;

    if(pulHDMIRxInst == NULL)
    {
        MDRV_HDMIRX_MSG_WARNING("[%s,%5d]No instance existed, please get an instance by calling MDrv_HDMI_init() first\n",__FUNCTION__,__LINE__);
    }
    else if(UtopiaIoctl(pulHDMIRxInst, MAPI_CMD_HDMIRX_GET_HDR_INFO_FRAME, (void*)&stHDMIRxArgs) != 0)
    {
        MDRV_HDMIRX_MSG_ERROR("Obtain HDMI Rx engine fail\n");
    }

    return stHDMIRxArgs.bHDRReceiveFlag;
}

//**************************************************************************
//  [Function Name]:
//                  MDrv_HDMI_ExtendPacketReceived
//  [Description]:
//
//  [Arguments]:
//
...
//
//**************************************************************************
MS_BOOL MDrv_HDMI_ExtendPacketReceived(MS_HDMI_EXTEND_PACKET_RECEIVE_t *pExtendPacketReceive)
{
    MS_BOOL bReturn = FALSE;
    MS_U16 usCopiedLength = sizeof(MS_HDMI_EXTEND_PACKET_RECEIVE_t);
    stHDMI_PACKET_RECEIVED stHDMIRxArgs = {0};
    MS_HDMI_EXTEND_PACKET_RECEIVE_t stExtendPacketReceive = {0};

    stHDMIRxArgs.bEnable = TRUE;
    stHDMIRxArgs.ucHDMIInfoSource = ucHDMIInfoSourceSelect;
    stHDMIRxArgs.ulPacketStatus = 0;

    if((NULL == pExtendPacketReceive) || (pExtendPacketReceive->u16Size == 0))
    {
        printf("MDrv_HDMI_Extend_Packet_Received: Null parameter or Wrong u16Size\n");
    }
    else if(pExtendPacketReceive->u16Version < 1)
    {
        printf("MDrv_HDMI_Extend_Packet_Received: please check your u16Version, it should not set to 0\n");
    }
    else if(pulHDMIRxInst == NULL)
    {
        MDRV_HDMIRX_MSG_WARNING("[%s,%5d]No instance existed, please get an instance by calling MDrv_HDMI_init() first\n",__FUNCTION__,__LINE__);
    }
    else if(UtopiaIoctl(pulHDMIRxInst, MAPI_CMD_HDMIRX_GET_PACKET_RECEIVE, (void*)&stHDMIRxArgs) != 0)
    {
        MDRV_HDMIRX_MSG_ERROR("Obtain HDMI Rx engine fail\n");
    }
    else
    {
        ...
        stExtendPacketReceive.bPKT_MPEG_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_MPEG_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUI_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_SPD_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_SPD_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AVI_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AVI_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_GC_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_GCP_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ASAMPLE_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_SAMPLE_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ACR_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_ACR_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_VS_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_VS_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_NULL_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_NULL_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ISRC2_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_ISRC2_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ISRC1_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_ISRC1_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ACP_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_ACP_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_ONEBIT_AUD_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_DSD_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_GM_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_GM_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_HBR_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_HBR_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_VBI_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_VBI_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_HDR_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_HDR_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_RSV_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_RESERVED_PACKET_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_EDR_VALID = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_EDR_VALID_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_DST_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_DST_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_3D_ASP_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_3D_ASP_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_3D_DSD_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_3D_DSD_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_METADATA_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_METADATA_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_MULTI_ASP_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_MULTI_ASP_RECEIVE_FLAG)? TRUE: FALSE;
        stExtendPacketReceive.bPKT_AUDIO_MULTI_DSD_RECEIVE = (stHDMIRxArgs.ulPacketStatus &HDMI_STATUS_AUDIO_MULTI_DSD_RECEIVE_FLAG)? TRUE: FALSE;

        memcpy(pExtendPacketReceive, &stExtendPacketReceive, usCopiedLength);
...
{
    stHDMI_DATA_RTERM_CONTROL stHDMIRxArgs = {0};

    stHDMIRxArgs.enInputPortType = enInputPortType;
    stHDMIRxArgs.bDataRtermEnable = bDataRtermEnable;

    if(pulHDMIRxInst == NULL)
    {
        MDRV_HDMIRX_MSG_WARNING("[%s,%5d]No instance existed, please get an instance by calling MDrv_HDMI_init() first\n",__FUNCTION__,__LINE__);
    }
    else if(UtopiaIoctl(pulHDMIRxInst, MAPI_CMD_HDMIRX_DATA_RTERM_CONTROL, (void*)&stHDMIRxArgs) != 0)
    {
        MDRV_HDMIRX_MSG_ERROR("Obtain HDMI Rx engine fail\n");
    }
}
...
MS_BOOL MDrv_HDMI_IOCTL(MS_U32 ulCommand, void *pBuffer, MS_U32 ulBufferSize)
{
    ...
    switch(ulCommand)
    {
        ...
        case E_HDMI_INTERFACE_CMD_HDR_INFOFRAME_GET:
            {
                MS_U16 ucHDRInfoFrame[HDMI_HDR_PACKET_SIZE] = {0};

                if(ulBufferSize == sizeof(MS_HDMI_HDR_InfoFrame_Get_Parameters))
                {
                    MS_HDMI_HDR_InfoFrame_Get_Parameters *pParameters = (MS_HDMI_HDR_InfoFrame_Get_Parameters*)pBuffer;

                    if(MDrv_HDMI_GetHDRInfoFrame(ucHDRInfoFrame))
                    {
                        _mdrv_HDMI_ParseHDRInfoFrame(ucHDRInfoFrame, pParameters->pstHDRMetadata);
        ...
        case E_HDMI_INTERFACE_CMD_EXTEND_PACKET_RECEIVE_GET:
            {
                if(ulBufferSize == sizeof(MS_HDMI_Extend_Packet_Receive_Get_Parameters))
                {
                    MS_HDMI_Extend_Packet_Receive_Get_Parameters *pParameters = (MS_HDMI_Extend_Packet_Receive_Get_Parameters*)pBuffer;

                    bret = MDrv_HDMI_ExtendPacketReceived(pParameters->pstPacketReceive);
                }
            }
            break;
        ...
        case E_HDMI_INTERFACE_CMD_DATA_RTERM_CONTROL:
            {
                ...
                {
                    MDrv_HDMI_DataRtermControl(((stCMD_HDMI_DATA_RTERM_CONTROL *)pBuffer)->enInputPortType, ((stCMD_HDMI_DATA_RTERM_CONTROL *)pBuffer)->bDataRtermEnable);
...
MS_BOOL mdrv_HDMI_GetHDRInfoFrame(void* pInstance, MS_U8 ucHDMIInfoSource, MS_U16 *pusHDRInfoFrame)
{
    ...
    {
        if(pHDMIRxResourcePrivate->ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_HDR_PACKET_RECEIVE_FLAG)
        {
            bHDRReceiveFlag = TRUE;

            Hal_HDMI_Get_InfoFrame(ucHDMIInfoSource, PKT_HDR, pusHDRInfoFrame);
        }
    }

    UtopiaResourceRelease(pHDMIRxResource);

    return bHDRReceiveFlag;
}
...
{
    Hal_HDMI_DataRtermControl(enInputPortType, bDataRtermEnable);
}
...
void mdrv_HDCP22_SetEvent(void* pInstance, MS_U16 usHDCPEventIndex)
{
    ...
    else
    {
        ...
        MsOS_SetEvent(pHDMIRxResourcePrivate->slHDMIHDCPEventID, usHDCPEventIndex);
    }
...
void mdrv_HDMI_MDCMDEchoCommand(void* pInstance, MS_U64* u64ReqHdl, char* pcCmdLine)
{
    ...
    char* pString[7] = {NULL};
    MS_U8 uctemp = 0;
    MS_U8 ucEQValue = 0xFF;
    E_MUX_INPUTPORT enInputPortType = INPUT_PORT_DVI_END +1;

    MdbPrint(u64ReqHdl, "LINE:%d, MDBCMD_CMDLINE\n", __LINE__);
    MdbPrint(u64ReqHdl, "pcCmdLine: %s\n", pcCmdLine);

    //psep = strsep(&pcCmdLine, space);
    //MdbPrint(u64ReqHdl, "%s\n", psep);

    for(uctemp = 0; uctemp < 7; uctemp++)
    {
        pString[uctemp] = strsep(&pcCmdLine, space);

        if(pString[uctemp] == NULL)
        {
            break;
        }
        else
    ...
    if(pString[0] != NULL)
    {
        if(strncmp("help", pString[0], 4) == 0)
        {
            MdbPrint(u64ReqHdl, "----------------  Mstar HDMI Rx help -----------------\n");
            MdbPrint(u64ReqHdl, "Support fixed EQ echo function\n");
            MdbPrint(u64ReqHdl, "ex: echo fixedeq port0 eq=8\n");
            MdbPrint(u64ReqHdl, "or  echo fixedeq port3 eq=31\n");
            MdbPrint(u64ReqHdl, "port0~3, eq=0~31\n");
            MdbPrint(u64ReqHdl, "------------------------------------------------------\n");
        }
        else if(strncmp("fixedeq", pString[0], 7) == 0)
        {
            if(pString[1] != NULL) // "port0"
            {
                if(strncmp("port", pString[1], 4) == 0)
                {
                    if(*(pString[1] +4) == 48)
                    {
                        enInputPortType = INPUT_PORT_DVI0;
                    }
                    else if(*(pString[1] +4) == 49)
                    {
                        enInputPortType = INPUT_PORT_DVI1;
                    }
                    else if(*(pString[1] +4) == 50)
                    {
                        enInputPortType = INPUT_PORT_DVI2;
                    }
                    else if(*(pString[1] +4) == 51)
                    {
                        enInputPortType = INPUT_PORT_DVI3;
                    }

                    if(enInputPortType <= INPUT_PORT_DVI_END)
                    {
                        if(pString[2] != NULL) // "eq=1"
                        {
                            if(strncmp("eq=", pString[2], 3) == 0)
                            {
                                pString[3] = strsep(&pString[2], equal);

                                if(pString[3] != NULL)
                                {
                                    pString[3] = strsep(&pString[2], equal);
                                    ucEQValue = atoi(pString[3]);
                                }

                                if(ucEQValue < 32)
                                {
                                    Hal_HDMI_Set_EQ(enInputPortType, MS_HDMI_EQ_USER_DEFINE, ucEQValue);

                                    MdbPrint(u64ReqHdl, "Setup fixed EQ port%d EQ=%d ready!!\n", HDMI_GET_PORT_SELECT(enInputPortType), ucEQValue);
                                }
                                else
                                {
                                    MdbPrint(u64ReqHdl, "EQ value is invalid\n");
                                }
                            }
                        }
                    }
                    ...
                    {
                        MdbPrint(u64ReqHdl, "Port number is invalid\n");
                    }
                }
            }
        }
...
void mdrv_HDMI_MDCMDGetInfo(void* pInstance, MS_U64* u64ReqHdl)
{
    ...
    void* pHDMIRxResource = NULL;
    MS_U8 uctemp = 0;
    MS_U8 ucHDMIInfoSource = HDMI_INFO_SOURCE0;
    MS_U8 ucPacketInfo[32] = {0};
    MS_U16 usHDEValue = 0;
    MS_U16 usVDEValue = 0;
    MS_U16 usDeepColorInfo = HDMI_GC_INFO_CD_NOT_INDICATED;
    ...
    else
    {
        ...
        for(enInputPortType = INPUT_PORT_DVI0; enInputPortType <= INPUT_PORT_DVI_END; enInputPortType++)
        {
            MdbPrint(u64ReqHdl, "----------------  Mstar HDMI port%d Info -----------------\n", HDMI_GET_PORT_SELECT(enInputPortType));

            if(pHDMIRxResourcePrivate->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bNoInputFlag)
            {
                MdbPrint(u64ReqHdl, "Input clock unstable\n");
            }
            else
            {
                MdbPrint(u64ReqHdl, "Input clock stable\n");

                if(pHDMIRxResourcePrivate->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag)
                {
                    MdbPrint(u64ReqHdl, "HDMI 2.0 timing\n");
                }
                else
                {
                    MdbPrint(u64ReqHdl, "HDMI 1.4 timing\n");
                }

                MdbPrint(u64ReqHdl, "TMDS clock rate:%dM\n", pHDMIRxResourcePrivate->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usClockCount);

                if(pHDMIRxResourcePrivate->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMIModeFlag)
                {
                    MdbPrint(u64ReqHdl, "HDMI mode\n");
                }
                else
                {
                    MdbPrint(u64ReqHdl, "DVI mode\n");
                }
            }
        }
        ...
        for(ucHDMIInfoSource = HDMI_INFO_SOURCE0; ucHDMIInfoSource < HDMI_INFO_SOURCE_MAX; ucHDMIInfoSource++)
        {
            MdbPrint(u64ReqHdl, "----------------  Mstar HDMI IP%d Info -----------------\n", ucHDMIInfoSource);

            usHDEValue = Hal_HDMI_GetDataInfo(E_HDMI_GET_HDE, ucHDMIInfoSource);
            usVDEValue = Hal_HDMI_GetDataInfo(E_HDMI_GET_VDE, ucHDMIInfoSource);

            if((usHDEValue > 0) && (usVDEValue > 0))
            {
                Hal_HDMI_err_status_update(ucHDMIInfoSource, 0xFF, FALSE);	//BCH error clear

                MdbPrint(u64ReqHdl, "HDE:%d\n", usHDEValue);
                MdbPrint(u64ReqHdl, "VDE:%d\n", usVDEValue);
                MdbPrint(u64ReqHdl, "HTT:%d\n", Hal_HDMI_GetDataInfo(E_HDMI_GET_HTT, ucHDMIInfoSource));
                MdbPrint(u64ReqHdl, "VTT:%d\n", Hal_HDMI_GetDataInfo(E_HDMI_GET_VTT, ucHDMIInfoSource));

                switch(Hal_HDMI_avi_infoframe_info(ucHDMIInfoSource, _BYTE_1)& 0x60)
                {
                    case 0x00:
                        MdbPrint(u64ReqHdl, "Color space RGB444\n");
                        break;

                    case 0x40:
                        MdbPrint(u64ReqHdl, "Color space YUV444\n");
                        break;

                    case 0x20:
                        MdbPrint(u64ReqHdl, "Color space YUV422\n");
                        break;

                    case 0x60:
                        MdbPrint(u64ReqHdl, "Color space YUV420\n");
                        break;
                ...
                usDeepColorInfo = Hal_HDMI_gcontrol_info(G_Ctrl_CD_VAL, ucHDMIInfoSource) >> 8;

                switch(usDeepColorInfo)
                {
                    case HDMI_GC_INFO_CD_24BITS:
                        MdbPrint(u64ReqHdl, "Color depth 8bit\n");
                        break;

                    case HDMI_GC_INFO_CD_30BITS:
                        MdbPrint(u64ReqHdl, "Color depth 10bit\n");
                        break;

                    case HDMI_GC_INFO_CD_36BITS:
                        MdbPrint(u64ReqHdl, "Color depth 12bit\n");
                        break;

                    case HDMI_GC_INFO_CD_48BITS:
                        MdbPrint(u64ReqHdl, "Color depth 16bit\n");
                        break;

                    default:
                        MdbPrint(u64ReqHdl, "Color depth not indicated\n");
                        break;
                };

                MdbPrint(u64ReqHdl, "BCH error:%x\n", Hal_HDMI_err_status_update(ucHDMIInfoSource, 0x0, TRUE));	//BCH error read, 0: no error; else: error happen
                MdbPrint(u64ReqHdl, "AVMute:%d\n", Hal_HDMI_gcontrol_info(G_CTRL_AVMUTE, ucHDMIInfoSource));
                MdbPrint(u64ReqHdl, "Packet receive status:%x\n", pHDMIRxResourcePrivate->ulPacketStatus[ucHDMIInfoSource]);

                if(pHDMIRxResourcePrivate->ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_AVI_PACKET_RECEIVE_FLAG)
                {
                    if(Hal_HDMI_get_packet_value(ucHDMIInfoSource, PKT_AVI, 13, ucPacketInfo))
                    {
                        MdbPrint(u64ReqHdl, "Receive AVI packet\n");

                        for(uctemp = 0; uctemp < 13; uctemp++)
                        {
                            MdbPrint(u64ReqHdl, "0x%x ", ucPacketInfo[uctemp]);
                        }
                ...
                if(pHDMIRxResourcePrivate->ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_VS_PACKET_RECEIVE_FLAG)
                {
                    if(Hal_HDMI_get_packet_value(ucHDMIInfoSource, PKT_VS, 28, ucPacketInfo))
                    {
                        MdbPrint(u64ReqHdl, "Receive VS packet\n");

                        for(uctemp = 0; uctemp < 28; uctemp++)
                        {
                            MdbPrint(u64ReqHdl, "0x%x ", ucPacketInfo[uctemp]);
                        }
                ...
                if(pHDMIRxResourcePrivate->ulPacketStatus[ucHDMIInfoSource] &HDMI_STATUS_HDR_PACKET_RECEIVE_FLAG)
                {
                    if(Hal_HDMI_get_packet_value(ucHDMIInfoSource, PKT_HDR, 28, ucPacketInfo))
                    {
                        MdbPrint(u64ReqHdl, "Receive HDR packet\n");
            ...
            {
                MdbPrint(u64ReqHdl, "Signal invalid\n");
            }
        }
    }
