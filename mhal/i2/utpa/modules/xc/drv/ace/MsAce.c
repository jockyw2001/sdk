...
#define IS_SC1()        (psACEInstPri->u32DeviceID == 1)
...
void msACESetColorCorrectionTable(void* pInstance, BOOL bWindow, short* psColorCorrectionTable )
{
    ...
    if(psColorCorrectionTable!=NULL)
    {
        ...
        ST_KDRV_XC_CFD_OSD stAceOSDControl;
        memset(&stAceOSDControl, 0, sizeof(ST_KDRV_XC_CFD_OSD));
        stAceOSDControl.u32Version = KDRV_XC_OSD_CONTROL_VERSION;
        stAceOSDControl.u16Length = sizeof(ST_KDRV_XC_CFD_OSD);
        ...
        stCFDCtrlInfo.u32ParamLen = sizeof(ST_KDRV_XC_CFD_OSD);
#ifdef UFO_XC_HDR
#if (UFO_XC_HDR_VERSION == 2)
...
static void SetVideoContrastMatrix(void *pInstance, BOOL bWindow )
{
    ...
    if( IS_SC1() )
    {
        s_AceInfo[bWindow].sContrastRGBMatrix[0][0] = ( (DWORD)s_AceInfo[bWindow].wRCon * (1024UL)) >> 10;
        s_AceInfo[bWindow].sContrastRGBMatrix[1][1] = ( (DWORD)s_AceInfo[bWindow].wGCon * (s_AceInfo[bWindow].wContrast)) >> 10;
        s_AceInfo[bWindow].sContrastRGBMatrix[2][2] = ( (DWORD)s_AceInfo[bWindow].wBCon * (1024UL)) >> 10;
    }
    else
    {
...
static void msWriteColorMatrix(void *pInstance, BOOL bWindow, short* psMatrix )
{
    ...
    if( IS_SC1() )
    {
        ...
        ucAddr = REG_ADDR_SC1_CM_MAIN&0xFF;
    }
    else
    {
...
#if 0
void PrintMatrix(MS_U32 u32Id, MS_BOOL bWin, short *YUVtoRGB, short *SatHue, short *ColorCorrect, short *Contrast, short *Result)
{
    printk(KERN_EMERG "-------------------------------  ID=%d bWin=%d\n", u32Id, bWin);
    printk(KERN_EMERG "YUVToRGB::\n");
    printk(KERN_EMERG " %04x %04x %04x %04x %04x %04x %04x %04x %04x\n\n",
       YUVtoRGB[0], YUVtoRGB[1], YUVtoRGB[2],
       YUVtoRGB[3], YUVtoRGB[4], YUVtoRGB[5],
       YUVtoRGB[6], YUVtoRGB[7], YUVtoRGB[8]);


    printk(KERN_EMERG "SatHue::\n");
    printk(KERN_EMERG " %04x %04x %04x %04x %04x %04x %04x %04x %04x\n\n",
        SatHue[0], SatHue[1], SatHue[2],
        SatHue[3], SatHue[4], SatHue[5],
        SatHue[6], SatHue[7], SatHue[8]);

    printk(KERN_EMERG "ColorCrt::\n");
    printk(KERN_EMERG " %04x %04x %04x %04x %04x %04x %04x %04x %04x\n\n",
        ColorCorrect[0], ColorCorrect[1], ColorCorrect[2],
        ColorCorrect[3], ColorCorrect[4], ColorCorrect[5],
        ColorCorrect[6], ColorCorrect[7], ColorCorrect[8]);

    printk(KERN_EMERG "Contrast::\n");
    printk(KERN_EMERG " %04x %04x %04x %04x %04x %04x %04x %04x %04x\n\n",
        Contrast[0], Contrast[1], Contrast[2],
        Contrast[3], Contrast[4], Contrast[5],
        Contrast[6], Contrast[7], Contrast[8]);

    printk(KERN_EMERG "Result::\n");
    printk(KERN_EMERG " %04x %04x %04x %04x %04x %04x %04x %04x %04x\n\n",
...
void msSetVideoColorMatrix(void *pInstance, BOOL bScalerWin )
{
    ...
    //PrintMatrix(psACEInstPri->u32DeviceID, bScalerWin, (short *)s_AceInfo[bScalerWin].psYVUtoRGBMatrix, (short *)s_AceInfo[bScalerWin].sVideoSatHueMatrix,
    //  (short *)s_AceInfo[bScalerWin].psColorCorrectionMatrix, (short *)s_AceInfo[bScalerWin].sContrastRGBMatrix, (short *)psLastResult);
    ...
    {
        SET_SC_BANK(BANK_CM_SC1_MAIN);
        ACE_WRITE_CM_CTL(psACEInstPri->u32DeviceID,  REG_ADDR_SC1_CM_CTL_MAIN, ENABLE_SC1_CM_CTL1, MASK_SC1_CM_CTL); // Enable color matrix&Change R/B range
    }
    else
    {
...
MS_BOOL msAdjustCFDOSDInfo(void *pInstance, BOOL bScalerWin)
{
    ...
    stAceOSDControl.bRValid = TRUE;
    stAceOSDControl.bGValid = TRUE;
    stAceOSDControl.bBValid = TRUE;
...
void msAdjustVideoContrast(void *pInstance, BOOL bScalerWin, BYTE ucContrast )
{
    ...
    msSetVideoColorMatrix(pInstance,  bScalerWin);
#else
    msAdjustCFDOSDInfo(pInstance, bScalerWin);
#endif
}
void msAdjustVideoContrast_10Bits(void *pInstance, BOOL bScalerWin, WORD wContrast )
{
...
void msSetPCColorMatrix(void *pInstance, BOOL bScalerWin )
{
    ...
    if( (s_AceInfo[bScalerWin].bForceYUVtoRGB) || (bScalerWin == SUB_WINDOW) )
    {
        ...
        //ucCMCtlVal = 0x35;
    #ifdef CHIP_I2
        CTLVal = IS_SC1() ? ENABLE_SC1_CM_CTL1 : ENABLE_CM_CTL1;
    #else
    ...
    {
    #ifdef CHIP_I2
        CTLVal = IS_SC1() ? ENABLE_SC1_CM_CTL2 : ENABLE_CM_CTL2;
    #else
    ...
    {
        SET_SC_BANK(BANK_CM_SC1_MAIN);
        ACE_WRITE_CM_CTL(psACEInstPri->u32DeviceID, REG_ADDR_SC1_CM_CTL_MAIN, CTLVal, MASK_SC1_CM_CTL );
    }
    else
    {
        if( bScalerWin == MAIN_WINDOW )
...
void msACE_SetColorMatrixControl(void *pInstance, BOOL bScalerWin, BOOL bEnable )
{
...
#ifdef CHIP_I2
    if( bEnable )
        CTLVal = IS_SC1() ?  MASK_SC1_CM_CTL_EN : MASK_CM_CTL_EN;
    else
        CTLVal = 0;
    ...
    {
        SET_SC_BANK(BANK_CM_SC1_MAIN);
        ACE_WRITE_CM_CTL(psACEInstPri->u32DeviceID,  REG_ADDR_SC1_CM_CTL_MAIN, CTLVal, MASK_SC1_CM_CTL_EN );
    }
    else
    {
        if( bScalerWin == MAIN_WINDOW )
...
void msACE_SetRBChannelRange(void *pInstance, BOOL bScalerWin, BOOL bRange )
{
    ...
    if( bRange )
        CTLVal = IS_SC1() ?  MASK_SC1_CM_CTL_RB_RANGE : MASK_CM_CTL_RB_RANGE;
    else
        CTLVal = 0;
    ...
    {
        SET_SC_BANK(BANK_CM_SC1_MAIN);
        ACE_WRITE_CM_CTL(psACEInstPri->u32DeviceID,  REG_ADDR_SC1_CM_CTL_MAIN, CTLVal, MASK_SC1_CM_CTL_RB_RANGE );
    }
    else
    {
        if( bScalerWin == MAIN_WINDOW )
