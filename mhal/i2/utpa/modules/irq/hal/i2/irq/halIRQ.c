...
#define IRQ_MUTEX_LOCK(flags)  spin_lock_irqsave(&_HAL_IRQ_Mutex, flags)

#define IRQ_MUTEX_UNLOCK(flags)      spin_unlock_irqrestore(&_HAL_IRQ_Mutex, flags)
...
{
    unsigned long flags;
    IRQ_MUTEX_LOCK(flags);
    if (g_bISRInit)
    {
        IRQ_MUTEX_UNLOCK(flags);
        return;
    }
    g_bISRInit = TRUE;
    HAL_InitIrqTable();
...
void HAL_IRQ_Detech_Shared(MS_U32 u32Vector)
{
    MS_U16 u16VectorIndex = 0;
    int irq;
    unsigned long flags;
