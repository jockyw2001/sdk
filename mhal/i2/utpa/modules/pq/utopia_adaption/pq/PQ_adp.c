...
UADP_SPT_NAMEnNXT_DEF(PQ_GetVideoDelayTime,0);
...
UADP_SPT_NAMEnNXT_DEF(PQ_LoadTMOSettings,0);
UADP_SPT_NAMEnNXT_DEF(ADP_PQ_SET_MEMORY_INFO,0);
UADP_SPT_NAMEnNXT_DEF(PQ_SetMemoryAddress,1);
UADP_SPT_NAMEnNXT_DEF(PQ_LoadTMOModeGRuleTable,0);
...
#ifdef STELLAR
//drvPQ_ex.c
UADP_SPT_NAMEnNXT_DEF(PQ_EXCtrl,0);
#endif
...
//drvPQ.c
    ...
    UADP_SPT_NAME0NXT(PQ_GetVideoDelayTime,stPQ_GetVideoDelayTime);
    ...
    UADP_SPT_NAME0NXT(PQ_LoadTMOSettings,stPQ_LoadTMOSettings);
    UADP_SPT_NAME0NXT(ADP_PQ_SET_MEMORY_INFO,MS_PQ_SET_MEMORY_INFO);
    UADP_SPT_NAME1NXT(PQ_SetMemoryAddress,stPQ_SetMemoryAddress,pstPQ_Set_Memory_Address,ADP_PQ_SET_MEMORY_INFO);
    UADP_SPT_NAME0NXT(PQ_LoadTMOModeGRuleTable,stPQ_LoadTMOModeGRuleTable);
#if (PQ_XRULE_DB_ENABLE == 1)
...
#ifdef STELLAR
    UADP_SPT_NAME0NXT(PQ_EXCtrl,stPQ_CMD_EX_CTRL);
#endif
...
MS_U32 PQ_adp_Ioctl(void* pInstanceTmp, MS_U32 u32Cmd, void* const pArgs)
{
    ...
    switch(u32Cmd)
    {
        ...
        case E_PQ_CMD_GET_VIDEO_DELAY_TIME:
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp,u32Cmd,pArgs,spt_PQ_GetVideoDelayTime,spt_PQ_GetVideoDelayTime,buffer_arg,sizeof(buffer_arg));
            break;
        }
        ...
        case E_PQ_CMD_SET_PQBIN_PATH:
        {
            ...
            if(copy_from_user(&tmp1,pArgs,sizeof(stPQ_SetPQBinPath)))
            {
            }
            tmp.b_PQBinFilePath = malloc(tmp1.u8size);
            if(copy_from_user(tmp.b_PQBinFilePath,tmp1.b_PQBinFilePath,tmp1.u8size))
            {
            }
        ...
        case E_PQ_CMD_LOAD_TMO_SETTINGS:
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp,u32Cmd,pArgs,spt_PQ_LoadTMOSettings,NULL,buffer_arg,sizeof(buffer_arg));
            break;
        }
        case E_PQ_CMD_SET_MEMORY_ADDRESS:
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp,u32Cmd,pArgs,spt_PQ_SetMemoryAddress,spt_PQ_SetMemoryAddress,buffer_arg,sizeof(buffer_arg));
            break;
        }
        case E_PQ_CMD_LOAD_TMO_MODE_GRULE_TABLE:
        {
            u32Ret=UADPBypassIoctl(pInstanceTmp,u32Cmd,pArgs,spt_PQ_LoadTMOModeGRuleTable,spt_PQ_LoadTMOModeGRuleTable,buffer_arg,sizeof(buffer_arg));
            break;
        }
//drvPQ_cus.c
...
#ifdef STELLAR
//drvPQ_ex.c
        case E_PQ_CMD_EX_CTRL:
        {
            stPQ_CMD_EX_CTRL tmp;
            stPQ_CMD_EX_CTRL tmp1;
            copy_from_user(&tmp1,pArgs,sizeof(stPQ_CMD_EX_CTRL));
            switch(tmp1.u32Cmd)
            {
                case E_PQ_CMD_EX_SET_ICC_REGION:
                case E_PQ_CMD_EX_SET_IHC_REGION:
                    tmp.pbuf = malloc(tmp1.u32BufSize);
                    copy_from_user(tmp.pbuf, tmp1.pbuf, tmp1.u32BufSize);
                    tmp.u32Cmd = tmp1.u32Cmd;
                    tmp.u32BufSize = tmp1.u32BufSize;
                    u32Ret = UtopiaIoctl(pInstanceTmp,u32Cmd,&tmp);
                    free(tmp.pbuf);
                    break;
                ...
                {
                    void *ptmp_buf;
                    EN_IP_Info_Ex table_ex;
                    EN_IP_Info table;
                    if (tmp1.u32BufSize == 0)
                    {
                        break;
                    }

                    ptmp_buf = malloc(tmp1.u32BufSize); // u32BufSize = table_ex.u32TableSize + sizeof(U8)*3 + sizeof(U32);
                    ptmp_buf= (void*)&table_ex;
                    copy_from_user(ptmp_buf, tmp1.pbuf, tmp1.u32BufSize);

                    if (table_ex.u32TableSize == 0)
                    {
                        free(ptmp_buf );
                        break;
                    }
                    table.pIPTable = malloc(table_ex.u32TableSize);
                    copy_from_user(table.pIPTable,table_ex.pIPTable, table_ex.u32TableSize);

                    table.u8TabIdx = table_ex.u8TabIdx;
                    table.u8TabNums = table_ex.u8TabNums;
                    table.u8TabType = table_ex.u8TabType;
            ...
            break;
            //u32Ret=UADPBypassIoctl(pInstanceTmp,u32Cmd,pArgs,spt_PQ_EXCtrl, spt_PQ_EXCtrl,buffer_arg,sizeof(buffer_arg));
            //break;
        }
#endif
        default:
