...
#ifndef NOT_SUPPORT_PEAKING_CAG_SRAM
static MS_U16 _u16VIPSRAM_Peaking_CAG_Table = 0xFFFF;
#endif
...
static /*MS_PHYADDR*/size_t _u32VRAddress=0;
static /*MS_PHYADDR*/size_t _u32VRSize=0;
static MS_BOOL _bVRInit=FALSE;
#endif

extern MS_U8 *gpu8TmoSettingData;
extern MS_U16 gu16RegNum;
extern MS_BOOL gbTmoSettingMalloc;
...
void MDrv_PQBin_DumpGeneralRegTable(void *pInstance,const MS_PQBin_IP_Table_Info* const pTabInfo)
{
    ...
    MS_U8  u8Value;
    MS_U16 i;
    /*MS_PHYADDR*/size_t u32Addr, u32RegAddr;
    MS_U32 u32Offset;
...
#if defined(PQ_QM_VR) && defined(PQ_TMO_SETTING_CFD)
void MDrv_PQBin_DumpDumyRegister(void *pInstance, const MS_PQBin_IP_Table_Info* const pTabInfo)
{
    if(NULL == pTabInfo)
    {
        return;
    ...
    MS_U32 u32Offset,i;
    /*MS_PHYADDR*/size_t u32Addr;

    if(0 == gbTmoSettingMalloc)
    {
#if !defined(MSOS_TYPE_LINUX_KERNEL)
        gpu8TmoSettingData = malloc(PQ_IP_NUM_TMO * pTabInfo->u16RegNum);
#else
        gpu8TmoSettingData = vmalloc(PQ_IP_NUM_TMO * pTabInfo->u16RegNum);
#endif
        ...
        gu16RegNum = pTabInfo->u16RegNum;
        pu8Data = gpu8TmoSettingData;
        gbTmoSettingMalloc = 1;
    }
    ...
    {
        u32Offset = ( PQ_BIN_BANK_SIZE + PQ_BIN_ADDR_SIZE + PQ_BIN_MASK_SIZE + (MS_U32)pTabInfo->u16GroupNum) * (MS_U32)i;
        u32Offset += 3;                             // skip bank & address & mask in PQ bin
...
MS_BOOL MDrv_PQBin_SetVRAddress(void *pInstance, /*MS_PHYADDR*/size_t u32VRAddress, /*MS_PHYADDR*/size_t u32VRSize)
{
    if(u32VRSize>0)
    {
        _bVRInit = TRUE;
    ...
    _u32VRAddress = u32VRAddress;
    _u32VRSize = u32VRSize;
    return TRUE;
}

void MDrv_PQBin_DumpVirtualRegTable(void *pInstance, const MS_PQBin_IP_Table_Info* const pTabInfo)
{
    //MS_U32 u32RegAddr;
    //MS_U8 u8Mask;
    MS_U8 u8Addr;
    ...
    for(i=0; i<pTabInfo->u16RegNum; i++)
    {
        ...
        PQ_BIN_DBG_SRAMERROR(printf("[SCALER][read addr=%x], GroupIdx=%x,RegNum=%x,reg=%04x, msk=%02x, val=%02x, u8CurBank=%x\n",
        ...
        DstAddr = _u32VRAddress + (u8CurBank * VR_SIZE_PER_BANK + (MS_U16)u8Addr);
        pu8Addr = (MS_U8 *)MS_PA2KSEG1(DstAddr);

        *pu8Addr =(*pu8Addr & (~u8Mask)) | (u8Value & u8Mask);
    }
...
#endif
#ifndef NOT_SUPPORT_PEAKING_CAG_SRAM
    case PQ_TABTYPE_VIP_Peaking_CAG_SRAM:
        Hal_PQ_set_sram_peaking_cag_table(pInstance,SC_FILTER_SRAM_PEAKING_CAG,(void *)u32Addr);
        break;
#endif
    default:
...
void MDrv_PQBin_DumpTable(void *pInstance,MS_PQBin_IP_Table_Info* pTabInfo)
{
    ...
    case PQ_TABTYPE_SCALER:
        ...
        break;
#if defined(PQ_QM_VR) && defined(PQ_TMO_SETTING_CFD)
    case PQ_TABTYPE_DumF_SERIES:
#if (ENABLE_PQ_BIN_TMO)
        MDrv_PQBin_DumpVirtualRegTable(pInstance,pTabInfo);
#else
        MDrv_PQBin_DumpDumyRegister(pInstance, pTabInfo);
#endif
        break;
#endif
...
#ifndef NOT_SUPPORT_PEAKING_CAG_SRAM
    case PQ_TABTYPE_VIP_Peaking_CAG_SRAM:
        if(_u16VIPSRAM_Peaking_CAG_Table != pTabInfo->u16GroupIdx)
        {
            PQ_BIN_DUMP_FILTER_DBG(printf("[PQBin][UFSC] old sram1: %u, new sram1: %u\n",
            _u16VIPSRAM_Peaking_CAG_Table, pTabInfo->u16GroupIdx));
            _u16VIPSRAM_Peaking_CAG_Table = pTabInfo->u16GroupIdx;
            MDrv_PQBin_DumpFilterTable(pInstance,pTabInfo);
        }
        break;
...
MS_U16 MDrv_PQBin_GetTableIndex(void *pInstance,MS_U16 u16PQSrcType, MS_U16 u16PQIPIdx, MS_U16 u16PQPnlIdx, const MS_PQBin_Header_Info* const pPQBinHeader)
{
    /*MS_PHYADDR*/size_t u32Addr;
    MS_U32 u32Offset;
    /*MS_PHYADDR*/size_t u32SourceLUT_Addr;
...
{
    /*MS_PHYADDR*/size_t u32PQBin_Addr;
    MS_U32 u32Offset;
...
MS_U16 MDrv_PQBin_GetXRuleTableIndex(void *pInstance,MS_U16 u16XRuleType, MS_U16 u16XRuleIdx, MS_U16 u16XRuleIP, const MS_PQBin_Header_Info* const pPQBinHeader)
{
    /*MS_PHYADDR*/size_t u32Addr;
    MS_U32 u32Offset;
    /*MS_PHYADDR*/size_t u32Rule_Addr;
...
{
    /*MS_PHYADDR*/size_t u32Addr, u32Lvl_Addr;
    MS_U32 u32Offset;
    MS_U16 u16LvlNum;
    MS_U16 u16LvlIdx;
...
MS_U16 MDrv_PQBin_GetGRule_IPIndex(
    ...
    const MS_PQBin_Header_Info* const pPQBinHeader)
{
    /*MS_PHYADDR*/size_t u32Addr, u32IP_Addr;
...
MS_U16 MDrv_PQBin_GetGRule_TableIndex(
    ...
    const MS_PQBin_Header_Info* const pPQBinHeader)
{
    /*MS_PHYADDR*/size_t u32Addr, u32Rule_Addr;
