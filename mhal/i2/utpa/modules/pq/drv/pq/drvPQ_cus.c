...
#ifndef PQ_SUPPORT_DVI_IMODE
#define PQ_SUPPORT_DVI_IMODE 0
#endif
...
MS_U16 QM_InputSourceToIndex(void* pInstance, PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    ...
    stXCGetFBLevel.stCropWin.height = u16Input_VSize;
    stXCGetFBLevel.stCropWin.width  = u16Input_HSize;
    stXCGetFBLevel.stDispWin.height = u16Disp_VSize;
    stXCGetFBLevel.stDispWin.width  = u16Disp_HSize;
    stXCGetFBLevel.bInterlace = bIsInterlaced;
    stXCGetFBLevel.eFBLevel = E_XC_FB_LEVEL_FBL;
    if(eWindow == PQ_MAIN_WINDOW)
    {
        stXCGetFBLevel.eWindow = MAIN_WINDOW;
    }
    else
    {
        stXCGetFBLevel.eWindow = SUB_WINDOW;
    }
#if (PQ_QM_CVBS)
...
#ifdef STELLAR
        ||( QM_IsSourceHDMI_PC(enInputSourceType, eWindow))  //for L project
#endif
        )
#else
...
if( QM_IsSourceHDMI(enInputSourceType) && QM_HDMIPC_COLORRGB(eWindow) &&
        ...
        else
        {
            ...
            if ((u16Input_HSize < 800) && (u16Input_VSize < 500))
            {
                if (bIsInterlaced)
                {
#if (PQ_SUPPORT_DVI_IMODE == 1)
                    if(QM_HDMIPC_COLORRGB(eWindow))
                        u16SrcType = QM_DVI_480i_Main;
                    else
#endif
            ...
            else
            {
                if (bIsInterlaced)
                {
                    ...
                    if(QM_HDMIPC_COLORRGB(eWindow))
                    {
                        if (u16Input_VFreq > 550)
                        {
                            u16SrcType = QM_DVI_1080i_60hz_Main;
                        }
                        else
                        {
                            u16SrcType = QM_DVI_1080i_50hz_Main;
                        }
                    }
                    else
#endif
...
#endif
        //When we get chipcaps is FbLevel,but the input SrcType is DTV4k2k(FBL),we should translate the srcType
        //so that do something about FbLevel like scaling
        if (MApi_XC_GetChipCaps(E_XC_FB_CAPS_GET_FB_LEVEL, (MS_U32*)&stXCGetFBLevel, sizeof(XC_GET_FB_LEVEL)) == E_APIXC_RET_OK)
        {
            if(stXCGetFBLevel.eFBLevel == E_XC_FB_LEVEL_FB)
            {
                switch(u16SrcType)
                {
#if PQ_QM_DTV_4K2K
                    case QM_DTV_4K2K_MPEG2_Main:
                    case QM_DTV_4K2K_60Hz_MPEG2_Main:
                        if (u16Input_VFreq > 550)
                        {
                            u16SrcType = QM_DTV_1080p_60hz_MPEG2_Main;
                        }
                        ...
                        break;
                    case QM_DTV_4K2K_H264_Main:
                    case QM_DTV_4K2K_60Hz_H264_Main:
                        if (u16Input_VFreq > 550)
                        {
                            u16SrcType = QM_DTV_1080p_60hz_H264_Main;
                        }
...
#endif
        //When we get chipcaps is FbLevel,but the input SrcType is MM(FBL),we should translate the srcType
        //so that do something about FbLevel like scaling
        if (MApi_XC_GetChipCaps(E_XC_FB_CAPS_GET_FB_LEVEL, (MS_U32*)&stXCGetFBLevel, sizeof(XC_GET_FB_LEVEL)) == E_APIXC_RET_OK)
        {
            if(stXCGetFBLevel.eFBLevel == E_XC_FB_LEVEL_FB)
            {
                ...
                {
#if (PQ_DS_ENABLE)
                    case QM_MM_FHD_P_DS_Main:
                        u16SrcType = QM_Multimedia_video_HD_progressive_Main;
                        break;
#endif
#if (PQ_QM_4K2K)
#if (PQ_DS_ENABLE)
                    case QM_MM_4K2K_DS_Main:
                    case QM_NetWork_4K2K_DS_Main:
                        u16SrcType = QM_Multimedia_video_HD_progressive_Main;
                        break;
#endif
                    case QM_MM_4K2K_Main:
                    case QM_MM_4K2K_60Hz_Main:
                    case QM_MM_non_4K2K_Main:
                        u16SrcType = QM_Multimedia_video_HD_progressive_Main;
                        break;
#endif
                    default:
                        printf("MM curret is FB");
                        break;
                }
            }
        }
    ...
    if (Hal_PQ_get_cfd_info(u16SrcType, &stCfdAnalog.u8ColorFormat, &stCfdAnalog.u8ColorDataFormat, &stCfdAnalog.bIsFullRange, &stCfdAnalog.u8ColorPrimaries, &stCfdAnalog.u8TransferCharacteristics, &stCfdAnalog.u8MatrixCoefficients) == TRUE)
    {
        ...
        {
            MDrv_PQ_SendCfd(E_PQ_CFD_CTRL_SET_MM, &stCfdAnalog, sizeof(ST_PQ_CFD_ANALOG));
        }
    }
#endif
...
MS_U16 QM_InputSourceToIndex_TMO(void* pInstance, PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;
#if (CFD_STATUS_VERSION >=2)
    XC_CFD_STATUS stCfdStatus;
    ...
    u16SrcType=stCfdStatus.u8TMOColorType;
#endif
...
#define QMAP_TABLE_PANEL_NUMBER 3 // 1920, 1366 , 4K2K
#if PQ_ENABLE_PIP
...
QMAP_TBL_INIT_INFO QmapTblInitInfo_Main[QMAP_TABLE_PANEL_NUMBER][QMAP_TABLE_DISPLAY_NUMBER] = {
    ...
    { // 1366
        { // ONE
            (void*)QMAP_1366_Main,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_Main_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                (void*)MST_GRule_1366_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                (void*)MST_GRule_1366_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                (void*)MST_GRule_1366_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                (void*)MST_GRule_1366_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                (void*)MST_GRule_1366_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                (void*)MST_GRule_1366_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                (void*)MST_GRule_1366_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                (void*)MST_GRule_1366_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                (void*)MST_GRule_1366_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                (void*)MST_GRule_1366_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                (void*)MST_GRule_1366_HDR_Main,
#endif
            }
        },
#if PQ_ENABLE_PIP
        { // PIP
            (void*)QMAP_1366_PIP_Main,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_PIP_Main_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_PIP_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                (void*)MST_GRule_1366_PIP_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                (void*)MST_GRule_1366_PIP_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                (void*)MST_GRule_1366_PIP_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                (void*)MST_GRule_1366_PIP_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                (void*)MST_GRule_1366_PIP_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                (void*)MST_GRule_1366_PIP_HDR_Main,
#endif
            }
        },
        { // POP
            (void*)QMAP_1366_POP_Main,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_POP_Main_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_POP_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                (void*)MST_GRule_1366_POP_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                (void*)MST_GRule_1366_POP_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                (void*)MST_GRule_1366_POP_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                (void*)MST_GRule_1366_POP_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                (void*)MST_GRule_1366_POP_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                (void*)MST_GRule_1366_POP_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                (void*)MST_GRule_1366_POP_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                (void*)MST_GRule_1366_POP_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                (void*)MST_GRule_1366_POP_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                (void*)MST_GRule_1366_POP_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                (void*)MST_GRule_1366_POP_HDR_Main,
#endif
            }
        },
#endif
    },
#if PQ_SUPPORT_HDMI_4K2K_P2P
    { // 4k2k
            { // ONE
                (void*)QMAP_4K2K_Main,
#if (ENABLE_PQ_EX)
                (void*)QMAP_4K2K_Main_Ex,
#endif
                {
#if (PQ_GRULE_NR_ENABLE)
                    (void*)MST_GRule_4K2K_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                    (void*)MST_GRule_4K2K_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                    (void*)MST_GRule_4K2K_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                    (void*)MST_GRule_4K2K_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                    (void*)MST_GRule_4K2K_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                    (void*)MST_GRule_4K2K_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                    (void*)MST_GRule_4K2K_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                    (void*)MST_GRule_4K2K_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                    (void*)MST_GRule_4K2K_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                    (void*)MST_GRule_4K2K_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                    (void*)MST_GRule_4K2K_HDR_Main,
#endif
                }
            },
#if PQ_ENABLE_PIP
            { // PIP
                (void*)QMAP_4K2K_PIP_Main,
#if (ENABLE_PQ_EX)
                (void*)QMAP_4K2K_PIP_Main_Ex,
#endif
                {
#if (PQ_GRULE_NR_ENABLE)
                    (void*)MST_GRule_4K2K_PIP_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                    (void*)MST_GRule_4K2K_PIP_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                    (void*)MST_GRule_4K2K_PIP_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                    (void*)MST_GRule_4K2K_PIP_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                    (void*)MST_GRule_4K2K_PIP_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                    (void*)MST_GRule_4K2K_PIP_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                    (void*)MST_GRule_4K2K_PIP_HDR_Main,
#endif
                }
            },
            { // POP
                (void*)QMAP_4K2K_POP_Main,
#if (ENABLE_PQ_EX)
                (void*)QMAP_4K2K_POP_Main_Ex,
#endif
                {
#if (PQ_GRULE_NR_ENABLE)
                    (void*)MST_GRule_4K2K_POP_NR_Main,
#endif
#if (PQ_GRULE_OSD_BW_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_OSD_BW_Main,
#endif
#if (PQ_GRULE_MPEG_NR_ENABLE)
                    (void*)MST_GRule_4K2K_POP_MPEG_NR_Main,
#endif
#if (PQ_GRULE_FILM_MODE_ENABLE)
                    (void*)MST_GRule_4K2K_POP_FILM_MODE_Main,
#endif
#if (PQ_GRULE_DYNAMIC_CONTRAST_ENABLE)
                    (void*)MST_GRule_4K2K_POP_DYNAMIC_CONTRAST_Main,
#endif
#if (PQ_GRULE_DDR_SELECT_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_DDR_SELECT_Main,
#endif
#if (PQ_GRULE_ULTRAT_CLEAR_ENABLE)
                    (void*)MST_GRule_4K2K_POP_ULTRAT_CLEAR_Main,
#endif
#if (PQ_GRULE_PTP_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_PTP_Main,
#endif
#if (PQ_GRULE_XVYCC_ENABLE == 1 )
                    (void*)MST_GRule_4K2K_POP_XVYCC_Main,
#endif
#if (PQ_GRULE_GAME_MODE_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_GAME_MODE_Main,
#endif
#if (PQ_GRULE_DS_PARAMETERS_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_DS_PARAMETERS_Main,
#endif
#if (PQ_GRULE_HDR_ENABLE == 1)
                    (void*)MST_GRule_4K2K_POP_HDR_Main,
#endif
                }
            },
#endif
...
QMAP_TBL_INIT_INFO QmapTblInitInfo_Sub[QMAP_TABLE_PANEL_NUMBER][QMAP_TABLE_DISPLAY_NUMBER] = {
    ...
    { // 1366
        { // ONE
            (void*)QMAP_1366_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_NR_Sub,
#endif
            }
        },
        { // PIP
            (void*)QMAP_1366_PIP_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_PIP_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_PIP_NR_Sub,
#endif
            }
        },
        { // POP
            (void*)QMAP_1366_POP_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_1366_POP_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_1366_POP_NR_Sub,
#endif
            }
        },
    },
    ...
    { // 4K2K
        { // ONE
            (void*)QMAP_4K2K_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_4K2K_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_4K2K_NR_Sub,
#endif
            }
        },
        { // PIP
            (void*)QMAP_4K2K_PIP_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_4K2K_PIP_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_4K2K_PIP_NR_Sub,
#endif
            }
        },
        { // POP
            (void*)QMAP_4K2K_POP_Sub,
#if (ENABLE_PQ_EX)
            (void*)QMAP_4K2K_POP_Sub_Ex,
#endif
            {
#if (PQ_GRULE_NR_ENABLE)
                (void*)MST_GRule_4K2K_POP_NR_Sub,
#endif
            }
        },
    },
...
static MS_U8 _MDrv_PQ_Get_PanelIndexByDisplayWidth(MS_U16 u16DisplayWidth)
{
    ...
    {
        case 1366:
            u8PanelIndex = PQ_BIN_PNLTYPE_1366;
            break;
        case 3840:
            u8PanelIndex = PQ_BIN_PNLTYPE_4K2K;
            break;
        case 1920:
        default:
...
void MDrv_PQ_AdaptiveTuningByClient_U2(void* pInstance, PQ_ADAPTIVE_TUNING_CLIENT enClient)
{
    switch(enClient)
    {
        case PQ_ADAPTIVE_TUNING_CLIENT_SCALER:
            Hal_PQ_AdaptiveTuning(pInstance);
            break;
        default:
            break;
...
void MDrv_PQ_AdaptiveTuningByClient(PQ_ADAPTIVE_TUNING_CLIENT enClient)
{
    if (pu32PQInst == NULL)
    {
        if(UtopiaOpen(MODULE_PQ , &pu32PQInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
    ...
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_ADAPTIVE_TUNING_BY_CLIENT, NULL) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return ;
    }
