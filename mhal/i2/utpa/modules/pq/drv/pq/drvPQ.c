...
#ifndef PQ_SUPPORT_MAIN_DHD
#define PQ_SUPPORT_MAIN_DHD 1
#endif
...
MS_U16 _u16PQSrcType_TMO[PQ_MAX_WINDOW];
#if (ENABLE_PQ_BIN_CF)
...
MS_U8 *gpu8TmoSettingData = NULL;
MS_U16 gu16RegNum = 0;
MS_BOOL gbTmoSettingMalloc = 0;
...
#endif
#define PQ_BIN_TMO_PANEL_INDEX        0
...
#endif
extern MS_U16 QM_InputSourceToIndex_TMO(void* pInstance, PQ_WIN eWindow, MS_U8 enInputSourceType);
...
const char DefaltPath_Ini_Ext[] = "/tvconfig/config/pq/";
...
const char TMOBinName[] = "Main_TMO.bin";
#if (ENABLE_PQ_BIN_CF)
...
MS_U16 MDrv_PQ_GetGRule_TableIndex_PQDS(void* pInstance,PQ_WIN eWindow, MS_U16 u16GRuleType, MS_U16 u16PQ_NRIdx, MS_U16 u16GRuleIPIndex)
{
    ...
    if(eWindow == PQ_MAIN_WINDOW)
    {
        if(gbPQBinEnable)
        {
            ...
            {
                /// For PQ DS, Always use QM_MM_4K2K_DS_Main for P mode Video
                //                        QM_MM_FHD_I_DS_Main for I mode Video
                if(!imode_PQDS)
                {
                ...
                {
                    _u16PQSrcType[eWindow] = QM_MM_FHD_I_DS_Main;
                }
            }
#endif
...
MS_U8 * Open_PQ_Bin(MS_U8 * pName)
{
    ...
    if(u32Rst != (MS_U32)lFileSize)
    {
        ...
        printf("Read Main.bin Error %ld %ld\n", u32Rst, lFileSize);
#else
    ...
    if(IS_ERR(pFile))
    {
        printk("Open %s Failed   PTR_ERR_fp=%ld\n", pName,PTR_ERR(pFile));
...
void _MDrv_PQ_LoadPQBin(void* pInstance,MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    else
    {
        ...
        if(pFile == NULL)
        {
            ...
            ///For AN7.1 the Bin Path is in /tvconfig/config/pq,inorder to Handle Usermode PQDS case,We need to check PQ bin Path for /tvconfig/config/pq
            if(pFile == NULL)
            {
                memset(pm_PQBinFilePathName, 0, PQ_FILE_NAME_LENGTH);
                strncpy(pm_PQBinFilePathName, DefaltPath_Ini_Ext, strlen(DefaltPath_Ini_Ext));
                strncat(pm_PQBinFilePathName, MainBinName, strlen(MainBinName));
                pFile = fopen(pm_PQBinFilePathName, "r");
            }
            if (pFile != NULL)
        ...
        if(pu8MainBuf && pu8MainExBuf)
        {
            ...
            pstPQInitInfo->stPQBinInfo[0].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8MainBuf;
            pstPQInitInfo->stPQBinInfo[1].u8PQID = (MS_U8) PQ_BIN_EXT_MAIN;
            pstPQInitInfo->stPQBinInfo[1].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t)pu8MainExBuf;
        }
        else
            {
            pstPQInitInfo->u8PQBinCnt = 0;
        ...
        if(pu8MainCustomerBuf)
        {
            ...
            pstPQInitInfo->stPQBinInfo[4].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8MainCustomerBuf;
        }
        else
        {
            pstPQInitInfo->u8PQBinCustomerCnt = 0;
       ...
       //Skip USFC now, which may cause kernel panic, wait PQ Driver complated.
        ...
        if(pu8UFSCBuf)
        {
            ...
            pstPQInitInfo->stPQBinInfo[6].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8UFSCBuf;
        }
        else
        {
            pstPQInitInfo->u8PQBinUFSCCnt = 0;
        ...
        if(pu8MainCFBuf)
        {
            ...
            pstPQInitInfo->stPQBinInfo[7].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8MainCFBuf;
        }
#endif

        MS_U8 *pu8TMOBuf = NULL;
        pstPQInitInfo->u8PQBinTMOCnt = 0;
        // if UFSC.bin exists in /Customer/pq, read it
        memset(pm_PQBinFilePathName, 0, PQ_FILE_NAME_LENGTH);
        strncpy(pm_PQBinFilePathName, CustomerPath, strlen(CustomerPath));
        strncat(pm_PQBinFilePathName, TMOBinName, strlen(TMOBinName));
        ...
        if(pFile == NULL)
        {
            ...
            if (pFile != NULL)
            {
                fclose(pFile);
                pFile = NULL;
                pu8TMOBuf = Open_PQ_Bin((MS_U8 *)pm_PQBinFilePathName);
            }
            else
            {
                pu8TMOBuf = NULL;
        ...
        if(pu8TMOBuf)
        {
            printf("Load PQ bin file, PQ_BIN_TMO path [%s]\n", pm_PQBinFilePathName);
            pstPQInitInfo->u8PQBinTMOCnt = 1;
            pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].u8PQID = (MS_U8) PQ_BIN_TMO;
            pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8TMOBuf;
        }
        else
        {
            pstPQInitInfo->u8PQBinTMOCnt = 0;
        ...
        if(MDrv_PQ_GetCaps(&stPQCaps))
        {
            if(stPQCaps.b3DVideo_Supported || stPQCaps.bPIP_Supported)
            {
                ...
                if(pu8SubBuf && pu8SubExBuf)
                {
                    ...
                    pstPQInitInfo->stPQBinInfo[2].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8SubBuf;
                    pstPQInitInfo->stPQBinInfo[3].u8PQID = (MS_U8) PQ_BIN_EXT_SUB;
                    pstPQInitInfo->stPQBinInfo[3].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t)pu8SubExBuf;
                }
                ...
                if(pu8MainCustomerBuf && pu8SubCustomerBuf)
                {
                    ...
                    pstPQInitInfo->stPQBinInfo[5].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8SubCustomerBuf;
                }
            }
            #if (ENABLE_PQ_BIN_CF)
                ...
                if(pu8SubCFBuf)
                {
                    ...
                    pstPQInitInfo->stPQBinInfo[8].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8SubCFBuf;
                }
#endif
        }
#else
...
void _MDrv_PQ_LoadPQBin(void* pInstance,MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    else   //TV case
    {
        ...
        MS_U8 *pu8TMOBuf = NULL;
        pstPQInitInfo->u8PQBinTMOCnt = 0;
        // if TMO.bin exists in /Customer/pq, read it
        memset(pm_PQBinFilePathName, 0, PQ_FILE_NAME_LENGTH);
        strncpy(pm_PQBinFilePathName, CustomerPath, strlen(CustomerPath));
        strncat(pm_PQBinFilePathName, TMOBinName, strlen(TMOBinName));
        pFile = filp_open(pm_PQBinFilePathName, O_RDONLY, 0);
        ...
        if(pu8TMOBuf)
        {
            printk("Utopia kernelmode Load PQ bin file, PQ_BIN_TMO path [%s]\n", pm_PQBinFilePathName);
            pstPQInitInfo->u8PQBinTMOCnt = 1;
            pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].u8PQID = (MS_U8) PQ_BIN_TMO;
            pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].PQBin_PhyAddr = /*(MS_PHYADDR)*/(size_t) pu8TMOBuf;
        }
...
#if defined(PQ_QM_VR) && defined(PQ_TMO_SETTING_CFD)
static MS_BOOL _MDrv_PQ_SetGRuleStatus_Ex(void* pInstance, PQ_WIN eWindow, MS_U16 u16GruleType, MS_U16 u16PQ_GroupIdx, PQ_ENGINE_TYPE ePQEngineType)
{
    ...
    MS_U16 i = 0, u16IPIdx = PQ_BIN_IP_NULL, u16TabIdx = PQ_BIN_IP_NULL, u16GroupNum = PQ_BIN_IP_NULL, u16IPNum = PQ_BIN_IP_NULL;
    u16GroupNum = MDrv_PQBin_GetGRule_GroupNum(pInstance, u16GruleType, &stPQBinHeaderInfo[enPQBinType]);
    u16IPNum = MDrv_PQBin_GetGRule_IPNum(pInstance, u16GruleType, &stPQBinHeaderInfo[enPQBinType]);
    ...
    {
        printf("[PQ ERROR] %s:%d\nenPQBinType = %d, u16PQ_GroupIdx = %d out of %d\n", __FUNCTION__, __LINE__, enPQBinType, u16PQ_GroupIdx, u16GroupNum);
        return FALSE;
    }
    ...
    for(i = 0; i < u16IPNum; i++)
    {
        ...
        u16IPIdx = MDrv_PQ_GetGRule_IPIndex_Ex(eWindow, u16GruleType, i, ePQEngineType);
        u16TabIdx = MDrv_PQ_GetGRule_TableIndex_Ex(eWindow, u16GruleType, u16PQ_GroupIdx, i, ePQEngineType);
        MDrv_PQ_LoadTable_Ex(eWindow, u16TabIdx, u16IPIdx, ePQEngineType);
    }
...
void MDrv_PQ_Init_U2(void* pInstance,MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    {
        printk("%s %d, _status.bIsInitialized %d, return.\033[0m\n",__FUNCTION__,__LINE__,_status.bIsInitialized);
        return;
    }
#endif
...
#ifdef STELLAR
            if(!MApi_XC_Is_SupportSWDS() || (!MApi_XC_GetDynamicScalingStatus()))
#else
#if (!defined(MSOS_TYPE_LINUX_KERNEL))
...
#if defined(PQ_QM_VR) && defined(PQ_TMO_SETTING_CFD)
        //TMO
        if(pstPQInitInfo->u8PQBinTMOCnt == 1)
        {
            MS_U16 u16PQ_CurLvlIdx = 0;

            u8ID = pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].u8PQID;
            stPQBinHeaderInfo[u8ID].u8BinID = (MS_U8)pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].u8PQID;
#ifndef MSOS_TYPE_LINUX
            stPQBinHeaderInfo[u8ID].u32BinStartAddress = /*MS_PA2KSEG0*/(pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].PQBin_PhyAddr);
#else
            stPQBinHeaderInfo[u8ID].u32BinStartAddress = /*MS_PA2KSEG0*/(pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].PQBin_PhyAddr);
#endif
            MDrv_PQBin_Parsing(pInstance, &stPQBinHeaderInfo[u8ID]);
...
#if (ENABLE_PQ_BIN_TMO)
            ST_TMO_LEVEL stTmoLevel[PQ_GRULE_TMO_MODE_LVL_NUM_Main];
            memset(stTmoLevel, 0, PQ_GRULE_TMO_MODE_LVL_NUM_Main*sizeof(ST_TMO_LEVEL));

            _u16PQSrcType_TMO[0] = QM_SMPTE2084_TMO_Main;

            for(i=0; i<PQ_GRULE_TMO_MODE_LVL_NUM_Main; i++)
            {
                gbTmoSettingMalloc = 0;
                gpu8TmoSettingData = NULL;

                u16PQ_CurLvlIdx = MDrv_PQ_GetGRule_LevelIndex_Ex(PQ_MAIN_WINDOW, PQ_GRule_TMO_MODE_Main, i, PQ_XC0_TMO);
                _MDrv_PQ_SetGRuleStatus_Ex(pInstance, PQ_MAIN_WINDOW, PQ_GRule_TMO_MODE_Main, u16PQ_CurLvlIdx, PQ_XC0_TMO);

                if(NULL != gpu8TmoSettingData)
                {
                    stTmoLevel[i].u16HdrType = E_XC_CFD_HDR_TYPE_OPEN;
                    stTmoLevel[i].u16Level = i;
                    stTmoLevel[i].u16ControlSize = PQ_IP_TMO_NUM_Main* gu16RegNum;
                    stTmoLevel[i].pu8data = gpu8TmoSettingData;
                }
                else
                {
                    stTmoLevel[i].u16HdrType = 0xFFFF;
                    stTmoLevel[i].u16Level = 0xFFFF;
                    stTmoLevel[i].u16ControlSize = 0xFFFF;
                    stTmoLevel[i].pu8data = NULL;
                }
            }
            ...
            stCfdTmo.u16LevelCount = PQ_GRULE_TMO_MODE_LVL_NUM_Main;
            stCfdTmo.pstTmoLevel = stTmoLevel;

            XC_CFD_CONTROL_INFO stCfdCtrlInfo;
            memset(&stCfdCtrlInfo, 0, sizeof(XC_CFD_CONTROL_INFO));
            stCfdCtrlInfo.enCtrlType = E_XC_CFD_CTRL_SET_TMO;
            stCfdCtrlInfo.pParam = &stCfdTmo;
            stCfdCtrlInfo.u32ParamLen = sizeof(ST_KDRV_XC_CFD_TMO);

            MApi_XC_HDR_Control(E_XC_HDR_CTRL_CFD_CONTROL, &stCfdCtrlInfo);

            for(i=0; i<PQ_GRULE_TMO_MODE_LVL_NUM_Main; i++)
            {
                if(NULL != stTmoLevel[i].pu8data)
                {
#if !defined(MSOS_TYPE_LINUX_KERNEL)
                    free(stTmoLevel[i].pu8data);            //data is malloc in MDrv_PQBin_DumpDumyRegister function
#else
                    vfree(stTmoLevel[i].pu8data);
#endif
                }
            }
#else
            ST_TMO_LEVEL stTmoLevel[PQ_GRULE_TMO_MODE_LVL_NUM_TMO];
            memset(stTmoLevel, 0, PQ_GRULE_TMO_MODE_LVL_NUM_TMO*sizeof(ST_TMO_LEVEL));

            _u16PQSrcType_TMO[0] = QM_SMPTE2084_TMO;
            for(i=0; i<PQ_GRULE_TMO_MODE_LVL_NUM_TMO; i++)
            {
                gbTmoSettingMalloc = 0;
                gpu8TmoSettingData = NULL;

                u16PQ_CurLvlIdx = MDrv_PQ_GetGRule_LevelIndex_Ex(PQ_MAIN_WINDOW, PQ_GRule_TMO_MODE_TMO, i, PQ_XC0_TMO);
                _MDrv_PQ_SetGRuleStatus_Ex(pInstance, PQ_MAIN_WINDOW, PQ_GRule_TMO_MODE_TMO, u16PQ_CurLvlIdx, PQ_XC0_TMO);

                if(NULL != gpu8TmoSettingData)
                {
                    stTmoLevel[i].u16HdrType = E_XC_CFD_HDR_TYPE_OPEN;
                    stTmoLevel[i].u16Level = i;
                    stTmoLevel[i].u16ControlSize = PQ_IP_NUM_TMO * gu16RegNum;
                    stTmoLevel[i].pu8data = gpu8TmoSettingData;
                }
                else
                {
            ...
            stCfdTmo.u32Version = CFD_TMO_VERSION;
            stCfdTmo.u16Length = sizeof(ST_KDRV_XC_CFD_TMO);
            stCfdTmo.u16LevelCount = PQ_GRULE_TMO_MODE_LVL_NUM_TMO;
            stCfdTmo.pstTmoLevel = stTmoLevel;
...
#else
        PQINFO_DBG(printf("[%s %d] PQ do not transmit TMO data\n", __FUNCTION__,__LINE__));
#endif
    }
    else
    {
...
void MDrv_PQ_Init(MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    if (pu32PQInst == NULL)
    {
        ...
        PQ_DBG(printf("\33[1;33m[MARCOS](%s %d) OPEN FINE\33[m\n", __FUNCTION__, __LINE__));
    }
    ...
    if (eWindow == PQ_MAIN_WINDOW)
    {
        switch (ePQEngineType)
        {
            ...
            case PQ_XC0_TMO:
                if (_bTMOPQEn)
                {
                    u8PanelIdx = PQ_BIN_TMO_PANEL_INDEX;
                    enPQBinType = PQ_BIN_TMO;
                }
                break;
            default:
...
MS_U16 MDrv_PQ_GetVideoDelayTime_U2(void* pInstance,SCALER_WIN eWindow,EN_PQ_DelayTime_Type enDelayType,ST_PQ_Delay_Info stdelayinfo)
{
    MS_U8 u8FrameNum=0;
    MS_U8 u8WRBankMappingNum=0;
    MS_U8 u8PQDelayCountNum=u8PQDelayCount;
    MS_U16 u16WRDelayTime=0;
    u8FrameNum = MApi_XC_Get_FrameNumFactor(eWindow);
    u8WRBankMappingNum = MApi_XC_GetWRBankMappingNum(eWindow);

    switch(enDelayType)
    {
        case E_PQ_Delay_DisplayWithPQ:
        {
            if(u8FrameNum > (u8PQDelayCount +u8WRBankMappingNum ))
            {
                u8PQDelayCountNum = u8PQDelayCount +u8WRBankMappingNum;
            ...
            u16WRDelayTime = _u16PQDelyaTime;
        }
        break;
        case E_PQ_Delay_FirstFrameOut:
        {
            XC_INITMISC sXC_Init_Misc;
            memset(&sXC_Init_Misc, 0, sizeof(XC_INITMISC));
            MApi_XC_GetMISCStatus(&sXC_Init_Misc);
            ...
            if ((QM_GetInputVFreq(eWindow) != 0) && (QM_GetInputVFreq(eWindow) != 1))
            {
                if (( sXC_Init_Misc.u32MISC_A & E_XC_INIT_MISC_A_FRC_INSIDE) &&(sXC_ApiStatusEx.bFRCEnabled == true))
                {
                    u16WRDelayTime = (u8WRBankMappingNum + 6) * ((10000 + (QM_GetInputVFreq(eWindow)/2))/QM_GetInputVFreq(eWindow));  // frame duration is (1 second/input V freq)
                }
                else
                {
                    u16WRDelayTime = u8WRBankMappingNum * ((10000 + (QM_GetInputVFreq(eWindow)/2))/QM_GetInputVFreq(eWindow));  // frame duration is (1 second/input V freq)
                }
            }
            else
            {
                u16WRDelayTime = u8WRBankMappingNum * 20; // default keep original rule, use 20ms as frame duration
            }
        }
        break;
        case E_PQ_Delay_FirstFrameOut_Avg:
            if(stdelayinfo.u32Length == 0)
                u16WRDelayTime = 0;
            else
            {
                MS_PQ_INPUT_SOURCE_TYPE enInputSource = (MS_PQ_INPUT_SOURCE_TYPE)stdelayinfo.enSourceType;
                u16WRDelayTime = MHal_PQ_GetPQDelayAvg(enInputSource);
            }
        break;
        default:
        break;
    }
    return u16WRDelayTime;
}
MS_U16 MDrv_PQ_GetVideoDelayTime(SCALER_WIN eWindow,EN_PQ_DelayTime_Type enDelayType,ST_PQ_Delay_Info stdelayinfo)
{
#ifdef TRACE_DBG
        printf("\33[1;33m[MARCOS](%s %d)\33[m\n", __FUNCTION__, __LINE__);
#endif
    ...
    stPQ_GetVideoDelayTime PQArgs;
    PQArgs.eWindow=eWindow;
    PQArgs.enDelayType=enDelayType;
    memcpy(&(PQArgs.stdelayinfo), &stdelayinfo, sizeof(ST_PQ_Delay_Info));
    PQArgs.u16Ret = 0;
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_GET_VIDEO_DELAY_TIME, (void*)&PQArgs) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return 0;
    }
        ...
        MS_U16 u16SizeAfterScaling)
{
    ...
    MApi_XC_GetDSInfo(&stGetDSInfo, sizeof(XC_GET_DS_INFO), eWindow);
...
MS_BOOL MDrv_PQ_Get_RFBL_Info_U2(void* pInstance,PQ_WIN eWindow)
{
    ...
    if (eWindow == PQ_MAIN_WINDOW)
    {
        ...
        E_XC_FB_LEVEL eFBLevel = E_XC_FB_LEVEL_NUM;
        eFBLevel = MApi_XC_Get_FB_Level(MAIN_WINDOW);
        if((eFBLevel == E_XC_FB_LEVEL_FBL)||(eFBLevel == E_XC_FB_LEVEL_RFBL_DI))
        {
            bRet = TRUE;
        }
        else
...
#else
         bRet = MApi_XC_IsCurrentRequest_FrameBufferLessMode();
#endif
     }
     else
     {
         printf("%s  %d,  sub have no FBL mode!\n",__FUNCTION__,__LINE__);
     }
     return bRet;
}
                                              ...
                                              )
{
    _u16PQDelyaTime= 0;
#if PQ_ENABLE_RFBL
    E_XC_FB_LEVEL eFBLevel = E_XC_FB_LEVEL_FB;
#endif
    ...
    if(bMemFmt422)
    {
        ...
        else if(u16TabIdx_MADi == Hal_PQ_get_madi_idx(pInstance,(eWindow == PQ_MAIN_WINDOW) ? TRUE : FALSE, MS_MADI_25_12F_6R_MC))
        {
            *pu8BitsPerPixel = 24;
            *peDeInterlaceMode = PQ_DEINT_3DDI_HISTORY;
            MApi_XC_Set_OPWriteOffEnable(DISABLE, MAIN_WINDOW);
            *pu8FrameCount = 12;
            u8PQDelayCount = 7; //When MC mode and read/write bank mapping mode set 2, have to delay 125 ms .
            PQMADi_DBG(printf("25 12F 6R MC_PQ_DEINT_3DDI_HISTORY\n"));
        }
...
//#endif
        ...
        else if (u16TabIdx_MADi == Hal_PQ_get_madi_idx(pInstance,(eWindow == PQ_MAIN_WINDOW) ? TRUE : FALSE, MS_MADi_P_MODE8_M))
        {
            ...
            *peDeInterlaceMode = PQ_DEINT_3DDI_HISTORY;
            u8PQDelayCount = 3;
            printf("24_2R MS_MADi_P_MODE8_M\n");
        }
        ...
        else if(u16TabIdx_MADi == Hal_PQ_get_madi_idx(pInstance,(eWindow == PQ_MAIN_WINDOW) ? TRUE : FALSE, MS_MADi_P_MODE_NO_DNR_NO_MOT))
        {
            *pu8BitsPerPixel = 24;
            *peDeInterlaceMode = PQ_DEINT_2DDI_BOB;
            u8PQDelayCount = 2;
            PQMADi_DBG(printf("P PQ_DEINT_2DDI_BOB MS_MADi_P_MODE_NO_DNR_NO_MOT\n"));
...
// if there is no video mute, don't DO set FBL enable/disable for prevent screen flash
#if PQ_ENABLE_RFBL
        if (TRUE == MApi_XC_IsBlackVideoEnable((SCALER_WIN)eWindow))
        {
            if (eWindow == PQ_MAIN_WINDOW)
            {
#ifdef UFO_XC_FB_LEVEL
               MApi_XC_Set_FB_Level(eFBLevel, MAIN_WINDOW);
#else
               MS_BOOL bFBL = (eFBLevel == E_XC_FB_LEVEL_FBL) ? ENABLE : DISABLE;
               MApi_XC_EnableRequest_FrameBufferLess(bFBL);
#endif
            }
            else
            {
               PQMADi_DBG(printf("%s  %d,  sub have no FBL mode!\n",__FUNCTION__,__LINE__));
            }
        }
#endif
    ...
    if (eWindow == PQ_MAIN_WINDOW)
    {
        switch (ePQEngineType)
        {
            ...
            case PQ_XC0_TMO:
                if (_bTMOPQEn)
                {
                    u16SrcIdx = _u16PQSrcType_TMO[eWindow];
                    enPQBinType = PQ_BIN_TMO;
                }
                break;
            default:
...
MS_U32 MDrv_PQ_IsFieldOrderIncorrect_U2(void* pInstance, PQ_WIN eWindow)
{
    //Need Impl
    return FALSE;
    //return Hal_PQ_IsReversedFieldOrder(pInstance);
}

MS_U32 MDrv_PQ_IsFieldOrderIncorrect(PQ_WIN eWindow)
{
#ifdef TRACE_DBG
        printf("\33[1;33m(%s %d)\33[m\n", __FUNCTION__, __LINE__);
#endif
    ...
    stPQ_IsFieldOrderIncorrect PQArgs;
    PQArgs.eWindow = eWindow;
    PQArgs.u32Ret = 0;
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_IS_FIELD_ORDER_INCORRECT, (void*)&PQArgs) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return 0;
    }
...
#if defined(PQ_QM_VR) && defined(PQ_TMO_SETTING_CFD)
void MDrv_PQ_LoadTMOSettings_U2(void* pInstance, PQ_WIN eWindow)
{
    if(_bTMOPQEn && (eWindow == PQ_MAIN_WINDOW))
    {
        _u16PQSrcType_TMO[eWindow] = QM_InputSourceToIndex_TMO(pInstance, eWindow, _enInputSourceType[eWindow]);
        MDrv_PQBin_LoadTableBySrcType(pInstance, _u16PQSrcType_TMO[eWindow],
                                      PQ_BIN_IP_ALL,
                                      PQ_BIN_TMO_PANEL_INDEX,
                                      &stPQBinHeaderInfo[PQ_BIN_TMO],
                                      eWindow);
    }
    else
    {
        printf("\33[1;33m[TMO](%s %d) Do Not Have TMO bin",__FUNCTION__, __LINE__);
        //MS_ASSERT(0);
    }
}

void MDrv_PQ_LoadTMOSettings(PQ_WIN eWindow)
{
#ifdef TRACE_DBG
        printf("\33[1;33m[MARCOS](%s %d)\33[m\n", __FUNCTION__, __LINE__);
        //return;
    ...
    stPQ_LoadTMOSettings PQArgs;
    PQArgs.eWindow = eWindow;
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_LOAD_TMO_SETTINGS, (void*)&PQArgs) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return ;
    }
...
MS_BOOL MDrv_PQ_SetMemoryAddress_U2(void* pInstance, MS_PQ_SET_MEMORY_INFO *pstPQ_Set_Memory_Address)
{

    MS_BOOL bReturn = FALSE;

    switch(pstPQ_Set_Memory_Address->eType)
    {
        case E_PQ_MEMORY_TMO_VR_REQUEST:
             bReturn = MDrv_PQBin_SetVRAddress(pInstance, pstPQ_Set_Memory_Address->phyAddress ,pstPQ_Set_Memory_Address->u32Size );
            break;
        default:
            return FALSE;
    }
...
MS_BOOL MDrv_PQ_SetMemoryAddress(MS_PQ_SET_MEMORY_INFO *pstPQ_Set_Memory_Address)
{
#ifdef TRACE_DBG
        printf("\33[1;33m[MARCOS](%s %d)\33[m\n", __FUNCTION__, __LINE__);
        //return;
    ...
    stPQ_SetMemoryAddress PQArgs;
    PQArgs.pstPQ_Set_Memory_Address = pstPQ_Set_Memory_Address;
    PQArgs.bReturnValue = FALSE;
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_SET_MEMORY_ADDRESS, (void*)&PQArgs) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return FALSE;
    }
...
MS_U16 MDrv_PQ_GetTMOGRule_LevelIndex(void* pInstance, MS_U16 u16GRuleType, MS_U16 u16TMOGRuleLevelIndex)
{
    MS_U16 u16Ret = 0;
    MS_U16 u16PanelIdx = 0;
        ...
        if(gbPQBinEnable)
        {
            if(u16GRuleType >= stPQBinHeaderInfo[PQ_BIN_TMO].u16GRule_RuleNum)
            {
                if(_bTMOPQEn)
                {
                    PQGRULE_DBG(printf("(%s, %d)GRuleType(%u) is bigger than TMO PQ's(%u), so consider it as TMO PQ's GRule Type\n",
                                __FUNCTION__, __LINE__, u16GRuleType, stPQBinHeaderInfo[PQ_BIN_TMO].u16GRule_RuleNum));
                    u16GRuleType = u16GRuleType - stPQBinHeaderInfo[PQ_BIN_TMO].u16GRule_RuleNum;
                    u16PanelIdx = PQ_BIN_TMO_PANEL_INDEX;
                    if(u16GRuleType >= stPQBinHeaderInfo[PQ_BIN_CUSTOMER_MAIN].u16GRule_RuleNum)
                    {
                        printf("CustomerPQ: =ERROR!!!=%s, %d, =%d, out of %d, =\n", __FUNCTION__, __LINE__, u16GRuleType, stPQBinHeaderInfo[PQ_BIN_CUSTOMER_MAIN].u16GRule_RuleNum);
                        return 0;
            ...
            u16Ret = (MS_U8) MDrv_PQBin_GetGRule_LevelIndex(pInstance, u16PanelIdx,
                     u16GRuleType,
                     u16TMOGRuleLevelIndex,
                     &stPQBinHeaderInfo[PQ_BIN_TMO]);
        }
        else
        {
            u16Ret = MDrv_PQ_GetGRule_LevelIndex_(MAIN, pInstance, (MS_U8)u16GRuleType, (MS_U8)u16TMOGRuleLevelIndex);
        }
        return u16Ret;
}

MS_U16 MDrv_PQ_GetTMOGRule_IPIndex(void* pInstance, PQ_WIN eWindow, MS_U16 u16GRuleType, MS_U16 u16GRuleIPIndex)
{
    if(eWindow == PQ_SUB_WINDOW)
    {
        return 0;
...
MS_U16 MDrv_PQ_GetTMOTableIndex(void* pInstance, PQ_WIN eWindow, MS_U16 u16IPIdx)
{
    if(eWindow == PQ_SUB_WINDOW)
    {
        return 0;
    ...
    if(eWindow == PQ_MAIN_WINDOW)
    {
        if(gbPQBinEnable)
        {
            u16TabIdx = MDrv_PQBin_GetTableIndex(pInstance, _u16PQSrcType_TMO[eWindow],
                                                 u16IPIdx,
                                                 MDrv_PQBin_GetPanelIdx(pInstance),
                                                 &stPQBinHeaderInfo[PQ_BIN_TMO]);
        }
        ...
        {
            u16TabIdx = MDrv_PQ_GetTableIndex_(MAIN, pInstance, _u16PQSrcType_TMO[eWindow], (MS_U8)u16IPIdx);
        }
    }
    else
    {
...
MS_U16 MDrv_PQ_GetTMOGRule_TableIndex(void* pInstance, PQ_WIN eWindow, MS_U16 u16GRuleType, MS_U16 u16PQ_NRIdx, MS_U16 u16GRuleIPIndex)
{
    if(eWindow == PQ_SUB_WINDOW)
    {
        return 0;
    ...
    if(eWindow == PQ_MAIN_WINDOW)
    {
        ...
        {
            u16Ret = MDrv_PQ_GetGRule_TableIndex_(MAIN,pInstance, (MS_U8)u16GRuleType, _u16PQSrcType_TMO[eWindow], (MS_U8)u16PQ_NRIdx, (MS_U8)u16GRuleIPIndex);
        }
    }
    else
    {
...
MS_BOOL MDrv_PQ_LoadTMOModeGRuleTable_U2(void* pInstance, PQ_WIN eWindow, MS_PQ_HDR_GRULE_INDEX  eHDRGRuleLevelIndex, MS_U16 u16Width, MS_U16 u16Height)
{
#if (ENABLE_PQ_BIN_TMO)
    MS_U16 i, u16IPIdx=0, u16TabIdx=0;
    MS_U16 u16PQ_TMOMode_Idx=0;
    ...
    if(E_PQ_HDR_OPEN_AUTO == eHDRGRuleLevelIndex)
    {
        if(PQ_IS_4K_INPUT(u16Width, u16Height))
        {
            u16PQ_TMOMode_Idx = MDrv_PQ_GetTMOGRule_LevelIndex(pInstance, PQ_GRule_TMO_MODE_Main, PQ_GRule_TMO_MODE_High_Main);
        }
        else
        {
            u16PQ_TMOMode_Idx = MDrv_PQ_GetTMOGRule_LevelIndex(pInstance, PQ_GRule_TMO_MODE_Main, PQ_GRule_TMO_MODE_Mid_Main);
        }
    }
    else if(E_PQ_HDR_OPEN_HIGH == eHDRGRuleLevelIndex)
    {
        u16PQ_TMOMode_Idx = MDrv_PQ_GetTMOGRule_LevelIndex(pInstance, PQ_GRule_TMO_MODE_Main, PQ_GRule_TMO_MODE_High_Main);
    }
    else if(E_PQ_HDR_OPEN_MID == eHDRGRuleLevelIndex)
    {

        u16PQ_TMOMode_Idx = MDrv_PQ_GetTMOGRule_LevelIndex(pInstance, PQ_GRule_TMO_MODE_Main, PQ_GRule_TMO_MODE_Mid_Main);

    }
    else if(E_PQ_HDR_OPEN_LOW == eHDRGRuleLevelIndex)
    {

        u16PQ_TMOMode_Idx = MDrv_PQ_GetTMOGRule_LevelIndex(pInstance, PQ_GRule_TMO_MODE_Main, PQ_GRule_TMO_MODE_Low_Main);
    }
    else
    {
        return FALSE;
    ...
    if (u16PQ_TMOMode_Idx > PQ_GRULE_TMO_MODE_NUM_Main)
    {
        MS_ASSERT(0);
        return FALSE;
    }
    ...
    for(i=0; i<PQ_GRULE_TMO_MODE_IP_NUM_Main; i++)
    {
        u16IPIdx = MDrv_PQ_GetTMOGRule_IPIndex(pInstance, eWindow, PQ_GRule_TMO_MODE_Main, i);

        if (u16PQ_TMOMode_Idx == PQ_GRULE_TMO_MODE_NUM_Main) // see it as default
            u16TabIdx = MDrv_PQ_GetTMOTableIndex(pInstance, eWindow, u16IPIdx);
        else
            u16TabIdx = MDrv_PQ_GetTMOGRule_TableIndex(pInstance, eWindow, PQ_GRule_TMO_MODE_Main, u16PQ_TMOMode_Idx, i);

        PQGRULE_DBG(printf("[TMOMode]SRC  PQSrcType:%u, u16PQ_HDRMode_Idx: %u, HDRModeIPIdx:%u, IPIdx:%u, u8TabIdx:%u\n",
            _u16PQSrcType_TMO[eWindow], u16PQ_TMOMode_Idx, i, u16IPIdx, u16TabIdx));

        if(gbPQBinEnable)
        {
            MDrv_PQBin_LoadTable(pInstance, PQ_BIN_TMO_PANEL_INDEX,
                                 u16TabIdx,
                                 u16IPIdx,
                                 &stPQBinHeaderInfo[PQ_BIN_TMO],eWindow);
        }
        else
        {
            MDrv_PQ_LoadTable_(MAIN,pInstance, (MS_U8)u16TabIdx, (MS_U8)u16IPIdx);
        }
...
MS_BOOL MDrv_PQ_LoadTMOModeGRuleTable(PQ_WIN eWindow, MS_PQ_HDR_GRULE_INDEX  eHDRGRuleLevelIndex, MS_U16 u16Width, MS_U16 u16Height)
{
#ifdef TRACE_DBG
        printf("\33[1;33m[MARCOS](%s %d)\33[m\n", __FUNCTION__, __LINE__);
        //return;
    ...
    stPQ_LoadTMOModeGRuleTable PQArgs;
    PQArgs.eWindow = eWindow;
    PQArgs.eHDRGRuleLevelIndex= eHDRGRuleLevelIndex;
    PQArgs.u16Width= u16Width;
    PQArgs.u16Height= u16Height;
    PQArgs.bReturnValue = FALSE;
    if(UtopiaIoctl(pu32PQInst, E_PQ_CMD_LOAD_TMO_MODE_GRULE_TABLE, (void*)&PQArgs) != 0)
    {
        printf("Obtain PQ engine fail\n");
        return FALSE;
    }
