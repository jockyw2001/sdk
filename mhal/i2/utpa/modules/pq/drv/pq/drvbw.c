...
extern void* g_pPQResource[E_PQ_DEVICE_ID_MAX];
extern MS_U32 MDrv_PQ_Get_Semaphore(void *pInstance,E_PQ_POOL_ID eID_TYPE);
extern MS_U32 MDrv_PQ_Release_Semaphore(void *pInstance,E_PQ_POOL_ID eID_TYPE);
#endif
...
static void _MDrv_BW_LoadTable(void *pInstance,MS_U8 u8TabIdx)
{
    ...
    strncpy(pm_PQBinFilePathName, pPQResourcePrivate->stPQBin_Path.pm_PQ_BANDWIDTH_PATH, strlen(pPQResourcePrivate->stPQBin_Path.pm_PQ_BANDWIDTH_PATH));
    strncat(pm_PQBinFilePathName, bandwidthfileName, strlen(bandwidthfileName));
    MDrv_PQ_Release_Semaphore(pInstance,E_PQ_POOL_ID_INTERNAL);
    printf("open %s \n", pm_PQBinFilePathName);
    pu8Buffer = Open_bandwidthtable_Bin(pm_PQBinFilePathName);
#else
