...
MS_BOOL     _bDemodType=FALSE;//DVBS:FALSE   ;  S2:TRUE
...
MS_BOOL INTERN_DVBS_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    // ==== Command Phase ===================
    DBG_INTERN_DVBS(ULOGD("DEMOD","--->INTERN_DVBS (cmd=0x%x)(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,) \n",pCmdPacket->cmd_code,
    ...
    do
    {
        ...
        if (timeout > 200)
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD","---> INTERN_DVBS_Cmd_Packet_Send fail on 'wait _BIT_END clear' \n"));
            return false;
        }
        timeout++;
    } while (1);
    ...
    do
    {
        ...
        if (timeout > 200)
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD","---> INTERN_DVBS_Cmd_Packet_Send fail on 'wait _BIT_START clear' \n"));
            return false;
        }
        timeout++;
    } while (1);
    ...
    for (indx = 0; indx < param_cnt; indx++)
    {
        ...
        do
        {
            ...
            if (timeout > 200)
            {
                DBG_INTERN_DVBS(ULOGD("DEMOD","---> INTERN_DVBS_Cmd_Packet_Send fail on 'wait _BIT_DRQ clear' \n"));
                return false;
            }
            timeout++;
        } while (1);
...
MS_BOOL INTERN_DVBS_SoftStop ( void )
{
    ...
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD",">> MB Busy!\n"));
        return FALSE;
    }
    ...
    while(HAL_DMD_RIU_ReadByte(MBRegBase + 0x00)!= 0x5A)           // wait MB_CNTL set done
    {
        ...
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD",">> DVBT SoftStop Fail!\n"));
            return FALSE;
        }
    }
...
{
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_reset\n"));
...
MS_BOOL INTERN_DVBS_Exit ( void )
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_Exit\n"));
    INTERN_DVBS_SoftStop();
...
*/
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD",">Verify Code...\n"));
    ...
    for ( i = 0; i < sizeof(INTERN_DVBS_table); i++)
    {
        ...
        if (udata != INTERN_DVBS_table[i])
        {
            ...
            if (fail_cnt > 10)
            {
                ULOGD("DEMOD",">DVB-S DSP Loadcode fail!");
                return false;
            }
            fail_cnt++;
        }
...
*/
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD",">DSP Loadcode done."));
#if 0
...
//wriu 0x101e39 0x03
    ...
    //reg_diseqc_out_config : ¡¥h68, [5] = 1¡¦b1
    u8Temp = HAL_DMD_RIU_ReadByte(0x101ED0);
    u8Temp|=0x10;
    HAL_DMD_RIU_WriteByte(0x101ED0, u8Temp);
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_InitClkgen\n"));
}
...
MS_BOOL INTERN_DVBS_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBS_DSPRegInitExt, MS_U8 u8DMD_DVBS_DSPRegInitSize)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_Power_On_Initialization\n"));
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","@@@@@INTERN_DVBS_Power_On_Initialization1:bRFAGCTristateEnable=%d ;u8ADCIQMode=%d \n",bRFAGCTristateEnable,u8ADCIQMode));
    DBG_INTERN_DVBS(ULOGD("DEMOD","@@@@@INTERN_DVBS_Power_On_Initialization2:u8PadSel=%d ;bPGAEnable=%d \n",u8PadSel,bPGAEnable));
    DBG_INTERN_DVBS(ULOGD("DEMOD","@@@@@INTERN_DVBS_Power_On_Initialization2:u8PGAGain=%d \n",u8PGAGain));

    //// Firmware download //////////
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS Load DSP...\n"));
    //MsOS_DelayTask(100);

    {
        if (INTERN_DVBS_LoadDSPCode() == FALSE)
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD","DVB-S Load DSP Code Fail\n"));
            return FALSE;
        }
        else
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD","DVB-S Load DSP Code OK\n"));
        }
    }
    ...
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS Reset...Fail\n"));
        return FALSE;
    }
    else
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS Reset...OK\n"));
    }
...
void INTERN_DVBS_Driving_Control(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","---> INTERN_DVBS_Driving_Control(Bit0) = 0x%x \n",u8Temp));
...
void INTERN_DVBS_Clk_Inversion_Control(MS_BOOL bInversionEnable)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","---> Inversion(Bit9) = 0x%x \n",u8Temp));
...
MS_BOOL INTERN_DVBS_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8   temp_val;
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_ts... u8TSClk=%d\n", u8TSClk));
...
#if(INTERN_DVBS_TS_SERIAL_INVERSION == 0)
    DBG_INTERN_DVBS(ULOGD("DEMOD","---> Inversion(Bit5) = %d \n",0 ));
#else
    DBG_INTERN_DVBS(ULOGD("DEMOD","---> Inversion(Bit5) = %d \n",1 ));
#endif
...
{
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_TS1_Enable... \n"));
...
MS_BOOL INTERN_DVBS_Config(MS_U32 u32SymbolRate, DMD_DVBS_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U8 u8TSClk, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_config+, QAM=%d, u32IFFreq=%d, bSpecInv=%d, bSerialTS=%d, u8TSClk=%d\n", eQamMode, u32IFFreq, bSpecInv, bSerialTS, u8TSClk));
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_Config, t = %d\n",MsOS_GetSystemTime()));
...
*/
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Config TS Serial %d<<<\n", bSerialTS));
    ...
    {
        MsOS_DelayTask(1);
        ULOGD("DEMOD","TOP_WR_DBG_90=0x%x, status=%d, u8counter=%d\n", u8Data, status, u8counter);
        u8Data|=0x01;
        MDrv_SYS_DMD_VD_MBX_WriteReg(TOP_REG_BASE + 0x60*2, u8Data);
        MDrv_SYS_DMD_VD_MBX_ReadReg(TOP_REG_BASE + 0x60*2, &u8Data);
        DBG_INTERN_DVBS(ULOGD("DEMOD",">>>(while)REG read check: addr=%d    value=%d<<<\n",(TOP_REG_BASE + 0x60*2), u8Data));
        u8counter--;
    }
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_config done\n"));
    return status;
}
/************************************************************************************************
  Subject:    channel change config
...
MS_BOOL INTERN_DVBS_BlindScan_Config(MS_U32 u32SymbolRate, DMD_DVBS_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U8 u8TSClk, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_blindScan_Config, t = %d\n",MsOS_GetSystemTime()));
    ...
    */
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_blindScan_Config done\n"));
    return status;
}
...
MS_BOOL INTERN_DVBS_Active(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD"," @INTERN_DVBS_Active\n"));
...
MS_BOOL INTERN_DVBS_GetTsDivNum(MS_U32 *u32SymbolRate, MS_U8* system_type_reg, MS_U8 *code_rate_idx, MS_U8 *fec_type_idx, MS_U8 *pilot_flag, MS_U32 *u32temp, MS_U8 *code_rate_reg)
{
   ...
   // MS_U8 fec_type_idx = 0;
    ...
    //MS_U8 time_counter=0;
     ...
     DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum u32SymbolRate=%d\n", *u32SymbolRate));
//     DMD_DVBS_MODULATION_TYPE pQAMMode;

    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_SYSTEM_TYPE, &u8Data);//V
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum GetCurrentDemodType E_DMD_S2_SYSTEM_TYPE=%d\n", u8Data));//u8Data:0 is S2;  1 is DVBS
    *system_type_reg=u8Data;
    if(!u8Data)//DVBS2
    {
        /*
        //Get DVBS2 Code Rate
	...
	MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_ISSY_ACTIVE, ISSY_EN);
        if(*ISSY_EN==0)
        {
            ...
            {
                *fTSDivNum =(288000.0/(k_bch/((n_ldpc/modulation_order+90+pilot_term)/u32SymbolRate)) - ts_div_num_offset);
                *fTSDivNum = (*fTSDivNum-1)/2;// since  288/(2(fTSDivNum+1)) = 288/TS_RATE = A  ==> fTSDivNum = (A-1)/2
            }
            else//parallel mode
            {
	 ...
	 *code_rate_reg=u8Data;
        switch (u8Data)
        {
            case 0x00: //CR 1/2
                  _u8_DVBS2_CurrentCodeRate = 0;
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=1_2\n"));
               /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum = (288000.0/((1.0/2.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
                      *fTSDivNum = (288000.0/((1.0/2.0)*(188.0/204.0)*u32SymbolRate*2/8.0) - 5);
            ...
            case 0x01: //CR 2/3
               ...
               DBG_INTERN_DVBS(printf("INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=2_3\n"));
                  /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum = (MS_U8)(288000.0/((2.0/3.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
                *fTSDivNum = (MS_U8)(288000.0/((2.0/3.0)*(188.0/204.0)*u32SymbolRate*2/8.0) - 5);
            ...
            case 0x02: //CR 3/4
               ...
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=3_4\n"));
                 /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum = (288000.0/((3.0/4.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
                *fTSDivNum = (288000.0/((3.0/4.0)*(188.0/204.0)*u32SymbolRate*2/8.0) - 5);
               *fTSDivNum = (*fTSDivNum-1)/2;
                if(*fTSDivNum>255)
                    *fTSDivNum=255;
                if(*fTSDivNum<1)
            ...
            case 0x03: //CR 5/6
               ...
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=5_6\n"));
                  /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum = (288000.0/((5.0/6.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
                *fTSDivNum = (288000.0/((5.0/6.0)*(188.0/204.0)*u32SymbolRate*2/8.0) - 5);
            ...
            case 0x04: //CR 7/8
               ...
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=7_8\n"));
                  /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum =(288000.0/((7.0/8.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
                *fTSDivNum = (288000.0/((7.0/8.0)*(188.0/204.0)*u32SymbolRate*2/8.0) - 5);
            ...
            default:
                  _u8_DVBS2_CurrentCodeRate = 4;
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate= default 7_8\n"));
                 /*
		    if(sDMD_DVBS_Info.bSerialTS)
                      *fTSDivNum = (288000.0/((7.0/8.0)*(188.0/204.0)*u32SymbolRate*2) - 5);
                  else
...
MS_BOOL INTERN_DVBS_GetLock(DMD_DVBS_GETLOCK_TYPE eType, MS_U16 fCurrRFPowerDbm, MS_U16 fNoChannelRFPowerDbm, MS_U32 u32TimeInterval)
{
    ...
    switch( eType )
    {
        case DMD_DVBS_GETLOCK:
            ...
            DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_GetLock manual tune=%d<<<\n", u8Data));
            if ((u8Data&0x02)==0x00)//manual mode
            {
                bRet &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_STATE_FLAG, &u8Data);
                DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_GetLock MailBox state=%d<<<\n", u8Data));
                ...
                if((u8Data == 15) || (u8Data == 16))
                {
                    ...
                    {
                        _bDemodType=FALSE;   //S
                        DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod DVBS Lock<<<\n"));
                    }
                    else if(u8Data==16)
                    {
                        _bDemodType=TRUE;    //S2
                        DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod DVBS2 Lock<<<\n"));
                    }
                    if(g_dvbs_lock == 0)
                    {
                        g_dvbs_lock = 1;
                    ...
                    DBG_INTERN_DVBS(ULOGD("DEMOD","@INTERN_DVBS_Demod Lock+++\n"));
                    bRet = TRUE;
                }
                else
                {
                    ...
                    DBG_INTERN_DVBS(ULOGD("DEMOD","@INTERN_DVBS_Demod UnLock---\n"));
                    bRet = FALSE;
                }
...
MS_BOOL INTERN_DVBS_GetTunrSignalLevel_PWR(MS_U16 *u16Data)// Need check debug out table
{
    MS_BOOL status=TRUE;
    MS_U8  u8Data =0;
    //MS_U8  u8Index =0;
...
*/
   // printf("#### INTERN_DVBS_GetTunrSignalLevel_PWR u16Data = %d\n", (int)u16Data);
	/*
...
MS_BOOL INTERN_DVBS_GetPostViterbiBer(MS_U32 *BitErr, MS_U16 *BitErrPeriod)//POST BER //V
{
    MS_BOOL           status = true;
...
MS_BOOL INTERN_DVBS_GetPacketErr(MS_U16 *pktErr)//V
{
    ...
    *pktErr = u16PktErr;
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS PktErr = %d \n", (int)u16PktErr));
...
MS_BOOL INTERN_DVBS_GetSNR(MS_U32 *u32NDA_SNR_A, MS_U32 *u32NDA_SNR_AB)//V
{
    MS_BOOL status= TRUE;
    ...
    *u32NDA_SNR_AB=(*u32NDA_SNR_AB<<8)|u8Data;
    ...
    if (status== FALSE)
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetSNR Fail! \n"));
        return 0;
    }
...
MS_BOOL INTERN_DVBS_GetSignalStrength(MS_U16 fRFPowerDbm, DMD_DVBS_DEMOD_TYPE *pDemodType, MS_U8  *u8_DVBS2_CurrentCodeRateLocal,  MS_U8   *u8_DVBS2_CurrentConstellationLocal)
{
    ...
    //DBG_INTERN_DVBS_TIME(printf("INTERN_DVBS_GetSignalStrength, t=%ld, RF level=%f, Table=%lx\n",MsOS_GetSystemTime(), fRFPowerDbm, (MS_U32)(sDMD_DVBS_InitData->pTuner_RfagcSsi)));
     ...
     */
        ...
        *u8_DVBS2_CurrentCodeRateLocal = (u8Data & 0x3C)>>2;
        ...
        {
           *u8_DVBS2_CurrentConstellationLocal = DMD_DVBS_QPSK;
        }
        else if (((u8Data & 0x03)==0x01) && ((u8Data2 & 0x80)==0x80))
        {
            *u8_DVBS2_CurrentConstellationLocal = DMD_DVBS_8PSK;//8PSK
        }
...
MS_BOOL INTERN_DVBS_Get_FreqOffset(MS_S16 *s16CFO)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD",">>> INTERN_DVBS_Get_FreqOffset DVBS_Estimated_CFO <<<\n"));
...
MS_BOOL INTERN_DVBS_GetCurrentModulationType(DMD_DVBS_MODULATION_TYPE *pQAMMode)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType\n"));
    ...
    if(u8Data)
    {
        *pQAMMode = DMD_DVBS_QPSK;
        modulation_order=2;
        ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs]ModulationType=DVBS_QPSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS_QPSK\n"));
        //return TRUE;
    }
    else                                        //S2
    {
      ...
      printf("@@@@@  E_DMD_S2_IS_ID=%d  \n",E_DMD_S2_IS_ID);*/
        ...
        if(MOD_type==2)
        {
            *pQAMMode = DMD_DVBS_QPSK;
        modulation_order=2;
            ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_QPSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_QPSK\n"));
            //return TRUE;
        }
        else if(MOD_type==3)
        {
        ...
        modulation_order=3;
            ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_8PSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_8PSK\n"));
            //return TRUE;
        }
         else if(MOD_type==4)
         {
        ...
        modulation_order=4;
            ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_16APSK\n");
         }
        else
        {
            *pQAMMode = DMD_DVBS_QPSK;
            modulation_order=2;
            ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=NOT SUPPORT\n");
            return FALSE;
        }
...
MS_BOOL INTERN_DVBS_GetCurrentDemodType(DMD_DVBS_DEMOD_TYPE *pDemodType)//V
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentDemodType\n"));
    ...
    if(!u8Data)                                                       //S2
    {
        *pDemodType = DMD_SAT_DVBS2;
        DBG_INTERN_DVBS(ULOGD("DEMOD","[dvbs]DemodType=DVBS2\n"));
    }
    else                                                                            //S
    {
        *pDemodType = DMD_SAT_DVBS;
        DBG_INTERN_DVBS(ULOGD("DEMOD","[dvbs]DemodType=DVBS\n"));
    }
    return status;
}
/****************************************************************************
...
MS_BOOL INTERN_DVBS_GetCurrentDemodCodeRate(DMD_DVBS_CODE_RATE_TYPE *pCodeRate)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate\n"));
    //DMD_DVBS_DEMOD_TYPE pDemodType = 0;
    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_SYSTEM_TYPE, &u8Data);
    //status &= INTERN_DVBS_GetCurrentDemodType(&pDemodType);
    if(!u8Data)
    //if((MS_U8)pDemodType == (MS_U8)DMD_SAT_DVBS2 )  //S2
    {
        ...
        case 0x03:
            *pCodeRate = DMD_CONV_CODE_RATE_1_2;
            ...
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=1_2\n"));
            break;
        case 0x01:
            *pCodeRate = DMD_CONV_CODE_RATE_1_3;
            _u8_DVBS2_CurrentCodeRate = 6;//1;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=1_3\n"));
            break;
        case 0x05:
            *pCodeRate = DMD_CONV_CODE_RATE_2_3;
            _u8_DVBS2_CurrentCodeRate = 7;//2;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=2_3\n"));
            break;
        case 0x00:
            *pCodeRate = DMD_CONV_CODE_RATE_1_4;
            _u8_DVBS2_CurrentCodeRate = 8;//3;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=1_4\n"));
            break;
        case 0x06:
            *pCodeRate = DMD_CONV_CODE_RATE_3_4;
            _u8_DVBS2_CurrentCodeRate = 9;//4;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=3_4\n"));
            break;
        case 0x02:
            *pCodeRate = DMD_CONV_CODE_RATE_2_5;
            _u8_DVBS2_CurrentCodeRate = 10;//5;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=2_5\n"));
            break;
        case 0x04:
            *pCodeRate = DMD_CONV_CODE_RATE_3_5;
            _u8_DVBS2_CurrentCodeRate = 11;//6;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=3_5\n"));
            break;
        case 0x07:
            *pCodeRate = DMD_CONV_CODE_RATE_4_5;
            _u8_DVBS2_CurrentCodeRate = 12;//7;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=4_5\n"));
            break;
        case 0x08:
            *pCodeRate = DMD_CONV_CODE_RATE_5_6;
            _u8_DVBS2_CurrentCodeRate = 13;//8;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=5_6\n"));
            break;
        case 0x09:
            *pCodeRate = DMD_CONV_CODE_RATE_8_9;
            _u8_DVBS2_CurrentCodeRate = 14;//9;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=8_9\n"));
            break;
        case 0x0a:
            *pCodeRate = DMD_CONV_CODE_RATE_9_10;
            _u8_DVBS2_CurrentCodeRate = 15;//10;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=9_10\n"));
            break;
        default:
            *pCodeRate = DMD_CONV_CODE_RATE_9_10;
            _u8_DVBS2_CurrentCodeRate = 15;//10;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=DVBS2_Default\n"));
        }
    }
    else                                            //S
    {
        ...
        case 0x00:
            *pCodeRate = DMD_CONV_CODE_RATE_1_2;
            _u8_DVBS2_CurrentCodeRate = 0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=1_2\n"));
            break;
        case 0x01:
            *pCodeRate = DMD_CONV_CODE_RATE_2_3;
            _u8_DVBS2_CurrentCodeRate = 1;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=2_3\n"));
            break;
        case 0x02:
            *pCodeRate = DMD_CONV_CODE_RATE_3_4;
            _u8_DVBS2_CurrentCodeRate = 2;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=3_4\n"));
            break;
        case 0x03:
            *pCodeRate = DMD_CONV_CODE_RATE_5_6;
            _u8_DVBS2_CurrentCodeRate = 3;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=5_6\n"));
            break;
        case 0x04:
            *pCodeRate = DMD_CONV_CODE_RATE_7_8;
            _u8_DVBS2_CurrentCodeRate = 4;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=7_8\n"));
            break;
        default:
            *pCodeRate = DMD_CONV_CODE_RATE_7_8;
            _u8_DVBS2_CurrentCodeRate = 4;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetCurrentCodeRate=DVBS_Default\n"));
        }
    }
    return status;
}
...
MS_BOOL INTERN_DVBS_GetCurrentSymbolRate(MS_U32 *u32SymbolRate)
{
    ...
    *u32SymbolRate = (tmp<<16)|u16SymbolRateTmp;
    ...
    DBG_INTERN_DVBS_LOCK(ULOGD("DEMOD","[dvbs]Symbol Rate=%d\n",*u32SymbolRate));
...
MS_BOOL INTERN_DVBS_GetRollOff(MS_U8 *pRollOff)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetRollOff:%d\n", *pRollOff));

    return status;
}
MS_BOOL INTERN_DVBS_GetBCHEflagWindow(MS_U16 *BCH_Eflag_Window)
{
    ...
    status &=MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_REG_BASE + 0x25*2 + 1, &u8Data);
    u16Data = u8Data;
    status &=MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_REG_BASE + 0x25*2 , &u8Data);
    u16Data =	(u16Data<<8) | u8Data;
    *BCH_Eflag_Window = u16Data;
...
MS_BOOL INTERN_DVBS_BlindScan_Start(MS_U16 u16StartFreq,MS_U16 u16EndFreq)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_Start+\n"));
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_Start- _u16BlindScanStartFreq%d u16StartFreq %d u16EndFreq %d\n", _u16BlindScanStartFreq, u16StartFreq, u16EndFreq));
...
MS_BOOL INTERN_DVBS_BlindScan_NextFreq(MS_BOOL* bBlindScanEnd)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_NextFreq+\n"));

    * bBlindScanEnd=FALSE;

    if (_u16TunerCenterFreq >=_u16BlindScanEndFreq)
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_NextFreq . _u16TunerCenterFreq %d _u16BlindScanEndFreq%d\n", _u16TunerCenterFreq, _u16BlindScanEndFreq));
        * bBlindScanEnd=TRUE;
...
MS_BOOL INTERN_DVBS_BlindScan_GetTunerFreq(MS_U16 *u16TunerCenterFreq, MS_U16 *u16TunerCutOffFreq)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_SetTunerFreq+\n"));
...
//end claire test
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_SetTunerFreq- _u16TunerCenterFreq:%d\n", _u16TunerCenterFreq));
...
MS_BOOL INTERN_DVBS_BlindScan_WaitCurFreqFinished(MS_U8* u8Progress,MS_U8 *u8FindNum, MS_U8 *substate_reg, MS_U32  *u32Data, MS_U16 *symbolrate_reg, MS_U16 *CFO_reg)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_WaitCurFreqFinished+\n"));
    ...
    *u8Progress=0;
    ...
    do
    {
        ...
        u16WaitCount++;
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_BlindScan_WaitCurFreqFinished State: 0x%x Status: 0x%x u16WaitCount:%d\n", u8Data, u8Data2, u16WaitCount));
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_WaitCurFreqFinished OuterCheckStatus:0x%x\n", u16Data));

    if (u16WaitCount>=INTERN_DVBS_DEMOD_WAIT_TIMEOUT)
    {
        status=false;
        ULOGD("DEMOD","Debug blind scan wait finished time out!!!!\n");
    }
    else
    {
        ...
        *substate_reg=u8Data;
	     ...
	     *symbolrate_reg=u16Data;
	     ...
	     if (u16Data*1000 >= 0x8000)
            ...
            */
            ...
            DBG_INTERN_DVBS(ULOGD("DEMOD","Current Locked BWH:%d BWL:%d Step:%d\n ",_u16PreLockedHB, _u16PreLockedLB, _u16CurrentStepSize));
        }
        else if (u8Data==1)
        {
		...
		/*
            ...
            */
            ...
            DBG_INTERN_DVBS(ULOGD("DEMOD","Pre Locked BWH:%d BWL:%d Step:%d\n ",_u16PreLockedHB, _u16PreLockedLB, _u16CurrentStepSize));
        }
    }
    *u8Progress=100;

    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_WaitCurFreqFinished- u8Progress: %d u8FindNum %d\n", *u8Progress, *u8FindNum));
...
MS_BOOL INTERN_DVBS_BlindScan_Cancel(void)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_Cancel+\n"));
...
MS_BOOL INTERN_DVBS_BlindScan_End(void)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_End+\n"));
...
MS_BOOL INTERN_DVBS_BlindScan_GetChannel(MS_U16 u16ReadStart,MS_U16* u16TPNum,HAL_DEMOD_MS_FE_CARRIER_PARAM *pTable)
{
    ...
    *u16TPNum=_u16ChannelInfoIndex-u16ReadStart;
    for(u16TableIndex = 0; u16TableIndex < (*u16TPNum); u16TableIndex++)
    {
        ...
        DBG_INTERN_DVBS(ULOGD("DEMOD","MDrv_Demod_BlindScan_GetChannel Freq: %d SymbolRate: %d\n", pTable[u16TableIndex].u32Frequency, pTable[u16TableIndex].SatParam.u32SymbolRate));
    }
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_u16TPNum:%d\n", *u16TPNum));
...
{
    MS_BOOL status=TRUE;
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_BlindScan_GetCurrentFreq+\n"));

    *u32CurrentFeq=_u16TunerCenterFreq;
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_BlindScan_GetCurrentFreq-: %d\n", _u16TunerCenterFreq));
    return status;
}
...
MS_BOOL INTERN_DVBS_DiSEqC_Init(void)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_DiSEqC_Init\n"));
...
MS_BOOL INTERN_DVBS_DiSEqC_SetTone(MS_BOOL bTone1)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_DiSEqC_SetTone:%d\n", bTone1));
    //MsOS_DelayTask(100);
    return status;
}
...
MS_BOOL INTERN_DVBS_DiSEqC_Set22kOnOff(MS_BOOL b22kOn)
{
    ...
    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_DiSEqC_Set22kOnOff:%d\n", b22kOn));
    return status;
}
...
*/
    ...
    for (u8Index=0; u8Index < u8CmdSize; u8Index++)
    {
        ...
        status &= MDrv_SYS_DMD_VD_MBX_WriteReg(DVBS2_REG_BASE + 0xC4 + u8Index, u8Data);//#define DVBS2_DISEQC_TX1                            (_REG_DVBS2(0x62)+0)//[7:0]
         DBG_INTERN_DVBS(ULOGD("DEMOD","=============INTERN_DVBS_DiSEqC_SendCmd(Demod1) = 0x%X\n",u8Data));
    }
   ...
   //add this only for check mailbox R/W
    #if 1
    DBG_INTERN_DVBS(ULOGD("DEMOD"," Write into E_DMD_S2_MB_DMDTOP_DBG_8 = 0x%X!!!\n",u8Data));
    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_MB_DMDTOP_DBG_8, &u8Data);
    DBG_INTERN_DVBS(ULOGD("DEMOD"," Read from E_DMD_S2_MB_DMDTOP_DBG_8 = 0x%X!!!\n",u8Data));
    #endif
    ...
    if (u16WaitCount >= INTERN_DVBS_DEMOD_WAIT_TIMEOUT)
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS DiSEqC Send Command Busy!!!\n"));
        return FALSE;
    }
     else
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS DiSEqC Send Command Success!!!\n"));
        return TRUE;
    }
