...
static MS_BOOL _HAL_EXTERN_ATSC_Download(void)
{
    ...
    {    
        ...
        _SSPI_MIU_Writes(SRAM_SSPI_Address, (MS_U8 *)EXTERN_ATSC_table, u16Lib_size);
...
static void _HAL_EXTERN_ATSC_FWVERSION(void)
{
    ...
    printf("EXTERN_ATSC_FW_VERSION:%x.%x.%x\n", data1, data2, data3);
}
...
#ifdef UTPA2
static MS_BOOL _HAL_EXTERN_ATSC_ReadBER(MS_U32 *pBitErr, MS_U16 *pError_window, MS_U32 *pWin_unit)
#else
...
#endif
{
    ...
    *pMode = eMode;
    ...
    {
        _I2C_READ_BYTE(0x1303,&u8tempValue);
        _I2C_WRITE_BYTE(0x1303,u8tempValue|0x80);      
        ...
        *pFF = PTK_LOOP_FF;
        #else
        if(u8PTK_LOOP_FF_R3&0x80)
            PTK_LOOP_FF = -1*(0x10000-PTK_LOOP_FF);
        ...
        *pRate = u8PTK_RATE_1;
        #else
        if(u8PTK_RATE_1 == 0xF3)
            FreqOffset = FreqOffset-70;
        else if (u8PTK_RATE_1 == 0x99)
            FreqOffset = FreqOffset+70;
        else if (u8PTK_RATE_1 == 0xD3)
            FreqOffset = FreqOffset-20;
        else
            FreqOffset = FreqOffset+20;
        #endif
    ...
    {
        _I2C_READ_BYTE(0x1904, &u8AD_CRL_LOOP_VALUE0);
        _I2C_READ_BYTE(0x1905, &u8AD_CRL_LOOP_VALUE1);
        
        AD_CRL_LOOP_VALUE = (u8AD_CRL_LOOP_VALUE1<<8) | u8AD_CRL_LOOP_VALUE0;
        
        _I2C_READ_BYTE(0x3E58, &u8MIX_RATE_0);
        _I2C_READ_BYTE(0x3E59, &u8MIX_RATE_1);
        _I2C_READ_BYTE(0x3E5A, &u8MIX_RATE_2);
        _I2C_READ_BYTE(0x3E5B, &u8MIX_RATE_3);
        
        //MIX_RATE = (u8MIX_RATE_2<<12)|(u8MIX_RATE_1<<4)|(u8MIX_RATE_0>>4);
        MIX_RATE = (u8MIX_RATE_3<<24)|(u8MIX_RATE_2<<16)|(u8MIX_RATE_1<<8)|u8MIX_RATE_0;
        
        #ifdef UTPA2
        *pFF = AD_CRL_LOOP_VALUE;
        *pRate = MIX_RATE;
        #else
        IFkHZ_diff = MIX_RATE * 25410.0 / 134217728.0 - pRes->sDMD_ATSC_InitData.u16IF_KHZ; //134217728 = 2^27
        
        if (u8AD_CRL_LOOP_VALUE1 & 0x80) 
            AD_CRL_LOOP_VALUE = AD_CRL_LOOP_VALUE - 0x10000;
        
        if (eMode == DMD_ATSC_DEMOD_ATSC_256QAM) //256QAM//
        {
            //FreqOffset = (float)(AD_CRL_LOOP_VALUE*0.0025561); //5.360537E6/2^21*1000
            FreqOffset = (float)(AD_CRL_LOOP_VALUE * 5361.0 / 524288.0);
        }
        else if (eMode == DMD_ATSC_DEMOD_ATSC_64QAM)//64QAM//
        {
            //FreqOffset = (float)(AD_CRL_LOOP_VALUE*0.00241134); //5.056941E6/2^21*1000
            FreqOffset = (float)(AD_CRL_LOOP_VALUE * 5057.0 / 524288.0);
        }
        
        //FreqOffset = FreqOffset+(float)(MIX_RATE-0x3D70)/2.62144; //(0.001/25*2^20/16)
        FreqOffset = FreqOffset+IFkHZ_diff;
        #endif
    }
...
static MS_BOOL _HAL_EXTERN_ATSC_DoIQSwap(MS_BOOL bIsQPad)
{
    ...
    pRes->sDMD_ATSC_PriData.bIsQPad = bIsQPad;
    
    //Set I&Q pad
    if (pRes->sDMD_ATSC_PriData.bIsQPad)
    {
...
MS_BOOL HAL_EXTERN_ATSC_IOCTL_CMD(DMD_ATSC_HAL_COMMAND eCmd, void *pArgs)
{
    ...
    case DMD_ATSC_HAL_CMD_GetPostViterbiBer:
        #ifdef UTPA2
        bResult = _HAL_EXTERN_ATSC_ReadBER(&((*((DMD_ATSC_BER_DATA *)pArgs)).BitErr), &((*((DMD_ATSC_BER_DATA *)pArgs)).Error_window), &((*((DMD_ATSC_BER_DATA *)pArgs)).Win_unit));
        #else
    ...
    case DMD_ATSC_HAL_CMD_ReadFrequencyOffset:
        #ifdef UTPA2
        bResult = _HAL_EXTERN_ATSC_ReadFrequencyOffset(&((*((DMD_ATSC_CFO_DATA *)pArgs)).Mode), &((*((DMD_ATSC_CFO_DATA *)pArgs)).FF), &((*((DMD_ATSC_CFO_DATA *)pArgs)).Rate));
        #else
