...
#define DMD_MSB124X_MAX_DEMOD_NUM          1

// share memory setting
typedef enum _MSB124X_POOL_ID
{
    MSB124X_POOL_ID_DMD0 = 0
} MSB124X_POOL_ID;

typedef struct DLL_PACKED
{
    //DMD_MSB124X_SHARE_MEMORY_InitData  sDMD_MSB124X_SHARE_MEMORY_InitData;
	  //DMD_MSB124X_SHARE_MEMORY_PreviousData sDMD_MSB124X_SHARE_MEMORY_PreviousData;
	  //DMD_MSB124X_InitData_Transform sDMD_MSB124X_InitData_Transform;
} DMD_MSB124X_ResData;

DMD_MSB124X_ResData *psDMD_MSB124X_ResData;	// pointer to share memory

typedef struct DLL_PACKED _MSB124X_RESOURCE_PRIVATE
{
    DMD_MSB124X_ResData sDMD_MSB124X_ResData[DMD_MSB124X_MAX_DEMOD_NUM];
} MSB124X_RESOURCE_PRIVATE, *PMSB124X_RESOURCE_PRIVATE;
#endif
//-------------------------------------------------------------------------------------------------
...
// set frequence 180MHz
    ...
    //delay 1
    MsOS_DelayTask(1); //MsOS_DelayTaskUs(100);
...
// $display("--------Initial DRAM start here!!!");
    ...
    while (u8_tmp != 0x80)
    {
        ...
        // 10us delay
        MsOS_DelayTask(1); //MsOS_DelayTaskUs(10);
        bRet &= _MDrv_DMD_MSB124X_GetReg(0x1201, &u8_tmp);
    }
...
/*
    ...
    do
    {
        ...
        MsOS_DelayTask(1); //MsOS_DelayTaskUs(1*1000);
...
//    MAPI_U8    data[5] = {0x53, 0x45, 0x52, 0x44, 0x42};
    ...
    while( (u8Data != 0x99) && (u8Data != 0xaa) && (u8_timeout-->0))
    {
        ...
        bRet &= (pDemod->_sDMD_MSB124X_InitData.fpMSB124X_I2C_Access)(E_DMD_MSB124X_DEMOD_I2C_DYNAMIC_SLAVE_ID_1, E_DMD_MSB124X_DEMOD_I2C_WRITE_BYTES, 0, NULL, 1, u8MsbData);
        // 10ms
        MsOS_DelayTask(10); //MsOS_DelayTaskUs(1000*10);
    }
    // end read register
    *ptimeout = 0;
    if (u8_timeout == 0x00)
...
static MS_BOOL _IspProcFlash(MS_U8* pLibArry, MS_U32 dwLibSize)
{
    ...
    for(dwLoop=dwStartAddr; (dwLoop < dwEndAddr); dwLoop+=PAGE_WRITE_SIZE)
    {
        ...
        while(dwTimeOut--)
        {
            ...
            MsOS_DelayTask(0); //MsOS_DelayTaskUs(0);
        }
            ...
            do
            {
                ...
                if(status == FALSE)
                {
                    if (pDemod->_sDMD_MSB124X_InitData.fpGPIOReset != NULL)
                    {
                        ...
                        MsOS_DelayTask(resetDemodTime); //MsOS_DelayTaskUs(resetDemodTime*1000);
                        (pDemod->_sDMD_MSB124X_InitData.fpGPIOReset)(TRUE); // gptr->SetOn();
                        MsOS_DelayTask(waitFlashTime); //MsOS_DelayTaskUs(waitFlashTime * 1000);
                    }
                    ...
                    {
                        // 10 ms
                        MsOS_DelayTask(10); //MsOS_DelayTaskUs(10*1000);
                        u16_counter--;
                        _MDrv_DMD_MSB124X_GetReg(0x0900+(0x4f)*2, &u8DoneFlag);
...

                {
                    ...
                    if (pDemod->_sDMD_MSB124X_InitData.bFlashWPEnable)
                    {
                        if (bMatch == FALSE)
                        {
                            ...
                            MsOS_DelayTask(100); //MsOS_DelayTaskUs(100*1000);
                        }
                    }
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
MS_BOOL MDrv_DMD_MSB124X_DVBC_GetDEMODMdbInfo(MS_U64 *u64ReqHdl)
{   // Arthur add
    // define the variable and struct here
    MS_BOOL status = true;
    MS_BOOL bRet = true;
    MS_U8 QAM_Mode, QAM_default, reg_frz;
    MS_U8 u8_tmp0 = 0, u8_tmp1 = 0, u8_tmp2 = 0, u8_tmp3 = 0;
    MS_U16 SymbolRate = 0, SR_default = 0, IF_defualt = 0;  
    MS_U16 BitErrPeriod = 0, BitErr = 0, PktErr = 0;
    MS_U32 reg_intp_ratem1, noise_power, f_Fc,FreqOff, Fc_over_Fs_reg, u32_tmp, log_ber = 0;
    float SQI = 0, log_ber_f = 0, fber = 0, f_snr = (float)0.0;
    float f_symbol_rate,Intp_ratem1, sfo_ofs;
        
	  MdbPrint(u64ReqHdl,"---------MStar DEMOD Info---------\n");
	  // Printf the Info
    MdbPrint(u64ReqHdl,"Spec           : DVBC\n");
    // lock   
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x11E0, &u8_tmp0);    
    //MdbPrint(u64ReqHdl,"0x11E0 u8_tmp = 0x%x\n",u8_tmp0);         
    if(u8_tmp0==0x0C)
    MdbPrint(u64ReqHdl,"Lock Status    : Lock\n");
    else
    MdbPrint(u64ReqHdl,"Lock Status    : Unlock\n");
    // fw version
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_FW_VER_0, &u8_tmp0);
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_FW_VER_1, &u8_tmp1);   
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_FW_VER_2, &u8_tmp2);
    MdbPrint(u64ReqHdl,"Version Number : 0x%x, 0x0%x, 0x0%x\n",u8_tmp0, u8_tmp1,u8_tmp2);
    // QAM
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2302, &u8_tmp0);
    QAM_Mode = u8_tmp0&0x07;            
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_opmode_auto_scan_qam, &u8_tmp0);
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_config_qam, &QAM_default);
    
    MdbPrint(u64ReqHdl,"QAM Mode       : %d-QAM        Default: %d-QAM\n",(16<<QAM_Mode), (16<<QAM_default));  //(0,1,2,3,4) = (16,32,64,128,256)
    if(0!=u8_tmp0)
    MdbPrint(u64ReqHdl,"Auto QAM Detect: Enable        Detect : %d-QAM\n",(16<<QAM_Mode));
    else
    MdbPrint(u64ReqHdl,"Auto QAM Detect: Disable       Default : %d-QAM\n",(16<<QAM_default));   	
    
    // SR
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2C00+0x27*2+0, &u8_tmp0); // read SR
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2C00+0x27*2+1, &u8_tmp1);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2C00+0x27*2+2, &u8_tmp2);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2C00+0x27*2+3, &u8_tmp3);
    reg_intp_ratem1= ((MS_U32)(u8_tmp0))+(((MS_U32)(u8_tmp1))<<8)+(((MS_U32)(u8_tmp2))<<16)+(((MS_U32)(u8_tmp3))<<24);
    Intp_ratem1=(float)reg_intp_ratem1/(float)(1<<22);    
    f_symbol_rate=48000/(2.0*(Intp_ratem1+1.0));
    SymbolRate=(MS_U16) f_symbol_rate;  

    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_config_bw_l, &u8_tmp0);
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_config_bw_h, &u8_tmp1);
    SR_default = (((MS_U16)(u8_tmp1))<<8) | u8_tmp0;
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_opmode_auto_scan_sym_rate, &u8_tmp0);
    
    MdbPrint(u64ReqHdl,"Symbol Rate    : %d kS/s       Default: %d kS/s\n",SymbolRate,SR_default);    
    
    if(0!=u8_tmp0)
    MdbPrint(u64ReqHdl,"Auto SR Detect : Enable        Detect : %d kS/s\n",SymbolRate); 
    else
    MdbPrint(u64ReqHdl,"Auto SR Detect : Disable       Default : %d kS/s\n",SR_default); 
    
    // IF
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_config_fc_l, &u8_tmp0);
    bRet = _MDrv_DMD_MSB124X_GetDSPReg(C_config_fc_h, &u8_tmp1);
    IF_defualt = (((MS_U16)(u8_tmp1))<<8) | u8_tmp0;   
    MdbPrint(u64ReqHdl,"IF             : %d kHz\n",IF_defualt);
      
    // SNR  
    bRet = _MDrv_DMD_MSB124X_SetReg(0x2300+0x3a, 0x20);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x2200+0x05, 0x80);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2300+0x44, &u8_tmp0);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x2300+0x45, &u8_tmp1);
    noise_power = (((MS_U16)u8_tmp1)<<8) | u8_tmp0;
       
    if (noise_power==0)//protect value 0
        noise_power=1;

    bRet = _MDrv_DMD_MSB124X_SetReg(0x2300+0x3a, 0x00);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x2200+0x05, 0x00);
    ...
    // BER
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x1132, &reg_frz);
         
    bRet = _MDrv_DMD_MSB124X_SetReg(0x1132, reg_frz| 0x80);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x1131, &u8_tmp0);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x1130, &u8_tmp1);
    BitErrPeriod = (((MS_U16)u8_tmp0) << 8) | u8_tmp1;
        
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113D, &u8_tmp0);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113C, &u8_tmp1); 
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113B, &u8_tmp2);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113A, &u8_tmp3);
    BitErr = ((MS_U32)(u8_tmp3))+(((MS_U32)(u8_tmp2))<<8)+(((MS_U32)(u8_tmp1))<<16)+(((MS_U32)(u8_tmp0))<<24);
         
    bRet = _MDrv_DMD_MSB124X_SetReg(0x1132, reg_frz);                 

        if (BitErrPeriod == 0)
            BitErrPeriod = 1;//protect 0
        if (BitErr <= 0)
    ...
    // SQI
       log_ber_f = log10(fber);

        if (QAM_Mode == 0)//DMD_DVBC_QAM16
        {
            if(log_ber_f  <= (-5.5f))
                SQI = 100;
            else if(log_ber_f  <= (-5.1f))
                SQI = (90.0f + ((-5.1f)-log_ber_f)*10.0f/((-5.1f)-(-5.5f)));
            else if(log_ber_f  <= (-4.9f))
                SQI = (80.0f + ((-4.9f)-log_ber_f)*10.0f/((-4.9f)-(-5.1f)));
            else if(log_ber_f  <= (-4.5f))
                SQI = (70.0f + ((-4.5f)-log_ber_f)*10.0f/((-4.5f)-(-4.9f)));
            else if(log_ber_f  <= (-3.7f))
                SQI = (60.0f + ((-3.7f)-log_ber_f)*10.0f/((-3.7f)-(-4.5f)));
            else if(log_ber_f  <= (-3.2f))
                SQI = (50.0f + ((-3.2f)-log_ber_f)*10.0f/((-3.2f)-(-3.7f)));
            else if(log_ber_f  <= (-2.9f))
                SQI = (40.0f + ((-2.9f)-log_ber_f)*10.0f/((-2.9f)-(-3.2f)));
            else if(log_ber_f  <= (-2.5f))
                SQI = (30.0f + ((-2.5f)-log_ber_f)*10.0f/((-2.5f)-(-2.9f)));
            else if(log_ber_f  <= (-2.2f))
                SQI = (20.0f + ((-2.2f)-log_ber_f)*10.0f/((-2.2f)-(-2.5f)));
            else if(log_ber_f  <= (-2.0f))
                SQI = (0.0f + ((-2.0f)-log_ber_f)*10.0f/((-2.0f)-(-2.2f)));
            else
                SQI = 0;
        }
        else if (QAM_Mode == 1)//DMD_DVBC_QAM32
        {
            if(log_ber_f  <= (-5.0f))
                SQI = 100;
            else if(log_ber_f  <= (-4.7f))
                SQI = (90.0f  + ((-4.7f)-log_ber_f)*10.0f/((-4.7f)-(-5.0f)));
            else if(log_ber_f  <= (-4.5f))
                SQI = (80.0f  + ((-4.5f)-log_ber_f)*10.0f/((-4.5f)-(-4.7f)));
            else if(log_ber_f  <= (-3.8f))
                SQI = (70.0f  + ((-3.8f)-log_ber_f)*10.0f/((-3.8f)-(-4.5f)));
            else if(log_ber_f  <= (-3.5f))
                SQI = (60.0f  + ((-3.5f)-log_ber_f)*10.0f/((-3.5f)-(-3.8f)));
            else if(log_ber_f  <= (-3.0f))
                SQI = (50.0f  + ((-3.0f)-log_ber_f)*10.0f/((-3.0f)-(-3.5f)));
            else if(log_ber_f  <= (-2.7f))
                SQI = (40.0f  + ((-2.7f)-log_ber_f)*10.0f/((-2.7f)-(-3.0f)));
            else if(log_ber_f  <= (-2.4f))
                SQI = (30.0f  + ((-2.4f)-log_ber_f)*10.0f/((-2.4f)-(-2.7f)));
            else if(log_ber_f  <= (-2.2f))
                SQI = (20.0f  + ((-2.2f)-log_ber_f)*10.0f/((-2.2f)-(-2.4f)));
            else if(log_ber_f  <= (-2.0f))
                SQI = (0.0f  + ((-2.0f)-log_ber_f)*10.0f/((-2.0f)-(-2.2f)));
            else
                SQI = 0;
        ...
        else if (QAM_Mode == 2)//DMD_DVBC_QAM64
        {
            if(log_ber_f  <= (-5.4f))
                ...
                SQI = (90.0f + ((-5.1f)-log_ber_f)*10.0f/((-5.1f)-(-5.4f)));
            else if(log_ber_f  <= (-4.9f))
                SQI = (80.0f + ((-4.9f)-log_ber_f)*10.0f/((-4.9f)-(-5.1f)));
            else if(log_ber_f  <= (-4.3f))
                SQI = (70.0f + ((-4.3f)-log_ber_f)*10.0f/((-4.3f)-(-4.9f)));
            else if(log_ber_f  <= (-3.7f))
                SQI = (60.0f + ((-3.7f)-log_ber_f)*10.0f/((-3.7f)-(-4.3f)));
            else if(log_ber_f  <= (-3.2f))
                SQI = (50.0f + ((-3.2f)-log_ber_f)*10.0f/((-3.2f)-(-3.7f)));
            else if(log_ber_f  <= (-2.9f))
                SQI = (40.0f + ((-2.9f)-log_ber_f)*10.0f/((-2.9f)-(-3.2f)));
            else if(log_ber_f  <= (-2.4f))
                SQI = (30.0f + ((-2.4f)-log_ber_f)*10.0f/((-2.4f)-(-2.9f)));
            else if(log_ber_f  <= (-2.2f))
                SQI = (20.0f + ((-2.2f)-log_ber_f)*10.0f/((-2.2f)-(-2.4f)));
            else if(log_ber_f  <= (-2.05f))
                SQI = (0.0f + ((-2.05f)-log_ber_f)*10.0f/((-2.05f)-(-2.2f)));
            else
                SQI = 0;
        }
        else if (QAM_Mode == 3)//DMD_DVBC_QAM128
        {
            if(log_ber_f  <= (-5.1f))
                ...
                SQI = (90.0f + ((-4.9f)-log_ber_f)*10.0f/((-4.9f)-(-5.1f)));
            else if(log_ber_f  <= (-4.7f))
                SQI = (80.0f + ((-4.7f)-log_ber_f)*10.0f/((-4.7f)-(-4.9f)));
            else if(log_ber_f  <= (-4.1f))
                SQI = (70.0f + ((-4.1f)-log_ber_f)*10.0f/((-4.1f)-(-4.7f)));
            else if(log_ber_f  <= (-3.5f))
                SQI = (60.0f + ((-3.5f)-log_ber_f)*10.0f/((-3.5f)-(-4.1f)));
            else if(log_ber_f  <= (-3.1f))
                SQI = (50.0f + ((-3.1f)-log_ber_f)*10.0f/((-3.1f)-(-3.5f)));
            else if(log_ber_f  <= (-2.7f))
                SQI = (40.0f + ((-2.7f)-log_ber_f)*10.0f/((-2.7f)-(-3.1f)));
            else if(log_ber_f  <= (-2.5f))
                SQI = (30.0f + ((-2.5f)-log_ber_f)*10.0f/((-2.5f)-(-2.7f)));
            else if(log_ber_f  <= (-2.06f))
                SQI = (20.0f + ((-2.06f)-log_ber_f)*10.0f/((-2.06f)-(-2.5f)));
            else
            {
                if (f_snr >= 27.2f)
                    SQI = 20;
        ...
        else //256QAM
        {
            if(log_ber_f  <= (-4.8f))
                SQI= 100;
            else if(log_ber_f  <= (-4.6f))
                SQI = (90.0f + ((-4.6f)-log_ber_f)*10.0f/((-4.6f)-(-4.8f)));
            else if(log_ber_f  <= (-4.4f))
                SQI = (80.0f + ((-4.4f)-log_ber_f)*10.0f/((-4.4f)-(-4.6f)));
            else if(log_ber_f  <= (-4.0f))
                SQI = (70.0f + ((-4.0f)-log_ber_f)*10.0f/((-4.0f)-(-4.4f)));
            else if(log_ber_f  <= (-3.5f))
                SQI = (60.0f + ((-3.5f)-log_ber_f)*10.0f/((-3.5f)-(-4.0f)));
            else if(log_ber_f  <= (-3.1f))
                SQI = (50.0f + ((-3.1f)-log_ber_f)*10.0f/((-3.1f)-(-3.5f)));
            else if(log_ber_f  <= (-2.7f))
                SQI = (40.0f + ((-2.7f)-log_ber_f)*10.0f/((-2.7f)-(-3.1f)));
                ...
                SQI = (20.0f + ((-2.06f)-log_ber_f)*10.0f/((-2.06f)-(-2.4f)));
            else
            {
                if (f_snr >= 29.6f)
                    SQI = 20;
    ...
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x1132, &reg_frz);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x1132, reg_frz| 0x80);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113F, &u8_tmp0);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x113E, &u8_tmp1);
    PktErr = (((MS_U16)u8_tmp0) << 8) | u8_tmp1;
    bRet = _MDrv_DMD_MSB124X_SetReg(0x1132, reg_frz);
    MdbPrint(u64ReqHdl,"Packet Error   : %d\n",PktErr);
    
    // cfo  
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3E54, &u8_tmp0);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3E55, &u8_tmp1); 
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3E56, &u8_tmp2);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3E57, &u8_tmp3);  
    Fc_over_Fs_reg= ((MS_U32)(u8_tmp0))+(((MS_U32)(u8_tmp1))<<8)+(((MS_U32)(u8_tmp2))<<16)+(((MS_U32)(u8_tmp3))<<24); 
         
    f_Fc = (MS_U32)((((float)(Fc_over_Fs_reg))/134217728)*48000);
    FreqOff = f_Fc - IF_defualt;// IF = 5000
    MdbPrint(u64ReqHdl,"CFO            : %d kHz\n",(MS_U32)FreqOff);  
   
    // sfo
    bRet = _MDrv_DMD_MSB124X_SetReg(0x233A, 0x00);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x224A, 0x00);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x2204, 0x80);

    bRet &= _MDrv_DMD_MSB124X_GetReg(0x224C+2, &u8_tmp2);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x224C+1, &u8_tmp1);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x224C, &u8_tmp0);
    u32_tmp= ((MS_U32)(u8_tmp0))+(((MS_U32)(u8_tmp1))<<8)+(((MS_U32)(u8_tmp2))<<16); 
    bRet = _MDrv_DMD_MSB124X_SetReg(0x2204, 0x00);
            
     if (u32_tmp > 0x800000)
          sfo_ofs =-1.0*3.725290e-3*(float)SymbolRate*(float)(0x1000000-u32_tmp)/(float)48000; // ppm
     else
          sfo_ofs =+1.0*3.725290e-3*(float)SymbolRate*(float)(u32_tmp)/(float)48000;  //ppm
                          
    MdbPrint(u64ReqHdl,"SFO            : %d ppm\n",(MS_S32)sfo_ofs);
         
    // gain value
    bRet = _MDrv_DMD_MSB124X_SetReg(0x3e00 + 0x13 * 2, 0x03);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x3e00 + 0x02 * 2, 0x03);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x3e00 + 0x02 * 2 + 1, 0x80);

    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3e00 + 0x14 * 2 + 2, &u8_tmp2);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3e00 + 0x14 * 2 + 1, &u8_tmp1);
    bRet &= _MDrv_DMD_MSB124X_GetReg(0x3e00 + 0x14 * 2 + 0, &u8_tmp0);
    u32_tmp= ((MS_U32)(u8_tmp0))+(((MS_U32)(u8_tmp1))<<8)+(((MS_U32)(u8_tmp2))<<16);

    bRet = _MDrv_DMD_MSB124X_SetReg(0x3e00 + 0x02 * 2, 0x00);
    bRet = _MDrv_DMD_MSB124X_SetReg(0x3e00 + 0x02 * 2 + 1, 0x00);
    
    MdbPrint(u64ReqHdl,"Gain Value     : 0x%x\n",u32_tmp);     
...
MS_BOOL MDrv_DMD_MSB124X_DVBC_Reset(void)
{
	    MS_U8     u8Data = 0x00;
      ...
      MS_BOOL bRet = true;
        ...
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0B00 + (0x19) * 2, 0x01);
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0B00 + (0x10) * 2, 0x01); 
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0C80, 0x00); 
        ...
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0B00 + (0x10) * 2, 0x00);
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0B00 + (0x19) * 2, 0x00);
       
        bRet = _MDrv_DMD_MSB124X_SetReg(0x0B00 + (0x19) * 2, 0x00);
        bRet &= _MDrv_DMD_MSB124X_GetReg(0x0C80, &u8Data);      
        ...
        while (u8Data != 0xff)
        {
            ...
            if (u32Retry++ > 200)
            {
                printf(">>>MSB124X: MSB124X_Reset Fail!\n");
                break;
            }
        }
...
MS_BOOL MDrv_DMD_MSB124X_DVBC_SetMdb(MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
{
	  MS_BOOL bRet = true;
    char* ptr = NULL;
    char strbuf[128] = {0};
    ...
    MdbPrint(pu64ReqHdl,"---------MStar DEMOD Set Command---------\n");
    MdbPrint(pu64ReqHdl,"Input cmd: %s\n",ptr);

    if(strncmp(ptr, "ifvalue=", 8) == 0)  // compare the first 8 digit
    {
        MS_U16 ifvalue_input;
        ...
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_config_fc_l, ifvalue_input&0xFF);     
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_config_fc_h, (ifvalue_input>>8)&0xFF);   
        ...
        MdbPrint(pu64ReqHdl,"Fail to Set IF Value. \n");
         ...
         MDrv_DMD_MSB124X_DVBC_Reset();          
         bRet = _MDrv_DMD_MSB124X_SetReg(0x0CB8, 0x01);  // FSM_EN    
    ...
    else if(strncmp(ptr, "SetQAM=", 7) == 0)  // compare the first 7 digit
    {
        ...
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_config_qam, qam_input );   
    
        if(128 == qam_input)
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_opmode_auto_scan_qam,1);  
        else 
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_opmode_auto_scan_qam,0);   
    ...
    else if(strncmp(ptr, "SetSR=", 6) == 0)  // compare the first 6 digit
    {
        ...
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_config_bw_l, sr_input&0xFF);    
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_config_bw_h, (sr_input>>8)&0xFF); 
   
        if(0==sr_input)
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_opmode_auto_scan_sym_rate,1); 
        else	
        bRet = _MDrv_DMD_MSB124X_SetDSPReg(C_opmode_auto_scan_sym_rate,0); 	
    ...
    {
        MdbPrint(pu64ReqHdl,"----------------- MStar Demod help ----------------\n");
        ...
        MdbPrint(pu64ReqHdl,"Gain Value                      Range from 0x0000 to 0xFFFF. If this is not close to min(0x0000)/max(0xFFFF), it's normal.\n");     
        return true;	  	
  	}
    else
    {
...
MS_U32 EXTDEMOD_DVBC_MdbIoctl(MS_U32 u32Cmd, const void* const pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            MDrv_DMD_MSB124X_DVBC_SetMdb(paraCmdLine->u64ReqHdl, paraCmdLine->u32CmdSize, paraCmdLine->pcCmdLine);  
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MDrv_DMD_MSB124X_DVBC_GetDEMODMdbInfo(paraGetInfo->u64ReqHdl);
...
//    UtopiaModuleCreate(MODULE_DVBT, 8, &pUtopiaModule);   
    DMD_DBG(ULOGD("DEMOD","[drvDMD_EXTERN_MSB124X.c] MSB124XRegisterToUtopia\n"));

    //UtopiaModuleCreate(MODULE_MSB124X, 8, &pUtopiaModule);
    UtopiaModuleCreate(MODULE_DVBC, 8, &pUtopiaModule);
    UtopiaModuleRegister(pUtopiaModule);
    //UtopiaModuleSetupFunctionPtr(pUtopiaModule, (FUtopiaOpen)EXTDVBCOpen, (FUtopiaClose)EXTDVBCClose, (FUtopiaIOctl)EXTDVBCIoctl);

#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
               UtopiaModuleRegisterMdbNode("extdemod_dvbc", (FUtopiaMdbIoctl)EXTDEMOD_DVBC_MdbIoctl);
#endif
    ...
    // start func to add res, call once will create 2 access in resource. Also can declare BDMA_POOL_ID_BDMA1 for another channel depend on driver owner.
	  UtopiaModuleAddResourceStart(pUtopiaModule, MSB124X_POOL_ID_DMD0);
    // resource can alloc private for internal use, ex, BDMA_RESOURCE_PRIVATE
    UtopiaResourceCreate("msb124x0", sizeof(MSB124X_RESOURCE_PRIVATE), &psResource);
    // func to reg res
    UtopiaResourceRegister(pUtopiaModule, psResource, MSB124X_POOL_ID_DMD0);
    // end function to add res
	  UtopiaModuleAddResourceEnd(pUtopiaModule, MSB124X_POOL_ID_DMD0);
}
#endif