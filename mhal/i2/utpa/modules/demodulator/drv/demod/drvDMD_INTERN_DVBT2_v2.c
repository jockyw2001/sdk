...
typedef enum _DVBT2_POOL_ID
{
    DVBT2_POOL_ID_DMD0 = 0
} DVBT2_POOL_ID;

typedef struct DLL_PACKED _DVBT2_RESOURCE_PRIVATE
{
    DMD_DVBT2_ResData sDMD_DVBT2_ResData[DMD_DVBT2_MAX_DEMOD_NUM];
} DVBT2_RESOURCE_PRIVATE, *PDVBT2_RESOURCE_PRIVATE;
// end share memory setting

DMD_DVBT2_ResData *psDMD_DVBT2_ResData;	// pointer to share memory
...
MS_BOOL DMD_DVBT2_Init(DMD_DVBT2_InitData_Transform *pDMD_DVBT2_InitData, MS_U32 u32InitDataLen)
{
    ...
    DMD_DVBT2_ResData *pRes = psDMD_DVBT2_ResData;
	...
	DMD_DBG(printf("[DEMOD][DMD_DVBT2_INIT] _prev_u16PowerState is %d \n", _prev_u16PowerState));

	if(E_POWER_MECHANICAL==_prev_u16PowerState)
	{	
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBT2_InitData_Transform) 1111 enter \n"));
		memcpy(&(pRes->sDMD_DVBT2_InitData_Transform), pDMD_DVBT2_InitData, sizeof(DMD_DVBT2_InitData_Transform));
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBT2_InitData_Transform) 2222 exit \n"));

		DMD_DBG(printf("memcpy(&_sDMD_DVBT2_InitData, pDMD_DVBT2_InitData, u32InitDataLen) 3333 enter \n"));
		memcpy(&_sDMD_DVBT2_InitData, pDMD_DVBT2_InitData, sizeof(_sDMD_DVBT2_InitData));
		DMD_DBG(printf("memcpy(&_sDMD_DVBT2_InitData, pDMD_DVBT2_InitData, u32InitDataLen) 4444 exit \n"));
	}
	else if(E_POWER_RESUME==_prev_u16PowerState)
	{
		DMD_DBG(printf("memcpy E_POWER_RESUME Enter  \n"));
		memcpy(&_sDMD_DVBT2_InitData, &(pRes->sDMD_DVBT2_InitData_Transform), sizeof(DMD_DVBT2_InitData_Transform));
		DMD_DBG(printf("memcpy E_POWER_RESUME Exit  \n"));
	}
	else
	{
		ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c][DMD_DVBT2_Init][check !!!]\n");
	}

	pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat = 1;
	ULOGD("DEMOD","[DMD_DVBT2_Init] b_IsInit_DVBT2Creat = %d;\n", pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat);
...
//        MDrv_SAR_Adc_Config(_sDMD_DVBT2_InitData.u8SarChannel, TRUE);
    ...
    if(0)// (_sDMD_DVBT2_InitData.u8DMD_DVBT2_DSPRegInitExt != NULL)
    {
    ...
    {
	      ULOGD("DEMOD","[DMD_DVBT2_Init]INTERN_DVBT2_Power_On_Initialization Enter \n");
	      INTERN_DVBT2_Power_On_Initialization(bRFAGCTristateEnable, u8ADCIQMode, u8PadSel, bPGAEnable, u8PGAGain,  NULL, 0);
	      ULOGD("DEMOD","[DMD_DVBT2_Init]INTERN_DVBT2_Power_On_Initialization exit \n");		               
    }

    INTERN_DVBT2_Version(&sDMD_DVBT2_Info.u16Version);
    pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.u16Version = sDMD_DVBT2_Info.u16Version;
...
MS_BOOL DMD_DVBT2_Exit(void)
{
    ...
    #endif
    pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat = 0; 
	  ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c] DVBT2Creat = %d\n", pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat);
...
MS_BOOL DMD_DVBT2_SetConfig(DMD_DVBT2_RF_CHANNEL_BANDWIDTH BW, MS_BOOL bSerialTS, MS_U8 u8PlpID)
{
	...
	ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c][_prev_u16PowerState = %d] DMD_DVBT2_SetConfig\n", _prev_u16PowerState);

	if(E_POWER_MECHANICAL==_prev_u16PowerState)
	{
		pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.BW = BW;
		pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.bSerialTS = bSerialTS;
		pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.u8PlpID = u8PlpID;
		
		ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c][E_POWER_MECHANICAL]DMD_DVBT2_SetConfig\n");
		ULOGD("DEMOD","BW = %d, bSerialTS = %d, u8PlpID = %d\n",BW, bSerialTS, u8PlpID);
	}
	else if(E_POWER_RESUME==_prev_u16PowerState)
	{
		BW = pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.BW;
		bSerialTS = pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.bSerialTS;
		u8PlpID = pRes->sDMD_DVBT2_SHARE_MEMORY_PreviousData.u8PlpID;

		ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c][E_POWER_RESUME]DMD_DVBT2_SetConfig\n");
		ULOGD("DEMOD","BW = %d, bSerialTS = %d, u8PlpID = %d\n",BW, bSerialTS, u8PlpID);
	}
	else
	{
			ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c][check!!]DMD_DVBT2_SetConfig\n");
	}
...
MS_U32 DMD_DVBT2_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBT2_v2.c]DMD_DVBT2_SetPowerState\n"));	

	if (u16PowerState == E_POWER_SUSPEND)
	{
		pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT2 = pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat;
		pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.eLastState  = u16PowerState;
		_prev_u16PowerState = u16PowerState;

		ULOGD("DEMOD","[E_POWER_SUSPEND] DVBT2Creat =%d \n",pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat);
		ULOGD("DEMOD","[E_POWER_SUSPEND] b_DCOFF_IsDVBT2 =%d \n",pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT2);
		if(pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_IsInit_DVBT2Creat)
		{			
			DMD_DVBT2_Exit();
		}
		else
  ...
  else if (u16PowerState == E_POWER_RESUME)
  {
            if (_prev_u16PowerState == E_POWER_SUSPEND)
            {
			         _prev_u16PowerState = u16PowerState;
			         pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.eLastState  = u16PowerState;			
			         ULOGD("DEMOD","[E_POWER_RESUME + E_POWER_SUSPEND] b_DCOFF_IsDVBT2 = %d \n",pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT2);
			         if(pRes->sDMD_DVBT2_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT2)
			         {
				           DMD_DVBT2_Init(&_sDMD_DVBT2_InitData, sizeof(_sDMD_DVBT2_InitData));
				           DMD_DVBT2_SetConfig(0, 0, 0);  // use the saving value
				           DMD_DVBT2_SetActive(1);	
...
MS_U32 DVBT2Ioctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    void* pResource = NULL;
    DVBT2_RESOURCE_PRIVATE* psDVBT2ResPri = NULL;
    ...
    if (UtopiaResourceObtain(pModule, DVBT2_POOL_ID_DMD0, &pResource) != 0)
    {
        DMD_DBG(ULOGD("DEMOD","UtopiaResourceObtainToInstant fail\n"));
	    return UTOPIA_STATUS_ERR_RESOURCE;
    }

    if(UtopiaResourceGetPrivate(pResource,(void*)&psDVBT2ResPri) != 0)
    {
        printf("UtopiaResourceGetPrivate fail\n");
        return UTOPIA_STATUS_FAIL;
    }

	psDMD_DVBT2_ResData = ((PDVBT2_RESOURCE_PRIVATE)psDVBT2ResPri)->sDMD_DVBT2_ResData;
...
MS_U32 DVBT2Str(MS_U32 u32PowerState, void* pModule)
{
    ...
    //printf("[YPDEBUG][drvDMD_INTERN_DVBT2_v2.c] DVBT2Str\n");
	  ...
	  if (UtopiaResourceObtain(pModule, DVBT2_POOL_ID_DMD0, &pResource) != 0)
    {
        printf("DVBT2 - UtopiaResourceObtainToInstant fail\n");
	      return UTOPIA_STATUS_ERR_RESOURCE;
    }

    if(UtopiaResourceGetPrivate(pResource,(void*)&psDVBT2ResPri) != 0)
    {
        printf("DVBT2 - UtopiaResourceGetPrivate fail\n");
        return UTOPIA_STATUS_FAIL;
    }
...
printf("[YPDEBUG][drvDMD_INTERN_DVBT2_v2.c] DVBT2RegisterToUtopia\n");
   // Utopia2K STR
	...
	// 20170511 arthur. add share memory to control DC ON/OFF variable
    ...
    // start func to add res, call once will create 2 access in resource. Also can declare BDMA_POOL_ID_BDMA1 for another channel depend on driver owner.
	  UtopiaModuleAddResourceStart(pUtopiaModule, DVBT2_POOL_ID_DMD0);
    // resource can alloc private for internal use, ex, BDMA_RESOURCE_PRIVATE
    UtopiaResourceCreate("dvbt20", sizeof(DVBT2_RESOURCE_PRIVATE), &psResource);
    // func to reg res
    UtopiaResourceRegister(pUtopiaModule, psResource, DVBT2_POOL_ID_DMD0);
    // end function to add res
	  UtopiaModuleAddResourceEnd(pUtopiaModule, DVBT2_POOL_ID_DMD0);
