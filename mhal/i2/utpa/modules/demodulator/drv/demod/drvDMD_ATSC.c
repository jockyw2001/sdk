...
//-------------------------------------------------------------------------------------------------
 ...
 #define DMD_LOCK()      \
     do {                         \
        ...
        if (_u8DMD_ATSC_DbgLevel == DMD_ATSC_DBGLV_DEBUG) PRINT("%s lock mutex\n", __FUNCTION__);\
 ...
 #define DMD_UNLOCK()      \
     do {                         \
        ...
        if (_u8DMD_ATSC_DbgLevel == DMD_ATSC_DBGLV_DEBUG) PRINT("%s unlock mutex\n", __FUNCTION__); \
        } while(0)
...
static DMD_ATSC_LOCK_STATUS _VSB_CheckLock(void)
{
    ...
    else //FEC UNLOCK
    {
        ...
        else //Step 1,2
        {
            ...
            {
                if (!ioctl(DMD_ATSC_HAL_CMD_AGCLock, NULL)) 
                    timeout = pInit->u16VSBAGCLockCheckTime;
                else timeout = pInit->u16VSBPreLockCheckTime;
                
                #if DMD_ATSC_UTOPIA_EN || DMD_ATSC_UTOPIA2_EN
                if ((MsOS_GetSystemTime()-pInfo->u32ATSCScanTimeStart) < timeout)
                #else
                if ((pInit->GetSystemTimeMS()-pInfo->u32ATSCScanTimeStart) < timeout)
                #endif
                {
                    return DMD_ATSC_CHECKING;
                }
...
static DMD_ATSC_LOCK_STATUS _QAM_CheckLock(void)
{
    ...
    else
    {
        ...
        else //STEP 1,2
        {
            ...
            {
                if (!ioctl(DMD_ATSC_HAL_CMD_AGCLock, NULL)) 
                    timeout = pInit->u16QAMAGCLockCheckTime;
                else timeout = pInit->u16QAMPreLockCheckTime;   
...
#endif
{
    ...
    if (!(pRes->sDMD_ATSC_PriData.bInit))
    {
        ...
        {
            DMD_DBG(PRINT("MDrv_DMD_ATSC_Init input data structure incorrect\n"));
            return FALSE;
        }
        #else
...
#endif
{
    ...
    if (pRes->sDMD_ATSC_PriData.bInit == FALSE) return bRet;
...
#endif
{
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        ...
        {
            PRINT("[%s,%5d]It is not suspended yet. We shouldn't resume\n",__FUNCTION__,__LINE__); 
