...
char* _MDrv_DMD_Str_Tok(char* pstrSrc, char* pstrDes, char delimit)
{
...
int _MDrv_DMD_StrToInt(char* pstrSrc)
{
    int iRes = 0;
    char *pstrRes = pstrSrc;
...
#ifdef UTPA2
MS_BOOL _MDrv_DMD_DTMB_DBG_GetModuleInfo(MS_U8 id, MS_U64* pu64ReqHdl)
#else
MS_BOOL MDrv_DMD_DTMB_DBG_GetModuleInfo(MS_U8 id, MS_U64* pu64ReqHdl)
#endif
{
    DMD_DTMB_MODULATION_INFO sDtmbMdbModulationInfo = {{0}};
    DMD_DTMB_InitData sDtmbMdbInitData = {0};
    //DMD_DTMB_CFO_DATA sDtmbMdbCfo = {0};
    MS_U8 u8LockStatus = 0, u8data1 = 0, u8data2 = 0, u8data3 = 0, u8LdpcErr1 = 0, u8LdpcErr2 = 0, u8PNmode = 0, u8PNphase = 0, u8Cmode = 0, u8TDItemp = 0, u8IQSwap = 0;
    MS_U16 u16LdpcErr3 = 0;
    MS_U32 u32TDIStartAdd = 0;
    #ifdef UTPA2
    //DMD_DTMB_CFO_DATA sDtmbMdbCfo = {0};
    #else
    MS_S16 s16CFO = 0;
    ...
    #ifdef UTPA2
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x20C1, &u8LockStatus);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x20C4, &u8data1);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x20C5, &u8data2);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x20C6, &u8data3);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x1190, &u8Cmode);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x3332, &u8LdpcErr1);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x3333, &u8LdpcErr2);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x11BB, &u8PNmode);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x11BA, &u8PNphase);
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x16A3, &u8TDItemp);
    u32TDIStartAdd=u8TDItemp;
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x16A2, &u8TDItemp);
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x16A1, &u8TDItemp);
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x16A0, &u8TDItemp);
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= _MDrv_DMD_DTMB_MD_GetReg(0, 0x285E, &u8IQSwap);
    #else
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x20C1, &u8LockStat
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x20C4, &u8data1);
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x20C5, &u8data2);
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x20C6, &u8data3);
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x1190, &u8Cmode);
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x3332, &u8LdpcErr1
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x3333, &u8LdpcErr2
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x11BB, &u8PNmode);
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x11BA, &u8PNphase)
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x16A3, &u8TDItemp)
    u32TDIStartAdd=u8TDItemp;
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x16A2, &u8TDItemp)
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x16A1, &u8TDItemp)
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x16A0, &u8TDItemp)
    u32TDIStartAdd=u32TDIStartAdd<<8|u8TDItemp;
    bRet &= MDrv_DMD_DTMB_MD_GetReg(0, 0x285E, &u8IQSwap);
    #endif
    ...
    #ifdef UTPA2
    bRet &= _MDrv_DMD_DTMB_MD_GetModulationMode(0, &sDtmbMdbModulationInfo);
    #else
    bRet &= MDrv_DMD_DTMB_MD_GetModulationMode(0, &sDtmbMdbModulationInfo);
    #endif

    #ifdef UTPA2
    if( _MDrv_DMD_DTMB_MD_GetConfig(0, &sDtmbMdbInitData) != UTOPIA_STATUS_SUCCESS)
    #else
    if( MDrv_DMD_DTMB_MD_GetConfig(0, &sDtmbMdbInitData) != UTOPIA_STATUS_SUCCESS)
    #endif
        bRet = false;
    else
        bRet &= true;
    ...
    #ifdef UTPA2
    ;//bRet &= _MDrv_DMD_DTMB_MD_ReadFrequencyOffset(0, &sDtmbMdbCfo);
    #else
    bRet &= MDrv_DMD_DTMB_MD_ReadFrequencyOffset(0, &s16CFO);
    #endif
    ...
    MdbPrint(pu64ReqHdl,"Version Number: %x.%x.%x\n", u8data1, u8data2, u8data3);
    ...
    MdbPrint(pu64ReqHdl,"QAM Mode: %d\n", sDtmbMdbModulationInfo.u8SiQamMode);
    ...
    MdbPrint(pu64ReqHdl,"Interleaving Mode: %d\n", sDtmbMdbModulationInfo.u8SiInterLeaver);
    MdbPrint(pu64ReqHdl,"IF: %d", sDtmbMdbInitData.u16IF_KHZ);
    MdbPrint(pu64ReqHdl,"TDI Address: %X", u32TDIStartAdd);
    #ifdef UTPA2
    MdbPrint(pu64ReqHdl,"SNR: %d%", (_MDrv_DMD_DTMB_MD_GetSignalQuality(0)*3)/10);
    #else
    MdbPrint(pu64ReqHdl,"SNR: %d%", (MDrv_DMD_DTMB_MD_GetSignalQuality(0)*3)/10);
    #endif
    MdbPrint(pu64ReqHdl,"LDPC error count: %d", u16LdpcErr3);
    #ifdef UTPA2
    #else//#ifdef UTPA2
    MdbPrint(pu64ReqHdl,"CFO: %d", s16CFO);
    #endif//#ifdef UTPA2
    MdbPrint(pu64ReqHdl,"IQ SWAP: %d", u8IQSwap&0x01);
    //MdbPrint(pu64ReqHdl,"SFO: ");
...
#ifdef UTPA2
MS_BOOL _MDrv_DMD_DTMB_DBG_EchoCmd(MS_U8 id, MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
#else
MS_BOOL MDrv_DMD_DTMB_DBG_EchoCmd(MS_U8 id, MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
#endif
{
    char* ptr = NULL;
    char strbuf[128] = {0};
    ...
    if (strncmp(pcmd, "TDIAdd=0x", 9) == 0)
    {
        MS_U32 u32TDIaddress;
        ptr = pcmd + 9;
        ptr = _MDrv_DMD_Str_Tok(ptr, strbuf, ';');
        u32TDIaddress = _MDrv_DMD_StrToInt(strbuf);
        #ifdef UTPA2
        bRet &= _MDrv_DMD_DTMB_MD_SetReg(0, 0x16A0, u32TDIaddress&&0xFF);
        bRet &= _MDrv_DMD_DTMB_MD_SetReg(0, 0x16A1, u32TDIaddress>>8&&0xFF);
        bRet &= _MDrv_DMD_DTMB_MD_SetReg(0, 0x16A2, u32TDIaddress>>16&&0xFF);
        bRet &= _MDrv_DMD_DTMB_MD_SetReg(0, 0x16A3, u32TDIaddress>>24&&0xFF);
        #else
        bRet &= MDrv_DMD_DTMB_MD_SetReg(0, 0x16A0, u32TDIaddress&&0xFF);
        bRet &= MDrv_DMD_DTMB_MD_SetReg(0, 0x16A1, u32TDIaddress>>8&&0xFF);
        bRet &= MDrv_DMD_DTMB_MD_SetReg(0, 0x16A2, u32TDIaddress>>16&&0xFF);
        bRet &= MDrv_DMD_DTMB_MD_SetReg(0, 0x16A3, u32TDIaddress>>24&&0xFF);
        #endif
    }
    else if (strncmp(pcmd, "IQSWAP=", 7) == 0)
    {
        MS_U8 u8IQSwap;
        ptr = pcmd + 7;
        ptr = _MDrv_DMD_Str_Tok(ptr, strbuf, ';');
        ...
        #ifdef UTPA2
        bRet &= _MDrv_DMD_DTMB_MD_SetReg(0, 0x285E, u8IQSwap&&0x01);
        #else
        bRet &= MDrv_DMD_DTMB_MD_SetReg(0, 0x285E, u8IQSwap&&0x01);
        #endif
    }
    else if(strncmp(pcmd, "help", 4) == 0)
    {
        MdbPrint(pu64ReqHdl,"----------MStar help----------\n");
        MdbPrint(pu64ReqHdl,"1.  Get DEMOD DTMB Infomation\n");
...
MS_BOOL MDrv_DMD_DTMB_DBG_GetModuleInfo(MS_U8 id, MS_U64* pu64ReqHdl)
{
    DTMB_MDB_PARAM MdbParam = {0};

    if (!_u32DTMBopen) return FALSE;

    MdbParam.id = id;

    if (UtopiaIoctl(_ppDTMBInstant, DMD_DTMB_DRV_CMD_MD_DBG_GetModuleInfo, &MdbParam) == UTOPIA_STATUS_SUCCESS)
    {
        *pu64ReqHdl = MdbParam.u64Data;
...
MS_BOOL MDrv_DMD_DTMB_DBG_EchoCmd(MS_U8 id, MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
{
    ...
    MdbParam.id = id;
    MdbParam.u32Data = u32CmdSize;

    if (UtopiaIoctl(_ppDTMBInstant, DMD_DTMB_DRV_CMD_MD_DBG_EchoCmd, &MdbParam) == UTOPIA_STATUS_SUCCESS)
    {
        *pu64ReqHdl = MdbParam.u64Data;
        *pcmd = MdbParam.chData;
        return TRUE;
    }
    else
    {
