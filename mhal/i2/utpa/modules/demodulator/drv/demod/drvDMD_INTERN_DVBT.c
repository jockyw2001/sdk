...
memcpy (&(AgcSsi_Para), pDMD_DVBT_InitData, sizeof(DMD_DVBT_InitData));
	...
	Init_Para_Temp.u8DMD_DVBT_DSPRegInitExt=pDMD_DVBT_InitData->u8DMD_DVBT_DSPRegInitExt;
	 ...
	 Init_Para_Temp.u32EqStartAddr=pDMD_DVBT_InitData->u32EqStartAddr;
     Init_Para_Temp.u32TdiStartAddr=pDMD_DVBT_InitData->u32TdiStartAddr;
     Init_Para_Temp.u32DjbStartAddr=pDMD_DVBT_InitData->u32DjbStartAddr;
     Init_Para_Temp.u32FwStartAddr=pDMD_DVBT_InitData->u32FwStartAddr;
...
MS_BOOL	MDrv_DMD_DVBT_GetSignalStrengthWithRFPower(MS_U16	*u16Strength,	float	fRFPowerDbm)
{
	...
	if(u32DVBTopen==1)
	{
		...
		else
		{
				...
				if(MDrv_DMD_DVBT_GetTPSInfo(&u16_tmp) ==	TRUE)
						tps_info_qam	=	(MS_U8)u16_tmp&0x07;  // bit0~bit2
				else
				printf("[dvbt]TPS constellation parameter	retrieve failure\n");		

				if(MDrv_DMD_DVBT_GetTPSInfo(&u16_tmp)	== TRUE)
						tps_info_cr = (MS_U8)((u16_tmp>>9)&0x07);// bit9 ~bit11
				else
				printf("[dvbt]TPS CR parameter	retrieve failure\n");

    DMD_DBG(ULOGD("DEMOD","tps_info_qam = %d\n", tps_info_qam));
		DMD_DBG(ULOGD("DEMOD","tps_info_cr  = %d\n",tps_info_cr));
...
MS_BOOL	MDrv_DMD_DVBT_GetSignalQualityWithRFPower(MS_U16 *u16Quality,	float	fRFPowerDbm)
{
			...
			if ( MsOS_Timer_DiffTimeFromNow(u32FecFirstLockTime) < 300)
	{
		MsOS_DelayTask(300 - MsOS_Timer_DiffTimeFromNow(u32FecFirstLockTime));
				...
				if(MDrv_DMD_DVBT_GetTPSInfo(&u16_tmp) ==	TRUE)
						tps_cnstl	=	(MS_U8)u16_tmp&0x07;  // bit0~bit2
				else
				printf("[dvbt]TPS constellation parameter	retrieve failure\n");		
						
				if(MDrv_DMD_DVBT_GetTPSInfo(&u16_tmp)	== TRUE)
						tps_cr = (MS_U8)((u16_tmp>>9)&0x07);// bit9 ~bit11
				else
				printf("[dvbt]TPS CR parameter	retrieve failure\n");					
						
    DMD_DBG(ULOGD("DEMOD","tps_cnstl = %d\n", tps_cnstl));
		DMD_DBG(ULOGD("DEMOD","tps_cr    = %d\n", tps_cr));
