...
static MS_U8 pre_TS_CLK_DIV=0;
static MS_U8 cur_TS_CLK_DIV=0;
//-------------------------------------------------------------------------------------------------
//  Local Functions
//-------------------------------------------------------------------------------------------------
        ...
        //pilot_flag = *Drv_DVBS_GetTsDivNum_PARAM.pilot_flag;
         ...
         if (u8Data > 0x1F) 
             u8Data = 0x1F;
                    ...
                    u8Data=HAL_DMD_RIU_ReadByte(0x103301); // read current TS clock source setting
                    #ifdef MS_DEBUG
                    ULOGD("DEMOD", "cur_TS_CLK_DIV=0x%x\n", cur_TS_CLK_DIV);// print current TS clock div
                    ULOGD("DEMOD", "pre_TS_CLK_DIV=0x%x\n", pre_TS_CLK_DIV);// print previous TS clock div
                    #endif
                    if(pre_TS_CLK_DIV !=cur_TS_CLK_DIV)
                    {
                        u8Data=HAL_DMD_RIU_ReadByte(0x103302);
                        HAL_DMD_RIU_WriteByte(0x103302, u8Data | 0x01);//disbale TS module

                            phase_turning_num=cur_TS_CLK_DIV>>1;//calculate TS phase tuning number for 90 and 270 degree
                            HAL_DMD_RIU_WriteByte(0x10330b, phase_turning_num); // set phase tuning number = 3
                            ...
                            HAL_DMD_RIU_WriteByte(0x103300, cur_TS_CLK_DIV);//set TS div

                        u8Data=HAL_DMD_RIU_ReadByte(0x103300); // read current TS clock div
                        ...
                        #endif
                        u8Data=HAL_DMD_RIU_ReadByte(0x103302);
                        HAL_DMD_RIU_WriteByte(0x103302, u8Data & ~0x01);//enable TS module
                    }
             HAL_DMD_RIU_WriteByte(0x103300, cur_TS_CLK_DIV);
              //HAL_DMD_RIU_WriteByte(0x103301, 0x11);          
             //Ts Output Enable
             TS_Enable=TRUE;
             #ifdef UFO_DEMOD_DVBS_TSCONTROL
                 MDrv_DMD_DVBS_TS_Enable(TS_Enable);
	      #endif	 
   ...
   // if (TRUE == INTERN_DVBS_GetLock(DMD_DVBS_GETLOCK, 200.0f, -200.0f, 0))
    {
        if(_bDemodType)  //S2
        {
           ...
           DVBS_GetBCHEflagWindow_PARAM Drv_DVBS_GetBCHEflagWindow_PARAM;
           Drv_DVBS_GetBCHEflagWindow_PARAM.BCH_Eflag_Window=&BCH_Eflag2_Window;
           Drv_DVBS_GetBCHEflagWindow_PARAM.ret=false;
	   UtopiaIoctl(ppDVBSInstant,DMD_DVBS_DRV_CMD_GetBCHEflagWindow,&Drv_DVBS_GetBCHEflagWindow_PARAM);
	     ...
	     #ifdef MS_DEBUG
                ULOGD("DEMOD","BCH_Eflag_Window = 0x%x\n", BCH_Eflag2_Window);
            #endif
        ...
        else  //S
        {
            ...
            if (status==FALSE)
            {
                ULOGD("DEMOD","M-Series_GetSignalQuality GetPostViterbiBer Fail! \n");
                bRet= FALSE;
            }
...
#ifdef UFO_DEMOD_DVBS_TSCONTROL
MS_BOOL MDrv_DMD_DVBS_TS_Enable(MS_BOOL bEnable)
{
    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBS.c]MDrv_DMD_DVBS2_TS_Enable\n"));  
    DVBS_TS_Enable_PARAM Drv_DVBS_TS_Enable_PARAM;
    
    Drv_DVBS_TS_Enable_PARAM.TS_Enable=bEnable;
    Drv_DVBS_TS_Enable_PARAM.ret = false;

   if(u32DVBSopen==1)
    {
        UtopiaIoctl(ppDVBSInstant, DMD_DVBS_DRV_CMD_TS_Enable, &Drv_DVBS_TS_Enable_PARAM); 
    }    

   return Drv_DVBS_TS_Enable_PARAM.ret;    
}
#endif