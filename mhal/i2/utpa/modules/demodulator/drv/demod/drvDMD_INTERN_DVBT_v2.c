...
typedef enum _DVBT_POOL_ID
{
    DVBT_POOL_ID_DMD0 = 0
} DVBT_POOL_ID;

typedef struct DLL_PACKED _DVBT_RESOURCE_PRIVATE
{
    DMD_DVBT_ResData sDMD_DVBT_ResData[DMD_DVBT_MAX_DEMOD_NUM];
} DVBT_RESOURCE_PRIVATE, *PDVBT_RESOURCE_PRIVATE;
// end share memory setting

DMD_DVBT_ResData *psDMD_DVBT_ResData;	// pointer to share memory
...
// add for T/T2 merged FW
MS_U32  u32DMD_DVBT_EQ_START_ADDR;
MS_U32  u32DMD_DVBT_TDI_START_ADDR;
MS_U32  u32DMD_DVBT_DJB_START_ADDR;
MS_U32  u32DMD_DVBT_FW_START_ADDR;
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
#if(0)
...
MS_BOOL DMD_DVBT_Init(DMD_DVBT_InitData_Transform *pDMD_DVBT_InitData, MS_U32 u32InitDataLen)
{
    ...
    MS_BOOL bRFAGCTristateEnable = 1;
    MS_BOOL bIFAGCTristateEnable = 0;
    DMD_DVBT_ResData *pRes = psDMD_DVBT_ResData;
	...
	DMD_DBG(printf("[DEMOD][DMD_DVBT_INIT] _prev_u16PowerState is %d \n", _prev_u16PowerState));

	if(E_POWER_MECHANICAL==_prev_u16PowerState)
	{		
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBT_InitData_Transform) 1111 enter \n"));
		memcpy(&(pRes->sDMD_DVBT_InitData_Transform), pDMD_DVBT_InitData, sizeof(DMD_DVBT_InitData_Transform));
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBT_InitData_Transform) 2222 exit \n"));

		DMD_DBG(printf("memcpy(&_sDMD_DVBT_InitData, pDMD_DVBT_InitData, u32InitDataLen) 3333 enter \n"));
		memcpy(&_sDMD_DVBT_InitData, pDMD_DVBT_InitData, sizeof(_sDMD_DVBT_InitData));
		DMD_DBG(printf("memcpy(&_sDMD_DVBT_InitData, pDMD_DVBT_InitData, u32InitDataLen) 4444 exit \n"));
	}
	else if(E_POWER_RESUME==_prev_u16PowerState)
	{
		DMD_DBG(printf("memcpy E_POWER_RESUME Enter  \n"));
		memcpy(&_sDMD_DVBT_InitData, &(pRes->sDMD_DVBT_InitData_Transform), sizeof(DMD_DVBT_InitData_Transform));
		DMD_DBG(printf("memcpy E_POWER_RESUME Exit  \n"));
	}
	else
	{
		ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c][DMD_DVBT_Init][check !!!]\n");
	}

	pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat = 1;
	ULOGD("DEMOD","[DMD_DVBT_Init] b_IsInit_DVBTCreat = %d;\n", pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat);
    ...
    // add for T/T2 merged FW
    u32DMD_DVBT_EQ_START_ADDR = _sDMD_DVBT_InitData.u32EqStartAddr;
    u32DMD_DVBT_TDI_START_ADDR = _sDMD_DVBT_InitData.u32TdiStartAddr;
    u32DMD_DVBT_DJB_START_ADDR = _sDMD_DVBT_InitData.u32DjbStartAddr;
    u32DMD_DVBT_FW_START_ADDR = _sDMD_DVBT_InitData.u32FwStartAddr;
	...
	DVBT_TS_PHASE_EN =0;
	DVBT_TS_PHASE_NUM = 0;
   if(0) //(_sDMD_DVBT_InitData.u8DMD_DVBT_InitExt != NULL)
   {
/*
   	if (_sDMD_DVBT_InitData.u8DMD_DVBT_InitExt[0]>=5)	// version bigger than 5, apply TS phase solution
   	{
...
*/
   ...
   else	// if init board define is NULL TS phase needs check.
   {
    ...
    if(0)// (_sDMD_DVBT_InitData.u8DMD_DVBT_DSPRegInitExt != NULL)
    {
        if (_sDMD_DVBT_InitData.u8DMD_DVBT_DSPRegInitExt[0]>=1)
        {
            INTERN_DVBT_Power_On_Initialization(bRFAGCTristateEnable, u8ADCIQMode, u8PadSel, bPGAEnable, u8PGAGain, _sDMD_DVBT_InitData.u8DMD_DVBT_DSPRegInitExt, _sDMD_DVBT_InitData.u8DMD_DVBT_DSPRegInitSize);
    ...
    {
        	ULOGD("DEMOD","[DMD_DVBT_Init]INTERN_DVBT_Power_On_Initialization Enter \n");
          INTERN_DVBT_Power_On_Initialization(bRFAGCTristateEnable, u8ADCIQMode, u8PadSel, bPGAEnable, u8PGAGain,  NULL, 0);
        	ULOGD("DEMOD","[DMD_DVBT_Init]INTERN_DVBT_Power_On_Initialization exit \n");
    }

    INTERN_DVBT_Version(&sDMD_DVBT_Info.u16Version);
    pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u16Version = sDMD_DVBT_Info.u16Version;
    DMD_UNLOCK();
    #ifdef MS_DEBUG
    ULOGD("DEMOD","firmware version: %x\n",sDMD_DVBT_Info.u16Version);
    #endif
...
MS_BOOL DMD_DVBT_Exit(void)
{
    ...
    pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat = 0; 
    ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c] DVBTCreat = %d\n", pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat);
    DMD_LOCK();
...
MS_BOOL DMD_DVBT_SetConfigHPLPSetIF(DMD_RF_CHANNEL_BANDWIDTH BW, MS_BOOL bSerialTS, MS_BOOL bPalBG, MS_BOOL bLPSel, MS_U32 u32IFFreq, MS_U32 u32FSFreq, MS_U8 u8IQSwap)
{
    ...
    DMD_DVBT_ResData *pRes = psDMD_DVBT_ResData;
	  ...
	  ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c][_prev_u16PowerState = %d] DMD_DVBT_SetConfigHPLPSetIF\n", _prev_u16PowerState);


	if(E_POWER_MECHANICAL==_prev_u16PowerState)
	{
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.BW = BW;
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bSerialTS = bSerialTS;
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bPalBG = bPalBG;
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bLPSel = bLPSel;
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u32IFFreq = u32IFFreq;
		pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u32FSFreq = u32FSFreq;
    pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u8IQSwap = u8IQSwap;

		ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c][E_POWER_MECHANICAL] DMD_DVBT_SetConfigHPLPSetIF\n");
		//ULOGD("DEMOD","BW = %d, bSerialTS = %d, bPalBG = %d, bLPSel = %d, u32IFFreq = %ld, u32FSFreq = %ld, u8IQSwap = %d\n", BW, bSerialTS, bPalBG, bLPSel, u32IFFreq, u32FSFreq, u8IQSwap);
	}
	else if(E_POWER_RESUME==_prev_u16PowerState)
	{
		BW = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.BW;
		bSerialTS = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bSerialTS;
		bPalBG = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bPalBG;
		bLPSel = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.bLPSel;
		u32IFFreq = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u32IFFreq;
		u32FSFreq = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u32FSFreq;
		u8IQSwap = pRes->sDMD_DVBT_SHARE_MEMORY_PreviousData.u8IQSwap;		
		
		ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c][E_POWER_RESUME] DMD_DVBT_SetConfigHPLPSetIF\n");
		//ULOGD("DEMOD","BW = %d, bSerialTS = %d, bPalBG = %d, bLPSel = %d, u32IFFreq = %ld, u32FSFreq = %ld, u8IQSwap = %d\n", BW, bSerialTS, bPalBG, bLPSel, u32IFFreq, u32FSFreq, u8IQSwap);
	}
	else
	{
			ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c][check!!] DMD_DVBT_SetConfigHPLPSetIF\n");
	}
...
MS_U32 DMD_DVBT_SetPowerState(EN_POWER_MODE u16PowerState)
{
	...
	{
		pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT = pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat;
		pRes->sDMD_DVBT_SHARE_MEMORY_InitData.eLastState  = u16PowerState;
		_prev_u16PowerState = u16PowerState;

		ULOGD("DEMOD","[E_POWER_SUSPEND] DVBTCreat =%d \n",pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat);
		ULOGD("DEMOD","[E_POWER_SUSPEND] b_DCOFF_IsDVBT =%d \n",pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT);
		if(pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_IsInit_DVBTCreat)
		{			
			DMD_DVBT_Exit();
		}
		else
  ...
  else if (u16PowerState == E_POWER_RESUME)
  {
            if (_prev_u16PowerState == E_POWER_SUSPEND)
            {
			         _prev_u16PowerState = u16PowerState;
			         pRes->sDMD_DVBT_SHARE_MEMORY_InitData.eLastState  = u16PowerState;			
			         ULOGD("DEMOD","[E_POWER_RESUME + E_POWER_SUSPEND] b_DCOFF_IsDVBT = %d \n",pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT);
			         if(pRes->sDMD_DVBT_SHARE_MEMORY_InitData.b_DCOFF_IsDVBT)
			         {
				           DMD_DVBT_Init(&_sDMD_DVBT_InitData, sizeof(_sDMD_DVBT_InitData));
				           DMD_DVBT_SetConfigHPLPSetIF(0, 0, 0, 0, 0, 0, 0);  // use the saving value
				           DMD_DVBT_SetActive(1);	
				           u32Return = UTOPIA_STATUS_SUCCESS;//RESUME_OK;						
			         }
			         else
			         {
			         	    ULOGD("DEMOD","[%s,%5d]Previous state is not SUSPEND. We shouldn't resume\n",__FUNCTION__,__LINE__);
...
MS_U32 DVBTIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    void* pResource = NULL;
    DVBT_RESOURCE_PRIVATE* psDVBTResPri = NULL;
    ...
    if(UtopiaResourceGetPrivate(pResource,(void*)&psDVBTResPri) != 0)
    {
        printf("UtopiaResourceGetPrivate fail\n");
        return UTOPIA_STATUS_FAIL;
    }

	  psDMD_DVBT_ResData = ((PDVBT_RESOURCE_PRIVATE)psDVBTResPri)->sDMD_DVBT_ResData;
...
MS_U32 DVBTStr(MS_U32 u32PowerState, void* pModule)
{
    ...
    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBT_v2.c]DVBTStr\n"));
	  ...
	  if (UtopiaResourceObtain(pModule, DVBT_POOL_ID_DMD0, &pResource) != 0)
    {
        printf("DVBT - UtopiaResourceObtainToInstant fail\n");
	    return UTOPIA_STATUS_ERR_RESOURCE;
    }

    if(UtopiaResourceGetPrivate(pResource,(void*)&psDVBTResPri) != 0)
    {
        printf("DVBT - UtopiaResourceGetPrivate fail\n");
        return UTOPIA_STATUS_FAIL;
    }
...
//    UtopiaModuleCreate(MODULE_DVBT, 8, &pUtopiaModule);   //bryan: why this was taken out
   ...
   // Utopia2K STR
	...
	// 20170509 arthur. add share memory to control DC ON/OFF variable
    ...
    // start func to add res, call once will create 2 access in resource. Also can declare BDMA_POOL_ID_BDMA1 for another channel depend on driver owner.
	  UtopiaModuleAddResourceStart(pUtopiaModule, DVBT_POOL_ID_DMD0);
    // resource can alloc private for internal use, ex, BDMA_RESOURCE_PRIVATE
    UtopiaResourceCreate("dvbt0", sizeof(DVBT_RESOURCE_PRIVATE), &psResource);
    // func to reg res
    UtopiaResourceRegister(pUtopiaModule, psResource, DVBT_POOL_ID_DMD0);
    // end function to add res
	  UtopiaModuleAddResourceEnd(pUtopiaModule, DVBT_POOL_ID_DMD0);
}
