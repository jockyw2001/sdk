...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define DMD_DVBS_MAX_DEMOD_NUM          1

//-------------------------------------------------------------------------------------------------
//  Local Structurs
//-------------------------------------------------------------------------------------------------

typedef enum _DVBS_POOL_ID
{
    DVBS_POOL_ID_DMD0 = 0
} DVBS_POOL_ID;

typedef struct DLL_PACKED _DVBS_RESOURCE_PRIVATE
{
    DMD_DVBS_ResData sDMD_DVBS_ResData[DMD_DVBS_MAX_DEMOD_NUM];
} DVBS_RESOURCE_PRIVATE, *PDVBS_RESOURCE_PRIVATE;

DMD_DVBS_ResData *psDMD_DVBS_ResData;	
...
typedef MS_BOOL (*IOCTL_DVBS_TS_ENABLE)(MS_BOOL bEnable);
#endif
typedef MS_BOOL (*IOCTL_DVBS_GetBCHEflagWindow)(MS_U16 *BCH_Eflag_Window);
...
#endif
#ifdef UFO_DEMOD_DVBS_TSCONTROL
        IOCTL_DVBS_TS_ENABLE  fpDVBS_TS_ENABLE;
#endif
        IOCTL_DVBS_GetBCHEflagWindow fpDVBS_GetBCHEflagWindow;
...
MS_BOOL DMD_DVBS_Init(DMD_DVBS_InitData_Transform *pDMD_DVBS_InitData, MS_U32 u32InitDataLen)
{
    ...
    DMD_DVBS_ResData *pRes = psDMD_DVBS_ResData;	
	...
	DMD_DBG(printf("[DEMOD][DMD_DVBS_INIT] _prev_u16PowerState is %d \n", _prev_u16PowerState));

	if(E_POWER_MECHANICAL==_prev_u16PowerState)
	{
		
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBS_InitData_Transform) 1 enter \n"));
		memcpy(&(pRes->sDMD_DVBS_InitData_Transform), pDMD_DVBS_InitData, sizeof(DMD_DVBS_InitData_Transform));
		DMD_DBG(printf("memcpy(&(pRes->sDMD_DVBS_InitData_Transform) 1 exit \n"));

		DMD_DBG(printf("memcpy(&_sDMD_DVBS_InitData, pDMD_DVBS_InitData, u32InitDataLen) 2 enter \n"));
		memcpy(&_sDMD_DVBS_InitData, pDMD_DVBS_InitData, sizeof(_sDMD_DVBS_InitData));
		DMD_DBG(printf("memcpy(&_sDMD_DVBS_InitData, pDMD_DVBS_InitData, u32InitDataLen) 2 exit \n"));

	}
	else if(E_POWER_RESUME==_prev_u16PowerState)
	{
		DMD_DBG(printf("memcpy E_POWER_RESUME Enter  \n"));
		memcpy(&_sDMD_DVBS_InitData, &(pRes->sDMD_DVBS_InitData_Transform), sizeof(DMD_DVBS_InitData_Transform));
		DMD_DBG(printf("memcpy E_POWER_RESUME Exit  \n"));
	}
	else
	{
		ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c][DMD_DVBS_Init][check !!!]\n");
	}


	pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend = 1;
	ULOGD("DEMOD","[DMD_DVBS_Init] b_IsInit_DVBSSuspend = %d;\n", pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend );
    ...
    pRes->sDMD_DVBS_PreviousData.u16Version = sDMD_DVBS_Info.u16Version;
    DMD_UNLOCK();
    #ifdef MS_DEBUG
...
MS_BOOL DMD_DVBS_Exit(void)
{
    ...
    #endif
    //bIsDVBS = FALSE;
    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c]DMD_DVBS_Exit\n"));
    pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend = 0; 
	ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c] DVBSSuspend = %d\n", pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend);
    DMD_LOCK();
    bRet=INTERN_DVBS_Exit();
    DMD_UNLOCK();
...
MS_BOOL DMD_DVBS_SetConfig_symbol_rate_list(MS_U32 u32SymbolRate, DMD_DVBS_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    if(E_POWER_MECHANICAL==_prev_u16PowerState)
    {
		pRes->sDMD_DVBS_PreviousData.u32SymbolRate = u32SymbolRate;
		pRes->sDMD_DVBS_PreviousData.eQamMode = eQamMode;
		pRes->sDMD_DVBS_PreviousData.u32IFFreq = u32IFFreq;
		pRes->sDMD_DVBS_PreviousData.bSpecInv = bSpecInv;
		pRes->sDMD_DVBS_PreviousData.bSerialTS = bSerialTS;

		ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c][E_POWER_MECHANICAL]DMD_DVBS_SetConfig_symbol_rate_list\n");
		ULOGD("DEMOD","SR = %d, QamMode = %d, IFFreq = %d, SpecInv = %d, SerialTS = %d\n",(int)u32SymbolRate, (int)eQamMode,(int) u32IFFreq, (int)bSpecInv, bSerialTS);
    }
    else if(E_POWER_RESUME==_prev_u16PowerState)
    {
		u32SymbolRate = pRes->sDMD_DVBS_PreviousData.u32SymbolRate;
		eQamMode = pRes->sDMD_DVBS_PreviousData.eQamMode;
		u32IFFreq = pRes->sDMD_DVBS_PreviousData.u32IFFreq;
		bSpecInv = pRes->sDMD_DVBS_PreviousData.bSpecInv;
		bSerialTS = pRes->sDMD_DVBS_PreviousData.bSerialTS;
		ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c][E_POWER_RESUME]DMD_DVBS_SetConfig_symbol_rate_list\n");
		ULOGD("DEMOD","SR = %d, QamMode = %d, IFFreq = %d, SpecInv = %d, SerialTS = %d\n",(int)u32SymbolRate, (int)eQamMode, (int)u32IFFreq, (int)bSpecInv, (int)bSerialTS);
    }
    else
    {
		ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c][check!!]DMD_DVBS_SetConfig_symbol_rate_list\n");
    }
...
MS_BOOL DMD_DVBS_GetLockWithRFPower(DMD_DVBS_GETLOCK_TYPE eType, DMD_DVBS_LOCK_STATUS *eLockStatus, MS_U32 fCurrRFPowerDbm, MS_U32 fNoChannelRFPowerDbm)
{
    ...
    if ( eType == DMD_DVBS_GETLOCK ) // for channel scan
    {
        ...
        {
            if(INTERN_DVBS_GetLock(DMD_DVBS_GETLOCK_TR_EVER_LOCK, 200, -200, 0) == TRUE)
                u32Timeout = 3000;
            else
                u32Timeout = 1500;
            DMD_DBG(printf("MDrv_DMD_DVBS_GetLockWithRFPower u32Timeout=%ld\n",u32Timeout));
...
MS_U32 DMD_DVBS_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c]DMD_DVBS_SetPowerState\n"));	

    if (u16PowerState == E_POWER_SUSPEND)
    {
        pRes->sDMD_DVBS_InitData_STR.b_DCOFF_IsDVBS= pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend;
	 pRes->sDMD_DVBS_InitData_STR.eLastState  = u16PowerState;
	 _prev_u16PowerState = u16PowerState;

	 ULOGD("DEMOD","[E_POWER_SUSPEND] DVBSSuspend =%d \n",pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend);
	 ULOGD("DEMOD","[E_POWER_SUSPEND] b_DCOFF_IsDVBS =%d \n",pRes->sDMD_DVBS_InitData_STR.b_DCOFF_IsDVBS);
	 if(pRes->sDMD_DVBS_InitData_STR.b_IsInit_DVBSSuspend)
	 {			
	     DMD_DVBS_Exit();
	 }
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        if (_prev_u16PowerState == E_POWER_SUSPEND)
        {
	     _prev_u16PowerState = u16PowerState;
	     pRes->sDMD_DVBS_InitData_STR.eLastState  = u16PowerState;			
	     ULOGD("DEMOD","[E_POWER_RESUME + E_POWER_SUSPEND] b_DCOFF_IsDVBS = %d \n",pRes->sDMD_DVBS_InitData_STR.b_DCOFF_IsDVBS);
	     if(pRes->sDMD_DVBS_InitData_STR.b_DCOFF_IsDVBS)
	     {			
	         DMD_DVBS_Init(&_sDMD_DVBS_InitData, sizeof(_sDMD_DVBS_InitData));
		  DMD_DVBS_SetConfig_symbol_rate_list(1,2,3,0,0,NULL,0);
		  DMD_DVBS_SetActive(1);				
	     }
...
#endif 
#ifdef UFO_DEMOD_DVBS_TSCONTROL
MS_BOOL DMD_DVBS_TS_ENABLE(MS_BOOL bEnable)
{

    MS_BOOL bRet;

    DMD_LOCK();
    bRet = INTERN_DVBS_TS_Enable(bEnable);
    DMD_UNLOCK();
...
#endif
MS_BOOL DMD_DVBS_GetBCHEflagWindow(MS_U16 *BCH_Eflag_Window)
{

    MS_BOOL bRet;

    DMD_LOCK();
    bRet = INTERN_DVBS_GetBCHEflagWindow(BCH_Eflag_Window);
    DMD_UNLOCK();
...
#endif
#ifdef UFO_DEMOD_DVBS_TSCONTROL
        pDVBSPri->fpDVBS_TS_ENABLE= DMD_DVBS_TS_ENABLE;
#endif
        pDVBSPri->fpDVBS_GetBCHEflagWindow= DMD_DVBS_GetBCHEflagWindow;
    //return TRUE;
    return UTOPIA_STATUS_SUCCESS;
}
...
MS_U32 DVBSIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    DVBS_RESOURCE_PRIVATE* psDVBSResPri = NULL;
    ...
    if (UtopiaResourceObtain(pModule, DVBS_POOL_ID_DMD0, &pResource) != 0)
    {
        DMD_DBG(ULOGD("DEMOD","UtopiaResourceObtainToInstant fail\n"));
	    return UTOPIA_STATUS_ERR_RESOURCE;
    }
	
    if(UtopiaResourceGetPrivate(pResource,(void*)&psDVBSResPri) != 0)
    {
        printf("UtopiaResourceGetPrivate fail\n");
        return UTOPIA_STATUS_FAIL;
    }

	psDMD_DVBS_ResData = ((PDVBS_RESOURCE_PRIVATE)psDVBSResPri)->sDMD_DVBS_ResData;
#if 0    
	...
	case DMD_DVBS_DRV_CMD_GetCurrentDemodCodeRate: 
       ...
       case DMD_DVBS_DRV_CMD_Set_Default_IS_ID:
        ...
        case DMD_DVBS_DRV_CMD_TS_Enable:
            DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c]DVBSIoctl - MDrv_DMD_DVBS_DRV_CMD_TS_Enable\n"));            
            bRet = psDVBSInstPri->fpDVBS_TS_ENABLE(((PDVBS_TS_Enable_PARAM)pArgs)->TS_Enable);     
            ((PDVBS_TS_Enable_PARAM)pArgs)->ret = bRet;
        break;
#endif
        case DMD_DVBS_DRV_CMD_GetBCHEflagWindow:
            DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBS_v2.c]DVBSIoctl - MDrv_DMD_DVBS_DRV_CMD_GetBCHEflagWindow\n"));            
            bRet = psDVBSInstPri->fpDVBS_GetBCHEflagWindow(((PDVBS_GetBCHEflagWindow_PARAM)pArgs)->BCH_Eflag_Window);     
            ((PDVBS_GetBCHEflagWindow_PARAM)pArgs)->ret = bRet;
        break;
         default:
            break;
    }
...
MS_U32 DVBSStr(MS_U32 u32PowerState, void* pModule)
{
    ...
    if (UtopiaResourceObtain(pModule, DVBS_POOL_ID_DMD0, &pResource) != 0)
    {
        printf("DVBS - UtopiaResourceObtainToInstant fail\n");
	    return UTOPIA_STATUS_ERR_RESOURCE;
    }
...
//    UtopiaModuleCreate(MODULE_DVBT, 8, &pUtopiaModule);   
   ...
   // Utopia2K STR
	...
	UtopiaModuleAddResourceStart(pUtopiaModule, DVBS_POOL_ID_DMD0);
    // resource can alloc private for internal use, ex, BDMA_RESOURCE_PRIVATE
    UtopiaResourceCreate("dvbs0", sizeof(DVBS_RESOURCE_PRIVATE), &psResource);
    // func to reg res
    UtopiaResourceRegister(pUtopiaModule, psResource, DVBS_POOL_ID_DMD0);
    // end function to add res
	UtopiaModuleAddResourceEnd(pUtopiaModule, DVBS_POOL_ID_DMD0);
