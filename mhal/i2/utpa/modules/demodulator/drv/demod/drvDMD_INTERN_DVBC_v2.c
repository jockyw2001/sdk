...
typedef MS_BOOL         (*IOCTL_DVBC_Get_Info_Generalization)(DMD_DVBC_GET_INFO_FUNC InfoType, void* pin, void* pout);
#endif
...
//arthur add 
typedef MS_BOOL                    (*IOCTL_DVBC_GetCurrentSymbolRateOffset)(MS_U32 *pData);
...
typedef struct DVBC_INSTANT_PRIVATE
{
...
#ifdef UFO_DEMOD_DVBC_SUPPORT_DMD_INT
	IOCTL_DVBC_Get_Info_Generalization		fpDVBC_Get_Info_Generalization; 
#endif
	...
	IOCTL_DVBC_GetCurrentSymbolRateOffset		     fpDVBC_GetCurrentSymbolRateOffset;	
...
MS_BOOL DMD_DVBC_Get_Info_Generalization(DMD_DVBC_GET_INFO_FUNC InfoType, void *pin, void *pout)
{
	MS_BOOL bRet=0;
	
	DMD_DBG(printf("===>[120]  DMD_DVBC_Get_Info_Generalization \n"));
	switch(InfoType)
	{
		case GET_INFO_TR_EVER_LOCK:

		bRet=INTERN_DVBC_GetLock_EXT(
			((DVBC_GET_INFO_TR_EVER_LOCK_IN *)pin)->demod_no, 
			DMD_DVBC_GETLOCK_TR_EVER_LOCK , 
			((DVBC_GET_INFO_TR_EVER_LOCK_OUT *)pout)->peLockStatus);

		((DVBC_GET_INFO_TR_EVER_LOCK_OUT *)pout)->ret=bRet;
		break;
	}
	return bRet;
}
...
MS_BOOL DMD_DVBC_GetCurrentSymbolRateOffset(MS_U32 *pData)
{
    MS_BOOL bRet;

    DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBC_v2.c]DMD_DVBC_GetCurrentSymbolRateOffset\n"));

    DMD_LOCK();
    bRet=INTERN_DVBC_GetCurrentSymbolRateOffset(pData);
    DMD_UNLOCK();
...
MS_U32 DVBCOpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
...
#ifdef UFO_DEMOD_DVBC_SUPPORT_DMD_INT
	pDvbcPri->fpDVBC_Get_Info_Generalization=DMD_DVBC_Get_Info_Generalization;
#endif	
  ...
  #ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
  pDvbcPri->fpDVBC_GetCurrentSymbolRateOffset=DMD_DVBC_GetCurrentSymbolRateOffset;
  #endif
...
#ifdef UFO_DEMOD_DVBC_SUPPORT_DMD_INT
	case DMD_DVBC_DRV_CMD_Get_Info_Generalization:
	            DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBC_v2.c]DVBCIoctl - DMD_DVBC_Get_Info_Generalization\n"));            

		bRet=psDVBCInstPri->fpDVBC_Get_Info_Generalization(\
			((PDVBC_Get_Info_Generalization_PARAM) pArgs)->InfoType,\
			((PDVBC_Get_Info_Generalization_PARAM) pArgs)->pin,\
			((PDVBC_Get_Info_Generalization_PARAM) pArgs)->pout);
			
			((PDVBC_Get_Info_Generalization_PARAM) pArgs)->ret=bRet;
...
*/
       ...
       case DMD_DVBC_DRV_CMD_GetPostViterbiBer:
		...
		((PDVBC_GetPostViterbiBer_PARAM)pArgs)->ret=bRet;
           break;
   #ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
       case DMD_DVBC_DRV_CMD_GetCurrentSymbolRateOffset:
            DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBC_v2.c]DVBCIoctl - MDrv_DMD_DVBC_GetCurrentSymbolRateOffset\n"));                    
		bRet=psDVBCInstPri->fpDVBC_GetCurrentSymbolRateOffset(((PDVBC_GetCurrentSymbolRateOffset_PARAM)pArgs)->pData);
		((PDVBC_GetCurrentSymbolRateOffset_PARAM)pArgs)->ret=bRet;
           break;
	 #endif
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
MS_U32 DEMOD_DVBC_MdbIoctl(MS_U32 u32Cmd, const void* const pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            MDrv_DMD_DVBC_SetMdb(paraCmdLine->u64ReqHdl, paraCmdLine->u32CmdSize, paraCmdLine->pcCmdLine);
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MDrv_DMD_DVBC_GetDEMODMdbInfo(paraGetInfo->u64ReqHdl);
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
               UtopiaModuleRegisterMdbNode("demod_dvbc", (FUtopiaMdbIoctl)DEMOD_DVBC_MdbIoctl);
#endif
