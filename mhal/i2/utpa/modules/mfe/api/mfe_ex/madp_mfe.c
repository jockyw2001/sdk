...
static MS_BOOL MAdp_MFE_Proc_nonThread(MEMMAP_CUR_t input_frame)
{
    ...
    MS_VIRT Vaddr;      // VA of HW output location
    MS_U8 *dst = NULL;  // outbuf buffer specified by API user
    MS_S32 Vsize=0;
...
//    MsOS_DelayTask(20); //delay 2 ms
    ...
    while(1) {
        ...
        if((appPVR_Info->BitstreamOutBufferSize-Vsize) < bitsinfo.size) {
            ms_dprintf(ADP_L0, "BitstreamOutBufferSize is too small [%x, %x]\n", (unsigned int)appPVR_Info->BitstreamOutBufferSize, (unsigned int)bitsinfo.size);
            return FALSE;
        }
        if (appPVR_Info->reserved0 && Vaddr != NULL) { // cached memory
           MsOS_Dcache_Flush(Vaddr, CEILING_ALIGN(bitsinfo.size, 16UL));
        }

#ifdef MSOS_TYPE_LINUX_KERNEL
        dst = (MS_U8*)MsOS_PA2KSEG1(appPVR_Info->BitstreamOutBuffer.miuAddress);
#else
        dst = appPVR_Info->BitstreamOutBuffer.miuPointer;
#endif

        if (dst != NULL && Vaddr != NULL)
        {
            memcpy(dst + Vsize, (MS_U8 *)Vaddr, bitsinfo.size);
        }
...
MS_BOOL _MApi_MFE_Init(PVR_Info* mfe_Info)
{
...
#if defined(MFE_SUPPORT_IMI)
    if ((appPVR_Info->MfeAdvInfo.input_imi_en != 0) && (appPVR_Info->bColorFormat != MSTTILE)) {
        ms_dprintf(ADP_L0, "MFE input format must be MSTTILE when using DIP-MFE handshake mode.\n");
        return FALSE;
    }
#else
    if ((appPVR_Info->MfeAdvInfo.input_imi_en != 0) || (appPVR_Info->MfeAdvInfo.low_bandwidth_en != 0)) {
        ms_dprintf(ADP_L4, "MFE does not support IMI. IMI settings are ignored.\n");
    }
#endif
    ...
    if(appPVR_Info->MfeApiInfo.mfe_phy_buf_size < MallocToDrvSize) {
        ms_dprintf(ADP_L0,"appPVR_Info->MfeApiInfo.mfe_phy_buf_size (%d) < MallocToDrvSize (%d)\n",
            (int)appPVR_Info->MfeApiInfo.mfe_phy_buf_size, (int)MallocToDrvSize);
        return FALSE;
    }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
        void* sps_buffer = MfeDrvMemMalloc(128, (const MS_S8*)("mfe_sps"));
        SPSPPS_INFO_t spspps_data = { .pHeaderSPS = sps_buffer, .SPSLen = 0, .pHeaderPPS = 0, .PPSLen = 0 };
        SPSPPS_INFO_t *spspps_info = &spspps_data;
        MS_U8 *u8SPSData = (MS_U8*)MsOS_PA2KSEG1(appPVR_Info->BitstreamOutBuffer.miuAddress);
#else
...
#ifdef MSOS_TYPE_LINUX_KERNEL
        if (spspps_info->pHeaderSPS && u8SPSData && appPVR_Info->BitstreamOutBuffer.size < 128)
        {
            memcpy((void*)(u8SPSData), (void*)spspps_info->pHeaderSPS, appPVR_Info->BitstreamOutBuffer.size);
        }
        else
        {
            printk("%s  size: %d  FATAL ERROR\n", __FUNCTION__, (unsigned int)appPVR_Info->BitstreamOutBuffer.size);
        }

        if (sps_buffer)
        {
            MfeDrvMemFree(&sps_buffer,(const MS_S8*)("mfe_sps"));
        }
#endif
    }
    else {
...
#if defined(MFE_SUPPORT_IMI)
    if (appPVR_Info->MfeAdvInfo.input_imi_en)
    {
        // YUVPlane is not used in DIP-MFE handshake mode.
        // Ignore alignment check and always return TRUE.
        return TRUE;
    }
#endif
...
#if defined(MFE_SUPPORT_IMI)
    if (appPVR_Info->MfeAdvInfo.input_imi_en && appPVR_Info->bColorFormat != MSTTILE)
    {
        ms_dprintf(ADP_L0, "MFE input format must be MSTTILE when using DIP-MFE handshake mode.\n");
        return FALSE;
    }
...
MS_BOOL _MApi_MFE_ISREnable(PVR_Info* mfe_Info)
{
    ...
    mfe_ioctl((MS_VIRT)appPVR_Info->mfe_drvHandle, MFE_IOC_SET_ISR, (MS_VIRT)(appPVR_Info));
    return TRUE;
}
...
EXPORT_SYMBOL(MApi_MFE_CompressOnePicture_ex);
EXPORT_SYMBOL(MApi_MFE_Encode);
EXPORT_SYMBOL(MApi_MFE_DeInitialize);
EXPORT_SYMBOL(MApi_MFE_GetState);
EXPORT_SYMBOL(MApi_MFE_GetVOL_ex);
EXPORT_SYMBOL(MApi_MFE_Initialize_ex);
EXPORT_SYMBOL(MApi_MFE_SetConfig);
EXPORT_SYMBOL(MApi_MFE_GetConfig);
EXPORT_SYMBOL(MApi_MFE_GetHWCap);
