...
MS_VIRT u32MFERegOSBase;

void MHAL_MFE_InitRegBase(MS_VIRT U32RegBase)
{
...
#ifdef CONFIG_MSTAR_CLKM
    MS_S32 handle = Drv_Clkm_Get_Handle("g_clk_mfe");
    if (is_off) {
        Drv_Clkm_Clk_Gate_Disable(handle);
        ms_dprintk(HAL_L1, "CLKM disable clock.\n");
    } else {
        switch (clock_level)
        {
            case MFE_CLK_VERY_SLOW:
                Drv_Clkm_Set_Clk_Source(handle, "Debug_mode");          break;   // 4'b0000
            case MFE_CLK_SLOW:
                Drv_Clkm_Set_Clk_Source(handle, "SDp30_mode");          break;   // 4'b0100
            case MFE_CLK_MEDIUM:
                Drv_Clkm_Set_Clk_Source(handle, "720p30_mode");         break;   // 4'b1000
            case MFE_CLK_FAST:
                Drv_Clkm_Set_Clk_Source(handle, "720p60_1080p30_mode"); break;   // 4'b1100
            default:
                Drv_Clkm_Set_Clk_Source(handle, "720p60_1080p30_mode"); break;   // 4'b1100
        }
        ms_dprintk(HAL_L1, "CLKM enable clock level %d.\n", clock_level);
    }
#else
    if (is_off) {
        *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0x1;   // 4'b0001, disable MFE clock
        ms_dprintk(HAL_L1, "Disable clock.\n");
    } else {
        switch (clock_level)
        {
            case MFE_CLK_VERY_SLOW:
                *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0x0; break;   // 4'b0000
            case MFE_CLK_SLOW:
                *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0x4; break;   // 4'b0100
            case MFE_CLK_MEDIUM:
                *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0x8; break;   // 4'b1000
            case MFE_CLK_FAST:
                *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0xC; break;   // 4'b1100
            default:
                *(MS_U16*)(MS_VIRT)(T8_RIU_BASE + (REG_CKG_MFE * 4)) = 0xC; break;   // 4'b1100
        }
        ms_dprintk(HAL_L1, "Enable clock level %d.\n", clock_level);
    }
#endif // #ifdef CONFIG_MSTAR_CLKM

#ifdef CONFIG_MSTAR_SRAMPD
    // SRAM power: reg_codec_sram_sd_en[20] (32-bit register)
    //     Base 0x1712, offset 0x10, bit 20
    //         1: power down
    //         0: power on
    if (is_off) {
        *((volatile MS_U32 *)(MS_VIRT)(T8_RIU_BASE + 0x71200*2 + 0x10*2*2)) |= (1UL << 20);
        ms_dprintk(HAL_L1, "[MFE] SRAM power down.\n");
    } else {
        *((volatile MS_U32 *)(MS_VIRT)(T8_RIU_BASE + 0x71200*2 + 0x10*2*2)) &= ~(1UL << 20);
        ms_dprintk(HAL_L1, "[MFE] SRAM power on.\n");
    }
    MsOS_DelayTaskUs(1);
#endif
...
void MHal_MFE_SWReset(MFE_REG* mfe_reg)
{
    ...
    WriteRegMFE(0x0, mfe_reg->reg00, (MS_S8*)(""), 0, (MS_S8*)(""));
    ReadRegMFE(0x0, &temp);
    ms_dprintk(HAL_L1, "[HAL] SWReset reg00: 0x%04x\n", (unsigned int)temp);
}
...
void MHal_MFE_ClearIRQ(MS_U16 irq_bits)
{
    ...
    WriteRegMFE(0x1d, irq_bits, (MS_S8*)(""), 0, (MS_S8*)(""));
    ms_dprintk(HAL_L2, "[HAL] ClearIRQ: %u\n", irq_bits);
}

void MHal_MFE_GetIRQ(MS_U16 *irq_bits)
{
    ReadRegMFE(0x1e, irq_bits);
    ms_dprintk(HAL_L2, "[HAL] GetIRQ reg1e: 0x%x\n", (unsigned int)(*irq_bits));
}
...
MS_U32 MHal_MFE_CycleReport(void)
{
    ...
    WriteRegMFE(0x73, tmp_reg, (MS_S8*)(""), 0, (MS_S8*)(""));
...
{
    WriteRegMFE(0x0a, (MS_U16)mfe_reg->reg0a, (MS_S8*)(""), 0, (MS_S8*)(""));
}
...
void MHal_MFE_GetCRC(MS_U8 checksum_HW[8])
{
	...
	WriteRegMFE(0x73, u16Reg1, (MS_S8*)(""), 0, (MS_S8*)(""));
