...
void rc_Init(RateCtrl_t* rcctx, H263RCInfo* pRCInfo)
{
    ...
    ms_dprintf(DRV_L7,
        "[SWVENC] rc_Init() w=%d, h=%d, FPS=%d, MinFPS=%d, Bitrate=%d, nSkipCountMax=%d\n",
        (int)rcctx->m_nWidth, (int)rcctx->m_nHeight, (int)rcctx->m_fTargetFrameRate, (int)rcctx->m_fMinFrameRate,
...
static MS_S32 _rc_ComputeFrameQP(RateCtrl_t* rcctx, MS_S8 chFrameType)
{
    ...
    ms_dprintk(DRV_L7, "Kernel mode does not support VT mode (_rc_ComputeFrameQP)!!\n");
    return -1;
#else
    MS_S32 newQP=0, dQP;
    MS_FLOAT buf_rest;
