...
void OutputSwCfg1_H264(MS_S32 nFrmNum, MFE_CONFIG* pConfig)
{
    ...
    if (YUVTILE != pConfig->bColorFormat) {
        ...
        if (pConfig->MfeAdvInfo.input_imi_en) {
            mfe_reg1->reg_mfe_g_dip_hsk_en = 1;
        }
#endif
    }
#endif
    ...
    if(pConfig->g_nUseYUV422 != 0) {
        ...
        if (pConfig->MfeAdvInfo.input_imi_en)
        {
            // Input buffer addresses in IMI.
            // These fixed values are specified by HW designers.
            mfe_reg->reg_mfe_g_cur_y_adr_low  = MFE_IMI_CUR_Y_LO;
            mfe_reg->reg_mfe_g_cur_y_adr_high = MFE_IMI_CUR_Y_HI;
            mfe_reg->reg_mfe_g_cur_c_adr_low  = MFE_IMI_CUR_C_LO;
            mfe_reg->reg_mfe_g_cur_c_adr_high = MFE_IMI_CUR_C_HI;
        }
        else
#endif
        {
    ...
    {
        mfe_reg->reg_mfe_s_marb_imi_burst_thd = 0x0;
        mfe_reg->reg_mfe_s_marb_imi_timeout   = 0x0;
        mfe_reg->reg_mfe_s_marb_imilast_thd   = 0x0;

        // GN buffer in IMI
        mfe_reg->reg_mfe_s_gn_sadr_low  = MFE_IMI_GN_SADR_LO;
        mfe_reg->reg_mfe_s_gn_sadr_high = MFE_IMI_GN_SADR_HI;

        // DBF buffer in IMI
        mfe_reg->reg_mfe_s_gn_bwr_mode = 0x1;
        mfe_reg->reg_mfe_s_gn_mvibuf_sadr_low  = MFE_IMI_DBF_SADR_LO;
        mfe_reg->reg_mfe_s_gn_mvibuf_sadr_high = MFE_IMI_DBF_SADR_HI;

        // LBW buffer in IMI
        mfe_reg->reg_mfe_s_marb_lbwd_mode = 0x1;
        mfe_reg->reg_mfe_s_marb_imi_sadr_low  = MFE_IMI_LBW_SADR_LO;
        mfe_reg->reg_mfe_s_marb_imi_sadr_high = MFE_IMI_LBW_SADR_HI;
    }
    else
#endif
    {
...
#endif
#if defined(MFE_SUPPORT_IMI)
    nTarWriteCount++;   // mfe_reg.reg4e
#endif
#if defined(MFE_SUPPORT_IMI) || defined(MFE_DBF_PACKED_MODE)
...
#if defined(MFE_SUPPORT_IMI)
    WriteRegMFE(0x4e, mfe_reg->reg4e, (MS_S8*)("[%d] reg4e"), nRegWriteCount++, (MS_S8*)("DBF sadr low"));
#endif
