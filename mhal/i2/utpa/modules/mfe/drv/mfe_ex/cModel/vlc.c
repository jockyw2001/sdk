...
#define ZEROBYTES_SHORTSTARTCODE 2UL //indicates the number of zero bytes in the short start-code prefix





static __inline MS_S32 iabs(MS_S32 x)
{
    return ((x) < 0) ? -(x) : (x);
}
...
MS_S32 RBSPtoEBSP(MS_U8 *streamBuffer, MS_S32 begin_bytepos, MS_S32 end_bytepos, MS_S32 min_num_bytes)
{
    ...
    MS_S32 i, j, count;
    MS_U8* NAL_Payload_buffer = (MS_U8*)MfeDrvMemMalloc(sizeof(MS_U8) * (MAXRBSPSIZE << 1), (const MS_S8*)("NAL_Payload_buffer"));

    if (NAL_Payload_buffer == NULL)
    {
        ms_dprintk(DRV_L4, "[RBSPtoEBSP] NAL_Payload_buffer is null!\n");
        return -1;
    }
    memset(NAL_Payload_buffer, 0, MAXRBSPSIZE << 1);

    memcpy(&NAL_Payload_buffer[begin_bytepos],&streamBuffer[begin_bytepos], (end_bytepos - begin_bytepos) * sizeof(MS_U8));
    count = 0;
    j = begin_bytepos;
    ...
    MfeDrvMemFree((void**)&NAL_Payload_buffer, (const MS_S8*)("NAL_Payload_buffer"));
    return j;
}
                ...
                MS_S32 min_num_bytes, MS_S32 UseAnnexbLongStartcode)
{
    ...
    if (nalu == NULL)
    {
        ms_dprintk(DRV_L4, "[RBSPtoNALU] n is null\n");
        return -1;
    }

    MS_ASSERT (nal_reference_idc <=3 && nal_reference_idc >=0);
    MS_ASSERT (nal_unit_type > 0 && nal_unit_type <= 10);
    MS_ASSERT (rbsp_size < MAXRBSPSIZE);
...
MS_S32 symbol2uvlc(SyntaxElement *sym)
{
    MS_S32 suffix_len = sym->len >> 1;
    MS_ASSERT (suffix_len<32);
...
MS_S32 ue_v (MS_S8 *tracestring, MS_S32 value, Bitstream *bitstream)
{
    ...
    MS_ASSERT (bitstream->streamBuffer != NULL);
...
************************************************************************
*/
void ue_linfo(MS_S32 ue, MS_S32 dummy, MS_S32 *len,MS_S32 *info)
{
    MS_S32 i,nn;
...
************************************************************************
*/
void se_linfo(MS_S32 se, MS_S32 dummy, MS_S32 *len,MS_S32 *info)
{
    MS_S32 sign = (se <= 0) ? 1 : 0;
    MS_S32 n = iabs(se) << 1;   //  n+1 is the number in the code table.  Based on this we find length and info
    MS_S32 nn = (n >> 1);
    MS_S32 i;
    for (i=0; i < 33 && nn != 0; i++)
    {
