...
void MDrv_MFE_SetOSRegBase(MFE_CONFIG *pConfig, MS_VIRT MFE_U32RegBaseAddr )
{
    MHAL_MFE_InitRegBase((MS_VIRT)MFE_U32RegBaseAddr );
}
...
static MS_BOOL copybits2file(MFE_CONFIG *pConfig, MS_U16 val_1e)
{
    ...
    pConfig->bitsframe[wbits_index].miuPointer = (MS_VIRT)addr_phy2log(start + pConfig->MFE_U32MFEMIUOffset);
...
MS_S32 MDrv_MFE_Ioctl(MS_VIRT MFE_fd, MS_U32 cmd, MS_VIRT  arg)
{
    ...
    case MFE_IOC_INIT:
        if (gpMfeConfig != NULL && arg != 0) {
            ...
            gpMfeConfig->MfeAdvInfo.low_bandwidth_en = pappPVR_Info->MfeAdvInfo.low_bandwidth_en;
            gpMfeConfig->MfeAdvInfo.input_imi_en = pappPVR_Info->MfeAdvInfo.input_imi_en;
#else
            gpMfeConfig->MfeAdvInfo.low_bandwidth_en = 0;
            gpMfeConfig->MfeAdvInfo.input_imi_en = 0;
#endif
