...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static MS_S32 _s32SERFLASH_Mutex = -1;
...
static ST_HYBRID_BLOCK _stHybridBlocks_S25FL128S[] =
{
                      //start            //end        //size
    {ISP_SPI_CMD_64BE, 0,           253,       SIZE_64KB },
    {ISP_SPI_CMD_SE,   254,         269,       SIZE_4KB  },
    {ISP_SPI_CMD_SE,   270,         285,       SIZE_4KB  },
    {0x0,              0xFFFF,      0xFFFF,    0xFFFFFFFF},
} ;

static ST_HYBRID_BLOCK _stHybridBlocks_S25FL128S_bot[] =
{
                       //start             //end          //size
    {ISP_SPI_CMD_SE,   0,           15,        SIZE_4KB  },
    {ISP_SPI_CMD_SE,   16,           31,        SIZE_4KB  },
    {ISP_SPI_CMD_64BE, 32,          285,       SIZE_64KB },
    {0x0,              0xFFFF,      0xFFFF,    0xFFFFFFFF},
...
static ST_WRITE_PROTECT _pstWriteProtectTable_S25FL128S[] =
{
    {    BITS(4:2, 7),  FROM_BLK(0,  SIZE_64KB),    TO_BLK(255, SIZE_64KB) }, // 0~255
    {    BITS(4:2, 6),  FROM_BLK(0,  SIZE_64KB),    TO_BLK(127, SIZE_64KB) }, // 0~127
    {    BITS(4:2, 5),  FROM_BLK(0,  SIZE_64KB),    TO_BLK(63,  SIZE_64KB) }, // 0~63
...
static ST_WRITE_PROTECT _pstWriteProtectTable_S25FL128S_bot[] =
{
    {    BITS(4:2, 7),  FROM_BLK(0,   SIZE_64KB),    TO_BLK(255, SIZE_64KB) }, // 0~255
    {    BITS(4:2, 6),  FROM_BLK(128, SIZE_64KB),    TO_BLK(255, SIZE_64KB) }, // 128~255
    {    BITS(4:2, 5),  FROM_BLK(192, SIZE_64KB),    TO_BLK(255, SIZE_64KB) }, // 192~255
...
static MS_BOOL HAL_SERFLASH_HyBridBlock_Erase(MS_U32 u32StartBlock, MS_U32 u32EndBlock);
static MS_BOOL HAL_SERFLASH_DYB_WRITE(MS_U32 u32Addr, MS_U8 u8Data);
    ...
    if (HAL_SERFLASH_ReadID(u8FlashId, sizeof(u8FlashId))== TRUE)
    {
        ...
        if( _hal_SERFLASH.u16FlashType == FLASH_IC_S25FL128P)
        {
            ...
            if( u8FlashCFI[5] == 0x80)
            {
                ...
                printf("u8Status1 =%x \r\n",u8Status1);
                if(_hal_SERFLASH.u16FlashType == FLASH_IC_S25FL128S)
                {
                    if(u8Status1 & BIT(5))
                        _hal_SERFLASH.pWriteProtectTable = _pstWriteProtectTable_S25FL128S_bot;
                    else
                        _hal_SERFLASH.pWriteProtectTable = _pstWriteProtectTable_S25FL128S;

                    if(u8Status1 & BIT(2))
                        _hal_SERFLASH.pHybridBlcoks = _stHybridBlocks_S25FL128S;
                    else
                        _hal_SERFLASH.pHybridBlcoks = _stHybridBlocks_S25FL128S_bot;
                }
                else
                {
                    if(u8Status1 & BIT(5))
...
MS_BOOL HAL_SERFLASH_AddressToBlock(MS_U32 u32FlashAddr, MS_U32 *pu32BlockIndex)
{
    ...
    else if(_hal_SERFLASH.pHybridBlcoks != NULL)
    {
        for(u32NextAddr = 0, *pu32BlockIndex = 0; _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16Start != 0xFFFF; u16hybridindex++)
        {
            ...
            {
                *pu32BlockIndex = _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16Start + (u32FlashAddr /_hal_SERFLASH.pHybridBlcoks[u16hybridindex].u32Size);
                bRet = TRUE;
                break;
            }
...
MS_BOOL HAL_SERFLASH_BlockToAddress(MS_U32 u32BlockIndex, MS_U32 *pu32FlashAddr)
{
    ...
    else if(_hal_SERFLASH.pHybridBlcoks != NULL)
    {
        for(u16hybridindex = 0, *pu32FlashAddr = 0; _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16Start != 0xFFFF; u16hybridindex++)
        {
            ...
            {
                *pu32FlashAddr += ((_hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16End - _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16Start + 1)* _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u32Size);
            }
            else
            {
                *pu32FlashAddr += ((u32BlockIndex - _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u16Start) * _hal_SERFLASH.pHybridBlcoks[u16hybridindex].u32Size);
                 break;
            }
        }
    }
...
MS_BOOL HAL_SERFLASH_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    {
        bRet = HAL_SERFLASH_HyBridBlock_Erase(u32StartBlock, u32EndBlock);
         goto HAL_SERFLASH_BlockErase_return;
    }
