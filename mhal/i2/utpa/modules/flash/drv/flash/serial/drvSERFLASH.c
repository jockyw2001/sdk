...
//-------------------------------------------------------------------------------------------------
/// Description : Set flash disable write protect based on lower bound and size, but enable the rest area
/// Function will execute the last write protect command, for example, Block one is protected currently,
/// and we want to disable the protection of block one, when this function is execute, Block one's write protection will be disabled, but the rest wil be enabled
/// @param  u32DisableLowerBound    \b IN: the lower bound to disable write protect
/// @param  u32DisableSize    \b IN: size to disable write protect
/// @return TRUE : succeed
/// @return FALSE : fail before timeout or illegal parameters
...
MS_BOOL MDrv_SERFLASH_WriteProtect_Disable_Range_Set(MS_U32 u32DisableLowerBound, MS_U32 u32DisableSize)
{
    ...
    // Step 3. get u8BlockProtectBit
    enWpAreaExistedRtn = HAL_SERFLASH_WP_Area_Existed(u32EnableUpperBound, u32EnableLowerBound, &u8BlockProtectBit);
    if( enWpAreaExistedRtn == WP_AREA_NOT_AVAILABLE
        || enWpAreaExistedRtn == WP_TABLE_NOT_SUPPORT )
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGD(TAG_SERFLASH," = FALSE, Write Protect Table does not support this area, u32DisableLowerBound(0x%08X), u32DisableUpperBound(0x%08X), u32FlashIndexMax(0x%08X)\n", (int)u32DisableLowerBound, (int)u32DisableUpperBound, (int)u32FlashIndexMax));
        return FALSE;
    }
    else
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH," = TRUE, Write Protect Table support this area, u32DisableLowerBound(0x%08X), u32DisableUpperBound(0x%08X), u32FlashIndexMax(0x%08X)\n", (int)u32DisableLowerBound, (int)u32DisableUpperBound, (int)u32FlashIndexMax));
        return HAL_SERFLASH_WriteProtect_Area(FALSE, u8BlockProtectBit);
    }
#endif
}
...
MS_BOOL MDrv_FSP_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    MS_ASSERT( u32StartBlock<=u32EndBlock && u32EndBlock<NUMBER_OF_SERFLASH_SECTORS );
    HAL_FSP_Entry();
    bRet = HAL_FSP_BlockErase(u32StartBlock, u32EndBlock);
    HAL_FSP_Exit();
    return bRet;
