...
void MDrv_DAC_Init(void)
{
    ...
    Hal_HDMITx_InitSeq();
#if defined(CHIP_I2)
    Hal_DAC_SetVGAHsyncVsync(TRUE);
#else
    Hal_DAC_SetVGAHsyncVsync(FALSE);
#endif
    Hal_DAC_Dump_AtopRegTab();
}
...
void MDrv_DACMdbIoctl_EchoCmd(MDBCMD_CMDLINE_PARAMETER *paraCmdLine)
{
    MHal_DAC_MdbIoctl_EchoCmd(paraCmdLine);
}
void MDrv_DACMdbIoctl_Cat(MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    MHal_DAC_Mdb_Cat(paraGetInfo);
}
#endif
