...
////////////////////////////////////////////////////////////////////////////////
//define hwi2c ports
#ifdef CONFIG_MSTAR_UTPA2K_HWI2C_ENABLE
#define HWI2C_UTOPIA20 (1)
#else
#define HWI2C_UTOPIA20 (0)
#endif

#ifdef CONFIG_MSTAR_HWI2C_KERNEL_MODE
...
HWI2C_UnitCfg *gp_HWI2CinitCfg = &g_HWI2CinitCfg;
static MS_BOOL g_bStopBitEnable[HWI2C_PORTS];
HWI2C_UnitCfg_Sub gp_HWI2CinitCfg_Sub[HWI2C_PORTS];
...
static MS_BOOL _MDrv_HWI2C_ReadBytes(MS_U8 u8Port, MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    MS_U16 u16Dummy = (g_bEnableRetry==TRUE? I2C_ACCESS_DUMMY_TIME: 1); // loop dummy
    ...
    while (u16Dummy--)
    {
        ...
        if(MDrv_HWI2C_GetStopBit(u8Port))
            HAL_HWI2C_Stop(u32Offset);
        //add extral delay by user configuration
        MsOS_DelayTaskUs_Poll(g_u32StopDelay[u8Port]);
...
static MS_BOOL _MDrv_HWI2C_InitPort(HWI2C_UnitCfg *psCfg)
{
    ...
    for(u8PortIdx=0; u8PortIdx < HWI2C_PORTS; u8PortIdx++)
    {
        ...
        g_bStopBitEnable[u8PortIdx]=TRUE; //Init for WriteByte Enable Stop Bit
    }
...
void MDrv_HWI2C_SetStopBit(MS_U8 u8Port, MS_BOOL bGenStop)
{
    ...
    g_bStopBitEnable[u8Port]=bGenStop;
}
...
MS_BOOL MDrv_HWI2C_GetStopBit(MS_U8 u8Port)
{
    ...
    return g_bStopBitEnable[u8Port];
}
...
MS_BOOL MDrv_HWI2C_Init_U2K(HWI2C_UnitCfg *psCfg)
{
    ...
    if(psCfg->ePort > E_HWI2C_PORT_0 && psCfg->ePort < E_HWI2C_PORT_NOSUP)
    {
        if((psCfg->ePort/8) <  HWI2C_PORTS)
        {
            gp_HWI2CinitCfg_Sub[psCfg->ePort / 8].eReadMode = psCfg->eReadMode;
	    gp_HWI2CinitCfg_Sub[psCfg->ePort / 8].ePort = psCfg->ePort;
            gp_HWI2CinitCfg_Sub[psCfg->ePort / 8].eSpeed = psCfg->eSpeed;
            gp_HWI2CinitCfg_Sub[psCfg->ePort / 8].sI2CPin = psCfg->sI2CPin;
        }
    }
...
MS_U16 MDrv_HWI2C_SetPowerState(EN_POWER_MODE u16PowerState)
{
	...
	else if (u16PowerState == E_POWER_RESUME)
	{
		...
		if (_prev_u16PowerState == E_POWER_SUSPEND)
		{
            ...
            for(u8Port=0; u8Port<HWI2C_PORTS; u8Port++)
            {
                if(gp_HWI2CinitCfg_Sub[u8Port].ePort > E_HWI2C_PORT_0 && gp_HWI2CinitCfg_Sub[u8Port].ePort < E_HWI2C_PORT_NOSUP)
                {
                    if(gp_HWI2CinitCfg_Sub[u8Port].ePort >= E_HWI2C_PORT_1)
                    {
                        MDrv_HWI2C_SelectPort1(gp_HWI2CinitCfg_Sub[u8Port].ePort);
                        MDrv_HWI2C_SetClkP1(gp_HWI2CinitCfg_Sub[u8Port].eSpeed);
                        MDrv_HWI2C_SetReadModeP1(E_HWI2C_READ_MODE_DIRECTION_CHANGE);
                    }
                    else
                    {
                        MDrv_HWI2C_SelectPort(gp_HWI2CinitCfg_Sub[u8Port].ePort);
                        MDrv_HWI2C_SetClk(gp_HWI2CinitCfg_Sub[u8Port].eSpeed);
