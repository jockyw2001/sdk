...
static Context _ctx[NUMBER_OF_CMDQ_HW] = {
    { {0,0,0}, RING_BUFFER_MODE, 0, 0, 0, 0, 0, NULL, -1, "cmdq0.mutex", NULL, "cmdq0.shm", CMDQ_FALSE }
};
#elif (NUMBER_OF_CMDQ_HW == 2)
static Context _ctx[NUMBER_OF_CMDQ_HW] = {
    { {0,0,0}, RING_BUFFER_MODE, 0, 0, 0, 0, 0, NULL, -1, "cmdq0.mutex", NULL, "cmdq0.shm", CMDQ_FALSE },
    { {0,0,0}, RING_BUFFER_MODE, 0, 0, 0, 0, 0, NULL, -1, "cmdq1.mutex", NULL, "cmdq1.shm", CMDQ_FALSE }
};
#else
#endif
...
static DrvCMDQ_DbgLevel _u32CMDQDBGLevel = 3;
...
void CMDQRegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    UtopiaModuleRegisterMdbNode("cmdq", (FUtopiaMdbIoctl)CMDQMdbIoctl);
    #endif
    ...
    // MDrv_CMDQ_Init before MMIO_Init may casue unexpected issue
    // MDrv_CMDQ_Init(_ctx[0].CMDQ_BUFFER_MIU);

    // get shm default init hnd = 0
    /*
    int hnd = 0;
    MS_U32 u32ShmId, u32shmBufSize;
    if(FALSE == MsOS_SHM_GetId((MS_U8*)_ctx[hnd].shm_name, sizeof(ShmCtx), &u32ShmId, (MS_VIRT*)&_ctx[hnd].shm, &u32shmBufSize, MSOS_SHM_QUERY))
    ...
    // init shm
    // only first init will assign shm data, and not from STR_RESUME
    if(_ctx[hnd].shm->init == CMDQ_FALSE)
    {
        _ctx[hnd].shm->init = CMDQ_TRUE;
        _ctx[hnd].shm->miu_select = _ctx[hnd].CMDQ_BUFFER_MIU;
        _ctx[hnd].shm->buffer_wr = CMDQ_INIT_VALUE; //following code flow check 0 to know its not setup.
        _ctx[hnd].shm->buffer_start = CMDQ_INIT_VALUE;
        _ctx[hnd].shm->buffer_end = CMDQ_INIT_VALUE;
    }
    */
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
MS_U32 CMDQMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            MDrv_CMDQ_MDCMD_EchoCmd(NULL, paraCmdLine->u64ReqHdl, paraCmdLine->pcCmdLine);
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MDrv_CMDQ_MDCMD_GETINFO(NULL, paraGetInfo->u64ReqHdl);
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
void MDrv_CMDQ_MDCMD_GETINFO(void* pInstance, MS_U64* u64ReqHdl)
{
    int i;
    MS_U32 r;

    MdbPrint(u64ReqHdl, "------------ Total %d CMDQ HW ------------\n", NUMBER_OF_CMDQ_HW);
    for(i=0;i<NUMBER_OF_CMDQ_HW;i++)
    {
        MdbPrint(u64ReqHdl, " ============ CMDQ #%d ============\n", i);
        MdbPrint(u64ReqHdl, " Mode: (%d) %s\n",
             _ctx[i].CMDQ_MODE, (_ctx[i].CMDQ_MODE==0)?"ALWAYS_WORK":"TRIGGER_MODE");
        MdbPrint(u64ReqHdl, " Basis: \n");
        MdbPrint(u64ReqHdl, "    miu:        %d\n",_ctx[i].CMDQ_BUFFER_MIU);
        MdbPrint(u64ReqHdl, "    size:       %ld  \n",_ctx[i].TOTAL_BUFFER_SIZE);
        MdbPrint(u64ReqHdl, "    start-end:  0x%llx-0x%llx  \n",_ctx[i].buffer_start, _ctx[i].buffer_end);
        MdbPrint(u64ReqHdl, "    wr_ptr:     0x%llx  \n",_ctx[i].buffer_wr);


        MdbPrint(u64ReqHdl, " Shm:\n");
        if(_ctx[i].shm != NULL)
        {
            MdbPrint(u64ReqHdl, "    status:    %s\n",(_ctx[i].shm->init==0)?"NOINIT":"INITED");
            MdbPrint(u64ReqHdl, "    miu:       %d\n",_ctx[i].shm->miu_select);
            MdbPrint(u64ReqHdl, "    start-end: 0x%llx-0x%llx \n",_ctx[i].shm->buffer_start, _ctx[i].shm->buffer_end);
            MdbPrint(u64ReqHdl, "    wr_ptr:    0x%llx\n",_ctx[i].shm->buffer_wr);
        }
        else
        {
            MdbPrint(u64ReqHdl, "    SHM not allocated yet.\n");
            continue;
        }

        if(_ctx[i].cmdq_bInit)
            MdbPrint(u64ReqHdl, " rdptr: 0x%llx\n", HAL_CMDQ_Read_Pointer(i));

        MdbPrint(u64ReqHdl, " Stats:\n");
        MdbPrint(u64ReqHdl, "    cmd_setcount:0x%ld\n",_ctx[i].stats.cmd_setcount);
        MdbPrint(u64ReqHdl, "    cmd_count:   0x%ld\n",_ctx[i].stats.cmd_count);
        MdbPrint(u64ReqHdl, "    null_cnt:    0x%ld\n",_ctx[i].stats.cmd_nullcnt);

        r = HAL_CMDQ_Read_Misc_Status(i);
        MdbPrint(u64ReqHdl, " Misc: cmdptr %s, sram %s, sram bist %s\n",
             (r&CMDQ_CMD_PTR_VLD)?"  VALID":"INVALID",
             (r&CMDQ_RD_SRAM_EMPTY)?" EMPTY":"!EMPTY",
             (r&CMDQ_SRAM_BIST_FAIL)?"FAIL":"  OK");

        r = HAL_CMDQ_Get_Debug_Step_Mode(i);
        MdbPrint(u64ReqHdl, " Debug: debug mode %s, step mode %s\n",
             (r&CMDQ_DEBUG_MODE_ENABLE)?"ON":"OFF",
             (r&CMDQ_DEBUG_ONESTEP_ENABLE)?"DEBUG_MODE":"NORMAL_MODE");
        ...
        //access some statics or debug-assisted register here. Ex. IRQ
        {
            ...
            if(_ctx[i].cmdq_bInit)
                r = HAL_CMDQ_Read_Pointer(i);
            //MdbPrint(u64ReqHdl, "s,e,w,r: %llx, %llx, %llx, %llx\n", s,e,w,r);
        }

    }
    MdbPrint(u64ReqHdl, "------------ CMDQ MDBG Context ------------\n");
}
void MDrv_CMDQ_MDCMD_EchoCmd(void* pInstance, MS_U64* u64ReqHdl, char* pcCmdLine)
{
    if( !strncmp("wd", pcCmdLine, 2) )
    {
        // wd: Write Dummy
        MdbPrint(u64ReqHdl, "wd\n");
    }
    else if ( !strncmp("rd", pcCmdLine, 2) )
    {
        // rd: Read Dummy
        MdbPrint(u64ReqHdl, "rd\n");
    }
    else if ( !strncmp("dbgp", pcCmdLine, 4) )
    {
        // dbgp: MDrv_CMDQv2_Dbg_Pause
        MdbPrint(u64ReqHdl, "dbgp\n");
    }
    else if ( !strncmp("dbgc", pcCmdLine, 4) )
    {
        // dbgc: MDrv_CMDQv2_Dbg_Continue
        MdbPrint(u64ReqHdl, "dbgc\n");
    }
    else
        MdbPrint(u64ReqHdl, "Invalid parameter for mdb cmdq\n");
