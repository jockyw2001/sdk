...
#ifdef CONFIG_API_SWI2C
#define SWI2C_UTOPIA20 (1)
#else
#define SWI2C_UTOPIA20 (0)
#endif
...
//  Local Structures
//-------------------------------------------------------------------------------------------------
extern void MsOS_DelayTaskUs_Poll(MS_U32 u32Us);
...
static MS_BOOL bEnableRetry = TRUE;

//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
#define SWI2C_DBG_FUNC()               do { if (_gSWI2CDbgLevel >= E_SWI2C_DBGLVL_ALL) \
                                        {MS_DEBUG_MSG(ULOGI("SWIIC", "\t====   %s   ====\n", __FUNCTION__));} } while(0)
#define SWI2C_DBG_INFO(x, args...)     do { if (_gSWI2CDbgLevel >= E_SWI2C_DBGLVL_INFO ) \
                                        {MS_DEBUG_MSG(ULOGI("SWIIC", "[%s]: ", __FUNCTION__); printf(x, ##args));} } while(0)
#define SWI2C_DBG_ERR(x, args...)      do { if (_gSWI2CDbgLevel >= E_SWI2C_DBGLVL_ERROR) \
                                        {MS_DEBUG_MSG(ULOGE("SWIIC", "[%s]: ", __FUNCTION__); printf(x, ##args);)} } while(0)
#define SWI2C_DBG_WARN(x, args...)     do { if (_gSWI2CDbgLevel >= E_SWI2C_DBGLVL_WARNING) \
                                        {MS_DEBUG_MSG(ULOGW("SWIIC", "[%s]: ", __FUNCTION__); printf(x, ##args);)} } while(0)
...
void* pInstantSWI2C = NULL;
void* pAttributeSWI2C = NULL;

MS_BOOL bU20SWI2COpened = FALSE;
MS_BOOL _MApi_SWI2C_CheckUtopia20Open(void** pInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
	if (FALSE == bU20SWI2COpened)
	{
		if(UtopiaOpen(MODULE_SWI2C|KERNEL_MODE, pInstance, u32ModuleVersion, pAttribute) != UTOPIA_STATUS_SUCCESS)
		{
			return FALSE;
		}
		bU20SWI2COpened= TRUE;
	}
	return TRUE;
}
#endif
...
#define DELAY_CNT(SpeedKHz)  ((u32FactorDelay/(SpeedKHz))-((u32Parameter1+u32AdjParam)-((SpeedKHz)/u32AdjParam))+((1<<((u32Parameter2-SpeedKHz)/40))))
    ...
    if(u32CpuSpeedMHz>=1700) u32ParamBase1 = 370UL;
    else if(u32CpuSpeedMHz>=1600) u32ParamBase1 = 340UL;
    else if(u32CpuSpeedMHz>=1500) u32ParamBase1 = 280UL;
    else if(u32CpuSpeedMHz>=1400) u32ParamBase1 = 250UL;
    else if(u32CpuSpeedMHz>=1300) u32ParamBase1 = 250UL;
    else if(u32CpuSpeedMHz>=1200) u32ParamBase1 = 220UL;
    else if(u32CpuSpeedMHz>=1100) u32ParamBase1 = 220UL;
    else if(u32CpuSpeedMHz>=1000) u32ParamBase1 = 190UL;
...
void MApi_SWI2C_Init(SWI2C_BusCfg SWI2CCBusCfg[],MS_U8 u8CfgBusNum)
{
    ...
    MDrv_SYS_RegisterIoProc(E_SYS_IO_SWI2C_READ, MApi_SWI2C_ReadBytes);
    MDrv_SYS_RegisterIoProc(E_SYS_IO_SWI2C_WRITE, MApi_SWI2C_WriteBytes);
}
...
{
    MS_U8 u8Dummy = (bEnableRetry==TRUE? I2C_ACCESS_DUMMY_TIME: 1); //I2C_ACCESS_DUMMY_TIME; // loop dummy
    MS_U8 u8BusNum = HIBYTE(u16BusNumSlaveID);
    MS_U8 u8SlaveID = LOBYTE(u16BusNumSlaveID);
    MS_BOOL bRet = FALSE;
    MS_U8 AddrCnt_temp=0;
    MS_U16 u16size_temp=0;
    MS_U8* pu8addr_temp=(MS_U8*)NULL;
    MS_U8* pBuf_temp=(MS_U8*)NULL;
    ...
    while (u8Dummy--)
    {
        ...
        pu8addr_temp =  (MS_U8*)pu8addr;
        pBuf_temp    =  (MS_U8*)pBuf;
...
{
    MS_U8 u8Dummy = (bEnableRetry==TRUE? I2C_ACCESS_DUMMY_TIME: 1); // loop dummy
    MS_U8 u8BusNum = HIBYTE(u16BusNumSlaveID);
    MS_U8 u8SlaveID = LOBYTE(u16BusNumSlaveID);
    MS_BOOL bRet = FALSE;
    MS_U8 *pAddr_bk = paddr;

    MS_U8 ucSubAdr_temp=0;
    MS_U16 ucBufLen_temp=0;
    MS_U8* paddr_temp=(MS_U8*)NULL;
    MS_U8* pBuf_temp=(MS_U8*)NULL;
    ...
    while (u8Dummy--)
    {
        ...
        {
            *pBuf_temp = IIC_GetByte(u8BusNum,ucBufLen_temp); // receive byte
            pBuf_temp++; // next byte pointer
        }
		bRet = TRUE;
        break;
...
/// @brief \b Function  \b Name: MApi_SWI2C_EnableRetry^M
