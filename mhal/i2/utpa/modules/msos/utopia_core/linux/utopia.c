...
static MS_U32 UtopiaConfigReadFile(char* filePath)
{
    ...
    {
		for(u32CntErrMsg = 0;u32CntErrMsg<100;u32CntErrMsg++)
		{
    ...
    while(fgets(sConf,sizeof(sConf),fpUtopiaModuleMode)!=NULL)
    {
        ...
        len = (strlen(pch)>sizeof(sModule))?(sizeof(sModule)-1):strlen(pch);
        strncpy(sModule,pch,len);
        pch = strtok(NULL, tok);
        len = (strlen(pch)>sizeof(sEqual))?(sizeof(sEqual)-1):strlen(pch);
...
#undef PREFIX
    ...
    UtopiaCreateMdbThread(getpid());
    UtopiaCreateMdbNode();
#endif
    if (0 > (agentFd = open("/proc/utopia_mdb/agent", O_RDWR)))
    {
        printu("open proc/utopia_mdb/agent fail\n");
    }
    else
    {
        ioctl(agentFd,MDB_IOCTL_SetModuleMode,moduleMode);
        close(agentFd);
    }
...
//                printu("utopia.c : malloc ok,Module= %s \n",moduleNames[u32ModuleIndex]);
             ...
             if(moduleMode[u32ModuleIndex] & KERNEL_MODE)
			 {
			     ...
			     {
			         printu("aa : 0x%tX  Open /proc/utopia fail aa\n", (ptrdiff_t)u32ModuleID);
...
MS_U32 UtopiaClose(void* pInstantTmp)
{
    ...
    if((pInstance->u32KernelSpaceIdentify) & KERNEL_MODE)
    {
        if(ioctl((int)(pInstance->s32Fd),UTOPIA_IOCTL_CloseMODULE,NULL)!=UTOPIA_STATUS_SUCCESS)
        {
            printu("utopia.c : UtopiaClose fail %d \n",__LINE__);
        }
