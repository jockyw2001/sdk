...
extern unsigned int cache_maintenance_l1(int op, void *va, size_t len) __attribute__((weak));
extern unsigned int cache_maintenance_l2(int op, paddr_t pa, size_t len) __attribute__((weak));

enum cache_op {
	NEW_DCACHE_CLEAN,
	NEW_DCACHE_AREA_CLEAN,
	NEW_DCACHE_INVALIDATE,
	NEW_DCACHE_AREA_INVALIDATE,
	NEW_ICACHE_INVALIDATE,
	NEW_ICACHE_AREA_INVALIDATE,
	NEW_DCACHE_CLEAN_INV,
	NEW_DCACHE_AREA_CLEAN_INV,
};

extern unsigned int cache_op_inner(enum cache_op op, void *va, size_t len) __attribute__((weak));
extern unsigned int cache_op_outer(enum cache_op op, paddr_t pa, size_t len) __attribute__((weak));
//-------------------------------------------------------------------------------------------------
// Local Variables
//-------------------------------------------------------------------------------------------------
...
MS_BOOL MsOS_Dcache_Flush( MS_VIRT u32Start, MS_SIZE u32Size )
{
    ...
    if(cache_maintenance_l1){
        ret = cache_maintenance_l1(DCACHE_AREA_CLEAN, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Clean L1 error, OPTEE Result nummber=%d\n",ret);
        }
    }
    else if(cache_op_inner){
        ret = cache_op_inner(NEW_DCACHE_AREA_CLEAN, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Clean L1 error, OPTEE Result nummber=%d\n",ret);
        }
        ...
        printf( "Optee Not have flush api cache_maintenance_l1 or cache_op_inner\n");

#ifdef CFG_PL310
    if(cache_maintenance_l2){
    ...
    else if(cache_op_outer){
        ret = cache_op_outer(NEW_DCACHE_AREA_CLEAN_INV, core_va2pa(u32Start), u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Flush L2 error, OPTEE Result nummber=%d\n",ret);
        }
    ...
    if(cache_maintenance_l1){
        ret = cache_maintenance_l1(DCACHE_AREA_CLEAN_INV, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
        ...
        ret = cache_op_inner(NEW_DCACHE_AREA_CLEAN_INV, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Flush L1 error, OPTEE Result nummber=%d\n",ret);
        }
...
MS_BOOL MsOS_Dcache_Invalidate( MS_VIRT u32Start, MS_SIZE u32Size )
{
    ...
    if(cache_maintenance_l2){
        ...
        ret = cache_op_outer(NEW_DCACHE_AREA_INVALIDATE, core_va2pa(u32Start), u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Flush L2 error, OPTEE Result nummber=%d\n",ret);
        }
    ...
    if(cache_maintenance_l1){
        ret = cache_maintenance_l1(DCACHE_AREA_INVALIDATE, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
        ...
        ret = cache_op_inner(NEW_DCACHE_AREA_INVALIDATE, (void *)u32Start, u32Size);
        if (ret != TEE_SUCCESS)
        {
            printf( "Optee Clean L1 error, OPTEE Result nummber=%d\n",ret);
        }
...
{
    MS_BOOL bRet = ( HAL_MsOS_MPool_PA2KSEG1(pAddrPhys) == 0 ? FALSE : TRUE );
    return bRet;
}

MS_BOOL MsOS_MPool_IsPA2KSEG0Mapped(MS_PHY pAddrPhys) // cache
{
    MS_BOOL bRet = ( HAL_MsOS_MPool_PA2KSEG0(pAddrPhys) == 0 ? FALSE : TRUE );
    return bRet;
}
