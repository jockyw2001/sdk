...
MS_BOOL __attribute__((weak)) MSOS_MMPool_MapVA(struct MSOS_MMPool_Map_Param * map_param)
{
	 ...
	 mmaping_info_object* mmaping_info_ptr;
      ...
      if(map_param == NULL)
      {
         VPRINTF("\033[1;31m[%s] MSOS_MMPool_MapVA Pool Id %d  fail !!! \033[m\n",__FUNCTION__,__LINE__);
         goto done;
      }
      ...
      list_for_each_entry(mmaping_info_ptr,&mmaping_info_list,list)
      {
            ...
            {
                   map_param->virt_addr = (MS_U8*)mmaping_info_ptr->start_va;
                   ret = TRUE;
                   goto done;
            }
      }
      ...
      {
              VPRINTF("\033[1;31m[%s][%d] Warning : Do not support  MSOSMM_VA_CACHE_WRITE_BACK_Param\033[m\n",__FUNCTION__,__LINE__);
               goto done;
      }
      else if(map_param->cache_type &  MSOSMM_VA_CACHE_NONE_CACHE_Param)
      {
	    ...
	    object->start_va = (MS_VIRT)pdummy;
	    object->len = (MS_U64)map_param->chunk.length;
...
void    __attribute__((weak)) MSOS_MMPool_UnmapVA(struct MSOS_MMPool_Unmap_Param * unmap_param)
{
       ...
       list_for_each_entry(mmaping_info_ptr,&mmaping_info_list,list)
      {
            ...
            {
                   ms_list_remove(&mmaping_info_ptr->list);
                   ret = TRUE;
                   goto done;
            }
      }
      ...
      if(ret == FALSE)
      {
           VPRINTF("\033[1;31m Warning: can not find virtual address:0x%x,length=0x%llx!!! \033[m\n",(MS_VIRT)unmap_param->virt_addr, unmap_param->length);
      }
