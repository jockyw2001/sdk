...
static MS_U32 MsOS_Mutex_Block_Size = 256;
static MS_U32 MsOS_Mutex_Max_Support;

#if defined (TV_OS)
    static MsOS_Mutex_Info*         _MsOS_Mutex_Info;
...
static MS_U32 MsOS_Semaphore_Block_Size = 256;
static MS_U32 MsOS_Semaphore_Max_Support;

static MsOS_Semaphore_Info*     _MsOS_Semaphore_Info;
...
#if defined (TV_OS)
    // since pthread_mutex_t, pthread_cond_t have different typedef in below 3 libs, MsOS_EventGroupPS
    // is not support communication between processes which linked different utopia libs.
    static MS_S32 u32MSos_EventPS_Index = -10;
    #define MUTEX_EVENTPS_LOCK()      MsOS_LockMutex(u32MSos_EventPS_Index,0)
    #define MUTEX_EVENTPS_UNLOCK()    MsOS_UnlockMutex(u32MSos_EventPS_Index,2)
    #if defined (ANDROID) && !defined (__LP64__)
	#define MSOS_EVENTGROUP_MAX_PS		32
	#define MUTEX_EVENTPS_NAME		"msosEventPS_AN"
	#define EVENTPS_SHM_NAME                "EVENTPS_AN"
    #elif defined (__LP64__)
	#define MSOS_EVENTGROUP_MAX_PS		32
	#define MUTEX_EVENTPS_NAME		"msosEventPS_LP64"
	#define EVENTPS_SHM_NAME                "EVENTPS_LP64"
    #else
	#define MSOS_EVENTGROUP_MAX_PS		32
	#define MUTEX_EVENTPS_NAME		"msosEventPS_GLIB"
	#define EVENTPS_SHM_NAME                "EVENTPS_GLIB"
    #endif
#endif
static MsOS_EventGroup_Info     _MsOS_EventGroup_Info_PRI[MSOS_EVENTGROUP_MAX];
static pthread_mutex_t          _MsOS_EventGroup_Mutex;
static MS_U32			PS_offset = MSOS_EVENTGROUP_MAX * 2;
static MsOS_EventGroup_Info*    _MsOS_EventGroup_Info_PS;
...
MS_BOOL MsOS_Init (void)
{
    ...
    // skip the case that MsOS_SHM_Init is already done
    if (-1 == _shm_id)
    {
    ...
    //
#if defined (TV_OS)
    // default support MsOS_Mutex_Block_Size mutex
    MsOS_Mutex_Max_Support = MsOS_Mutex_Block_Size;
    if(u32MSos_Mutex_Index < 0)
    {
        u32MSos_Mutex_Index = MsOS_CreateNamedMutex((MS_S8*)MUTEX_MUTEX_NAME);
        if(u32MSos_Mutex_Index < 0)
        {
            MSOS_ERROR("[%s][%d] create named mutex %s \n",__FUNCTION__,__LINE__,MUTEX_MUTEX_NAME);
            bRet = FALSE;
            goto ret;
        }
    }
    _MsOS_Mutex_Info = (MsOS_Mutex_Info*) malloc(sizeof(MsOS_Mutex_Info)*MsOS_Mutex_Max_Support);
    if (_MsOS_Mutex_Info == NULL)
    {
	MSOS_ERROR("[%s][%d] allocate mem fail\n",__FUNCTION__,__LINE__);
	bRet = FALSE;
	goto ret;
    }
    for( u32I=0; u32I < MsOS_Mutex_Max_Support; u32I++)
    {
    ...
    if (FALSE == MsOS_SHM_GetId((MS_U8*)"_proc_mutex", MSOS_MUTEX_MAX* sizeof(MsOS_Mutex_Info), &u32ShmId, &u32Addr, &u32Size, MSOS_SHM_QUERY))
    {
        ...
        {
            MSOS_ERROR("[%s] MsOS_SHM_GetId _proc_mutex failed!\n"__FUNCTION__);
            bRet = FALSE;
            goto ret;
        }
    }
    ...
    MsOS_Semaphore_Max_Support = MsOS_Semaphore_Block_Size;
    _MsOS_Semaphore_Info = (MsOS_Semaphore_Info*)malloc(sizeof(MsOS_Semaphore_Info)*MsOS_Semaphore_Max_Support);
    memset(_MsOS_Semaphore_Info, 0, sizeof(MsOS_Semaphore_Info)*MsOS_Semaphore_Max_Support);
    if (_MsOS_Semaphore_Info == NULL)
    {
        MSOS_ERROR("[%s][%d] allocate mem fail\n",__FUNCTION__,__LINE__);
        bRet = FALSE;
        goto ret;
    ...
    for( u32I=0; u32I<MsOS_Semaphore_Max_Support; u32I++)
    {
    ...
    {
        _MsOS_EventGroup_Info_PRI[u32I].bUsed = FALSE;
    }
...
MS_S32 MsOS_CreateMutex ( MsOSAttribute eAttribute, char *pMutexName1, MS_U32 u32Flag)
{
    ...
    if (NULL == pMutexName1)
    {
        MSOS_WARN("[%s][%d] invalid name of mutex\n",__FUNCTION__,__LINE__);
        return -1L;
    }
    if (strlen(pMutexName1) >= (MAX_MUTEX_NAME_LENGTH-1))
    {
        MSOS_WARN("[%s][%d] Warning strlen(%s) is longer than MAX_MUTEX_NAME_LENGTH(%d). Oversize char will be discard.\n",
        __FUNCTION__,__LINE__,pMutexName1,MAX_MUTEX_NAME_LENGTH);
    }
    ...
    MUTEX_MUTEX_LOCK();
    for (s32Id=0;s32Id<MsOS_Mutex_Max_Support;s32Id++)
    {
        ...
        if (FALSE == _MsOS_Mutex_Info[s32Id].bUsed)
        {
            break;
        }
    }
    if(MsOS_Mutex_Max_Support <= s32Id)
    {
        // Extend max number of msos mutex (MsOS_Mutex_Max_Support + MsOS_Mutex_Block_Size)
        MsOS_Mutex_Info* tmp_MsOS_Mutex_Info = (MsOS_Mutex_Info*) malloc(sizeof(MsOS_Mutex_Info)*(MsOS_Mutex_Max_Support+MsOS_Mutex_Block_Size) );
        MS_U32 u32I;

        if (tmp_MsOS_Mutex_Info == NULL)
        {
            MSOS_ERROR("[%s][%d] malloc fail\n",__FUNCTION__,__LINE__);
            MUTEX_MUTEX_UNLOCK();
            return -1L;
        }
        memset(tmp_MsOS_Mutex_Info, 0, sizeof(MsOS_Mutex_Info)*(MsOS_Mutex_Max_Support+MsOS_Mutex_Block_Size));
        memcpy(tmp_MsOS_Mutex_Info, _MsOS_Mutex_Info, sizeof(MsOS_Mutex_Info)*MsOS_Mutex_Max_Support );
        free(_MsOS_Mutex_Info);
        _MsOS_Mutex_Info = tmp_MsOS_Mutex_Info;
        for( u32I=MsOS_Mutex_Max_Support; u32I<MsOS_Mutex_Max_Support+MsOS_Mutex_Block_Size; u32I++)
        {
            _MsOS_Mutex_Info[u32I].bUsed = FALSE;
        }
        s32Id = MsOS_Mutex_Max_Support;
        MsOS_Mutex_Max_Support += MsOS_Mutex_Block_Size;
        MSOS_ERROR("[%s][%d] Update max number of msos mutex: %d\n",__FUNCTION__,__LINE__,(int)MsOS_Mutex_Max_Support);
    }
    ...
    if ( MsOS_Mutex_Max_Support > s32Id )
    {
        ...
        if (PTHREAD_PROCESS_SHARED == s32Prop)       //want shared mutex
        {
            ...
            strncpy((char*)_MsOS_Mutex_Info[s32Id].u8Name, (const char*)pMutexName,MIN(strlen((const char*)pMutexName),(MAX_MUTEX_NAME_LENGTH-1)));
            _MsOS_Mutex_Info[s32Id].msos_mutex.u32Index = MsOS_CreateNamedMutex((MS_S8*)_MsOS_Mutex_Info[s32Id].u8Name);
            if(_MsOS_Mutex_Info[s32Id].msos_mutex.u32Index < 0)
            {
                MSOS_ERROR("~!~%s create %s fail \n",__FUNCTION__,_MsOS_Mutex_Info[s32Id].u8Name);
                MUTEX_MUTEX_UNLOCK();
                return -1L;
            }
            _MsOS_Mutex_Info[s32Id].bShared = TRUE;
        }
        else        //want local mutex
        {
           ...
           //PTH_RET_CHK(pthread_mutexattr_setprotocol(&_MsOS_Mutex_Attr, PTHREAD_PRIO_INHERIT));
            ...
            _MsOS_Mutex_Info[s32Id].bUsed = TRUE;
            strncpy((char*)_MsOS_Mutex_Info[s32Id].u8Name, (const char*)pMutexName,MIN(strlen((const char*)pMutexName),(MAX_MUTEX_NAME_LENGTH-1)));
            PTH_RET_CHK(pthread_mutex_init(&_MsOS_Mutex_Info[s32Id].msos_mutex.stMutex, &_MsOS_Mutex_Attr));
            _MsOS_Mutex_Info[s32Id].bShared = FALSE;
        }
#else
        PTH_RET_CHK(pthread_mutexattr_init(&_MsOS_Mutex_Attr));
        PTH_RET_CHK(pthread_mutexattr_setpshared(&_MsOS_Mutex_Attr, s32Prop));
       //PTH_RET_CHK(pthread_mutexattr_setprotocol(&_MsOS_Mutex_Attr, PTHREAD_PRIO_INHERIT));
        ...
        _MsOS_Mutex_Info[s32Id].bUsed = TRUE;
        strncpy((char*)_MsOS_Mutex_Info[s32Id].u8Name, (const char*)pMutexName,MIN(strlen((const char*)pMutexName),(MAX_MUTEX_NAME_LENGTH-1)));
        PTH_RET_CHK(pthread_mutex_init(&_MsOS_Mutex_Info[s32Id].stMutex, &_MsOS_Mutex_Attr));
#endif
    }
    MUTEX_MUTEX_UNLOCK();
                          ...
                          char *pSemaphoreName)
{
    ...
    PTH_RET_CHK(pthread_mutex_lock(&_MsOS_Semaphore_Mutex));
    // search unused msos semaphore
    for(s32Id=0;s32Id<MsOS_Semaphore_Max_Support;s32Id++)
    {
    ...
    if(s32Id >= MsOS_Semaphore_Max_Support)
    {
        // Extend max number of msos semaphore (MsOS_Semaphore_Max_Support + MsOS_Semaphore_Block_Size)
        MsOS_Semaphore_Info* tmp_MsOS_Semaphore_Info = (MsOS_Semaphore_Info*) malloc(sizeof(MsOS_Semaphore_Info)*(MsOS_Semaphore_Max_Support + MsOS_Semaphore_Block_Size) );
        if (tmp_MsOS_Semaphore_Info == NULL)
        {
            MSOS_ERROR("[%s][%d] malloc fail\n",__FUNCTION__,__LINE__);
            goto fail;
        }
        memset(tmp_MsOS_Semaphore_Info, 0, sizeof(MsOS_Semaphore_Info)*(MsOS_Semaphore_Max_Support + MsOS_Semaphore_Block_Size));
        memcpy(tmp_MsOS_Semaphore_Info, _MsOS_Semaphore_Info, sizeof(MsOS_Semaphore_Info)*MsOS_Semaphore_Max_Support );
        free(_MsOS_Semaphore_Info);
        _MsOS_Semaphore_Info = tmp_MsOS_Semaphore_Info;
        for( u32I=MsOS_Semaphore_Max_Support; u32I<MsOS_Semaphore_Max_Support + MsOS_Semaphore_Block_Size; u32I++)
        {
            _MsOS_Semaphore_Info[u32I].bUsed = FALSE;
        }
        s32Id = MsOS_Semaphore_Max_Support;
        MsOS_Semaphore_Max_Support += MsOS_Semaphore_Block_Size;
        MSOS_ERROR("[%s][%d] Update max number of msos Semaphore: %d\n",__FUNCTION__,__LINE__,(int)MsOS_Semaphore_Max_Support);
    }
...
MS_BOOL MsOS_ObtainSemaphore (MS_S32 s32SemaphoreId, MS_U32 u32WaitMs)
{
	...
	while(1)
	{
	    ...
	    err = sem_wait(&_MsOS_Semaphore_Info[s32SemaphoreId].stSemaphore);
		...
		switch(errno)
		{
		    ...
		    default:
			if(errno !=(int)NULL)
			    MSOS_ERROR("[%s][%d]sem_wait is interrupted: %d\n",__FUNCTION__,__LINE__, errno);
			bRet = FALSE;
			break;
		}
...
// Event management
//
MS_BOOL MsOS_Eventgroup_PS_Init()
{
    MS_U32 event_group_shm_id, u32I;
    MS_VIRT event_group_shm_addr;
    MS_U32 event_group_buf_size;
    MS_BOOL bRet = TRUE;
    // create msos event ps mutex
    if(u32MSos_EventPS_Index < 0)
    {
        u32MSos_EventPS_Index = MsOS_CreateNamedMutex((MS_S8*)MUTEX_EVENTPS_NAME);
        if(u32MSos_EventPS_Index < 0)
        {
            MSOS_ERROR("%s : %d,create named mutex %s \n",__FUNCTION__,__LINE__,MUTEX_EVENTPS_NAME);
            return FALSE;
        }
    }
    ...
    if (FALSE == MsOS_SHM_GetId( (MS_U8*) EVENTPS_SHM_NAME,
                                          sizeof(MsOS_EventGroup_Info) * MSOS_EVENTGROUP_MAX_PS,
                                          &event_group_shm_id,
                                          &event_group_shm_addr,
                                          &event_group_buf_size,
                                          MSOS_SHM_QUERY))
    {
        if (FALSE == MsOS_SHM_GetId((MS_U8*) EVENTPS_SHM_NAME,
                                             sizeof(MsOS_EventGroup_Info) * MSOS_EVENTGROUP_MAX_PS,
                                             ...
                                             MSOS_SHM_CREATE))
	{
		MSOS_ERROR("[%s][%d] got new shm ptr!\n",__FUNCTION__,__LINE__);
		bRet = FALSE;
		goto ret;
	}
	else
	{
		memset( (MS_U8*)event_group_shm_addr, 0, sizeof(MsOS_EventGroup_Info)*MSOS_EVENTGROUP_MAX_PS );
		_MsOS_EventGroup_Info_PS = (MsOS_EventGroup_Info*)event_group_shm_addr;
		MSOS_ERROR("[%s][%d] got new shm ptr!\n",__FUNCTION__,__LINE__);
		for( u32I=0; u32I<MSOS_EVENTGROUP_MAX_PS; u32I++)
		{
		    _MsOS_EventGroup_Info_PS[u32I].bUsed = FALSE;
		}
    ...
    {
	_MsOS_EventGroup_Info_PS = (MsOS_EventGroup_Info*)event_group_shm_addr;
	if (_MsOS_EventGroup_Info_PS == NULL) {
	    MSOS_ERROR("[%s][%d] event get shm id failed!\n",__FUNCTION__,__LINE__);
	    bRet = FALSE;
	    goto ret;
	}
	MSOS_ERROR("[%s][%d] got shm ptr!\n",__FUNCTION__,__LINE__);
    }


ret:
    MUTEX_EVENTPS_UNLOCK();
    return bRet;
...
#endif
    MsOS_EventGroup_Info* _MsOS_EventGroup_Info = _MsOS_EventGroup_Info_PRI;
    PTH_RET_CHK(pthread_mutexattr_init(&_MsOS_Mutex_Attr));
    //PTH_RET_CHK(pthread_mutexattr_setprotocol(&_MsOS_Mutex_Attr, PTHREAD_PRIO_INHERIT));
#ifdef MS_DEBUG
    PTH_RET_CHK(pthread_mutexattr_settype(&_MsOS_Mutex_Attr, PTHREAD_MUTEX_ERRORCHECK));
...
/// @return <0 : fail
//-------------------------------------------------------------------------------------------------
MS_S32 MsOS_CreateEventGroup_PS (char *pEventName)
{
    MS_S32 bRet = -1;
#if defined (TV_OS)
    MS_S32 s32Id;
    pthread_mutexattr_t _MsOS_Mutex_Attr;
    pthread_condattr_t _MsOS_Cond_Attr;
    MsOS_EventGroup_Info* _MsOS_EventGroup_Info = _MsOS_EventGroup_Info_PS;

    PTH_RET_CHK(pthread_mutexattr_init(&_MsOS_Mutex_Attr));
    PTH_RET_CHK(pthread_mutexattr_setpshared(&_MsOS_Mutex_Attr, PTHREAD_PROCESS_SHARED));
    //PTH_RET_CHK(pthread_mutexattr_setprotocol(&_MsOS_Mutex_Attr, PTHREAD_PRIO_INHERIT));
#ifdef MS_DEBUG
    PTH_RET_CHK(pthread_mutexattr_settype(&_MsOS_Mutex_Attr, PTHREAD_MUTEX_ERRORCHECK));
#endif
    ...
    MUTEX_EVENTPS_LOCK();
    for(s32Id=0; s32Id<MSOS_EVENTGROUP_MAX_PS; s32Id++)
    {
        if(_MsOS_EventGroup_Info[s32Id].bUsed == FALSE)
        {
            break;
    ...
    if(s32Id < MSOS_EVENTGROUP_MAX_PS)
    {
        PTH_RET_CHK(pthread_mutex_init(&_MsOS_EventGroup_Info[s32Id].stMutexEvent, &_MsOS_Mutex_Attr));
        PTH_RET_CHK(pthread_mutex_lock(&_MsOS_EventGroup_Info[s32Id].stMutexEvent));
        _MsOS_EventGroup_Info[s32Id].bUsed = TRUE;
    ...
    pthread_condattr_init(&_MsOS_Cond_Attr);
    pthread_condattr_setpshared(&_MsOS_Cond_Attr, PTHREAD_PROCESS_SHARED);
#if !defined (ANDROID) || defined (__LP64__)
    // Set the clock to be CLOCK_MONOTONIC
    PTH_RET_CHK(pthread_condattr_setclock(&_MsOS_Cond_Attr, CLOCK_MONOTONIC));
#endif
    PTH_RET_CHK(pthread_mutex_init(&_MsOS_EventGroup_Info[s32Id].stMutex, &_MsOS_Mutex_Attr));
    PTH_RET_CHK(pthread_cond_init(&_MsOS_EventGroup_Info[s32Id].stSemaphore, &_MsOS_Cond_Attr));


    s32Id += PS_offset;
    bRet = s32Id | MSOS_ID_PREFIX;
#endif
ret:
    return bRet;
}
...
{
    MS_BOOL PS_flag = FALSE;
    MsOS_EventGroup_Info* _MsOS_EventGroup_Info = NULL;
    ...
    if( s32EventGroupId >= PS_offset )
    {
	PS_flag = TRUE;
	s32EventGroupId -= PS_offset;
	_MsOS_EventGroup_Info = _MsOS_EventGroup_Info_PS;
    }
    else
    {
	_MsOS_EventGroup_Info = _MsOS_EventGroup_Info_PRI;
    }
                      ...
                      char *pTimerName)
{
    ...
    if(0> timer_create(CLOCK_MONOTONIC, &SignalEvent, &_MsOS_Timer_Info[s32Id].TimerId))
    {
        return -1;
    }
...
MS_BOOL MsOS_KernelMode(MS_U32 u32ModuleId, MS_BOOL bSet)
{
#if defined(CONFIG_UTOPIA_FRAMEWORK_KERNEL_DRIVER)
    MS_U32 u32Id = 0;
    MS_ASSERT(u32ModuleId < eMODULE_END);
    if(bSet == FALSE)
    {
        if(u32ModuleId == MODULE_UTOPIA)
        {
            for(u32Id = 0; u32Id < eMODULE_END; u32Id++)
            {
                if(moduleMode[u32Id] == KERNEL_MODE)
                {
                    return TRUE;
                }
            }
        ...
        {
            return (moduleMode[u32ModuleId] == KERNEL_MODE);
        }
    }
    else
    {
        if(u32ModuleId == MODULE_UTOPIA)
        {
            for(u32Id = 0; u32Id < eMODULE_END; u32Id++)
            {
                moduleMode[u32Id] = KERNEL_MODE;
            }
        }
        else
        {
            moduleMode[u32ModuleId] = KERNEL_MODE;
        }
        return TRUE;
    }
#else
