...
MS_BOOL __attribute__((weak)) PA_in_IPA_POOL_info(struct PA_In_IPA_Pool_Param * in_ipa_pool_info)
{
    ...
    if (res < 0 ||  in_ipa_pool_info_args.error_code < 0) 
    {
        ...
        VPRINTF("ipa error: %s , PA 0x%lx,res=%d ,error_code=0x0x%tX\n",__FUNCTION__, (unsigned long)in_ipa_pool_info_args.PA,res,(ptrdiff_t)in_ipa_pool_info_args.error_code);
        goto PA_in_IPA_POOL_info;     
    ...
    VPRINTF(" %s  PA:0x%lx   pa_state=%d\n",__FUNCTION__, (unsigned long)in_ipa_pool_info_args.PA,in_ipa_pool_info_args.pa_state);
     ...
     {
        VPRINTF("ipa error: %s failed, PA 0x%lx,miu=0x%tX\n",__FUNCTION__, (unsigned long)in_ipa_pool_info_args.PA,(ptrdiff_t)in_ipa_pool_info_args.miu);
     ...
     {
        VPRINTF("ipa error: %s failed, PA 0x%lx,miu=0x%tX,in_heap is FALSE\n",__FUNCTION__, (unsigned long)in_ipa_pool_info_args.PA,(ptrdiff_t)in_ipa_pool_info_args.miu);
        in_ipa_pool_info->allocated = FALSE;
     ...
     else
     {
         ...
         {
             VPRINTF("ipa error: %s failed, PA 0x%lx,miu=0x%tX,in_ipa_pool_info_args.heap_id=0x%tX,allocated is FALSE\n",__FUNCTION__, (unsigned long)in_ipa_pool_info_args.PA,(ptrdiff_t)in_ipa_pool_info_args.miu,(ptrdiff_t)in_ipa_pool_info_args.heap_id);
...
MS_BOOL __attribute__((weak)) IN_IPA_POOL_To_PA(struct Pool_To_PA_Param * pool_to_pa_param)
{
    ...
    VPRINTF("%s handle_id 0x0x%tX offset_in_pool 0x%lx\n", 
        __FUNCTION__,(ptrdiff_t)pool_to_pa_param->pool_handle_id, (unsigned long)pool_to_pa_param->offset_in_pool);
    ...
    {  
        VPRINTF("%s fail: pool_handle_id 0x%tX, offset 0x%lx, error_code=0x0x%tX  res=%d\n",__FUNCTION__, (ptrdiff_t)pool_to_pa_param->pool_handle_id, 
            (unsigned long)pool_to_pa_param->offset_in_pool,(ptrdiff_t)pool_to_pa_args.error_code,res);
        ret = FALSE;
        goto POOL_To_PA_DONE;
    }
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_Init(struct IPA_Pool_Init_Param * Init_Param)
{
    ...
    int res = 0;
    //VPRINTF("%s start\n",__FUNCTION__);
    VPRINTF("%s heap_id 0x%tX\n",__FUNCTION__, (ptrdiff_t)Init_Param->space_id);
    pthread_mutex_lock(&_IPA_POOL_Mutex);
    if (_s32FdIPAPool <= 0)
    {
        if ((_s32FdIPAPool = open("/dev/ipapool", O_RDWR)) < 0)
    ...
    if(ret == TRUE)
    {
         ...
         VPRINTF("pool_handle_id 0x%tX already init!\n", (ptrdiff_t)IPAPool_Info[idx].Init_Param.pool_handle_id);
         goto IPA_POOL_INIT_DONE;
    }
    ...
    {
        VPRINTF("ipa error: ipa init failed, heapid 0x%tX error_code 0x%tX\n", (ptrdiff_t)Init_Param->space_id, (ptrdiff_t)ipa_init_args.error_code);
        ret =  FALSE;
        goto IPA_POOL_INIT_CLOSE;
    }

    VPRINTF("%s heap_id 0x%tX, pool_handle_id 0x%tX heap_length %llu\n", 
		__FUNCTION__,(ptrdiff_t)Init_Param->space_id, (ptrdiff_t)ipa_init_args.pool_handle_id, (long long unsigned int)ipa_init_args.heap_length);
    ...
    if(ret == FALSE)
    {
        VPRINTF("ipa error: pool_handle_id 0x%tX init failed!\n", (ptrdiff_t)ipa_init_args.pool_handle_id);
        goto IPA_POOL_INIT_DONE;
    }
    ...
    VPRINTF("%s heap_id 0x%tX pool_handle_id 0x%tX miu 0x%tX offset 0x%lx len 0x%lx  idx=0x%tX\n",__FUNCTION__,
		(ptrdiff_t)Init_Param->space_id, (ptrdiff_t)Init_Param->pool_handle_id, (ptrdiff_t)Init_Param->miu, (unsigned long)Init_Param->offset_in_heap, (unsigned long)Init_Param->len,(ptrdiff_t)idx);
    pthread_mutex_unlock(&_IPA_POOL_Mutex);
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_GetMem(struct IPA_Pool_GetMem_Param * get_param)
{
    ...
    VPRINTF("%s %d pool_handle_id 0x0x%tX [offset_in_pool ,length]=[0x%lx,0x%lx]\n",__FUNCTION__,__LINE__,
		(ptrdiff_t)ipa_alloc_args.pool_handle_id,(unsigned long)ipa_alloc_args.offset_in_pool, (unsigned long)ipa_alloc_args.length);
    res = ioctl(_s32FdIPAPool, IPA_POOL_IOC_ALLOC, &ipa_alloc_args);
    if (res < 0 || ipa_alloc_args.error_code != IPAERROR_OK)
    {  
        VPRINTF("%s fail: pool_handle_id 0x%tX, offset 0x%lx, len 0x%lx  ipa_alloc_args.error_code=0x0x%tX  res=%d\n",__FUNCTION__, (ptrdiff_t)get_param->pool_handle_id,
			(unsigned long)get_param->offset_in_pool, (unsigned long)get_param->length,(ptrdiff_t)ipa_alloc_args.error_code,res);
        ret = FALSE;
        goto IPA_POOL_GETMEM_DONE;
    }
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_PutMem(struct IPA_Pool_PutMem_Param * put_param)
{
    ...
    VPRINTF("%s %d pool_handle_id 0x0x%tX [offset_in_pool,length]=[0x%lx,0x%lx]\n", __FUNCTION__,__LINE__,
		(ptrdiff_t)ipa_free_args.pool_handle_id, (unsigned long)ipa_free_args.offset_in_pool, (unsigned long)ipa_free_args.length);

    if(ioctl(_s32FdIPAPool, IPA_POOL_IOC_FREE, &ipa_free_args))
    {        
        VPRINTF("%s fail: pool_handle_id 0x0x%tX, offset 0x%lx, len 0x%lx\n",__FUNCTION__,
			(ptrdiff_t)put_param->pool_handle_id, (unsigned long)put_param->offset_in_pool,(unsigned long)put_param->length);
        ret = FALSE;
        goto IPA_POOL_PUTMEM_DONE;
    }
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_Release(MS_U32 pool_handle_id)
{
    ...
    VPRINTF("%s handle_id 0x%tX\n",__FUNCTION__, (ptrdiff_t)pool_handle_id);
    pthread_mutex_lock(&_IPA_POOL_Mutex);
    if (_s32FdIPAPool < 0)
    {
        ret = FALSE;
    ...
    if(ret == FALSE)
    {
        VPRINTF("pool_handle_id 0x%tX _findPoolHandleId_InIPA_Pool_Table fail\n", (ptrdiff_t)pool_handle_id);
        goto IPA_POOL_RELEASE_DONE;
    }
    ...
    for(i=0;i<MAX_CLIENT_MAP_NUM;i++)
    {
		    ...
		    && (0 != IPAPool_Info[idx].map_info[i].length))
        {
            ...
            if (res < 0)
            {
                ...
                VPRINTF("error: pool_handle_id 0x%tX unmap failed\n", (ptrdiff_t)pool_handle_id);
            }
    ...
    {        
        VPRINTF("pool_handle_id 0x%tX deinit fail\n", (ptrdiff_t)pool_handle_id);            
        ret = FALSE;
        goto IPA_POOL_RELEASE_DONE;
    }
    ...
    if(IPAPool_Info[idx].pthIPAPollingId != -1)
    {
        ...
        if(1 != IPAPool_Info[idx].polling_thread_delete_task_flag)
        {
            VPRINTF("error !!!!!!   after pthread_join  polling_thread_delete_task_flag is not 1!!!  idx=0x%tX  IPAPool_Info[0x%tX].polling_thread_delete_task_flag=%u\n",(ptrdiff_t)idx,(ptrdiff_t)idx,(ptrdiff_t)IPAPool_Info[idx].polling_thread_delete_task_flag);
            ret = FALSE;
        }
...
static MS_BOOL MApi_IPA_Pool_MapUserVA(struct IPA_Pool_Map_Param * map_param)
{
    ...
    MS_U32 idx = 0;
    VPRINTF("%s pool_handle_id 0x%tX, offset_in_pool 0x%lx, len 0x%lx,  cache_type %u\n",__FUNCTION__, (ptrdiff_t)map_param->pool_handle_id,
    ...
    {  
        VPRINTF("%s  fail: pool_handle_id 0x%tX, offset_in_pool 0x%lx, len 0x%lx,  cache_type %u\n",__FUNCTION__, (ptrdiff_t)map_param->pool_handle_id,
			(unsigned long)map_param->offset_in_pool, (unsigned long)map_param->length,map_param->cache_type);
        ret = FALSE;
        goto IPA_POOL_MapUserVA_DONE;
    }
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_HEAP_ATTR(struct IPA_Pool_Heap_Attr_Param* heap_attr_param)
{
    ...
    VPRINTF("%s  heap_id 0x0x%tX\n",__FUNCTION__, (ptrdiff_t)heap_attr_param->heap_id);
    pthread_mutex_lock(&_IPA_POOL_Mutex);
    if (_s32FdIPAPool < 0)
    {
        ret = FALSE;
    ...
    {  
        VPRINTF("%s  fail:  heap_id 0x0x%tX\n",__FUNCTION__,(ptrdiff_t)heap_attr_param->heap_id);
        ret = FALSE;
        goto IPA_POOL_HEAP_ATTR_DONE;
    } 
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_GETIPCHANDLE(struct IPA_Pool_GetIpcHandle_Param* getipchandle_param)
{
    ...
    VPRINTF("%s  pool_handle_id 0x0x%tX\n",__FUNCTION__, (ptrdiff_t)getipchandle_param->pool_handle_id);
    pthread_mutex_lock(&_IPA_POOL_Mutex);
    if (_s32FdIPAPool < 0)
    {
        VPRINTF("MApi_IPA_Pool_GETIPCHANDLE  fail  pool_handle_id=0x0x%tX,   _s32FdIPAPool=0x%tX \n", (ptrdiff_t)getipchandle_param->pool_handle_id,(ptrdiff_t)_s32FdIPAPool);      	
        ret = FALSE;
    ...
    {
        VPRINTF("%s  fail:  pool_handle_id 0x0x%tX\n",__FUNCTION__,(ptrdiff_t)getipchandle_param->pool_handle_id);
        ret = FALSE;
        goto IPA_POOL_GETIPCHANDLE_DONE;
    } 
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_InstallIpcHandle(struct IPA_Pool_InstallIpcHandle_Param* installipchandle_param)
{
    ...
    VPRINTF("%s  ipc_handle_id 0x0x%tX\n",__FUNCTION__, (ptrdiff_t)installipchandle_param->ipc_handle_id);
    pthread_mutex_lock(&_IPA_POOL_Mutex);
    if (_s32FdIPAPool < 0)
    {
        VPRINTF("%s ipc_handle_id=0x0x%tX,   _s32FdIPAPool=0x%tX \n",__FUNCTION__, (ptrdiff_t)installipchandle_param->ipc_handle_id,(ptrdiff_t)_s32FdIPAPool);
        //this process may not open ipapool yet,need we open for this process.
        if ((_s32FdIPAPool = open("/dev/ipapool", O_RDWR)) < 0)
        {
            VPRINTF("open /dev/ipapool fail   ipc_handle_id=0x0x%tX,   _s32FdIPAPool=0x%tX\n", (ptrdiff_t)installipchandle_param->ipc_handle_id,(ptrdiff_t)_s32FdIPAPool);
            ret =  FALSE;
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_GetMem_Timeout(struct IPA_Pool_GetMem_Param* get_param,MS_U32 timeout_ms)
{
    ...
    VPRINTF("%s %d pool_handle_id 0x0x%tX [offset_in_pool,length]= [0x%lx,0x%lx] timeout 0x0x%tX\n", __FUNCTION__,__LINE__,
		(ptrdiff_t)ipa_alloc_args.pool_handle_id, (unsigned long)ipa_alloc_args.offset_in_pool,(unsigned long)ipa_alloc_args.length,(ptrdiff_t)timeout_ms);
    ...
    if (res < 0 || ipa_alloc_args.error_code != IPAERROR_OK)
    {
        ...
        {    
            ...
            VPRINTF("%s:%d each_delay_ms=0x%tX, delay and try alloc again now_delay_ms=0x%tX\n",__FUNCTION__,__LINE__,(ptrdiff_t)each_delay_ms,(ptrdiff_t)now_delay_ms);
        ...
        if (res < 0 || ipa_alloc_args.error_code != IPAERROR_OK)
        {
            VPRINTF("ipa pool get memory fail: pool_handle_id 0x%tX, offset 0x%lx, len 0x%lx  ipa_alloc_args.error_code=0x%x ipa_alloc_args.timeout=0x%tX res=0x%tX\n", (ptrdiff_t)get_param->pool_handle_id, 
			(unsigned long)get_param->offset_in_pool, (unsigned long)get_param->length,(ptrdiff_t)ipa_alloc_args.error_code,(ptrdiff_t)ipa_alloc_args.timeout,res);
            ret = FALSE;
            goto IPA_POOL_GETMEM_DONE;
        }
    }
...
void *__attribute__((weak)) IPA_Pool_Polling_Task(void *argc)
{
    ...
    VPRINTF("%s %d pool_handle_id=0x%tX,argc=0x%tX\n",__FUNCTION__,__LINE__,(ptrdiff_t)pool_handle_id,(ptrdiff_t)argc);
    ...
    VPRINTF("%s %d pool_event_args.pool_handle_id=0x%tX\n",__FUNCTION__,__LINE__,(ptrdiff_t)pool_event_args.pool_handle_id);
    ...
    IPAPool_Info[idx].polling_thread_delete_task_flag = 0;
    VPRINTF("%s %d set for idx=0x%tX\n",__FUNCTION__,__LINE__,(ptrdiff_t)idx);
    ...
    while(1)
    {
        ...
        if (fds.revents&POLLIN)
        {
            ...
            if(IPA_EVENT_CONFLICT == pool_event_args.event)
            {
                ...
                VPRINTF("%s %d  pool_handle_id=0x%tX  start=0x%lx,length=0x%lx\n",__FUNCTION__,__LINE__,(ptrdiff_t)pool_event_args.pool_handle_id,(unsigned long)pool_event_args.start,(unsigned long)pool_event_args.length);
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_Notify(struct IPA_Pool_Polling_Param *polling_param)
{
    ...
    {
        VPRINTF("0x%tX have no polling_callback function , %s return directly\n",(ptrdiff_t)polling_param->pool_handle_id,__FUNCTION__);
        pthread_mutex_unlock(&_IPA_POOL_Mutex);
        return TRUE;
    }
    ...
    {
         VPRINTF(" %s ipa pool fail not find pool_handle_id 0x%tX in pool table\n",__FUNCTION__,(ptrdiff_t)polling_param->pool_handle_id);
         pthread_mutex_unlock(&_IPA_POOL_Mutex);
         return FALSE;
    }
    VPRINTF("%s  polling_param->pool_handle_id=0x0x%tX\n",__FUNCTION__,(ptrdiff_t)polling_param->pool_handle_id);
    ...
    {
        ret=FALSE;
        VPRINTF("pool handle id 0x%tX has its polling id %lu ,error,will not set callback again\n",(ptrdiff_t)polling_param->pool_handle_id,IPAPool_Info[idx].pthIPAPollingId);
    }
