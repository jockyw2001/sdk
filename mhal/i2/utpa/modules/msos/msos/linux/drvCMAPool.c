...
MS_BOOL __attribute__((weak)) MApi_CMA_Pool_Init(struct CMA_Pool_Init_Param *Init_Param)
{
    ...
    MS_U32 idx = 0;
    char * ptr = NULL;
    VPRINTF("[user mode %s:%d]debug    Init_Param->heap_id=0x%tX\n",__FUNCTION__,__LINE__, (ptrdiff_t)Init_Param->heap_id);
...
MS_BOOL __attribute__((weak)) MApi_CMA_Pool_GetMem(struct CMA_Pool_Alloc_Param *alloc_param)
{
    ...
    {  
        VPRINTF("cma pool get memory fail: heap id %tu, offset %llu, size %lu, flags 0x%tX\n", (ptrdiff_t)alloc_args.heap_id,  
...
MS_BOOL CMA_Pool_Get_Info_From_PA(MS_S32 fd,struct CMA_Pool_Get_Info_From_Pa_Param *get_info_param)
{
    ...
    if (fd <= 0)
    {
       VPRINTF("%s:%d  fd=0x%tX is false\n",__FUNCTION__,__LINE__,(ptrdiff_t)fd);
       return false;
    }
...
MS_BOOL CMA_Pool_CMA_Heap_Get_User_VA(MS_S32 fd,struct CMA_Pool_Heap_Get_User_Va_Param *vma_valid_Param)
{
    ...
    {
        VPRINTF("cma pool kernel mode get user va fail: heap_id 0x%tX,flags 0x%tX\n", (ptrdiff_t)vma_valid_args.heap_id,(ptrdiff_t)vma_valid_args.flags);
		ret = FALSE;
    }
...
MS_BOOL CMA_Pool_CMA_Mmap_User_VA_Page(MS_S32 fd,struct CMA_Pool_Mmap_User_Va_Page_Param *mmap_user_va_page)
{
    ...
    {
        VPRINTF("cma pool kernel mode mapping user va page fail: heap_id 0x%tX,flags 0x%tX\n", (ptrdiff_t)mmap_user_va_page_args.heap_id,(ptrdiff_t)mmap_user_va_page_args.flags);
		ret = FALSE;
    }
    VPRINTF("%s:%d  ret=%d\n",__FUNCTION__,__LINE__,ret);
    return ret;
...
MS_VIRT __attribute__((weak)) MApi_CMA_Pool_GetKernelCMAPooLUserVA(MS_PHY pAddrPhys,MS_BOOL un_cache_flags)
{
...
*/
    ...
    VPRINTF("%s:%d get_info_param.heap_id=0x%tX\n",__FUNCTION__,__LINE__,(ptrdiff_t)get_info_param.heap_id);
    vma_valid_Param.heap_id = get_info_param.heap_id;
    if(un_cache_flags)
        vma_valid_Param.flags &= ~CMA_FLAG_KERNEL_MODE_USER_SPACE_MAP_CACHED;
    else
        vma_valid_Param.flags |= CMA_FLAG_KERNEL_MODE_USER_SPACE_MAP_CACHED;
    VPRINTF("%s:%d vma_valid_Param.heap_id=0x%tX   vma_valid_Param.flags=0x%tX\n",__FUNCTION__,__LINE__, (ptrdiff_t)vma_valid_Param.heap_id , (ptrdiff_t)vma_valid_Param.flags);
    ret = CMA_Pool_CMA_Heap_Get_User_VA(_s32FdCMAPool,&vma_valid_Param);
    if(ret == FALSE)
    {
        VPRINTF("%s:%d  CMA_Pool_CMA_Heap_Get_User_VA fail  heap_id=0x%tX,flags=0x%tX\n",__FUNCTION__,__LINE__, (ptrdiff_t)vma_valid_Param.heap_id, (ptrdiff_t)vma_valid_Param.flags);
        pAddrVirt=NULL;
        goto CMA_POOL_USER_VA_DONE;
    }
    ...
    else if(NO_USER_VA == (int32_t)vma_valid_Param.user_va_valid_flag)
    {
        ...
        if(un_cache_flags)
			...
			mmap_user_va_page.flags |= CMA_FLAG_KERNEL_MODE_USER_SPACE_MAP_CACHED;
        VPRINTF("%s:%d mmap_user_va_page.heap_id=0x%tX \n",__FUNCTION__,__LINE__,(ptrdiff_t)mmap_user_va_page.heap_id);
        ret = CMA_Pool_CMA_Mmap_User_VA_Page(_s32FdCMAPool,&mmap_user_va_page);
        if(ret == FALSE)
        {
            VPRINTF("%s:%d  CMA_Pool_CMA_Mmap_User_VA_Page fail heap_id=0x%tX,flags=0x%tX\n",__FUNCTION__,__LINE__,
                                             (ptrdiff_t)mmap_user_va_page.heap_id,(ptrdiff_t)mmap_user_va_page.flags);
            pAddrVirt=NULL;
