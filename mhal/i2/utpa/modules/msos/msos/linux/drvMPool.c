...
static MS_U8 _MPOOL_DELAY_BINDING(MS_S32 idx)
{
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%td].u64VirtStart =0x%tX\n", (ptrdiff_t)idx, (ptrdiff_t)mpool_info[idx].u64VirtStart));
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%td].u64VirtEnd = 0x%tX\n", (ptrdiff_t)idx, (ptrdiff_t)mpool_info[idx].u64VirtEnd));
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%td].u64Phyaddr = 0x%tX\n", (ptrdiff_t)idx, (ptrdiff_t)mpool_info[idx].u64Phyaddr));
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%td].u64MpoolSize = 0x%tX\n", (ptrdiff_t)idx, (ptrdiff_t)mpool_info[idx].u64MpoolSize));
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%td].s32V2Poff = 0x%tX\n", (ptrdiff_t)idx, (ptrdiff_t)mpool_info[idx].s32V2Poff));
...
inline MS_PHY MsOS_MPool_VA2PA(MS_VIRT pAddrVirt)
{
	...
	#else
    printf("ERROR!!! CAN'T SUCCESSFULLY CONVERT FROM VA(0x%tX) TO PA(%x)\n", (ptrdiff_t)pAddrVirt,0);
	#endif
    return (MS_PHY) 0xFFFFFFFFUL;
}
inline MS_BOOL MsOS_MPool_IsPA2KSEG1Mapped(MS_PHY pAddrPhys) // un-cache
{
    MS_VIRT pAddrVirt;
    pAddrVirt = MsOS_MPool_PA2KSEG1(pAddrPhys);
    if ( pAddrVirt == 0x0 )
	return FALSE;
    else
	return TRUE;
}
inline MS_BOOL MsOS_MPool_IsPA2KSEG0Mapped(MS_PHY pAddrPhys) // cache
{
    MS_VIRT pAddrVirt;
    pAddrVirt = MsOS_MPool_PA2KSEG0(pAddrPhys);
    if ( pAddrVirt == 0x0 )
	return FALSE;
    else
	return TRUE;
        ...
        for(i = 0; i < MAX_MAPPINGSIZE; i++)
        {
            ...
            if(mpool_info[i].bIsDyn)
            {
    	        ...
    	        if((mpool_info[i].s32V2Poff == 0) && (mpool_info[i].u64VirtStart == 0))
        	    {
        		    if(!_MPOOL_DELAY_BINDING(i))
    	            {
    	                printf("PA2KSEG0 delay binding fail!, pa = 0x%tX", (ptrdiff_t)mpool_info[i].u64Phyaddr);
...
MS_BOOL MsOS_MPool_Mapping_Dynamic(MS_U8 u8MiuSel, MS_SIZE u32Offset, MS_SIZE u32MapSize, MS_U8 u8MapMode)
{
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("u32Offset = 0x%tX, u32MapSize = 0x%tX, u8MapMode = %02x\n", (ptrdiff_t)u32Offset, (ptrdiff_t)u32MapSize, u8MapMode));
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("MPOOL_MAPPING =0x%tX\n",(ptrdiff_t)MPOOL_MAPPING));//coverity MPOOL_MAPPING unused
    return TRUE;
}
...
MS_BOOL MsOS_MPool_UnMapping(MS_VIRT ptrVirtStart, MS_SIZE tMapSize)
{
    ...
    printf("Unmapping Fail !! VA:[0x%tX], Size:[0x%tX].\n", (ptrdiff_t)ptrVirtStart, (ptrdiff_t)tMapSize);
...
MS_BOOL MsOS_MPool_Add_PA2VARange(MS_U64 u64PhysAddr, MS_VIRT u64VirtAddr, MS_SIZE u64MapSize, MS_BOOL bNonCache)
{
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("u64PhysAddr = 0x%tX, u64MapSize = 0x%tX, u64VirtAddr = 0x%tX\n", (ptrdiff_t)u64PhysAddr, (ptrdiff_t)u64MapSize, (ptrdiff_t)u64VirtAddr));
