...
#ifdef CHIP_I2
        INIT_WORK(&_MsOS_EventGroup_Info[u32I].stWorkData.stWork, (void *)irqContext);
#else
        INIT_WORK(&_MsOS_EventGroup_Info[u32I].stWorkData.stWork, (void(*)(unsigned long))irqContext);
#endif
    }
...
//
//workqueue for MsOS_SetEvent
//
static int irqContext(struct work_struct *work)
{
    struct work_data * pstWorkData = container_of(work, struct work_data, stWork);
    MsOS_EventGroup_Info*  pMsOS_EventGroup_Info = container_of(pstWorkData, MsOS_EventGroup_Info, stWorkData);

    spin_lock(&pMsOS_EventGroup_Info->stMutexEvent);
    SET_FLAG(pMsOS_EventGroup_Info->u32EventGroup, pstWorkData->u32EventFlag);
    wake_up_all(&pMsOS_EventGroup_Info->stEventWaitQueue);
    spin_unlock(&pMsOS_EventGroup_Info->stMutexEvent);
...
MS_S32 MsOS_CreateMutex ( MsOSAttribute eAttribute, char *pMutexName1, MS_U32 u32Flag)
{
    ...
    if ((MSOS_MUTEX_MAX == s32Id) && (MSOS_MUTEX_MAX > s32LstUnused))
    {
        ...
        sema_init(&_MsOS_Mutex_Info[s32LstUnused].stSem, 1);
        _MsOS_Mutex_Info[s32LstUnused].owner = NULL;
        s32Id = s32LstUnused;
    }
    MUTEX_MUTEX_UNLOCK();
...
MS_BOOL MsOS_ObtainMutex (MS_S32 s32MutexId, MS_U32 u32WaitMs)
{
    ...
    {
        down(&_MsOS_Mutex_Info[s32MutexId].stSem);
        bRet = TRUE;
    }
    ...
    else //blocking wait with timeout
    {
        ...
        ret = down_timeout(&_MsOS_Mutex_Info[s32MutexId].stSem, msecs_to_jiffies(u32WaitMs));
        if ( ret == 0 )
        {
            bRet = TRUE;
        }
        ...
        {
            printk("[%s][%d] msos mutex %s timeout with %d ms\n", __FUNCTION__, __LINE__, _MsOS_Mutex_Info[s32MutexId].u8Name, u32WaitMs);
        }
        else
        {
            printk("[%s][%d] msos mutex %s err %d\n", __FUNCTION__, __LINE__, _MsOS_Mutex_Info[s32MutexId].u8Name, u32WaitMs);
        }
    ...
    {
        MUTEX_MUTEX_LOCK();
        _MsOS_Mutex_Info[s32MutexId].owner = current;
        MUTEX_MUTEX_UNLOCK();
    }
...
MS_BOOL MsOS_ReleaseMutex (MS_S32 s32MutexId)
{
    ...
    if (_MsOS_Mutex_Info[s32MutexId].owner != current)
    {
        MUTEX_MUTEX_UNLOCK();
        printk("\033[35m[%s][%d] Try to unlock a mutex which locked by others \033[m\n", __func__, __LINE__);
        return FALSE;
    }
    MUTEX_MUTEX_UNLOCK();
...
MS_BOOL MsOS_SetEvent (MS_S32 s32EventGroupId, MS_U32 u32EventFlag)
{
    ...
    if(in_interrupt())
    {
        ...
        _MsOS_EventGroup_Info[s32EventGroupId].stWorkData.u32EventFlag = u32EventFlag;
        queue_work(pstWorkQueue, &_MsOS_EventGroup_Info[s32EventGroupId].stWorkData.stWork);
    }
    else
    {
        EVENT_MUTEX_LOCK();
...
{
    //HAL_IRQ_Detech((MS_U32)eIntNum);
    return TRUE;
}

MS_BOOL MsOS_DetachInterrupt_Shared (InterruptNum eIntNum)
{
    //HAL_IRQ_Detech_Shared((MS_U32)eIntNum);
    return TRUE;
}
...
{
    // Kernel mode utopia driver do not need to call complete interrupt since there is no
    // multi-process condition in kernel mode.
    return TRUE;
}
...
{
    int i;
    MS_BOOL match = FALSE, result = FALSE;

    // skip case size = 0 to avoid register module fail
    if( u32BufSize == 0 )
        return TRUE;;
    ...
    else if (u32Flag == MSOS_SHM_CREATE)
    {
        ...
        *pu32Addr = (MS_VIRT)vmalloc(u32BufSize);
        if(NULL == *pu32Addr)
        {
            SHM_MUTEX_UNLOCK();
            return result;
