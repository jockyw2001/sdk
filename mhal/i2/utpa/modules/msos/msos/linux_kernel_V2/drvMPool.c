...
static MS_BOOL _MPool_isPAmapped(MS_PHY pAddrPhys, MS_BOOL bNonCache);
...
MS_BOOL MsOS_MPool_Init(void)
{
    ...
    if(!MDrv_MMIO_GetBASE((MS_VIRT *)&u32MpoolRegBase, &phySize, MS_MODULE_PM))
    {
...
void MsOS_MPool_InfoMsg(void)
{
    ...
    for (i = 0; i < MAX_MAPPINGSIZE; i++)
    {
        if(mpool_info[i].bIsUsed == true)
            printk("map miu:%d-pa:0x%x with size:0x%x to VA(0x%x ~ 0x%x)\n", mpool_info[i].u8MiuSel, (unsigned int)mpool_info[i].pPhyaddr, (unsigned int)mpool_info[i].u32MpoolSize, mpool_info[i].pVirtStart, mpool_info[i].pVirtEnd);
    }
    printk("======================================================================\n");
    return;
}
...
// below 2 functions for check is mpool already allocate va to this pa
MS_BOOL MsOS_MPool_IsPA2KSEG1Mapped(MS_PHY pAddrPhys)   // non-cache
{
    return _MPool_isPAmapped(pAddrPhys, TRUE);
}
MS_BOOL MsOS_MPool_IsPA2KSEG0Mapped(MS_PHY pAddrPhys)   //cache
{
    return _MPool_isPAmapped(pAddrPhys, FALSE);
}

MS_BOOL _MPool_isPAmapped(MS_PHY pAddrPhys, MS_BOOL bNonCache)
{
    int i, idx, maxID = -1;
    MS_BOOL bRet = FALSE;
    ...
    for(i = 0; i < MAX_MAPPINGSIZE; i++)
    {
        ...
        maxID = i;
        // check if va is already allocated
        if( !( mpool_info[i].pVirtStart == mpool_info[i].pVirtEnd ) )
        {
            bRet = TRUE;
            goto DONE_CHECK;
        }
    ...
    // Debug
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printk("=== MPool Table Print! ===\n"));
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printk("[%s][%d] maxid = %ld\n", __FUNCTION__, __LINE__, (long int)maxID));
    for(idx = 0; idx < MAX_MAPPINGSIZE; idx++)
    {
        if(!mpool_info[idx].bIsUsed)
            continue;
...
#ifdef CHIP_I2
    printk(KERN_EMERG "Chip_Flush_Cache_All() not support\n");
#else
...
MS_BOOL MsOS_MPool_Mapping_Dynamic(MS_U8 u8MiuSel, MS_SIZE u32Offset, MS_SIZE u32MapSize, MS_BOOL bNonCache)
{
    ...
    for (i = 0; i < MAX_MAPPINGSIZE; i++)
    {
        ...
        else
        {
            ...
            if ( (mpool_info[i].pPhyaddr == Phyaddr) && ((mpool_info[i].pPhyaddr + mpool_info[i].u32MpoolSize - 1) == PhyaddrEnd) )
            {
                ret = TRUE;
                printk("[Utopia2K] Duplicated PA mapping.\n");
                MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printk("[Utopia2K] Duplicated PA(0x%lx ~ 0x%lx) mapping with Table[%d](0x%lx ~0x%lx).\n",
                              (long unsigned int)Phyaddr, (long unsigned int)PhyaddrEnd, i, (long unsigned int)mpool_info[i].pPhyaddr, (long unsigned int)(mpool_info[i].pPhyaddr + mpool_info[i].u32MpoolSize - 1)));
...
static MS_VIRT map_kdriver_mem(MS_PHY u32BusStart, MS_SIZE u32MapSize, MS_BOOL bNonCache)
{
    ...
    if(pfn_valid(__phys_to_pfn(u32BusStart & PAGE_MASK)))
    {
        ...
        if(bNonCache)
        {
            ...
            VirtAddr = (MS_VIRT)__va(u32BusStart);
    }
    else
    {
        if(bNonCache)
        ...
        {
#ifdef CHIP_I2
            //VirtAddr = (MS_VIRT)ioremap_cache(u32BusStart, u32MapSize);
            VirtAddr = (MS_VIRT)ioremap(u32BusStart, u32MapSize);
