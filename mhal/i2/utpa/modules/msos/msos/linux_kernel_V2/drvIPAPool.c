...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_Init(struct IPA_Pool_Init_Param * Init_Param)
{
    ...
    if (!_fileIPAPool)
    {
        ...
        {
            printk("%s:%d open /dev/ipapool fail  PTR_ERR_fp=%ld\n",__FUNCTION__,__LINE__,PTR_ERR(_fileIPAPool));
            ret =  FALSE;
...
MS_BOOL __attribute__((weak)) MApi_IPA_Pool_InstallIpcHandle(struct IPA_Pool_InstallIpcHandle_Param* installipchandle_param)
{
    ...
    if ( !_fileIPAPool || IS_ERR(_fileIPAPool))
    {
        ...
        if (IS_ERR(_fileIPAPool) )
        {
            printk("%s:%d open /dev/ipapool fail   ipc_handle_id=0x%x,   PTR_ERR_fp=%ld\n",__FUNCTION__,__LINE__, installipchandle_param->ipc_handle_id,,PTR_ERR(_fileIPAPool));
            ret =  FALSE;
