...
MS_VIRT                               _virtVDEC_Bank               = 0x0 ;
MS_PHY                              _u32VDEC_BankSize           = 0x0 ;

MS_VIRT                               _virtEXTRIU_Bank                = 0x0 ;
MS_PHY                              _u32EXTRIU_Bank_SIZE           = 0x0 ;
#else
...
MS_PHY                              _virtVDEC_Bank               = 0x0 ;
MS_PHY                              _u32VDEC_BankSize           = 0x0 ;

MS_PHY                              _virtEXTRIU_Bank                = 0x0 ;
MS_PHY                              _u32EXTRIU_Bank_SIZE           = 0x0 ;
...
MS_BOOL MDrv_MMIO_Init(void)
{
    ...
    if (FALSE == HAL_MMIO_GetBase(&EXTRIU_Bank, &EXTRIU_Bank_SIZE, DRV_MMIO_EXTRIU_BANK))
    {
        MS_ASSERT(0);
    }
    ...
    if (EXTRIU_Bank_SIZE)
    {
        stMIOMapInfo.virtAddr = EXTRIU_Bank ;
        stMIOMapInfo.u32Size = EXTRIU_Bank_SIZE ;
        if (ioctl(_s32MIOMapFd, MMIO_IOC_SET_MAP , &stMIOMapInfo))
        {
            return FALSE;
        }
        if ((MS_VIRT)MAP_FAILED == (_virtEXTRIU_Bank= (MS_VIRT)mmap(0, stMIOMapInfo.u32Size, PROT_READ| PROT_WRITE, MAP_SHARED, _s32MIOMapFd, 0)))
        {
            MS_ASSERT(0);
            return FALSE;
        }
        _u32EXTRIU_Bank_SIZE  = EXTRIU_Bank_SIZE ;
        MMIO_PRINT("EXTRIU Bank mapping success. Base=[%lx], Size=[%x]\n",(MS_VIRT) _virtEXTRIU_Bank, (MS_PHY)_u32EXTRIU_Bank_SIZE);
    }
...
//        MMIO_PRINT("NSK Bank mapping success. Base=[%lx], Size=[%x]\n", (MS_VIRT)_u32NSK_Bank, (MS_PHY)_u32NSK_BankSize);
    ...
    // VDEC SRAM
    if (HAL_MMIO_GetBase(&bank_addr, &bank_size, DRV_MMIO_VDEC_BANK))
    {
        stMIOMapInfo.virtAddr = bank_addr;
        stMIOMapInfo.u32Size = bank_size;
        if (ioctl(_s32MIOMapFd, MMIO_IOC_SET_MAP, &stMIOMapInfo))
        ...
        _virtVDEC_Bank = (MS_VIRT)mmap(0, stMIOMapInfo.u32Size, PROT_READ | PROT_WRITE, MAP_SHARED, _s32MIOMapFd, 0);
        if ((MS_VIRT)MAP_FAILED == _virtVDEC_Bank)
        {
            _virtVDEC_Bank = 0x0;
            MS_ASSERT(0);
            return FALSE;
        }
        _u32VDEC_BankSize = bank_size;
        MMIO_PRINT("VDEC Bank mapping success. Base=[%lx], Size=[%x]\n", (MS_VIRT)_virtVDECBank, (MS_PHY)_u32VDEC_BankSize);
    }
...
MS_BOOL MDrv_MMIO_Close(void)
{
    ...
    munmap((void *)_virtEXTRIU_Bank, _u32EXTRIU_Bank_SIZE);
    close(_s32MIOMapFd);
    _s32MIOMapFd = -1;
    return TRUE;
}
...
MS_BOOL MDrv_MMIO_GetBASE(MS_VIRT *virtBaseaddr, MS_PHY *u32Basesize, MS_U32 u32Module)
{
    ...
    if (u16Type == DRV_MMIO_EXTRIU_BANK)
    {
        *virtBaseaddr = _virtEXTRIU_Bank;
        *u32Basesize = _u32EXTRIU_Bank_SIZE;
        return TRUE ;
    }

    if (((u16Type == DRV_MMIO_VDEC_BANK) && (_u32VDEC_BankSize))) // not support if BankSize = 0
    {
        *virtBaseaddr = _virtVDEC_Bank;
        *u32Basesize = _u32VDEC_BankSize;
        return TRUE ;
    }
...
MS_BOOL MDrv_MMIO_Init(void)
{
    ...
    HAL_MMIO_GetBase(&_virtEXTRIU_Bank,   &_u32EXTRIU_Bank_SIZE,   DRV_MMIO_EXTRIU_BANK);
	HAL_MMIO_GetBase(&_virtVDEC_Bank,    &_u32VDEC_BankSize,     DRV_MMIO_VDEC_BANK);
...
MS_BOOL MDrv_MMIO_GetBASE(MS_VIRT *virtBaseaddr, MS_PHY *u32Basesize, MS_U32 u32Module)
{
    ...
    case DRV_MMIO_EXTRIU_BANK:
		bRet = HAL_MMIO_GetBase(&virtBasetemp, &pu32Basetemp, DRV_MMIO_EXTRIU_BANK);
		*virtBaseaddr = virtBasetemp;
		*u32Basesize = pu32Basetemp;
        break;
