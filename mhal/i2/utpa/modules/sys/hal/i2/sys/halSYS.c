...
MS_BOOL HAL_SYS_SetPadMux(SYS_PAD_MUX_SET ePadMuxType,SYS_PAD_SEL ePadSel)
{
    ...
    if(ePadMuxType == E_TS0_PAD_SET)
    {
        ...
        else if(ePadSel == E_MSPI_PAD_ON)
        {
            ...
            u16data = HAL_SYS_Read2Byte(REG_I2CCM_MODE_OFFSET) & (~(REG_FUART_MODE_MASK<<REG_FUART_MODE_SHIFT));
            HAL_SYS_Write2Byte(REG_I2CCM_MODE_OFFSET, u16data);

            u16data = HAL_SYS_Read2Byte(REG_MCU_JTAG_MODE_OFFSET) & (~(REG_MSPI1_MODE1_MASK<<REG_MSPI1_MODE1_SHIFT));
            u16data |= REG_MSPI1_MODE1_TS0<<REG_MSPI1_MODE1_SHIFT;
    ...
    else if(ePadMuxType == E_TS2_PAD_SET)
    {
        ...
        {
            u16data |= REG_TS_MODE_1S<<REG_TS2_MODE_SHIFT;
            HAL_SYS_Write2Byte(REG_TS_MODE_OFFSET, u16data);
        }
        else if(ePadSel == E_MSPI_PAD_ON)
        {
...
MS_BOOL HAL_SYS_Query(E_SYS_QUERY id)
{
    ...
    switch (id)
    {
        ...
        case E_SYS_QUERY_DD_SUPPORTED:
        {
            MS_U32 u32OTPValue = 0;
            u32OTPValue = (u32OTP_0x3D0C_shadow >> 0) & 0x1;
            bRet = (u32OTPValue == 1)?FALSE:TRUE;
            break;
        }
        case E_SYS_QUERY_DDP_SUPPORTED:
        {
            MS_U32 u32OTPValue = 0;
            u32OTPValue = (u32OTP_0x3D0C_shadow >> 1) & 0x1;
            bRet = (u32OTPValue == 1)?FALSE:TRUE;
            break;
        }
        default:
