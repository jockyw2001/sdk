...
static MS_U8            u8IsInit = 0;
static pthread_mutex_t  _SYS_Global_Init_Mutex = PTHREAD_MUTEX_INITIALIZER;
...
#if 0
extern TEEC_Result TEEC_InitializeContext(const char *name, TEEC_Context *context);
extern void TEEC_FinalizeContext(TEEC_Context *context);
extern TEEC_Result TEEC_OpenSession(TEEC_Context *context,
...
extern void TEEC_CloseSession(TEEC_Session *session);
extern TEEC_Result TEEC_InvokeCommand(TEEC_Session *session,
			       uint32_t commandID,
			       TEEC_Operation *operation,
			       uint32_t *returnOrigin);
/*
...
{
	return TEEC_OpenSession(context, session, destination, connection_method, connection_data, operation, error_origin);
}
...
{
	return TEEC_InvokeCommand(session, cmd_id, operation, error_origin);
}
#else
MS_U32 MDrv_SYS_TEEC_InitializeContext(const char *name, TEEC_Context *context)
{
...
MS_U32 MDrv_SYS_TEEC_InitializeContext(const char *name, TEEC_Context *context)
{
	return _MDrv_SYS_TEEC_InitializeContext(name, context);
}
...
MS_U32 MDrv_SYS_TEEC_Open(TEEC_Context *context, TEEC_Session *session, const TEEC_UUID *destination, MS_U32 connection_method, const void *connection_data, TEEC_Operation *operation, MS_U32 *error_origin)
{
	return _MDrv_SYS_TEEC_Open(context, session, destination, connection_method, connection_data, operation, error_origin);
}
...
void MDrv_SYS_TEEC_FinalizeContext(TEEC_Context *context)
{
	_MDrv_SYS_TEEC_FinalizeContext(context);
}
...
MS_U32 MDrv_SYS_TEEC_InvokeCmd(TEEC_Session *session, MS_U32 cmd_id, TEEC_Operation *operation, MS_U32 *error_origin)
{
	return _MDrv_SYS_TEEC_InvokeCmd(session, cmd_id, operation, error_origin);
}
#endif
...
void MDrv_SYS_GetSoftwareVersion(MS_SW_VERSION_INFO *pSoftwareVersionInfo)
{
    ...
    sprintf(pSoftwareVersionInfo->UtopiaBspVersion,"%d", UTOPIA_BSP_VERSION);
#if 0
    snprintf(
    ...
    sprintf(pSoftwareVersionInfo->ChangeList_API, "%d", SW_VERSION_API_SYS);
#if 0
    snprintf(
    ...
    sprintf(pSoftwareVersionInfo->ChangeList_DRV,"%d", SW_VERSION_DRV_SYS);
#if 0
    snprintf(
    ...
    sprintf(pSoftwareVersionInfo->ChangeList_HAL, "%d", SW_VERSION_HAL_SYS);
#if 0
    snprintf(
...
MS_BOOL MDrv_SYS_ReadHDCPKey(MS_U8 *u8HDCPKey)
{
    MS_U16  efuse_addr  = 0;
    MS_U32  efuse_data  = 0;
    MS_U16  rdata_lo    = 0;
    MS_U16  rdata_hi    = 0;
    while(efuse_addr < HDCP_KEY_SIZE)
    {
        if (!HAL_SYS_ReadEfuseHDCPKey((efuse_addr/4), &efuse_data))
        {
            return FALSE;
        }
        rdata_lo = (MS_U16) (efuse_data & 0xffff);
        rdata_hi = (MS_U16) ((efuse_data >> 16) & 0xffff);

        if (efuse_addr < HDCP_KEY_SIZE)
            u8HDCPKey[efuse_addr++] = (rdata_lo >> 8) & 0xff;
        if (efuse_addr < HDCP_KEY_SIZE)
            u8HDCPKey[efuse_addr++] = rdata_lo & 0xff;
        if (efuse_addr < HDCP_KEY_SIZE)
            u8HDCPKey[efuse_addr++] = (rdata_hi >> 8) & 0xff;
        if (efuse_addr < HDCP_KEY_SIZE)
            u8HDCPKey[efuse_addr++] = rdata_hi & 0xff;
    }
    return TRUE;
}
...
void MDrv_SYS_ReadKernelCmdLine(void)
{
    ...
    if(fd<0)
    {
    		SYS_DBG_ERR("Open failed! Try to open /proc/system_cmdline \n");
      		fd = open("/proc/system_cmdline",O_RDONLY);
		if(fd<0)
		{
			SYS_DBG_ERR("Open /proc/system_cmdline failed!\n");
                  	cmdline[0] = 0;
			goto fail_open;
		}
     }
     n = read(fd, cmdline, 1023);
     if (n < 0) n = 0;
