...
HAL_DSCMB_KL_Dst _HAL_DSCMB_GetKLDst(DSCMB_KLDst eDst)
{
    switch (eDst)
    {
        ...
        case E_DSCMB_KL_DST_TSIO:
            return E_HAL_DSCMB_KL_DST_TSIO;
...
HAL_DSCMB_KL_Src _HAL_DSCMB_GetKLSrc(DSCMB_KLSrc eSrc)
{
    switch (eSrc)
    {
        ...
        case E_DSCMB_KL_SRC_PRIVATE_EX_0:
            return E_HAL_DSCMB_KL_SRC_SK8;
        case E_DSCMB_KL_SRC_PRIVATE_EX_1:
            return E_HAL_DSCMB_KL_SRC_SK9;
        case E_DSCMB_KL_SRC_PRIVATE_EX_2:
            return E_HAL_DSCMB_KL_SRC_SK10;
        case E_DSCMB_KL_SRC_PRIVATE_EX_3:
            return E_HAL_DSCMB_KL_SRC_SK11;
...
{
    eKeyType &= ~(E_DSCMB_KEY_FSCB_ENABLE|E_DSCMB_KEY_SECURE_KEYS_ENABLE);
    switch (eKeyType)
    {
...
HAL_DSCMB_MainAlgo_Type _HAL_DSCMB_GetAlgoType(DSCMB_MainAlgo_Type type)
{
    switch (type)
    {
        ...
        case E_DSCMB_MAIN_ALGO_ESSA:
            return E_HAL_DSCMB_MAIN_ALGO_ESSA;
        default:
...
{
    *((volatile MS_U32*)(_u32RegBase + ((u32Reg)<< 2))) = u32Value;
    HALDSCMB2_DBG(DSCMB2_DBGLV_ARRAY, "REG32_W(0x%08X, 0x%08X)\n", (MS_U32)(_u32RegBase + ((u32Reg)<< 2)), u32Value);
}

static MS_U32 REG32_R(MS_VIRT u32Reg)
{
    return *((volatile MS_U32*)(_u32RegBase + ((u32Reg)<< 2)));
}
...
void _HAL_DSCMB_UppPidFLT_EN(HAL_DSCMB_OutPut_Eng_Sel EngId, MS_BOOL bEnable)
{
    MS_U32 u32Disable = (EngId == E_HAL_DSCMB_CIPHER_OUTPUT_SPS0)?REG_TSCE_UPP_PATH0_PID_FLT_DISABLE:REG_TSCE_UPP_PATH1_PID_FLT_DISABLE;
    MS_U32 u32Tmp = REG32_R(REG_TSCE_CTRL);

    if(bEnable)
    {
        REG32_W(REG_TSCE_CTRL, u32Tmp & ~u32Disable);
    }
    else
    {
        REG32_W(REG_TSCE_CTRL, u32Tmp | u32Disable);
...
MS_BOOL HAL_DSCMB_Init(void)
{
    ...
    // disable the upper path pid filter
    _HAL_DSCMB_UppPidFLT_EN(E_HAL_DSCMB_CIPHER_OUTPUT_SPS0, FALSE);
    _HAL_DSCMB_UppPidFLT_EN(E_HAL_DSCMB_CIPHER_OUTPUT_SPS1, FALSE);
...
MS_BOOL _HAL_DSCMB_PidIdx_MuxRead(MS_U32 u32fltid, MS_U32 *u32rdata )
{
    ...
    // read  data
    u32rdata[0] = REG32_R(REG_PIDIDX_READ_DATA);
    u32rdata[1] = REG32_R(REG_PIDIDX_READ_DATA + 1);
    return TRUE ;
...
{
    HALDSCMB2_DBG(DSCMB2_DBGLV_DEBUG, "Set PidSlotMap Entry=%u, bSecure=%d\n",u32fltid, bSecure);
    return _HAL_DSCMB_PidIdx_MuxWrite(u32fltid , REG_PIDIDX_WMUX_SEC , bSecure );
}

MS_BOOL HAL_DSCMB_PidIdx_SetPrivilege(MS_U32 u32fltid , MS_BOOL bEnable )
{
    HALDSCMB2_DBG(DSCMB2_DBGLV_DEBUG, "Set PidSlotMap Entry=%u, bEnable=%d\n",u32fltid, bEnable);
    return _HAL_DSCMB_PidIdx_MuxWrite(u32fltid , REG_PIDIDX_WMUX_PRV , bEnable );
}
...
MS_BOOL _HAL_DSCMB_KTE_Write_Ex(MS_U32 u32fltid, DSCMB_Key_Type type, MS_U32 u32Field, MS_U32 u32Cmd, MS_U32 u32CAVid,MS_U32 *u32data, MS_BOOL bIsKL)
{
    MS_U32 u32Tmp  = 0;
    MS_U32 u32Alog = 0;
    MS_BOOL bSecure = ((type & E_DSCMB_KEY_SECURE_KEYS_ENABLE) != 0);
    HAL_DSCMB_FSCB _eFSCB = E_HAL_DSCMB_FSCB_UNCHG;
    type &= ~E_DSCMB_KEY_SECURE_KEYS_ENABLE;
    ...
    //Enable secure pidslot
    if(bSecure)
    {
        u32Tmp |= REG_KTE_TO_SECURE_SLOT;
        HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Enable to secure pidslot\n");
    }
...
MS_BOOL HAL_DSCMB_KTE_Read_Switch(MS_U32 u32fltid, DSCMB_Key_Type type, MS_U32 u32CAVid, MS_U32 *u32data)
{
    ...
    u32Tmp = REG_KTE_CMD_READSW | REG_KTE_AKI_GO | ((u32CAVid<<REG_KTE_CAVID_SHT)&REG_KTE_CAVID_MSK );
    //Enable secure pidslot
    if(bSecure)
    {
        u32Tmp |= REG_KTE_TO_SECURE_SLOT;
...
MS_BOOL HAL_DSCMB_KTE_GetStatus(MS_U32 u32FltId ,DSCMB_Key_Type eKeyType, MS_U32 *KeyStatus)
{
    ...
    MS_U32 u32Tmp = 0 ;
    MS_U32 u32StatNum = 0;
    MS_U32 u32ReadData[2]={0,0};

    PidSlotMapRead_t* pReadData0 = (PidSlotMapRead_t*)u32ReadData;
    //PidSlotMapRead2_t* pReadData1 = (PidSlotMapRead2_t*)&u32ReadData[1];
    _HAL_DSCMB_PidIdx_MuxRead(u32FltId, u32ReadData);

    // Filter specific key slot info //
    switch (eKeyType)
    {
        case E_DSCMB_KEY_CLEAR:
            u32KeyIdx = pReadData0->ClrSlotIndex;
            break;
        case E_DSCMB_KEY_EVEN:
            u32KeyIdx = pReadData0->EvenSlotIndex;
            break;
        case E_DSCMB_KEY_ODD:
            u32KeyIdx = pReadData0->OddSlotIndex;
            break;
        default:
            HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s][%d]  FAIL\n",__func__, __LINE__);
            return FALSE ;
    ...
    u32Status |= pReadData0->DualSloEn?DSCMB_KTE_DUAL_EN:0;
    u32Status |= pReadData0->PidSlotEn?DSCMB_PIDIDX_EN:0;
...
#define DELAY_TIME 10
    MS_U32 u32Value = REG32_R(REG_KTE_HW_SEM);
    MS_U32 u32Tmp = 0;
    MS_U32 u32Cnt = u32WaitMs/DELAY_TIME;
    MS_U32 i = 0;
    u32Value &=  ~(REG_KTE_XIU_ID_MSK | REG_KTE_XIU_NS_MSK);
    u32Value |= (DSCMB_XIU_ID << REG_KTE_XIU_ID_SHT) & REG_KTE_XIU_ID_MSK;
    u32Value |= (DSCMB_XIU_NS << REG_KTE_XIU_NS_SHT) & REG_KTE_XIU_NS_MSK;

    while(i <= u32Cnt)
    {
        REG32_W(REG_KTE_HW_SEM, u32Value);
        u32Tmp = REG32_R(REG_KTE_HW_SEM);
        HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Try Lock 0x%X, Read 0x%X\n",(unsigned int)u32Value, (unsigned int)u32Tmp);

        if((u32Tmp & (REG_KTE_XIU_ID_MSK | REG_KTE_XIU_NS_MSK)) ==
            (u32Value & (REG_KTE_XIU_ID_MSK | REG_KTE_XIU_NS_MSK)))
        {
            return TRUE;
        }
        ...
        {
            HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Lock Fail, retry");
            MsOS_DelayTask (DELAY_TIME);
            i ++;
        }
    }
    return FALSE;
...
MS_BOOL HAL_DSCMB_KTE_HW_SEM_Unlock(void)
{
    ...
    u32Value = REG32_R(REG_KTE_HW_SEM);
    u32Value &= (REG_KTE_XIU_ID_MSK | REG_KTE_XIU_NS_MSK);
    if(u32Value != 0)
    {
        HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Unlock Fail 0x%X\n", REG32_R(REG_KTE_HW_SEM));
        return FALSE;
    }
    HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Unlock Ok 0x%X\n", REG32_R(REG_KTE_HW_SEM));
...
{
    if(u32EngId >= REG_DSCMB_CIPHER_CNT)
    {
        HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s][%d]Invalid EngId = %d\n",__func__, __LINE__, u32EngId);
        return FALSE;
    }

    if((NULL == pu8StaticKey) || (REG_CIPHER_ES_KEY_LEN != u32KeyLen))
    {
        HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s][%d]Key is NULL or inlavid key length = [%d]\n",__func__, __LINE__, u32KeyLen);
        return FALSE;
    }
    ...
    //Set Key
    REG32_W(REG_CIPHER_ES_SWKEY(u32EngId)+0,(pu8StaticKey[12] <<24)|(pu8StaticKey[13] <<16)|(pu8StaticKey[14] <<8)|(pu8StaticKey[15] <<0));
    REG32_W(REG_CIPHER_ES_SWKEY(u32EngId)+1,(pu8StaticKey[8]  <<24)|(pu8StaticKey[9]  <<16)|(pu8StaticKey[10] <<8)|(pu8StaticKey[11] <<0));
    REG32_W(REG_CIPHER_ES_SWKEY(u32EngId)+2,(pu8StaticKey[4]  <<24)|(pu8StaticKey[5]  <<16)|(pu8StaticKey[6]  <<8)|(pu8StaticKey[7]  <<0));
    REG32_W(REG_CIPHER_ES_SWKEY(u32EngId)+3,(pu8StaticKey[0]  <<24)|(pu8StaticKey[1]  <<16)|(pu8StaticKey[2]  <<8)|(pu8StaticKey[3]  <<0));

    //Triger
    REG32_W(REG_CIPHER_ES_START(u32EngId), 1);

    //Polling
    while(1)
    {
        u32Status = REG32_R(REG_CIPHER_ES_STATUS(u32EngId));
        if((u32Status & REG_CIPHER_ES_SET_KEY_DONE) != 0)   break;
        HALDSCMB2_DBG(DSCMB2_DBGLV_DEBUG, "[%s][%d]Waiting set key to es\n",__func__, __LINE__);
    }

    HALDSCMB2_DBG(DSCMB2_DBGLV_DEBUG, "[%s][%d]Set key to es superxor done\n",__func__, __LINE__);
...
MS_U32 HAL_DSCMB_GetPidFLTOffset(HAL_DSCMB_OutPut_Eng_Sel EngId)
{
    switch (EngId)
    {
        case E_HAL_DSCMB_CIPHER_OUTPUT_SPS0:
            return REG_TSCE_PVRPID_PROG0;
        case E_HAL_DSCMB_CIPHER_OUTPUT_SPS1:
            return REG_TSCE_PVRPID_PROG1;
        default:
            HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s][%d] Unknow CAPVR Program!!!\n",__FUNCTION__, __LINE__);
            break;
    }
...
MS_BOOL HAL_DSCMB_KL_Reset(DSCMB_KL_SelEng eKLEng)
{
    ...
    u32Tmp = REG32_R(REG_DSCMB_KL_SWRST(u32KLEng));
    u32Tmp |= REG_DSCMB_KL_RST;
    REG32_W(REG_DSCMB_KL_SWRST(u32KLEng), u32Tmp);
    u32Tmp &= ~REG_DSCMB_KL_RST;
        ...
        if((u32ErrMsg & (u32ErrorFlag << i)) >> i)
        {
            switch(i)
            {
                ...
                case E_HAL_DSCMB_KL_ERR_MSG_DMA:
                    HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Key forbidden using for DMA/MAC/IMLB/CHK/CFB\n");
                    break;
                case E_HAL_DSCMB_KL_ERR_MSG_TSIO:
                    HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Key forbidden using for TSIO\n");
                    break;
                case E_HAL_DSCMB_KL_ERR_MSG_UACPU:
                    HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Key forbidden using for reg_cw_sw_cw\n");
                    break;
                case E_HAL_DSCMB_KL_ERR_MSG_SEP:
                    HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Key forbidden using for reg_cw_secr2_private_key\n");
                    break;
                case E_HAL_DSCMB_KL_ERR_MSG_NONCE:
                    HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Key forbidden using for  NONCE/TA/XOR\n");
                    break;
...
MS_BOOL HAL_DSCMB_KL_KDF_Busy(DSCMB_KL_SelEng eKLEng, MS_BOOL *pbBusy)
{
    ...
    u32Tmp = REG32_R(REG_DSCMB_KL_KDF_STATUS(u32KLEng));
    if((u32Tmp & REG_DSCMB_KL_KDF_BUSY) == 0)
    {
        *pbBusy = FALSE;
    }
    else
...
#define DELAY_TIME 10
    MS_U32 u32KLEng = _HAL_DSCMB_GetKLEng(eKLEng);
    MS_U32 u32Value = REG32_R(REG_DSCMB_KL_HW_SEM(u32KLEng));
    MS_U32 u32Tmp = 0;
    MS_U32 u32Cnt = u32WaitMs/DELAY_TIME;
    MS_U32 i = 0;
    u32Value &=  ~(REG_DSCMB_KL_XIU_ID_MSK | REG_DSCMB_KL_XIU_NS_MSK);
    u32Value |= (DSCMB_XIU_ID << REG_DSCMB_KL_XIU_ID_SHT) & REG_DSCMB_KL_XIU_ID_MSK;
    u32Value |= (DSCMB_XIU_NS << REG_DSCMB_KL_XIU_NS_SHT) & REG_DSCMB_KL_XIU_NS_MSK;

    while(i <= u32Cnt)
    {
        REG32_W(REG_DSCMB_KL_HW_SEM(u32KLEng), u32Value);
        u32Tmp = REG32_R(REG_DSCMB_KL_HW_SEM(u32KLEng));
        HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Try Lock 0x%X, Read 0x%X\n",(unsigned int)u32Value, (unsigned int)u32Tmp);

        if((u32Tmp & (REG_DSCMB_KL_XIU_ID_MSK | REG_DSCMB_KL_XIU_NS_MSK)) ==
            (u32Value & (REG_DSCMB_KL_XIU_ID_MSK | REG_DSCMB_KL_XIU_NS_MSK)))
        {
            return TRUE;
        }
...
MS_BOOL HAL_DSCMB_KL_HW_SEM_Unlock(DSCMB_KL_SelEng eKLEng)
{
    ...
    u32Value = REG32_R(REG_DSCMB_KL_HW_SEM(u32KLEng));
    u32Value &= (REG_DSCMB_KL_XIU_ID_MSK | REG_DSCMB_KL_XIU_NS_MSK);
    if(u32Value != 0)
    {
        HALDSCMB2_DBG(DSCMB2_DBGLV_INFO, "Unlock Fail 0x%X\n", REG32_R(REG_DSCMB_KL_HW_SEM(u32KLEng)));
        return FALSE;
    }
...
void HAL_DSCMB_KL_DstSecure(DSCMB_KL_SelEng eKLEng, MS_BOOL bDstSecure)
{
    ...
    if(bDstSecure)
    {
        u32Tmp |= REG_DSCMB_KL_CTRL_SECURE_EN;
    }
    else
    {
        u32Tmp &= ~REG_DSCMB_KL_CTRL_SECURE_EN;
...
MS_BOOL HAL_DSCMB_CAModule_SetPwd(DSCMB_CAMod_ActType ePwdType, MS_U8 *pu8Pwd, DSCMB_CAMOD_STATUS *u32Status)
{
    HALDSCMB2_DBG(DSCMB2_DBGLV_WARNING, "Not support!!!\n");

    return TRUE;
}

MS_BOOL _HAL_DSCMB_CAPVR_SetPidFLT(MS_U32 u32Pid, MS_U32 u32Offset, MS_BOOL bSet)
{
    MS_U32 u32Tmp = 0;

    if(bSet)
    {
        u32Tmp |= REG_TSCE_PVRPID_EN;
    }
    u32Tmp |= u32Pid & REG_TSCE_PVRPID_PID_MSK;

    REG32_W(REG_DSCMB_PVRPID_CTRL(u32Offset), u32Tmp);

    return FALSE;
}

MS_BOOL _HAL_DSCMB_CAPVR_IsPidDuplicate(HAL_DSCMB_OutPut_Eng_Sel EngId, MS_U32 u32Pid, MS_U32* u32Idx)
{
    MS_U32 u32i;
    MS_U32 u32Tmp = 0;
    MS_U32 u32Offset = HAL_DSCMB_GetPidFLTOffset(EngId);

    for(u32i = 0; u32i < HAL_DSCMB_SPSPVR_FLT_NUM; u32i++)
    {
        *u32Idx =  u32i;
        u32Tmp = REG32_R(REG_DSCMB_PVRPID_CTRL(u32Offset + u32i));
        if(REG_TSCE_PVRPID_EN == (u32Tmp & REG_TSCE_PVRPID_EN))
        {
            if(u32Pid == (u32Tmp & REG_TSCE_PVRPID_PID_MSK))
                return TRUE;
        }
    }
...
MS_BOOL _HAL_DSCMB_CAPVR_AllocPidFLT(HAL_DSCMB_OutPut_Eng_Sel EngId, MS_U32 u32Pid)
{
    MS_U32 u32i;
    MS_U32 u32Offset = HAL_DSCMB_GetPidFLTOffset(EngId);
    ...
    for(u32i = 0; u32i < HAL_DSCMB_SPSPVR_FLT_NUM; u32i++)
    {
        if(REG_TSCE_PVRPID_EN != (REG32_R(REG_DSCMB_PVRPID_CTRL(u32Offset + u32i)) & REG_TSCE_PVRPID_EN))
        {
            _HAL_DSCMB_CAPVR_SetPidFLT(u32Pid, u32Offset + u32i, TRUE);
            break;
        }
    }

    if(u32i == HAL_DSCMB_SPSPVR_FLT_NUM)
    {
        HALDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Pid Fliter is full!!!\n");
        return FALSE;
    }

    _HAL_DSCMB_UppPidFLT_EN(EngId, TRUE);

    return TRUE;
}

void _HAL_DSCMB_CAPVR_CleanPidFLT(HAL_DSCMB_OutPut_Eng_Sel EngId, MS_U32 u32Idx)
{
    MS_U32 u32i;
    MS_U32 u32Offset = HAL_DSCMB_GetPidFLTOffset(EngId);

    _HAL_DSCMB_CAPVR_SetPidFLT(0, u32Offset + u32Idx, FALSE);
    ...
    _HAL_DSCMB_UppPidFLT_EN(EngId, FALSE);
}

MS_BOOL HAL_DSCMB_CAPVR_SetPidFLT(HAL_DSCMB_OutPut_Eng_Sel EngId, MS_U32 u32Pid, MS_BOOL bSet)
{
    MS_U32 u32Idx = 0;

    if(_HAL_DSCMB_CAPVR_IsPidDuplicate(EngId, u32Pid, &u32Idx))
    {
        if(bSet)
        {
            HALDSCMB2_DBG(DSCMB2_DBGLV_WARNING, "Pid Fliter has already been set!!!\n");
            return TRUE;
        }
        else
        {
            HALDSCMB2_DBG(DSCMB2_DBGLV_WARNING, "Clean Pid Fliter[%u]  Pid=0x%d\n", (unsigned int)u32Idx, u32Pid);
            _HAL_DSCMB_CAPVR_CleanPidFLT(EngId, u32Idx);
            return TRUE;
        }
    }
    ...
    if(bSet)
    {
        return _HAL_DSCMB_CAPVR_AllocPidFLT(EngId, u32Pid);
    }
                            ...
                            DSCMB_Key_Type type,  MS_U32 *u32data)
{
    ...
    if(bSecure)
    {
        u32Reg0 |= REG_KTE_TO_SECURE_SLOT;
    }
    //write reg 1
