...
static void HAL_NSK2_DeadPolling(void)
{
    ...
    //read ACPU Error....
    HALNSK2_DBG(NSK2_DBGLV_INFO,"ACPU Error = %x\n", HAL_NSK2_ReadReg(REG_NSK2_ACPU_ERROR));
#endif
}
...
[15:0]	    V_PubOtpOID	         Owner ID, 0x3b00
    ...
    if(OTP_PubOtpGP != CHK_PubOtpGP)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpGP = (%x, %x)\n",CHK_PubOtpGP,OTP_PubOtpGP);
        status = FALSE;
    }
    ...
    if(OTP_PubOtpMinConfVer != CHK_PubOtpMinConfVer)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpMinConfVer = (%x, %x)\n",CHK_PubOtpMinConfVer,OTP_PubOtpMinConfVer);
        status = FALSE;
    }
    ...
    if(OTP_PubOtpRsaIndex != CHK_PubOtpRsaIndex)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpRsaIndex = (%x, %x)\n",CHK_PubOtpRsaIndex,OTP_PubOtpRsaIndex);
        status = FALSE;
    }
    ...
    if(OTP_PubOtpBID != CHK_PubOtpBID)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpBID = (%x, %x)\n",CHK_PubOtpBID,OTP_PubOtpBID);
        status = FALSE;
    }
    ...
    if(OTP_PubOtpVID != CHK_PubOtpVID)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpVID = (%x, %x)\n",CHK_PubOtpVID,CHK_PubOtpVID);
        status = FALSE;
    }
    ...
    if(OTP_PubOtpOID != CHK_PubOtpOID)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_ERR,"OtpOID = (%x, %x)\n",CHK_PubOtpOID,OTP_PubOtpOID);
        status = FALSE;
    }
...
static ASST_t K6Asst[] =
{
    ...
    {0x5A,  1,  0x3C60,   3,   2}, //U_OTP_OBFUSCATEVideoStream
    {0x5B,  1,  0x3C60,   1,   1}, //U_OTP_MOBF_TOP_use_DES
    {0x5C,  1,  0x3C74,  11,   8}, //U_OTP_ena_PlayBackRec0
    {0x5D,  1,  0x3C74,  15,  12}, //U_OTP_ena_PlayBackRec1
    {0x5E,  1,  0x3C74,  19,  16}, //U_OTP_ena_PlayBackRec2
    {0x5F,  1,  0x3C74,  23,  20}, //U_OTP_ena_PlayBackRec3
    {0x60,  1,  0x3C78,   3,   0}, //U_OTP_ena_PVR_secure_protect_0
    {0x61,  1,  0x3C78,  11,   8}, //U_OTP_ena_TSO
    {0x62,  1,  0x3C80,  17,  16}, //U_OTP_forbid_SW_SPSD_Key
    {0x63,  1,  0x3C80,  21,  20}, //U_OTP_allow_TSPCPUCodeProt
    {0x64,  1,  0x3CC0,  13,  12}, //U_OTP_allow_RANDOM
    {0x65,  1,  0x3CC8,   3,   0}, //U_OTP_SBoot
    {0x66,  1,  0x3CC8,   7,   4}, //U_OTP_SecretAreaEnable
    {0x67,  2,  0x3CCC,   8,   0}, //U_OTP_SCAN_MODE
    {0x68,  1,  0x3CCC,  15,  10}, //U_OTP_MBIST_MODE
    {0x69,  1,  0x3CCC,  21,  16}, //U_OTP_I2C_MODE
    {0x6A,  1,  0x3CCC,  31,  26}, //U_OTP_EJTAG_MODE
    {0x6B,  1,  0x3C68,   3,   0}, //U_OTP_forbid_USBSlaveMode
    {0x6C,  1,  0x3CD0,   9,   8}, //U_OTP_allow_DRAM_MOBF
    {0x6D,  1,  0x3CD4,  15,  14}, //U_OTP_forbid_BCKGND_CHK
    {0x6E,  1,  0x3CD4,  21,  16}, //U_OTP_SCPUBootMode
    {0x6F,  1,  0x3CE0,  31,  30}, //U_OTP_forbid_STR
    {0x70,  1,  0x3D24,  11,   8}, //U_OTP_ena_NSK2
    {0x71,  1,  0x3D24,  15,  14}, //U_OTP_ena_DMA
    {0x72,  1,  0x3D24,  19,  16}, //U_OTP_ena_SCPU
    {0x73,  1,  0x3D44,   3,   0}, //U_OTP_ena_ACPU2KT
    {0x74,  1,  0x3D44,   7,   4}, //U_OTP_ena_NSK2KT
    {0x75,  1,  0x3D44,  19,  16}, //U_OTP_ena_ACPUWrNSKKey2KT
    {0x76,  1,  0x3D44,  27,  24}, //U_OTP_ena_NSKCW2CryptoDMA
    {0x77,  1,  0x3D4C,   7,   4}, //U_OTP_ena_DMA_DESBasedCipher
    {0x78,  1,  0x3D6C,  19,  16}, //U_OTP_ena_LowerPathRec
    {0x79,  1,  0x3B2C,  17,  16}, //U_OTP_forbid_Kilo_ProgRepair
    {0x7A,  1,  0x3CC8,  19,  16}, //U_OTP_SecR2_Sboot
    {0x7B,  1,  0x3C88,  27,  24}, //U_OTP_NDS_CODE_INTEGRITY_RSA_KEY_SEL
    {0x7C,  1,  0x3CC8,  23,  20}, //U_OTP_xCPU_Sboot
};
...
{
    MS_U32 NumOfTag = sizeof(K6Asst)/sizeof(ASST_t);
    ...
    *desc_size = ret_size;
    ...
    for( i=0; i<NumOfTag; i++)
    {
        *pDesc++ = K6Asst[i].u8Tag;
        *pDesc++ = K6Asst[i].u8Length;

        if(0x20 == K6Asst[i].u8Tag) //pubOTP...special case....
        {
            ...
            *pDesc++ = 0;
            ...
            {
                MS_U32 u32Tmp;
                u32Tmp = MDrv_CA_OTP_Read(K6Asst[i].Offset + j);
        ...
        else if (0x25 == K6Asst[i].u8Tag) //NV Counter...special case....
        {
        ...
        else
        {
            ...
            RunLens = K6Asst[i].u8Length;
            RunOffset = K6Asst[i].Offset;
            HALNSK2_DBG(NSK2_DBGLV_DEBUG,"Offset = %x, length = %x\n",RunOffset, RunLens);
            if(K6Asst[i].u8Length <= 4)
            {
                HAL_NSK2_OTP_Get(K6Asst[i].Offset, K6Asst[i].MSB, K6Asst[i].LSB, &OTPValue);
                if(1 == RunLens)
...
static ChipOTPCfg_t K6ChipCfg[] =
{
    ...
    {0x3C50, 17,  16,   2},  //[83:82]	U_OTP_NDS_Key1_integrity_chk[1:0]	11
    {0x3C50,  3,   2,   2},  //[81:80]	U_OTP_NDS_ESCK_Key2_obfuscation[1:0]	11
    {0x3C50, 19,  18,   2},  //[79:78]	U_OTP_NDS_Key2_integrity_chk[1:0]	11
    {0x3C50,  5,   4,   2},  //[77:76]	U_OTP_NDS_ESCK_Key3_obfuscation[1:0]	11
    {0x3C50, 21,  20,   2},  //[75:74]	U_OTP_NDS_Key3_integrity_chk[1:0]	11
    {0x3C50,  7,   6,   2},  //[73:72]	U_OTP_NDS_ESCK_Key4_obfuscation[1:0]	11
    {0x3C50, 23,  22,   2},  //[71:70]	U_OTP_NDS_Key4_integrity_chk[1:0]	11
    {0x3C50,  9,   8,   2},  //[69:68]	U_OTP_NDS_ESCK_Key5_obfuscation[1:0]	11
    {0x3C50, 25,  24,   2},  //[67:66]	U_OTP_NDS_Key5_integrity_chk[1:0]	11
    {0x3C50, 11,  10,   2},  //[65:64]	U_OTP_NDS_ESCK_Key6_obfuscation[1:0]	11
    {0x3C50, 27,  26,   2},  //[63:62]	U_OTP_NDS_Key6_integrity_chk[1:0]	11
    {0x3C50, 13,  12,   2},  //[61:60]	U_OTP_NDS_ESCK_Key7_obfuscation[1:0]	11
    {0x3C50, 29,  28,   2},  //[59:58]	U_OTP_NDS_Key7_integrity_chk[1:0]	11
    {0x3C50, 15,  14,   2},  //[57:56]	U_OTP_NDS_ESCK_Key8_obfuscation[1:0]	11
...
MS_U32 HAL_NSK2_GetFullChipConfig(MS_U32 *desc_size, MS_U8 *desc)
{
    *desc_size = FullChipConfigSize;
    ...
    for(i=0; i<NumOfConfigs; i++)
    {
        ...
        HALNSK2_DBG(NSK2_DBGLV_INFO,"%d, %x\n",i, TotalOTPValue[i]);
    }
...
// Read NV Counter from register directly
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"NVValue = %d, LeftNVCounter = %d \n", u32NvCounter, u32LeftNvCounter);
...
void HAL_NSK2_ReadAllOTP(void)
{
    ...
    for( addr=0 ; addr<0x4000; addr+=4)
    {
        ...
        if(u32RetValue != 0)
        {
            HALNSK2_DBG(NSK2_DBGLV_INFO,"addr = %x, value = %x \n",addr, u32RetValue);
        }
    }
}
...
//U_OTP_v_PubOtpOID                 0x3B00  15  0
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpOID = %x\n",ReadValue);
    memcpy(pRunPubOTP,&ReadValue,2);
    pRunPubOTP += 2;

    HAL_NSK2_OTP_Get(0x3B04, 15, 0, &ReadValue);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpVID = %x\n",ReadValue);
    memcpy(pRunPubOTP,&ReadValue,2);
    pRunPubOTP += 2;

    HAL_NSK2_OTP_Get(0x3B0C, 3, 0, &ReadValue);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpBID = %x\n",ReadValue);
    HAL_NSK2_OTP_Get(0x3B10, 3, 0, &ReadValue2);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpRsaIndex = %x\n",ReadValue2);
    *pRunPubOTP = (MS_U8)(ReadValue&0xF) + (MS_U8)((ReadValue2&0xF)<<4);
    pRunPubOTP ++;

    HAL_NSK2_OTP_Get(0x3B14, 7, 0, &ReadValue);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpMinConfVer = %x\n",ReadValue);
    memcpy(pRunPubOTP,&ReadValue,1);
    pRunPubOTP ++;

    HAL_NSK2_OTP_Get(0x3B08, 15, 0, &ReadValue);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_PubOtpGP = %x\n",ReadValue);
    memcpy(pRunPubOTP,&ReadValue,2);
    pRunPubOTP += 2;

    HAL_NSK2_OTP_Get(0x3DA8, 31, 0, &ReadValue);
    HALNSK2_DBG(NSK2_DBGLV_INFO,"U_OTP_v_pubOtpUniqueID1 = %x\n",ReadValue);
...
MS_U32 HAL_NSK21_ModifyGenIn(MS_U32 MaskVal,MS_U32 XorVal)
{
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"WriteGenIn = %x\n",WriteGenIn);
...
MS_U32 HAL_NSK2_ReadPWD_Status(void)
{
    ...
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"JTAG PWD0 ON \n");
    }

    if(RegPwd & N2_JTAGPWD1_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"JTAG PWD1 ON \n");
    }

    if(RegPwd & N2_SCANPWD_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"SCAN PWD ON \n");
    }

    if(RegPwd & N2_MBISTPWD_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"MBIST PWD ON \n");
    }

    if(RegPwd & N2_M2MKEY_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"M2M KEY ON \n");
    }

    if(RegPwd & N2_SCPUKEY0_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"SCPU KEY0 ON \n");
    }

    if(RegPwd & N2_SCPUKEY1_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"SCPU KEY1 ON \n");
    }

    if(RegPwd & N2_RNGVALUE0_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"RNG VALUE0 ON \n");
    }

    if(RegPwd & N2_RNGVALUE1_ON)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"RNG VALUE1 ON \n");
    }
...
MS_U32 HAL_NSK2_CtrlClk(MS_BOOL Enable)
{
    if(Enable)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"enable NSK clock\n");
        NI_REG(REG_NI_NSK2_CTRL) |= (NI_NSK2_CLK_ENABLE | NI_NSK2_RESET_DISABLE);
        HAL_NSK2_ColdReset();
    }
    else
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"disable NSK clock\n");
        NI_REG(REG_NI_NSK2_CTRL) &= ~(NI_NSK2_CLK_ENABLE | NI_NSK2_RESET_DISABLE);
        _gReset = FALSE;
        _gCheckBusyFlag = FALSE;
    }
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"REG_NI_NSK2_CTRL = %x\n",NI_REG(REG_NI_NSK2_CTRL));
    return TRUE;
}
...
MS_U32 HAL_NSK2_ReadClkStatus(void)
{
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"REG_NI_KTE_STATUS = %x\n",NI_REG(REG_NI_KTE_STATUS));
    HALNSK2_DBG(NSK2_DBGLV_INFO,"NI_SLOW_CLOCK_DETECT = %x\n", (status>>5));

    if(status)
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"no clock for NSK\n");
    }
    else
    {
        HALNSK2_DBG(NSK2_DBGLV_INFO,"clock is alive for NSK\n");
    }

    HALNSK2_DBG(NSK2_DBGLV_INFO,"read NSK ROM value\n");
    HAL_NSK2_ReadData32(0x8000, 4, &rom[0]);

    HALNSK2_DBG(NSK2_DBGLV_INFO,"%x, %x, %x, %x,\n",rom[0],rom[1],rom[2],rom[3]);
