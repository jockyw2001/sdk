...
static MS_BOOL  _bEnableTEE = FALSE;
...
{
    DSCMB_ASSERT(u32DscmbId < HAL_DSCMB_KTE_MAX, "The Dscmb source is out of range\n", FALSE);
    DSCMB_ASSERT(NULL != pu32SlotId, "The pointer is null\n", FALSE);
...
{
    DSCMB_ASSERT(NULL != pFltId, "The pointer is null\n", FALSE);
    DSCMB_ASSERT(u32DscmbId < HAL_DSCMB_KTE_MAX, "The Dscmb source is out of range\n", FALSE);
...
MS_BOOL _DumpMapInfo(MS_U32 idx)
{
    DSCMB_ASSERT(idx < HAL_DSCMB_KTE_MAX, "The Dscmb source is out of range\n", FALSE);
...
{
    DSCMB_DEFAULT_CAVID stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_DEFAULT_CAVID));

    stArgs.u32EngId   = u32EngId;
    stArgs.u32CAVid   = u32CAVid;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_SetDefaultCAVid, &stArgs, sizeof(DSCMB_DEFAULT_CAVID)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_SetDefaultCAVid);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_FLTALLOC stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_FLTALLOC));

    stArgs.u32EngId = u32EngId;
    stArgs.eFltType = eFltType;
    stArgs.u32DscmbId = 0xFF;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltAlloc, &stArgs, sizeof(DSCMB_FLTALLOC)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltAlloc);
        return DRV_DSCMB_FLT_NULL;
    }
    return stArgs.u32DscmbId;
}

MS_BOOL _MDrv_DSCMB2_TEE_FltFree(MS_U32 u32EngId, MS_U32 u32DscmbId)
{
    DSCMB_FLTFREE stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_FLTFREE));

    stArgs.u32EngId = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltFree, &stArgs, sizeof(DSCMB_FLTFREE)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltFree);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_CONNECTFLT stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_CONNECTFLT));

    stArgs.u32EngId = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;
    stArgs.u32DmxFltId = u32FltId;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltConnectFltId, &stArgs, sizeof(DSCMB_CONNECTFLT)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltConnectFltId);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL _MDrv_DSCMB2_TEE_FltDisconnectFltId(MS_U32 u32EngId, MS_U32 u32DscmbId, MS_U32 u32FltId)
{
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltDisconnectFltId, &stArgs, sizeof(DSCMB_CONNECTFLT)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltDisconnectFltId);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_TYPESET stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_TYPESET));

    stArgs.u32EngId   = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;
    stArgs.eType = DscmbType;
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltTypeSet, &stArgs, sizeof(DSCMB_TYPESET)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltTypeSet);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_FLTKEYSET stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_FLTKEYSET));

    stArgs.u32EngId      = u32EngId;
    stArgs.u32DscmbId    = u32DscmbId;
    stArgs.eKeyType      = eKeyType;
    stArgs.pu8Key        = pu8Key;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltKeySet, &stArgs, sizeof(DSCMB_FLTKEYSET)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltKeySet);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_FLTKEYRESET stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_FLTKEYRESET));

    stArgs.u32EngId = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;
    stArgs.eKeyType = eKeyType;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltKeyReset, &stArgs, sizeof(DSCMB_FLTKEYRESET)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltKeyReset);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_FLTIVSET stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_FLTIVSET));
    ...
    if(TRUE !=  _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltIVSet, &stArgs, sizeof(DSCMB_FLTIVSET)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltIVSet);
        return FALSE;
    }
    return TRUE;
}

MS_BOOL _MDrv_DSCMB2_TEE_FltSwitchType(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Flt_Type eFltType)
{
    // TODO: Add TEE Command
...
{
    DSCMB_ENGALGO stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGALGO));

    stArgs.u32EngId    = u32EngId;
    stArgs.u32DscmbId  = u32DscmbId;
    stArgs.eEngType    = eEngType;
    stArgs.stConfig    = stConfig;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetAlgo, &stArgs, sizeof(DSCMB_ENGALGO)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetAlgo);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_ENGKEYFSCB stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGKEY));

    stArgs.u32EngId    = u32EngId;
    stArgs.u32DscmbId  = u32DscmbId;
    stArgs.eEngType    = eEngType;
    stArgs.eKeyType    = eKeyType;
    stArgs.eForceSCB   = eForceSCB;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetKeyFSCB, &stArgs, sizeof(DSCMB_ENGKEY)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetKeyFSCB);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL _MDrv_DSCMB2_TEE_EngSetKey(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_U8* pu8Key)
{
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetKey, &stArgs, sizeof(DSCMB_ENGKEY)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetKey);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_ENGRESETKEY stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGRESETKEY));
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_FltKeyReset, &stArgs, sizeof(DSCMB_ENGRESETKEY)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_FltKeyReset);
        return FALSE;
    }
...
{
    DSCMB_ENGENABLEKEY stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGENABLEKEY));
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngEnableKey, &stArgs, sizeof(DSCMB_ENGENABLEKEY)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngEnableKey);
        return FALSE;
    }
    return TRUE;
}
...
{
    return _MDrv_DSCMB2_TEE_EngSetIV_Ex(u32EngId, u32DscmbId, E_DSCMB_ENG_LSAD | E_DSCMB_ENG_ESA | E_DSCMB_ENG_LSAS, eKeyType, pu8IV);
}
...
{
    DSCMB_ENGIV_EX stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGIV_EX));
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetIV_Ex, &stArgs, sizeof(DSCMB_ENGIV_EX)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetIV_Ex);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_HDCP2_SETRIV_EX stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_HDCP2_SETRIV_EX));
    stArgs.u32EngId   = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;
    stArgs.pu8Riv  = pu8RIV;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetRIV, &stArgs, sizeof(DSCMB_HDCP2_SETRIV_EX)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetRIV);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_ENGFSCB stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGFSCB));

    stArgs.u32EngId   = u32EngId;
    stArgs.u32DscmbId = u32DscmbId;
    stArgs.eForceSCB  = eForceSCB;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetFSCB, &stArgs, sizeof(DSCMB_ENGFSCB)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetFSCB);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_ENGSWITCH stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_ENGSWITCH));

    stArgs.u32EngId    = u32EngId;
    stArgs.u32DscmbId  = u32DscmbId;
    stArgs.eUppSwitch  = eUppSwitch;
    stArgs.eLowSwitch  = eLowSwitch;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_EngSetSwitch, &stArgs, sizeof(DSCMB_ENGSWITCH)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_EngSetSwitch);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_MULTI2_SETROUND stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_MULTI2_SETROUND));

    stArgs.u32EngId = u32EngId;
    stArgs.u32Round = u32Round;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_Multi2_SetRound, &stArgs, sizeof(DSCMB_MULTI2_SETROUND)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_Multi2_SetRound);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_MULTI2_SETSYSKEY stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_MULTI2_SETSYSKEY));

    stArgs.u32EngId  = u32EngId;
    stArgs.pu8SysKey = pu8Syskey;

    if(TRUE !=  _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_Multi2_SetSysKey, &stArgs, sizeof(DSCMB_MULTI2_SETSYSKEY)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_Multi2_SetSysKey);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_CAPVR_FLOWSET stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_CAPVR_FLOWSET));

    stArgs.u32EngId    = u32EngId;
    stArgs.eCaMode     = eCaMode;
    stArgs.ePvrSrcTsif = ePvrSrcTSIf;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_CAPVR_FlowSet, &stArgs, sizeof(DSCMB_CAPVR_FLOWSET)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_CAPVR_FlowSet);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_DUALPATH stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_DUALPATH));

    stArgs.u32EngId     = u32EngId;
    stArgs.u32DmxFltId  = u32FltId;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_DualPath_Enable, &stArgs, sizeof(DSCMB_DUALPATH)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_DualPath_Enable);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL _MDrv_DSCMB2_TEE_DualPath_Disable(MS_U32 u32EngId, MS_U32 u32FltId)
{
    ...
    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_DualPath_Disable, &stArgs, sizeof(DSCMB_DUALPATH)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_DualPath_Disable);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_KL_ATOMICEXEC stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_KL_ATOMICEXEC));

    stArgs.KLCfg     = KLCfg;
    stArgs.ACPU_Out  = ACPU_Out;
    stArgs.u32Status = u32Status;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_KLadder_AtomicExec, &stArgs, sizeof(DSCMB_KL_ATOMICEXEC)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_KLadder_AtomicExec);
        return FALSE;
    }
    return TRUE;
}
...
{
    DSCMB_KL_ETSI stArgs;
    memset(&stArgs, 0, sizeof(DSCMB_KL_ETSI));

    stArgs.KLCfg       = KLCfg;
    stArgs.ACPU_Out    = ACPU_Out;
    stArgs.pu8Nonce    = pu8Nonce;
    stArgs.pu8Response = pu8Response;
    stArgs.u32Status   = u32Status;

    if(TRUE != _MDrv_DSCMB2_TEE_Send_CMD(E_TEE_CMD_DSCMB_KLadder_ETSI, &stArgs, sizeof(DSCMB_KL_ETSI)))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_DSCMB_KLadder_ETSI);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL _MDrv_DSCMB2_InitBySWFlag(MS_BOOL bSWInit)
{
    ...
    MDrv_SYS_GetTEEInfo(&stTeeMode);
    _bEnableTEE = (stTeeMode.OsType == SYS_TEEINFO_OSTYPE_OPTEE);

    DRVDSCMB2_DBG(DSCMB2_DBGLV_INFO, "stTeeMode.OsType=%x\n", stTeeMode.OsType);

    if(_bEnableTEE)
    {
        if(FALSE == _MDrv_DSCMB2_TEE_Open())
        {
            #ifndef DSCMB_NO_PRINTF
            printf("\033[31mOpen DSCMB TA Fail\n\033[m");
            #endif
        }
        else
        {
            #ifndef DSCMB_NO_PRINTF
            printf("\033[32mOpen DSCMB TA Success\n\033[m");
            #endif
        }
    }
...
MS_U32 _MDrv_DSCMB2_FltAllocWithCAVid(MS_U32 u32EngId, DSCMB_Flt_Type eFltType, MS_U32 u32CAVid)
{
    ...
    if(FALSE == _DumpMapInfo(i))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Dump SlotMapInfo Fail\n");
        goto MDrv_DSCMB2_FltAlloc_Ex_FAIL;
    }
...
{
/*
 * 1.1.130 [TEE <--> REE] E_DSCMB2_ENG_ENABLE_KEY doesn’t check parameters
 */
    DSCMB_INIT_CHECK(FALSE);
    DSCMB_ASSERT(u32EngId == 0, "EngId input error", FALSE);
    DSCMB_ASSERT(u32DscmbId < HAL_DSCMB_KTE_MAX, "DscmbId input error", FALSE);
...
MS_U32  _MDrv_DSCMB2_GetPidSlotMapIndex(MS_U32 u32EngId, MS_U32 u32DscmbId)
{
    ...
    pMap = &DscmbTspMap[u32EngId][u32FltId];
    if ( pMap->bUsed == FALSE )
    {
        return DRV_DSCMB_FLT_NULL;
    }
...
{
    DSCMB_INIT_CHECK(FALSE);
    DSCMB_ASSERT(u32FltId < HAL_DSCMB_PIDFLT_NUM, "The Pid filter is out of range\n", FALSE);
    DSCMB_ASSERT((eKeyType) < HAL_DSCMB_KEY_TYPE_ODD, "The key type error\n", FALSE)
...
{
    DSCMB_INIT_CHECK(FALSE);
    DSCMB_ASSERT(u32UpDst < DSCMB_CADST_MAX, "The CA Dst is out of range\n", FALSE);
    DSCMB_ASSERT(u32LowDst < DSCMB_CADST_MAX, "The CA Dst is out of range\n", FALSE);
    DSCMB_ASSERT(u32FltId < HAL_DSCMB_PIDFLT_NUM, "The Pid filter is out of range\n", FALSE);
...
MS_BOOL MDrv_DSCMB2_CAMod_Active(DSCMB_CAMod_ActType  eActType , MS_U8* pu8Pwd , DSCMB_CAMOD_STATUS* u32Status)
{
    DSCMB_INIT_CHECK(FALSE);
    DSCMB_ASSERT(eActType == E_DSCMB_CAMOD_ACT_CA, "eActType error", FALSE);
    DSCMB_ASSERT(pu8Pwd != NULL, "pu8Pwd is NULL", FALSE);

    DSCMB_LOCK(_s32DscMutexId);

    if(!HAL_DSCMB_CAModule_SetPwd(eActType, pu8Pwd, u32Status))
    {
        DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set Password Fail\n");
        DSCMB_UNLOCK(_s32DscMutexId);
        return FALSE;
    }
...
MS_BOOL MDrv_DSCMB2_CAPVR_SetPidFLT(DSCMB_CAPVR_MODE eCaMode, MS_U32 u32Pid, MS_BOOL bEnable)
{
    DSCMB_INIT_CHECK(FALSE);

    MS_U32 u32ProgId  = HAL_DSCMB_GetProgId(eCaMode);

    return HAL_DSCMB_CAPVR_SetPidFLT(u32ProgId, u32Pid, bEnable);
}
...
MS_BOOL _MDrv_DSCMB2_KLadder_AtomicExec(DSCMB_KLCfg_All* KLCfg , MS_U8 *ACPU_Out, DSCMB_KL_Status* u32Status )
{
    ...
    MS_U32 u32FltId = 0;
    MS_U32 u32Field = KLCfg->eKeyType;
    ...
    *u32Status = KL_STATUS_KEY_OK ;
    ...
    if (((DSCMB_KL_DST_CLASS_SPSSPD & eKLDst) == DSCMB_KL_DST_CLASS_SPSSPD) ||
        ((E_DSCMB_KL_DST_DMA_PVRLUT0 & eKLDst) == E_DSCMB_KL_DST_DMA_PVRLUT0))
    {
        // KL flow for output  to CryptoDMA  //
...
{
    DSCMB_INIT_CHECK(FALSE);
    DSCMB_ASSERT(pu8Statickey != NULL, "pu8Statickey input NULL", FALSE);
...
{
    DSCMB_INIT_CHECK();
    DSCMB_ASSERT(u32FltId < HAL_DSCMB_PIDFLT_NUM, "The Dscmb source is out of range\n", );
...
MS_U32 MDrv_DSCMB2_FltAlloc_Ex(MS_U32 u32EngId, DSCMB_Flt_Type eFltType)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltAlloc_Ex(u32EngId, eFltType);
    }
...
MS_BOOL MDrv_DSCMB2_FltFree(MS_U32 u32EngId, MS_U32 u32DscmbId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltFree(u32EngId, u32DscmbId);
    }
...
MS_BOOL MDrv_DSCMB2_FltConnectFltId(MS_U32 u32EngId, MS_U32 u32DscmbId, MS_U32 u32FltId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltConnectFltId(u32EngId, u32DscmbId, u32FltId);
    }
...
MS_BOOL MDrv_DSCMB2_FltDisconnectFltId(MS_U32 u32EngId, MS_U32 u32DscmbId, MS_U32 u32FltId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltDisconnectFltId(u32EngId, u32DscmbId, u32FltId);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetFSCB(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_FSCB eForceSCB )
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetFSCB(u32EngId, u32DscmbId, eForceSCB);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetSwitch(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eUppSwitch, DSCMB_Eng_Type eLowSwitch)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetSwitch(u32EngId, u32DscmbId, eUppSwitch, eLowSwitch);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetAlgo(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Algo_Cfg stConfig)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetAlgo(u32EngId, u32DscmbId, eEngType, stConfig);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetRIV(MS_U32 u32EngId, MS_U32 u32DscmbId, MS_U8* pu8RIV)
{
    ...
    {
        // TODO: Add TEE Command
        return _MDrv_DSCMB2_TEE_EngSetRIV(u32EngId, u32DscmbId, pu8RIV);
    }
    else
    {
        return _MDrv_DSCMB2_EngSetRIV(u32EngId, u32DscmbId, pu8RIV);
...
MS_BOOL MDrv_DSCMB2_EngSetIV(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetIV(u32EngId, u32DscmbId, eKeyType, pu8IV);
    }
    return _MDrv_DSCMB2_EngSetIV(u32EngId, u32DscmbId, eKeyType, pu8IV);
#endif
}
...
MS_BOOL MDrv_DSCMB2_EngSetIV_Ex(MS_U32 u32EngId, MS_U32 u32DscmbId,  DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetIV_Ex(u32EngId, u32DscmbId, eEngType, eKeyType, pu8IV);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetKeyFSCB(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, DSCMB_FSCB eForceSCB)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetKeyFSCB(u32EngId, u32DscmbId, eEngType, eKeyType, eForceSCB);
    }
...
MS_BOOL MDrv_DSCMB2_EngSetKey(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_U8* pu8Key)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngSetKey(u32EngId, u32DscmbId, eEngType, eKeyType, pu8Key);
    }
...
MS_BOOL MDrv_DSCMB2_EngResetKey(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngResetKey(u32EngId, u32DscmbId, eEngType, eKeyType);
    }
...
MS_BOOL MDrv_DSCMB2_SetDefaultCAVid(MS_U32 u32EngId, MS_U32 u32CAVid)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_SetDefaultCAVid(u32EngId, u32CAVid);
    }
...
MS_BOOL MDrv_DSCMB2_DualPath_Enable(MS_U32 u32EngId, MS_U32 u32FltId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_DualPath_Enable(u32EngId, u32FltId);
    }
...
MS_BOOL MDrv_DSCMB2_DualPath_Disable(MS_U32 u32EngId, MS_U32 u32FltId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_DualPath_Disable(u32EngId, u32FltId);   //enable small switch pid_no dual path
    }
...
MS_BOOL MDrv_DSCMB2_CAPVR_FlowSet(MS_U32 u32EngId, DSCMB_CAPVR_MODE eCaMode, DSCMB_TSIF ePvrSrcTsif)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_CAPVR_FlowSet(u32EngId, eCaMode, ePvrSrcTsif);
    }
...
MS_BOOL MDrv_DSCMB2_FltTypeSet(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Type DscmbType)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltTypeSet(u32EngId, u32DscmbId, DscmbType);
    }
...
MS_BOOL MDrv_DSCMB2_FltKeySet(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Key_Type eKeyType, MS_U8* pu8Key)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltKeySet(u32EngId, u32DscmbId, eKeyType, pu8Key);
    }
...
MS_BOOL MDrv_DSCMB2_FltIVSet(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltIVSet(u32EngId, u32DscmbId, eKeyType, pu8IV);
    }
...
MS_BOOL MDrv_DSCMB2_FltKeyReset(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Key_Type eKeyType)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltKeyReset(u32EngId, u32DscmbId, eKeyType);
    }
...
MS_BOOL MDrv_DSCMB2_KLadder_AtomicExec(DSCMB_KLCfg_All* KLCfg , MS_U8 *ACPU_Out, DSCMB_KL_Status* u32Status)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_KLadder_AtomicExec(KLCfg, ACPU_Out, u32Status);
    }
...
MS_BOOL MDrv_DSCMB2_KLadder_ETSI(DSCMB_KLCfg_All* KLCfg , MS_U8 *ACPU_Out, MS_U8 *pu8Nonce, MS_U8 *pu8Response, DSCMB_KL_Status* u32Status )
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_KLadder_ETSI(KLCfg, ACPU_Out, pu8Nonce, pu8Response, u32Status);
    }
...
MS_BOOL MDrv_DSCMB2_Multi2_SetRound(MS_U32 u32EngId, MS_U32 u32Round)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_Multi2_SetRound(u32EngId, u32Round);
    }
...
MS_BOOL MDrv_DSCMB2_Multi2_SetSystemKey(MS_U32 u32EngId, MS_U8* pu8Syskey)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_Multi2_SetSystemKey(u32EngId, pu8Syskey);
    }
...
MS_BOOL MDrv_DSCMB2_EngEnableKey(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_BOOL bEnable)
{
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_EngEnableKey(u32EngId, u32DscmbId, eEngType, eKeyType, bEnable);
    }
...
MS_BOOL MDrv_DSCMB2_FltSwitchType(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Flt_Type eFltType)
{
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_DSCMB2_TEE_FltSwitchType(u32EngId, u32DscmbId, eFltType);
    }
