...
void DSCMBRegisterToUtopia(void)
{
    ...
    if(pResPri->u32Magic != MAGIC_NUM)
    {
        ...
        MDrv_SYS_GetTEEInfo(&pResPri->stTeeInfo);
    }

    DSCMB_MSG("pResPri->stTeeInfo.OsType=%x\n", pResPri->stTeeInfo.OsType);
...
MS_U32 DSCMBIoctl(void* pInstance, MS_U32 u32Cmd, void* pu32Args)
{
    ...
    switch(u32Cmd)
    {
        ...
        case E_MDRV_CMD_DSCMB_FltAlloc:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                pFltAlloc->u32DscmbId = _MDrv_DSCMB2_TEE_FltAlloc_Ex(pFltAlloc->u32EngId,
                                        pFltAlloc->eFltType);
            }
            else
            {
        ...
        case E_MDRV_CMD_DSCMB_FltFree:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltFree(pFltFree->u32EngId, pFltFree->u32DscmbId))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_FltConnectFltId:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltConnectFltId(pConnectFlt->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_FltDisconnectFltId:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltDisconnectFltId(pDisconnectFlt->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_FltTypeSet:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltTypeSet(pstTypeSet->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_FltKeySet:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltKeySet(pstFltKeySet->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_FltKeyReset:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltKeyReset(pstFltKeyReset->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_FltIVSet:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_FltIVSet(pstFltIVSet->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_Multi2_SetRound:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_Multi2_SetRound(pstSetRnd->u32EngId,
                        pstSetRnd->u32Round))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
        ...
        case E_MDRV_CMD_DSCMB_Multi2_SetSysKey:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_Multi2_SetSystemKey(psSetSysKey->u32EngId,
        ...
        case E_MDRV_CMD_DSCMB_SetDefaultCAVid:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_SetDefaultCAVid(pu32DefaultVid->u32EngId, pu32DefaultVid->u32CAVid))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetAlgo:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetAlgo(pAlgo->u32EngId, pAlgo->u32DscmbId, pAlgo->eEngType, pAlgo->stConfig))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetKey:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetKey(pKey->u32EngId, pKey->u32DscmbId, pKey->eEngType, pKey->eKeyType, pKey->pu8Key))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetKeyFSCB:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetKeyFSCB(pKeyFSCB->u32EngId, pKeyFSCB->u32DscmbId, pKeyFSCB->eEngType, pKeyFSCB->eKeyType, pKeyFSCB->eForceSCB))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngResetKey:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngResetKey(pKey->u32EngId, pKey->u32DscmbId, pKey->eEngType, pKey->eKeyType))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetIV:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetIV(pIV->u32EngId, pIV->u32DscmbId, pIV->eKeyType, pIV->pu8IV))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetSwitch:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetSwitch(pSwitch->u32EngId, pSwitch->u32DscmbId, pSwitch->eUppSwitch, pSwitch->eLowSwitch))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_EngSetFSCB:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetFSCB(pFSCB->u32EngId, pFSCB->u32DscmbId, pFSCB->eForceSCB))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
            }
        ...
        case E_MDRV_CMD_DSCMB_KLadder_AtomicExec:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_KLadder_AtomicExec(pKLArgs->KLCfg,
                        pKLArgs->ACPU_Out,
                        pKLArgs->u32Status))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
        ...
        case E_MDRV_CMD_DSCMB_KLadder_ETSI:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_KLadder_ETSI(pKLArgs->KLCfg,
                        pKLArgs->ACPU_Out,
                        pKLArgs->pu8Nonce,
                        pKLArgs->pu8Response,
                        pKLArgs->u32Status))
        ...
        case E_MDRV_CMD_DSCMB_CAPVR_FlowSet:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_CAPVR_FlowSet (pStatusArgs->u32EngId,
                        pStatusArgs->eCaMode,
        ...
        case E_MDRV_CMD_DSCMB_DualPath_Enable:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_DualPath_Enable (pStatusArgs->u32EngId,
                        pStatusArgs->u32DmxFltId))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
        ...
        case E_MDRV_CMD_DSCMB_DualPath_Disable:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_DualPath_Disable (pStatusArgs->u32EngId,
                        pStatusArgs->u32DmxFltId))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
        ...
        case E_MDRV_CMD_DSCMB_EngSetIV_Ex:
        {
            ...
            if(pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE)
            {
                if(TRUE == _MDrv_DSCMB2_TEE_EngSetIV_Ex (pStatusArgs->u32EngId,
                        pStatusArgs->u32DscmbId,
