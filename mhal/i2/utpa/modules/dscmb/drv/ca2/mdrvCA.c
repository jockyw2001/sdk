...
MS_U32 CAMdbIoctl(MS_U32 u32Cmd, const void* const pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            char temp[255]={0};
            MS_U32 u32ParaSize;
            EN_CA_MDBCMDLINE eMdbCmdLine = CA_MDBCMDLINE_NONE;
            char *pch;
            ...
            switch(u32ParaSize)
            {
                ...
                case 4:
                    ...
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = CA_MDBCMDLINE_HELP;
                    }
                    break;
                case 3:
                    strcpy(temp,"otp");
                    ...
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = CA_MDBCMDLINE_OTP;
                    }
                    strcpy(temp,"bgc");
                    pstr = strstr(paraCmdLine->pcCmdLine, temp);
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = CA_MDBCMDLINE_BGC;
                    }
                    strcpy(temp,"mbx");
                    pstr = strstr(paraCmdLine->pcCmdLine, temp);
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = CA_MDBCMDLINE_MBX;
                    }
                    break;
                default:
                    eMdbCmdLine = CA_MDBCMDLINE_NONE;
                    break;
            }
            MDrv_CA_MdbCmdLine(paraCmdLine, eMdbCmdLine);
        ...
        case MDBCMD_GETINFO:
            ...
            MdbPrint(paraGetInfo->u64ReqHdl,"A. OTP\n");
            MdbPrint(paraGetInfo->u64ReqHdl,"B. BGC\n");
            MdbPrint(paraGetInfo->u64ReqHdl,"C. MBX\n");
...
void CARegisterToUtopia(void)
{
    ...
    #ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
    UtopiaModuleRegisterMdbNode("ca2", (FUtopiaMdbIoctl)CAMdbIoctl);
    #endif
...
{
    MS_DEBUG_MSG(printf("[%s][%d] CA open \n", __FUNCTION__, __LINE__));
...
MS_U32 CAIoctl(void* pInstance, MS_U32 u32Cmd, void* pu32Args)
{
    ...
    switch(u32Cmd)
    {
        ...
        {
            CA_OTP_READCAVENDOR* pReadCAVendor = (CA_OTP_READCAVENDOR*)pu32Args;

            pReadCAVendor->u32CAVendor = _MDrv_CA_OTP_ReadCAVendor();
            if(FALSE != pReadCAVendor->u32CAVendor)
            {
                u32Ret = UTOPIA_STATUS_SUCCESS;
            }
            else
        ...
        case E_MDRV_CMD_CA_OTP_SetFunctionConfig:
        {
            CA_OTP_SETFUNCTIONCONFIG* pSetFunctionConfig = (CA_OTP_SETFUNCTIONCONFIG*)pu32Args;

            if(TRUE == _MDrv_CA_OTP_SetFunctionConfig(pSetFunctionConfig->eFunction, pSetFunctionConfig->eActive))
            {
                u32Ret = UTOPIA_STATUS_SUCCESS;
            }
            else
            {
                u32Ret = UTOPIA_STATUS_FAIL;
                MS_DEBUG_MSG(printf("[%s][%d] E_MDRV_CMD_CA_OTP_SetFunctionConfig fail\n", __FUNCTION__, __LINE__));
            }
            MS_DEBUG_MSG(printf("[%s][%d] E_MDRV_CMD_CA_OTP_SetFunctionConfig\n", __FUNCTION__, __LINE__));
        }
        break;

        case E_MDRV_CMD_CA_OTP_GetFunctionConfig:
        {
            CA_OTP_GETFUNCTIONCONFIG* pGetFunctionConfig = (CA_OTP_GETFUNCTIONCONFIG*)pu32Args;

            if(TRUE == _MDrv_CA_OTP_GetFunctionConfig(pGetFunctionConfig->eFunction, pGetFunctionConfig->peActive, pGetFunctionConfig->pbDefault))
            {
                u32Ret = UTOPIA_STATUS_SUCCESS;
            }
            else
            {
                u32Ret = UTOPIA_STATUS_FAIL;
                MS_DEBUG_MSG(printf("[%s][%d] E_MDRV_CMD_CA_OTP_GetFunctionConfig fail\n", __FUNCTION__, __LINE__));
            }
            MS_DEBUG_MSG(printf("[%s][%d] E_MDRV_CMD_CA_OTP_GetFunctionConfig\n", __FUNCTION__, __LINE__));
        }
        break;

        default:
            MS_DEBUG_MSG(printf("[%s][%d] CAIoctl - Unknown commend 0x%x!!!\n", __FUNCTION__, __LINE__));
