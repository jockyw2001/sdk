...
static MS_S32 _s32Secure_Mutex = -1;
static MS_BOOL _bTADrv_Inited = FALSE;
...
#define DRV_TA_INIT_CHECK(ret) DRV_CA_ASSERT(_bTADrv_Inited == TRUE, "The drvTA is not initialized\n", ret)
...
#include "utopia.h"
static DrvCA_MDB_OTP sDrvCA_MDB_OTP;
static DrvCA_MDB_BGC sDrvCA_MDB_BGC;
static DrvCA_MDB_MBX sDrvCA_MDB_MBX;
...
//-------------------------------------------------------------------------------------------------
/// @brief Set the OTP function config
/// @param[in] eFunction: The OTP function config want to set
/// @param[in] eActive: Set config
/// @return TRUE: Process success.
/// @return FALSE: Process fail.
//-------------------------------------------------------------------------------------------------
MS_BOOL _MDrv_CA_OTP_SetFunctionConfig(CA_OTP_FUNCATION eFunction, CA_OTP_CONFIG_TYPE eActive)
{
    ...
    CA_OTP_FUNCATION _eFunction = eFunction;
    CA_OTP_CONFIG_TYPE _eActive;
    MS_BOOL _bDefault;

    DRV_CA_INIT_CHECK(FALSE);
    brc = HAL_CA_OTP_SetFunctionConfig(eFunction, eActive);
    if(FALSE == brc)
    {
        return FALSE;
    }
    brc = HAL_CA_OTP_GetFunctionConfig(_eFunction, &_eActive, &_bDefault);
    if(TRUE == brc)
    {
        if(_eActive == eActive)
        {
...
//-------------------------------------------------------------------------------------------------
/// @brief Query the OTP function config
/// @param[in] eFunction: The OTP function config want to query
/// @param[out] peActive: Return config
/// @param[out] pbDefault: Return the config is default setting or not
/// @return TRUE: Process success.
/// @return FALSE: Process fail.
//-------------------------------------------------------------------------------------------------
...
{
    DRV_CA_INIT_CHECK(FALSE);
    return HAL_CA_OTP_GetFunctionConfig(eFunction, peActive, pbDefault);
}
...
MS_BOOL MDrv_CA_BGC_Init(CA_BGC_CFG stBGCCfg)
{
    ...
    if (_s32Secure_Mutex < 0)
    {
        _s32Secure_Mutex = MsOS_CreateMutex(E_MSOS_FIFO,
                                           (char*) "Secure_Mutex",
                                           MSOS_PROCESS_SHARED);
        if (_s32Secure_Mutex < 0)
        {
...
MS_BOOL MDrv_CA_BGC_SetSection(MS_U32 u32SecId, const CA_BGC_SEC_CFG* pstSecCfg)
{
    ...
    MsOS_ObtainMutex(_s32Secure_Mutex, MSOS_WAIT_FOREVER);
    ...
    MsOS_ReleaseMutex(_s32Secure_Mutex);
...
//--------------------------------------------------------------------------------------------------
/// @brief Initialize TransAlgo function, including load TA firmware
/// @param[in] stTACfg The firmware address and size
/// @return TRUE - Success
/// @return FALSE - Failure
//--------------------------------------------------------------------------------------------------
MS_BOOL MDrv_CA_TA_Init(CA_TA_CFG stTACfg)
{
    DRV_CA_INIT_CHECK(FALSE);

    if (TRUE == _bTADrv_Inited)
    {
        return TRUE;
    }

    if (FALSE == HAL_CA_TA_Init(&stTACfg) )
    {
        return FALSE;
    }
...
//--------------------------------------------------------------------------------------------------
/// @brief Send encrypted TASK or encrypted TDC for setting up TransAlgo.
/// @param[in] pstTDCCfg The data/keys for generating transformation data.
/// @return TRUE - Success
/// @return FALSE - Failure
/// @note After that, the Transformation Data (LUT, BC, and M) will be ready for Inverse Transformaion.
//--------------------------------------------------------------------------------------------------
MS_BOOL MDrv_CA_TA_Setup(CA_TA_TDC_CFG * const pstTDCCfg)
{
    DRV_CA_INIT_CHECK(FALSE);
    DRV_TA_INIT_CHECK(FALSE);
    MS_BOOL ret = FALSE;
...
/// @return FALSE: Process fail.
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_CA_OTP_SetFunctionConfig(CA_OTP_FUNCATION eFunction, CA_OTP_CONFIG_TYPE eActive)
{
#ifdef CA_UTOPIA20
    MS_U32 u32Ret = UTOPIA_STATUS_SUCCESS;
    CA_OTP_SETFUNCTIONCONFIG pArgs;
    if(pu32CAInst == NULL)
    {
        return FALSE;
    }

    pArgs.eFunction = eFunction;
    pArgs.eActive = eActive;

    u32Ret = UtopiaIoctl(pu32CAInst, E_MDRV_CMD_CA_OTP_SetFunctionConfig, (void*)&pArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        MS_DEBUG_MSG(printf("[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret));
        return FALSE;
    ...
    return TRUE;
#else
    return _MDrv_CA_OTP_SetFunctionConfig(eFunction, eActive);
#endif
}
...
/// @return FALSE: Process fail.
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_CA_OTP_GetFunctionConfig(CA_OTP_FUNCATION eFunction, CA_OTP_CONFIG_TYPE *peActive, MS_BOOL *pbDefault)
{
#ifdef CA_UTOPIA20
    MS_U32 u32Ret = UTOPIA_STATUS_SUCCESS;
    CA_OTP_GETFUNCTIONCONFIG pArgs;
    if(pu32CAInst == NULL)
    {
        return FALSE;
    }
    ...
    pArgs.eFunction = eFunction;
    pArgs.peActive = peActive;
    pArgs.pbDefault = pbDefault;

    u32Ret = UtopiaIoctl(pu32CAInst, E_MDRV_CMD_CA_OTP_GetFunctionConfig, (void*)&pArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        MS_DEBUG_MSG(printf("[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret));
        return FALSE;
    ...
    return TRUE;
#else
    return _MDrv_CA_OTP_GetFunctionConfig(eFunction, peActive, pbDefault);
#endif
}
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
static MS_BOOL _MDrv_CA_GetOTPInfo(DrvCA_MDB_OTP *psDrvCA_MDB_OTP)
{
    return HAL_CA_GetOTPInfo(psDrvCA_MDB_OTP);
}

static MS_BOOL _MDrv_CA_GetBGCInfo(DrvCA_MDB_BGC *psDrvCA_MDB_BGC)
{
    return HAL_CA_GetBGCInfo(psDrvCA_MDB_BGC);
}

static MS_BOOL _MDrv_CA_GetMBXInfo(DrvCA_MDB_MBX *psDrvCA_MDB_MBX)
{
    return HAL_CA_GetMBXInfo(psDrvCA_MDB_MBX);
}
...
MS_BOOL MDrv_CA_MdbCmdLine(MDBCMD_CMDLINE_PARAMETER *paraCmdLine, EN_CA_MDBCMDLINE eMdbCmdLine)
{
    ...
    switch(eMdbCmdLine)
    {
        case CA_MDBCMDLINE_HELP:
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar CA help---------\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"otp\tget otp information\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"bgc\tget bgc information\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"mbx\tget mbx information\n");
            break;
        case CA_MDBCMDLINE_OTP:
            _MDrv_CA_GetOTPInfo(&sDrvCA_MDB_OTP);
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar CA OTP Info---------\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"HASH0_version:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_HASH0);
            MdbPrint(paraCmdLine->u64ReqHdl,"HASH1_version:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_HASH1);
            MdbPrint(paraCmdLine->u64ReqHdl,"HASH2_version:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_HASH2);
            MdbPrint(paraCmdLine->u64ReqHdl,"RSAextID:\t0x%x\n",sDrvCA_MDB_OTP.DBG_CA_OTP_MSID);
            MdbPrint(paraCmdLine->u64ReqHdl,"DeviceID:\n");
            for (i=0;i<32;i=i+4)
            {
                MdbPrint(paraCmdLine->u64ReqHdl,"0x%x 0x%x 0x%x 0x%x\n",sDrvCA_MDB_OTP.DBG_CA_OTP_DeviceID[i],sDrvCA_MDB_OTP.DBG_CA_OTP_DeviceID[i+1],sDrvCA_MDB_OTP.DBG_CA_OTP_DeviceID[i+2],sDrvCA_MDB_OTP.DBG_CA_OTP_DeviceID[i+3]);
            }
            MdbPrint(paraCmdLine->u64ReqHdl,"SecureBoot status:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_SecureBoot);
            MdbPrint(paraCmdLine->u64ReqHdl,"I2C mode:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_I2C_MODE);
            MdbPrint(paraCmdLine->u64ReqHdl,"EJTAG mode:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_EJTAG_MODE);
            MdbPrint(paraCmdLine->u64ReqHdl,"SCAN mode:\t0x%x \n",sDrvCA_MDB_OTP.DBG_CA_OTP_SCAN_MODE);
            break;
        case CA_MDBCMDLINE_BGC:
            _MDrv_CA_GetBGCInfo(&sDrvCA_MDB_BGC);
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar CA BGC Info---------\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"max section number:\t0x%d \n",sDrvCA_MDB_BGC.DBG_CA_BGC_SECTION_MAX);
            MdbPrint(paraCmdLine->u64ReqHdl,"section protect range:\t0x%x \n",sDrvCA_MDB_BGC.DBG_CA_BGC_RANGE);
            break;
        case CA_MDBCMDLINE_MBX:
            _MDrv_CA_GetMBXInfo(&sDrvCA_MDB_MBX);
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar CA MailBox info:---------\n");
            for (i=0;i<32;i=i+4)
            {
                MdbPrint(paraCmdLine->u64ReqHdl,"\t0x%x\t0x%x\t0x%x\t0x%x\n",sDrvCA_MDB_MBX.DBG_CA_MBX_INFO[i],sDrvCA_MDB_MBX.DBG_CA_MBX_INFO[i+1],sDrvCA_MDB_MBX.DBG_CA_MBX_INFO[i+2],sDrvCA_MDB_MBX.DBG_CA_MBX_INFO[i+3]);
            }
            break;
        default:
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar CA---------\n");
