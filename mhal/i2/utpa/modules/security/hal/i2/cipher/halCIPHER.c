...
MS_BOOL _HAL_CIPHER_TransMIUAddr(MS_PHY* phyAddr)
{
    ...
    if(*phyAddr& HAL_CRYPTODMA_SUPPORT_ADDR_MASK)
    {
        HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Error:truncates 64 bit address into a 32 bit address!\n",
            __func__, __LINE__));
        return FALSE;
    }
...
MS_BOOL HAL_CIPHER_IsSupportTEE(void)
{
    return TRUE;
}
...
MS_BOOL HAL_CIPHER_DMA_Start(MS_BOOL Decrypt, HAL_CIPHER_INTMODE IntMode, MS_BOOL bWaitCmdQ, MS_U16 u16CmdID)
{
    ...
    if(((_u32DmaInAddr + _u32DmaInNum) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
        ((_u32DmaOut_StartAddr + _u32DmaInNum) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK))
    {
        HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Error:input address with size is out of 32bits\n",
            __func__, __LINE__));
        return FALSE;
    }
...
DMA_RETRY:
    ...
    if (u32SpareCnt > CmdLen)
    {
        ...
        if (_uDmaCtrl1.DmaCtrl1.RR == TRUE )
        {
            if( (_u32ReportAddr& HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
                ((_u32ReportAddr + sizeof(_AESDMACtrl->Dma_Reportp)) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK))
            {
                HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Error:truncates 64 bit address into a 32 bit address!\n",
                    __func__, __LINE__));
                return FALSE;
...
MS_BOOL HAL_CIPHER_DMA_CmdDone(MS_U32 u32CmdID, MS_U32 *u32Ret)
{
    ...
    MS_U32 u32Status = REG32(&_AESDMACtrl->Dma_Ctrl[REG_CRYPTODMA_DEBUG]);

    if((0 == (u32Status & REG_DEBUG_DMA_BUSY)) && (u32RptId == u32CmdID))
    {
...
MS_BOOL HAL_CIPHER_Hash_SetHMACKey(DRV_CIPHER_HMAC_KEY stHMACKey, CIPHER_HMAC_KPAD eKpad, MS_BOOL bClear)
{
    ...
    //Check key length
    if(HASH_CMD_HMAC_HOSTKEY_SIZE < stHMACKey.u8KeyLen)
    {
        return FALSE;
    }
    ...
    if(KeySrc == E_DMA_KSEL_REGKEY)
    {
        ...
        {
            memset(_pu8HashHMACHostKey, 0x0 , HASH_CMD_HMAC_HOSTKEY_SIZE );
...
MS_BOOL HAL_CIPHER_Hash_Start(HAL_CIPHER_INTMODE IntMode , MS_BOOL bWaitCmdQ , MS_BOOL bRst, MS_U16 u16CmdID)
{
    ...
    if((_u32HashSrcAddr & HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
        (_u32HashDestAddr & HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
        ((_u32HashSrcAddr + _u32HashMsgLen) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
        ((_u32HashDestAddr + sizeof(_AESDMACtrl->Hash_Reportp)) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK))
    {
        HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Error:truncates 64 bit address into a 32 bit address!\n",
            __func__, __LINE__));
        return FALSE;
...
SHA_RETRY:
        ...
        if (u32SpareCnt > CmdLen)
        {
            ...
            {
                if( (_u32ReportAddr& HAL_CRYPTODMA_SUPPORT_ADDR_MASK) ||
                ((_u32ReportAddr + sizeof(_AESDMACtrl->Hash_Reportp)) & HAL_CRYPTODMA_SUPPORT_ADDR_MASK))
                {
                    HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]error : truncates 64 bit address into a 32 bit address!\n",
                        __func__, __LINE__));
                    return FALSE;
...
MS_BOOL HAL_CIPHER_Hash_CmdDone(MS_U32 u32CmdID, MS_U32 *u32Ret)
{
    ...
    if((0 == (u32Status & REG_DEBUG_HASH_BUSY)) && (u32RptId == u32CmdID))
    {
    	u32ExcTmp = (MS_U32)REG32(&_AESDMACtrl->Dma_Ctrl[REG_EXCEPT_FLAG]);
        *u32Ret = u32ExcTmp;
    }
