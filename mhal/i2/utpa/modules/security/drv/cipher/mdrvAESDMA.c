...
static MS_U32 _AESDMALockResource(void**ppstRes, DRVAESDMA_RESOURCE_PRIVATE**ppstResPri)
{
    if(UtopiaResourceObtain(pModuleAESDMA, AESDMA_POOL_ID_DRVAES, ppstRes) != UTOPIA_STATUS_SUCCESS)
    {
...
static MS_U32 _AESDMAUnLockResource(void * pstRes)
{
...
MS_U32 AESDMAIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    if(UTOPIA_STATUS_SUCCESS != _AESDMALockResource(&pDRVAESResource, &pstResPri))
    {
        printf("UtopiaResourceObtain fail\n");
        return UTOPIA_STATUS_ERR_RESOURCE;
    }
    ...
    {
        _AESDMAUnLockResource(pDRVAESResource);
        ...
        case MDrv_CMD_AESDMA_IsFinished:
            {
                ...
                while ((psAesInstPri->fpAESDMAIsFinished)(&u32Ret) != DRVAESDMA_OK)
                {
                    ...
                    //timeout
                    if(u32timeout_count > AES_MAX_TIMEOUT_VALUE)
                    {
                        break;
                    }
                }
