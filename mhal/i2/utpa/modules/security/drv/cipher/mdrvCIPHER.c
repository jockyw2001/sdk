...
//  Local Variables
//-------------------------------------------------------------------------------------------------
MS_U16 _u16CIPHER_ThreadID = 1;

//-------------------------------------------------------------------------------------------------
//  Local Functions
//-------------------------------------------------------------------------------------------------
static MS_U32 _CIPHERLockResource(void**ppstRes, CIPHER_RESOURCE_PRIVATE**ppstResPri)
{
    if(UtopiaResourceObtain(pModuleCipher, E_CIPHER_RESOURCE, ppstRes) != UTOPIA_STATUS_SUCCESS)
    {
        CIPHER_MSG("UtopiaResourceObtain fail\n");
        return UTOPIA_STATUS_ERR_RESOURCE;
    }
    UtopiaResourceGetPrivate(*ppstRes, (void**)ppstResPri);
    return UTOPIA_STATUS_SUCCESS;

}
static MS_U32 _CIPHERUnLockResource(void * pstRes)
{
...
MS_U32 CIPHERIoctl(void* pInstance, MS_U32 u32Cmd, void* pu32Args)
{
    ...
    if(UTOPIA_STATUS_SUCCESS != _CIPHERLockResource(&pstRes, &pstResPri))
    {
        CIPHER_MSG("UtopiaResourceObtain fail\n");
        return UTOPIA_STATUS_ERR_RESOURCE;
    }
    ...
    if((FALSE == pstResPri->bInited_Drv)  &&
        (u32Cmd != E_MDRV_CMD_CIPHER_Init) &&
        (u32Cmd != E_MDRV_CMD_CIPHER_DbgLevel))
    {
        _CIPHERUnLockResource(pstRes);
    ...
    switch(u32Cmd)
    {
        ...
        case E_MDRV_CMD_CIPHER_Init:
        {
            ...
            if(pstResPri->bInited_Drv == FALSE)
            {
                ...
                MDrv_SYS_GetTEEInfo(&pstResPri->stTeeInfo);
                pstResPri->bEnableTEE =((_MDrv_CIPHER_IsSupportTEE() == FALSE) && (pstResPri->stTeeInfo.OsType == SYS_TEEINFO_OSTYPE_OPTEE));
                printf("CIPHER pstResPri->stTeeInfo.OsType=%x\n", pstResPri->stTeeInfo.OsType);
                printf("CIPHER pstResPri->bEnableTEE=%x\n", pstResPri->bEnableTEE);
            }
            else
            {
        ...
        case E_MDRV_CMD_CIPHER_DMACalc:
        {
            ...
            if(pstResPri->bEnableTEE)
            {
                pstDMACalc->ret = _MDrv_CIPHER_TEE_DMACalc(pstDMACalc->stCfg, pstDMACalc->pu32CmdId);
            }
            else
            {
                //set value from resource private to local file variable
                //the local file pointer variable will extern to drvCIPHER.c
                if(_u16CIPHER_ThreadID < pstResPri->u16ThreadID)
                {
                    _u16CIPHER_ThreadID = pstResPri->u16ThreadID;
                }
                else
                {
                    pstResPri->u16ThreadID = _u16CIPHER_ThreadID;
                }
        ...
        case E_MDRV_CMD_CIPHER_HASH:
        {
            ...
            if(pstResPri->bEnableTEE)
            {
                 pstHASHCalc->ret = _MDrv_CIPHER_TEE_HASH(pstHASHCalc->stCfg, pstHASHCalc->pu32CmdId);
            }
            else
            {
                if(_u16CIPHER_ThreadID < pstResPri->u16ThreadID)
...
//	       TODO: Add E_MDRV_CMD_CIPHER_HMAC ioctl
//	        case E_MDRV_CMD_CIPHER_HMAC:
//	        {
//
//	        }
//	        break;

//	       TODO: Add E_MDRV_CMD_CIPHER_HASHManual ioctl
//	        case E_MDRV_CMD_CIPHER_HASHManual:
//	        {
//
//	        }
//	        break;
        ...
        case E_MDRV_CMD_CIPHER_IsDMADone:
        {
            ...
            if(pstResPri->bEnableTEE)
            {
                if(TRUE == _MDrv_CIPHER_TEE_IsDMADone(pstIsDMADone->u32CmdId, pstIsDMADone->pu32Exception))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
                else
        ...
        case E_MDRV_CMD_CIPHER_IsHASHDone:
        {
            ...
            if(pstResPri->bEnableTEE)
            {
                if(TRUE == _MDrv_CIPHER_TEE_IsHASHDone(pstIsHASHDone->u32CmdId, pstIsHASHDone->pu32Exception))
                {
                    u32Ret = UTOPIA_STATUS_SUCCESS;
                }
                else
