...
#if CIPHER_UTOPIA2
    extern MS_U16 _u16CIPHER_ThreadID;
#else
    static MS_U16 _u16CIPHER_ThreadID = 1;
#endif
...
static void _CIPHER_Isr(void)
{
    ...
    if(E_CIPHER_DMA_DONE == _eCipherWait)
    {
        ...
        HAL_CIPHER_GetException(&u32ExcMsg[0]);
        ...
        _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[0] = u32ExcMsg[0];
        _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[1] = u32ExcMsg[1];
...
static void _CIPHER_Isr_Proc(void)
{
    ...
    while (1)
    {
        ...
        if (HAS_FLAG(u32Events, DRV_CIPHER_DONE))
        {
            ...
            u32ExcTmp[0]  = _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[0];
            u32ExcTmp[1]  = _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[1];
            eKeySrc    = _CIPHER_ReportBuffer[u8BufNum].eKeySrc;
            u8KeyIdx   = _CIPHER_ReportBuffer[u8BufNum].u8KeyIdx;

            u32ExcMsg  = HAL_CIPHER_ReadException(&u32ExcTmp[0]);
            ...
            _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[0]       = 0;
            _CIPHER_ReportBuffer[u8BufNum].u32ExcMsg[1]       = 0;
...
MS_BOOL _MDrv_CIPHER_IsDMADone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(FALSE == HAL_CIPHER_DMA_CmdDone(u32CmdId, &u32Ret[0]))
    {
    ...
    if((u32Rpt[0] & DRV_CIPHER_REPORT_OK) == 0)
    {
		...
		if(u32Ret[0] != 0 || u32Ret[1] != 0)
    	{
        	*pu32Exception = HAL_CIPHER_ReadException(&u32Ret[0]);
        	HAL_CIPHER_ResetException();
    	}
    }
...
MS_BOOL _MDrv_CIPHER_TEE_IsDMADone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(TRUE != _MDrv_CIPHER_TEE_Send_CMD(E_TEE_CMD_CIPHER_IsDMADone, &pArgs, sizeof(CIPHER_ISDMADONE)))
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_CIPHER_IsDMADone);
        return FALSE;
    }
...
DLL_PUBLIC MS_BOOL MDrv_CIPHER_IsDMADone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_CIPHER_TEE_IsDMADone(u32CmdId, pu32Exception);
    }
...
MS_BOOL _MDrv_CIPHER_IsHASHDone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(FALSE == HAL_CIPHER_Hash_CmdDone(u32CmdId, &u32Ret[0]))
    {
...
MS_BOOL _MDrv_CIPHER_TEE_IsHASHDone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(TRUE != _MDrv_CIPHER_TEE_Send_CMD(E_TEE_CMD_CIPHER_IsHASHDone, &pArgs, sizeof(CIPHER_ISHASHDONE)))
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_CIPHER_IsHASHDone);
        return FALSE;
    }
...
DLL_PUBLIC MS_BOOL MDrv_CIPHER_IsHASHDone(MS_U32 u32CmdId, MS_U32 *pu32Exception)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_CIPHER_TEE_IsHASHDone(u32CmdId, pu32Exception);
    }
...
{
    return HAL_CIPHER_IsSupportTEE();
}
...
DRV_CIPHER_RET _MDrv_CIPHER_InitBySWFlag(MS_BOOL bSWInit)
{
    ...
    for(i=0;i<CIPHER_REPORT_BUFFER_MAX;i++)
    {
        ...
        _CIPHER_ReportBuffer[i].u32ExcMsg[0]       = 0;
        _CIPHER_ReportBuffer[i].u32ExcMsg[1]       = 0;
    ...
    // TODO: need to check if it is SW init or HW init
    MDrv_SYS_ReadKernelCmdLine();
    MDrv_SYS_GetTEEInfo(&stTeeMode);
    _bEnableTEE = ((stTeeMode.OsType == SYS_TEEINFO_OSTYPE_OPTEE) && (HAL_CIPHER_IsSupportTEE() == FALSE));
    DEBUG_CIPHER(CIPHER_DBG_INFO, "stTeeMode.OsType=%x\n", stTeeMode.OsType);

    if(_bEnableTEE == TRUE)
    {
        if(FALSE == _MDrv_CIPHER_TEE_Open())
        {
            printf("\033[31mOpen CIPHER TA Fail\n\033[m");
        }
        else
        {
            printf("\033[32mOpen CIPHER TA Success\n\033[m");
        }
    }
    return DRV_CIPHER_OK;
}
...
DRV_CIPHER_RET _MDrv_CIPHER_ResetKey(MS_U32 u32CAVid, MS_U32 u32KeyIdx)
{
    ...
    HAL_CIPHER_DMA_Set_InputSrcFrom(E_CIPHER_DIRECT, u8Data, sizeof(u8Data));
    ...
    //Check register report
    while((FALSE == HAL_CIPHER_DMA_CmdDone(0, &u32Ret[0])) && (u32WaitCnt < HAL_CIPHER_RESETKEY_TIMEOUT_VALUE))
    {
...
DRV_CIPHER_RET _MDrv_CIPHER_DMACalc(DRV_CIPHER_DMACFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    //Check report buffer
    u8BufNum = (MS_U8)(_u16CIPHER_ThreadID % (MS_U16)CIPHER_REPORT_BUFFER_MAX);
    ...
    if(FALSE == HAL_CIPHER_DMA_Start(stCfg.bDecrypt, eIntMode , TRUE , _u16CIPHER_ThreadID))
    {
    ...
    DEBUG_CIPHER(CIPHER_DBG_INFO, "INFO:IntMode[%d], CmdId[%hu]", eIntMode, _u16CIPHER_ThreadID);

    *pu32CmdId = (MS_U32)_u16CIPHER_ThreadID;

    //Check thread id max
    if(_u16CIPHER_ThreadID == HAL_CRYPTODMA_THREAD_ID_MAX)
    {
        _u16CIPHER_ThreadID = 1;
    }
    else
...
DRV_CIPHER_RET _MDrv_CIPHER_TEE_DMACalc(DRV_CIPHER_DMACFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    pArgs.ret = DRV_CIPHER_OK;
    if(TRUE != _MDrv_CIPHER_TEE_Send_CMD(E_TEE_CMD_CIPHER_DMACalc, &pArgs, sizeof(CIPHER_DMACALC)))
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_CIPHER_DMACalc);
        return DRV_CIPHER_FAIL;
    }
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_DMACalc(DRV_CIPHER_DMACFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_CIPHER_TEE_DMACalc(stCfg, pu32CmdId);
    }
    return _MDrv_CIPHER_DMACalc(stCfg, pu32CmdId);
#endif
}

DRV_CIPHER_RET _MDrv_CIPHER_OTPHASHCalc(DRV_CIPHER_DMACFG stCfg, MS_U32 *pu32CmdId)
{
    DRV_CIPHER_RET ret = DRV_CIPHER_OK;
    HAL_CIPHER_INTMODE eIntMode = E_DMA_INT_NONE;
    MS_U8 u8BufNum = 0;
    ...
    for(u32CurrentRound=0; u32CurrentRound<=u32OTPHashRound; u32CurrentRound++)
    {
        ...
        if(FALSE == HAL_CIPHER_OTPHash_Start(eIntMode , TRUE , _u16CIPHER_ThreadID))
        {
            DEBUG_CIPHER(CIPHER_DBG_ERR, "Error:DMA Fail!\n");
            ret =  DRV_CIPHER_FAIL;
            goto DMA_FINISH;
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_OTPHASHCalc(DRV_CIPHER_DMACFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    return _MDrv_CIPHER_OTPHASHCalc(stCfg, pu32CmdId);
}
...
DRV_CIPHER_RET _MDrv_CIPHER_HASH(DRV_CIPHER_HASHCFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    //Start hash
    if(FALSE == HAL_CIPHER_Hash_Start(eIntMode , TRUE, FALSE, _u16CIPHER_ThreadID))
    {
        ret = DRV_CIPHER_FAIL;
        goto HASHAUTO_FINISH;
    }
...
DRV_CIPHER_RET _MDrv_CIPHER_TEE_HASH(DRV_CIPHER_HASHCFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    pArgs.pu32CmdId = pu32CmdId;
    pArgs.ret = DRV_CIPHER_OK;
    if(TRUE != _MDrv_CIPHER_TEE_Send_CMD(E_TEE_CMD_CIPHER_HASH, &pArgs, sizeof(CIPHER_HASHCALC)))
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_CIPHER_HASH);
        return DRV_CIPHER_FAIL;
    }
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_HASH(DRV_CIPHER_HASHCFG stCfg, MS_U32 *pu32CmdId)
{
    ...
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_CIPHER_TEE_HASH(stCfg, pu32CmdId);
    }
    return _MDrv_CIPHER_HASH(stCfg, pu32CmdId);
#endif
}

DRV_CIPHER_RET _MDrv_CIPHER_HASHManual(DRV_CIPHER_HASHCFG_MANUAL stCfg, MS_U32 *pu32CmdId)
{
    DRV_CIPHER_RET      ret      = DRV_CIPHER_OK;
    HAL_CIPHER_INTMODE  eIntMode = E_DMA_INT_NONE;
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_HASHManual(DRV_CIPHER_HASHCFG_MANUAL stCfg, MS_U32 *pu32CmdId)
{
    if(_bEnableTEE == TRUE)
    {
        DEBUG_CIPHER(CIPHER_DBG_WARN, "Not Support in TEE");

        return DRV_CIPHER_FAIL;
    }
    return _MDrv_CIPHER_HASHManual(stCfg, pu32CmdId);
}

DRV_CIPHER_RET _MDrv_CIPHER_HMAC(DRV_CIPHER_HMACCFG stCfg, MS_U32 *pu32CmdId)
{
    DRV_CIPHER_RET      ret      = DRV_CIPHER_OK;
    HAL_CIPHER_INTMODE  eIntMode = E_DMA_INT_NONE;
    MS_U32 u32OutLen   = 0;
    ...
    while(FALSE == HAL_CIPHER_Hash_CmdDone(_u16CIPHER_ThreadID, &u32ErrorMsg[0]) && u32WaitCnt < HASH_TIMEOUT_VALUE)
    {
    ...
    if(u32ErrorMsg[0] != 0 || u32ErrorMsg[1] != 0)
    {
        HAL_CIPHER_ReadException(&u32ErrorMsg[0]);
    }
    ...
    u8BufNum = _u16CIPHER_ThreadID % (MS_U32)CIPHER_REPORT_BUFFER_MAX;
...
{
    CIPHER_HMACCALC pArgs;

    memset(&pArgs, 0, sizeof(CIPHER_HMACCALC));
    memcpy(&pArgs.stCfg, &stCfg, sizeof(DRV_CIPHER_HMACCFG));

    pArgs.pu32CmdId = pu32CmdId;
    pArgs.ret = DRV_CIPHER_OK;
    if(TRUE != _MDrv_CIPHER_TEE_Send_CMD(E_TEE_CMD_CIPHER_HMAC, &pArgs, sizeof(CIPHER_HASHCALC)))
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "[%s]ERROR : Send_TEE_CMD 0x%x fail\n", __FUNCTION__, E_TEE_CMD_CIPHER_HMAC);
        return DRV_CIPHER_FAIL;
    }
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_HMAC(DRV_CIPHER_HMACCFG stCfg, MS_U32 *pu32CmdId)
{
    if(_bEnableTEE == TRUE)
    {
        return _MDrv_CIPHER_TEE_HMAC(stCfg, pu32CmdId);
    }
    return _MDrv_CIPHER_HMAC(stCfg, pu32CmdId);
}
...
DLL_PUBLIC DRV_CIPHER_RET MDrv_CIPHER_DMAParser(CIPHER_PARSERCFG stParserCfg, MS_U32 *pu32CmdId)
{
    ...
    if(FALSE == HAL_CIPHER_DMA_Start(stParserCfg.bDecrypt, eIntMode , TRUE , _u16CIPHER_ThreadID))
    {
...
DRVAESDMA_RESULT _MDrv_AESDMA_IsFinished(MS_U32 *u32RetVal)
{
    ...
    if(NULL == u32RetVal)
    {
        DEBUG_CIPHER(CIPHER_DBG_ERR, "Error: u32RetVal is NULL\n");
        return DRVAESDMA_FAIL;
    }
    ...
    if(TRUE == bDone)
    {
        if(0 != u32Ret)
        {
            DEBUG_CIPHER(CIPHER_DBG_ERR, "Error:Cipher error [0x%08x]!", (unsigned int)u32Ret);
