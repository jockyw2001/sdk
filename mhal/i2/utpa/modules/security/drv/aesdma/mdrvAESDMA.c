...
MS_U32 AESDMAMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
...
/////////
            char temp[255]={0};
            MS_U32 u32ParaSize;
            EN_AESDMA_MDBCMDLINE eMdbCmdLine = AESDMA_MDBCMDLINE_NONE;
            char *pch;
            pch = strchr(paraCmdLine->pcCmdLine, ' ');
            if(pch == NULL)
                u32ParaSize = paraCmdLine->u32CmdSize;
            else
                u32ParaSize = pch - &paraCmdLine->pcCmdLine[0];
            //strcpy(temp," ");
            //pch = strtok(paraCmdLine->pcCmdLine,temp);

            MdbPrint(paraCmdLine->u64ReqHdl,"u32ParaSize = %d \n",u32ParaSize);
            switch(u32ParaSize)
            {
                char *pstr;
                case 4:
                    strcpy(temp,"help");
                    pstr = strstr(paraCmdLine->pcCmdLine, temp);
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = AESDMA_MDBCMDLINE_HELP;
                    }
                    break;
                case 6:
                    strcpy(temp,"aesdma");
                    ...
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = AESDMA_MDBCMDLINE_AESDMA;
                    }
                    break;
                case 3:
                    strcpy(temp,"sha");
                    ...
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = AESDMA_MDBCMDLINE_SHA;
                    }
                    strcpy(temp,"rsa");
                    pstr = strstr(paraCmdLine->pcCmdLine, temp);
                    if(pstr != NULL)
                    {
                        eMdbCmdLine = AESDMA_MDBCMDLINE_RSA;
                    }
                    break;
                default:
                    eMdbCmdLine = AESDMA_MDBCMDLINE_NONE;
                    break;
            }
            MDrv_AESDMA_MdbCmdLine(paraCmdLine, eMdbCmdLine);
            //////////
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MdbPrint(paraGetInfo->u64ReqHdl,"---------MStar Info---------\n");
            MdbPrint(paraGetInfo->u64ReqHdl,"A. AESDMA \n");
            MdbPrint(paraGetInfo->u64ReqHdl,"B. SHA\n");
            MdbPrint(paraGetInfo->u64ReqHdl,"C. RSA\n");
            MdbPrint(paraGetInfo->u64ReqHdl,"Use 'echo' to get miu information\n");
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
    UtopiaModuleRegisterMdbNode("aesdma", (FUtopiaMdbIoctl)AESDMAMdbIoctl);
#endif
...
MS_U32 AESDMAOpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
    ...
    pAesdmaPri->fpCIPHERDMACalc = _MDrv_CIPHER_DMACalc;
    pAesdmaPri->fpCIPHERIsDMADone = _MDrv_CIPHER_IsDMADone;
    pAesdmaPri->fpCIPHERDMACalcList = _MDrv_CIPHER_DMACalcList;
#endif
...
MS_U32 AESDMAIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
...
#if AESDMS_SECURED_BANK_EN
    PCIPHER_DMACALC pCipherDMACalc = NULL;
    PCIPHER_ISDMADONE pCipherIsDMADone = NULL;
    PCIPHER_DMACALCLIST pCipherDMACalcList = NULL;
#endif
    ...
    switch(u32Cmd)
    {
        ...
        case MDrv_CMD_AESDMA_Start:
            {
                ...
                if (psAesInstPri->bHwParserEn == TRUE)
                {
                    ...
                    if (psAesInstPri->bBypassPid == TRUE)
                    {
                        ...
                        (psAesInstPri->fpAESDMAParserBypassPid)(TRUE);
                    }
...
#if AESDMS_SECURED_BANK_EN
        case MDrv_CMD_CIPHER_DMACALC:
            {
                pCipherDMACalc = (PCIPHER_DMACALC) pArgs;
                if (pCipherDMACalc == NULL){
                    return UTOPIA_STATUS_FAIL;
                }
                if ( psAesInstPri->fpCIPHERDMACalc(pCipherDMACalc->stCfg, pCipherDMACalc->pu32CmdId) == DRV_CIPHER_OK){
                    return UTOPIA_STATUS_SUCCESS;
                }
                else{
                    return UTOPIA_STATUS_FAIL;
        ...
        case MDrv_CMD_CIPHER_ISDMADONE:
            {
                pCipherIsDMADone = (PCIPHER_ISDMADONE) pArgs;
                if( pCipherIsDMADone == NULL){
                    return UTOPIA_STATUS_FAIL;
                }
                if ( psAesInstPri->fpCIPHERIsDMADone(pCipherIsDMADone->u32CmdId, pCipherIsDMADone->pu32Exception) == DRV_CIPHER_OK){
                    return UTOPIA_STATUS_SUCCESS;
                }
                else{
                    return UTOPIA_STATUS_FAIL;
        ...
        case MDrv_CMD_CIPHER_DMACALCLIST:
            {
                pCipherDMACalcList = (PCIPHER_DMACALCLIST) pArgs;
                if ( pCipherDMACalcList == NULL){
                    return UTOPIA_STATUS_FAIL;
                }
                if ( psAesInstPri->fpCIPHERDMACalcList(pCipherDMACalcList->pstCfg, pCipherDMACalcList->u32NumOfCmds, pCipherDMACalcList->pu32CmdId, pCipherDMACalcList->BufferAddr, pCipherDMACalcList->BufferLen, pCipherDMACalcList->CMDQ_MIU_SELECT) == DRV_CIPHER_OK){
                    return UTOPIA_STATUS_SUCCESS;
                }
                else{
                    return UTOPIA_STATUS_FAIL;
