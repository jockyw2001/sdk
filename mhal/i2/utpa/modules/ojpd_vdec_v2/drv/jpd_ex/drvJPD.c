...
{
    JPD_DEBUG_DRV_MSG("_JPD_ISRHandler: IsrNum = %td\n", (ptrdiff_t)IsrNum);
 #ifdef MSOS_TYPE_LINUX_KERNEL
    gbCallbackFlag = TRUE;
#else
    if(_pJPDIsrCb)
...
#if 1
    JPD_DEBUG_DRV_MSG ("   @JPD on MIU %d !!!\n", u8MiuSel);
    HAL_JPD_SetMIU(u8MiuSel);
#else
    switch(u8MiuSel)
    {
        case 0:
            JPD_DEBUG_DRV_MSG ("   JPD on MIU 0 !!!\n");
            _MDrv_MIU_SelMIU(MIU_CLIENT_JPD_RW, MIU_SELTYPE_MIU0);
            break;
        case 1:
            JPD_DEBUG_DRV_MSG ("   JPD on MIU 1 !!!\n");
            _MDrv_MIU_SelMIU(MIU_CLIENT_JPD_RW, MIU_SELTYPE_MIU1);
            break;
        case 2:
            JPD_DEBUG_DRV_MSG ("   JPD on MIU 2 !!!\n");
            _MDrv_MIU_SelMIU(MIU_CLIENT_JPD_RW, MIU_SELTYPE_MIU2);
            break;
        case 3:
            JPD_DEBUG_DRV_MSG ("   JPD on MIU 3 !!!\n");
            _MDrv_MIU_SelMIU(MIU_CLIENT_JPD_RW, MIU_SELTYPE_MIU3);
            break;
        default:
            JPD_DEBUG_DRV_MSG ("   JPD on MIU 0 !!!@\n");
            _MDrv_MIU_SelMIU(MIU_CLIENT_JPD_RW, MIU_SELTYPE_MIU0);
...
///@return none
/******************************************************************************/
void MDrv_JPD_SetReadBuffer(MS_PHY u32BufAddr, MS_U32 u32BufSize)
{
...
///@return none
/******************************************************************************/
void MDrv_JPD_SetMRCStartAddr(MS_PHY u32ByteOffset)
{
    MS_U8 u8MiuSel;
...
///@return none
/******************************************************************************/
void MDrv_JPD_SetOutputFrameBuffer(MS_PHY u32BufAddr, MS_U16 u16LineNum)
{
    MS_U8 u8MiuSel;
...
///@return none
/******************************************************************************/
void MDrv_JPD_SetWPENEndAddr(MS_PHY u32ByteOffset)
{
    MS_U8 u8MiuSel;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
...
void MDrv_JPD_PrintGTable(void)
{
       ...
       MDrv_JPD_Set_S_Config(MDrv_JPD_Get_S_Config() & ~JPD_TBL_RDY);
        HAL_JPD_Set_TID_Addr(JPD_MEM_SCWGIF_BASE);
        for(u16Cnt=0; u16Cnt<128; u16Cnt++)
        {
            printf("%04x\n", HAL_JPD_Get_TID_Dat());
            if(u16Cnt%16==15)
            {
...
void MDrv_JPD_PrintHTable(void)
{
       ...
       MDrv_JPD_Set_S_Config(MDrv_JPD_Get_S_Config() & ~JPD_TBL_RDY);
        HAL_JPD_Set_TID_Addr(JPD_MEM_SYMIDX_BASE);
        for(u16Cnt=0; u16Cnt<272; u16Cnt++)
        {
            printf("%04x\n", HAL_JPD_Get_TID_Dat());
            if(u16Cnt%16==15)
            {
