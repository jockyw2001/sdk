...
static void ms_usb_blocking_completion(struct urb *request_block, struct stPtRegs *registers)
{
    ...
    MsOS_SetEvent(request_block->WaitFlag_storage, BLK_COMPLETE);
}
...
static void ms_usb_wait_for_completion_timeout(struct stCompletion *cpt, S32 flag, struct ms_usdata *ptr, U32 wait_ms)
{
	...
	ms_usbhost_err("[timeout] kill urb (qh:%p)!\n", urb->hcpriv);
	ms_usbhost_err("SCSI command %x, len %d\n",
		ptr->srb.cmnd[0], urb->u32TransferBufferLength);
	...
	if (cpt->done == 1) {
        ...
        ms_usbhost_err("<%s> unlink urb done ret(%d)...\n", __func__, retv);
