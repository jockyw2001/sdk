...
static U16 ms_periodic_usecs (struct ehci_hcd *pEhci, U32 u32Frame, U32 u32uFrame)
{
	 ...
	 */
	if (u32uSecs > 100)
		ms_debug_err("overrun u32uFrame %d, periodic is %d u32uSecs\n",
...
#if 1
	u32Usecs = pEhci->uframe_periodic_max - u32Usecs;
#else
...
static int ms_qh_link_periodic (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	for (i = qh->u16Start; i < ehci->u32PeriodicSize; i += period) {
		...
		while (here.ptr) {
			...
			if (type == QH_TYPE)
				break;
			prev = ms_periodic_next_shadow(prev, type);
			hw_p = &here.qh->hw_next_qh;
			here = *prev;
		}
		#endif
		...
		if (pQh->u16Period) {
			...
			for (i = pQh->u16Period; iStatus && i > 0; --i) {
				...
				for (u32uFrame1 = 0; u32uFrame1 < 8; u32uFrame1++)
				{
					...
					{
						ms_debug_debug("[USB] first fit (frame,uframe)=(%d,%d)\n", u32Frame1, u32uFrame1);
						break;
					}
				}
	...
	)
{
	...
	dev = (struct s_hcd_dev *)pUrb->dev->hcpriv; // for ep pointer
	...
	/* not care about usbfs bw periodic stats */
...
static void ms_scan_periodic_intr(struct ehci_hcd *pEhci, struct stPtRegs *pRegs)
{
...
restart:
		...
		while (q.ptr != 0)
		{
			...
			{
				ms_debug_err("corrupt u32Type %d u32Frame %d shadow %p\n",
