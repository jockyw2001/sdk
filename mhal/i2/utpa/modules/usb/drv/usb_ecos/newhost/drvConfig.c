...
/**
     ...
     */
static int ms_usb_parse_endpoint(
        ...
        int iBufSize)
{
    ...
    {
        ms_usbhost_err("pEpt descriptor size is too large (%d)\n", desc->bLength);
        return -1;
    }
    ...
    pEpt->desc.wMaxPacketSize = pEpt->desc.wMaxPacketSize; // le_to_cpu

    if (dev->eSpeed == USB_HIGH_SPEED
        && (pEpt->desc.bmAttributes & 0x03) == USB_ENDPOINT_XFER_BULK) {
        if (512 != pEpt->desc.wMaxPacketSize)
        {
            ms_usbhost_err("bulk pEpt max packet size is not 512\n");
        }
    }
        ...
        int bufsize)
{
    ...
    if (!pIntf->altsetting)
    {
        ms_usbhost_err("failed to alloc altsetting\n");
        return -1;
    }
    ...
    while (bufsize > 0)
    {
        if (pIntf->num_altsetting >= pIntf->max_altsetting) //Allocate more memory for altset
        {
            ...
            {
                ms_usbhost_err("altsettings are over (%d)\n",
			USB_MAX_ALTSETTING);
                return -1;
            }
            ...
            if (pInft_tmp == NULL)
            {
                ms_usbhost_err("failed to alloc mem for altsetting\n");
                return -1;
            }
        ...
        if ( (pIf->desc.bNumEndpoints > 0) && (pIf->desc.bNumEndpoints <= USB_MAX_ENDPOINTS) )
        {
            ...
            if (!pIf->endpoint)
            {
                ms_usbhost_err("fail to alloc mem for endpoint\n");
                return -1;
            }
            ...
            {
                retval = ms_usb_parse_endpoint(dev, pIf->endpoint + i, pBuf, bufsize);
                if (retval < 0)
                    return retval;
        ...
        char *pBuf)
{
    ...
    {
        ms_usbhost_err("too many interfaces %d\n",
		pConfig->desc.bNumInterfaces);
        return -EINVAL;
    }
    ...
    for (i = 0; i < pConfig->desc.bNumInterfaces; i++)
    {
        ...
        {
            ms_usbhost_err("fail to alloc mem for interface\n");
            retval = -ENOMEM;
            goto error_exit;
        }
...
int ms_usb_get_config(struct usb_device *dev)
{
    ...
    {
        ms_usbhost_err("number of config is invalid\n");
        return -EINVAL;
    }
    ...
    if (!dev->config)
    {
        ms_usbhost_err("fail to alloc mem for config\n");
        return -ENOMEM;
    }
    ...
    if (!dev->pRawDescriptors)
    {
        ms_usbhost_err("fail to alloc mem for descriptors\n");
        return -ENOMEM;
    }

    pBuf = (unsigned char*) kmalloc(USB_DT_CONFIG_SIZE, GFP_KERNEL);
    if (!pBuf)
    {
        ms_usbhost_err("unable to allocate memory for configuration descriptors\n");
        return -ENOMEM;
    }
    ...
    {
        retval = ms_usb_get_descriptor(dev, USB_DT_CONFIG, cfg_num, pBuf, USB_DT_CONFIG_SIZE);
        if (retval < 0)
        {
            ms_usbhost_err("fail to get config descriptor %d\n", retval);
            goto error_exit;
        }
        ...
        if (pBuf_tmp == NULL)
        {
            ms_usbhost_err("fail to alloc mem for config desc\n");
            retval = -ENOMEM;
            goto error_exit;
        }
