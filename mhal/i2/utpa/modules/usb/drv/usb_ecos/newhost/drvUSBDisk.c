...
#define US_SCSI_ABORT		(1 << 0)
#define US_REMOVE_DISK		(1 << 1)
MS_BOOL MDrv_USB_Force_HDD_Standby(char *str, int attr)
{
	...
	U8 uPort = 0;
	U8 uLunNum = 0;
	U8 abort = !!(attr & US_SCSI_ABORT);
	U8 rm_disk = !!(attr & US_REMOVE_DISK);
	BOOL retval = FALSE;
	...
	for (t = &__DEVTAB__[0]; t != &__DEVTAB_END__; t++) {
		...
		if((p_usb_info->DevId != (-1)) && (strcmp(str, t->name) == 0)) {
			uPort = p_usb_info->uPort;
			uLunNum = p_usb_info->uLunNum;
			break;
		}
	}

	if ((uPort == 0) && (uLunNum == 0)) {
		diag_printf("[USB] Not found any device!\n");
		return FALSE;
	}
	retval =  ms_bSCSI_ATA_THRU_STANDBY(uPort, uLunNum, abort);
	if (rm_disk)
