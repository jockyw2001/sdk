...
int ms_usb_register(struct usb_driver *n_driver)
{
    ...
    if (!s32Retval)
    {
        ms_usbhost_msg("usbcore: regtered new driver %s\n", n_driver->name);
    }
    else
    {
        ms_usbhost_err("usbcore: problem %d when registering driver %s\n",
...
int ms_usb_new_device(struct usb_device *pUdev, struct device_s *parent)
{
    ...
    /* announce device information */
    ms_usbhost_msg("usb %s: "
	    "New USB device strings: Mfr=%d, Product=%d, SerialNumber=%d\n",
	    pUdev->dev.bus_id,
	    pUdev->descriptor.iManufacturer,
	    pUdev->descriptor.iProduct,
	    pUdev->descriptor.iSerialNumber);
    ...
    if (pUdev->pManufacturerString)
        ms_usbhost_msg("usb %s: USB device manufacturer = %s\n",
            pUdev->dev.bus_id,
            pUdev->pManufacturerString);
    ...
    if (pUdev->pProductString)
        ms_usbhost_msg("usb %s: USB device product = %s\n",
            pUdev->dev.bus_id,
            pUdev->pProductString);
    ...
    if (pUdev->pSerialNumberString)
        ms_usbhost_msg("usb %s: USB device serial number = %s\n", 
            pUdev->dev.bus_id,
            pUdev->pSerialNumberString);
