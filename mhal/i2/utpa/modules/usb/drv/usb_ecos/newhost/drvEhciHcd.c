...
#if defined(_USB_128_ALIGMENT) && (_USB_128_ALIGMENT==1)
#define TD_QH_OFFSET 0x80
#else
#define TD_QH_OFFSET 0x60
#endif
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	struct ehci_qh *qh;
	unsigned int *hw_p;
	unsigned int hw_next, async_reg;
	int cqh_n_list = 1;

	/* print out async register */
	async_reg = hcd_reg_readl ((U32)&ehci->op_regs->asynclistaddr);
	diag_printf("[EHCI] async register %x, %s\n", async_reg,
		ms_isHcdRunning(hcd) ? "RUNNING" : "IDLE");
	/* print out hardware asynchronous qh link */
	hw_next = (U32)ehci->stAsync;
	diag_printf("--> %8x", hw_next);
	diag_printf("--> %8x", hw_next+TD_QH_OFFSET*1);
	diag_printf("--> %8x", hw_next+TD_QH_OFFSET*2);
	diag_printf("--> %8x", hw_next+TD_QH_OFFSET*3);
	diag_printf("--> %8x\n", hw_next+TD_QH_OFFSET*4);

	hw_p = (unsigned int *)hw_next;
	diag_printf("--> %8x", *hw_p);
	hw_next += TD_QH_OFFSET;
	hw_p = (unsigned int *)hw_next;
	diag_printf("--> %8x", *hw_p);
	hw_next += TD_QH_OFFSET;
	hw_p = (unsigned int *)hw_next;
	...
	if (qh == 0) {
		diag_printf("[EHCI] no SW QH list\n");
		return;
	}
	do {
		unsigned int c_qtd;
		...
		if (qh->qh_dma_addr == async_reg)
			cqh_n_list = 0;
		c_qtd = (unsigned int)PA2VA(qh->hw_current_qtd);
		//if ((qh->hw_token & QTD_STS_ACT) == 0)
		//	cqtd_n_list = 0;
	...
	if (cqh_n_list)
		Dump_TD((unsigned int)ehci->stAsync, 1);
}
#endif
...
)
{
  ...
  case Req_ClearPortFeature:
    ...
    case USB_PORT_FEAT_ENABLE:
      ...
      // not to reset EHCI at this time
      //Halt HCD
      //ms_host_halt(pEhci);
...
ms_urb_done (struct ehci_hcd *pEhci, struct urb *pUrb, struct stPtRegs *pRegs)
{
  ...
  osapi_spin_unlock (&pUrb->lock);
	  ...
	  diag_printf ("%s %s urb %p ep%d%s status %d len %d/%d\n",
		  __func__, pUrb->dev->devpath, pUrb,
		  usb_pipeendpoint (pUrb->u32Pipe),
		  usb_pipein (pUrb->u32Pipe) ? "in" : "out",
		  pUrb->s32Status,
		  pUrb->actual_length, pUrb->u32TransferBufferLength);
#endif
...
)
{
  ...
  if (pQh == 0)
  {
      ms_qtd_list_free (pEhci, qtd_list);
      return -ENOMEM;
  }
  ms_debug_func("--[%s] line:[%d]\n", __FUNCTION__, __LINE__);
  return 0;
...
//      || !spin_is_locked (&pEhci->tHcdLock)
  ...
  Chip_Flush_Memory(); /* MStar patch (20140827 to push the pipe) */
...
static void ms_ehci_watchdog (U32 u32Param)
{
    ...
    ms_debug_func("++[%s] %s:%d\n", __FUNCTION__, __FILE__, __LINE__);
    //diag_printf("[%s] action: 0x%x\n", __func__, pEhci->u32Actions);
    osapi_spin_lock_irqsave (&pEhci->tHcdLock, u32Flags);
...
static void ms_ehci_work(struct ehci_hcd *pEhci, struct stPtRegs *regs)
{
  ...
  if (pEhci->iNextUframe != -1)
    ms_scan_periodic_intr(pEhci, regs);
  pEhci->iScanning = 0;
...
void ms_ehci_irq (struct usb_hcd *pHcd, struct stPtRegs *pRegs)
{
  ...
  if (u32Status & USBSTS_PCD)
  {
      ...
      if (iPsc == pHcd->saved_ehci_state.iPsc &&
            u32Status == pHcd->saved_ehci_state.usbsts &&
      ...
      pHcd->saved_ehci_state.usbsts = u32Status;
 ...
 int ms_urb_dequeue (struct usb_hcd *pHcd, struct urb *pUrb)
{
  ...
  case EP_INTERRUPT:
    ...
    //break; // seems dummy part

    /* renewed old one, and reschedule being moved to intr_deschedule */
    //if (pQh->qh_status == QH_STS_LINKED)
    //{
