...
static __inline__ char *ms_device_speed(enum usb_device_speed speed)
{
...
__inline__ struct usb_hub *ms_hdev_to_struct_hub(struct usb_device *ms_dev)
{
	if (!ms_dev || !ms_dev->actconfig || !ms_dev->u32MaxChild)
		return NULL;
	return ms_usb_get_intfdata(ms_dev->actconfig->interface[0]);
}
...
/**
     ...
     * @return          error code
     */
int ms_hub_port_status(struct usb_device *ms_dev, int port1,
...
/**
     ...
     */
static int ms_hub_port_wait_reset(struct usb_device *hub, int port1,
        struct usb_device *ms_dev, U32 delay)
{
    ...
    for (s32Dtime = 0; s32Dtime < HUB_RESET_TIMEOUT; s32Dtime += delay)
    {
        ...
        s32Ret = ms_hub_port_status(hub, port1, &u16Portstatus, &u16Portchange);
        if (s32Ret < 0) {
            if (s32Ret == -ENODEV)
                return 1;
            else
            ...
            (u16Portstatus & USB_PORT_STAT_ENABLE))
        {
            ...
            ms_usbhost_debug("::Device Speed is %s\n", ms_portspeed(u16Portstatus));
            return 0;
        }
...
/**
     ...
     * @return          error code
     */
static int ms_hub_port_reset(struct usb_device *hub, int port1,
        struct usb_device *ms_dev, U32 delay)
{
    int i, status;
    ...
    for (i = 0; i < HUB_RESET_TRIES; i++)
    {
        ...
        ms_usb_control_cmd(hub, usb_sndctrlpipe(hub, 0),
            USB_REQ_SET_FEATURE, USB_RT_PORT, USB_PORT_FEAT_RESET, port1, NULL, 0, HZ);

        status = ms_hub_port_wait_reset(hub, port1, ms_dev, delay);
        ms_usbhost_debug("<hub_port_reset> loop %d, status %d\n", i, status);
        if (status != -1)
        {
            // (USB_PORT_FEAT_C_RESET)
            ms_usb_control_cmd(hub, usb_sndctrlpipe(hub, 0),
                USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_RESET, port1, NULL, 0, HZ);
...
/**
     ...
     * @return          error code
     */
int ms_hub_port_disable(struct usb_device *udev, int port1, int new_state)
{
	S32 s32Err = 0;
	struct usb_hub *pHub = ms_hdev_to_struct_hub(udev);

	if (udev->children[port1 - 1] && new_state) {
		diag_printf("[UD] %s set NOTATTACHED\n", __func__);
		ms_usb_set_device_state(udev->children[port1 - 1], USB_STATE_NOTATTACHED);
	}

	if (!pHub->error) { // check upstream hub status
		// (USB_PORT_FEAT_ENABLE)
		s32Err = ms_usb_control_cmd(udev, usb_sndctrlpipe(udev, 0),
        		USB_REQ_CLEAR_FEATURE, USB_RT_PORT,
			USB_PORT_FEAT_ENABLE, port1, NULL, 0, HZ);
	}
	if (s32Err) {
...
/**
     ...
     * @return          error code
     */
static int ms_hub_port_debounce(struct usb_device *hub, int port1)
{
    S32 s32Err;
    ...
    for (s32Dtime = 0; s32Dtime < HUB_DEBOUNCE_TIMEOUT; s32Dtime += HUB_DEBOUNCE_STEP)
    {
        ...
        s32Err = ms_hub_port_status(hub, port1, &u16Portstatus, &u16Portchange);
        if (s32Err < 0)
            return s32Err;
        ...
        if ((u16Portchange & USB_PORT_STAT_C_CONNECTION))
        {
            ...
            ms_usb_control_cmd(hub, usb_sndctrlpipe(hub, 0),
                USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_CONNECTION, port1, NULL, 0, HZ);
        }
    }

    ms_usbhost_debug("debounce: port %d: delay %dms stable %d status 0x%x\n",
        port1, s32Dtime, s32Stable_count, u16Portstatus);

    if (s32Stable_count < HUB_DEBOUNCE_STABLE)
        return -ETIMEDOUT;
    return u16Portstatus;
...
static struct usb_device* ms_hub_port_connect_change(
	struct usb_hub *hubstate, int port1, U16 portstatus, U16 portchange)
{
    ...
    if (pHub->children[port1 - 1])
        ms_usb_disconnect(&pHub->children[port1 - 1]);

    if (!(portstatus & USB_PORT_STAT_CONNECTION))
    {
        if (portstatus & USB_PORT_STAT_ENABLE)
            ms_hub_port_disable(pHub, port1, 1);
    ...
    // FIXME: debounce here?
    if (ms_hub_port_debounce(pHub, port1) < 0)
    {
  ...
  for (i = 0; i < HUB_PROBE_TRIES; i++) {
    ...
    if ((retv = ms_hub_port_reset(pHub, port1, pUdev, u32Delay))) {
        ms_usb_put_dev(pUdev);
	ms_usbhost_err("hub port reset tier %d, retv %d\n", i, retv);
        continue;
    }
    ...
    if (pHub->parent == NULL)
    {
	if (!ms_RoothubPortEnabled(pHcd))
        {
		diag_printf("[USB] ERR, after reset no PE\n");
		ms_usb_put_dev(pUdev);
		break;
        }
    }
    ...
    if (pHub->tt) {
      ...
      pUdev->u32TTPort = port1;
    }

    pdev = pUdev->parent;
    if (pdev->devpath [0] != '0')
      len = USB_sprintf (pUdev->devpath,"%s.%d", pdev->devpath, port1);
    else
      len = USB_sprintf (pUdev->devpath,"%d", port1);
    ...
    ms_usbhost_msg("new USB %s device on path usb:%s, assigned address %d\n",
      ms_device_speed(pUdev->eSpeed), pUdev->devpath, (int)pUdev->u32DevNum);
    ...
    ms_hub_port_disable(pHub, port1, 1); // patch from Linux code
...
int ms_usb_physical_reset_device(struct usb_device *ms_dev)
{
    ...
    if (!parent)
    {
        ms_usbhost_err("%s attempting to reset root hub!\n", __func__);
        return -EISDIR;
    }
...
#if 1 // TBD
    diag_printf("[P-RESET] disable ep 0, enable ep 0\n");
    ms_usb_disable_endpoint(ms_dev, 0+USB_DIR_OUT);
    ms_usb_disable_endpoint(ms_dev, 0+USB_DIR_IN);
    //ms_usb_enable_endpoint(ms_dev, &pAlt_set->endpoint[i].desc);
#endif

    s32Err = ms_hub_port_reset(parent, port1, ms_dev, HUB_SHORT_RESET_TIME);
    if (s32Err)
    {
        ms_usbhost_err("hub port[%d] reset fail, to disable the port (error=%d)\n", (int)port1, (int)s32Err);
        ms_hub_port_disable(parent, port1, 0);
        kfree(d_desc);
        return -ENODEV;
    }
    ...
    if (s32Err < 0)
    {
        ...
        ms_hub_port_disable(parent, port1, 1);
        kfree(d_desc);
    ...
    if (memcmp(&ms_dev->descriptor, d_desc, sizeof(*d_desc)))
    {
        ...
        diag_printf("[UD] device descriptor changed\n");
        return 1;
    }
    ...
    diag_printf("[UD]usb_physical_reset_device done\n");
    return 0;
}
...
testtry:
        ...
        for (i = 1; i <= hub->descriptor.bNbrPorts; i++)
        {
            ...
            {
                diag_printf("current portchange [0x%4x], portstatus [0x%4x]\n", u16Portchange, u16Portstatus);
            ...
            if (u16Portchange & USB_PORT_STAT_C_CONNECTION)
            {
                // (USB_PORT_FEAT_C_CONNECTION)
                ms_usb_control_cmd(ms_dev, usb_sndctrlpipe(ms_dev, 0),
                    USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_CONNECTION, i, NULL, 0, HZ);
                connect_change = 1;
            }
            if (u16Portchange & USB_PORT_STAT_C_ENABLE)
            {
                ...
                ms_usb_control_cmd(ms_dev, usb_sndctrlpipe(ms_dev, 0),
                    USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_ENABLE, i, NULL, 0, HZ);
            ...
            if (u16Portchange & USB_PORT_STAT_C_SUSPEND)
            {
                ...
                ms_usb_control_cmd(ms_dev, usb_sndctrlpipe(ms_dev, 0),
                    USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_SUSPEND, i, NULL, 0, HZ);
            }
            ...
            if (u16Portchange & USB_PORT_STAT_C_OVERCURRENT)
            {
                ...
                ms_usb_control_cmd(ms_dev, usb_sndctrlpipe(ms_dev, 0),
                    USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_OVER_CURRENT, i, NULL, 0, HZ);
                ms_hub_power_on(hub);
            }
            ...
            if (u16Portchange & USB_PORT_STAT_C_RESET)
            {
                ...
                // (USB_PORT_FEAT_C_RESET)
                ms_usb_control_cmd(ms_dev, usb_sndctrlpipe(ms_dev, 0),
                    USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_RESET, i, NULL, 0, HZ);
            }
            if (connect_change) {
                struct usb_device *pUdev;
            ...
            /* MStar patch */
            if ((ms_dev->parent == NULL)
	    	//&& (hub->ports[i - 1]->child == NULL)
                && (ms_dev->children[i - 1] == NULL)
                && (u16Portstatus == 0) && (u16Portchange & USB_PORT_STAT_C_ENABLE)
                && (hcd_reg_readl(((U32)pRootHub->p_UsbHcd->uhc_regs)+0x86) & 0x800) == 0x800) {
                    ms_ResetMstarUsb(pRootHub->p_UsbHcd);
                    diag_printf("[USB] IN-NAK reset EHCI...\n");
                    goto testtry;
            }
	    /* end of MStar patch */
        } /* end for i */
...
static void ms_hub_tt_kevent(struct s_gVar4UsbPort *pRootHub)
{
	...
	list_t = tmp_list->next;
	hub = entry_to_container(list_t, struct usb_hub, event_list);
	while (--limit && !ms_is_empty_list (&hub->tt.clear_list)) {
