...
#ifdef ENABLE_ATA_COMMAND_PASS_THROUGH
static void ms_vSCSICmd_ATA_THRU_STANDBY(struct ms_usdata *us, U8 u8LunNum)
{
	stCmd_scsi *srb = &us->srb;

	/* set the command and the LUN */
	memset(srb->cmnd, 0, MAX_COMMAND_SIZE);
	srb->cmnd[0] = ATA_PASS_THROUGH_COMMAND;
	srb->cmnd[1] = 0x06;
	srb->cmnd[2] = 0x28;
	srb->cmnd[4] = 0x03;
	srb->cmnd[8] = 0xa0;
	srb->cmnd[9] = 0xe2; /* STANDBY Command */

	srb->cmd_len = CB_LENGTH_ATA_PASS_THROUGH_COMMAND;
...
static BOOL ms_bSCSI_TEST_UNIT_READY(U8 uPort, U8 uLunNum, BOOL CheckCBI)
{
    ...
    host_id = pMass_stor->host_id;
    if(!ms_USBCriticalSectionIn_TimeOut(host_id, MS_MSC_WAIT_MUTEX_TIMEOUT))
        return LunDevice[uLunNum].bDeviceReady ? TRUE : FALSE;
...
#ifdef ENABLE_ATA_COMMAND_PASS_THROUGH
BOOL ms_bSCSI_ATA_THRU_STANDBY(U8 uPort, U8 uLunNum, U8 abort)
{
	int i;
	BOOL bRet = FALSE;
	struct ms_usdata *pMass_stor;
	...
	for (i = 0; i < ScsiCmd_Fail_TUR_Retry ; i++) {
		memset(&pMass_stor->srb, 0, sizeof(struct scMsc_cmd));
		ms_vSCSICmd_ATA_THRU_STANDBY(pMass_stor, uLunNum);
		ms_usb_msc_control_thread(pMass_stor);

		if (pMass_stor->srb.result == MS_STAT_GOOD) {
			if (abort)
				ms_set_bit(MS_IDX_ABORTING, &pMass_stor->flags, U32);
			bRet = TRUE;
			break;
		}
		else if (pMass_stor->srb.result == (DID_NO_CONNECT << 16)) {
			diag_printf("ATA pass through command break by NO_CONNECT\n");
			break;
		}
		else {
			SCSI_DbgPrint("ATA pass through command failed retry %d\n", i);
		}
	}
