...
MS_BOOL HAL_COPRO_RegWrite4Byte(MS_U32 u32RegAddr, MS_U32 u32Val)
{
    ...
    HAL_COPRO_RegWrite2Byte(u32RegAddr, u32Val & 0x0000FFFF);
...
void HAL_COPRO_Disable(void)
{
    ...
    {
#if defined(__mips__) || defined(__arm__) || defined(__aarch64__)// HK = MIPS or ARM
...
void HAL_COPRO_Enable(MS_U32 u32_ADR)
{
    ...
    MS_U32 base_addr;
    MS_U16 Sw_Reset;
    Sw_Reset = HAL_CPU_PM_RegRead2Byte(PM_REG_CPUX_SW_RSTZ);
    Sw_Reset |= 0x2000;
    HAL_CPU_PM_RegWrite2Byte(PM_REG_CPUX_SW_RSTZ,Sw_Reset);
    //(1) Disable R2 -> Stop(Reset)
    HAL_COPRO_Disable();

    //(2) Set R2 clock
    // [1:0] = 2'b00
    // [3:2] =  2'b00: 288MHz,  2'b01: 240HMz, 2'b10: 216MHz, 2'b11: 12MHz(Xtal)
    // Set to 216 MHz
    HAL_COPRO_RegWriteByte(NPM_REG_CLKGEN0_SECR2, 0x1CUL);
    HAL_COPRO_RegWriteByte(NPM_REG_CLKGEN0_SECR2, 0x3CUL);
    ...
    HAL_COPRO_RegWrite2Byte(R2_MAULV1_REG, 0x8500UL);
    //(4) Set RIU   base
    ...
    //HAL_COPRO_RegWrite2Byte(PM_REG_CPUX_SW_RSTZ, HAL_CPU_PM_RegRead2Byte(PM_REG_CPUX_SW_RSTZ) &(~BIT13));
    //HAL_COPRO_RegWrite2Byte(PM_REG_CPUX_SW_RSTZ, HAL_CPU_PM_RegRead2Byte(PM_REG_CPUX_SW_RSTZ) | (BIT13));
...
MS_U32 HAL_CPU_QueryClock(void)
{
    ...
    u16RegValue = HAL_COPRO_RegRead2Byte(0x1896);
    u16RegValue &= (~0x7);
    u16RegValue |= 0x1;
    HAL_COPRO_RegWrite2Byte(0x1896,u16RegValue);        // reg_testout_sel , 0x1018_4B[2:0] = 001b

    u16RegValue = HAL_COPRO_RegRead2Byte(0x1eec);
    u16RegValue &= (~0x7);
    u16RegValue |= 0x1;
    HAL_COPRO_RegWrite2Byte(0x1eec,u16RegValue);        // reg_signal_clk_out_sel , 0x101E_76[2:0] = 001b

    u16RegValue = HAL_COPRO_RegRead2Byte(0x1eea);
    u16RegValue &= ~(0x7|(0x7<<5));                     // reg_clk_out_sel , 0x101E_75[2:0] = 000b
                                                        // reg_test_rg , reg_test_gb , reg_test_rb , 0x101E_75[7:5] = 000b
    u16RegValue |= (0x1<<14);                           // reg_testbus_en , 0x101E_75[14] = 1b
    HAL_COPRO_RegWrite2Byte(0x1eea,u16RegValue);

    u16RegValue = HAL_COPRO_RegRead2Byte(0x1eee);
    u16RegValue &= (~0x3F);
    u16RegValue |= 0x2;
    HAL_COPRO_RegWrite2Byte(0x1eee,u16RegValue);        // reg_test_bus24b_sel , 0x101E_77[5:0] = 000010b

    u16RegValue = HAL_COPRO_RegRead2Byte(0x1ee0);
    u16RegValue &= ~(0x1<<15);
    HAL_COPRO_RegWrite2Byte(0x1ee0,u16RegValue);        // reg_clk_cakc_en = 0 , 0x101E_70[15] = 0b

    u16RegValue |= (0x1<<15);
    HAL_COPRO_RegWrite2Byte(0x1ee0,u16RegValue);        // reg_clk_cakc_en = 1 , 0x101E_70[15] = 1b

    MsOS_DelayTaskUs_Poll(200);

    dwCpuSpeed = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(0x1ee2));        // read DAC
    dwCpuSpeed = (dwCpuSpeed*48)/1000;                                      // calculate CPU frequency

    COPRO_HAL_ERR("(CPU Speed %d MHz)\n", (MS_U32) dwCpuSpeed);

    return (dwCpuSpeed * 1000000);
}
