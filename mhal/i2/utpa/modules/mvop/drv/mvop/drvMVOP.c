...
static void _MVOP_SubInitVar(MVOP_DevID eID)
{
    ...
    if (pstDrvCtrl)
    {
        ...
        pstDrvCtrl->_bEn265TCH = FALSE;
        pstDrvCtrl->_bForceP = FALSE;
                                   ...
                                   EN_MVOP_Output_3D_TYPE en3DType)
{
    ...
    if (E_MVOP_DEV_0 == eID)
    {
        if(HAL_MVOP_Get4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_Get4k2kClk()))
        {
    ...
    {
        if(HAL_MVOP_SubGet4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_SubGet4k2kClk()))
        {
    ...
    if(HAL_MVOP_IsSupport4k2k2P && (TRUE == HAL_MVOP_IsSupport4k2k2P()))
    {
#if defined(STB_DC_MODE)||(STB_DC)
        if((E_MVOP_OUTPUT_3D_NONE != en3DType) || (HAL_MVOP_GetIsCurrentXCGenTiming && (HAL_MVOP_GetIsCurrentXCGenTiming(E_MVOP_DEV_0) == FALSE)))
                                      ...
                                      MS_BOOL bEnHDup)
{
    ...
    if(MVOP_Clock >= MVOP_FREQ_320M)
    {
        if (bIsMain)
        {
            if((HAL_MVOP_SetEnable4k2k2P && HAL_MVOP_SetEnable4k2kClk)&& (HAL_MVOP_Get4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_Get4k2kClk())))
            {
        ...
        {
            if((HAL_MVOP_SubSetEnable4k2k2P && HAL_MVOP_SubSetEnable4k2kClk) && (HAL_MVOP_SubGet4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_SubGet4k2kClk())))
            {
    ...
    else if(MVOP_Clock > MVOP_FREQ_160M)
    {
        ...
        if (bIsMain)
        {
            if(HAL_MVOP_SetEnable4k2kClk && HAL_MVOP_Get4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_Get4k2kClk()))
            {
                HAL_MVOP_SetEnable4k2kClk(TRUE);
            }
            else
        ...
        {
            if(HAL_MVOP_SubSetEnable4k2kClk && (HAL_MVOP_SubGet4k2kClk && (HALMVOP_NOT_SUPPORT != HAL_MVOP_SubGet4k2kClk())))
            {
                HAL_MVOP_SubSetEnable4k2kClk(TRUE);
            }
            else
...
MS_U16 MDrv_MVOP_GetVStart_V2(void* pInstance)
{
    ...
    if(HAL_MVOP_GetIsCurrentXCGenTiming && (HAL_MVOP_GetIsCurrentXCGenTiming(E_MVOP_DEV_0) == FALSE))
    {
        ...
        if ((p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming.bInterlace==1) ||
            (p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._bForceP == TRUE))
        {
            u16Vst*=2;
            u16Vst-=18;
        }
...
void MDrv_MVOP_Exit_V2(void* pInstance)
{
    ...
    p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._stMVOPDrvStat.bIsSetTiming = FALSE;
...
MVOP_Result MDrv_MVOP_SetInputCfg_V2 (void* pInstance, MVOP_InputSel in, MVOP_InputCfg* pCfg)
{
    ...
    switch (in)
    {
        ...
        case MVOP_INPUT_EVD_DV:
            ...
            break;
#endif
        case MVOP_INPUT_EVD_TCH:
            HAL_MVOP_SetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SetEVDHardwireMode_TCH();
            HAL_MVOP_EnableTCHSetting(E_MVOP_DEV_0, ENABLE);
            p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._bEn265TCH = TRUE;
            break;
        case MVOP_INPUT_AVS2:
            HAL_MVOP_SetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SetAVS2HardwireMode();
            break;
        default:
            bRet = E_MVOP_INVALID_PARAM;
            break;
...
MVOP_Result MDrv_MVOP_SetOutputCfg_V2(void* pInstance, MVOP_VidStat *pstVideoStatus, MS_BOOL bEnHDup)
{
    ...
    // Rotate function do not call MDrv_MVOP_SetInputCfg to reset.
    p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._u16SrcVSize = 0;
    p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._u16SrcHSize = 0;
    ...
    if(HAL_MVOP_SetbyInputTiming)
    {
        HAL_MVOP_SetbyInputTiming(E_MVOP_DEV_0, pstVideoStatus);
    }
...
MS_U16 MDrv_MVOP_SubGetVStart_V2(void* pInstance)
{
    ...
    if ((p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopTiming.bInterlace==1) ||
            (p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._bForceP == TRUE))
    {
        u16Vst*=2;
        u16Vst-=18;
    }
...
void MDrv_MVOP_SubExit_V2(void* pInstance)
{
   ...
   // if(HAL_MVOP_Exit)
    ...
    p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._stMVOPDrvStat.bIsSetTiming = FALSE;
    _DRV_MVOP_Release(E_MVOP_DEV_1);
#endif
}
...
MVOP_Result MDrv_MVOP_SubSetInputCfg_V2(void * pInstance, MVOP_InputSel in, MVOP_InputCfg * pCfg)
{
    ...
    switch (in)
    {
        ...
        case MVOP_INPUT_EVD_TCH:
            HAL_MVOP_SubSetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SubSetEVDHardwireMode_TCH();
            HAL_MVOP_EnableTCHSetting(E_MVOP_DEV_1, ENABLE);
            p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._bEn265TCH = TRUE;
            break;
        case MVOP_INPUT_AVS2:
            HAL_MVOP_SubSetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SubSetAVS2HardwireMode();
            break;
        default:
...
MVOP_Result MDrv_MVOP_SubSetOutputCfg_V2(void* pInstance, MVOP_VidStat *pstVideoStatus, MS_BOOL bEnHDup)
{
    ...
    // Rotate function do not call MDrv_MVOP_SetInputCfg to reset.
    p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._u16SrcVSize = 0;
    p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._u16SrcHSize = 0;
...
//  Only support in Eagle.
        ...
        case E_MVOP_CMD_SET_BASE_ADD_MULTI_VIEW:
        {
            ...
            if(stBaseAddInfo)
            {
                ...
                {
                    HAL_MVOP_SetYUVBaseAddMultiView(eDevID, stBaseAddInfo);
                }
            }
            else
#endif
        ...
        case E_MVOP_CMD_SET_SAVE_MIU_AREA:
        {
            MVOP_DBG(MVOP_PRINTF("\n %s.\n", __FUNCTION__);)
            MVOP_MIUReadPrct stMiuSave;
            stMiuSave =*(MVOP_MIUReadPrct*)pPara;
            if(HAL_MVOP_SetMiuSaveArea)
            {
                HAL_MVOP_SetMiuSaveArea(eDevID, stMiuSave);
            }
            break;
        }
        case E_MVOP_CMD_SET_MFDEC_ROTATE:
        {
            MVOP_DBG(MVOP_PRINTF("\n %s.\n", __FUNCTION__);)
            MVOP_MFDECRotate eRotate;
            eRotate =*(MVOP_MFDECRotate*)pPara;
            if(HAL_MVOP_SetMFDECRotate)
            {
                HAL_MVOP_SetMFDECRotate(eDevID, eRotate);
            }
            break;
        }
        case E_MVOP_CMD_SET_ENABLE_FOD:
        {
            MVOP_DBG(MVOP_PRINTF("\n %s.\n", __FUNCTION__);)
            MS_BOOL bEnable;
            bEnable =*(MS_BOOL*)pPara;
            if(HAL_MVOP_EnableFODtoWB)
            {
                HAL_MVOP_EnableFODtoWB(eDevID, bEnable);
            }
            break;
        }
        case E_MVOP_CMD_SET_FORCE_P_MODE:
        {
            MS_BOOL bEnable;
            bEnable =*(MS_BOOL*)pPara;
            if (HAL_MVOP_SetForcePMode)
            {
                switch(eDevID)
                {
                    case E_MVOP_DEV_0:
                        p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._bForceP = bEnable;
                        HAL_MVOP_SetForcePMode(E_MVOP_DEV_0, bEnable);
                        break;
#if MVOP_SUPPORT_SUB
                    case E_MVOP_DEV_1:
                        p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._bForceP = bEnable;
                        HAL_MVOP_SetForcePMode(E_MVOP_DEV_1, bEnable);
                        break;
#endif
                    default:
                        MVOP_DBG(MVOP_PRINTF("\n %s not support this device 0x%x!! \n", __FUNCTION__, eCmd);)
    ...
    switch (eCmd)
    {
        ...
        case E_MVOP_CMD_GET_IS_XC_GEN_TIMING:
         ...
         case E_MVOP_CMD_GET_IS_MVOP_AUTO_GEN_BLACK:
            ...
            break;
#endif
        case E_MVOP_CMD_GET_IS_BOT_FIELD:
            if ((HAL_MVOP_GetIsBotField) && (sizeof(MS_BOOL) == u32ParaSize))
            {
                *(MS_BOOL*)pPara = HAL_MVOP_GetIsBotField(eDevID);
            }
            else
            {
                eRet = E_MVOP_INVALID_PARAM;
...
void _MVOP_MdbInfoSWInput(MS_U64 *u64ReqHdl, MVOP_DevID eDevID, MVOP_InputCfg* stMvopInputCfg, MVOP_InputSel _eInputSel)
{
    ...
    {
        if(MVOP_INPUT_DRAM == _eInputSel)
        {
            MdbPrint(u64ReqHdl,"Yoffset                   :    0x%x\n",stMvopInputCfg->u32YOffset);
            MdbPrint(u64ReqHdl,"UVoffset                  :    0x%x\n",stMvopInputCfg->u32UVOffset);
...
void _MVOP_MdbInfoFeature(MS_U64 *u64ReqHdl, MVOP_FeatureMdb* pstFeature)
{
    ...
    {
        switch(pstFeature->enMirror)
        {
            case E_VOPMIRROR_VERTICAL:
                MdbPrint(u64ReqHdl,"Mirror                    :    Vmir\n");
                break;
            case E_VOPMIRROR_HORIZONTALL:
                MdbPrint(u64ReqHdl,"Mirror                    :    Hmir\n");
                break;
            case E_VOPMIRROR_HVBOTH:
                MdbPrint(u64ReqHdl,"Mirror                    :    HVmir\n");
                break;
            case E_VOPMIRROR_NONE:
                MdbPrint(u64ReqHdl,"Mirror                    :    None\n");
                break;
            default:
                MdbPrint(u64ReqHdl,"Mirror                    :    None\n");
                break;
        }
        switch(pstFeature->en3D)
        {
            case E_MVOP_OUTPUT_3D_NONE:
                MdbPrint(u64ReqHdl,"3D type                   :    None\n");
                break;
            case E_MVOP_OUTPUT_3D_TB:
                MdbPrint(u64ReqHdl,"3D type                   :    TopBot\n");
                break;
            case E_MVOP_OUTPUT_3D_SBS:
                MdbPrint(u64ReqHdl,"3D type                   :    SBS\n");
                break;
            case E_MVOP_OUTPUT_3D_LA:
                MdbPrint(u64ReqHdl,"3D type                   :    LA\n");
                break;
            default:
                MdbPrint(u64ReqHdl,"3D type                   :    None\n");
                break;
...
void _MVOP_MdbInfoSWOutputTiming(MS_U64 *u64ReqHdl, MVOP_Timing* pMVOPTiming)
{
    ...
    else
    {
        ...
        MdbPrint(u64ReqHdl,"Vtotal                    :    0x%x\n",pMVOPTiming->u16V_TotalCount);
        MdbPrint(u64ReqHdl,"Htotal                    :    0x%x\n",pMVOPTiming->u16H_TotalCount);
        MdbPrint(u64ReqHdl,"u16VImg_Start0            :    0x%x\n",pMVOPTiming->u16VBlank1_Start);
        MdbPrint(u64ReqHdl,"u16HImg_Start             :    0x%x\n",pMVOPTiming->u16HImg_Start);
        ...
        MdbPrint(u64ReqHdl,"Hstar                     :    0x%x\n",(pMVOPTiming->u16HImg_Start/2)+2);
        u16Vst = (pMVOPTiming->u16VBlank0_End - pMVOPTiming->u16TopField_VS);
        MdbPrint(u64ReqHdl,"Vstart                    :    0x%x\n",((TRUE == pMVOPTiming->bInterlace) ? (u16Vst-5):((u16Vst*2)-18)));
    }
}

void _MVOP_MdbInfoRegInputCfg(MS_U64 *u64ReqHdl, MVOP_RegInputMdb* stRegInput)
{
    MdbPrint(u64ReqHdl,"\n=== Input Setting ===\n");
    ...
    {
        MdbPrint(u64ReqHdl,"Reg_Yoffset               :    0x%x\n",stRegInput->u32RegYoffset);
        MdbPrint(u64ReqHdl,"Reg_UVoffset              :    0x%x\n",stRegInput->u32RegUVoffset);
        ...
        if(stRegInput->bReg422)
            MdbPrint(u64ReqHdl,"Reg_YUVtype               :    422\n");
        else
            MdbPrint(u64ReqHdl,"Reg_YUVtype               :    420\n");
        ...
        MdbPrint(u64ReqHdl,"Reg_bitlen address        :    0x%x\n",stRegInput->u32RegBitenAdd);
    }

}

void _MVOP_MdbInfoRegOutputCfg(MS_U64 *u64ReqHdl, MVOP_RegOutputMdb* stRegOutput)
{
    MdbPrint(u64ReqHdl,"\n=== Output Setting ===\n");
    ...
    {
        MdbPrint(u64ReqHdl,"Reg_Vtotal                :    0x%x\n",stRegOutput->u32RegVtt);
        MdbPrint(u64ReqHdl,"Reg_Htotal                :    0x%x\n",stRegOutput->u32RegHtt);
        MdbPrint(u64ReqHdl,"Reg_u16VImg_Start0        :    0x%x\n",stRegOutput->u16RegVImgst);
        MdbPrint(u64ReqHdl,"Reg_u16HImg_Start         :    0x%x\n",stRegOutput->u16RegHImgst);
...
{
    MVOP_DevID  eDevID = E_MVOP_DEV_0; //default is to control main mvop
    MVOP_InputCfg* pstMvopInputCfg = NULL;
    MVOP_InputSel _eInputSel = MVOP_INPUT_UNKNOWN;
    MVOP_RgbFormat _eRGB = E_MVOP_RGB_NONE;
    MVOP_VidStat* pstMvopOutputCfg = NULL;
    MVOP_FeatureMdb* pstFeature = NULL;
    MVOP_Timing* pstTiming = NULL;
    MVOP_RegInputMdb* pstRegInput = NULL;
    MVOP_RegOutputMdb* pstRegOutput = NULL;
    MVOP_FeatureMdb* pstRegFeature = NULL;
    MS_BOOL  bIsInint = FALSE;
    MS_BOOL  bIsEnable = FALSE;
    MVOP_FeatureMdb stSWFtr;
    MVOP_RegInputMdb stRegInput;
    MVOP_RegOutputMdb stRegOutput;
    MVOP_FeatureMdb stRegFeature;
    ...
    if(NULL != p_gDrvMVOPCtx)
    {
        ...
        switch(eDevID)
        {
            case E_MVOP_DEV_0:
            {
                ...
                _eRGB = p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._eRgbFmt;
                pstMvopInputCfg = &p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopInputCfg;
                pstMvopOutputCfg = &p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopOutputCfg;
                memset(&stSWFtr, 0, sizeof(MVOP_FeatureMdb));
                stSWFtr.enMirror = HAL_MVOP_GetMirrorMode(E_MVOP_DEV_0);
                if(HAL_MVOP_GetOutput3DType)
                    stSWFtr.en3D = HAL_MVOP_GetOutput3DType();
                if(HAL_MVOP_GetIsCurrentHSK)
                    stSWFtr.bHSK = HAL_MVOP_GetIsCurrentHSK(E_MVOP_DEV_0);

                stSWFtr.bXCGenTiming = HAL_MVOP_GetIsCurrentXCGenTiming(E_MVOP_DEV_0);
                HAL_MVOP_GetCropInfo(E_MVOP_DEV_0, &stSWFtr.stCropSt, &stSWFtr.stCropSize);
                stSWFtr.bOneField = 0;
                stSWFtr.b2P = HAL_MVOP_Get4k2k2PMode(E_MVOP_DEV_0);
                stSWFtr.bForceP = p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._bForceP;
                stSWFtr.u32Framerate = 0;//fixme
                pstFeature = &stSWFtr;
                pstTiming = &p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming;

                if(!bIsEnable) //check devide on power
                {
                    break;
                }
                else
                {
                    ...
                    memset(&stRegFeature, 0, sizeof(MVOP_FeatureMdb));
                    pstRegInput = &stRegInput;
                    pstRegOutput = &stRegOutput;
                    pstRegFeature = &stRegFeature;
                    HAL_MVOP_MDBRegInputCfg(eDevID, _eInputSel, pstRegInput);
                    HAL_MVOP_MDBRegOutputCfg(eDevID, pstRegOutput);
                    HAL_MVOP_MDBRegFeature(eDevID, pstRegFeature);
                }
                break;
            }
#if MVOP_SUPPORT_SUB
            case E_MVOP_DEV_1:
            {
                ...
                _eRGB = p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._eRgbFmt;
                pstMvopInputCfg = &p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopInputCfg;
                pstMvopOutputCfg = &p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopOutputCfg;

                memset(&stSWFtr, 0, sizeof(MVOP_FeatureMdb));
                stSWFtr.enMirror = HAL_MVOP_GetMirrorMode(E_MVOP_DEV_1);
                if(HAL_MVOP_GetOutput3DType)
                    stSWFtr.en3D = HAL_MVOP_SubGetOutput3DType();
                if(HAL_MVOP_GetIsCurrentHSK)
                    stSWFtr.bHSK = HAL_MVOP_GetIsCurrentHSK(E_MVOP_DEV_1);
                stSWFtr.bXCGenTiming = HAL_MVOP_GetIsCurrentXCGenTiming(E_MVOP_DEV_1);
                #if defined(CHIP_MASERATI)
                HAL_MVOP_GetCropInfo(E_MVOP_DEV_1, &stSWFtr.stCropSt, &stSWFtr.stCropSize);
                #endif
                stSWFtr.bOneField = 0;
                stSWFtr.b2P = HAL_MVOP_Get4k2k2PMode(E_MVOP_DEV_1);
                stSWFtr.bForceP = p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._bForceP;
                stSWFtr.u32Framerate = 0;//fixme
    ...
    {
        case MVOP_INPUT_DRAM:
            MdbPrint(u64ReqHdl,"Input Select              :    MCU\n");
                break;
        case MVOP_INPUT_H264:
            MdbPrint(u64ReqHdl,"Input Select              :    H264 hardwire\n");
                break;
        case MVOP_INPUT_HVD_3DLR:
            MdbPrint(u64ReqHdl,"Input Select              :    HVD3D hardwire\n");
                break;
        case MVOP_INPUT_MVD:
            MdbPrint(u64ReqHdl,"Input Select              :    MVD hardwire\n");
                break;
        case MVOP_INPUT_MVD_3DLR:
            MdbPrint(u64ReqHdl,"Input Select              :    MVD3D hardwire\n");
                break;
        case MVOP_INPUT_RVD:
            MdbPrint(u64ReqHdl,"Input Select              :    RVD hardwire\n");
                break;
        case MVOP_INPUT_CLIP:
            MdbPrint(u64ReqHdl,"Input Select              :    CLIP hardwire\n");
                break;
        case MVOP_INPUT_JPD:
            MdbPrint(u64ReqHdl,"Input Select              :    MJPEG hardwire\n");
                break;
        case MVOP_INPUT_EVD:
            MdbPrint(u64ReqHdl,"Input Select              :    H265 hardwire\n");
                break;
        case MVOP_INPUT_VP9:
            MdbPrint(u64ReqHdl,"Input Select              :    VP9 hardwire\n");
                break;
        case MVOP_INPUT_EVD_DV:
            MdbPrint(u64ReqHdl,"Input Select              :    H265 Dolby hardwire\n");
                break;
        case MVOP_INPUT_H264_DV:
            MdbPrint(u64ReqHdl,"Input Select              :    H264 Dolby hardwire\n");
                break;
        default:
            MdbPrint(u64ReqHdl,"Input Select              :    NA\n");
            _eInputSel = MVOP_INPUT_UNKNOWN;
                break;
    }

    _MVOP_MdbInfoSWInput(u64ReqHdl, eDevID, pstMvopInputCfg, _eInputSel);

    switch(_eRGB)
    {
        case E_MVOP_RGB_565:
            MdbPrint(u64ReqHdl,"RGB format                :    565\n");
                break;
        case E_MVOP_RGB_888:
            MdbPrint(u64ReqHdl,"RGB format                :    888\n");
                break;
        case E_MVOP_RGB_NONE:
    ...
    _MVOP_MdbInfoSWOutput(u64ReqHdl, pstMvopOutputCfg, FALSE);
    ...
    _MVOP_MdbInfoRegInputCfg(u64ReqHdl, pstRegInput);

    _MVOP_MdbInfoRegOutputCfg(u64ReqHdl, pstRegOutput);

    _MVOP_MdbInfoRegFeature(u64ReqHdl, pstRegFeature);
}

MVOP_Result MDrv_MVOP_SetMVOPMdb_V2(MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
{
    char* ptr = NULL;
    //char strbuf[128] = {0};
    ...
    ptr = pcmd;
    MdbPrint(pu64ReqHdl,"---------MStar MVOP Set Command---------\n\n");
    MdbPrint(pu64ReqHdl,"cmd: %s\n",ptr);

    if(strncmp(ptr, "vsyncline ", 10) == 0)
    {
        int u32lines = 0;
        int u32len = 0;
        int i;
        ...
        ptr = pcmd + 10;
        //MdbPrint(pu64ReqHdl,"[MVOP][Mdb][SetCMD] line char = %s\n",ptr);
        u32len = strlen(ptr);

        for(i=0; i<u32len; i++)
        {
            u32lines = u32lines * 10 + ( ptr[i] - '0' );
        }
        MdbPrint(pu64ReqHdl,"vsync forwarding line number = %d\n",u32lines);
        HAL_MVOP_MDBSetVsyncFwdLine(pu64ReqHdl, E_MVOP_DEV_0, u32lines);
        HAL_MVOP_MDBSetVsyncFwdLine(pu64ReqHdl, E_MVOP_DEV_1, u32lines);
        return E_MVOP_OK;
    }
    else if(strncmp(ptr, "mvoppattern ", 12) == 0)
    {
        int u32pattern_id = 0;
        int u32len = 0;
        int i;
        ...
        for(i=0; i<u32len; i++)
        {
            u32pattern_id = u32pattern_id * 10 + ( ptr[i] - '0' );
        }
        MdbPrint(pu64ReqHdl,"set mvop pattern = %d\n",u32pattern_id);
        HAL_MVOP_SetPattern(u32pattern_id);
        HAL_MVOP_SubSetPattern(u32pattern_id);
        return E_MVOP_OK;
    }
    else
    {
...
MVOP_Result MDrv_MVOP_DMS_SetDispADDInfo(MVOP_Handle* stHd, MVOP_DMS_WIN WIN_ID,
MVOP_DMSDisplayADD *pstMVOPDISPADD, HAL_XCWrite fpXCWrite)
{
    //MS_U16 u16CopiedLength = sizeof(MVOP_DMSDisplayADD);
    MVOP_DevID  eDevID = E_MVOP_DEV_0; //default is to control main mvop

     if (stHd != NULL)
    {
        eDevID = _MVOP_MapModNum2DevID(stHd->eModuleNum);
        MVOP_DBG(MVOP_PRINTF("%s eDevID=%x\n", __FUNCTION__, eDevID);)
    }
    ...
    if(NULL == fpXCWrite)
    {
        MVOP_EVDBaseAddInput stEVDBaseAddInfo;

        if(pstMVOPDISPADD->u8BotFlag)
        {
            stEVDBaseAddInfo.u32MSBYOffset = pstMVOPDISPADD->u32MSB_FB_ADDR[0]+16;
            stEVDBaseAddInfo.u32MSBUVOffset = pstMVOPDISPADD->u32MSB_FB_ADDR[1]+16;
            stEVDBaseAddInfo.u32LSBYOffset = pstMVOPDISPADD->u32LSB_FB_ADDR[0]+16;
            stEVDBaseAddInfo.u32LSBUVOffset = pstMVOPDISPADD->u32LSB_FB_ADDR[1]+16;
        }
        else
        {
            stEVDBaseAddInfo.u32MSBYOffset = pstMVOPDISPADD->u32MSB_FB_ADDR[0];
            stEVDBaseAddInfo.u32MSBUVOffset = pstMVOPDISPADD->u32MSB_FB_ADDR[1];
            stEVDBaseAddInfo.u32LSBYOffset = pstMVOPDISPADD->u32LSB_FB_ADDR[0];
            stEVDBaseAddInfo.u32LSBUVOffset = pstMVOPDISPADD->u32LSB_FB_ADDR[1];
        }
        stEVDBaseAddInfo.bProgressive = 1;
        stEVDBaseAddInfo.b422Pack = 0;
        stEVDBaseAddInfo.bEnLSB = (pstMVOPDISPADD->u32BIT_DEPTH[0]>8) || (pstMVOPDISPADD->u32BIT_DEPTH[1]>8);

        if(eDevID == E_MVOP_DEV_0)
        {
            HAL_MVOP_SetEVDYUVBaseAdd(&stEVDBaseAddInfo);
            //HAL_MVOP_SetYUVBaseAdd(u32MSB_FB_ADDR[0], u32MSB_FB_ADDR[1], TRUE, FALSE);
        }
#if MVOP_SUPPORT_SUB
        else if(eDevID == E_MVOP_DEV_1)
        {
            HAL_MVOP_SubSetYUVBaseAdd(pstMVOPDISPADD->u32MSB_FB_ADDR[0], pstMVOPDISPADD->u32MSB_FB_ADDR[1], TRUE, FALSE);
        }
#endif
        if((pstMVOPDISPADD->u32BIT_DEPTH[0] > 8) || (pstMVOPDISPADD->u32BIT_DEPTH[1] > 8))
        {
            HAL_MVOP_SelMIU(eDevID, pstMVOPDISPADD->u8DMSB_FB_MIU[0], pstMVOPDISPADD->u8DMSB_FB_MIU[1],
            pstMVOPDISPADD->u32LSB_FB_MIU[0], pstMVOPDISPADD->u32LSB_FB_MIU[1]);
        }
        else
        {
            HAL_MVOP_SelMIU(eDevID, pstMVOPDISPADD->u8DMSB_FB_MIU[0], pstMVOPDISPADD->u8DMSB_FB_MIU[1],
            pstMVOPDISPADD->u8DMSB_FB_MIU[0], pstMVOPDISPADD->u8DMSB_FB_MIU[1]);
        }

        HALMVOPMFDECINFO stMFDECInfo;

        stMFDECInfo.bMFDEC_EN = pstMVOPDISPADD->bMFDEC_EN;
        stMFDECInfo.u8MFDEC_ID = pstMVOPDISPADD->u8MFDEC_ID;
        stMFDECInfo.u32UNCOMPRESS_MODE = pstMVOPDISPADD->u32UNCOMPRESS_MODE;
        stMFDECInfo.u32BITLEN_FB_ADDR = pstMVOPDISPADD->u32BITLEN_FB_ADDR;
        stMFDECInfo.u8BITLEN_FB_MIU = pstMVOPDISPADD->u8BITLEN_FB_MIU;
        stMFDECInfo.u32BITLEN_FB_PITCH = pstMVOPDISPADD->u32BITLEN_FB_PITCH;

        HAL_MVOP_SetMFDECInfo(eDevID, &stMFDECInfo);

        if(pstMVOPDISPADD->bDualDVEN)
        {
            MVOP_BaseAddInput stBaseAddInfo;
            stBaseAddInfo.eView = E_MVOP_DV_EL_VIEW;
            stBaseAddInfo.u32YOffset = pstMVOPDISPADD->u32DualDV_EL_FB[0];
            stBaseAddInfo.u32UVOffset = pstMVOPDISPADD->u32DualDV_EL_FB[1];
            HAL_MVOP_SetYUVBaseAddMultiView(eDevID, &stBaseAddInfo);

            HAL_MVOP_SelMIU(E_MVOP_DEV_1, pstMVOPDISPADD->u8DualDV_EL_MIU[0], pstMVOPDISPADD->u8DualDV_EL_MIU[1],
            pstMVOPDISPADD->u8DualDV_EL_MIU[0], pstMVOPDISPADD->u8DualDV_EL_MIU[1]);
        }

        if(pstMVOPDISPADD->bEN3D)
        {
            MVOP_BaseAddInput stBaseAddInfo;
            stBaseAddInfo.eView = E_MVOP_2ND_VIEW;
            stBaseAddInfo.u32YOffset = pstMVOPDISPADD->u32RViewFB[0];
            stBaseAddInfo.u32UVOffset = pstMVOPDISPADD->u32RViewFB[1];

            HAL_MVOP_SetYUVBaseAddMultiView(eDevID, &stBaseAddInfo); //3d
        }

        HALMVOPInterlace stMVOPInterlace;
        stMVOPInterlace.bOutputIMode = pstMVOPDISPADD->bOutputIMode;
        stMVOPInterlace.u8FDMask = pstMVOPDISPADD->u8FDMask;
        stMVOPInterlace.u8BotFlag = pstMVOPDISPADD->u8BotFlag;
        stMVOPInterlace.bXCBOB_EN = pstMVOPDISPADD->bXCBOB_EN;

        HAL_MVOP_SetFrameInterlace(eDevID, &stMVOPInterlace);
    }
    _DRV_MVOP_Return(E_MVOP_DEV_0,E_MVOP_OK);
}
...
{
    //MS_U16 u16CopiedLength = sizeof(MVOP_DMSDisplaySize);
    MVOP_DevID  eDevID = E_MVOP_DEV_0; //default is to control main mvop
    ...
    if(NULL == fpXCWrite)
    {
        HALMVOPDISPSize stDispSizeInfo;
        stDispSizeInfo.u16Width = pstMVOPDISPSIZE->u16Width;
        stDispSizeInfo.u16Height = pstMVOPDISPSIZE->u16Height;
        stDispSizeInfo.u16Pitch[0] = pstMVOPDISPSIZE->u16Pitch[0];
        stDispSizeInfo.u16Pitch[1] = pstMVOPDISPSIZE->u16Pitch[1];
        stDispSizeInfo.U8DSIndex = pstMVOPDISPSIZE->U8DSIndex;

        stDispSizeInfo.bHDup = pstMVOPDISPSIZE->bHDup;
        stDispSizeInfo.bVDup = pstMVOPDISPSIZE->bVDup;

        HAL_MVOP_SetDISPSize(eDevID, &stDispSizeInfo);

        if(pstMVOPDISPSIZE->bDualDV_EN)
        {
            HALMVOPDISPDualDVSize stDispDVSizeInfo;

            stDispDVSizeInfo.bDualDV_EN = pstMVOPDISPSIZE->bDualDV_EN;
            stDispDVSizeInfo.u16Width_EL = pstMVOPDISPSIZE->u16Width;
            stDispDVSizeInfo.u16Width_EL = pstMVOPDISPSIZE->u16Height;
            stDispDVSizeInfo.u16Pitch_EL[0] = pstMVOPDISPSIZE->u16Pitch[0];
            stDispDVSizeInfo.u16Pitch_EL[1] = pstMVOPDISPSIZE->u16Pitch[1];

            HAL_MVOP_SetDISPDVSize(eDevID, &stDispDVSizeInfo);
        }
    }
    _DRV_MVOP_Return(E_MVOP_DEV_0,E_MVOP_OK);
}
...
{
    //MS_U16 u16CopiedLength = sizeof(MVOP_DMSStreamINFO);
    MVOP_DevID  eDevID = E_MVOP_DEV_0; //default is to control main mvop
    ...
    if(NULL == fpXCWrite)
    {
        HALMVOStreamINFO stStreamInfo;
        stStreamInfo.eTileFormat = pstMVOPSTREAMINFO->eTileFormat;
        stStreamInfo.bIs422Mode = pstMVOPSTREAMINFO->bIs422Mode;
        stStreamInfo.bIsDRAMCont = pstMVOPSTREAMINFO->bIsDRAMCont;
        stStreamInfo.bDDR4_REMAP = pstMVOPSTREAMINFO->bDDR4_REMAP;

        HAL_MVOP_SetStreamInfo(eDevID, &stStreamInfo);
    }
    _DRV_MVOP_Return(E_MVOP_DEV_0,E_MVOP_OK);
}
...
{
    //MS_U16 u16CopiedLength = sizeof(MVOP_DMSCropINFO);
    MVOP_DevID  eDevID = E_MVOP_DEV_0; //default is to control main mvop
    ...
    if(NULL == fpXCWrite)
    {
        if(eDevID == E_MVOP_DEV_0)
        {
            ...
            HAL_MVOP_SetStartX(eDevID, pstMVOPCROPINFO->stCropStart.u16Xpos);
            HAL_MVOP_SetStartY(eDevID, pstMVOPCROPINFO->stCropStart.u16Ypos, FALSE);

            if((pstMVOPCROPINFO->stCropSize.u16Width == 0) || (pstMVOPCROPINFO->stCropSize.u16Height == 0))
            {
                HAL_MVOP_SetPicWidthMinus(eDevID, 0, p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming.u16Width);
                HAL_MVOP_SetPicHeightMinus(eDevID, 0, p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming.u16Height);
            }
            ...
            {
                HAL_MVOP_SetPicWidthMinus(eDevID, pstMVOPCROPINFO->stCropSize.u16Width-1, p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming.u16Width);
                HAL_MVOP_SetPicHeightMinus(eDevID, pstMVOPCROPINFO->stCropSize.u16Height-1, p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming.u16Height);
            }
        }
#if MVOP_SUPPORT_SUB
        else if(eDevID == E_MVOP_DEV_1)
        {
            ...
            if((pstMVOPCROPINFO->stCropSize.u16Width == 0) || (pstMVOPCROPINFO->stCropSize.u16Height == 0))
            {
                HAL_MVOP_SetPicWidthMinus(eDevID, 0, p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopTiming.u16Width);
                HAL_MVOP_SetPicHeightMinus(eDevID, 0, p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopTiming.u16Height);
            }
            else
            {
                HAL_MVOP_SetPicWidthMinus(eDevID, pstMVOPCROPINFO->stCropSize.u16Width-1, p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopTiming.u16Width);
                HAL_MVOP_SetPicHeightMinus(eDevID, pstMVOPCROPINFO->stCropSize.u16Height-1, p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopTiming.u16Height);
            }
        }
#endif
    }
