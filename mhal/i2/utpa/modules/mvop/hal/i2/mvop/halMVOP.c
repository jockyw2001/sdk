...
void _HAL_MVOP_InitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->u16ECONum = 0;
    g_pHalMVOPCtx->bDramRdContd = FALSE;
    g_pHalMVOPCtx->bIsEnLSB = 0;
}
...
void _HAL_MVOP_SubInitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->bSubDramRdContd = FALSE;
    g_pHalMVOPCtx->bSubIsEnLSB = 0;
#endif
}
...
void HAL_MVOP_ForceWriteReg(void)
{
    HAL_WriteRegBit(VOP_REG_WR, 1, BIT4);
    HAL_WriteRegBit(VOP_REG_WR, 0, BIT4);
}
...
void HAL_MVOP_Rst(void)
{
    ...
    HAL_WriteRegBit(VOP_CTRL0, 0, BIT4);
    HAL_WriteRegBit(VOP_CTRL0, 1, BIT4);

    g_pHalMVOPCtx->bIsEnable = 1;
}
...
MS_BOOL HAL_MVOP_IsSupport4k2k2P(void)
{
    return TRUE;
}
...
void HAL_MVOP_SetOutputInterlace(MS_BOOL bEnable, MS_U16 u16ECOVersion)
{
    MS_U8 regval;
    ...
    if ( bEnable )
    {
        ...
        if(u16ECOVersion == 0)
        {
            HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x4, CKG_DC0_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
        }
    }
    else
    {
...
MS_BOOL HAL_MVOP_SetTileFormat(MVOP_TileFormat eTileFmt)
{
    ...
    else if (eTileFmt == E_MVOP_TILE_32x16)
    {
        HAL_WriteRegBit(VOP_REG_WR, 0, BIT1);
        HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, EVD_ENABLE);
        return TRUE;
    }
    else if (eTileFmt == E_MVOP_TILE_32x32)
    {
        HAL_WriteRegBit(VOP_REG_WR, 0, BIT1);
        HAL_WriteRegBit(VOP_INPUT_SWITCH0, 0, EVD_ENABLE);
        HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_TILE_32x32);
        return TRUE;
    }
    else
    {
...
void HAL_MVOP_SetBlackBG(void)
{
    ...
    HAL_WriteByteMask(VOP_TST_IMG, 0x02, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(VOP_TST_IMG, 0x00, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(VOP_TST_IMG, regval, BIT2 | BIT1 | BIT0);
}
...
void HAL_MVOP_SetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion)
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        g_pHalMVOPCtx->bDramRdContd = pparam->bDramRdContd;
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    HAL_WriteByteMask(VOP_TST_IMG, 0x00, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(VOP_TST_IMG, regval, BIT2 | BIT1 | BIT0);
    HAL_WriteRegBit(VOP_CTRL0, ptiming->bHDuplicate, VOP_HDUP);// H pixel duplicate
...
{
    MVOP_DBG(MVOP_PRINTF("[%s] eco num = %d\n",__FUNCTION__,g_pHalMVOPCtx->u16ECONum);)
    MS_ASSERT( (clkNum==0) || (clkNum==1) );
    if (clkNum==0)
    {
        if(bEnable)
        {
            if(g_pHalMVOPCtx->u16ECONum == 0)
            {
                HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x4, CKG_DC0_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
            }
            else // k6 eco for mfdec sram clock switch.
            {
                HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC0_SRAM);
                HAL_WriteByteMask(VOP_REG_DUMMY_5D, 0x0, VOP_SRAM_CLK_AUTO);
                HAL_MVOP_LoadReg();
            }
        }
        else
...
void HAL_MVOP_SetSynClk(MVOP_Timing *ptiming)
{
    ...
    if(g_pHalMVOPCtx->u32MVOPFixClk != 0)
    {
        ...
        if(g_pHalMVOPCtx->b3DLRMode)//4k1k mvc
        {
            //burst and priority
    ...
    else if(g_pHalMVOPCtx->bEnableFreerunMode)
    {
        ...
        MS_U64 u64exp_clock = ((MS_U64)ptiming->u16H_TotalCount * (MS_U64)ptiming->u16V_TotalCount * (MS_U64)ptiming->u16ExpFrameRate);
        do_div(u64exp_clock, 1000);
        do_div(u64mpll_clock, u64exp_clock);
...
/******************************************************************************/
void HAL_MVOP_SetH264HardwireMode(MS_U16 u16ECOVersion)
{
    // Hardwire mode
    ...
    if(u16ECOVersion == 0)
    {
        HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC0_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
    }
    // Write trigger
    HAL_MVOP_LoadReg();
}
...
/******************************************************************************/
void HAL_MVOP_SetRMHardwireMode(MS_U16 u16ECOVersion)
{
...
/******************************************************************************/
/// Set MVOP for EVD TCH Vision Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SetEVDHardwireMode_TCH(void)
{
    // Hardwire mode
    HAL_WriteByte(VOP_MPG_JPG_SWITCH, 0x00);
...
/******************************************************************************/
void HAL_MVOP_SetVP9HardwireMode(MS_U16 u16ECOVersion)
{
    HAL_MVOP_SetEVDHardwireMode(u16ECOVersion);
}

/******************************************************************************/
/// Set MVOP for AVS2 Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SetAVS2HardwireMode()
{
    MVOP_PRINTF("%s not support AVS2, please check MDrv_MVOP_SetInputCfg\n", __FUNCTION__);
}
...
MS_BOOL HAL_MVOP_SetYUVBaseAddMultiView(MVOP_DevID eID, MVOP_BaseAddInput *stBaseAddInfo)
{
    MS_PHY u64tmpY = 0;
    MS_PHY u64tmpUV = 0;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
       {
            switch(stBaseAddInfo->eView)
            {
                ...
                break;
                case E_MVOP_DV_EL_VIEW:
                {
                    // Y offset
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR0_L), u64tmpY & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR0_L+1), (u64tmpY >> 8) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR0_L+2), (u64tmpY >> 16) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR0_L+3), (u64tmpY >> 24) & VOP_YUV_STR_HIBITS);

                    // UV offset
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR0_L), u64tmpUV & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR0_L+1), (u64tmpUV >> 8) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR0_L+2), (u64tmpUV >> 16) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR0_L+3), (u64tmpUV >> 24) & VOP_YUV_STR_HIBITS);
                }
                break;
                default:
                    break;
        ...
        case E_MVOP_DEV_1:
            ...
            switch(stBaseAddInfo->eView)
            {
                ...
                {
                    // Y offset
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR1_L), u64tmpY & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR1_L+1), (u64tmpY >> 8) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR1_L+2), (u64tmpY >> 16) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_YSTR1_L+3), (u64tmpY >> 24) & VOP_YUV_STR_HIBITS);

                    //UV offset
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR1_L), u64tmpUV & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR1_L+1), (u64tmpUV >> 8) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR1_L+2), (u64tmpUV >> 16) & 0xff);
                    HAL_WriteByte(SUB_REG(VOP_JPG_UVSTR1_L+3), (u64tmpUV >> 24) & VOP_YUV_STR_HIBITS);
                }
                break;
                case E_MVOP_DV_EL_VIEW:
                {
...
MS_BOOL HAL_MVOP_SetHorDup(MS_BOOL bEnable)
{
    HAL_WriteRegBit(VOP_CTRL0, bEnable, BIT2);// V line duplicate
    return TRUE;
}
...
{
    HAL_WriteRegBit(VOP_CTRL0, bEnable, VOP_HDUP);// x4 duplicate should raise H pixel duplicate first
...
void HAL_MVOP_SetStartY(MVOP_DevID eID, MS_U16 u16YPos, MS_BOOL bIsInterlace)
{
    ...
    case E_MVOP_DEV_0:
        ...
        HAL_WriteByteMask((VOP_REG_CROP_VSTART + 1), ((u16YPos) >> (8)) & (0x7f), 0x7f);
        // Write trigger
        HAL_MVOP_LoadReg();
        break;
    case E_MVOP_DEV_1:
        ...
        HAL_WriteByteMask(SUB_REG(VOP_REG_CROP_VSTART + 1), ((u16YPos) >> (8)) & (0x7f), 0x7f);
        // Write trigger
        HAL_MVOP_SubLoadReg();
#endif
        break;
...
void HAL_MVOP_SetPicWidthMinus(MVOP_DevID eID, MS_U16 u16XSizes, MS_U16 u16Width)
{
    ...
    case E_MVOP_DEV_0:
        ...
        MVOP_DBG(MVOP_PRINTF("[MVOP][DBG] 3D FORMAT = %d\n",en3DType);)
        if(en3DType == E_MVOP_OUTPUT_3D_SBS)
        {
            u16XSizes = u16XSizes >> 1;
        }
...
void HAL_MVOP_SetPicHeightMinus(MVOP_DevID eID, MS_U16 u16YSizes, MS_U16 u16Height)
{
    ...
    case E_MVOP_DEV_0:
        ...
        en3DType = HAL_MVOP_GetOutput3DType();
        MVOP_DBG(MVOP_PRINTF("[MVOP][DBG] 3D FORMAT = %d\n",en3DType);)
        if((en3DType == E_MVOP_OUTPUT_3D_TB) || (en3DType == E_MVOP_OUTPUT_3D_LA))
        {
            u16YSizes = u16YSizes >> 1;
        }
...
MS_BOOL HAL_MVOP_SetEVDYUVBaseAdd(MVOP_EVDBaseAddInput *stEVDBaseAddInfo)
{
    ...
    // Set MSB YUV Address
    //----------------------------------------------------
    g_pHalMVOPCtx->bIsEnLSB = stEVDBaseAddInfo->bEnLSB;
    if(stEVDBaseAddInfo->bEnLSB)
    {
        ...
        HAL_WriteByte((VOP_LSB_YSTR0_H), (u64tmp >> 16) & 0xff);
        HAL_WriteByte((VOP_LSB_YSTR0_H+1), (u64tmp >> 24) & VOP_YUV_STR_HIBITS);
    ...
    {
        //Disable LSB: for miu select
        HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_Y_EN);
        HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_UV_EN);
        HAL_WriteByteMask(VOP_REG_MASK, 0X3, VOP_LSB_REQ_MASK);
    }
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECONumber)
{
    ...
    /*****************************************************/
    // Reset MVOP setting
    HAL_WriteByteMask(VOP_TST_IMG, 0x00, BIT2 | BIT1 | BIT0);//reset test pattern or BG
    HAL_WriteRegBit(VOP_TST_IMG, BIT6, BIT6);//reset test pattern or BG
    HAL_MVOP_Set3DLRAltOutput_VHalfScaling(DISABLE); //reset to default: disable 3D L/R alternative output.
    HAL_MVOP_Set3DLR2ndCfg(DISABLE);    //reset to default: disable 3D L/R 2nd pitch.
    HAL_MVOP_SetRgbFormat(E_MVOP_RGB_NONE); //reset rgb format
    HAL_WriteRegBit(VOP_CTRL1, 0, VOP_MVD_VS_MD); //default use original vsync
    ...
    HAL_MVOP_SetTimingFromXC(E_MVOP_DEV_0, TRUE);
    HAL_WriteRegBit(VOP_MIRROR_CFG_HI, ENABLE, VOP_HK_MASK); //default hsk mode bk background
    ...
    HAL_WriteRegBit(VOP_REG_MASK, 0, VOP_LSB_REQ_MASK);// EVD 10 bits disable
    HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_Y_EN);
    HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_UV_EN);
    HAL_WriteRegBit(VOP_UV_SHIFT, 1, VOP_GCLK_MIU_ON);
    ...
    if(u16ECONumber > 0)
        HAL_WriteByteMask(VOP_REG_DUMMY_5D, 0x0, VOP_LSB_422_BW_SAVE); //default enable lsb bw saving.
    HAL_WriteRegBit(VOP_INPUT_SWITCH0, 0, VOP_TILE_32x32);
    g_pHalMVOPCtx->bIsEnLSB = 0;
    HAL_MVOP_SetOutputInterlace(FALSE, g_pHalMVOPCtx->u16ECONum);
    //mfdec
    HAL_WriteRegBit(VOP_REG_MFDEC_0_L, FALSE, VOP_MFDEC_EN);
    //fd mask.BOB
    HAL_WriteRegBit(VOP_SW_FDMASK, FALSE, VOP_FDMASK);
    HAL_WriteRegBit(VOP_REG_FD_MASK_SEL, FALSE, VOP_FD_MASK_FROM_VDEC); //fd_mask double buffer
    HAL_WriteRegBit(VOP_SW_FDMASK, FALSE, VOP_FORCE_BOB);
    HAL_MVOP_LoadReg();
    HAL_MVOP_ForceWriteReg();
    if(TRUE == _HAL_MVOP_IsSupport4k2k2P())
    {
        HAL_Write2Byte(VOP_JPG_HSIZE, uHD_Width);
        HAL_Write2Byte(VOP_JPG_VSIZE, uHD_Height);
    }
    else
    {
        HAL_Write2Byte(VOP_JPG_HSIZE, fHD_Width);
        HAL_Write2Byte(VOP_JPG_VSIZE, fHD_Height);
    }
    HAL_WriteByte(VOP_DC_STRIP_H, 0);
    HAL_WriteByte(VOP_DS_IDX, 0xff);
    HAL_MVOP_SetHorDup(FALSE);
    HAL_MVOP_LoadReg();
    /*****************************************************/
        break;
    }
    ...
    case E_MVOP_DEV_1:
    {
        ...
        /*****************************************************/
        // Reset MVOP setting
        HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), 0x00, BIT2 | BIT1 | BIT0);//reset test pattern or BG
        HAL_WriteRegBit(SUB_REG(VOP_TST_IMG), BIT6, BIT6);//reset test pattern or BG
        ...
        HAL_WriteRegBit(SUB_REG(VOP_MIRROR_CFG_HI), ENABLE, VOP_HK_MASK); //default hsk mode bk background
        // Kano hsk default 2p
        ...
        /* only for STB DC */
        HAL_MVOP_SubSetFDMaskFromMVD(ENABLE);
        // Use frame sync (CLK_DC1 domain) to reset CLK_DC0 and CLK_MIU domain every frame
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_MASK), 0, VOP_LSB_REQ_MASK);// EVD 10 bits
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_Y_EN);
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_UV_EN);
        // Disable 420 BW Saving mode
        ...
        if(u16ECONumber > 0)
            HAL_WriteByteMask(SUB_REG(VOP_REG_DUMMY_5D), 0x0, VOP_LSB_422_BW_SAVE); //default enable lsb bw saving.
        HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 0, VOP_TILE_32x32);
        g_pHalMVOPCtx->bSubIsEnLSB = 0;
        HAL_MVOP_SubSetOutputInterlace(FALSE, g_pHalMVOPCtx->u16ECONum);
        //fd mask.BOB
        HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), FALSE, VOP_FDMASK);
        HAL_WriteRegBit(SUB_REG(VOP_REG_FD_MASK_SEL), FALSE, VOP_FD_MASK_FROM_VDEC); //fd_mask double buffer
        HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), FALSE, VOP_FORCE_BOB);
        //mfdec
        HAL_WriteRegBit(SUB_REG(VOP_REG_MFDEC_0_L), FALSE, VOP_MFDEC_EN);
        HAL_MVOP_SubLoadReg();
        HAL_MVOP_SubForceWriteReg();
        if(TRUE == _HAL_MVOP_IsSupport4k2k2P())
        {
            HAL_Write2Byte(SUB_REG(VOP_JPG_HSIZE), uHD_Width);
            HAL_Write2Byte(SUB_REG(VOP_JPG_VSIZE), uHD_Height);
        }
        else
        {
            HAL_Write2Byte(SUB_REG(VOP_JPG_HSIZE), fHD_Width);
            HAL_Write2Byte(SUB_REG(VOP_JPG_VSIZE), fHD_Height);
        }
        HAL_WriteByte(SUB_REG(VOP_DC_STRIP_H), 0);
        HAL_WriteByte(SUB_REG(VOP_DS_IDX), 0xff);
        HAL_MVOP_SubSetHorDup(FALSE);
        HAL_MVOP_SubLoadReg();
        HAL_MVOP_SubForceWriteReg();
        HAL_MVOP_SubSetVerDup(FALSE);
        HAL_MVOP_SubLoadReg();
        /*****************************************************/
        break;
    }
...
void HAL_MVOP_SetCropforXC(MVOP_DevID eID, MVOP_XCGetCrop* stXCCrop, MS_U16 u16Width, MS_U16 u16Height)
{
    MVOP_DBG(MVOP_PRINTF("%s [crop info] xst: 0x%x yst: 0x%x xsize: 0x%x ysize: 0x%x Width: 0x%x Height: 0x%x\n", __FUNCTION__,
        stXCCrop->u16XStart, stXCCrop->>u16YStart, stXCCrop->>u16XSize, stXCCrop->u16YSize, u16Width, u16Height);)
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            ...
            en3DType = HAL_MVOP_GetOutput3DType();
            MVOP_DBG(MVOP_PRINTF("%s 3D FORMAT = %d\n", __FUNCTION__, en3DType);)
            stXCCrop->u16XStart = ALIGN_UPTO_2(stXCCrop->u16XStart);
            if(en3DType == E_MVOP_OUTPUT_3D_SBS)
            {
                stXCCrop->u16XSize = stXCCrop->u16XSize >> 1;
            }
            stXCCrop->u16XSize = ALIGN_UPTO_2(stXCCrop->u16XSize);
            ...
            stXCCrop->u16YStart |= 0x6000;
            if((en3DType == E_MVOP_OUTPUT_3D_TB) || (en3DType == E_MVOP_OUTPUT_3D_LA))
            {
                stXCCrop->u16YSize = stXCCrop->u16YSize >> 1;
            }
...
void HAL_MVOP_SetInterlaceType(MVOP_DevID eDevID, MS_U16 u16ECONumber, MS_U8 u8Interlace)
{
    ...
    {
    case E_MVOP_DEV_0:
        if(g_pHalMVOPCtx->eInterlaceType == E_MVOP_INT_TB_SEP_FRAME)
        {
            HAL_WriteRegBit(VOP_INT_TYPE, 1, VOP_EVD_INT_SEP);
        }
        else if(g_pHalMVOPCtx->eInterlaceType == E_MVOP_INT_TB_ONE_FRAME)
        {
...
#if MVOP_SUPPORT_SUB
    case E_MVOP_DEV_1:
        if(g_pHalMVOPCtx->eSubInterlaceType == E_MVOP_INT_TB_SEP_FRAME)
        {
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 1, VOP_EVD_INT_SEP);
        }
        else if(g_pHalMVOPCtx->eSubInterlaceType == E_MVOP_INT_TB_ONE_FRAME)
        {
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 0, VOP_EVD_INT_SEP);
...
void HAL_MVOP_Exit(MVOP_DevID eID)
{
    switch(eID)
    {
    case E_MVOP_DEV_0:
...
void HAL_MVOP_UVSwapEnable(MVOP_DevID eID, MS_BOOL bEnable)
{
    switch(eID)
    {
    case E_MVOP_DEV_0:
        HAL_WriteRegBit(VOP_TST_IMG, bEnable, VOP_UV_SWAP);
        break;
#if MVOP_SUPPORT_SUB
    case E_MVOP_DEV_1:
        HAL_WriteRegBit(SUB_REG(VOP_TST_IMG), bEnable, VOP_UV_SWAP);
        break;
#endif
    default:
        break;
...
void HAL_MVOP_SetECONumber(MS_U16 u16ECOVersion)
{
    if (g_pHalMVOPCtx == NULL)
    {
        MVOP_DBG(MVOP_PRINTF("%s g_pHalMVOPCtx is NULL pointer\n", __FUNCTION__);)
    ...
    g_pHalMVOPCtx->u16ECONum = u16ECOVersion;
    MVOP_DBG(MVOP_PRINTF("[%s] version num = %d\n",__FUNCTION__);)
}

void HAL_MVOP_GetCropInfo(MVOP_DevID eID, MVOP_SetCropPos* stCropStart, MVOP_SetImageWH* stCropSize)
{
    switch(eID)
    {
        case E_MVOP_DEV_0:
            stCropStart->u16Xpos = g_pHalMVOPCtx->u16CropXStart;
            stCropStart->u16Xpos = g_pHalMVOPCtx->u16CropYStart;
            stCropSize->u16Width = g_pHalMVOPCtx->u16CropXSize;
            stCropSize->u16Height = g_pHalMVOPCtx->u16CropYSize;
            break;
        case E_MVOP_DEV_1:
#if MVOP_SUPPORT_SUB
            stCropStart->u16Xpos = g_pHalMVOPCtx->u16SubCropXStart;
            stCropStart->u16Xpos = g_pHalMVOPCtx->u16SubCropYStart;
            stCropSize->u16Width = g_pHalMVOPCtx->u16SubCropXSize;
            stCropSize->u16Height = g_pHalMVOPCtx->u16SubCropYSize;
#endif
            break;
        default:
            break;
...
void HAL_MVOP_MDBRegInputCfg(MVOP_DevID eID, MVOP_InputSel _eInputSel, MVOP_RegInputMdb* stRegInput)
{
    if(MVOP_INPUT_UNKNOWN != _eInputSel)
    {
        return;
    }
    else if(MVOP_INPUT_DRAM == _eInputSel)
    {
        switch(eID)
        {
            case E_MVOP_DEV_0:
            {
                stRegInput->u32RegYoffset = HAL_Read2Byte(VOP_JPG_YSTR0_L);
                stRegInput->u32RegYoffset |=((HAL_ReadByte(VOP_JPG_YSTR0_H)<<16)&0x3ff0000);
                stRegInput->u32RegYoffset = stRegInput->u32RegYoffset << 3; //address 8 byte align
                stRegInput->u32RegUVoffset = HAL_Read2Byte(VOP_JPG_UVSTR0_L);
                stRegInput->u32RegUVoffset |=((HAL_ReadByte(VOP_JPG_UVSTR0_H)<<16)&0x3ff0000);
                stRegInput->u32RegUVoffset = stRegInput->u32RegUVoffset << 3;
                stRegInput->u16RegHsize = HAL_Read2Byte(VOP_JPG_HSIZE);
                stRegInput->u16RegVsize = HAL_Read2Byte(VOP_JPG_VSIZE);
                stRegInput->u16RegStrip = HAL_ReadByte(VOP_DC_STRIP);
                stRegInput->bReg422 = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_IMG422) == VOP_IMG422);
                stRegInput->bRegPMode = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_PRO_SEQ) == VOP_PRO_SEQ);
                stRegInput->bRegDramContd = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_DRAM_RD_MODE) == VOP_DRAM_RD_MODE);
                stRegInput->bRegField = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_FIELD) == VOP_FIELD);
                stRegInput->bReg422Pack = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_422PACK) == VOP_422PACK);
                stRegInput->u8RegDSIndex = HAL_ReadByte(VOP_DS_IDX);
                stRegInput->u8RegFDMask = (HAL_ReadRegBit(VOP_DS_IDX, VOP_SW_FD_MASK) == VOP_SW_FD_MASK);
                stRegInput->bRegMfdecEn = (HAL_ReadRegBit(VOP_REG_MFDEC_0_L, VOP_MFDEC_EN) == VOP_MFDEC_EN);
                stRegInput->u32RegBitenAdd = HAL_Read2Byte(VOP_REG_BITLEN_ADD_L);
                stRegInput->u32RegBitenAdd |=(HAL_ReadByte((VOP_REG_BITLEN_ADD_H))<<16); //BIT0-31
                stRegInput->u32RegBitenAdd = stRegInput->u32RegBitenAdd << 3;
                break;
            }
#if MVOP_SUPPORT_SUB
            case E_MVOP_DEV_1:
            {
                    stRegInput->u32RegYoffset = HAL_Read2Byte(SUB_REG(VOP_JPG_YSTR0_L));
                    stRegInput->u32RegYoffset |=((HAL_ReadByte(SUB_REG(VOP_JPG_YSTR0_H))<<16)&0x3ff0000);
                    stRegInput->u32RegYoffset = stRegInput->u32RegYoffset << 3; //address 8 byte align
                    stRegInput->u32RegUVoffset = HAL_Read2Byte(SUB_REG(VOP_JPG_UVSTR0_L));
                    stRegInput->u32RegUVoffset |=((HAL_ReadByte(SUB_REG(VOP_JPG_UVSTR0_H))<<16)&0x3ff0000);
                    stRegInput->u32RegUVoffset = stRegInput->u32RegUVoffset << 3;
                    stRegInput->u16RegHsize = HAL_Read2Byte(SUB_REG(VOP_JPG_HSIZE));
                    stRegInput->u16RegVsize = HAL_Read2Byte(SUB_REG(VOP_JPG_VSIZE));
                    stRegInput->u16RegStrip = HAL_ReadByte(SUB_REG(VOP_DC_STRIP));
                    stRegInput->bReg422 = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_IMG422) == VOP_IMG422);
                    stRegInput->bRegPMode = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_PRO_SEQ) == VOP_PRO_SEQ);
                    stRegInput->bRegDramContd = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_DRAM_RD_MODE) == VOP_DRAM_RD_MODE);
                    stRegInput->bRegField = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_FIELD) == VOP_FIELD);
                    stRegInput->bReg422Pack = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_422PACK) == VOP_422PACK);
                    stRegInput->u8RegDSIndex = HAL_ReadByte(SUB_REG(VOP_DS_IDX));
                    stRegInput->u8RegFDMask = (HAL_ReadRegBit(SUB_REG(VOP_DS_IDX), VOP_SW_FD_MASK) == VOP_SW_FD_MASK);
                    stRegInput->bRegMfdecEn = (HAL_ReadRegBit(SUB_REG(VOP_REG_MFDEC_0_L), VOP_MFDEC_EN) == VOP_MFDEC_EN);
                    stRegInput->u32RegBitenAdd = HAL_Read2Byte(SUB_REG(VOP_REG_BITLEN_ADD_L));
                    stRegInput->u32RegBitenAdd |=(HAL_ReadByte(SUB_REG(VOP_REG_BITLEN_ADD_H))<<16); //BIT0-31
                    stRegInput->u32RegBitenAdd = stRegInput->u32RegBitenAdd << 3;
                    break;
                }
#endif
    ...
    else //mvop hardwire mode
    {
        switch(eID)
        {
            ...
            {
                //reg_riu_debug_sel = 1
                HAL_WriteByteMask(VOP_DEBUG_2F_H, VOP_DBG_SEL1, VOP_RIU_DEBUG_SEL);
                stRegInput->u32RegYoffset = HAL_Read2Byte(VOP_DEBUG_2A);
                stRegInput->u32RegYoffset |=((HAL_ReadByte((VOP_DEBUG_2B))<<16)&0x3ff0000);
                stRegInput->u32RegYoffset = stRegInput->u32RegYoffset << 3; //address 8 byte align
                stRegInput->u32RegUVoffset = HAL_Read2Byte(VOP_DEBUG_2C);
                stRegInput->u32RegUVoffset |=((HAL_ReadByte((VOP_DEBUG_2D))<<16)&0x3ff0000);
                stRegInput->u32RegUVoffset = stRegInput->u32RegUVoffset << 3;
                stRegInput->u16RegHsize = HAL_Read2Byte(VOP_DEBUG_2E);
                stRegInput->u16RegVsize = (HAL_Read2Byte(VOP_DEBUG_2F_L)&0x1ff);
                //reg_riu_debug_sel = 2
                HAL_WriteByteMask(VOP_DEBUG_2F_H, VOP_DBG_SEL2, VOP_RIU_DEBUG_SEL);
                stRegInput->u16RegStrip = HAL_Read2Byte(VOP_DEBUG_2E)&0x7ff; //bit 0-9
                stRegInput->bReg422 = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_422PACK) == VOP_422PACK); //cannot get from wb
                stRegInput->bRegPMode = (HAL_ReadRegBit((VOP_DEBUG_2E+1), BIT3) == BIT3);
                stRegInput->bRegDramContd = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_DRAM_RD_MODE) == VOP_DRAM_RD_MODE);//cannot get from wb
                stRegInput->bRegField = (HAL_ReadRegBit((VOP_DEBUG_2E+1), BIT4) == BIT4);
                stRegInput->bReg422Pack = (HAL_ReadRegBit(VOP_MPG_JPG_SWITCH, VOP_422PACK) == VOP_422PACK);//cannot get from wb
                stRegInput->u8RegDSIndex = HAL_ReadByte(VOP_DEBUG_2F_L);
                stRegInput->u8RegFDMask = (HAL_ReadRegBit((VOP_DEBUG_2E+1), BIT5) == BIT5);
                //reg_riu_debug_sel = 6
                HAL_WriteByteMask(VOP_DEBUG_2F_H, VOP_DBG_SEL6, VOP_RIU_DEBUG_SEL);
                stRegInput->bRegMfdecEn = (HAL_ReadRegBit(VOP_DEBUG_2C, BIT2) == BIT2);
                stRegInput->u32RegBitenAdd = HAL_Read2Byte(VOP_DEBUG_2D);
                stRegInput->u32RegBitenAdd |=(HAL_ReadByte((VOP_DEBUG_2E))<<16); //BIT0-31
                stRegInput->u32RegBitenAdd = stRegInput->u32RegBitenAdd << 3;
                HAL_WriteByteMask(VOP_DEBUG_2F_H, VOP_DBG_SEL0, VOP_RIU_DEBUG_SEL); //RESET
                break;
            }
#if MVOP_SUPPORT_SUB
            case E_MVOP_DEV_1:
            {
                //reg_riu_debug_sel = 1
                HAL_WriteByteMask(SUB_REG(VOP_DEBUG_2F_H), VOP_DBG_SEL1, VOP_RIU_DEBUG_SEL);
                stRegInput->u32RegYoffset = HAL_Read2Byte(SUB_REG(VOP_DEBUG_2A));
                stRegInput->u32RegYoffset |=((HAL_ReadByte(SUB_REG(VOP_DEBUG_2B))<<16)&0x3ff0000);
                stRegInput->u32RegYoffset = stRegInput->u32RegYoffset << 3; //address 8 byte align
                stRegInput->u32RegUVoffset = HAL_Read2Byte(SUB_REG(VOP_DEBUG_2C));
                stRegInput->u32RegUVoffset |=((HAL_ReadByte(SUB_REG(VOP_DEBUG_2D))<<16)&0x3ff0000);
                stRegInput->u32RegUVoffset = stRegInput->u32RegUVoffset << 3;
                stRegInput->u16RegHsize = HAL_Read2Byte(SUB_REG(VOP_DEBUG_2E));
                stRegInput->u16RegVsize = (HAL_Read2Byte(SUB_REG(VOP_DEBUG_2F_L))&0x1ff);
                //reg_riu_debug_sel = 2
                HAL_WriteByteMask(SUB_REG(VOP_DEBUG_2F_H), VOP_DBG_SEL2, VOP_RIU_DEBUG_SEL);
                stRegInput->u16RegStrip = HAL_Read2Byte(SUB_REG(VOP_DEBUG_2E))&0x7ff; //bit 0-9
                stRegInput->bReg422 = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_422PACK) == VOP_422PACK); //cannot get from wb
                stRegInput->bRegPMode = (HAL_ReadRegBit(SUB_REG(VOP_DEBUG_2E+1), BIT3) == BIT3);
                stRegInput->bRegDramContd = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_DRAM_RD_MODE) == VOP_DRAM_RD_MODE);//cannot get from wb
                stRegInput->bRegField = (HAL_ReadRegBit(SUB_REG(VOP_DEBUG_2E+1), BIT4) == BIT4);
                stRegInput->bReg422Pack = (HAL_ReadRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), VOP_422PACK) == VOP_422PACK);//cannot get from wb
                stRegInput->u8RegDSIndex = HAL_ReadByte(SUB_REG(VOP_DEBUG_2F_L));
                stRegInput->u8RegFDMask = (HAL_ReadRegBit(SUB_REG(VOP_DEBUG_2E+1), BIT5) == BIT5);
                //reg_riu_debug_sel = 6
                HAL_WriteByteMask(SUB_REG(VOP_DEBUG_2F_H), VOP_DBG_SEL6, VOP_RIU_DEBUG_SEL);
                stRegInput->bRegMfdecEn = (HAL_ReadRegBit(SUB_REG(VOP_DEBUG_2C), BIT2) == BIT2);
                stRegInput->u32RegBitenAdd = HAL_Read2Byte(SUB_REG(VOP_DEBUG_2D));
                stRegInput->u32RegBitenAdd |=(HAL_ReadByte(SUB_REG(VOP_DEBUG_2E))<<16); //BIT0-31
                stRegInput->u32RegBitenAdd = stRegInput->u32RegBitenAdd << 3;
                HAL_WriteByteMask(SUB_REG(VOP_DEBUG_2F_H), VOP_DBG_SEL0, VOP_RIU_DEBUG_SEL); //RESET
                break;
            }
#endif
            default:
...
void HAL_MVOP_MDBRegOutputCfg(MVOP_DevID eID, MVOP_RegOutputMdb* stRegOutput)
{
    MS_U16 u16Vblk;
    MS_U16 u16Hblk;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            stRegOutput->u32RegVtt = HAL_Read2Byte(VOP_FRAME_VCOUNT);
            stRegOutput->u32RegHtt = HAL_Read2Byte(VOP_FRAME_HCOUNT);
            stRegOutput->u16RegVImgst = HAL_Read2Byte(VOP_IMG_VSTR0);
            stRegOutput->u16RegHImgst = HAL_Read2Byte(VOP_IMG_HSTR);
            stRegOutput->bRegInterlace = (HAL_ReadRegBit(VOP_CTRL0, BIT7) == BIT7);
            stRegOutput->enRegCLOCK = HAL_MVOP_GetCurrentClk();
            stRegOutput->u8RegFPS = stRegOutput->enRegCLOCK / (((MS_U32)stRegOutput->u32RegVtt)*((MS_U32)stRegOutput->u32RegHtt));//FIXME
            stRegOutput->u16RegExpFPS = (stRegOutput->enRegCLOCK*1000) / ((MS_U32)stRegOutput->u32RegVtt*(MS_U32)stRegOutput->u32RegHtt);//FIXME
            u16Vblk = (HAL_Read2Byte(VOP_VB0_END) - HAL_Read2Byte(VOP_VB0_STR)) + (HAL_Read2Byte(VOP_VB1_END) - HAL_Read2Byte(VOP_VB1_STR));
            u16Hblk = HAL_Read2Byte(VOP_IMG_HSTR);
            stRegOutput->u16RegWidth = stRegOutput->u32RegHtt - u16Hblk;
            stRegOutput->u16RegHeight = stRegOutput->u32RegVtt - u16Vblk;
            stRegOutput->bRegHDup = (HAL_ReadRegBit(VOP_CTRL0, VOP_HDUP) == VOP_HDUP);
            stRegOutput->bReg2P = (HAL_ReadRegBit(VOP_REG_4K2K_2P, VOP_4K2K_2P) == VOP_4K2K_2P);
            stRegOutput->u16RegVsForward = (VOP_MVD_VS_MD == HAL_ReadRegBit(VOP_CTRL1, VOP_MVD_VS_MD)) ? (HAL_Read2Byte(VOP_FRAME_VCOUNT)-HAL_Read2Byte(VOP_TF_VS_MVD)):0 ;
            break;
        }
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            stRegOutput->u32RegVtt = HAL_Read2Byte(SUB_REG(VOP_FRAME_VCOUNT));
            stRegOutput->u32RegHtt = HAL_Read2Byte(SUB_REG(VOP_FRAME_HCOUNT));
            stRegOutput->u16RegVImgst = HAL_Read2Byte(SUB_REG(VOP_IMG_VSTR0));
            stRegOutput->u16RegHImgst = HAL_Read2Byte(SUB_REG(VOP_IMG_HSTR));
            stRegOutput->bRegInterlace = (HAL_ReadRegBit(SUB_REG(VOP_CTRL0), BIT7) == BIT7);
            stRegOutput->enRegCLOCK = HAL_MVOP_SubGetCurrentClk();
            stRegOutput->u8RegFPS = stRegOutput->enRegCLOCK / (((MS_U32)stRegOutput->u32RegVtt)*((MS_U32)stRegOutput->u32RegHtt));//FIXME
            stRegOutput->u16RegExpFPS = (stRegOutput->enRegCLOCK*1000) / ((MS_U32)stRegOutput->u32RegVtt*(MS_U32)stRegOutput->u32RegHtt);//FIXME
            u16Vblk = (HAL_Read2Byte(SUB_REG(VOP_VB0_END)) - HAL_Read2Byte(SUB_REG(VOP_VB0_STR)))
                + (HAL_Read2Byte(SUB_REG(VOP_VB1_END)) - HAL_Read2Byte(SUB_REG(VOP_VB1_STR)));
            u16Hblk = HAL_Read2Byte(SUB_REG(VOP_IMG_HSTR));
            stRegOutput->u16RegWidth = stRegOutput->u32RegHtt - u16Hblk;
            stRegOutput->u16RegHeight = stRegOutput->u32RegVtt - u16Vblk;
            stRegOutput->bRegHDup = (HAL_ReadRegBit(SUB_REG(VOP_CTRL0), VOP_HDUP) == VOP_HDUP);
            stRegOutput->bReg2P = (HAL_ReadRegBit(SUB_REG(VOP_REG_4K2K_2P), VOP_4K2K_2P) == VOP_4K2K_2P);
            stRegOutput->u16RegVsForward = (VOP_MVD_VS_MD == HAL_ReadRegBit(SUB_REG(VOP_CTRL1), VOP_MVD_VS_MD)) ? (HAL_Read2Byte(SUB_REG(VOP_FRAME_VCOUNT))-HAL_Read2Byte(SUB_REG(VOP_TF_VS_MVD))):0;
            break;
        }
#endif
        default:
...
void HAL_MVOP_MDBRegFeature(MVOP_DevID eID, MVOP_FeatureMdb* pstFeature)
{
    MS_U32 u32clock;
    MS_U32 u32Vtt;
    MS_U32 u32Htt;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            pstFeature->enMirror = HAL_ReadRegBit(VOP_MIRROR_CFG, VOP_MIRROR_CFG_HEN) + (HAL_ReadRegBit(VOP_MIRROR_CFG, VOP_MIRROR_CFG_VEN) == VOP_MIRROR_CFG_VEN);
            if(HAL_ReadRegBit(VOP_MULTI_WIN_CFG0, VOP_LR_BUF_MODE) == VOP_LR_BUF_MODE)
            {
                if(HAL_ReadRegBit(VOP_MULTI_WIN_CFG0, VOP_LR_LA_OUT) == VOP_LR_LA_OUT)
                {
                    if(HAL_ReadRegBit(VOP_MULTI_WIN_CFG0, VOP_LR_LA2SBS_OUT) == VOP_LR_LA2SBS_OUT)
                        pstFeature->en3D = E_MVOP_OUTPUT_3D_SBS;
                    else
                        pstFeature->en3D = E_MVOP_OUTPUT_3D_LA;
                }
                else
                {
                    pstFeature->en3D = E_MVOP_OUTPUT_3D_TB;
                }
            }
            else
            {
                pstFeature->en3D = E_MVOP_OUTPUT_3D_NONE;
            }
            pstFeature->bHSK = (HAL_ReadRegBit(VOP_HANDSHAKE, VOP_HANDSHAKE_MODE) == VOP_HANDSHAKE_MODE);
            pstFeature->bXCGenTiming = (0 ==(HAL_ReadByte(VOP_HANDSHAKE)&0x06))?0:1;
            pstFeature->stCropSt.u16Xpos = HAL_ReadByte(VOP_REG_CROP_HSTART)&0x1fff;
            pstFeature->stCropSt.u16Ypos = HAL_ReadByte(VOP_REG_CROP_VSTART)&0x1fff;
            pstFeature->stCropSize.u16Width = HAL_ReadByte(VOP_REG_CROP_HSIZE)&0x1fff;
            pstFeature->stCropSize.u16Height = HAL_ReadByte(VOP_REG_CROP_VSIZE)&0x1fff;
            pstFeature->bOneField = 0; //not support one field in tv chip.
            pstFeature->b2P = (HAL_ReadRegBit(VOP_REG_4K2K_2P, VOP_4K2K_2P) == VOP_4K2K_2P);
            pstFeature->bForceP = (!(HAL_ReadRegBit(VOP_CTRL0, VOP_FIELD_MD) == VOP_FIELD_MD)) && (!(HAL_ReadRegBit(VOP_REG_DUMMY_3D_1, VOP_RGB_FILED_BYPASS) == VOP_RGB_FILED_BYPASS));
            u32clock = HAL_MVOP_GetCurrentClk();
            u32Vtt = HAL_Read2Byte(VOP_FRAME_VCOUNT);
            u32Htt = HAL_Read2Byte(VOP_FRAME_HCOUNT);
            pstFeature->u32Framerate = u32clock / (u32Vtt*u32Htt);
            break;
        }
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            pstFeature->enMirror = HAL_ReadRegBit(SUB_REG(VOP_MIRROR_CFG), VOP_MIRROR_CFG_HEN) + (HAL_ReadRegBit(SUB_REG(VOP_MIRROR_CFG), VOP_MIRROR_CFG_VEN) == VOP_MIRROR_CFG_VEN);
            if(HAL_ReadRegBit(SUB_REG(VOP_MULTI_WIN_CFG0), VOP_LR_BUF_MODE) == VOP_LR_BUF_MODE)
            {
                if(HAL_ReadRegBit(SUB_REG(VOP_MULTI_WIN_CFG0), VOP_LR_LA_OUT) == VOP_LR_LA_OUT)
                {
                    if(HAL_ReadRegBit(SUB_REG(VOP_MULTI_WIN_CFG0), VOP_LR_LA2SBS_OUT) == VOP_LR_LA2SBS_OUT)
                        pstFeature->en3D = E_MVOP_OUTPUT_3D_SBS;
                    else
                        pstFeature->en3D = E_MVOP_OUTPUT_3D_LA;
                }
            ...
            pstFeature->bHSK = (HAL_ReadRegBit(SUB_REG(VOP_HANDSHAKE), VOP_HANDSHAKE_MODE) == VOP_HANDSHAKE_MODE);
            pstFeature->bXCGenTiming = (0 ==(HAL_ReadByte(SUB_REG(VOP_HANDSHAKE))&0x06))?0:1;
            pstFeature->stCropSt.u16Xpos = HAL_ReadByte(SUB_REG(VOP_REG_CROP_HSTART))&0x1fff;
            pstFeature->stCropSt.u16Ypos = HAL_ReadByte(SUB_REG(VOP_REG_CROP_VSTART))&0x1fff;
            pstFeature->stCropSize.u16Width = HAL_ReadByte(SUB_REG(VOP_REG_CROP_HSIZE))&0x1fff;
            pstFeature->stCropSize.u16Height = HAL_ReadByte(SUB_REG(VOP_REG_CROP_VSIZE))&0x1fff;
            pstFeature->bOneField = 0; //not support one field in tv chip.
            pstFeature->b2P = (HAL_ReadRegBit(SUB_REG(VOP_REG_4K2K_2P), VOP_4K2K_2P) == VOP_4K2K_2P);
            pstFeature->bForceP = (!(HAL_ReadRegBit(SUB_REG(VOP_CTRL0), VOP_FIELD_MD) == VOP_FIELD_MD)) && (!(HAL_ReadRegBit(SUB_REG(VOP_REG_DUMMY_3D_1), VOP_RGB_FILED_BYPASS) == VOP_RGB_FILED_BYPASS));
            u32clock = HAL_MVOP_SubGetCurrentClk();
            u32Vtt = HAL_Read2Byte(SUB_REG(VOP_FRAME_VCOUNT));
            u32Htt = HAL_Read2Byte(SUB_REG(VOP_FRAME_HCOUNT));
            pstFeature->u32Framerate = u32clock / (u32Vtt*u32Htt);
            break;
        }
#endif
...
MS_BOOL HAL_MVOP_GetDCClkIsOn(MVOP_DevID eID)
{
    MS_BOOL bIsDisable = FALSE;

    bIsDisable = (CKG_DC0_GATED == HAL_ReadRegBit(REG_CKG_DC0, CKG_DC0_GATED));
    return !(bIsDisable);
}

void HAL_MVOP_MDBSetVsyncFwdLine(MS_U64* pu64ReqHdl, MVOP_DevID eID, MS_U32 u32VsyncLines)
{
    MS_U16 u16VTT = 0;
    MS_U16 u16BotVS = 0;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            if(TRUE == HAL_MVOP_GetDCClkIsOn(E_MVOP_DEV_0))
            {
                HAL_MVOP_Enable(FALSE, 25);
                u16VTT = HAL_Read2Byte(VOP_FRAME_VCOUNT);

                if(VOP_FIELD_MD == HAL_ReadRegBit(VOP_CTRL0, VOP_FIELD_MD)) //i mode
                {
                    u16BotVS = HAL_Read2Byte(VOP_BF_VS);
                    HAL_Write2Byte(VOP_BF_VS_MVD, (u16BotVS-u32VsyncLines));
                }
                HAL_Write2Byte(VOP_TF_VS_MVD, (u16VTT-u32VsyncLines));
                HAL_WriteRegBit(VOP_CTRL1, ENABLE, VOP_MVD_VS_MD);
                HAL_MVOP_LoadReg();
                HAL_MVOP_Enable(TRUE, 25);
            }
            break;
        }
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            if(TRUE == HAL_MVOP_GetDCClkIsOn(E_MVOP_DEV_1))
            {
                HAL_MVOP_SubEnable(FALSE, 25);
                u16VTT = HAL_Read2Byte(SUB_REG(VOP_FRAME_VCOUNT));

                if(VOP_FIELD_MD == HAL_ReadRegBit(SUB_REG(VOP_CTRL0), VOP_FIELD_MD)) //i mode
                {
                    u16BotVS = HAL_Read2Byte(SUB_REG(VOP_BF_VS));
                    HAL_Write2Byte(SUB_REG(VOP_BF_VS_MVD), (u16BotVS-u32VsyncLines));
                }
                HAL_Write2Byte(SUB_REG(VOP_TF_VS_MVD), (u16VTT-u32VsyncLines));
                HAL_WriteRegBit(SUB_REG(VOP_CTRL1), ENABLE, VOP_MVD_VS_MD);
                HAL_MVOP_SubLoadReg();
                HAL_MVOP_SubEnable(TRUE, 25);
            }
            break;
        }
#endif
...
void HAL_MVOP_SetMiuSaveArea(MVOP_DevID eID ,MVOP_MIUReadPrct stMiuSave)
{
    UNUSED(eID);
    UNUSED(stMiuSave);
    MVOP_PRINTF("[Warning][%s] not support\n",__FUNCTION__);
    return;
}

void HAL_MVOP_SetbyInputTiming(MVOP_DevID eID, MVOP_VidStat *pstVideoStatus)
{
    UNUSED(eID);
    UNUSED(pstVideoStatus);
    MVOP_PRINTF("[Warning][%s] not support\n",__FUNCTION__);
    return;
}
...
void HAL_MVOP_SetMFDECRotate(MVOP_DevID eID ,MVOP_MFDECRotate eRotate)
{
    UNUSED(eID);
    UNUSED(eRotate);
    return;
...
void HAL_MVOP_EnableFODtoWB(MVOP_DevID eID, MS_BOOL bEnable)
{
    UNUSED(eID);
    UNUSED(bEnable);
    return;
...
void HAL_MVOP_SetMFDECInfo(MVOP_DevID eID, HALMVOPMFDECINFO *pMFDECInfo)
{
    MS_PHY u64tmp = 0;
    ...
    switch(eID)
    {
        ...
        {
            #if 1
            if(pMFDECInfo->bMFDEC_EN)
            {
                HAL_WriteRegBit(VOP_REG_MFDEC_0_L, TRUE, VOP_MFDEC_EN); // enable mfdec
                if(pMFDECInfo->u8MFDEC_ID >= HAL_MFDEC_MODULE_CNT)
                {
                    MVOP_PRINTF("[MVOP][ERR] not support mfdec id: %d\n",pMFDECInfo->u8MFDEC_ID);
                }
                else
                {
                    HAL_WriteRegBit(VOP_REG_MFDEC_0_L, pMFDECInfo->u8MFDEC_ID, VOP_MFDEC_SEL); //select 0/1 mfdec
                }
                u64tmp = pMFDECInfo->u32BITLEN_FB_ADDR >> 3;
                HAL_WriteByteMask(VOP_REG_MFDEC_2_L, (pMFDECInfo->u32UNCOMPRESS_MODE << 4), VOP_UNCOMP_MODE);
                HAL_Write2Byte(VOP_REG_BITLEN_ADD_L, u64tmp & 0xffff);
                HAL_Write2Byte(VOP_REG_BITLEN_ADD_H, (u64tmp >> 16) & 0x3ff);
                HAL_WriteByteMask(VOP_REG_MFDEC_2_L, (pMFDECInfo->u8BITLEN_FB_MIU << 2), VOP_BIT_MIU_SEL);
                HAL_WriteByte(VOP_REG_MFDEC_BIT_PITCH, pMFDECInfo->u32BITLEN_FB_PITCH);
            }
            else
            {
                HAL_WriteRegBit(VOP_REG_MFDEC_0_L, FALSE, VOP_MFDEC_EN); // disable mfdec
            }
            #endif
            HAL_MVOP_LoadReg();
        }
        ...
        case E_MVOP_DEV_1:
        {
            if(pMFDECInfo->bMFDEC_EN)
            {
                HAL_WriteRegBit(SUB_REG(VOP_REG_MFDEC_0_L), TRUE, VOP_MFDEC_EN); // enable mfdec
                if(pMFDECInfo->u8MFDEC_ID >= HAL_MFDEC_MODULE_CNT)
                {
                    MVOP_PRINTF("[MVOP][ERR] not support mfdec id: %d\n",pMFDECInfo->u8MFDEC_ID);
                }
                ...
                {
                    HAL_WriteRegBit(SUB_REG(VOP_REG_MFDEC_0_L), pMFDECInfo->u8MFDEC_ID, VOP_MFDEC_SEL); //select 0/1 mfdec
                }
                u64tmp = pMFDECInfo->u32BITLEN_FB_ADDR >> 3;
                HAL_WriteByteMask(SUB_REG(VOP_REG_MFDEC_2_L), (pMFDECInfo->u32UNCOMPRESS_MODE << 4), VOP_UNCOMP_MODE);
                HAL_Write2Byte(SUB_REG(VOP_REG_BITLEN_ADD_L), u64tmp);
                HAL_Write2Byte(SUB_REG(VOP_REG_BITLEN_ADD_H), u64tmp >> 16);
                HAL_WriteByteMask(SUB_REG(VOP_REG_MFDEC_2_L), (pMFDECInfo->u8BITLEN_FB_MIU << 2), VOP_BIT_MIU_SEL);
                HAL_WriteByte(SUB_REG(VOP_REG_MFDEC_BIT_PITCH), pMFDECInfo->u32BITLEN_FB_PITCH);
            }
            else
            {
                HAL_WriteRegBit(SUB_REG(VOP_REG_MFDEC_0_L), FALSE, VOP_MFDEC_EN); // disable mfdec
            }
            HAL_MVOP_SubLoadReg();
        }
        break;
...
void HAL_MVOP_SetFrameInterlace(MVOP_DevID eID, HALMVOPInterlace *pIModeInfo)
{
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            HAL_MVOP_SetOutputInterlace(pIModeInfo->bOutputIMode, g_pHalMVOPCtx->u16ECONum);
            if(pIModeInfo->bOutputIMode)
            {
                HAL_WriteRegBit(VOP_SW_FDMASK, pIModeInfo->u8FDMask, VOP_FDMASK);
                //sw patch(tmp solution): for mvop hw limitation, in field mode 0x20[6] can not fire register in current vsync.
                if(g_pHalMVOPCtx->eInterlaceType == E_MVOP_INT_TB_SEP_FRAME)
                {
                    HAL_WriteRegBit(VOP_MPG_JPG_SWITCH, !(pIModeInfo->u8BotFlag), VOP_FIELD);
                }
                else
                {
                    HAL_WriteRegBit(VOP_MPG_JPG_SWITCH, pIModeInfo->u8BotFlag, VOP_FIELD);
                }
                HAL_WriteRegBit(VOP_SW_FDMASK, pIModeInfo->bXCBOB_EN, VOP_FORCE_BOB);
            }
            else
            {
                HAL_WriteRegBit(VOP_SW_FDMASK, 0, VOP_FDMASK);
                HAL_WriteRegBit(VOP_MPG_JPG_SWITCH, 0, VOP_FIELD);
                HAL_WriteRegBit(VOP_SW_FDMASK, 0, VOP_FORCE_BOB);
            }
            HAL_MVOP_LoadReg();
        }
        break;
        ...
        case E_MVOP_DEV_1:
        {
            HAL_MVOP_SubSetOutputInterlace(pIModeInfo->bOutputIMode, g_pHalMVOPCtx->u16ECONum);
            if(pIModeInfo->bOutputIMode)
            {
                HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), pIModeInfo->u8FDMask, VOP_FDMASK);
                //sw patch(tmp solution):  for mvop hw limitation, in field mode 0x20[6] can not fire register in current vsync.
                if(g_pHalMVOPCtx->eInterlaceType == E_MVOP_INT_TB_SEP_FRAME)
                {
                    HAL_WriteRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), !pIModeInfo->u8BotFlag, VOP_FIELD);
                }
                else
                {
                    HAL_WriteRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), pIModeInfo->u8BotFlag, VOP_FIELD);
                }
                HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), pIModeInfo->bXCBOB_EN, VOP_FORCE_BOB);
            }
            else
            {
                HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), 0, VOP_FDMASK);
                HAL_WriteRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), 0, VOP_FIELD);
                HAL_WriteRegBit(SUB_REG(VOP_SW_FDMASK), 0, VOP_FORCE_BOB);
            }
            HAL_MVOP_SubLoadReg();
        }
        break;
...
void HAL_MVOP_SetDISPSize(MVOP_DevID eID, HALMVOPDISPSize *pDispSize)
{
    MS_U16 u16strip;
    MS_U16 u16strip_lsb;
    MS_U16 regval;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            HAL_Write2Byte(VOP_JPG_HSIZE, pDispSize->u16Width);
            HAL_Write2Byte(VOP_JPG_VSIZE, pDispSize->u16Height);
            u16strip = pDispSize->u16Pitch[0];
            u16strip_lsb = pDispSize->u16Pitch[1];

            if ( g_pHalMVOPCtx->bDramRdContd == 0 )
            {
                u16strip = ((u16strip + 31) >> 5) * 32; //need align for monaco
                u16strip = u16strip >> 3;
                u16strip_lsb = (u16strip_lsb+127) >> 7;
                u16strip_lsb *= 4;
            }
            else
            {
            ...
            regval = u16strip;
            HAL_WriteByte(VOP_DC_STRIP, regval);
            HAL_Write2Byte(VOP_DC_STRIP_LSB, u16strip_lsb & 0x3ff);
            HAL_WriteByte(VOP_DS_IDX, pDispSize->U8DSIndex);
            HAL_MVOP_SetHorDup(pDispSize->bHDup);
            HAL_MVOP_LoadReg();
            HAL_MVOP_SetVerDup(pDispSize->bVDup);
            HAL_MVOP_LoadReg();
        }
        break;
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            HAL_Write2Byte(SUB_REG(VOP_JPG_HSIZE), pDispSize->u16Width);
            HAL_Write2Byte(SUB_REG(VOP_JPG_VSIZE), pDispSize->u16Height);
            u16strip = pDispSize->u16Pitch[0];
            u16strip_lsb = pDispSize->u16Pitch[1];

            if ( g_pHalMVOPCtx->bSubDramRdContd == 0 )
            {
                u16strip = ((u16strip + 31) >> 5) * 32; //need align for monaco
                u16strip = u16strip >> 3;
                u16strip_lsb = (u16strip_lsb+127) >> 7;
            ...
            HAL_Write2Byte(SUB_REG(VOP_DC_STRIP_LSB), u16strip_lsb & 0x3ff);
            HAL_WriteByte(SUB_REG(VOP_DS_IDX), pDispSize->U8DSIndex);
            HAL_MVOP_SubSetHorDup(pDispSize->bHDup);
            HAL_MVOP_SubLoadReg();
            HAL_MVOP_SubSetVerDup(pDispSize->bVDup);
            HAL_MVOP_SubLoadReg();
        }
        break;
#endif
...
void HAL_MVOP_SetDISPDVSize(MVOP_DevID eID, HALMVOPDISPDualDVSize *pDispSize)
{
    MS_U16 u16strip;
    MS_U16 u16strip_lsb;
    MS_U16 regval;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            HAL_Write2Byte(SUB_REG(VOP_JPG_HSIZE), pDispSize->u16Width_EL);
            HAL_Write2Byte(SUB_REG(VOP_JPG_VSIZE), pDispSize->u16Height_EL);
            u16strip = pDispSize->u16Pitch_EL[0];
            u16strip_lsb = pDispSize->u16Pitch_EL[1];
        ...
        case E_MVOP_DEV_1:
        {
            HAL_Write2Byte(VOP_JPG_HSIZE, pDispSize->u16Width_EL);
            HAL_Write2Byte(VOP_JPG_VSIZE, pDispSize->u16Height_EL);
            u16strip = pDispSize->u16Pitch_EL[0];
            u16strip_lsb = pDispSize->u16Pitch_EL[1];
...
void HAL_MVOP_SetStreamInfo(MVOP_DevID eID, HALMVOStreamINFO *pStreamInfo)
{
    MS_U8 regval;
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            HAL_MVOP_SetTileFormat(pStreamInfo->eTileFormat);
            regval = HAL_ReadByte(VOP_MPG_JPG_SWITCH);
            if (pStreamInfo->bIs422Mode)
                regval |= 0x90;
            else
                regval &= ~0x90;

            if (pStreamInfo->bIsDRAMCont)
                regval |= 0x20;
            else
                regval &= ~0x20;

            HAL_WriteByte(VOP_MPG_JPG_SWITCH, regval);
            g_pHalMVOPCtx->bIs422 = pStreamInfo->bIs422Mode;
            g_pHalMVOPCtx->bDramRdContd = pStreamInfo->bIsDRAMCont;
            if(pStreamInfo->bDDR4_REMAP)
                HAL_WriteByteMask(VOP_REG_DDR4_REMAP, 0x60, VOP_MSB_REMAP|VOP_LSB_REMAP);
            HAL_MVOP_LoadReg();
        }
        break;
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            HAL_MVOP_SubSetTileFormat(pStreamInfo->eTileFormat);
            ...
            g_pHalMVOPCtx->bSubIs422 = pStreamInfo->bIs422Mode;
            g_pHalMVOPCtx->bSubDramRdContd = pStreamInfo->bIsDRAMCont;
            if(pStreamInfo->bDDR4_REMAP)
                HAL_WriteByteMask(SUB_REG(VOP_REG_DDR4_REMAP), 0x60, VOP_MSB_REMAP|VOP_LSB_REMAP);
            HAL_MVOP_SubLoadReg();
        }
        break;
#endif
...
void HAL_MVOP_EnableTCHSetting(MVOP_DevID eID, MS_BOOL bEnable)
{
    UNUSED(eID);
    UNUSED(bEnable);
    return;
...
MS_BOOL HAL_MVOP_GetIsBotField(MVOP_DevID eID)
{
    MS_BOOL bRet = FALSE;
    switch(eID)
    {
        case E_MVOP_DEV_0:
            bRet = (HAL_ReadRegBit(VOP_DEBUG_2C_H, VOP_DC2MVD_FLD_FLAG) == VOP_DC2MVD_FLD_FLAG);
            MVOP_PRINTF("HAL_MVOP_GetIsBotField = 0x%x\n",(unsigned int)HAL_ReadRegBit(VOP_DEBUG_2C_H, VOP_DC2MVD_FLD_FLAG));
            break;
        case E_MVOP_DEV_1:
#if MVOP_SUPPORT_SUB
            bRet = (HAL_ReadRegBit(SUB_REG(VOP_DEBUG_2C_H), VOP_DC2MVD_FLD_FLAG) == VOP_DC2MVD_FLD_FLAG);
#endif
            break;
        default:
            bRet = E_MVOP_HS_INVALID_PARAM;
...
void HAL_MVOP_SetForcePMode(MVOP_DevID eID, MS_BOOL bEnable)
{
    UNUSED(eID);
    UNUSED(bEnable);
}
...
void HAL_MVOP_SubForceWriteReg(void)
{
    HAL_WriteRegBit(SUB_REG(VOP_REG_WR), 1, BIT4);
    HAL_WriteRegBit(SUB_REG(VOP_REG_WR), 0, BIT4);
}
...
void HAL_MVOP_SubRst(void)
{
    ...
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), 0, BIT4);
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), 1, BIT4);

    g_pHalMVOPCtx->bIsSubEnable = 1;
}
...
void HAL_MVOP_SubSetOutputInterlace(MS_BOOL bEnable, MS_U16 u16ECOVersion)
{
    MS_U8 regval;
    ...
    if ( bEnable )
    {
        ...
        if(u16ECOVersion == 0)
        {
            HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x40, CKG_DC1_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
        }
    }
    else
    {
...
MS_BOOL HAL_MVOP_SubSetTileFormat(MVOP_TileFormat eTileFmt)
{
    ...
    else if (eTileFmt == E_MVOP_TILE_32x32)
    {
        HAL_WriteRegBit(SUB_REG(VOP_REG_WR), 0, BIT1);
        HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 0, EVD_ENABLE);
        HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 1, VOP_TILE_32x32);
        return TRUE;
    }
    else
    {
...
void HAL_MVOP_SubSetBlackBG(void)
{
    ...
    HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), 0x02, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), 0x00, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), regval, BIT2 | BIT1 | BIT0);
}
...
void HAL_MVOP_SubSetFDMaskFromMVD(MS_BOOL bEnable)
{
    HAL_WriteRegBit(SUB_REG(VOP_REG_FD_MASK_SEL), bEnable, BIT5); //FD MASK from MVD
}
...
void HAL_MVOP_SubSetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        g_pHalMVOPCtx->bSubDramRdContd = pparam->bDramRdContd;
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    //if(((ptiming->u16V_TotalCount > 2160) || (ptiming->u16H_TotalCount > 3840)) && (ptiming->u8Framerate >= 30))
    if((!g_pHalMVOPCtx->bSubIsXcTrig) && (g_pHalMVOPCtx->bSubIsHS))
    {
    ...
    HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), 0x00, BIT2 | BIT1 | BIT0);
    HAL_WriteByteMask(SUB_REG(VOP_TST_IMG), regval, BIT2 | BIT1 | BIT0);
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), ptiming->bHDuplicate, VOP_HDUP);// H pixel duplicate
...
void HAL_MVOP_SubSetDCSRAMClk(MS_U8 clkNum, MS_BOOL bEnable)
{
    ...
    if (clkNum==0)
    {
        if(bEnable)
        {
            if(g_pHalMVOPCtx->u16ECONum == 0)
            {
                HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x4, CKG_DC1_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
            }
            else
            {
                HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC1_SRAM);
                HAL_WriteByteMask(SUB_REG(VOP_REG_DUMMY_5D), 0x0, VOP_SRAM_CLK_AUTO);
                HAL_MVOP_SubLoadReg();
            }
        }
        else
...
void HAL_MVOP_SubSetSynClk(MVOP_Timing *ptiming)
{
    ...
    if(g_pHalMVOPCtx->u32SubMVOPFixClk != 0)
    {
        ...
        if(g_pHalMVOPCtx->bSub3DLRMode)//4k1k mvc
        {
            //burst and priority
            HAL_WriteByte(SUB_REG(VOP_DMA1), 0x26);
            HAL_WriteByte(SUB_REG(VOP_BURST_CTRL0), 0x05);
            HAL_WriteByte(SUB_REG(VOP_BURST_CTRL1), 0x00);
        }
    }
...
/// Set MVOP for H264  Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetH264HardwireMode(MS_U16 u16ECOVersion)
{
    // Hardwire mode
    ...
    if(u16ECOVersion == 0)
    {
        HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC1_SRAM);//Dc0 enable & dc0_mfdec_en =0  57[3:0]= 0x4, others  57[3:0]=0x0
    }
...
/// Set MVOP for RM  Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetRMHardwireMode(MS_U16 u16ECOVersion)
{
...
/// Set MVOP for JPEG Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetJpegHardwireMode(MS_U16 u16ECOVersion)
{
    MS_U8 regval = 0x00;
...
/// Set MVOP for EVD TCH Vision Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetEVDHardwireMode_TCH(void)
{
    // Hardwire mode
    HAL_WriteByte(SUB_REG(VOP_MPG_JPG_SWITCH), 0x00);
...
/// Set MVOP for VP9 Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetVP9HardwireMode(MS_U16 u16ECOVersion)
{
...
/// Set MVOP for AVS2 Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetAVS2HardwireMode()
{
    MVOP_PRINTF("%s not support AVS2, please check MDrv_MVOP_SetInputCfg\n", __FUNCTION__);
}
...
MS_BOOL HAL_MVOP_SubSetEVDYUVBaseAdd(MVOP_EVDBaseAddInput *stEVDBaseAddInfo)
{
    ...
    // Set MSB YUV Address
    //----------------------------------------------------
    g_pHalMVOPCtx->bSubIsEnLSB = stEVDBaseAddInfo->bEnLSB;
    if(stEVDBaseAddInfo->bEnLSB)
    {
        //Enable LSB
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 1, VOP_EVD_10B_Y_EN);
        ...
        HAL_WriteByte(SUB_REG(VOP_LSB_YSTR0_H), (u64tmp >> 16) & 0xff);
        HAL_WriteByte(SUB_REG(VOP_LSB_YSTR0_H+1), (u64tmp >> 24) & VOP_YUV_STR_HIBITS);
    ...
    else
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_Y_EN);
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_UV_EN);
        HAL_WriteRegBit(SUB_REG(VOP_REG_MASK), 1, VOP_LSB_REQ_MASK);
    }
...
HALMVOPFREQUENCY HAL_MVOP_SubGetCurrentClk(void)
{
    MS_U8 u8DC0Clk;
    HALMVOPFREQUENCY mvopFreq;

    u8DC0Clk = HAL_ReadByte(REG_CKG_SUB_DC0)&CKG_SUB_DC0_MASK;

    switch(u8DC0Clk)
    {
        case CKG_SUB_DC0_SYNCHRONOUS:
            mvopFreq = HALMVOP_SYNCMODE;
            break;
        case CKG_SUB_DC0_FREERUN:
            mvopFreq = HALMVOP_FREERUNMODE;
            break;
        case CKG_SUB_DC0_144MHZ:
            mvopFreq = HALMVOP_144MHZ;
            break;
        case CKG_SUB_DC0_160MHZ:
            mvopFreq = HALMVOP_160MHZ;
            break;
        case CKG_SUB_DC0_320MHZ:
            mvopFreq = HALMVOP_320MHZ;
            break;
        default:
            mvopFreq = 0;
...
MS_BOOL HAL_MVOP_SubSetVerDup(MS_BOOL bEnable)
{
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), bEnable, BIT3);// V line duplicate
    return TRUE;
}

MS_BOOL HAL_MVOP_SubSetHorDup(MS_BOOL bEnable)
{
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), bEnable, BIT2);// V line duplicate
    return TRUE;
}
