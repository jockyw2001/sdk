...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
SC_MdbgInfo                         _scMdbInfo[SC_DEV_NUM];
#endif
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
void Mdb_SC_UpdateToKernel(MS_U8 u8SCID);
#endif
...
static MS_BOOL _SC_CheckRC(const MS_U8 u8RCType, const MS_U8* pu8Data, const MS_U16 u16Length, const MS_U16 u16RCValue, MS_BOOL* const pbRCCheckPass)
{
    if (pu8Data == NULL)
    {
        return FALSE;
...
static SC_Result _SC_T1SendRecvBlock( MS_U8 u8SCID,MS_U8 *pu8Block, MS_U16 *pu16BlockLen, MS_U8 *pu8RspBlock, MS_U16 *pu16RspBlockLen)
{
    ...
    MS_U16 PrologueSize = SC_T1_PROLOGUE_SIZE;
    ...
    if (PrologueSize >= SC_T1_PROLOGUE_SIZE) // Got length byte
    {
        if (pu8RspBlock[2] != 0)
        {
            if (_sc_T1State[u8SCID].u8RCType == SC_RC_TYPE_CRC)
                *pu16RspBlockLen = pu8RspBlock[2] + 2;
            else
...
void Mdb_SC_UpdateToKernel(MS_U8 u8SCID)
{
    memset(&_scMdbInfo[0], 0x0, sizeof(SC_MdbgInfo));
#if (SC_DEV_NUM > 1) // no more than 2
    memset(&_scMdbInfo[1], 0x0, sizeof(SC_MdbgInfo));
#endif

    _scMdbInfo[u8SCID].bOpened = _scInfo[u8SCID].bOpened;
    _scMdbInfo[u8SCID].bCardIn = _scInfo[u8SCID].bCardIn;
    _scMdbInfo[u8SCID].u8Fi = _scInfo[u8SCID].u8Fi;
    _scMdbInfo[u8SCID].u8Di = _scInfo[u8SCID].u8Di;
    _scMdbInfo[u8SCID].u8Protocol = _scInfo[u8SCID].u8Protocol;
    _scMdbInfo[u8SCID].eVccCtrl = _scInfo[u8SCID].eVccCtrl;
    _scMdbInfo[u8SCID].eCardClk = _scInfo[u8SCID].eCardClk;
    _scMdbInfo[u8SCID].u16AtrLen = _scInfo[u8SCID].u16AtrLen;
    memcpy(_scMdbInfo[u8SCID].pu8Atr, _scInfo[u8SCID].pu8Atr, _scInfo[u8SCID].u16AtrLen);
    _scMdbInfo[u8SCID].u8PadConfig = (HW_READ(REG_TOP_SM0) & REG_TOP_SM0_PAD_MASK) >> 8;

    if (0 != ioctl(_scInfo[u8SCID].s32DevFd, MDRV_SC_MDB_WRITE_INFO, &_scMdbInfo[u8SCID]))
    {
        SC_DBG(E_SC_DBGLV_ERR_ONLY, "[%s][%d] ioctl: MDRV_SC_MDB_WRITE_INFO failed\n",
                __FUNCTION__, __LINE__);
    }
}
#endif
    ...
    *u16ReadDataLen = 0;
    ...
    while (u16SentIdx <  *u16SendDataLen-u8EDCByteNum)
    {
        ...
        if (_sc_T1State[u8SCID].u8RCType == SC_RC_TYPE_CRC)
        {
            ...
            u16RCValue = ( u8RspBlock[(u8RspRBlkLen - 2)] << 8 ) + u8RspBlock[(u8RspRBlkLen - 1)];
        }
        else
        {
        ...
        if( ret_Result == E_SC_OK )
        {
            if( _SC_CheckRC( _sc_T1State[u8SCID].u8RCType, u8RspBlock, (u8RspRBlkLen - u8EDCByteNum), u16RCValue, &bRCCheckPass ) == TRUE )
            {
                if( bRCCheckPass == FALSE )
                {
                    SC_DBG(E_SC_DBGLV_ERR_ONLY, "%s _SC_CheckRC RC check fail, read data to clean read buffer", __FUNCTION__);
                    ret_Result = E_SC_DATAERROR;
                    // Clean read buffer
                    u8RspRBlkLen = *u16ReadDataLen;
                    _SC_Recv(u8SCID, u8RspBlock, &u8RspRBlkLen, SC_T1_RECV_TIMEOUT);
                }
            }
            else
            {
                ret_Result = E_SC_FAIL;
                SC_DBG(E_SC_DBGLV_ERR_ONLY, "%s _SC_CheckRC() error", __FUNCTION__);
            }
        }
