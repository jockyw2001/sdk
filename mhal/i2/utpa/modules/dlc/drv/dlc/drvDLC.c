...
#ifdef MSOS_TYPE_LINUX_KERNEL
extern MS_BOOL KDrv_DLC_SetInitInfo(ST_KDRV_XC_DLC_INIT_INFO *pstDlcInitInfo);
#endif
...
#if defined(UFO_XC_HDR_VERSION) && (UFO_XC_HDR_VERSION == 2)
#ifdef CHIP_I2
    printk(KERN_EMERG "%s %d :: I2 Not Support, KDrv_DLC_SetInitInfo\n", __FUNCTION__, __LINE__);
#else
...
#elif defined(MSOS_TYPE_LINUX_KERNEL)
#ifdef CHIP_I2
    printk(KERN_EMERG "%s %d, I2 Not Support, KDrv_DLC_SetTMOInfo\n", __FUNCTION__, __LINE__);
#else
...
#ifdef MSOS_TYPE_LINUX_KERNEL
extern  MS_BOOL KMDrv_XC_SetVRAddress(ST_KDRV_XC_DLC_SET_VR_ADDRESS stDlcVRInfo);
#endif

MS_BOOL MDrv_DLC_SetVRAddress(MS_U64 u64VRAddress, MS_U32 u32VRSize)
{
#if defined(UFO_XC_HDR_VERSION) && (UFO_XC_HDR_VERSION == 2)
    ST_KDRV_XC_DLC_SET_VR_ADDRESS stDlcVRInfo;
    stDlcVRInfo.u64VRAddress = u64VRAddress;
    stDlcVRInfo.u32VRSize = u32VRSize;
#ifdef MSOS_TYPE_LINUX
    ...
    if (ioctl(s32FdDlcBle, MDRV_XC_IOC_DLCSETVRADDRESS, &stDlcVRInfo))
    {
        DLC_PRINT("MDrv_DLC_SetVRAddress send MDRV_XC_IOC_DLCSETVRADDRESS fail!!!!\n");
        return FALSE;
    }
#elif defined(MSOS_TYPE_LINUX_KERNEL)
#ifdef CHIP_I2
    printk(KERN_EMERG "%s %d: I2 Not Support, KMDrv_XC_SetVRAddress\n", __FUNCTION__, __LINE__);
#else
    KMDrv_XC_SetVRAddress(stDlcVRInfo);
#endif
#endif
    return TRUE;
#else
