...
// Compiler option
#define DLC_LIB_VERSION             0x3300 //v32.61 // fix bugs of u8_L_H_U & u8_L_H_D
...
char* dlc_c51version = C51_VERSION(__C51__);
#endif
...
XDATA BYTE g_ucTable[16];     // Final target curve (8 bit)
...
static WORD g_wPIAlpha=0;
static WORD g_wPrePIAlpha=0;
...
void msDlcInitWithCurve( WORD wHStart, WORD wHEnd, WORD wVStart, WORD wVEnd )
{
    ...
    msWriteByte(REG_ADDR_DLC_HANDSHAKE, msReadByte(REG_ADDR_DLC_HANDSHAKE)|_BIT5|_BIT4 );
...
void msDlc_SetCaptureRange(MS_U16 wHStart,MS_U16 wHEnd, MS_U16 wVStart, MS_U16 wVEnd)
{
    ...
    if (REG_ADDR_HISTOGRAM_RANGE_M_HST != REG_NULL)
    {
        ...
        // Virtical size need to change by display
        // Set histogram range - V
        // Main Window
        msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_VST, wVStart/8);
        msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_VEN, wVEnd/8);
...
void msDlc_SetCaptureRangeSub(MS_U16 wHStart,MS_U16 wHEnd, MS_U16 wVStart, MS_U16 wVEnd)
{
    msDlc_FunctionEnter();

    if (REG_ADDR_HISTOGRAM_RANGE_S_HST != REG_NULL)
    {
        // Set histogram range - H
        // Sub Window
        msWriteByte(REG_ADDR_HISTOGRAM_RANGE_S_HST, wHStart/8);
        ...
        // Enable SUB_WINDOW histogram
        msWriteByte(REG_ADDR_HISTOGRAM_RANGE_ENABLE, _BIT3|msReadByte(REG_ADDR_HISTOGRAM_RANGE_ENABLE) );
...
MS_BOOL msIsBlackVideoEnable( MS_BOOL bWindow )
{
    ...
    if( MAIN_WINDOW == bWindow)
    {
        if(msReadByte(REG_ADDR_VOP_SCREEN_CONTROL) & BIT(1))
        {
            bReturn = TRUE;
        }
        else
    ...
    {
        if(msReadByte(REG_ADDR_VOP_SCREEN_CONTROL) & BIT(5))
        {
            bReturn = TRUE;
        }
        else
...
WORD msGetAverageLuminous_x4(void) // = external msGetAverageLuminousValue()
{
    ...
    // Normalize
    if (g_wLumiTotalCount!=0)
        uwTmpAvr = (1024 * (DWORD)g_wLumiAverageTemp + (g_wLumiTotalCount/2)) / g_wLumiTotalCount;
    else
        uwTmpAvr = 0;
...
//
//////////////////////////////////////////////////////////////
BYTE msGetAverageLuminousSub(void)
{
...
void msDlcHandler(BOOL bWindow)
{
    ...
    if(g_DlcParameters.ucDlcAlgorithmMode == XC_DLC_ALGORITHM_OLD)
    {
        ...
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SD2 / (Lstep*128))+1;
            if (ucTmp > LD2)
                ucTmp = LD2;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SD1 / (Hstep*128))+1;
            if (ucTmp > LD1)
                ucTmp = LD1;
            g_ucTable[6] = 0x67 + ucTmp;
        }
        ...
        if (uwArrayComp < g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SC2 / (Lstep*128))+1;
            if (ucTmp > LC2)
                ucTmp = LC2;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SC1 / (Hstep*128))+1;
            if (ucTmp > LC1)
                ucTmp = LC1;
            g_ucTable[4] = g_ucTable[4] + ucTmp;
        }
        ...
        if (uwArrayComp < g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SB2 / (Lstep*128))+1;
            if (ucTmp > LB2)
                ucTmp = LB2;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SB1 / (Hstep*128))+1;
            if (ucTmp > LB1)
                ucTmp = LB1;
            g_ucTable[2] = g_ucTable[2] + ucTmp;
        }
        ...
        if (uwArrayComp < g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SA2 / (Lstep*128))+1;
            if (ucTmp > LA2)
                ucTmp = LA2;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SA1 / (Hstep*128))+1;
            if (ucTmp > LA1)
                ucTmp = LA1;
            g_ucTable[1] = g_ucTable[1] + ucTmp;
        }
        ...
        if (uwArrayComp < g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SE1 / (Lstep*128))+1;
            if (ucTmp > LE1)
                ucTmp = LE1;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SE2 / (Hstep*128))+1;
            if (ucTmp > LE2)
                ucTmp = LE2;
            g_ucTable[9] = 0x97 - ucTmp;
        }
        ...
        if (uwArrayComp< g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SF1 / (Lstep*128))+1;
            if (ucTmp > LF1)
                ucTmp = LF1;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SF2 / (Hstep*128))+1;
            if (ucTmp > LF2)
                ucTmp = LF2;
            g_ucTable[11] = g_ucTable[11] - ucTmp;
        }
        ...
        if (uwArrayComp< g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SG1 / (Lstep*128))+1;
            if (ucTmp > LG1)
                ucTmp = LG1;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SG2 / (Hstep*128))+1;
            if (ucTmp > LG2)
                ucTmp = LG2;
            g_ucTable[13] = g_ucTable[13] - ucTmp;
        }
        ...
        if (uwArrayComp < g_dwFactory_7)
        {
            ucTmp = ((g_dwFactory_7 - uwArrayComp) * SH1 / (Lstep*128))+1;
            if (ucTmp > LH1)
            ucTmp = LH1;
        ...
        {
            ucTmp = ((uwArrayComp - g_dwFactory_9) * SH2 / (Hstep*128))+1;
            if (ucTmp > LH2)
            ucTmp = LH2;
            g_ucTable[14] =g_ucTable[14] - ucTmp;
        }
...
void msDlc_CGC_Handler(void)
{
    ...
    u16Ytemp = (DWORD) (Y01_weight*(g_ucTable[0]+g_ucTable[1])+
             Y23_weight*(g_ucTable[2]+g_ucTable[3]))/
            (Y01_weight+Y23_weight);

    u16Ytemp =  u16Ytemp/2+(DWORD)(Y45_weight*(g_ucTable[4]+g_ucTable[5])+
    ...
    g_u16Ydark_pre = (DWORD)(31*g_u16Ydark_pre+u16Ytemp)/32;
