...
MS_BOOL MApi_XC_DLC_SetTMOInfo_U2(void * pInstance, ST_XC_DLC_TMO_INFO *pstDLCTmoInfo)
{
    ...
    ST_DLC_TMO_INFO* stDLCTMOInfo = (ST_DLC_TMO_INFO*) malloc(sizeof(ST_DLC_TMO_INFO));
    if(stDLCTMOInfo == NULL)
    {
        printf("stDLCTMOInfo is not malloc!!!!\n");
        return FALSE;
    }
    memset(stDLCTMOInfo, 0, sizeof(ST_DLC_TMO_INFO));
    if(pstDLCTmoInfo == NULL)
    {
        printf("pstDLCTmoInfo is not NULL!!!!\n");
        free(stDLCTMOInfo);
        return FALSE;
    }
    VERSION_COMPATIBLE_CHECK(pstDLCTmoInfo, sizeof(ST_DLC_TMO_INFO), u16CopiedLength, u32TMOInfo_Version, u16TMOInfo_Length, DLC_TMO_INFO_VERSION);
...
{
    MS_BOOL bReturn = FALSE;
    switch(pstDLC_SetMemoryAddress->eType)
    {
        case E_DLC_MEMORY_TMO_VR_REQUEST:
             bReturn = MDrv_DLC_SetVRAddress(pstDLC_SetMemoryAddress->phyAddress ,pstDLC_SetMemoryAddress->u32Size );
            break;
        default:
            return FALSE;
    }
...
MS_BOOL SYMBOL_WEAK MApi_XC_DLC_SetMemoryAddress(ST_XC_DLC_SET_MEMORY_INFO *pstDLC_SetMemoryAddress )
{
    if (pu32DLCInst == NULL)
    {
        if(UtopiaOpen(MODULE_DLC, &pu32DLCInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
    ...
    stDLC_SetMemoryAddress DLCArgs;
    DLCArgs.pstDLC_Set_Memory_Address = pstDLC_SetMemoryAddress;
    DLCArgs.bReturn = FALSE ;

    // wait until Jesse.Huang set the definition of return value, I will exchange this with another return value
    // instead of 0
    if(UtopiaIoctl(pu32DLCInst, E_DLC_CMD_SET_MEMORY_ADDRESS, (void*)&DLCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain DLC engine fail\n");
        return FALSE;
    }
