...
#include "drvSEAL_priv.h"
//-------------------------------------------------------------------------------------------------
// Local Defines
//-------------------------------------------------------------------------------------------------
#define TAG_SEAL "SEAL"
#define SEAL_DEBUGLOG    (0)
...
#endif
{
    ...
    {
        Seal_PortectInfo sPInfo_miu;
      
        MDrv_Seal_GetHittedInfo(E_SEAL_MIU_DEV0, &sPInfo_miu);
        MDrv_Seal_GetHittedInfo(E_SEAL_MIU_DEV1, &sPInfo_miu);
        MDrv_Seal_GetHittedInfo(E_SEAL_MIU_DEV2, &sPInfo_miu);
        MDrv_Seal_GetHittedInfo(E_SEAL_MIU_DEV3, &sPInfo_miu);
    }
#endif
...
{
    return MDrv_Seal_SetPowerState((EN_POWER_MODE) u32PowerState);
}

// this func will be call to init by utopia20 framework
void SEALRegisterToUtopia(FUtopiaOpen ModuleType)
{
    void* pModuleSEAL = NULL;
    UtopiaModuleCreate(MODULE_SEAL, 0, &pModuleSEAL);
    UtopiaModuleRegister(pModuleSEAL);

    UtopiaModuleSetupSTRFunctionPtr(pModuleSEAL,(FUtopiaSTR)SEAL_STR);
}
