...
MS_U32 VBIMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    extern VBI_StoreInfo _pStoreInfo;
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            paraCmdLine = (MDBCMD_CMDLINE_PARAMETER *)pArgs;
            MdbPrint(paraCmdLine->u64ReqHdl,"---------MStar vbi state---------\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"vbi only support get info function now\n");
            ...
            else
            {
                switch(_vbiStatus.eInitType)
                {
                    case E_VBI_TELETEXT:
                        ...
                        MdbPrint(paraGetInfo->u64ReqHdl,"buf phyAddr : %lx\n", _pStoreInfo._TTXBufAddr);
                        break;
                    case E_VBI_CC:
                    ...
                    case E_VBI_WSS:
                        ...
                        MdbPrint(paraGetInfo->u64ReqHdl,"wss data : %x\n", MDrv_VBI_GetWSS_Data());
                        break;
                    default:
                        break;
                }
...
MS_U32 VBIOpen(void** ppInstance, const void* const pAttribute)
{
    ...
    pVbiPri->fpVBIGetRawVPSData                   = (IOCTL_VBI_GETRAWVPS_DATA                  )_MDrv_VBI_GetRawVPS_Data;
