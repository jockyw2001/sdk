...
MS_U32 AVDMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            Drv_AVD_DBG_EchoCmd(paraCmdLine->u64ReqHdl, paraCmdLine->u32CmdSize, paraCmdLine->pcCmdLine);
...
void AVDRegisterToUtopia(void)
{
    ...
    //Utopia process debug support
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
    UtopiaModuleRegisterMdbNode("avd", (FUtopiaMdbIoctl)AVDMdbIoctl);
#endif
#if defined (MSTAR_STR)
   // Utopia2K STR
#if defined(MSOS_TYPE_LINUX_KERNEL)
    ...
    */
    ...
    if (eSource == E_INPUT_SOURCE_ATV)
    {
        if (pResource->g_VD_InitData.bRFGainSel==VD_USE_FIX_GAIN)
        {
            ...
            {
                #if defined (STELLAR_FLAG)
                HAL_AVD_AFEC_AGCSetCoarseGain(VD_AGC_COARSE_GAIN_X_13);
                #elif defined (MSTAR_FLAG)
                HAL_AVD_AFEC_AGCSetCoarseGain(VD_AGC_COARSE_GAIN);
                #endif
                HAL_AVD_AFEC_AGCSetFineGain(pResource->g_VD_InitData.u8RFGain);
...
MS_BOOL Drv_AVD_DBG_EchoCmd(MS_U64* pu64ReqHdl, MS_U32 u32CmdSize, char* pcmd)
{
    ...
    {
        u16status = ((HAL_AVD_GetReg(BK_AFEC_CD)<<8)|HAL_AVD_GetReg(BK_AFEC_CC));
        u16status1 = ((HAL_AVD_GetReg(BK_AFEC_D1)<<8)|HAL_AVD_GetReg(BK_AFEC_D0));
        u8tmp = Drv_AVD_GetNoiseMag();
        u8tmp1 = HAL_AVD_GetReg(BK_AFEC_D6);
        u8tmp2 =  HAL_AVD_GetReg(BK_AFEC_CE);
        MdbPrint(pu64ReqHdl, " --------- VD Standard Report ---------\n");
        MdbPrint(pu64ReqHdl, " AVD Lock Status:     %x \n",u8tmp1);
        MdbPrint(pu64ReqHdl, " Status Ready:        %x \n",(u16status & VD_STATUS_RDY));
        MdbPrint(pu64ReqHdl, " VD Status:         %x \n",u16status);
        MdbPrint(pu64ReqHdl, " DSP Version:         %x \n",u16status1);
        ...
        MdbPrint(pu64ReqHdl, " V Sync Type:       %x \n",(u16status & VD_VSYNC_50HZ)>>12);
        MdbPrint(pu64ReqHdl, " FSC Type:        %x \n",(u16status & VD_FSC_TYPE));
        MdbPrint(pu64ReqHdl, " Noise Mag:       %x \n",u8tmp);
        MdbPrint(pu64ReqHdl, " Force Standard::     %x \n",(u8tmp2 & BIT(0)));
        return TRUE;
    }
    else if(strncmp(pcmd, "CVBS_Signal_Information", 23) == 0)
    {
        u8tmp = HAL_AVD_GetReg(BK_AFEC_DA);
        u16status = Drv_AVD_GetVTotal();
        u16status1 = HAL_AVD_AFEC_GetHTotal();
        MdbPrint(pu64ReqHdl, "--------- CVBS Signal Information-------\n");
        MdbPrint(pu64ReqHdl, " H total:             %d \n",u16status1);
        MdbPrint(pu64ReqHdl, " V total:             %d \n",u16status);
        MdbPrint(pu64ReqHdl, " H sync Mag:        %d \n",u8tmp);
        return TRUE;
    }
    else if(strncmp(pcmd, "3D_Comb_Status", 14) == 0)
    {
        u8tmp = HAL_AVD_COMB_Get3dCombTimingCheck();
        u8tmp1 = (HAL_AVD_GetReg(BK_COMB_E0)&BIT(1));
        MdbPrint(pu64ReqHdl, "-----------3D Comb Status ------------\n");
        MdbPrint(pu64ReqHdl, " 3dCombStatus:        %x \n",u8tmp);
        MdbPrint(pu64ReqHdl, " 3dCombBandwidth:     %x \n",u8tmp1);
        return TRUE;
    }
    else if(strncmp(pcmd, "VDReset", 7) == 0)
    {
        ...
        switch(itmp)
        {
             case MDBCMD_Reset_DSP_CODE:
                HAL_AVD_SetReg(BK_AFEC_CB,(HAL_AVD_GetReg(BK_AFEC_CB)|0x80));
                break;
             case MDBCMD_Reset_AFEC_HW:
                HAL_AVD_SetReg(BK_AFEC_14,(HAL_AVD_GetReg(BK_AFEC_14)|0x80));
                MsOS_DelayTask(1);
                HAL_AVD_SetReg(BK_AFEC_14,(HAL_AVD_GetReg(BK_AFEC_14)&0x7F));
                break;
             case MDBCMD_Reset_COMB:
                HAL_AVD_SetReg(BK_COMB_13,(HAL_AVD_GetReg(BK_COMB_13)|0x40));
                MsOS_DelayTask(1);
                HAL_AVD_SetReg(BK_COMB_13,(HAL_AVD_GetReg(BK_COMB_13)&0xBF));
                break;
            default:
    ...
    else if(strncmp(pcmd, "VDTestPattern", 13) == 0)
    {
        ...
        switch(itmp)
        {
            case MDBCMD_PATTERN_CANCEL:
                HAL_AVD_SetReg(BK_AFEC_7E,(HAL_AVD_GetReg(BK_AFEC_7E)&0xFC));
                break;
            case MDBCMD_PATTERN_ROW:
                HAL_AVD_SetReg(BK_AFEC_7E,(HAL_AVD_GetReg(BK_AFEC_7E)|0x02));
                break;
            case MDBCMD_PATTERN_COLUMN:
                HAL_AVD_SetReg(BK_AFEC_7E,(HAL_AVD_GetReg(BK_AFEC_7E)|0x01));
                break;
            default:
                    MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
                return FALSE;
    ...
    {
        HAL_AVD_SetReg(BK_AFEC_66,(HAL_AVD_GetReg(BK_AFEC_66)|0xC0));
        itmp = ptr[0]-'0';
        switch(itmp)
        {
            ...
            case MDBCMD_LEVEL_44:
                    Drv_AVD_SetRegValue(BK_AFEC_6E, 0x44 );
                break;
            case MDBCMD_LEVEL_88:
                    Drv_AVD_SetRegValue(BK_AFEC_6E, 0x88 );
                break;
            case MDBCMD_LEVEL_CC:
                    Drv_AVD_SetRegValue(BK_AFEC_6E, 0xCC );
                break;
            case MDBCMD_LEVEL_FF:
                    Drv_AVD_SetRegValue(BK_AFEC_6E, 0xFF );
                break;
            default:
                    MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
                return FALSE;
    ...
    else if(strncmp(pcmd, "HsyncPLL", 8) == 0)
    {
        itmp = ptr[0]-'0';
        switch(itmp)
        {
            case MDBCMD_PLL_CANCEL:
                    HAL_AVD_SetReg(BK_AFEC_A0,(HAL_AVD_GetReg(BK_AFEC_A0) & 0x7F));
                break;
            case MDBCMD_PLL_Level1:
                    Drv_AVD_SetRegValue(BK_AFEC_A0, 0x82 );
                    Drv_AVD_SetRegValue(BK_AFEC_A1, 0x04 );
                break;
            case MDBCMD_PLL_Level2:
                    Drv_AVD_SetRegValue(BK_AFEC_A0, 0x90 );
                    Drv_AVD_SetRegValue(BK_AFEC_A1, 0x20 );
                break;
            case MDBCMD_PLL_Level3:
                    Drv_AVD_SetRegValue(BK_AFEC_A0, 0x9A );
                    Drv_AVD_SetRegValue(BK_AFEC_A1, 0x35 );
                break;
            case MDBCMD_PLL_Level4:
                    Drv_AVD_SetRegValue(BK_AFEC_A0, 0xAA );
                    Drv_AVD_SetRegValue(BK_AFEC_A1, 0x6A );
                break;
            case MDBCMD_PLL_Level5:
                    Drv_AVD_SetRegValue(BK_AFEC_A0, 0xBC );
                    Drv_AVD_SetRegValue(BK_AFEC_A1, 0x6A );
                break;
            default:
                    MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
    ...
    else if(strncmp(pcmd, "COMB ", 4) == 0)
    {
        ...
        switch(itmp)
        {
            case MDBCMD_COMB_CANCEL:
                    HAL_AVD_SetReg(BK_AFEC_7F,(HAL_AVD_GetReg(BK_AFEC_7F) | 0x04));
                break;
            case MDBCMD_COMB_ByPassComb_Filter:
                    HAL_AVD_SetReg(BK_AFEC_7F,(HAL_AVD_GetReg(BK_AFEC_7F) & 0xFB));
                break;
            default:
                    MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
                return FALSE;
        ...
        if(ptr[0] == '0' && ptr[1] == 'x')
        {
            ...
            switch(itmp)
            {
                ...
                if(itmp1 <= 0x1F && itmp1 >= 0x0)
                    {
                        Drv_AVD_SetRegValue(BK_AFEC_4C, (HAL_AVD_GetReg(BK_AFEC_4C) & 0xE0) | itmp1 );
                    }
                    break;
                case MDBCMD_Window_End:
                if(itmp1 <= 0x7F && itmp1 >= 0x0)
                    {
                        HAL_AVD_SetReg(BK_AFEC_4D,(HAL_AVD_GetReg(BK_AFEC_4D) | 0x80));
                        Drv_AVD_SetRegValue(BK_AFEC_4D, (HAL_AVD_GetReg(BK_AFEC_4D) & 0x80) | itmp1 );
                    }
                    break;
                default:
                        MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
    ...
    else if(strncmp(pcmd, "3DCombThreshold", 15) == 0)
    {
        if(ptr[0] == '0' && ptr[1] == 'x')
        {
            ...
            itmp1 = Drv_AVD_StrToHex(ptr);
            Drv_AVD_SetRegValue(BK_AFEC_17, itmp1 );
            MdbPrint(pu64ReqHdl,"[%d]DBG CMD Success!!!\n", __LINE__);
            return TRUE;
        }
    }
    else if(strncmp(pcmd, "FixGain", 7) == 0)
    {
        ...
        switch(itmp)
        {
             case MDBCMD_FixGain_CANCEL:
                    HAL_AVD_SetReg(BK_AFEC_43,(HAL_AVD_GetReg(BK_AFEC_43) & 0x9F));
                break;
             case MDBCMD_FixGain_ENABLE:
                    HAL_AVD_SetReg(BK_AFEC_43,(HAL_AVD_GetReg(BK_AFEC_43) | 0x60));
            break;
            default:
                    MdbPrint(pu64ReqHdl,"[%d]DBG CMD Error!!!\n", __LINE__);
                return FALSE;
    ...
    else if(strncmp(pcmd, "rdAFEC", 6) == 0)
    {
        if(ptr[0] == '0' && ptr[1] == 'x')
            {
                ptr = Drv_AVD_Str_Tok(pcmd, strtmp, 'x');
                itmp = Drv_AVD_StrToHex(ptr);
                itmp1 = Drv_AVD_GetRegValue(AFEC_REG_BASE+itmp);
                MdbPrint(pu64ReqHdl," Get AFEC[%x] => [%x]\n",AFEC_REG_BASE+itmp, itmp1);
                return TRUE;
            }
    }
    else if(strncmp(pcmd, "rdCOMB", 6) == 0)
    {
        if(ptr[0] == '0' && ptr[1] == 'x')
            {
                ptr = Drv_AVD_Str_Tok(pcmd, strtmp, 'x');
                itmp = Drv_AVD_StrToHex(ptr);
                itmp1 = Drv_AVD_GetRegValue(COMB_REG_BASE+itmp);
                MdbPrint(pu64ReqHdl," Get COMB[%x] => [%x]\n",COMB_REG_BASE+itmp, itmp1);
                return TRUE;
            }
    }
    else if(strncmp(pcmd, "help", 4) == 0)
    {
        ...
        MdbPrint(pu64ReqHdl,"1.  Module Infomation VD_Standard_Report\n");
        MdbPrint(pu64ReqHdl,"     [e.g.]echo VD_Standard_Report > avd\n");
        MdbPrint(pu64ReqHdl,"\n");
        MdbPrint(pu64ReqHdl,"2.  Module Infomation CVBS_Signal_Information\n");
        MdbPrint(pu64ReqHdl,"     [e.g.]echo CVBS_Signal_Information > avd\n");
        MdbPrint(pu64ReqHdl,"\n");
        MdbPrint(pu64ReqHdl,"3.  Module Infomation 3D_Comb_Status\n");
