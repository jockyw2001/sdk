...
#define MSIF_VIF_LIBVER       {'1','9'}
#define MSIF_VIF_BUILDNUM     {'0','3'}
#define MSIF_VIF_CHANGELIST   {'0','1','4','9','7','8','2','3'}
#define MSIF_VIF_DASH          {'_'}
#define MSIF_VIF_DATE          {'1','7','0','8','2','2'}    // yymmdd
...
void VIFRegisterToUtopia(void)
{
   ...
   // Utopia2K STR
    ...
    UtopiaModuleAddResourceStart(pUtopiaModule, VIF_POOL_ID_VIF);
    UtopiaResourceCreate("VIF", sizeof(VIF_RESOURCE_PRIVATE), &psResource);
    UtopiaResourceRegister(pUtopiaModule, psResource, VIF_POOL_ID_VIF);
    UtopiaModuleAddResourceEnd(pUtopiaModule, VIF_POOL_ID_VIF);
}
...
MS_U32 VIFIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    void* pResource = NULL;
    VIF_RESOURCE_PRIVATE* psVifResPri = NULL;
    if(UtopiaResourceObtain(pModule, VIF_POOL_ID_VIF , &pResource) != 0)
    {
        printf("UtopiaResourceObtainToInstant fail\n");
        return UTOPIA_STATUS_ERR_RESOURCE;
    }

    if(UtopiaResourceGetPrivate(pResource,(void*)&psVifResPri) != 0)
    {
...
void VIF_Init(VIFInitialIn * pVIF_InitData, DWORD u32InitDataLen)
{
    ...
    //From Marxvel chip start, VIF will run on DMD_MCU.
    if (strcmp(CHIP_TYPE, MARxVEL) >= 0)
        HAL_VIF_Download();
...
void VIF_SetFreqBand(FrequencyBand u8FreqBand)
{
    ...
    if (strcmp(CHIP_TYPE, MARxVEL) >= 0)
    {
        ...
        u8tmp = VIF_ReadByte(VIF_RF_RESERVED_1+2);

        u8tmp = (u8tmp & (~0x0F)) | u8FreqBand;
    
        //g_FreqBand = u8FreqBand; // 0x1121_D4
        VIF_WriteByte(VIF_RF_RESERVED_1+2, u8tmp/*u8FreqBand*/);
    }
    else
    {
        //g_FreqBand = u8FreqBand; // 0x1121_D4
    ...
    switch (paraGroup)
    {
        case VIF_PARA_USER_FILTER:			
            ...
            {		   
                ...
                if (strcmp(CHIP_TYPE, MARxVEL) >= 0)
                    HAL_VIF_UpdateFWBackend(VIF_PARA_USER_FILTER);
            }
            else
        ...
        case VIF_PARA_SET_NOTCH_A1A2:
            if ( sizeof(VIFNotchA1A2) == u32DataLen)
            {
                memcpy(&sVIFNotchA1A2, (VIFNotchA1A2 *) pVIF_Para, u32DataLen);
		             ...
		             if (strcmp(CHIP_TYPE, MARxVEL) >= 0)
                    HAL_VIF_UpdateFWBackend(VIF_PARA_SET_NOTCH_A1A2);
		             else
		             {
	      ...
	      case VIF_PARA_SET_SOS_1112:
                 ...
                 if (strcmp(CHIP_TYPE, MARxVEL) >= 0)
                     HAL_VIF_UpdateFWBackend(VIF_PARA_SET_SOS_1112);
                 else
                 {
