...
#if !defined (MSOS_TYPE_ECOS) && !defined (MSOS_TYPE_UCOS) && !defined (MSOS_TYPE_ATOMTHREADS)
int putchar(int c)
{
    char x;
...
void MDrv_UART_Debug_cat(MS_U64 Hdl)
{
    MS_U8 u8Count;
    MdbPrint(Hdl, "\n---------MStar UART Info--------- \n");

    for(u8Count = 0; u8Count < UART_MAX; u8Count++)
    {
        MdbPrint(Hdl, "UART_%d : Enable %d. \n", u8Count, _UartDisc[u8Count].enable);
        MdbPrint(Hdl, "UART_%d : Type %d. \n", u8Count, _UartDisc[u8Count].Uart_dev);
        MdbPrint(Hdl, "UART_%d : baudrate %d. \n\n", u8Count, _UartDisc[u8Count].baudrate);
    }
