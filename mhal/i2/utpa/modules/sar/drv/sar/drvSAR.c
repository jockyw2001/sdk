...
#define SAR_LEVEL_FAILURE_NUM_MAX   SAR_LEVEL_STABLE_NUM - SAR_LEVEL_STABLE_NUM_MIN
...
static MS_U8 gu8KeyValue[CFG_SAR_KPD_AVLBCHS];
static MS_U8 gu8SamplingInterval = 5; //ms
...
static MS_U8 MDrv_SAR_Kpd_GetCHKey(MS_U8 *pu8Key , MS_U8 *pu8Repeat)
{
    MS_U8 ret_val=FALSE;
    MS_U8 i,k,KEY_LV[CFG_SAR_KPD_AVLBLVLS][CFG_SAR_KPD_AVLBLVLS];
    MS_U8 u8Index = 0;
    MS_U8 u8Channel = 0;
    MS_U8 u8StableCount = 0;

    for(i=0; i<CFG_SAR_KPD_AVLBCHS; i++)
    {
      gu8KeyValue[i] = 0;
      for (k = 0; k < CFG_SAR_KPD_AVLBLVLS; k++)
      {
        KEY_LV[i][k] = 0;
      }
    }
    ...
    *pu8Repeat = 0;
    ...
    {
      for(u8Index = 0; u8Index < g_SARKpdDrvInfo.u8ConfigUsedChs; ++u8Index)
      {
        u8Channel = MDrv_SAR_Kpd_GetChID(u8Index) ;
        if((u8Channel >= CFG_SAR_KPD_AVLBCHS) || (FALSE == gu8StorageChUsed[u8Channel]))
        {
          continue;
        }
        gu8KeyValue[u8Index] = MDrv_SAR_Adc_GetValue(u8Channel);
        for (i=0;i<MDrv_SAR_Kpd_GetChLevels(u8Index);i++)
        {
           if (gu8KeyValue[u8Index] <= MDrv_SAR_Kpd_GetChIdxLevel(u8Index,i))
           {
              KEY_LV[u8Index][i] += 1;
              u8StableCount += 1;
              break;
      ...
      if ((k + 1 - u8StableCount) > SAR_LEVEL_FAILURE_NUM_MAX)
      {
         break;
      }
      MsOS_DelayTask(gu8SamplingInterval);
    }
    ...
    for(u8Index = 0; u8Index < g_SARKpdDrvInfo.u8ConfigUsedChs; ++u8Index)
    {
      for(i=0; i<MDrv_SAR_Kpd_GetChLevels(u8Index); i++)
      {
        if(KEY_LV[u8Index][i] >= SAR_LEVEL_STABLE_NUM_MIN)
        {
...
SAR_KpdResult MDrv_SAR_Kpa_GetKeyCode_Alogrithm(MS_U8 *pu8Key, MS_U8 *pu8Repeat, MS_U8 u8Idx)
{
    ...
    {
        if(MDrv_SAR_Kpd_GetCHKey(pu8Key, pu8Repeat))
            return E_SAR_KPD_OK;
    }
...
{
    SAR_KPD_DBG_FUNC();
    return MDrv_SAR_Kpd_GetCHKey(pu8Key, pu8Repeat);
}
...
SAR_KpdResult MDrv_SAR_Kpd_SetADCSamplingDuration(MS_U8 u8Duration) //ms
{
    if (u8Duration < SAR_LEVEL_STABLE_NUM)
    {
        return E_SAR_KPD_FAIL;
    }

    gu8SamplingInterval = u8Duration / SAR_LEVEL_STABLE_NUM;
...
/// @param u8Channel: sar GPIO channel 0~7
/// @return E_SAR_GPIO_OK: is Analog Input
/// @return E_SAR_GPIO_FAIL is GPIO
//-------------------------------------------------------------------------------------------------
SAR_GpioPinMode MDrv_SAR_Gpio_GetAnalogInput(MS_U8 u8Channel)
{
    MS_BOOL ret;

    ret = HAL_SAR_CfgAanlogInputGet(u8Channel);

    if (ret == TRUE)
        return E_SAR_GPIO_ANALOG_INPUT;
    else
        return E_SAR_GPIO_GPIO_MODE;
}
#endif
