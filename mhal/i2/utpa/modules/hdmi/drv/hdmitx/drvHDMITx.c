...
//------------------------------------------------------------------------------
#ifdef MSOS_TYPE_LINUX_KERNEL
static void _HDMITx_Isr(InterruptNum eIntNum)//irqreturn_t _HDMITx_Isr(InterruptNum eIntNum, void* dev_id)
{
...
MS_U32 MDrv_HDMITx_Get_Semaphore(void *pInstance)
{
    ...
    if (pInstance == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling HDMITx() first\n",__FUNCTION__,__LINE__);
        return u32Return;
    }
    ...
    if (psHDMITXInstPri->u32DeviceID>= E_HDMITX_POOL_ID_MAX)
    {
        printf("[%s,%5d] Unknown HDMITX Pool ID\n",__func__, __LINE__);
        return u32Return;
    }

    void* pModule = NULL;
    UtopiaInstanceGetModule(pInstance, &pModule);
    if(UtopiaResourceObtain(pModule, psHDMITXInstPri->u32DeviceID, &pu32HDMITXRes[psHDMITXInstPri->u32DeviceID]) != UTOPIA_STATUS_SUCCESS)
    {
...
MS_U32 MDrv_HDMITx_Release_Semaphore(void *pInstance)
{
    ...
    u32Return = UtopiaResourceRelease(pu32HDMITXRes[psHDMITXInstPri->u32DeviceID]);
    return u32Return;
}
...
{
    MsOS_SetEvent(_s32HDMITxEventId, E_HdcpTX_EVENT_RITIMER); //wilson@kano
}
...
void MDrv_HDCPTx_StartAuth(void* pInstance, MS_BOOL bFlag)
{
    ...
    if (bFlag)
    {
        ...
        MsOS_SetEvent(_s32HDMITxEventId, E_HdcpTX_EVENT_RUN);
    }
    else
    {
        				...
        				case HDCP14Tx_SubFSM_IDLE:
                            {
                                ...
                                if (MDrv_HDMITx_GetOutputMode(pInstance) & 0x02) //if HDMI mode
                                {
                                    ...
                                    {
                                        if (bPolling == FALSE)
                                            gdwPreTime = MsOS_GetSystemTime(); //first time

                                        bPolling = TRUE;
                                        gdwCurTime = MsOS_GetSystemTime();
        				...
        				case HDCP14Tx_SubFSM_ExchangeKSV:
                            {
            					...
            					if (MHal_HDMITx_HdcpWriteAn(USE_AUTO_GEN_AN) == TRUE)
            					{
                                    ...
                                    {
                						gdwPreTime = MsOS_GetSystemTime();//MAsm_GetSystemTime();
...
static void _HDCPTx_Task(void) //wilson@kano
{
    ...
    {
        MsOS_WaitEvent(_s32HdcpTxEventId, E_HdcpTX_EVENT_RUN | E_HdcpTX_EVENT_IRQ | E_HdcpTX_EVENT_RITIMER, &u32Events, E_OR_CLEAR, 5000);

        if (gHdcpTxInfo.hdcp_Rx_valid == TRUE)
            HDCPTx_Handler(u32Events);

        MsOS_DelayTask(HDCPTX_MONITOR_DELAY);
    } // Task loop
}
...
MS_BOOL MDrv_HDCPTx_Init(void *pInstance) //wilson@kano
{
    ...
    _s32HdcpTxEventId = MsOS_CreateEventGroup("HDCPTx_Event");
    ...
    if (_s32HdcpTxTaskId < 0)
    {
        MsOS_DeleteEventGroup(_s32HdcpTxEventId);
                    ...
                    else
                    {
                        ...
                        if (_MDrv_HDMITx_ParseHdmiVSDB(pInstance, pu8BlkData + u8CurIdx, u8BlkLen, g_u8SupTimingNum))
                        {
                            ...
                            switch (*(pu8BlkData + 3) & 0x30)
                            {
                                ...
                                case 0x10:
                                    if (psHDMITXResPri->stHDMITxInfo.input_color == E_HDMITX_VIDEO_COLOR_YUV422)
                                    {
...
#define DBG_HDMITx_CheckEDID 1
    ...
    while (MHal_HDMITx_EdidReadBlock(0x00, blkData) == FALSE)
    {
		...
		#if (DBG_HDMITx_CheckEDID == 1)
            DBG_HDMITX(printf("[HDMITx] Check EDID fail!!\n"));
		#endif
...
#if (DBG_HDMITx_CheckEDID == 1)
        DBG_HDMITX(printf("%s::block 0 ==>\n", __FUNCTION__));
        for ( i = 0; i < HDMITX_EDID_BLK_SIZE; i++ )
        {
            DBG_HDMITX(printf("0x%02X, ", blkData[i]));
    ...
    for ( i = 0; i < sizeof(EDID_HEADER_TAG); i++ )
    {
        ...
        {
            DBG_HDMITX(printf("%s::invalid EDID header!!\n", __FUNCTION__));
            //return FALSE;
        }
    }
    ...
    for ( i = 1; i <= u8ValidBlkNum; i++)
    {
        while (MHal_HDMITx_EdidReadBlock(i, blkData) == FALSE)
        {
			...
			#if (DBG_HDMITx_CheckEDID == 1)
                DBG_HDMITX(printf("[HDMITx] Check EDID fail--2!!\n"))
			#endif
        ...
        if (u8EDIDRetryCnt != 0)
        {
            ...
            {
                DBG_HDMITX(printf("0x%02X, ", blkData[j]));
                if ( (j+1) % 16 == 0 )
                    DBG_HDMITX(printf("\r\n"));
            }
            DBG_HDMITX(printf("\r\n"));
...
MS_BOOL MDrv_HDMITx_GetColorFormatFromEDID(void* pInstance, MsHDMITX_VIDEO_TIMING timing, MsHDMITX_EDID_COLOR_FORMAT *pColorFmt)
{
    ...
    if ((psHDMITXResPri->stHDMITxInfo.RxEdidInfo.EdidBlk1[0] == 0x02) && (psHDMITXResPri->stHDMITxInfo.RxEdidInfo.EdidBlk1[1] <= 0x04))
    {
        ...
        {
            if (enVICCode == psHDMITXResPri->stHDMITxInfo.RxEdidInfo.VideoDataBlk[i])
            {
...
void MDrv_HDMITx_SetAudioFrequency(void* pInstance, MsHDMITX_AUDIO_FREQUENCY enAudFreq)
{
    ...
    {
        MHal_HDMITx_SetAudioFrequency(psHDMITXResPri->stHDMITxInfo.output_audio_frequncy, psHDMITXResPri->stHDMITxInfo.output_audio_channel, psHDMITXResPri->stHDMITxInfo.output_audio_type);
        MDrv_HDMITx_SendPacket(E_HDMITX_ACR_PACKET, E_HDMITX_CYCLIC_PACKET);
        MDrv_HDMITx_SendPacket(E_HDMITX_AUDIO_INFOFRAME, E_HDMITX_CYCLIC_PACKET);
    }
}
...
{
    _HDMITX_GET_VARIABLE();
    memcpy(&psHDMITXResPri->stHDMITxInfo.analog_setting, (drvHDMITX_ANALOG_TUNING*)pInfo, sizeof(drvHDMITX_ANALOG_TUNING));
}
...
MS_BOOL MDrv_HDMITx_ForceHDMIOutputColorFormat(void* pInstance, MS_BOOL bflag, MsHDMITX_VIDEO_COLOR_FORMAT output_color)
{
    ...
    if (bflag)
    {
        ...
        psHDMITXResPri->stHDMITxInfo.hdmitx_force_output_color = bflag;
            	...
            	case E_HDMITX_FSM_TRANSMIT:
                    {
                        ...
                        else
                        {
                            ...
                            {
                                MsOS_SetEvent(_s32HDMITxEventId, E_HdcpTX_EVENT_RUN); //start hdcp authentication flow
                            }
                        }
...
static void _HDMITx_Task(void* pInstance) //wilson@kano
{
    ...
    while (g_bHDMITxTask == TRUE)
    {
        ...
        {
            if (UtopiaOpen(MODULE_HDMITX, &pu32HDMITxTaskInst, 0, NULL)!= UTOPIA_STATUS_SUCCESS)
            {
        ...
        MsOS_WaitEvent(_s32HDMITxEventId, E_HDMITX_EVENT_RUN|E_HDMITX_EVENT_IRQ|E_HDMITX_EVENT_RXTIMER|E_HDMITX_EVENT_CECRX | E_HdcpTX_EVENT_RUN | E_HdcpTX_EVENT_IRQ | E_HdcpTX_EVENT_RITIMER, &u32Events, E_OR_CLEAR, 5000);
...
MS_BOOL MDrv_HDMITx_Init(void *pInstance)//MS_BOOL MDrv_HDMITx_Init(void)
{
    ...
    if (!MDrv_MMIO_GetBASE(&psHDMITXResPri->stHDMITxInfo.u32PMRIUBaseAddress, &psHDMITXResPri->stHDMITxInfo.u32PMBankSize, MS_MODULE_PM ))
    {
    ...
    #if (defined(MSOS_TYPE_LINUX_KERNEL))
    {
        ...
        if( !MDrv_MMIO_GetBASE( &psHDMITXResPri->stHDMITxInfo.u32CoproRIUBase, &psHDMITXResPri->stHDMITxInfo.u32PMBankSize, MS_MODULE_HW))
        {
    ...
    #else
    DBG_HDMITX(printf("Get IOMAP ID:%u Base:%x!\n", MS_MODULE_PM, (unsigned int)psHDMITXResPri->stHDMITxInfo.u32PMRIUBaseAddress));
    MHal_HDMITx_SetIOMapBase(psHDMITXResPri->stHDMITxInfo.u32PMRIUBaseAddress, psHDMITXResPri->stHDMITxInfo.u32PMRIUBaseAddress);
    #endif
    ...
    _s32HDMITxTaskId = MsOS_CreateTask((TaskEntry)_HDMITx_Task,
                                    (MS_VIRT)pInstance,//(MS_U32)pInstance,
                                    E_TASK_PRI_HIGH,
                                    TRUE,
                                    _pHDMITxTaskStack,
...
/// @return power state
//------------------------------------------------------------------------------
MS_U32 MDrv_HDMITx_SetPowerState(void* pInstance, EN_POWER_MODE u16PowerState)
{
...
void MDrv_HDMITx_MDCMDGetInfo(void* pInstance, MS_U64* u64ReqHdl)
{
...

    {
        ...
        MdbPrint(u64ReqHdl, "----------------  Mstar HDMITx Info -----------------\n");
        //Basic Status
        ...
        //Video
        MdbPrint(u64ReqHdl, "----------------  Mstar HDMITx Video Info -----------\n");
        ...
        //Audio
        MdbPrint(u64ReqHdl, "----------------  Mstar HDMITx Audio Info -----------\n");
        ...
        //EDID
        MdbPrint(u64ReqHdl, "----------------  Mstar HDMITx EDID Info ------------\n");
        ...
        //HDCPTx
        MdbPrint(u64ReqHdl, "----------------  Mstar REE HDCPTx Info -------------\n");
