   ...
   // MS_BOOL ret = FALSE;
    ...
    if(mhal_CEC_IsMessageReceived())
    {
        if(bGetResourceFlag)
        {
            ...
            pCECResourcePrivate->CecRxRecBuf[pCECResourcePrivate->CecRxRecFifoIdxE].ucLength = pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].ucLength;
            ...
            if((pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].ucLength>1) && (pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].ucLength<17)) //1: polling message, 17: over size, not needed to handle
            {
                for(i = 0 ; i < pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].ucLength ; i++)
                {
                    ...
                    pCECResourcePrivate->CecRxRecBuf[pCECResourcePrivate->CecRxRecFifoIdxE].tRxData[i] = pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].tRxData[i];
                }
                if(pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].tRxData[0] != mhal_CEC_HeaderSwap(pCECResourcePrivate->CecRxBuf[pCECResourcePrivate->CecFifoIdxE].tRxData[0]) )
                {
                    ...
                    pCECResourcePrivate->CecRxStartPoint = (pCECResourcePrivate->CecRxRecFifoIdxE > 0)? (pCECResourcePrivate->CecRxStartPoint + 1)%CEC_REC_FIFO_NUM_COUNT : 0;
                    pCECResourcePrivate->CecRxRecFifoIdxE = ((++pCECResourcePrivate->CecRxRecFifoIdxE)>=CEC_REC_FIFO_NUM_COUNT)? 0 : pCECResourcePrivate->CecRxRecFifoIdxE;
                }
...
void _MDrv_CEC_InitVariable(CEC_RESOURCE_PRIVATE *pCECResourcePrivate)
{
    ...
    pCECResourcePrivate->CecRxRecFifoIdxE = 0;
    pCECResourcePrivate->CecTxRecFifoIdxE = 0;
    pCECResourcePrivate->CecRxStartPoint  = 0;
    pCECResourcePrivate->CecTxStartPoint  = 0;
...
CEC_ERROR_CODE _MDrv_CEC_TxApi(MS_U8 ucInitiatorLA, MsCEC_DEVICELA dst_address, MsCEC_MSGLIST msg, MS_U8* operand_ptr, MS_U8 len)
{
    ...
    if((len > CEC_CMD_PARA_MAX_LEN) || (len < 0) || (dst_address > 0x0f))
    {
        return error_code;
    }
...
CEC_ERROR_CODE MDrv_CEC_TxSendMsg(void* pInstance, MsCEC_DEVICELA dst_address, MsCEC_MSGLIST msg, MS_U8 *cmd, MS_U8 len)
{
    ...
    else
    {
        ...
        {
            pCECResourcePrivate->CecTxStartPoint = (pCECResourcePrivate->CecTxRecFifoIdxE > 0)? (pCECResourcePrivate->CecTxStartPoint + 1)%CEC_REC_FIFO_NUM_COUNT : 0;
            pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].ucLength = len + 2;

#if ENABLE_CEC_MULTIPLE
            pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].tTxData[0] = ((pCECResourcePrivate->IniLogicalAddress<<4)&0xF0)|dst_address;
#else
            pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].tTxData[0] = ((pCECResourcePrivate->MyLogicalAddress<<4)&0xF0)|dst_address;
#endif

            pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].tTxData[1] = msg;
            for(i = 2 ; i < pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].ucLength ; i++)
            {
                pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].tTxData[i] = *(cmd+j);
                j++;
            }
            pCECResourcePrivate->CecTxRecBuf[pCECResourcePrivate->CecTxRecFifoIdxE].tTxErrCode = res;
            pCECResourcePrivate->CecTxRecFifoIdxE = ((++pCECResourcePrivate->CecTxRecFifoIdxE)>=CEC_REC_FIFO_NUM_COUNT)? 0 : pCECResourcePrivate->CecTxRecFifoIdxE;
        }
    }
...
//**************************************************************************
//  [Function Name]:
//                  MDrv_CEC_SetChipType()
//  [Description]
//                  Set Chip type : TV or STB
//  [Arguments]:
//
//  [Return]:
//
//**************************************************************************
void MDrv_CEC_SetChipType(void* pInstance, MsCEC_CHIP_TYPE ChipType)
{
    void* pModule = NULL;
    void* pCECResource = NULL;
    CEC_RESOURCE_PRIVATE *pCECResourcePrivate = NULL;
    ...
    else
    {
        ...
        else
        {
            ...
            pCECResourcePrivate->bCECIsSource = pCECResourcePrivate->bCECIsSource | ChipType;
        }
    }
...
void mdrv_CEC_MDCMDEchoCommand(void* pInstance, MS_U64* u64ReqHdl, char* pcCmdLine)
{
    ...
    if(strncmp("help", psep, 4) == 0)
    {
        MdbPrint(u64ReqHdl, "----------------  Mstar CEC help ----------------");
        MdbPrint(u64ReqHdl, "CEC only support GetInfo function now.");
        MdbPrint(u64ReqHdl, "--------------------------------------------------");
    }
}
...
void mdrv_CEC_MDCMDGetInfo(void* pInstance, MS_U64* u64ReqHdl)
{
    ...
    else
    {
        ...
        MdbPrint(u64ReqHdl, "------------  Receive Command Record -------------");
        MdbPrint(u64ReqHdl, "\n\n");

        u8cnt = 1;
        for(i=(pCECResourcePrivate->CecRxStartPoint+1);i<=(pCECResourcePrivate->CecRxStartPoint+CEC_REC_FIFO_NUM_COUNT);i++)
        {
            if(pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength > 0)
            {
                MdbPrint(u64ReqHdl,"[%d]RX_length= %d\n",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength);
                if(pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength == 1)
                {
                    MdbPrint(u64ReqHdl, "[%d]RX_Header=0x%x",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[0]);
                }
                else if(pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength == 2)
                {
                    MdbPrint(u64ReqHdl, "[%d]RX_Header=0x%x\n",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[0]);
                    MdbPrint(u64ReqHdl, "[%d]RX_Opcode=0x%x",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[1]);
                }
                else
                {
                    MdbPrint(u64ReqHdl, "[%d]RX_Header=0x%x\n",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[0]);
                    MdbPrint(u64ReqHdl, "[%d]RX_Opcode=0x%x\n",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[1]);
                    for(j=0;j<((pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength)-2);j++)
                    {
                        if(j==0)
                        {
                            MdbPrint(u64ReqHdl, "[%d]RX_Operand=0x%x",u8cnt,pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[j+2]);
                        }
                        else
                        {
                            MdbPrint(u64ReqHdl, ", 0x%lx",pCECResourcePrivate->CecRxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tRxData[j+2]);
                        }
                    }
                }
                u8cnt++;
        ...
        MdbPrint(u64ReqHdl, "------------  Send Command Record -------------");
        MdbPrint(u64ReqHdl, "\n\n");

        u8cnt = 1;
        for(i=(pCECResourcePrivate->CecTxStartPoint+1);i<=(pCECResourcePrivate->CecTxStartPoint+CEC_REC_FIFO_NUM_COUNT);i++)
        {
            if(pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength > 0)
            {
                MdbPrint(u64ReqHdl,"[%d]TX_length== %d\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength);
                if(pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength == 1)
                {
                    MdbPrint(u64ReqHdl, "[%d]TX_Header=0x%x\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[0]);
                }
                else if(pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength == 2)
                {
                    MdbPrint(u64ReqHdl, "[%d]TX_Header=0x%x\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[0]);
                    MdbPrint(u64ReqHdl, "[%d]TX_Opcode=0x%x\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[1]);
                }
                else
                {
                    MdbPrint(u64ReqHdl, "[%d]TX_Header=0x%x\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[0]);
                    MdbPrint(u64ReqHdl, "[%d]TX_Opcode=0x%x\n",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[1]);
                    for(j=0;j<((pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].ucLength)-2);j++)
                    {
                        if(j==0)
                        {
                            MdbPrint(u64ReqHdl, "[%d]TX_Operand=0x%x",u8cnt,pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[j+2]);
                        }
                        else
                        {
                            MdbPrint(u64ReqHdl, ", 0x%x",pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxData[j+2]);
                        }
                    }
                    MdbPrint(u64ReqHdl, "\n");
                }

                switch(pCECResourcePrivate->CecTxRecBuf[i%CEC_REC_FIFO_NUM_COUNT].tTxErrCode)
                {
                    case E_CEC_FEATURE_ABORT:
                        MdbPrint(u64ReqHdl, "[%d]TX_Status=Feature Abort",u8cnt);
                        break;
                    case E_CEC_TX_SUCCESS:
                        MdbPrint(u64ReqHdl, "[%d]TX_Status=Send Success",u8cnt);
                        break;
                    case E_CEC_RF:
                        MdbPrint(u64ReqHdl, "[%d]TX_Status=Retry Fail",u8cnt);
                        break;
                    case E_CEC_LOST_ABT:
                        MdbPrint(u64ReqHdl, "[%d]TX_Status=Lost Arbitration",u8cnt);
                        break;
                    case E_CEC_SYSTEM_BUSY:
                        MdbPrint(u64ReqHdl, "[%d]TX_Status=System Busy",u8cnt);
                        break;
                    default:
                        break;
                }
