...
//
//**************************************************************************
void MApi_CEC_SetChipType(MsCEC_CHIP_TYPE ChipType)
{
    stCEC_SET_CHIPTYPE pCECArgs = {0};

    pCECArgs.eChipType= ChipType;

    if(pulCECInst == NULL)
    {
        MAPI_CEC_MSG_WARNING("[%s,%5d]No instance existed, please get an instance by calling mapi_cec_initchip() first\n",__FUNCTION__,__LINE__);
    }
    else if(UtopiaIoctl(pulCECInst, MAPI_CMD_CEC_SET_CHIP_TYPE, (void*)&pCECArgs) != UTOPIA_STATUS_SUCCESS)
    {
        MAPI_CEC_MSG_ERROR("Obtain CEC engine fail\n");
    }
}
...
MS_BOOL MApi_CEC_Ctrl(MS_U32 u32Cmd, void *pBuf, MS_U32 u32BufSize)
{
    ...
    switch(u32Cmd)
    {
        ...
        case E_CEC_CMD_SET_CHIP_TYPE:
            {
                if(u32BufSize == sizeof(stCMD_CEC_SET_CHIP_TYPE))
                {
                    MApi_CEC_SetChipType(((stCMD_CEC_SET_CHIP_TYPE *)pBuf)->eChipType);
