...
MS_U32 HDMITXOpen(void** ppInstance, const void* const pAttribute)
{
    ...
    psHDMITXInstPri->fpHDMITx_Cmd_Hdcp1x_Compare_Ri = MApi_HDMITx_Hdcp1xCompareRi_U2;
    psHDMITXInstPri->fpHDMITx_Cmd_Get_TMDS_Status = MApi_HDMITx_GetTMDSStatus_U2;
    psHDMITXInstPri->fpHDMITx_Cmd_Get_Color_Format = Mpi_HDMITx_GetAVIInfo_ColorFmt_U2;
    psHDMITXInstPri->fpHDMITx_Cmd_Get_Color_Depth = MApi_HDMITx_GetGC_ColorDepth_U2;
    psHDMITXInstPri->fpHDMITx_Cmd_Check_EDID_Quantization_Range = MApi_HDMITx_CheckEDIDQuantizationRange_U2;
...
MS_U32 HDMITXIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
        ...
        case E_HDMITX_CMD_HDCP1XCOMPARERI:
            {
                pstHDMITX_HDCP1X_COMPARE_RI pHDMITXArgs = (pstHDMITX_HDCP1X_COMPARE_RI)pArgs;
                pHDMITXArgs->bReturn = psHDMITXInstPri->fpHDMITx_Cmd_Hdcp1x_Compare_Ri(pInstance);
                return UTOPIA_STATUS_SUCCESS;
                break;
            }
        ...
        case E_HDMITX_CMD_GET_COLOR_FORMAT:
            {
                pstHDMITX_GET_COLOR_FMT pHDMITXArgs = (pstHDMITX_GET_COLOR_FMT)pArgs;
                pHDMITXArgs->enColorFmt = psHDMITXInstPri->fpHDMITx_Cmd_Get_Color_Format(pInstance);
                pHDMITXArgs->u32Ret = (MS_U32)pHDMITXArgs->enColorFmt;

                return UTOPIA_STATUS_SUCCESS;
                break;
            }

        case E_HDMITX_CMD_GET_COLOR_DEPTH:
            {
                pstHDMITX_GET_COLOR_DEPTH pHDMITXArgs = (pstHDMITX_GET_COLOR_DEPTH)pArgs;
                pHDMITXArgs->enColorDepth = psHDMITXInstPri->fpHDMITx_Cmd_Get_Color_Depth(pInstance);
                pHDMITXArgs->u32Ret = (MS_U32)pHDMITXArgs->enColorDepth;

                return UTOPIA_STATUS_SUCCESS;
                break;
            }

        case E_HDMITX_CMD_CHECK_EDID_QUANT_RANGE:
            {
                pstHDMITX_CHECK_EDID_QUAN_RANG pHDMITXArgs = (pstHDMITX_CHECK_EDID_QUAN_RANG)pArgs;
                pHDMITXArgs->u8Return= psHDMITXInstPri->fpHDMITx_Cmd_Check_EDID_Quantization_Range(pInstance, pHDMITXArgs->eTiming, pHDMITXArgs->eOutColor);
                return UTOPIA_STATUS_SUCCESS;
                break;
            }
        ...
        case E_HDMITX_CMD_VANCLEEF_LOADHDCPKEY:
            {
                ...
                pHDMITXArgs->bReturn = psHDMITXInstPri->fpHDMITX_VANCLEEF_LOADHDCPKEY(pInstance, pHDMITXArgs->pHdcpKey, pHDMITXArgs->u32Size);
