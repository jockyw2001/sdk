...
void MDrv_PCMCIA_Init(MS_BOOL bCD_Reverse)
{
    ...
    if(NULL == pPcmInstant)
    {
        if(UTOPIA_STATUS_FAIL == UtopiaOpen(MODULE_CI, &pPcmInstant, 0, NULL))
        {
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Init, &bCD_Reverse);
...
MS_BOOL MDrv_PCMCIA_DetectV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Detect, &stPcmDetect);
...
MS_BOOL MDrv_PCMCIA_PollingV2( PCMCIA_MODULE eModule )
{
    ...
    UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Polling, &stPcmPoll);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
void MDrv_PCMCIA_Set_HW_ResetDuration( MS_U8 u8HW_ResetDuration )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Set_HW_ResetDuration, &u8HW_ResetDuration);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_ResetHW_V2( PCMCIA_MODULE eModule)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ResetHW, &eModule);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_WriteAttribMemV2( PCMCIA_MODULE eModule, MS_U16 u16Addr, MS_U8 u8Value)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WriteAttribMem, &stWrite);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_ReadAttribMemV2( PCMCIA_MODULE eModule, MS_U16 u16Addr, MS_U8 *pDest )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ReadAttribMem, &stRead);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        *pDest = 0xFF;
...
void MDrv_PCMCIA_WriteIOMemV2( PCMCIA_MODULE eModule, MS_U16 u16Addr, MS_U8 u8Value)
{
    ...
    stWrite.u16Addr = u16Addr;
    stWrite.u8Value = u8Value;
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WriteIOMem, &stWrite);
...
void MDrv_PCMCIA_WriteIOMemLongV2( PCMCIA_MODULE eModule, MS_U16 u16Addr, MS_U8 u8Value, MS_U16 u16DataLen, MS_U8* u8pWriteBuffer)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WriteIOMemLong, &stWrite);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_U8 MDrv_PCMCIA_ReadIOMemV2( PCMCIA_MODULE eModule, MS_U16 u16Addr )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ReadIOMem, &stRead);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_BOOL MDrv_PCMCIA_IsModuleStillPluggedV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_IsModuleStillPlugged, &stPlug);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
void MDrv_PCMCIA_Set_Detect_Trigger( MS_BOOL bHighActive )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Set_Detect_Trigger, &bHighActive);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_Set_Detect_Enable( MS_BOOL bEnable )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Set_Detect_Enable, &bEnable);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_U32 MDrv_PCMCIA_Get_CD_IntervalV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Get_CD_Interval, &stInterval);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return (MS_U32)-1;
...
void MDrv_PCMCIA_ParseAttribMem( MS_U8 *pAttribMem, MS_U16 dwLen, PCMCIA_INFO *pInfo )
{
    ...
    stParse.pInfo = pInfo;
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ParseAttribMem, &stParse);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_SetCommandBitV2( PCMCIA_MODULE eModule, MS_U8 u8CommandBit, MS_BOOL bValue )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_SetCommandBit, &stCmdBit);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_BOOL MDrv_PCMCIA_ResetInterfaceV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ResetInterface, &stRst);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_BOOL MDrv_PCMCIA_IsDataAvailableV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_IsDataAvailable, &stData);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_U16 MDrv_PCMCIA_ReadDataV2( PCMCIA_MODULE eModule, MS_U8* u8pReadBuffer, MS_U16 u16ReadBufferSize )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_ReadData, &stRead);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_BOOL MDrv_PCMCIA_WriteDataV2( PCMCIA_MODULE eModule, MS_U8* u8pWriteBuffer, MS_U16 u16DataLen )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WriteData, &stWrite);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_BOOL MDrv_PCMCIA_SwitchToIOmodeV2( PCMCIA_MODULE eModule, PCMCIA_INFO *pInfo )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_SwitchToIOmode, &stCmd);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_U16 MDrv_PCMCIA_NegotiateBufferSizeV2( PCMCIA_MODULE eModule, PCMCIA_INFO *pInfo )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_NegotiateBufferSize, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return 0xFF;
...
void MDrv_PCMCIA_WriteBufferSizeV2( PCMCIA_MODULE eModule, MS_U16 u16BufferSize )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WriteBufferSize, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_BOOL MDrv_PCMCIA_GetLibVer( const MSIF_Version **ppVersion )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_GetLibVer, ppVersion);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_Enable_InterruptV2( PCMCIA_MODULE eModule, MS_BOOL bEnable )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Enable_Interrupt, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
void MDrv_PCMCIA_Set_InterruptStatusV2( PCMCIA_MODULE eModule, MS_BOOL Status )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Set_InterruptStatus, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_BOOL MDrv_PCMCIA_Get_InterruptStatusV2( PCMCIA_MODULE eModule )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_Get_InterruptStatus, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
void MDrv_PCMCIA_InstarllIsrCallbackV2( PCMCIA_MODULE eModule, IsrCallback fnIsrCallback )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_InstallIsrCallback, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
    }
...
MS_BOOL MDrv_PCMCIA_WaitForStatusBitV2( PCMCIA_MODULE eModule, MS_U8 u8StatusBit )
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_WaitForStatusBit, &stArgs);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_U32 MDrv_PCMCIA_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_SetPowerState, (void* const)&u16PowerState);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_BOOL MDrv_PCMCIA_GetConfig(MS_BOOL* bCD_Reverse)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_GetConfig, bCD_Reverse);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
...
MS_BOOL MDrv_PCMCIA_OptConfig(PCMCIA_MODULE eModule, PCMCIA_OPTION eOpt, MS_U32* pInput, MS_U32* pOutput, MS_BOOL bSet)
{
    ...
    u32Ret = UtopiaIoctl(pPcmInstant, E_MDRV_CMD_PCM_OptConfig, &opt);
    if(UTOPIA_STATUS_SUCCESS != u32Ret)
    {
        ULOGE("PCMCIA", "[%s]ERROR : UtopiaIoctl return value 0x%x\n", __FUNCTION__, (unsigned int)u32Ret);
        return FALSE;
