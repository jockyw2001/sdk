...
void CIRegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    UtopiaModuleRegisterMdbNode("pcmcia", (FUtopiaMdbIoctl)PCMCIAMdbIoctl);
    #endif
        ...
        case E_MDRV_CMD_PCM_OptConfig:
            {
                ...
                if(pResPri->bInited_Drv == TRUE)
                {
                    PCM_OPTCONFIG* pstOptConfig = (PCM_OPTCONFIG*)pu32Args;
                    if(TRUE == _MDrv_PCMCIA_OptConfig(pstOptConfig->eModule, 
                                                      pstOptConfig->opt,
                                                      &pstOptConfig->in, 
                                                      &pstOptConfig->out,
                                                      pstOptConfig->bSet))
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
MS_U32 PCMCIAMdbIoctl(MS_U32 cmd, const void* const pArgs)
{
    ...
    switch(cmd)
    {
        case MDBCMD_CMDLINE:
            ...
            MdbPrint(paraCmdLine->u64ReqHdl,"-------------MStar pcmcia state----------\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"pcmcia only support get info function now\n");
            MdbPrint(paraCmdLine->u64ReqHdl,"-----------------------------------------\n");
            paraCmdLine->result = MDBRESULT_SUCCESS_FIN;
            break;
        case MDBCMD_GETINFO:
            paraGetInfo = (MDBCMD_GETINFO_PARAMETER *)pArgs;
            MdbPrint(paraGetInfo->u64ReqHdl,"---------MStar pcmcia state---------\n");
            if(UtopiaResourceObtain(pModulePcm, E_PCMCIA_RESOURCE, &psResource) == UTOPIA_STATUS_FAIL)
            {
                ULOGE("PCMCIA", "[%s][%d]UtopiaResourceObtain PCMCIA fail\n", __FUNCTION__, __LINE__);
                return FALSE;
            ...
            {
                if(MDrv_PCMCIA_DetectV2(E_PCMCIA_MODULE_A) == FALSE)
                {
                    MdbPrint(paraGetInfo->u64ReqHdl,"PCMCIA not insert\n");
                }
                else
                {
                    MdbPrint(paraGetInfo->u64ReqHdl,"PCMCIA insert\n");
                }
            }
