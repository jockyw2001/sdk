...
static void _MDrv_PCMCIA_COMPANION_SetGPIO(PCMCIA_MODULE eModule, MS_BOOL bHigh);
#ifdef CONFIG_PCMCIA_MSPI_BURST
...
MS_BOOL _MDrv_PCMCIA_PollingV2( PCMCIA_MODULE eModule )
{
    ...
    if(bModuleStatusChange)
    {
        if(_gbCardInside[eModule])
        {
            ...
            _MDrv_PCMCIA_COMPANION_SetGPIO(eModule, TRUE);
            PCMCIA_DEBUG( ( "Card detected\n" ) );
        }
        else
        {
            _MDrv_PCMCIA_COMPANION_SetGPIO(eModule, FALSE);
...
MS_BOOL _MDrv_PCMCIA_OptConfig(PCMCIA_MODULE eModule, PCMCIA_OPTION eOpt, MS_U32* pInput, MS_U32* pOutput, MS_BOOL bSet)
{
    return HAL_PCMCIA_OptConfig(eModule, eOpt, pInput, pOutput, bSet);
}
...
{
#ifdef CONFIG_PCMCIA_MSPI    
    HAL_PCMCIA_Write_ByteMask(REG_PCM1_PWR_OEN, PCM1_PWR_OUT, PCM1_PWR_OEN_MASK);
    if(bHigh)
    {
        HAL_PCMCIA_Write_ByteMask(REG_PCM1_PWR_OUT, PCM1_PWR_OUT_HIGH, PCM1_PWR_OUT_MASK);
    }
    else
    {
        HAL_PCMCIA_Write_ByteMask(REG_PCM1_PWR_OUT, PCM1_PWR_OUT_LOW, PCM1_PWR_OUT_MASK);
    }
#endif    
}
// backward compatible
...
#ifdef MSOS_TYPE_LINUX_KERNEL
EXPORT_SYMBOL(MDrv_PCMCIA_WriteBufferSizeV2);
EXPORT_SYMBOL(MDrv_PCMCIA_Init);
EXPORT_SYMBOL(MDrv_PCMCIA_Exit);
EXPORT_SYMBOL(MDrv_PCMCIA_InstarllIsrCallbackV2);
EXPORT_SYMBOL(MDrv_PCMCIA_WriteDataV2);
EXPORT_SYMBOL(MDrv_PCMCIA_ResetHW_V2);
EXPORT_SYMBOL(MDrv_PCMCIA_SwitchToIOmodeV2);
EXPORT_SYMBOL(MDrv_PCMCIA_NegotiateBufferSizeV2);
EXPORT_SYMBOL(MDrv_PCMCIA_SetPowerState);
EXPORT_SYMBOL(MDrv_PCMCIA_ReadDataV2);
EXPORT_SYMBOL(MDrv_PCMCIA_Enable_InterruptV2);
EXPORT_SYMBOL(MDrv_PCMCIA_ParseAttribMem);
EXPORT_SYMBOL(MDrv_PCMCIA_IsDataAvailableV2);
EXPORT_SYMBOL(MDrv_PCMCIA_IsModuleStillPluggedV2);
EXPORT_SYMBOL(MDrv_PCMCIA_PollingV2);
