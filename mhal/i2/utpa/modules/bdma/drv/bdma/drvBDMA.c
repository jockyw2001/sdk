...
typedef struct _BDMA_MDEBUG_INFO  //control block
{
    MS_U32      source_device;
    MS_U32      destination_device;
    MS_U32      status;
    MS_PHYADDR  source_address;
    MS_PHYADDR  destination_address;
    MS_PHYADDR  length;
#if defined(MSOS_TYPE_LINUX_KERNEL)
...
#define MDEBUG_INFO_NUMBER 10
static MS_U32 mdebug_info_index;
static BDMA_MDEBUG_INFO mdebug_info[MDEBUG_INFO_NUMBER];
#endif
...
static void _BDMA_MemFlush(BDMA_Dev eDev, MS_U32 u32Base, MS_U32 u32Size)
{
    ...
    case E_BDMA_DEV_MIU3:
        if(MsOS_MPool_IsPA2KSEG0Mapped(u32Base))
        {
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
    #if defined(MSOS_TYPE_LINUX_KERNEL)
        getnstimeofday(&mdebug_info[mdebug_info_index].done_time);
    #else
        mdebug_info[mdebug_info_index].done_time = MsOS_GetSystemTime();
    #endif

    if(u8Ch == E_BDMA_CH0)
    {
        mdebug_info[mdebug_info_index].status= HAL_BDMA_ReadByte(BDMA_REG_CH0_STATUS);
    }
    mdebug_info_index = (mdebug_info_index + 1) % MDEBUG_INFO_NUMBER;
#endif
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
    #if defined(MSOS_TYPE_LINUX_KERNEL)
        getnstimeofday(&mdebug_info[mdebug_info_index].start_time);
    #else
        mdebug_info[mdebug_info_index].start_time = MsOS_GetSystemTime();
    #endif
#endif
...
static BDMA_Dev _BDMA_Check_MIUDev(BDMA_Dev eDev, MS_PHY phy64Addr)
{
    ...
    {
        BDMA_DBG_FUNC();
        BDMA_DBG_INFO("u32Offset = 0x%08x", u32Offset);
        BDMA_DBG_INFO("u8MiuSel = 0x%02x", u8MiuSel);
    }
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
        mdebug_info_index = 0;
        while(mdebug_info_index < MDEBUG_INFO_NUMBER)
        {
            mdebug_info[mdebug_info_index].source_device = 0;
            mdebug_info[mdebug_info_index].destination_device = 0;
            mdebug_info[mdebug_info_index].status = 0;
            mdebug_info[mdebug_info_index].source_address = 0;
            mdebug_info[mdebug_info_index].destination_address = 0;
            mdebug_info[mdebug_info_index].length = 0;
#if defined(MSOS_TYPE_LINUX_KERNEL)
            mdebug_info[mdebug_info_index].start_time.tv_sec = 0;
            mdebug_info[mdebug_info_index].start_time.tv_nsec = 0;
            mdebug_info[mdebug_info_index].done_time.tv_sec = 0;
            mdebug_info[mdebug_info_index].done_time.tv_nsec = 0;
#else
            mdebug_info[mdebug_info_index].start_time = 0;
            mdebug_info[mdebug_info_index].done_time = 0;
#endif
            mdebug_info_index++;
        }
        mdebug_info_index = 0;
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
    mdebug_info[mdebug_info_index].source_device=eSrcDev;
    mdebug_info[mdebug_info_index].destination_device=eDstDev;
    mdebug_info[mdebug_info_index].source_address= phy64SrcAddr;
    mdebug_info[mdebug_info_index].destination_address= phy64DstAddr;
    mdebug_info[mdebug_info_index].length= u32Len;
#endif
...
#if defined(CONFIG_UTOPIA_PROC_DBG_SUPPORT)
void MDrv_BDMA_Debug_echo(MS_PHYADDR Src_addr, MS_PHYADDR Dst_Addr, MS_PHY Src_Device, MS_PHY Dst_Device, MS_PHYADDR Length)
{
    MS_PHY phy64Reg ;
    MS_U32 u32Delay;

    mdebug_info[mdebug_info_index].source_address = Src_addr;
    mdebug_info[mdebug_info_index].destination_address = Dst_Addr;
    mdebug_info[mdebug_info_index].source_device = Src_Device;
    mdebug_info[mdebug_info_index].destination_device = Dst_Device;
    mdebug_info[mdebug_info_index].length = Length;

    phy64Reg = HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_SRC_ADDR_L, 0);
    HAL_BDMA_Write4Byte(BDMA_REG_CH0_SRC_ADDR_L , Src_addr);

    phy64Reg = HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_DST_ADDR_L, 0);
    HAL_BDMA_Write4Byte(BDMA_REG_CH0_DST_ADDR_L , Dst_Addr);

    phy64Reg = HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_SIZE_L, 0);
    HAL_BDMA_Write4Byte(BDMA_REG_CH0_SIZE_L , Length);

    phy64Reg = HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_DST_SEL, 0);
    HAL_BDMA_WriteByte(BDMA_REG_CH0_DST_SEL, Dst_Device);

    phy64Reg = HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_SRC_SEL, 0);
    HAL_BDMA_WriteByte(BDMA_REG_CH0_SRC_SEL, Src_Device);
    ...
    HAL_BDMA_WriteRegBit(BDMA_REG_CH0_CTRL, BDMA_CH_TRIGGER, ENABLE);

    u32Delay = BDMA_WAIT_TIME;

    do
    {
        if (HAL_BDMA_Is_Done(E_BDMA_ACT_COPY_MEM, E_BDMA_CH0))
            break;
...
static void MDrv_BDMA_Debug_print_info_entry(MS_U32 index, MS_U64* Hdl)
{
#if defined(MSOS_TYPE_LINUX_KERNEL)
    printk("\nSource Address        : %llx\n", mdebug_info[index].source_address);
    printk("\nDestination Address   : %llx\n", mdebug_info[index].destination_address);
#else
    printf("\nSource Address        : %x\n", (size_t)mdebug_info[index].source_address);
    printf("\nDestination Address   : %x\n", (size_t)mdebug_info[index].destination_address);
#endif
    switch(mdebug_info[index].source_device)
    {
        case E_BDMA_SRCDEV_MIU0:
            MdbPrint(Hdl,"\nSource Device         : MIU0(%lx)\n", mdebug_info[index].source_device);
            break;
        case E_BDMA_SRCDEV_MIU1:
            MdbPrint(Hdl,"\nSource Device         : MIU1(%lx)\n", mdebug_info[index].source_device);
            break;
        case E_BDMA_SRCDEV_MEM_FILL:
            MdbPrint(Hdl,"\nSource Device         : MEM_FILL(%lx)\n", mdebug_info[index].source_device);
            break;
        case E_BDMA_SRCDEV_FLASH:
            MdbPrint(Hdl,"\nSource Device         : FLASH(%lx)\n", mdebug_info[index].source_device);
            break;
        case E_BDMA_SRCDEV_MIU2:
            MdbPrint(Hdl,"\nSource Device         : MIU2(%lx)\n", mdebug_info[index].source_device);
            break;
        case E_BDMA_SRCDEV_MIU3:
            MdbPrint(Hdl,"\nSource Device         : MIU3(%lx)\n", mdebug_info[index].source_device);
            break;
        default:
            MdbPrint(Hdl,"\nSource Device         : NOT_SUPPORT(%lx)\n", mdebug_info[index].source_device);
    }

    switch(mdebug_info[index].destination_device)
    {
        case E_BDMA_DSTDEV_MIU0:
            MdbPrint(Hdl,"\nDestination Device    : MIU0(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_MIU1:
            MdbPrint(Hdl,"\nDestination Device    : MIU1(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_SEARCH:
            MdbPrint(Hdl,"\nDestination Device    : SEARCH(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_CRC32:
            MdbPrint(Hdl,"\nDestination Device    : CRC32(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_DMDMCU:
            MdbPrint(Hdl,"\nDestination Device    : DMDMCU(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_VDMCU:
            MdbPrint(Hdl,"\nDestination Device    : VDMCU(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_DSP:
            MdbPrint(Hdl,"\nDestination Device    : DSP(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_TSP:
            MdbPrint(Hdl,"\nDestination Device    : TSP(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_HK51_1KSRAM:
            MdbPrint(Hdl,"\nDestination Device    : HK51_1KSRAM(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DEV_MIU2:
            MdbPrint(Hdl,"\nDestination Device    : MIU2(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DEV_MIU3:
            MdbPrint(Hdl,"\nDestination Device    : MIU3(%lx)\n", mdebug_info[index].destination_device);
            break;
        case E_BDMA_DSTDEV_FCIC:
            MdbPrint(Hdl,"\nDestination Device    : FCIC(%lx)\n", mdebug_info[index].destination_device);
            break;
        default:
            MdbPrint(Hdl,"\nDestination Device    : NOT_SUPPORT(%lx)\n", mdebug_info[index].destination_device);
    }

    MdbPrint(Hdl,"\nTransmit Length       : %lx\n", mdebug_info[index].length);

    if(mdebug_info[index].status & BDMA_CH_DONE)
    {
        MdbPrint(Hdl,"\nStatus                : Done(%lx)\n", mdebug_info[index].status);
    }
    else
    {
        MdbPrint(Hdl,"\nStatus                : Error(%lx)\n", mdebug_info[index].status);
    ...
    MdbPrint(Hdl,"\nBdma miu select       : HW\n");
#if defined(MSOS_TYPE_LINUX_KERNEL)
    printk("\nTime                  : %d.%09ld(sec)\n", mdebug_info[index].done_time.tv_sec - mdebug_info[index].start_time.tv_sec, mdebug_info[index].done_time.tv_nsec - mdebug_info[index].start_time.tv_nsec);
#else
    printf("\nTime                  : %u.%03u(sec)\n", (unsigned int)(mdebug_info[index].done_time - mdebug_info[index].start_time)/1000, (unsigned int)(mdebug_info[index].done_time - mdebug_info[index].start_time)%1000);
#endif
}

void MDrv_BDMA_Debug_cat(MS_U64* Hdl)
{
    MS_U32 print_index;
    MS_U32 latest_index;

    latest_index = MDEBUG_INFO_NUMBER;
    print_index = mdebug_info_index;

    while(print_index < MDEBUG_INFO_NUMBER)
    {
        MdbPrint(Hdl,"\n---------MStar BDMA latest Info [%d]--------- \n", latest_index);
        MDrv_BDMA_Debug_print_info_entry(print_index, Hdl);
        print_index++;
        latest_index--;
    }
    ...
    while(print_index < mdebug_info_index)
    {
        MdbPrint(Hdl,"\n---------MStar BDMA latest Info [%d]--------- \n", latest_index);
        MDrv_BDMA_Debug_print_info_entry(print_index, Hdl);
        print_index++;
