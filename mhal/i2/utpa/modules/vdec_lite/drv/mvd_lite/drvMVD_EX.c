...
{
    return HAL_MVD_GetMaxPixel(u32Id);
}
...
MS_BOOL MDrv_MVD_Init(MS_U32 u32Id, MVD_CodecType eCodecType, MS_BOOL bShareBBU)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_Init:start u32Id=0x%x, sid=0x%x, halIdx=0x%x, bShareBBU=0x%x\n",
        u32Id, _MVD_GET_HAL_SID(u32Id), u8HalIdx, bShareBBU));
...
MS_BOOL MDrv_MVD_Exit(MS_U32 u32Id)
{
    ...
    if (FALSE == MVD_GetIsUsed(u8DrvIdx))
    {
        ...
        //HAL_VPU_EX_PowerCtrl(u32Id, DISABLE);
        return FALSE;
    }
...
MS_BOOL MDrv_MVD_DecodeIFrame(MS_U32 u32Id, MS_PHY u32FrameBufAddr, MS_PHY u32StreamBufAddr, MS_PHY u32StreamBufEndAddr )
{
    ...
    bRet = HAL_MVD_DecodeIFrame(u8HalIdx, u32FrameBufAddr, u32StreamBufAddr, u32StreamBufEndAddr, u8ChipECORev);
    return bRet;
}
...
//    MDrv_MVD_SetAVSync(u32Id, FALSE, 0); // jeffrey 0920
    return TRUE;
}
...
E_MVD_Result MDrv_MVD_SetSpeed(MS_U32 u32Id, MVD_SpeedType eSpeedType, MS_U8 u8Multiple)
{
    ...
    {
        if((eSpeedType == E_MVD_SPEED_FAST && u8Multiple == 2 && pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] == TRUE) == FALSE)
        {
...
E_MVD_Result MDrv_MVD_SetIsrEvent(MS_U32 u32Id, MS_U32 eEvent, MVD_InterruptCb fnHandler)
{
    ...
    if (eEvent == E_MVD_EVENT_DISABLE_ALL)
    {
        ...
        if((_bDisableFlag != TRUE) && (FALSE == bUsed))
        {
            ...
            {
                MVD_DEBUGERROR(MVD_PRINT("%s Disable Interrupt Fail\n", __FUNCTION__));
            }
...
MS_BOOL MDrv_MVD_DisableEsFullStop(MS_U32 u32Id, MS_BOOL bDisable)
{
    ...
    return TRUE;
#else
    MVD_PRINT( "Command: (CMD_ES_FULL_STOP) NOT SUPPORT!!!\r\n");
    return FALSE;
#endif
}
...
#ifdef VDEC3
    u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(eVpuStreamType, FALSE, bIsNStreamMode);
#else
    u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(eVpuStreamType, FALSE);
#endif
...
#define SIZE_3K 0xC00
    ...
    {
        MVD_PRINT("this chip does not support set external DS buffer since VPU_FORCE_MIU_MODE == 1 !!\n");
        return E_MVD_RET_FAIL;
    }
#endif
...
{
    HAL_VPU_EX_DynamicFBMode(u32Id,bEnable,u32Address,u32Size);
}
...
{
    MS_U8 u8HalIdx = _MVD_GET_IDX(u32Id);
...
MS_BOOL MDrv_MVD_EX_IsDispQueueEmpty(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsDispQueueEmpty(u8HalIdx);
}
...
{
    *u32PC = HAL_VPU_EX_GetProgCnt(u32Id);
}
