...
MS_BOOL bEVDIsIsrAttached      = FALSE;  //check EVD isr already attached, avoid EVD+EVD re-attached
MS_BOOL bEVDLITEIsIsrAttached  = FALSE;  //check EVDLITE isr already attached, avoid EVDLITE+EVDLITE re-attached
MS_BOOL bHVDDisableISRFlag     = TRUE;
MS_BOOL bEVDDisableISRFlag     = TRUE;
MS_BOOL bEVDLITEDisableISRFlag = TRUE;
...
static HVD_EX_Result _HVD_EX_InitVariables(MS_U32 u32Id, HVD_EX_MemCfg *pStMemCfg, HVD_Init_Params *pStInitParams)
{
    ...
    HVD_EX_MSG_INF("HVD mmap: MIU1base:0x%lx, MIU2base:0x%lx,FW(type:%d addr=0x%lx size=0x%x), VLC(addr=0x%lx size=0x%x), Code(addr=0x%lx size=0x%x), FB(addr=0x%lx size=0x%x), Bit(addr=0x%lx size=0x%x), Drv(addr=0x%lx size=0x%x)\n",
         (unsigned long)pCtrl->MemMap.u32MIU1BaseAddr,
         (unsigned long)pCtrl->MemMap.u32MIU2BaseAddr,
         pCtrl->MemMap.eFWSourceType, (unsigned long)pCtrl->MemMap.u32FWBinaryAddr, pCtrl->MemMap.u32FWBinarySize,
         (unsigned long)pCtrl->MemMap.u32VLCBinaryAddr, pCtrl->MemMap.u32VLCBinarySize,
         (unsigned long)pCtrl->MemMap.u32CodeBufAddr, pCtrl->MemMap.u32CodeBufSize,
         (unsigned long)pCtrl->MemMap.u32FrameBufAddr, pCtrl->MemMap.u32FrameBufSize,
         (unsigned long)pCtrl->MemMap.u32BitstreamBufAddr, pCtrl->MemMap.u32BitstreamBufSize,
         (unsigned long)pCtrl->MemMap.u32DrvProcessBufAddr, pCtrl->MemMap.u32DrvProcessBufSize);
    ...
    switch (eCmd)
    {
        case E_HVD_CHECK_CMD_INIT:
        {
            if (!(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u8TurboInit & E_HVD_EX_TURBOINIT_CHECK))
            {
                ...
                {
                    HVD_EX_MSG_ERR("Init: main type can not be recognized:%d\n", u32Tmp0);
                    return E_HVD_EX_RET_INVALID_PARAMETER;
                }
            ...
            if (u32Tmp0 != HVD_INIT_MAIN_LIVE_STREAM)
            {
                if (u32Tmp1 == HVD_INIT_INPUT_DRV)
                {
                    ...
                    {
                        HVD_EX_MSG_ERR
                            ("Init: File mode(TS or MP4) and Drv input must set the process buffer size and must be larger than %u Bytes:%d\n",
            ...
            if (u8BitMiuSel != u8CodeMiuSel)
            {
                    ...
                    (HVD_BBU_ST_ADDR_IN_BITSTREAMBUF + (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_BBU_TBL_ENTRY_NUMB) + 8)))
                {
                    ...
                    if (((pCtrl->u32BBUTblInBitstreamBufAddr + pCtrl->MemMap.u32DrvProcessBufAddr) % 8) != 0)
                    {
                        ...
                        pCtrl->u32BBUTblInBitstreamBufAddr -= pCtrl->MemMap.u32DrvProcessBufAddr;
                        HVD_EX_MSG_ERR
                            ("Nal table in bitstream buffer start address is not aligned. old:%x new:%x",
                ...
                {
                    HVD_EX_MSG_ERR
                        ("Driver process buffer size is not enough for driver input path. input:%x required:%lx",
...
static HVD_EX_Result _HVD_EX_CheckFWVersion(MS_U32 u32Id)
{
    ...
    if (!(pCtrl->InitParams.u8TurboInit & E_HVD_EX_TURBOINIT_CHECK))
    {
        ...
        {
            HVD_EX_MSG_ERR("fw interface and binary is not the same. fw ID: interface:%x binary:%lx\n",
                        (MS_U32) HVD_FW_IF_VERSION, (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_IF_VERSION_ID));
            return E_HVD_EX_FAIL;
        }
    }
...
static MS_BOOL _HVD_EX_IsAllBufferEmpty(MS_U32 u32Id, MS_BOOL bFlush)
{
    ...
    if (u32PlaybackType == HVD_INIT_MAIN_FILE_RAW)
    {
        ...
        if (!HAL_VPU_EX_IsVBBUEmpty(u32Id, u32VBBUAddr))
        #else
    ...
    {
        #ifdef VDEC3
        //if ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DEC_Q_NUMB) == 0) && HAL_HVD_EX_DispFrameAllViewed(u32Id) && (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_FLUSH_STATUS) == E_HVD_EX_FLUSH_DONE || HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_PLAYBACK_FINISH)))
...
static void _HVD_EX_ISRHandler(void)
{
    ...
    for (u8Idx = 0; u8Idx < HVD_MAX_STREAMS; u8Idx++)
    {
        if (pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_HVD)
        {
            ...
            if (pfnCb)
            {
                ...
                pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.u32ISRInfo = HAL_HVD_EX_GetData(pHVDDrvContext->gHVDCtrl_EX[u8Idx].u32Sid, E_HVD_GDATA_HVD_ISR_STATUS);
    ...
    HAL_HVD_EX_SetClearISR(REG_HVD_BASE);

    if (TRUE == OSAL_HVD_ISR_Enable(HVD_ISR_VECTOR))//enable cpu interrupt mask
    {
        bHVDDisableISRFlag = FALSE;
    }
}
...
static void _EVD_EX_ISRHandler(void)
{
    ...
    HAL_HVD_EX_SetClearISR(REG_EVD_BASE);

    if (TRUE == OSAL_HVD_ISR_Enable(EVD_ISR_VECTOR))//enable cpu interrupt mask
    {
        bEVDDisableISRFlag = FALSE;
    }
}
...
static void _EVDLITE_EX_ISRHandler(void)
{
    ...
    for (u8Idx = 0; u8Idx < HVD_MAX_STREAMS; u8Idx++)
    {
        if (pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVDLITE)
        {
            pfnCb = pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.pfnISRCallBack;
            ...
            if (pfnCb)
            {
                ...
                if (pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.u32ISRInfo)
                {
                    #if defined(CHIP_KANO) || defined(CHIP_KASTOR) || defined(CHIP_CURRY)
                    //make fake stream id to disable/enable ISR.(only for calling HAL_HVD_EX_EnableISR)
    ...
    HAL_HVD_EX_SetClearISR(REG_EVD_LITE_BASE);

    if (TRUE == OSAL_HVD_ISR_Enable(EVD_LITE_ISR_VECTOR))//enable cpu interrupt mask
    {
        bEVDLITEDisableISRFlag = FALSE;
    }
}
#endif
...
HVD_EX_Result _HVD_EX_Rst(MS_U32 u32Id, MS_BOOL bErrHandle)
{
    ...
    MS_U8 u8Cidx = HAL_HVD_EX_GetCidx(u32Id);
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
...
#if defined(HIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(HAL_CHIP_SUPPORT_EVD)
    MS_BOOL bUsed;
    #if defined(CHIP_KANO) || defined(CHIP_CURRY)
    if (u8Cidx == HWDEC_EVD_LITE)
    {
        bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_EVDLITE);
    }
    else
    #endif
    if (((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC) || ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9))
    {
...
static HVD_EX_Result _HVD_EX_GetDispInfo(MS_U32 u32Id, HVD_EX_DispInfo *pInfo, MS_BOOL bClearSeqChg)
{
  ...
  HVD_GETDISPINFO_START:
    ...
    {
        HVD_EX_MSG_DBG("u32FrameRate=%u, u8Interlace=%x, u16HorSize=%u, u16VerSize=%u, \
...
void MDrv_HVD_EX_SetOSRegBase(MS_VIRT u32RegBaseAddr)
{
    ...
    #else
    HVD_EX_MSG_DBG("u32RiuBaseAddr=%x\n", u32RegBaseAddr);
    #endif
...
void MDrv_HVD_EX_MJPEG_InitSharemem(MS_U32 u32Id, MS_VIRT u32CodeBufVAddr, MS_BOOL bPreConnEnable, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH eMvopPath, HVD_EX_Original_Stream eStream)
{
    ...
    pCtrl1->InitParams.u32ModeFlag |=HVD_INIT_HW_MJPEG;
...
void MDrv_HVD_EX_ForceSwRst(MS_U32 u32Id)
{
    HAL_VPU_EX_ForceSwRst(u32Id);
}

// Action
#ifdef VDEC3
HVD_EX_Result MDrv_HVD_EX_GetFreeStream(MS_U32 *pu32Id, HVD_EX_DRV_StreamType eStreamType, MS_BOOL bIsEVD, MS_BOOL bIsNStreamMode)
#else
HVD_EX_Result MDrv_HVD_EX_GetFreeStream(MS_U32 *pu32Id, HVD_EX_DRV_StreamType eStreamType, MS_BOOL bIsEVD)
#endif
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U32 i = 0;
    ...
    {
#ifdef VDEC3
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_MAIN_STREAM, bIsEVD, bIsNStreamMode);
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_MAIN_STREAM, bIsEVD, bIsNStreamMode);
#else
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_MAIN_STREAM, bIsEVD);
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_MAIN_STREAM, bIsEVD);
#endif
    ...
    {
#ifdef VDEC3
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_SUB_STREAM, bIsEVD, bIsNStreamMode);
#else
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_SUB_STREAM, bIsEVD);
#endif
...
#ifdef VDEC3
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_SUB_STREAM, bIsEVD, bIsNStreamMode);
#else
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_SUB_STREAM, bIsEVD);
#endif
    ...
    {
#ifdef VDEC3
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_MVC_STREAM, bIsEVD, bIsNStreamMode);
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_MVC_STREAM, bIsEVD, bIsNStreamMode);
#else
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_MVC_STREAM, bIsEVD);
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_MVC_STREAM, bIsEVD);
#endif
    ...
    {
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_N_STREAM + (eStreamType - E_HVD_EX_DRV_N_STREAM), bIsEVD, bIsNStreamMode);
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_N_STREAM + (eStreamType - E_HVD_EX_DRV_N_STREAM), bIsEVD, bIsNStreamMode);
...
#endif
{
    ...
    if (u32UartCtrl & E_HVD_UART_CTRL_INFO)
    {
        ...
        HVD_EX_MSG_DBG("MSOS API check: 1st:%u 2nd:%u delay:%u dif:%u\n",
    ...
    #endif
    if (((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC) || ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9))
    {
        bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_EVD);
    }
    else
    ...
    VPRINTF("[NDec][0x%x][%d] bShareBBU %d, eStream %d \n", u32Id, u8DrvId, bShareBBU, eStream);
#endif
    ...
    if (u8Cidx == HWDEC_EVD_LITE)
    {
        pCtrl->HVDISRCtrl.eHWDecIsr = E_HWDEC_ISR_EVDLITE;
    }
    else
#endif
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set u32FrmBuffAddr = 0x%X\n", (MS_U32) u32FrmBuffAddr);
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set FrmBuffSize = 0x%X\n", (MS_U32) u32FrmBuffSize);
...
HVD_EX_Result MDrv_HVD_EX_SetMaxCMASize(MS_U32 u32Id, MS_U32 u32MaxCMASize, MS_U32 u32MaxCMASize2)
{
    ...
    HVD_EX_MSG_DBG("Drv: set max CMA Size=0x%x Size2=0x%x\n", u32MaxCMASize, u32MaxCMASize2);
...
HVD_EX_Result MDrv_HVD_EX_SetDVXCShmBuff(MS_U32 u32Id, MS_PHY u32FWBaseAddr, MS_PHY u32DVXCShmAddr, MS_SIZE u32DVXCShmSize)
{
    ...
    HVD_PRINT("Drv: SetDVXCShmAddr=0x%llx FWBaseAddr=0x%llx SetDVXCShmSize=0x%x\n",
        (unsigned long long int) u32DVXCShmAddr, (unsigned long long int)u32FWBaseAddr, u32DVXCShmSize);
...
HVD_EX_Result MDrv_HVD_EX_SetFrmBuff2(MS_U32 u32Id, MS_PHY u32FrmBuffAddr, MS_U32 u32FrmBuffSize)
{
    ...
    u32MIUSel = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);
#if defined(CHIP_MONET) || defined(CHIP_KANO) || defined(CHIP_CURRY)
...
//-----------------------------------------------------------------------------
void MDrv_HVD_EX_SetDbgLevel(MS_U32 u32Id, HVD_EX_UartLevel eLevel)
{
    ...
    switch (eLevel)
    {
        ...
        case E_HVD_EX_UART_LEVEL_FW:
        {
            ...
            HAL_HVD_EX_UartSwitch2FW(u32Id, TRUE);
            break;
        }
        default:
        {
...
HVD_EX_Result MDrv_HVD_EX_SetISREvent(MS_U32 u32Id, MS_U32 u32Event, HVD_InterruptCb fnISRHandler)
{
    ...
    if (u32Event == E_HVD_EX_ISR_NONE)
    {
        ...
        if (pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_HVD)  // hvd case
        {
            ...
            if ((bHVDDisableISRFlag != TRUE) && (FALSE == bUsed))
            {
#if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(HAL_CHIP_SUPPORT_EVD)
                HAL_HVD_EX_EnableISR(u32Id, FALSE);
#else
                HAL_HVD_EX_EnableISR(FALSE);
#endif
                bHVDDisableISRFlag = OSAL_HVD_ISR_Disable(HVD_ISR_VECTOR);
                ...
                {
                    OSAL_HVD_ISR_Detach(HVD_ISR_VECTOR);
        ...
        else if (pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVD)  // evd case
        {
            ...
            if ((bEVDDisableISRFlag != TRUE) && (FALSE == bUsed))
            {
#if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(HAL_CHIP_SUPPORT_EVD)
                HAL_HVD_EX_EnableISR(u32Id, FALSE);
#else
                HAL_HVD_EX_EnableISR(FALSE);
#endif
                bEVDDisableISRFlag = OSAL_HVD_ISR_Disable(EVD_ISR_VECTOR);
                ...
                {
                    OSAL_HVD_ISR_Detach(EVD_ISR_VECTOR);
        ...
        #if defined(CHIP_KANO) || defined(CHIP_CURRY)
        else if (pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVDLITE)  // evdlite case
        {
            for (i = 0; i < HVD_MAX_STREAMS; i++)
            {
                if (i != u8DrvId)
                {
                    if ((E_HWDEC_ISR_EVDLITE == pHVDDrvContext->gHVDCtrl_EX[i].HVDISRCtrl.eHWDecIsr)
                     && (TRUE == pHVDDrvContext->gHVDCtrl_EX[i].bUsed))
                    {
                        bUsed = TRUE;
                        break;
            ...
            if ((bEVDLITEDisableISRFlag != TRUE) && (FALSE == bUsed))
            {
#if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(HAL_CHIP_SUPPORT_EVD)
                HAL_HVD_EX_EnableISR(u32Id, FALSE);
#else
                HAL_HVD_EX_EnableISR(FALSE);
#endif
                bEVDLITEDisableISRFlag = OSAL_HVD_ISR_Disable(EVD_LITE_ISR_VECTOR);
                if (bEVDLITEDisableISRFlag == FALSE)
                {
                    HVD_EX_MSG_ERR("%s Disable Interrupt Fail\n", __FUNCTION__);
                }

#if !defined(SUPPORT_X_MODEL_HVD_FEATURE) && !defined(DISABLE_ISR_DETACH)
                if (TRUE == bEVDLITEIsIsrAttached)
                {
                    OSAL_HVD_ISR_Detach(EVD_LITE_ISR_VECTOR);
                    bEVDLITEIsIsrAttached = FALSE;
                }
#endif
            }
        }
    ...
    else
    {
        if (fnISRHandler != NULL)
        {
            ...
            {
                MS_U32 u32IsrVector = HVD_ISR_VECTOR;
                pCtrl->HVDISRCtrl.pfnISRCallBack = (HVD_ISRCallBack) fnISRHandler;

                if (E_HWDEC_ISR_HVD == pCtrl->HVDISRCtrl.eHWDecIsr)
                {
                    u32IsrVector = HVD_ISR_VECTOR;

                    if (FALSE == bHVDIsIsrAttached)
                    {
                        if (OSAL_HVD_ISR_Attach(u32IsrVector, (void*)_HVD_EX_ISRHandler) != TRUE)
                        {
                            HVD_EX_MSG_ERR("fail to attach _HVD_EX_ISRHandler!\n");
                            return E_HVD_EX_FAIL;
                        }
                        HVD_PRINT("attach ISR number:%d\n", HVD_ISR_VECTOR);
                        bHVDIsIsrAttached = TRUE;
                    }
                }
                #if SUPPORT_EVD
                else if (E_HWDEC_ISR_EVD == pCtrl->HVDISRCtrl.eHWDecIsr)
                {
                    u32IsrVector = EVD_ISR_VECTOR;

                    if (FALSE == bEVDIsIsrAttached)
                    {
                        if (OSAL_HVD_ISR_Attach(u32IsrVector, (void*)_EVD_EX_ISRHandler) != TRUE)
                        {
                            HVD_EX_MSG_ERR("fail to attach _EVD_EX_ISRHandler!\n");
                            return E_HVD_EX_FAIL;
                        }
                        HVD_PRINT("attach ISR number:%d\n", EVD_ISR_VECTOR);
                        bEVDIsIsrAttached = TRUE;
                    }
                }
                #if defined(CHIP_KANO) || defined(CHIP_CURRY)
                else if (E_HWDEC_ISR_EVDLITE == pCtrl->HVDISRCtrl.eHWDecIsr)
                {
                    u32IsrVector = EVD_LITE_ISR_VECTOR;

                    if (FALSE == bEVDLITEIsIsrAttached)
                    {
                        if (OSAL_HVD_ISR_Attach(u32IsrVector, (void*)_EVDLITE_EX_ISRHandler) != TRUE)
                        {
                            HVD_EX_MSG_ERR("fail to attach _EVDLITE_EX_ISRHandler!\n");
                            return E_HVD_EX_FAIL;
                        }
                        HVD_PRINT("attach ISR number:%d\n", EVD_LITE_ISR_VECTOR);
                        bEVDLITEIsIsrAttached = TRUE;
                    }
                }
                #endif
                #endif

                if (OSAL_HVD_ISR_Enable(u32IsrVector) != TRUE)
                {
        ...
        {
            HVD_EX_MSG_ERR("SetISREvent with NULL pointer. ISR type:%u\n", u32Event);
            eRet = E_HVD_EX_RET_INVALID_PARAMETER;
        }
    }
...
//                            (u8FB2MiuSel << VDEC_LUMA2_MIUSEL) |
    ...
    if (u32ErrCode != 0)
    {
        ...
        {
            HVD_EX_MSG_ERR
                ("FW error that Out of Memory:%x. Allocated frame buffer size is smaller than required.\n",
                 u32ErrCode);
            pHVDDrvContext->gHVDCtrl_EX[u8DrvId].bIsDispInfoChg = FALSE;
            return E_HVD_EX_RET_OUTOF_MEMORY;
        }
...
MS_U32 MDrv_HVD_EX_GetBBUVacancy(MS_U32 u32Id)
{
    ...
    return HAL_VPU_EX_GetVBBUVacancy(u32Id, u32VBBUAddr);
#else
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
MS_VIRT MDrv_HVD_EX_GetUserData_Packet(MS_U32 u32Id, MS_U32 u32Idx, MS_U32 *u32Size)
{
    ...
    *u32Size = 0;
    ...
    if (u32Idx >= tmp)
    {
        HVD_EX_MSG_ERR("input user data index(%u) is larger than max index(%lu)\n", u32Idx, (unsigned long)tmp);
        return 0;
    }
    ...
    switch (eType)
    {
        case E_HVD_EX_DS_BUF_MIUSEL:
            ...
            else
            {
                ...
                _phy_to_miu_offset(u8MiuSel, u32StartOffset, pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr);
                /* comment the dead code, the u8MiuSel must be MIU_0 or MIU_1
                if(u8MiuSel >= E_CHIP_MIU_2)
                {
...
/// @param -u8Interlace \b IN : output scan type: 0:progress, 1:interlace field, 2: interlace frame.
...
MS_BOOL MDrv_HVD_SetSTCMode(MS_U32 u32Id, MS_U32 STCindex)
{
    return HAL_VPU_EX_SetSTCMode(u32Id,STCindex);
}

MS_BOOL MDrv_HVD_SetDecodeMode(MS_U32 u32Id, HVD_EX_DecModCfg *pstCfg)
{
    MS_U8 i;
    ...
    return HAL_VPU_EX_SetDecodeMode(u32Id, &stVpuDecModCfg);
}
...
{
#if (defined(CHIP_EDISON) || defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3)) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MUJI) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(CHIP_MANHATTAN) || defined(CHIP_KANO) || defined(CHIP_CURRY)
...
#define SIZE_3K 0xC00
    ...
    #if defined(CHIP_MONACO) || defined(CHIP_MUJI) || defined(CHIP_MONET) || defined(CHIP_MANHATTAN) || defined(CHIP_KANO) || defined(CHIP_CURRY)
...
static void _HVD_EX_CC_Parse_CC_data(MS_U32 u32Id, MS_U8 u8CCMode,MS_U8 **pSrc, MS_U8 *pDst608, MS_U32 *pu32Dst608Len, MS_U8 *pDst708, MS_U32 *pu32Dst708Len, HVD_CC_608EnhanceInfo *pEhInfo608)
{
    ...
    for (i=0;i<u32CC_Count;i++)
    {
        ...
        if ((u8CC_Type == 0) || (u8CC_Type == 1))
        {
            ...
            if(pCCInfo->b608InfoEnhance)
            {
                ...
                {
                    HVD_EX_MSG_ERR("u32Dst608Len+16: %d is larger than MAX_608_CC_LEN !! \n",((*pu32Dst608Len)+16));
                    return;
                }
            }
            else
            {
                ...
                {
                    HVD_EX_MSG_ERR("u32Dst608Len+4: %d is larger than MAX_608_CC_LEN !! \n",((*pu32Dst608Len)+4));
                    return;
                }
            }
        ...
        else // ATSC_CC 708
        {
            ...
            {
                HVD_EX_MSG_ERR(" u32Dst708Len+4: %d is larger than MAX_708_CC_LEN !! \n",((*pu32Dst708Len)+4));
                return;
            }
    ...
    if(u8CCMode & (CC_NTSC1|CC_NTSC2))
    {
        ...
        {   // for original cc-608 data dump, 16bytes/pkt
            ...
            {
                HVD_EX_MSG_ERR("u32Dst608Len:%d + u32Pad: %d is larger than MAX_608_CC_LEN !! \n",(*pu32Dst608Len), u32Pad);
                return;
            }
            for(i=0;i<u32Pad;i++)
                pDst608[(*pu32Dst608Len)++] = 0xff;
...
static MS_BOOL _HVD_EX_CC_Copy_CCdata_To_RB(MS_U32 u32Id, MS_U8 u8CC608,MS_U8 *pSrcBuf, MS_U32 u32SrcLen)
{
    ...
    {
        HVD_EX_MSG_ERR("overflow !!!, u32RingBufVacancy: %d, u32AlignLen: %d\n",pCCInfo->u32RingBufVacancy[u8CC608], u32AlignCCLen);
        return TRUE;
    }
...
static MS_BOOL _HVD_EX_CC_Add_PaddingData_To_608(MS_U32 u32Id, MS_U8 *pSrcBuf, MS_U32 u32SrcLen)
{
    ...
    {
        HVD_EX_MSG_ERR("u32AlignCCLen:%d > MAX_608_CC_LEN", u32AlignCCLen);
        return TRUE;
    }
...
HVD_EX_Result MDrv_HVD_EX_CC_Adv_RB_ReadAddr(MS_U32 u32Id, MS_U32 u32EachPacketSize, MS_U8 u8CC608)
{
    ...
    {
        HVD_EX_MSG_ERR("MDrv_HVD_CC_Adv_RB_ReadAddr fail!, no enough data!, vacancy: %d, u32ParsedDataSize: %d, u32RingBufLen:%d, u32RingBufRPAddr: 0x%lx, u32RingBufWPAddr: 0x%lx\n",
                        pCCInfo->u32RingBufVacancy[u8CC608], u32ParsedDataSize, pCCInfo->u32RingBufLen[u8CC608], (unsigned long)pCCInfo->u32RingBufRPAddr[u8CC608],(unsigned long)pCCInfo->u32RingBufWPAddr[u8CC608]);
        return E_HVD_EX_FAIL;
    }
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_DirectSTCMode()
/// @brief \b Function \b Description: enable/disable direct stc mode with u32StcInMs
/// @param -u32StcInMs \b IN : enable with stc value/disable
/// @return -E_HVD_EX_OK for success; E_HVD_EX_FAIL for failure.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_DirectSTCMode(MS_U32 u32Id, MS_U32 u32StcInMs)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    ...
    if (HWDEC_EVD_LITE == HAL_HVD_EX_GetCidx(u32Id))
    {
        if (u32StcInMs == 0xFFFFFFFF)
        {
            HVD_EX_MSG_DBG("[EVD LITE] disable direct stc mode \n");
            eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DIRECT_STC_MODE, FALSE);
            pCtrl->bDirectSTCModeEnabled = FALSE;
        }
        else
        {
            pShm->u32DirectStcInMs = u32StcInMs;
            MsOS_FlushMemory();

            if (pCtrl->bDirectSTCModeEnabled == FALSE)
            {
                HVD_EX_MSG_DBG("[EVD LITE] enable direct stc mode \n");
                eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DIRECT_STC_MODE, TRUE);
                pCtrl->bDirectSTCModeEnabled = TRUE;
            }
        }
    }
    else
    {
        // only Kano HWDEC_EVD_LITE need this patch
    }
...
HVD_EX_Result MDrv_HVD_EX_PreSetDisplayMode(MS_U32 u32Id, HVD_EX_Display_mode eDisplayMode)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    ...
    switch(eDisplayMode)
    {
        case E_HVD_EX_DISPLAY_MODE_MCU:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eDisplayMode = E_HVD_DISPLAY_MODE_MCU;
            _DRV_HVD_SetCtrl(u8DrvId, HVD_CTRL_DISP_OUTSIDE);
            break;
        case E_HVD_EX_DISPLAY_MODE_HARDWIRE:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eDisplayMode = E_HVD_DISPLAY_MODE_HARDWIRE;
            break;
        default:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eDisplayMode = E_HVD_DISPLAY_MODE_DEFAULT;
            break;
    }
...
HVD_EX_Result MDrv_HVD_EX_PreSetVdecFeature(MS_U32 u32Id, HVD_EX_Feature eVdecFeature)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].eVdecFeature = eVdecFeature;
    HVD_EX_MSG_DBG("MDrv_HVD_EX_PreSetVdecFeature 0x%x\n", eVdecFeature);
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}
...
HVD_EX_Result MDrv_HVD_EX_PreSetNDSMode(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        ...
        eRet = (HVD_EX_Result)HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_NDS_AVC_FAST_FLUSH, 1);
    }
...
HVD_EX_Result MDrv_HVD_EX_PUSI_Control(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    if(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PUSI_CONTROL, bEnable) != E_HVD_RETURN_SUCCESS)
    {
       eRet = E_HVD_EX_FAIL;
    }
...
HVD_EX_Result MDrv_HVD_EX_PVRTimeShiftSeamlessMode(MS_U32 u32Id, MS_U8 u8Arg)
{
    ...
    if ((u8Arg == 1) || (u8Arg == 2))  //u8Arg=1 needs about 50ms, u8Arg=2 needs about 30ms
    {
        ...
        HVD_EX_MSG_DBG("TS Seamless. Arg=%d, polling status start at %d \n", u8Arg, HVD_GetSysTime_ms());

        while (1)
        {
            if (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_TS_SEAMLESS_STATUS) & u32Status)
            {
                HVD_EX_MSG_DBG("TS Seamless status change done. Arg=%d. Status=0x%x at %d \n", u8Arg, u32Status, HVD_GetSysTime_ms());
                break;
            }
            ...
            if (u32Times > 500)
            {
                HVD_EX_MSG_ERR("Timeout!!! TS Seamless status change is not finished. Arg=%d. Status=0x%x at %d \n", u8Arg, u32Status, HVD_GetSysTime_ms());
                eRet = E_HVD_EX_FAIL;
                break;
            }
        }
...
MS_SIZE MDrv_HVD_EX_GetFrameBufferDefaultSize(HVD_EX_CodecType eCodecType)
{
    return HAL_VPU_EX_GetFrameBufferDefaultSize((VPU_EX_CodecType)eCodecType);
}
...
{
    return HAL_VPU_EX_GetCMAMemSize((VPU_EX_CodecType)eCodecType, (VPU_EX_SrcMode)eSrcMode, offset, length, total_length, unUseSize);
}
...
///@return status
/******************************************************************************/
MS_BOOL MDrv_HVD_EX_IsDispQueueEmpty(MS_U32 u32Id)
{
    if(HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DISP_Q_NUMB) == 0)
        return TRUE;
    else
        return FALSE;
}
...
MS_BOOL MDrv_HVD_EX_SetLockDownRegister(void* param,MS_U8 u8IsHVD)
{
    if(param == NULL)
    {
        return FALSE;
    }
    return HAL_VPU_EX_SetLockDownRegister(param,u8IsHVD);
}

#if (defined(MSOS_TYPE_OPTEE))
MS_BOOL MDrv_HVD_EX_OPTEE_SetRegister(void* param)
{
    if(param == NULL)
    {
        return FALSE;
    }
    return HAL_HVD_EX_SetLockDownOpteeRegister(param);
}
#endif
