...
/// Stop VPU
/******************************************************************************/
void MDrv_MJPEG_StopVPU(MS_U32 u32Id)
{
    VirtualHal_MJPEG_StopVPU(u32Id);
}
...
/// Release VPU
/******************************************************************************/
void MDrv_MJPEG_ReleaseVPU(MS_U32 u32Id)
{
    VirtualHal_MJPEG_ReleaseVPU(u32Id);
}
...
/******************************************************************************/
MS_BOOL MDrv_MJPEG_IsFirmwareReady(MS_U32 u32Id)
{
    ...
    while(u32Timeout)
    {
        if(VirtualHal_MJPEG_IsVPUMBoxReady(u32Id, VPU_RISC_MBOX0))
        {
            VirtualHal_MJPEG_ReadVPUMBox(u32Id, VPU_RISC_MBOX0, (MS_U32 *)&u32FirmVer);
            VirtualHal_MJPEG_ClearVPUMBox(u32Id, VPU_RISC_MBOX0);
            break;
        }
        u32Timeout--;
    }
...
static MS_BOOL MDrv_MJPEG_SetVPUCmd(MS_U32 u32Id, MS_U32 u32Cmd)
{
    ...
    while(u32TimeOut)
    {
        if(VirtualHal_MJPEG_IsVPUMBoxReady(u32Id, VPU_HI_MBOX0))
        {
...
/******************************************************************************/
static MS_BOOL MDrv_MJPEG_SetVPUCmdArg(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    while(--u32TimeOut)
    {
        if( VirtualHal_MJPEG_IsVPUMBoxReady(u32Id, VPU_HI_MBOX0)&& VirtualHal_MJPEG_IsVPUMBoxReady(u32Id, VPU_HI_MBOX1))
        {
...
MS_U32 MDrv_MJPEG_SendVPUCommand(MS_U32 u32Id, MJPEG_User_Cmd eCmd, MS_VIRT u32Arg)
{
    ...
    if (bDisplayOutSideMode[u8DrvId])
    {
        ...
        case E_MJPEG_CMD_SET_DISPLAY_OUTSIDE_MODE:
            ...
            MDrv_MJPEG_SetVPUCmdArg(u32Id, u32Arg);
        ...
        case E_MJPEG_CMD_FREE_FRAME_BUFFER:
            MDrv_MJPEG_SetVPUCmdArg(u32Id, 0);
...
void MDrv_MJPEG_EnableTurboFWMode(MS_U32 u32Id, MS_BOOL bEnable)
{
//    gbEnableTurboFWMode = bEnable;
    HAL_VPU_EX_SetFWReload(u32Id, !bEnable);
}
...
MJPEG_Return MDrv_MJPEG_GetFreeStream(MS_U32 *pu32Id, MJPEG_DRV_StreamType eStreamType)
{
    ...
    u32VPUStreamId   = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_N_STREAM + i, FALSE, FALSE);
    ...
    {
        u32VPUStreamId   = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_MAIN_STREAM, FALSE);
    ...
    {
        u32VPUStreamId   = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_SUB_STREAM, FALSE);
