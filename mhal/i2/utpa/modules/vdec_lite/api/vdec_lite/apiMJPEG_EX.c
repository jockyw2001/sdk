...
#endif
{
    ...
    if(MApi_JPEG_IsNJPD()==TRUE)
    {
        if(pInitParam->u32MWCBufSize >= MJPEG_MAX_SUPPORT_WIDTH*2*MJPEG_MAX_SUPPORT_HEIGHT*DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
        }
        else
    ...
    {
        if(pInitParam->u32MWCBufSize >= MJPEG_SD_WIDTH*2*MJPEG_SD_HEIGHT*DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
        }
        else
...
#endif
#endif  //#if (!defined(MSOS_TYPE_NUTTX) && !defined(MSOS_TYPE_OPTEE)) || defined(SUPPORT_X_MODEL_FEATURE)