...
#define MASK_CLK_ISP_MLOAD                  0x001F
#define SHIFT_CLK_ISP_MLOD_DOMAIN(a)        ((a)<<2)
#define SHIFT_CLK_ISP_MLOD_ENABLE(a)        ((a)<<0)
...
#define MLOAD_TBL_DATA_WIDTH(mload_mem_cfg,id) (mload_mem_cfg->tblSramWidth[id])

typedef enum
{
    eMLOAD_SUCCESS = SUCCESS,
    eMLOAD_FAIL = FAIL,
    eMLOAD_NO_CHANGE = 1,
    eMLOAD_TIMEOUT = 2,
}HalMloadErrCode_e;
...
//int SCL_MLoadRead(int nDataWidth,FrameSyncMode mode,int id,int sram_offset,ISP_MLOAD_OUTPUT *output);
