...
int DrvIsp_MLoadInit(MLOAD_HANDLE *handle,MLOAD_ATTR attr)
{
  ...
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE0_FPN] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE0_GMA16TO16] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE0_ALSC] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE1_FPN] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE1_GMA12TO12] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_PIPE1_ALSC] = ISP_MLOAD_DOMAIN_FRONT;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_DPC] = ISP_MLOAD_DOMAIN_IMAGE;
  isp->mload_mem_cfg->eTarget[MLOAD_ID_GMA10TO10] = ISP_MLOAD_DOMAIN_IMAGE;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_YUV_GAMMA_Y] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_YUV_GAMMA_UV] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_RGB12TO12_PRE] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_RGB12TO12_POST] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_WDR] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_C_SC1] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_Y_SC1] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_C_SC1] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_Y_SC1] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_C_SC2] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_Y_SC2] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_C_SC2] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_Y_SC2] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_C_SC3] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_Y_SC3] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_C_SC3] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_Y_SC3] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_C_SC4] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_HSP_Y_SC4] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_C_SC4] = SCL_MLOAD;
  isp->mload_mem_cfg->eTarget[SCL_MLOAD_ID_VSP_Y_SC4] = SCL_MLOAD;
...
const u16* DrvIsp_MLoadTableGet(MLOAD_HANDLE handle, IspMloadID_e idx)
{
    mload_handle *isp = (mload_handle*)handle;
    if(isp)
        return ISP_MLoadTableGet(idx,isp->mload_mem_cfg);
    else
        return NULL;
}

MloadCmdqIf_t* DrvIsp_MLoadChangeCmdQ(MLOAD_HANDLE *handle,MloadCmdqIf_t *pNewCmdQ)
{
    MloadCmdqIf_t *pRet;
    mload_handle *isp = (mload_handle*)handle;
    pRet = isp->pCmqInterface_t;
    isp->pCmqInterface_t = pNewCmdQ;
    if(pNewCmdQ)
        isp->mode = CMDQ_MODE;
    else
        isp->mode = RIU_MODE;
...
int DrvIsp_MLoadApply(MLOAD_HANDLE handle,IspMloadDomain_e domain)
{
    ...
    switch(domain)
    {
    case ISP_MLOAD_DOMAIN_FRONT:
        if(isp->mode == CMDQ_MODE)
        {
            ISP_PreWaitMloadDone(isp->pCmqInterface_t,isp->mode);
            ISP_MLoadSwitchClk(isp->pCmqInterface_t,isp->mode,0,1);
            if( eMLOAD_SUCCESS == ISP_MLoadApplyIspDomain(isp->pCmqInterface_t, isp->mload_mem_cfg,isp->mode,ISP_MLOAD_DOMAIN_FRONT))
                ISP_WaitMloadDone(isp->pCmqInterface_t,isp->mode,1);
            ISP_PostWaitMloadDone(isp->pCmqInterface_t,isp->mode);
        }
        else
        {
            ISP_MLoadSwitchClk(isp->pCmqInterface_t,isp->mode,0,1);
            ISP_MLoadApplyIspDomain(isp->pCmqInterface_t, isp->mload_mem_cfg,isp->mode,ISP_MLOAD_DOMAIN_FRONT);
        }
    break;
    case ISP_MLOAD_DOMAIN_IMAGE:
        ISP_MLoadSwitchClk(isp->pCmqInterface_t,isp->mode,1,1);
        ISP_PreWaitMloadDone(isp->pCmqInterface_t,isp->mode);
        if( eMLOAD_SUCCESS == ISP_MLoadApplyIspDomain(isp->pCmqInterface_t, isp->mload_mem_cfg,isp->mode,ISP_MLOAD_DOMAIN_IMAGE) )
            ISP_WaitMloadDone(isp->pCmqInterface_t,isp->mode,1);
        ISP_PostWaitMloadDone(isp->pCmqInterface_t,isp->mode);
        ISP_MLoadSwitchClk(isp->pCmqInterface_t,isp->mode,0,1);
    break;
...
{
    MLoadRead(MLOAD_BANK,nDataWidth,RIU_MODE, id, sram_offset, output);
    return SUCCESS;
}

int DrvScl_MLoadRead(int nDataWidth,int id,int sram_offset,ISP_MLOAD_OUTPUT *output)
{
    MLoadRead(SCL_MLOAD_BANK,nDataWidth,RIU_MODE, id, sram_offset, output);
    return SUCCESS;
}
...
void DrvIsp_MLoadDump(MLOAD_HANDLE handle,IspMloadID_e ID, short *buf)
{
    ...
    for(i=0,j=0; i< item_num; i++){
        ...
        DrvIsp_MLoadRead(isp->mload_mem_cfg->tblSramWidth[ID],ID,i,&mload_output);
...
void DrvScl_MLoadDump(MLOAD_HANDLE handle,IspMloadID_e ID, short *buf){
    ...
    for(i=0,j=0; i< item_num; i++){
        ...
        DrvScl_MLoadRead(isp->mload_mem_cfg->tblSramWidth[ID],ID,i,&mload_output);
