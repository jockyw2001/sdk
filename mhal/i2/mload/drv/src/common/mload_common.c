...
void MloadWarperInit(void){
    ...
    gpIoBase = (void*) ioremap(0x1F000000, 0x400000);
	pr_info("ISP Mload base remap: 0x%X\n",(int)gpIoBase);
    _gaddr_cnt = 0;

    return;
}

void MloadDeinit(void){
...
void MloadRegW(MHAL_CMDQ_CmdqInterface_t *pCmqInterface_t,FrameSyncMode mode,u32 uBank,u32 uOffset,u16 uVal,u16 Mask){
    ...
    if(mode == RIU_MODE){
        ...
        u16* reg = (unsigned short*)((unsigned long)gpIoBase + CPU_RIU_REG(uBank,uOffset));
        u16 val = *reg & ~Mask;
        val |= uVal&Mask;
        *reg = val;
        //*reg &= ~Mask;
        ...
        if(pCmqInterface_t != NULL){
			...
			//pr_info("MloadRegW with CMDQ, bank=0x%X,offset=%X,mask=0x%X,val=0x%X\n",uBank,uOffset,Mask,uVal);
...
u32 MloadPollRegBit(MHAL_CMDQ_CmdqInterface_t *pCmqInterface_t, u32 uBank,u32 uOffset, u16 uValue, u16 Mask, u32 bPollEq){
    ...
    if(pCmqInterface_t != NULL)
    {
        //pr_info("MloadPollWait with CMDQ, bank=0x%X,offset=%X,mask=0x%X,val=0x%X\n",uBank,uOffset,Mask,uValue);
