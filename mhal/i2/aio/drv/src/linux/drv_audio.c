...
#define NO_EXHAUST_DMA_BUF  ( 1 )
#define DMA_BUF_REMAINDER   ( 128 ) // 128 Bytes ( 8 MIU Line )

#define DMA_IRQ_REFINE      ( 1 )
...
static BOOL _AudDmaStopChannel(AudDmaChn_e eDmaChannel)
{
    ...
    // bRet &= HalAudDmaReset(eDmaChannel); // Move to DrvAudPrepareDma
...
static irqreturn_t  _AudDmaIrq(int irq, void *dev_id)
{
...
#if DMA_IRQ_REFINE
    BOOL bTriggerInt,bBoundaryInt,bLocalDataInt;
#endif
    ...
    HalAudDmaGetInt(ptDmaData->nChnId, &bTriggerInt, &bBoundaryInt, &bLocalDataInt);
...
s32 _AudDmaRdCopyI16(struct PcmRuntimeData *ptRuntimeData, u32 nHwoff, u8* pBuf, u32 nBufSize, u32 nPos, PcmFrames_t tFrames)
{
    ...
    if(nChannels==1)//hw only supports stereo
    {
        while(tFrames>0)
        {
...
s32 _AudDmaKernelCopy(struct PcmRuntimeData *ptRuntimeData, u32 nHwoff, u8* pBuf, u32 nBufSize, u32 nPos, PcmFrames_t tFrames)
{
    ...
    if (PCM_STREAM_PLAYBACK == ptRuntimeData->eStream)
    {
        ...
        if(nRetry==20)
        {
            ...
            {
                udelay(10);
                // When nTrigCount is small, it must occur nRetry timeout, and we don't want to show error message.
                // but we must ensure that level count has be updated, so we delay 10us.
            }
            else
            {
                AUD_PRINTF(ERROR_LEVEL,"Reader - update level count too slow!!!!!!!!\n");
            }
        }
    ...
    else if (PCM_STREAM_CAPTURE == ptRuntimeData->eStream)
    {
        ...
        if(nRetry==20)
        {
            ...
            {
                AUD_PRINTF(ERROR_LEVEL,"Writer - update level count too slow!!!!!!!!\n");
            }
        }
...
BOOL DrvAudConfigDmaParam(AudDmaChn_e eDmaChannel, DmaParam_t *ptParam)
{
    ...
    switch(ptParam->nSampleRate)
    {
        ...
        {
        if(ptParam->nChannels<2)//SW should support mono playback, but HW only support stereo
            {
    ...
    if(bRet)
    {
        ...
        if(ptParam->nChannels<2)//writer does not support mono, so only reader 1~4 should support sw mono
        {
