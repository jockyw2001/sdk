...
static WARP_DEV_PROC_STATE_E warp_dev_check_config(MHAL_WARP_CONFIG *config)
{
    ...
    if (CHECK_ALIGNEMT(input_data->data[MHAL_WARP_IMAGE_PLANE_RGBA]) ||
        CHECK_ALIGNEMT(output_data->data[MHAL_WARP_IMAGE_PLANE_RGBA]))
    {
    ...
    {
        if (CHECK_ALIGNEMT(input_data->data[MHAL_WARP_IMAGE_PLANE_UV]) ||
            CHECK_ALIGNEMT(output_data->data[MHAL_WARP_IMAGE_PLANE_UV]))
        {
            return WARP_DEV_PROC_STATE_ADDRESS_NOT_ALIGNED_ERROR;
        }
    }

    //check bb size from b.b header
    ...
    switch(op)
    {
        case MHAL_WARP_OP_MODE_MAP:
            ...
            if (CHECK_ALIGNEMT(disp_table->table))
            {
                return WARP_DEV_PROC_STATE_ADDRESS_NOT_ALIGNED_ERROR;
            }
...
void warp_dev_post_proc(struct work_struct *wq)
{
    ...
    {
        current_request->callback(handle, current_request->user_data, handle->state);
    }
