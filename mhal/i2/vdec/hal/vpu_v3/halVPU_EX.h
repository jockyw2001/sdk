...
#define VPU_ENABLE_IQMEM   1
#define VPU_IQMEM_BASE  0xe0000000


#define ENABLE_DECOMPRESS_FUNCTION          TRUE

#define VPU_CLOCK_576MHZ                BITS(6:2,1)
#define VPU_CLOCK_480MHZ                BITS(6:2,3)
#define VPU_CLOCK_432MHZ                BITS(6:2,6)
#define VPU_CLOCK_384MHZ                BITS(6:2,7)
#define VPU_ICG_EN                      BIT(8)
//#define VPU_LITE_ICG_EN                 BIT(9)
...
#ifdef VDEC3
#define VPU_BBU_NAL_TBL    BIT(0)
#define VPU_BBU_ES_BUFFER  BIT(1)
#define HAL_VPU_INVALID_BBU_ID 0xFFFFFFFF
#define VPU_MAX_DEC_NUM 32
#else
...
typedef enum
{
    E_VPU_EX_CLOCK_576MHZ   = VPU_CLOCK_576MHZ,
    E_VPU_EX_CLOCK_480MHZ   = VPU_CLOCK_480MHZ,
...
typedef enum
{
    E_VPU_DEC_MODE_DUAL_INDIE = 0,                     ///< Two independent tasks
    E_VPU_DEC_MODE_DUAL_3D    = 1,                     ///< Two dependent tasks for 3D
    E_VPU_DEC_MODE_SINGLE     = 2,                     ///< One task use the whole SRAM
    E_VPU_DEC_MODE_MVC        = E_VPU_DEC_MODE_SINGLE,
    E_VPU_DEC_MODE_AUTO_MCU   = 3,
} VPU_EX_DecMode;
...
/// record origin stream type for VPU hal
typedef enum
{
    E_VPU_ORIGINAL_MAIN_STREAM = 0,
    E_VPU_ORIGINAL_SUB_STREAM,
    E_VPU_ORIGINAL_N_STREAM,
} VPU_EX_Original_Stream;

typedef enum
{
    E_VPU_EX_DISP_SPEED_FF_32X     = 32,    ///< Speed fast forward 32x.
    E_VPU_EX_DISP_SPEED_FF_16X     = 16,    ///< Speed fast forward 16x.
    E_VPU_EX_DISP_SPEED_FF_8X      = 8,     ///< Speed fast forward 8x.
    E_VPU_EX_DISP_SPEED_FF_4X      = 4,     ///< Speed fast forward 4x.
    E_VPU_EX_DISP_SPEED_FF_2X      = 2,     ///< Speed fast forward 2x.
    E_VPU_EX_DISP_SPEED_NORMAL_1X  = 1,     ///< Normal display speed.
    E_VPU_EX_DISP_SPEED_SF_2X      = -2,    ///< Slow forward 2X.
    E_VPU_EX_DISP_SPEED_SF_4X      = -4,    ///< Slow forward 4X.
    E_VPU_EX_DISP_SPEED_SF_8X      = -8,    ///< Slow forward 8X.
    E_VPU_EX_DISP_SPEED_SF_16X     = -16,   ///< Slow forward 16X.
    E_VPU_EX_DISP_SPEED_SF_32X     = -32,   ///< Slow forward 32X.
} VPU_EX_DispSpeed;
...
typedef enum
{
    ...
    VPU_EX_SMOOTH_SCAN_NONE = 0,
    /// smooth scan by hw
    VPU_EX_SMOOTH_SCAN_HW,
    /// smooth scan by vdec plus
    VPU_EX_SMOOTH_SCAN_VDEC_PLUS,
    /// smooth scan is unsupport
    VPU_EX_SMOOTH_SCAN_UNSUPPORT,
} VPU_EX_SmoothScanMode;

typedef struct
{
    VPU_EX_DispSpeed eDispSpeed;//in
    VPU_EX_SmoothScanMode eSmoothScanMode;//out
    MS_BOOL bEnable;//inout
} VPU_EX_TrickSmoothScan;
