...
#define INTERFACE_VERSION        0x00000376  //add CMD_TOPFIELD_FIRST_CONTROL
...
//    D[0] vol_rvlc_enable;
    ...
    volatile unsigned char          u8DramObfRes0;            //117
    ...
    unsigned char       reserved[6];                           //138
    volatile unsigned char          u32VCHandshake;            //144 64bit dram obf need 128bits(16bytes) alignment
    unsigned char       u8DramObfRes1[15];                     //145
...
#define OFFSET_VCHANDSHAKE             144

typedef struct
{
    unsigned int mvdcmd_handshake_pause;        // 1 for handshake ready with CMD_PAUSE,
    unsigned int u32DramobfRes0[3];
    unsigned int mvdcmd_handshake_slq_reset;    // 1 for handshake ready with CMD_VC1_HW_SLQ_RESET
    unsigned int u32DramobfRes1[3];
    unsigned int mvdcmd_handshake_stop ;         // 1 for handshake ready with CMD_STOP
    unsigned int u32DramobfRes2[3];
    unsigned int mvdcmd_handshake_skip_data;    // 1 for handshake ready with CMD_SKIP_DATA
    unsigned int u32DramobfRes3[3];
    unsigned int mvdcmd_handshake_skip_to_pts;  // 1 for handshake ready with CMD_SKIP_TO_PTS
    unsigned int u32DramobfRes4[3];
    unsigned int mvdcmd_handshake_single_step;  // 1 for handshake ready with CMD_SINGLE_STEP
    unsigned int u32DramobfRes5[3];
    unsigned int mvdcmd_handshake_scaler_data_ready;
    unsigned int u32DramobfRes6[3];
    unsigned int mvdcmd_handshake_get_nextdispfrm_ready; // for Mstreamer mode
    unsigned int u32DramobfRes7[3];
    unsigned int mvdcmd_handshake_parser_rst;  // 1 for handshake done with CMD_PTS_TBL_RESET
    unsigned int u32DramobfRes8[3];
    unsigned int mvdcmd_handshake_cc608_rst;    // "0" for handshake done with mstar cc608
    unsigned int u32DramobfRes9[3];
    unsigned int mvdcmd_handshake_cc708_rst;    // "0" for handshake done with mstar cc708
    unsigned int u32DramobfRes10[3];
    unsigned int mvdcmd_handshake_fast_rst;     // 1 for handshake done with CMD_FAST_RST, 1 for fast_rst_done...
    unsigned int u32DramobfRes11[3];
    unsigned int mvdcmd_handshake_detatch;      // 1 for handshake ready with exit main loop
    unsigned int u32DramobfRes12[3];
    unsigned int mvdcmd_handshake_pvr_seamless_mode;      // 1 for handshake ready
    unsigned int u32DramobfRes13[3];
    unsigned int mvdcmd_handshake_virtualCommand;      // 1 for handshake ready with virtual command get status done
    unsigned int u32DramobfRes14[3];
    unsigned int mvdcmd_handshake_flush;      // 1 for handshake ready with flush disp queue command
    unsigned int u32DramobfRes15[3];
    unsigned int mvdcmd_handshake_vsync_control;      // 1 for handshake ready with flush disp queue command
    unsigned int u32DramobfRes16[3];
    unsigned int mvdcmd_handshake_set_playback_interval;
    unsigned int u32DramobfRes17[3];
...
typedef struct
{
    union
    {
        struct
        {
            ...
            unsigned int mvdcmd_handshake_set_playback_interval : 1;      // 1 for handshake ready with flush disp queue command
            unsigned int mvdcmd_handshake_reserved : 14;    // reserved for extend
        };
        unsigned int value;
    };
} MVD_CMD_HANDSHADE_STATUS;
...
typedef struct _FW_FRAME_INFO
{
    ...
    unsigned int            u32DramObfResv;              //80 not used
    ...
    unsigned int            u32ICount;                   //380
    unsigned int            u32PCount;                   //384
    unsigned int            u32BCount;                   //388
    unsigned int            u32VSYNCCount;               //392
    unsigned int            u32CreateTask;               //396
    unsigned int            u32Play;                     //400
    unsigned int            u32ReceiveData;              //404
    unsigned int            u32Seqheaderfound;           //408
    unsigned int            u32DecodeFirstI;             //412
    unsigned int            u32PushFirstI;               //416
    unsigned int            u32FirstVsync;               //420
    unsigned int            u32FirstFrameDisp;           //424
    unsigned char           u8TFF;                       //428
    unsigned char           u8FrameBufNum;               //429
    unsigned char           u8Bluescreen;                //430
    unsigned char           u8SeqChgCount;               //431
    MVD_CMD_HANDSHADE_INDEX cmd_handshake_index;         //432
    unsigned int            u32EsRdPtr;                  //720
    unsigned int            u32EsWrPtr;                  //724
...
#define OFFSET_IFRAMECOUNT                 380
#define OFFSET_PFRAMECOUNT                 384
#define OFFSET_BFRAMECOUNT                 388
#define OFFSET_VSYNCCOUNT                  392
#define OFFSET_CREATETASK                  396
#define OFFSET_PLAY                        400
#define OFFSET_RECEIVEDATA                 404
#define OFFSET_SEQHEADERFOUND              408
#define OFFSET_DECODEFIRSTI                412
#define OFFSET_PUSHFIRSTI                  416
#define OFFSET_FIRSTVSYNC                  420
#define OFFSET_FIRSTFRAMEDISP              424
#define OFFSET_TFF                         428
#define OFFSET_FBNUM                       429
#define OFFSET_BLUESCREEN                  430
#define OFFSET_SEQCHGCOUNT                 431
#define OFFSET_CMD_HANDSHAKE_INDEX         432
#define OFFSET_ES_RD_PTR                   720
#define OFFSET_ES_WR_PTR                   724
...
#define FW_VBBU VDEC_VBBU
#define pFW_VBBU pVDEC_VBBU
...
#define MVD_H263_ERR_1 1
#define MVD_H263_ERR_2 2
#define MVD_H263_ERR_3 3
#define MVD_H263_ERR_4 4
#define MVD_H263_ERR_5 5
#define MVD_H263_ERR_6 6
#define MVD_H263_ERR_7 7
#define MVD_H263_ERR_8 8
#define MVD_H263_ERR_9 9
#define MVD_H263_ERR_10 10
#define MVD_H263_ERR_11 11
#define MVD_H263_ERR_12 12
#define MVD_H263_ERR_13 13
#define MVD_H263_ERR_14 14
#define MVD_H263_ERR_15 15
#define MVD_H263_ERR_16 16

#define MVD_PROFILE_UNSUPPORT_1 1
#define MVD_PROFILE_UNSUPPORT_2 2
#define MVD_PROFILE_UNSUPPORT_3 3
#define MVD_PROFILE_UNSUPPORT_4 4
...
#define CMD_CUSTOMER_MODE              0xCB
#define CMD_DISABLE_FDMASK             0xCC
#define CMD_PLAYBACK_INTERVAL_END      0xCD
#define CMD_PLAYBACK_INTERVAL_START    0xCE
#define CMD_ERROR_CONCEALMENT_CONTROL  0xCF
#define CMD_ENABLE_FAST_FLUSH          0xD0
#define CMD_TOPFIELD_FIRST_CONTROL     0xD1
#define CMD_MPEG4_CAL_FRAMERATE        0xD2
#define CMD_SUPPORT_MULTI_ULTRA_RES    0xD3
