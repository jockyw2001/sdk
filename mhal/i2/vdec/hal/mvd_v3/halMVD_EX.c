...
#include "regMVD_EX.h"
#include "halMVD_EX.h"
#include "../vpu_v3/halVPU_EX.h"
#include "../../drv/mvd_v3/osalMVD_EX.h"
#include "mvd4_interface.h" //firmware header
#include "asmCPU.h"
#include "../vpu_v3/controller.h"
#include "../../drv/mvd_v3/MVD_EX_Common.h"
#include "halCHIP.h"
...
#define MVDHW_ON_MIU1      ((HAL_MVD_RegReadByte(MIU0_SEL2_L) & BIT6) == BIT6)
...
#define MVD_FW_DYN_SCALE_BUF_LEN            (0x2000UL)   //  8K
...
#define MVD_HW_MAX_PIXEL            (1920*1088*61000ULL) // FHD@60p
...
#define SET_CMD_RET_FALSE(_cmdVal, _pStcmdArg)                                  \
    do {                                                                        \
        ...
        {                                                                       \
            MVD_DEBUGERROR(MVD_PRINT("Command: 0x%x fail!!\r\n", _cmdVal));        \
            return FALSE;                                                       \
        }                                                                       \
    } while(0)
...
#define HAL_MVD_InvalidBuffRetFalse(x)                                                              \
    do {                                                                                            \
        if ((x)==0)                                                                                 \
        {                                                                                           \
            MVD_DEBUGERROR(MVD_PRINT("%s(%d) error: NULL buffer address.\n", __FUNCTION__, __LINE__)); \
            return FALSE;                                                                           \
        }                                                                                           \
    } while(0)
...
typedef struct
{
    ...
    MS_U32 u32PCounter;
    MS_U32 u32BCounter;
    MS_U32 u32VCounter;//vsync counter
    MS_U32 u32SCounter;//average slice counter
    MS_U8  u8SeqChgCounter;
} VDEC_EX_MDBCounter;

typedef struct
{
    MS_U32 u32CreateTask;
    MS_U32 u32Play;
    MS_U32 u32ReceiveData;
    MS_U32 u32SeqHeadFound;
    MS_U32 u32FirstIDecode;
    MS_U32 u32FirstFramePush;
    MS_U32 u32FirstVsync;
    MS_U32 u32FirstFrameDisp;
} VDEC_EX_MDBPerformance;
...
typedef struct
{
    ...
    MS_BOOL bTrickPlay2xAVSync[MAX_DEC_NUM];
    MS_BOOL bEnSmoothScan[MAX_DEC_NUM];
} MVD_Hal_CTX;
...
static MS_U32 MVD_GetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u16Offset, MS_U8 u8Size)
{
    ...
    switch (eBuffType)
    {
        case FW_BUFF_VOLINFO:
            u32BufStart = GET_VOL_BUFFADD(u8Idx);
            break;
        case FW_BUFF_FRMINFO:
            u32BufStart = GET_FRMINFO_BUFFADD(u8Idx);
            break;
        case FW_BUFF_HDR:
            u32BufStart = GET_HDR_BUFFADD(u8Idx);
            break;
        case FW_BUFF_USRDATA:
            u32BufStart = GET_USRDATA_BUFFADD(u8Idx);
            break;
        case FW_BUFF_FWSLQTAB:
            u32BufStart = GET_SLQ_BUFFADD(u8Idx);
            break;
        case FW_BUFF_PTSTBL:
            u32BufStart = GET_PTSTBL_BUFFADD(u8Idx);
            break;
        case FW_BUFF_DS:
            u32BufStart = GET_DS_BUFFADD(u8Idx);
            break;
        case FW_BUFF_XCINFO:
            u32BufStart = GET_XCINFO_BUFFADD(u8Idx);
            break;
        case FW_BUFF_DECFRMINFO:
            u32BufStart = GET_DECFRMINFO_BUFFADD(u8Idx);
            break;
        default:
            break;
    }
    if (NULL == u32BufStart)
    {
        MVD_PRINT("%s err: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("%s: u32Val=%d for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
                              __FUNCTION__, u32Val, u8Idx, eBuffType, u16Offset, u8Size));
    return u32Val;
}

#ifdef VDEC3
void MVD_SetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u16Offset, MS_U8 u8Size, MS_U32 u32Value)
{
    MS_VIRT u32BufStart = NULL;
    ...
    if (NULL == u32BufStart)
    {
        MVD_PRINT("%s err: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d, value=%d\n",
                  __FUNCTION__, (unsigned int)u8Idx, (unsigned int) eBuffType, (unsigned int) u16Offset, (unsigned int) u8Size, (unsigned int)u32Value);
    }
    ...
    if (u8Size == sizeof(MS_U8))
    {
        MS_U8* temp = (MS_U8*)MsOS_PA2KSEG1(u32BufStart + u16Offset);
        *temp = (MS_U8)u32Value;
    }
    else if (u8Size == sizeof(MS_U32))
    {
        MS_U32* temp = (MS_U32*)MsOS_PA2KSEG1(u32BufStart + u16Offset);
        *temp = (MS_U32)u32Value;
    }
    else if (u8Size == sizeof(MS_U16))
    {
        MS_U16* temp = (MS_U16*)MsOS_PA2KSEG1(u32BufStart + u16Offset);
        *temp = (MS_U16)u32Value;
    }

    MVD_DEBUGVERBAL(MVD_PRINT("%s: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d, value=%d\n",
                              __FUNCTION__,  (unsigned int)u8Idx, (unsigned int)eBuffType, (unsigned int)u16Offset, (unsigned int)u8Size, (unsigned int)u32Value));
}
#endif
...
{
    return (MS_U64)MVD_HW_MAX_PIXEL / 1000;
}
...
void HAL_MVD_MemGetMap(MS_U8 u8Idx, MS_U8 u8type, MS_VIRT* pu32addr, MS_SIZE* pu32len)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("HAL_MVD_MemGetMap[%d] add=0x%lx len=0x%lx\n", u8type, (unsigned long)*pu32addr, (unsigned long)*pu32len));
    return;
}
...
void* HAL_MVD_MemReadPoiter(MS_VIRT u32Address)
{
    ...
    return (void*) HAL_MVD_PA2NonCacheSeg(u32Address);
}
...
void HAL_MVD_SetReqMask(MS_BOOL bEnMask)
{
    ...
    if (MVDHW_ON_MIU1) // miu1
    {
        HAL_MVD_RegWriteBit(MIU1_RQ2_MASK_L, bEnMask, BIT6);  //MVD R/W
        HAL_MVD_RegWriteBit(MIU1_RQ0_MASK_H, bEnMask, BIT2);  //MVD bbu R/W
    }
    else // miu0
    {
        HAL_MVD_RegWriteBit(MIU0_RQ2_MASK_L, bEnMask, BIT6);  //MVD R/W
        HAL_MVD_RegWriteBit(MIU0_RQ0_MASK_H, bEnMask, BIT2);  //MVD bbu R/W
    }
    HAL_MVD_Delayms(1);
...
MS_BOOL HAL_MVD_RstHW(void)
{
...
#if 0 //bring up
    MVD_PRINT("====================>>>>MVD Ctrl status1.5a : 0x%x\n", HAL_MVD_RegReadByte(MVD_STATUS));
    ...
    MVD_DEBUGINFO(MVD_PRINT("MVD Ctrl status : 0x%x\n", HAL_MVD_RegReadByte(MVD_STATUS)));
    MVD_PRINT("====================>>>>MVD Ctrl status1.5b : 0x%x\n", HAL_MVD_RegReadByte(MVD_STATUS));
#endif
...
MS_BOOL HAL_MVD_ReleaseFW(void)
{
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("%s VPU_IsRsted\n", __FUNCTION__));
        return TRUE;
    }
...
MS_BOOL HAL_MVD_SetMEMCfg(MS_U8 u8Idx, MVD_MEMCfg* pMEMCfg)
{
    ...
    pMVDHalContext->stMiuCfg.u8FBMiuSel = pMEMCfg->u8FBMiuSel;
    return TRUE;
}

MVD_MEMCfg* HAL_MVD_GetMEMCfg(MS_U8 u8Idx)
{
    //MVD_PRINT("%s: u8Idx=0x%x\n", __FUNCTION__, u8Idx);
...
MS_U8 HAL_MVD_GetFBMode(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("FBMode=0x%x\n", u8Mode));
    return u8Mode;
}
...
void HAL_MVD_PowerCtrl(MS_BOOL bOn)
{
    ...
    HAL_VPU_EX_SetClkManagement(E_VPU_EX_CLKPORT_MVD, bOn);
#else
    HAL_MVD_RegWriteBit(REG_CKG_MVD_ENABLE, bOn, CKG_MVD_ENABLE_GATED);
    HAL_MVD_RegWriteBit(REG_CKG_MVD_ENABLE, bOn, CKG_MVD_PAS_ENABLE_GATED);

    HAL_MVD_RegWriteByteMask(REG_CKG_MVD, CKG_MVD_288MHZ, CKG_MVD_MASK);
...
{
    MVD_CMD_HANDSHADE_INDEX* handshake;
    MS_U32 u32BufAddr = GET_FRMINFO_BUFFADD(u8Idx);
    HAL_MVD_CPU_Sync();
    HAL_MVD_ReadMemory();

    handshake = HAL_MVD_MemReadPoiter(u32BufAddr + OFFSET_CMD_HANDSHAKE_INDEX);

    switch (cmd)
    {
        case MVD_HANDSHAKE_PAUSE:
            (handshake->mvdcmd_handshake_pause) = 0;
            break;
        case MVD_HANDSHAKE_SLQ_RST:
            (handshake->mvdcmd_handshake_slq_reset) = 0;
            break;
        case MVD_HANDSHAKE_STOP:
            (handshake->mvdcmd_handshake_stop) = 0;
            break;
        case MVD_HANDSHAKE_SKIP_DATA:
            (handshake->mvdcmd_handshake_skip_data) = 0;
            break;
        case MVD_HANDSHAKE_SINGLE_STEP:
            (handshake->mvdcmd_handshake_single_step) = 0;
            break;
        case MVD_HANDSHAKE_SCALER_INFO:
            (handshake->mvdcmd_handshake_scaler_data_ready) = 0;
            break;
        case MVD_HANDSHAKE_GET_NXTDISPFRM:
            (handshake->mvdcmd_handshake_get_nextdispfrm_ready) = 0;
            break;
        case MVD_HANDSHAKE_PARSER_RST:
            (handshake->mvdcmd_handshake_parser_rst) = 0;
            break;
        case MVD_HANDSHAKE_RST_CC608:
            (handshake->mvdcmd_handshake_cc608_rst) = 0;
            break;
        case MVD_HANDSHAKE_RST_CC708:
            (handshake->mvdcmd_handshake_cc708_rst) = 0;
            break;
        case MVD_HANDSHAKE_VIRTUAL_COMMAND:
            (handshake->mvdcmd_handshake_virtualCommand) = 0;
            break;
        case MVD_HANDSHAKE_FLUSHQUEUE_COMMAND:
            (handshake->mvdcmd_handshake_flush) = 0;
            break;
        case MVD_HANDSHAKE_VSYNC_CONTROL:
            (handshake->mvdcmd_handshake_vsync_control) = 0;
            break;
        default:
            break;
    }
...
MS_BOOL HAL_MVD_TimeOut(MS_U8 u8Idx)
{
    ...
    for ( i = 0; i < MVD_PollingTimes; i++ )
    {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("%s: bStopped(%x) or timeout(%d)\n", __FUNCTION__, pMVDHalContext->bStopped[u8Idx], MsOS_GetSystemTime() - u32StartTime));
            return TRUE;
        }

        //HAL_MVD_Sleep(5);
    }
    MVD_DEBUGERROR( MVD_PRINT("MVD_TimeOut=%x\n", i) );
    return TRUE;
}
...
MS_BOOL HAL_MVD_MVDCommand ( MS_U8 u8cmd, MVD_CmdArg *pstCmdArg )
{
    ...
    if(HAL_MVD_IsDisplayCommand(u8cmd) == TRUE)
    {
        if(HAL_MVD_IsNeedResponseCommand(u8cmd) == TRUE)
        {
            ...
            temp2 = (MS_U8*)MsOS_PA2KSEG1(u32BufStart2 + OFFSET_VCHANDSHAKE);
...
#endif
    {
        ...
        {
            MVD_DEBUG_FWCMD(MVD_PRINT("MVD_CMD: %x; %x, %x, %x, %x, %x, %x\n", u8cmd, pstCmdArg->Arg0,
...
_CMD_DONE:
    ...
    {
        MS_U32 x = 0;
        MVD_DEBUG_FWCMD(MVD_PRINT("%s timeout dump pc\n", __FUNCTION__));
        for (x = 0; x < 30; x++)
            MVD_DEBUG_FWCMD(MVD_PRINT("0x%x\n", HAL_VPU_EX_GetProgCnt()));
        MVD_DEBUG_FWCMD(MVD_PRINT("###\n"));
    }
...
MS_BOOL HAL_MVD_InitHW(VPU_EX_SourceType SourceType, VPU_EX_DecoderType eDecType)
{
    ...
    {
        HAL_VPU_EX_SetClkManagement(E_VPU_EX_CLKPORT_MVD_CORE, ENABLE);
    }
    if(SourceType == E_VPU_EX_INPUT_TSP)
    {
        HAL_VPU_EX_SetClkManagement(E_VPU_EX_CLKPORT_MVD_PAS, ENABLE);
    }
#endif

#ifdef CONFIG_MSTAR_SRAMPD
    HAL_MVD_RegWriteBit(REG_CODEC_SRAM_SD_EN, ENABLE, SRAM_SD_EN_MVD);
    HAL_MVD_Delayms(1);
#endif
    ...
    if(!HAL_MVD_RstHW())
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_MVDInit:MVD4ResetHW failed\n"));
        return FALSE;
    }
    else
    {
        MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_MVDInit:MVD4ResetHW success\n"));
    }
...
MS_U32 HAL_MVD_GetFWVer(MS_U32 u32VpuSid)
{
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg5 = HAL_VPU_EX_GetTaskId(u32VpuSid);
    if (HAL_MVD_MVDCommand( CMD_GET_FW_VERSION, &mvdcmd ) == FALSE)
    {
        MVD_ERROR( MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_GET_FW_VERSION ) );
        return 0;
    }
...
static MS_BOOL _MVD_Check_FW_Rdy(MS_U32 u32VpuSid)
{
    ...
    if (ret == FALSE)
    {
        MVD_ERROR(MVD_PRINT("MVD f/w header version is wrong,%x,%x,%x,%x\n", (*(pVersion + 4)), INTERFACE_VERSION, (*pVersion), FW_VERSION));
    }
...
static MS_BOOL _MVD_Check_FW_Rdy(MS_U32 u32VpuSid)
{
    ...
    if (u32TimeOut == 0)
    {
        MVD_ERROR(MVD_PRINT("MVD_FW_IF_Version=%lx inconsistent with header file(%x)!\n",
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("Warning! FWBinVer(%lx) != FWHdrVer(%x)\n", HAL_MVD_GetFWVer(u32VpuSid), FW_VERSION));
    }
    MVD_DEBUGINFO(MVD_PRINT("MVD version Interface = %x, FW = %x\n", INTERFACE_VERSION, FW_VERSION));
...
MS_BOOL _DumpCtrl(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("version=0x%x, statue=0x%x, last_ctl_cmd=0x%x, last_ctl_arg=0x%x\n",
    ...
    while (CTL_TASK_CMDRDY != ctl_ptr->statue)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("CTRL timeout!!! %d\n", __LINE__));
            return FALSE;
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        MVD_DEBUGINFO(MVD_PRINT("%s: Task %d, status=%d\n", __FUNCTION__, i, ctl_ptr->task_statue[i]));
    }
    return TRUE;
}
...
MS_BOOL HAL_MVD_CreateTask(MS_U8 u8Idx, HAL_VPU_StreamId eVpuId, MS_BOOL bShareBBU)
{
    MS_BOOL bRet = FALSE;
    ...
    MVD_DEBUGINFO(MVD_PRINT("\n\n Create Task(%x)!!!\n", u8Idx));
    pMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    ...
    pMVDHalContext->_stMVDStream[u8Idx].u32SLQId = HAL_VPU_EX_GetBBUId(stTaskInfo.u32Id, &stTaskInfo, bShareBBU);
    if(pMVDHalContext->_stMVDStream[u8Idx].u32SLQId == HAL_VPU_INVALID_BBU_ID)
    {
        MVD_DEBUGINFO(MVD_PRINT("ERROR!!!MVD GetBBU Id Fail \n"));
    }
    else
    {
    ...
    if(bRet == FALSE)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("[%s][%d]ERROR!!! Free BBU Id Fail !!!\n", __FUNCTION__, __LINE__));
        }
    }
#else
    bRet = HAL_VPU_EX_TaskCreate((MS_U32)eVpuId, (void*)&stVpuInitPara);
#endif
    MVD_DEBUGINFO(MVD_PRINT(" Create Task!!! bRet=%x ###\n\n", bRet));
    _DumpCtrl(u8Idx);
...
MS_BOOL HAL_MVD_DeleteTask(MS_U8 u8Idx, HAL_VPU_StreamId eVpuId)
{
    ...
    if(!HAL_VPU_EX_FreeBBUId(stTaskInfo.u32Id, pMVDHalContext->_stMVDStream[u8Idx].u32SLQId, &stTaskInfo))
    {
        MVD_DEBUGINFO(MVD_PRINT("ERROR!!! Free BBU Id Fail !!!\n"));
    }
#endif
    bRet = HAL_VPU_EX_TaskDelete((MS_U32) eVpuId, &stVpuInitPara);
    MVD_DEBUGINFO(MVD_PRINT(" Delete Task!!! ###\n\n"));
    _DumpCtrl(u8Idx);
    return bRet;
}
...
MS_BOOL HAL_MVD_Init(MS_U8 u8HalIdx, MVD_CodecType eCodecType, MS_U32 u32VpuSid, MS_BOOL bShareBBU)
{
    //OSAL_MVD_MutexInit();
    ...
    if (!HAL_MVD_CreateTask(u8HalIdx, (HAL_VPU_StreamId)u32VpuSid, bShareBBU))
    {
        _MVD_INIT_FAIL_RET();
    }
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s:HAL_MVD_InitFW(%x) failed\n", __FUNCTION__, u8HalIdx));
        if (!HAL_MVD_DeleteTask(u8HalIdx, (HAL_VPU_StreamId)u32VpuSid))
        {
            MVD_DEBUGERROR(MVD_PRINT("%s:HAL_MVD_DeleteTask failed\n", __FUNCTION__));
        }
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("%s:HAL_MVD_InitFW(%x) success\n", __FUNCTION__, u8HalIdx));
    }
...
MS_BOOL HAL_MVD_SetSpeed(MS_U8 u8Idx, MVD_SpeedType eSpeedType, MS_U8 u8Multiple)
{
    ...
    if(pMVDHalContext->bTrickPlay2xAVSync[u8Idx] == TRUE && eSpeedType == E_MVD_SPEED_FAST && (u8Multiple == 2 || pMVDHalContext->bEnSmoothScan[u8Idx] == TRUE))
    {
        mvdcmd.Arg2 = 1;
    }
    else
...
void HAL_MVD_SetFrameBuffAddr(MS_U8 u8Idx, MS_VIRT u32addr, MS_U8 u8fbMode)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_SetFrameBuffAddr: only support 28bit add!\n"));
        return;
    }
    ...
    MVD_DEBUGINFO(MVD_PRINT("FramebufferAdd 0x%lx, FB Mode 0x%x, arg3=0x%x\n", (unsigned long)u32addr, u8fbMode, mvdcmd.Arg3));
    SET_DECNUM(mvdcmd, u8Idx);
...
static void _MVD_SetUserDataBufStart(MS_U8 u8Idx, MS_VIRT u32addr, MS_U8 u8arg3)
{
    ...
    MS_ASSERT((u32addr % 8) == 0);
    u32addr >>= 3;
    MVD_DEBUGINFO(MVD_PRINT("%s add=0x%lx arg3=0x%x\n", __FUNCTION__, (unsigned long)u32addr, u8arg3));
...
static void _MVD_SetUserDataBufSize(MS_U8 u8Idx, MS_U32 u32size, MS_U8 u8arg3)
{
    ...
    u32size >>= 3;
    MVD_DEBUGINFO(MVD_PRINT("%s add=0x%x arg3=0x%x\n", __FUNCTION__, u32size, u8arg3));
...
void HAL_MVD_SetSLQTblBufStartEnd(MS_U8 u8Idx, MS_VIRT u32start, MS_VIRT u32end)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s st=0x%lx end=0x%lx OK!!!\n", __FUNCTION__, (unsigned long)u32start, (unsigned long)u32end));
    return;
}
...
MS_BOOL HAL_MVD_StepDisp(MS_U8 u8Idx)
{
    ...
    MVD_SetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_STEP_DISP_DONE, sizeof(MS_U8), 0);
    ...
    {
        MVD_DEBUGERROR( MVD_PRINT( "Command: HAL_MVD_Resume fail!!\r\n" ) );
        return FALSE;
    }
...
MS_VIRT HAL_MVD_GetTsFileESReadPtr(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR( MVD_PRINT( "Ctrl: 0x%x fail!!\n", CMD_PARSER_READ_POSITION) );
    }
...
MS_VIRT HAL_MVD_GetTsFileESWritePtr(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("ES wrapping Wr=0x%lx ==> ", (unsigned long)u32WrPtr));
        u32WrPtr -= pstMemCfg->u32BSSize;
        MVD_DEBUGINFO(MVD_PRINT("0x%lx\n", (unsigned long)u32WrPtr));
    }
    return u32WrPtr;
}
...
MS_BOOL HAL_MVD_EnableLastFrameShow(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    if (E_MVD_SLQ_TBL_MODE == curSrcMode)
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("fe=%lx, rd=%lx, wr=%lx\n", (unsigned long)*pu32FileEndPtr,
...
MS_BOOL HAL_MVD_Set_Playback_Interval(MS_U8 u8Idx, MS_U32 u32StartPts, MS_U32 u32EndPts)
{
    MVD_CmdArg mvdcmd;

    SET_CMDARG(mvdcmd, u32EndPts, u8Idx);
    SET_CMD_RET_FALSE(CMD_PLAYBACK_INTERVAL_END, &mvdcmd);

    SET_CMDARG(mvdcmd, u32StartPts, u8Idx);
    SET_CMD_RET_FALSE(CMD_PLAYBACK_INTERVAL_START, &mvdcmd);
...
#define STOP_TIMEOUT 500 //ms
    ...
    while(!HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_FLUSHQUEUE_COMMAND))
    {
        ...
        {
            MVD_DEBUGERROR( MVD_PRINT( "Ctrl: 0x%x fail timeout!!\r\n", CMD_FLUSH_DISP_QUEUE ) );
            break;
        }
    }
...
MS_BOOL HAL_MVD_SetFileModeAVSync(MS_U8 u8Idx, MVD_TIMESTAMP_TYPE eSyncMode)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s eSyncMode=%d\n", __FUNCTION__, eSyncMode));
    SETUP_CMDARG(mvdcmd);
    switch (eSyncMode)
    {
...
MS_BOOL HAL_MVD_DispCtrl(MS_U8 u8Idx, MS_BOOL bDecOrder, MS_BOOL bDropErr, MS_BOOL bDropDisp, MVD_FrcMode eFrcMode)
{
    ...
    {
        MVD_DEBUGINFO( MVD_PRINT("bDropErrFrm(%d) != bDropErr(%d)\n", *pbDropErrFrm, bDropErr));
        *pbDropErrFrm = bDropErr;
    }
...
MS_BOOL HAL_MVD_SkipData(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("====> %s (t1=%u t2=%u diff=%u)\n", __FUNCTION__,
...
MS_BOOL HAL_MVD_PauseDisp(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR( MVD_PRINT( "Command: 0x%x(ON) fail!!\r\n", CMD_DISPLAY_PAUSE) );
        bRst = FALSE;
    }
    return bRst;
}
...
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_SetDynamicScaleAddr(MS_U8 u8Idx, MS_VIRT u32addr, MS_U32 u32Size)
{
    MVD_CmdArg mvdcmd;
    ...
    SET_CMD_RET_FALSE(CMD_DYNAMIC_SCALE_SIZE, &mvdcmd);
...
MS_BOOL HAL_MVD_Mpeg4CalFramerate(MS_U8 u8Idx, MS_BOOL bEnable)
{
#ifdef CMD_MPEG4_CAL_FRAMERATE
    MVD_CmdArg mvdcmd;

    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable; //1 -> Enable  0 -> Disable
    SET_DECNUM(mvdcmd, u8Idx);
    SET_CMD_RET_FALSE(CMD_MPEG4_CAL_FRAMERATE, &mvdcmd);
    return TRUE;
#else
    return FALSE;
#endif
...
MS_U32 HAL_MVD_GetIntState(MS_U8 u8Idx)
{
    ...
    else
    {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("Cmd 0x%x normal timeout.\n", CMD_GET_INT_STAT));
        }
    }
...
{
    //MVD_PRINT("MVD_IntHasXcLowDelay=%x \n ", u32IntStat);
...
static MS_U8 MVD_GetSyncStat(MS_U8 u8Idx, MS_U8 u8ArgIdx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("Sync On/Off %x, Done %x, Stat %x, diff=%d\n",
...
MS_BOOL HAL_MVD_FrameOpt(MS_U8 u8Idx, MS_U8 u8FrmIdx, MVD_FrmOpt eOpt)
{
    ...
    if (eOpt == E_MVD_FRAME_ADD_REF)
    {
        ...
        pTemp = (DISPQ_IN_DRAM*)(MS_PA2KSEG1(pstMemCfg->u32FWCodeAddr + DISP_QUEUE_START + u8VPUIdx * 0x100000));
        pTemp->bUsedByOutside[u8FrmIdx]++;
        return TRUE;
#endif
    }
    ...
    MVD_DEBUGINFO(MVD_PRINT("FLIP_RELEASE_FRAME: idx=0x%x, opt=0x%x\n", u8FrmIdx, eOpt));
    SET_DECNUM(mvdcmd, u8Idx);
...
{
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
    if(E_MVD_MCU_DISPOUT_N_FRAMING == pstCtrlCfg->u8McuMode)
        return TRUE;
    else
        return FALSE;
}

MS_BOOL HAL_MVD_IsDrvMcuMode(MS_U8 u8Idx)
{
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
    if(E_MVD_MCU_DISPOUT == pstCtrlCfg->u8McuMode)
        return TRUE;
    else
        return FALSE;
}
...
MS_BOOL HAL_MVD_FrameCapture(MS_U8 u8Idx, MS_U8 u8FrmIdx, MS_BOOL bEnable)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("CAPTURE_FRAME: idx=0x%x, enable=0x%x\n", u8FrmIdx, bEnable));
    SET_DECNUM(mvdcmd, u8Idx);
...
//------------------------------------------------------------------------------
/// Set firmware as NDS mode
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_SetNDSMode(MS_U8 u8Idx, MS_BOOL bEnable, MS_BOOL bSendCmd)
{
#ifdef CMD_ENABLE_NDS

    pMVDHalContext->gMVDPreCtrl[u8Idx].bEnableNDS = bEnable;
    ...
    if(bEnable && bSendCmd)
    {
        ...
        SETUP_CMDARG(mvdcmd);
        mvdcmd.Arg0 = bEnable; //1: enable, 0:disable NDS
        SET_DECNUM(mvdcmd, u8Idx);
        SET_CMD_RET_FALSE(CMD_ENABLE_NDS, &mvdcmd);
    }
...
#define MVD_FBNUM_MIN     MVD_FBNUM_DEFAULT
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u8FrmNum = %d\n", __FUNCTION__, u8FrmNum));
    if (u8FrmNum < MVD_FBNUM_MIN)
    {
        MVD_DEBUGINFO(MVD_PRINT("%s set u8FrmNum as %d instead of %d!\n",
...
#if 0
static MS_BOOL MVD_WriteFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u16Offset, MS_U8 u8Size, MS_U32 u32Val)
{
    MS_BOOL bRet = FALSE;
    MS_VIRT u32BufStart = NULL;
    switch (eBuffType)
    ...
    if (u8Size == sizeof(MS_U8))
    {
        bRet = HAL_MVD_MemWriteByte(u32BufStart + u16Offset, (MS_U8)u32Val);
    }
    else if (u8Size == sizeof(MS_U32))
    {
        bRet = HAL_MVD_MemWrite4Byte(u32BufStart + u16Offset, u32Val);
    }

    MVD_DEBUGINFO(MVD_PRINT("%s: u32Val=%d for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
                            __FUNCTION__, u32Val, u8Idx, eBuffType, u16Offset, u8Size));
    return bRet;
}
#endif
...
MS_U32 HAL_MVD_EsRdPtr(MS_U8 u8Idx)
{
    return (MS_U32)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_ES_RD_PTR, sizeof(MS_U32));
}
MS_U32 HAL_MVD_EsWrPtr(MS_U8 u8Idx)
{
    return (MS_U32)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_ES_WR_PTR, sizeof(MS_U32));
}
//================== FrameInfo ==================//
...
#if 0
MS_U8 HAL_MVD_IsSeqHeaderFound(MS_U8 u8Idx)
{
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_SEQ_FOUND, sizeof(MS_U8));
}
#endif
...
MVD_PicType HAL_MVD_GetPicType(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s error: pu8MVDGetFrameInfoBufStart=NULL\n", __FUNCTION__));
...
MS_S64 HAL_MVD_GetVideoFirstPts(MS_U8 u8Idx)
{
    ...
    if (NULL == u32BufStart)
    {
        MVD_DEBUGERROR(printf("%s error: u32BufStart=NULL\n", __FUNCTION__));
        return 0;
    }

    u32PtsH = HAL_MVD_MemRead4Byte(u32BufStart + OFFSET_FIRST_PTS_H);
    u32PtsL = HAL_MVD_MemRead4Byte(u32BufStart + OFFSET_FIRST_PTS_L);
    s64PTS = (((MS_S64)u32PtsH << 32) | u32PtsL);

    HAL_MVD_MemWrite4Byte(u32BufStart + OFFSET_FIRST_PTS_H, 0);
    HAL_MVD_MemWrite4Byte(u32BufStart + OFFSET_FIRST_PTS_L, 0);
...
MS_BOOL HAL_MVD_IsCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    u32BufAddr = GET_FRMINFO_BUFFADD(u8Idx);
    ...
    {
        case MVD_HANDSHAKE_PAUSE:
            bCmdRdy = handshake->mvdcmd_handshake_pause;
            break;
        case MVD_HANDSHAKE_SLQ_RST:
            bCmdRdy = handshake->mvdcmd_handshake_slq_reset;
            break;
        case MVD_HANDSHAKE_STOP:
            bCmdRdy = handshake->mvdcmd_handshake_stop;
            break;
        case MVD_HANDSHAKE_SKIP_DATA:
            bCmdRdy = handshake->mvdcmd_handshake_skip_data;
            break;
        case MVD_HANDSHAKE_SINGLE_STEP:
            bCmdRdy = handshake->mvdcmd_handshake_single_step;
            break;
        case MVD_HANDSHAKE_SCALER_INFO:
            bCmdRdy = handshake->mvdcmd_handshake_scaler_data_ready;
            break;
        case MVD_HANDSHAKE_GET_NXTDISPFRM:
            bCmdRdy = handshake->mvdcmd_handshake_get_nextdispfrm_ready;
            break;
        case MVD_HANDSHAKE_PARSER_RST:
            bCmdRdy = handshake->mvdcmd_handshake_parser_rst;
            break;
        case MVD_HANDSHAKE_RST_CC608:
            bCmdRdy = handshake->mvdcmd_handshake_cc608_rst;
            break;
        case MVD_HANDSHAKE_RST_CC708:
            bCmdRdy = handshake->mvdcmd_handshake_cc708_rst;
            break;
        case MVD_HANDSHAKE_VIRTUAL_COMMAND:
            bCmdRdy = handshake->mvdcmd_handshake_virtualCommand;
            break;
        case MVD_HANDSHAKE_FLUSHQUEUE_COMMAND:
            bCmdRdy = handshake->mvdcmd_handshake_flush;
            break;
        case MVD_HANDSHAKE_VSYNC_CONTROL:
            bCmdRdy = handshake->mvdcmd_handshake_vsync_control;
            break;
        default:
            bCmdRdy = FALSE;
            break;
...
MS_BOOL HAL_MVD_ClearCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    {
        case MVD_HANDSHAKE_SCALER_INFO:
            //HAL_MVD_MemWrite4Byte(u32BufAddr + OFFSET_CMD_HANDSHAKE_INDEX + E_MVD_HANDSHAKE_SCALER_INFO*16, 0);
            (handshake->mvdcmd_handshake_get_nextdispfrm_ready) = 0;
            break;
        default:
            break;
...
MS_BOOL HAL_MVD_DisableFDMask(MS_U8 u8Idx, MS_BOOL bDisable)
{
    MVD_CmdArg mvdcmd;
    ...
    SET_DECNUM(mvdcmd, u8Idx);
    MVD_DEBUGINFO(printf("%s CMD_DISABLE_FDMASK arg0=%x\n", __FUNCTION__, bDisable));
    if (HAL_MVD_MVDCommand(CMD_DISABLE_FDMASK, &mvdcmd) == FALSE)
    {
...
MS_U32 HAL_MVD_GetDropCnt(MS_U8 u8Idx)
{
...
MS_U32 HAL_MVD_GetXcLowDelayIntState(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_GetXcLowDelayIntState error: pu8MVDGetFrameInfoBufStart=NULL\n"));
        return 0;
    }

    MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_GetXcLowDelayIntState()=%x,%x,%x,%x,%x\n",
...
void HAL_MVD_GetErrInfo(MS_U8 u8Idx, MVD_ErrCode *errCode, MVD_ErrStatus *errStatus, MS_U32* u32errstatus)
{
    ...
    if(pu8MVDGetFrameInfoBufStart == 0)
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_GetErrInfo error!\n"));
        return;
    }
...
MS_BOOL HAL_MVD_SLQTblSendPacket(MS_U8 u8Idx, MVD_PacketInfo *pstVideoPKT)
{
    ...
    MVD_PRINT("Pkt[%x] st=0x%x len=0x%x pts=0x%x id_l=0x%x id_h=0x%x\n", u32SendTimes[u8Idx]++,
...
#endif
    {
        ...
        if (u32MVDFWPtsTblAddr)
        {
            ...
            //MVD_PRINT("PTS=0x%x(%x), idx=0x%x add=0x%x\n", pstVideoPKT->u32TimeStamp,
    ...
    *pu32EsNew += 8;
    if (*pu32EsNew >= pstSlqTblInfo->pDrvEsTbl->u32EndAdd)
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("...ES wrapping to the beginning!\n"));
        *pu32EsNew = pstSlqTblInfo->pDrvEsTbl->u32StAdd;
    }

    //MVD_PRINT("===>[%x] u32ESStart=0x%x u32ESEnd=0x%x u32EsLast=0x%x\n",
    ...
    //MVD_PRINT("===> u32EntryWord1 addr=0x%x\n", (*u32LastEntry));
    //MVD_PRINT("===> u32EntryWord0=0x%x\n", u32EntryWord);

    u32EntryWord = u32ESStart;
    HAL_MVD_MemWrite4Byte((*u32LastEntry) + 4, u32EntryWord);
    //MVD_PRINT("===> u32EntryWord1 addr=0x%x\n", (*u32LastEntry)+4);
    //MVD_PRINT("===> u32EntryWord1=0x%x\n", u32EntryWord);

    *u32LastEntry += 8;
    if (*u32LastEntry >= pstSlqTblInfo->pDrvSlqTbl->u32EndAdd)
    {
        //wrap to the beginning of the table
        MVD_DEBUGINFO(MVD_PRINT("...wrapping to the beginning!\n"));
...
static MVD_EX_VC1_PROFILE_BIT _MVD_EX_VC1_Profile_IdcMap2Bit(MVD_EX_VC1_PROFILE_IDC eIdc)
{
    MVD_EX_VC1_PROFILE_BIT eRet = E_MVD_EX_VC1_PROFILE_BIT_NOTSUPPORT;
    switch(eIdc)
    {
        case E_MVD_EX_VC1_PROFILE_IDC_NONE:
            eRet = E_MVD_EX_VC1_PROFILE_BIT_NONE;
            break;
        case E_MVD_EX_VC1_PROFILE_IDC_SP:
            eRet = E_MVD_EX_VC1_PROFILE_BIT_SP;
            break;
        case E_MVD_EX_VC1_PROFILE_IDC_MP:
            eRet = E_MVD_EX_VC1_PROFILE_BIT_MP;
            break;
        case E_MVD_EX_VC1_PROFILE_IDC_AP:
            eRet = E_MVD_EX_VC1_PROFILE_BIT_AP;
            break;
            //case E_MVD_EX_VC1_PROFILE_IDC_SP_RP227:
            //    eRet = E_MVD_EX_VC1_PROFILE_BIT_SP_RP227;
            //    break;
            //case E_MVD_EX_VC1_PROFILE_IDC_MP_RP227:
            //    eRet = E_MVD_EX_VC1_PROFILE_BIT_MP_RP227;
            //    break;
            //case E_MVD_EX_VC1_PROFILE_IDC_AP_RP227:
            //    eRet = E_MVD_EX_VC1_PROFILE_BIT_AP_RP227;
            //    break;
        default:
            break;
    }
...
static MVD_EX_VC1_PROFILE_LEVEL_BIT _MVD_EX_VC1_ProfileLevel_IdcMap2Bit(MVD_EX_VC1_PROFILE_LEVEL_IDC eIdc, MVD_EX_VC1_PROFILE_IDC eProfile)
{
    MVD_EX_VC1_PROFILE_LEVEL_BIT eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_NOTSUPPORT;
    switch(eProfile)
    {
        case E_MVD_EX_VC1_PROFILE_IDC_NONE:
            eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_NONE;
            break;

        case E_MVD_EX_VC1_PROFILE_IDC_SP:
            switch(eIdc)
            {
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_NONE:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_NONE;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_LL:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_LL;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_ML:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_ML;
                    break;
                default:
                    break;
            }
        ...
        case E_MVD_EX_VC1_PROFILE_IDC_MP:
            switch(eIdc)
            {
                ...
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_ML:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_ML;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_HL:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_HL;
                    break;
                default:
                    break;
            }
        ...
        case E_MVD_EX_VC1_PROFILE_IDC_AP:
            switch(eIdc)
            {
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_NONE:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_NONE;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_L0:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_L0;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_L1:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_L1;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_L2:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_L2;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_L3:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_L3;
                    break;
                case E_MVD_EX_VC1_PROFILE_LEVEL_IDC_L4:
                    eRet = E_MVD_EX_VC1_PROFILE_LEVEL_BIT_L4;
                    break;
                default:
                    break;
            }
...
static MVD_EX_MPEG2_PROFILE_BIT _MVD_EX_MPEG2_Profile_IdcMap2Bit(MVD_EX_MPEG2_PROFILE_IDC eIdc)
{
    MVD_EX_MPEG2_PROFILE_BIT eRet = E_MVD_EX_MPEG2_PROFILE_BIT_NOTSUPPORT;

    switch(eIdc)
    {
        case E_MVD_EX_MPEG2_PROFILE_IDC_NONE:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_NONE;
            break;
        case E_MVD_EX_MPEG2_PROFILE_IDC_SP:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_SP;
            break;
        case E_MVD_EX_MPEG2_PROFILE_IDC_MP:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_MP;
            break;
        case E_MVD_EX_MPEG2_PROFILE_IDC_SNR:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_SNR;
            break;
        case E_MVD_EX_MPEG2_PROFILE_IDC_SP_SPATIAL:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_SP_SPATIAL;
            break;
        case E_MVD_EX_MPEG2_PROFILE_IDC_HP:
            eRet = E_MVD_EX_MPEG2_PROFILE_BIT_HP;
            break;
        default:
            break;
    }
...
static MVD_EX_MPEG2_PROFILE_LEVEL_BIT _MVD_EX_MPEG2_ProfileLevel_IdcMap2Bit(MVD_EX_MPEG2_PROFILE_LEVEL_IDC eIdc)
{
    MVD_EX_MPEG2_PROFILE_LEVEL_BIT eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_NOTSUPPORT;
    switch(eIdc)
    {
        case E_MVD_EX_MPEG2_PROFILE_LEVEL_IDC_NONE:
            eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_NONE;
            break;
        case E_MVD_EX_MPEG2_PROFILE_LEVEL_IDC_LL:
            eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_LL;
            break;
        case E_MVD_EX_MPEG2_PROFILE_LEVEL_IDC_ML:
            eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_ML;
            break;
        case E_MVD_EX_MPEG2_PROFILE_LEVEL_IDC_H_14:
            eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_H_14;
            break;
        case E_MVD_EX_MPEG2_PROFILE_LEVEL_IDC_HL:
            eRet = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_HL;
            break;
        default:
            break;
    }
...
static MVD_EX_MPEG4_P2_PROFILE_BIT _MVD_EX_MPEG4P2_Profile_IdcMap2Bit(MVD_EX_MPEG4_P2_PROFILE_IDC eIdc)
{
    MVD_EX_MPEG4_P2_PROFILE_BIT eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_NOTSUPPORT;
    switch(eIdc)
    {
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_NONE:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_NONE;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_SP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_SP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_ARTSP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_ARTSP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_SSP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_SSP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_CP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_CP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_ACP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_ACP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_CSP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_CSP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_MP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_MP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_ACEP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_ACEP;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_N_BIT:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_N_BIT;
            break;
            //case E_MVD_EX_MPEG4_P2_PROFILE_IDC_SP_STUDIO:
            //    eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_SP_STUDIO;
            //    break;
            //case E_MVD_EX_MPEG4_P2_PROFILE_IDC_CP_STUDIO:
            //    eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_CP_STUDIO;
            //    break;
        case E_MVD_EX_MPEG4_P2_PROFILE_IDC_ASP:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_ASP;
            break;
            //case E_MVD_EX_MPEG4_P2_PROFILE_IDC_FGSP:
            //    eRet = E_MVD_EX_MPEG4_P2_PROFILE_BIT_FGSP;
            //    break;
        default:
            break;
    }
...
static MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT _MVD_EX_MPEG4P2_ProfileLevel_IdcMap2Bit(MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC eIdc)
{
    MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_NOTSUPPORT;
    switch(eIdc)
    {
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_NONE:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_NONE;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L0:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L0;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L1:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L1;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L2:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L2;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L3:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L3;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L4:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L4;
            break;
        case E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_IDC_L5:
            eRet = E_MVD_EX_MPEG4_P2_PROFILE_LEVEL_BIT_L5;
            break;
        default:
            break;
    }
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_GetFrameInfo error: pu8MVDGetVolBufStart=NULL\n"));
        return;
    }
    ...
    // Determine the profile and level
    curCodecType = HAL_MVD_GetCodecType(u8Idx);

    pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_NOTSUPPORT;
    pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_NOTSUPPORT;

    switch(curCodecType)
    {
        case E_MVD_CODEC_VC1_ADV:
        case E_MVD_CODEC_VC1_MAIN:
            pinfo->u32Profile = _MVD_EX_VC1_Profile_IdcMap2Bit(gvolInfo.u16Profile);
            pinfo->u32Level = _MVD_EX_VC1_ProfileLevel_IdcMap2Bit(gvolInfo.u16Level, gvolInfo.u16Profile);
            break;

        case E_MVD_CODEC_MPEG4:
        case E_MVD_CODEC_MPEG4_SHORT_VIDEO_HEADER:
            pinfo->u32Profile = _MVD_EX_MPEG4P2_Profile_IdcMap2Bit(gvolInfo.u16Profile);
            pinfo->u32Level = _MVD_EX_MPEG4P2_ProfileLevel_IdcMap2Bit(gvolInfo.u16Level);
            break;

        case E_MVD_CODEC_MPEG2:
        {
            MS_U8 u8ProfileLevel = (MS_U8)((gvolInfo.u16Profile << 4) | (gvolInfo.u16Level & 0xF));
            MS_U8 u8EscapeBit = u8ProfileLevel & 0x80;

            if(u8EscapeBit == 0)
            {
                pinfo->u32Profile = _MVD_EX_MPEG2_Profile_IdcMap2Bit(gvolInfo.u16Profile);
                pinfo->u32Level = _MVD_EX_MPEG2_ProfileLevel_IdcMap2Bit(gvolInfo.u16Level);
            }
            else
            {
                /*  MPEG2_ISO_13818-2.pdf P.122, Table 8-4
                 *
                 *  u8ProfileLevel  Name
                 ...
                 */
                switch(u8ProfileLevel)
                {
                        /*
                        case 0x8E:
                            pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_MULTIVIEW;
                            pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_LL;
                            break;
                        case 0x8D:
                            pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_MULTIVIEW;
                            pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_ML;
                            break;
                        case 0x8B:
                            pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_MULTIVIEW;
                            pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_H_14;
                            break;
                        case 0x8C:
                            pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_MULTIVIEW;
                            pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_HL;
                            break;
                        */
                    case 0x85:
                        pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_422P;
                        pinfo->u32Level = E_MVD_EX_MPEG2_PROFILE_LEVEL_BIT_ML;
                        break;
                    case 0x82:
                        pinfo->u32Profile = E_MVD_EX_MPEG2_PROFILE_BIT_422P;
    ...
    pinfo->u32DispWidth = gvolInfo.u32DispWidth;
    pinfo->u32DispHeight = gvolInfo.u32DispHeight;
    ...
    if (pinfo->u16par_width == 0 || pinfo->u16par_height == 0)
    {
        if ((pinfo->u8AspectRate > 0) && (pinfo->u8AspectRate < 5 ))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    MVD_DEBUGVERBAL(MVD_PRINT("u32PAR_Width:%d,u32PAR_Height:%d,GCD:%d\n", u32PAR_Width, u32PAR_Height, u32GCD));
                }
            }
        }
        else
        ...
        MVD_DEBUGVERBAL(MVD_PRINT("u32DAR_Width:%d,u32DAR_Height%d\n", u32DAR_Width, u32DAR_Height));
    }
    MVD_DEBUGVERBAL(MVD_PRINT("pinfo->u16par_width:%d, pinfo->u16par_height:%d\n", pinfo->u16par_width, pinfo->u16par_height));
    ...
    else if ((curCodecType == E_MVD_CODEC_VC1_ADV) || (curCodecType == E_MVD_CODEC_VC1_MAIN))
    {
        ...
        MVD_DEBUGVERBAL(MVD_PRINT("MVD: vc1_frameRate=%d\n", pinfo->u32FrameRate));
    }
    else if (curCodecType == E_MVD_CODEC_MPEG4)
    {
        ...
        MVD_DEBUGVERBAL(MVD_PRINT("MVD: vol_time_incr_res=%d, fixed_vop_time_incr=%d\n", gvolInfo.vol_time_incr_res, gvolInfo.fixed_vop_time_incr));
    }
    else
    {
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("===> MVD: frameRate=%d  curCodecType=0x%x\n", pinfo->u32FrameRate, curCodecType));
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("u8MiuSel is %d !!, but return type is only Boolean. Please use MDrv_HVD_EX_GetDSBufMiuSelect() to get right miu select of DS buffer!\n", pMVDHalContext->stMiuCfg.u8FWMiuSel));
    }
...
E_MVD_Result MVD_RstFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType)
{
    ...
    //MVD_PRINT("%s u32DecFrmInfoAdd = 0x%x\n", __FUNCTION__, u32DecFrmInfoAdd);
    if (E_MVD_FRMINFO_DECODE == eType)
    {
...
static MS_BOOL MVD_GetFWBuffAdd(MS_VIRT u32Start, MS_U32 u32AvailLen, MVD_FWBuff* pBuff)
{
    ...
    if(pBuff == NULL)
    {
        MVD_DEBUGERROR(MVD_PRINT("%s err: NULL pBuff\n", __FUNCTION__));
        return FALSE;
    }
    ...
    MVD_DEBUGINFO(MVD_PRINT("set pu8MVDGetVolBufStart=%lx\n", (unsigned long)pBuff->pu8MVDGetVolBufStart));
    MVD_DEBUGINFO(MVD_PRINT("set pu8MVDGetFrameInfoBufStart=%lx\n", (unsigned long)pBuff->pu8MVDGetFrameInfoBufStart));
    MVD_DEBUGINFO(MVD_PRINT("set pu8MVDSetHeaderBufStart=%lx\n", (unsigned long)pBuff->pu8MVDSetHeaderBufStart));
    MVD_DEBUGINFO(MVD_PRINT("u32UserDataBuf start=%lx\n", (unsigned long)pBuff->u32UserDataBuf));
    MVD_DEBUGINFO(MVD_PRINT("u32MVDFWSLQTABTmpbufAdr start=%lx\n", (unsigned long)pBuff->u32MVDFWSLQTABTmpbufAdr));
    MVD_DEBUGINFO(MVD_PRINT("PtsTblAddr start=%lx\n", (unsigned long)pBuff->u32MVDFWPtsTblAddr));
    MVD_DEBUGINFO(MVD_PRINT("u32DynScalingAdd start=%lx\n", (unsigned long)pBuff->u32DynScalingAdd));
    MVD_DEBUGINFO(MVD_PRINT("ScalerInfo start=%lx end=%lx\n",
                            (unsigned long)pBuff->u32ScalerInfoAdd, (pBuff->u32ScalerInfoAdd + MVD_FW_SCALER_INFO_BUF_LEN)));
    MVD_DEBUGINFO(MVD_PRINT("DecFrmInfo start=%lx end=%lx\n",
    ...
    if ((pBuff->u32VBBUTableAdd + MVD_FW_VBBU_TABLE_LEN - u32Start) > FW_SHM_SIZE)
    {
        ...
        MVD_DEBUGERROR(MVD_PRINT("%s err: 0x%lx out of memory boundary!\n", __FUNCTION__,
...
static MS_BOOL MVD_SetFWBuffAdd(MS_U8 u8Idx, MVD_FWBuff* pBuff)
{
            ...
            && (curSrcMode != E_MVD_TS_MODE))
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("PTS tbl start=%lx end=%lx\n",
    ...
    if (pstMemCfg->bEnableDynScale)
    {
        HAL_MVD_SetDynamicScaleAddr(u8Idx, pBuff->u32DynScalingAdd, 0);
        HAL_MVD_EnableDynamicScale(u8Idx, 0); //default old DS style
        pCtrlCfg->u8DynScalingDepth = 32; //HAL_MVD_GetDynamicScaleDepth
        MVD_DEBUGINFO(MVD_PRINT("u8EnableMIUSel    = 0x%x\n", pMVDHalContext->stMiuCfg.u8FWMiuSel));
        MVD_DEBUGINFO(MVD_PRINT("u32DynScalingAddr= 0x%lx\n", (unsigned long)pBuff->u32DynScalingAdd));
        MVD_DEBUGINFO(MVD_PRINT("u8DynScalingDepth= 0x%x\n", pCtrlCfg->u8DynScalingDepth));
    }
    MVD_DEBUGINFO(MVD_PRINT("DynScaling start=%lx end=%lx\n",
    ...
    MVD_DEBUGINFO(MVD_PRINT("DecFrmInfo start=%lx\n", (unsigned long)pBuff->u32DecFrmInfoAdd));
...
E_MVD_Result HAL_MVD_GetFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType, MVD_FrmInfo* pInfo)
{
    ...
    if (NULL == pInfo)
    {
        MVD_DEBUGERROR(MVD_PRINT("GetFrmInfo NULL pInfo!\n"));
        return E_MVD_RET_INVALID_PARAM;
    }
    if (NULL == u32DecFrmInfoAdd)
    {
        MVD_DEBUGERROR(MVD_PRINT("GetFrmInfo NULL u32DecFrmInfoAdd!\n"));
        return E_MVD_RET_FAIL;
    }
    //MVD_PRINT("%s u32DecFrmInfoAdd = 0x%x\n", __FUNCTION__, u32DecFrmInfoAdd);
    ...
    else if (E_MVD_FRMINFO_NEXT_DISPLAY == eType)
    {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("NextDispFrm not ready!\n"));
            return E_MVD_RET_FAIL;
        }
        ...
        if(HAL_MVD_IsMcuMode(u8Idx) || HAL_MVD_IsDrvMcuMode(u8Idx))
        {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("GetFrmInfo no available frame!\n"));
            return E_MVD_RET_FAIL;
        }
...
#endif
#if 0
        MVD_PRINT("NxtFrm:: Idx=0x%x, ", pInfo->u16FrmIdx);
        MVD_PRINT("Type=0x%x, ", pInfo->eFrmType      );
        MVD_PRINT("Luma=0x%x, ", pInfo->u32LumaAddr  );
        MVD_PRINT("Chroma=0x%x, ", pInfo->u32ChromaAddr);
        MVD_PRINT("Pts=%lu, ", pInfo->u32TimeStamp );
        MVD_PRINT("ID_H=%lu, ", pInfo->u32ID_H      );
        MVD_PRINT("ID_L=0x%x\n", pInfo->u32ID_L      );
#endif
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("GetFrmInfo not ready!\n"));
        return E_MVD_RET_FAIL;
    }

    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FBMiuSel, (pInfo->u32LumaAddr * 8), pInfo->u32LumaAddr);
    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FBMiuSel, (pInfo->u32ChromaAddr * 8), pInfo->u32ChromaAddr);

    //MVD_PRINT("===> Luma=0x%x, Chroma=0x%x\n", pInfo->u32LumaAddr, pInfo->u32ChromaAddr);
    return eRet;
}
...
#define SCALER_INFO_TIMEOUT 0x1000
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s invalid para u32StAddr=0x%lx, u32Size=0x%lx\n",
                                 __FUNCTION__, (unsigned long)u32StAddr, (unsigned long)u32Size));
        return E_MVD_RET_INVALID_PARAM;
    }
    if (TRUE != pstMemCfg->bEnableDynScale)
    {
        MVD_DEBUGERROR(MVD_PRINT("%s !bEnableDynScale\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s fail to set ScalerInfoAdd\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s timeout!!!\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    MVD_DEBUGINFO(MVD_PRINT("=====> %s u32TimeOut = 0x%x\n", __FUNCTION__, u32TimeOut));
    return E_MVD_RET_OK;
}
...
static MS_BOOL MVD_CheckFrmBuffSizeMin(MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("Framebuffer size(0x%lx) < (0x%x+0x%lx)!\n",
...
static MS_U8 MVD_GetFbMaxNum(MS_U8 u8Idx)
{
    if(pMVDHalContext->gMVDPreCtrl[u8Idx].bEnableNDS)
    {
        return MVD_FBNUM_MAX_NDS;
    }
    else
    {
        return MVD_FBNUM_MAX;
    }
}
...
MS_BOOL HAL_MVD_CheckFrmBuffSize(MS_U8 u8Idx, MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    MS_U8 u8FBMaxNum = MVD_GetFbMaxNum(u8Idx);
    ...
    *pu8FBMode = MVD3_SD_MODE;
    ...
    if (MVD3_SD_MODE == *pu8FBMode)
    {
        ...
        *pu8FBNum = MVD_FBNUM_MIN;
        MVD_DEBUGINFO(MVD_PRINT("[MVD_SD_MODE] u8FBNum=%d, FBSize=0x%lx\n", *pu8FBNum, (unsigned long)memCfg->u32FBSize));
        goto _GET_USED_SIZE;
    }

    if (u32AvailFrmBuffSize >= (MVD_DHD_FBSIZE * u8FBMaxNum))
    {
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) DHD*5\n", __FUNCTION__, __LINE__));
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) DHD*4\n", __FUNCTION__, __LINE__));
        //Dual HD: (1920*2)*1088 or 1920*(1088*2)
        if (!_IS_VC1(fwCfg->eCodecType)) //mpeg2/4
        {
            *pu8FBMode = MVD3_DHD_MODE;
    ...
    else if (u32AvailFrmBuffSize >= (MVD_HD_FBSIZE * u8FBMaxNum))
    {
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) HD*5\n", __FUNCTION__, __LINE__));
        *pu8FBMode = MVD3_HD_MODE;
        *pu8FBNum = u8FBMaxNum;
    }
    else if (u32AvailFrmBuffSize >= (MVD_HD_FBSIZE * MVD_FBNUM_MIN))
    {
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) HD*4\n", __FUNCTION__, __LINE__));
        *pu8FBMode = MVD3_HD_MODE;
        *pu8FBNum = MVD_FBNUM_MIN;
    }
    else
    {
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) SD\n", __FUNCTION__, __LINE__));
        ret = MVD_CheckFrmBuffSizeMin(fwCfg, memCfg);
        if (TRUE == ret)
        {
            *pu8FBMode = MVD3_SD_MODE;
            *pu8FBNum = MVD_FBNUM_MIN;
            MVD_DEBUGINFO(MVD_PRINT("Framebuffer [SD] mode\n"));
        }
    }

    if(pMVDHalContext->gMVDPreCtrl[u8Idx].bEnableNDS == FALSE)
    {
...
_GET_USED_SIZE:
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u8FBMode=0x%x, u8FBNum=%d, FBSize=0x%lx, used=0x%lx\n",
...
MS_BOOL HAL_MVD_SetCodecInfo(MS_U8 u8Idx, MVD_CodecType eCodecType, MVD_SrcMode eSrcMode, MS_U8 bDisablePESParsing)
{
    ...
    stCmdArg.Arg3 = 0;
    MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetCodecInfo: Cmd: %x, Arg0: %x, Arg1: %x. Arg2: %x\n",
    ...
    if (u32Len == 0) MVD_PRINT("%s err: u32Len=0!\n", __FUNCTION__);
    i = u32Addr >> 3;
    SET_CMDARG(stCmdArg, i, u8Idx);
    SET_CMD_RET_FALSE(CMD_CODE_OFFSET, &stCmdArg);

#define _mvdAssert(x) if(!x) MVD_PRINT("%s(%d) inconsistent cfg!\n", __FUNCTION__, __LINE__);
    pCurFwCfg = HAL_MVD_GetFWCfg(u8Idx);
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_Init:_MVD_MVDSetInternalBuffAddr failed\n"));
        return FALSE;
    }
    else
    {
        MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_Init:_MVD_MVDSetInternalBuffAddr success\n"));
    }
...
void HAL_MVD_InitVar(MS_U8 u8Idx)
{
...
#ifndef MSOS_TYPE_LINUX_KERNEL//BDMA
    MVD_MEMCfg*  pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    ...
    MVD_DEBUGINFO(MVD_PRINT("bSlqTblSync = %x\n", pstCtrlCfg->bSlqTblSync));
    if (pstCtrlCfg->bSlqTblSync)
    {
        ...
        if ((pBDMA == NULL) || (pBDMA->bInit != TRUE))
        {
            ...
            {
                MVD_PRINT("%s fail at MDrv_BDMA_Init!!!\n", __FUNCTION__);
            }
        }
        pstSlqTblInfo->bdmaCpyType = MVD_GetBDMAType();
    }
#else
    pstCtrlCfg->bSlqTblSync = 0;
#endif
...
#define STOP_TIMEOUT 500 //ms
    ...
    if (HAL_MVD_MVDCommand(CMD_STOP, &mvdcmd) == FALSE)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("*** MVD ERR: STOP TIMEOUT!!! ***\n"));
        }
    }
...
MS_BOOL HAL_MVD_DeinitHW(VPU_EX_SourceType SourceType, VPU_EX_DecoderType eDecType)
{
    //MVD HW reset
    if (!HAL_MVD_RstHW())
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_Exit:MVD4ResetHW failed\n"));
    }
    else
    {
        MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_Exit:MVD4ResetHW success\n"));
    }
    HAL_MVD_PowerCtrl(DISABLE);
#ifdef CONFIG_MSTAR_CLKM
    if(eDecType == E_VPU_EX_DECODER_MVD)
    {
        HAL_VPU_EX_SetClkManagement(E_VPU_EX_CLKPORT_MVD_CORE, DISABLE);
    }
    if(SourceType == E_VPU_EX_INPUT_TSP)
    {
        HAL_VPU_EX_SetClkManagement(E_VPU_EX_CLKPORT_MVD_PAS, DISABLE);
    }
#endif

#ifdef CONFIG_MSTAR_SRAMPD
    HAL_MVD_RegWriteBit(REG_CODEC_SRAM_SD_EN, DISABLE, SRAM_SD_EN_MVD);
    HAL_MVD_Delayms(1);
#endif
    return TRUE;
}
...
MS_BOOL HAL_MVD_Exit(MS_U8 u8Idx)
{
    ...
    pMVDHalContext->bTrickPlay2xAVSync[u8Idx] = FALSE;
    pMVDHalContext->bEnSmoothScan[u8Idx] = FALSE;
...
{
    FW_DIVX_INFO* pDivxInfo;
    if (GET_HDR_BUFFADD(u8Idx) == 0)
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_SetFrameInfo error: pu8MVDSetHeaderBufStart=NULL\n"));
        return;
    }

    pDivxInfo = &(pMVDHalContext->gdivxInfo[u8Idx]);
...
//    pDivxInfo->invalidstream=0;
    MVD_DEBUGINFO(MVD_PRINT("set vol info,pts_incr=%d,\n", pDivxInfo->pts_incr));
    MVD_DEBUGINFO(MVD_PRINT("set vol info,width=%x,height=%x,frame_rate=%d,aspect_ratio=%x,\n",
                            (unsigned int)pDivxInfo->width, (unsigned int)pDivxInfo->height, pDivxInfo->frame_rate, pDivxInfo->aspect_ratio));
    MVD_DEBUGINFO(MVD_PRINT("set vol info,progressive_sequence=%x,mpeg1=%x,play_mode=%x,\n",
...
///////////////////////////////////////////////////////////////////////////////
MS_VIRT HAL_CC_CM_GetMVDRB_HWAddr(MS_U8 u8Idx, MS_U8 u8CC608)
{
#if 1
    MVD_CmdArg mvdcmd;
...
MS_BOOL HAL_MVD_GetUsrDataIsAvailable(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("IsAvail:%x rd=%lx wr=%lx\n", bIsAvail,
...
MS_BOOL HAL_MVD_GetUsrDataInfo(MS_U8 u8Idx, MVD_UsrDataInfo* pUsrInfo)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s: NULL ptr.\n", __FUNCTION__));
        return FALSE;
    }
...
#if 0
    MVD_PRINT("xxInfo: ");
    MVD_PRINT("%02d, ", pUsrInfo->u16TmpRef);
    MVD_PRINT("%d, ", pUsrInfo->u8PicStruct);
    MVD_PRINT("%d, ", pUsrInfo->u8TopFieldFirst);
    MVD_PRINT("0x%x, ", pUsrInfo->u32DataBuf);
    MVD_PRINT("%d, ", pUsrInfo->u8ByteCnt);
    MVD_PRINT("%ld, ", pUsrInfo->u32Pts);
    MVD_PRINT("%d\n", pUsrInfo->u8PicType);
#endif
    return TRUE;
}
...
MS_VIRT HAL_MVD_Map2DrvSlqTbl(MS_U8 u8Idx, MS_VIRT u32HWPtr)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("Invalid u32HWPtr=0x%lx\n", (unsigned long)u32HWPtr));
        return 0;
    }
...
static void MVD_SLQTblGetFileEndPkt(MS_U8 u8Idx, MVD_PacketInfo* pFileEnd)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("u32EndPattern(0x%lx)=0x%x 0x%x 0x%x 0x%x\n", (unsigned long)pFileEnd->u32StAddr,
...
#if 0
    MVD_PRINT("u32DummyES(0x%x-->0x%x, size=0x%x)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES,
...
MS_BOOL HAL_MVD_SLQTblInsertPattern(MS_U8 u8Idx, MVD_PatternType ePattern)
{
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("SLQTbl full!(0x%lx) Cannot insert pattern any more!\n", (unsigned long)pstSlqTblInfo->pDrvSlqTbl->u32Empty));
        return FALSE;
    }
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("Invalid MVD_PatternType! Won't insert pattern!\n"));
            return FALSE;
        }
#if SLQ_NEW_PUSH
        pstSlqTblInfo->pSlqStatus->bSlqFireRdy = TRUE;
#endif
        //MVD_PRINT("WrPtr 0x%x ", pstSlqTblInfo->pDrvSlqTbl->u32WrPtr);
...
E_MVD_Result HAL_MVD_IsDispFinish(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("MDrv_MVD_IsDispFinish::");
    if (GET_FRMINFO_BUFFADD(u8Idx) == 0)
    {
        MVD_DEBUGERROR(MVD_PRINT("%s err: pu8MVDGetFrameInfoBufStart=NULL\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
...
#ifdef VDEC3
        if(HAL_MVD_IsMcuMode(u8Idx) == TRUE)
        {
            u32TimeCnt = HAL_MVD_GetTime();
            ...
            if (TRUE != FLAG_LAST_FRM_SHOW)
            {
                MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_IsDispFinish TIMEOUT\n"));
                    ...
                    (HAL_MVD_IsMStreamerMode(u8Idx) == FALSE))
            {
                ...
                if ((HAL_MVD_GetTime() - u32TimeCnt) >= CMD_TIMEOUT_MS)
                {
                    ...
                    if (E_MVD_CODEC_VC1_MAIN == HAL_MVD_GetCodecType(u8Idx))
                    {
                        ...
                        {
                            MVD_DEBUGERROR(MVD_PRINT("RCV payloadLen(0x%x) invalid!\n",
                                                     HAL_MVD_GetPayloadLen(u8Idx)));
                        }
                    }
                    MVD_DEBUGERROR(MVD_PRINT("***** fe=%lx, rd=%lx(%lx,%lx), wr=%lx, empty=%lx, u32FeByteCnt=%x\n",
...
static void HAL_MVD_SetSLQStartEnd(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32end)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("MDrv_MVD_SetSLQStartEnd: only support 27bit add!\n"));
    }
...
MS_BOOL HAL_MVD_DecodeIFrame(MS_U8 u8Idx, MS_PHY u32FrameBufAddr, MS_PHY u32StreamBufAddr, MS_PHY u32StreamBufEndAddr )
{
    ...
    {
        MVD_DEBUGERROR( MVD_PRINT( "HAL_MVD_StepDecode fail!!\r\n") );
        return FALSE;
    }
...
#define WAIT_INIT_SUCCESS_TIME 100 //100ms
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s: wait init_success timeout!!!\n", __FUNCTION__));
    }
    HAL_MVD_SetSLQStartEnd(u8Idx, u32StreamBufAddr, u32StreamBufEndAddr);
    MVD_DEBUGINFO(MVD_PRINT("set MVD3_FW_SLQ_TAB_TMPBUF_ADR=%lx\n", (unsigned long)u32MVDFWSLQTABTmpbufAdr));
...
#define WAIT_DECODE_DONE_TIME 33 //To decode 1 frame should take less than 33ms
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT ("MDrv_MVD_DecodeIFrame time out(du=%d, st=%d, now=%d)\n", u32time, u32deley, HAL_MVD_GetTime()));
        MVD_DEBUGERROR(MVD_PRINT("frmCnt=%d state=0x%x lastCmd=0x%x\n", HAL_MVD_GetPicCounter(u8Idx), HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx)));
        return FALSE;
    }
    MVD_DEBUGINFO(MVD_PRINT ("MDrv_MVD_DecodeIFrame time (%d, %d)\n", u32time, u32deley));
    //MVD_PRINT("frmCnt=%ld state=0x%x lastCmd=0x%x\n", HAL_MVD_GetPicCounter(u8Idx), HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx));
...
MS_BOOL HAL_MVD_SetInternalBuffAddr(MS_U8 u8Idx, MS_VIRT u32start, MS_U32 u32len)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("MVD FW shared mem start = 0x%lx\n", (unsigned long)u32BuffStart));
    ...
    MVD_DEBUGINFO(MVD_PRINT("set MVD_FRAMEBUFFER_ADR=%lx\n", (unsigned long)tmpAdr));
...
// The reason is that these 3 buffers are used by MVD HW engine.
    if(pMVDHalContext->bCMAUsed == FALSE)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("MVD HW buffers larger than FB size!!!\n"));
        }
        MVD_DEBUGINFO(MVD_PRINT("MVD FB boundary =0x%lx\n", (unsigned long)tmpAdr));
        tmpAdr = HAL_MVD_SetHWBuffer(u8Idx, tmpAdr);
    }

    HAL_MVD_MemGetMap(u8Idx, E_MVD_MMAP_BS, &tmpAdr, &tmpLen);
    MVD_DEBUGINFO(MVD_PRINT("set MVD_BITSTREAM_ADR=%lx\n", (unsigned long)tmpAdr));
            ...
            || (pFwCfg->stFBReduction.ChromaFBReductionMode  != E_MVD_FB_REDUCTION_NONE))
    {
        ...
        MVD_DEBUGERROR(MVD_PRINT("MVD Err: Not support FB reduction mode!!\n"));
    }
...
static void _SLQTbl_DumpPtsTbl(MS_U8 u8Idx, MS_U32 u32EntryStart, MS_U32 u32EntryEnd)
{
    ...
    for (i = u32EntryStart; i < u32EntryEnd; i++)
    {
        ...
        MVD_PRINT("ES[%x] Start=0x%x End=0x%x u32EsRp=%x\n",
                  i, u32EsStart, u32EsEnd, u32EsRp);
    }

    MVD_PRINT("\n=======Dump PTS table========\n");
    MVD_PRINT("addr\t byte_cnt\t dummy_cnt\t id_low\t id_high\t time_stamp\n");
    for (i = u32EntryStart; i < u32EntryEnd; i++)
    {
        MVD_PRINT("0x%x\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\n", u32MVDFWPtsTblAddr + i * MVD_FW_SLQTBL_PTS_LEN,
...
#endif
{
    ...
    //MVD_PRINT("u32DivXPattern(0x%x==>0x%x)=0x%x 0x%x\n", u32DivXPattern, pDivxHdr->u32StAddr,
...
#if SLQ_NEW_PUSH
    if(pstSlqTblInfo->pSlqStatus->bSlqPicStart && HAL_MVD_IsMcuMode(u8Idx) == FALSE)
    {
        if (E_MVD_CODEC_VC1_ADV == curCodecType)
        {
            pstSlqTblInfo->pDrvDivxTbl->u32WrPtr += 4;
...
{
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_PHY u32EndPattern = HAL_MVD_GetMemOffset(pstMemCfg->u32DrvBufAddr + SLQ_TBL_SIZE * 3);
    MS_PHY temp = 0;
    ...
    if ((E_MVD_CODEC_FLV == eType) || (E_MVD_CODEC_MPEG4_SHORT_VIDEO_HEADER == eType))
    {
        ...
        //MVD_PRINT("##########FileEnd for FLV/SVH!, u32EndPattern=%x\n",u32EndPattern);
    }
    else if (E_MVD_CODEC_DIVX311 == eType)
    {
        ...
        HAL_MVD_MemWrite4Byte(u32EndPattern + 12, END_PATTERN_2); //scw
        HAL_MVD_MemWrite4Byte(u32EndPattern + 16, END_PATTERN_3); //scw
        //MVD_PRINT("##########FileEnd for DIVX311!, u32EndPattern=%x\n",u32EndPattern);
    }
    else if ((E_MVD_CODEC_MPEG2 == eType) || (E_MVD_CODEC_MPEG4 == eType))
    {
        ...
        //MVD_PRINT("##########FileEnd for MPEG2/4!, u32EndPattern=%x\n",u32EndPattern);
    }
    else
    {
        ...
        //MVD_PRINT("##########FileEnd for VC1!, u32EndPattern=%x\n",u32EndPattern);
    }

    MVD_DEBUGINFO(MVD_PRINT("u32EndPattern(0x%lx)=0x%x 0x%x\n", (unsigned long)u32EndPattern,
                            HAL_MVD_MemRead4Byte(u32EndPattern), HAL_MVD_MemRead4Byte(u32EndPattern + 4)));
}
...
{
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_PHY u32DummyES = HAL_MVD_GetMemOffset(pstMemCfg->u32DrvBufAddr + SLQ_TBL_SIZE * 2);
    MS_U32 u32DummyPattern[3];
    MS_U32 u32PatternSize;
    MS_U32 i;
    MS_PHY temp = 0;

    //MVD_PRINT("eType = 0x%x\n", eType);
...
#if 0
    MVD_PRINT("u32DummyES(0x%x)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES, HAL_MVD_MemRead4Byte(u32DummyES),
              HAL_MVD_MemRead4Byte(u32DummyES + 4), HAL_MVD_MemRead4Byte(u32DummyES + 8), HAL_MVD_MemRead4Byte(u32DummyES + 12));
#endif
...
MS_BOOL HAL_MVD_SLQTblInit(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s: unknow codec type!\n", __FUNCTION__));
        return FALSE;
    }
...
MS_U32 HAL_MVD_GetQueueVacancy(MS_U8 u8Idx, MS_BOOL bCached)
{
    ...
    //MVD_PRINT("QV=0x%x rd=0x%x==>", u32Empty, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr);
    if (pstSlqTblInfo->pDrvSlqTbl->u32RdPtr >= (pstSlqTblInfo->pDrvSlqTbl->u32StAdd + SLQ_ENTRY_LEN))
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("%s: readPtr 0x%lx out of range: too large!\n",
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s: readPtr 0x%lx out of range: too small!\n",
                                 __FUNCTION__, (unsigned long)pstSlqTblInfo->pDrvSlqTbl->u32RdPtr));
    ...
    //MVD_PRINT("%s r=0x%x w=0x%x u32Empty=0x%x\n", __FUNCTION__,
...
E_MVD_Result HAL_MVD_PushQueue(MS_U8 u8Idx, MVD_PacketInfo* pInfo)
{
    ...
    if (pInfo == NULL)
    {
        MVD_DEBUGERROR(MVD_PRINT("PushQueue NULL pInfo\n"));
        return E_MVD_RET_INVALID_PARAM;
    }
...
#endif
    {
        MS_U8* temp = (MS_U8*)(MS_PA2KSEG1( (MS_PHY)(pInfo->u32StAddr) + pstMemCfg->u32BSAddr));
        MS_U32 i;
        MS_U32 u32DummyPattern[3];
        MS_U32 u32PatternSize;
...
#if _SLQTBL_DUMP_PUSHQ
    {
        ...
        MVD_PRINT("Push[%x] st=0x%x len=0x%x pts=0x%x\n", pMVDHalContext->u32pqTimes[u8Idx]++,
    ...
    if (pInfo->u32StAddr >= pstMemCfg->u32BSSize)
    {
        ...
        MVD_DEBUGERROR(MVD_PRINT("PushQueue invalid u32StAddr=0x%lx, bsSize=0x%lx\n", (unsigned long)pInfo->u32StAddr, (unsigned long)pstMemCfg->u32BSSize));
        return E_MVD_RET_INVALID_PARAM;
    }
    ...
    else
    {
        ...
        MVD_DEBUGERROR(MVD_PRINT("PushQueue FULL!!! empty=0x%lx\n", (unsigned long)pstSlqTblInfo->pDrvSlqTbl->u32Empty));
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("%s ret = %d\n", __FUNCTION__, eRet));
    }
    return eRet;
}
...
#define MAX_VALIDLEN    0x200000    //2M
    ...
    else
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("(%x) ValidLen=0x%x CurLen=0x%x\n",
...
static E_MVD_Result MVD_FlushSlqTblQueue(MS_U8 u8Idx)
{
            ...
            (TRUE != MVD_PatternLenIsValid(u8Idx, u32PatternByteCnt)))
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("RCV timeout!!! pl=%d, pbc=%d, emp=%ld\n",
    ...
    if (TRUE != HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_PAUSE))
    {
        ...
        MVD_DEBUGERROR(MVD_PRINT("ValidLen=0x%x CurPatternLen=0x%x\n",
                                 HAL_MVD_GetPayloadLen(u8Idx), u32PatternByteCnt));
...
E_MVD_Result HAL_MVD_FlushQueue(MS_U8 u8Idx)
{
    ...
    else if (pMVDHalContext->gMVDPreCtrl[u8Idx].bEnableNDS && (E_MVD_TS_MODE == curSrcMode))
    {
        // to support flush es buffer in DTV mode
        // need fw support (new flush rule + DTV mode modify)
        eRet = MVD_FlushTSQueue(u8Idx);
    }
...
MS_BOOL HAL_MVD_EnableInt(MS_U8 u8Idx, MS_U32 bEn)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("u32IntFlag = 0x%x\n", u32IntFlag));
    SETUP_CMDARG(mvdcmd);
...
E_MVD_Result HAL_MVD_DisablePBFrameMode(MS_U8 u8Idx, MS_BOOL bEn)
{
    ...
    SET_DECNUM(stCmdArg, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_DISABLE_PATCH_PBFRAME, &stCmdArg) == FALSE)
    {
        MVD_DEBUGERROR( MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_PTS_DETECTOR_EN ) );
        return E_MVD_RET_FAIL;
    }
...
E_MVD_Result HAL_MVD_SetDisableSeqChange(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("MVD CMD_FORBID_RESOLUTION_CHANGE(0x%x) OK\n", bEnable));
...
MS_U8 HAL_MVD_GetDispRdy(MS_U8 u8Idx)
{
    ...
    if(HAL_MVD_IsSeqHeaderFound(u8Idx))
    {
        return 1;
    }
    else
...
MS_U32 HAL_MVD_GetParserByteCnt(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("  parser byte count = %lu byte \n", u32Cnt);
    return u32Cnt;
}
...
MS_VIRT MVD_GetSlqTblESReadPtr(MS_U8 u8Idx)
{
    ...
    MS_U32 u32EsEnd;
    u32EsEnd = HAL_MVD_MemRead4Byte(u32EsRp + 4);
    MVD_PRINT("GetESReadPtr ES[%x] Start=0x%x End=0x%x u32EsRp=%x u32SlqRp=%x\n",
    ...
    if ((_IsNotInStreamBuff(u32EsStart)) && (u32EsStart != 0))
    {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("0x%lx Not in BS, report u32PreEsRd=0x%lx\n", (unsigned long)u32EsStart, (unsigned long)pstSlqTblInfo->u32PreEsRd));
            return (pstSlqTblInfo->u32PreEsRd);
        }
    }
...
MS_VIRT MVD_GetSlqTblESWritePtr(MS_U8 u8Idx)
{
...
#if 0
    MVD_PRINT("GetESWritePtr[%x] ES Start=0x%x End=0x%x u32EsWp=%x\n",
    ...
    if ((_IsNotInStreamBuff(u32EsEnd)) && (u32EsEnd != 0))
    {
        ...
        //Report the last ESRead, instead of this one.
        MVD_DEBUGINFO(MVD_PRINT("0x%lx Not in BS, report u32PreEsWr=0x%lx\n", (unsigned long)u32EsEnd, (unsigned long)pstSlqTblInfo->u32PreEsWr));
...
MS_VIRT HAL_MVD_GetSLQReadPtr(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("##### HAL_MVD_GetSLQReadPtr 0x%x\n", readPtr);
...
MS_BOOL HAL_MVD_SetSLQWritePtr(MS_U8 u8Idx, MS_BOOL bCheckData)
{
    ...
    if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
    {
        ...
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_GetDecodeStatus(u8Idx) != E_MVD_STAT_IDLE)
            {
                ...
                MVD_DEBUGVERBAL(MVD_PRINT("time=0x%x ms, ", (HAL_MVD_GetTime() - u32TimeCnt)));
                break;
            }
        }
        if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
        {
            MVD_DEBUGERROR(MVD_PRINT("%s: err timeout(%d)! stat=0x%x\n", __FUNCTION__,
                                     HAL_MVD_GetTime() - u32TimeCnt, HAL_MVD_GetDecodeStatus(u8Idx)));
            return FALSE;
        }
        else
        {
            MVD_DEBUGVERBAL(MVD_PRINT("%s: ready to update WrPtr.\n", __FUNCTION__));
        }
    }
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("**** pstSlqTblInfo->pSlqStatus->bSlqFireRdy is not Ready ** \n"));
        return FALSE;
    }
    pstSlqTblInfo->pSlqStatus->bSlqFireRdy = FALSE;
#endif
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("**** SlqWrPtr(0x%lx) is not update!(%x, %x) ****\n",
    ...
    if (pstCtrlCfg->bSlqTblSync)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_PRINT("%s MDrv_BDMA_MemCopy fail ret=%x!\n", __FUNCTION__, bdmaRlt));
        }
#endif
        MVD_DEBUGINFO(MVD_PRINT("SlqWrPtr_BDMA src=0x%lx dst=0x%lx size=0x%lx cpyType=0x%x\n",
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    MVD_PRINT("state: fw=0x%x, lastCmd=0x%x, pc=0x%x\n",
              HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx), HAL_VPU_EX_GetProgCnt());
    MVD_PRINT("cnt: dec=%d, skip=%d, drop=%ld\n", HAL_MVD_GetPicCounter(u8Idx), HAL_MVD_GetSkipPicCounter(u8Idx), 0x0UL);
    HAL_MVD_GetFrmInfo(u8Idx, E_MVD_FRMINFO_DECODE, &stFrm);
    MVD_PRINT("frm Dec Y=%lx UV=%lx Pitch=%x; ", (unsigned long)stFrm.u32LumaAddr, (unsigned long)stFrm.u32ChromaAddr, stFrm.u16Pitch);
    HAL_MVD_GetFrmInfo(u8Idx, E_MVD_FRMINFO_DISPLAY, &stFrm);
    MVD_PRINT("Disp Y=%lx UV=%lx Pitch=%x\n", (unsigned long)stFrm.u32LumaAddr, (unsigned long)stFrm.u32ChromaAddr, stFrm.u16Pitch);
    if (curSrcMode == E_MVD_SLQ_TBL_MODE)
    {
#ifndef VDEC3
        MVD_PRINT("fe=%x, rd=%x(%x), wr=%x, empty=%x; ", pstSlqTblInfo->u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr,
...
void MVD_DumpMVDStream(void)
{
    ...
    for (i = 0; i < MAX_DEC_NUM; i++)
    {
        MVD_DEBUGINFO(MVD_PRINT("[%d] eStreamId=0x%x(%s) bUsed=0x%x\n", i, pMVDHalContext->_stMVDStream[i].eStreamId,
...
MS_BOOL HAL_MVD_Init_Share_Mem(void)
{
                                 ...
                                 MSOS_SHM_QUERY))
    {
        if (FALSE == MsOS_SHM_GetId((MS_U8*)"Linux MVD HAL",
                                    ...
                                    MSOS_SHM_CREATE))
        {
            MVD_PRINT("[%s]SHM allocation failed!!! use global structure instead!!!\n", __FUNCTION__);
            if(pMVDHalContext == NULL)
            {
...
MS_BOOL HAL_MVD_ReleaseFdMask(MS_U8 u8Idx, MS_BOOL bRls)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s: release=0x%x\n", __FUNCTION__, bRls));
    SET_DECNUM(mvdcmd, u8Idx);
...
MS_BOOL HAL_MVD_SetAutoMute(MS_U8 u8Idx, MS_BOOL bEn)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s: bEn=%x\n", __FUNCTION__, bEn));
    SET_DECNUM(mvdcmd, u8Idx);
...
MS_BOOL HAL_MVD_GetColorInfo(MS_U8 u8Idx, MVD_Color_Info* pstColorInfo)
{
    ...
    if((pu8MVDGetFrameInfoBufStart == 0) || (pstColorInfo == 0))
    {
        MVD_DEBUGERROR(MVD_PRINT("HAL_MVD_GetColorInfo error: pu8MVDGetFrameInfoBufStart=NULL\n"));
        return FALSE;
    }

    pstColorInfo->bColor_Descript = (MS_BOOL)(HAL_MVD_MemReadByte(pu8MVDGetFrameInfoBufStart + OFFSET_COLOR_DESCRIPT));

    if(pstColorInfo->bColor_Descript)
    {
        pstColorInfo->u8Color_Primaries = HAL_MVD_MemReadByte(pu8MVDGetFrameInfoBufStart + OFFSET_COLOR_PRIMARIES);
        pstColorInfo->u8Transfer_Char = HAL_MVD_MemReadByte(pu8MVDGetFrameInfoBufStart + OFFSET_TRANSFER_CHAR);
        pstColorInfo->u8Matrix_Coef = HAL_MVD_MemReadByte(pu8MVDGetFrameInfoBufStart + OFFSET_MATRIX_COEF);
    }
    return TRUE;
...
MS_BOOL _HAL_MVD_CrcTimeOut(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGERROR( MVD_PRINT("_HAL_MVD_CrcTimeOut=%x\n", i) );
    return TRUE;
}
...
E_MVD_Result HAL_MVD_GetCrcValue(MS_U8 u8Idx, MVD_CrcIn *pCrcIn, MVD_CrcOut *pCrcOut)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s width=0x%x, height=0x%x, pitch=0x%x, luma=0x%lx, chroma=0x%lx\n",
...
MS_BOOL HAL_MVD_SetIdctMode(MS_U8 u8Idx, MS_U8 u8Mode)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s CMD_IDCT_SEL arg0=%x)\n", __FUNCTION__, u8Mode));
...
{
    UNUSED(eClockSpeed);
    MVD_PRINT("This chip does not support mvd clock presetting~\n");
    return E_MVD_RET_FAIL;
}
...
MS_BOOL HAL_MVD_SetExternalDSBuff(MS_U8 u8Idx, MS_VIRT u32VPUAddr, MS_VIRT u32DrvAddr, MS_U32 u32DSBufSize)
{
    MS_BOOL bRet = TRUE;
    MVD_FWBuff* pBuff = &(pMVDHalContext->stFWBuff[u8Idx]);
    MS_U8 u8Miu = 0;
    ...
    _phy_to_miu_offset(u8Miu, u32Offset, u32DrvAddr);
    UNUSED(u8Miu);
    pBuff->u32DynScalingAdd = u32Offset;
    bRet = HAL_MVD_SetDynamicScaleAddr(u8Idx, u32VPUAddr, u32DSBufSize);
    return bRet;
}
...
MS_BOOL HAL_MVD_ShowFirstFrameDirect(MS_U8 u8Idx, MS_U8 bEnable)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s CMD_PUSH_FIRST_FRAME_DISP arg0=%x)\n", __FUNCTION__, bEnable));
...
MS_BOOL HAL_MVD_Field_Polarity_Display_One_field(MS_U8 u8Idx, MS_BOOL bEn, MS_U8 top_bottom)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s: bEn=%x,top_bottom=%x\n", __FUNCTION__, bEn, top_bottom));
    SET_DECNUM(mvdcmd, u8Idx);
...
MS_BOOL HAL_MVD_SetExternal_CC_Buffer(MS_U8 u8Idx, MS_VIRT u32base, MS_U8 u8size, MS_U8 cc_type)
{
    ...
    MS_U32 *pu32ReadPtr;
    MS_U32 *pu32WritePtr;
    MS_U32 *pu32LatchOverflow;
    MS_U32 *pu32CpuBaseAddr;
    ...
    pu32ReadPtr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base));
    pu32WritePtr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base + sizeof(MS_U32)));
    pu32LatchOverflow = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base + sizeof(MS_U32) * 2));
    pu32CpuBaseAddr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base + sizeof(MS_U32) * 3));

    //rptr,wptr,overflow,latch 16 bytes
    if(cc_type == 1)
    {
        u32base += (4 * sizeof(MS_U32));
        u32cc_size -= (4 * sizeof(MS_U32));
    }
    ...
    *pu32ReadPtr = *pu32WritePtr = u32base;
    *pu32LatchOverflow = 0;
    *pu32CpuBaseAddr = HAL_MVD_GetMEMCfg(u8Idx)->u32FWBinAddr;
    ...
    //MVD_DEBUGINFO(MVD_PRINT("%s: u32base=%x,u32size=%x,cc_type=%x\n", __FUNCTION__,u32base,u32cc_size,cc_type));
...
MS_BOOL HAL_MVD_GetPVRSeamlessInfo(MS_U8 u8Idx, void* param)
{
    ...
    MVD_PVR_Seamless_Info* temp = (MVD_PVR_Seamless_Info*)param;

    temp->u64PTS = ((MS_U64)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PVRSEAMLESSTARGETPTS, sizeof(MS_U32))) |
                   (((MS_U64)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PVRSEAMLESSTARGETPTSHIGH, sizeof(MS_U8))) << 32);

    temp->u32POC = 0xFF; // not support in mvd
    temp->u8FrameType = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PVRSEAMLESSTARGETFRAMETYPE, sizeof(MS_U8));
...
E_MVD_Result HAL_MVD_SetTimeIncPredictParam(MS_U8 u8Idx, MS_U32 u32time)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("CMD_TIME_INCR_PREDICT=%x\n", u32time));
...
E_MVD_Result HAL_MVD_Set_Smooth_Rewind(MS_U8 u8Idx, MS_U8 btype)
{
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = btype;
    MVD_DEBUGINFO(MVD_PRINT("%s: btype=%x\n", __FUNCTION__, btype));
    SET_DECNUM(mvdcmd, u8Idx);
...
E_MVD_Result HAL_MVD_IsAlive(MS_U8 u8Idx)
{
    ...
    {
        if(pMVDHalContext->u32LastAliveTime[u8Idx] == 0 || ((MsOS_GetSystemTime() - pMVDHalContext->u32LastAliveTime[u8Idx]) < 500))
        {
    ...
    if((pAliveInfo->u32decode_count != (MS_U32)(MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_DECODEDONE_COUNT, sizeof(MS_U32)))) ||
            ...
            (pAliveInfo->u32IdleCount != (MS_U32)(MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_IDLE_COUNT, sizeof(MS_U32)))))
    {
...
#define STOP_TIMEOUT 500 //ms
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = u8Arg;
    MVD_DEBUGINFO(MVD_PRINT("%s: arg=%d\n", __FUNCTION__, (unsigned int)u8Arg));
    SET_DECNUM(mvdcmd, u8Idx);
    ...
    if(u8Arg == 1)
    {
        ...
        while(1)
        {
            ...
            if((HAL_MVD_GetTime() - u32StartTime) > STOP_TIMEOUT)
            {
                MVD_DEBUGERROR(MVD_PRINT("PVR seamless pause timeout\n" ) );
                ret = E_MVD_RET_FAIL;
                break;
            }
        }
    ...
    else if(u8Arg == 2)
    {
        while(1)
        {
            ...
            if((HAL_MVD_GetTime() - u32StartTime) > STOP_TIMEOUT)
            {
                MVD_DEBUGERROR(MVD_PRINT("PVR seamless hw reset timeout\n" ) );
                ret = E_MVD_RET_FAIL;
                break;
            }
        }
...
E_MVD_Result HAL_MVD_SetDVXCShmAddr(MS_U8 u8Idx, MS_PHY u32FWBaseAddr, MS_PHY u32DVXCShmAddr)
{
    E_MVD_Result ret = E_MVD_RET_OK;
    ...
    {
        MVD_DEBUGERROR(MVD_PRINT("[ERROR] XC share memory and VDEC Code base MIU selections are different\n"));
        return E_MVD_RET_FAIL;
    }

    u32temp = u32XCShmStartOffset - u32FWBaseOffset;

    SETUP_CMDARG(mvdcmd);

    mvdcmd.Arg0 = L_WORD(u32temp);
    mvdcmd.Arg1 = H_WORD(u32temp);
    mvdcmd.Arg2 = L_DWORD(u32temp);
    mvdcmd.Arg3 = H_DWORD(u32temp);

    MVD_DEBUGINFO(MVD_PRINT("%s: arg=%x\n", __FUNCTION__, (unsigned int)u32temp));

    SET_DECNUM(mvdcmd, u8Idx);

    if (HAL_MVD_MVDCommand(CMD_SET_DV_XC_SHM_ADDR, &mvdcmd) == FALSE)
    {
...
E_MVD_Result HAL_MVD_SetDisplayFinishMode(MS_U8 u8Idx, MS_U8 u8Mode)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s CMD_ENABLE_LAST_FRAME_QUALIFIER arg0=%x\n", __FUNCTION__, u8Mode));
...
E_MVD_Result HAL_MVD_Set_SlowSyncParam(MS_U8 u8Idx, MS_U8 u8RepeatPeriod, MS_U8 u8DropPeriod)
{
    ...
    mvdcmd.Arg0 = u8RepeatPeriod;

    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_SLOW_SYNC_REPEAT, &mvdcmd) == FALSE)
    {
    ...
    mvdcmd.Arg0 = u8DropPeriod;

    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_SLOW_SYNC_SKIP, &mvdcmd) == FALSE)
    {
        MVD_DEBUGERROR( MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_SLOW_SYNC_SKIP ) );
        return E_MVD_RET_FAIL;
    }
...
MS_BOOL HAL_MVD_VariableFrameRate(MS_U8 u8Idx)
{
    ...
    mvdcmd.Arg0 = 1;
    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_VARIABLE_FRAMERATE, &mvdcmd) == FALSE)
    {
...
MS_BOOL HAL_MVD_FRC_OnlyShowTopField(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable; //1 -> show Only Show TopField.  0 -> normal case.
    SET_DECNUM(mvdcmd, u8Idx);
    SET_CMD_RET_FALSE(CMD_FRC_ONLY_SHOW_TOP_FIELD, &mvdcmd);
    return TRUE;
}

MS_BOOL HAL_MVD_DisableESFullStop(MS_U8 u8Idx, MS_BOOL bDisable)
{
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bDisable;
    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand( CMD_ES_FULL_STOP, &mvdcmd ) == FALSE)
    {
        MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_ES_FULL_STOP);
        return FALSE;
    }
    return TRUE;
}

MS_BOOL HAL_MVD_PUSI_Control(MS_U8 u8Idx, MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable;
...
{
    pMVDHalContext->bTrickPlay2xAVSync[u8Idx] = bEnable;
}

void HAL_MVD_EnSmoothScan(MS_U8 u8Idx, MS_BOOL bEnable)
{
    pMVDHalContext->bEnSmoothScan[u8Idx] = bEnable;
}
...
MS_BOOL HAL_MVD_SetAVSyncDispAutoDrop(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s CMD_AUTO_DROP_FRAME_IN_DECODE arg0=%x\n", __FUNCTION__, bEnable));
...
MS_BOOL HAL_MVD_SetDynmcDispPath(MS_U8 u8Idx, MS_BOOL bConnect, MVD_DISPLAY_PATH eValue, MS_BOOL bPreSet)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s CMD_DYNAMIC_MVOP_CONNECT arg0=%x, arg1=%x\n", __FUNCTION__, bConnect, eValue));
...
MS_U32 HAL_MVD_GetSTC(MS_U8 u8Idx)
{
    MS_U32 u32STC = 0;

    u32STC = (MS_U32)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_DISP_STC, sizeof(MS_U32));
    u32STC = _90K_TO_MS(u32STC);
    return u32STC;
}
MS_U32 HAL_MVD_GetBBUId(MS_U8 u8Idx)
{
    return pMVDHalContext->_stMVDStream[u8Idx].u32SLQId;
}

void HAL_MVD_GetIPBV(MS_U8 u8Idx, void* pCounter)
{
    VDEC_EX_MDBCounter* temp = (VDEC_EX_MDBCounter*)pCounter;
    temp->u32ICounter = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_IFRAMECOUNT, sizeof(MS_U32));
    temp->u32PCounter = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PFRAMECOUNT, sizeof(MS_U32));
    temp->u32BCounter = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_BFRAMECOUNT, sizeof(MS_U32));
    temp->u32VCounter = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_VSYNCCOUNT, sizeof(MS_U32));
    temp->u8SeqChgCounter = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_SEQCHGCOUNT, sizeof(MS_U8));
}

void HAL_MVD_GetPerf(MS_U8 u8Idx, void* pPerf)
{
    VDEC_EX_MDBPerformance* temp = (VDEC_EX_MDBPerformance*)pPerf;
    temp->u32CreateTask = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_CREATETASK, sizeof(MS_U32));
    temp->u32Play = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PLAY, sizeof(MS_U32));
    temp->u32ReceiveData = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_RECEIVEDATA, sizeof(MS_U32));
    temp->u32SeqHeadFound = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_SEQHEADERFOUND, sizeof(MS_U32));
    temp->u32FirstIDecode = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_DECODEFIRSTI, sizeof(MS_U32));
    temp->u32FirstFramePush = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_PUSHFIRSTI, sizeof(MS_U32));
    temp->u32FirstVsync = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FIRSTVSYNC, sizeof(MS_U32));
    temp->u32FirstFrameDisp = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FIRSTFRAMEDISP, sizeof(MS_U32));
}

MS_U32 HAL_MVD_GetFBNum(MS_U8 u8Idx)
{
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FBNUM, sizeof(MS_U8));
}

MS_BOOL HAL_MVD_GetTFF(MS_U8 u8Idx)
{
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_TFF, sizeof(MS_U8));
}

MS_BOOL HAL_MVD_GetBlueScreen(MS_U8 u8Idx)
{
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_BLUESCREEN, sizeof(MS_U8));
}

MS_BOOL HAL_MVD_SetTFF(MS_U8 u8Idx, MS_BOOL bTFF)
{
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = 1;
    mvdcmd.Arg1 = bTFF;
    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand( CMD_TOPFIELD_FIRST_CONTROL, &mvdcmd ) == FALSE)
    {
        MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_TOPFIELD_FIRST_CONTROL);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL HAL_MVD_Parser_Control(MS_U8 u8Idx, MS_BOOL bEnable)
{
#define MVD_PARSER_BYPASS 2
#define MVD_PARSER_ENABLE 3
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    if(bEnable == TRUE)
    {
        mvdcmd.Arg0 = MVD_PARSER_ENABLE;
    }
    else
    {
        mvdcmd.Arg0 = MVD_PARSER_BYPASS;
    }
    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand( CMD_PUSI_CONTROL, &mvdcmd ) == FALSE)
    {
...
MS_BOOL HAL_MVD_ErrConceal_Control(MS_U8 u8Idx, MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable;
    SET_DECNUM(mvdcmd, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_ERROR_CONCEALMENT_CONTROL, &mvdcmd ) == FALSE)
    {
        MVD_PRINT( "Command: 0x%x fail!!\r\n", CMD_ERROR_CONCEALMENT_CONTROL);
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL HAL_MVD_PreConnectInputTsp(MS_U8 u8Idx, MS_BOOL bEnable, MVD_INPUT_TSP eInputTsp, MVD_Original_Stream eStream)
{
#ifdef CMD_PRESET_CONNET_INPUT_TSP

    MVD_SrcMode curSrcMode = HAL_MVD_GetSrcMode(u8Idx);

    if (curSrcMode == E_MVD_TS_MODE)
    {
        MVD_CmdArg mvdcmd;
        ...
        if (bEnable)
        {
            u8Input = (MS_U8)eInputTsp;
            VPRINTF("[NDec][%d] MVD preset tsp, eInputTsp %d\n", u8Idx, eInputTsp);
        }
        else
        {
            switch (eStream)
            {
                case E_MVD_ORIGINAL_MAIN_STREAM:
                    u8Input = E_CTL_INPUT_TSP_0;
                    break;
                case E_MVD_ORIGINAL_SUB_STREAM:
                    u8Input = E_CTL_INPUT_TSP_1;
                    break;
                case E_MVD_ORIGINAL_N_STREAM:
                default:
                    u8Input = E_CTL_INPUT_TSP_NONE;
                    break;
            }
            VPRINTF("[NDec][%d] MVD no preset tsp, input %d\n", u8Idx, u8Input);
        }
        ...
        MVD_DEBUGINFO(MVD_PRINT("%s CMD_PRESET_CONNET_INPUT_TSP arg0=%x\n", __FUNCTION__, u8Input));
        if (HAL_MVD_MVDCommand(CMD_PRESET_CONNET_INPUT_TSP, &mvdcmd) == FALSE)
        {
    ...
    {
        VPRINTF("[NDec][%d] MVD not TSP input, ignore PRESET_CONNECT_INPUT_TSP\n", u8Idx);
    }
...
#endif
MS_U32 HAL_MVD_IsSeqHeaderRdy(MS_U8 u8Idx)
{
#ifdef OFFSET_SEQHEADERFOUND
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_SEQHEADERFOUND, sizeof(MS_U32));
#else
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_SEQ_FOUND, sizeof(MS_U8));
#endif
}

E_MVD_Result HAL_MVD_SUPPORT_MULTI_ULTRA_RES(MS_U8 u8Idx, MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable;
    SET_DECNUM(mvdcmd, u8Idx);
#ifdef CMD_SUPPORT_MULTI_ULTRA_RES
    SET_CMD_RET_FALSE(CMD_SUPPORT_MULTI_ULTRA_RES, &mvdcmd);
#endif
    return E_MVD_RET_OK;
}
