...
//  Hardware Capability
//-------------------------------------------------------------------------------------------------
#define HVD_FW_VERSION 0x00001515
#define HVD_FW_IF_VERSION 0x00870215
...
#define HVD_DRAM_SIZE    0x1B000      // Default HVD DRAM heap size, (256-20)k
#define EVD_DRAM_SIZE    0x3B000      // Default EVD DRAM heap size, (256-20)k
#else
#define HVD_DRAM_SIZE    0x40000       // Default HVD DRAM heap size, 256k
#define EVD_DRAM_SIZE    0xD0000       // Default EVD DRAM heap size, 832k
#endif /* ENABLE_LOW_CPU_BUF */
...
#define HVD_DISP_FRM_INFO_EXT_ST_SIZE   (0x3200)
...
#define HVD_DBG_LV_ERROR    0x1 //bit 0
#define HVD_DBG_LV_FLOW     0x2 //bit 1
#define HVD_DBG_LV_DEBUG    0x4 //bit 2
#define HVD_DBG_LV_TRACE    0x8 //bit 3
...
// For Git Changes
#define GIT_TIMESTAMP 1507811912
...
typedef enum
{
    E_HVD_DS_MODE_NONE = 0,                                 ///< NONE flag
    E_HVD_DS_MODE_AR_CHANGE = BIT(0),               ///<AR change
    E_HVD_DS_MODE_DONOT_CHECK_AR_CHANGE = BIT(1),   /// Do not check AR when enable DS
}HVD_DS_MODE;
...
typedef enum
{
    E_HVD_PP_FD_NONE = 0,                  ///< NONE Flag
    E_HVD_PP_FD_TOP_FIELD_FIRST = BIT(0) ,            ///< Field Detector: top field first? (0:bottom first; 1: top first)
} HVD_Frm_Info_Mode_Flag;
...
typedef enum
{
    ...
    E_HVD_HDR_METATYPE_CONTENT_LIGHT_OFFSET       = 5,
    E_HVD_HDR_METATYPE_DYNAMIC_OFFSET             = 6,
} HVD_HDR_EXT_META_TYPE_OFFSET;

typedef enum
{
    E_HVD_HDR_TCH_MODE_INIT                       = 0,
    E_HVD_HDR_TCH_MODE_ENABLE                     = 1,
    E_HVD_HDR_TCH_MODE_DISABLE                    = 0xFF
} HVD_HDR_TCH_MODE_TYPE;
...
/****************************************************************************************
          ...
          +---------------------------------------------------------------+
***************************************************************************************/
#define HVD_HISTOGRAM_INDEX_NUM 32 // Frame histogram include 32 sets of data, each 16bit
typedef struct
{
    ...
    MS_U32 u32LumaAddr_subsample;
    MS_U32 u32ChromaAddr_subsample;
    MS_U16 u16Pitch_subsample;
    MS_U8  u8TileMode_subsample;
    /// SWDR histogram
    MS_U8  u8HistogramDataValid;
    MS_U16 u16Histogram[HVD_HISTOGRAM_INDEX_NUM];

    //// content light start ////
    MS_U16 u16maxContentLightLevel;
    MS_U16 u16maxPicAverageLightLevel;

    // Frame rate info (streamFrameRate = u32TimeScale/u32NumUnitsInTick , Ex : 60000, 59940, 60002)
...
typedef struct
{
    ...
    union
    {
        struct
        {
            MS_U16 constraint_set0_flag:1;
            MS_U16 constraint_set1_flag:1;
            MS_U16 constraint_set2_flag:1;
            MS_U16 constraint_set3_flag:1;
            MS_U16 constraint_set4_flag:1;
            MS_U16 constraint_set5_flag:1;
            MS_U16 constraint_set6_flag:1;
            MS_U16 constraint_set7_flag:1;
            MS_U16 constraint_set8_flag:1;
            MS_U16 constraint_set9_flag:1;
            MS_U16 constraint_reserved_zero_bits1:6;
        };
        MS_U16 constraint_set;
    };
} HVD_Extend_Display_Info;                  // 4 Bytes
...
typedef struct
{
    ...
    MS_U8  u8IsDynamicHDR;              //0x0E57
    MS_U32 u32CurMinTspDataSize;        //0x0E58 //byte
    HVD_Config_VP9HDR10 VP9HDR10Info;   //0x0E5C
    MS_U32 u32FirstPts_H;               //0x0E88
    MS_U32 u32FirstPts_L;               //0x0E8C
    HVD_Extend_Display_Info ExtendDispInfo;//0x0E90
    MS_U8  bFastFlush;                  //0x0E94
    MS_U8  u8SmoothScanMode;            //0x0E95 //HVD_SmoothScanMode
    MS_U8  IsRunMFcodec;                //0x0E96 //run mfcodec or not
    MS_U8  reserved8_8[1];              //0x0E97
    MS_U32 u32TaskTime;                 //0x0E98
    MS_U32 u32PlayCmdTime;              //0x0E9C
    MS_U32 u32MVDParserWriteTime;       //0x0EA0
    MS_U32 u32SEQHeaderTime;            //0x0EA4
    MS_U32 u32Decode_I_Time;            //0x0EA8
    MS_U32 u32Push_I_Time;              //0x0EAC
    MS_U32 u32ReceiveVsyncTime;         //0x0EB0
    MS_U32 u32DisplayOneFrameTime;      //0x0EB4
    MS_U32 u32IFrameNum;                //0x0EB8
    MS_U32 u32PFrameNum;                //0x0EBC
    MS_U32 u32BFrameNum;                //0x0EC0
    MS_U8  u8FrameBufNum;               //0x0EC4
    MS_U8  u8AvgSliceNum;               //0x0EC5
    MS_U16 u16AvgVsyncTime;             //0x0EC6
    MS_U16 u16AvgVsyncDuration;         //0x0EC8
    MS_U8  u8TopFieldFirst;             //0x0ECA
    MS_U8  u8TCHMode;                   //0x0ECB
    MS_U32 u32PlayIntvlStartPts;       //0x0ECC;
    MS_U32 u32PlayIntvlEndPts;         //0x0ED0;
    MS_U16 u32SeqChgCount;              //0x0ED4
    MS_U8 u8DSModeSetting;              //0x0ED6
    MS_U8  reserved8_10;                //0x0ED7
    MS_U32 u32HDRRegSetAddr;            //0x0ED8
    MS_U32 u32HDRRegSetSize;            //0x0EDC
    MS_U8  reserved8_11[0xF74-0x0EE0];  //0x0EE0

    MS_U32 u32VDEC_MIU_SEL;             //0x0F74
    MS_U32 u32MaxVideoWidth;            //0x0F78 // for VDEC3_FB usage
    MS_U32 u32MaxVideoHeight;           //0x0F7C // for VDEC3_FB usage
    CMD_QUEUE cmd_queue;                //0x0F80~0x0FBC DISPCMDQ and normal CMDQ ----- 0x0FA8~0x0FBC DISPCMDQ and normal CMDQ
...
typedef enum {
    MS_COLOR_FORMAT_HW_420_16x32 = 0,      //YUV420 16x32 tiled format
    MS_COLOR_FORMAT_YUYV,                  //YUV422 YUYV
    MS_COLOR_FORMAT_HW_420_10BIT_32x16,    //YUV420 32x16 tiled 10 bits mode
    MS_COLOR_FORMAT_HW_420_32x32,          //YUV420 32x32 tiled format
    MS_COLOR_FORMAT_HW_420_32x16,          //YUV420 32x16 tiled format
} VDEC_ColorFormat;
...
typedef struct
{
    ...
    MS_U8  u2ForcePGear:2;              // 0: 25i or 30i, 1: 25p or 30p, 2: 50p or 60p
    MS_U8  u2Reserved:4;
    MS_U8  u8FieldType;                 ///< HVD_Field_Type, none, top , bottom, both field
    MS_U8  u8Interlace;
    MS_U8  u8ColorFormat;               // 0 -> 420, 1 -> 422, 2 -> 420 10 bit, 3 -> 420(32x32)
...
typedef enum
{
    E_VDEC_FRM_INFO_PROGRESS_MODE,
    E_VDEC_FRM_INFO_FORCE_I_MODE,
    E_VDEC_FRM_INFO_I_FRAME_MODE,
    E_VDEC_FRM_INFO_I_FIELD_MODE
} E_VDEC_FRM_INFO_I_P_MODE;

typedef enum
{
    E_VDEC_FRM_INFO_ID_L_DECODE_TIME     = 0,
    E_VDEC_FRM_INFO_ID_L_TOGGLE_TIME     = 17,
    E_VDEC_FRM_INFO_ID_L_INTERLACE_MODE  = 19,
} VDEC_FRM_INFO_ID_L_SHIFT_BITS;
...
typedef enum
{
    E_HVD_FORCE_INTERLACE_DISABLE = 0,                      // Arg: 0; Diable.
    E_HVD_FORCE_INTERLACE_DTV_TS_UNDER_FHD_25_30,           // Arg: 1; Support DTV and TS file mode with framerate 25 or 30 (under FHD).
    E_HVD_FORCE_INTERLACE_DTV_TS_LARGER_FHD_25_30,          // Arg: 2; Support DTV and TS file mode with framerate 25 or 30,but only works on 1080P (width large less 1920)
    E_HVD_FORCE_INTERLACE_DTV_TS_23_to_30,                  // Arg: 3; Support DTV and TS file mode with framerate 23 to 30 and all resolution.
    E_HVD_FORCE_INTERLACE_DTV_TS_UNDER_30,                  // Arg: 4; Support DTV and TS file mode with under 30 fps and all resolution.
    E_HVD_FORCE_INTERLACE_DTV_TS_UNDER_FHD_LARGER_HD_25_30, // Arg: 5; Support DTV and TS file mode with with framerate 25 or 30 (under FHD and width large less 720)
    E_HVD_FORCE_INTERLACE_ALL,                              // Arg: 6: Support all mode with under all fps and all resolution.
    E_HVD_FORCE_INTERLACE_ALL_NONE_4K2K,                    // Arg: 7: Support all mode but not 4k resolution.
} HVD_FORCE_INTERLACE_MODE;

/// VDEC smooth scan
typedef enum
{
    /// smooth scan is disabled
    E_HVD_SMOOTH_SCAN_NONE = 0,
    /// smooth scan by hw
    E_HVD_SMOOTH_SCAN_HW,
    /// smooth scan by vdec plus
    E_HVD_SMOOTH_SCAN_VDEC_PLUS,
    /// smooth scan is unsupport
    E_HVD_SMOOTH_SCAN_UNSUPPORT,
} HVD_SmoothScanMode;
...
typedef enum
{
    ...
    /*0x20113*/E_HVD_CMD_FORCE_INTERLACE,           // HVD_FORCE_INTERLACE_MODE
                                                    // Arg: 0; Diable. Arg: 1; Force interlace only support DTV and TS file mode with framerate 25 or 30 (all resolution, under FHD)
    ...
    /*0x2022B*/E_HVD_CMD_SET_HDR_XC_SHM_ADDR,        // Arg, address for dolby vision xc DM/composer shm
    /*0x2022C*/E_HVD_CMD_SET_SUB_SAMPLE_MODE,        // Arg: 0: Disable; 1:Enable; 2.AUTO(Enable that resolution larger than FHD)
    /*0x2022D*/E_HVD_CMD_SUPPORT_MULTI_ULTRA_RES,    // Arg: 0: Disable; 1:Enable;
    /*0x2022E*/E_HVD_CMD_SET_DRV_MCU_MODE,           // Arg: 0: Disable; 1:Enable;
    ...
    /*0x20332*/E_HVD_CMD_DISPLAY_SUB_SAMPLE,        // Arg, 0:disable, 1:enable. to display sub sample video on hardwire mode
    /*0x20333*/E_HVD_CMD_PUSI_CONTROL,              // Arg, 0:disable, 1:enable. MVD parser data check
    /*0x20334*/E_HVD_CMD_ADD_REF_DISPQ,             // Add reference count in Display Queue
    /*0x20335*/E_HVD_CMD_ENABLE_PLAYBACK_INTERVAL,  //
    /*0x20336*/E_HVD_CMD_APPLY_HISTOGRAM_ROI,       // Set histogram ROI, each unit as 32 pixel, Arg, 0:disable, 32bit, MSB to LSB: EndY(8bit) EndX(8bit), StartY(8bit), StartX(8bit)
    /*0x20337*/E_HVD_CMD_TOPFIELD_FIRST_CONTROL,   // Set top/bottom field first cmd , Arg, 0:bottom field first, 1:top field first
    /*0x20338*/E_HVD_CMD_SET_DS_MODE,               // Aef, 0:disable, bit 0 = HVD_DS_AR_CHANGE_MODE
    /*0x20339*/E_HVD_CMD_SET_DEBUG_LEVEL,           // 0:disable, bit 0 = HVD_DBG_LV_ERROR, bit 1 = HVD_DBG_LV_FLOW, bit 2 = HVD_DBG_LV_DEBUG, bit 3 = HVD_DBG_LV_TRACE
...
#define E_HVD_CMD_SET_DV_XC_SHM_ADDR E_HVD_CMD_SET_HDR_XC_SHM_ADDR
...
typedef enum
{
    ...
    E_HVD_ERR_HDR_WRONG_DISP_PATH,  // not support this display path for HDR
    ...
    E_HVD_ERR_AVC_ERROR_CONCEAL,
    E_HVD_ERR_AVC_ERROR_NAL,
    ...
    E_HVD_ERR_HEVC_SPS_BROKEN,           // SPS is not valid
    E_HVD_ERR_HEVC_SPS_NOT_IN_SPEC,
    E_HVD_ERR_HEVC_SPS_NOT_ENOUGH_FRM,   // DPB size at specified level is smaller than the specified number of reference frames. This is not allowed
    E_HVD_ERR_HEVC_PPS_BROKEN,           // PPS is not valid
    E_HVD_ERR_HEVC_VPS_BROKEN,           // VPS is not valid
    E_HVD_ERR_HEVC_REF_LIST,
    E_HVD_ERR_HEVC_NO_REF,
    E_HVD_ERR_HEVC_RES,                  // out of supported resolution
    E_HVD_ERR_HEVC_ERROR_CONCEAL,
    E_HVD_ERR_HEVC_ERROR_NAL,
...
typedef enum
{
    ...
    E_HVD_POST_PROC_FIELD_DETECT   = BIT(1),
    E_HVD_POST_PROC_FIELD_DETECT_ONGOING        = BIT(2),
    E_HVD_POST_PROC_FIELD_DETECT_NEED_REVERSE   = BIT(3),
} HVD_Post_Process;
...
typedef enum
{
    ...
    // Not support depth chroma & luma
    E_HVD_NOT_SUPPORT_DEPTH_CHROMA = BIT(4),
