...
#define MVD_ISR_VECTOR2 E_INT_IRQ_MVD2_INT

extern MS_U32 HAL_MVD_GetEngId(MS_U8 u8Idx);

#if (defined(MVD_ENG_NUM) )
inline MS_BOOL OSAL_MVD_IntDisable(MS_U8 u8Idx)
{
    return MsOS_DisableInterrupt( ((HAL_MVD_GetEngId(u8Idx) == 0) ? MVD_ISR_VECTOR : MVD_ISR_VECTOR2));
}
#else
...
#if ( defined(MVD_ENG_NUM) )
inline MS_BOOL OSAL_MVD_IntEnable(MS_U8 u8Idx)
{
    MS_BOOL ret = FALSE;
    ret = MsOS_EnableInterrupt(((HAL_MVD_GetEngId(u8Idx) == 0) ? MVD_ISR_VECTOR : MVD_ISR_VECTOR2));
#ifdef MSOS_TYPE_LINUX_KERNEL // TODO: check if this function can be support in kernel mode in later version
...
#if ( defined(MVD_ENG_NUM) )
inline MS_BOOL OSAL_MVD_IsrAttach(MS_U8 u8Idx, void* pfnIsr)
{
    return MsOS_AttachInterrupt(((HAL_MVD_GetEngId(u8Idx) == 0) ? MVD_ISR_VECTOR : MVD_ISR_VECTOR2), (InterruptCb)pfnIsr);
}
#else
...
#if ( defined(MVD_ENG_NUM) )
inline MS_BOOL OSAL_MVD_IsrDetach(MS_U8 u8Idx)
{
    return MsOS_DetachInterrupt(((HAL_MVD_GetEngId(u8Idx) == 0) ? MVD_ISR_VECTOR : MVD_ISR_VECTOR2));
}
#else
