...
typedef struct
{
    ...
    MS_BOOL bTrickPlay2xAVSync[MVD_MAX_STREAMS];
    MVD_TrickDec _eTrickDec[MVD_MAX_STREAMS];
    MVD_TrickDec enSmoothScan[MVD_MAX_STREAMS];
} MVD_Drv_CTX;
...
MS_U8 _u8DbgLevel = 0;
MS_BOOL bMVDIsrAttached = FALSE;
MS_BOOL _bDisableFlag = TRUE;
...
static MS_BOOL MVD_SaveStreamId(MS_U8 u8DrvIdx, MS_U32 u32MvdSid, MS_U32 u32VpuSid)
{
    ...
    {
        MVD_ERR("%s err IsUsed: drv=%x mvd=%tx vpu=%tx\n", __FUNCTION__,
                u8DrvIdx, (ptrdiff_t)u32MvdSid, (ptrdiff_t)u32VpuSid);
        return FALSE;
    }
...
MS_U32 MVD_GetVpuStreamId(MS_U32 u32Id)
{
    MS_U8 u8DrvIdx = _MVD_GET_IDX(u32Id);
...
{
    return MVD_GetVpuStreamId(u32Id);
}
...
static void MVD_InitVar(MS_U8 u8DrvIdx, MS_U8 u8HalIdx)
{
    ...
    pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] = FALSE;
    pMVDDrvContext->_eTrickDec[u8DrvIdx] = FALSE;
    pMVDDrvContext->enSmoothScan[u8DrvIdx] = FALSE;
...
E_MVD_Result MDrv_MVD_SetCfg(MS_U32 u32Id, MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u32Id=0x%tx, u8HalIdx=0x%x\n", __FUNCTION__, (ptrdiff_t)u32Id, u8HalIdx));
...
#ifdef STELLAR
    MVD_DEBUGINFO(MVD_PRINT("FWBuf u8MiuSel:%d u32StartOffset:0x%x\n", u8MiuSel, u32StartOffset));
#endif
    if(u8MiuSel >= 1)
    {
        memCfg->bFWMiuSel = 1;
...
#ifdef STELLAR
    MVD_DEBUGINFO(MVD_PRINT("BSBuf u8MiuSel:%d u32StartOffset:0x%x\n", u8MiuSel, u32StartOffset));
#endif
    ...
    _phy_to_miu_offset(memCfg->u8FBMiuSel, u32StartOffset, memCfg->u32FBAddr);
#ifdef STELLAR
    MVD_DEBUGINFO(MVD_PRINT("FBBuf u8MiuSel:%d u32StartOffset:0x%x\n", memCfg->u8FBMiuSel, u32StartOffset));
#endif
    ...
    MVD_DEBUGINFO(MVD_PRINT("MIUSEL:: FW=%x HW=%x, miu1base=0x%lx,miu2base=0x%lx,miu3base=0x%lx\n",
                            memCfg->u8FWMiuSel, memCfg->u8HWMiuSel, (unsigned long)memCfg->u32Miu1BaseAddr, (unsigned long)memCfg->u32Miu2BaseAddr, (unsigned long)memCfg->u32Miu3BaseAddr));
...
MS_BOOL MDrv_MVD_Init(MS_U32 u32Id, MVD_CodecType eCodecType, MS_BOOL bShareBBU)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_Init:start u32Id=0x%tx, sid=0x%tx, halIdx=0x%x, bShareBBU=0x%x\n",
                            (ptrdiff_t)u32Id, (ptrdiff_t)_MVD_GET_HAL_SID(u32Id), u8HalIdx, bShareBBU));
    ...
    if (!HAL_MVD_Init(u8HalIdx, eCodecType, u32VpuSid, bShareBBU))
    {
...
MS_BOOL MDrv_MVD_Exit(MS_U32 u32Id)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u8DrvIdx=0x%x u8HalIdx=0x%x u32Id=0x%tx\n", __FUNCTION__, u8DrvIdx, u8HalIdx, (ptrdiff_t)u32Id));
...
_HW_TILE_DOWN:
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s i=0x%x u32Id=0x%tx\n", __FUNCTION__, u8DrvIdx, (ptrdiff_t)u32Id));
...
E_MVD_Result MDrv_MVD_Rst(MS_U32 u32Id, MVD_CodecType eCodecType, MS_BOOL bShareBBU)
{
    ...
    if (FALSE == MDrv_MVD_Init(u32Id, eCodecType, bShareBBU))
    {
    ...
    if(MDrv_MVD_SetCodecInfo(u32Id, eCodecType, eSrcMode, bMvdParserDisable[eSrcMode]) == FALSE)
    {
        return E_MVD_RET_FAIL;
    }
...
{
    return _MVD_Init(u32Id, u8CodecType, u8BSProviderMode, bDisablePESParsing);
}
...
//------------------------------------------------------------------------------
/// Get First PTS in previous input video data
/// @return PTS
//------------------------------------------------------------------------------
MS_S64 MDrv_MVD_GetVideoFirstPts(MS_U32 u32Id)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    MVD_FUNC_ENTRY();
    return HAL_MVD_GetVideoFirstPts(u8HalIdx);
}
//------------------------------------------------------------------------------
...
void MDrv_MVD_GetErrInfo(MS_U32 u32Id, MVD_ErrCode *errCode, MVD_ErrStatus *errStatus, MS_U32* u32errstatus)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    MVD_FUNC_ENTRY();
    HAL_MVD_GetErrInfo(u8HalIdx, errCode, errStatus, u32errstatus);
    return;
}
...
void MDrv_MVD_RstIFrameDec(MS_U32 u32Id, MVD_CodecType eCodecType, MS_BOOL bShareBBU)
{
    ...
    MDrv_MVD_Rst(u32Id, eCodecType, bShareBBU);
    return;
}
...
MS_U32 MDrv_MVD_GetSTC(MS_U32 u32Id)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    MVD_FUNC_ENTRY();
    return HAL_MVD_GetSTC(u8HalIdx);
}
...
MS_U8 MDrv_MVD_GetDispRdy(MS_U32 u32Id)
{
    ...
    if(pMVDDrvContext->bCMAUsed == TRUE && pMVDDrvContext->bCMAAllocDone == FALSE)
    {
        ...
        HAL_MVD_GetCMAInformation(u8HalIdx, &u64Addr, &u64Size, &bAllocateDone);

        if(u64Addr == 0 || u64Size == 0)
        {
            return FALSE;
        }
        else
        {
            ...
            alloc_param.offset_in_pool = u64Addr - pMVDDrvContext->cmaInitParam.heap_miu_start_offset;
            alloc_param.length = u64Size;
            ...
            VPRINTF("[VDEC][%d]MApi_CMA_Pool_GetMem in mvd: alloc_param.pool_handle_id=%x, alloc_param.flags=%x, alloc_param.offset_in_pool=%llx, alloc_param.length=%x\n",
                    (unsigned int)u8DrvIdx,
...
MS_U8 MDrv_MVD_GetSeqRdy(MS_U32 u32Id)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsSeqHeaderRdy(u8HalIdx);
}
...
MS_BOOL MDrv_MVD_IsStepDecodeDone(MS_U32 u32Id)
{
    ...
    MVD_FUNC_ENTRY();
    pCtrlCfg = HAL_MVD_GetCtrlCfg(u8HalIdx);
...
//------------------------------------------------------------------------------
/// Set the Playback Interval (u32StartPts ms, u32EndPts ms)
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_Set_Playback_Interval(MS_U32 u32Id, MS_U32 u32StartPts, MS_U32 u32EndPts)
{
    ...
    HAL_MVD_Set_Playback_Interval(u8HalIdx, u32StartPts, u32EndPts);
    return TRUE;
}
...
MS_BOOL MDrv_MVD_TrickPlay(MS_U32 u32Id, MVD_TrickDec trickDec, MS_U8 u8DispDuration)
{
    ...
    pMVDDrvContext->_eTrickDec[u8DrvIdx]  = trickDec;
...
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_EnSmoothScan(MS_U32 u32Id, MS_BOOL bEnSmoothScan)
{
    MS_U8 u8DrvIdx;
    MVD_FUNC_ENTRY();
    u8DrvIdx = _MVD_GET_IDX(u32Id);

    pMVDDrvContext->enSmoothScan[u8DrvIdx]  = bEnSmoothScan;

#ifdef HAL_MVD_EnSmoothScan
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    HAL_MVD_EnSmoothScan(u8HalIdx, bEnSmoothScan )
#endif
...
//------------------------------------------------------------------------------
/// Get max pixels of MPEG Video Decoder.
/// @return -max pixel supportted .
//------------------------------------------------------------------------------
MS_U32 MDrv_MVD_GetMaxPixel(MS_U32 u32Id)
{
    return (MS_U32)HAL_MVD_GetMaxPixel(u32Id);
}
...
MS_VIRT MDrv_MVD_GetESReadPtr(MS_U32 u32Id)
{
    ...
    if(pMVDDrvContext->u32FrameBaseMode[u8DrvIdx] == TRUE && ! (E_MVD_TS_FILE_MODE == curSrcMode || E_MVD_TS_MODE == curSrcMode))
    {
        MS_VIRT u32VBBUAddr = HAL_MVD_GetVBBUTableAddr(u8VPUIdx);
...
E_MVD_Result MDrv_MVD_SetSpeed(MS_U32 u32Id, MVD_SpeedType eSpeedType, MS_U8 u8Multiple)
{
    ...
    if (pstCtrlCfg->ePreSpeedType == E_MVD_SPEED_DEFAULT)
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetSpeed save avsync: bEnable=0x%x u32Delay=0x%tx u16Tolerance=0x%x\n",
                                pstNFPSyncCfg->bEnable, (ptrdiff_t)pstNFPSyncCfg->u32Delay, pstNFPSyncCfg->u16Tolerance));
    }

    if((eSpeedType == E_MVD_SPEED_FAST && (u8Multiple == 2 || pMVDDrvContext->enSmoothScan[u8DrvIdx] == TRUE) && pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] == TRUE) == FALSE && E_MVD_SPEED_DEFAULT != eSpeedType)
    {
        //disable avsyn when fast/slow forward
        MDrv_MVD_SetAVSync(u32Id, FALSE, 0);
    }
    if(eSpeedType == E_MVD_SPEED_FAST && pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] == TRUE && pMVDDrvContext->enSmoothScan[u8DrvIdx] == TRUE)
    {
        if(u8Multiple >= 16)
        {
            HAL_MVD_TrickPlay(u8HalIdx, E_MVD_TRICK_DEC_I, 0);
        }
        else if(u8Multiple >= 4)
        {
            HAL_MVD_TrickPlay(u8HalIdx, E_MVD_TRICK_DEC_IP, 0);
        }
        else
        {
            HAL_MVD_TrickPlay(u8HalIdx, E_MVD_TRICK_DEC_ALL, 0);
        }
    }
    else
    {
        HAL_MVD_TrickPlay(u8HalIdx, pMVDDrvContext->_eTrickDec[u8DrvIdx], 0);
    }

    if (HAL_MVD_SetSpeed(u8HalIdx, eSpeedType, u8Multiple) == TRUE)
    {
        if ((E_MVD_SPEED_DEFAULT == eSpeedType) || (eSpeedType == E_MVD_SPEED_FAST && (u8Multiple == 2 || pMVDDrvContext->enSmoothScan[u8DrvIdx] == TRUE) && pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] == TRUE))
        {
            if (pstCtrlCfg->ePreSpeedType != E_MVD_SPEED_DEFAULT)
            {
                //set AVSync again for firmware doesn't remember AVSync settings after FF
                MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetSpeed reset avsync: bEnable=0x%x u32Delay=0x%tx u16Tolerance=0x%x\n",
                                        pstNFPSyncCfg->bEnable, (ptrdiff_t)pstNFPSyncCfg->u32Delay, pstNFPSyncCfg->u16Tolerance));
    ...
    for (u8DrvIdx = 0; u8DrvIdx < MVD_MAX_STREAMS; u8DrvIdx++)
    {
        ...
        if (u32IntStat != 0)
        {
            ...
            *pu32CurEvent = E_MVD_EVENT_DISABLE_ALL;
            ...
            {
                MVD_DEBUGINFO(MVD_PRINT("%s Callback for *pu32CurEvent=0x%tx, cbData=0x%tx\n",
                                        __FUNCTION__, (ptrdiff_t)*pu32CurEvent, (ptrdiff_t)u32CbData));
#ifdef VDEC3
                pfnCb(u32Id);
#else
                pfnCb(u32CbData);
...
#if ( defined(MVD_ENG_NUM) )
    HAL_MVD_ClearIRQ(u8HalIdx); //clear interrupt
#else
    HAL_MVD_ClearIRQ(); //clear interrupt
#endif

#if ( defined(MVD_ENG_NUM) )
    if(TRUE == OSAL_MVD_IntEnable(u8HalIdx))//enable cpu interrupt mask
#else
...
E_MVD_Result MDrv_MVD_SetIsrEvent(MS_U32 u32Id, MS_U32 eEvent, MVD_InterruptCb fnHandler)
{
    ...
    {
        // mark this code to avoid the double disable in kernel and driver
#if 0
        MS_U32 i = 0;
        MS_BOOL bUsed = FALSE;
        ...
        if((_bDisableFlag != TRUE) && (FALSE == bUsed))
        {
            _bDisableFlag = OSAL_MVD_IntDisable();
            if(_bDisableFlag == FALSE)
            {
                MVD_DEBUGERROR(VPRINTF("%s Disable Interrupt Fail\n", __FUNCTION__));
            }
#if !defined(SUPPORT_X_MODEL_FEATURE) && !defined(DISABLE_ISR_DETACH)
            if(TRUE == bMVDIsrAttached)
            {
                OSAL_MVD_IsrDetach();
                bMVDIsrAttached = FALSE;
            }
#endif
        }
#endif
    ...
    else
    {
        if(fnHandler != NULL)
        {
            ...
            MVD_DEBUGINFO(MVD_PRINT("%s Hook callback u32Id=0x%tx u8HalIdx=0x%x eEvent=0x%tx\n",
                                    __FUNCTION__, (ptrdiff_t)u32Id, u8DrvIdx, (ptrdiff_t)eEvent));
            pMVDDrvContext->_eEventFlag[u8DrvIdx] = eEvent;

#if !defined(SUPPORT_X_MODEL_FEATURE) && !defined(DISABLE_ISR_DETACH)
            if (FALSE == bMVDIsrAttached)
            {
#if ( defined(MVD_ENG_NUM) )
                if (OSAL_MVD_IsrAttach(u8HalIdx, (void*)MVD_IsrProc) != TRUE)
#else
                ...
                bMVDIsrAttached = TRUE;
#if ( defined(MVD_ENG_NUM) )
                if (OSAL_MVD_IntEnable(u8HalIdx) != TRUE)
#else
            ...
            _bDisableFlag = FALSE;
#endif
            MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetIsrEvent eEventFlag=0x%tx\n", (ptrdiff_t)pMVDDrvContext->_eEventFlag[u8DrvIdx]));
        ...
        {
            MVD_DEBUGERROR(MVD_ERR( "SetISREvent with NULL pointer. ISR type:%tu\n", (ptrdiff_t)eEvent));
            return E_MVD_RET_INVALID_PARAM;
        }
    }
    return E_MVD_RET_OK;
...
void MDrv_MVD_TrickPlay2xAVSync(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8DrvIdx = _MVD_GET_IDX(u32Id);
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    pMVDDrvContext->bTrickPlay2xAVSync[u8DrvIdx] = bEnable;
    return HAL_MVD_TrickPlay2xAVSync(u8HalIdx, bEnable);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_FRC_OnlyShowTopField(u8HalIdx, bEnable);
}

//------------------------------------------------------------------------------
/// Set MPEG4 Cal Frame Rate
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------

MS_BOOL MDrv_MVD_Mpeg4CalFramerate(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_Mpeg4CalFramerate(u8HalIdx, bEnable);
}
...
//------------------------------------------------------------------------------
/// Get Color info of the decoded/displaying frame.
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_GetColorInfo(MS_U32 u32Id, MVD_Color_Info* pstColorInfo)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    if (NULL == pstColorInfo)
    {
        MVD_DEBUGERROR(MVD_ERR("GetColorInfo NULL pInfo!\n"));
        return FALSE;
    }
    return HAL_MVD_GetColorInfo(u8HalIdx, pstColorInfo);
}
...
MS_U32 MDrv_MVD_GetDropCount(MS_U32 u32Id)
{
    ...
    MVD_FUNC_ENTRY();
    u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GetDropCnt(u8HalIdx);
}
...
//------------------------------------------------------------------------------
/// Force Enable/Disable Es Full stop
/// @param bDisable-  0: force enable es full stop mode in ts/dtv 1:  force disable es full stop mode in ts/dtv
/// @return -E_MVD_RET_OK for success; E_MVD_RET_FAIL for failure.
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_DisableEsFullStop(MS_U32 u32Id, MS_BOOL bDisable)
{
    ...
    MVD_FUNC_ENTRY();
    u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_DisableESFullStop(u8HalIdx, bDisable);
}
...
MS_BOOL MDrv_MVD_PUSI_Control(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    MVD_FUNC_ENTRY();
    u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_PUSI_Control(u8HalIdx, bEnable);
}
...
void MDrv_MVD_DbgDump(MS_U32 u32Id)
{
    ...
    MVD_PRINT("avsync on=%x, delay=%td, tolerance=%d, done=%x, skip=%x, repeat=%x, pts=%tdms\n",
              pAVSyncCfg->bEnable, (ptrdiff_t)pAVSyncCfg->u32Delay, pAVSyncCfg->u16Tolerance, (MDrv_MVD_GetSyncStatus(u32Id) == 1),
              MDrv_MVD_GetIsSyncSkip(u32Id), MDrv_MVD_GetIsSyncRep(u32Id), (ptrdiff_t)MDrv_MVD_GetPTS(u32Id));
    MVD_PRINT("seq(%d): w=%d, h=%d, i/p=%x, fps=%td\n", MDrv_MVD_GetDispRdy(u32Id),
              pPreInfo->u16HorSize, pPreInfo->u16VerSize,
              pPreInfo->u8Interlace, (ptrdiff_t)pPreInfo->u32FrameRate);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetNDSMode(u8HalIdx, bEnable, bSendCmd);
}

//------------------------------------------------------------------------------
/// Add reference for the specified frame
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_AddRef(MS_U32 u32Id, MS_U8 u8FrmIdx)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    if (TRUE != HAL_MVD_IsMcuMode(u8HalIdx))
    {
        MVD_DEBUGINFO(MVD_PRINT("Need to MCU before using this function\n"));
        return FALSE;
    }

    return HAL_MVD_FrameOpt(u8HalIdx, u8FrmIdx, E_MVD_FRAME_ADD_REF);
}
...
E_MVD_Result MDrv_MVD_GetFreeStream(MS_U32 *pu32Id, MVD_DRV_StreamType eStreamType)
{
    ...
    else if ((eStreamType >= E_MVD_DRV_N_STREAM) && (eStreamType < (E_MVD_DRV_N_STREAM + MVD_MAX_STREAMS)))
    {
        ...
        i = eStreamType - E_MVD_DRV_N_STREAM;
    }
#endif
    ...
    if(u32MVDStreamId == E_HAL_MVD_STREAM_NONE)
    {
        HAL_VPU_EX_ReleaseFreeStream(i);
    }

    MVD_DEBUGVERBAL(MVD_PRINT("%s:: i=0x%x, mvd=0x%tx, vpu=0x%tx\n", __FUNCTION__,
                              i, (ptrdiff_t)u32MVDStreamId, (ptrdiff_t)u32VPUStreamId));

    if (u32VPUStreamId && u32MVDStreamId)
    {
        MVD_DEBUGINFO(MVD_PRINT("%s   i=0x%x, mvd=0x%tx, vpu=0x%tx\n", __FUNCTION__,
                                i, (ptrdiff_t)u32MVDStreamId, (ptrdiff_t)u32VPUStreamId));
#ifdef VDEC3
    ...
    {
#ifdef VDEC3
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) NG  i=0x%tx, mvd=0x%tx, vpu=0x%tx\n", __FUNCTION__, __LINE__,
                                (ptrdiff_t)(u32MVDStreamId & 0xf), (ptrdiff_t)u32MVDStreamId, (ptrdiff_t)u32VPUStreamId));
#else
...
#define SIZE_3K 0xC00
    ...
    {
        VPRINTF("this chip does not support set external DS buffer since VPU_FORCE_MIU_MODE == 1 !!\n");
        return E_MVD_RET_FAIL;
    }
#endif

#if (defined(CHIP_EDISON) || defined(CHIP_EINSTEIN) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MAXIM) || defined(CHIP_M5621) || defined(CHIP_M7621) || defined(CHIP_K7U))
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8HalIdx);
    MS_BOOL bRet = FALSE;
    ...
    bRet = HAL_MVD_SetExternalDSBuff(u8HalIdx, u32VPUAddr, pExternalBuf->u32DSBufAddr, pExternalBuf->u32DSBufSize);
...
MS_BOOL MDrv_MVD_GetPVRSeamlessInfo(MS_U8 u8Idx, void* param)
{
    ...
    return HAL_MVD_GetPVRSeamlessInfo(u8Idx, param);
}
...
{
    return HAL_VPU_EX_REE_SetSHMBaseAddr(U32Type, u32SHMAddr, u32SHMSize, u32MIU1Addr);
}
#endif
//------------------------------------------------------------------------------
...
#if defined(VDEC_FW30)
    *u32VsyncSHMOffset = HAL_MVD_GetVsyncAddrOffset();
#elif defined(VDEC_FW31)
    *u32VsyncSHMOffset = HAL_VPU_EX_GetVsyncAddrOffset(u32Id);
#else
#error "controller.h should provide definition of VDEC_FW30 or VDEC_FW31"
#endif
}
...
#elif defined(VDEC_FW31)
    *u32VsyncExtShmOffset = HAL_VPU_EX_GetVsyncExtAddrOffset(u32Id);
#else
#error "controller.h should provide definition of VDEC_FW30 or VDEC_FW31"
#endif
...
E_MVD_Result MDrv_MVD_SetDVXCShmAddr(MS_U32 u32Id, MS_PHY u32FWBaseAddr, MS_PHY u32DVXCShmAddr)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetDVXCShmAddr(u8HalIdx, u32FWBaseAddr, u32DVXCShmAddr);
}
...
void MDrv_MVD_Dynamic_FB_Mode(MS_BOOL bEnable, MS_PHY u32Address, MS_U32 u32Size)
{
    HAL_VPU_EX_DynamicFBMode(bEnable, u32Address, u32Size);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_VPU_EX_ReleaseFreeStream(u8HalIdx);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_PreConnectInputTsp(u8HalIdx, bEnable, eInputTsp, eStream);
}

//------------------------------------------------------------------------------
/// Disable PB Frame Mode
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
E_MVD_Result MDrv_MVD_DisablePBFrameMode(MS_U32 u32Id, MS_BOOL bEn)
{
    ...
    MVD_FUNC_ENTRY();
    u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_DisablePBFrameMode(u8HalIdx, bEn);
}

MS_U8 MDrv_MVD_CheckFreeStream(MVD_Original_Stream eStream)
{
    return HAL_VPU_EX_CheckFreeStream(eStream);
}

E_MVD_Result MDrv_MVD_Set_SlowSyncParam(MS_U32 u32Id, MS_U8 u8RepeatPeriod, MS_U8 u8DropPeriod)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_Set_SlowSyncParam(u8HalIdx, u8RepeatPeriod, u8DropPeriod);
}
MS_BOOL MDrv_MVD_VariableFrameRate(MS_U32 u32Id)
{
    ...
    MVD_FUNC_ENTRY();
    u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_VariableFrameRate(u8HalIdx);
}

//------------------------------------------------------------------------------
/// Disable PB Frame Mode
/// @return Min pre buffer size
//------------------------------------------------------------------------------
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    HAL_MVD_GetPerf(u8HalIdx, pPerf);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GetTFF(u8HalIdx);
}

MS_BOOL MDrv_MVD_GotFileEndPattern(MS_U32 u32Id)
{
    MS_U8 u8Idx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GotFileEndPattern(u8Idx);
}
MS_BOOL MDrv_MVD_SetTFF(MS_U32 u32Id, MS_BOOL bTFF)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetTFF(u8HalIdx, bTFF);
}

MS_BOOL MDrv_MVD_ParserControl(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_Parser_Control(u8HalIdx, bEnable);
}
MS_BOOL MDrv_MVD_ErrConcealControl(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_ErrConceal_Control(u8HalIdx, bEnable);
}

//------------------------------------------------------------------------------
/// Flush queue: enable support 4k2k + full HD multi deocode
/// @return -return E_MVD_Result success
//------------------------------------------------------------------------------
E_MVD_Result MDrv_MVD_SUPPORT_MULTI_ULTRA_RES(MS_U32 u32Id, MS_BOOL bSupport)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    MVD_FUNC_ENTRY();
    return HAL_MVD_SUPPORT_MULTI_ULTRA_RES(u8HalIdx, bSupport);
}
