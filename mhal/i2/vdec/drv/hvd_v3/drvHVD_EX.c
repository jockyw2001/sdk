...
#include "../../hal/hvd_v3/fwHVD_if.h"
#include "../../hal/vpu_v3/halVPU_EX.h"
#include "../../hal/hvd_v3/halHVD_EX.h"
#include "drvSYS.h"
#include "../../hal/vpu_v3/controller.h"
...
#ifdef CHIP_I2_FPGA
#define HVD_FW_CMD_TIMEOUT_DEFAULT      1000     // ms
#define HVD_FW_EXIT_ACTION_TIMEOUT      35000    // ms
#define HVD_CMD_TIMEOUT                 5000   // ms
#else
#define HVD_FW_CMD_TIMEOUT_DEFAULT      100     // ms
#define HVD_FW_EXIT_ACTION_TIMEOUT      3500    // ms
#define HVD_CMD_TIMEOUT                 500    // ms
#endif

#ifdef VDEC3
#define HVD_MAX_STREAMS         32
#else
#define HVD_MAX_STREAMS         2
#endif
#define HVD_MIN_PACKET_SIZE     16
...
static MS_BOOL      _HVD_EX_IsAllBufferEmpty(MS_U32 u32Id, MS_BOOL bFlush);
#ifndef VDEC3
...
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
static void _EVDLITE_EX_ISRHandler(void);
#endif
#endif
...
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    MS_BOOL bEVDLITEIsrAttached;    //EVD LITE ISR is attach or not
    MS_BOOL bEVDLITEMSOSIsrEnable;  //EVD LITE msos isr enable or not
#endif
...
MS_BOOL bHVDIsIsrAttached = FALSE;  //check HVD isr already attached, avoid HVD+HVD re-attached
MS_BOOL bEVDIsIsrAttached = FALSE;  //check EVD isr already attached, avoid HVD+HVD re-attached
MS_BOOL bHVDDisableISRFlag = TRUE;
MS_BOOL bEVDDisableISRFlag = TRUE;
...
static HVD_EX_Result _HVD_MapRetEx(HVD_Return eHvd_Ret)
{
    ...
    switch(eHvd_Ret)
    {
        case E_HVD_RETURN_FAIL              :
            eRes = E_HVD_EX_FAIL;
            break;
        case E_HVD_RETURN_SUCCESS           :
            eRes = E_HVD_EX_OK;
            break;
        case E_HVD_RETURN_INVALID_PARAMETER :
            eRes = E_HVD_EX_RET_INVALID_PARAMETER;
            break;
        case E_HVD_RETURN_ILLEGAL_ACCESS    :
            eRes = E_HVD_EX_RET_ILLEGAL_ACCESS;
            break;
        case E_HVD_RETURN_HARDWARE_BREAKDOWN:
            eRes = E_HVD_EX_RET_HARDWARE_BREAKDOWN;
            break;
        case E_HVD_RETURN_OUTOF_MEMORY      :
            eRes = E_HVD_EX_RET_OUTOF_MEMORY;
            break;
        case E_HVD_RETURN_UNSUPPORTED       :
            eRes = E_HVD_EX_RET_UNSUPPORTED;
            break;
        case E_HVD_RETURN_TIMEOUT           :
            eRes = E_HVD_EX_RET_TIMEOUT;
            break;
        case E_HVD_RETURN_NOTREADY          :
            eRes = E_HVD_EX_RET_NOTREADY;
            break;
        case E_HVD_RETURN_MEMORY_OVERWIRTE  :
            eRes = E_HVD_EX_RET_MEMORY_OVERWIRTE;
            break;
        case E_HVD_RETURN_ES_FULL           :
            eRes = E_HVD_EX_RET_QUEUE_FULL;
            break;
        case E_HVD_RETURN_RE_INIT           :
            eRes = E_HVD_EX_RET_RE_INIT;
            break;
        case E_HVD_RETURN_NOT_RUNNING       :
            eRes = E_HVD_EX_RET_NOT_RUNNING;
            break;
        case E_HVD_RETURN_CMA_ERROR         :
            eRes = E_HVD_EX_RET_CMA_ERROR;
            break;
        default:
            break;
    }
...
{
    pHVDDrvContext->bHVDIsrAttached = FALSE;
    pHVDDrvContext->bEVDIsrAttached = FALSE;
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    pHVDDrvContext->bEVDLITEIsrAttached = FALSE;
#endif
    return;
}
...
static HVD_EX_Result _HVD_EX_InitVariables(MS_U32 u32Id, HVD_EX_MemCfg *pStMemCfg, HVD_Init_Params *pStInitParams)
{
    ...
    {
        HVD_EX_MSG_ERR("HVD struct define is diff: HVD_Init_Params(%tu) vs HVD_Init_Settings(%tu)\n",
                       (ptrdiff_t) sizeof(HVD_Init_Params), (ptrdiff_t) sizeof(HVD_EX_InitSettings));
            ...
            ((pCtrl->InitParams.u32ModeFlag & HVD_INIT_DUAL_ES_MASK) == HVD_INIT_DUAL_ES_ENABLE))
    {
        ...
        pCtrl->MemMap.u32BitstreamBufAddr = (MS_PHY) pStMemCfg->u32BitstreamBufAddr ;
#if 1
        ...
        pDrvCtrl_sub->MemMap.u32BitstreamBufAddr  = (MS_PHY) pStMemCfg->u32BitstreamBufAddr + pStMemCfg->u32BitstreamBufSize / 2;
        ...
        pDrvCtrl_sub->MemMap.u32BitstreamBufAddr  = (MS_PHY) pStMemCfg->u32BitstreamBufAddr + (pStMemCfg->u32BitstreamBufSize * 57 / 128);
        ...
        pCtrl->MemMap.u32DrvProcessBufAddr  = pDrvCtrl_sub->MemMap.u32DrvProcessBufAddr  = (MS_PHY) pStMemCfg->u32DrvProcessBufAddr;
        ...
        pDrvCtrl_sub->MemMap.u32CodeBufAddr = (MS_PHY) pStMemCfg->u32CodeBufAddr;
        ...
        HVD_EX_MSG_INF("[MVC] Bitstream buffer: 1st = [0x%lx, 0x%lx, 0x%lx], 2nd = [0x%lx, 0x%lx, 0x%lx].\n",
                       (unsigned long) pCtrl->MemMap.u32BitstreamBufVAddr,
                       (unsigned long)pCtrl->MemMap.u32BitstreamBufAddr,
                       (unsigned long)pCtrl->MemMap.u32BitstreamBufSize,
                       (unsigned long) pDrvCtrl_sub->MemMap.u32BitstreamBufVAddr,
                       (unsigned long)pDrvCtrl_sub->MemMap.u32BitstreamBufAddr,
                       (unsigned long)pDrvCtrl_sub->MemMap.u32BitstreamBufSize);
    }
    else
#endif
    {
    ...
    HVD_EX_MSG_INF("HVD mmap: MIU1base:0x%lx, MIU2base:0x%lx,FW(type:%d addr=0x%lx size=0x%tx), VLC(addr=0x%lx size=0x%tx), Code(addr=0x%lx size=0x%tx), FB(addr=0x%lx size=0x%tx), Bit(addr=0x%lx size=0x%tx), Drv(addr=0x%lx size=0x%tx)\n",
                   (unsigned long)pCtrl->MemMap.u32MIU1BaseAddr,
                   (unsigned long)pCtrl->MemMap.u32MIU2BaseAddr,
                   pCtrl->MemMap.eFWSourceType, (unsigned long)pCtrl->MemMap.u32FWBinaryAddr, (ptrdiff_t)pCtrl->MemMap.u32FWBinarySize,
                   (unsigned long)pCtrl->MemMap.u32VLCBinaryAddr, (ptrdiff_t)pCtrl->MemMap.u32VLCBinarySize,
                   (unsigned long)pCtrl->MemMap.u32CodeBufAddr, (ptrdiff_t)pCtrl->MemMap.u32CodeBufSize,
                   (unsigned long)pCtrl->MemMap.u32FrameBufAddr, (ptrdiff_t)pCtrl->MemMap.u32FrameBufSize,
                   (unsigned long)pCtrl->MemMap.u32BitstreamBufAddr, (ptrdiff_t)pCtrl->MemMap.u32BitstreamBufSize,
                   (unsigned long)pCtrl->MemMap.u32DrvProcessBufAddr, (ptrdiff_t)pCtrl->MemMap.u32DrvProcessBufSize);

    pCtrl->InitParams.u16DecoderClock = HAL_HVD_EX_GetCorretClock(pStInitParams->u16DecoderClock);

#ifdef _DISABLE_MDRV_SYS
    pCtrl->InitParams.u16ChipECONum = 1;
#else
    ...
    switch (eCmd)
    {
        case E_HVD_CHECK_CMD_INIT:
        {
            if (!(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u8TurboInit & E_HVD_EX_TURBOINIT_CHECK))
            {
                ...
                {
                    HVD_EX_MSG_ERR("Init: main type can not be recognized:%td\n", (ptrdiff_t)u32Tmp0);
                    return E_HVD_EX_RET_INVALID_PARAMETER;
                }
                ...
                if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr & 0x7FF) != 0)
                {
                    HVD_EX_MSG_ERR("Init: Error HVD code address(0x%lx) must 2048 byte alignment\n",
                                   (unsigned long)pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr);
                    return E_HVD_EX_RET_INVALID_PARAMETER;
                }
            }
...
#if 0
                        HVD_EX_MSG_ERR
                        ("Init: File mode(TS or MP4) and Drv input must set the process buffer size and must be larger than %tu Bytes:%td\n",
                         (ptrdiff_t)pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32DrvProcessBufSize,
                         (ptrdiff_t) (HVD_BBU_ST_ADDR_IN_BITSTREAMBUF +
...
#endif
#ifdef STELLAR
    HVD_EX_MSG_DBG("u32CodeStartOffset:0x%x u32BitStartOffset:0x%x u32FrmStartOffset:0x%x u32DrvProccStartOffset:0x%x\n", (unsigned int)u32CodeStartOffset, (unsigned int)u32BitStartOffset, (unsigned int)u32FrmStartOffset, (unsigned int)u32DrvProccStartOffset);
#endif
            ...
            if (u8BitMiuSel != u8CodeMiuSel)
            {
                        ...
                        (pCtrl->MemMap.u32DrvProcessBufSize > (HVD_BBU_ST_ADDR_IN_BITSTREAMBUF + ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_BBU_TBL_ENTRY_NUMB) << 1 ) << 3) + 8) ))
                {
                    ...
                    if( (( pCtrl->u32BBUTblInBitstreamBufAddr + pCtrl->MemMap.u32DrvProcessBufAddr) % 8) != 0)
                    {
                        ...
                        HVD_EX_MSG_ERR("Nal table in bitstream buffer start address is not aligned. old:%lx new:%lx" , (unsigned long) HVD_BBU_ST_ADDR_IN_BITSTREAMBUF, (unsigned long) pCtrl->u32BBUTblInBitstreamBufAddr  );
                    }

                    /// Setting BBU for MVC dual bbu input
                    pDrvCtrl_sub = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId + 1]);
                    ...
                    if( (( pDrvCtrl_sub->u32BBUTblInBitstreamBufAddr + pDrvCtrl_sub->MemMap.u32DrvProcessBufAddr) % 8) != 0)
                    {
                        ...
                        HVD_EX_MSG_ERR("[MVC]: Nal table in bitstream buffer start address is not aligned. old:%lx new:%lx" ,  (unsigned long) HVD_BBU_ST_ADDR_IN_BITSTREAMBUF + (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_BBU_TBL_ENTRY_NUMB) << 3)  , (unsigned long) pCtrl->u32BBUTblInBitstreamBufAddr);
                    }
                            ...
                            (HVD_BBU_ST_ADDR_IN_BITSTREAMBUF + (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_BBU_TBL_ENTRY_NUMB) + 8)))
                    {
                        ...
                        if (((pCtrl->u32BBUTblInBitstreamBufAddr + pCtrl->MemMap.u32DrvProcessBufAddr) % 8) != 0)
                        {
                            ...
                            pCtrl->u32BBUTblInBitstreamBufAddr -= pCtrl->MemMap.u32DrvProcessBufAddr;
                            HVD_EX_MSG_ERR
                            ("Nal table in bitstream buffer start address is not aligned. old:%tx new:%tx \n",
                             (ptrdiff_t) HVD_BBU_ST_ADDR_IN_BITSTREAMBUF, (ptrdiff_t)pCtrl->u32BBUTblInBitstreamBufAddr);
                        }
                    }
                    else
                    {
                        HVD_EX_MSG_ERR
                        ("Driver process buffer size is not enough for driver input path. input:%tx required:%lx \n",
                         (ptrdiff_t) pCtrl->MemMap.u32DrvProcessBufSize,
                         (unsigned long)(HVD_BBU_ST_ADDR_IN_BITSTREAMBUF + (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_BBU_TBL_ENTRY_NUMB) << 3) +
                                         8));
                        return E_HVD_EX_RET_OUTOF_MEMORY;
                    }
            }
...
static HVD_EX_Result _HVD_EX_CheckFWVersion(MS_U32 u32Id)
{
    ...
    if (!(pCtrl->InitParams.u8TurboInit & E_HVD_EX_TURBOINIT_CHECK))
    {
        ...
        {
            HVD_EX_MSG_ERR("fw interface and binary is not the same. fw ID: interface:%tx binary:%lx\n",
                           (ptrdiff_t) HVD_FW_IF_VERSION, (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_IF_VERSION_ID));
            return E_HVD_EX_FAIL;
        }
    }
...
static HVD_EX_Result _HVD_EX_InitFW(MS_U32 u32Id)
{
    ...
    // Set sub sample mode
    if (pHVDDrvContext->gHVDPreCtrl[_HVD_EX_GetDrvId(u32Id)].eVdecFeature & E_HVD_DEF_FEATURE_DISABLE_SUB_SAMPLE)
    {
        HVD_EX_MSG_DBG("HAL_HVD_EX_SetSubSampleMode Disable\n");
        // BIT2=1: Disable;
        HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_SUB_SAMPLE_MODE, 0);// Disable
    }
    else
    {
        HVD_EX_MSG_DBG("HAL_HVD_EX_SetSubSampleMode Default(AUTO)\n");
        // BIT2=0: Default;
        HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_SUB_SAMPLE_MODE, 2);// AUTO(Enable that resolution larger than FHD)
    }
...
static MS_BOOL _HVD_EX_IsAllBufferEmpty(MS_U32 u32Id, MS_BOOL bFlush)
{
    ...
    {
#ifdef VDEC3
        MS_VIRT u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
    ...
    {
#ifdef VDEC3
        bCondition = (bFlush) ? (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_FLUSH_STATUS) == E_HVD_EX_FLUSH_DONE) :
                     (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_FLUSH_STATUS) == E_HVD_EX_FLUSH_DONE || HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_PLAYBACK_FINISH));

        if (bCondition && (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DEC_Q_NUMB) == 0) && HAL_HVD_EX_DispFrameAllViewed(u32Id))
#else // VDEC3
...
static void _EVDLITE_EX_ISRHandler(void)
{
    ...
    for (u8Idx = 0; u8Idx < HVD_MAX_STREAMS; u8Idx++)
    {
        ...
        if (pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.eHWDecIsr != E_HWDEC_ISR_EVD_LITE)
        {
            continue;
        }
    ...
    HAL_HVD_EX_SetClearISR(E_HWDEC_ISR_EVD_LITE);
    OSAL_HVD_ISR_Enable(E_HWDEC_ISR_EVD_LITE);
}
#endif
#endif
...
// TODO: complete this function. and consider more cases. step decoder, display?
    ...
    switch (pCtrl->InitParams.u32ModeFlag & HVD_INIT_MAIN_MASK)
    {
        case HVD_INIT_MAIN_LIVE_STREAM:
        {
            ...
            {
                HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DISP_INFO_TH, (MS_VIRT) (&pCtrl->Settings.DispInfoTH));
...
#if defined(HAL_CHIP_SUPPORT_EVD)
...
HVD_EX_Result _HVD_EX_Rst(MS_U32 u32Id, MS_BOOL bErrHandle)
{
    ...
    {
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
        if (HAL_HVD_EX_GetIsEVDLiteFlag(u32Id) == TRUE)
            bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_EVD_LITE);
        else
#endif
...
static MS_U32 _HVD_EX_Map2HVDErrCode(MS_U32 u32ErrCode)
{
    ...
    switch (u32ErrCode)
    {
        ...
        case E_HVD_ERR_CMA_FAILED:
            u32Ret = E_HVD_EX_ERRCODE_OUT_OF_MEMORY;
            break;
        case E_HVD_ERR_UNKNOWN_CODEC:
            u32Ret = E_HVD_EX_ERRCODE_UNKNOWN_CODEC;
            break;
        case E_HVD_ERR_RES_NOT_SUPPORT:
            u32Ret = E_HVD_EX_ERRCODE_RES_NOT_SUPPORT;
            break;
        case E_HVD_ERR_HDR_WRONG_DISP_PATH:
            u32Ret = E_HVD_EX_ERRCODE_HDR_WRONG_DISP_PATH;
            break;
        ...
        case E_HVD_ERR_AVC_RES:
            ...
            break;
        case E_HVD_ERR_AVC_ERROR_CONCEAL:
            u32Ret = E_HVD_EX_ERRCODE_AVC_ERROR_CONCEAL;
            break;
        case E_HVD_ERR_AVC_ERROR_NAL:
            u32Ret = E_HVD_EX_ERRCODE_AVC_ERROR_NAL;
            break;
        ...
        case E_HVD_ERR_RM_SIZE_OUT_FB_LAYOUT:
            ...
            break;
        case E_HVD_ERR_HEVC_SPS_BROKEN:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_SPS_BROKEN;
            break;
        case E_HVD_ERR_HEVC_SPS_NOT_IN_SPEC:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_SPS_NOT_IN_SPEC;
            break;
        case E_HVD_ERR_HEVC_SPS_NOT_ENOUGH_FRM:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_SPS_NOT_ENOUGH_FRM;
            break;
        case E_HVD_ERR_HEVC_PPS_BROKEN:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_PPS_BROKEN;
            break;
        case E_HVD_ERR_HEVC_VPS_BROKEN:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_VPS_BROKEN;
            break;
        case E_HVD_ERR_HEVC_REF_LIST:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_REF_LIST;
            break;
        case E_HVD_ERR_HEVC_NO_REF:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_NO_REF;
            break;
        case E_HVD_ERR_HEVC_RES:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_RES;
            break;
        case E_HVD_ERR_HEVC_ERROR_CONCEAL:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_ERROR_CONCEAL;
            break;
        case E_HVD_ERR_HEVC_ERROR_NAL:
            u32Ret = E_HVD_EX_ERRCODE_HEVC_ERROR_NAL;
            break;
        default:
            break;
    }
...
//-----------------------------------------------------------------------------
void MDrv_HVD_EX_SetOSRegBase(MS_VIRT u32RegBaseAddr)
{
#if defined (__aarch64__)
    HVD_EX_MSG_DBG("u32RiuBaseAddr=%lx\n", u32RegBaseAddr);
#else
    HVD_EX_MSG_DBG("u32RiuBaseAddr=%tx\n", (ptrdiff_t)u32RegBaseAddr);
#endif
...
void MDrv_HVD_EX_MJPEG_InitSharemem(MS_U32 u32Id, MS_VIRT u32CodeBufVAddr, MS_BOOL bPreConnEnable, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH eMvopPath, HVD_EX_Original_Stream eStream)
{
    HVD_Pre_Ctrl *pCtrl;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
#ifdef VDEC3
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].eStream = (HVD_Original_Stream)eStream;
#endif

    pCtrl = &(pHVDDrvContext->gHVDPreCtrl[u8DrvId]);
    ...
    HVD_EX_MSG_INF("u8DrvId=%d, PA:[0x%lx, 0x%lx], VA:[0x%lx, 0x%lx], [0x%lx]\n", u8DrvId,
                   (unsigned long)pHVDDrvContext->gHVDCtrl_EX[0].MemMap.u32CodeBufAddr,
                   (unsigned long)pHVDDrvContext->gHVDCtrl_EX[1].MemMap.u32CodeBufAddr,
                   (unsigned long)pHVDDrvContext->gHVDCtrl_EX[0].MemMap.u32CodeBufVAddr,
                   (unsigned long)pHVDDrvContext->gHVDCtrl_EX[1].MemMap.u32CodeBufVAddr,
                   (unsigned long)HAL_HVD_EX_GetShmAddr(u32Id));
...
{
    HVD_EX_Drv_Ctrl *pCtrl;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U8 u8HalIdx   = _HVD_EX_GetStreamIdx(u32Id);
    pHVDDrvContext->bHVDIsInited[u8DrvId] = FALSE;
    HAL_VPU_EX_ReleaseFreeStream(u8HalIdx);

    pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    HAL_HVD_EX_ClearBBUSetting(u32Id, pCtrl->u32BBUId);
}
...
void MDrv_HVD_EX_ForceSwRst(void)
{
    HAL_VPU_EX_ForceSwRst();
}
...
HVD_EX_Result MDrv_HVD_EX_GetFreeStream(MS_U32 *pu32Id, HVD_EX_DRV_StreamType eStreamType)
{
    ...
    if (E_HVD_EX_DRV_MAIN_STREAM == eStreamType)
    {
        ...
        if(u32HVDStreamId == E_HAL_HVD_STREAM_NONE)
        {
    ...
    else if ((eStreamType >=  E_HVD_EX_DRV_N_STREAM) && (eStreamType < E_HVD_EX_DRV_N_STREAM + HVD_MAX_STREAMS))
    {
        ...
        if(u32HVDStreamId == E_HAL_HVD_STREAM_NONE)
        {
            HAL_VPU_EX_ReleaseFreeStream(eStreamType - E_HVD_EX_DRV_N_STREAM);
        }

        if (u32VPUStreamId && u32HVDStreamId)
        {
            *pu32Id = ((u32HVDStreamId & 0x3F) << 16 | u32HVDStreamId << 8 | u32VPUStreamId);
            eRet = E_HVD_EX_OK;
        }
    }
#endif
    ...
    {
        VPRINTF("%s:%s:Stream type=%d is unexpected\n", __FILE__, __FUNCTION__, eStreamType);
    }
...
//-----------------------------------------------------------------------------
#ifdef VDEC3
HVD_EX_Result MDrv_HVD_EX_Init(MS_U32 u32Id, HVD_EX_MemCfg *pStMemCfg, HVD_EX_InitSettings *pStInitSettings, MS_BOOL bFWdecideFB, MS_BOOL bShareBBU, HVD_EX_Original_Stream eStream)
#else
...
#endif
{
    ...
    if (u32UartCtrl & E_HVD_UART_CTRL_INFO)
    {
        ...
        HVD_EX_MSG_DBG("MSOS API check: 1st:%tu 2nd:%tu delay:%tu dif:%tu\n",
                       (ptrdiff_t)u32FirstTime, (ptrdiff_t)u32SecondTime, (ptrdiff_t)u32delaytime, (ptrdiff_t)(u32SecondTime - u32FirstTime));
    }
    ...
    if (((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC) ||
            ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC_DV))
    {
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
        if (HAL_HVD_EX_GetIsEVDLiteFlag(u32Id) == TRUE)
            bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_EVD_LITE);
...
#ifdef VDEC3
    pCtrl->bShareBBU = bShareBBU;
    pCtrl->eStream = (HVD_Original_Stream)eStream;
    HVD_EX_MSG_INF("[NDec][0x%tx][%d] bShareBBU %d, eStream %d \n", (ptrdiff_t)u32Id, u8DrvId, bShareBBU, eStream);
#endif

#if SUPPORT_EVD
    if (0
            || ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC)
            || ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC_DV)
#if SUPPORT_MSVP9
            || ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9)
#endif
#if defined(SUPPORT_AVS2)
            || ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_AVS2)
#endif
       )
    {
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
        if (HAL_HVD_EX_GetIsEVDLiteFlag(u32Id) == TRUE)
            pCtrl->HVDISRCtrl.eHWDecIsr = E_HWDEC_ISR_EVD_LITE;
        else
#endif
    ...
    eRet = _HVD_EX_Init_(u32Id, pStInitSettings->u32ModeFlag);
...
MS_U32 MDrv_HVD_EX_GetBBUId(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    return pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId;
}
...
HVD_EX_Result MDrv_HVD_EX_Exit(MS_U32 u32Id)
{
    ...
    {
        HVD_EX_MSG_ERR("HVD free bbu id (%td) failed\n", (ptrdiff_t)pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId);
    }
#endif
    ...
    do
    {
        ...
        if (u32Timer >= HVD_FW_EXIT_ACTION_TIMEOUT)
        {
            ...
            HVD_EX_MSG_ERR("HVD exit timeout!!!\n");
            //return eRet;
            break;
        }
        else
...
HVD_EX_Result MDrv_HVD_EX_Flush(MS_U32 u32Id, MS_BOOL bShowLast)
{
    ...
    if (MDrv_HVD_EX_GetPlayState(u32Id) == E_HVD_EX_GSTATE_STOP)
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
    ...
    if (MDrv_HVD_EX_GetPlayState(u32Id) != E_HVD_EX_GSTATE_PAUSE)
    {
        ...
        while (1)
        {
            ...
            if (u32Times > 100 || MsOS_GetSystemTime() - u32StartTime > 100)
            {
    ...
    if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_MAIN_MASK) == HVD_INIT_MAIN_FILE_RAW)
    {
        ...
        while (1)
        {
            ...
            {
#ifdef VDEC3
                if (_HVD_EX_IsAllBufferEmpty(u32Id, TRUE))
#else
            ...
            if (u32Times > 1000 || MsOS_GetSystemTime() - u32StartTime > 1000)
            {
                HVD_EX_MSG_ERR("Flush() timeout failed: BBU:%lu Dec:%lu Disp:%lu Idle:%lu ESr:%lx ESw:%lx\n",
                               (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_BBU_Q_NUMB),
                               (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DEC_Q_NUMB),
                               (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DISP_Q_NUMB),
                               (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_VPU_IDLE_CNT),
                               (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_ES_READ_PTR), (unsigned long)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_ES_WRITE_PTR));
                eRet = E_HVD_EX_FAIL;
                break;
            }
        }
...
HVD_EX_Result MDrv_HVD_EX_SetDynmcDispPath(MS_U32 u32Id, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH eDispPath)
{
    ...
    eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DYNAMIC_CONNECT_DISP_PATH, u32Arg);
    HVD_PRINT("%s waiting command\n", __FUNCTION__);
    //u32TimeOut = MsOS_GetSystemTime() + 500; // it can not block
    u32TimeOut = MsOS_GetSystemTime() + HVD_CMD_TIMEOUT;
    while (1)
    {
        if(bConnect)
        {
    ...
    HVD_PRINT("%s command finish eRet:%d\n", __FUNCTION__, eRet);
    return eRet;
}
...
HVD_EX_Result MDrv_HVD_EX_SetAVSyncDispAutoDrop(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AVSYNC_DISP_AUTO_DROP, bEnable);
...
HVD_EX_Result MDrv_HVD_EX_StepDecode(MS_U32 u32Id)
{
    ...
    if (pHVDDrvContext->gHVDPreCtrl[u8DrvId].u32CustomerMode & HVD_EX_CUSTOMER_ENABLE_NDS)
    {
        // Check fw state first when going to step decode
...
HVD_EX_Result MDrv_HVD_EX_PushQueue(MS_U32 u32Id, HVD_EX_PacketInfo *pInfo)
{
    ...
    //patch for mantis:0965296 stream AVS_MP3_FileFormat.mp4 hang
    if(((pCtrl->InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_AVS) && (pInfo->u32Length > 0) && (pInfo->u32Length < HVD_MIN_PACKET_SIZE))
    {
        stVpuPkt.u32Length = HVD_MIN_PACKET_SIZE;
    }
    ...
    stVpuPkt.u8Version    = pInfo->u8Version;

    u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
...
#if 0
        {
            HVD_EX_MSG_INF("HVD : %u (%u %u) ID:%x input packet (%x %x %x) (%x %x %x) (%u %u %u)\n ",
                ...
                ((!pCtrl->bCannotAccessMIU256) || ((pCtrl->bCannotAccessMIU256) && (pCtrl->MemMap.u32BitstreamBufAddr < pCtrl->MemMap.u32MIU1BaseAddr))))
        {
            if ((pCtrl->u8SecureMode == E_HVD_SECURE_MODE_NONE) && (!(pHVDDrvContext->bVPUIsSecureMode)))
            {
                        ...
                        && ((*pByte == 0) && (*(pByte - 1) == 0) && (*(pByte - 2) == 0)))
                {
                    ...
                    //HVD_PRINT("no need to remove pending zero~~\n");
                }
                else
                {
                ...
                if (ModifyCnt != 0)
                {
                    //HVD_MSG_INFO("HVD remove last zero byte:%u\n" , ModifyCnt);
                }

                if (bbuInfo.u32Length == 0)
                {
                    HVD_EX_MSG_ERR("Packet with all zero bytes(staddr:0x%x remove zero bytes:%u)\n ",
...
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_DecodeIFrame(MS_U32 u32Id, MS_PHY u32SrcSt, MS_U32 u32SrcSize)
{
    MS_U32 timer = 300;
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
HVD_EX_Result MDrv_HVD_EX_SetCalFrameRate(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bCalFrameRate = bEnable;

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_SkipNFrame(MS_U32 u32Id, MS_U32 u32FrameCnt)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_N_FRAME, u32FrameCnt);
...
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetFrmBuffAddr(MS_U32 u32Id, MS_PHY u32FrmBuffAddr)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set u32FrmBuffAddr = 0x%tX\n", (ptrdiff_t)u32FrmBuffAddr);

//#if HVD_ENABLE_CHECK_STATE_BEFORE_SET_CMD
//    if (HAL_HVD_EX_GetData(E_HVD_GDATA_DISPLAY_DURATION) != u32FrmBuffAddr)
//#endif
    {
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBADDR, u32FrmBuffAddr);
    }
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set FrmBuffSize = 0x%tX\n", (ptrdiff_t)u32FrmBuffSize);

//#if HVD_ENABLE_CHECK_STATE_BEFORE_SET_CMD
//    if (HAL_HVD_EX_GetData(E_HVD_GDATA_DISPLAY_DURATION) != u32FrmBuffSize)
//#endif
    {
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBSIZE, u32FrmBuffSize);
    }
    /*#if HVD_ENABLE_WAIT_CMD_FINISHED
        if (eRet == E_HVD_EX_OK)
        {
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetMaxCMASize()
/// @brief \b Function \b Description:  specify the max cma frame buffer size.
/// @param -u32MaxCMASize \b IN : max CMA frame buffer size.
/// @param -u32MaxCMASize2 \b IN : max CMA frame buffer2 size.
/// @return -The result of command set frame buffer address.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetMaxCMASize(MS_U32 u32Id, MS_U32 u32MaxCMASize, MS_U32 u32MaxCMASize2)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    HVD_EX_MSG_DBG("Drv: set max CMA Size=0x%tx Size2=0x%tx\n", (ptrdiff_t)u32MaxCMASize, (ptrdiff_t)u32MaxCMASize2);

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_MAX_CMA_SIZE, u32MaxCMASize);

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_MAX_CMA_SIZE2, u32MaxCMASize2);

    eRet = E_HVD_RETURN_SUCCESS;
...
/// @return -The result of command set frame buffer address.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_GetCodecCapInfo( int eCodecType, void *pCodecCapInfo)
{
    if(HAL_VPU_EX_GetCodecCapInfo(  eCodecType, pCodecCapInfo))
        return E_HVD_RETURN_SUCCESS;
    else
        return E_HVD_RETURN_FAIL;
}

HVD_EX_Result MDrv_HVD_EX_GetCodecProfileCapInfo(int eCodecType, MS_U32 u32CodecCapProfileTableSize, void *pCodecProfileCapInfo)
{
    if (HAL_VPU_EX_GetCodecProfileCapInfo(eCodecType, u32CodecCapProfileTableSize, pCodecProfileCapInfo))
        return E_HVD_RETURN_SUCCESS;
    else
        return E_HVD_RETURN_FAIL;
}

HVD_EX_Result MDrv_HVD_EX_SetEnalbeHDR(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    HVD_EX_MSG_DBG("Drv: SetEnalbeHDR = %d\n", (unsigned int) bEnable);

    if (pHVDDrvContext->gHVDPreCtrl[u8DrvId].bDVSingleLayerMode != TRUE)
    {
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_ENABLE_HDR, bEnable);
    }

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_SetHDR10PerFrame(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    HVD_EX_MSG_DBG("Drv: SetHDR10PerFrame = %d\n", (unsigned int) bEnable);

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_HDR_PERFRAME, bEnable);

    _DRV_HVD_EX_RET(u8DrvId, eRet);

}

HVD_EX_Result MDrv_HVD_EX_SetHDRDynamic(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_HDR_DYNAMIC, bEnable);

    _DRV_HVD_EX_RET(u8DrvId, eRet);

}

HVD_EX_Result MDrv_HVD_EX_GetHDRMeta22bytes(MS_U32 u32Id, MS_U16 HDRMDMIU, MS_PHY u32MetaAddr, MS_U16 u16HDRVer, MS_U32 *u32FirstMetaAddr,
        MS_U32 *u32SecondMetaAddr, MS_U8 *u8FirstMetaEnable, MS_U8 *u8SecondMetaEnable,
        MS_U8 *u8payload_mode, MS_U8 *u8transferCharacteristics, MS_U32 *u32maxContentLightLevel)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;

    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U8 u8XCShmMiuSel;
    MS_U32 u32XCShmStartOffset;
    MS_VIRT u32CodeVaddr = 0;
    MS_VIRT u32T35Vaddr = 0;
    MS_PHY  u32MetaPhyAddr = 0;
    _DRV_HVD_Inited(u8DrvId, eRet);
    _DRV_HVD_EX_Entry(u8DrvId);

    _phy_to_miu_offset(u8XCShmMiuSel, u32XCShmStartOffset, u32MetaAddr);

    u32CodeVaddr = MS_PA2KSEG1((MS_PHY)u32MetaAddr);

    if(u32MetaAddr == (MS_PHY)NULL || u32CodeVaddr == (MS_VIRT)NULL ||
            u16HDRVer < 2 || u16HDRVer > 4 || CHECK_NULL_PTR(u32FirstMetaAddr) || CHECK_NULL_PTR(u32SecondMetaAddr) ||
            CHECK_NULL_PTR(u8FirstMetaEnable) || CHECK_NULL_PTR(u8SecondMetaEnable))
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
    else if (u16HDRVer == 2 || u16HDRVer == 4)
    {
        *u32FirstMetaAddr = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 6)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 7)) << 8) & 0x0000FF00) |
                            (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 8)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 9)) << 24) & 0xFF000000);
        *u32SecondMetaAddr = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 16)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 17)) << 8) & 0x0000FF00) |
                             (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 18)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 19)) << 24) & 0xFF000000);
        *u8FirstMetaEnable = *((MS_U8 *)(u32CodeVaddr + 11));
        *u8SecondMetaEnable = *((MS_U8 *)(u32CodeVaddr + 21));
        eRet = E_HVD_EX_OK;
    }
    else if (u16HDRVer == 3)
    {
        *u32FirstMetaAddr = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 8)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 9)) << 8) & 0x0000FF00) |
                            (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 10)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 11)) << 24) & 0xFF000000);
        *u32SecondMetaAddr = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 18)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 19)) << 8) & 0x0000FF00) |
                             (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 20)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 21)) << 24) & 0xFF000000);
        *u8FirstMetaEnable = *((MS_U8 *)(u32CodeVaddr + 2));
        *u8SecondMetaEnable = *((MS_U8 *)(u32CodeVaddr + 12));
        _miu_offset_to_phy(HDRMDMIU, (MS_PHY)*u32FirstMetaAddr, u32MetaPhyAddr);
        u32T35Vaddr = MS_PA2KSEG1(u32MetaPhyAddr);
        *u8payload_mode = *((MS_U8 *) u32T35Vaddr + 2);
        *u8transferCharacteristics = *((MS_U8 *) u32T35Vaddr + 18);
        *u32maxContentLightLevel = ((MS_U32) * ((MS_U8 *) (u32T35Vaddr + 128)) & 0x000000FF) | (((MS_U32) * ((MS_U8 *)(u32T35Vaddr + 129)) << 8) & 0x0000FF00);
        eRet = E_HVD_EX_OK;
    }
...
HVD_EX_Result MDrv_HVD_EX_GetOpenHDRInfo(MS_U32 u32Id, MS_U16 u16HDRVer, MS_U8 *u8ColorFormat, MS_U8 *u8dataColorFormat, MS_U8 *u8FullRange,
        MS_U8 *u8VUIEnable, MS_U8 *u8ColorPrimaries, MS_U8 *TransferCharacteristics, MS_U8 *u8MatrixCoefficients,
        MS_U8 *u8SEIEnable, MS_U16 u16WhitePoint[2], MS_U32 *u32MaxLuminance, MS_U32 *u32MinLuminance,
        MS_U8 *u8ContentLightLevelEnable, MS_U16 *u32MaxContentLightLevel, MS_U16 *u32MaxAvgContentLightLevel)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    _phy_to_miu_offset(u8XCShmMiuSel, u32XCShmStartOffset,  pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr);

    u32CodeVaddr = MS_PA2KSEG1((MS_PHY) pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr);

    if(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr == (MS_PHY)NULL || u32CodeVaddr == (MS_VIRT)NULL ||
            u16HDRVer != 1 || CHECK_NULL_PTR(u8ColorFormat) || CHECK_NULL_PTR(u8dataColorFormat) ||
            CHECK_NULL_PTR(u8FullRange) || CHECK_NULL_PTR(u8VUIEnable) || CHECK_NULL_PTR(u8ColorPrimaries) || CHECK_NULL_PTR(TransferCharacteristics) || CHECK_NULL_PTR(u8MatrixCoefficients) ||
            CHECK_NULL_PTR(u8SEIEnable) || CHECK_NULL_PTR(u16WhitePoint) || CHECK_NULL_PTR(u32MaxLuminance) || CHECK_NULL_PTR(u32MinLuminance) || CHECK_NULL_PTR(u8ContentLightLevelEnable) ||
            CHECK_NULL_PTR(u32MaxContentLightLevel) || CHECK_NULL_PTR(u32MaxAvgContentLightLevel))
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
    ...
    {
        *u8ColorFormat = *((MS_U8 *)(u32CodeVaddr + 2));
        *u8dataColorFormat = *((MS_U8 *)(u32CodeVaddr + 3));
        *u8FullRange = *((MS_U8 *)(u32CodeVaddr + 4));
        *u8VUIEnable = *((MS_U8 *)(u32CodeVaddr + 5));
        *u8ColorPrimaries = *((MS_U8 *)(u32CodeVaddr + 6));
        *TransferCharacteristics = *((MS_U8 *)(u32CodeVaddr + 7));
        *u8MatrixCoefficients = *((MS_U8 *)(u32CodeVaddr + 8));
        *u8SEIEnable = *((MS_U8 *)(u32CodeVaddr + 9));
        u16WhitePoint[0] = ((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 22)) & 0x00FF) | (((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 23)) << 8) & 0xFF00);
        u16WhitePoint[1] = ((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 24)) & 0x00FF) | (((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 25)) << 8) & 0xFF00);
        *u32MaxLuminance = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 26)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 27)) << 8) & 0x0000FF00) |
                           (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 28)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 29)) << 24) & 0xFF000000);
        *u32MinLuminance = ((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 30)) & 0x00000FF) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 31)) << 8) & 0x0000FF00) |
                           (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 32)) << 16) & 0x00FF0000) | (((MS_U32) * ((MS_U8 *)(u32CodeVaddr + 33)) << 24) & 0xFF000000);
        *u8ContentLightLevelEnable = *((MS_U8 *)(u32CodeVaddr + 34));
        *u32MaxContentLightLevel = ((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 35)) & 0x00FF) | (((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 36)) << 8) & 0xFF00);
        *u32MaxAvgContentLightLevel = ((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 37)) & 0x00FF) | (((MS_U16) * ((MS_U8 *)(u32CodeVaddr + 38)) << 8) & 0xFF00);
        eRet = E_HVD_EX_OK;
    }
...
HVD_EX_Result MDrv_HVD_EX_GetHDREntries(MS_U32 u32Id, MS_U16 u16HDRVer, MS_U32 u32Idx, MS_U32 *u32CFDRdy, MS_U32 *MetaAddr,
                                        MS_U32 *RegAddr, MS_U32 *RegSize, MS_U32 *LutAddr, MS_U32 *LutSize)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    if(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr == (MS_PHY)NULL || u32CodeVaddr == (MS_VIRT)NULL ||
            u16HDRVer < 1 || u16HDRVer > 5 || CHECK_NULL_PTR(u32CFDRdy) || CHECK_NULL_PTR(MetaAddr) ||
            CHECK_NULL_PTR(RegAddr) || CHECK_NULL_PTR(RegSize) || CHECK_NULL_PTR(LutAddr) || CHECK_NULL_PTR(LutSize))
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
    ...
    {
        *u32CFDRdy = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24)));
        *MetaAddr = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24) + 4));
        *RegAddr = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24) + 8));
        *RegSize = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24) + 12));
        *LutAddr = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24) + 16));
        *LutSize = *((MS_U32 *)(u32CodeVaddr + 32 + (u32Idx * 24) + 20));
        eRet = E_HVD_EX_OK;
    }
...
HVD_EX_Result MDrv_HVD_EX_GetHDRCommInfo(MS_U32 u32Id, MS_U16 *u16HDRVer, MS_U16 *u16MDMIU, MS_U16 *u16LUTMIU,
        MS_U32 *u32Rdptr, MS_U32 *u32wrptr, MS_U32 *u32MDbase, MS_U32 *u32LutTotalSize)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    if(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr == (MS_PHY)NULL ||
            u32CodeVaddr == (MS_VIRT)NULL || CHECK_NULL_PTR(u16HDRVer))
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
    ...
    {
        *u16HDRVer = *((MS_U16 *)(u32CodeVaddr)) & 0xFF;
        eRet = E_HVD_EX_OK;
    }

    if (*u16HDRVer > 1 && *u16HDRVer < 6 && !CHECK_NULL_PTR(u32CodeVaddr) && !CHECK_NULL_PTR(u16HDRVer) &&
            !CHECK_NULL_PTR(u16LUTMIU) && !CHECK_NULL_PTR(u32Rdptr) && !CHECK_NULL_PTR(u32wrptr) &&
            !CHECK_NULL_PTR(u32MDbase) && !CHECK_NULL_PTR(u32LutTotalSize))
    {
        *u16MDMIU = *((MS_U16 *)(u32CodeVaddr + 4));
        *u16LUTMIU = *((MS_U16 *)(u32CodeVaddr + 6));
        *u32Rdptr = *((MS_U32 *)(u32CodeVaddr + 8));
        *u32wrptr = *((MS_U32 *)(u32CodeVaddr + 12));
        *u32MDbase = *((MS_U32 *)(u32CodeVaddr + 24));
        *u32LutTotalSize = (*((MS_U32 *)(u32CodeVaddr + 32 + (2 * 24) + 16)) - * ((MS_U32 *)(u32CodeVaddr + 32 + (1 * 24) + 16)));
    }
...
HVD_EX_Result MDrv_HVD_EX_SetDVXCShmBuff(MS_U32 u32Id, MS_PHY u32FWBaseAddr, MS_PHY u32DVXCShmAddr, MS_SIZE u32DVXCShmSize)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    MS_U8 u8FWBaseMiuSel;
    MS_U32 u32FWBaseOffset;
    ...
    HVD_EX_MSG_DBG("Drv: SetDVXCShmAddr=0x%llx FWBaseAddr=0x%llx SetDVXCShmSize=0x%x\n",
                   (unsigned long long int) u32DVXCShmAddr, (unsigned long long int)u32FWBaseAddr, (int)u32DVXCShmSize);

    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr = u32DVXCShmAddr;
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32HDRShmSize = u32DVXCShmSize;
    _phy_to_miu_offset(u8FWBaseMiuSel, u32FWBaseOffset, u32FWBaseAddr);
    _phy_to_miu_offset(u8XCShmMiuSel, u32XCShmStartOffset, u32DVXCShmAddr);

    if (u8FWBaseMiuSel != u8XCShmMiuSel)
        HVD_EX_MSG_ERR("[ERROR] XC share memory and VDEC Code base MIU selections are different\n");
    eRet = (HVD_EX_Result)HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_DV_XC_SHM_ADDR, (unsigned long)(u32XCShmStartOffset - u32FWBaseOffset));

    if (eRet == E_HVD_EX_OK)
    {
        eRet = (HVD_EX_Result)HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DV_XC_SHM_SIZE, u32DVXCShmSize);
        u32MIUSel = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);

        u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_XCSHM_MIUSEL));
        u32MIUSel |= (u8XCShmMiuSel << VDEC_XCSHM_MIUSEL);
...
/// @return -The result of command set frame buffer address.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetFrmBuff2(MS_U32 u32Id, MS_PHY u32FrmBuffAddr, MS_U32 u32FrmBuffSize)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U8 u8Frm2MiuSel;
    ...
    {
        u32MIUSel = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);
#if defined(CHIP_MASERATI) || defined(CHIP_MAXIM) || defined(CHIP_M7621)
        if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_HEVC    ||
                (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_HEVC_DV ||
#if SUPPORT_AVS2
                (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_AVS2    ||
#endif
                0)
        {
            u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_HWBUF_MIUSEL));
            u32MIUSel |= (u8Frm2MiuSel << VDEC_HWBUF_MIUSEL);
        }
#endif
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_Set_Playback_Interval()
/// @brief \b Function \b Description: VDEC only play data whose pts is within the desired interval.
/// @param -u32StartPts \b IN : Start PTS.
/// @param -u32EndPts \b IN : End PTS.
///                 -u32StartPts = 0 , u32EndPts = (MS_U32)(-1): disable this mode. FW will play normally.
/// @return -The result of command seek to specific PTS.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_Set_Playback_Interval(MS_U32 u32Id, MS_U32 u32StartPts, MS_U32 u32EndPts)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_PLAYBACK_INTERVAL_START_PTS, u32StartPts);
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_PLAYBACK_INTERVAL_END_PTS, u32EndPts);


    eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ENABLE_PLAYBACK_INTERVAL, 0);
...
#endif
#if 0
                bHVDDisableISRFlag = OSAL_HVD_ISR_Disable(TRUE);
                if(bHVDDisableISRFlag == FALSE)
                {
                    HVD_EX_MSG_ERR("%s Disable Interrupt Fail\n", __FUNCTION__);
                }
                //OSAL_HVD_ISR_Detach();
#if !defined(SUPPORT_X_MODEL_HVD_FEATURE) && !defined(DISABLE_ISR_DETACH)
                if(TRUE == bHVDIsIsrAttached)
                {
                    OSAL_HVD_ISR_Detach(pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_HVD);
                    bHVDIsIsrAttached = FALSE;
                }
#endif
#endif
            }
...
#endif
#if 0
                bEVDDisableISRFlag = OSAL_HVD_ISR_Disable(FALSE);
                if(bEVDDisableISRFlag == FALSE)
                {
                    HVD_EX_MSG_ERR("%s Disable Interrupt Fail\n", __FUNCTION__);
                }
                //OSAL_HVD_ISR_Detach();
#if !defined(SUPPORT_X_MODEL_HVD_FEATURE) && !defined(DISABLE_ISR_DETACH)
                if(TRUE == bEVDIsIsrAttached)
                {
                    OSAL_HVD_ISR_Detach(pCtrl->HVDISRCtrl.bIsHvdIsr);
                    bEVDIsIsrAttached = FALSE;
                }
#endif
#endif
            }
...
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
        else if (pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVD_LITE) // evd lite case
        {
            for (i = 0; i < HVD_MAX_STREAMS; i++)
            {
                if (i != u8DrvId)
                {
                    if ((E_HWDEC_ISR_EVD_LITE == pHVDDrvContext->gHVDCtrl_EX[i].HVDISRCtrl.eHWDecIsr)
                            && (TRUE == pHVDDrvContext->gHVDCtrl_EX[i].bUsed))
                    {
                        bUsed = TRUE;
                        break;
...
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
                if (FALSE == pHVDDrvContext->bEVDLITEIsrAttached)
                {
                    if (OSAL_HVD_ISR_Attach(E_HWDEC_ISR_EVD_LITE, (void*)_EVDLITE_EX_ISRHandler) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to attach HVD_IsrProc!\n");
                        return E_HVD_EX_FAIL;
                    }
                    pHVDDrvContext->bEVDLITEIsrAttached = TRUE;
                }

                if (FALSE == pHVDDrvContext->bEVDLITEMSOSIsrEnable)
                {
                    if (OSAL_HVD_ISR_Enable(E_HWDEC_ISR_EVD_LITE) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to OSAL_HVD_IntEnable!\n");
                        return E_HVD_EX_FAIL;
                    }
                    pHVDDrvContext->bEVDLITEMSOSIsrEnable = TRUE;
                }
#endif
            }
#endif
...
HVD_EX_Result MDrv_HVD_EX_SetDynScalingParam(MS_U32 u32Id, void *pStAddr, MS_U32 u32Size)
{
    ...
    else
    {
        ...
        addr = (MS_PHY)MDrv_HVD_EX_GetDynamicScalingInfo(u32Id, E_HVD_EX_DS_INFO_ADDR);
...
HVD_EX_Result MDrv_HVD_EX_SetDispInfoTH(MS_U32 u32Id, HVD_EX_DispInfoThreshold *DispInfoTH)
{
    ...
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DISP_INFO_TH, (MS_VIRT) (&DispInfoTHTmp));
...
//change to open max setting ,wait f/w release new fwHVD_if
    ...
    if(bEnable)
    {
        ...
        HVD_EX_MSG_INF("Disable Field Detector when display outside mode!\n");
        pShm->u8FrmPostProcSupport &= ~E_HVD_POST_PROC_FIELD_DETECT;
        HAL_HVD_EX_FlushMemory();
    }
...
MS_BOOL MDrv_HVD_EX_IsISROccured(MS_U32 u32Id)
{
    HVD_EX_MSG_TRACE();
    //_DRV_HVD_Inited(FALSE);
#if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(HAL_CHIP_SUPPORT_EVD)
...
#endif
        {
            if (_HVD_EX_IsAllBufferEmpty(u32Id, FALSE))
            {
                return TRUE;
            }
            else
...
MS_BOOL MDrv_HVD_EX_IsStepDecodeDone(MS_U32 u32Id)
{
    ...
    if (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].bStepDecoding)
    {
        ...
        {
            if ( (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32StepDecodeCnt != HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DECODE_CNT))
                    && (E_HVD_FW_PAUSE == HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_STATE)))
            {
                pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32StepDecodeCnt = 0;
                bRet = TRUE;
                pHVDDrvContext->gHVDCtrl_EX[u8DrvId].bStepDecoding = FALSE;
...
HVD_EX_Result MDrv_HVD_EX_CheckDispInfoRdy(MS_U32 u32Id)
{
    ...
    if (pHVDDrvContext->bCMAUsed && !HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_CMA_ALLOC_DONE))
    {
        ...
        VPRINTF("[VDEC][%d]MApi_CMA_Pool_GetMem in hvd: alloc_param.pool_handle_id=%x, alloc_param.flags=%x, alloc_param.offset_in_pool=%llx, alloc_param.length=%x\n",
        ...
        if (pHVDDrvContext->bCMATwoMIU[u8DrvId])
        {
            ...
            VPRINTF("[VDEC][%d]MApi_CMA_Pool_GetMem in hvd2: alloc_param.pool_handle_id=%x, alloc_param.flags=%x, alloc_param.offset_in_pool=%llx, alloc_param.length=%x\n",
                    (unsigned int)u8DrvId,
                    (unsigned int)alloc_param.pool_handle_id,
                    (unsigned int)alloc_param.flags,
                    (unsigned long long int)alloc_param.offset_in_pool,
...
//                            (u8FB2MiuSel << VDEC_LUMA2_MIUSEL) |
...
#else
        u32MIUSel |= (u8FBMiuSel << VDEC_CHROMA8_MIUSEL) |
                     (u8FB2MiuSel << VDEC_LUMA2_MIUSEL);
#endif
#if defined(CHIP_MASERATI) || defined(CHIP_MAXIM)
        if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_HEVC ||
                (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_HEVC_DV)
        {
            u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_HWBUF_MIUSEL));
            u32MIUSel |= (u8FB2MiuSel << VDEC_HWBUF_MIUSEL);
        }
#endif
#if SUPPORT_G2VP9
        if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_VP9)
        ...
        HVD_EX_MSG_DBG("E_HVD_SDATA_MIU_SEL = 0x%lx\n", u32MIUSel);
    ...
    if (u32ErrCode != 0)
    {
        ...
        {
            HVD_EX_MSG_ERR
            ("FW error that Out of Memory:%tx. Allocated frame buffer size is smaller than required.\n",
             (ptrdiff_t)u32ErrCode);
            pHVDDrvContext->gHVDCtrl_EX[u8DrvId].bIsDispInfoChg = FALSE;
            return E_HVD_EX_RET_OUTOF_MEMORY;
        }
    }
...
MS_BOOL MDrv_HVD_EX_IsDispInfoChg(MS_U32 u32Id)
{
...
#if 0                           // debug use
    {
        ...
        if (u32IdleCnt > 1000)
        {
            HVD_PRINT("HVD: seems IDLE: %u decode cnt:%x PTS:%x\n", (MS_U32) bRet, tmp, MDrv_HVD_EX_GetPTS(u32Id));
            u32IdleCnt = 0;
        }
    }
#endif
...
MS_BOOL MDrv_HVD_EX_IsAllBufferEmpty(MS_U32 u32Id)
{
    ...
    if (u32Times > 200)
    {
        HVD_EX_MSG_INF("Flush() timeout failed:BBU:%u Dec:%u Disp:%u Idle:%u ESr:%x ESw:%x  \n",
...
HVD_EX_MPEG4_AVC_PROFILE_BIT _HVD_EX_AVC_Profile_IdcMap2Bit(HVD_EX_MPEG4_AVC_PROFILE_IDC eIdc, MS_U8 u8Constraint_set)
{
    HVD_EX_MPEG4_AVC_PROFILE_BIT eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_NONE;
    switch(eIdc)
    {
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_BP:
            if(u8Constraint_set & 0x40)
                eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_CBP;
            else
                eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_BP;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_XP:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_XP;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_MP:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_MP;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_HiP:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_HiP;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_Hi10P:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_Hi10P;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_Hi422P:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_Hi422P;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_IDC_Hi444P:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_Hi444P;
            break;
        default:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_NOTSUPPORT;
            break;
    }
    return eRet;
}

HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT _HVD_EX_AVC_ProfileLevel_IdcMap2Bit(HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC eIdc, MS_U8 u8Constraint_set)
{
    HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_NONE;
    switch(eIdc)
    {
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L1:
            if(u8Constraint_set & 0x10)
                eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L1;
            else
                eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L1b;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L1_1:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L1_1;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L1_2:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L1_2;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L1_3:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L1_3;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L2:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L2;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L2_1:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L2_1;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L2_2:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L2_2;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L3:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L3;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L3_1:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L3_1;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L3_2:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L3_2;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L4:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L4;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L4_1:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L4_1;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L4_2:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L4_2;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L5:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L5;
            break;
        case E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_IDC_L5_1:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_L5_1;
            break;
        default:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_NOTSUPPORT;
            break;
    }
    return eRet;
}

HVD_EX_MPEGH_HEVC_PROFILE_BIT _HVD_EX_HEVC_Profile_IdcMap2Bit(HVD_EX_MPEGH_HEVC_PROFILE_IDC eIdc, MS_U16 u8Constraint_set)
{
    HVD_EX_MPEG4_AVC_PROFILE_BIT eRet = E_HVD_EX_MPEG4_AVC_PROFILE_BIT_NONE;
    switch(eIdc)
    {
        case E_HVD_EX_MPEGH_HEVC_PROFILE_IDC_MP:
            eRet = E_HVD_EX_MPEGH_HEVC_PROFILE_BIT_MP;
            break;
        case E_HVD_EX_MPEGH_HEVC_PROFILE_IDC_MP10:
            eRet = E_HVD_EX_MPEGH_HEVC_PROFILE_BIT_MP10;
            break;
        case E_HVD_EX_MPEGH_HEVC_PROFILE_IDC_MP_STILL:
            eRet = E_HVD_EX_MPEGH_HEVC_PROFILE_BIT_MP_STILL;
            break;
        default:
            eRet = E_HVD_EX_MPEGH_HEVC_PROFILE_BIT_NOTSUPPORT;
            break;
    }
    return eRet;
}

HVD_EX_MPEGH_HEVC_LEVEL_BIT _HVD_EX_HEVC_ProfileLevel_IdcMap2Bit(HVD_EX_MPEGH_HEVC_LEVEL_IDC eIdc, MS_U16 u8Constraint_set)
{
    HVD_EX_MPEGH_HEVC_LEVEL_BIT eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_NOTSUPPORT;
    switch(eIdc)
    {
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L2:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L2;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L2_1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L2_1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L3:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L3;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L3_1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L3_1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L4:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L4;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L4_1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L4_1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L5:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L5;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L5_1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L5_1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L5_2:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L5_2;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L6:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L6;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L6_1:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L6_1;
            break;
        case E_HVD_EX_MPEGH_HEVC_LEVEL_IDC_L6_2:
            eRet = E_HVD_EX_MPEGH_HEVC_LEVEL_BIT_L6_2;
            break;
        default:
            eRet = E_HVD_EX_MPEG4_AVC_PROFILE_LEVEL_BIT_NOTSUPPORT;
            break;
...
static HVD_EX_Result _HVD_EX_GetExtendDispInfo(MS_U32 u32Id, HVD_EX_ExtendDispInfo *pInfo, MS_BOOL bClearSeqChg)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;

#ifndef DISABLE_GET_EXTEND_DISP_INFO
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_Extend_Display_Info stExtendDisplayInfo;
    MS_VIRT u32ExtendDispInfoAddr = 0;
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);

    u32ExtendDispInfoAddr = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_EXTEND_DISP_INFO_ADDR);
    if(u32ExtendDispInfoAddr == 0)
    {
        return E_HVD_EX_RET_NOTREADY;
    }
    HVD_memcpy((void *) &stExtendDisplayInfo, (void *)u32ExtendDispInfoAddr, sizeof(HVD_Extend_Display_Info));

    if ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC)
    {
        pInfo->u32EncodedFormatProfileBit       = _HVD_EX_HEVC_Profile_IdcMap2Bit(     stExtendDisplayInfo.u8ProfileIdc, stExtendDisplayInfo.constraint_set);
        pInfo->u32EncodedFormatProfileLevelBit  = _HVD_EX_HEVC_ProfileLevel_IdcMap2Bit(stExtendDisplayInfo.u8LevelIdc  , stExtendDisplayInfo.constraint_set);
        pInfo->u8BitsPerComponent               = ((pShm->u8BitDepth & 0xF0) >> 4);
    }
    else
    {
        pInfo->u32EncodedFormatProfileBit       = _HVD_EX_AVC_Profile_IdcMap2Bit(     stExtendDisplayInfo.u8ProfileIdc , stExtendDisplayInfo.constraint_set);
        pInfo->u32EncodedFormatProfileLevelBit  = _HVD_EX_AVC_ProfileLevel_IdcMap2Bit(stExtendDisplayInfo.u8LevelIdc   , stExtendDisplayInfo.constraint_set);
        pInfo->u8BitsPerComponent               = 0xFF;
    }
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetExtendDispInfo()
/// @brief \b Function \b Description:  Get video Extend Display information
/// @param -pinfo \b OUT : pointer to video display information.
/// @return -The result of command get display information
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_GetExtendDispInfo(MS_U32 u32Id, HVD_EX_ExtendDispInfo *pInfo)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;
    //MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    eRet = _HVD_EX_GetExtendDispInfo(u32Id, pInfo, TRUE);
    return eRet;
}
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetHDRRegSetAddr()
/// @brief \b Function \b Description:  get the HDR XC Reg setting address
/// @return - PHY address
//-----------------------------------------------------------------------------
MS_PHY MDrv_HVD_EX_GetHDRRegSetAddr(MS_U32 u32Id, MS_PHY u32FWBaseAddr)
{
    MS_PHY ePhyRegSetAddr = 0;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    ePhyRegSetAddr = u32FWBaseAddr + (MS_PHY)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_HDR_REG_SET_ADDR);

    return ePhyRegSetAddr;
}
...
/// @return - PHY address
//-----------------------------------------------------------------------------
MS_U32 MDrv_HVD_EX_GetHDRRegSetSize(MS_U32 u32Id)
{
    MS_U32 u32RegSetSize = 0;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    u32RegSetSize = (MS_U32)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_HDR_REG_SET_SIZE);

    return u32RegSetSize;
}
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetVideoFirstPts()
/// @brief \b Function \b Description:  Get the value of First PTS in previous input video data. unit: 90KHz
/// @return - PTS
//-----------------------------------------------------------------------------
MS_S64 MDrv_HVD_EX_GetVideoFirstPts(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_S64 s64PTS = 0;
    ...
    HAL_HVD_EX_ReadMemory();
    u32PtsH = pShm->u32FirstPts_H;
    u32PtsL = pShm->u32FirstPts_L;
    s64PTS = (((MS_S64)u32PtsH << 32) | u32PtsL);

    pShm->u32FirstPts_H = 0;
    pShm->u32FirstPts_L = 0;
    HAL_HVD_EX_FlushMemory();
...
//-----------------------------------------------------------------------------
MS_VIRT MDrv_HVD_EX_GetESWritePtr(MS_U32 u32Id)
{
#ifdef VDEC3
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl  = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    ...
    {
        MS_VIRT u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
        MS_VIRT u32Wptr = HAL_VPU_EX_GetESWritePtr(u32Id, u32VBBUAddr);

        if (pCtrl->bShareBBU)
        {
            if (u32Wptr != 0)
                u32Wptr = u32Wptr - (pCtrl->MemMap.u32BitstreamBufAddr - pCtrl->MemMap.u32TotalBitstreamBufAddr); // NStream will share the same ES buffer
        }

        if (u32Wptr & HVD_RV_BROKEN_BY_US_MASK)
            u32Wptr &= (~HVD_RV_BROKEN_BY_US_MASK);
...
//-----------------------------------------------------------------------------
MS_VIRT MDrv_HVD_EX_GetESReadPtr(MS_U32 u32Id)
{
#ifdef VDEC3
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl  = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    ...
    {
        MS_VIRT u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
        MS_VIRT u32Rptr = HAL_VPU_EX_GetESReadPtr(u32Id, u32VBBUAddr);

        if (pCtrl->bShareBBU)
        {
            if (u32Rptr != 0)
                u32Rptr = u32Rptr - (pCtrl->MemMap.u32BitstreamBufAddr - pCtrl->MemMap.u32TotalBitstreamBufAddr); // NStream will share the same ES buffer
        }

        if (u32Rptr & HVD_RV_BROKEN_BY_US_MASK)
            u32Rptr &= (~HVD_RV_BROKEN_BY_US_MASK);
...
HVD_EX_Result MDrv_HVD_EX_GetFrmInfo(MS_U32 u32Id, HVD_EX_GetFrmInfoType eType, HVD_EX_FrameInfo * pInfo)
{
    ...
    if (pFrmInfo != NULL)
    {
        MS_U32 u32Luma8MiuSel = (MS_U32)pFrmInfo->u2Luma0Miu;
        MS_U32 u32Luma2MiuSel = (MS_U32)pFrmInfo->u2Luma1Miu;
        MS_U32 u32Chroma8MiuSel = (MS_U32)pFrmInfo->u2Chroma0Miu;
        MS_U32 u32Chroma2MiuSel = (MS_U32)pFrmInfo->u2Chroma1Miu;

        pInfo->u32LumaAddr = (MS_PHY) (pFrmInfo->u32LumaAddr + _HVD_EX_GetMIUBase(u8DrvId, u32Luma8MiuSel));
        pInfo->u32ChromaAddr = (MS_PHY) (pFrmInfo->u32ChromaAddr + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma8MiuSel));
        pInfo->u32TimeStamp = pFrmInfo->u32TimeStamp;
        ...
        pInfo->u32LumaAddr_2bit = (MS_PHY) (pFrmInfo->u32LumaAddr_2bit + _HVD_EX_GetMIUBase(u8DrvId, u32Luma2MiuSel)) ;
        pInfo->u32ChromaAddr_2bit = (MS_PHY) (pFrmInfo->u32ChromaAddr_2bit + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma2MiuSel));
        ...
        MS_U32 frameDuration = (pInfo->u32ID_L & 0xFFFF);
        MS_U32 i = 0, u32FBcnt = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUFFER_NUM);
        if (u32FBcnt > 20)
            u32FBcnt = 20;
        else if (u32FBcnt < 1)
            u32FBcnt = 1;
        for (i = 1; i < u32FBcnt; i++)
            pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32DecTime[i - 1] = pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32DecTime[i];
        pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32DecTime[u32FBcnt - 1] = frameDuration;
        pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32NAvgDecTime = 0;
        for (i = 0; i < u32FBcnt; i++)
            pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32NAvgDecTime += pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32DecTime[i];
        pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32NAvgDecTime /= u32FBcnt;
#endif
        ...
        if(pFrmInfo_ext != NULL)
        {
            ...
            pInfo->u32LumaAddrI = (MS_PHY)(pFrmInfo_ext->u32LumaAddrExt[HVD_FRM_INFO_EXT_TYPE_INTERLACE]
                                           + _HVD_EX_GetMIUBase(u8DrvId, u32Luma8MiuSel));
            ...
            pInfo->u32ChromaAddrI = (MS_PHY)(pFrmInfo_ext->u32ChromaAddrExt[HVD_FRM_INFO_EXT_TYPE_INTERLACE]
                                             + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma8MiuSel));
            ...
            /* FIXME: force mfcodec Id=0 9((u32MFCodecInfo >> 8) & 0x1) */
            pInfo->u32MFCodecInfo = pInfo->u32MFCodecInfo & 0xFFFFFEFF;
            ...
            pInfo->u32MaxLuminance    = pFrmInfo_ext->maxLuminance;
            pInfo->u32MinLuminance    = pFrmInfo_ext->minLuminance;
            pInfo->u16Primaries[0][0] = pFrmInfo_ext->primaries[0][0];
            pInfo->u16Primaries[0][1] = pFrmInfo_ext->primaries[0][1];
            pInfo->u16Primaries[1][0] = pFrmInfo_ext->primaries[1][0];
            pInfo->u16Primaries[1][1] = pFrmInfo_ext->primaries[1][1];
            pInfo->u16Primaries[2][0] = pFrmInfo_ext->primaries[2][0];
            pInfo->u16Primaries[2][1] = pFrmInfo_ext->primaries[2][1];
            pInfo->u16WhitePoint[0]   = pFrmInfo_ext->whitePoint[0];
            pInfo->u16WhitePoint[1]   = pFrmInfo_ext->whitePoint[1];

            pInfo->u8Frm_Info_Ext_avail       = pFrmInfo_ext->Frm_Info_Ext_avail;
            pInfo->u8Colour_primaries         = pFrmInfo_ext->colour_primaries;
            pInfo->u8Transfer_characteristics = pFrmInfo_ext->transfer_characteristics;
            pInfo->u8Matrix_coefficients      = pFrmInfo_ext->matrix_coefficients;

            pInfo->u8DVMode = pFrmInfo_ext->u8DVMode;
            pInfo->u32DVMetadataAddr = (MS_PHY)(pFrmInfo_ext->u32DVMetadataAddr);
            if (pInfo->u8DVMode & 0x03) // we hide dolby vision infomation into Luma Interlace Address when dolby mode enable;
            {
                pInfo->u32LumaAddrI = pInfo->u8DVMode;
            }
            pInfo->u32DVDMSize = pFrmInfo_ext->u32DVDMSize;
            pInfo->u32DVCompSize = pFrmInfo_ext->u32DVCompSize;
            pInfo->u8CurrentIndex = pFrmInfo_ext->u8CurrentIndex;
            pInfo->u32HDRRegAddr = pFrmInfo_ext->u32DVRegAddr;
            pInfo->u32HDRRegSize = pFrmInfo_ext->u32DVRegSize;
            pInfo->u32HDRLutAddr = pFrmInfo_ext->u32DVLutAddr;
            pInfo->u32HDRLutSize = pFrmInfo_ext->u32DVLutSize;
            pInfo->bDMEnable = pFrmInfo_ext->bDMEnable;
            pInfo->bCompEnable = pFrmInfo_ext->bCompEnable;
            pInfo->u8ComplexityLevel = pFrmInfo_ext->u8ComplexityLevel;

            pInfo->u32ParWidth = pFrmInfo_ext->u32ParWidth;
            pInfo->u32ParHeight = pFrmInfo_ext->u32ParHeight;
            pInfo->u16CropRight = pFrmInfo_ext->u16CropRight;
            pInfo->u16CropLeft = pFrmInfo_ext->u16CropLeft;
            pInfo->u16CropBottom = pFrmInfo_ext->u16CropBottom;
            pInfo->u16CropTop = pFrmInfo_ext->u16CropTop;

            pInfo->u8TileMode         = pFrmInfo_ext->u8TileMode;
            pInfo->u16MIUBandwidth    = pFrmInfo_ext->u16MIUBandwidth;
            pInfo->u16Bitrate         = pFrmInfo_ext->u16Bitrate;
            pInfo->u8HTLBTableId      = pFrmInfo_ext->u8HTLBTableId;
            pInfo->u8HTLBEntriesSize  = pFrmInfo_ext->u8HTLBEntriesSize;
            pInfo->u32HTLBEntriesAddr = pFrmInfo_ext->u32HTLBEntriesAddr;

            pInfo->u16Width_subsample      = pFrmInfo_ext->u16Width_subsample;
            pInfo->u16Height_subsample     = pFrmInfo_ext->u16Height_subsample;
            pInfo->phyLumaAddr_subsample   = pFrmInfo_ext->u32LumaAddr_subsample;
            pInfo->phyChromaAddr_subsample = pFrmInfo_ext->u32ChromaAddr_subsample;
            pInfo->u16Pitch_subsample      = pFrmInfo_ext->u16Pitch_subsample;
            pInfo->u8TileMode_subsample    = pFrmInfo_ext->u8TileMode_subsample;
...
//-----------------------------------------------------------------------------
MS_VIRT MDrv_HVD_EX_GetUserData_Packet(MS_U32 u32Id, MS_U32 u32Idx, MS_U32 *u32Size)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U32 u32Ret = 0;
    MS_VIRT tmp = 0;
    ...
    *u32Size = 0;
    ...
    if (u32Idx >= tmp)
    {
        HVD_EX_MSG_ERR("input user data index(%tu) is larger than max index(%lu)\n", (ptrdiff_t)u32Idx, (unsigned long)tmp);
        return 0;
    }
    tmp = (MS_VIRT) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_USERDATA_IDX_TBL_ADDR);
    if (tmp == 0)
    {
    ...
    tmp = (MS_VIRT) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_USERDATA_PACKET_TBL_SIZE);
    if ((*pIdx) >= tmp)
    {
        HVD_EX_MSG_INF("HVD FW ERR: user data packet tbl ptr(%lu) is larger than max size(%lu)\n", (unsigned long) (*pIdx), (unsigned long)tmp);
        return 0;
    }
    tmp = (MS_VIRT) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_USERDATA_PACKET_TBL_ADDR);
    if (tmp == 0)
    {
    ...
    u32Ret = tmp;
    tmp = (MS_VIRT) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_USERDATA_PACKET_SIZE);
    if (tmp == 0)
    {
...
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_GenPattern(MS_U32 u32Id, HVD_EX_PatternType eType, MS_VIRT u32VAddr, MS_U32 * pu32Size)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    ...
    if (eType == E_HVD_EX_PATTERN_FLUSH)   // flush pattern
    {
        ...
        {
            HVD_EX_MSG_ERR("Flush Pattern must have at least %tu bytes, input:%tu\n"
                           , (ptrdiff_t)u32MinPatternSize, (ptrdiff_t)(*pu32Size));
...
#if defined(MSOS_TYPE_LINUX_KERNEL) && defined(__aarch64__)
        memset_io((void *) pDummyData, 0, *pu32Size);
#else
    ...
    else if (eType == E_HVD_EX_PATTERN_FILEEND)    // dummy pattern
    {
        ...
        if (*pu32Size < u32MinPatternSize)
        {
            HVD_EX_MSG_ERR("Dummy Pattern must have at least %tu bytes, input:%tu\n"
                           , (ptrdiff_t)u32MinPatternSize, (ptrdiff_t)(*pu32Size));
            *pu32Size = u32MinPatternSize;
            eRet = E_HVD_EX_RET_OUTOF_MEMORY;
            _DRV_HVD_EX_RET(u8DrvId, eRet);
...
#ifdef STELLAR
    HVD_EX_MSG_DBG("u8MiuSel:%x u32StartOffset:0x%x \n", u8MiuSel, (unsigned int)u32StartOffset);
#endif
    return u8MiuSel;
}

MS_BOOL MDrv_HVD_EX_GetPVRSeamlessInfo(MS_U32 u32Id, HVD_EX_PVR_Seamless_Info* param)
{
    param->u64PTS = (MS_U64) HAL_HVD_EX_GetData_EX(u32Id, E_HVD_GDATA_TS_SEAMLESS_TARGET_PTS);
    param->u32POC = (MS_U32) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_TS_SEAMLESS_TARGET_POC);
    param->u8FrameType = 0xFF;  // not support now
...
/// @return -the information of choosed type
//-----------------------------------------------------------------------------
MS_U64 MDrv_HVD_EX_GetDynamicScalingInfo(MS_U32 u32Id, HVD_EX_DynamicScalingInfo eType)
{
    MS_U32 u32Ret = 0;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U8 u8MiuSel = 0;
...
//-----------------------------------------------------------------------------
MS_VIRT MDrv_HVD_EX_GetData(MS_U32 u32Id, HVD_EX_GDataType eType)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_VIRT u32Ret = 0;
    ...
    switch (eType)
    {
        ...
        case E_HVD_EX_GDATA_TYPE_FW_CODEC_TYPE:
        {
            ...
            switch(HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_CODEC_TYPE))
            {
                ...
                case E_HVD_Codec_VP9:
                {
                    u32Ret = E_HVD_EX_VP9;
                    break;
                }
                case E_HVD_Codec_HEVC_DV:
                {
                    u32Ret = E_HVD_EX_HEVC_DV;
                    break;
                }
                case E_HVD_Codec_AVS2:
                {
                    u32Ret = E_HVD_EX_AVS2;
                    break;
                }
                default:
                {
        ...
        case E_HVD_EX_GDATA_TYPE_SEQ_CHANGE_INFO:
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_SEQ_CHANGE_INFO);
            break;
        }
        case E_HVD_EX_GDATA_TYPE_GET_NOT_SUPPORT_INFO:
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_GET_NOT_SUPPORT_INFO);
            break;
        }
        case E_HVD_EX_GDATA_TYPE_GET_MIN_TSP_DATA_SIZE :
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_GET_MIN_TSP_DATA_SIZE);
            break;
        }
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetDVInfo()
/// @brief \b Function \b Description:  set DV profile and level
/// @param -u32Arg \b IN : //[15:8]+[7:0] = (profile+level)
/// @return -the result of set DV profile and level
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetDVInfo(MS_U32 u32Id, MS_U32 u32Arg)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    ...
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DV_INFO, u32Arg);
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetVP9HDR10Info()
/// @brief \b Function \b Description:  set VP9 HDR10 Info
/// @param -VP9HDR10Info \b IN : // VP9 HDR10 Info
/// @return -the result of set VP9 HDR10 Info
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetVP9HDR10Info(MS_U32 u32Id, HVD_EX_Config_VP9HDR10 *stVP9HDR10Info)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    ...
    if (stVP9HDR10Info == NULL)
    {
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_VP9HDR10INFO, (MS_VIRT)stVP9HDR10Info);
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_Set_Slow_Sync()
/// @brief \b Function \b Description:  set slow sync
/// @param -u32Arg \b IN : //[15:8]+[7:0] = (repeat period + drop period)
/// @return -the result of set err tolerance
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_Set_Slow_Sync(MS_U32 u32Id, MS_U32 u32Arg)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_SLOW_SYNC, u32Arg));

    _DRV_HVD_EX_RET(u8DrvId, eRet);

}

MS_BOOL MDrv_HVD_EX_DecideIsEVDLiteFlag(MS_U32 u32Id)
{
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    return HAL_HVD_EX_DecideIsEVDLiteFlag(u32Id);
#else
    return FALSE;
#endif
}

void MDrv_HVD_EX_SetIsEVDLiteFlag(MS_U32 u32Id, MS_BOOL bIsEVDLite)
{
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    HAL_HVD_EX_SetIsEVDLiteFlag(u32Id, bIsEVDLite);
#endif
    return;
}

MS_BOOL MDrv_HVD_EX_GetIsEVDLiteFlag(MS_U32 u32Id)
{
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    return HAL_HVD_EX_GetIsEVDLiteFlag(u32Id);
#else
    return FALSE;
#endif
}

void MDrv_HVD_EX_SetShmbIsDriverEVDInit(MS_U32 u32Id)
{
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    HAL_HVD_EX_ReadMemory();
    pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_0] = HAL_VPU_EX_GetIsEVDInitFlag(E_VPU_EX_DECODER_EVD);//0 means EVD Hicodec
    pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_1] = HAL_VPU_EX_GetIsEVDInitFlag(E_VPU_EX_DECODER_EVD_LITE);//1 means EVD Lite
    //HVD_PRINT("[%s][%d] pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_0] = %d pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_1] = %d , taskid = %d\n",__FUNCTION__,__LINE__,pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_0],pShm->bIsDriverEVDInit[E_TYPE_EVD_ID_1],_HVD_EX_GetDrvId(u32Id));
    HAL_HVD_EX_FlushMemory();
#endif
    return;
}
...
#if defined(HAL_CHIP_SUPPORT_EVD_LITE)
static void MDrv_HVD_EX_SetHWDecIsr(MS_U32 u32Id, HWDEC_ISR_TYPE eHWDecIsr)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    pCtrl->HVDISRCtrl.eHWDecIsr = eHWDecIsr;
    return;
}

static MS_U8 MDrv_HVD_EX_GetShmEVDId(MS_U32 u32Id)
{
    MS_U8 u8EVDId;
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    HAL_HVD_EX_ReadMemory();
    u8EVDId = pShm->u8EVD_ID;
    HAL_HVD_EX_FlushMemory();
    //HVD_PRINT("[%s][%d] pShm->u8EVD_ID = %d , taskid = %d\n",__FUNCTION__,__LINE__,pShm->u8EVD_ID,_HVD_EX_GetDrvId(u32Id));
    return u8EVDId;
}
#endif

HVD_EX_Result MDrv_HVD_EX_UseEvdHicodecCallBack(MS_U32 u32Id)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    ...
    HAL_HVD_EX_SetIsEVDLiteFlag(u32Id, FALSE);
    MDrv_HVD_EX_SetHWDecIsr(u32Id, E_HWDEC_ISR_EVD);
    HAL_HVD_EX_SetHwRegBase(u32Id, pCtrl->InitParams.u32ModeFlag);
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId = HAL_HVD_EX_GetBBUId(u32Id);
    HAL_HVD_EX_InitHW(u32Id, E_VPU_EX_DECODER_EVD);
    HAL_VPU_EX_SetDecoderType(u32Id, E_VPU_EX_DECODER_EVD);
    HAL_VPU_EX_SetIsEVDInitFlag(E_VPU_EX_DECODER_EVD, TRUE);
    MDrv_HVD_EX_SetShmbIsDriverEVDInit(u32Id);

    if (MDrv_HVD_EX_GetShmEVDId(u32Id) == 1)
    {
        HVD_PRINT("[%s][%d] driver evd hicodec init is too late , recover evd lite. u8DrvId = %d \n", __FUNCTION__, __LINE__, u8DrvId);
        ...
        HAL_HVD_EX_SetIsEVDLiteFlag(u32Id, TRUE);
        MDrv_HVD_EX_SetHWDecIsr(u32Id, E_HWDEC_ISR_EVD_LITE);
        HAL_HVD_EX_SetHwRegBase(u32Id, pCtrl->InitParams.u32ModeFlag);
        pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId = HAL_HVD_EX_GetBBUId(u32Id);
        HAL_HVD_EX_InitHW(u32Id, E_VPU_EX_DECODER_EVD_LITE);
        HAL_VPU_EX_SetDecoderType(u32Id, E_VPU_EX_DECODER_EVD_LITE);
    }
    else
    {
        HVD_PRINT("[%s][%d] driver evd hicodec init is in time. u8DrvId = %d\n", __FUNCTION__, __LINE__, u8DrvId);
    }
#endif
    eRet = E_HVD_EX_OK;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
...
HVD_EX_Result MDrv_HVD_EX_UseEvdLiteCallBack(MS_U32 u32Id)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    ...
    HAL_HVD_EX_InitHW(u32Id, E_VPU_EX_DECODER_EVD_LITE);
    HAL_VPU_EX_SetDecoderType(u32Id, E_VPU_EX_DECODER_EVD_LITE);
    HAL_VPU_EX_SetIsEVDInitFlag(E_VPU_EX_DECODER_EVD_LITE, TRUE);
    MDrv_HVD_EX_SetShmbIsDriverEVDInit(u32Id);

    if (MDrv_HVD_EX_GetShmEVDId(u32Id) == 0)
    {
        HVD_PRINT("[%s][%d] driver evd lite init is too late , recover evd hicodec. u8DrvId = %d\n", __FUNCTION__, __LINE__, u8DrvId);
    ...
    {
        HVD_PRINT("[%s][%d] driver evd lite init is in time. u8DrvId = %d\n", __FUNCTION__, __LINE__, u8DrvId);
    }
#endif
    eRet = E_HVD_EX_OK;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
...
MS_BOOL MDrv_HVD_EX_GetCaps(HVD_EX_Codec u32Type)
{
    ...
    if((E_HVD_EX_HEVC == u32Type) || (E_HVD_EX_HEVC_DV == u32Type))
#if SUPPORT_EVD
        return TRUE;
#else
        return FALSE;
...
#if defined(SUPPORT_AVS2) && defined(VDEC3)
    if (E_HVD_EX_AVS2 == u32Type)
        return TRUE;
#endif

#if ( HVD_HW_VERSION == HVD_HW_HVD)
    verID = HAL_HVD_EX_GetHWVersionID();
    verID = verID >> 12;
    switch (u32Type)
    {
        case E_HVD_EX_AVC:
...
HVD_EX_Result MDrv_HVD_EX_IsAlive(MS_U32 u32Id)
{
    ...
    if(((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_MAIN_MASK) == HVD_INIT_MAIN_FILE_RAW) ||
            ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_MAIN_MASK) == HVD_INIT_MAIN_FILE_TS))
    {
        if(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].LivingStatus.u32LastAliveTime == 0 || ((MsOS_GetSystemTime() - pHVDDrvContext->gHVDCtrl_EX[u8DrvId].LivingStatus.u32LastAliveTime) < 500))
        {
            if(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].LivingStatus.u32LastAliveTime == 0 )
                pHVDDrvContext->gHVDCtrl_EX[u8DrvId].LivingStatus.u32LastAliveTime = MsOS_GetSystemTime();
            return E_HVD_EX_OK;
        }
    }
...
HVD_EX_Result MDrv_HVD_EX_SetOutputFRCMode(MS_U32 u32Id, MS_U8 u8FrameRate, MS_U8 u8Interlace)
{
    ...
    if ((u8Interlace != 0) && (u8Interlace != 1) && (u8Interlace != 2))
    {
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
...
HVD_EX_Result MDrv_HVD_EX_AddRef(MS_U32 u32Id, MS_U32 u32FrmIdx)
{
    HVD_EX_MSG_TRACE();

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DISPQ_ADD_REF, u32FrmIdx);
...
MS_BOOL MDrv_HVD_SetSTCMode(MS_U32 u32Id, MS_U32 u32STCIndex)
{
    return HAL_VPU_EX_SetSTCMode(u32Id, u32STCIndex);
}

MS_BOOL MDrv_HVD_SetDecodeMode(HVD_EX_DecModCfg *pstCfg)
{
    MS_U8 i;
    VPU_EX_DecModCfg stVpuDecModCfg;

    stVpuDecModCfg.u8DecMod   = pstCfg->u8DecMod;
    stVpuDecModCfg.u8CodecCnt = pstCfg->u8CodecCnt;
    for (i = 0; ((i < pstCfg->u8CodecCnt) && (i < HVD_MAX_DEC_NUM)); i++)
    {
        stVpuDecModCfg.u8CodecType[i] = pstCfg->u8CodecType[i];
    }
    stVpuDecModCfg.u8ArgSize  = pstCfg->u8ArgSize;
    stVpuDecModCfg.u32Arg     = pstCfg->u32Arg;

    return HAL_VPU_EX_SetDecodeMode(&stVpuDecModCfg);
}
...
HVD_EX_Result MDrv_HVD_EX_GetUserDataInfo(MS_U32 u32Id, HVD_EX_UserData_Info* pUsrInfo)
{
    ...
    pUsrInfo->u32DataBuf = (MS_VIRT)pHVD_User_Data->buf;
...
MS_BOOL MDrv_HVD_EX_GetUsrDataIsAvailable(MS_U32 u32Id)
{
    ...
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_BOOL bRet = FALSE;
    MS_U32 u32UsrDataIdxWptr;

    HVD_EX_MSG_TRACE();
    _DRV_HVD_EX_Entry(u8DrvId);
    u32UsrDataIdxWptr = 0xFFFFFFFF;
    u32UsrDataIdxWptr = MDrv_HVD_EX_GetUserData_Wptr(u32Id);
...
HVD_EX_Result MDrv_HVD_EX_SetDTVUserDataMode(MS_U32 u32Id, MS_U8 u8UserDataMode)
{
    ...
    else
    {
        ...
        VPRINTF("Debug Mode for set user data mode \n");
    }
...
HVD_EX_Result MDrv_HVD_EX_GetFrmPackingArrSEI(MS_U32 u32Id, HVD_EX_FrmPackingSEI *pFrmPacking)
{
    ...
    u32CodeVaddr = MS_PA2KSEG1((MS_PHY)(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr));
    if(u32CodeVaddr == 0x0)
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    ...
    u32FPAFrameAddr = MS_PA2KSEG1((MS_PHY)(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr + u32FPAFrameAddr)); // change to virtual address
    ...
    if(pFrmPacking != NULL)
    {
        if(pFrmPackingSrc->bvaild == TRUE)
        {
            ...
            pFrmPacking->u32payload = (MS_VIRT)pFrmPackingSrc->u8payload;
...
HVD_EX_Result MDrv_HVD_EX_GetDisplayColourVolumeArrSEI(MS_U32 u32Id, HVD_EX_DisplayColourVolumeSEI *pDisplayColourVolume)
{
    ...
    u32DCVFrameAddr = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DISPLAY_COLOUR_VOLUME_SEI_DATA);//get data
    ...
    {
        pDisplayColourVolume->bUsed                           = pDisplayColourVolumeSrc->bUsed;
...
HVD_EX_Result MDrv_HVD_EX_GetContentLightLevelInfoSEI(MS_U32 u32Id, HVD_EX_ContentLightLevelInfoSEI *pContentLightLevel)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;
    MS_VIRT u32CLLIAddr = 0 ;

    HVD_EX_ContentLightLevelInfoSEI *pContentLightLevelInfoSrc = NULL;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    u32CLLIAddr = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_CONTENT_LIGHT_LEVEL_INFO);//get data
    ...
    u32CLLIAddr = MS_PA2KSEG1((MS_U32)(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr + u32CLLIAddr)); // change to virtual address
    pContentLightLevelInfoSrc = (HVD_EX_ContentLightLevelInfoSEI *)u32CLLIAddr;
    if(pContentLightLevel != NULL)
    {
        pContentLightLevel->bUsed                     = pContentLightLevelInfoSrc->bUsed;
        pContentLightLevel->ContentLightLevelEnabled  = pContentLightLevelInfoSrc->ContentLightLevelEnabled;
        pContentLightLevel->maxContentLightLevel      = pContentLightLevelInfoSrc->maxContentLightLevel;
        pContentLightLevel->maxPicAverageLightLevel   = pContentLightLevelInfoSrc->maxPicAverageLightLevel;
        eRet = E_HVD_EX_OK;
    }
    else
    {
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetIPBVCounter()
/// @brief \b Function \b Description:  Get I/P/B frame number
/// @return - none
//-----------------------------------------------------------------------------
void MDrv_HVD_GetIPBVCounter(MS_U32 u32Id, void* pCounter)
{
    HAL_HVD_GetIPBV(u32Id, pCounter);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetIPBVCounter()
/// @brief \b Function \b Description:  Get fw time info
/// @return - none
//-----------------------------------------------------------------------------
void MDrv_HVD_GetPerfInfo(MS_U32 u32Id, void* pPerf)
{
    HAL_HVD_GetPerf(u32Id, pPerf);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetIPBVCounter()
/// @brief \b Function \b Description:  Get max frame buffer number
/// @return - max frame buffer number
//-----------------------------------------------------------------------------
MS_U32 MDrv_HVD_GetFBNum(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUFFER_NUM);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_GetBitDepth()
/// @brief \b Function \b Description:  Get stream bit dipth information
/// @return - bit depth info
//-----------------------------------------------------------------------------
MS_U32 MDrv_HVD_GetBitDepth(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_BITDEPTH);
}
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_GetTFF()
/// @brief \b Function \b Description:  Get interlace stream top or bottom first information
/// @return - TRUE : Top first, FALSE : Buttom first
//-----------------------------------------------------------------------------
MS_BOOL MDrv_HVD_GetTFF(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_TOP_FIRST);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_GetBlueScreen()
/// @brief \b Function \b Description:  Get Blue screan information
/// @return - TRUE : ENABLE, FALSE : DISABLE
//-----------------------------------------------------------------------------
MS_U32 MDrv_HVD_GetBlueScreen(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_BLUE_SCREEN);
}

MS_U32 MDrv_HVD_GetDVIsSingleLayer(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);


    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DV_IS_SINGLE_LAYER);
}

MS_U32 MDrv_HVD_GetVSyncTime(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);


    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_AVG_VSYNC_TIME);
}

MS_U32 MDrv_HVD_GetVSyncDuration(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId, FALSE);


    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_AVG_VSYNC_DURATION);
}

HVD_EX_Result MDrv_HVD_GetHDRShmAddr(MS_U32 u32Id, MS_U32 *u32HDRShmOff, MS_U32 *u32HDRShmSize, MS_U8 *u8MIU)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    _phy_to_miu_offset(*u8MIU, *u32HDRShmOff, pHVDDrvContext->gHVDCtrl_EX[u8DrvId].phyHDRShmAddr);
    *u32HDRShmSize = pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32HDRShmSize;

    eRet = E_HVD_EX_OK;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

MS_U32 MDrv_HVD_GetNAvgDecTime(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    _DRV_HVD_Inited(u8DrvId, FALSE);

    return pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32NAvgDecTime;
}

MS_U32 MDrv_HVD_GetAvgSliceNum(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_AVG_SLICE_NUM);
}

MS_U32 MDrv_HVD_GetBuffMIUSel(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    _DRV_HVD_Inited(u8DrvId, FALSE);

    return HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);
}
...
#else
    UNUSED(eClockSpeed);
    VPRINTF("This chip does not support hvd clock presetting~\n");
    return E_HVD_EX_RET_UNSUPPORTED;
#endif
}
...
#else
    UNUSED(eClockSpeed);
    VPRINTF("This chip does not support vpu clock presetting~\n");
    return E_HVD_EX_RET_UNSUPPORTED;
#endif
}
...
{
#if (defined(CHIP_EDISON) || defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3)) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MUJI) || defined(CHIP_CLIPPERS) || defined(CHIP_MONET) || defined(CHIP_MANHATTAN) || defined(CHIP_MESSI) || defined(CHIP_MASERATI) || defined(CHIP_MAXIM) || defined(CHIP_K6) || defined(CHIP_I2) || defined(CHIP_K6LITE) || defined(CHIP_K7U) || defined(CHIP_M7621)
...
#define SIZE_3K 0xC00
    ...
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    MS_PHY u32DSBufAddr = pExternalBuf->u32DSBufAddr;
...
#if defined(CHIP_MONACO) || defined(CHIP_MUJI) || defined(CHIP_MONET) || defined(CHIP_MANHATTAN) || defined(CHIP_MESSI) || defined(CHIP_MASERATI) || defined(CHIP_K6) || defined(CHIP_I2) || defined(CHIP_K6LITE) || defined(CHIP_K7U)
...
MS_U32 MDrv_HVD_EX_SetCMAReleaseStatus(MS_U32 u32Id, HVD_EX_CMA_Release_Status eCMAReleaseStatus)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_U32 u32Ret = FALSE;
    HVD_CMA_Release_Status eStatus;

    switch (eCMAReleaseStatus)
    {
        case E_HVD_EX_CMA_RELEASE_NONE:
            eStatus = E_HVD_CMA_RELEASE_NONE;
            break;
        case E_HVD_EX_CMA_RELEASE_WAITING:
            eStatus = E_HVD_CMA_RELEASE_WAITING;
            break;
        case E_HVD_EX_CMA_RELEASE_DONE:
            eStatus = E_HVD_CMA_RELEASE_DONE;
            break;
        default:
            VPRINTF("ERROR! Unhandled CMA release status %d\n", eCMAReleaseStatus);
            return u32Ret;
    }

    if (pShm != NULL)
    {
        pShm->u8CMAReleaseStatus = eStatus;
        MsOS_FlushMemory();
        u32Ret = TRUE;
    }
    return u32Ret;
...
HVD_EX_CMA_Release_Status MDrv_HVD_EX_GetCMAReleaseStatus(MS_U32 u32Id)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    HVD_CMA_Release_Status eStatus;
    ...
    if (pShm != NULL)
    {
        eStatus = (HVD_CMA_Release_Status)pShm->u8CMAReleaseStatus;
        switch (eStatus)
        {
            case E_HVD_CMA_RELEASE_NONE:
                return E_HVD_EX_CMA_RELEASE_NONE;
            case E_HVD_CMA_RELEASE_WAITING:
                return E_HVD_EX_CMA_RELEASE_WAITING;
            case E_HVD_CMA_RELEASE_DONE:
                return E_HVD_EX_CMA_RELEASE_DONE;
            default:
                VPRINTF("ERROR! Unhandled CMA release status %d\n", eStatus);
        }
    }
...
MS_U32 MDrv_HVD_EX_SetCMAAllocateStatus(MS_U32 u32Id, HVD_EX_CMA_Allocation_Status eCMAAllocStatus)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_U32 u32Ret = FALSE;
    HVD_CMA_Allocation_Status eStatus;

    switch (eCMAAllocStatus)
    {
        case E_HVD_EX_CMA_ALLOCATION_NONE:
            eStatus = E_HVD_CMA_ALLOCATION_NONE;
            break;
        case E_HVD_EX_CMA_ALLOCATION_WAITING:
            eStatus = E_HVD_CMA_ALLOCATION_WAITING;
            break;
        case E_HVD_EX_CMA_ALLOCATION_DONE:
            eStatus = E_HVD_CMA_ALLOCATION_DONE;
            break;
        case E_HVD_EX_CMA_ALLOCATION_FAILED:
            eStatus = E_HVD_CMA_ALLOCATION_FAILED;
            break;
        default:
            VPRINTF("ERROR! Unhandled CMA allocation status %d\n", eCMAAllocStatus);
            return FALSE;
    }

    if (pShm != NULL)
    {
        pShm->u8CMAAllocationStatus = eStatus;
        MsOS_FlushMemory();
        u32Ret = TRUE;
    }
    return u32Ret;
...
HVD_EX_CMA_Allocation_Status MDrv_HVD_EX_GetCMAAllocateStatus(MS_U32 u32Id)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    HVD_CMA_Allocation_Status eStatus;
    ...
    if (pShm != NULL)
    {
        eStatus = (HVD_CMA_Allocation_Status)pShm->u8CMAAllocationStatus;
        switch (eStatus)
        {
            case E_HVD_CMA_ALLOCATION_NONE:
                return E_HVD_EX_CMA_ALLOCATION_NONE;
            case E_HVD_CMA_ALLOCATION_DONE:
                return E_HVD_EX_CMA_ALLOCATION_DONE;
            case E_HVD_CMA_ALLOCATION_WAITING:
                return E_HVD_EX_CMA_ALLOCATION_WAITING;
            case E_HVD_CMA_ALLOCATION_FAILED:
                return E_HVD_EX_CMA_ALLOCATION_FAILED;
            default:
                VPRINTF("ERROR! Unhandled CMA allocation status %d\n", eStatus);
        }
    }
...
MS_BOOL MDrv_HVD_EX_GetCMAAllocationInfo(MS_U32 u32Id, MS_U8 *Miu_sel, MS_U64 *offset, MS_SIZE *length)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_BOOL u32Ret = FALSE;
    MS_U32 i = 0;
    if (pShm != NULL)
    {
        for (i = 0; i < HVD_CMA_SUPPORT_MAX_MIU_NUM; i++)
        {
            HAL_HVD_EX_ReadMemory();
            if (pShm->u32AllocateCMABuffSize[i] > 0)
            {
                *offset = (MS_U64)pShm->u32AllocateCMABuffAddr[i];
                *length = (MS_SIZE)pShm->u32AllocateCMABuffSize[i];
                *Miu_sel = i;

                pShm->u32AllocateCMABuffAddr[i] = 0;
                pShm->u32AllocateCMABuffSize[i] = 0;
                HAL_HVD_EX_FlushMemory();
                u32Ret = TRUE;
                break;
            }
...
MS_BOOL MDrv_HVD_EX_GetCMAReleaseInfo(MS_U32 u32Id, MS_U8 *Miu_sel, MS_U8 *block_sel, MS_U64 *offset, MS_SIZE *length)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_U32 u32Ret = FALSE;
    MS_U8 i = 0, j = 0;
    if (pShm != NULL)
    {
        for (i = 0; i < HVD_CMA_SUPPORT_MAX_MIU_NUM; i++)
        {
            for (j = 0; j < HVD_CMA_SUPPORT_MAX_MIU_NUM; j++)
            {
                HAL_HVD_EX_ReadMemory();
                if (pShm->pending_release_queue[i][j].u32HVD_PENDING_RELEASE_SIZE > 0 && pShm->pending_release_queue[i][j].u32HVD_COLLISION_NUM == 0)
                {
                    *offset = (MS_U64)pShm->pending_release_queue[i][j].u32HVD_PENDING_RELEASE_ST_ADDR;
                    *length = (MS_SIZE)pShm->pending_release_queue[i][j].u32HVD_PENDING_RELEASE_SIZE;
                    *Miu_sel = i;
                    *block_sel = j;

                    pShm->pending_release_queue[i][j].u32HVD_PENDING_RELEASE_ST_ADDR = 0;
                    pShm->pending_release_queue[i][j].u32HVD_PENDING_RELEASE_SIZE = 0;
//                    HVD_EX_MSG_DBG("offset=0x%llx addr=0x%x i=%d j=%d\n", (unsigned long long int)(*offset),
//                        (unsigned int)(*length), (unsigned int)i, (unsigned int)j);
                    HAL_HVD_EX_FlushMemory();
                    return TRUE;
                }
            }
...
MS_BOOL MDrv_HVD_EX_SetCMAAllocateData(MS_U32 u32Id, MS_U8 miu_sel, MS_PHY FWBaseAddr, MS_PHY offset, MS_U32 length)
{
    volatile HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    ...
    HVD_EX_MSG_DBG("[%s] miu=%d Size=0x%tx Addr=0x%llx\n", __FUNCTION__, miu_sel, (ptrdiff_t)length, (unsigned long long int)offset);
    HAL_HVD_EX_ReadMemory();
    UNUSED(FWBaseAddr);
    if (pShm != NULL)
    {
        _phy_to_miu_offset(u8FrmMiuSel, u32FrmStartOffset, offset);
        if (miu_sel == 0)
        {
            pShm->u32FrameBufAddr = (MS_U32)u32FrmStartOffset;
            pShm->u32FrameBufSize = length;
        }
        else
        {
            pShm->u32FrameBuf2Addr = (MS_U32)u32FrmStartOffset;
            pShm->u32FrameBuf2Size = length;
        }
        bRet = TRUE;
        HAL_HVD_EX_FlushMemory();
    }
#ifdef STELLAR
    HVD_EX_MSG_DBG("[%s] u8FrmMiuSel=%d u32FrmStartOffset=0x%x \n", __FUNCTION__, u8FrmMiuSel, (unsigned int) u32FrmStartOffset);
#endif
    return bRet;
}

MS_BOOL MDrv_HVD_EX_WaitCMAStatusDone(MS_U32 u32Id, MS_BOOL bCMAInitPool0, MS_BOOL bCMAInitPool1)
{
    MS_BOOL bRet = TRUE;
    MS_U32 u32Timer = 0;
    if (bCMAInitPool0 == FALSE && bCMAInitPool1 == FALSE)
        bRet = TRUE;
    else
    {
        do
        {
            ...
            {
                bRet = FALSE;
                HVD_EX_MSG_ERR("HVD CMA wait status done timeout!!!\n");
                break;
            }
            else
            {
        ...
        while (MDrv_HVD_EX_GetCMAAllocateStatus(u32Id) == E_HVD_EX_CMA_ALLOCATION_WAITING || MDrv_HVD_EX_GetCMAReleaseStatus(u32Id) == E_HVD_EX_CMA_RELEASE_WAITING);
    }
...
static void _HVD_EX_CC_Parse_CC_data(MS_U32 u32Id, MS_U8 u8CCMode, MS_U8 **pSrc, MS_U8 *pDst608, MS_U32 *pu32Dst608Len, MS_U8 *pDst708, MS_U32 *pu32Dst708Len, HVD_CC_608EnhanceInfo *pEhInfo608)
{
    ...
    for (i = 0; i < u32CC_Count; i++)
    {
        ...
        if ((u8CC_Type == 0) || (u8CC_Type == 1))
        {
            ...
            if(pCCInfo->b608InfoEnhance)
            {
                ...
                {
                    HVD_EX_MSG_ERR("u32Dst608Len+16: %td is larger than MAX_608_CC_LEN !! \n", (ptrdiff_t)((*pu32Dst608Len) + 16));
                    return;
                }
            }
            else
            {
                ...
                {
                    HVD_EX_MSG_ERR("u32Dst608Len+4: %td is larger than MAX_608_CC_LEN !! \n", (ptrdiff_t)((*pu32Dst608Len) + 4));
                    return;
                }
            }
        ...
        else // ATSC_CC 708
        {
            ...
            {
                HVD_EX_MSG_ERR(" u32Dst708Len+4: %td is larger than MAX_708_CC_LEN !! \n", (ptrdiff_t)((*pu32Dst708Len) + 4));
                return;
            }
    ...
    if(u8CCMode & (CC_NTSC1 | CC_NTSC2))
    {
        if (pCCInfo->b608InfoEnhance == 0)
        {
            ...
            {
                HVD_EX_MSG_ERR("u32Dst608Len:%td + u32Pad: %td is larger than MAX_608_CC_LEN !! \n", (ptrdiff_t)(*pu32Dst608Len), (ptrdiff_t)u32Pad);
                return;
            }
            for(i = 0; i < u32Pad; i++)
                pDst608[(*pu32Dst608Len)++] = 0xff;
...
static MS_BOOL _HVD_EX_CC_Copy_CCdata_To_RB(MS_U32 u32Id, MS_U8 u8CC608, MS_U8 *pSrcBuf, MS_U32 u32SrcLen)
{
    ...
    {
        HVD_EX_MSG_ERR("overflow !!!, u32RingBufVacancy: %td, u32AlignLen: %td\n", (ptrdiff_t)pCCInfo->u32RingBufVacancy[u8CC608], (ptrdiff_t)u32AlignCCLen);
        return TRUE;
    }
...
static MS_BOOL _HVD_EX_CC_Add_PaddingData_To_608(MS_U32 u32Id, MS_U8 *pSrcBuf, MS_U32 u32SrcLen)
{
    ...
    {
        HVD_EX_MSG_ERR("u32AlignCCLen:%td > MAX_608_CC_LEN", (ptrdiff_t)u32AlignCCLen);
        return TRUE;
    }
...
static MS_BOOL _HVD_EX_CC_Add_PaddingData_To_708(MS_U32 u32Id, MS_U8 *pSrcBuf, MS_U32 u32SrcLen)
{
    ...
    MS_VIRT *u32RingBufWAddr = (MS_VIRT *)(HVD_PA2VA(pCCInfo->u32RingBufWPAddr[0]));
    ...
    for(j = 0; j < u32SrcLen; j += 4)
    {
        ...
        if(u8CC_Type == 3) //padd_out();
        {
            ...
            pCCInfo->u32PktHdrAddr708 = (MS_VIRT)(u32RingBufWAddr);
        }
...
///   - # FALSE for CC708 parser
///////////////////////////////////////////////////////////////////////////////
HVD_EX_Result MDrv_HVD_EX_CC_Set_RB_StartAddr(MS_U32 u32Id, MS_PHY u32StartPAddress, MS_U8 u8CC608)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
...
#ifdef STELLAR
    HVD_EX_MSG_DBG("u8MiuSel:%d  u32StartOffset:0x%x\n", u8MiuSel, (unsigned int)u32StartOffset);
#endif
    ...
    HVD_EX_MSG_DBG("u32StartPAddress: 0x%lx ,u8CC608: 0x%x, pCCInfo->u8BufMiuSel[u8CC608]: %d, pCtrl->MemMap.u32MIU1BaseAddr: 0x%lx, pCtrl->MemMap.u32MIU2BaseAddr: 0x%lx\n",
                   (unsigned long)u32StartPAddress , u8CC608, pCCInfo->u8BufMiuSel[u8CC608], (unsigned long)pCtrl->MemMap.u32MIU1BaseAddr, (unsigned long)pCtrl->MemMap.u32MIU2BaseAddr);
...
HVD_EX_Result MDrv_HVD_EX_CC_Adv_RB_ReadAddr(MS_U32 u32Id, MS_U32 u32EachPacketSize, MS_U8 u8CC608)
{
    ...
    {
        HVD_EX_MSG_ERR("MDrv_HVD_CC_Adv_RB_ReadAddr fail!, no enough data!, vacancy: %td, u32ParsedDataSize: %td, u32RingBufLen:%td, u32RingBufRPAddr: 0x%lx, u32RingBufWPAddr: 0x%lx\n",
                       (ptrdiff_t)pCCInfo->u32RingBufVacancy[u8CC608], (ptrdiff_t)u32ParsedDataSize, (ptrdiff_t)pCCInfo->u32RingBufLen[u8CC608], (unsigned long)pCCInfo->u32RingBufRPAddr[u8CC608], (unsigned long)pCCInfo->u32RingBufWPAddr[u8CC608]);
        return E_HVD_EX_FAIL;
    }
...
MS_U32 MDrv_HVD_EX_CC_Get_RB_WriteAddr(MS_U32 u32Id, MS_U8 u8CC608)
{
    ...
    {
        // User Data Buffer Empty
        HVD_EX_MSG_DBG("RIdx(%td) == WIdx(%td) , empty !!!\n", (ptrdiff_t)pCCInfo->u32FWUsrDataRIdx, (ptrdiff_t)u32UsrDataIdxWptr);
        u32PAddr = pCCInfo->u32RingBufWPAddr[u8CC608];
    ...
    HVD_EX_MSG_DBG("u32FwParseCnt(%td)  !!!,  pCCInfo->u32RingBufWPAddr[%d]: 0x%lx\n", (ptrdiff_t)u32FwParseCnt, u8CC608, (unsigned long)pCCInfo->u32RingBufWPAddr[u8CC608] );

    pCC608buf = &(pCCInfo->u8CC608buf[0]);
    pCC708buf = &(pCCInfo->u8CC708buf[0]);
...
HVD_EX_Result MDrv_HVD_EX_SetExternal_CC608_Buffer(MS_U32 u32Id, MS_PHY u32Addr, MS_U32 u32Len)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    ...
    MS_U32 *u32ReadPtr;
    MS_U32 *u32WritePtr;
    MS_U32 *u32LatchOverflow;
    MS_U32 *u32FWBinaryAddr;
    ...
    u32ReadPtr = (MS_U32 *)(HVD_PA2VA(u32Addr));
    u32WritePtr = (MS_U32 *)(HVD_PA2VA(u32Addr + 4));
    u32LatchOverflow = (MS_U32 *)(HVD_PA2VA(u32Addr + 8));
    u32FWBinaryAddr = (MS_U32 *)(HVD_PA2VA(u32Addr + 12));
...
HVD_EX_Result MDrv_HVD_EX_SetExternal_CC708_Buffer(MS_U32 u32Id, MS_PHY u32Addr, MS_U32 u32Len)
{
    ...
    u8ResvAddr = (MS_U8 *)((HVD_PA2VA(u32Addr)) + EXT_CC_INFO_LENGTH);
...
HVD_EX_Result MDrv_HVD_EX_IapGnBufShareBWMode(MS_U32 u32Id, MS_BOOL bEnable, MS_PHY u32IapGnBufAddr, MS_U32 u32IapGnBufSize)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_FRC_OnlyShowTopField()
/// @brief \b Function \b Description: enable/disable only show top filed for FRC mode
/// @param -bEnable \b IN :enable/disable
/// @return -E_HVD_EX_OK for success; E_HVD_EX_FAIL for failure.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_FRC_OnlyShowTopField(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRC_ONLY_SHOW_TOP_FIELD, bEnable);

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_DisableEsFullStop()
/// @brief \b Function \b Description: enable/disable es full stop mode
/// @param -bDisable \b IN :enable/disable
/// @return -E_HVD_EX_OK for success; E_HVD_EX_FAIL for failure.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_DisableEsFullStop(MS_U32 u32Id, MS_BOOL bDisable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    if(bDisable)
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ES_FULL_STOP, FALSE);
    else
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ES_FULL_STOP, TRUE);
...
HVD_EX_Result MDrv_HVD_EX_PreSetForce8BitMode(MS_U32 u32Id, MS_BOOL bForce8BitMode)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bForce8BitMode = bForce8BitMode;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PreSetDVSingleLayerMode(MS_U32 u32Id, MS_BOOL bEnableDVSingleLayerMode)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bDVSingleLayerMode = bEnableDVSingleLayerMode;
    HVD_EX_MSG_DBG("MDrv_HVD_EX_PreSetDVSingleLayerMode 0x%x\n", bEnableDVSingleLayerMode);
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PreSetVdecFeature(MS_U32 u32Id, MS_U32 eVdecFeature)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].eVdecFeature = eVdecFeature;
    HVD_EX_MSG_DBG("MDrv_HVD_EX_PreSetVdecFeature 0x%tx\n", (ptrdiff_t)eVdecFeature);
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PreSetNDSMode(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    if(bEnable)
    {
        pHVDDrvContext->gHVDPreCtrl[u8DrvId].u32CustomerMode |= HVD_EX_CUSTOMER_ENABLE_NDS;
        eRet = HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_NDS_AVC_FAST_FLUSH, 1);
    }

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PreSetDynamicCMA(MS_U32 u32Id, MS_BOOL bDynamicCMAMode)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bEnableDynamicCMA = bDynamicCMAMode;
    HVD_EX_MSG_DBG("MDrv_HVD_EX_PreSetDynamicCMA 0x%x\n", bDynamicCMAMode);
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}
...
HVD_EX_Result MDrv_HVD_EX_PreConnectInputTsp(MS_U32 u32Id, MS_BOOL bEnable, HVD_EX_INPUT_TSP eInputTsp)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;

    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].stPreConnectInputTsp.bEnable = bEnable;
    pHVDDrvContext->gHVDPreCtrl[u8DrvId].stPreConnectInputTsp.u8InputTsp = eInputTsp;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PUSI_Control(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    if(bEnable) //enable
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PUSI_CONTROL, TRUE);
    else  // disable
        eRet = (HVD_EX_Result) HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PUSI_CONTROL, FALSE);
...
HVD_EX_Result MDrv_HVD_EX_PVRTimeShiftSeamlessMode(MS_U32 u32Id, MS_U8 u8Arg)
{
    ...
    if ((u8Arg == 1) || (u8Arg == 2))  //u8Arg=1 needs about 50ms, u8Arg=2 needs about 30ms
    {
        ...
        HVD_EX_MSG_DBG("TS Seamless. Arg=%d, polling status start at %td \n", u8Arg, (ptrdiff_t)HVD_GetSysTime_ms());

        while (1)
        {
            if (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_TS_SEAMLESS_STATUS) & u32Status)
            {
                HVD_EX_MSG_DBG("TS Seamless status change done. Arg=%d. Status=0x%tx at %td \n", u8Arg, (ptrdiff_t)u32Status, (ptrdiff_t)HVD_GetSysTime_ms());
                break;
            }
            ...
            if (u32Times > 500)
            {
                HVD_EX_MSG_ERR("Timeout!!! TS Seamless status change is not finished. Arg=%d. Status=0x%tx at %td \n", u8Arg, (ptrdiff_t)u32Status, (ptrdiff_t)HVD_GetSysTime_ms());
                eRet = E_HVD_EX_FAIL;
                break;
            }
        }
    ...
    if (u8Arg > 0)
    {
        pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag = (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & ~HVD_INIT_MAIN_MASK) | HVD_INIT_MAIN_FILE_TS;
    }
...
HVD_EX_Result MDrv_HVD_EX_GetDVSupportProfiles(MS_U32 *pu32DVSupportProfiles)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;

    if (pu32DVSupportProfiles == NULL)
    {
        //E_HVD_EX_FAIL
    }
    else
    {
        *pu32DVSupportProfiles = HAL_HVD_EX_GetDVSupportProfiles();
        return E_HVD_EX_OK;
    }
...
HVD_EX_Result MDrv_HVD_EX_GetDVSupportHighestLevel(MS_U32 u32DVProfile, MS_U32 *pu32DVLevel)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;
    ...
    {
        *pu32DVLevel = HAL_HVD_EX_GetDVSupportHighestLevel(u32DVProfile);
        eRet = E_HVD_EX_OK;
    }
...
void MDrv_HVD_EX_SetCMAInformation(void* cmaInitParam)
{
    UNUSED(cmaInitParam);
    pHVDDrvContext->bCMAUsed = TRUE;
#if defined(MSOS_TYPE_LINUX) && defined(SUPPORT_CMA)
    memcpy((void*)(pHVDDrvContext->cmaInitParam), cmaInitParam, sizeof(struct CMA_Pool_Init_Param) * 2);
#endif
}

MS_SIZE MDrv_HVD_EX_GetFrameBufferDefaultSize(HVD_EX_CodecType eCodecType, MS_BOOL bUseSubSample)
{
    return HAL_VPU_EX_GetFrameBufferDefaultSize((VPU_EX_CodecType)eCodecType, bUseSubSample);
}

MS_BOOL MDrv_HVD_EX_GetCMAMemSize(HVD_EX_CodecType eCodecType, HVD_EX_SrcMode eSrcMode,
                                  MS_U64 *offset, MS_SIZE *length, MS_U64 total_length, MS_SIZE unUseSize, MS_BOOL bUseSubSample)
{
    return HAL_VPU_EX_GetCMAMemSize((VPU_EX_CodecType)eCodecType, (VPU_EX_SrcMode)eSrcMode, offset, length, total_length, unUseSize, bUseSubSample);
}

HVD_EX_Result MDrv_HVD_EX_GetFbMemUsageSize(HVD_EX_FbMemUsage_Param *pInfo)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;

#if SUPPORT_EVD
    if (pInfo->eCodecType == E_HVD_EX_HEVC)
    {
        MS_U32 _u32MemUsageSize = 0;
        eRet = _HVD_EX_DemuxTS_ParseHEVCSPS(pInfo->u32DataVAddr, pInfo->u16DataSize, &_u32MemUsageSize);
        if (eRet == E_HVD_EX_OK)
        {
            pInfo->u32MemUsageSize = _u32MemUsageSize;
        }
    }
#endif
    return eRet;
...
MS_BOOL MDrv_HVD_EX_GetCapability(MS_U8 *pu8CmdNameIn, void *pParamIn, void *pParamOut)
{
    return HAL_VPU_EX_GetCapability(pu8CmdNameIn, pParamIn, pParamOut);
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_CPBRemovalDelay()
/// @brief \b Function \b Description: enable/disable OneSegment mode for AVC
/// @param -bEnable \b IN :enable/disable
/// @return -E_HVD_EX_OK for success; E_HVD_EX_FAIL for failure.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_CPBRemovalDelay(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_USE_CPB_REMOVAL_DEALY, bEnable));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

MS_BOOL MDrv_HVD_EX_IsAVSYNC_MODULE_DEFAULT_ON(void)
{
#ifdef AVSYNC_MODULE_DEFAULT_ON
    return TRUE;
#else
    return FALSE;
#endif
...
HVD_EX_Result MDrv_HVD_EX_SET_MVDPARSER(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PARSER_BYPASS, bEnable));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_SET_TOPFIELD_FIRST(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    //eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_TOPFIELD_FIRST_CONTROL, bEnable));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_GetVdecPlusStatus(MS_U32 u32Id, HVD_EX_VdecPlus_Info *stVdecPlusInfo)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    _DRV_HVD_Inited(u8DrvId, eRet);
    _DRV_HVD_EX_Entry(u8DrvId);
    stVdecPlusInfo->u32VdecPlusDecCnt = pShm->u32VdecPlusDecCnt;
    stVdecPlusInfo->u32VdecPlusDropCnt = pShm->u32VdecPlusDropCnt;
    stVdecPlusInfo->u32VdecPlusDispPicCnt = pShm->u32VdecPlusDispPicCnt;
    stVdecPlusInfo->u8VdecPlusDropRatio = pShm->u8VdecPlusDropRatio;
    stVdecPlusInfo->bTemporalScalability = !(pHVDDrvContext->gHVDPreCtrl[u8DrvId].eVdecFeature & E_HVD_DEF_FEATURE_DISABLE_TEMPORAL_SCALABILITY);
    eRet = E_HVD_EX_OK;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_SetARChangeMode(MS_U32 u32Id, MS_U32 u32ARChangeMode)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
#ifdef HVD_FW_VERSION
#if (HVD_FW_VERSION >= 0x00001510)//R31,R32 AR change cmd hvd fw version
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_DS_MODE, u32ARChangeMode));
#endif
#endif
...
HVD_EX_Result MDrv_HVD_EX_SET_FW_DEBUG_LEVEL(MS_U32 u32Id, MS_U32 u32DebugLevel)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_DEBUG_LEVEL, u32DebugLevel));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_SUPPORT_MULTI_ULTRA_RES(MS_U32 u32Id, MS_BOOL bSupport)
{
#ifdef VDEC_FW32
    return E_HVD_EX_OK;
#else
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SUPPORT_MULTI_ULTRA_RES, bSupport));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
#endif
}

HVD_EX_Result MDrv_HVD_EX_SetTrickSmoothScan(MS_U32 u32Id, HVD_EX_TrickSmoothScan *stTrickSmoothScan)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    HVD_EX_MSG_TRACE();
    _DRV_HVD_Inited(u8DrvId, eRet);
    eRet = (HVD_EX_Result)HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_TRICK_SMOOTH_SCAN, (MS_VIRT)stTrickSmoothScan);
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}
...
MS_BOOL MDrv_HVD_EX_TEE_AllocateEsBufHandle(MS_U8 u8Idx, MS_PHY pVPUCodecAddr, MS_U32 u32ReqSize, MS_U32 u32BuffEnd, MS_U32 *pu32EsHandle)
{
    return _MDrv_HVD_EX_TEE_AllocateEsBufHandle_(u8Idx, pVPUCodecAddr, u32ReqSize, u32BuffEnd, pu32EsHandle);
}

MS_BOOL MDrv_HVD_EX_TEE_GetESBufByHandle(MS_U8 u8Idx, MS_PHY pVPUCodecAddr, MS_U32 u32EsHandle, MS_PHY *pPhyAddr)
{
    return _MDrv_HVD_EX_TEE_GetESBufByHandle_(u8Idx, pVPUCodecAddr, u32EsHandle, pPhyAddr);
}
