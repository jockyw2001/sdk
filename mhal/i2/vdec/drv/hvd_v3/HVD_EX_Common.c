...
#ifdef CHIP_I2_FPGA
#define HVD_CMD_TIMEOUT                 20000   // ms
#else
...
MS_BOOL  _MDrv_HVD_AUTH_IPCheck(MS_U32 u32CodecType)
{
    ...
    UNUSED(pCodecCapInfo);

//#if !defined(SUPPORT_X_MODEL_FEATURE) && !(defined(STELLAR) && defined(MSOS_TYPE_LINUX_KERNEL))
//sylvia: compared to K6NVR: this maybe a fault-modifier
#if !defined(SUPPORT_X_MODEL_FEATURE) && !defined(STELLAR) && !defined(MSOS_TYPE_LINUX_KERNEL)
    ...
    switch (u32CodecType)
    {
        ...
        case HVD_INIT_HW_AVS2:
            if (MDrv_AUTH_IPCheck(IPAUTH_VIDEO_AVS2) == TRUE)
            {
                bModeSelectHD = TRUE;
                bIPAUTHReady  = TRUE;
                bAutoIpcheck  = TRUE;
...
#if defined(CHIP_K6LITE) || defined(CHIP_K6) || defined(CHIP_K7U) || defined(CHIP_I2)
...
MS_BOOL _MDrv_HVDSendCodecKey(MS_U32 u32Id, MS_U8 u8Decoder, MS_U16 u16CodecKey)
{
    ...
    u32Timeout = HVD_GetSysTime_ms() + HVD_CMD_TIMEOUT;
...
#endif
        MsOS_ReadMemory();
        if ((ctl_ptr->task_statue[0] != lastSt0) || (ctl_ptr->task_statue[1] != lastSt1))
        {
            HVD_EX_MSG_INF("pooling: task_status t0=%d, t1=%d\n", ctl_ptr->task_statue[0], ctl_ptr->task_statue[1]);
            lastSt0 = ctl_ptr->task_statue[0];
            lastSt1 = ctl_ptr->task_statue[1];
        }
...
#ifndef CONFIG_MBOOT
#if defined(CHIP_K6LITE) || defined(CHIP_K6) || defined(CHIP_K7U) || defined(CHIP_I2)
        u32CodecType = u32ModeFlag & HVD_CODEC_INFO_MASK;
        if(eRst == E_HVD_EX_OK)
        {
...
static MS_U32 _HVD_EX_DemuxTS_ReadSyntaxElement_FLC(MS_U16 u16ReadBits, const MS_U8* buffer, MS_U16 bufferLen, MS_U16* u16bitLen, MS_BOOL bUnsige)
{
    ...
    HVD_EX_MSG_DBG("BitLen=%d, Readbits=%d, Codeword=%td.\n", *u16bitLen, u16readbitLen, (ptrdiff_t)u32codeword);
...
#define maxDpbPicBuf (6)
                ...
                *pMemUsageSize += HAL_HVD_EX_GetEVDHWBuffer();
                ...
                HVD_EX_MSG_INF("[DemuxTS] EVD Memory usage = %td\n",
                               (ptrdiff_t)(*pMemUsageSize));
