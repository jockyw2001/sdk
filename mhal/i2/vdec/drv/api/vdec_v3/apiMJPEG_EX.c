...
// JPEG
#ifndef UTOPIA_STRIP
#include "../../drv/mjpeg_v3/drvMJPEG.h"
#include "../../drv/mjpeg_v3/VirtualhalMJPEG.h"
#else
#include "../../drv/mjpeg_v3/drvMJPEG.h"
#include "../../drv/mjpeg_v3/VirtualhalMJPEG.h"
#endif
#include "apiJPEG.h"
#ifdef STELLAR
//#include "../njpeg_ex/apiJPEG_priv.h"
...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define JPEGBASEWIDTHMAX    3840UL//1920UL
#define JPEGBASEHEIGHTMAX   2160UL//1080UL
...
#define MJPEG_HD_1280x720_WIDTH      1280UL
#define MJPEG_HD_1280x720_HEIGHT     720UL

#define MJPEG_FHD_1920x1080_WIDTH    1920UL
#define MJPEG_FHD_1920x1080_HEIGHT   1088UL

#define MJPEG_UHD_3840x2160_WIDTH    3840UL
#define MJPEG_UHD_3840x2160_HEIGHT   2160UL

#define MJPEG_MAX_SUPPORT_WIDTH      MJPEG_UHD_3840x2160_WIDTH
#define MJPEG_MAX_SUPPORT_HEIGHT     MJPEG_UHD_3840x2160_HEIGHT

#define MAX_SUPPORTED_FRAME_RATE_1080P   30000UL

#define DISPLAY_QUEUE_NUM           2UL
...
#define SKIP_ERROR_FRAME_THRESHOLD                  3000UL //60UL
...
typedef enum
{
    E_MJPEG_MAX_SUPPORTED_NONE,
    E_MJPEG_MAX_SUPPORTED_FHD,
    E_MJPEG_MAX_SUPPORTED_720P,
    E_MJPEG_MAX_SUPPORTED_480P,
    E_MJPEG_MAX_SUPPORTED_4K2K,
} MJPEG_MAX_SUPPORTED_RESOLUTION;
...
static MJPEG_MAX_SUPPORTED_RESOLUTION eMaxSupport[E_MJPEG_MJPEG_TOTAL];
...
//-------------------------------------------------------------------------------------------------
#if (defined CONFIG_MLOG)
#define MJPEG_DEBUG(format, args...)          do{if(_u8MJPEG_ApiDbgLevel & E_JPEG_DEBUG_MJPEG) ULOGI("MJPEG", format, ##args);}while(0)
#define MJPEG_DEBUG_API_MSG(format, args...)  do{if(_u8MJPEG_ApiDbgLevel & E_JPEG_DEBUG_API) ULOGI("MJPEG", format, ##args);}while(0)
#define MJPEG_DEBUG_API_ERR(format, args...)  do{if(_u8MJPEG_ApiDbgLevel & E_JPEG_DEBUG_ERR) ULOGE("MJPEG", format, ##args);}while(0)
...
// Speed Control
    ...
    eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_NONE;
...
#endif
{
    ...
    u8DrvId = MDrv_MJPEG_GetDrvId(u32Id);
...
#endif
    {
        ...
        eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_FHD;
        ...
        if(bCMAUsed && u8DrvId == 0)
        {
            ...
            else
            {
                ...
                MJPEG_DEBUG("[VDEC][%d]MApi_CMA_Pool_GetMem in mjpeg: alloc_param.pool_handle_id=%x, alloc_param.flags=%x, alloc_param.offset_in_pool=%llx, alloc_param.length=%x\n",
    ...
    if(MApi_JPEG_IsNJPD() == TRUE)
    {
        if((pInitParam->u32MWCBufSize - MJPEG_RESERVED_MJPEG_COLLECT_BUF_SIZE)
                >= MJPEG_UHD_3840x2160_WIDTH * MJPEG_UHD_3840x2160_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_UHD with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_4K2K;
        }
        else if((pInitParam->u32MWCBufSize - MJPEG_RESERVED_MJPEG_COLLECT_BUF_SIZE)
                >= MJPEG_FHD_1920x1080_WIDTH * MJPEG_FHD_1920x1080_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_FHD with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_FHD;
        }
        else if((pInitParam->u32MWCBufSize - MJPEG_RESERVED_MJPEG_COLLECT_BUF_SIZE)
                >= MJPEG_HD_1280x720_WIDTH * MJPEG_HD_1280x720_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_720P with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_720P;
        }
        else if((pInitParam->u32MWCBufSize - MJPEG_RESERVED_MJPEG_COLLECT_BUF_SIZE)
                >= MJPEG_SD_WIDTH * MJPEG_SD_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_480P with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_480P;
        }
        else
        {
            MJPEG_DEBUG_API_ERR("Not enough framebuffer for 480p asf-mjpeg mode support!!\n");
            return E_MJPEG_RET_FAIL;
        }
    }
    else
    ...
    if(MApi_JPEG_IsNJPD() == TRUE)
    {
        if(pInitParam->u32MWCBufSize >= MJPEG_MAX_SUPPORT_WIDTH * MJPEG_MAX_SUPPORT_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_FHD with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_FHD;
    ...
    {
        if(pInitParam->u32MWCBufSize >= MJPEG_SD_WIDTH * MJPEG_SD_HEIGHT * 2 * DISPLAY_QUEUE_NUM)
        {
            MDrv_MJPEG_SetFrameBuffTotalNum(u8DrvId, DISPLAY_QUEUE_NUM);
            MJPEG_DEBUG("Max Support: MJPEG_480P with pInitParam->u32MWCBufSize=0x%tx\n", (ptrdiff_t)pInitParam->u32MWCBufSize);
            eMaxSupport[u8DrvId] = E_MJPEG_MAX_SUPPORTED_480P;
...
MJPEG_Result MApi_MJPEG_Stop(MS_U32 u32Id)
{
    ...
    if (bCMAUsed && u8DrvId == 0)
    {
        ...
        MJPEG_DEBUG("[VDEC][%d]MApi_CMA_Pool_PutMem in mjpeg: cmaFreeParam.pool_handle_id=%x, cmaFreeParam.offset_in_pool=%llx, cmaFreeParam.length=%x\n",
                    (unsigned int)u8DrvId,
...
//        memset((void *)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr), 0, (pInfo->u32StAddr + __u32MRCBufAddr[u8DrvId]-jpegInitParam.u32MRCBufAddr));
...
#if 0
        printf("bitstream addr[PA]: 0x%llx, size=0x%x\n", jpegInitParam.u32MRCBufAddr, pInfo->u32Size);
        ...
        if (__bIsFirstFrameRdy[u8DrvId] == FALSE)
        {
            ...
            if(MApi_JPEG_IsNJPD() == TRUE)
            {
                if(((MDrv_MJPEG_GetWidth(u8DrvId) > MJPEG_MAX_SUPPORT_WIDTH) ||
                        ...
                        && (eMaxSupport[u8DrvId] == E_MJPEG_MAX_SUPPORTED_4K2K))
                {
                    MJPEG_DEBUG_API_ERR("Set MJPEG error code=E_MJPEG_ERR_CODE_RES > UHD\n");
                    _MApi_MJPEG_SetErrCode(u32Id, E_MJPEG_ERR_CODE_RES);
                }
                else if(((MDrv_MJPEG_GetWidth(u8DrvId) > MJPEG_FHD_1920x1080_WIDTH) ||
                        (MDrv_MJPEG_GetHeight(u8DrvId) > MJPEG_FHD_1920x1080_HEIGHT))
                        && (eMaxSupport[u8DrvId] == E_MJPEG_MAX_SUPPORTED_FHD))
                {
                    MJPEG_DEBUG_API_ERR("Set MJPEG error code=E_MJPEG_ERR_CODE_RES > FHD\n");
                    _MApi_MJPEG_SetErrCode(u32Id, E_MJPEG_ERR_CODE_RES);
                }
                else if(((MDrv_MJPEG_GetWidth(u8DrvId) > MJPEG_HD_1280x720_WIDTH) ||
                         (MDrv_MJPEG_GetHeight(u8DrvId) > MJPEG_HD_1280x720_HEIGHT))
                        && (eMaxSupport[u8DrvId] == E_MJPEG_MAX_SUPPORTED_720P))
                {
                    MJPEG_DEBUG_API_ERR("Set MJPEG error code=E_MJPEG_ERR_CODE_RES > 720P\n");
                    _MApi_MJPEG_SetErrCode(u32Id, E_MJPEG_ERR_CODE_RES);
                }
                else if(((MDrv_MJPEG_GetWidth(u8DrvId) > MJPEG_SD_WIDTH) ||
                         (MDrv_MJPEG_GetHeight(u8DrvId) > MJPEG_SD_HEIGHT))
                        && (eMaxSupport[u8DrvId] == E_MJPEG_MAX_SUPPORTED_480P))
                {
                    MJPEG_DEBUG_API_ERR("Set MJPEG error code=E_MJPEG_ERR_CODE_RES > 480P\n");
                    _MApi_MJPEG_SetErrCode(u32Id, E_MJPEG_ERR_CODE_RES);
                }
            }
            else
        ...
        if (_MApi_JPEG_DecodeHdr() == E_JPEG_FAILED)
        {
            ...
            jpeg_errcode = _MApi_JPEG_GetErrorCode();
    ...
    {
        switch(_MApi_JPEG_Decode())
        {
            case E_JPEG_DONE:
                // This state is only for progressive JPEG file
                MJPEG_DEBUG_API_MSG("Decode_state: E_JPEG_DONE\n");
                g_state = E_JPEG_DECODE_BEGIN;
                break;
            case E_JPEG_OKAY:
                MJPEG_DEBUG_API_MSG("Decode_state: E_JPEG_OKAY\n");
                g_state = E_JPEG_WAITDONE;
                break;
            case E_JPEG_FAILED:
            default:
                MJPEG_DEBUG_API_MSG("Decode_state: E_JPEG_FAILED\n");
                MJPEG_DEBUG_API_MSG("JPDApp_StartDecode: DecodeBegin failed\n");
...
//    ret = __MApi_MJPEG_CheckDecodeDone(u32Id);
    ...
    if (__bIsFirstFrameRdy[u8DrvId] == 0 && bLstFrame[u8DrvId] == 1)
    {
        MJPEG_DEBUG("MApi_MJPEG_IsDispFinish()...no input frame!!!\n");
        return TRUE;
    }
...
MJPEG_Result MApi_MJPEG_GetDispInfo(MS_U32 u32Id, MJPEG_DISP_INFO *pInfo)
{
    ...
    pInfo->u16CropLeft = 0;
    if(pInfo->u16Width >= MDrv_MJPEG_GetWidth(u8DrvId))
    {
        pInfo->u16CropRight = pInfo->u16Width - MDrv_MJPEG_GetWidth(u8DrvId) - pInfo->u16CropLeft;
    }
    else
    {
        // for the case we cannot get correct width/pitch
...
MS_BOOL MApi_MJPEG_IsIdle(MS_U32 u32Id)
{
    ...
    u16Value = _MApi_JPEG_GetJPDEventFlag();
...
MJPEG_Result MApi_MJPEG_GetDispFrameInfo(MS_U32 u32Id, MJPEG_FrameInfo *pFrmInfo)
{
    ...
    {
        MJPEG_DEBUG ("ID 0x%08lx%08lx    PTS %d     addr 0x%lx\n",
...
MJPEG_Result MApi_MJPEG_GetNextDispFrame(MS_U32 u32Id, MJPEG_FrameInfo *pFrmInfo)
{
    ...
    eRet = __MApi_MJPEG_CheckDecodeDone(u32Id);
    UNUSED(eRet);
    pSrc = (MJPEG_FrameInfo*)MDrv_MJPEG_GetNextDispFrame(u32Id);
    if (pSrc != NULL)
    {
...
MS_BOOL MApi_MJPEG_IsDisableVpuControl(void)
{
    return MDrv_MJPEG_IsDisableVpuControl();
}
