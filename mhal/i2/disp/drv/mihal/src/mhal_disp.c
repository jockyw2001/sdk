...
#define XC_FBL_ALWAYS                   1
#define XC_FBL_SC1_DYNAMIC              2
#define XC_FBL_DISABLE                  3
#define XC_FBL_SC1                      4

#define XC_FBL_TYPE                     XC_FBL_ALWAYS

#if (XC_FBL_TYPE == XC_FBL_ALWAYS)
#define XC_SC0_MAIN_DNR_BUF_SIZE        0x00000000
#define XC_SC1_MAIN_DNR_BUF_SIZE        0x00000000
#elif (XC_FBL_TYPE == XC_FBL_SC1_DYNAMIC)
#define XC_SC0_MAIN_DNR_BUF_SIZE        0
#define XC_SC1_MAIN_DNR_BUF_SIZE        0x008f7000
#elif (XC_FBL_TYPE == XC_FBL_SC1)
#define XC_SC0_MAIN_DNR_BUF_SIZE        0x03f48000
#define XC_SC1_MAIN_DNR_BUF_SIZE        0x00000000
#else
#define XC_SC0_MAIN_DNR_BUF_SIZE        0x03f48000
#define XC_SC1_MAIN_DNR_BUF_SIZE        0x008f7000
...
typedef enum
{
    ...
    E_MHAL_DISP_PNL_LVDS                 = 46,
    E_MHAL_DISP_PNL_RES_MAX_NUM          = 47,
...
extern unsigned int u32MiuDramSize;
...
MHalDispVideoLayerCtx_t gstFakeVideoLayerCtx;
...
MS_BOOL _MHalDispSynInfo2TransVideoTimingType(MHAL_DISP_SyncInfo_t *pSyncInfo, E_OUTPUT_VIDEO_TIMING_TYPE *penVideTiming)
{
    MS_BOOL bRet = TRUE;

    if(pSyncInfo->u16Hact == 1920 && pSyncInfo->u16Vact == 1080)
    {
         if(pSyncInfo->u32FrameRate == 24)
         {
            *penVideTiming = E_RES_1920x1080P_24Hz;
         }
         else if(pSyncInfo->u32FrameRate == 25)
         {
            *penVideTiming = E_RES_1920x1080P_25Hz;
         }
         else if(pSyncInfo->u32FrameRate == 30)
         {
            *penVideTiming = E_RES_1920x1080P_30Hz;
         }
         else if(pSyncInfo->u32FrameRate == 50)
         {
            *penVideTiming = E_RES_1920x1080P_50Hz;
         }
         else if(pSyncInfo->u32FrameRate == 60)
         {
    ...
    else if(pSyncInfo->u16Hact == 1280 && pSyncInfo->u16Vact == 800)
    {
        if(pSyncInfo->u32FrameRate == 60)
        {
    ...
    else if(pSyncInfo->u16Hact == 1280 && pSyncInfo->u16Vact == 1024)
    {
        if(pSyncInfo->u32FrameRate == 60)
        {
...
MHalDispPnlResolutionType_e _MHalDispDevTiming2PnlResType(MHAL_DISP_DeviceTiming_e enTiming)
{
    ...
    switch(enTiming)
    {
        ...
        case E_MHAL_DISP_OUTPUT_USER:
            enPnlResType = E_MHAL_DISP_PNL_LVDS;
            break;
...
MS_BOOL _MHalDispCheckFBL(MHalDispDeviceCtx_t *pMHalDispCtx)
{
    MS_BOOL bFBL;

#if (XC_FBL_TYPE == XC_FBL_SC1_DYNAMIC)
    if(pMHalDispCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1)
    {
        bFBL = (pMHalDispCtx->bAttached) ? DISABLE : ENABLE;
    }
    else
    {
        bFBL = (pMHalDispCtx->stXcInitData.u32Main_FB_Size == 0) ? ENABLE : DISABLE;
    }
#else
    bFBL = (pMHalDispCtx->stXcInitData.u32Main_FB_Size == 0) ? ENABLE : DISABLE;
#endif
...
void _MHalDispUpdateUserTimingCfg(PNL_EX_PanelType * pstPnlType, MHAL_DISP_SyncInfo_t *pSyncInfo)
{
    MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "H(%d %d %d %d) V(%d %d %d %d) Fps:%d\n",
        pSyncInfo->u16Hpw, pSyncInfo->u16Hbb, pSyncInfo->u16Hact, pSyncInfo->u16Hfb,
        pSyncInfo->u16Vpw, pSyncInfo->u16Vbb, pSyncInfo->u16Vact, pSyncInfo->u16Vfb,
        pSyncInfo->u32FrameRate);

    pstPnlType->m_ucPanelHSyncWidth     = pSyncInfo->u16Hpw;
    pstPnlType->m_ucPanelHSyncBackPorch = pSyncInfo->u16Hbb;
    pstPnlType->m_ucPanelVSyncWidth     = pSyncInfo->u16Vpw;
    pstPnlType->m_ucPanelVBackPorch     = pSyncInfo->u16Vbb;
    pstPnlType->m_wPanelHStart          = pSyncInfo->u16Hpw + pSyncInfo->u16Hbb;
    pstPnlType->m_wPanelVStart          = pSyncInfo->u16Vpw + pSyncInfo->u16Vbb;
    pstPnlType->m_wPanelWidth           = pSyncInfo->u16Hact;
    pstPnlType->m_wPanelHeight          = pSyncInfo->u16Vact;

    pstPnlType->m_wPanelHTotal =
        pSyncInfo->u16Hpw + pSyncInfo->u16Hbb + pSyncInfo->u16Hfb + pSyncInfo->u16Hact;

    pstPnlType->m_wPanelVTotal =
        pSyncInfo->u16Vpw + pSyncInfo->u16Vbb + pSyncInfo->u16Vfb + pSyncInfo->u16Vact;


    pstPnlType->m_wPanelMaxHTotal = pstPnlType->m_wPanelHTotal + 50;
    pstPnlType->m_wPanelMinHTotal = pstPnlType->m_wPanelHTotal - 50;

    pstPnlType->m_wPanelMaxVTotal = pstPnlType->m_wPanelVTotal + 50;
    pstPnlType->m_wPanelMinVTotal = pstPnlType->m_wPanelVTotal - 50;

    pstPnlType->m_dwPanelDCLK =
        ((MS_U32)pstPnlType->m_wPanelHTotal * (MS_U32)pstPnlType->m_wPanelVTotal * (MS_U32)pSyncInfo->u32FrameRate) / 1000000;

    pstPnlType->m_dwPanelMaxDCLK =
        ((MS_U32)pstPnlType->m_wPanelMaxHTotal * (MS_U32)pstPnlType->m_wPanelMaxVTotal * (MS_U32)pSyncInfo->u32FrameRate) / 1000000;

    pstPnlType->m_dwPanelMinDCLK =
        ((MS_U32)pstPnlType->m_wPanelMinHTotal * (MS_U32)pstPnlType->m_wPanelMinVTotal * (MS_U32)pSyncInfo->u32FrameRate) / 1000000;

    MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "H(%d %d %d %d) V(%d %d %d %d) DCLK:%d\n",
        pstPnlType->m_ucPanelHSyncWidth, pstPnlType->m_ucPanelHSyncBackPorch, pstPnlType->m_wPanelWidth, pstPnlType->m_wPanelHTotal,
        pstPnlType->m_ucPanelVSyncWidth, pstPnlType->m_ucPanelVBackPorch, pstPnlType->m_wPanelHeight, pstPnlType->m_wPanelVTotal,
        pstPnlType->m_dwPanelDCLK);
}
...
void _MHalDispPatchVsyncStart(MHalDispDeviceCtx_t *pMHalDispCtx, PNL_EX_PanelType *pPnlInfo)
{
    ...
    {   //Device1 is for VGA out , the VDE start must be 0 and Vsync start = 0
        pPnlInfo->m_wPanelVStart = (_MHalDispCheckFBL(pMHalDispCtx) && pMHalDispCtx->bAttached)? 0 : pPnlInfo->m_ucPanelVSyncWidth  + pPnlInfo->m_ucPanelVBackPorch;
    }
}
...
MS_BOOL _MHalDispInitDac(MHalDispDeviceCtx_t *pCtx)
{
    ...
    if(gbDacInited[pCtx->enDeviceId] == FALSE)
    {
        ...
        if(pCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1)
        {
...
MS_BOOL _MHalDispChangePnlTiming(MHalDispDeviceCtx_t *pMHalDispCtx, MHAL_DISP_DeviceTiming_e enTiming, MHAL_DISP_SyncInfo_t *pSyncInfo)
{
    PNL_DeviceId stPnlDeviceId;
    ...
    if(enTiming == E_MHAL_DISP_OUTPUT_USER)
    {
        pstPnlType = _MHalDispGetPanelSpec(_MHalDispDevTiming2PnlResType(enTiming));
        _MHalDispUpdateUserTimingCfg(pstPnlType, pSyncInfo);
    }
    else
    {
...
MS_BOOL _MHalDispSetFrameLock(MHalDispDeviceCtx_t *pSrcDevCtx, MHalDispDeviceCtx_t *pDstDevCtx)
{
    MS_BOOL bFBL;
    E_OUTPUT_VIDEO_TIMING_TYPE enOutputVideoTiming;
    MS_BOOL bRet = TRUE;

    bFBL = _MHalDispCheckFBL(pDstDevCtx);

    if(bFBL && pDstDevCtx->bAttached && pDstDevCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1)
    {

        if(pSrcDevCtx->enDeviceTiming[E_MHAL_DISP_OUPUT_INTERFACE_0] !=
           pDstDevCtx->enDeviceTiming[E_MHAL_DISP_OUPUT_INTERFACE_0])
        {
            MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "Src / Dst Timing is not same\n");
            bRet = FALSE;
        }
        else
        {
            MHAL_DISP_DeviceTiming_e enTiming = pDstDevCtx->enDeviceTiming[E_MHAL_DISP_OUPUT_INTERFACE_0];

            _MHalDispChangePnlTiming(pDstDevCtx, enTiming, NULL);
            ...
            else
            {
                ...
                MApi_DAC_DumpTable(NULL, E_DAC_DUMP_TABTYPE_FRAMELOCK_CLK_INIT);
                MApi_DAC_DumpTable(NULL, E_DAC_DUMP_TABTYPE_FRAMELOCK_SC_INIT);
            }
        }
    }
    return bRet;
...
MS_BOOL _MHalDispSetXcWindow(MHalDispDeviceCtx_t *pMHalDispCtx, MHAL_DISP_VidWinRect_t *ptSrcWin, MHAL_DISP_VidWinRect_t *ptCropWin, MHAL_DISP_VidWinRect_t *ptDstWin)
{
    MS_BOOL bRet = TRUE;
#if ENABLE_UTPA_XC
    XC_DEVICE_ID stXcDeviceId;
    ...
    bFBL = _MHalDispCheckFBL(pMHalDispCtx);

    MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "XC:%d, Attach:%d, FBSize=%x, FBL=%d\n",
        pMHalDispCtx->enDeviceId, pMHalDispCtx->bAttached, (MS_U32)pMHalDispCtx->stXcInitData.u32Main_FB_Size, bFBL);

    MApi_XC_EX_EnableFrameBufferLess(&stXcDeviceId, bFBL);
...
MS_BOOL _MHalDispChangeOutpuResolution(MHalDispDeviceCtx_t *pMHalDispCtx, MHAL_DISP_DeviceTimingInfo_t *pstTimingInfo, MHalDispDeviceOutputInterfaceType_e enInterface)
{
    ...
    {
        bRet = _MHalDispSynInfo2TransVideoTimingType(pstTimingInfo->pstSyncInfo, &enOutputVideoTiming);
    }
    else
    {
        bRet = _MHalDispDevTiming2VideoTimingType(pstTimingInfo->eTimeType, &enOutputVideoTiming);
    }
    ...
    {
        pMHalDispCtx->enDeviceTiming[enInterface] = pstTimingInfo->eTimeType;

        if( _MHalDispChangePnlTiming(pMHalDispCtx, pstTimingInfo->eTimeType, pstTimingInfo->pstSyncInfo) == FALSE)
        {
        ...
        else
        {
            ...
            if(pMHalDispCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_0)
            {
                ...
                if(pMHalDispCtx->u32Interface == MHAL_DISP_INTF_LCD)
                {
                    MApi_DAC_DumpTable(NULL, E_DAC_DUMP_TABTYPE_PNL_CLK_INIT);
                }
            }
            else
            {
...
MS_U16 _MHalDispCalculateNonLinear(MHalDispPictureNonLinearCurveType_t *pNonLinearCurve, MS_U8 u8AdjustValue)
{
    MS_U16 u16Value,u16Y0,u16Y1,u16X0,u16X1,u16Intercept;
 ...
 // printf("u8AdjustValue %bu\n", u8AdjustValue);
    ...
    u16Value = ((MS_U16)u16DistanceOfY*u8AdjustValue/(u16DistanceOfX)) + u16Intercept;
    return u16Value;
}
...
MS_BOOL _MHalDispHDgenSetGain(MHalDispDeviceCtx_t *pMHalDispCtx, MS_U16 u16Val)
{
    MHAL_DISP_DBG(E_MHAL_DISP_DBG_FUNC, "Device =%d  %04x\n", pMHalDispCtx->enDeviceId, u16Val);

#if ENABLE_UTPA_XC
    MApi_XC_Write2ByteMask(0x103000, 0x0001, 0xFFFF);
    MApi_XC_Write2ByteMask(0x1030D6, u16Val, 0xFFFF);
    MApi_XC_Write2ByteMask(0x1030D8, u16Val, 0xFFFF);
    MApi_XC_Write2ByteMask(0x1030DA, u16Val, 0xFFFF);
#endif
    return TRUE;
}
...
MS_BOOL _MHalDispInputPortMute(MHalDispInputPortCtx_t *pInputPortCtx, MS_BOOL bMute)
{
    ...
    pInputPortCtx->bMute = stSubWinOnOffCfg.bEnable ;
    ...
    if(pVideoLayerCtx)
    {
        for(i=0; i< E_MHAL_DISP_INPUT_PORT_ID_NUM; i++)
        {
            if(pVideoLayerCtx->u32InputPortEnFlag & (1<<1))
            {
                ...
                pInputPortCtx->bMuteByVideoLayer = bMute;

                if(pInputPortCtx->bMute != bMute)
                {
        ...
        if(gbDeviceCreated[u32DeviceId] == FALSE)
        {
           ...
           if( _MHalDispInitVariable(u32DeviceId) == FALSE)
           {
            ...
            if(gstDeviceAllocMemFunc != NULL)
            {
                ...
                {
                    if(XC_SC0_MAIN_DNR_BUF_SIZE)
                    {
                        if(stMhalMemConfig.u32Xc0MainFbAddr == 0)
                        {
                            gstDeviceAllocMemFunc->alloc(NULL, XC_SC0_MAIN_DNR_BUF_SIZE, &u64PhyAddr);

                            stMhalMemConfig.u32Xc0MainFbAddr = u64PhyAddr;
                            stMhalMemConfig.u32Xc0MainFbSize = XC_SC0_MAIN_DNR_BUF_SIZE;
                            if(stMhalMemConfig.u32Xc0MainFbAddr == 0)
                            {
                                MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "Allocate Buffer fail \n");
                                return FALSE;
                        ...
                        MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "XC0 Allocate DNR Buffer,DeviceId=%d,  Base=%x Size=%d\n",
                            u32DeviceId, stMhalMemConfig.u32Xc0MainFbAddr, stMhalMemConfig.u32Xc0MainFbSize );
                    }
                    else
                    {
                        MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "XC0 No DNR Buffer\n");
                    }
                ...
                {
                    if(XC_SC1_MAIN_DNR_BUF_SIZE)
                    {
                        if(stMhalMemConfig.u32Xc1MainFbAddr == 0)
                        {
                            gstDeviceAllocMemFunc->alloc(NULL, XC_SC1_MAIN_DNR_BUF_SIZE, &u64PhyAddr);
                            stMhalMemConfig.u32Xc1MainFbAddr = u64PhyAddr;
                            stMhalMemConfig.u32Xc1MainFbSize = XC_SC1_MAIN_DNR_BUF_SIZE;
                        ...
                        MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "XC1 Allocate DNR Buffer,DeviceId=%d,  Base=%x Size=%d\n",
                            u32DeviceId, stMhalMemConfig.u32Xc1MainFbAddr, stMhalMemConfig.u32Xc1MainFbSize  );
                    }
                    else
                    {
                        MHAL_DISP_DBG(E_MHAL_DISP_DBG_XC, "XC1 No DNR Buffer\n");
                    }
                }
            }
            else
...
MS_BOOL MHAL_DISP_DeviceAttach(const void *pSrcDevCtx, const void *pDstDevCtx)
{
    ...
    if(pMHalDispSrcCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_0 || pMHalDispDstCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1)
    {
        ...
        {
            pMHalDispDstCtx->bAttached = TRUE;

            if(_MHalDispSetXcWindow(pMHalDispDstCtx, NULL, NULL, NULL) == FALSE)
            {
                bRet = FALSE;
            ...
            {
                if(_MHalDispSetFrameLock(pMHalDispSrcCtx, pMHalDispDstCtx) == FALSE)
                {
                    bRet = FALSE;
                    MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, " Xc Set FrameLock fail \n");
                }
                else
                {
                    //Use a Fake VideoLyaer to Bind Device1 for VideoLyaerMute
                    pMHalDispDstCtx->u8BindVideoLayerNum = 1;
                    pMHalDispDstCtx->pVideLayerCtx[MHAL_DISP_VIDEOLAYER_MASTER] = &gstFakeVideoLayerCtx;
                    gstFakeVideoLayerCtx.enDeviceId = E_MHAL_DISP_DEVICE_ID_1;
                    gstFakeVideoLayerCtx.u32InputPortEnFlag = 0x01;
                    gstFakeVideoLayerCtx.bEn = TRUE;

                    _MHalDispVideoLayerMute(&gstFakeVideoLayerCtx, FALSE);
                    bRet = TRUE;
                }
            }
        }
...
MS_BOOL MHAL_DISP_DeviceDetach(const void *pSrcDevCtx, const void *pDstDevCtx)
{
    ...
    if(pMHalDispSrcCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_0 || pMHalDispDstCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1)
    {
        if(pMHalDispDstCtx->bAttached)
        {
            _MHalDispVideoLayerMute(&gstFakeVideoLayerCtx, TRUE);

            if(_MHalDispSetXcDisConnect(pMHalDispDstCtx, pMHalDispDstCtx->enInputSrc))
            {
                pMHalDispDstCtx->bAttached = FALSE;
                pMHalDispDstCtx->u8BindVideoLayerNum = 0;
                pMHalDispDstCtx->pVideLayerCtx[MHAL_DISP_VIDEOLAYER_MASTER] = NULL;
                gstFakeVideoLayerCtx.enDeviceId = E_MHAL_DISP_DEVICE_ID_NUM;
                gstFakeVideoLayerCtx.u32InputPortEnFlag = 0;
                gstFakeVideoLayerCtx.bEn = FALSE;
...
// Device: Set/Get BackGround Color in YUV format
MS_BOOL MHAL_DISP_DeviceSetBackGroundColorYUV(void *pDevCtx, const MS_U32 u32BgColor)
{
    MHalDispDeviceCtx_t *pMHalDispCtx =(MHalDispDeviceCtx_t *)pDevCtx;
    XC_DEVICE_ID stXcDeviceId;
    ...
    FILL_UTPA_DEVICE_STRUCT(stXcDeviceId.u32Id, stXcDeviceId.u32Version, pMHalDispCtx->enDeviceId);
#if ENABLE_UTPA_XC
    MApi_XC_EX_SetFrameColorYUV(&stXcDeviceId, u32BgColor);
#endif
...
MS_BOOL MHAL_DISP_DeviceAddOutInterface(void *pDevCtx, const MS_U32 u32Interface)
{
    ...
    switch(pMHalDispCtx->enDeviceId)
    {
        case E_MHAL_DISP_DEVICE_ID_0:
            u32Mask = (MHAL_DISP_INTF_HDMI | MHAL_DISP_INTF_CVBS | MHAL_DISP_INTF_LCD);
            break;
...
MS_BOOL MHAL_DISP_DeviceSetOutputTiming(void *pDevCtx, const MS_U32 u32Interface, const MHAL_DISP_DeviceTimingInfo_t *pstTimingInfo)
{
    ...
    if(pMHalDispCtx->enDeviceId == E_MHAL_DISP_DEVICE_ID_1 && pstTimingInfo->eTimeType == E_MHAL_DISP_OUTPUT_USER)
    {
        bRet = FALSE;
        ...
        switch(pMHalDispCtx->enDeviceId)
        {
            case E_MHAL_DISP_DEVICE_ID_0:
                    ...
                    if(pstTimingInfo->eTimeType <= E_MHAL_DISP_OUTPUT_960H_NTSC)
                    {
                        ...
                        else
                        {
                            if(pMHalDispCtx->enDeviceTiming[E_MHAL_DISP_OUPUT_INTERFACE_0] == E_MHAL_DISP_OUTPUT_MAX)
                            {
                                ...
                                if(_MHalDispChangeOutpuResolution(pMHalDispCtx, (MHAL_DISP_DeviceTimingInfo_t *)pstTimingInfo, E_MHAL_DISP_OUPUT_INTERFACE_1) == FALSE)
                                {
                                    bRet = FALSE;
                ...
                else if(u32Interface == MHAL_DISP_INTF_HDMI)
                {
                    ...
                    {
                        if(_MHalDispChangeOutpuResolution(pMHalDispCtx, (MHAL_DISP_DeviceTimingInfo_t *)pstTimingInfo, E_MHAL_DISP_OUPUT_INTERFACE_0) == FALSE)
                        {
                            bRet = FALSE;
                            MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "Device Id_%d Change Output Resolution fail\n", pMHalDispCtx->enDeviceId);
                        }
                ...
                else if(u32Interface == MHAL_DISP_INTF_LCD)
                {
                    if(_MHalDispChangeOutpuResolution(pMHalDispCtx, (MHAL_DISP_DeviceTimingInfo_t *)pstTimingInfo, E_MHAL_DISP_OUPUT_INTERFACE_0) == FALSE)
                    {
                        bRet = FALSE;
...
MS_BOOL MHAL_DISP_DeviceSetVgaParam(void *pDevCtx, const MHAL_DISP_VgaParam_t *pstVgaInfo)
{
    ...
    else
    {
        ...
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->stCsc.u32Contrast);
            _MHalDispAceSetContrast(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->stCsc.u32Hue);
            _MHalDispAceSetHue(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->stCsc.u32Saturation);
            _MHalDispAceSetSaturation(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->stCsc.u32Luma);
            _MHalDispAceSetBrigtness(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->u32Sharpness);
            _MHalDispAceSetSharpness(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        pstNonLinearCurve = _MHalDispGetNonLinearCurve(MHAL_DISP_INTF_VGA, E_MHAL_DISP_PICTURE_GAIN);
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstVgaInfo->u32Gain);
            _MHalDispHDgenSetGain(pMHalDispCtx, u16Value);
        }
        else
        {
            MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "Gain NonLinearCurve NULL\n");
            bRet = FALSE;
        }
...
MS_BOOL MHAL_DISP_DeviceSetHdmiParam(void *pDevCtx, const MHAL_DISP_HdmiParam_t *pstHdmiInfo)
{
    ...
    else
    {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstHdmiInfo->stCsc.u32Contrast);
            _MHalDispAceSetContrast(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstHdmiInfo->stCsc.u32Hue);
            _MHalDispAceSetHue(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstHdmiInfo->stCsc.u32Saturation);
            _MHalDispAceSetSaturation(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
        ...
        if(pstNonLinearCurve)
        {
            u16Value = _MHalDispCalculateNonLinear(pstNonLinearCurve, (MS_U8)pstHdmiInfo->stCsc.u32Luma);
            _MHalDispAceSetBrigtness(pMHalDispCtx, (MS_U8)u16Value);
        }
        else
        {
    ...
    if(pVidLayerCtx && pstAttr)
    {
        ...
        if(_DrvSclMgwinIoSetFrameConfig(pstVideoLayerCtx->s32Handler, &stFrameCfg) == E_DRV_SCLMGWIN_IO_ERR_OK)
        {
            ...
            else
            {
                ...
                if(_MHalDispSetXcWindow(pstDevceCtx, &pstDevceCtx->stSrcWin, NULL, &pstDevceCtx->stDispWin) == FALSE)
                {
                    bRet = FALSE;
                    MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, " Xc Set Window fail \n");
                }
    ...
    if(bInputPortFristInit == TRUE)
    {
        ...
        for(i=0; i<E_MHAL_DISP_VIDEO_LAYER_ID_NUM; i++)
        {
            for(j=0; j<E_MHAL_DISP_INPUT_PORT_ID_NUM; j++)
            {
                ...
                stMHalDispInputPortCtx[i][j].bMuteByVideoLayer = FALSE;
    ...
    if(pstVideoLayerCtx)
    {
        ...
        else
        {
            if(pstVideoLayerCtx->s32Handler != -1)
            {
                ...
                stMHalDispInputPortCtx[pstVideoLayerCtx->enVideoLayerId][u32PortId].bMuteByVideoLayer = FALSE;
...
MS_BOOL MHAL_DISP_InputPortEnable(void *pInputPortCtx, const MS_BOOL bEnable)
{
    ...
    if(pMhalDispInputPortCtx)
    {
        ...
        if(bEnable)
        {
            ...
            {
                if(pMhalDispInputPortCtx->bMute != FALSE)
                {
                   bMuteRet = _MHalDispInputPortMute(pMhalDispInputPortCtx, FALSE);
                }
        ...
        else
        {
            ...
            if(pMhalDispInputPortCtx->bMute != TRUE)
            {
                bMuteRet = _MHalDispInputPortMute(pMhalDispInputPortCtx, TRUE);
            }
...
MS_BOOL MHAL_DISP_InputPortShow(void *pInputPortCtx)
{
    ...
    if(pMhalDispInputPortCtx)
    {
        ...
        pMhalDispInputPortCtx->bShow = TRUE;
    }
    else
    {
        bRet = FALSE;
...
MS_BOOL MHAL_DISP_InputPortHide(void *pInputPortCtx)
{
    ...
    if(pMhalDispInputPortCtx)
    {
        ...
        pMhalDispInputPortCtx->bShow = FALSE;
...
{
    MHalDispInputPortCtx_t *pMhalDispInputPortCtx = (MHalDispInputPortCtx_t *)pInputPortCtx;
    MHalDispDeviceCtx_t *pMhalDispDevCtx = NULL;
    ...
    if(pMhalDispInputPortCtx)
    {
        ...
        pMhalDispDevCtx = &stMHalDispDeviceCtx[pMhalDispInputPortCtx->pstVideoLayerCtx->enDeviceId];

        if((pMhalDispDevCtx->u32Interface == MHAL_DISP_INTF_LCD))
        {
            stSubWinFlipCfg.enOutColor = E_DRV_SCLMGWIN_IO_OUT_COLOR_RGB;
        }
        else
        {
            stSubWinFlipCfg.enOutColor = E_DRV_SCLMGWIN_IO_OUT_COLOR_YUV;
        }
        ...
        if(_MHalDispPixelFmt2MemFmt(
            ...
            &u32Stride))
        {
            if(u32Stride *pstVideoFrameBuffer->u32Height + pstVideoFrameBuffer->aPhyAddr[0] > u32MiuDramSize)
            {
                MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "DRAM size:%08x > %08x\n",
                    (unsigned int)(u32Stride *pstVideoFrameBuffer->u32Height + pstVideoFrameBuffer->aPhyAddr[0]),
                    (unsigned int)u32MiuDramSize);
                bRet =  FALSE;
            }
            else
            {
                ...
                if(_DrvSclMgwinIoSetSubWinFlipConfig(pMhalDispInputPortCtx->pstVideoLayerCtx->s32Handler, &stSubWinFlipCfg) == E_DRV_SCLMGWIN_IO_ERR_OK)
                {
                    ...
                    if(pMhalDispInputPortCtx->bMuteByVideoLayer == FALSE && pMhalDispInputPortCtx->bShow)
                    {
                        if(pMhalDispInputPortCtx->bMute != FALSE)
                        {
...
MS_BOOL MHAL_DISP_EnableDevIrq(void *pDevCtx, MS_U32 u32DevIrq, MS_BOOL bEnable)
{
    ...
    for(i=0; i<pMHalDispCtx->u8BindVideoLayerNum; i++)
    {
        ...
        if(bEnable)
        {
            gbVideoLayerIrqEnable[pMHalDispCtx->pVideLayerCtx[i]->enVideoLayerId] = TRUE;
            _DrvSclMgwinIoSetRegisterConfig(pMHalDispCtx->pVideLayerCtx[i]->s32Handler, &stRegsterCfg);
        }
        else
        {
            _DrvSclMgwinIoSetRegisterConfig(pMHalDispCtx->pVideLayerCtx[i]->s32Handler, &stRegsterCfg);
            gbVideoLayerIrqEnable[pMHalDispCtx->pVideLayerCtx[i]->enVideoLayerId] = FALSE;
        }
    }
...
{
    #define REG_MGWIN_0_INT_CLEAR   (0x130000 + (0x52<<1))
    #define REG_MGWIN_1_INT_CLEAR   (0x130000 + (0x53<<1))
    #define BIT_MGWIN_0_INT_CLEAR    (0x0100)
    #define BIT_MGWIN_1_INT_CLEAR    (0x0010)

    DrvSclMgwinIoRegisterConfig_t stRegCfg;
    MS_BOOL bRet =TRUE;
    ...
    {
        stRegCfg.u32Addr = REG_MGWIN_0_INT_CLEAR;
        stRegCfg.u16Val  = BIT_MGWIN_0_INT_CLEAR;
        stRegCfg.u16Msk  = BIT_MGWIN_0_INT_CLEAR;
        _DrvSclMgwinIoSetRegisterConfig(NULL, &stRegCfg);
    }

    if((pstIrqFlag->u32IrqMask & 0x02) && (pstIrqFlag->u32IrqFlag & 0x02))
    {
        stRegCfg.u32Addr = REG_MGWIN_1_INT_CLEAR;
        stRegCfg.u16Val  = BIT_MGWIN_1_INT_CLEAR;
        stRegCfg.u16Msk  = BIT_MGWIN_1_INT_CLEAR;
        _DrvSclMgwinIoSetRegisterConfig(NULL, &stRegCfg);
    }
...
{
#define REG_MGWIN_0_INT_STATUS   (0x130000 + (0x50<<1))
#define REG_MGWIN_1_INT_STATUS   (0x130000 + (0x51<<1))
#define BIT_MGWIN_0_INT_STATUS   (0x0100)
#define BIT_MGWIN_1_INT_STATUS   (0x0010)

    MS_U8 i;
    DrvSclMgwinIoRegisterConfig_t stRegCfg;
    MHalDispDeviceCtx_t *pMHalDispDevCtx = (MHalDispDeviceCtx_t *)pDevCtx;

    pstIrqFlag->u32IrqMask = 0;
    pstIrqFlag->u32IrqFlag = 0;

    for(i=0; i< pMHalDispDevCtx->u8BindVideoLayerNum; i++)
    {
        if(pMHalDispDevCtx->pVideLayerCtx[i]->enVideoLayerId == E_MHAL_DISP_VIDEO_LAYER_ID_0)
        {
            stRegCfg.u32Addr = REG_MGWIN_0_INT_STATUS;
            _DrvSclMgwinIoGetRegisterConfig(NULL, &stRegCfg);
            pstIrqFlag->u32IrqFlag |= (stRegCfg.u16Val & BIT_MGWIN_0_INT_STATUS) ?  1 : 0;
            pstIrqFlag->u32IrqMask |= (gbVideoLayerIrqEnable[E_MHAL_DISP_VIDEO_LAYER_ID_0]) ? 0x01 : 0x00;
        }
        else
        {
            stRegCfg.u32Addr = REG_MGWIN_1_INT_STATUS;
            _DrvSclMgwinIoGetRegisterConfig(NULL, &stRegCfg);
            pstIrqFlag->u32IrqFlag |= (stRegCfg.u16Val & BIT_MGWIN_1_INT_STATUS) ?  2 : 0;
...
EXPORT_SYMBOL(MHAL_DISP_DeviceSetBackGroundColorYUV);
