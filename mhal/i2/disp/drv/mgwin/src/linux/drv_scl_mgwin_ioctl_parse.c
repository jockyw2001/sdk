...
#define CMD_PARSING(x)  (x==IOCTL_SCLMGWIN_SET_FRAME_CONFIG                     ?   "IOCTL_SCLMGWIN_SET_FRAME_CONFIG" : \
                         ...
                         x==IOCTL_SCLMGWIN_SET_DEBUG_CONFIG                     ?   "IOCTL_SCLMGWIN_SET_DEBUG_CONFIG" :\
                         x==IOCTL_SCLMGWIN_SET_SUBWIN_MASKBLACK_ONOFF_CONFIG    ?   "IOCTL_SCLMGWIN_SET_SUBWIN_MASKBLACK_ONOFF_CONFIG" :\
                                                                                    "UNKNOWN")
...
int _DrvSclMgwinIoctlSetSubWinMaskBlackOnOffConfig(struct file *filp, unsigned long arg)
{
    DrvSclMgwinIoSubWinOnOffConfig_t stIoCfg;

    if(_gstSclMgwinIoFunc.DrvSclMgwinIoSetSubWinMaskBlackOnOffConfig == NULL)
    {
        SCL_ERR("%s %d: Null Function\n", __FUNCTION__, __LINE__);
        return -EFAULT;
    }
    ...
    else
    {
        ...
        eErrType = _gstSclMgwinIoFunc.DrvSclMgwinIoSetSubWinMaskBlackOnOffConfig(s32Handler, &stIoCfg);
        return _DrvSclMgwinIoctlTransErrType(eErrType);
    }
}
...
int _DrvSclMgwinIoctlSetDebugConfig(struct file *filp, unsigned long arg)
{
    DrvSclMgwinIoDebugConfig_t stIoCfg;

    if(_gstSclMgwinIoFunc.DrvSclMgwinIoSetDebugConfig == NULL)
    {
        SCL_ERR("%s %d: Null Function\n", __FUNCTION__, __LINE__);
        return -EFAULT;
    }

    if(copy_from_user(&stIoCfg, (DrvSclMgwinIoDebugConfig_t __user *)arg, sizeof(DrvSclMgwinIoDebugConfig_t)))
    {
        return -EFAULT;
    }
    else
    {
        ...
        eErrType = _gstSclMgwinIoFunc.DrvSclMgwinIoSetDebugConfig(s32Handler, &stIoCfg);
        return _DrvSclMgwinIoctlTransErrType(eErrType);
    }
}
...
int DrvSclMgwinIoctlParse(struct file *filp, unsigned int u32Cmd, unsigned long u32Arg)
{
    ...
    switch(u32Cmd)
    {
        ...
        case IOCTL_SCLMGWIN_SET_SUBWIN_MASKBLACK_ONOFF_CONFIG:
            retval = _DrvSclMgwinIoctlSetSubWinMaskBlackOnOffConfig(filp, u32Arg);
            break;
        ...
        case IOCTL_SCLMGWIN_SET_REGISTER_CONFIG:
            retval = _DrvSclMgwinIoctlSetRegsiterConfig(filp, u32Arg);
            break;

        case IOCTL_SCLMGWIN_SET_DEBUG_CONFIG:
            retval = _DrvSclMgwinIoctlSetDebugConfig(filp, u32Arg);
            break;
