...
MHalHdmitxColorType_e    enHdmiTxInColor  = E_MHAL_HDMITX_COLOR_YUV444;
...
void _MHalHdmitxUpdate(void)
{
    ...
    MApi_HDMITx_SetColorFormat(_MHalHdmiTxTransColor(enHdmiTxInColor), _MHalHdmiTxTransColor(enHdmiTxOutColor));
...
MS_BOOL MHalHdmitxInit(void)
{
    ...
    MApi_HDMITx_SetHPDGpioPin(0x19); //ToDo_Ryan
    MApi_HDMITx_Init();
...
MS_BOOL MHalHdmitxChangeResolution(MHAL_DISP_DeviceTiming_e enTiming)
{
    ...
    {
        MHAL_DISP_DBG(E_MHAL_DISP_DBG_ERR, "Time is out of range\n");
        bRet = FALSE;
    }
    else
    {
...
MS_BOOL MHalHdmitxChangeInColor(MHalHdmitxCDType_e enInputColor)
{
    enHdmiTxInColor = enInputColor;
    return TRUE;
}
