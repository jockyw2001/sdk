...
bool _DrvSclMgwinSetMuxConfig(DrvSclMgwinWinIdType_e enWinId, DrvSclMgwinCtxConfig_t *pSclMgwinCtxCfg, DrvSclMgwinMuxCmdType_e enCmd)
{
    ...
    else
    {
        if((enMgwin1Path == E_DRV_SCLMGWIN_PATH_SC1 && enMgwin2Path != enMgwin1Path) ||
           ...
           (enMgwin2Path == E_DRV_SCLMGWIN_PATH_SC1 && enMgwin2Path != enMgwin1Path) ||
           (enMgwin2Path == E_DRV_SCLMGWIN_PATH_SC2 && enMgwin2Path != enMgwin1Path) )
        {
...
bool DrvSclMgwinInit(void)
{
    ...
    {
        for(j=0; j<DRV_SCLMGWIN_SUBWIN_NUM; j++)
        {
            HalSclMgwinSetSubWinOnOff(i, j, FALSE);
        }
    }
...
bool DrvSclMgwinSetFramePathConfig(void *pCtx, DrvSclMgwinFramePathConfig_t *pstFramePathCfg)
{
    ...
    if(pMgwinContain->enPath != E_DRV_SCLMGWIN_PATH_NUM)
    {
        bRet = _DrvSclMgwinSetMuxConfig(pstFramePathCfg->enWinId, pSclMgwinCtxCfg, E_DRV_SCLMGWIN_MUX_CMD_PATH);
    }
    else
    {
        bRet = TRUE;
...
bool DrvSclMgwinSetSubWinFlipConfig(void *pCtx, DrvSclMgwinSubWinFlipConfig_t *pSubWinFlipCfg)
{
    ...
    {
        SCL_ERR("[SCLMGWIN] %s %d, [Mgwin_%d_%d], Pitch(%x) must be 16 alighment\n",
            __FUNCTION__, __LINE__, pSubWinFlipCfg->enWinId, pSubWinFlipCfg->enSubWinId, pSubWinFlipCfg->u32Pitch);

        return FALSE;
    }
#endif

    SCL_DBG(SCL_DBG_LV_DRVSCLMGWIN() & EN_DBGMG_SCLMGWINLEVEL_SUBWIN, "[MGWIN_%d_%d], Mem=%d, Color=%d, Base=%x, Pitch=%x\n",
        pSubWinFlipCfg->enWinId, pSubWinFlipCfg->enSubWinId,
        pSubWinFlipCfg->enMemFmt,
        pSubWinFlipCfg->enOutColor,
    ...
    HalSclMgwinSetSubWinMemFmt(pSubWinFlipCfg->enWinId, pSubWinFlipCfg->enSubWinId, pSubWinFlipCfg->enMemFmt,  pSubWinFlipCfg->enOutColor);
...
bool DrvSclMgwinSetDbfConfig(void *pCtx, DrvSclMgwinDbfConfig_t *pstDbfCfg)
{
    ...
    else
    {
        ...
        {
            SCL_ERR("[SCLMGWIN] %s %d, Frame Id is not correct\n", __FUNCTION__, __LINE__);
            bRet = FALSE;
        }
    }
...
bool DrvSclMgwinSetDebugLevel(u8 u8DbgOpt, u8 u8DbgLvl)
{
    if(u8DbgOpt >= EN_DBGMG_NUM_CONFIG)
    {
        SCL_ERR("[SCLMGWIN] %s %d, DbgOpt not correct %d\n", __FUNCTION__, __LINE__, u8DbgOpt);
        return FALSE;
    }
    else
    {
        gMgwinbdbgmessage[u8DbgOpt] = u8DbgLvl;
    }

    return TRUE;
}

bool DrvSclMgwinSetSubWinMaskBlackOnOffConfig(void *pCtx, DrvSclMgwinSubWinOnOffConfig_t *pstSubWinOnOffCfg)
{
    DrvSclMgwinCtxConfig_t *pSclMgwinCtxCfg = (DrvSclMgwinCtxConfig_t *)pCtx;
    DrvSclMgwinSubWinContain_t *pMgwinSubWinContain;
    ...
    SCL_DBG(SCL_DBG_LV_DRVSCLMGWIN() & EN_DBGMG_SCLMGWINLEVEL_SUBWIN, "[MGWIN_%d_%d], bMaskBlackEn=%d\n",
        pstSubWinOnOffCfg->enWinId, pstSubWinOnOffCfg->enSubWinId,
        pstSubWinOnOffCfg->bEnable);
    ...
    //Ctx Data saving
    pMgwinSubWinContain = &pSclMgwinCtxCfg->stContain[pstSubWinOnOffCfg->enWinId].stSubWinContain[pstSubWinOnOffCfg->enSubWinId];
    pMgwinSubWinContain->bMaskBlackEn = pstSubWinOnOffCfg->bEnable;
    HalSclMgwinSetSubWinMaskBlackOnOff(pstSubWinOnOffCfg->enWinId, pstSubWinOnOffCfg->enSubWinId, pMgwinSubWinContain->bMaskBlackEn);
    return TRUE;
}
