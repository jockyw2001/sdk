...
//  Variable
//-------------------------------------------------------------------------------------------------
u16 gu16Y2RMatirx[CSC_MATRIX_REG_NUM] = {0x0015, 0x064D, 0x0400, 0x0000, 0x1E21, 0x0400, 0x1F41, 0x1E21, 0x0400, 0x076D};
u16 gu16R2YMatrix[CSC_MATRIX_REG_NUM] = {0xE010, 0x0200, 0x1E2F, 0x1FD1, 0x00DA, 0x02DC, 0x004A, 0x1F8A, 0x1E76, 0x0200};
u32 gu32CscMatriReg[CSC_MATRIX_REG_NUM] = {REG_MGWIN2_2B_L, REG_MGWIN2_2D_L, REG_MGWIN2_2E_L, REG_MGWIN2_2F_L, REG_MGWIN2_30_L, REG_MGWIN2_31_L, REG_MGWIN2_32_L, REG_MGWIN2_33_L, REG_MGWIN2_34_L, REG_MGWIN2_35_L};
...
void HalSclMgwinInit(DrvSclMgwinWinIdType_e enWinId)
{
    ...
    W2BYTEMSK(SCLMGWIN_REG(enWinId, REG_MGWIN2_10_L), BIT5, BIT5); // reg_422_yc_swap
}
...
void HalSclMgwinSetFrameTrigSel(DrvSclMgwinWinIdType_e enWinId, DrvSclMgwinFramePath_e enPath)
{
    ...
    if(enWinId == E_DRV_SCLMGWIN_WIN_ID_1)
    {
        u16Val = enPath == E_DRV_SCLMGWIN_PATH_SC1 ? 0 : BIT11;
    }
    else
    {
        u16Val = enPath == E_DRV_SCLMGWIN_PATH_SC2 ? 0 : BIT11;

    }
    W2BYTEMSK(SCLMGWIN_REG(enWinId, REG_MGWIN1_07_L), u16Val, BIT11);
}
...
void HalSclMgwinSetSubWinMemFmt(DrvSclMgwinWinIdType_e enWinId, DrvSclMgwinSubWinIdType_e enSubWinId, DrvSclMgwinMemFormat_e enMemFmt, DrvSclMgwinOutColor_e enOutColor)
{
    ...
    W2BYTEMSK(SCLMGWIN_REG(enWinId, REG_MGWIN2_10_L), enOutColor == E_DRV_SCLMGWIN_OUT_COLOR_RGB ? BIT4 : 0, BIT4);

    if(enMemFmt >= E_DRV_SCLMGWIN_MEM_FMT_YUV444_8B && enOutColor == E_DRV_SCLMGWIN_OUT_COLOR_RGB)
    { // Y2R
        pCscMatrix = gu16Y2RMatirx;
    }
    else if(enMemFmt<= E_DRV_SCLMGWIN_MEM_FMT_RGB565 && enOutColor == E_DRV_SCLMGWIN_OUT_COLOR_YUV)
    { //R2Y
        pCscMatrix = gu16R2YMatrix;
    }
    else
    {
        pCscMatrix = gu16Y2RMatirx;
    ...
    for(i=0; i<CSC_MATRIX_REG_NUM; i++)
    {
        W2BYTEMSK(SCLMGWIN_REG(enWinId, gu32CscMatriReg[i]), pCscMatrix[i], 0xFFFF);
    }
}
...
void HalSclMgwinSetSubWinMaskBlackOnOff(DrvSclMgwinWinIdType_e enWinId, DrvSclMgwinSubWinIdType_e enSubWinId, bool bEn)
{
    u32 u32Reg = SCLMGWIN_REG(enWinId, REG_MGWIN1_15_L);
    u16 u16Msk = (1 << enSubWinId);
    u16 u16Val = bEn ? (1 << enSubWinId) : 0;
    W2BYTEMSK(u32Reg, u16Val, u16Msk);
}
