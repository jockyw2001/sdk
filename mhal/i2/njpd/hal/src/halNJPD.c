...
#include "MsCommon.h"
#include "MsOS.h"
#include "../../../vdec/hal/inc/halCHIP.h"
// Internal Definition
//#include "regCHIP.h"
#include "drvMMIO.h" //get RIU base
#include "../../drv/inc/osalNJPD.h"
#include "../../drv/inc/drvNJPD.h"
#include "regNJPD.h"
#include "halNJPD.h"
...
static void _HAL_NJPD_SetMIUProtectMask(MS_BOOL bEnable)
{
    ...
    if(eNJPDNum == E_NJPD_NJPD1)
    {
        ...
        NJPD_DEBUG_HAL_MSG("%s MIU0(0x%x) : 0x%04x\n", __FUNCTION__, NJPD_MIU0_RQ2_MASK, __HAL_NJPD_Read2Byte(NJPD_MIU0_RQ2_MASK));
        NJPD_DEBUG_HAL_MSG("%s MIU1(0x%x) : 0x%04x\n", __FUNCTION__, NJPD_MIU1_RQ2_MASK, __HAL_NJPD_Read2Byte(NJPD_MIU1_RQ2_MASK));
    }
    else
    {
        __HAL_NJPD_WriteBit(NJPD_MIU0_RQ2_MASK, bEnable, NJPD0_MIU0_CLIENT_NJPD);
...
void HAL_NJPD_SetMIU(MS_U8 u8Idx)
{
    ...
    if(eNJPDNum == E_NJPD_NJPD1)
    {
        ...
        NJPD_DEBUG_HAL_MSG("%s NJPD0(0x%x) : 0x%04x\n", __FUNCTION__, BK_NJPD1_IBUF_READ_LENGTH, u16Reg);
    }
    else
    {
        ...
        NJPD_DEBUG_HAL_MSG("%s NJPD1(0x%x) : 0x%04x\n", __FUNCTION__, BK_NJPD2_IBUF_READ_LENGTH, u16Reg);
    }
...
void HAL_NJPD_PowerOn()
{
    ...
    u16reg_val = u16reg_val & 0xFFF0;
    u16reg_val |= 0x0004;  // set NJPD disable [0] : 0, [3:2] : 01 432MHz
...
// 9. ECO new error concealment(reg_spare00[9])      // Nike have this bit
    ...
    else
    {
        if(HAL_NJPD_IsNeedToPatch(E_NJPD_EAGLE_SW_PATCH))
        {
            ...
            u16Value = (HAL_NJPD_Get_MARB07() | 0xc01f);
...
void HAL_NJPD_ResetMarb(void)
{
    ...
    if(eNJPDNum == E_NJPD_NJPD1)
    {
        NJPD_DEBUG_HAL_MSG("set BK_NJPD1_MARB_RESET(0x%x) to 0x%02x\n", BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) & 0xFE);
        __HAL_NJPD_WriteByte(BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) & 0xFE);
        NJPD_DEBUG_HAL_MSG("set BK_NJPD1_MARB_RESET(0x%x) to 0x%02x\n", BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) | 0x01);
        __HAL_NJPD_WriteByte(BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) | 0x01);
        NJPD_DEBUG_HAL_MSG("set BK_NJPD1_MARB_RESET(0x%x) to 0x%02x\n", BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) & 0xFD);
        __HAL_NJPD_WriteByte(BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) & 0xFD);
        NJPD_DEBUG_HAL_MSG("set BK_NJPD1_MARB_RESET(0x%x) to 0x%02x\n", BK_NJPD1_MARB_RESET, __HAL_NJPD_ReadByte(BK_NJPD1_MARB_RESET) | 0x02);
