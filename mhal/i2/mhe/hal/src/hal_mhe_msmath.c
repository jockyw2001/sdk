...
// ------------------------------------------------
static uint32 _get_pow_of_two(int x)
{
    //int val = x;
    int p = -1;//, bit_pos = 0;
    while(x > 0)
...
uint32 MhefSqrt(uint32 x)
{
    ...
    int bit_num = _get_pow_of_two(x >> 16);
...
uint32 MhefExp(int exponment)
{
    ...
    if(rst == 0 && exponment < 0)
    {
        CDBZ(rst,"rst");
        MHE_MSG(MHE_MSG_WARNING, "rst(%lld), exponment(%d)\n", rst, exponment);
    }
    ...
    if (rst>UINT_MAX){
        //MHE_MSG(MHE_MSG_WARNING, "rst(%lld), UINT_MAX(%u)\n", rst, UINT_MAX);
        rst = UINT_MAX;
    }
...
static uint32 fLog2_range1_to_2(uint32 x)
{
    ...
    if(x == 0)
    {
        //printf("Error input x in MheMhefLog()");
...
static uint32 fPow_frac(uint32 A, int exponment)
{
    ...
    //assert(abs_Y_x_abs_ln_A<=INT_MAX,"bs_Y_x_abs_ln_A>INT_MAX");
    if (abs_Y_x_abs_ln_A > INT_MAX){
        //MHE_MSG(MHE_MSG_WARNING, "abs_Y_x_abs_ln_A(%lld), INT_MAX(%u)\n", abs_Y_x_abs_ln_A, INT_MAX);
        abs_Y_x_abs_ln_A = INT_MAX;
    }

    Y_x_ln_A = (sign_Y_x_ln_A) ? -abs_Y_x_abs_ln_A : abs_Y_x_abs_ln_A;

    ret = MhefExp((int)Y_x_ln_A);
    return ret;
}
...
static uint64 fPow64_int(uint32 base, int exponment)
{
    ...
    if(rst == 0 && exponment < 0)
    {
        MHE_MSG(MHE_MSG_WARNING, "rst(%llu), exponment(%d), n(%d)\n", rst, exponment, n);
    }
