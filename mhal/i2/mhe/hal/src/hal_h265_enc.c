...
#define MHE_HEVC_LEVEL HEVC_LEVEL_5_1

#if defined(MODULE_PARAM_SUPPORT)
extern unsigned int fdc_dbg;
#endif
    ...
    ushort  nuh)
{
    ...
    if(nuh == NUH_PACK(IDR_W_RADL, 0, 0))
        bits_cnt = 40;
    else
        bits_cnt = 48;
    /* start code */
    ...
    *bs->curr++ = (uchar)(nuh >> 8);
    *bs->curr++ = (uchar)(nuh + 1);
    bs->bits_pos += bits_cnt;   //48; derek check
    bs->left_cnt = 16;
    bs->code_bit = 0;
    bs->zero_cnt = 0;
    return bits_cnt;//48; derek check
}

#define _put_ue(v,coment)       bs_write_ue(bs,(uint)v,coment)
#define _put_se(v,coment)       bs_write_se(bs,( int)v,coment)
#define _put_u1(v,coment)       bs_write_u1(bs,(uint)v,coment)
#define _put(l, v,coment)       bs_write(bs, l,(uint)v,coment)
#define _put_32(v,coment)       bs_write_32(bs,(uint)v,coment)
    ...
    int     sublayers)
{
    ...
    for(i = 0; i < 32; i++)
    {
        //derek check, match c model
        //if(i == 1 || i == 2)
        if(i == ptl->i_profile)
            _put_u1(1,                                      "profile_compatibility_flag");
        else
            _put_u1(0,                                      "profile_compatibility_flag");
    }

    _put_u1(/*!ptl->b_interlaced*/0,                         "progressive_source_flag");
    ...
    sps_t*  sps)
{
    ...
    _put_ue(sps->i_max_tr_hierarchy_depth_inter - 1,            "max_transform_hierarchy_depth_inter");
    _put_ue(sps->i_max_tr_hierarchy_depth_intra - 1,            "max_transform_hierarchy_depth_intra");
...
int get_aspect_ratio_idc(int, int);
    ...
    vui_t*  vui)
{
    ...
    else
    {
        ...
        _put(8, vui->i_aspect_ratio_idc/*sar_idc*/,         "aspect_ratio_idc");
    ...
    _put_u1(vui->b_overscan_info_present_flag/*0*/,         "overscan_info_present_flag");
    if (vui->b_overscan_info_present_flag)
    {
        _put_u1(vui->b_overscan_appropriate_flag,           "b_overscan_appropriate_flag");
    }
    ...
    if(vui->b_timing_info_pres)
    {
        ...
        _put_u1(1,                                          "poc_proportional_to_timing_flag");
        _put_ue(vui->i_num_ticks_poc_diff_one_minus1,       "vui_num_ticks_poc_diff_one_minus1");
    ...
    h265_enc* h265)
{
...
#if defined(MODULE_PARAM_SUPPORT)
    if(fdc_dbg)
        CamOsPrintf("-------------(slice header)-----------------\n");
#endif

    h265_reset_bs(bs, h265->m_swsh, SLICE_HEADER_BIAS_SIZE);

    if(sh->i_slice_type == SLICE_TYPE_I && sh->i_poc == 0)
        sh->i_nal_type = IDR_W_RADL;    //NAL_UNIT_CODED_SLICE_IDR_W_RADL;
    else
        sh->i_nal_type = TRAIL_R;       //NAL_UNIT_CODED_SLICE_TRAIL_R;

    sp_nal_size = bs_write_nal_sp(bs, NUH_PACK(sh->i_nal_type, 0, 0));
    ...
    if(sh->i_nal_type != IDR_W_RADL && sh->i_nal_type != IDR_N_LP)
    {
        _put(sps->i_log2_max_poc_lsb, (uint)sh->i_poc & ((1 << sps->i_log2_max_poc_lsb) - 1),
...
//                }
    ...
    {
//        _put_u1(sh->b_active_override,                      "num_ref_idx_active_override_flag");
        _put_u1(0,                                              "num_ref_idx_active_override_flag");
        if(pps->b_cabac_init_pres)
        {
//            _put_u1(sh->b_cabac_init,                       "cabac_init_flag");
            _put_u1(0,                                          "cabac_init_flag");
        }
        _put_ue(5 - sh->i_max_num_merge_cand,               "five_minus_max_num_merge_cand");
//        _put_ue(1,                                              "five_minus_max_num_merge_cand");
    }

    _put_se(sh->i_qp - pps->i_init_qp,                          "slice_qp_delta");

    if(pps->b_slice_chroma_qp_offsets_pres)
    {
        _put_se(pps->i_cb_qp_offset,                          "slice_cb_qp_offset");
        _put_se(pps->i_cr_qp_offset,                          "slice_cr_qp_offset");
    }
    ...
    if(pps->b_loop_filter_across_slices_enabled && !pps->b_deblocking_filter_disabled)
    {
        sh->slice_loop_filter_across_slices_enabled_flag = 1;
    }

    _put_u1(sh->slice_loop_filter_across_slices_enabled_flag,   "slice_loop_filter_across_slices_enabled_flag");

    bs_trail(bs);
    h265_flush_bs(bs);

    sh_size = (bs->bits_pos - bpos) / 8 + sp_nal_size / 8;

#if 0
    //check last byte is 0x80,and remove 0x80 (fixed deblocking disable cause image borken)
    if(bs->buff[sh_size-1] == 0x80)
    {
        bs->buff[sh_size-1] = 0x00;
        sh_size--;
    }
#endif
    //CamOsHexdump(bs->buff, sh_size);

    return sh_size;
    //return (bs->bits_pos - bpos) / 8 + sp_nal_size / 8; //6; //derek check
}
...
static sps_t sps_def =
{
    ...
    .i_log2_max_cb_size = 5,    //6,    derek check, match c model
    .i_log2_min_tr_size = 2,    //6,    derek check, match c model
    .i_log2_max_tr_size = 5,    //6,    derek check, match c model
    .i_max_tr_hierarchy_depth_inter = 3,
    .i_max_tr_hierarchy_depth_intra = 3,
    ...
    /* vui params */
    .b_vui_param_pres = 0,  //1, derek check
    .vui = {
...
{
    .i_id = 0,
    .b_cabac_init_pres = 0,//1, derek check
    ...
    .b_deblocking_filter_control_pres = 0, //derek check
    .b_deblocking_filter_override_enabled = 0,
    .b_deblocking_filter_disabled = 1,
    .i_beta_offset = 0,     //10,
    .i_tc_offset = 0,       //-4,
...
static slice_t sh_def =
{
    ...
    .b_active_override = 1,
    .b_cabac_init = 0,
    .i_max_num_merge_cand = 5,//4, derek check
...
static void _release(void* vptr)
{
    MEM_FREE(vptr);
}
...
h265enc_acquire(void)
{
    ...
    do
    {
        ...
        if(!(h265 = MEM_ALLC(sizeof(h265_enc))))
            return NULL;
    ...
    int         rpbn)
{
    ...
    {
        h265->m_pics[i].i_id = i;  //derek add for test
    ...
    h265_enc*   h265)
{
...
#if 1  //derek check, match c model
    ...
    if(sps->b_conformance_window)
    {
        ...
        sps->i_crop_r = (sps->i_pic_w_min_cbs - h265->i_picw) >> 1; //derek check
        sps->i_crop_b = (sps->i_pic_h_min_cbs - h265->i_pich) >> 1; //derek check
    }
#endif
    h265->i_poc = 0;
    ...
    int         refp)
{
    ...
    if(rec)
    {
        ...
        rec->i_type = type;
        //CamOsPrintf("%s %d:<idx-%d>-poc(%d), ref.(%d), keep(%d), cached(%d)\n",__FUNCTION__,__LINE__,rec->i_id,rec->i_poc,rec->b_reference,rec->b_ltr_keep,rec->b_cached);
        return rec->i_id;
    }
    ...
    MHE_MSG(MHE_MSG_ERR,"MHE-found null recn buffer\n");
    return -1;
}
    ...
    h265_enc*   h265)
{
...
#if defined(HEVC_RPS_PRESET)
    pic = _rps_reset(h265, h265->i_poc + 1, !h265->b_refp);

    if(pic)
    {
        pic->i_poc = POC_BUFFER_FREE;
        idx = pic->i_id;
        //MHE_MSG(MHE_MSG_ERR,"clear buf[%d]-poc(%d), ref.(%d), keep(%d), cached(%d)\n", pic->i_id, pic->i_poc, pic->b_reference, pic->b_ltr_keep, pic->b_cached);
    }
    else
    {
        //first and second frame don't be checked.
        if(h265->i_poc > 1)
            MHE_MSG(MHE_MSG_ERR,"MHE-return buffer is null\n");
    }
#endif
    h265->p_recn = NULL;
    ...
    {
        pic = h265->m_pics+i;
        //MHE_MSG(MHE_MSG_ERR,"[%d<idx-%d>]-poc(%d), ref.(%d), keep(%d), cached(%d)\n", i, pic->i_id, pic->i_poc, pic->b_reference, pic->b_ltr_keep, pic->b_cached);
    }
#if 0
    for (i = 0, pic = h265->m_pics; i < MAX_RPB_SIZE; i++, pic++)
    {
    ...
    for (i = 0, pic = h265->m_pics; i < h265->i_rpbn; i++, pic++)
    {
        ...
        if (IS_POC_FREE(pic->i_poc) && !pic->b_reference && !pic->b_cached)
        {
            if (!h265->i_poc && pic->b_ltr_keep)
                MHE_MSG(MHE_MSG_ERR,"%s(idx-%d)-ref.(%d)/keep(%d)/cached(%d)\n", __func__, i, pic->b_reference, pic->b_ltr_keep, pic->b_cached);
            //MHE_MSG(MHE_MSG_ERR,"return <idx-%d>-poc.(%d)-ref.(%d)-keep(%d)-cached(%d)\n", i, pic->i_poc, pic->b_reference, pic->b_ltr_keep, pic->b_cached);
            return pic;
        }
    }

    MHE_MSG(MHE_MSG_ERR,"%s()-found null picture buffer(idx<%d>\n", __func__, i);
    for (i = 0, pic = h265->m_pics; i < MAX_RPB_SIZE; i++, pic++)
    {
        MHE_MSG(MHE_MSG_ERR,"m_pics[%d-idx<%d>] poc<%d>, ref.<%d>, keep<%d>, cached(%d)\n", i, pic->i_id, pic->i_poc, pic->b_reference, pic->b_ltr_keep, pic->b_cached);
    }
    ...
    int         rpsi)
{
    ...
    for (i = 0; i < rps->i_num_neg_pics + rps->i_num_pos_pics; i++)
    {
        ...
        if (!pic)
        {
            MHE_MSG(MHE_MSG_ERR,"MHE-%s()null ref. pic and return\n", __func__);
            return pic;
        }
    ...
    //if(out)
    //    MHE_MSG(MHE_MSG_ERR,"%d <idx-%d>-poc(%d), ref.(%d), keep(%d), cached(%d)\n",__LINE__,out->i_id,out->i_poc,out->b_reference,out->b_ltr_keep,out->b_cached);
    return out;
}
