...
struct pic_t
{
    short   i_id;           // pic id (ltr buffer if id < 0)
...
#define HEVC_LTRPSLICE      2
    short   i_type;         // HEVC_ISLICE/HEVC_PSLICE/HEVC_LTRPSLICE
    short   b_reference;
...
typedef struct h265_enc
{
    ...
    /* nalu information */
    int     i_seqh;                             // VPS+SPS+PPS size length in by
...
void* h265_find_rpl(h265_enc*, int, int);   /* find reference frame */
void* h265_find_set(h265_enc*, int, int);   /* find SPS/PPS/VPS/Slice */
int   h265_seq_init(h265_enc*, int);        /* sequence initialize */
int   h265_seq_sync(h265_enc*);             /* sequence synchronous */
int   h265_seq_conf(h265_enc*);             /* sequence configure(write SPS/PPS/VPS header) */
int   h265_seq_done(h265_enc*);             /* NULL function */
int   h265_enc_buff(h265_enc*, int, int);   /* find recn buffer idx */
int   h265_enc_done(h265_enc*);             /* return available can be free buffer */
int   h265_sh_parser(h265_enc*);            /* slice header parser */
int   h265_sh_writer(h265_enc*);            /* slice header writer */

int   h265_write_padding_head(void* buff, int len); /* write an RSV_NVCL41 type slice */
