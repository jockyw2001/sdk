
#ifndef _PMBR_CFG_H_
#define _PMBR_CFG_H_

#define PMBR_MEM_MB_MAP_IN  0
#define PMBR_MEM_MB_MAP_OUT 1
#define PMBR_MEM_LUT        2
#define PMBR_MEM_NUM        3

#define PMBR_LUT_QP             0
#define PMBR_LUT_TARGET         1
#define PMBR_LUT_LAMBDA_SCALED  2
#define PMBR_LUT_AIIDCHIST      3
#define PMBR_LUT_NUM            4

// Look-up Table Size
#define PMBR_LUT_SIZE       15
#define PMBR_LOG2_HIST_SIZE 5

// minimun bin number threshold for perceptual MBR enable
#define PMBR_BIN_NUM_THR 5

typedef union pmbr_cfg
{
    enum pmbr_cfg_e
    {
        PMBR_CFG_SEQ = 0,
        PMBR_CFG_TC,
        PMBR_CFG_PC,
        PMBR_CFG_SS,
        PMBR_CFG_MEM,
        PMBR_CFG_LUT,
        PMBR_CFG_FRM
    } type;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_SEQ"
        int             i_enable;   /* PMBR Enable */
        int             i_ctuw;     /* CTU Width */
        int             i_ctuh;     /* CTU Height */
    } seq;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_TC"
        int             i_LutEntryClipRange;
        int             i_TextWeightType;
        int             i_SkinLvShift;
        int             i_WeightOffset;
        int             i_TcOffset;
        int             i_TcToSkinAlpha;
        int             i_TcGradThr;
    } tc;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_PC"
        int             i_PwYMax;
        int             i_PwYMin;
        int             i_PwCbMax;
        int             i_PwCbMin;
        int             i_PwCrMax;
        int             i_PwCrMin;
        int             i_PwCbPlusCrMin;
        int             i_PwAddConditionEn;
        int             i_PwCrOffset;
        int             i_PwCbCrOffset;
        int             i_PcDeadZone;
    } pc;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_SS"
        int             i_SmoothEn;
        int             i_SmoothClipMax;
        int             i_SecStgAlpha;
        int             i_SecStgBitWghtOffset;
    } ss;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_MEM"
        unsigned int    u_phys[PMBR_MEM_NUM];
        void*           p_kptr[PMBR_MEM_NUM];
        int             i_size[PMBR_MEM_NUM];
    } mem;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_LUT"
        unsigned int*   p_kptr[PMBR_LUT_NUM];
        int             i_size[PMBR_LUT_NUM];
    } lut;

    struct
    {
        enum pmbr_cfg_e i_type;     //!< MUST BE "PMBR_CFG_FRM"
        int             i_cb_qp_offset;   /* PPS Cb QP Offset */
        int             i_cr_qp_offset;   /* PPS Cr QP Offset */
    } frm;
} pmbr_cfg;

#endif //_PMBR_CFG_H_
