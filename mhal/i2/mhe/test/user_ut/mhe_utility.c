...
{
    printf("please select item to set mhe I2..\n");
    printf("===============================\n");
    printf("[1] set video encoder basic settings\n");
    printf("[2] set h265 deblocking\n");
    printf("[3] set h265 disposable mode\n");
    printf("[4] set h265 poc type\n");
    printf("[5] set h265 entropy\n");
    printf("[6] set h265 rate control\n");
    printf("[7] set h265 intra prediction\n");
    printf("[8] set h265 inter prediction\n");
    printf("[9] set h265 slice split\n");
    printf("[10] set h265 roi\n");
    printf("[11] insert user data\n");
    printf("[12] set h265 New Sequence\n");
    printf("[13] set h265 clock setting\n");
    printf("[14] set h265 VUI\n");
    printf("[15] set h265 LTR\n");
    printf("[0] exit\n");
    printf("===============================\n");
}
...
void getROISettings(int idx, char* name, char* value, Mheparam* p)
{
    int i = idx;
...
void GenSEINALU(Mheparam* p, int idx, char* value)
{
    int i = 0;
...
int ReadDefaultConfig(Mheparam* p)
{
    ...
    nInstanceId = p->nInstanceId;
    //fp = fopen("avc.cfg","rb");
    //printf("--> Read Cfg file: %s\n",p->cfgName);
    strcpy(cfgName, p->cfgName);
    fp = fopen(cfgName, "rb");

    memset(p, 0, sizeof(Mheparam));

    //restore parameters
    strcpy(p->cfgName, cfgName);
    p->nInstanceId = nInstanceId;

    if(fp == NULL)
    {
        printf("%s %d: %s [read config fail]\n", __func__, __LINE__, cfgName);
        return -1;
    }
    ...
    while(fgets(s, 128, fp) != NULL)
    {
        ...
        if(!p->sTrans.bEn)
        {
            if(!strncmp(name, "#Trans Settings", strlen("#Trans Settings")))
            {
                p->sTrans.bEn = atoi(value);
            }
        }
    ...
    while(fgets(s, 128, fp) != NULL)
    {
        ...
        if(p->sDeblk.bEn)
        {
            if(!strncmp(name, "b_deblocking_cross_slice_enable", strlen("b_deblocking_cross_slice_enable")))
            {
                p->sDeblk.b_deblocking_cross_slice_enable = atoi(value);
            }
            else if(!strncmp(name, "b_deblocking_disable", strlen("b_deblocking_disable")))
            {
                p->sDeblk.b_deblocking_disable = atoi(value);
            }
            else if(!strncmp(name, "slice_tc_offset_div2", strlen("slice_tc_offset_div2")))
            {
                p->sDeblk.slice_tc_offset_div2 = atoi(value);
            }
        ...
        if(p->sTrans.bEn)
        {
            if(!strncmp(name, "qpoffset", strlen("qpoffset")))
            {
                p->sTrans.nQpOffset = atoi(value);
            }
        }
        ...
        if(p->sIntraP.bEn)
        {
            ...
            else if(!strncmp(name, "bIntraCu8Lose", strlen("bIntraCu8Lose")))
            {
                p->sIntraP.bIntraCu8Lose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu8Lose", strlen("bInterCu8Lose")))
            {
                p->sIntraP.bInterCu8Lose = atoi(value);
            }
            else if(!strncmp(name, "bIntraCu16Lose", strlen("bIntraCu16Lose")))
            {
                p->sIntraP.bIntraCu16Lose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu16Lose", strlen("bInterCu16Lose")))
            {
                p->sIntraP.bInterCu16Lose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu16SkipLose", strlen("bInterCu16SkipLose")))
            {
                p->sIntraP.bInterCu16SkipLose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu16MergeLose", strlen("bInterCu16MergeLose")))
            {
                p->sIntraP.bInterCu16MergeLose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu16MergeSkipLose", strlen("bInterCu16MergeSkipLose")))
            {
                p->sIntraP.bInterCu16MergeSkipLose = atoi(value);
            }
            else if(!strncmp(name, "bIntraCu32Lose", strlen("bIntraCu32Lose")))
            {
                p->sIntraP.bIntraCu32Lose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu32MergeLose", strlen("bInterCu32MergeLose")))
            {
                p->sIntraP.bInterCu32MergeLose = atoi(value);
            }
            else if(!strncmp(name, "bInterCu32MergeSkipLose", strlen("bInterCu32MergeSkipLose")))
            {
                p->sIntraP.bInterCu32MergeSkipLose = atoi(value);
            }
            else if(!strncmp(name, "nIntraCu8Pen", strlen("nIntraCu8Pen")))
            {
                p->sIntraP.nIntraCu8Pen = atoi(value);
            }
            else if(!strncmp(name, "nInterCu8Pen", strlen("nInterCu8Pen")))
            {
                p->sIntraP.nInterCu8Pen = atoi(value);
            }
            else if(!strncmp(name, "nIntraCu16Pen", strlen("nIntraCu16Pen")))
            {
                p->sIntraP.nIntraCu16Pen = atoi(value);
            }
            else if(!strncmp(name, "nInterCu16Pen", strlen("nInterCu16Pen")))
            {
                p->sIntraP.nInterCu16Pen = atoi(value);
            }
            else if(!strncmp(name, "nInterCu16MergePen", strlen("nInterCu16MergePen")))
            {
                p->sIntraP.nInterCu16MergePen = atoi(value);
            }
            else if(!strncmp(name, "nInterCu16SkipPen", strlen("nInterCu16SkipPen")))
            {
                p->sIntraP.nInterCu16SkipPen = atoi(value);
            }
            else if(!strncmp(name, "nInterCu16MergeSkipPen", strlen("nInterCu16MergeSkipPen")))
            {
                p->sIntraP.nInterCu16MergeSkipPen = atoi(value);
            }
            else if(!strncmp(name, "nIntra32Pen", strlen("nIntra32Pen")))
            {
                p->sIntraP.nIntra32Pen = atoi(value);
            }
            else if(!strncmp(name, "nIntra32MergePen", strlen("nIntra32MergePen")))
            {
                p->sIntraP.nIntra32MergePen = atoi(value);
            }
            else if(!strncmp(name, "nIntra32MergeSkipPen", strlen("nIntra32MergeSkipPen")))
            {
                p->sIntraP.nIntra32MergeSkipPen = atoi(value);
            }
...
void ShowCurrentMHESetting(Mheparam* pParam)
{
    ...
    {
        printf("-b_deblocking_cross_slice_enable: %d\n", pParam->sDeblk.b_deblocking_cross_slice_enable);
        printf("-b_deblocking_disable: %u\n", pParam->sDeblk.b_deblocking_disable);
        printf("-slice_tc_offset_div2: %d\n", pParam->sDeblk.slice_tc_offset_div2);
    ...
    {
        printf("-QpOffset: %d\n", pParam->sTrans.nQpOffset);
    }

    if(pParam->sRC.bEn)
    {
        printf("-eControlRate: %d (0:ConstQp 1:CBR 2:VBR)\n", pParam->sRC.eControlRate);
        if(pParam->sRC.eControlRate == Mhe_RQCT_METHOD_VBR)
        {
    ...
    if(pParam->sIntraP.bEn)
    {
        ...
        printf("-bIntraCu8Lose: %d\n", pParam->sIntraP.bIntraCu8Lose);
        printf("-bInterCu8Lose: %d\n", pParam->sIntraP.bInterCu8Lose);
        printf("-bIntraCu16Lose: %d\n", pParam->sIntraP.bIntraCu16Lose);
        printf("-bInterCu16Lose: %d\n", pParam->sIntraP.bInterCu16Lose);
        printf("-bInterCu16MergeLose: %d\n", pParam->sIntraP.bInterCu16MergeLose);
        printf("-bInterCu16SkipLose: %d\n", pParam->sIntraP.bInterCu16SkipLose);
        printf("-bInterCu16MergeSkipLose: %d\n", pParam->sIntraP.bInterCu16MergeSkipLose);
        printf("-bIntraCu32Lose: %d\n", pParam->sIntraP.bIntraCu32Lose);
        printf("-bInterCu32MergeLose: %d\n", pParam->sIntraP.bInterCu32MergeLose);
        printf("-bInterCu32MergeSkipLose: %d\n", pParam->sIntraP.bInterCu32MergeSkipLose);
        printf("-nIntraCu8Pen: %d\n", pParam->sIntraP.nIntraCu8Pen);
        printf("-nInterCu8Pen: %d\n", pParam->sIntraP.nInterCu8Pen);
        printf("-nIntraCu16Pen: %d\n", pParam->sIntraP.nIntraCu16Pen);
        printf("-nInterCu16Pen: %d\n", pParam->sIntraP.nInterCu16Pen);
        printf("-nInterCu16MergePen: %d\n", pParam->sIntraP.nInterCu16MergePen);
        printf("-nInterCu16SkipPen: %d\n", pParam->sIntraP.nInterCu16SkipPen);
        printf("-nInterCu16MergeSkipPen: %d\n", pParam->sIntraP.nInterCu16MergeSkipPen);
        printf("-nIntra32Pen: %d\n", pParam->sIntraP.nIntra32Pen);
        printf("-nIntra32MergePen: %d\n", pParam->sIntraP.nIntra32MergePen);
        printf("-nIntra32MergeSkipPen: %d\n", pParam->sIntraP.nIntra32MergeSkipPen);
    }

    if(pParam->sInterP.bEn)
    {
        printf("-nDmv_X: %u nDmv_Y: %u\n", pParam->sInterP.nDmv_X, pParam->sInterP.nDmv_Y);
    ...
    for(i = 0; i < MAX_ROI_NUM; i++)
    {
        ...
        {
            printf("-Idx: %u Enable: %d (X,Y)->(%u,%u) (W,H)->(%u,%u) Qp:%d AbsQp:%d\n", pParam->sRoi[i].nIdx, pParam->sRoi[i].bEnable,
    ...
    {
        printf("-Clock Disable: %u\n", pParam->sClock.nClkEn);
        printf("-Clock Source: %u (0:288, 1:240, 2:192, 3:123, 4:384, 5:320, 6:172 Mhz)\n", pParam->sClock.nClkSor);
    }
...
void SetVideoBasicSettings(Mheparam* pParam)
{
...
void SetH265LTR(Mheparam* pParam){
    int a,b,c;
    printf("> nBase(1 ~ N): ");
    if(scanf("%d",&a) != EOF){
        pParam->sLTR.nBase = a;
...
void SetH265Dblk(Mheparam* pParam)
{
    int a, b, c, d;
    printf("> b_deblocking_cross_slice_enable(0/1): ");
    if(scanf("%d", &a) != EOF)
    {
        pParam->sDeblk.b_deblocking_cross_slice_enable = a;
    }
    printf("> b_deblocking_disable(0/1/): ");
    if(scanf("%d", &b) != EOF)
    {
        pParam->sDeblk.b_deblocking_disable = b;
    }
    printf("> slice_tc_offset_div2(-6~6): ");
    if(scanf("%d", &c) != EOF)
    {
        pParam->sDeblk.slice_tc_offset_div2 = c;
    }
...
void SetH265Disposable(Mheparam* pParam)
{
    int a;
...
void SetH265Poc(Mheparam* pParam)
{
    int a;
    printf("> Poc type(0/2): ");
    if(scanf("%d", &a) != EOF)
...
void SetH265Trans(Mheparam* pParam)
{
    int a;
    printf("> QP offset (-12 ~ 12): ");
    if(scanf("%d", &a) != EOF)
    {
        pParam->sTrans.nQpOffset = a;
    }
    pParam->sTrans.bEn = TRUE;
    printf("Done!\n");
}


void SetH265RC(Mheparam* pParam)
{
    int a;
...
void SetH265IntraP(Mheparam* pParam)
{
    int a;
...
void SetH265InterP(Mheparam* pParam)
{
...
void SetH265SS(Mheparam* pParam)
{
    int a, b;
...
void SetH265ROI(Mheparam* pParam)
{
    int a, b, c, d, e, f, g, h;
...
void SetH265QPMAP(Mheparam* pParam)
{
    int a, b, c;
...
void SetH265SEI(Mheparam* pParam)
{
    char pstr[1024];
    int a = 0;
    int i = 0;
...
void SetH265VUI(Mheparam* pParam)
{
    int a, b, c, d, e, f, g;
    printf("> u16SarWidth: ");
    if(scanf("%d", &a) != EOF)
...
void SetH265NewSeq(Mheparam* pParam)
{
    int a, b, c;
    printf("> NewSeq Width:");
    if(scanf("%d", &a) != EOF)
...
void SetVideoClkSettings(Mheparam* pParam)
{
    int a, b;
    ...
    do
    {
        ...
        switch(num)
        {
            ...
            case 6:
                ...
                break;
            case 7:
                SetH265IntraP(pParam);
                break;
            case 8:
                SetH265InterP(pParam);
                break;
            case 9:
                SetH265SS(pParam);
                break;
