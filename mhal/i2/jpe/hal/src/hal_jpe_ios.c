...
static JpeHalOutBufCfg_t tCurrentOutBufCfg;
...
{
    addr = (u32)ADDR_TYPE_SELECT((void *)addr);
...
static void _HalJpeSetOutBuf(u32 nBaseAddr, JpeReg_t* pJpeReg, JpeHalOutBufCfg_t *pOutBufCfg)
{
    ...
    {
        if (!pOutBufCfg->nPartialNotifySize)
        {
        ...
        {
            __HalJpeSetOutBuf(nBaseAddr, pJpeReg,  pOutBufCfg->nOutBufAddr[0] + pOutBufCfg->nPartialOffset[0], 0);
            __HalJpeSetOutBuf(nBaseAddr, pJpeReg, pOutBufCfg->nOutBufAddr[0] + pOutBufCfg->nPartialOffset[0] + pOutBufCfg->nPartialNotifySize - 1, 1);
            //JPE_MSG(JPE_MSG_DEBUG, "Set[0] 0x%08X, size %d\n", pOutBufCfg->nOutBufAddr[0] + pOutBufCfg->nPartialOffset[0], pOutBufCfg->nPartialNotifySize);
        }
    }
    ...
    if(pOutBufCfg->nOutBufAddr[1] != 0 && pOutBufCfg->nOutBufSize[1] != 0)
    {
        ...
        {
            __HalJpeSetOutBuf(nBaseAddr, pJpeReg,  pOutBufCfg->nOutBufAddr[1] + pOutBufCfg->nPartialOffset[1], 0);
            __HalJpeSetOutBuf(nBaseAddr, pJpeReg, pOutBufCfg->nOutBufAddr[1] + pOutBufCfg->nPartialOffset[1] + pOutBufCfg->nPartialNotifySize - 1, 1);
            //JPE_MSG(JPE_MSG_DEBUG, "Set[1] 0x%08X, size %d\n", pOutBufCfg->nOutBufAddr[1] + pOutBufCfg->nPartialOffset[1], pOutBufCfg->nPartialNotifySize);
        }
    }
...
static void _HalJpeSetInBuf(JpeHalHandle_t* handle, pJpeHalInBufCfg ptInBufCfg)
{
    ...
    {
        addr = (u32)ADDR_TYPE_SELECT((void *)ptInBufCfg->nInBufYAddr[0]);
    }
    else
    {
        addr = 0;
    ...
    {
        addr = (u32)ADDR_TYPE_SELECT((void *)ptInBufCfg->nInBufYAddr[1]);
    }
    else
    {
        addr = 0;
    ...
    {
        addr = (u32)ADDR_TYPE_SELECT((void *)ptInBufCfg->nInBufCAddr[0]);
    }
    else
    {
        addr = 0;
    ...
    {
        addr = (u32)ADDR_TYPE_SELECT((void *)ptInBufCfg->nInBufCAddr[1]);
    }
    else
    {
        addr = 0;
...
static JPE_IOC_RET_STATUS_e _EncFire(void* pIosIn, void* pJpeHalHandleIn, JpeJob_t* pJob)
{
    ...
    JpeHalOutBufCfg_t *pJpeCurrOutBufCfg = &pIos->tJpeHalOutBufCfg;
    JpeHalInBufCfg_t *pJpeCurrInBufCfg = &pIos->tJpeHalInBufCfg;
...
#define _UPDATE_REG_VAR(nBaseAddr,reg_no) pJpeReg->uReg##reg_no.nReg##reg_no = REGR(nBaseAddr,0x##reg_no, "\n");//REGR(0x##reg_no, "\n");
    ...
    memcpy(&tCurrentOutBufCfg, pJpeHalOutBufCfg, sizeof(tCurrentOutBufCfg));
    memcpy(pJpeCurrOutBufCfg, &pJpeHalHandle->tJpeHalOutBufCfg, sizeof(JpeHalOutBufCfg_t));
    pJpeCurrOutBufCfg->nOutBufAddr[0] = pJpeCurrOutBufCfg->nOutBufAddr[0] - pJpeCurrOutBufCfg->nHeaderLen;
    ...
    pJpeReg->uReg5e.tReg5e.nRegGJpeYPitch = (u16)((pJpeHalHandle->nPitchWidth+ 15) >> 4);
    pJpeReg->uReg5e.tReg5e.nRegGJpeCPitch = (u16)((pJpeHalHandle->nPitchWidth + 15) >> 4);
    pJpeReg->uReg62.tReg62.nRegJpeGYPitchMsb = (u16)(((pJpeHalHandle->nPitchWidth + 15) >> 4) >> 8);
    pJpeReg->uReg62.tReg62.nRegJpeGCPitchMsb = (u16)(((pJpeHalHandle->nPitchWidth + 15) >> 4) >> 8);
    ...
    REGW(nBaseAddr,0x62, pJpeReg->uReg62.nReg62, "[%x] reg[%x] : picture pitch msb\n");
    ...
    memcpy(pJpeCurrInBufCfg, &pJpeHalHandle->tJpeHalInBufCfg, sizeof(JpeHalInBufCfg_t));


#if 0   //VERIFY_EVD_HVD_ROW MODE_COSIM
    pJpeReg->uReg00.tReg00.nRegJpeGSoftHskRstz = 0;
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 1\n");
    // delay for lower MCM frequence
    // Repeated write to replace udelay(1) becase 1 us is too long
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 0\n");
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 0\n");
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 0\n");
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 0\n");
    pJpeReg->uReg00.tReg00.nRegJpeGSoftHskRstz = 1;
    REGW(nBaseAddr, 0x0, pJpeReg->uReg00.nReg00, "[%x] reg[%x] : hsk reset 1\n");

    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrImiBound = 0;
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrRowEn = 1;
#if 1   // EVD
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrTileEn = 1;
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrHskSel = 2;    // 2: EVD, 3: HVD
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrTileMode = 1;  // 0: 16x32(for HVD), 1: 32x16(for EVD)
#else   // HVD
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrTileEn = 1;
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrHskSel = 3;    // 2: EVD, 3: HVD
    pJpeReg->uReg61.tReg61.nRegJpeSYuvldrTileMode = 0;  // 0: 16x32(for HVD), 1: 32x16(for EVD)
#endif
    REGW(nBaseAddr, 0x61, pJpeReg->uReg61.nReg61, "[%x] reg[%x] : loader handshake select\n");
#endif
...
static JPE_IOC_RET_STATUS_e _IsrFunc(void* pIosIn, int msk)
{
    ...
    if(_HalJpeGetIrqCheck(nBaseAddr, pJpeReg, JPE_HAL_IRQ_BSPOBUF0_FULL))
    {
        ...
        if (tCurrentOutBufCfg.nPartialNotifySize)
        {
            /* Status update */
            if (!tCurrentOutBufCfg.nPartialOffset[0])
                pIos->nEncodeSize = _HalJpeGetOutputByteSize(nBaseAddr, pJpeReg, 0);
            else
                pIos->nEncodeSize += _HalJpeGetOutputByteSize(nBaseAddr, pJpeReg, 0);

            if (tCurrentOutBufCfg.notifyFunc)
            {
                tCurrentOutBufCfg.notifyFunc(tCurrentOutBufCfg.nOutBufAddr[0] - tCurrentOutBufCfg.nHeaderLen,
                                             tCurrentOutBufCfg.nPartialOffset[0],
                                             tCurrentOutBufCfg.nPartialNotifySize,
                                             0);
            }

            if (tCurrentOutBufCfg.nOutBufAddr[0] + tCurrentOutBufCfg.nOutBufSize[0] >=
                tCurrentOutBufCfg.nPartialOffset[0] + tCurrentOutBufCfg.nPartialNotifySize + tCurrentOutBufCfg.nPartialNotifySize)
            {
                pIos->eJpeDevStatus = JPE_DEV_BUSY;

                /* Config output address */
                tCurrentOutBufCfg.nPartialOffset[0] += tCurrentOutBufCfg.nPartialNotifySize;
                _HalJpeSetOutBuf(nBaseAddr, pJpeReg, &tCurrentOutBufCfg);
            }
        }
        else
        {
    ...
    else if(_HalJpeGetIrqCheck(nBaseAddr, pJpeReg, JPE_HAL_IRQ_BSPOBUF1_FULL))
    {
        ...
        if (tCurrentOutBufCfg.nPartialNotifySize)
        {
            ...
            if (tCurrentOutBufCfg.notifyFunc)
            {
                tCurrentOutBufCfg.notifyFunc(tCurrentOutBufCfg.nOutBufAddr[1],
                                             tCurrentOutBufCfg.nPartialOffset[1],
                                             tCurrentOutBufCfg.nPartialNotifySize,
                                             0);
            }

            if (tCurrentOutBufCfg.nOutBufAddr[1] + tCurrentOutBufCfg.nOutBufSize[1] >=
                tCurrentOutBufCfg.nPartialOffset[1] + tCurrentOutBufCfg.nPartialNotifySize + tCurrentOutBufCfg.nPartialNotifySize)
            {
                pIos->eJpeDevStatus = JPE_DEV_BUSY;

                /* Config output address */
                tCurrentOutBufCfg.nPartialOffset[1] += tCurrentOutBufCfg.nPartialNotifySize;
                _HalJpeSetOutBuf(nBaseAddr, pJpeReg, &tCurrentOutBufCfg);
            }
        }
        else
    ...
    else if(_HalJpeGetIrqCheck(nBaseAddr, pJpeReg, JPE_HAL_IRQ_FRAME_DONE))
    {
        ...
        {
            /* Status update */
            // TBD, if support double buffer, need judge frame done happen in which buffer
            nEncodeSize = _HalJpeGetOutputByteSize(nBaseAddr, pJpeReg, 0);
            if (!tCurrentOutBufCfg.nPartialOffset[0])
                pIos->nEncodeSize = nEncodeSize;
            else
                pIos->nEncodeSize += nEncodeSize;

            if (tCurrentOutBufCfg.nPartialNotifySize && tCurrentOutBufCfg.notifyFunc)
            {
                // TBD, if support double buffer, header length should be handled carefully
                tCurrentOutBufCfg.notifyFunc(tCurrentOutBufCfg.nOutBufAddr[0] - tCurrentOutBufCfg.nHeaderLen,
                                             tCurrentOutBufCfg.nPartialOffset[0],
                                             nEncodeSize,
                                             1);
...
static JPE_IOC_RET_STATUS_e _AddOutBuff(void* pIosIn, u32  phyBuffAddr, u32 phyBuffSize)
{
    JpeIos_t* pIos = (JpeIos_t*)pIosIn;
    u32 nBaseAddr = pIos->nBaseAddr;
    JpeReg_t* pJpeReg = &pIos->tJpeReg;
   JpeHalOutBufCfg_t *pJpeHalOutBufCfg = &pIos->tJpeHalOutBufCfg;
    ...
    pJpeHalOutBufCfg->nOutBufAddr[0] = phyBuffAddr;
    pJpeHalOutBufCfg->nOutBufSize[0] = phyBuffSize;
    _HalJpeSetOutBuf(nBaseAddr, pJpeReg, pJpeHalOutBufCfg);
    memcpy(&tCurrentOutBufCfg, pJpeHalOutBufCfg, sizeof(tCurrentOutBufCfg));
