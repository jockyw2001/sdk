...
#define JPE_MIAW 29//27//25//24
#define ADDR_HI_BITS  8
...
typedef struct
{
    ...
    union
    {
        struct
        {
            ...
            u16 nRegJpeGSoftHskRstz: 1;    // handshake register reset; 0/1: reset/not reset
    ...
    union
    {
        struct
        {
            ...
            u16 nRegJpeGSramGateEn: 1;  //0: sram clock gating disabled, 1: there's dynamic sram clock gating with sram cen
        } tReg16;
        u16 nReg16;
    } uReg16;
    ...
    union
    {
        struct
        {
            u16 nReg1802Dummy: 2;               // h18[0:1] dummy
            ...
            u16 nReg1804Dummy: 4;               // h18[11:14] dummy
    ...
    union
    {
        struct
        {
            u16 nRegJpeGIrqMask: 11;               // 0/1: irq not-mask/mask
        } tReg1c;
        u16 nReg1c;
    } uReg1c;
    ...
    union
    {
        struct
        {
            ...
            u16 nRegJpeGIrqClr9: 1;               // 0/1: not clear interrupt/clear interrupt 9 (write one clear)
            u16 nRegJpeGIrqClr10: 1;              // 0/1: not clear interrupt/clear interrupt 10 (write one clear)
#else
            u16 nRegJpeGIrqClr: 11;               // bitwise 0/1: not clear interrupt/clear interrupt 8 (write one clear)
#endif
    ...
    union
    {
        struct
        {
            u16 nRegJpeGIrqCpu: 11;               // status of interrupt on CPU side ({1'b0, netrigger, fs_fail_irq, txipime_out, bspobuf_full_irq, img_buf_full_irq, marb_bspobuf_ful, frame_done_irq})
        } tReg1e;
        u16 nReg1e;
    } uReg1e;
    ...
    union
    {
        struct
        {
            u16 nRegJpeGIrqIp: 11;               // status of interrupt on IP side ({1'b0, netrigger, fs_fail_irq, txipime_out, bspobuf_full_irq, img_buf_full_irq, marb_bspobuf_ful, frame_done_irq})
        } tReg1f;
        u16 nReg1f;
    } uReg1f;
    ...
    union
    {
        struct
        {
            u16 nRegJpeGIrqForce: 11;               // 0/1: set corresponding interrupt as usual/force corresponding interrupt
        } tReg20;
        u16 nReg20;
    } uReg20;
    ...
    union
    {
        struct
        {
            u16 nRegJpeSTxipMbx: 10;             //txip mbx
    ...
    union
    {
        struct
        {
            u16 nRegJpeSTxipMby: 10;               // txip mby
        } tReg2e;
        u16 nReg2e;
    } uReg2e;
    ...
    union
    {
        struct
        {
            u16 nRegMfeSTxipIrfshMbE1Dummy: 11;  //h32[0:10] dummy
    ...
    union
    {
        struct
        {
            u16 nRegJpeSRstMcuCntHigh: 10;
            u16 nRegJpeSRstIrqEn: 1;
            //u16 nRegJpeSRstMcuCnt: 26;        // JPEG MCU counter of restart marker
        } tReg38;
        u16 nReg38;
    } uReg38;
    ...
    union
    {
        struct
        {
            ...
            u16 nRegJpeSObufIdx: 2;         // HW obuf index(0->1->2->3->0->1...)
    ...
    union
    {
        struct
        {
            u16 nRegJpeSRstByteLow: 16; // Buffer 1 Restart Marker byte address at DRAM (31bits)
        } tReg45;
        u16 nReg45;
    } uReg46;
    ...
    union
    {
        struct
        {
            u16 nRegJpeSRstByteHigh: 15; // Buffer 1 Restart Marker byte address at DRAM (31bits)
        } tReg45;
        u16 nReg45;
    } uReg47;
    ...
    union
    {
        struct
        {
            u16 nRegJpeSYuvldrImiBound: 11;          // YUV loader IMI bound (unit: Macro)
            u16 nRegJpeSYuvldrRowEn: 1;              // YUV loader row mode enable
            u16 nRegJpeSYuvldrTileEn: 1;             // YUV loader tile mode enable 0: planar mode ; 1: tile mode
            u16 nRegJpeSYuvldrTileMode: 1;           // YUV loader tile mode 0: 16x32 ; 1: 32x16
            u16 nRegJpeSYuvldrHskSel: 2;             // YUV loader handshake select 0: with Scaler ; 1: Reserved ; 2: with EVD ; 3: with HVD
        } tReg61;
        u16 nReg61;
    } uReg61;
    ...
    union
    {
        struct
        {
            u16 nRegJpeGYPitchMsb: 2;               // Y frame buffer pitch MSB
            u16 nReg0062Dummy: 6;                   // h62[2:7] dummy
            u16 nRegJpeGCPitchMsb: 2;               // C frame buffer pitch MSB
        } tReg62;
        u16 nReg62;
    } uReg62;
    ...
    union
    {
        struct
        {
            u16 nRegJpeImiRdLatCntLow: 16;          // counter for imi write (128bit)
        } tReg65;
        u16 nReg65;
    } uReg65;
    ...
    union
    {
        struct
        {
            u16 nRegJpeImiRdLatCntHigh: 8;          // counter for imi write (128bit)
        } tReg66;
        u16 nReg66;
    } uReg66;
    ...
    union
    {
        struct
        {
            u16 nRegJpeMiWdEnCntLow: 16;                // counter for imi write (128bit)
        } tReg6c;
        u16 nReg6c;
    } uReg6c;
    ...
    union
    {
        struct
        {
            u16 nRegJpeMiWdEnCntHigh: 8;                // counter for imi write (128bit)
            u16 nRegJpeMiRdLatCntLow: 8;                // counter for imi write (128bit)
        } tReg6d;
        u16 nReg6d;
    } uReg6d;
    ...
    union
    {
        struct
        {
            u16 nRegJpeMiRdLatCntHigh: 16;                // counter for imi write (128bit)
        } tReg6e;
        u16 nReg6e;
    } uReg6e;
    ...
    union
    {
        struct
        {
            u16 nRegJpeGDebugMode: 6;           // debug mode
    ...
    union
    {
        struct
        {
            u16 nRegJpeGDebugTrigMbx: 11;       // debug trigger mbx
        } tReg71;
        u16 nReg71;
    } uReg71;
    ...
    union
    {
        struct
        {
            u16 nRegJpeGDebugTrigMby: 11;       // debug trigger mby
        } tReg72;
        u16 nReg72;
    } uReg72;
    ...
    union
    {
        struct
        {
            u16 nRegJpeFrmCycCntLow: 16; //reserved registers
        } tReg7d;
        u16 nReg7d;
    } uReg7d;
    ...
    union
    {
        struct
        {
            u16 nRegJpeFrmCycCntHigh: 16; //reserved registers
        } tReg7e;
        u16 nReg7e;
    } uReg7e;
...
typedef struct
{
    ...
    unsigned long nPartialOffset[2];
...
typedef struct JpeIosCB_t
{
    ...
    JPE_IOC_RET_STATUS_e(*addOutBuff)(void*, u32, u32);    //! add Output Buffer
} JpeIosCB_t;
