...
static int JpeProbe(struct platform_device *pPlatformDev)
{
    ...
    do
    {
        ...
        if (IS_ERR(pClock))
        {
            ...
            //goto _ProbeErr;       // fixme: not break for FPGA easy test
        }
        else
        {
        ...
        if (0 != JpeDevClkInit(pDev, 0))
        {
            CamOsPrintf("JpeProbe clock init fail\n");
