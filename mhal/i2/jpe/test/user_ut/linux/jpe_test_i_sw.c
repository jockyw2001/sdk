...
static int _DoEncode(JPEG_TEST_CLOCK_e eClkSelect, JPEG_TEST_RESOLUTION_e nRes, JPEG_TEST_FORMAT_e eRawFormat, int bDebugMsg, int nTaskId)
{
    ...
    if(nDev <= 0)
    {
        CamOsPrintf("Can't open device %s return=%d errno=%d\n", DEVICE_PATH, nDev, errno);
        return -1;
    }
...
static int _DoTest(JPEG_TEST_FORMAT_e eRawFormat, int bDebugMsg, int nTaskId)
{
...
#endif
    {
        ...
        {
            case eCLK_288:
                CamOsPrintf("%s Task:0x%x Clock: eCLK_288 %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_216:
                CamOsPrintf("%s Task:0x%x Clock: eCLK_216 %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_54:
                CamOsPrintf("%s Task:0x%x Clock: eCLK_54  %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_27:
                CamOsPrintf("%s Task:0x%x Clock: eCLK_27  %s \n", BLUE, nTaskId, NORMAL);
                break;
        }
...
int TestJpeDriverWith2Tasks()
{
    ...
    CamOsPrintf("Joined with thread1 0x%x\n",_gTask1);

    pthread_join(_gTask2, NULL);
    CamOsPrintf("Joined with thread2 0x%x\n",_gTask2);
