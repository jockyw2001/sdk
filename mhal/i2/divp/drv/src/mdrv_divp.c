...
u32 gDipDbgLvl = DIP_DBG_NONE;    // {DIP_DBG_NONE, DIP_DBG_LVL0, DIP_DBG_LVL1, DIP_DBG_LVL2}
...
DivpPrivateDevData g_stDipRes[MAX_DIP_WINDOW];
static MS_BOOL g_bIsInit[MAX_DIP_WINDOW] = {FALSE, FALSE, FALSE};
static DIP_OUTPUT_PATH g_DIVPOutPath = DIP_OUTPUT_PATH_DRAM;

Divp_DI_DebugInfo_t g_stDivpDiDebugInfo[MAX_CHANNEL_NUM];

static MS_U32 time_start, time_end;

MS_U32 GetCurTimeMs(void)
{
    MS_U32 cur_time;
    struct timespec ct;

    getnstimeofday(&ct);
    cur_time = (ct.tv_nsec / 1000000) + (ct.tv_sec * 1000);
    return cur_time;
}

MS_U32 GetCurTimeUs(void)
{
    MS_U32 cur_time;
    struct timespec ct;

    getnstimeofday(&ct);
    cur_time = (ct.tv_nsec / 1000) + (ct.tv_sec * 1000000);
    return cur_time;
}

void mDrv_XC_DIVP_UpdateFPS(MHAL_DIVP_InstPrivate_t* pDivpInst)
{
    MS_U32 cur_time;
    MS_U32 dt;
    ...
    if(pDivpInst->u64PreTime == 0)
    {
        pDivpInst->u64PreTime = GetCurTimeMs();
        pDivpInst->u32PreFrameCnt = 0;
        pDivpInst->u32CurFrameCnt = 0;
    }
    else if((cur_time < pDivpInst->u64PreTime) || (pDivpInst->u32CurFrameCnt < pDivpInst->u32PreFrameCnt))
    {
        pDivpInst->u64PreTime = cur_time;
        pDivpInst->u32PreFrameCnt = pDivpInst->u32CurFrameCnt;
    }
    else if(cur_time >= (pDivpInst->u64PreTime + 1000))
    {
        dt = pDivpInst->u32CurFrameCnt - pDivpInst->u32PreFrameCnt;
        pDivpInst->fps = (MS_U32)((dt * 1000) / (cur_time - pDivpInst->u64PreTime));
        pDivpInst->u64PreTime = cur_time;
        pDivpInst->u32PreFrameCnt = pDivpInst->u32CurFrameCnt;
    }
}
...
SCALER_DIP_SOURCE_TYPE MDrv_XC_DIVP_GetSrcType(MHAL_DIVP_DisplayId_e eDispId, MHAL_DIVP_CapStage_e eCapStage)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "eDispId=%d, eCapStage=%d\n", eDispId, eCapStage);
...
EN_DIP_DI_FIELD _DipGetDiFieldType(MHAL_DIVP_DiType_e eDiType, MHAL_DIVP_ScanMode_e eScanType, MHAL_DIVP_FieldType_e eFieldType)
{
    ...
    //DIP_DBG(DIP_DBG_LVL0, "eDiType=%d, eScanType=%d, eFieldType=%d\n", eDiType, eScanType, eFieldType);
    ...
    if(eDiType == E_MHAL_DIVP_DI_TYPE_3D)
    {
        if(eScanType == E_MHAL_DIVP_SCAN_MODE_INTERLACE_FRAME)
        {
            ...
            {
                //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3D >>> [E_DIP_3DDI_TOP_BOTH_FIELD_STAGGER]\n");
                eDIField = E_DIP_3DDI_TOP_BOTH_FIELD_STAGGER;
            }
            else if(eFieldType == E_MHAL_DIVP_FIELD_TYPE_BOTTOM)
            {
                //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3D >>> [E_DIP_3DDI_BOTTOM_BOTH_FIELD_STAGGER]\n");
        ...
        else if(eScanType == E_MHAL_DIVP_SCAN_MODE_INTERLACE_FIELD)
        {
            if(eFieldType == E_MHAL_DIVP_FIELD_TYPE_TOP)
            {
                ...
                //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3D >>> [E_DIP_3DDI_TOP_SINGLE_FIELD]\n");
            }
            else if(eFieldType == E_MHAL_DIVP_FIELD_TYPE_BOTTOM)
            {
                eDIField = E_DIP_3DDI_BOTTOM_SINGLE_FIELD;
                //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3D >>> [E_DIP_3DDI_BOTTOM_SINGLE_FIELD]\n");
            }
        }
        return eDIField;
    }
    ...
    {
        DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_MED\n");
        if(eScanType == E_MHAL_DIVP_SCAN_MODE_INTERLACE_FRAME)
        {
            if(eFieldType == E_MHAL_DIVP_FIELD_TYPE_TOP)
            {
...
DIP_OUTPUT_PATH MDrv_XC_DIVP_GetDestinationPath(MHAL_DIVP_PixelFormat_e OutFmt)
{
    ...
    {
        DIP_DBG(DIP_DBG_LVL0, "=====[DIP_OUTPUT_MDWIN]=====\n");
        return DIP_OUTPUT_PATH_MDWIN;
    }
    else
    {
        DIP_DBG(DIP_DBG_LVL0, "=====[DIP_OUTPUT_DRAM]=====\n");
        return DIP_OUTPUT_PATH_DRAM;
    }
}

void MDrv_XC_DIVP_Set2Pto1P_Mode(void* pstCmdQInfo, MS_BOOL bEnable, SCALER_DIP_WIN eWindow)
{
    HAL_XC_DIP_Set2Pto1P_Mode(bEnable);
}

MS_BOOL MDrv_XC_DIVP_Get2Pto1P_Mode(void* pstCmdQInfo, SCALER_DIP_WIN eWindow)
{
    return HAL_XC_DIP_Get2Pto1P_Mode();
}
...
void MDrv_XC_DIVP_DeInit(void* pstCmdQInfo, SCALER_DIP_WIN eWindow)
{
    ...
    HAL_XC_DIP_DeInit(pstCmdQInfo, eWindow);
}
...
void MDrv_XC_DIVP_SetDIPRProperty(void* pstCmdQInfo, MHAL_DIVP_InstPrivate_t* pInstance, ST_XC_DIPR_PROPERTY *pstDIPRProperty, SCALER_DIP_WIN eWindow)
{
    ...
    //DIP_DBG(DIP_DBG_LVL0, "[DIPR]u16Pitch=%d\n", pstDIPRProperty->u16Pitch);
    ...
    DIP_DBG(DIP_DBG_LVL0, "[DIPR]u16Bpp=%d, u16LineOft=%d\n", u16Bpp, u16LineOft);
...
void MDrv_XC_DIVP_set_fetch_number_limit(void* pInstance, XC_DIP_InternalStatus *pSrcInfo, SCALER_DIP_WIN eWindow)
{
    ...
    //DIP_DBG(DIP_DBG_LVL0, "u16DNRFetch=%u\n", g_stDBreg.u16DNRFetch);
    //DIP_DBG(DIP_DBG_LVL0, "u16DNROffset=%u\n", g_stDBreg.u16DNROffset);
...
void MDrv_XC_DIVP_SetWindow(void* pstCmdQInfo, MHAL_DIVP_InstPrivate_t* pInstance, MHAL_DIVP_OutPutInfo_t* pstDivpOutputInfo, SCALER_DIP_WIN eWindow)
{
...

    {
        ...
        if(pSrcInfo->bPreHCusScaling)
        {
            ...
            DIP_DBG(DIP_DBG_LVL0, "Customer H scaling: eWindow[%u] HSrc=%u, HDst=%u\n", eWindow, u16SrcHWidth, pSrcInfo->u16H_SizeAfterPreScaling);
        }
        else
        {
            u16SrcHWidth = pSrcInfo->u16PreHCusScalingSrc;
            // if 2P mode is disable and Src width > 1920 then do H pre-scaling down
            if((u16SrcHWidth > 1920) && (HAL_XC_DIP_Get2Pto1P_Mode() == FALSE))
            {
                pSrcInfo->u16H_SizeAfterPreScaling = 1920;
                DIP_DBG(DIP_DBG_LVL0, "DIPR Src width is larger than 1920, then do H scaling!!!\n");
            }
            else
            {
                pSrcInfo->u16H_SizeAfterPreScaling = u16SrcHWidth;
    ...
    DIP_DBG(DIP_DBG_LVL0, "Pre H scaling (%u -> %u)\n", pSrcInfo->u16PreHCusScalingSrc, pSrcInfo->u16PreHCusScalingDst);
    DIP_DBG(DIP_DBG_LVL0, "Pre V scaling (%u -> %u)\n", pSrcInfo->u16PreVCusScalingSrc, pSrcInfo->u16PreVCusScalingDst);
...
void MDrv_XC_DIVP_SetWinProperty(void* pstCmdQInfo, MHAL_DIVP_InstPrivate_t* pInstance, MHAL_DIVP_OutPutInfo_t* pstDivpOutputInfo, SCALER_DIP_WIN eWindow)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "u16Bpp=%d, u16LineOft=%d, u64Offset=0x%x, u64TotalLen=0x%x\n", u16Bpp, u16LineOft, (MS_U32)u64Offset, (MS_U32)u64TotalLen);
    ...
    DIP_DBG(DIP_DBG_LVL0, "Post H cus scaling (%u -> %u)\n", g_DIPSrcInfo[eWindow].u16HCusScalingSrc, g_DIPSrcInfo[eWindow].u16HCusScalingDst);
    DIP_DBG(DIP_DBG_LVL0, "Post V cus scaling (%u -> %u)\n", g_DIPSrcInfo[eWindow].u16VCusScalingSrc, g_DIPSrcInfo[eWindow].u16VCusScalingDst);
    //DIP_DBG(DIP_DBG_LVL0, "bOSDDummyEn = %d\n", bOSDDummyEn);

    if(bOSDDummyEn == TRUE)
    {
        HAL_XC_DIP_SetHVSP(pstCmdQInfo, TRUE, DIP_WINDOW);  // force HVSP enable to add dummy pixel
    }
    else
    {
        if(pInstance ->bIsDiEnable == TRUE)
        {
            if (g_DIPSrcInfo[eWindow].u16HCusScalingDst != g_DIPSrcInfo[eWindow].u16HCusScalingSrc ||
                ...
                HAL_XC_DIP_SetHVSP(pstCmdQInfo, TRUE, DIP_WINDOW);
            else
                HAL_XC_DIP_SetHVSP(pstCmdQInfo, FALSE, DIP_WINDOW);
        }
        else
        {
            if (g_DIPSrcInfo[eWindow].u16HCusScalingDst != g_DIPSrcInfo[eWindow].u16HCusScalingSrc ||
...
MS_BOOL DipDiInit(void* pstCmdQInfo, MHAL_DIVP_InstPrivate_t* pDivpInst, MS_BOOL bEnableDi)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "DipDiInit, u8ChannelId = %d, bEnableDi=%d.\n", u8ChnID, bEnableDi);
    ...
    else
    {
        ...
        DIP_DBG(DIP_DBG_LVL0, "pDipDiAddr = 0x%x, u32DipDiSize = 0x%x\n", (MS_U32)pDipDiAddr, (MS_U32)u32DipDiSize);
        ...
        DIP_DBG(DIP_DBG_LVL0, "NwayMaxBufCnt = %d\n", dev->u8NwayMaxBufCnt);
...
MS_S32 MDrv_XC_DIVP_SetDIPRConfig_3DDI(void * pstCmdQInfo, MHAL_DIVP_InstPrivate_t * pDivpInst, MHAL_DIVP_InputInfo_t* pstDivpInputInfo, SCALER_DIP_WIN eWindow)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "[%d]eDiType=%d, eScanType=%d, eFieldType=%d, eDiMode = %d\n", u8ChnID, eDiType, eScanType, eFieldType, eDiMode);
    //if(u8ChnID == 1)
    //    printf("[%d]eFieldType=%d, eDiMode = %d\n", u8ChnID, eFieldType, eDiMode);
    //if(u8ChnID == 2)
    //    printf("[%d]eFieldType=%d, eDiMode = %d\n", u8ChnID, eFieldType, eDiMode);
    ...
    if (eScanType != E_MHAL_DIVP_SCAN_MODE_PROGRESSIVE )
    {
        if(pDivpInst->bIsDiEnable == FALSE)
        {
            ...
            DIP_DBG(DIP_DBG_LVL0, "p_u32DiBufAddr=0x%x\n", (MS_U32)p_u32DiBufAddr);
#endif
    ...
    if (eDiType == E_MHAL_DIVP_DI_TYPE_3D)
    {
        //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3DDI >>>\n");
        ...
        //DIP_DBG(DIP_DBG_LVL0, "E_MHAL_DIVP_DI_TYPE_3DDI >>> enDI_Field = %d\n", pstDipr3DDiSetting->enDI_Field);
        ...
        {
            if(pDivpInst->u32DiFrameCnt >= 2)
                pDivpInst->u32DiFrameCnt = 0;  // reset frame count to 0
        }
        else
        {

        }
        if(pDivpInst->u32DiFrameCnt < 2) //the first && the second field need run bob mode
        {
            DIP_DBG(DIP_DBG_LVL0, "[%d][Bob]\n", u8ChnID);
            //if(u8ChnID == 2)
            //    printf("[%d][Bob]\n", u8ChnID);
            pstDipr3DDiSetting->bEnableBob = TRUE;// Fix me
            pDivpInst->u32DiFrameCnt ++;
        }
        else //run nonbob mode
        {
            //DIP_DBG(DIP_DBG_LVL0, "[Disable Bob mode]\n");
...
MS_S32 mDrv_XC_DIVP_InputConfigCheck(MHAL_DIVP_InstPrivate_t *pDivpInst, MHAL_DIVP_InputInfo_t* pstDivpInputInfo, MHAL_DIVP_OutPutInfo_t* pstDivpOutputInfo)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "TNR=%d, 3DDI=%d, ROT=%d\n", pDivpInst->stAttr.eTnrLevel, pDivpInst->stAttr.eDiType, pDivpInst->stAttr.eRotateType);
    //DIP_DBG(DIP_DBG_LVL0, "HMirror=%d, VMirror=%d\n", pDivpInst->stAttr.stMirror.bHMirror, pDivpInst->stAttr.stMirror.bVMirror);
    DIP_DBG(DIP_DBG_LVL0, "2P mode=%d\n", pstDivpInputInfo->b2P_Enable);
    ...
    if((pstDivpInputInfo->stCropWin.u16Y + pstDivpInputInfo->stCropWin.u16Height) < 64)
    {
        FUNC_ERR("Crop height is too small!!!!\n");
        return -1;
    }
    ...
    if(((pstDivpInputInfo->stCropWin.u16X + pstDivpInputInfo->stCropWin.u16Width) > pstDivpInputInfo->u16InputWidth) || \
            ((pstDivpInputInfo->stCropWin.u16Y + pstDivpInputInfo->stCropWin.u16Height) > pstDivpInputInfo->u16InputHeight))
    {
        FUNC_ERR("Crop Width/Height  is larger than input Width/Height!!!!\n");
        return -1;
    }
    ...
    if(pDivpInst->stAttr.eRotateType !=  E_MHAL_DIVP_ROTATE_NONE)
    {
        ...
        if (pstDivpInputInfo->stMfdecInfo.bDbEnable == TRUE)
        {
            FUNC_ERR("Can't support MFDEC with Rotation 90/270!!!!\n");
            return -1;
        }

        if(pstDivpInputInfo->eScanMode != E_MHAL_DIVP_SCAN_MODE_PROGRESSIVE)
        {
            FUNC_ERR("Can't support 3DDI with Rotation 90/270!!!!\n");
            return -1;
        }

        if(TRUE == pstDivpInputInfo->b2P_Enable)
        {
            FUNC_ERR("Don't support 2P to 1P mode with Rotation 90/270!!!!\n");
            return -1;
        }
    }
    ...
    if(pstDivpInputInfo->stMfdecInfo.bDbEnable == TRUE)
    {
        ...
        if((pstDivpInputInfo->stMfdecInfo.u16StartX % 32) || (pstDivpInputInfo->stMfdecInfo.u16Width % 32))
        {
            FUNC_ERR("MFDEC crop StartX and Width need 32 alignment!!!!\n");
            return -1;
        }

        if(((pstDivpInputInfo->stCropWin.u16X + pstDivpInputInfo->stCropWin.u16Width) > pstDivpInputInfo->stMfdecInfo.u16Width) || \
                ((pstDivpInputInfo->stCropWin.u16Y + pstDivpInputInfo->stCropWin.u16Height) > pstDivpInputInfo->stMfdecInfo.u16Height))
        {
            FUNC_ERR("Crop Width/Height  is larger than MFDEC input Width/Height!!!!\n");
            return -1;
        }

        if((pstDivpInputInfo->stMfdecInfo.stMirror.bHMirror == TRUE) || (pstDivpInputInfo->stMfdecInfo.stMirror.bVMirror == TRUE))  // don't support mirror/flip
        {
            FUNC_ERR("Don't support MFDEC mirror/flip!!!!\n");
            return -1;
        }

        if(pstDivpInputInfo->stMfdecInfo.bUncompressMode == TRUE)
        {
            FUNC_ERR("Don't support MFDEC Uncompress mode!!!!\n");
            return -1;
        }

        if(pstDivpInputInfo->stMfdecInfo.eDbMode != E_MHAL_DIVP_DB_MODE_H264_H265)
        {
            FUNC_ERR("Only support MFDEC eDbMode H264/H265!!!! eDbMode = %d\n", pstDivpInputInfo->stMfdecInfo.eDbMode);
            return -1;
        }

        if(pstDivpInputInfo->stMfdecInfo.u64DbBaseAddr % 8)
        {
            FUNC_ERR("MFDEC bitlen address base need 8 alignment!!!!\n");
            return -1;
        }
...
MS_BOOL DipInit(MHAL_DIVP_DeviceId_e eDevId)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "[%s]enDipWin = %d\n", __FUNCTION__, enDipWin);
    if(g_bIsInit[enDipWin])
    {
        DIP_DBG(DIP_DBG_LVL0, "dip[%d] has already init.\n", enDipWin);
        return TRUE;
    }
...
MS_BOOL DipDeInit(MHAL_DIVP_DeviceId_e eDevId)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "[%s]enDipWin = %d\n", __FUNCTION__, enDipWin);
    if(g_bIsInit[enDipWin] == FALSE)
    {
        DIP_DBG(DIP_DBG_LVL0, "dip[%d] has already Deinit.\n", enDipWin);
        return TRUE;
    }
                                ...
                                MS_U16 u16MaxHeight, PfnAlloc pfAlloc, PfnFree pfFree, MS_U8 u8ChannelId, void** ppCtx)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "id = %d, MaxW = %d, MaxH = %d\n", eDevId, u16MaxWidth, u16MaxHeight);

    if((u16MaxWidth > MAX_WIDTH) || (u16MaxHeight > MAX_HEIGHT))
    {
        FUNC_ERR("Input parameter error!!!!\n");
        return -1;
    }
    ...
    if(pDivpInst != NULL)
    {
        ...
        pDivpInst->u32DiFrameCnt = 0;
        pDivpInst->u32PreFrameCnt = 0;
        pDivpInst->u32CurFrameCnt = 0;
        pDivpInst->u64PreTime = 0;
...
MS_S32 MHAL_DIVP_DestroyInstance(void* pCtx)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "pDivpInst = 0x%x, enDipWin = %d\n", (int)pDivpInst, enDipWin);
...
MS_S32 MHAL_DIVP_CaptureTiming(void* pCtx, MHAL_DIVP_CaptureInfo_t* pstCaptureInfo, MHAL_CMDQ_CmdqInterface_t* pstCmdQInfo)
{
    ...
    if(HAL_CMDQ_MultiWrite_End(pstCmdQInfo) == FALSE)
    {
        return -1;
    }
                                 ...
                                 MHAL_DIVP_OutPutInfo_t* pstDivpOutputInfo, MHAL_CMDQ_CmdqInterface_t* pstCmdQInfo)
{
    ...
    DIP_MDWIN_PROPERTY stMDWinProperty = {0};
    MS_U16 u16MfdStartX, u16MfdCropW;
    MS_U16 u16DipCropStartX, u16DipCropW;
    MS_U16 u16Pitch, u16OffsetX;
    MS_U16 u16MfdCropW_new, u16DipCropW_new;
    MS_U16 u16DiprW, u16DiprW_new;
    ...
    dev->u8FinalChannelId = pDivpInst->u8ChannelId;
    memcpy(&pDivpInst->pstInputInfo, pstDivpInputInfo, sizeof(MHAL_DIVP_InputInfo_t));
    memcpy(&pDivpInst->pstOutputInfo, pstDivpOutputInfo, sizeof(MHAL_DIVP_OutPutInfo_t));

    //MDrv_XC_DIVP_SetMirror(pstCmdQInfo, pDivpInst->stAttr.stMirror.bHMirror, pDivpInst->stAttr.stMirror.bVMirror, DIP_WINDOW);

    MDrv_XC_DIVP_Set2Pto1P_Mode(pstCmdQInfo, pstDivpInputInfo->b2P_Enable, DIP_WINDOW);
...
#if 1  // fixed DIPW IRQ issue, force DIPR Vsize = (Crop StartY + Crop Vsize)
    pstDivpInputInfo->u16InputHeight = (pstDivpInputInfo->stCropWin.u16Y + pstDivpInputInfo->stCropWin.u16Height);
#endif
    ...
    {
#if 1  // fixed DIPW IRQ issue, force DIPR Vsize = (Crop StartY + Crop Vsize)
        pstDivpInputInfo->stMfdecInfo.u16Height = (pstDivpInputInfo->stCropWin.u16Y + pstDivpInputInfo->stCropWin.u16Height);
#endif

#if 1  // patch for DIPR MIU FIFO maximum burst size(64) issue
        if((pstDivpInputInfo->stMfdecInfo.u16Width > 2048) && (pstDivpInputInfo->stMfdecInfo.u16Width % 64))
        {
            u16MfdStartX = pstDivpInputInfo->stMfdecInfo.u16StartX;
            u16MfdCropW = pstDivpInputInfo->stMfdecInfo.u16Width;
            u16DipCropStartX = pstDivpInputInfo->stCropWin.u16X;
            u16DipCropW = pstDivpInputInfo->stCropWin.u16Width;
            u16Pitch = pstDiprInfo->u16Pitch;
            u16MfdCropW_new = ALIGN(u16MfdCropW, 64);
            u16OffsetX = u16MfdCropW_new - u16MfdCropW;
            DIP_DBG(DIP_DBG_LVL0, "mSx=%d, mCw=%d, dSx=%d, dCw = %d, pitch = %d, mCw_new = %d\n", u16MfdStartX, u16MfdCropW, u16DipCropStartX, u16DipCropW, u16Pitch, u16MfdCropW_new);

            if((u16MfdStartX + u16MfdCropW_new) <= u16Pitch)
            {
                pstDivpInputInfo->stMfdecInfo.u16Width = u16MfdCropW_new;
            }
            else
            {
                if(u16MfdStartX >= u16OffsetX)
                {
                    u16MfdStartX -= u16OffsetX;
                    pstDivpInputInfo->stMfdecInfo.u16StartX = u16MfdStartX;
                    pstDivpInputInfo->stMfdecInfo.u16Width = u16MfdCropW_new;
                    pstDivpInputInfo->stCropWin.u16X += u16OffsetX;
                }
                else
                {
                    u16MfdCropW_new -= 64;
                    pstDivpInputInfo->stMfdecInfo.u16Width = u16MfdCropW_new;
                    if((u16DipCropStartX + u16DipCropW) > u16MfdCropW_new)
                    {
                        u16DipCropW_new = (u16MfdCropW_new - u16DipCropStartX);
                        pstDivpInputInfo->stCropWin.u16Width = u16DipCropW_new;
                    }
                }
            }
        }
    ...
    else
    {
        ...
        if((pstDivpInputInfo->ePxlFmt == E_MHAL_DIVP_PIXEL_FORMAT_YUV_SEMIPLANAR_420) || \
                ...
                (pstDivpInputInfo->ePxlFmt == E_MHAL_DIVP_PIXEL_FORMAT_YUV_MST_420))
        {
            if((pstDivpInputInfo->u16InputWidth > 2048) && (pstDivpInputInfo->u16InputWidth % 64))
            {
                u16DiprW = pstDivpInputInfo->u16InputWidth;
                u16DipCropStartX = pstDivpInputInfo->stCropWin.u16X;
                u16DipCropW = pstDivpInputInfo->stCropWin.u16Width;
                u16Pitch = pstDiprInfo->u16Pitch;
                u16DiprW_new = ALIGN((u16DiprW - 32), 64);
                DIP_DBG(DIP_DBG_LVL0, "dipr_w=%d, dSx=%d, dCw = %d, pitch = %d, dipr_w_new = %d\n", u16DiprW, u16DipCropStartX, u16DipCropW, u16Pitch, u16DiprW_new);

                pstDivpInputInfo->u16InputWidth = u16DiprW_new;
                pstDiprInfo->u16Width = pstDivpInputInfo->u16InputWidth;
                if((u16DipCropStartX + u16DipCropW) > u16DiprW_new)
                {
                    u16DipCropW_new = (u16DiprW_new - u16DipCropStartX);
                    pstDivpInputInfo->stCropWin.u16Width = u16DipCropW_new;
                }
            }
        }
...
#if 1  // calculate fps
    mDrv_XC_DIVP_UpdateFPS(pDivpInst);
#endif

    time_end = GetCurTimeUs();
    pDivpInst->time_elapsed = (time_end - time_start);
...
MS_S32 MHAL_DIVP_WaitCmdQFrameDone(MHAL_CMDQ_CmdqInterface_t* pstCmdQInfo, MHAL_CMDQ_EventId_e eEventId)
{
    ...
    pstCmdQInfo->MHAL_CMDQ_CmdqPollRegBits(pstCmdQInfo, 0x133694, 0x0001,  0xFF, TRUE);
#endif
