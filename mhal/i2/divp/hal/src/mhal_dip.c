...
#include "../../../../include/cmdq_service/mhal_cmdq.h"
...
#define DIPR_TILE_FIFO_MAX  64
#define DIPR_TILE_FIFO_MIN  1
...
static MS_BOOL bDIPNwayInfoInit[DI_NWAY_MAX] = {};
...
static MS_BOOL g_bDIP2Pto1PEnable = FALSE;

MS_BOOL g_bDipMfdecFPEn = FALSE;
MS_U8      g_u8DipMfdecFPData;
static MS_U32   g_u32CmdQWriteBurstSize = 0;
MHAL_CMDQ_MultiCmdBufMask_t gCmdQWriteMaskBuf[CMDQ_MULTI_WRITE_BUF_MAX];
...
void MDrv_WriteRegBit_CmdQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_BOOL bEnable, MS_U16 u8Mask)
{
#ifdef USING_CMDQ_MULTI_WRITE
    MS_U16 u16Value;
#endif
    ...
    do
    {
        ...
        else
        {
            ...
            {
#ifdef USING_CMDQ_MULTI_WRITE
                if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = (u32Reg - 1);
                u16Value = (bEnable) ? (RIU_READ_2BYTE(  ((u32Reg - 1) << 1)  ) |  (u8Mask << 8)) : (RIU_READ_2BYTE( ((u32Reg - 1) << 1) ) & ~(u8Mask << 8));
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Value;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
                g_u32CmdQWriteBurstSize++;
#else
                CmdQ->MHAL_CMDQ_WriteRegCmdq(CmdQ, ((u32Reg - 1)), (bEnable) ? (RIU_READ_2BYTE(  ((u32Reg - 1) << 1)  ) |  (u8Mask << 8)) :
                                             (RIU_READ_2BYTE( ((u32Reg - 1) << 1) ) & ~(u8Mask << 8)));
#endif
...
#ifdef USING_CMDQ_MULTI_WRITE
                if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = (u32Reg);
                u16Value = (bEnable) ? (RIU_READ_2BYTE(  ((u32Reg) << 1)  ) |  (u8Mask)) : (RIU_READ_2BYTE( ((u32Reg) << 1) ) & ~(u8Mask));
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Value;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
                g_u32CmdQWriteBurstSize++;
#else
...
*/
void MDrv_Write2ByteMask_CmdQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_U16 u16Val, MS_U16 u16Mask)
{
    do
    {
        if(CmdQ == NULL)
        ...
        else
        {
            ...
            if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
            gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = u32Reg;
            gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Val;
            gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = u16Mask;
            g_u32CmdQWriteBurstSize++;
#else
            CmdQ->MHAL_CMDQ_WriteRegCmdqMask(CmdQ, (u32Reg), u16Val, u16Mask);
#endif
...
*/
void MDrv_WriteByteMask_CmdQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_U8 u8Val, MS_U8 u8Msk)
{
#ifdef USING_CMDQ_MULTI_WRITE
    MS_U16 u16Value;
#endif
    ...
    do
    {
        ...
        else
        {
            if ( ((u32Reg) & 0x01) )
            {
                ...
                if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = (u32Reg - 1);
                u16Value = (MS_U16)((RIU_READ_2BYTE((u32Reg - 1) << 1) & ~(u8Msk << 8)) | ((u8Val << 8) & (u8Msk << 8)));
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Value;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
                g_u32CmdQWriteBurstSize++;
#else
            ...
            else
            {
                ...
                if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = u32Reg;
                u16Value = (MS_U16)((RIU_READ_2BYTE((u32Reg) << 1) & ~(u8Msk)) | ((u8Val) & (u8Msk)));
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Value;
                gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
                g_u32CmdQWriteBurstSize++;
#else
...
*/
void SC_W2BYTE_CMDQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_U16 u16Val)
{
    if(CmdQ == NULL)
    {
        RIU_WRITE_2BYTE( (REG_SCALER_BASE + ((u32Reg) & 0xFFFF)) << 1, u16Val );
...
#ifdef USING_CMDQ_MULTI_WRITE
        if(g_u32CmdQWriteBurstSize >= CMDQ_MULTI_WRITE_BUF_MAX)    return;
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = ((REG_SCALER_BASE + ((u32Reg) & 0xFFFF) ));
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = u16Val;
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
        g_u32CmdQWriteBurstSize++;
#else
...
#define SC_W4BYTE_CMDQ(CmdQ, u32Reg, u32Val)\
    ...
    CHECK_XC_SWDS_ENABLE_END
*/
void SC_W4BYTE_CMDQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_U32 u32Val)
{
    ...
    {
#ifdef USING_CMDQ_MULTI_WRITE
        if(g_u32CmdQWriteBurstSize >= (CMDQ_MULTI_WRITE_BUF_MAX-1))    return;
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = ((REG_SCALER_BASE + ((u32Reg) & 0xFFFF) ));
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = (MS_U16)((u32Val) & 0x0000FFFF);
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
        g_u32CmdQWriteBurstSize++;
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u32RegAddr = ((REG_SCALER_BASE + ((u32Reg) & 0xFFFF) + 2 ));
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16RegValue = (MS_U16)(((u32Val) >> 16) & 0x0000FFFF);
        gCmdQWriteMaskBuf[g_u32CmdQWriteBurstSize].u16Mask = 0xFFFF;
        g_u32CmdQWriteBurstSize++;
#else
        CmdQ->MHAL_CMDQ_WriteRegCmdq(CmdQ, ((REG_SCALER_BASE + ((u32Reg) & 0xFFFF) )), (MS_U16)((u32Val) & 0x0000FFFF));
...
#define SC_W2BYTEMSK_CMDQ(CmdQ, u32Reg, u16Val, u16Mask)\
    ...
    CHECK_XC_SWDS_ENABLE_END
*/
void SC_W2BYTEMSK_CMDQ(MHAL_CMDQ_CmdqInterface_t *CmdQ, MS_U32 u32Reg, MS_U16 u16Val, MS_U16 u16Mask)
{
...
void HAL_CMDQ_MultiWrite_Init(void)
{
#ifdef USING_CMDQ_MULTI_WRITE
    g_u32CmdQWriteBurstSize = 0;
#endif
}

MS_BOOL HAL_CMDQ_MultiWrite_End(void *pstCmdQInfo)
{
#ifdef USING_CMDQ_MULTI_WRITE
    MHAL_CMDQ_CmdqInterface_t *pstCmdQ = (MHAL_CMDQ_CmdqInterface_t *)pstCmdQInfo;

    if((pstCmdQ->MHAL_CMDQ_WriteRegCmdqMaskMulti) && ((g_u32CmdQWriteBurstSize > 0) && (g_u32CmdQWriteBurstSize < CMDQ_MULTI_WRITE_BUF_MAX)))
    {
        pstCmdQ->MHAL_CMDQ_WriteRegCmdqMaskMulti(pstCmdQ, (MHAL_CMDQ_MultiCmdBufMask_t *)(&gCmdQWriteMaskBuf[0]), (MS_U16)g_u32CmdQWriteBurstSize);

        DIP_DBG(DIP_DBG_LVL0, "CmdQ Multi Write Size = %d\n", g_u32CmdQWriteBurstSize);
        g_u32CmdQWriteBurstSize = 0;
    }
    else
    {
        FUNC_ERR("CmdQ Multi-Write Size is too large!!!!\n");
        g_u32CmdQWriteBurstSize = 0;
        return FALSE;
    }
#endif
...
void HAL_XC_INFO_dump_Reg(MS_U16 BankNo, MS_U16 RegOffset, MS_U32 len)
{
    ...
    for (addr = start, l_offset = 0; addr < (start + (len * 2)); addr += 2)
    {
        if ((addr & 0xFF) == 0)
        {
            DIP_DBG(DIP_DBG_LVL0, "BK x%02X :", (addr >> 8));
        }
        if ((addr & 0xF) == 0)
        {
            l_offset = 0;
        ...
        if ((addr & 0xF) == 0xE)
        {
            DIP_DBG(DIP_DBG_LVL0, "%s\n", l_buf);
        }
    }
    DIP_DBG(DIP_DBG_LVL0, "Dump End\n");
}
...
void HAL_MFDEC_INFO_dump_Reg(void)
{
    ...
    for (addr = start, l_offset = 0; addr < (start + (0x80 * 2)); addr += 2)
    {
        if ((addr & 0xFF) == 0)
        {
            DIP_DBG(DIP_DBG_LVL0, "BK x%04X :", (addr >> 8));
        }
        if ((addr & 0xF) == 0)
        {
            l_offset = 0;
...
void HAL_XC_DIP_Set2Pto1P_Mode(MS_BOOL bEnable)
{
    g_bDIP2Pto1PEnable = bEnable;
}

MS_BOOL HAL_XC_DIP_Get2Pto1P_Mode(void)
{
    return g_bDIP2Pto1PEnable;
}
...
static MS_BOOL IsNeedAverageMode(SCALER_DIP_SOURCE_TYPE eSource, MS_U16 u16MaxWith)
{
    ...
    else
    {
        ...
        bNeedAverageMode = g_bDIP2Pto1PEnable;
    }
...
void HAL_XC_DIP_DI_Read_FIFO(void *pstCmdQInfo, MS_U16 u16Width, SCALER_DIP_WIN eWindow)
{
    ...
    bResult = FALSE;
    for(u16temp = (DIPR_TILE_FIFO_MAX >> 1) ; u16temp > DIPR_TILE_FIFO_MIN ; u16temp--)
    {
    ...
    if(bResult == FALSE)
    {
        for(u16temp = ((DIPR_TILE_FIFO_MAX >> 1) + 1) ; u16temp < DIPR_TILE_FIFO_MAX ; u16temp++)
        {
            if((u16MiuCnt % u16temp) == 0)
            {
                SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK43_7F_L, (u16temp - 1), BMASK(6: 0));
    ...
    if(bResult == FALSE)
    {
        FUNC_ERR("Unkonwn DI DIPR MIU FIFO burst number!!!!\n");
    }
...
{
    FUNC_ERR("[Warnning][%s], Line:%d, SCALER_DIP CMDQ not Support\n", __FUNCTION__, __LINE__);
    return FALSE;
}
...
#ifndef DIP_UT_ENABLE_DIPR_IRQ
    SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_38_L, 0xFF, BMASK(7: 0));  // DIPR interrupt mask
#endif
#ifndef DIP_UT_ENABLE_DIPR_CMDQ_IRQ
    SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_48_L, 0xFF, BMASK(7: 0));  // DIPR CmdQ interrupt mask
#endif
#endif

#if 0  // Test DIVP write done IRQ issue, Tomy@20180302
    //SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_7C_L, BIT(8), BIT(8));  // dipr force frame index

    //SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_0C_L, 0, BIT(15));  // dip int select, default is 1
#endif
...
void HAL_XC_DIP_DeInit(void *pstCmdQInfo, SCALER_DIP_WIN eWindow)
{
    MHAL_CMDQ_CmdqInterface_t *pstCmdQ = (MHAL_CMDQ_CmdqInterface_t *)pstCmdQInfo;

    HAL_XC_DIP_MDWinDeInit(pstCmdQInfo, 1);  // ID 1 for DIVP

    MDrv_WriteRegBit_CmdQ(pstCmdQ, REG_CKG_DIP_FCLK, ENABLE, CKG_DIP_FCLK_GATED);                    // Disable clock
...
void HAL_XC_DIP_EnableCaptureStream(void *pstCmdQInfo, MS_BOOL bEnable, SCALER_DIP_WIN eWindow)
{
    ...
    switch(eWindowSource)
    {
        ...
        case SCALER_DIP_SOURCE_TYPE_OP_SUB:
            ...
            break;
        default:
            FUNC_ERR("SCALER_DIP_SOURCE_TYPE does not support\n");
            return;
    }
...
{
    SC_W2BYTEMSK(0, REG_SC_BK36_09_L, u16mask & BMASK(7: 0), BMASK(7: 0));  // for DIPW
}
...
void HAL_XC_DIP_CMDQ_ClearIntr(void *pstCmdQInfo, MS_U16 u16mask, SCALER_DIP_WIN eWindow)
{
    ...
    SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_29_L, u16mask & BMASK(7: 0), BMASK(7: 0));  // for DIPW
#ifdef DIP_UT_ENABLE_DIPR_CMDQ_IRQ
    SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_49_L, u16mask & BMASK(7: 0), BMASK(7: 0));  // for DIPR
#endif
}
...
void HAL_XC_DIP_CpatureOneFrame(void *pstCmdQInfo, SCALER_DIP_WIN eWindow)//MS_BOOL benable, SCALER_DIP_WIN eWindow)
{
    ...
    //    FUNC_ERR("!!!Alert !!! DWIN Capture, wait ack time out!\n");
...
{
    DIP_DBG(DIP_DBG_LVL0, "DIP_Reg(REG_IPMUX_02_L) = x%x\n", MDrv_ReadByte(REG_IPMUX_02_L));
    DIP_DBG(DIP_DBG_LVL0, "DIP_Reg(REG_CKG_IDCLK3) = x%x\n", MDrv_ReadByte(REG_CKG_IDCLK3));
}
...
void HAL_XC_DIP_SetMux(void *pstCmdQInfo, MS_U8 u8Data_Mux, MS_U8 u8Clk_Mux, SCALER_DIP_WIN eWindow)
{
    ...
    if(pstCmdQ != NULL)
    {
        DIP_DBG(DIP_DBG_LVL0, "DIP_DummyReg = x%x\n", SC_R2BYTE(0, REG_SC_BK34_7D_L));
...
MS_BOOL HAL_XC_DIP_GetCaptureSourceStatus(void *pstCmdQInfo, SCALER_DIP_SOURCE_TYPE eSource, SCALER_DIP_WIN eWindow, MS_BOOL* bstatus)
{
    switch(eSource)
    {
        ...
        break;
        default:
            FUNC_ERR("[%s][%d] eWindow =%d ---->eSource = %d is not Support\n", __FUNCTION__, __LINE__, eWindow, eSource);
            *bstatus = FALSE;
            return FALSE;
            break;
...
void HAL_XC_DIP_HSPScaling(void *pstCmdQInfo, MS_BOOL bEnable, MS_U16 ScalingSrc, MS_U16 ScalingDst, MS_BOOL bOutputDummyEn, SCALER_DIP_WIN eWindow)
{
    MS_U32 u32Ratio = 0;
    MHAL_CMDQ_CmdqInterface_t *pstCmdQ = (MHAL_CMDQ_CmdqInterface_t *)pstCmdQInfo;
    ...
    if(bOutputDummyEn == TRUE)
        SC_W2BYTE_CMDQ(pstCmdQ, REG_SC_BK49_24_L, ScalingDst + 32);
    else
...
void HAL_XC_DIP_AdjustScaling(void *pstCmdQInfo, MS_BOOL bAVG, MS_BOOL bOutputDummyEn, SCALER_DIP_WIN eWindow)
{
    ...
    if(eHVSPWindow != MAX_DIP_WINDOW)
    {
        ...
        //if((HCusScalingSrc != g_DIPSrcInfo[eWindow].u16HCusScalingDst) || (g_DIPSrcInfo[eWindow].u16VCusScalingSrc != g_DIPSrcInfo[eWindow].u16VCusScalingDst))
        {
            ...
            //Hal_SC_DWIN_sw_db(pstCmdQInfo, &DBreg, eWindow);
            HAL_XC_DIP_HSPScaling(pstCmdQInfo, TRUE, HCusScalingSrc, g_DIPSrcInfo[eWindow].u16HCusScalingDst, bOutputDummyEn, eWindow);
        ...
        DIP_DBG(DIP_DBG_LVL0, "== Enable HVSP ==\n");
    }
    else
    {
        ...
        DIP_DBG(DIP_DBG_LVL0, "== Disable HVSP ==\n");
    }
}

void HAL_XC_DIP_SetWinProperty(void *pstCmdQInfo, MS_U8 u8BufCnt, MS_U16 u16Width, MS_U16 u16LineOft, MS_U16 u16Height, MS_U32 u64OffSet, SCALER_DIP_SOURCE_TYPE eSource, MS_BOOL bPIP, MS_BOOL b2P_Enable, MS_BOOL bOutputDummyEn, SCALER_DIP_WIN eWindow)
{
    MS_U16 u16SrcWith = 0;
    MS_U16 u16MaxWith = 0;
    MS_BOOL bNeedAverageMode = FALSE;
    ...
    HAL_XC_DIP_AdjustScaling(pstCmdQInfo, bNeedAverageMode, bOutputDummyEn, eWindow);

    //DIP_DBG(DIP_DBG_LVL0, "u16MaxWith = %d, bNeedAverageMode = %d\n", u16MaxWith, bNeedAverageMode);
...
#if (DIP_PLATFORM == DIP_I2)
    // reg_dip_wreq_thrd, TODO: FIXEDME, less than (DIPW hsize/32) - 1
    //SC_W2BYTEMSK_CMDQ(0, REG_SC_BK36_6F_L, 0x0A, BMASK(5: 0));
    u8WreqThrd = (MS_U8)((u16Width / 32) - 1);
    if(u16Width < 352)
    {
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BK36_6F_L, u8WreqThrd, BMASK(5: 0));
    }
    else
    {
...
void HAL_XC_DIP_SetOutputCapture(void *pstCmdQInfo, MS_BOOL bEnable, EN_XC_DIP_OP_CAPTURE eOpCapture, SCALER_DIP_WIN eWindow)
{
    ...
    if(bCaptureEna)
    {
        ...
        if(eOpCapture == E_XC_DIP_VOP2)
            ...
            FUNC_ERR("[DIP] OP capture source not support\n");
    }
...
void HAL_XC_DIP_SetDIPRProperty(void *pstCmdQInfo, ST_XC_DIPR_PROPERTY *pstDIPRProperty, SCALER_DIP_WIN eWindow)
{
    ...
    switch ((EN_DRV_XC_DWIN_DATA_FMT)pstDIPRProperty->enDataFmt)
    {
        ...
        case XC_DWIN_DATA_FMT_YUV420_SEMI_PLANER :
            ...
            if(bResult == FALSE)
            {
                FUNC_ERR("Unkonwn DIPR MIU FIFO burst number!!!!\n");
            }
            DIP_DBG(DIP_DBG_LVL0, "u16temp = %d\n", u16temp);
...
void HAL_XC_DIP_SetDIPRProperty_MFDEC(void *pstCmdQInfo, ST_DIP_MFDEC_INFO stDIPR_MFDecInfo, SCALER_DIP_WIN eWindow)
{
    ...
    {
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== stMFDec_VerCtl.u32version = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.stMFDec_VerCtl.u32version);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== stMFDec_VerCtl.u32size = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.stMFDec_VerCtl.u32size);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== bMFDec_Enable = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.bMFDec_Enable);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u8MFDec_Select = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u8MFDec_Select);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== enMFDec_tile_mode = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.enMFDec_tile_mode);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== bUncompress_mode = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.bUncompress_mode);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== bBypass_codec_mode = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.bBypass_codec_mode);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== en_MFDecVP9_mode = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.en_MFDecVP9_mode);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== phyLuma_FB_Base = 0x%x  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.phyLuma_FB_Base);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== phyChroma_FB_Base = 0x%x  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.phyChroma_FB_Base);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16FB_Pitch = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16FB_Pitch);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16StartX = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16StartX);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16StartY = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16StartY);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16HSize = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16HSize);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16VSize = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16VSize);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== phyBitlen_Base = 0x%x  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.phyBitlen_Base);
        DIP_DBG(DIP_DBG_LVL1, "\033[32m [%s][%d] ========== u16Bitlen_Pitch = %d  ========== \033[0m\n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16Bitlen_Pitch);
...
#if 1 // test fixed pattern enable mode for debug
        if(g_bDipMfdecFPEn == TRUE)
        {
            u16regtmp |= MFDEC_FIXED_PAT_MODE;
            MDrv_Write2ByteMask_CmdQ(pstCmdQ, REG_MFDEC_BITLEN_FB_PITCH(u8mfdec_id), (g_u8DipMfdecFPData << 8), MFDEC_BITLEN_FB_FIXED_PAT_MASK);
        }
#endif
        ...
        if(stDIPR_MFDecInfo.u16FB_Pitch % MFDEC_FB_PITCH_ALIGN != 0)
            FUNC_ERR("ERROR[%s][%d] ==========FB Pitch not alignment %d , 8P alignment ========== \n", __FUNCTION__, __LINE__, stDIPR_MFDecInfo.u16FB_Pitch);
...
void HAL_XC_DIP_DI_init(void *pstCmdQInfo, MS_U32 phyWriteAddr, MS_U32 phyReadAddr, SCALER_DIP_WIN eWindow)
{
    ...
    DIP_DBG(DIP_DBG_LVL2, "[%s,%d] ======== 3DDI Initialization start ========\n", __func__, __LINE__);
    //u16BusSize = HAL_XC_DIP_GetBusSize(pstCmdQInfo, eWindow);
    ...
    for(u8DI_queue_count = 0; u8DI_queue_count < DI_INIT_DI_QUEUE_COUNT; u8DI_queue_count++)
    {
        ...
        while(1)
        {
            ...
            {
                FUNC_ERR("[%s,%d]DI INIT STEP%d TIMEOUT\n", __func__, __LINE__, u8DI_queue_count);
                break;
            }
...
void HAL_XC_DIP_SetDIPRProperty_DI(void *pstCmdQInfo, ST_XC_DIPR_PROPERTY *pstDIPRProperty, ST_DIP_DIPR_3DDI_SETTING *stDIPR_DIInfo, SCALER_DIP_WIN eWindow)
{
    ...
    {
        phyDI_DIPBufAddr[u8DINwayindex] = stDIPRDISetting.stDINwayBufInfo[u8DINwayindex].phyDI_BufAddr;
        u32DI_DIPBufSize[u8DINwayindex] = stDIPRDISetting.stDINwayBufInfo[u8DINwayindex].u32DI_BufSize;
        u8DI_DIPBufCnt[u8DINwayindex] = stDIPRDISetting.stDINwayBufInfo[u8DINwayindex].u8DIBufCnt;
        bDIPNwayInfoInit[u8DINwayindex] = TRUE;
    }
    ...
    if(stDIPRDISetting.bEnableDI == TRUE)
    {
        ...
        else
        {
            ...
            {
                FUNC_ERR("[ERROR]NEED DI INIT FIRST\n");
                return;
            }
        }
    }
    ...
    if(bDIPNwayInfoInit[u8DINwayindex] == FALSE)
    {
        FUNC_ERR("[ERROR]DIP DI Need Nway Info Init First\n");
        return;
    }
    ...
    //DIP_DBG(DIP_DBG_LVL2, "[%d]phyDIBufAddr=0x%x,u32DIBufSize=0x%x\n", u8DIBufIndex, phyDIBufAddr, u32DIBufSize);
    ...
    //if(u8DINwayindex == 2)
    {
        if(enTB_DIPField[u8DIBufIndex][u8DINwayindex]  == E_DIP_TOP_FIELD)
        {
            DIP_DBG(DIP_DBG_LVL2, "[Ch%d][%d]=T%d=(%d, %d), (%d, %d)\n", u8DINwayindex, stDIPRDISetting.bEnableBob, u8DIBufIndex, \
                    u8DIBuf_RdTopIndex[u8DINwayindex], u8DIBuf_RdBotIndex[u8DINwayindex], u8DIBuf_WrTopIndex[u8DINwayindex], u8DIBuf_WrBotIndex[u8DINwayindex]);
        }
        else if(enTB_DIPField[u8DIBufIndex][u8DINwayindex]  == E_DIP_BOTTOM_FIELD)
        {
            DIP_DBG(DIP_DBG_LVL2, "[Ch%d][%d]=B%d=(%d, %d), (%d, %d)\n", u8DINwayindex, stDIPRDISetting.bEnableBob, u8DIBufIndex, \
                    u8DIBuf_RdTopIndex[u8DINwayindex], u8DIBuf_RdBotIndex[u8DINwayindex], u8DIBuf_WrTopIndex[u8DINwayindex], u8DIBuf_WrBotIndex[u8DINwayindex]);
        }
    }

#if 1  // save debug info
    {
        MS_U8 cur_frame = g_stDivpDiDebugInfo[u8DINwayindex].u8NextFrame;

        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].bBobEn = stDIPRDISetting.bEnableBob;
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8TB_Field = enTB_DIPField[u8DIBufIndex][u8DINwayindex];
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8DIBufIndex = u8DIBufIndex;
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8DIRdTopIndex = u8DIBuf_RdTopIndex[u8DINwayindex];
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8DIRdBotIndex = u8DIBuf_RdBotIndex[u8DINwayindex];
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8DIWrTopIndex = u8DIBuf_WrTopIndex[u8DINwayindex];
        g_stDivpDiDebugInfo[u8DINwayindex].stDebugInfo[cur_frame].u8DIWrBotIndex = u8DIBuf_WrBotIndex[u8DINwayindex];

        g_stDivpDiDebugInfo[u8DINwayindex].u8NextFrame++;
        if(g_stDivpDiDebugInfo[u8DINwayindex].u8NextFrame >= MAX_DEBUG_FRAME_NUM)
            g_stDivpDiDebugInfo[u8DINwayindex].u8NextFrame = 0;
    }
#endif

    //DIP_DBG(DIP_DBG_LVL2, "DIBufIndex = [%d]\n", u8DIBufIndex);
    //for(u8i = 0; u8i < u8DIBufTotal; u8i++)
    //{
    //    DIP_DBG(DIP_DBG_LVL2, " TBField[%d]=%d ", u8i, enTB_DIPField[u8i][u8DINwayindex]);
    //}
    //DIP_DBG(DIP_DBG_LVL2, " (1:TOP ; 2:bottom)\n");
...
void HAL_XC_DIP_Rotation(void *pstCmdQInfo, MS_BOOL bRotation, EN_XC_DIP_ROTATION eRoDirection, MS_U32 u64StartAddr, SCALER_DIP_WIN eTmpWindow)
{
    ...
    DIP_DBG(DIP_DBG_LVL0, "bDIPRotation=%d, RoDirection = %d\n", bDIPRotation, eRoDirection);
...
void HAL_XC_DIP_2P_Width_Check(void *pstCmdQInfo, XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_DIP_WIN eWindow)
{
    ...
    if(bNeedAverageMode)
    {
        ...
        DIP_DBG(DIP_DBG_LVL0, "New Dst PreH size=%d\n", pstXC_SetWin_Info->u16PreHCusScalingDst);
    }
...
void HAL_XC_DIP_MDWinInit(void *pstCmdQInfo, MS_U8 u8WinId)
{
    ...
    if( u8WinId == 0 )
    {
        ...
        // Set DWIN on
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF2_72_L, 0, BIT(0));  // DWIN on
        // DWIN dbf on
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF2_72_L, 0, BIT(7));  // DWIN dbf on
    }
    else
    {
        ...
        // Set DWIN on
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF6_72_L, 0, BIT(0));  // DWIN on
        // DWIN dbf on
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF6_72_L, 0, BIT(7));  // DWIN dbf on
    }

}

void HAL_XC_DIP_MDWinDeInit(void *pstCmdQInfo, MS_U8 u8WinId)
{
    MHAL_CMDQ_CmdqInterface_t *pstCmdQ = (MHAL_CMDQ_CmdqInterface_t *)pstCmdQInfo;
    ...
    {
        // Set DWIN off
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF2_72_L, BIT(0), BIT(0));  // DWIN off
        // DWIN dbf off
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF2_72_L, BIT(7), BIT(7));  // DWIN dbf off
    }
    else
    {
        // Set DWIN off
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF6_72_L, BIT(0), BIT(0));  // DWIN off
        // DWIN dbf off
        SC_W2BYTEMSK_CMDQ(pstCmdQ, REG_SC_BKF6_72_L, BIT(7), BIT(7));  // DWIN dbf off
    }
