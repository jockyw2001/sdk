...
//!!!Attention: MVOP stride buffer alignment must be equal to MI_BUFFER_STRIDE_ALIGNMENT which is defined in mi_disp_cus_allocator.c.
#define MVOP_STRIDE_ALIGNMENT (96)
...
typedef struct MHAL_DISP_InputPortInstance_s
{
    ...
    MS_U8 u8PQ_DelayFrameNum;
    MS_U8 u8PQ_DelayFrameCnt;
#if(KERNEL_MODE)
...
typedef struct MHAL_DISP_LayerInstance_s
{
    ...
    MHAL_DISP_PixelFormat_e ePixFormat;         /* Pixel format of the video layer */
...
static MHAL_DISP_PanelConfig_t* _pstPanelConfig;
static MS_U8 u8PanelSize = 0;
static MHAL_DISP_MmapInfo_t stMmapInfo[E_MHAL_DISP_MMAP_MAX];

MS_BOOL _MHAL_DISP_GetPanelInfo(MHAL_DISP_DeviceTiming_e eDeviceTiming, PanelType* pstPanelInfo)
{
    ...
    for(u8Count = 0; u8Count < u8PanelSize; u8Count++)
    {
       if((_pstPanelConfig[u8Count].bValid == TRUE) && (_pstPanelConfig[u8Count].eTiming == eDeviceTiming))
       {
           ...
           memcpy(pstPanelInfo, &_pstPanelConfig[u8Count].stPanelAttr, sizeof(*pstPanelInfo)) ;
           bFound = TRUE;
       }
    }
...
MS_BOOL _MHAL_DISP_PanelInit(void)
{
    MS_BOOL bRet = FALSE;
    ...
    if(!_MHAL_DISP_GetPanelInfo(E_MHAL_DISP_OUTPUT_1080P60, &stPanelInfo))
    {
        bRet = FALSE;
        goto EXIT;
    }
...
MS_BOOL _MHAL_DISP_ScalerInit(void)
{
    MS_BOOL bRet = FALSE;
    //Initialize XC
    ...
    memset(pstXC_InitData, 0 ,sizeof(XC_INITDATA));
...
MS_BOOL _MHAL_DISP_PqInit(void)
{
    MS_BOOL bRet = FALSE;
    u16 u16PnlWidth;
    //pq init
...
MS_BOOL MHAL_DISP_InitPanelConfig(MHAL_DISP_PanelConfig_t* pstPanelConfig, MS_U8 u8Size)
{
    ...
    _pstPanelConfig = kmalloc((sizeof(MHAL_DISP_PanelConfig_t) * u8Size), GFP_KERNEL);
    memset(_pstPanelConfig, 0, (sizeof(MHAL_DISP_PanelConfig_t) * u8Size));
    memcpy(_pstPanelConfig, pstPanelConfig, (sizeof(MHAL_DISP_PanelConfig_t) * u8Size));
    u8PanelSize = u8Size;
...
MS_BOOL _MHAL_DISP_DevInstanceInit(MHAL_DISP_DevInstance_t* pstDevIns, const MS_U32 u32DeviceId)
{
...
MS_BOOL MHAL_DISP_DeviceCreateInstance(const MHAL_DISP_AllocPhyMem_t *pstAlloc, const MS_U32 u32DeviceId, void **pDevCtx)
{
    ...
    {
        _MHAL_DISP_PanelInit();
        _MHAL_DISP_ScalerInit();
        _MHAL_DISP_PqInit();
...
#endif
        _MHAL_DISP_DevInstanceInit(pstDevInstance[u32DeviceId], u32DeviceId);
...
MS_BOOL MHAL_DISP_DeviceDestroyInstance(void *pDevCtx)
{
    ...
    //DBG_ERR("MHAL_DISP_DeviceDestroyInstance !!!\n");
...
struct task_struct    *pPQLHandlerThread;
int pql_handler_threat(void *pdata)
{
    while(!kthread_should_stop())
    {
        Mhal_PQL_Handler(FALSE);
        msleep(50);
    }
    return 0;
}
...
MS_BOOL MHAL_DISP_DeviceEnable(void *pDevCtx, const MS_BOOL bEnable)
{
    ...
    MS_U32 u32DevId = 0;
    MHAL_DISP_DevInstance_t* pstTmpDev = NULL;
    //DBG_ERR("MHAL_DISP_DeviceEnable !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
    ...
    if(bEnable == TRUE)
    {
        pPQLHandlerThread = kthread_create(pql_handler_threat, NULL, "pql/handlerthreat");
        if (IS_ERR(pPQLHandlerThread))
        {
            printk("Fail to create thread pPQLHandlerThread.\n");
        }
        else
        {
            printk("pPQLHandlerThread have create sucessed\r\n");
            wake_up_process(pPQLHandlerThread);
        }
    }
    else
    {
        kthread_stop(pPQLHandlerThread);
    }
...
MS_BOOL MHAL_DISP_DeviceSetBackGroundColor(void *pDevCtx, const MS_U32 u32BgColor)
{
    ...
    u8 u8MuteColor = 0;
    MHAL_DISP_DevInstance_t* pstTmpDev = NULL;
    //DBG_ERR("MHAL_DISP_DeviceSetBackGroundColor !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
...
MS_BOOL _MHAL_DISP_DeviceSetPanelDclk(PanelType* pstPanelInfo, MHAL_DISP_DeviceTiming_e eDeviceTiming)
{
    MS_BOOL bRet = FALSE;
    u16 u16PanelDclock = 0;
    MHAL_DISP_CHECK_NULLPOINTER(pstPanelInfo);
    //DBG_ERR("_MHAL_DISP_DeviceSetPanelDclk !!!\n");
...
MS_BOOL _MHAL_DISP_DeviceChangePanelType(PanelType* pstPanelInfo)
{
    MS_BOOL bRet = FALSE;
    XC_PANEL_INFO stPanelInfo;
    memset(&stPanelInfo, 0, sizeof(stPanelInfo));
    //DBG_ERR("_MHAL_DISP_DeviceChangePanelType !!!\n");
    //Change panel type
...
MS_BOOL _MHAL_DISP_DeviceSetDacTable(MHAL_DISP_DeviceTiming_e eDeviceTiming)
{
    MS_BOOL bRet = TRUE;
    MS_U8 *pDACTable = NULL;
...
MS_BOOL _MHAL_DISP_DeviceSetTxTiming(MHAL_DISP_DeviceTiming_e eDeviceTiming)
{
    MS_BOOL bRet = FALSE;
    MS_U8* pDACTable = NULL;
...
MS_BOOL _MHAL_DISP_DeviceSetPanelInfo(MHAL_DISP_DeviceTiming_e eDeviceTiming)
{
    MS_BOOL bRet = FALSE;
    PanelType stPanelInfo;
    memset(&stPanelInfo, 0, sizeof(PanelType));
    if(!_MHAL_DISP_GetPanelInfo(eDeviceTiming, &stPanelInfo))
    {
    ...
    if(!_MHAL_DISP_DeviceSetPanelDclk(&stPanelInfo, eDeviceTiming))
    {
        bRet = FALSE;
        goto EXIT;
    }
    if(!_MHAL_DISP_DeviceChangePanelType(&stPanelInfo))
    {
        bRet = FALSE;
        goto EXIT;
    }

    if(!_MHAL_DISP_DeviceSetDacTable(eDeviceTiming))
    {
        bRet = FALSE;
        goto EXIT;
    }
...
MS_BOOL MHAL_DISP_DeviceAddOutInterface(void *pDevCtx, const MS_U32 u32Interface)
{
    ...
    MS_U32 u32DevId = 0;
    MHAL_DISP_DevInstance_t* pstTmpDev = NULL;
    //DBG_ERR("MHAL_DISP_DeviceAddOutInterace !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
...
MS_BOOL MHAL_DISP_DeviceSetOutputTiming(void *pDevCtx, const MS_U32 u32Interface, const MHAL_DISP_DeviceTimingInfo_t *pstTimingInfo)
{
    ...
    MS_U32 u32DevId = 0;
    MHAL_DISP_DevInstance_t* pstTmpDev = NULL;
    //DBG_ERR("MHAL_DISP_DeviceSetOutputTiming !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
    ...
    if(u32Interface & 0x1)
    {
        switch(pstTimingInfo->eTimeType)
        {
            ...
            default:
            {
                _MHAL_DISP_DeviceSetPanelInfo(pstTimingInfo->eTimeType);
                break;
            }
...
MS_BOOL MHAL_DISP_GetDevIrqFlag(void *pDevCtx, MHAL_DISP_IRQFlag_t *pstIrqFlag)
{
    ...
    pstIrqFlag->u32IrqMask = 0x01;

    pstIrqFlag->u32IrqFlag = 0x01;
...
{
    MS_BOOL bRet = FALSE;
    //DBG_ERR("MHAL_DISP_VideoLayerCreateInstance Layer ID = %d!!!\n", u32LayerId);
    MHAL_DISP_CHECK_NULLPOINTER(pstAlloc);
...
MS_BOOL _MHAL_DISP_SetVideoLayerMux(MHAL_DISP_LayerInstance_t *pVidLayerCtx, MS_BOOL bEnable)
{
    MS_BOOL bRet = FALSE;
    MS_U32 u32LayerId = 0;
    s16 s16PathId = 0;
    XC_MUX_PATH_INFO PathInfo;
    //DBG_ERR("_MHAL_DISP_SelectVideoLayerMux !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
...
MS_BOOL MHAL_DISP_VideoLayerEnable(void *pVidLayerCtx, const MS_BOOL bEnable)
{
    ...
    if(!_MHAL_DISP_SetVideoLayerMux(pstLayerIns, bEnable))
    {
        //DBG_ERR("_MHAL_DISP_SetVideoLayerMux Fail !!!\n");
        bRet = FALSE;
        goto UP_LAYER_MUTEX;
    }
...
MS_BOOL MHAL_DISP_VideoLayerBind(void *pVidLayerCtx, void *pDevCtx)
{
    ...
    MS_U32 u32DevId = 0;
    MS_U32 u32LayerId = 0;
    //DBG_ERR("MHAL_DISP_VideoLayerBind !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
...
MS_BOOL MHAL_DISP_VideoLayerUnBind(void *pVidLayerCtx, void *pDevCtx)
{
    ...
    MS_U32 u32DevId = 0;
    MS_U32 u32LayerId = 0;
    //DBG_ERR("MHAL_DISP_VideoLayerUnBind !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
    MHAL_DISP_CHECK_NULLPOINTER(pDevCtx);
    pstDevIns = (MHAL_DISP_DevInstance_t*)pDevCtx;
    u32DevId  = pstDevIns->u32DevId;
...
MS_BOOL MHAL_DISP_VideoLayerSetAttr(void *pVidLayerCtx, const MHAL_DISP_VideoLayerAttr_t *pstAttr)
{
    ...
    MHAL_DISP_LayerInstance_t* pstLayerIns = NULL;
    MS_U32 u32LayerId = 0;
    MS_U32 u32BufWidth = 0;//pixel number
    MS_BOOL bHS = TRUE;
    MS_BOOL bXCGen = TRUE;
    ...
    //DBG_ERR("MHAL_DISP_VideoLayerSetAttr !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
    ...
    if(pstLayerIns->bEnable != TRUE)
    {
        ...
        if(E_MHAL_DISP_PIXEL_FRAME_YUV422_YUYV == pstAttr->ePixFormat)
        {
            u32BufWidth = ALIGN_UP(pstAttr->stVidLayerSize.u32Width * 2, MVOP_STRIDE_ALIGNMENT) / 2;//pixels
            
        }
        else//yuv420, tile H264 and H265 data
        {
            u32BufWidth = ALIGN_UP(pstAttr->stVidLayerSize.u32Width, MVOP_STRIDE_ALIGNMENT);//pixels
        }
        stMvopInputCfg.u16StripSize = (MS_U16)u32BufWidth;
#else
        stMvopInputCfg.u16StripSize = stMvopInputCfg.u16HSize;
#endif
        stMvopInputCfg.bSD = FALSE;
        if(pstAttr->ePixFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE1_H264 ||   //from vdec
            pstAttr->ePixFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE2_H265 ||
            pstAttr->ePixFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE3_H265)
        {
        ...
        if(pstAttr->stVidLayerSize.u32Width >= 2560)
        {
            bXCGen = FALSE;
...
MS_BOOL MHAL_DISP_VideoLayerBufferFire(void *pVidLayerCtx)
{
    ...
    MHAL_DISP_LayerInstance_t* pstLayerIns = NULL;
    MS_U32 u32LayerId = 0;
    //DBG_ERR("MHAL_DISP_VideoLayerBufferFire !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
...
MS_BOOL MHAL_DISP_VideoLayerCheckBufferFired(void *pVidLayerCtx)
{
    ...
    //DBG_ERR("MHAL_DISP_VideoLayerCheckBufferFired !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
...
MS_BOOL MHAL_DISP_InputPortCreateInstance(const MHAL_DISP_AllocPhyMem_t *pstAlloc, void *pVidLayerCtx, const MS_U32 u32PortId, void **pCtx)
{
    ...
    MHAL_DISP_LayerInstance_t* pstLayerIns = NULL;
    MS_U32 u32LayerId = 0;
    //DBG_ERR("MHAL_DISP_InputPortCreateInstance !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pstAlloc);
    MHAL_DISP_CHECK_NULLPOINTER(pVidLayerCtx);
...
MS_BOOL MHAL_DISP_InputPortDestroyInstance(const void *pInputPortCtx)
{
    ...
    //DBG_ERR("MHAL_DISP_InputPortDestroyInstance !!!\n");
...
MS_BOOL MHAL_DISP_InputPortFlip(void *pInputPortCtx, MHAL_DISP_VideoFrameData_t *pstVideoFrameBuffer)
{
    ...
    MHAL_DISP_InputPortInstance_t* pstPortIns = NULL;
    MS_U32 u32PortId = 0;
    //DBG_ERR("MHAL_DISP_InputPortFlip !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pInputPortCtx);
    ...
    if(pstPortIns->bEnable)
    {
        s8 s8PreMiuSelect = -1;
        s8 s8MiuSelect = 0;
        MS_U32 lumaAddr = pstVideoFrameBuffer->aPhyAddr[0];
        MS_U32 chromAddr = pstVideoFrameBuffer->aPhyAddr[1];
        ...
        if(pstVideoFrameBuffer->ePixelFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE1_H264 ||  //from vdec
            pstVideoFrameBuffer->ePixelFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE2_H265 ||
            pstVideoFrameBuffer->ePixelFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE3_H265)
        {
            MDrv_MVOP_SetTileFormat(pstVideoFrameBuffer->eTileMode);
            MDrv_MVOP_SetBaseAdd(lumaAddr, chromAddr, TRUE, FALSE);
        }
        else
        ...
        if(pstPortIns->u8PQ_DelayFrameCnt < pstPortIns->u8PQ_DelayFrameNum)
        {
            pstPortIns->u8PQ_DelayFrameCnt ++;
        }
        else if(pstPortIns->u8PQ_DelayFrameCnt == pstPortIns->u8PQ_DelayFrameNum)
        {
            MApi_XC_GenerateBlackVideo(FALSE, MAIN_WINDOW);
            pstPortIns->u8PQ_DelayFrameNum = 0;
        }
    }
    else
    {
...
MS_BOOL MHAL_DISP_InputPortEnable(void *pInputPortCtx, const MS_BOOL bEnable)
{
    ...
    MS_U32 u32PortId = 0;
    MS_U32 u32LayerId = 0;
    //DBG_ERR("MHAL_DISP_InputPortEnable !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pInputPortCtx);
    pstPortIns = (MHAL_DISP_InputPortInstance_t*)pInputPortCtx;
    u32PortId = pstPortIns->u32PortId;
    MHAL_DISP_CHECK_INVAILDINPUTPORT(u32PortId);
...
#if ENABLE_PQ
    {
        MS_U8 u8Delay_FrameNum = 0;
        Mhal_PQL_UpdatePicture(NULL);
        Mhal_PQL_SetQuality();

        //if(!MApi_XC_IsBlackVideoEnable(MAIN_WINDOW))
        {
            MS_U32 u32PQDelayTime = Mhal_PQL_GetPQDelayTime();

            u8Delay_FrameNum = u32PQDelayTime / stMVOPTiming.u8Framerate;
            if(u8Delay_FrameNum == 0)
            {
                u8Delay_FrameNum = 1;
            }
            else if(u32PQDelayTime % stMVOPTiming.u8Framerate)
            {
                //u8Delay_FrameCnt if float >=0.5 u8Delay_FrameCnt +1
                MS_U32 m = u32PQDelayTime % stMVOPTiming.u8Framerate;
                MS_U32 n = stMVOPTiming.u8Framerate /m;
        ...
        pstPortIns->u8PQ_DelayFrameNum = u8Delay_FrameNum;
        pstPortIns->u8PQ_DelayFrameCnt = 0;
    }
#endif
...
MS_BOOL MHAL_DISP_InputCheckDispWin(MHAL_DISP_InputPortInstance_t *pstPortIns, MHAL_DISP_LayerInstance_t *pstLayerIns)
{
    MS_BOOL bRet = FALSE;
...
MS_BOOL MHAL_DISP_InputPortSetAttr(void *pInputPortCtx, const MHAL_DISP_InputPortAttr_t *pstAttr)
{
    ...
    if(MHAL_DISP_InputCheckDispWin(pstPortIns, pstLayerIns) != TRUE)
    {
        //DBG_ERR("MHAL_DISP_InputCheckDispWin failed!!!\n");
        bRet = FALSE;
        goto UP_PORT_MUTEX;
    }
...
MS_BOOL MHAL_DISP_InputPortShow(void *pInputPortCtx)
{
    ...
    MHAL_DISP_InputPortInstance_t* pstPortIns = NULL;
    MS_U32 u32PortId = 0;
    //DBG_ERR("MHAL_DISP_InputPortShow !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pInputPortCtx);
    pstPortIns = (MHAL_DISP_InputPortInstance_t*)pInputPortCtx;
    u32PortId = pstPortIns->u32PortId;
    MHAL_DISP_CHECK_INVAILDINPUTPORT(u32PortId);
...
MS_BOOL MHAL_DISP_InputPortHide(void *pInputPortCtx)
{
    ...
    MHAL_DISP_InputPortInstance_t* pstPortIns = NULL;
    MS_U32 u32PortId = 0;
    //DBG_ERR("MHAL_DISP_InputPortHide !!!\n");
    MHAL_DISP_CHECK_NULLPOINTER(pInputPortCtx);
    pstPortIns = (MHAL_DISP_InputPortInstance_t*)pInputPortCtx;
    u32PortId = pstPortIns->u32PortId;
    MHAL_DISP_CHECK_INVAILDINPUTPORT(u32PortId);
...
MS_BOOL MHAL_DISP_InputPortAttrEnd(void *pVidLayerCtx)
{
    ...
    if(pstLayerIns->bEnable)
    {
        if(pstLayerIns->bEnableBatch == TRUE)
        {
            ...
            pstLayerIns->bEnableBatch = FALSE;
            list_for_each(pstPos, &(pstLayerInstance[u32LayerId]->stInputPortList))
            {
                pstTmpPort = list_entry(pstPos, MHAL_DISP_InputPortInstance_t, stInputPortNode);
                if(!MHAL_DISP_InputPortEnable(pstTmpPort, TRUE))
                {
                    //DBG_ERR("MHAL_DISP_InputPortEnable Fail!!!\n");
                    bRet = FALSE;
                    continue;
                }
                bRet = TRUE;
...
MS_BOOL MHAL_DISP_SetDbgLevel(void)
{
    //DBG_ERR("MHAL_DISP_SetDbgLevel !!!\n");
    MApi_XC_SetDbgLevel(0x83);
    return TRUE;
}

MS_BOOL MHAL_DISP_SetPanelParams(void)
{
    //DBG_ERR("MHAL_DISP_SetDbgLevel !!!\n");
    MApi_XC_SetDbgLevel(0x83);
    return TRUE;
...
{
    mhal_pql_ColorTemperature_t stPQLColorTemperature;
    memset(&stPQLColorTemperature, 0, sizeof(mhal_pql_ColorTemperature_t));

    stPQLColorTemperature.u32Contrast = (pstHdmiInfo->stCsc.u32Contrast*255/100);
    stPQLColorTemperature.u32Hue      = pstHdmiInfo->stCsc.u32Hue;
    stPQLColorTemperature.u32Brightness  = (pstHdmiInfo->stCsc.u32Luma*255/100);
    stPQLColorTemperature.u32Saturation  = (pstHdmiInfo->stCsc.u32Saturation*255/100);

    Mhal_PQL_UpdatePicture(&stPQLColorTemperature);
    return TRUE;
}
