...
bool DipCaptureTiming (MHAL_DIVP_CaptureInfo_t *pstCapOnceInfo, MHAL_CMDQ_CmdqInterface_t* pstCmdInf, DivpChannelInfo *pstDivpChnInfo)
{
    ...
    //H scaling rule
    if((stXC_SetWin_Info.stCapWin.width > 1920) //scaling down and scaling up, 2 step scaling //line buffer limitation
        && (pstTempCapInfo->u16Width > stXC_SetWin_Info.stCapWin.width))
    {
        stXC_SetWin_Info.bPreHCusScaling = TRUE;
        stXC_SetWin_Info.u16PreHCusScalingSrc = stXC_SetWin_Info.stCapWin.width;
        stXC_SetWin_Info.u16PreHCusScalingDst = 1920;
        stXC_SetWin_Info.bHCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
        stXC_SetWin_Info.u16HCusScalingSrc = 1920;          ///<post H customized scaling src width
        stXC_SetWin_Info.u16HCusScalingDst = pstTempCapInfo->u16Width;          ///<post H customized scaling dst width
        ...
        stXC_SetWin_Info.bVCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
        stXC_SetWin_Info.u16VCusScalingSrc = stXC_SetWin_Info.stCapWin.height;          ///<post H customized scaling src width
        stXC_SetWin_Info.u16VCusScalingDst = pstTempCapInfo->u16Height;          ///<post H customized scaling dst width

        bIsNeedHVSP = TRUE;
    }
    else if((pstTempCapInfo->u16Width > stXC_SetWin_Info.stCapWin.width)
        || (pstTempCapInfo->u16Height > stXC_SetWin_Info.stCapWin.height))//scaling up
    {
        stXC_SetWin_Info.bPreHCusScaling = TRUE;
        stXC_SetWin_Info.u16PreHCusScalingSrc = stXC_SetWin_Info.stCapWin.width;
        stXC_SetWin_Info.u16PreHCusScalingDst = stXC_SetWin_Info.stCapWin.width;
        stXC_SetWin_Info.bHCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
        stXC_SetWin_Info.u16HCusScalingSrc = stXC_SetWin_Info.stCapWin.width;          ///<post H customized scaling src width
        stXC_SetWin_Info.u16HCusScalingDst = pstTempCapInfo->u16Width;          ///<post H customized scaling dst width
    ...
    else //scaling down
    {
        stXC_SetWin_Info.bPreHCusScaling = TRUE;
        stXC_SetWin_Info.u16PreHCusScalingSrc = stXC_SetWin_Info.stCapWin.width;
        stXC_SetWin_Info.u16PreHCusScalingDst = pstTempCapInfo->u16Width;
        stXC_SetWin_Info.bHCusScaling = FALSE;               ///<assign post H customized scaling instead of using XC scaling
        stXC_SetWin_Info.u16HCusScalingSrc = pstTempCapInfo->u16Width;          ///<post H customized scaling src width
        stXC_SetWin_Info.u16HCusScalingDst = pstTempCapInfo->u16Width;          ///<post H customized scaling dst width

        stXC_SetWin_Info.bPreVCusScaling = TRUE;
        stXC_SetWin_Info.u16PreVCusScalingSrc = stXC_SetWin_Info.stCapWin.height;
        stXC_SetWin_Info.u16PreVCusScalingDst = pstTempCapInfo->u16Height;
        stXC_SetWin_Info.bVCusScaling = FALSE;               ///<assign post H customized scaling instead of using XC scaling
        stXC_SetWin_Info.u16VCusScalingSrc = pstTempCapInfo->u16Height;          ///<post H customized scaling src width
        stXC_SetWin_Info.u16VCusScalingDst = pstTempCapInfo->u16Height;          ///<post H customized scaling dst width
    }
    ...
    CHECK_APIXC_RET(MApi_XC_DIP_SetHVSP(bIsNeedHVSP, enDipWin));
...
bool DipUpdateFrameBufferIndex(DivpChannelInfo *pstDivpCtx, MHAL_DIVP_InputInfo_t* pstDivpInputInfo, MHAL_DIVP_OutPutInfo_t* pstDivpOutputInfo, MS_U8 u8ChannelId, bool bLastFrame, MHAL_DIVP_DeviceId_e eDevId)
{
    ...
    {
        LOG_INFO("enDipWin = %u, g_stDipRes[enDipWin].stChipCaps.u32DipChipCaps = 0x%x .\n", enDipWin, g_stDipRes[enDipWin].stChipCaps.u32DipChipCaps);
    ...
    g_stDipRes[enDipWin].bNeedScalingUp = bIsNeedHVSP;
    //set params
...

    {
        ...
        //H scaling rule
        if((pstDivpInputInfo->stCropWin.u16Width > 1920) //scaling down and scaling up, 2 step scaling //line buffer limitation
            && (pstDivpOutputInfo->u16OutputWidth > pstDivpInputInfo->stCropWin.u16Width))
        {
            pstDipWindowInfo->bPreHCusScaling = TRUE;
            pstDipWindowInfo->u16PreHCusScalingSrc = pstDipWindowInfo->stCapWin.width;
            pstDipWindowInfo->u16PreHCusScalingDst = 1920;
            pstDipWindowInfo->bHCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
            pstDipWindowInfo->u16HCusScalingSrc = 1920;          ///<post H customized scaling src width
            pstDipWindowInfo->u16HCusScalingDst = pstDivpOutputInfo->u16OutputWidth;          ///<post H customized scaling dst width

            pstDipWindowInfo->bPreVCusScaling = TRUE;
            pstDipWindowInfo->u16PreVCusScalingSrc = pstDipWindowInfo->stCapWin.height;
            pstDipWindowInfo->u16PreVCusScalingDst = pstDipWindowInfo->stCapWin.height;
            pstDipWindowInfo->bVCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
            pstDipWindowInfo->u16VCusScalingSrc = pstDipWindowInfo->stCapWin.height;          ///<post H customized scaling src width
            pstDipWindowInfo->u16VCusScalingDst = pstDivpOutputInfo->u16OutputHeight;          ///<post H customized scaling dst width

            bIsNeedHVSP = TRUE;
        }
        else if((pstDivpOutputInfo->u16OutputWidth > pstDivpInputInfo->stCropWin.u16Width)
            || (pstDivpOutputInfo->u16OutputHeight > pstDivpInputInfo->stCropWin.u16Height))//scaling up
        {
            pstDipWindowInfo->bPreHCusScaling = TRUE;
            pstDipWindowInfo->u16PreHCusScalingSrc = pstDipWindowInfo->stCapWin.width;
            pstDipWindowInfo->u16PreHCusScalingDst = pstDipWindowInfo->stCapWin.width;
            pstDipWindowInfo->bHCusScaling = TRUE;               ///<assign post H customized scaling instead of using XC scaling
            pstDipWindowInfo->u16HCusScalingSrc = pstDipWindowInfo->stCapWin.width;          ///<post H customized scaling src width
            pstDipWindowInfo->u16HCusScalingDst = pstDivpOutputInfo->u16OutputWidth;          ///<post H customized scaling dst width
        ...
        else //scaling down
        {
            pstDipWindowInfo->bPreHCusScaling = TRUE;
            pstDipWindowInfo->u16PreHCusScalingSrc = pstDipWindowInfo->stCapWin.width;
            pstDipWindowInfo->u16PreHCusScalingDst = pstDivpOutputInfo->u16OutputWidth;
            pstDipWindowInfo->bHCusScaling = FALSE;               ///<assign post H customized scaling instead of using XC scaling
            pstDipWindowInfo->u16HCusScalingSrc = pstDivpOutputInfo->u16OutputWidth;          ///<post H customized scaling src width
            pstDipWindowInfo->u16HCusScalingDst = pstDivpOutputInfo->u16OutputWidth;          ///<post H customized scaling dst width

            pstDipWindowInfo->bPreVCusScaling = TRUE;
            pstDipWindowInfo->u16PreVCusScalingSrc = pstDipWindowInfo->stCapWin.height;
            pstDipWindowInfo->u16PreVCusScalingDst = pstDivpOutputInfo->u16OutputHeight;
            pstDipWindowInfo->bVCusScaling = FALSE;               ///<assign post H customized scaling instead of using XC scaling
            pstDipWindowInfo->u16VCusScalingSrc = pstDivpOutputInfo->u16OutputHeight;          ///<post H customized scaling src width
            pstDipWindowInfo->u16VCusScalingDst = pstDivpOutputInfo->u16OutputHeight;          ///<post H customized scaling dst width
        }
    }
...
MS_S32 MHAL_DIVP_GetChipCapbility(MHAL_DIVP_DeviceId_e eDevId, MS_U32* pu32Caps)
{
    ...
    *pu32Caps = 0;
    ...
    MApi_XC_Init(&stXC_InitData, 0);
    if(E_MHAL_DIVP_Device0 == eDevId)//patch for K6: bringup DIP0 only
    {
    ...
    {
        LOG_ERR("#### patch for K6 bringup DIVP0 only. eRet = %u, *pu32Caps = %u .\n", eRet, *pu32Caps);
    }
