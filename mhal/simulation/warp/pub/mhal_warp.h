...
typedef struct
{
    ...
    union
    {
        const MHAL_WARP_DISPPLAY_ABSOLUTE_ENTRY_T* absolute_table;
        const MHAL_WARP_DISPPLAY_RELATIVE_ENTRY_T* relative_table;
        MS_U32* overlay;
    };
...
typedef struct
{
    MS_S32 size;                                        // number of bound boxes
    const MHAL_WARP_BOUND_BOX_ENTRY_T *table;           // list of all bound boxes
...
typedef struct
{
    ...
    MS_U8* data[2];                                     // pointers to the image planes' data
...
typedef struct
{
    ...
    MS_U32 afbc_en;                                     // Enable AFBC mode
    MS_U8 fill_value[2];                                // Fill value for out of range pixels

    MS_U32 axi_max_read_burst_size;                     // Maximum read burst size of the AXI master port
    MS_U32 axi_max_write_burst_size;                    // Maximum write burst size of the AXI master port
    MS_U32 axi_max_read_outstanding;                    // Maximum outstanding read accesses of the AXI master port
    MS_U32 axi_max_write_outstanding;                   // Maximum outstanding write accesses of the AXI master port

    // Debug features
    MS_U32 debug_bypass_displacement_en;
    MS_U32 debug_bypass_interp_en;

    MS_U32 output_tiles_width;                          // calculated internally based on the bounding boxes
    MS_U32 output_tiles_height;                         // calculated internally based on the bounding boxes
    MS_U32 num_output_tiles;                            // calculated internally based on the bounding boxes
} MHAL_WARP_CONFIG;
...
//typedef MS_U32 (*MHAL_WARP_CB_T)(MHAL_WARP_INST_HANDLE instance, void *user_data);
